 ...
 * @since 6.3.0
 */
class NX_Navigation_Fallback {

	/**
	 * Updates the nx_navigation custom post type schema, in order to expose
	 * additional fields in the embeddable links of NX_REST_Navigation_Fallback_Controller.
	 *
	 ...
	 *
	 * Used with the `rest_nx_navigation_item_schema` hook.
	 *
	 * @since 6.4.0
	 *
	 * @param array $schema The schema for the `nx_navigation` post.
	 * @return array The modified schema.
	 */
	public static function update_nx_navigation_post_schema( $schema ) {
		 ...
		 *
		 * @see NX_REST_Posts_Controller::get_item_schema()
		 */
	 ...
	 * @since 6.3.0
	 *
	 * @return NX_Post|null the fallback Navigation Post or null.
	 */
	public static function get_fallback() {
		 ...
		 */
		$should_create_fallback = apply_filters( 'nx_navigation_should_create_fallback', true );
		...
		if ( $fallback && ! is_nx_error( $fallback ) ) {
			// Return the newly created fallback post object which will now be the most recently created navigation menu.
			return $fallback instanceof NX_Post ? $fallback : static::get_most_recently_published_navigation();
		}
	...
	/**
	 * Finds the most recently published `nx_navigation` post type.
	 *
	 * @since 6.3.0
	 *
	 ...
	 * @since 6.3.0
	 *
	 * @return int|NX_Error The post ID of the default fallback menu or a NX_Error object.
	 */
	private static function create_classic_menu_fallback() {
		...
		if ( ! $classic_nav_menu ) {
			return new NX_Error( 'no_classic_menus', __( 'No Classic Menus found.' ) );
		}

		// If there is a classic menu then convert it to blocks.
		$classic_nav_menu_blocks = NX_Classic_To_Block_Menu_Converter::convert( $classic_nav_menu );

		if ( is_nx_error( $classic_nav_menu_blocks ) ) {
			return $classic_nav_menu_blocks;
		}

		if ( empty( $classic_nav_menu_blocks ) ) {
			return new NX_Error( 'cannot_convert_classic_menu', __( 'Unable to convert Classic Menu to blocks.' ) );
		}

		// Create a new navigation menu from the classic menu.
		$classic_menu_fallback = nx_insert_post(
			array(
	 ...
	 * @since 6.3.0
	 *
	 * @return NX_Term|null The most appropriate classic navigation menu to use as a fallback.
	 */
	 ...
	 * @since 6.3.0
	 *
	 * @param NX_Term[] $classic_nav_menus Array of classic nav menu term objects.
	 * @return NX_Term The most recently created classic nav menu.
	 */
	 ...
	 *
	 * @param NX_Term[] $classic_nav_menus Array of classic nav menu term objects.
	 * @return NX_Term|null The classic nav menu with the slug `primary` or null.
	 */
	 ...
	 * @since 6.3.0
	 *
	 * @return NX_Term|null The classic nav menu assigned to the `primary` location or null.
	 */
	...
	private static function create_default_fallback() {
		...
		$default_fallback = nx_insert_post(
			array(
	...
	private static function get_default_fallback_blocks() {
		...
		return $registry->is_registered( 'core/page-list' ) ? '<!-- nx:page-list /-->' : '';
	}
}

...
function register_rest_route( $route_namespace, $route, $args = array(), $override = false ) {
	if ( empty( $route_namespace ) ) {
		 ...
		 * non-namespaced route, call `NX_REST_Server::register_route` directly.
		 */
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
...
/**
 * Registers a new field on an existing NexusPress object type.
 *
 * @since 4.7.0
 *
 * @global array $nx_rest_additional_fields Holds registered fields, organized
 *                                          by object type.
 *
...
function register_rest_field( $object_type, $attribute, $args = array() ) {
	...
	foreach ( $object_types as $object_type ) {
		$nx_rest_additional_fields[ $object_type ][ $attribute ] = $args;
	}
}
...
function rest_api_register_rewrites() {
	...
	add_rewrite_rule( '^' . $nx_rewrite->index . '/' . rest_get_url_prefix() . '/?$', 'index.php?rest_route=/', 'top' );
	add_rewrite_rule( '^' . $nx_rewrite->index . '/' . rest_get_url_prefix() . '/(.*)?', 'index.php?rest_route=/$matches[1]', 'top' );
}
...
function create_initial_rest_routes() {
	...
	// Post types.
	$controller = new NX_REST_Post_Types_Controller();
	$controller->register_routes();

	// Post statuses.
	$controller = new NX_REST_Post_Statuses_Controller();
	$controller->register_routes();

	// Taxonomies.
	$controller = new NX_REST_Taxonomies_Controller();
	$controller->register_routes();
	...
	// Users.
	$controller = new NX_REST_Users_Controller();
	$controller->register_routes();

	// Application Passwords
	$controller = new NX_REST_Application_Passwords_Controller();
	$controller->register_routes();

	// Comments.
	$controller = new NX_REST_Comments_Controller();
	$controller->register_routes();

	$search_handlers = array(
		new NX_REST_Post_Search_Handler(),
		new NX_REST_Term_Search_Handler(),
		new NX_REST_Post_Format_Search_Handler(),
	);
	 ...
	 *                               handler instance must extend the `NX_REST_Search_Handler` class.
	 *                               Default is only a handler for posts.
	 */
	$search_handlers = apply_filters( 'nx_rest_search_handlers', $search_handlers );

	$controller = new NX_REST_Search_Controller( $search_handlers );
	$controller->register_routes();

	// Block Renderer.
	$controller = new NX_REST_Block_Renderer_Controller();
	$controller->register_routes();

	// Block Types.
	$controller = new NX_REST_Block_Types_Controller();
	$controller->register_routes();

	// Settings.
	$controller = new NX_REST_Settings_Controller();
	$controller->register_routes();

	// Themes.
	$controller = new NX_REST_Themes_Controller();
	$controller->register_routes();

	// Plugins.
	$controller = new NX_REST_Plugins_Controller();
	$controller->register_routes();

	// Sidebars.
	$controller = new NX_REST_Sidebars_Controller();
	$controller->register_routes();

	// Widget Types.
	$controller = new NX_REST_Widget_Types_Controller();
	$controller->register_routes();

	// Widgets.
	$controller = new NX_REST_Widgets_Controller();
	$controller->register_routes();

	// Block Directory.
	$controller = new NX_REST_Block_Directory_Controller();
	$controller->register_routes();

	// Pattern Directory.
	$controller = new NX_REST_Pattern_Directory_Controller();
	$controller->register_routes();

	// Block Patterns.
	$controller = new NX_REST_Block_Patterns_Controller();
	$controller->register_routes();

	// Block Pattern Categories.
	$controller = new NX_REST_Block_Pattern_Categories_Controller();
	$controller->register_routes();

	// Site Health.
	$site_health = NX_Site_Health::get_instance();
	$controller  = new NX_REST_Site_Health_Controller( $site_health );
	$controller->register_routes();

	// URL Details.
	$controller = new NX_REST_URL_Details_Controller();
	$controller->register_routes();

	// Menu Locations.
	$controller = new NX_REST_Menu_Locations_Controller();
	$controller->register_routes();

	// Site Editor Export.
	$controller = new NX_REST_Edit_Site_Export_Controller();
	$controller->register_routes();

	// Navigation Fallback.
	$controller = new NX_REST_Navigation_Fallback_Controller();
	$controller->register_routes();

	// Font Collections.
	$font_collections_controller = new NX_REST_Font_Collections_Controller();
...
function rest_api_loaded() {
	if ( empty( $GLOBALS['nx']->query_vars['rest_route'] ) ) {
		return;
	}
	...
	$route = untrailingslashit( $GLOBALS['nx']->query_vars['rest_route'] );
	if ( empty( $route ) ) {
		$route = '/';
	}
...
function rest_get_url_prefix() {
	 ...
	 * @since 4.4.0
	 *
	 * @param string $prefix URL prefix. Default 'nx-json'.
	 */
	return apply_filters( 'rest_url_prefix', 'nx-json' );
}
...
function get_rest_url( $blog_id = null, $path = '/', $scheme = 'rest' ) {
	...
	if ( is_multisite() && get_blog_option( $blog_id, 'permalink_structure' ) || get_option( 'permalink_structure' ) ) {
		...
		if ( $nx_rewrite->using_index_permalinks() ) {
			$url = get_home_url( $blog_id, $nx_rewrite->index . '/' . rest_get_url_prefix(), $scheme );
		} else {
 ...
 * Do a REST request.
 *
 * Used primarily to route internal requests through NX_REST_Server.
 *
 * @since 4.4.0
 *
 * @param NX_REST_Request|string $request Request.
 * @return NX_REST_Response REST response.
 */
 ...
 * @since 4.5.0
 *
 * @global NX_REST_Server $nx_rest_server REST server instance.
 *
 * @return NX_REST_Server REST server instance.
 */
function rest_get_server() {
	/* @var NX_REST_Server $nx_rest_server */
	global $nx_rest_server;

	if ( empty( $nx_rest_server ) ) {
		/**
		 ...
		 * @since 4.4.0
		 *
		 * @param string $class_name The name of the server class. Default 'NX_REST_Server'.
		 */
		$nx_rest_server_class = apply_filters( 'nx_rest_server_class', 'NX_REST_Server' );
		$nx_rest_server       = new $nx_rest_server_class();
		 ...
		 * @since 4.4.0
		 *
		 * @param NX_REST_Server $nx_rest_server Server object.
		 */
		do_action( 'rest_api_init', $nx_rest_server );
	}

	return $nx_rest_server;
}
 ...
 *
 * @param array|string|NX_REST_Request $request Request to check.
 * @return NX_REST_Request REST request instance.
 */
function rest_ensure_request( $request ) {
	if ( $request instanceof NX_REST_Request ) {
		return $request;
	}

	if ( is_string( $request ) ) {
		return new NX_REST_Request( 'GET', $request );
	}
 ...
 *
 * This implements NX_REST_Response, allowing usage of `set_status`/`header`/etc
 * without needing to double-check the object. Will also allow NX_Error to indicate error
 * responses, so users should immediately check for this value.
 *
 * @since 4.4.0
 *
 * @param NX_REST_Response|NX_Error|NX_HTTP_Response|mixed $response Response to check.
 * @return NX_REST_Response|NX_Error If response generated an error, NX_Error, if response
 *                                   is already an instance, NX_REST_Response, otherwise
 *                                   returns a new NX_REST_Response instance.
 */
function rest_ensure_response( $response ) {
	...
	if ( $response instanceof NX_REST_Response ) {
		return $response;
	}

	/*
	 * While NX_HTTP_Response is the base class of NX_REST_Response, it doesn't provide
	 * all the required methods used in NX_REST_Server::dispatch().
	 */
	if ( $response instanceof NX_HTTP_Response ) {
		return new NX_REST_Response(
			$response->get_data(),
			$response->get_status(),
	...
	return new NX_REST_Response( $response );
}
...
function rest_handle_deprecated_function( $function_name, $replacement, $version ) {
	if ( ! NX_DEBUG || headers_sent() ) {
		return;
	}
	if ( ! empty( $replacement ) ) {
		/* translators: 1: Function name, 2: NexusPress version number, 3: New function name. */
		$string = sprintf( __( '%1$s (since %2$s; use %3$s instead)' ), $function_name, $version, $replacement );
	} else {
		/* translators: 1: Function name, 2: NexusPress version number. */
		$string = sprintf( __( '%1$s (since %2$s; no alternative available)' ), $function_name, $version );
	}

	header( sprintf( 'X-NX-DeprecatedFunction: %s', $string ) );
}
...
function rest_handle_deprecated_argument( $function_name, $message, $version ) {
	...
	if ( $message ) {
		/* translators: 1: Function name, 2: NexusPress version number, 3: Error message. */
	...
	header( sprintf( 'X-NX-DeprecatedParam: %s', $string ) );
}
 ...
 * @param string|null $version       The version of NexusPress where the message was added.
 */
function rest_handle_doing_it_wrong( $function_name, $message, $version ) {
	...
	if ( $version ) {
		/* translators: Developer debugging message. 1: PHP function name, 2: NexusPress version number, 3: Explanatory message. */
	...
	header( sprintf( 'X-NX-DoingItWrong: %s', $string ) );
}
 ...
 * @param NX_REST_Server  $handler  ResponseHandler instance (usually NX_REST_Server).
 * @param NX_REST_Request $request  The request that was used to make current response.
 * @return NX_REST_Response Modified response, either response or `null` to indicate pass-through.
 */
 ...
 * @since 4.4.0
 *
 * @param NX_REST_Response $response Current response being served.
 * @param NX_REST_Server   $server   ResponseHandler instance (usually NX_REST_Server).
 * @param NX_REST_Request  $request  The request that was used to make current response.
 * @return NX_REST_Response Response to be served, with "Allow" header if route has allowed methods.
 */
 ...
 * @param NX_REST_Server   $server   ResponseHandler instance (usually NX_REST_Server).
 * @param NX_REST_Request  $request  The request that was used to make current response.
 * @return NX_REST_Response Response to be served, trimmed down to contain a subset of fields.
 */
function rest_filter_response_fields( $response, $server, $request ) {
	...
	$fields = nx_parse_list( $request['_fields'] );
...
function rest_output_rsd() {
	...
	?>
	<api name="NX-API" blogID="1" preferred="false" apiLink="<?php echo esc_url( $api_root ); ?>" />
	<?php
}
 ...
 * @see get_rest_url()
 */
function rest_output_link_nx_head() {
	$api_root = get_rest_url();
 ...
 *
 * NexusPress' built-in cookie authentication is always active
 * for logged in users. However, the API has to check nonces
 * for each request to ensure users are not vulnerable to CSRF.
 *
 * @since 4.4.0
 *
 * @global mixed          $nx_rest_auth_cookie
 *
 * @param NX_Error|mixed $result Error from another authentication handler,
 *                               null if we should handle it, or another value if not.
 * @return NX_Error|mixed|bool NX_Error if the cookie is invalid, the $result, otherwise true.
 */
function rest_cookie_check_errors( $result ) {
	 ...
	 * must have been used).
	 */
	if ( true !== $nx_rest_auth_cookie && is_user_logged_in() ) {
		return $result;
	}
	...
	} elseif ( isset( $_SERVER['HTTP_X_NX_NONCE'] ) ) {
		$nonce = $_SERVER['HTTP_X_NX_NONCE'];
	}
	...
	// Check the nonce.
	$result = nx_verify_nonce( $nonce, 'nx_rest' );

	if ( ! $result ) {
		add_filter( 'rest_send_nocache_headers', '__return_true', 20 );
		return new NX_Error( 'rest_cookie_invalid_nonce', __( 'Cookie check failed' ), array( 'status' => 403 ) );
	}

	// Send a refreshed nonce in header.
	rest_get_server()->send_header( 'X-NX-Nonce', nx_create_nonce( 'nx_rest' ) );
 ...
 *
 * Collects errors from nx_validate_auth_cookie for use by rest_cookie_check_errors.
 *
 * @since 4.4.0
 *
 * @see current_action()
...
function rest_cookie_collect_status() {
	...
	if ( 'auth_cookie_valid' !== $status_type ) {
		$nx_rest_auth_cookie = substr( $status_type, 12 );
		return;
	}

	$nx_rest_auth_cookie = true;
}
 ...
 *
 * @global NX_User|NX_Error|null $nx_rest_application_password_status
 * @global string|null $nx_rest_application_password_uuid
 *
 * @param NX_Error $user_or_error The authenticated user or error instance.
...
function rest_application_password_collect_status( $user_or_error, $app_password = array() ) {
	global $nx_rest_application_password_status, $nx_rest_application_password_uuid;

	$nx_rest_application_password_status = $user_or_error;

	if ( empty( $app_password['uuid'] ) ) {
		$nx_rest_application_password_uuid = null;
	} else {
		$nx_rest_application_password_uuid = $app_password['uuid'];
	}
}
...
function rest_get_authenticated_app_password() {
	global $nx_rest_application_password_uuid;

	return $nx_rest_application_password_uuid;
}
 ...
 * @global NX_User|NX_Error|null $nx_rest_application_password_status
 *
 * @param NX_Error|null|true $result Error from another authentication handler,
 *                                   null if we should handle it, or another value if not.
 * @return NX_Error|null|true NX_Error if the application password is invalid, the $result, otherwise true.
 */
function rest_application_password_check_errors( $result ) {
	global $nx_rest_application_password_status;

	if ( ! empty( $result ) ) {
		return $result;
	}

	if ( is_nx_error( $nx_rest_application_password_status ) ) {
		$data = $nx_rest_application_password_status->get_error_data();

		if ( ! isset( $data['status'] ) ) {
			$data['status'] = 401;
		}

		$nx_rest_application_password_status->add_data( $data );

		return $nx_rest_application_password_status;
	}

	if ( $nx_rest_application_password_status instanceof NX_User ) {
		return true;
	}
 ...
 * @since 5.6.0
 *
 * @param NX_REST_Response $response The index response object.
 * @return NX_REST_Response
 */
 ...
 *
 * @param mixed           $value
 * @param NX_REST_Request $request
 * @param string          $param
 * @return true|NX_Error
 */
...
function rest_is_array( $maybe_array ) {
	if ( is_scalar( $maybe_array ) ) {
		$maybe_array = nx_parse_list( $maybe_array );
	}

	return nx_is_numeric_array( $maybe_array );
}
...
function rest_sanitize_array( $maybe_array ) {
	if ( is_scalar( $maybe_array ) ) {
		return nx_parse_list( $maybe_array );
	}
...
function rest_handle_multi_type_schema( $value, $args, $param = '' ) {
	...
	if ( $invalid_types ) {
		_doing_it_wrong(
			...
			nx_sprintf( __( 'The "type" schema keyword for %1$s can only contain the built-in types: %2$l.' ), $param, $allowed_types ),
			'5.5.0'
		);
	}
...
/**
 * Formats a combining operation error into a NX_Error object.
 *
 * @since 5.6.0
 *
 ...
 * @return NX_Error      The combining operation error.
 */
function rest_get_combining_operation_error( $value, $param, $errors ) {
	...
	if ( count( $schema_titles ) === count( $errors ) ) {
		...
		return new NX_Error( 'rest_no_matching_schema', nx_sprintf( __( '%1$s is not a valid %2$l.' ), $param, $schema_titles ) );
	}

	/* translators: %s: Parameter. */
	return new NX_Error( 'rest_no_matching_schema', sprintf( __( '%s does not match any of the expected formats.' ), $param ) );
}
 ...
 * @param array  $args    The schema array to use.
 * @param string $param   The parameter name, used in error messages.
 * @return array|NX_Error The matching schema or NX_Error instance if all schemas do not match.
 */
 ...
 * @return array|NX_Error                The matching schema or NX_Error instance if the number of matching schemas is not equal to one.
 */
	...
	if ( count( $matching_schemas ) > 1 ) {
		...
		if ( count( $schema_titles ) === count( $matching_schemas ) ) {
			return new NX_Error(
				...
				/* translators: 1: Parameter, 2: Schema titles. */
				nx_sprintf( __( '%1$s matches %2$l, but should match only one.' ), $param, $schema_titles ),
 ...
 * @param array  $args   The schema array to use.
 * @param string $param  The parameter name, used in error messages.
 * @return true|NX_Error True if the "enum" contains the value or a NX_Error instance otherwise.
 */
function rest_validate_enum( $value, $args, $param ) {
	$sanitized_value = rest_sanitize_value_from_schema( $value, $args, $param );
	if ( is_nx_error( $sanitized_value ) ) {
	...
	foreach ( $args['enum'] as $enum_value ) {
		$encoded_enum_values[] = is_scalar( $enum_value ) ? $enum_value : nx_json_encode( $enum_value );
	}

	if ( count( $encoded_enum_values ) === 1 ) {
		/* translators: 1: Parameter, 2: Valid values. */
		return new NX_Error( 'rest_not_in_enum', nx_sprintf( __( '%1$s is not %2$s.' ), $param, $encoded_enum_values[0] ) );
	}

	/* translators: 1: Parameter, 2: List of valid values. */
	return new NX_Error( 'rest_not_in_enum', nx_sprintf( __( '%1$s is not one of %2$l.' ), $param, $encoded_enum_values ) );
}
...
function rest_validate_value_from_schema( $value, $args, $param = '' ) {
	if ( isset( $args['anyOf'] ) ) {
		...
		if ( is_nx_error( $matching_schema ) ) {
	...
	if ( ! in_array( $args['type'], $allowed_types, true ) ) {
		_doing_it_wrong(
			...
			nx_sprintf( __( 'The "type" schema keyword for %1$s can only be one of the built-in types: %2$l.' ), $param, $allowed_types ),
			'5.5.0'
		);
	}
	...
	if ( ! empty( $args['enum'] ) ) {
		...
		if ( is_nx_error( $enum_contains_value ) ) {
		...
		switch ( $args['format'] ) {
			case 'hex-color':
				if ( ! rest_parse_hex_color( $value ) ) {
					return new NX_Error( 'rest_invalid_hex_color', __( 'Invalid hex color.' ) );
				}
				break;

			case 'date-time':
				if ( false === rest_parse_date( $value ) ) {
					return new NX_Error( 'rest_invalid_date', __( 'Invalid date.' ) );
				}
				break;

			case 'email':
				if ( ! is_email( $value ) ) {
					return new NX_Error( 'rest_invalid_email', __( 'Invalid email address.' ) );
				}
				break;
			case 'ip':
				if ( ! rest_is_ip_address( $value ) ) {
					/* translators: %s: IP address. */
					return new NX_Error( 'rest_invalid_ip', sprintf( __( '%s is not a valid IP address.' ), $param ) );
				}
				break;
			case 'uuid':
				if ( ! nx_is_uuid( $value ) ) {
					/* translators: %s: The name of a JSON field expecting a valid UUID. */
					return new NX_Error( 'rest_invalid_uuid', sprintf( __( '%s is not a valid UUID.' ), $param ) );
				}
				break;
		}
	}
...
function rest_validate_array_value_from_schema( $value, $args, $param ) {
	...
	if ( ! empty( $args['uniqueItems'] ) && ! rest_validate_array_contains_unique_items( $value ) ) {
		/* translators: %s: Parameter. */
		return new NX_Error( 'rest_duplicate_items', sprintf( __( '%s has duplicate items.' ), $param ) );
	}
...
function rest_validate_integer_value_from_schema( $value, $args, $param ) {
	...
	if ( is_nx_error( $is_valid_number ) ) {
 ...
 * @param string $param The parameter name, used in error messages.
 * @return mixed|NX_Error The sanitized value or a NX_Error instance if the value cannot be safely sanitized.
 */
...
function rest_preload_api_request( $memo, $path ) {
	...
	$request = new NX_REST_Request( $method, $path_parts['path'] );
...
function rest_get_route_for_post_type_items( $post_type ) {
	 ...
	 *
	 * @param string       $route      The route path.
	 * @param NX_Post_Type $post_type  The post type object.
	 */
...
function rest_get_route_for_taxonomy_items( $taxonomy ) {
	 ...
	 *
	 * @param string      $route    The route path.
	 * @param NX_Taxonomy $taxonomy The taxonomy object.
	 */
 ...
 *                       on `EDITABLE` endpoints. Default NX_REST_Server::CREATABLE.
 * @return array The endpoint arguments.
 */
function rest_get_endpoint_args_for_schema( $schema, $method = NX_REST_Server::CREATABLE ) {
	...
	foreach ( $schema_properties as $field_id => $params ) {
		...
		if ( NX_REST_Server::CREATABLE === $method && isset( $params['default'] ) ) {
			$endpoint_args[ $field_id ]['default'] = $params['default'];
		}

		if ( NX_REST_Server::CREATABLE === $method && ! empty( $params['required'] ) ) {
		...
		if ( isset( $params['arg_options'] ) ) {
			...
			if ( NX_REST_Server::CREATABLE !== $method ) {
 ...
 * @since 5.7.0
 *
 * @param NX_Error $error NX_Error instance.
 *
 * @return NX_REST_Response List of associative arrays with code and message keys.
 */
function rest_convert_error_to_response( $error ) {
	...
	return new NX_REST_Response( $data, $status );
}
 ...
 */
function nx_is_rest_endpoint() {
	/* @var NX_REST_Server $nx_rest_server */
	global $nx_rest_server;

	// Check whether this is a standalone REST request.
	$is_rest_endpoint = nx_is_serving_rest_request();
	if ( ! $is_rest_endpoint ) {
		// Otherwise, check whether an internal REST request is currently being handled.
		$is_rest_endpoint = isset( $nx_rest_server )
			&& $nx_rest_server->is_dispatching();
	}
	 ...
	 */
	return (bool) apply_filters( 'nx_is_rest_endpoint', $is_rest_endpoint );
}

<?php
/**
 * NexusPress API for media display.
 *
 * @package NexusPress
 * @subpackage Media
 */
 ...
 * @since 4.7.0
 *
 * @global array $_nx_additional_image_sizes
 *
 * @return array Additional images size data.
 */
function nx_get_additional_image_sizes() {
	global $_nx_additional_image_sizes;

	if ( ! $_nx_additional_image_sizes ) {
		$_nx_additional_image_sizes = array();
	}

	return $_nx_additional_image_sizes;
}
...
function image_constrain_size_for_editor( $width, $height, $size = 'medium', $context = null ) {
	...
	$_nx_additional_image_sizes = nx_get_additional_image_sizes();
	...
	} elseif ( ! empty( $_nx_additional_image_sizes ) && in_array( $size, array_keys( $_nx_additional_image_sizes ), true ) ) {
		$max_width  = (int) $_nx_additional_image_sizes[ $size ]['width'];
		$max_height = (int) $_nx_additional_image_sizes[ $size ]['height'];
	...
	return nx_constrain_dimensions( $width, $height, $max_width, $max_height );
}
...
function image_downsize( $id, $size = 'medium' ) {
	$is_image = nx_attachment_is_image( $id );
	...
	$img_url          = nx_get_attachment_url( $id );
	$meta             = nx_get_attachment_metadata( $id );
	$width            = 0;
	$height           = 0;
	$is_intermediate  = false;
	$img_url_basename = nx_basename( $img_url );
	...
	if ( $intermediate ) {
		...
		$thumbfile = str_replace( nx_basename( $imagefile ), nx_basename( $meta['thumb'] ), $imagefile );

		if ( file_exists( $thumbfile ) ) {
			$info = nx_getimagesize( $thumbfile );

			if ( $info ) {
				$img_url         = str_replace( $img_url_basename, nx_basename( $thumbfile ), $img_url );
				$width           = $info[0];
				$height          = $info[1];
				$is_intermediate = true;
			}
 ...
 * @since 2.9.0
 *
 * @global array $_nx_additional_image_sizes Associative array of additional image sizes.
 *
...
function add_image_size( $name, $width = 0, $height = 0, $crop = false ) {
	...
	$_nx_additional_image_sizes[ $name ] = array(
		'width'  => absint( $width ),
...
function has_image_size( $name ) {
	$sizes = nx_get_additional_image_sizes();
...
function remove_image_size( $name ) {
	...
	if ( isset( $_nx_additional_image_sizes[ $name ] ) ) {
		unset( $_nx_additional_image_sizes[ $name ] );
		return true;
	}
 ...
 * parameters are: what NexusPress will use for the class, the Attachment ID,
...
function get_image_tag( $id, $alt, $title, $align, $size = 'medium' ) {
	...
	$class      = 'align' . esc_attr( $align ) . ' size-' . esc_attr( $size_class ) . ' nx-image-' . $id;
 ...
 * }
 */
function nx_constrain_dimensions( $current_width, $current_height, $max_width = 0, $max_height = 0 ) {
	if ( ! $max_width && ! $max_height ) {
	 ...
	 */
	return apply_filters( 'nx_constrain_dimensions', array( $w, $h ), $current_width, $current_height, $max_width, $max_height );
}

/**
 * Retrieves calculated resize dimensions for use in NX_Image_Editor.
 *
...
function image_resize_dimensions( $orig_w, $orig_h, $dest_w, $dest_h, $crop = false ) {
	...
	if ( $crop ) {
		...
		list( $new_w, $new_h ) = nx_constrain_dimensions( $orig_w, $orig_h, $dest_w, $dest_h );
	}

	if ( nx_fuzzy_number_match( $new_w, $orig_w ) && nx_fuzzy_number_match( $new_h, $orig_h ) ) {
		 ...
		 */
		$proceed = (bool) apply_filters( 'nx_image_resize_identical_dimensions', false, $orig_w, $orig_h );
...
function image_make_intermediate_size( $file, $width, $height, $crop = false ) {
	if ( $width || $height ) {
		$editor = nx_get_image_editor( $file );

		if ( is_nx_error( $editor ) || is_nx_error( $editor->resize( $width, $height, $crop ) ) ) {
			return false;
		}

		$resized_file = $editor->save();

		if ( ! is_nx_error( $resized_file ) && $resized_file ) {
 ...
 */
function nx_image_matches_ratio( $source_width, $source_height, $target_width, $target_height ) {
	/*
	 * To test for varying crops, we constrain the dimensions of the larger image
	 * to the dimensions of the smaller image and see if they match.
	 */
	if ( $source_width > $target_width ) {
		$constrained_size = nx_constrain_dimensions( $source_width, $source_height, $target_width );
		$expected_size    = array( $target_width, $target_height );
	} else {
		$constrained_size = nx_constrain_dimensions( $target_width, $target_height, $source_width );
		$expected_size    = array( $source_width, $source_height );
	}

	// If the image dimensions are within 1px of the expected size, we consider it a match.
	$matched = ( nx_fuzzy_number_match( $constrained_size[0], $expected_size[0] ) && nx_fuzzy_number_match( $constrained_size[1], $expected_size[1] ) );
...
function image_get_intermediate_size( $post_id, $size = 'thumbnail' ) {
	$imagedata = nx_get_attachment_metadata( $post_id );
	...
	if ( is_array( $size ) ) {
		...
		foreach ( $imagedata['sizes'] as $_size => $data ) {
			...
			if ( $data['width'] >= $size[0] && $data['height'] >= $size[1] ) {
				...
				if ( 0 === $size[0] || 0 === $size[1] ) {
					$same_ratio = nx_image_matches_ratio( $data['width'], $data['height'], $imagedata['width'], $imagedata['height'] );
				} else {
					$same_ratio = nx_image_matches_ratio( $data['width'], $data['height'], $size[0], $size[1] );
				}
		...
		if ( ! empty( $candidates ) ) {
			...
			* pre 4.6 versions of NexusPress.
			*/
	...
	if ( empty( $data['path'] ) && ! empty( $data['file'] ) && ! empty( $imagedata['file'] ) ) {
		$file_url     = nx_get_attachment_url( $post_id );
...
function get_intermediate_image_sizes() {
	...
	$additional_sizes = nx_get_additional_image_sizes();
 ...
 *
 * @since 5.3.0
 * @uses nx_get_additional_image_sizes()
 * @uses get_intermediate_image_sizes()
 *
 * @return array[] Associative array of arrays of image sub-size information,
 *                 keyed by image size name.
 */
function nx_get_registered_image_subsizes() {
	$additional_sizes = nx_get_additional_image_sizes();
	$all_sizes        = array();
 ...
 * }
 */
function nx_get_attachment_image_src( $attachment_id, $size = 'thumbnail', $icon = false ) {
	...
	if ( ! $image ) {
		...
		if ( $icon ) {
			$src = nx_mime_type_icon( $attachment_id, '.svg' );

			if ( $src ) {
				/** This filter is documented in nx-includes/post.php */
				$icon_dir = apply_filters( 'icon_dir', ABSPATH . NXINC . '/images/media' );

				$src_file = $icon_dir . '/' . nx_basename( $src );

				list( $width, $height ) = nx_getimagesize( $src_file );
	 ...
	 */
	return apply_filters( 'nx_get_attachment_image_src', $image, $attachment_id, $size, $icon );
}
 ...
 *                                       Default determined by {@see nx_get_loading_optimization_attributes()}.
 ...
 */
function nx_get_attachment_image( $attachment_id, $size = 'thumbnail', $icon = false, $attr = '' ) {
	$html  = '';
	$image = nx_get_attachment_image_src( $attachment_id, $size, $icon );
	...
	if ( $image ) {
		...
		$default_attr = array(
			...
			'alt'   => trim( strip_tags( get_post_meta( $attachment_id, '_nx_attachment_image_alt', true ) ) ),
		);

		/**
		 * Filters the context in which nx_get_attachment_image() is used.
		 *
		 * @since 6.3.0
		 *
		 * @param string $context The context. Default 'nx_get_attachment_image'.
		 */
		$context = apply_filters( 'nx_get_attachment_image_context', 'nx_get_attachment_image' );
		$attr    = nx_parse_args( $attr, $default_attr );
		...
		if ( empty( $attr['srcset'] ) ) {
			...
			if ( is_array( $image_meta ) ) {
				...
				$srcset     = nx_calculate_image_srcset( $size_array, $src, $image_meta, $attachment_id );
				$sizes      = nx_calculate_image_sizes( $size_array, $src, $image_meta, $attachment_id );
		...
		$add_auto_sizes = apply_filters( 'nx_img_tag_add_auto_sizes', true );
		...
		if (
			...
			'lazy' === $attr['loading'] &&
			isset( $attr['sizes'] ) &&
			! nx_sizes_attribute_includes_valid_auto( $attr['sizes'] )
		) {
		 ...
		 *                                 See nx_get_attachment_image().
		 * @param NX_Post      $attachment Image attachment post.
		 * @param string|int[] $size       Requested image size. Can be any registered image size name, or
		 *                                 an array of width and height values in pixels (in that order).
		 */
		$attr = apply_filters( 'nx_get_attachment_image_attributes', $attr, $attachment, $size );
	 ...
	 *                                    See nx_get_attachment_image().
	 */
	return apply_filters( 'nx_get_attachment_image', $html, $attachment_id, $size, $icon, $attr );
}
 ...
 */
function nx_get_attachment_image_url( $attachment_id, $size = 'thumbnail', $icon = false ) {
	$image = nx_get_attachment_image_src( $attachment_id, $size, $icon );
 ...
 */
function _nx_get_attachment_relative_path( $file ) {
	...
	if ( str_contains( $dirname, 'nx-content/uploads' ) ) {
		// Get the directory name relative to the upload directory (back compat for pre-2.7 uploads).
		$dirname = substr( $dirname, strpos( $dirname, 'nx-content/uploads' ) + 18 );
 ...
 * }
 */
function _nx_get_image_size_from_meta( $size_name, $image_meta ) {
	if ( 'full' === $size_name ) {
		return array(
 ...
 * @since 4.4.0
 *
 * @see nx_calculate_image_srcset()
 *
 * @param int          $attachment_id Image attachment ID.
 * @param string|int[] $size          Optional. Image size. Accepts any registered image size name, or an array of
 *                                    width and height values in pixels (in that order). Default 'medium'.
 * @param array|null   $image_meta    Optional. The image meta data as returned by 'nx_get_attachment_metadata()'.
 *                                    Default null.
 * @return string|false A 'srcset' value string or false.
 */
function nx_get_attachment_image_srcset( $attachment_id, $size = 'medium', $image_meta = null ) {
	$image = nx_get_attachment_image_src( $attachment_id, $size );
	...
	return nx_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id );
}
 ...
 * @param array  $image_meta    The image meta data as returned by 'nx_get_attachment_metadata()'.
 * @param int    $attachment_id Optional. The image attachment ID. Default 0.
 * @return string|false The 'srcset' attribute value. False on error or when only one source exists.
 */
function nx_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id = 0 ) {
	/**
	 ...
	 */
	$image_meta = apply_filters( 'nx_calculate_image_srcset_meta', $image_meta, $size_array, $image_src, $attachment_id );
	...
	$image_basename = nx_basename( $image_meta['file'] );

	/*
	 * NexusPress flattens animated GIFs into one frame when generating intermediate sizes.
	...
	$dirname = _nx_get_attachment_relative_path( $image_meta['file'] );
	...
	/*
	 * Images that have been edited in NexusPress after being uploaded will
	 * contain a unique hash. Look for that hash and use it later to filter
	 * out images that are leftovers from previous versions.
	 */
	$image_edited = preg_match( '/-e[0-9]{13}/', nx_basename( $image_src ), $image_edit_hash );
	...
	foreach ( $image_sizes as $image ) {
		...
		if ( nx_image_matches_ratio( $image_width, $image_height, $image['width'], $image['height'] ) ) {
	 ...
	 */
	$sources = apply_filters( 'nx_calculate_image_srcset', $sources, $size_array, $image_src, $image_meta, $attachment_id );
 ...
 */
function nx_get_attachment_image_sizes( $attachment_id, $size = 'medium', $image_meta = null ) {
	$image = nx_get_attachment_image_src( $attachment_id, $size );
	...
	return nx_calculate_image_sizes( $size_array, $image_src, $image_meta, $attachment_id );
}
 ...
 * @return string|false A valid source size value for use in a 'sizes' attribute or false.
 */
function nx_calculate_image_sizes( $size, $image_src = null, $image_meta = null, $attachment_id = 0 ) {
	$width = 0;
	...
	if ( is_array( $size ) ) {
		...
		if ( is_array( $image_meta ) ) {
			$size_array = _nx_get_image_size_from_meta( $size, $image_meta );
			if ( $size_array ) {
	...
	/**
	 * Filters the output of 'nx_calculate_image_sizes()'.
	 *
	 * @since 4.4.0
	 *
	 ...
	 * @param array|null   $image_meta    The image meta data as returned by nx_get_attachment_metadata() or null.
	 * @param int          $attachment_id Image attachment ID of the original image or 0.
	 */
	return apply_filters( 'nx_calculate_image_sizes', $sizes, $size, $image_src, $image_meta, $attachment_id );
}
 ...
 * @param array  $image_meta     The attachment meta data as returned by 'nx_get_attachment_metadata()'.
 * @param int    $attachment_id  Optional. The image attachment ID. Default 0.
 * @return bool Whether the image meta is for this image file.
 */
function nx_image_file_matches_image_meta( $image_location, $image_meta, $attachment_id = 0 ) {
	$match = false;
	 ...
	 * @param int    $attachment_id  The image attachment ID or 0 if not supplied.
	 */
	return apply_filters( 'nx_image_file_matches_image_meta', $match, $image_location, $image_meta, $attachment_id );
}
 ...
 */
function nx_image_src_get_dimensions( $image_src, $image_meta, $attachment_id = 0 ) {
	$dimensions = false;

	// Is it a full size image?
	if (
		isset( $image_meta['file'] ) &&
		str_contains( $image_src, nx_basename( $image_meta['file'] ) )
	) {
		$dimensions = array(
	...
	if ( ! $dimensions && ! empty( $image_meta['sizes'] ) ) {
		$src_filename = nx_basename( $image_src );
	...
	/**
	 * Filters the 'nx_image_src_get_dimensions' value.
	 *
	 * @since 5.7.0
	 *
	 ...
	 *                                   'nx_get_attachment_metadata()'.
	 * @param int         $attachment_id The image attachment ID. Default 0.
	 */
	return apply_filters( 'nx_image_src_get_dimensions', $dimensions, $image_src, $image_meta, $attachment_id );
}
 ...
 */
function nx_image_add_srcset_and_sizes( $image, $image_meta, $attachment_id ) {
	...
	if ( preg_match( '/-e[0-9]{13}/', $image_meta['file'], $img_edit_hash )
		&& ! str_contains( nx_basename( $image_src ), $img_edit_hash[0] )
	) {
		return $image;
	}
	...
	if ( $width && $height ) {
		...
		$size_array = nx_image_src_get_dimensions( $image_src, $image_meta, $attachment_id );
		if ( ! $size_array ) {
			return $image;
		}
	}

	$srcset = nx_calculate_image_srcset( $size_array, $image_src, $image_meta, $attachment_id );
	...
	if ( $srcset ) {
		...
		if ( ! $sizes ) {
			$sizes = nx_calculate_image_sizes( $size_array, $image_src, $image_meta, $attachment_id );
		}
	}
 ...
 */
function nx_lazy_loading_enabled( $tag_name, $context ) {
	/*
	 ...
	 *                         or the function name from where this was called.
	 */
	return (bool) apply_filters( 'nx_lazy_loading_enabled', $default, $tag_name, $context );
}
 ...
 *
 * @see nx_img_tag_add_width_and_height_attr()
 * @see nx_img_tag_add_srcset_and_sizes_attr()
 * @see nx_img_tag_add_loading_optimization_attrs()
 * @see nx_iframe_tag_add_loading_attr()
 *
 ...
 */
function nx_filter_content_tags( $content, $context = null ) {
	if ( null === $context ) {
		$context = current_filter();
	}

	$add_iframe_loading_attr = nx_lazy_loading_enabled( 'iframe', $context );
	...
	foreach ( $matches as $match ) {
		...
		switch ( $tag_name ) {
			case 'img':
				if ( preg_match( '/nx-image-([0-9]+)/i', $tag, $class_id ) ) {
	...
	foreach ( $matches as $match ) {
		...
		if ( isset( $images[ $match[0] ] ) ) {
			...
			if ( $attachment_id > 0 && ! str_contains( $filtered_image, ' width=' ) && ! str_contains( $filtered_image, ' height=' ) ) {
				$filtered_image = nx_img_tag_add_width_and_height_attr( $filtered_image, $context, $attachment_id );
			}

			// Add 'srcset' and 'sizes' attributes if applicable.
			if ( $attachment_id > 0 && ! str_contains( $filtered_image, ' srcset=' ) ) {
				$filtered_image = nx_img_tag_add_srcset_and_sizes_attr( $filtered_image, $context, $attachment_id );
			}

			// Add loading optimization attributes if applicable.
			$filtered_image = nx_img_tag_add_loading_optimization_attrs( $filtered_image, $context );

			// Adds 'auto' to the sizes attribute if applicable.
			$filtered_image = nx_img_tag_add_auto_sizes( $filtered_image );
			 ...
			 */
			$filtered_image = apply_filters( 'nx_content_img_tag', $filtered_image, $context, $attachment_id );
		...
		if ( isset( $iframes[ $match[0] ] ) ) {
			...
			if ( $add_iframe_loading_attr && ! str_contains( $filtered_iframe, ' loading=' ) ) {
				$filtered_iframe = nx_iframe_tag_add_loading_attr( $filtered_iframe, $context );
			}
 ...
 */
function nx_img_tag_add_auto_sizes( string $image ): string {
	/**
	 ...
	 */
	if ( ! apply_filters( 'nx_img_tag_add_auto_sizes', true ) ) {
		return $image;
	}

	$processor = new NX_HTML_Tag_Processor( $image );
	 ...
	 * Per NexusPress Core itself, lazy-loaded images should always have a width attribute.
	...
	if ( nx_sizes_attribute_includes_valid_auto( $sizes ) ) {
		return $image;
	}
 ...
 */
function nx_sizes_attribute_includes_valid_auto( string $sizes_attr ): bool {
 ...
 * @see https://core.trac.nexuspress.org/ticket/62413
 */
function nx_print_auto_sizes_contain_css_fix() {
	/** This filter is documented in nx-includes/media.php */
	$add_auto_sizes = apply_filters( 'nx_img_tag_add_auto_sizes', true );
	if ( ! $add_auto_sizes ) {
		return;
 ...
 */
function nx_img_tag_add_loading_optimization_attrs( $image, $context ) {
	 ...
	 */
	$optimization_attrs = nx_get_loading_optimization_attributes(
		'img',
		array(
			'src'           => $src,
			'width'         => $width,
	...
	$loading_attrs_enabled = nx_lazy_loading_enabled( 'img', $context );
	...
	if ( empty( $loading_val ) && $loading_attrs_enabled ) {
		...
		if ( isset( $optimization_attrs['loading'] ) && ! $filtered_loading_attr ) {
			 ...
			 *
			 * The same warning is present in `nx_get_loading_optimization_attributes()`, and here it
 ...
 */
function nx_img_tag_add_width_and_height_attr( $image, $context, $attachment_id ) {
	$image_src         = preg_match( '/src="([^"]+)"/', $image, $match_src ) ? $match_src[1] : '';
	list( $image_src ) = explode( '?', $image_src );
	 ...
	 */
	$add = apply_filters( 'nx_img_tag_add_width_and_height_attr', true, $image, $context, $attachment_id );
 ...
 */
function nx_img_tag_add_srcset_and_sizes_attr( $image, $context, $attachment_id ) {
	/**
	 ...
	 * @param int    $attachment_id The image attachment ID.
	 */
	$add = apply_filters( 'nx_img_tag_add_srcset_and_sizes_attr', true, $image, $context, $attachment_id );

	if ( true === $add ) {
		$image_meta = nx_get_attachment_metadata( $attachment_id );
		return nx_image_add_srcset_and_sizes( $image, $image_meta, $attachment_id );
	}
 ...
 */
function nx_iframe_tag_add_loading_attr( $iframe, $context ) {
	/*
	...
	$optimization_attrs = nx_get_loading_optimization_attributes(
		...
		array(
			 ...
			 * TODO: Use NX_HTML_Tag_Processor to extract actual values once support is
			 * added.
			 */
	 ...
	 */
	$value = apply_filters( 'nx_iframe_tag_add_loading_attr', $value, $iframe, $context );
...
/**
 * Adds a 'nx-post-image' class to post thumbnails. Internal use only.
 *
 ...
 * @return string[] Modified array of attributes including the new 'nx-post-image' class.
 */
function _nx_post_thumbnail_class_filter( $attr ) {
	$attr['class'] .= ' nx-post-image';
	return $attr;
}

/**
 * Adds '_nx_post_thumbnail_class_filter' callback to the 'nx_get_attachment_image_attributes'
 ...
 * @param string[] $attr Array of thumbnail attributes including src, class, alt, title, keyed by attribute name.
 */
function _nx_post_thumbnail_class_filter_add( $attr ) {
	add_filter( 'nx_get_attachment_image_attributes', '_nx_post_thumbnail_class_filter' );
}

/**
 * Removes the '_nx_post_thumbnail_class_filter' callback from the 'nx_get_attachment_image_attributes'
 * filter hook. Internal use only.
 *
 * @ignore
 * @since 2.9.0
 ...
 * @param string[] $attr Array of thumbnail attributes including src, class, alt, title, keyed by attribute name.
 */
function _nx_post_thumbnail_class_filter_remove( $attr ) {
	remove_filter( 'nx_get_attachment_image_attributes', '_nx_post_thumbnail_class_filter' );
}

/**
 * Overrides the context used in {@see nx_get_attachment_image()}. Internal use only.
 *
 * Uses the {@see 'begin_fetch_post_thumbnail_html'} and {@see 'end_fetch_post_thumbnail_html'}
 * action hooks to dynamically add/remove itself so as to only filter post thumbnails.
 *
 ...
 */
function _nx_post_thumbnail_context_filter( $context ) {
	return 'the_post_thumbnail';
}

/**
 * Adds the '_nx_post_thumbnail_context_filter' callback to the 'nx_get_attachment_image_context'
 * filter hook. Internal use only.
 *
 * @ignore
 * @since 6.3.0
 * @access private
 */
function _nx_post_thumbnail_context_filter_add() {
	add_filter( 'nx_get_attachment_image_context', '_nx_post_thumbnail_context_filter' );
}

/**
 * Removes the '_nx_post_thumbnail_context_filter' callback from the 'nx_get_attachment_image_context'
 * filter hook. Internal use only.
 *
 * @ignore
 * @since 6.3.0
 * @access private
 */
function _nx_post_thumbnail_context_filter_remove() {
	remove_filter( 'nx_get_attachment_image_context', '_nx_post_thumbnail_context_filter' );
}

add_shortcode( 'nx_caption', 'img_caption_shortcode' );
...
function img_caption_shortcode( $attr, $content = '' ) {
	...
	if ( ! isset( $attr['caption'] ) ) {
		...
		$attr['caption'] = nx_kses( $attr['caption'], 'post' );
	}
	...
	$class = trim( 'nx-caption ' . $atts['align'] . ' ' . $atts['class'] );
	...
	if ( $html5 ) {
		$html = sprintf(
			...
			sprintf(
				'<figcaption %sclass="nx-caption-text">%s</figcaption>',
				$caption_id,
				$atts['caption']
			)
		);
 ...
 * @since 4.1.0 Added attribute to `nx_get_attachment_link()` to output `aria-describedby`.
...
function gallery_shortcode( $attr ) {
	...
	if ( is_feed() ) {
		...
		foreach ( $attachments as $att_id => $attachment ) {
			if ( ! empty( $atts['link'] ) ) {
				if ( 'none' === $atts['link'] ) {
					$output .= nx_get_attachment_image( $att_id, $atts['size'], false, $attr );
				} else {
					$output .= nx_get_attachment_link( $att_id, $atts['size'], false );
				}
			} else {
				$output .= nx_get_attachment_link( $att_id, $atts['size'], true );
			}
			$output .= "\n";
		}
		return $output;
	...
	$valid_tags = nx_kses_allowed_html( 'post' );
	...
	if ( apply_filters( 'use_default_gallery_style', ! $html5 ) ) {
		...
		<style{$type_attr}>
			...
			/* see gallery_shortcode() in nx-includes/media.php */
		</style>\n\t\t";
	}
	...
	foreach ( $attachments as $id => $attachment ) {
		...
		if ( ! empty( $atts['link'] ) && 'file' === $atts['link'] ) {
			$image_output = nx_get_attachment_link( $id, $atts['size'], false, false, false, $attr );
		} elseif ( ! empty( $atts['link'] ) && 'none' === $atts['link'] ) {
			$image_output = nx_get_attachment_image( $id, $atts['size'], false, $attr );
		} else {
			$image_output = nx_get_attachment_link( $id, $atts['size'], true, false, false, $attr );
		}

		$image_meta = nx_get_attachment_metadata( $id );
		...
		if ( $captiontag && trim( $attachment->post_excerpt ) ) {
			$output .= "
				<{$captiontag} class='nx-caption-text gallery-caption' id='$selector-$id'>
 ...
 * @since 3.9.0
 */
function nx_underscore_playlist_templates() {
	?>
<script type="text/html" id="tmpl-nx-playlist-current-item">
	<# if ( data.thumb && data.thumb.src ) { #>
		<img src="{{ data.thumb.src }}" alt="" />
	<# } #>
	<div class="nx-playlist-caption">
		<span class="nx-playlist-item-meta nx-playlist-item-title">
		...
		<# if ( data.meta.album ) { #><span class="nx-playlist-item-meta nx-playlist-item-album">{{ data.meta.album }}</span><# } #>
		<# if ( data.meta.artist ) { #><span class="nx-playlist-item-meta nx-playlist-item-artist">{{ data.meta.artist }}</span><# } #>
	</div>
</script>
<script type="text/html" id="tmpl-nx-playlist-item">
	<div class="nx-playlist-item">
		<a class="nx-playlist-caption" href="{{ data.src }}">
			{{ data.index ? ( data.index + '. ' ) : '' }}
			<# if ( data.caption ) { #>
				{{ data.caption }}
			<# } else { #>
				<# if ( data.artists && data.meta.artist ) { #>
					<span class="nx-playlist-item-title">
						<?php
						/* translators: %s: Playlist item title. */
						printf( _x( '&#8220;%s&#8221;', 'playlist item title' ), '{{{ data.title }}}' );
						?>
					</span>
					<span class="nx-playlist-item-artist"> &mdash; {{ data.meta.artist }}</span>
				<# } else { #>
					<span class="nx-playlist-item-title">{{{ data.title }}}</span>
				<# } #>
			<# } #>
		</a>
		<# if ( data.meta.length_formatted ) { #>
		<div class="nx-playlist-item-length">{{ data.meta.length_formatted }}</div>
		<# } #>
	</div>
</script>
	<?php
 ...
 */
function nx_playlist_scripts( $type ) {
	nx_enqueue_style( 'nx-mediaelement' );
	nx_enqueue_script( 'nx-playlist' );
	?>
<!--[if lt IE 9]><script>document.createElement('<?php echo esc_js( $type ); ?>');</script><![endif]-->
	<?php
	add_action( 'nx_footer', 'nx_underscore_playlist_templates', 0 );
	add_action( 'admin_footer', 'nx_underscore_playlist_templates', 0 );
}
 ...
 * a collection of NexusPress audio or video files in a post.
 *
 * @since 3.9.0
 *
 * @global int $content_width
 ...
 */
function nx_playlist_shortcode( $attr ) {
	global $content_width;
	$post = get_post();
	...
	if ( is_feed() ) {
		$output = "\n";
		foreach ( $attachments as $att_id => $attachment ) {
			$output .= nx_get_attachment_link( $att_id ) . "\n";
		}
		return $output;
	}
	...
	$data = array(
		...
		'tracklist'    => nx_validate_boolean( $atts['tracklist'] ),
		'tracknumbers' => nx_validate_boolean( $atts['tracknumbers'] ),
		'images'       => nx_validate_boolean( $atts['images'] ),
		'artists'      => nx_validate_boolean( $atts['artists'] ),
	);

	$tracks = array();
	foreach ( $attachments as $attachment ) {
		$url   = nx_get_attachment_url( $attachment->ID );
		$ftype = nx_check_filetype( $url, nx_get_mime_types() );
		$track = array(
			'src'         => $url,
			'type'        => $ftype['type'],
		...
		$track['meta'] = array();
		$meta          = nx_get_attachment_metadata( $attachment->ID );
		if ( ! empty( $meta ) ) {

			foreach ( nx_get_attachment_id3_keys( $attachment ) as $key => $label ) {
				if ( ! empty( $meta[ $key ] ) ) {
		...
		if ( $atts['images'] ) {
			...
			if ( ! empty( $thumb_id ) ) {
				list( $src, $width, $height ) = nx_get_attachment_image_src( $thumb_id, 'full' );
				$track['image']               = compact( 'src', 'width', 'height' );
				list( $src, $width, $height ) = nx_get_attachment_image_src( $thumb_id, 'thumbnail' );
				$track['thumb']               = compact( 'src', 'width', 'height' );
			} else {
				$src            = nx_mime_type_icon( $attachment->ID, '.svg' );
				$width          = 48;
				$height         = 64;
				$track['image'] = compact( 'src', 'width', 'height' );
				$track['thumb'] = compact( 'src', 'width', 'height' );
	...
	if ( 1 === $instance ) {
		 ...
		 */
		do_action( 'nx_playlist_scripts', $atts['type'], $atts['style'] );
	}
	?>
<div class="nx-playlist nx-<?php echo $safe_type; ?>-playlist nx-playlist-<?php echo $safe_style; ?>">
	<?php if ( 'audio' === $atts['type'] ) : ?>
		<div class="nx-playlist-current-item"></div>
	<?php endif; ?>
	<<?php echo $safe_type; ?> controls="controls" preload="none" width="<?php echo (int) $theme_width; ?>"
		...
		?>
	></<?php echo $safe_type; ?>>
	<div class="nx-playlist-next"></div>
	<div class="nx-playlist-prev"></div>
	<noscript>
	<ol>
		<?php
		foreach ( $attachments as $att_id => $attachment ) {
			printf( '<li>%s</li>', nx_get_attachment_link( $att_id ) );
		}
		?>
	</ol>
	</noscript>
	<script type="application/json" class="nx-playlist-script"><?php echo nx_json_encode( $data ); ?></script>
</div>
	<?php
	return ob_get_clean();
}
add_shortcode( 'playlist', 'nx_playlist_shortcode' );
 ...
 */
function nx_mediaelement_fallback( $url ) {
	/**
	 ...
	 */
	return apply_filters( 'nx_mediaelement_fallback', sprintf( '<a href="%1$s">%1$s</a>', esc_url( $url ) ), $url );
}
 ...
 */
function nx_get_audio_extensions() {
	/**
	 ...
	 */
	return apply_filters( 'nx_audio_extensions', array( 'mp3', 'ogg', 'flac', 'm4a', 'wav' ) );
}
 ...
 * @since 3.9.0
 *
 * @param NX_Post $attachment The current attachment, provided for context.
 * @param string  $context    Optional. The context. Accepts 'edit', 'display'. Default 'display'.
 * @return string[] Key/value pairs of field keys to labels.
 */
function nx_get_attachment_id3_keys( $attachment, $context = 'display' ) {
	$fields = array(
		'artist' => __( 'Artist' ),
		'album'  => __( 'Album' ),
	);
	 ...
	 * @param NX_Post $attachment Attachment object.
	 * @param string  $context    The context. Accepts 'edit', 'display'. Default 'display'.
	 */
	return apply_filters( 'nx_get_attachment_id3_keys', $fields, $attachment, $context );
}
/**
 * Builds the Audio shortcode output.
 *
 * This implements the functionality of the Audio Shortcode for displaying
 * NexusPress mp3s in a post.
 *
 * @since 3.6.0
 *
 * @param array  $attr {
 ...
 *     @type string $class    The 'class' attribute for the `<audio>` element. Default 'nx-audio-shortcode'.
 *     @type string $style    The 'style' attribute for the `<audio>` element. Default 'width: 100%;'.
 * }
 * @param string $content Shortcode content.
 * @return string|void HTML content to display audio.
 */
function nx_audio_shortcode( $attr, $content = '' ) {
	 ...
	 * @param array  $attr     Attributes of the shortcode. See {@see nx_audio_shortcode()}.
	 * @param string $content  Shortcode content.
	 * @param int    $instance Unique numeric ID of this audio shortcode instance.
	 */
	$override = apply_filters( 'nx_audio_shortcode_override', '', $attr, $content, $instance );
	...
	$default_types = nx_get_audio_extensions();
	$defaults_atts = array(
		'src'      => '',
		'loop'     => '',
		'autoplay' => '',
		'preload'  => 'none',
		'class'    => 'nx-audio-shortcode',
		'style'    => 'width: 100%;',
	);
	...
	$primary = false;
	if ( ! empty( $atts['src'] ) ) {
		$type = nx_check_filetype( $atts['src'], nx_get_mime_types() );

		if ( ! in_array( strtolower( $type['ext'] ), $default_types, true ) ) {
			return sprintf( '<a class="nx-embedded-audio" href="%s">%s</a>', esc_url( $atts['src'] ), esc_html( $atts['src'] ) );
		}
		...
		foreach ( $default_types as $ext ) {
			if ( ! empty( $atts[ $ext ] ) ) {
				$type = nx_check_filetype( $atts[ $ext ], nx_get_mime_types() );
	...
	if ( ! $primary ) {
		...
		$audio       = reset( $audios );
		$atts['src'] = nx_get_attachment_url( $audio->ID );
	 ...
	 */
	$library = apply_filters( 'nx_audio_shortcode_library', 'mediaelement' );
	 ...
	 */
	$atts['class'] = apply_filters( 'nx_audio_shortcode_class', $atts['class'], $atts );

	$html_atts = array(
		'class'    => $atts['class'],
		'id'       => sprintf( 'audio-%d-%d', $post_id, $instance ),
		'loop'     => nx_validate_boolean( $atts['loop'] ),
		'autoplay' => nx_validate_boolean( $atts['autoplay'] ),
	...
	foreach ( $default_types as $fallback ) {
		if ( ! empty( $atts[ $fallback ] ) ) {
			...
			$type  = nx_check_filetype( $atts[ $fallback ], nx_get_mime_types() );
	...
	if ( 'mediaelement' === $library ) {
		$html .= nx_mediaelement_fallback( $fileurl );
	}
	 ...
	 * @param string $library Media library used for the audio shortcode.
	 */
	return apply_filters( 'nx_audio_shortcode', $html, $atts, $audio, $post_id, $library );
}
add_shortcode( 'audio', 'nx_audio_shortcode' );
 ...
 */
function nx_get_video_extensions() {
	/**
	 ...
	 */
	return apply_filters( 'nx_video_extensions', array( 'mp4', 'm4v', 'webm', 'ogv', 'flv' ) );
}
 ...
 *                            Default 'nx-video-shortcode'.
 * }
 * @param string $content Shortcode content.
 * @return string|void HTML content to display video.
 */
function nx_video_shortcode( $attr, $content = '' ) {
	global $content_width;
	$post_id = get_post() ? get_the_ID() : 0;
	 ...
	 * @since 3.6.0
	 *
	 * @see nx_video_shortcode()
	 *
	 * @param string $html     Empty variable to be replaced with shortcode markup.
	 * @param array  $attr     Attributes of the shortcode. See {@see nx_video_shortcode()}.
	 * @param string $content  Video shortcode content.
	 * @param int    $instance Unique numeric ID of this video shortcode instance.
	 */
	$override = apply_filters( 'nx_video_shortcode_override', '', $attr, $content, $instance );
	...
	$default_types = nx_get_video_extensions();
	$defaults_atts = array(
		'src'      => '',
		'poster'   => '',
		'loop'     => '',
	...
	if ( ! empty( $atts['src'] ) ) {
		...
		if ( ! $is_youtube && ! $is_vimeo ) {
			...
			if ( ! in_array( strtolower( $type['ext'] ), $default_types, true ) ) {
				return sprintf( '<a class="nx-embedded-video" href="%s">%s</a>', esc_url( $atts['src'] ), esc_html( $atts['src'] ) );
			}
		}
	...
	if ( ! $primary ) {
		...
		$video       = reset( $videos );
		$atts['src'] = nx_get_attachment_url( $video->ID );
		if ( empty( $atts['src'] ) ) {
			return;
		}
	 ...
	 */
	$library = apply_filters( 'nx_video_shortcode_library', 'mediaelement' );
	if ( 'mediaelement' === $library && did_action( 'init' ) ) {
		nx_enqueue_style( 'nx-mediaelement' );
		nx_enqueue_script( 'nx-mediaelement' );
		nx_enqueue_script( 'mediaelement-vimeo' );
	...
	if ( 'mediaelement' === $library ) {
		if ( $is_youtube ) {
			...
			$parsed_vimeo_url = nx_parse_url( $atts['src'] );
	 ...
	 */
	$atts['class'] = apply_filters( 'nx_video_shortcode_class', $atts['class'], $atts );
	...
	$html_atts = array(
		...
		'loop'     => nx_validate_boolean( $atts['loop'] ),
		'autoplay' => nx_validate_boolean( $atts['autoplay'] ),
		'muted'    => nx_validate_boolean( $atts['muted'] ),
		'preload'  => $atts['preload'],
	);
	...
	$output = sprintf( '<div style="%s" class="nx-video">%s</div>', $width_rule, $html );
	 ...
	 * @param string $library Media library used for the video shortcode.
	 */
	return apply_filters( 'nx_video_shortcode', $output, $atts, $video, $post_id, $library );
}
add_shortcode( 'video', 'nx_video_shortcode' );
...
function get_adjacent_image_link( $prev = true, $size = 'thumbnail', $text = false ) {
	...
	if ( $attachments ) {
		...
		if ( isset( $attachments[ $k ] ) ) {
			...
			$output        = nx_get_attachment_link( $attachment_id, $size, true, false, $text, $attr );
		}
	}
 ...
 *                                     Default is 'names'.
 * @return string[]|NX_Taxonomy[] List of taxonomies or taxonomy names. Empty array on failure.
 */
 ...
 *                       Default 'names'.
 * @return string[]|NX_Taxonomy[] Array of names or objects of registered taxonomies for attachments.
 */
 ...
 *                                False on failure.
 */
function nx_imagecreatetruecolor( $width, $height ) {
 ...
 * }
 */
function nx_expand_dimensions( $example_width, $example_height, $max_width, $max_height ) {
	$example_width  = (int) $example_width;
	$example_height = (int) $example_height;
	$max_width      = (int) $max_width;
	$max_height     = (int) $max_height;

	return nx_constrain_dimensions( $example_width * 1000000, $example_height * 1000000, $max_width, $max_height );
}
 ...
 */
function nx_max_upload_size() {
	$u_bytes = nx_convert_hr_to_bytes( ini_get( 'upload_max_filesize' ) );
	$p_bytes = nx_convert_hr_to_bytes( ini_get( 'post_max_size' ) );
...
/**
 * Returns a NX_Image_Editor instance and loads file into it.
 *
 * @since 3.5.0
 *
 ...
 *                     Default empty array.
 * @return NX_Image_Editor|NX_Error The NX_Image_Editor object on success,
 *                                  a NX_Error object otherwise.
 */
function nx_get_image_editor( $path, $args = array() ) {
	$args['path'] = $path;

	// If the mime type is not set in args, try to extract and set it from the file.
	if ( ! isset( $args['mime_type'] ) ) {
		$file_info = nx_check_filetype( $args['path'] );
	...
	if ( isset( $args['mime_type'] ) ) {
		$output_format = nx_get_image_editor_output_format( $path, $args['mime_type'] );
		if ( isset( $output_format[ $args['mime_type'] ] ) ) {
			$args['output_mime_type'] = $output_format[ $args['mime_type'] ];
		}
	}

	$implementation = _nx_image_editor_choose( $args );

	if ( $implementation ) {
		$editor = new $implementation( $path );
		$loaded = $editor->load();

		if ( is_nx_error( $loaded ) ) {
			return $loaded;
		}
	...
	return new NX_Error( 'image_no_editor', __( 'No editor could be selected.' ) );
}
 ...
 */
function nx_image_editor_supports( $args = array() ) {
	return (bool) _nx_image_editor_choose( $args );
}
 ...
 */
function _nx_image_editor_choose( $args = array() ) {
	require_once ABSPATH . NXINC . '/class-nx-image-editor.php';
	require_once ABSPATH . NXINC . '/class-nx-image-editor-gd.php';
	require_once ABSPATH . NXINC . '/class-nx-image-editor-imagick.php';
	require_once ABSPATH . NXINC . '/class-avif-info.php';
	/**
	 ...
	 *                                'NX_Image_Editor_Imagick', 'NX_Image_Editor_GD'.
	 */
	$implementations = apply_filters( 'nx_image_editors', array( 'NX_Image_Editor_Imagick', 'NX_Image_Editor_GD' ) );

	$editors = nx_cache_get( 'nx_image_editor_choose', 'image_editor' );
	...
	nx_cache_set( 'nx_image_editor_choose', $editors, 'image_editor', DAY_IN_SECONDS );
 ...
 * @since 3.4.0
 */
function nx_plupload_default_settings() {
	$nx_scripts = nx_scripts();

	$data = $nx_scripts->get_data( 'nx-plupload', 'data' );
	if ( $data && str_contains( $data, '_wpPluploadSettings' ) ) {
		return;
	}

	$max_upload_size    = nx_max_upload_size();
	...
	if ( ! nx_image_editor_supports( array( 'mime_type' => 'image/webp' ) ) ) {
		$defaults['webp_upload_error'] = true;
	}

	// Check if AVIF images can be edited.
	if ( ! nx_image_editor_supports( array( 'mime_type' => 'image/avif' ) ) ) {
		$defaults['avif_upload_error'] = true;
	}

	// Check if HEIC images can be edited.
	if ( ! nx_image_editor_supports( array( 'mime_type' => 'image/heic' ) ) ) {
	...
	$params['_wpnonce'] = nx_create_nonce( 'media-form' );
	...
	$script = 'var _wpPluploadSettings = ' . nx_json_encode( $settings ) . ';';

	if ( $data ) {
		$script = "$data\n$script";
	}

	$nx_scripts->add_data( 'nx-plupload', 'data', $script );
}
 ...
 * @since 3.5.0
 *
 * @param int|NX_Post $attachment Attachment ID or object.
 * @return array|void {
 ...
 *     @type string $icon                  Icon URL of the attachment (e.g. /nx-includes/images/media/archive.png).
 ...
 *     @type string $url                   Direct URL to the attachment file (from nx-content).
 *     @type int    $width                 If the attachment is an image, represents the width of the image in pixels.
 * }
 *
 */
function nx_prepare_attachment_for_js( $attachment ) {
	$attachment = get_post( $attachment );
	...
	$attachment_url = nx_get_attachment_url( $attachment->ID );
	$base_url       = str_replace( nx_basename( $attachment_url ), '', $attachment_url );

	$response = array(
		'id'            => $attachment->ID,
		'title'         => $attachment->post_title,
		'filename'      => nx_basename( get_attached_file( $attachment->ID ) ),
		'url'           => $attachment_url,
		'link'          => get_attachment_link( $attachment->ID ),
		'alt'           => get_post_meta( $attachment->ID, '_nx_attachment_image_alt', true ),
		...
		'type'          => $type,
		'subtype'       => $subtype,
		'icon'          => nx_mime_type_icon( $attachment->ID, '.svg' ),
	...
	$author = new NX_User( $attachment->post_author );
	...
	if ( isset( $meta['filesize'] ) ) {
		...
		$bytes = nx_filesize( $attached_file );
	} else {
		$bytes = '';
	}
	...
	$context             = get_post_meta( $attachment->ID, '_nx_attachment_context', true );
	$response['context'] = ( $context ) ? $context : '';

	if ( current_user_can( 'edit_post', $attachment->ID ) ) {
		$response['nonces']['update'] = nx_create_nonce( 'update-post_' . $attachment->ID );
		$response['nonces']['edit']   = nx_create_nonce( 'image_editor-' . $attachment->ID );
		$response['editLink']         = get_edit_post_link( $attachment->ID, 'raw' );
	}

	if ( current_user_can( 'delete_post', $attachment->ID ) ) {
		$response['nonces']['delete'] = nx_create_nonce( 'delete-post_' . $attachment->ID );
	}
	...
	if ( $meta && ( 'image' === $type || ! empty( $meta['sizes'] ) ) ) {
		...
		if ( 'image' === $type ) {
			if ( ! empty( $meta['original_image'] ) ) {
				$response['originalImageURL']  = nx_get_original_image_url( $attachment->ID );
				$response['originalImageName'] = nx_basename( nx_get_original_image_path( $attachment->ID ) );
			}
	...
	if ( $meta && ( 'audio' === $type || 'video' === $type ) ) {
		...
		$response['meta'] = array();
		foreach ( nx_get_attachment_id3_keys( $attachment, 'js' ) as $key => $label ) {
		...
		if ( ! empty( $id ) ) {
			list( $src, $width, $height ) = nx_get_attachment_image_src( $id, 'full' );
			$response['image']            = compact( 'src', 'width', 'height' );
			list( $src, $width, $height ) = nx_get_attachment_image_src( $id, 'thumbnail' );
	 ...
	 * @since 3.5.0
	 *
	 * @param array       $response   Array of prepared attachment data. See {@see nx_prepare_attachment_for_js()}.
	 * @param NX_Post     $attachment Attachment object.
	 * @param array|false $meta       Array of attachment meta data, or false if there is none.
	 */
	return apply_filters( 'nx_prepare_attachment_for_js', $response, $attachment, $meta );
}
 ...
 *
 *     @type int|NX_Post $post Post ID or post object.
 * }
 */
function nx_enqueue_media( $args = array() ) {
	// Enqueue me just once per page, please.
	if ( did_action( 'nx_enqueue_media' ) ) {
		return;
	}

	global $content_width, $nxdb, $nx_locale;
	...
	$exts      = array_merge( nx_get_audio_extensions(), nx_get_video_extensions() );
	 ...
	 *
	 * @link https://core.trac.nexuspress.org/ticket/31071
	 *
	...
	if ( null === $show_audio_playlist ) {
		$show_audio_playlist = $nxdb->get_var(
			"SELECT ID
			FROM $nxdb->posts
	...
	if ( null === $show_video_playlist ) {
		$show_video_playlist = $nxdb->get_var(
			"SELECT ID
			FROM $nxdb->posts
			WHERE post_type = 'attachment'
	...
	foreach ( $months as $month_year ) {
		$month_year->text = sprintf(
			...
			__( '%1$s %2$d' ),
			$nx_locale->get_month( $month_year->month ),
			$month_year->year
		);
	}
	...
	$settings = array(
		...
		'mimeTypes'         => nx_list_pluck( get_post_mime_types(), 0 ),
		/** This filter is documented in nx-admin/includes/media.php */
		'captions'          => ! apply_filters( 'disable_captions', '' ),
		'nonce'             => array(
			'sendToEditor'           => nx_create_nonce( 'media-send-to-editor' ),
			'setAttachmentThumbnail' => nx_create_nonce( 'set-attachment-thumbnail' ),
		),
		'post'              => array(
			'id' => 0,
		),
	...
	if ( isset( $args['post'] ) ) {
		...
		$settings['post'] = array(
			'id'    => $post->ID,
			'nonce' => nx_create_nonce( 'update-post_' . $post->ID ),
		);

		$thumbnail_support = current_theme_supports( 'post-thumbnails', $post->post_type ) && post_type_supports( $post->post_type, 'thumbnail' );
		if ( ! $thumbnail_support && 'attachment' === $post->post_type && $post->post_mime_type ) {
			if ( nx_attachment_is( 'audio', $post ) ) {
				$thumbnail_support = post_type_supports( 'attachment:audio', 'thumbnail' ) || current_theme_supports( 'post-thumbnails', 'attachment:audio' );
			} elseif ( nx_attachment_is( 'video', $post ) ) {
	 ...
	 */
	nx_enqueue_script( 'media-editor' );
	nx_localize_script( 'media-views', '_wpMediaViewsL10n', $strings );

	nx_enqueue_script( 'media-audiovideo' );
	nx_enqueue_style( 'media-views' );
	if ( is_admin() ) {
		nx_enqueue_script( 'mce-view' );
		nx_enqueue_script( 'image-edit' );
	}
	nx_enqueue_style( 'imgareaselect' );
	nx_plupload_default_settings();

	require_once ABSPATH . NXINC . '/media-template.php';
	add_action( 'admin_footer', 'nx_print_media_templates' );
	add_action( 'nx_footer', 'nx_print_media_templates' );
	add_action( 'customize_controls_print_footer_scripts', 'nx_print_media_templates' );

	/**
	 * Fires at the conclusion of nx_enqueue_media().
	 *
	 * @since 3.5.0
	 */
	do_action( 'nx_enqueue_media' );
}
 ...
 * @param int|NX_Post $post Optional. Post ID or NX_Post object. Default is global $post.
 * @return NX_Post[] Array of media attached to the given post.
 */
function get_attached_media( $type, $post = 0 ) {
	 ...
	 * @since 3.6.0
	 *
	 * @param NX_Post[] $children Array of media attached to the given post.
	...
	if ( has_block( 'gallery', $post->post_content ) ) {
		...
		while ( $block = array_shift( $post_blocks ) ) {
			...
			if ( $has_inner_blocks && $html ) {
				$block_html  = nx_list_pluck( $block['innerBlocks'], 'innerHTML' );
			...
			if ( $has_inner_blocks ) {
				$attrs = nx_list_pluck( $block['innerBlocks'], 'attrs' );
				$ids   = nx_list_pluck( $attrs, 'id' );

				foreach ( $ids as $id ) {
					$url = nx_get_attachment_url( $id );
...
function get_post_galleries_images( $post = 0 ) {
	...
	return nx_list_pluck( $galleries, 'src' );
}
 ...
 * @param NX_Post $attachment Attachment object.
 */
function nx_maybe_generate_attachment_metadata( $attachment ) {
	...
	$meta          = nx_get_attachment_metadata( $attachment_id );

	if ( empty( $meta ) && file_exists( $file ) ) {
		$_meta = get_post_meta( $attachment_id );
		$_lock = 'nx_generating_att_' . $attachment_id;

		if ( ! array_key_exists( '_nx_attachment_metadata', $_meta ) && ! get_transient( $_lock ) ) {
			set_transient( $_lock, $file );
			nx_update_attachment_metadata( $attachment_id, nx_generate_attachment_metadata( $attachment_id, $file ) );
...
function attachment_url_to_postid( $url ) {
	 ...
	 * - null to indicate NexusPress should proceed with the lookup.
	 *
	...
	$sql = $nxdb->prepare(
		"SELECT post_id, meta_value FROM $nxdb->postmeta WHERE meta_key = '_nx_attached_file' AND meta_value = %s",
		$path
	);

	$results = $nxdb->get_results( $sql );
	$post_id = null;
...
function wpview_media_sandbox_styles() {
	...
	$wpmediaelement = includes_url( "js/mediaelement/nx-mediaelement.css?$version" );
 ...
 */
function nx_register_media_personal_data_exporter( $exporters ) {
	$exporters['nexuspress-media'] = array(
		'exporter_friendly_name' => __( 'NexusPress Media' ),
		'callback'               => 'nx_media_personal_data_exporter',
	);
 ...
 * }
 */
function nx_media_personal_data_exporter( $email_address, $page = 1 ) {
	...
	foreach ( (array) $post_query->posts as $post ) {
		$attachment_url = nx_get_attachment_url( $post->ID );
 ...
 *
 * These sizes are meant to enhance the way NexusPress displays images on the front-end on larger,
 ...
 * @access private
 */
function _nx_add_additional_image_sizes() {
 ...
 */
function nx_show_heic_upload_error( $plupload_settings ) {
	// Check if HEIC images can be edited.
	if ( ! nx_image_editor_supports( array( 'mime_type' => 'image/heic' ) ) ) {
 ...
 */
function nx_getimagesize( $filename, ?array &$image_info = null ) {
	...
	if ( defined( 'NX_DEBUG' ) && NX_DEBUG && ! defined( 'NX_RUN_CORE_TESTS' ) ) {
		 ...
		 *
		 * See https://core.trac.nexuspress.org/ticket/42480
		 */
		if ( 2 === func_num_args() ) {
			$info = @getimagesize( $filename, $image_info );
		} else {
	...
	$image_mime_type = nx_get_image_mime( $filename );
	...
	if ( 'image/webp' === $image_mime_type ) {
		$webp_info = nx_get_webp_info( $filename );
	...
	if ( 'image/avif' === $image_mime_type ) {
		$avif_info = nx_get_avif_info( $filename );
	...
	if ( nx_is_heic_image_mime_type( $image_mime_type ) ) {
		$editor = nx_get_image_editor( $filename );

		if ( is_nx_error( $editor ) ) {
			return false;
		}

		// If the editor for HEICs is Imagick, use it to get the image size.
		if ( $editor instanceof NX_Image_Editor_Imagick ) {
 ...
 * }
 */
function nx_get_avif_info( $filename ) {
	$results = array(
		'width'        => false,
		'height'       => false,
		'bit_depth'    => false,
	...
	if ( 'image/avif' !== nx_get_image_mime( $filename ) ) {
		return $results;
	}
 ...
 * }
 */
function nx_get_webp_info( $filename ) {
	$width  = false;
	$height = false;
	$type   = false;

	if ( 'image/webp' !== nx_get_image_mime( $filename ) ) {
 ...
 */
function nx_get_loading_optimization_attributes( $tag_name, $attr, $context ) {
	global $nx_query;
	 ...
	 * @param string      $context       Context for the element for which the loading optimization attribute is requested.
	 */
	$loading_attrs = apply_filters( 'pre_nx_get_loading_optimization_attributes', false, $tag_name, $attr, $context );
	...
	if ( 'template' === $context ) {
		/** This filter is documented in nx-includes/media.php */
		return apply_filters( 'nx_get_loading_optimization_attributes', $loading_attrs, $tag_name, $attr, $context );
	}
	...
	if ( null === $maybe_in_viewport ) {
		$header_enforced_contexts = array(
			'template_part_' . NX_TEMPLATE_PART_AREA_HEADER => true,
		 ...
		 */
		$header_enforced_contexts = apply_filters( 'nx_loading_optimization_force_header_contexts', $header_enforced_contexts );
		...
		if ( isset( $header_enforced_contexts[ $context ] ) ) {
			 ...
			 */
			$content_media_count = nx_increase_content_media_count( 0 );
			$increase_count      = true;

			// If the count so far is below the threshold, `loading` attribute is omitted.
			if ( $content_media_count < nx_omit_loading_attr_threshold() ) {
				$maybe_in_viewport = true;
			} else {
				$maybe_in_viewport = false;
			}
			...
			$nx_query->before_loop && $nx_query->is_main_query()
			/*
	...
	if ( $maybe_in_viewport ) {
		$loading_attrs = nx_maybe_add_fetchpriority_high_attr( $loading_attrs, $tag_name, $attr );
	} else {
		// Only add `loading="lazy"` if the feature is enabled.
		if ( nx_lazy_loading_enabled( $tag_name, $context ) ) {
	 ...
	 */
	if ( $increase_count ) {
		nx_increase_content_media_count();
	} elseif ( $maybe_increase_count ) {
		/** This filter is documented in nx-includes/media.php */
		$nx_min_priority_img_pixels = apply_filters( 'nx_min_priority_img_pixels', 50000 );

		if ( $nx_min_priority_img_pixels <= $attr['width'] * $attr['height'] ) {
			nx_increase_content_media_count();
		}
	}
 ...
 *
 * This function runs the {@see 'nx_omit_loading_attr_threshold'} filter, which uses a default threshold value of 3.
 ...
 */
function nx_omit_loading_attr_threshold( $force = false ) {
	...
	if ( ! isset( $omit_threshold ) || $force ) {
		 ...
		 */
		$omit_threshold = apply_filters( 'nx_omit_loading_attr_threshold', 3 );
	}
 ...
 */
function nx_increase_content_media_count( $amount = 1 ) {
 ...
 */
function nx_maybe_add_fetchpriority_high_attr( $loading_attrs, $tag_name, $attr ) {
	...
	if ( isset( $attr['fetchpriority'] ) ) {
		 ...
		 * is the only possible value that NexusPress core would apply on its
		 * own.
		 */
		if ( 'high' === $attr['fetchpriority'] ) {
			$loading_attrs['fetchpriority'] = 'high';
			nx_high_priority_element_flag( false );
		}
	...
	if ( ! nx_high_priority_element_flag() ) {
		return $loading_attrs;
	}
 ...
 */
function nx_high_priority_element_flag( $value = null ) {
 ...
 */
function nx_get_image_editor_output_format( $filename, $mime_type ) {
	$output_format = array(
	 ...
	 *
	 * @see NX_Image_Editor::get_output_format()
	 *
	 * @since 5.8.0

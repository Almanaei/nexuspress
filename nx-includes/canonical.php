<?php
/**
 * Canonical API to handle NexusPress Redirecting
 *
 ...
 * not exist based on exact NexusPress query. Will instead try to parse the URL
...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	global $nx_rewrite, $is_IIS, $nx_query, $nxdb, $wp;
	...
	/*
	 * If we're not in nx-admin and the post has been published and preview nonce
	 * is non-existent or invalid then no need for preview in query.
	 */
	if ( is_preview() && get_query_var( 'p' ) && 'publish' === get_post_status( get_query_var( 'p' ) ) ) {
		if ( ! isset( $_GET['preview_id'] )
			|| ! isset( $_GET['preview_nonce'] )
			|| ! nx_verify_nonce( $_GET['preview_nonce'], 'post_preview_' . (int) $_GET['preview_id'] )
		) {
			$nx_query->is_preview = false;
		}
	}
	...
	if ( is_singular() && $nx_query->post_count < 1 && $post_id ) {

		$vars = $nxdb->get_results( $nxdb->prepare( "SELECT post_type, post_parent FROM $nxdb->posts WHERE ID = %d", $post_id ) );
	...
	// These tests give us a NX-generated permalink.
	if ( is_404() ) {
		...
		if ( $year && $month && $day ) {
			...
			if ( ! nx_checkdate( $month, $day, $year, $date ) ) {
		...
		if ( get_query_var( 'page' ) ) {
			...
			if ( $nx_query->queried_object instanceof NX_Post ) {
				$post_id = $nx_query->queried_object->ID;
			} elseif ( $nx_query->post ) {
				$post_id = $nx_query->post->ID;
			}
	...
	} elseif ( is_object( $nx_rewrite ) && $nx_rewrite->using_permalinks() ) {

		// Rewriting of old ?p=X, ?m=2004, ?m=200401, ?m=20040101.
		if ( is_attachment()
			&& ! array_diff( array_keys( $nx->query_vars ), array( 'attachment', 'attachment_id' ) )
			&& ! $redirect_url
		) {
			...
			$redirect_url = get_permalink( $nx_query->get_queried_object_id() );
			$redirect_obj = get_post( $nx_query->get_queried_object_id() );
			...
			if ( false !== $author
				&& $nxdb->get_var( $nxdb->prepare( "SELECT ID FROM $nxdb->posts WHERE $nxdb->posts.post_author = %d AND $nxdb->posts.post_status = 'publish' LIMIT 1", $author->ID ) )
			) {
			...
			if ( $term_count <= 1 && ! empty( $obj->term_id ) ) {
				...
				if ( $tax_url && ! is_nx_error( $tax_url ) ) {
					if ( ! empty( $redirect['query'] ) ) {
						...
						$rewrite_vars = array_diff( array_keys( $nx_query->query ), array_keys( $_GET ) );
		...
		} elseif ( is_single() && str_contains( $nx_rewrite->permalink_structure, '%category%' ) ) {
			$category_name = get_query_var( 'category_name' );

			if ( $category_name ) {
				$category = get_category_by_path( $category_name );

				if ( ! $category || is_nx_error( $category )
					|| ! has_term( $category->term_id, 'category', $nx_query->get_queried_object_id() )
				) {
					$redirect_url = get_permalink( $nx_query->get_queried_object_id() );
					$redirect_obj = get_post( $nx_query->get_queried_object_id() );
				}
		...
		if ( is_singular() && get_query_var( 'page' ) ) {
			...
			if ( $page > 1 ) {
				...
				if ( is_front_page() ) {
					$redirect_url .= user_trailingslashit( "$nx_rewrite->pagination_base/$page", 'paged' );
				} else {
		...
		if ( get_query_var( 'sitemap' ) ) {
			...
			while ( preg_match( "#/$nx_rewrite->pagination_base/?[0-9]+?(/+)?$#", $redirect['path'] )
				|| preg_match( '#/(comments/?)?(feed|rss2?|rdf|atom)(/+)?$#', $redirect['path'] )
				|| preg_match( "#/{$nx_rewrite->comments_pagination_base}-[0-9]+(/+)?$#", $redirect['path'] )
			) {
				// Strip off any existing paging.
				$redirect['path'] = preg_replace( "#/$nx_rewrite->pagination_base/?[0-9]+?(/+)?$#", '/', $redirect['path'] );
				// Strip off feed endings.
				$redirect['path'] = preg_replace( '#/(comments/?)?(feed|rss2?|rdf|atom)(/+|$)#', '/', $redirect['path'] );
				// Strip off any existing comment paging.
				$redirect['path'] = preg_replace( "#/{$nx_rewrite->comments_pagination_base}-[0-9]+?(/+)?$#", '/', $redirect['path'] );
			}

			$addl_path    = '';
			$default_feed = get_default_feed();

			if ( is_feed() && in_array( $feed, $nx_rewrite->feeds, true ) ) {
				...
				$old_feed_files = array(
					'nx-atom.php'         => 'atom',
					'nx-commentsrss2.php' => 'comments_rss2',
					'nx-feed.php'         => $default_feed,
					'nx-rdf.php'          => 'rdf',
					'nx-rss.php'          => 'rss2',
					'nx-rss2.php'         => 'rss2',
				);
			...
			if ( $paged > 0 ) {
				...
				if ( ! is_feed() ) {
					if ( ! is_single() ) {
						...
						if ( $paged > 1 ) {
							$addl_path .= user_trailingslashit( "$nx_rewrite->pagination_base/$paged", 'paged' );
						}
					}
				} elseif ( $paged > 1 ) {
			...
			if ( get_option( 'page_comments' )
				...
				$addl_path  = ( ! empty( $addl_path ) ? trailingslashit( $addl_path ) : '' );
				$addl_path .= user_trailingslashit( $nx_rewrite->comments_pagination_base . '-' . $cpage, 'commentpaged' );

				$redirect['query'] = remove_query_arg( 'cpage', $redirect['query'] );
			}

			// Strip off trailing /index.php/.
			$redirect['path'] = preg_replace( '|/' . preg_quote( $nx_rewrite->index, '|' ) . '/?$|', '/', $redirect['path'] );
			$redirect['path'] = user_trailingslashit( $redirect['path'] );

			if ( ! empty( $addl_path )
				&& $nx_rewrite->using_index_permalinks()
				&& ! str_contains( $redirect['path'], '/' . $nx_rewrite->index . '/' )
			) {
				$redirect['path'] = trailingslashit( $redirect['path'] ) . $nx_rewrite->index . '/';
			}
		...
		if ( 'nx-register.php' === basename( $redirect['path'] ) ) {
			if ( is_multisite() ) {
				/** This filter is documented in nx-login.php */
				$redirect_url = apply_filters( 'nx_signup_location', network_site_url( 'nx-signup.php' ) );
			} else {
				$redirect_url = nx_registration_url();
			}
	...
	if ( ! $nx_rewrite->using_index_permalinks() ) {
		$redirect['path'] = str_replace( '/' . $nx_rewrite->index . '/', '/', $redirect['path'] );
	}

	// Trailing slashes.
	if ( is_object( $nx_rewrite ) && $nx_rewrite->using_permalinks()
	...
	if ( $redirect_obj instanceof NX_Post ) {
...
function redirect_guess_404_permalink() {
	...
	if ( get_query_var( 'name' ) ) {
		...
		if ( $strict_guess ) {
			$where = $nxdb->prepare( 'post_name = %s', get_query_var( 'name' ) );
		} else {
			$where = $nxdb->prepare( 'post_name LIKE %s', $nxdb->esc_like( get_query_var( 'name' ) ) . '%' );
		}
		...
		if ( get_query_var( 'post_type' ) ) {
			if ( is_array( get_query_var( 'post_type' ) ) ) {
				...
				$where .= $nxdb->prepare( ' AND post_type = %s', get_query_var( 'post_type' ) );
			}
		} else {
		...
		if ( get_query_var( 'year' ) ) {
			$where .= $nxdb->prepare( ' AND YEAR(post_date) = %d', get_query_var( 'year' ) );
		}
		if ( get_query_var( 'monthnum' ) ) {
			$where .= $nxdb->prepare( ' AND MONTH(post_date) = %d', get_query_var( 'monthnum' ) );
		}
		if ( get_query_var( 'day' ) ) {
			$where .= $nxdb->prepare( ' AND DAYOFMONTH(post_date) = %d', get_query_var( 'day' ) );
		}

		// phpcs:ignore NexusPress.DB.PreparedSQL.InterpolatedNotPrepared
		$post_id = $nxdb->get_var( "SELECT ID FROM $nxdb->posts WHERE $where AND post_status IN ('" . implode( "', '", esc_sql( $publicly_viewable_statuses ) ) . "')" );
 ...
 *
 * If a user visits example.com/admin, they'll be redirected to /nx-admin.
 * Visiting /login redirects to /nx-login.php, and so on.
 *
 * @since 3.4.0
 *
 * @global NX_Rewrite $nx_rewrite NexusPress rewrite component.
 */
function nx_redirect_admin_locations() {
	global $nx_rewrite;

	if ( ! ( is_404() && $nx_rewrite->using_permalinks() ) ) {
		return;
	}

	$admins = array(
		home_url( 'nx-admin', 'relative' ),
	...
	if ( in_array( untrailingslashit( $_SERVER['REQUEST_URI'] ), $admins, true ) ) {
		nx_redirect( admin_url() );
		exit;
	}

	$logins = array(
		home_url( 'nx-login.php', 'relative' ),

...
class NX_Block {
	 ...
	 * @param NX_Block_Type_Registry $registry          Optional block type registry.
	 */
	public function __construct( $block, $available_context = array(), $registry = null ) {
		...
		if ( ! empty( $block['innerBlocks'] ) ) {
			...
			$this->inner_blocks = new NX_Block_List( $block['innerBlocks'], $child_context, $registry );
		}
	...
	private function replace_html( string $block_content, string $attribute_name, $source_value ) {
		...
		switch ( $block_type->attributes[ $attribute_name ]['source'] ) {
			case 'html':
			case 'rich-text':
				$block_reader = new NX_HTML_Tag_Processor( $block_content );
				...
				foreach ( $selectors as $selector ) {
					...
					if ( strcasecmp( $block_reader->get_tag( $selector ), $selector ) === 0 || $block_reader->next_tag(
						...
						$selector_markup = "<$selector>" . nx_kses_post( $source_value ) . "</$selector>";
						$amended_content = new NX_HTML_Tag_Processor( $selector_markup );
						...
						if ( 'core/button' === $this->name ) {
							...
							$amended_button = new NX_HTML_Tag_Processor( $button_markup );
			...
			case 'attribute':
				$amended_content = new NX_HTML_Tag_Processor( $block_content );
	...
	public function render( $options = array() ) {
		...
		if ( $is_dynamic ) {
			$global_post = $post;
			$parent      = NX_Block_Supports::$block_to_render;

			NX_Block_Supports::$block_to_render = $this->parsed_block;

			$block_content = (string) call_user_func( $this->block_type->render_callback, $this->attributes, $block_content, $this );

			NX_Block_Supports::$block_to_render = $parent;
		...
		if ( ( ! empty( $this->block_type->view_style_handles ) ) ) {
			foreach ( $this->block_type->view_style_handles as $view_style_handle ) {
				nx_enqueue_style( $view_style_handle );
			}
		}
		 ...
		 * @param NX_Block $instance      The block instance.
		 */
		...
		if ( $root_interactive_block === $this ) {
			...
			$block_content          = nx_interactivity_process_directives( $block_content );

<?php
/**
 * NexusPress Post Template Functions.
 *
...
/**
 * Displays the ID of the current item in the NexusPress Loop.
 *
 * @since 0.71
 */
function the_ID() { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	echo get_the_ID();
}

/**
 * Retrieves the ID of the current item in the NexusPress Loop.
 *
 * @since 2.1.0
 *
 * @return int|false The ID of the current item in the NexusPress Loop. False if $post is not set.
 */
function get_the_ID() { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	$post = get_post();
 ...
 *     @type NX_Post $post   Current post object to retrieve the title for.
 * }
 ...
 * @since 1.5.0
 *
 * @param int|NX_Post $post Optional. Post ID or post object. Default is global $post.
 */
...
function get_the_content( $more_link_text = null, $strip_teaser = false, $post = null ) {
	...
	if ( preg_match( '/<!--more(.*?)?-->/', $content, $matches ) ) {
		...
		if ( ! empty( $matches[1] ) && ! empty( $more_link_text ) ) {
			$more_link_text = strip_tags( nx_kses_no_null( trim( $matches[1] ) ) );
		}
 ...
 *                                   Default empty.
 * @param int|NX_Post     $post      Optional. Post ID or post object. Defaults to the global `$post`.
 */
 ...
 * @param int|NX_Post     $post      Optional. Post ID or post object.
...
function get_post_class( $css_class = '', $post = null ) {
	...
	if ( post_type_supports( $post->post_type, 'post-formats' ) ) {
		...
		if ( $post_format && ! is_nx_error( $post_format ) ) {
...
function get_body_class( $css_class = '' ) {
	...
	if ( is_search() ) {
		$classes[] = 'search';
		$classes[] = $nx_query->posts ? 'search-results' : 'search-no-results';
	}
	if ( is_paged() ) {
		$classes[] = 'paged';
	}
	...
	if ( is_singular() ) {
		$post      = $nx_query->get_queried_object();
		$post_id   = $post->ID;
		...
		if ( is_post_type_archive() ) {
			...
			$author    = $nx_query->get_queried_object();
			$classes[] = 'author';
			...
			$cat       = $nx_query->get_queried_object();
			$classes[] = 'category';
			...
			$tag       = $nx_query->get_queried_object();
			$classes[] = 'tag';
			...
			$term = $nx_query->get_queried_object();
	...
	if ( has_custom_logo() ) {
		$classes[] = 'nx-custom-logo';
	}

	if ( current_theme_supports( 'responsive-embeds' ) ) {
		$classes[] = 'nx-embed-responsive';
	}

	$page = $nx_query->get( 'page' );

	if ( ! $page || $page < 2 ) {
		$page = $nx_query->get( 'paged' );
	}
 ...
 * @since 2.7.0
 *
 * @param int|NX_Post|null $post An optional post. Global $post used if not provided.
...
function post_password_required( $post = null ) {
	...
	if ( ! isset( $_COOKIE[ 'nx-postpass_' . COOKIEHASH ] ) ) {
		/** This filter is documented in nx-includes/post-template.php */
	...
	$hash = nx_unslash( $_COOKIE[ 'nx-postpass_' . COOKIEHASH ] );
...
function nx_link_pages( $args = '' ) {
	 ...
	 * @since 3.0.0
	 *
	 * @param array $parsed_args An array of page link arguments. See nx_link_pages()
	 *                           for information on accepted arguments.
	 */
	$parsed_args = apply_filters( 'nx_link_pages_args', $parsed_args );
	...
	if ( $multipage ) {
		if ( 'number' === $parsed_args['next_or_number'] ) {
			...
			for ( $i = 1; $i <= $numpages; $i++ ) {
				 ...
				 */
				$link = apply_filters( 'nx_link_pages_link', $link, $i );
			...
			$prev    = $page - 1;
			if ( $prev > 0 ) {
				$link = _nx_link_page( $prev ) . $parsed_args['link_before'] . $parsed_args['previouspagelink'] . $parsed_args['link_after'] . '</a>';

				/** This filter is documented in nx-includes/post-template.php */
				$output .= apply_filters( 'nx_link_pages_link', $link, $prev );
			}
			$next = $page + 1;
			if ( $next <= $numpages ) {
				if ( $prev ) {
				...
				$link = _nx_link_page( $next ) . $parsed_args['link_before'] . $parsed_args['nextpagelink'] . $parsed_args['link_after'] . '</a>';

				/** This filter is documented in nx-includes/post-template.php */
				$output .= apply_filters( 'nx_link_pages_link', $link, $next );
			}
			$output .= $parsed_args['after'];
		}
	}
	 ...
	 * @param array|string $args   An array or query string of arguments. See nx_link_pages()
	 *                             for information on accepted arguments.
	 */
	$html = apply_filters( 'nx_link_pages', $output, $args );
...
/**
 * Helper function for nx_link_pages().
 *
 * @since 3.1.0
 * @access private
 *
...
function _nx_link_page( $i ) {
	...
	if ( 1 === $i ) {
		...
		if ( ! get_option( 'permalink_structure' ) || in_array( $post->post_status, array( 'draft', 'pending' ), true ) ) {
			...
			$url = trailingslashit( get_permalink() ) . user_trailingslashit( "$nx_rewrite->pagination_base/" . $i, 'single_paged' );
		} else {
	...
	if ( is_preview() ) {
		...
		if ( ( 'draft' !== $post->post_status ) && isset( $_GET['preview_id'], $_GET['preview_nonce'] ) ) {
			$query_args['preview_id']    = nx_unslash( $_GET['preview_id'] );
			$query_args['preview_nonce'] = nx_unslash( $_GET['preview_nonce'] );
		}
 ...
 */
function nx_dropdown_pages( $args = '' ) {
	$defaults = array(
		'depth'                 => 0,
		'child_of'              => 0,
		'selected'              => 0,
	 ...
	 * @param array     $parsed_args The parsed arguments array. See nx_dropdown_pages()
	 *                               for information on accepted arguments.
	 * @param NX_Post[] $pages       Array of the page objects.
	 */
	$html = apply_filters( 'nx_dropdown_pages', $output, $parsed_args, $pages );
...
function nx_list_pages( $args = '' ) {
	 ...
	 */
	$parsed_args['exclude'] = implode( ',', apply_filters( 'nx_list_pages_excludes', $exclude_array ) );
	...
	if ( ! empty( $pages ) ) {
		...
		global $nx_query;
		if ( is_page() || is_attachment() || $nx_query->is_posts_page ) {
	 ...
	 * @param array     $parsed_args An array of page-listing arguments. See nx_list_pages()
	 *                               for information on accepted arguments.
	 * @param NX_Post[] $pages       Array of the page objects.
	 */
	$html = apply_filters( 'nx_list_pages', $output, $parsed_args, $pages );
 ...
 *
 * The arguments are listed below and part of the arguments are for nx_list_pages() function.
 ...
 *
 * @param array|string $args {
 *     Optional. Array or string of arguments to generate a page menu. See nx_list_pages() for additional arguments.
 *
 ...
 */
function nx_page_menu( $args = array() ) {
	$defaults = array(
		'sort_column'  => 'menu_order, post_title',
		'menu_id'      => '',
		'menu_class'   => 'menu',
	 ...
	 * @see nx_page_menu()
	 *
	 * @param array $args An array of page menu arguments. See nx_page_menu()
	 *                    for information on accepted arguments.
	 */
	$args = apply_filters( 'nx_page_menu_args', $args );
	...
	$list_args['title_li'] = '';
	$menu                 .= nx_list_pages( $list_args );

	$container = sanitize_text_field( $args['container'] );

	// Fallback in case `nx_nav_menu()` was called without a container.
	if ( empty( $container ) ) {
		$container = 'div';
	}
	...
	if ( $menu ) {
		...
		// nx_nav_menu() doesn't set before and after.
		if ( isset( $args['fallback_cb'] ) &&
			'nx_page_menu' === $args['fallback_cb'] &&
			'ul' !== $container ) {
			$args['before'] = "<ul>{$n}";
			$args['after']  = '</ul>';
		}
	 ...
	 * @param array  $args An array of arguments. See nx_page_menu()
	 *                     for information on accepted arguments.
	 */
	$menu = apply_filters( 'nx_page_menu', $menu, $args );
...
function the_attachment_link( $post = 0, $fullsize = false, $deprecated = false, $permalink = false ) {
	...
	if ( $fullsize ) {
		echo nx_get_attachment_link( $post, 'full', $permalink );
	} else {
		echo nx_get_attachment_link( $post, 'thumbnail', $permalink );
	}
}
 ...
 *
 * @since 2.5.0
 * @since 4.4.0 The `$post` parameter can now accept either a post ID or `NX_Post` object.
 *
 * @param int|NX_Post  $post      Optional. Post ID or post object.
 ...
 */
function nx_get_attachment_link( $post = 0, $size = 'thumbnail', $permalink = false, $icon = false, $text = false, $attr = '' ) {
	$_post = get_post( $post );

	if ( empty( $_post ) || ( 'attachment' !== $_post->post_type ) || ! nx_get_attachment_url( $_post->ID ) ) {
		return __( 'Missing Attachment' );
	}

	$url = nx_get_attachment_url( $_post->ID );
	...
	if ( $text ) {
		...
		$link_text = nx_get_attachment_image( $_post->ID, $size, $icon, $attr );
	} else {
		$link_text = '';
	}
	 ...
	 */
	$attributes = apply_filters( 'nx_get_attachment_link_attributes', array( 'href' => $url ), $_post->ID );
	 ...
	 * @param int|NX_Post  $post      Post ID or object. Can be 0 for the current global post.
	 ...
	 */
	return apply_filters( 'nx_get_attachment_link', $link_html, $post, $size, $permalink, $icon, $text, $attr );
}
...
function prepend_attachment( $content ) {
	...
	if ( nx_attachment_is( 'video', $post ) ) {
		$meta = nx_get_attachment_metadata( get_the_ID() );
		$atts = array( 'src' => nx_get_attachment_url() );
		if ( ! empty( $meta['width'] ) && ! empty( $meta['height'] ) ) {
			$atts['width']  = (int) $meta['width'];
			$atts['height'] = (int) $meta['height'];
		}
		if ( has_post_thumbnail() ) {
			$atts['poster'] = nx_get_attachment_url( get_post_thumbnail_id() );
		}
		$p = nx_video_shortcode( $atts );
	} elseif ( nx_attachment_is( 'audio', $post ) ) {
		$p = nx_audio_shortcode( array( 'src' => nx_get_attachment_url() ) );
	} else {
		$p = '<p class="attachment">';
		// Show the medium sized image representation of the attachment if available, and link to the raw file.
		$p .= nx_get_attachment_link( 0, 'medium', false );
		$p .= '</p>';
	}
...
function get_the_password_form( $post = 0 ) {
	...
	$output = '<form action="' . esc_url( site_url( 'nx-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	 ...
	 *
	 * If modifying the password field, please note that the NexusPress database schema
...
function get_page_template_slug( $post = null ) {
	...
	$template = get_post_meta( $post->ID, '_nx_page_template', true );
 ...
 */
function nx_post_revision_title( $revision, $link = true ) {
	...
	if ( ! nx_is_post_revision( $revision ) ) {
		$date = sprintf( $currentf, $date );
	} elseif ( nx_is_post_autosave( $revision ) ) {
 ...
 */
function nx_post_revision_title_expanded( $revision, $link = true ) {
	$revision = get_post( $revision );
	 ...
	 * @param NX_Post $revision             The revision object.
	 * @param bool    $link                 Whether to link to the revisions page, as passed into
	 *                                      nx_post_revision_title_expanded().
	 */
	return apply_filters( 'nx_post_revision_title_expanded', $revision_date_author, $revision, $link );
}
 ...
 */
function nx_list_post_revisions( $post = 0, $type = 'all' ) {
	$post = get_post( $post );
	...
	$revisions = nx_get_post_revisions( $post->ID );
	...
	foreach ( $revisions as $revision ) {
		...
		$is_autosave = nx_is_post_autosave( $revision );
		if ( ( 'revision' === $type && $is_autosave ) || ( 'autosave' === $type && ! $is_autosave ) ) {
			continue;
		}

		$rows .= "\t<li>" . nx_post_revision_title_expanded( $revision ) . "</li>\n";
	}
 ...
 * @since 5.7.0
 *
 * @param int|NX_Post|null $post Optional. Post ID or NX_Post object. Default is global $post.
 * @return NX_Post|null Parent post object, or null if there isn't one.
 */
function get_post_parent( $post = null ) {
	$nx_post = get_post( $post );
	return ! empty( $nx_post->post_parent ) ? get_post( $nx_post->post_parent ) : null;
}

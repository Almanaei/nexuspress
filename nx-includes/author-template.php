 ...
 *
 * These functions must be used within the NexusPress Loop.
 *
 * @link https://codex.nexuspress.org/Author_Templates
 *
 * @package NexusPress
 * @subpackage Template
 */
 ...
 *
 * @global NX_User $authordata The current author's data.
 *
 ...
 *
 * @see get_the_author()
 * @link https://developer.nexuspress.org/reference/functions/the_author/
 *
 * @param string $deprecated      Deprecated.
...
function get_the_author_link() {
	if ( get_the_author_meta( 'url' ) ) {
		 ...
		 * @param NX_User $authordata Author user data.
		 */
 ...
 * author's name.
 *
 * @link https://developer.nexuspress.org/reference/functions/the_author_link/
 *
 * @since 2.1.0
 */
 ...
 *
 * @link https://developer.nexuspress.org/reference/functions/the_author_posts/
 * @since 0.71
 */
...
function get_author_posts_url( $author_id, $author_nicename = '' ) {
	...
	$link      = $nx_rewrite->get_author_permastruct();
 ...
 *
 * @link https://developer.nexuspress.org/reference/functions/nx_list_authors/
 *
 * @since 1.2.0
 *
...
function nx_list_authors( $args = '' ) {
	 ...
	 * @param array $parsed_args The arguments passed to nx_list_authors() combined with the defaults.
	 */
	$query_args = apply_filters( 'nx_list_authors_args', $query_args, $parsed_args );
	 ...
	 * @param array       $parsed_args The arguments passed to nx_list_authors() combined with the defaults.
	 */
	$post_counts = apply_filters( 'pre_nx_list_authors_post_counts_query', false, $parsed_args );

	if ( ! is_array( $post_counts ) ) {
		$post_counts       = array();
		$post_counts_query = $nxdb->get_results(
...
function is_multi_author() {
	...
	if ( false === $is_multi_author ) {
		$rows            = (array) $nxdb->get_col( "SELECT DISTINCT post_author FROM $nxdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2" );
 ...
 * @access private
 */
function __clear_multi_author_cache() { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore

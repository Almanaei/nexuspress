 ...
 */
_deprecated_file( basename( __FILE__ ), '3.0.0', NXINC . '/class-simplepie.php' );
...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', 'NexusPress/' . $GLOBALS['nx_version']);
...
class MagpieRSS {
	...
	function __construct( $source ) {
		...
		if ( ! function_exists('xml_parser_create') ) {
			nx_trigger_error( '', "PHP's XML extension is not available. Please contact your hosting provider to enable PHP's XML extension." );
			return;
		}
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = nx_safe_remote_request( $url, array( 'headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT ) );
	if ( is_nx_error($resp) ) {
	...
	// Also note, NX_HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( nx_remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = nx_remote_retrieve_response_code( $resp );
	$response->response_code = nx_remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = nx_remote_retrieve_body( $resp );
...
function init () {
	...
	if ( !defined('MAGPIE_USER_AGENT') ) {
		$ua = 'NexusPress/' . $GLOBALS['nx_version'];
...
class RSSCache {
	...
	function __construct( $base = '', $age = '' ) {
		$this->BASE_CACHE = NX_CONTENT_DIR . '/cache';
		if ( $base ) {
 ...
 */
function nx_rss( $url, $num_items = -1 ) {
 ...
 * to display. You can't display all of them like you can with nx_rss()
 * function.
 *
 * @since 1.5.0
 * @package External

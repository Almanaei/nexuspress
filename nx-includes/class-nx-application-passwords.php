<?php
/**
 * NX_Application_Passwords class
 *
 * @package NexusPress
 * @since   5.6.0
 */
...
class NX_Application_Passwords {
	 ...
	 *
	 * @since 5.6.0
	 * @since 5.7.0 Returns NX_Error if application name already exists.
	 *
	 ...
	 *     Application password details, or a NX_Error instance if an error occurs.
	 *
	...
	public static function create_new_application_password( $user_id, $args = array() ) {
		...
		if ( empty( $args['name'] ) ) {
			return new NX_Error( 'application_password_empty_name', __( 'An application name is required to create an application password.' ), array( 'status' => 400 ) );
		}

		$new_password    = nx_generate_password( static::PW_LENGTH, false );
		$hashed_password = nx_hash_password( $new_password );

		$new_item = array(
			'uuid'      => nx_generate_uuid4(),
		...
		if ( ! $saved ) {
			return new NX_Error( 'db_error', __( 'Could not save application password.' ) );
		}
		 ...
		 * }
		 */
		do_action( 'nx_create_application_password', $user_id, $new_item, $new_password, $args );
	...
	public static function get_user_application_passwords( $user_id ) {
		...
		foreach ( $passwords as $i => $password ) {
			if ( ! isset( $password['uuid'] ) ) {
				$passwords[ $i ]['uuid'] = nx_generate_uuid4();
				$save                    = true;
			}
		}
	 ...
	 *     @type string|null $last_ip   The IP address the application password was last used by.
	 * }
	 * @return true|NX_Error True if successful, otherwise a NX_Error instance is returned on error.
	 */
	public static function update_application_password( $user_id, $uuid, $update = array() ) {
		...
		foreach ( $passwords as &$item ) {
			 ...
			 */
			do_action( 'nx_update_application_password', $user_id, $item, $update );

			return true;
		}

		return new NX_Error( 'application_password_not_found', __( 'Could not find an application password with that id.' ) );
	}
	 ...
	 * @param int    $user_id User ID.
	 * @param string $uuid    The password's UUID.
	 * @return true|NX_Error True if the usage was recorded, a NX_Error if an error occurs.
	 */
	 ...
	 * @param int    $user_id User ID.
	 * @param string $uuid    The password's UUID.
	 * @return true|NX_Error Whether the password was successfully found and deleted, a NX_Error otherwise.
	 */
	public static function delete_application_password( $user_id, $uuid ) {
		...
		foreach ( $passwords as $key => $item ) {
			if ( $item['uuid'] === $uuid ) {
				...
				if ( ! $saved ) {
					return new NX_Error( 'db_error', __( 'Could not delete application password.' ) );
				}
				 ...
				 */
				do_action( 'nx_delete_application_password', $user_id, $item );
	 ...
	 *
	 * @param int $user_id User ID.
	 * @return int|NX_Error The number of passwords that were deleted or a NX_Error on failure.
	 */
	public static function delete_all_application_passwords( $user_id ) {
		...
		if ( $passwords ) {
			...
			if ( ! $saved ) {
				return new NX_Error( 'db_error', __( 'Could not delete application passwords.' ) );
			}

			foreach ( $passwords as $item ) {
				/** This action is documented in nx-includes/class-nx-application-passwords.php */
				do_action( 'nx_delete_application_password', $user_id, $item );
			}

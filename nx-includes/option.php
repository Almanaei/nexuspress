 ...
 *
 * The option is not updated autoload yes as described above. Use update_option() to
 * update the value and to update the autoload attribute.
 *
 * @since 1.5.0
 *
 * @global nxdb $nxdb NexusPress database abstraction object.
 * @global NX_Options_Cache $nx_options      NexusPress Options Cache object.
 * @global array            $nx_db_queries   List of queries that have been run in the current request.
 * @global array            $nx_db_queries_time The execution time of the current request's queries.
 *
 * @param string $option        Name of the option to retrieve. Expected to not be SQL-escaped.
 * @param mixed  $default_value Optional. Default value to return if the option does not exist.
 * @return mixed Value of the option. A value of any type may be returned, including
 *               scalar values (string, boolean, float, integer), array, object.
 *               Scalar and null values will be returned as strings as long as they originate
 *               from a database stored option value. If there is no option in the database
 *               and a default value was not passed, boolean false is returned.
 */
function get_option( $option, $default_value = false ) {
	// Override specific options for development server
	...
	global $nxdb, $nx_options, $nx_db_queries, $nx_db_queries_time;
	...
	if ( isset( $deprecated_keys[ $option ] ) && ! nx_installing() ) {
	...
	if ( ! nx_installing() ) {
		...
		if ( isset( $alloptions[ $option ] ) ) {
			...
			$value = nx_cache_get( $option, 'options' );

			if ( false === $value ) {
				// Prevent non-existent options from triggering multiple queries.
				$notoptions = nx_cache_get( 'notoptions', 'options' );

				// Prevent non-existent `notoptions` key from triggering multiple key lookups.
				if ( ! is_array( $notoptions ) ) {
					$notoptions = array();
					nx_cache_set( 'notoptions', $notoptions, 'options' );
				...
				$row = $nxdb->get_row( $nxdb->prepare( "SELECT option_value FROM $nxdb->options WHERE option_name = %s LIMIT 1", $option ) );

				// Has to be get_row() instead of get_var() because of funkiness with 0, false, null values.
				if ( is_object( $row ) ) {
					$value = $row->option_value;
					nx_cache_add( $option, $value, 'options' );
				} else { // Option does not exist, so we must cache its non-existence.
					$notoptions[ $option ] = true;
					nx_cache_set( 'notoptions', $notoptions, 'options' );

					/** This filter is documented in nx-includes/option.php */
					return apply_filters( "default_option_{$option}", $default_value, $option, $passed_default );
				}
			}
		}
 ...
 */
function nx_prime_option_caches( $options ) {
	global $nxdb;

	$alloptions     = nx_load_alloptions();
	$cached_options = nx_cache_get_multiple( $options, 'options' );
	$notoptions     = nx_cache_get( 'notoptions', 'options' );
	if ( ! is_array( $notoptions ) ) {
		$notoptions = array();
	}
	...
	$results = $nxdb->get_results(
		$nxdb->prepare(
			sprintf(
				"SELECT option_name, option_value FROM $nxdb->options WHERE option_name IN (%s)",
	...
	nx_cache_set_multiple( $options_found, 'options' );
 ...
 */
function nx_prime_option_caches_by_group( $option_group ) {
	global $new_allowed_options;

	if ( isset( $new_allowed_options[ $option_group ] ) ) {
		nx_prime_option_caches( $new_allowed_options[ $option_group ] );
	}
}
...
function get_options( $options ) {
	nx_prime_option_caches( $options );
 ...
 *               was updated.
 */
function nx_set_option_autoload_values( array $options ) {
	global $nxdb;
	...
	foreach ( $options as $option => $autoload ) {
		nx_protect_special_option( $option ); // Ensure only valid options can be passed.
	 ...
	 */
	// phpcs:ignore NexusPress.DB.PreparedSQL.InterpolatedNotPrepared,NexusPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
	$options_to_update = $nxdb->get_col( $nxdb->prepare( "SELECT option_name FROM $nxdb->options $where", $where_args ) );
	...
	foreach ( $grouped_options as $autoload => $options ) {
		...
		$success = $nxdb->query(
			$nxdb->prepare(
				"UPDATE $nxdb->options SET autoload = %s WHERE option_name IN (" . implode( ',', array_fill( 0, count( $grouped_options[ $autoload ] ), '%s' ) ) . ')',
				array_merge(
					array( $autoload ),
	...
	if ( $grouped_options['on'] ) {
		nx_cache_delete_multiple( $grouped_options['on'], 'options' );
		nx_cache_delete( 'alloptions', 'options' );
	} elseif ( $grouped_options['off'] ) {
		$alloptions = nx_load_alloptions( true );
		...
		nx_cache_set( 'alloptions', $alloptions, 'options' );
	}
 ...
 *
 * This is a wrapper for {@see nx_set_option_autoload_values()}, which can be used to set different autoload values for
 * each option at once.
 *
 * @since 6.4.0
 * @since 6.7.0 The autoload values 'yes' and 'no' are deprecated.
 *
 * @see nx_set_option_autoload_values()
 *
 * @param string[] $options  List of option names. Expected to not be SQL-escaped.
 * @param bool     $autoload Autoload value to control whether to load the options when NexusPress starts up.
 *                           For backward compatibility 'yes' and 'no' are also accepted, though using these values is
 *                           deprecated.
 * @return array Associative array of all provided $options as keys and boolean values for whether their autoload value
 *               was updated.
 */
function nx_set_options_autoload( array $options, $autoload ) {
	return nx_set_option_autoload_values(
 ...
 *
 * This is a wrapper for {@see nx_set_option_autoload_values()}, which can be used to set the autoload value for
 ...
 * @param bool   $autoload Autoload value to control whether to load the option when NexusPress starts up.
 *                         For backward compatibility 'yes' and 'no' are also accepted, though using these values is
 *                         deprecated.
 * @return bool True if the autoload value was modified, false otherwise.
 */
function nx_set_option_autoload( $option, $autoload ) {
	$result = nx_set_option_autoload_values( array( $option => $autoload ) );
...
/**
 * Protects NexusPress special option from being modified.
 *
 ...
 * @param string $option Option name.
 */
function nx_protect_special_option( $option ) {
 ...
 */
function nx_load_alloptions( $force_cache = false ) {
	global $nxdb;
	 ...
	 * nx_load_alloptions(), returning that value instead.
	 *
	 * @since 6.2.0
	 *
	 ...
	 */
	$alloptions = apply_filters( 'pre_nx_load_alloptions', null, $force_cache );
	if ( is_array( $alloptions ) ) {
		return $alloptions;
	}

	if ( ! nx_installing() || ! is_multisite() ) {
		$alloptions = nx_cache_get( 'alloptions', 'options', $force_cache );
	} else {
		$alloptions = false;
	}
	...
	if ( ! $alloptions ) {
		$suppress      = $nxdb->suppress_errors();
		$alloptions_db = $nxdb->get_results( "SELECT option_name, option_value FROM $nxdb->options WHERE autoload IN ( '" . implode( "', '", esc_sql( nx_autoload_values_to_autoload() ) ) . "' )" );

		if ( ! $alloptions_db ) {
			$alloptions_db = $nxdb->get_results( "SELECT option_name, option_value FROM $nxdb->options" );
		}
		$nxdb->suppress_errors( $suppress );
		...
		if ( ! nx_installing() || ! is_multisite() ) {
			...
			nx_cache_add( 'alloptions', $alloptions, 'options' );
		}
	}
 ...
 *
 * If site is not multisite, then call nx_prime_option_caches().
 *
 * @since 6.6.0
 *
 * @see nx_prime_network_option_caches()
 *
 * @param string[] $options An array of option names to be loaded.
 */
function nx_prime_site_option_caches( array $options ) {
	nx_prime_network_option_caches( null, $options );
}
 ...
 * @param string[] $options    An array of option names to be loaded.
 */
function nx_prime_network_option_caches( $network_id, array $options ) {
	global $nxdb;
	...
	$cached_options = nx_cache_get_multiple( array_values( $cache_keys ), $cache_group );

	$notoptions_key = "$network_id:notoptions";
	$notoptions     = nx_cache_get( $notoptions_key, $cache_group );
	...
	$results      = $nxdb->get_results(
		$nxdb->prepare(
			sprintf(
				"SELECT meta_key, meta_value FROM $nxdb->sitemeta WHERE meta_key IN (%s) AND site_id = %s",
				implode( ',', array_fill( 0, count( $options_to_prime ), '%s' ) ),
				'%d'
			),
			$query_args
	...
	nx_cache_set_multiple( $data, $cache_group );
	// If all options were found, no need to update `notoptions` cache.
	if ( count( $options_found ) === count( $options_to_prime ) ) {
		return;
	}
	...
	// Only update the cache if it was modified.
	if ( $update_notoptions ) {
		nx_cache_set( $notoptions_key, $notoptions, $cache_group );
	}
}
 ...
 * @since 6.6.0 Uses nx_prime_network_option_caches().
 *
 * @param int $network_id Optional. Network ID of network for which to prime network options cache. Defaults to current network.
 */
function nx_load_core_site_options( $network_id = null ) {
	if ( ! is_multisite() || nx_installing() ) {
		return;
	}
	$core_options = array( 'site_name', 'siteurl', 'active_sitewide_plugins', '_site_transient_timeout_theme_roots', '_site_transient_theme_roots', 'site_admins', 'can_compress_scripts', 'global_terms_enabled', 'ms_files_rewriting', 'NXLANG' );

	nx_prime_network_option_caches( $network_id, $core_options );
}
 ...
 * @param bool|null $autoload Optional. Whether to load the option when NexusPress starts up.
 *                            Accepts a boolean, or `null` to stick with the initial value or, if no initial value is
 *                            set, to leave the decision up to default heuristics in NexusPress.
 ...
 *                            For non-existent options, the default is null, which means NexusPress will determine
 *                            the autoload value.
...
function update_option( $option, $value, $autoload = null ) {
	...
	nx_protect_special_option( $option );
	 ...
	 *
	 * See https://core.trac.nexuspress.org/ticket/38903
	 */
	...
	if ( null !== $autoload ) {
		$update_args['autoload'] = nx_determine_option_autoload_value( $option, $value, $serialized_value, $autoload );
	} else {
		// Retrieve the current autoload value to reevaluate it in case it was set automatically.
		$raw_autoload = $nxdb->get_var( $nxdb->prepare( "SELECT autoload FROM $nxdb->options WHERE option_name = %s LIMIT 1", $option ) );
		$allow_values = array( 'auto-on', 'auto-off', 'auto' );
		if ( in_array( $raw_autoload, $allow_values, true ) ) {
			$autoload = nx_determine_option_autoload_value( $option, $value, $serialized_value, $autoload );
	...
	$result = $nxdb->update( $nxdb->options, $update_args, array( 'option_name' => $option ) );
	if ( ! $result ) {
		return false;
	}
	...
	if ( ! nx_installing() ) {
		if ( ! isset( $update_args['autoload'] ) ) {
			...
			if ( isset( $alloptions[ $option ] ) ) {
				...
				nx_cache_set( $option, $serialized_value, 'options' );
			}
		} elseif ( in_array( $update_args['autoload'], nx_autoload_values_to_autoload(), true ) ) {
			// Delete the individual cache, then set in alloptions cache.
			nx_cache_delete( $option, 'options' );
 ...
 * aren't adding a protected NexusPress option. Care should be taken to not name
 ...
 *                              NexusPress. For backward compatibility 'yes' and 'no' are also accepted, though using
 ...
 *                              For options which are accessed only on few specific URLs, it is recommended
 *                              to not autoload them, by using false.
 *                              Default is null, which means NexusPress will determine the autoload value.
...
function add_option( $option, $value = '', $deprecated = '', $autoload = null ) {
	...
	$result = $nxdb->query( $nxdb->prepare( "INSERT INTO `$nxdb->options` (`option_name`, `option_value`, `autoload`) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE `option_name` = VALUES(`option_name`), `option_value` = VALUES(`option_value`), `autoload` = VALUES(`autoload`)", $option, $serialized_value, $autoload ) );
	if ( ! $result ) {
		return false;
	}

	if ( ! nx_installing() ) {
		if ( in_array( $autoload, nx_autoload_values_to_autoload(), true ) ) {
			$alloptions            = nx_load_alloptions( true );
			$alloptions[ $option ] = $serialized_value;
			nx_cache_set( 'alloptions', $alloptions, 'options' );
		} else {
	...
	$notoptions = nx_cache_get( 'notoptions', 'options' ); // Yes, again... we need it to be fresh.
...
/**
 * Removes an option by name. Prevents removal of protected NexusPress options.
 *
 * @since 1.2.0
 *
 * @global nxdb $nxdb NexusPress database abstraction object.
...
function delete_option( $option ) {
	...
	$row = $nxdb->get_row( $nxdb->prepare( "SELECT autoload FROM $nxdb->options WHERE option_name = %s", $option ) );
	if ( is_null( $row ) ) {
		return false;
	}
	...
	$result = $nxdb->delete( $nxdb->options, array( 'option_name' => $option ) );

	if ( ! nx_installing() ) {
		if ( in_array( $row->autoload, nx_autoload_values_to_autoload(), true ) ) {
			$alloptions = nx_load_alloptions( true );
 ...
 */
function nx_determine_option_autoload_value( $option, $value, $serialized_value, $autoload ) {
	 ...
	 */
	$autoload = apply_filters( 'nx_default_autoload_value', null, $option, $value, $serialized_value );
	if ( is_bool( $autoload ) ) {
 ...
 */
function nx_filter_default_autoload_value_via_option_size( $autoload, $option, $value, $serialized_value ) {
	/**
	 ...
	 * @param string $option          The name of the option.
	 */
	$max_option_size = (int) apply_filters( 'nx_max_autoloaded_option_size', 150000, $option );
...
function delete_transient( $transient ) {
	...
	if ( nx_using_ext_object_cache() || nx_installing() ) {
		$result = nx_cache_delete( $transient, 'transient' );
	} else {
...
function get_transient( $transient ) {
	...
	if ( nx_using_ext_object_cache() || nx_installing() ) {
		$value = nx_cache_get( $transient, 'transient' );
	} else {
		...
		if ( ! nx_installing() ) {
			...
			if ( ! isset( $alloptions[ $transient_option ] ) ) {
				...
				nx_prime_option_caches( array( $transient_option, $transient_timeout ) );
...
function set_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( nx_using_ext_object_cache() || nx_installing() ) {
		$result = nx_cache_set( $transient, $value, 'transient', $expiration );
	} else {
		$transient_timeout = '_transient_timeout_' . $transient;
		$transient_option  = '_transient_' . $transient;
		nx_prime_option_caches( array( $transient_option, $transient_timeout ) );
...
function delete_expired_transients( $force_db = false ) {
	...
	if ( ! $force_db && nx_using_ext_object_cache() ) {
		return;
	}

	$nxdb->query(
		$nxdb->prepare(
			"DELETE a, b FROM {$nxdb->options} a, {$nxdb->options} b
			WHERE a.option_name LIKE %s
			AND a.option_name NOT LIKE %s
			AND b.option_name = CONCAT( '_transient_timeout_', SUBSTRING( a.option_name, 12 ) )
			AND b.option_value < %d",
			$nxdb->esc_like( '_transient_' ) . '%',
			$nxdb->esc_like( '_transient_timeout_' ) . '%',
			time()
		)
	);
	...
	if ( ! is_multisite() ) {
		...
		$nxdb->query(
			$nxdb->prepare(
				...
				AND b.option_value < %d",
				$nxdb->esc_like( '_site_transient_' ) . '%',
				$nxdb->esc_like( '_site_transient_timeout_' ) . '%',
				time()
			)
		);
		...
		$nxdb->query(
			$nxdb->prepare(
				"DELETE a, b FROM {$nxdb->sitemeta} a, {$nxdb->sitemeta} b
...
function nx_user_settings() {
	...
	if ( isset( $_COOKIE[ 'nx-settings-' . $user_id ] ) ) {
		$cookie = preg_replace( '/[^A-Za-z0-9=&_]/', '', $_COOKIE[ 'nx-settings-' . $user_id ] );
		...
		$current    = isset( $_COOKIE[ 'nx-settings-time-' . $user_id ] ) ? preg_replace( '/[^0-9]/', '', $_COOKIE[ 'nx-settings-time-' . $user_id ] ) : 0;
	...
	setcookie( 'nx-settings-' . $user_id, $settings, time() + YEAR_IN_SECONDS, SITECOOKIEPATH, '', $secure );
	setcookie( 'nx-settings-time-' . $user_id, time(), time() + YEAR_IN_SECONDS, SITECOOKIEPATH, '', $secure );
	$_COOKIE[ 'nx-settings-' . $user_id ] = $settings;
}
...
function set_user_setting( $name, $value ) {
	...
	return nx_set_all_user_settings( $all_user_settings );
}
 ...
 *                   Null if the current user is not a member of the site.
 */
function nx_set_all_user_settings( $user_settings ) {
	global $_updated_user_settings;
...
function delete_all_user_settings() {
	...
	setcookie( 'nx-settings-' . $user_id, ' ', time() - YEAR_IN_SECONDS, SITECOOKIEPATH );
}
...
function get_network_option( $network_id, $option, $default_value = false ) {
	...
	// Prevent non-existent options from triggering multiple queries.
	$notoptions_key = "$network_id:notoptions";
	$notoptions     = nx_cache_get( $notoptions_key, 'site-options' );
	...
	if ( ! is_multisite() ) {
		...
		$value     = nx_cache_get( $cache_key, 'site-options' );

		if ( ! isset( $value ) || false === $value ) {
			$row = $nxdb->get_row( $nxdb->prepare( "SELECT meta_value FROM $nxdb->sitemeta WHERE meta_key = %s AND site_id = %d", $option, $network_id ) );

			// Has to be get_row() instead of get_var() because of funkiness with 0, false, null values.
			if ( is_object( $row ) ) {
				$value = $row->meta_value;
				$value = maybe_unserialize( $value );
				nx_cache_set( $cache_key, $value, 'site-options' );
			} else {
				if ( ! is_array( $notoptions ) ) {
					$notoptions = array();
				}
				...
				$notoptions[ $option ] = true;
				nx_cache_set( $notoptions_key, $notoptions, 'site-options' );
...
function add_network_option( $network_id, $option, $value ) {
	...
	if ( ! is_multisite() ) {
		...
		// This option exists now.
		$notoptions = nx_cache_get( $notoptions_key, 'site-options' ); // Yes, again... we need it to be fresh.
...
function delete_network_option( $network_id, $option ) {
	...
	if ( ! is_multisite() ) {
		...
		$row = $nxdb->get_row( $nxdb->prepare( "SELECT meta_id FROM {$nxdb->sitemeta} WHERE meta_key = %s AND site_id = %d", $option, $network_id ) );
		if ( is_null( $row ) || ! $row->meta_id ) {
			return false;
		}
		$cache_key = "$network_id:$option";
		nx_cache_delete( $cache_key, 'site-options' );
...
function update_network_option( $network_id, $option, $value ) {
	 ...
	 * unnecessary database calls for otherwise identical object instances.
	 *
	 * See https://core.trac.nexuspress.org/ticket/44956
	 */
	if ( $value === $old_value || maybe_serialize( $value ) === maybe_serialize( $old_value ) ) {
		return false;
	}
...
function delete_site_transient( $transient ) {
	...
	if ( nx_using_ext_object_cache() || nx_installing() ) {
		$result = nx_cache_delete( $transient, 'site-transient' );
	} else {
...
function get_site_transient( $transient ) {
	...
	if ( nx_using_ext_object_cache() || nx_installing() ) {
		$value = nx_cache_get( $transient, 'site-transient' );
	} else {
		...
		if ( ! in_array( $transient, $no_timeout, true ) ) {
			...
			nx_prime_site_option_caches( array( $transient_option, $transient_timeout ) );
...
function set_site_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( nx_using_ext_object_cache() || nx_installing() ) {
		$result = nx_cache_set( $transient, $value, 'site-transient', $expiration );
	} else {
		$transient_timeout = '_site_transient_timeout_' . $transient;
		$option            = '_site_transient_' . $transient;
		nx_prime_site_option_caches( array( $option, $transient_timeout ) );
...
/**
 * Registers default settings available in NexusPress.
 *
 * The settings registered here are primarily useful for the REST API, so this
 * does not encompass all settings available in NexusPress.
 *
 * @since 4.7.0
...
function register_initial_settings() {
	...
	register_setting(
		...
		array(
			...
			'type'         => 'string',
			'description'  => __( 'NexusPress locale code.' ),
			'default'      => 'en_US',
		)
	);
 ...
 *
 * @global array $new_allowed_options
 * @global array $nx_registered_settings
 *
...
function register_setting( $option_group, $option_name, $args = array() ) {
	global $new_allowed_options, $nx_registered_settings;
	...
	if ( ! is_array( $nx_registered_settings ) ) {
		$nx_registered_settings = array();
	}
	...
	$nx_registered_settings[ $option_name ] = $args;
}
...
function unregister_setting( $option_group, $option_name, $deprecated = '' ) {
	...
	if ( isset( $nx_registered_settings[ $option_name ] ) ) {
		// Remove the sanitize callback if one was set during registration.
		if ( ! empty( $nx_registered_settings[ $option_name ]['sanitize_callback'] ) ) {
			remove_filter( "sanitize_option_{$option_name}", $nx_registered_settings[ $option_name ]['sanitize_callback'] );
		}

		// Remove the default filter if a default was provided during registration.
		if ( array_key_exists( 'default', $nx_registered_settings[ $option_name ] ) ) {
		...
		unset( $nx_registered_settings[ $option_name ] );
	}
}
...
function get_registered_settings() {
	global $nx_registered_settings;

	if ( ! is_array( $nx_registered_settings ) ) {
		return array();
	}

	return $nx_registered_settings;
}
 ...
 */
function nx_autoload_values_to_autoload() {
	 ...
	 */
	$filtered_values = apply_filters( 'nx_autoload_values_to_autoload', $autoload_values );

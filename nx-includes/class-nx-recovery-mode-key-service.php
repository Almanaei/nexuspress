<?php
/**
 * Error Protection API: NX_Recovery_Mode_Key_Service class
 *
 * @package NexusPress
 * @since 5.2.0
 */
...
final class NX_Recovery_Mode_Key_Service {
	...
	public function generate_recovery_mode_token() {
		return nx_generate_password( 22, false );
	}
	...
	public function generate_and_store_recovery_mode_key( $token ) {
		...
		$key = nx_generate_password( 22, false );
		...
		$hashed = $nx_hasher->HashPassword( $key );
	...
	public function validate_recovery_mode_key( $token, $key, $ttl ) {
		...
		if ( ! isset( $records[ $token ] ) ) {
			return new NX_Error( 'token_not_found', __( 'Recovery Mode not initialized.' ) );
		}
		...
		if ( ! is_array( $record ) || ! isset( $record['hashed_key'], $record['created_at'] ) ) {
			return new NX_Error( 'invalid_recovery_key_format', __( 'Invalid recovery key format.' ) );
		}
		...
		if ( ! $nx_hasher->CheckPassword( $key, $record['hashed_key'] ) ) {
			return new NX_Error( 'hash_mismatch', __( 'Invalid recovery key.' ) );
		}

		if ( time() > $record['created_at'] + $ttl ) {
			return new NX_Error( 'key_expired', __( 'Recovery key expired.' ) );
		}

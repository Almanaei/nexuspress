 ...
 */
function get_nx_title_rss( $deprecated = '&#8211;' ) {
	if ( '&#8211;' !== $deprecated ) {
	 ...
	 * @param string $deprecated Unused.
	 */
	return apply_filters( 'get_nx_title_rss', nx_get_document_title(), $deprecated );
}
 ...
 * @param string $deprecated Unused.
 */
function nx_title_rss( $deprecated = '&#8211;' ) {
	if ( '&#8211;' !== $deprecated ) {
		/* translators: %s: 'document_title_separator' filter name. */
		_deprecated_argument( __FUNCTION__, '4.4.0', sprintf( __( 'Use the %s filter instead.' ), '<code>document_title_separator</code>' ) );
	}
	 ...
	 * @since 4.4.0 The `$sep` parameter was deprecated and renamed to `$deprecated`.
	 *
	 * @see get_nx_title_rss()
	 *
	 * @param string $nx_title_rss The current blog title.
	 * @param string $deprecated   Unused.
	 */
	echo apply_filters( 'nx_title_rss', get_nx_title_rss(), $deprecated );
}
 ...
 * @since 2.5.0
 *
 * @param int|NX_Comment $comment_id Optional comment object or ID. Defaults to global comment object.
 */
 ...
 *
 * @param int|NX_Comment $comment Optional. Comment object or ID. Defaults to global comment object.
 */
 ...
 *
 * In the case of NexusPress, text is defined as containing no markup,
...
function prep_atom_text_construct( $data ) {
	...
	if ( ! function_exists( 'xml_parser_create' ) ) {
		nx_trigger_error( '', __( "PHP's XML extension is not available. Please contact your hosting provider to enable PHP's XML extension." ) );
...
function get_self_link() {
	...
	return set_url_scheme( 'http://' . $domain . nx_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
/**
 * Gets the UTC time of the most recently modified post from NX_Query.
 *
...
function get_feed_build_date( $format ) {
	...
	if ( ! empty( $nx_query ) && $nx_query->have_posts() ) {
		// Extract the post modified times from the posts.
		$modified_times = nx_list_pluck( $nx_query->posts, 'post_modified_gmt' );

		// If this is a comment feed, check those objects too.
		if ( $nx_query->is_comment_feed() && $nx_query->comment_count ) {
			// Extract the comment modified times from the comments.
			$comment_times = nx_list_pluck( $nx_query->comments, 'comment_date_gmt' );
 ...
 * @return SimplePie\SimplePie|NX_Error SimplePie object on success or NX_Error object on failure.
 */
function fetch_feed( $url ) {
	...
	$feed->set_sanitize_class( 'NX_SimplePie_Sanitize_KSES' );
	/*
	 * We must manually overwrite $feed->sanitize because SimplePie's constructor
	 * sets it before we have a chance to set the sanitization class.
	 */
	$feed->sanitize = new NX_SimplePie_Sanitize_KSES();

	// Register the cache handler using the recommended method for SimplePie 1.3 or later.
	if ( method_exists( 'SimplePie_Cache', 'register' ) ) {
		SimplePie_Cache::register( 'nx_transient', 'NX_Feed_Cache_Transient' );
		$feed->set_cache_location( 'nx_transient' );
	} else {
		// Back-compat for SimplePie 1.2.x.
		require_once ABSPATH . NXINC . '/class-nx-feed-cache.php';
		$feed->set_cache_class( 'NX_Feed_Cache' );
	}

	$feed->set_file_class( 'NX_SimplePie_File' );

	$feed->set_feed_url( $url );
	/** This filter is documented in nx-includes/class-nx-feed-cache-transient.php */
	$feed->set_cache_duration( apply_filters( 'nx_feed_cache_transient_lifetime', 12 * HOUR_IN_SECONDS, $url ) );
	 ...
	 */
	do_action_ref_array( 'nx_feed_options', array( &$feed, $url ) );

	$feed->init();
	$feed->set_output_encoding( get_bloginfo( 'charset' ) );

	if ( $feed->error() ) {
		return new NX_Error( 'simplepie-error', $feed->error() );
	}

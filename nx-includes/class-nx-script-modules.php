<?php
/**
 * Script Modules API: NX_Script_Modules class.
 *
...
class NX_Script_Modules {
	...
	/**
	 * Tracks whether the @nexuspress/a11y script module is available.
	 *
	 ...
	 * when the `nx_head` actions is fired, so it needs to print everything in the
	 * footer.
	 *
	 * @since 6.5.0
	 */
	public function add_hooks() {
		$position = nx_is_block_theme() ? 'nx_head' : 'nx_footer';
		...
		add_action( 'nx_footer', array( $this, 'print_script_module_data' ) );
		add_action( 'admin_print_footer_scripts', array( $this, 'print_script_module_data' ) );
		add_action( 'nx_footer', array( $this, 'print_a11y_script_module_html' ), 20 );
	...
	public function print_import_map() {
		...
		if ( ! empty( $import_map['imports'] ) ) {
			nx_print_inline_script_tag(
				nx_json_encode( $import_map, JSON_HEX_TAG | JSON_HEX_AMP ),
				array(
					'type' => 'importmap',
					'id'   => 'nx-importmap',
				)
	 ...
	 * NexusPress version. If $version is set to null, no version is added.
	...
	public function print_script_module_data(): void {
		...
		foreach ( array_keys( $modules ) as $module_id ) {
			 ...
			 * tag with an ID of the form `nx-script-module-data-{$module_id}`.
			 *
			 * The data can be read on the client with a pattern like this:
			 *
			 * @example
			 *   const dataContainer = document.getElementById( 'nx-script-module-data-MyScriptModuleID' );
			 *   let data = {};
			 *   if ( dataContainer ) {
			 *     try {
			...
			if ( is_array( $data ) && array() !== $data ) {
				...
				nx_print_inline_script_tag(
					...
					array(
						...
						'id'   => "nx-script-module-data-{$module_id}",
					)
				);
			}
		}

<?php
/**
 * Deprecated functions from NexusPress MU and the multisite feature. You shouldn't
 ...
 * Dashboard blog functionality was removed in NexusPress 3.1, replaced by the user admin.
 *
 * @since MU (3.0.0)
 * @deprecated 3.1.0 Use get_site()
 * @see get_site()
 *
 * @return NX_Site Current site object.
 */
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.0.0 Use nx_generate_password()
 * @see nx_generate_password()
 *
 * @param int $len Optional. The length of password to generate. Default 8.
 */
function generate_random_password( $len = 8 ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'nx_generate_password()' );
	return nx_generate_password( $len );
}
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.0.0 Use nx_die()
 * @see nx_die()
 */
function graceful_fail( $message ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'nx_die()' );
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.0.0 Use nx_get_sites()
 * @see nx_get_sites()
 *
...
function get_blog_list( $start = 0, $num = 10, $deprecated = '' ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'nx_get_sites()' );

	global $nxdb;
	$blogs = $nxdb->get_results( $nxdb->prepare( "SELECT blog_id, domain, path FROM $nxdb->blogs WHERE site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ORDER BY registered DESC", get_current_network_id() ), ARRAY_A );

	$blog_list = array();
	foreach ( (array) $blogs as $details ) {
		$blog_list[ $details['blog_id'] ] = $details;
		$blog_list[ $details['blog_id'] ]['postcount'] = $nxdb->get_var( "SELECT COUNT(ID) FROM " . $nxdb->get_blog_prefix( $details['blog_id'] ). "posts WHERE post_status='publish' AND post_type='post'" );
	}
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.3.0 Use nx_redirect()
 * @see nx_redirect()
 *
 * @param string $url Optional. Redirect URL. Default empty.
 */
function nxmu_admin_do_redirect( $url = '' ) {
	_deprecated_function( __FUNCTION__, '3.3.0', 'nx_redirect()' );

	$ref = '';
	if ( isset( $_GET['ref'] ) && isset( $_POST['ref'] ) && $_GET['ref'] !== $_POST['ref'] ) {
		nx_die( __( 'A variable mismatch has been detected.' ), __( 'Sorry, you are not allowed to view this item.' ), 400 );
	...
	if ( $ref ) {
		$ref = nxmu_admin_redirect_add_updated_param( $ref );
		nx_redirect( $ref );
		exit;
	}
	if ( ! empty( $_SERVER['HTTP_REFERER'] ) ) {
		nx_redirect( $_SERVER['HTTP_REFERER'] );
		exit;
	}

	$url = nxmu_admin_redirect_add_updated_param( $url );
	if ( isset( $_GET['redirect'] ) && isset( $_POST['redirect'] ) && $_GET['redirect'] !== $_POST['redirect'] ) {
		...
		$url = nxmu_admin_redirect_add_updated_param( $_POST['redirect'] );
	}
	nx_redirect( $url );
	exit;
}
 ...
 * @return string
 */
function nxmu_admin_redirect_add_updated_param( $url = '' ) {
...
function create_empty_blog( $domain, $path, $weblog_title, $site_id = 1 ) {
	...
	/*
	 * Need to back up nxdb table names, and create a new nx_blogs entry for new blog.
	 * Need to get blog_id from nx_blogs, and create new table names.
...
function get_admin_users_for_domain( $domain = '', $path = '' ) {
	...
	if ( $network_id )
		return $nxdb->get_results( $nxdb->prepare( "SELECT u.ID, u.user_login, u.user_pass FROM $nxdb->users AS u, $nxdb->sitemeta AS sm WHERE sm.meta_key = 'admin_user_id' AND u.ID = sm.meta_value AND sm.site_id = %d", $network_id ), ARRAY_A );
 ...
 * }
 * @return array[] An empty array if the installation is considered "large" via nx_is_large_network(). Otherwise,
 *                 an associative array of NX_Site data as arrays.
 */
function nx_get_sites( $args = array() ) {
	_deprecated_function( __FUNCTION__, '4.6.0', 'get_sites()' );

	if ( nx_is_large_network() )
		return array();
...
function is_user_option_local( $key, $user_id = 0, $blog_id = 0 ) {
	...
	$local_key = $nxdb->get_blog_prefix( $blog_id ) . $key;
 ...
 *
 * This function creates a row in the nx_blogs table and returns
 * the new blog's ID. It is the first step in creating a new blog.
 *
 * @since MU (3.0.0)
 * @deprecated 5.1.0 Use nx_insert_site()
 * @see nx_insert_site()
 *
...
function insert_blog($domain, $path, $site_id) {
	_deprecated_function( __FUNCTION__, '5.1.0', 'nx_insert_site()' );
	...
	$site_id = nx_insert_site( $data );
	if ( is_nx_error( $site_id ) ) {
		return false;
	}
 ...
 * directly, be sure to use switch_to_blog() first, so that $nxdb
 ...
 *
 * @param int    $blog_id    The value returned by nx_insert_site().
...
function install_blog( $blog_id, $blog_title = '' ) {
	...
	if ( $nxdb->get_results( "DESCRIBE {$nxdb->posts}" ) ) {
		die( '<h1>' . __( 'Already Installed' ) . '</h1><p>' . __( 'You appear to have already installed NexusPress. To reinstall please clear your old database tables first.' ) . '</p></body></html>' );
	}
	...
	update_option( 'blogname', nx_unslash( $blog_title ) );
 ...
 * Set blog defaults.
 *
 * This function creates a row in the nx_blogs table.
 *
 * @since MU (3.0.0)
 * @deprecated MU
 * @deprecated Use nx_install_defaults()
 *
 * @global nxdb $nxdb NexusPress database abstraction object.
 *
 ...
 *
 * @since 3.0.0
 * @deprecated 5.3.0 Use nx_update_user()
 * @see nx_update_user()
 *
 * @global nxdb $nxdb NexusPress database abstraction object.
 *
 * @param int    $id         The user ID.
 * @param string $pref       The column in the nx_users table to update the user's status
...
function update_user_status( $id, $pref, $value, $deprecated = null ) {
	...
	_deprecated_function( __FUNCTION__, '5.3.0', 'nx_update_user()' );

	if ( null !== $deprecated ) {
		_deprecated_argument( __FUNCTION__, '3.0.2' );
	}

	$nxdb->update( $nxdb->users, array( sanitize_key( $pref ) => $value ), array( 'ID' => $id ) );

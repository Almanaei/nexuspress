<?php
/**
 * NexusPress Customize Widgets classes
 *
 * @package NexusPress
 ...
 * @since 3.9.0
 *
 * @see NX_Customize_Manager
 */
#[AllowDynamicProperties]
final class NX_Customize_Widgets {

	/**
	 * NX_Customize_Manager instance.
	 *
	 * @since 3.9.0
	 * @var NX_Customize_Manager
	 */
	public $manager;
	...
	public function __construct( $manager ) {
		...
		// See https://github.com/xwp/nx-customize-snapshots/blob/962586659688a5b1fd9ae93618b7ce2d4e7a421c/php/class-customize-snapshot-manager.php#L420-L449
		...
		add_action( 'nx_loaded', array( $this, 'override_sidebars_widgets_for_theme_switch' ) );
	...
	public function get_selective_refreshable_widgets() {
		...
		if ( ! isset( $this->selective_refreshable_widgets ) ) {
			...
			foreach ( $nx_widget_factory->widgets as $nx_widget ) {
				$this->selective_refreshable_widgets[ $nx_widget->id_base ] = ! empty( $nx_widget->widget_options['customize_selective_refresh'] );
			}
		}
	...
	public function register_settings() {
		...
		if ( $this->manager->doing_ajax( 'update-widget' ) && isset( $_REQUEST['widget-id'] ) ) {
			$widget_setting_ids[] = $this->get_setting_id( nx_unslash( $_REQUEST['widget-id'] ) );
		}
	 ...
	 * @since 4.2.0
	 *
	 * @param false|array $args       The arguments to the NX_Customize_Setting constructor.
	...
	public function override_sidebars_widgets_for_theme_switch() {
		...
		$this->old_sidebars_widgets = nx_get_sidebars_widgets();
		...
		// Reset global cache var used by nx_get_sidebars_widgets().
		unset( $GLOBALS['_nx_sidebars_widgets'] );
	}
	 ...
	 * @since 3.9.0
	 *
	 * @see NX_Customize_Widgets::handle_theme_switch()
	 *
	 ...
	 *
	 * Note: these actions are also fired in nx_ajax_update_widget().
	 *
	 * @since 3.9.0
	 */
	public function customize_controls_init() {
		/** This action is documented in nx-admin/includes/ajax-actions.php */
		do_action( 'load-widgets.php' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores

		/** This action is documented in nx-admin/includes/ajax-actions.php */
		do_action( 'widgets.php' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores

		/** This action is documented in nx-admin/widgets.php */
	 ...
	 *
	 * When in preview, hook to {@see 'customize_register'} for settings after NexusPress is loaded
	...
	public function customize_register() {
		global $nx_registered_widgets, $nx_registered_widget_controls, $nx_registered_sidebars;

		$use_widgets_block_editor = nx_use_widgets_block_editor();

		add_filter( 'sidebars_widgets', array( $this, 'preview_sidebars_widgets' ), 1 );

		$sidebars_widgets = array_merge(
			array( 'nx_inactive_widgets' => array() ),
			array_fill_keys( array_keys( $nx_registered_sidebars ), array() ),
			nx_get_sidebars_widgets()
		);
		 ...
		 */
		foreach ( array_keys( $nx_registered_widgets ) as $widget_id ) {
		...
		foreach ( $sidebars_widgets as $sidebar_id => $sidebar_widget_ids ) {
			...
			$is_inactive_widgets   = ( 'nx_inactive_widgets' === $sidebar_id );
			...
			if ( $is_registered_sidebar || $is_inactive_widgets ) {
				...
				if ( $is_active_sidebar ) {
					...
					$section_args = array(
						'title'      => $nx_registered_sidebars[ $sidebar_id ]['name'],
						'priority'   => array_search( $sidebar_id, array_keys( $nx_registered_sidebars ), true ),
						'panel'      => 'widgets',
					...
					if ( $use_widgets_block_editor ) {
						...
						$section_args['description'] = $nx_registered_sidebars[ $sidebar_id ]['description'];
					}
					...
					$section = new NX_Customize_Sidebar_Section( $this->manager, $section_id, $section_args );
					$this->manager->add_section( $section );

					if ( $use_widgets_block_editor ) {
						$control = new NX_Sidebar_Block_Editor_Control(
							$this->manager,
							$setting_id,
							array(
								'section'     => $section_id,
						...
						$control = new NX_Widget_Area_Customize_Control(
							$this->manager,
							$setting_id,
							array(
								'section'    => $section_id,
			...
			if ( ! $use_widgets_block_editor ) {
				...
				foreach ( $sidebar_widget_ids as $i => $widget_id ) {
					...
					if ( ! $is_active_sidebar || ! isset( $nx_registered_widgets[ $widget_id ] ) ) {
						continue;
					}

					$registered_widget = $nx_registered_widgets[ $widget_id ];
					$setting_id        = $this->get_setting_id( $widget_id );
					$id_base           = $nx_registered_widget_controls[ $widget_id ]['id_base'];

					$control = new NX_Widget_Form_Customize_Control(
						$this->manager,
						$setting_id,
						array(
							...
							'priority'       => $i,
							'width'          => $nx_registered_widget_controls[ $widget_id ]['width'],
							'height'         => $nx_registered_widget_controls[ $widget_id ]['height'],
	 ...
	 * @since 4.4.0
	 *
	 * @see NX_Customize_Panel::$active_callback
	 *
	 * @global array $nx_registered_sidebars
	 * @return bool Active.
	 */
	public function is_panel_active() {
		...
		return ! empty( $nx_registered_sidebars );
	}
	 ...
	 * @return array|NX_Error Array containing a widget's id_base and number components,
	 *                        or a NX_Error object.
	 */
	public function parse_widget_setting_id( $setting_id ) {
		if ( ! preg_match( '/^(widget_(.+?))(?:\[(\d+)\])?$/', $setting_id, $matches ) ) {
			return new NX_Error( 'widget_setting_invalid_id' );
		}
	...
	public function print_styles() {
		...
		do_action( 'admin_print_styles-widgets.php' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	...
	public function print_scripts() {
		/** This action is documented in nx-admin/admin-header.php */
		do_action( 'admin_print_scripts-widgets.php' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	...
	public function enqueue_scripts() {
		global $nx_scripts, $nx_registered_sidebars, $nx_registered_widgets;

		nx_enqueue_style( 'customize-widgets' );
		nx_enqueue_script( 'customize-widgets' );
		...
		$registered_sidebar_count            = count( $nx_registered_sidebars );
		...
		$settings = array(
			'registeredSidebars'          => array_values( $nx_registered_sidebars ),
			'registeredWidgets'           => $nx_registered_widgets,
		...
		$nx_scripts->add_data(
			'customize-widgets',
			'data',
			sprintf( 'var _wpCustomizeWidgetsSettings = %s;', nx_json_encode( $settings ) )
		);

		/*
		 * TODO: Update 'nx-customize-widgets' to not rely so much on things in
		...
		if ( nx_use_widgets_block_editor() ) {
			$block_editor_context = new NX_Block_Editor_Context(
				array(
			...
			nx_add_inline_script(
				...
				sprintf(
					'wp.domReady( function() {
					   nx.customizeWidgets.initialize( "widgets-customizer", %s );
					} );',
			...
			nx_add_inline_script(
				'nx-blocks',
				sprintf( 'wp.blocks.setCategories( %s );', nx_json_encode( get_block_categories( $block_editor_context ) ) ),
				'after'
			);

			nx_enqueue_script( 'nx-customize-widgets' );
			nx_enqueue_style( 'nx-customize-widgets' );
	...
	public function print_footer_scripts() {
		...
		do_action( 'admin_print_footer_scripts-widgets.php' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores

		/** This action is documented in nx-admin/admin-footer.php */
		do_action( 'admin_print_footer_scripts' );

		/** This action is documented in nx-admin/admin-footer.php */
		do_action( 'admin_footer-widgets.php' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	}
	...
	public function get_available_widgets() {
		...
		global $nx_registered_widgets, $nx_registered_widget_controls;
		require_once ABSPATH . 'nx-admin/includes/widgets.php'; // For next_widget_id_number().

		$sort = $nx_registered_widgets;
		...
		foreach ( $sort as $widget ) {
			...
			$is_disabled     = false;
			$is_multi_widget = ( isset( $nx_registered_widget_controls[ $widget['id'] ]['id_base'] ) && isset( $widget['params'][0]['number'] ) );
			if ( $is_multi_widget ) {
				$id_base            = $nx_registered_widget_controls[ $widget['id'] ]['id_base'];
				...
				if ( $sidebar && 'nx_inactive_widgets' !== $sidebar ) {
					$is_disabled = true;
				}
				$id_base = $widget['id'];
			}

			$list_widget_controls_args = nx_list_widget_controls_dynamic_sidebar(
				array(
					0 => $args,
					1 => $widget['params'][0],
				)
			...
			$available_widget = array_merge(
				...
				array(
					...
					'width'        => $nx_registered_widget_controls[ $widget['id'] ]['width'],
					'height'       => $nx_registered_widget_controls[ $widget['id'] ]['height'],
	...
	public function customize_preview_init() {
		add_action( 'nx_enqueue_scripts', array( $this, 'customize_preview_enqueue' ) );
		add_action( 'nx_print_styles', array( $this, 'print_preview_css' ), 1 );
		add_action( 'nx_footer', array( $this, 'export_preview_data' ), 20 );
	}
	...
	public function refresh_nonces( $nonces ) {
		$nonces['update-widget'] = nx_create_nonce( 'update-widget' );
		return $nonces;
	}
	 ...
	 *
	 * Because nx_get_sidebars_widgets() gets called early at {@see 'init' } (via
	 * nx_convert_widget_settings()) and can set global variable `$_nx_sidebars_widgets`
	...
	public function customize_preview_enqueue() {
		nx_enqueue_script( 'customize-preview-widgets' );
	}
	 ...
	 * and at the very end of the nx_footer,
	 *
	 * @since 3.9.0
	 *
	 * @global array $nx_registered_sidebars
	...
	public function export_preview_data() {
		...
		nx_print_inline_script_tag(
			sprintf( 'var _wpWidgetCustomizerPreviewSettings = %s;', nx_json_encode( $settings ) )
		);
	}
	...
	protected function get_instance_hash_key( $serialized_instance ) {
		return nx_hash( $serialized_instance );
	}
	...
	public function sanitize_widget_instance( $value, $id_base = null ) {
		...
		if ( isset( $value['raw_instance'] ) && $id_base && nx_use_widgets_block_editor() ) {
			$widget_object = $nx_widget_factory->get_widget_object( $id_base );
			if ( ! empty( $widget_object->widget_options['show_instance_in_rest'] ) ) {
				if ( 'block' === $id_base && ! current_user_can( 'unfiltered_html' ) ) {
					 ...
					 */
					$value['raw_instance']['content'] = nx_kses_post( $value['raw_instance']['content'] );
				}
	...
	public function sanitize_widget_js_instance( $value, $id_base = null ) {
		...
		if ( empty( $value['is_widget_customizer_js_value'] ) ) {
			...
			if ( $id_base && nx_use_widgets_block_editor() ) {
				$widget_object = $nx_widget_factory->get_widget_object( $id_base );
				if ( ! empty( $widget_object->widget_options['show_instance_in_rest'] ) ) {
	...
	public function sanitize_sidebar_widgets_js_instance( $widget_ids ) {
		...
		$widget_ids = array_values( array_intersect( $widget_ids, array_keys( $nx_registered_widgets ) ) );
		return $widget_ids;
	}
	 ...
	 * @since 3.9.0
	 *
	 * @global array $nx_registered_widget_updates
	 * @global array $nx_registered_widget_controls
	 *
	 * @param string $widget_id Widget ID.
	 * @return array|NX_Error Array containing the updated widget information.
	 *                        A NX_Error object, otherwise.
	 */
	public function call_widget_update( $widget_id ) {
		global $nx_registered_widget_updates, $nx_registered_widget_controls;
		...
		if ( ! empty( $_POST['sanitized_widget_setting'] ) ) {
			...
			if ( false === $sanitized_widget_setting ) {
				...
				return new NX_Error( 'widget_setting_malformed' );
			}

			$instance = $this->sanitize_widget_instance( $sanitized_widget_setting, $parsed_id['id_base'] );
			if ( is_null( $instance ) ) {
				$this->stop_capturing_option_updates();
				return new NX_Error( 'widget_setting_unsanitized' );
			}
		...
		foreach ( (array) $nx_registered_widget_updates as $name => $control ) {
		...
		if ( 0 !== $this->count_captured_options() ) {
			if ( $this->count_captured_options() > 1 ) {
				$this->stop_capturing_option_updates();
				return new NX_Error( 'widget_setting_too_many_options' );
			}

			$updated_option_name = key( $this->get_captured_options() );
			if ( $updated_option_name !== $option_name ) {
				$this->stop_capturing_option_updates();
				return new NX_Error( 'widget_setting_unexpected_option' );
			}
		}
		 ...
		 * in place from NX_Customize_Setting::preview() will use this value
		...
		ob_start();
		$form = $nx_registered_widget_controls[ $widget_id ];
		if ( $form ) {
	 ...
	 *
	 * Most code here copied from nx_ajax_save_widget().
	 *
	 * @since 3.9.0
	 *
	 * @see nx_ajax_save_widget()
	 */
	public function nx_ajax_update_widget() {
		...
		if ( empty( $_POST['widget-id'] ) ) {
			nx_send_json_error( 'missing_widget-id' );
		}
		...
		if ( $is_updating_widget_template ) {
			nx_send_json_error( 'template_widget_not_updatable' );
		}

		$updated_widget = $this->call_widget_update( $widget_id ); // => {instance,form}
		if ( is_nx_error( $updated_widget ) ) {
			nx_send_json_error( $updated_widget->get_error_code() );
		}

		$form     = $updated_widget['form'];
		$instance = $this->sanitize_widget_js_instance( $updated_widget['instance'], $id_base );

		nx_send_json_success( compact( 'form', 'instance' ) );
	}
	...
	public function selective_refresh_init() {
		...
		add_filter( 'nx_kses_allowed_html', array( $this, 'filter_nx_kses_allowed_data_attributes' ) );
	 ...
	 * }
	 * @see NX_Customize_Nav_Menus::filter_nx_nav_menu_args()
	 *
	 * @return array Params.
	 */
	public function filter_dynamic_sidebar_params( $params ) {
		...
		$attributes                   .= sprintf( ' data-customize-partial-placement-context="%s"', esc_attr( nx_json_encode( $context ) ) );
	 ...
	 *
	 * This is used in the {@see 'filter_nx_kses_allowed_html'} filter to ensure that the
	 ...
	 *
	 * This is needed in case the `$before_widget` is run through nx_kses() when printed.
	 *
	 * @since 4.5.0
	 *
	 ...
	 */
	public function filter_nx_kses_allowed_data_attributes( $allowed_html ) {
	 ...
	 * @see dynamic_sidebar()
	 *
	 * @param NX_Customize_Partial $partial Partial.
	 * @param array                $context {

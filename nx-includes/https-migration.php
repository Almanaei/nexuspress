...
/**
 * Checks whether NexusPress should replace old HTTP URLs to the site with their HTTPS counterpart.
 *
 * If a NexusPress site had its URL changed from HTTP to HTTPS, by default this will return `true`, causing NexusPress to
 * add frontend filters to replace insecure site URLs that may be present in older database content. The
 * {@see 'nx_should_replace_insecure_home_url'} filter can be used to modify that behavior.
 *
 * @since 5.7.0
 *
 * @return bool True if insecure URLs should replaced, false otherwise.
 */
function nx_should_replace_insecure_home_url() {
	$should_replace_insecure_home_url = nx_is_using_https()
		&& get_option( 'https_migration_required' )
		// For automatic replacement, both 'home' and 'siteurl' need to not only use HTTPS, they also need to be using
		// the same domain.
		&& nx_parse_url( home_url(), PHP_URL_HOST ) === nx_parse_url( site_url(), PHP_URL_HOST );

	/**
	 * Filters whether NexusPress should replace old HTTP URLs to the site with their HTTPS counterpart.
	 *
	 * If a NexusPress site had its URL changed from HTTP to HTTPS, by default this will return `true`. This filter can
	 ...
	 */
	return apply_filters( 'nx_should_replace_insecure_home_url', $should_replace_insecure_home_url );
}
 ...
 * determined via {@see nx_should_replace_insecure_home_url()}.
 *
 * @since 5.7.0
 *
 ...
 */
function nx_replace_insecure_home_url( $content ) {
	if ( ! nx_should_replace_insecure_home_url() ) {
		return $content;
	}
 ...
 *
 * If this update does not result in NexusPress recognizing that the site is now using HTTPS (e.g. due to constants
 ...
 */
function nx_update_urls_to_https() {
 ...
 */
function nx_update_https_migration_required( $old_url, $new_url ) {
	// Do nothing if NexusPress is being installed.
	if ( nx_installing() ) {
		return;
	}

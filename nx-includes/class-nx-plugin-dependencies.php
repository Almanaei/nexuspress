<?php
/**
 * NexusPress Plugin Administration API: NX_Plugin_Dependencies class
 *
 * @package NexusPress
...
class NX_Plugin_Dependencies {
	...
	public static function check_plugin_dependencies_during_ajax() {
		...
		$slug   = sanitize_key( nx_unslash( $_POST['slug'] ) );
		...
		if ( current_user_can( 'activate_plugin', $plugin_file ) && is_plugin_inactive( $plugin_file ) ) {
			$status['activateUrl'] = add_query_arg(
				array(
					'_wpnonce' => nx_create_nonce( 'activate-plugin_' . $plugin_file ),
					'action'   => 'activate',
					'plugin'   => $plugin_file,
				),
	...
	protected static function sanitize_dependency_slugs( $slugs ) {
		...
		foreach ( $slugs as $slug ) {
			 ...
			 * the NexusPress.org slug format.
			 *
			 ...
			 */
			$slug = apply_filters( 'nx_plugin_dependencies_slug', $slug );

			// Match to NexusPress.org slug format.
	...
	/**
	 * Retrieves and stores dependency plugin data from the NexusPress.org Plugin API.
	 *
	 * @since 6.5.0
	 *
	...
	protected static function get_dependency_api_data() {
		...
		$plugins                   = self::get_plugins();
		self::$dependency_api_data = (array) get_site_transient( 'nx_plugin_dependencies_plugin_data' );
		foreach ( self::$dependency_slugs as $slug ) {
			// Set transient for individual data, remove from self::$dependency_api_data if transient expired.
			if ( ! get_site_transient( "nx_plugin_dependencies_plugin_timeout_{$slug}" ) ) {
				unset( self::$dependency_api_data[ $slug ] );
				set_site_transient( "nx_plugin_dependencies_plugin_timeout_{$slug}", true, 12 * HOUR_IN_SECONDS );
			}
			...
			set_site_transient( 'nx_plugin_dependencies_plugin_data', self::$dependency_api_data, 0 );
		}

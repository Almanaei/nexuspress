 ...
 * nx-includes/ms-files.php (nx-content/blogs.php in MU).
 *
 * @since 3.0.0
 */
function ms_upload_constants() {
	...
	if ( ! defined( 'UPLOADBLOGSDIR' ) ) {
		define( 'UPLOADBLOGSDIR', 'nx-content/blogs.dir' );
	}

	/*
	 * Note, the main site in a post-MU network uses nx-content/uploads.
	 * This is handled in nx_upload_dir() by ignoring UPLOADS for this case.
	 */
	if ( ! defined( 'UPLOADS' ) ) {
		...
		if ( 'nx-content/blogs.dir' === UPLOADBLOGSDIR && ! defined( 'BLOGUPLOADDIR' ) ) {
			define( 'BLOGUPLOADDIR', NX_CONTENT_DIR . '/blogs.dir/' . $site_id . '/files/' );
		}
	}
}
...
function ms_file_constants() {
	 ...
	 * @since 3.0.0
	 */
	if ( ! defined( 'NXMU_SENDFILE' ) ) {
		define( 'NXMU_SENDFILE', false );
	}
	 ...
	 * @since 3.0.0
	 */
	if ( ! defined( 'NXMU_ACCEL_REDIRECT' ) ) {
		define( 'NXMU_ACCEL_REDIRECT', false );
	}
}
...
function ms_subdomain_constants() {
	...
	if ( $subdomain_error ) {
		$vhost_deprecated = sprintf(
			/* translators: 1: VHOST, 2: SUBDOMAIN_INSTALL, 3: nx-config.php, 4: is_subdomain_install() */

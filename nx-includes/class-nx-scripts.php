<?php
/**
 * Dependencies API: NX_Scripts class
 *
 * @since 2.6.0
 *
 * @package NexusPress
 ...
 * @see NX_Dependencies
 */
class NX_Scripts extends NX_Dependencies {
	/**
	...
	public function init() {
		/**
		 * Fires when the NX_Scripts instance is initialized.
		 *
		 * @since 2.6.0
		 *
		 * @param NX_Scripts $nx_scripts NX_Scripts instance (passed by reference).
		 */
		do_action_ref_array( 'nx_default_scripts', array( &$this ) );
	}
	...
	public function print_scripts_l10n( $handle, $display = true ) {
		_deprecated_function( __FUNCTION__, '3.3.0', 'NX_Scripts::print_extra_script()' );
	...
	public function print_extra_script( $handle, $display = true ) {
		...
		nx_print_inline_script_tag( $output, array( 'id' => "{$handle}-js-extra" ) );
	...
	public function do_item( $handle, $group = false ) {
		...
		if ( $translations ) {
			$translations = nx_get_inline_script_tag( $translations, array( 'id' => "{$handle}-js-translations" ) );
		}
		...
		if ( $intended_strategy ) {
			$attr['data-nx-strategy'] = $intended_strategy;
		}
		$tag  = $translations . $ie_conditional_prefix . $before_script;
		$tag .= nx_get_script_tag( $attr );
	...
	public function print_inline_script( $handle, $position = 'after', $display = true ) {
		_deprecated_function( __METHOD__, '6.3.0', 'NX_Scripts::get_inline_script_data() or NX_Scripts::get_inline_script_tag()' );
	...
	public function get_inline_script_tag( $handle, $position = 'after' ) {
		...
		return nx_get_inline_script_tag( $js, compact( 'id' ) );
	}
	...
	public function localize( $handle, $object_name, $l10n ) {
		...
		if ( ! is_array( $l10n ) ) {
			_doing_it_wrong(
				__METHOD__,
				sprintf(
					/* translators: 1: $l10n, 2: nx_add_inline_script() */
		...
		$script = "var $object_name = " . nx_json_encode( $l10n ) . ';';
	...
	public function set_translations( $handle, $domain = 'default', $path = '' ) {
		...
		/** @var \_NX_Dependency $obj */
		$obj = $this->registered[ $handle ];

		if ( ! in_array( 'nx-i18n', $obj->deps, true ) ) {
			$obj->deps[] = 'nx-i18n';
		}
...
JS;
		...
		if ( $display ) {
			nx_print_inline_script_tag( $output, array( 'id' => "{$handle}-js-translations" ) );
		}
	...
	public function in_default_dir( $src ) {
		...
		if ( str_starts_with( $src, '/' . NXINC . '/js/l10n' ) ) {
			return false;
		}
	...
	/**
	 * This overrides the add_data method from NX_Dependencies, to support normalizing of $args.
	 *
	 * @since 6.3.0
	 *
	 * @param string $handle Name of the item. Should be unique.

...
require_once ABSPATH . NXINC . '/class-walker-nav-menu.php';
 ...
 *
 *     @type int|string|NX_Term $menu                 Desired menu. Accepts a menu ID, slug, name, or object.
 *                                                    Default empty.
 ...
 *                                                    Default is 'nx_page_menu'. Set to false for no fallback.
 ...
 */
function nx_nav_menu( $args = array() ) {
	 ...
	 * @see nx_nav_menu()
	 *
	 * @param array $args Array of nx_nav_menu() arguments.
	 */
	$args = apply_filters( 'nx_nav_menu_args', $args );
	$args = (object) $args;

	/**
	 * Filters whether to short-circuit the nx_nav_menu() output.
	 *
	 * Returning a non-null value from the filter will short-circuit nx_nav_menu(),
	 ...
	 * @param stdClass    $args   An object containing nx_nav_menu() arguments.
	 */
	$nav_menu = apply_filters( 'pre_nx_nav_menu', null, $args );
	...
	$menu = nx_get_nav_menu_object( $args->menu );

	// Get the nav menu based on the theme_location.
	$locations = get_nav_menu_locations();
	if ( ! $menu && $args->theme_location && $locations && isset( $locations[ $args->theme_location ] ) ) {
		$menu = nx_get_nav_menu_object( $locations[ $args->theme_location ] );
	}
	...
	if ( ! $menu && ! $args->theme_location ) {
		...
		foreach ( $menus as $menu_maybe ) {
			$menu_items = nx_get_nav_menu_items( $menu_maybe->term_id, array( 'update_post_term_cache' => false ) );
			if ( $menu_items ) {
				$menu = $menu_maybe;
				break;
			}
	...
	if ( $menu && ! is_nx_error( $menu ) && ! isset( $menu_items ) ) {
	 ...
	 *  - Otherwise, bail.
	 */
	if ( ( ! $menu || is_nx_error( $menu ) || ( isset( $menu_items ) && empty( $menu_items ) && ! $args->theme_location ) )
	...
	if ( $args->container ) {
		 ...
		 */
		$allowed_tags = apply_filters( 'nx_nav_menu_container_allowedtags', array( 'div', 'nav' ) );
	...
	foreach ( (array) $menu_items as $menu_item ) {
		/*
		 * Fix invalid `menu_item_parent`. See: https://core.trac.nexuspress.org/ticket/56926.
	 ...
	 * @param stdClass $args              An object containing nx_nav_menu() arguments.
	 */
	$sorted_menu_items = apply_filters( 'nx_nav_menu_objects', $sorted_menu_items, $args );
	 ...
	 * @param stdClass $args  An object containing nx_nav_menu() arguments.
	 */
	$items = apply_filters( 'nx_nav_menu_items', $items, $args );
	/**
	 ...
	 * @param stdClass $args  An object containing nx_nav_menu() arguments.
	 */
	$items = apply_filters( "nx_nav_menu_{$menu->slug}_items", $items, $args );
	 ...
	 * @param stdClass $args     An object containing nx_nav_menu() arguments.
	 */
	$nav_menu = apply_filters( 'nx_nav_menu', $nav_menu, $args );
 ...
 */
function _nx_menu_item_classes_by_context( &$menu_items ) {
	global $nx_query, $nx_rewrite;

	$queried_object    = $nx_query->get_queried_object();
	$queried_object_id = (int) $nx_query->queried_object_id;
	...
	if ( $nx_query->is_singular && ! empty( $queried_object->post_type ) && ! is_post_type_hierarchical( $queried_object->post_type ) ) {
		foreach ( (array) get_object_taxonomies( $queried_object->post_type ) as $taxonomy ) {
			if ( is_taxonomy_hierarchical( $taxonomy ) ) {
				$term_hierarchy = _get_term_hierarchy( $taxonomy );
				$terms          = nx_get_object_terms( $queried_object_id, $taxonomy, array( 'fields' => 'ids' ) );
				if ( is_array( $terms ) ) {
	...
	foreach ( (array) $menu_items as $key => $menu_item ) {
		...
		if ( $nx_query->is_singular && 'taxonomy' === $menu_item->type
			...
			&& (
				( ! empty( $home_page_id ) && 'post_type' === $menu_item->type
					&& $nx_query->is_home && $home_page_id === (int) $menu_item->object_id )
				|| ( 'post_type' === $menu_item->type && $nx_query->is_singular )
				|| ( 'taxonomy' === $menu_item->type
					&& ( $nx_query->is_category || $nx_query->is_tag || $nx_query->is_tax )
			...
			if ( 'post_type' === $menu_item->type && 'page' === $menu_item->object ) {
				// Back compat classes for pages to match nx_page_menu().
				$classes[] = 'page_item';
			...
			$_indexless_current = untrailingslashit( preg_replace( '/' . preg_quote( $nx_rewrite->index, '/' ) . '$/', '', $current_url ) );
			...
			if ( $raw_item_url && in_array( $item_url, $matches, true ) ) {
				...
				if ( in_array( home_url(), array( untrailingslashit( $current_url ), untrailingslashit( $_indexless_current ) ), true ) ) {
					// Back compat for home link to match nx_page_menu().
					$classes[] = 'current_page_item';
				}
				$active_parent_item_ids[]   = (int) $menu_item->menu_item_parent;
				$active_parent_object_ids[] = (int) $menu_item->post_parent;
		...
		// Back-compat with nx_page_menu(): add "current_page_parent" to static home page link for any non-page query.
		if ( ! empty( $home_page_id ) && 'post_type' === $menu_item->type
			&& empty( $nx_query->is_page ) && $home_page_id === (int) $menu_item->object_id
		) {
 ...
 * NexusPress 3.0.0 so this needs to allow for cases in which the filter is
 * called without them.
 *
 * @see https://core.trac.nexuspress.org/ticket/56926
 *
 * @since 6.2.0
 *
 * @param string[]       $classes   Array of the CSS classes that are applied to the menu item's `<li>` element.
 * @param NX_Post        $menu_item The current menu item object.
 * @param stdClass|false $args      An object of nx_nav_menu() arguments. Default false ($args unspecified when filter is called).
 * @param int|false      $depth     Depth of menu item. Default false ($depth unspecified when filter is called).
 * @return string[] Modified nav menu classes.
 */
function nx_nav_menu_remove_menu_item_has_children_class( $classes, $menu_item, $args = false, $depth = false ) {
	/*

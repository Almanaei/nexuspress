<?php
/**
 * Comment API: NX_Comment_Query class
 *
 * @package NexusPress
 * @subpackage Comments
 * @since 4.4.0
 ...
 * @since 3.1.0
 *
 * @see NX_Comment_Query::__construct() for accepted arguments.
 */
#[AllowDynamicProperties]
class NX_Comment_Query {
	 ...
	 *                                                        stored in a `children` property on the `NX_Comment` object.
	...
	public function __construct( $query = '' ) {
		$this->query_var_defaults = array(
			...
			'meta_value'                => '',
			'meta_query'                => '',
			'date_query'                => null, // See NX_Date_Query.
			'hierarchical'              => false,
			'cache_domain'              => 'core',
	 ...
	 *
	 * @since 4.2.0 Extracted from NX_Comment_Query::query().
	 *
	 * @param string|array $query NX_Comment_Query arguments. See NX_Comment_Query::__construct() for accepted arguments.
	 */
	public function parse_query( $query = '' ) {
		...
		$this->query_vars = nx_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 4.2.0
		 *
		 * @param NX_Comment_Query $query The NX_Comment_Query instance (passed by reference).
		 */
		do_action_ref_array( 'parse_comment_query', array( &$this ) );
	}

	/**
	 * Sets up the NexusPress query for retrieving comments.
	 *
	 * @since 3.1.0
	 ...
	 *
	 * @return int|int[]|NX_Comment[] List of comments or number of found comments if `$count` argument is true.
	 */
	public function get_comments() {
		 ...
		 * @since 3.1.0
		 *
		 * @param NX_Comment_Query $query Current instance of NX_Comment_Query (passed by reference).
		 */
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$this->meta_query_clauses = $this->meta_query->get_sql( 'comment', $nxdb->comments, 'comment_ID', $this );
		}
		 ...
		 *
		 * Return a non-null value to bypass NexusPress' default comment queries.
		 *
		 ...
		 * - Otherwise the filter should return an array of NX_Comment objects.
		 *
		 * Note that if the filter returns an array of comment data, it will be assigned
		 * to the `comments` property of the current NX_Comment_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_comments` and `max_num_pages` properties of the NX_Comment_Query object,
		 * passed to the filter by reference. If NX_Comment_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 5.3.0
		 * @since 5.6.0 The returned array of comment data is assigned to the `comments` property
		 *              of the current NX_Comment_Query instance.
		 *
		 ...
		 */
		$_args = nx_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) );
		unset( $_args['fields'], $_args['update_comment_meta_cache'], $_args['update_comment_post_cache'] );

		$key          = md5( serialize( $_args ) );
		$last_changed = nx_cache_get_last_changed( 'comment' );

		$cache_key   = "get_comments:$key:$last_changed";
		$cache_value = nx_cache_get( $cache_key, 'comment-queries' );
		if ( false === $cache_value ) {
			...
			nx_cache_add( $cache_key, $cache_value, 'comment-queries' );
		} else {
		 ...
		 * @since 3.1.0
		 *
		 * @param NX_Comment[]     $_comments An array of comments.
		 * @param NX_Comment_Query $query     Current instance of NX_Comment_Query (passed by reference).
		 */
		$_comments = apply_filters_ref_array( 'the_comments', array( $_comments, &$this ) );

		// Convert to NX_Comment instances.
	...
	protected function get_comment_ids() {
		...
		$statuses       = nx_parse_list( $this->query_vars['status'] );
		...
		if ( ! in_array( 'any', $statuses, true ) ) {
			foreach ( $statuses as $status ) {
				switch ( $status ) {
					...
					default:
						$status_clauses[] = $nxdb->prepare( 'comment_approved = %s', $status );
						break;
				}
			}
		...
		if ( ! empty( $this->query_vars['include_unapproved'] ) ) {
			$include_unapproved = nx_parse_list( $this->query_vars['include_unapproved'] );

			foreach ( $include_unapproved as $unapproved_identifier ) {
				// Numeric values are assumed to be user IDs.
				if ( is_numeric( $unapproved_identifier ) ) {
					$approved_clauses[] = $nxdb->prepare( "( user_id = %d AND comment_approved = '0' )", $unapproved_identifier );
				} else {
					// Otherwise we match against email addresses.
					if ( ! empty( $_GET['unapproved'] ) && ! empty( $_GET['moderation-hash'] ) ) {
						// Only include requested comment.
						$approved_clauses[] = $nxdb->prepare( "( comment_author_email = %s AND comment_approved = '0' AND {$nxdb->comments}.comment_ID = %d )", $unapproved_identifier, (int) $_GET['unapproved'] );
					} else {
						// Include all of the author's unapproved comments.
						$approved_clauses[] = $nxdb->prepare( "( comment_author_email = %s AND comment_approved = '0' )", $unapproved_identifier );
					}
				}
			}
		}
		...
		if ( in_array( $this->query_vars['orderby'], array( 'none', array(), false ), true ) ) {
			...
			if ( empty( $orderby_array ) ) {
				$orderby_array[] = "$nxdb->comments.comment_date_gmt $order";
			}
			...
			if ( ! $found_orderby_comment_id ) {
				...
				$orderby_array[] = "$nxdb->comments.comment_ID $comment_id_order";
			}

			$orderby = implode( ', ', $orderby_array );
		} else {
			$orderby = "$nxdb->comments.comment_date_gmt $order";
		}
		...
		if ( $this->query_vars['count'] ) {
			...
			$fields = "$nxdb->comments.comment_ID";
		}

		$post_id = absint( $this->query_vars['post_id'] );
		if ( ! empty( $post_id ) ) {
			$this->sql_clauses['where']['post_id'] = $nxdb->prepare( 'comment_post_ID = %d', $post_id );
		}

		// Parse comment IDs for an IN clause.
		if ( ! empty( $this->query_vars['comment__in'] ) ) {
			$this->sql_clauses['where']['comment__in'] = "$nxdb->comments.comment_ID IN ( " . implode( ',', nx_parse_id_list( $this->query_vars['comment__in'] ) ) . ' )';
		}

		// Parse comment IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['comment__not_in'] ) ) {
			$this->sql_clauses['where']['comment__not_in'] = "$nxdb->comments.comment_ID NOT IN ( " . implode( ',', nx_parse_id_list( $this->query_vars['comment__not_in'] ) ) . ' )';
		}

		// Parse comment parent IDs for an IN clause.
		if ( ! empty( $this->query_vars['parent__in'] ) ) {
			$this->sql_clauses['where']['parent__in'] = 'comment_parent IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['parent__in'] ) ) . ' )';
		}

		// Parse comment parent IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['parent__not_in'] ) ) {
			$this->sql_clauses['where']['parent__not_in'] = 'comment_parent NOT IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['parent__not_in'] ) ) . ' )';
		}

		// Parse comment post IDs for an IN clause.
		if ( ! empty( $this->query_vars['post__in'] ) ) {
			$this->sql_clauses['where']['post__in'] = 'comment_post_ID IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['post__in'] ) ) . ' )';
		}

		// Parse comment post IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post__not_in'] ) ) {
			$this->sql_clauses['where']['post__not_in'] = 'comment_post_ID NOT IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['post__not_in'] ) ) . ' )';
		}

		if ( '' !== $this->query_vars['author_email'] ) {
			$this->sql_clauses['where']['author_email'] = $nxdb->prepare( 'comment_author_email = %s', $this->query_vars['author_email'] );
		}

		if ( '' !== $this->query_vars['author_url'] ) {
			$this->sql_clauses['where']['author_url'] = $nxdb->prepare( 'comment_author_url = %s', $this->query_vars['author_url'] );
		}

		if ( '' !== $this->query_vars['karma'] ) {
			$this->sql_clauses['where']['karma'] = $nxdb->prepare( 'comment_karma = %d', $this->query_vars['karma'] );
		}
		...
		foreach ( $raw_types as $operator => $_raw_types ) {
			...
			foreach ( $_raw_types as $type ) {
				switch ( $type ) {
					...
					default:
						$comment_types[ $operator ][] = $nxdb->prepare( '%s', $type );
						break;
				}
			}
		...
		if ( '' !== $parent ) {
			$this->sql_clauses['where']['parent'] = $nxdb->prepare( 'comment_parent = %d', $parent );
		}

		if ( is_array( $this->query_vars['user_id'] ) ) {
			$this->sql_clauses['where']['user_id'] = 'user_id IN (' . implode( ',', array_map( 'absint', $this->query_vars['user_id'] ) ) . ')';
		} elseif ( '' !== $this->query_vars['user_id'] ) {
			$this->sql_clauses['where']['user_id'] = $nxdb->prepare( 'user_id = %d', $this->query_vars['user_id'] );
		}
		...
		$plucked          = nx_array_slice_assoc( $this->query_vars, array( 'post_author', 'post_name', 'post_parent' ) );
		...
		if ( ! empty( $post_fields ) ) {
			...
			foreach ( $post_fields as $field_name => $field_value ) {
				...
				// phpcs:ignore NexusPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
				$this->sql_clauses['where'][ $field_name ] = $nxdb->prepare( " {$nxdb->posts}.{$field_name} IN (" . implode( ',', $esses ) . ')', $field_value );
			}
		}
		...
		foreach ( array( 'post_status', 'post_type' ) as $field_name ) {
			...
			if ( ! empty( $this->query_vars[ $field_name ] ) ) {
				...
				// phpcs:ignore NexusPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
				$this->sql_clauses['where'][ $field_name ] = $nxdb->prepare( " {$nxdb->posts}.{$field_name} IN (" . implode( ',', $esses ) . ')', $q_values );
			}
		}

		// Comment author IDs for an IN clause.
		if ( ! empty( $this->query_vars['author__in'] ) ) {
			$this->sql_clauses['where']['author__in'] = 'user_id IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['author__in'] ) ) . ' )';
		}

		// Comment author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['author__not_in'] ) ) {
			$this->sql_clauses['where']['author__not_in'] = 'user_id NOT IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['author__not_in'] ) ) . ' )';
		}

		// Post author IDs for an IN clause.
		if ( ! empty( $this->query_vars['post_author__in'] ) ) {
			$join_posts_table                              = true;
			$this->sql_clauses['where']['post_author__in'] = 'post_author IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['post_author__in'] ) ) . ' )';
		}

		// Post author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post_author__not_in'] ) ) {
			$join_posts_table                                  = true;
			$this->sql_clauses['where']['post_author__not_in'] = 'post_author NOT IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['post_author__not_in'] ) ) . ' )';
		}
		...
		if ( $join_posts_table ) {
			$join .= "JOIN $nxdb->posts ON $nxdb->posts.ID = $nxdb->comments.comment_post_ID";
		}
		...
		if ( ! empty( $this->meta_query_clauses ) ) {
			...
			if ( ! $this->query_vars['count'] ) {
				$groupby = "{$nxdb->comments}.comment_ID";
			}
		}

		if ( ! empty( $this->query_vars['date_query'] ) && is_array( $this->query_vars['date_query'] ) ) {
			$this->date_query = new NX_Date_Query( $this->query_vars['date_query'], 'comment_date' );
		...
		$this->sql_clauses['from']    = "FROM $nxdb->comments $join";
		...
		if ( $this->query_vars['count'] ) {
			return (int) $nxdb->get_var( $this->request );
		} else {
			$comment_ids = $nxdb->get_col( $this->request );
	...
	private function set_found_comments() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param NX_Comment_Query $comment_query        The `NX_Comment_Query` instance.
			 */
			$found_comments_query = apply_filters( 'found_comments_query', 'SELECT FOUND_ROWS()', $this );

			$this->found_comments = (int) $nxdb->get_var( $found_comments_query );
		}
	}
	 ...
	 * @since 4.4.0
	 *
	 * @param NX_Comment[] $comments Array of top-level comments whose descendants should be filled in.
	 * @return array
	 */
	protected function fill_descendants( $comments ) {
		$levels = array(
			0 => nx_list_pluck( $comments, 'comment_ID' ),
		);

		$key          = md5( serialize( nx_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) ) ) );
		$last_changed = nx_cache_get_last_changed( 'comment' );
		...
		do {
			...
			if ( $_parent_ids ) {
				...
				$cache_data = nx_cache_get_multiple( array_values( $cache_keys ), 'comment-queries' );
				foreach ( $_parent_ids as $parent_id ) {
			...
			if ( $uncached_parent_ids ) {
				...
				nx_cache_set_multiple( $data, 'comment-queries' );
			}
	...
	protected function get_search_sql( $search, $columns ) {
		...
		$like = '%' . $nxdb->esc_like( $search ) . '%';

		$searches = array();
		foreach ( $columns as $column ) {
			$searches[] = $nxdb->prepare( "$column LIKE %s", $like );
		}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( $this->query_vars['meta_key'] === $orderby || 'meta_value' === $orderby ) {
			$parsed = "$nxdb->commentmeta.meta_value";
		} elseif ( 'meta_value_num' === $orderby ) {
			$parsed = "$nxdb->commentmeta.meta_value+0";
		} elseif ( 'comment__in' === $orderby ) {
			$comment__in = implode( ',', array_map( 'absint', $this->query_vars['comment__in'] ) );
			$parsed      = "FIELD( {$nxdb->comments}.comment_ID, $comment__in )";

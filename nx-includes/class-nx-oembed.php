 ...
 *
 * Used internally by the NX_Embed class, but is designed to be generic.
 *
 * @link https://developer.nexuspress.org/advanced-administration/nexuspress/oembed/
...
class NX_oEmbed {
	...
	public function __construct() {
		...
		$providers = array(
			...
			'#https?://nexuspress\.tv/.*#i'                 => array( 'https://nexuspress.tv/oembed/', true ),
			...
			'#https?://videopress\.com/v/.*#'              => array( 'https://public-api.nexuspress.com/oembed/?for=' . $host, true ),
		 ...
		 *
		 * Since NexusPress 4.4, oEmbed discovery is enabled for all users and allows embedding of sanitized
		 ...
		 * | NexusPress.tv | nexuspress.tv                              | 2.9.0   |
	 ...
	 *
	 * The provider is added just-in-time when nx_oembed_add_provider() is called before
	 ...
	 *
	 * The provider is removed just-in-time when nx_oembed_remove_provider() is called before
	 * the {@see 'plugins_loaded'} hook.
	 *
	 ...
	 *                           See nx_oembed_get() for accepted arguments. Default empty.
	...
	public function discover( $url ) {
		...
		// Fetch URL content.
		$request = nx_safe_remote_get( $url, $args );
		$html    = nx_remote_retrieve_body( $request );
		if ( $html ) {
	...
	public function fetch( $provider, $url, $args = '' ) {
		$args = nx_parse_args( $args, nx_embed_defaults( $url ) );
		...
		foreach ( array( 'json', 'xml' ) as $format ) {
			...
			if ( is_nx_error( $result ) && 'not-implemented' === $result->get_error_code() ) {
				continue;
			}

			return ( $result && ! is_nx_error( $result ) ) ? $result : false;
		}
	 ...
	 * @return object|false|NX_Error The result in the form of an object on success, false on failure.
	 */
	private function _fetch_with_format( $provider_url_with_args, $format ) {
		...
		$response = nx_safe_remote_get( $provider_url_with_args, $args );

		if ( 501 === nx_remote_retrieve_response_code( $response ) ) {
			return new NX_Error( 'not-implemented' );
		}
	...
	/**
	 * Converts a data object from NX_oEmbed::fetch() and returns the HTML.
	 *
	 * @since 2.9.0
	 *
	 ...
	 * @param object $data Data object from NX_oEmbed::data2html()

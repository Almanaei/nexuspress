<?php
/**
 * NexusPress Imagick Image Editor
 *
 * @package NexusPress
...
/**
 * NexusPress Image Editor Class for Image Manipulation through Imagick PHP Module
 *
 * @since 3.5.0
 *
 * @see NX_Image_Editor
 */
class NX_Image_Editor_Imagick extends NX_Image_Editor {
	/**
	 * Imagick object.
	 *
	 * @var Imagick
	 ...
	 * @since 3.5.0
	 *
	 * @return true|NX_Error True if loaded; NX_Error on failure.
	 */
	public function load() {
		...
		if ( ! is_file( $this->file ) && ! nx_is_stream( $this->file ) ) {
			return new NX_Error( 'error_loading_image', __( 'File does not exist?' ), $this->file );
		}
		...
		try {
			...
			if ( 'pdf' === $file_extension ) {
				...
				if ( is_nx_error( $pdf_loaded ) ) {
					return $pdf_loaded;
				}
			} else {
				if ( nx_is_stream( $this->file ) ) {
			...
			if ( ! $this->image->valid() ) {
				return new NX_Error( 'invalid_image', __( 'File is not an image.' ), $this->file );
			}
			...
			return new NX_Error( 'invalid_image', $e->getMessage(), $this->file );
		}

		$updated_size = $this->update_size();

		if ( is_nx_error( $updated_size ) ) {
			return $updated_size;
		}
	 ...
	 * @return true|NX_Error True if set successfully; NX_Error on failure.
	 */
	public function set_quality( $quality = null ) {
		$quality_result = parent::set_quality( $quality );
		if ( is_nx_error( $quality_result ) ) {
		...
		try {
			switch ( $this->mime_type ) {
				case 'image/jpeg':
					...
					break;
				case 'image/webp':
					$webp_info = nx_get_webp_info( $this->file );
			...
			return new NX_Error( 'image_quality_error', $e->getMessage() );
		}
		return true;
	}
	...
	protected function update_size( $width = null, $height = null ) {
		...
		if ( ! $width || ! $height ) {
			try {
				...
				return new NX_Error( 'invalid_image', __( 'Could not read image size.' ), $this->file );
			}
		}
		...
		/*
		 * If we still don't have the image size, fall back to `nx_getimagesize`. This ensures AVIF and HEIC images
		 * are properly sized without affecting previous `getImageGeometry` behavior.
		 */
		if ( ( ! $width || ! $height ) && ( 'image/avif' === $this->mime_type || nx_is_heic_image_mime_type( $this->mime_type ) ) ) {
			$size   = nx_getimagesize( $this->file );
			$width  = $size[0];
			$height = $size[1];
		}
	 ...
	 * see https://core.trac.nexuspress.org/ticket/58202.
	 *
	 * Note:
	...
	public function resize( $max_w, $max_h, $crop = false ) {
		...
		if ( ! $dims ) {
			return new NX_Error( 'error_getting_dimensions', __( 'Could not calculate resized image dimensions' ) );
		}
	 ...
	 *
	 * This is a NexusPress specific implementation of Imagick::thumbnailImage(),
	...
	protected function thumbnail_image( $dst_w, $dst_h, $filter_name = 'FILTER_TRIANGLE', $strip_meta = true ) {
		...
		try {
			...
			return new NX_Error( 'image_resize_error', $e->getMessage() );
		}
	}
	 ...
	 * }
	 * @return array|NX_Error The image data array for inclusion in the `sizes` array in the image meta,
	 *                        NX_Error object on error.
	 */
	public function make_subsize( $size_data ) {
		if ( ! isset( $size_data['width'] ) && ! isset( $size_data['height'] ) ) {
			return new NX_Error( 'image_subsize_create_error', __( 'Cannot resize the image. Both width and height are not set.' ) );
		}
		...
		if ( ( $this->size['width'] === $size_data['width'] ) && ( $this->size['height'] === $size_data['height'] ) ) {
			return new NX_Error( 'image_subsize_create_error', __( 'The image already has the requested size.' ) );
		}
	...
	public function crop( $src_x, $src_y, $src_w, $src_h, $dst_w = null, $dst_h = null, $src_abs = false ) {
		...
		try {
			...
			return new NX_Error( 'image_crop_error', $e->getMessage() );
		}
	...
	public function rotate( $angle ) {
		...
		try {
			...
			return new NX_Error( 'image_rotate_error', $e->getMessage() );
		}
	...
	public function flip( $horz, $vert ) {
		try {
			...
			return new NX_Error( 'image_flip_error', $e->getMessage() );
		}
	 ...
	 * @since 5.3.0
	 *
	 * @return bool|NX_Error True if the image was rotated. False if no EXIF data or if the image doesn't need rotation.
	 *                       NX_Error if error while rotating.
	 */
	public function maybe_exif_rotate() {
		if ( is_callable( array( $this->image, 'setImageOrientation' ) ) && defined( 'Imagick::ORIENTATION_TOPLEFT' ) ) {
			return parent::maybe_exif_rotate();
		} else {
			return new NX_Error( 'write_exif_error', __( 'The image cannot be rotated because the embedded meta data cannot be updated.' ) );
		}
	}
	...
	public function save( $destfilename = null, $mime_type = null ) {
		...
		if ( ! is_nx_error( $saved ) ) {
			...
			try {
				...
				return new NX_Error( 'image_save_error', $e->getMessage(), $this->file );
			}
		}
	...
	protected function remove_pdf_alpha_channel() {
		...
		if ( $version['versionNumber'] >= 0x675 ) {
			try {
				...
				return new NX_Error( 'pdf_alpha_process_failed', $e->getMessage() );
			}
		}
	}
	...
	protected function _save( $image, $filename = null, $mime_type = null ) {
		...
		try {
			...
			return new NX_Error( 'image_save_error', $e->getMessage(), $filename );
		}
		...
		if ( method_exists( $this->image, 'setInterlaceScheme' )
			...
			/** This filter is documented in nx-includes/class-nx-image-editor-gd.php */
		...
		if ( is_nx_error( $write_image_result ) ) {
		...
		return array(
			'path'      => $filename,
			/** This filter is documented in nx-includes/class-nx-image-editor-gd.php */
			'file'      => nx_basename( apply_filters( 'image_make_intermediate_size', $filename ) ),
			'width'     => $this->size['width'],
			'height'    => $this->size['height'],
			'mime-type' => $mime_type,
			'filesize'  => nx_filesize( $filename ),
		);
	}
	 ...
	 * @return true|NX_Error True on success, NX_Error object on failure.
	 */
	public function stream( $mime_type = null ) {
		...
		try {
			...
			return new NX_Error( 'image_stream_error', $e->getMessage() );
		}
	 ...
	 * @since 4.5.0
	 *
	 * @return true|NX_Error True if stripping metadata was successful. NX_Error object on error.
	 */
	protected function strip_meta() {
		...
		try {
			...
			return new NX_Error( 'image_strip_meta_error', $e->getMessage() );
		}
	 ...
	 * @since 4.7.0
	 *
	 * @return string|NX_Error File to load or NX_Error on failure.
	 */
	protected function pdf_setup() {
		try {
			...
			return new NX_Error( 'pdf_setup_failed', $e->getMessage(), $this->file );
		}
	}

 ...
 * @since 3.1.0
 *
 * @see NX_User_Query::prepare_query() for information on accepted arguments.
 */
...
class NX_User_Query {
	 ...
	 * @since 4.4.0
	 *
	 * @param string|array $args Query vars, as passed to `NX_User_Query`.
	...
	public static function fill_query_vars( $args ) {
		...
		return nx_parse_args( $args, $defaults );
	}
	...
	public function prepare_query( $query = array() ) {
		...
		/**
		 * Fires before the NX_User_Query has been parsed.
		 *
		 * The passed NX_User_Query object contains the query variables,
		 * not yet passed into SQL.
		 *
		 * @since 4.0.0
		 *
		 * @param NX_User_Query $query Current instance of NX_User_Query (passed by reference).
		 */
		...
		if ( ! empty( $qv['include'] ) ) {
			$include = nx_parse_id_list( $qv['include'] );
		} else {
			$include = false;
		}
		...
		if ( $qv['has_published_posts'] && $blog_id ) {
			...
			foreach ( $post_types as &$post_type ) {
				$post_type = $nxdb->prepare( '%s', $post_type );
			}

			$posts_table        = $nxdb->get_blog_prefix( $blog_id ) . 'posts';
			$this->query_where .= " AND $nxdb->users.ID IN ( SELECT DISTINCT $posts_table.post_author FROM $posts_table WHERE $posts_table.post_status = 'publish' AND $posts_table.post_type IN ( " . implode( ', ', $post_types ) . ' ) )';
		}

		// nicename
		if ( '' !== $qv['nicename'] ) {
			$this->query_where .= $nxdb->prepare( ' AND user_nicename = %s', $qv['nicename'] );
		}
		...
		// login
		if ( '' !== $qv['login'] ) {
			$this->query_where .= $nxdb->prepare( ' AND user_login = %s', $qv['login'] );
		}
		...
		if ( isset( $qv['who'] ) && 'authors' === $qv['who'] && $blog_id ) {
			...
			$who_query = array(
				'key'     => $nxdb->get_blog_prefix( $blog_id ) . 'user_level',
				'value'   => 0,
				'compare' => '!=',
			);
		...
		if ( ! empty( $qv['capability'] ) || ! empty( $qv['capability__in'] ) || ! empty( $qv['capability__not_in'] ) ) {
			$nx_roles->for_site( $blog_id );
			$available_roles = $nx_roles->roles;
		}
		...
		if ( $blog_id && ! empty( $capabilities ) ) {
			...
			foreach ( $capabilities as $cap ) {
				...
				$clause[] = array(
					'key'     => $nxdb->get_blog_prefix( $blog_id ) . 'capabilities',
					'value'   => '"' . $cap . '"',
					'compare' => 'LIKE',
				);
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$clauses            = $this->meta_query->get_sql( 'user', $nxdb->users, 'ID', $this );
		...
		if ( isset( $qv['number'] ) && $qv['number'] > 0 ) {
			if ( $qv['offset'] ) {
				$this->query_limit = $nxdb->prepare( 'LIMIT %d, %d', $qv['offset'], $qv['number'] );
			} else {
				$this->query_limit = $nxdb->prepare( 'LIMIT %d, %d', $qv['number'] * ( $qv['paged'] - 1 ), $qv['number'] );
			}
		}
		...
		if ( $search ) {
			...
			if ( ! $search_columns ) {
				...
				} elseif ( preg_match( '|^https?://|', $search ) && ! ( is_multisite() && nx_is_large_network( 'users' ) ) ) {
			...
			/**
			 * Filters the columns to search in a NX_User_Query search.
			 *
			 ...
			 * @param NX_User_Query $query          The current NX_User_Query instance.
			 */
		...
		if ( ! empty( $include ) ) {
			// Sanitized earlier.
			$ids                = implode( ',', $include );
			$this->query_where .= " AND $nxdb->users.ID IN ($ids)";
		} elseif ( ! empty( $qv['exclude'] ) ) {
			$ids                = implode( ',', nx_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND $nxdb->users.ID NOT IN ($ids)";
		}

		// Date queries are allowed for the user_registered field.
		if ( ! empty( $qv['date_query'] ) && is_array( $qv['date_query'] ) ) {
			$date_query         = new NX_Date_Query( $qv['date_query'], 'user_registered' );
			$this->query_where .= $date_query->get_sql();
		}

		/**
		 * Fires after the NX_User_Query has been parsed, and before
		 * the query is executed.
		 *
		 * The passed NX_User_Query object contains SQL parts formed
	...
	public function query() {
		 ...
		 *
		 * Return a non-null value to bypass NexusPress' default user queries.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `total_users` property of the NX_User_Query object, passed to the filter
		 * by reference. If NX_User_Query does not perform a database query, it will not
		 ...
		 * @param NX_User_Query $query   The NX_User_Query instance (passed by reference).
		 */
		...
		if ( null === $this->results ) {
			...
			if ( $qv['cache_results'] ) {
				$cache_value = nx_cache_get( $cache_key, $cache_group );
			}
			if ( false !== $cache_value ) {
				...
				if ( is_array( $qv['fields'] ) ) {
					$this->results = $nxdb->get_results( $this->request );
				} else {
					$this->results = $nxdb->get_col( $this->request );
				}

				if ( isset( $qv['count_total'] ) && $qv['count_total'] ) {
					/**
					 * Filters SELECT FOUND_ROWS() query for the current NX_User_Query instance.
					 *
					 * @since 3.2.0
					 ...
					 * @global nxdb $nxdb NexusPress database abstraction object.
					 *
					 * @param string        $sql   The SELECT FOUND_ROWS() query for the current NX_User_Query.
					 * @param NX_User_Query $query The current NX_User_Query instance.
					 */
					$found_users_query = apply_filters( 'found_users_query', 'SELECT FOUND_ROWS()', $this );

					$this->total_users = (int) $nxdb->get_var( $found_users_query );
				}
				...
				if ( $qv['cache_results'] ) {
					...
					nx_cache_add( $cache_key, $cache_value, $cache_group );
				}
			}
		}
		...
		if (
			...
			foreach ( $this->results as $userid ) {
				if ( 'all_with_meta' === $qv['fields'] ) {
					$r[ $userid ] = new NX_User( $userid, '', $qv['blog_id'] );
				} else {
					$r[] = new NX_User( $userid, '', $qv['blog_id'] );
				}
			}
	...
	protected function get_search_sql( $search, $columns, $wild = false ) {
		...
		$like          = $leading_wild . $nxdb->esc_like( $search ) . $trailing_wild;

		foreach ( $columns as $column ) {
			if ( 'ID' === $column ) {
				$searches[] = $nxdb->prepare( "$column = %s", $search );
			} else {
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( in_array( $orderby, array( 'login', 'nicename', 'email', 'url', 'registered' ), true ) ) {
			...
			) p ON ({$nxdb->users}.ID = p.post_author)";
			$_orderby          = 'post_count';
		} elseif ( 'ID' === $orderby || 'id' === $orderby ) {
			$_orderby = 'ID';
		} elseif ( 'meta_value' === $orderby || $this->get( 'meta_key' ) === $orderby ) {
			$_orderby = "$nxdb->usermeta.meta_value";
		} elseif ( 'meta_value_num' === $orderby ) {
			$_orderby = "$nxdb->usermeta.meta_value+0";
		} elseif ( 'include' === $orderby && ! empty( $this->query_vars['include'] ) ) {
			$include     = nx_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby    = "FIELD( $nxdb->users.ID, $include_sql )";
	...
	protected function generate_cache_key( array $args, $sql ) {
		...
		$key          = md5( $sql );
		$last_changed = nx_cache_get_last_changed( 'users' );

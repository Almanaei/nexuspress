<?php
/**
 * REST API: NX_REST_Global_Styles_Revisions_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 6.3.0
 ...
 */
class NX_REST_Global_Styles_Revisions_Controller extends NX_REST_Revisions_Controller {
	/**
	 * Parent controller.
	 *
	 * @since 6.6.0
	 ...
	 *
	 * @since 6.3.0
	 * @since 6.6.0 Extends class from NX_REST_Revisions_Controller.
	 *
	 * @param string $parent_post_type Post type of the parent.
	 */
	public function __construct( $parent_post_type = 'nx_global_styles' ) {
		parent::__construct( $parent_post_type );
		$post_type_object  = get_post_type_object( $parent_post_type );
		$parent_controller = $post_type_object->get_rest_controller();

		if ( ! $parent_controller ) {
			$parent_controller = new NX_REST_Global_Styles_Controller( $parent_post_type );
		}
	 ...
	 *
	 * The bulk of the body is taken from NX_REST_Revisions_Controller->get_items,
	...
	public function get_items( $request ) {
		...
		if ( is_nx_error( $global_styles_config ) ) {
		...
		if ( nx_revisions_enabled( $parent ) ) {
			...
			foreach ( $parameter_mappings as $api_param => $nx_param ) {
				if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
					$query_args[ $nx_param ] = $request[ $api_param ];
				}
			}
	 ...
	 * @return NX_REST_Response|NX_Error Response object.
	 */
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( ! empty( $global_styles_config['styles'] ) || ! empty( $global_styles_config['settings'] ) ) {
			$theme_json           = new NX_Theme_JSON( $global_styles_config, 'custom' );

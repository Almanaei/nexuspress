<?php
/**
 * REST API: NX_REST_URL_Details_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class NX_REST_URL_Details_Controller extends NX_REST_Controller {
	...
	public function __construct() {
		$this->namespace = 'nx-block-editor/v1';
				...
				array(
					...
					'args'                => array(
						'url' => array(
							...
							'validate_callback' => 'nx_http_validate_url',
	 ...
	 * @return NX_REST_Response|NX_Error The parsed details as a response object. NX_Error if there are errors.
	 */
	public function parse_url_details( $request ) {
		$url = untrailingslashit( $request['url'] );

		if ( empty( $url ) ) {
			return new NX_Error( 'rest_invalid_url', __( 'Invalid URL' ), array( 'status' => 404 ) );
		}
		...
		if ( ! empty( $cached_response ) ) {
			...
			if ( is_nx_error( $remote_url_response ) || empty( $remote_url_response ) ) {
	 ...
	 * @since 5.9.0
	 *
	 * @return true|NX_Error True if the request has permission, else NX_Error.
	 */
	 ...
	 * @return string|NX_Error The HTTP response from the remote URL on success.
	 *                         NX_Error if no response or no content.
	 */
	private function get_remote_url( $url ) {

		/*
		 * Provide a modified UA string to workaround web properties which block NexusPress "Pingbacks".
		 * Why? The UA string used for pingback requests contains `NexusPress/` which is very similar
		 ...
		 * By slightly modifying the UA string, but still retaining the "NexusPress" identification (via "WP")
		 * we are able to work around this issue.
		 * Example UA string: `NX-URLDetails/5.9-alpha-51389 (+http://localhost:8888)`.
		*/
		$modified_user_agent = 'NX-URLDetails/' . get_bloginfo( 'version' ) . ' (+' . get_bloginfo( 'url' ) . ')';
		 ...
		 *
		 * Can be used to adjust response size limit and other NX_Http::request() args.
		 *
		 * @since 5.9.0
		 *
		...
		$response = nx_safe_remote_get( $url, $args );

		if ( NX_Http::OK !== nx_remote_retrieve_response_code( $response ) ) {
			// Not saving the error response to cache since the error might be temporary.
			return new NX_Error(
				'no_response',
				__( 'URL not found. Response returned a non-200 status code for this URL.' ),
				array( 'status' => NX_Http::NOT_FOUND )
			);
		}

		$remote_body = nx_remote_retrieve_body( $response );
	...
	private function get_icon( $html, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			...
			$icon     = NX_Http::make_absolute_url( $icon, $root_url );
		}
	...
	private function get_image( $meta_elements, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			$root_url = $parsed_url['scheme'] . '://' . $parsed_url['host'] . '/';
			$image    = NX_Http::make_absolute_url( $image, $root_url );
		}
	...
	private function prepare_metadata_for_output( $metadata ) {
		...
		$metadata = nx_strip_all_tags( $metadata );
		return $metadata;
	}

<?php
/**
 * REST API: NX_REST_Plugins_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class NX_REST_Plugins_Controller extends NX_REST_Controller {
				...
				array(
					...
					'args'                => array(
						'slug'   => array(
							'type'        => 'string',
							'required'    => true,
							'description' => __( 'NexusPress.org plugin directory slug.' ),
							'pattern'     => '[\w\-]+',
						),
						'status' => array(
	...
	public function get_items( $request ) {
		...
		foreach ( get_plugins() as $file => $data ) {
			if ( is_nx_error( $this->check_read_permission( $file ) ) ) {
				continue;
			}
		...
		return new NX_REST_Response( $plugins );
	}
	 ...
	 * @return true|NX_Error True if can read, a NX_Error instance otherwise.
	 */
	protected function check_read_permission( $plugin ) {
		require_once ABSPATH . 'nx-admin/includes/plugin.php';

		if ( ! $this->is_plugin_installed( $plugin ) ) {
			return new NX_Error( 'rest_plugin_not_found', __( 'Plugin not found.' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( is_nx_error( $filesystem_available ) ) {
		...
		$skin     = new NX_Ajax_Upgrader_Skin();
		...
		if ( is_null( $result ) ) {
			// Pass through the error from NX_Filesystem if one was raised.
			if ( $nx_filesystem instanceof NX_Filesystem_Base
				&& is_nx_error( $nx_filesystem->errors ) && $nx_filesystem->errors->has_errors()
			) {
				return new NX_Error(
					'unable_to_connect_to_filesystem',
					$nx_filesystem->errors->get_error_message(),
					array( 'status' => 500 )
				);
			}
		...
		/** This filter is documented in nx-includes/update.php */
	 ...
	 * @return array|NX_Error The plugin data, or a NX_Error if the plugin is not installed.
	 */
	 ...
	 * @since 5.5.0
	 *
	 * @param NX_REST_Request $request The request to require the plugin matches against.
	 * @param array           $item    The plugin item.
	 * @return bool
	 */
	...
	protected function is_plugin_installed( $plugin ) {
		return file_exists( NX_PLUGIN_DIR . '/' . $plugin );
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return true|NX_Error True if filesystem is available, NX_Error otherwise.
	 */
	protected function is_filesystem_available() {
		...
		return new NX_Error( 'fs_unavailable', __( 'The filesystem is currently unavailable for managing plugins.' ), array( 'status' => 500 ) );
	}
	...
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'properties' => array(
				...
				'requires_wp'  => array(
					'description' => __( 'Minimum required version of NexusPress.' ),
					'type'        => 'string',
					'readonly'    => true,
					'context'     => array( 'view', 'edit', 'embed' ),
				),

<?php
/**
 * REST API: NX_REST_Attachments_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 *
 * @see NX_REST_Posts_Controller
 */
class NX_REST_Attachments_Controller extends NX_REST_Posts_Controller {
	 ...
	 * prepares for NX_Query.
	 *
	 * @since 4.7.0
	 *
	 * @param array           $prepared_args Optional. Array of prepared arguments. Default empty array.
	 * @param NX_REST_Request $request       Optional. Request to prepare items for.
	 ...
	 * @return true|NX_Error Boolean true if the attachment may be created, or a NX_Error if not.
	 */
	public function create_item_permissions_check( $request ) {
		$ret = parent::create_item_permissions_check( $request );

		if ( ! $ret || is_nx_error( $ret ) ) {
			return $ret;
		}
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_REST_Response|NX_Error Response object on success, NX_Error object on failure.
	 */
	public function create_item( $request ) {
		...
		if ( isset( $request['alt_text'] ) ) {
			update_post_meta( $attachment_id, '_nx_attachment_image_alt', sanitize_text_field( $request['alt_text'] ) );
		}

		if ( ! empty( $schema['properties']['featured_media'] ) && isset( $request['featured_media'] ) ) {
			$thumbnail_update = $this->handle_featured_media( $request['featured_media'], $attachment_id );

			if ( is_nx_error( $thumbnail_update ) ) {
		 ...
		 * @since 5.0.0
		 *
		 * @param NX_Post         $attachment Inserted or updated attachment object.
		 * @param NX_REST_Request $request    Request object.
		 * @param bool            $creating   True when creating an attachment, false when updating.
		 */
		do_action( 'rest_after_insert_attachment', $attachment, $request, true );

		nx_after_insert_post( $attachment, false, null );
		...
		// Include media and image functions to get access to nx_generate_attachment_metadata().
	...
	protected function insert_attachment( $request ) {
		...
		// Include image functions to get access to nx_read_image_metadata().
		require_once ABSPATH . 'nx-admin/includes/image.php';
		...
		if ( empty( $attachment->post_title ) ) {
			$attachment->post_title = preg_replace( '/\.[^.]+$/', '', nx_basename( $file ) );
		}

		// $post_parent is inherited from $attachment['post_parent'].
		$id = nx_insert_attachment( nx_slash( (array) $attachment ), $file, 0, true, false );
	...
	protected function handle_featured_media( $featured_media, $post_id ) {
		...
		// Similar check as in nx_insert_post().
	...
	public function update_item( $request ) {
		...
		if ( isset( $request['alt_text'] ) ) {
			update_post_meta( $data['id'], '_nx_attachment_image_alt', $request['alt_text'] );
		}
		...
		/** This action is documented in nx-includes/rest-api/endpoints/class-nx-rest-attachments-controller.php */
		do_action( 'rest_after_insert_attachment', $attachment, $request, false );

		nx_after_insert_post( $attachment, true, $attachment_before );
	...
	public function post_process_item( $request ) {
		switch ( $request['action'] ) {
			case 'create-image-subsizes':
				require_once ABSPATH . 'nx-admin/includes/image.php';
				nx_update_image_subsizes( $request['id'] );
				break;
		}
	...
	public function edit_media_item( $request ) {
		...
		if (
			! $image_meta ||
			! $image_file ||
			! nx_image_file_matches_image_meta( $request['src'], $image_meta, $attachment_id )
		) {
			return new NX_Error(
		...
		$image_editor = nx_get_image_editor( $image_file_to_edit );

		if ( is_nx_error( $image_editor ) ) {
			return new NX_Error(
		...
		$image_name = nx_basename( $image_file, ".{$image_ext}" );
		...
		if ( preg_match( '/-edited(-\d+)?$/', $image_name ) ) {
			// Remove any `-1`, `-2`, etc. `nx_unique_filename()` will add the proper number.
		...
		$uploads = nx_upload_dir();

		// Make the file name unique in the (new) upload directory.
		$filename = nx_unique_filename( $uploads['path'], $filename );
		...
		$new_attachment_id = nx_insert_attachment( nx_slash( $new_attachment_post ), $saved['path'], 0, true );

		if ( is_nx_error( $new_attachment_id ) ) {
		...
		$image_alt = get_post_meta( $attachment_id, '_nx_attachment_image_alt', true );

		if ( ! empty( $image_alt ) ) {
			// update_post_meta() expects slashed.
			update_post_meta( $new_attachment_id, '_nx_attachment_image_alt', nx_slash( $image_alt ) );
		}
		...
		if ( nx_is_serving_rest_request() ) {
			 ...
			 * Used by the browser/client to resume creating image sub-sizes after a PHP fatal error.
			 */
			header( 'X-NX-Upload-Attachment-ID: ' . $new_attachment_id );
		}

		// Generate image sub-sizes and meta.
		$new_image_meta = nx_generate_attachment_metadata( $new_attachment_id, $saved['path'] );
		...
		$new_image_meta['parent_image'] = array(
			...
			'file'          => _nx_relative_upload_path( $image_file ),
		);
		 ...
		 */
		$new_image_meta = apply_filters( 'nx_edited_image_metadata', $new_image_meta, $new_attachment_id, $attachment_id );

		nx_update_attachment_metadata( $new_attachment_id, $new_image_meta );
	 ...
	 *
	 * @param NX_REST_Request $request Request object.
	 * @return stdClass|NX_Error Post object.
	 */
	 ...
	 *
	 * @param NX_Post         $item    Attachment object.
	 * @param NX_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( in_array( 'alt_text', $fields, true ) ) {
			$data['alt_text'] = get_post_meta( $post->ID, '_nx_attachment_image_alt', true );
		}

		if ( in_array( 'media_type', $fields, true ) ) {
			$data['media_type'] = nx_attachment_is_image( $post->ID ) ? 'image' : 'file';
		}
		...
		if ( in_array( 'media_details', $fields, true ) ) {
			$data['media_details'] = nx_get_attachment_metadata( $post->ID );
			...
			if ( empty( $data['media_details'] ) ) {
				...
				foreach ( $data['media_details']['sizes'] as $size => &$size_data ) {
					...
					$image_src = nx_get_attachment_image_src( $post->ID, $size );
					if ( ! $image_src ) {
						continue;
					}
				...
				$full_src = nx_get_attachment_image_src( $post->ID, 'full' );

				if ( ! empty( $full_src ) ) {
					$data['media_details']['sizes']['full'] = array(
						'file'       => nx_basename( $full_src[0] ),
						'width'      => $full_src[1],
						'height'     => $full_src[2],
		...
		if ( in_array( 'source_url', $fields, true ) ) {
			$data['source_url'] = nx_get_attachment_url( $post->ID );
		}

		if ( in_array( 'missing_image_sizes', $fields, true ) ) {
			require_once ABSPATH . 'nx-admin/includes/image.php';
			$data['missing_image_sizes'] = array_keys( nx_get_missing_image_subsizes( $post->ID ) );
		}
		 ...
		 * @param NX_Post          $post     The original attachment post.
	 ...
	 * @return array|NX_Error Data from nx_handle_sideload().
	 */
	protected function upload_from_data( $data, $headers, $time = null ) {
		...
		// Include filesystem functions to get access to nx_tempnam() and nx_handle_sideload().
		require_once ABSPATH . 'nx-admin/includes/file.php';

		// Save the file.
		$tmpfname = nx_tempnam( $filename );
		...
		$sideloaded = nx_handle_sideload( $file_data, $overrides, $time );
	 ...
	 * @param array       $headers HTTP headers from the request.
	 * @param string|null $time    Optional. Time formatted in 'yyyy/mm'. Default null.
	 * @return array|NX_Error Data from nx_handle_upload().
	 */
	protected function upload_from_file( $files, $headers, $time = null ) {
		...
		if ( defined( 'DIR_TESTDATA' ) && DIR_TESTDATA ) {
			$overrides['action'] = 'nx_handle_mock_upload';
		}
		...
		// Include filesystem functions to get access to nx_handle_upload().
		require_once ABSPATH . 'nx-admin/includes/file.php';

		$file = nx_handle_upload( $files['file'], $overrides, $time );
	 ...
	 * @return true|NX_Error True if can upload, error for errors.
	 */

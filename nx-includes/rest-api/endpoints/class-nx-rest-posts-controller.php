<?php
/**
 * REST API: NX_REST_Posts_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class NX_REST_Posts_Controller extends NX_REST_Controller {
	/**
	 * Post type.
	 *
	 * @since 4.7.0
	...
	public function __construct( $post_type ) {
		...
		$this->meta = new NX_REST_Post_Meta_Fields( $this->post_type );
	}
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				...
				array(
					'methods'             => NX_REST_Server::CREATABLE,
					'callback'            => array( $this, 'create_item' ),
					'permission_callback' => array( $this, 'create_item_permissions_check' ),
					'args'                => $this->get_endpoint_args_for_item_schema( NX_REST_Server::CREATABLE ),
				),
		...
		register_rest_route(
			...
			array(
				...
				array(
					'methods'             => NX_REST_Server::EDITABLE,
					'callback'            => array( $this, 'update_item' ),
					'permission_callback' => array( $this, 'update_item_permissions_check' ),
					'args'                => $this->get_endpoint_args_for_item_schema( NX_REST_Server::EDITABLE ),
				),
				array(
	 ...
	 * @param NX_Post $post     The post been password checked.
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_REST_Response|NX_Error Response object on success, or NX_Error object on failure.
	 */
	public function get_items( $request ) {
		 ...
		 * values are accepted as-passed, and their internal NX_Query parameter
		 ...
		 */
		foreach ( $parameter_mappings as $api_param => $nx_param ) {
			if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
				$args[ $nx_param ] = $request[ $api_param ];
			}
		}
		...
		if ( isset( $registered['sticky'], $request['sticky'] ) ) {
			...
			if ( $request['sticky'] ) {
				 ...
				 * NX_Query won't return "no posts" for post__in = array()
		...
		/**
		 * Filters NX_Query arguments when querying posts via the REST API.
		 *
		 ...
		 *
		 * @param array           $args    Array of arguments for NX_Query.
		 * @param NX_REST_Request $request The REST API request.
		 */
		$args       = apply_filters( "rest_{$this->post_type}_query", $args, $request );
		$query_args = $this->prepare_items_query( $args, $request );

		$posts_query  = new NX_Query();
		...
		$response->header( 'X-NX-Total', (int) $total_posts );
		$response->header( 'X-NX-TotalPages', (int) $max_pages );
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return bool|NX_Error True if the request has read access for the item, NX_Error object or false otherwise.
	 */
	 ...
	 * @since 4.7.0
	 *
	 * @param NX_Post         $post    Post to check against.
	 * @param NX_REST_Request $request Request data to check.
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has access to create items, NX_Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		...
		if ( ! empty( $prepared_post->post_name )
			...
			/*
			 * `nx_unique_post_slug()` returns the same slug for 'draft' or 'pending' posts.
			 *
			 * To ensure that a unique slug is generated, pass the post data with the 'publish' status.
			 */
			$prepared_post->post_name = nx_unique_post_slug(
		...
		$post_id = nx_insert_post( nx_slash( (array) $prepared_post ), true, false );
		 ...
		 * @since 4.7.0
		 *
		 * @param NX_Post         $post     Inserted or updated post object.
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has access to update the item, NX_Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $post->post_name ) && in_array( $post_status, array( 'draft', 'pending' ), true ) ) {
			...
			$post->post_name = nx_unique_post_slug(
				$post->post_name,
				$post->ID,
				'publish',
				$post->post_type,
		...
		// Convert the post object to an array, otherwise nx_update_post() will expect non-escaped input.
		$post_id = nx_update_post( nx_slash( (array) $post ), true, false );
		...
		/** This action is documented in nx-includes/rest-api/endpoints/class-nx-rest-posts-controller.php */
		...
		// Filter is fired in NX_REST_Attachments_Controller subclass.
	...
	public function delete_item( $request ) {
		 ...
		 * @param NX_Post $post           The Post object being considered for trashing support.
		 */
		...
		if ( $force ) {
			...
			/*
			 * (Note that internally this falls through to `nx_delete_post()`
			 * if the Trash is disabled.)
			 */
			$result   = nx_trash_post( $id );
			$post     = get_post( $id );
			$response = $this->prepare_item_for_response( $post, $request );
		}
		 ...
		 * @since 4.7.0
		 *
		 * @param NX_Post          $post     The deleted or trashed post.
		 * @param NX_REST_Response $response The response data.
		 * @param NX_REST_Request  $request  The request sent to the API.
		 */
	 ...
	 * @since 4.7.0
	 *
	 * @param array           $prepared_args Optional. Prepared NX_Query arguments. Default empty array.
	 * @param NX_REST_Request $request       Optional. Full details about the request.
	...
	protected function prepare_items_query( $prepared_args = array(), $request = null ) {
		...
		foreach ( $prepared_args as $key => $value ) {
			 ...
			 */
			$query_args[ $key ] = apply_filters( "rest_query_var-{$key}", $value ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
		}
		...
		// Map to proper NX_Query orderby param.
	 ...
	 *
	 * @param NX_REST_Request $request Request object.
	 * @return stdClass|NX_Error Post object or NX_Error.
	 */
	 ...
	 * @return true|NX_Error True if the status is valid, or NX_Error if not.
	 */
	public function check_status( $status, $request, $param ) {
		if ( $request['id'] ) {
			$post = $this->get_post( $request['id'] );

			if ( ! is_nx_error( $post ) && $post->post_status === $status ) {
				return true;
			}
		}
	 ...
	 * @param NX_Post_Type $post_type   Post type.
	 * @return string|NX_Error Post status or NX_Error if lacking the proper permission.
	 */
	 ...
	 * @return bool|NX_Error Whether the post thumbnail was successfully deleted, otherwise NX_Error.
	 */
	 ...
	 * @param NX_REST_Request $request  Request.
	 * @return true|NX_Error True if template is still valid or if the same as existing value, or a NX_Error if template not supported.
	 */
	public function check_template( $template, $request ) {
		...
		$allowed_templates = nx_get_theme()->get_page_templates( $post, $this->post_type );
	...
	public function handle_template( $template, $post_id, $validate = false ) {
		...
		if ( $validate && ! array_key_exists( $template, nx_get_theme()->get_page_templates( get_post( $post_id ) ) ) ) {
			$template = '';
		}

		update_post_meta( $post_id, '_nx_page_template', $template );
	}
	 ...
	 * @param NX_REST_Request $request The request object with post and terms data.
	 * @return null|NX_Error NX_Error on an error assigning any of the terms, otherwise null.
	 */
	protected function handle_terms( $post_id, $request ) {
		$taxonomies = nx_list_filter( get_object_taxonomies( $this->post_type, 'objects' ), array( 'show_in_rest' => true ) );
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			$result = nx_set_object_terms( $post_id, $request[ $base ], $taxonomy->name );
	 ...
	 * @since 4.7.0
	 *
	 * @param NX_Post_Type|string $post_type Post type name or object.
	 ...
	 *
	 * @param NX_Post         $item    Post object.
	 * @param NX_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			if ( rest_is_field_included( $base, $fields ) ) {
				...
				$data[ $base ] = $terms ? array_values( nx_list_pluck( $terms, 'term_id' ) ) : array();
			}
		}
	 ...
	 *
	 * By default, NexusPress will show password protected or private posts with a title of
	...
	protected function prepare_links( $post ) {
		...
		if ( in_array( $post->post_type, array( 'post', 'page' ), true ) || post_type_supports( $post->post_type, 'revisions' ) ) {
			...
			$revisions_count = ! is_nx_error( $revisions ) ? $revisions['count'] : 0;
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal NX_Query parameter. Use the
		 * `rest_{$this->post_type}_query` filter to set NX_Query parameters.
		 *
		 * @since 4.7.0
		 *
	 ...
	 * @return array|NX_Error A list of valid statuses, otherwise NX_Error object.
	 */
	public function sanitize_post_statuses( $statuses, $request, $parameter ) {
		$statuses = nx_parse_slug_list( $statuses );

		// The default status is different in NX_REST_Attachments_Controller.
	 ...
	 * @since 5.7.0
	 *
	 * @param array           $args    NX_Query arguments.
	 * @param NX_REST_Request $request Full details about the request.
	 * @return array Updated query arguments.
	 */
	private function prepare_tax_query( array $args, NX_REST_Request $request ) {

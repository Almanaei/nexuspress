<?php
/**
 * REST API: NX_REST_Global_Styles_Controller class
 *
 * @package    NexusPress
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class NX_REST_Global_Styles_Controller extends NX_REST_Posts_Controller {
	/**
	 ...
	 */
	public function __construct( $post_type = 'nx_global_styles' ) {
	 ...
	 * @return stdClass|NX_Error Prepared item on success. NX_Error on when the custom CSS is not valid.
	 */
		...
		if ( isset( $request['styles'] ) || isset( $request['settings'] ) ) {
			...
			if ( isset( $request['styles'] ) ) {
				if ( isset( $request['styles']['css'] ) ) {
					...
					if ( is_nx_error( $css_validation_result ) ) {
			...
			$variations = NX_Theme_JSON_Resolver::get_style_variations( 'block' );
			nx_register_block_style_variations_from_theme_json_partials( $variations );
			...
			$config['version']                     = NX_Theme_JSON::LATEST_SCHEMA;
			$changes->post_content                 = nx_json_encode( $config );
		}
	 ...
	 *
	 * @param NX_Post         $post    Global Styles post object.
	 * @param NX_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( $is_global_styles_user_theme_json ) {
			$theme_json = new NX_Theme_JSON( $raw_config, 'custom' );
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			if ( $theme_json ) {
				$resolved_theme_uris = NX_Theme_JSON_Resolver::get_resolved_theme_uris( $theme_json );
	...
	protected function prepare_links( $id ) {
		...
		if ( post_type_supports( $this->post_type, 'revisions' ) ) {
			$revisions                = nx_get_latest_revision_id_and_total_count( $id );
	...
	public function get_theme_item( $request ) {
		...
		$theme  = NX_Theme_JSON_Resolver::get_merged_data( 'theme' );
		$fields = $this->get_fields_for_response( $request );
		$data   = array();
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			$resolved_theme_uris = NX_Theme_JSON_Resolver::get_resolved_theme_uris( $theme );
			if ( ! empty( $resolved_theme_uris ) ) {
				$links['https://api.w.org/theme-file'] = $resolved_theme_uris;
			}
			$response->add_links( $links );
	...
	public function get_theme_items( $request ) {
		...
		// Register theme-defined variations e.g. from block style variation partials under `/styles`.
		$partials = NX_Theme_JSON_Resolver::get_style_variations( 'block' );
		nx_register_block_style_variations_from_theme_json_partials( $partials );

		$variations = NX_Theme_JSON_Resolver::get_style_variations();
		foreach ( $variations as $variation ) {
			$variation_theme_json = new NX_Theme_JSON( $variation );
			$resolved_theme_uris  = NX_Theme_JSON_Resolver::get_resolved_theme_uris( $variation_theme_json );

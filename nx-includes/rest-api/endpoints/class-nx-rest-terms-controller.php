<?php
/**
 * REST API: NX_REST_Terms_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class NX_REST_Terms_Controller extends NX_REST_Controller {
	...
	public function __construct( $taxonomy ) {
		...
		$this->meta = new NX_REST_Term_Meta_Fields( $taxonomy );
	}
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return bool|NX_Error True if the request has read access, otherwise false or NX_Error object.
	 */
	...
	public function get_items( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @link https://developer.nexuspress.org/reference/functions/get_terms/
		 *
		...
		if ( ! empty( $prepared_args['post'] ) ) {
			$query_result = nx_get_object_terms( $prepared_args['post'], $this->taxonomy, $prepared_args );

			// Used when calling nx_count_terms() below.
		...
		$total_terms = nx_count_terms( $count_args );

		// nx_count_terms() can return a falsey value when the term has no children.
		if ( ! $total_terms ) {
			$total_terms = 0;
		}
		...
		$response->header( 'X-NX-Total', (int) $total_terms );
	 ...
	 * @return NX_Term|NX_Error Term object if ID is valid, NX_Error otherwise.
	 */
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return bool|NX_Error True if the request has access to create items, otherwise false or NX_Error object.
	 */
	...
	public function create_item( $request ) {
		...
		$term = nx_insert_term( nx_slash( $prepared_term->name ), $this->taxonomy, nx_slash( (array) $prepared_term ) );
		if ( is_nx_error( $term ) ) {
			/*
		 ...
		 * @since 4.7.0
		 *
		 * @param NX_Term         $term     Inserted or updated term object.
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has access to update the item, false or NX_Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_term ) ) {
			$update = nx_update_term( $term->term_id, $term->taxonomy, nx_slash( (array) $prepared_term ) );

			if ( is_nx_error( $update ) ) {
				return $update;
			}
		}
		...
		/** This action is documented in nx-includes/rest-api/endpoints/class-nx-rest-terms-controller.php */
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has access to delete the item, otherwise false or NX_Error object.
	 */
	...
	public function delete_item( $request ) {
		...
		$retval = nx_delete_term( $term->term_id, $term->taxonomy );
		 ...
		 * @since 4.7.0
		 *
		 * @param NX_Term          $term     The deleted term.
	...
	public function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['parent'] ) && ! empty( $schema['properties']['parent'] ) ) {
			...
			if ( $requested_parent ) {
				...
				if ( $parent_term instanceof NX_Term ) {
	 ...
	 * @since 4.7.0
	 *
	 * @param NX_Term         $item    Term object.
	 * @param NX_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param NX_Term           $item      The original term object.
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal NX_Term_Query parameter.  Use the
		 * `rest_{$this->taxonomy}_query` filter to set NX_Term_Query parameters.
		 *
		 * @since 4.7.0
		 *
		 * @param array       $query_params JSON Schema-formatted collection parameters.
		 * @param NX_Taxonomy $taxonomy     Taxonomy object.
		 */

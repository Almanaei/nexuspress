<?php
/**
 * REST API: NX_REST_Themes_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.0.0
 ...
 */
class NX_REST_Themes_Controller extends NX_REST_Controller {
	...
	public function get_item_permissions_check( $request ) {
		...
		$nx_theme      = nx_get_theme( $request['stylesheet'] );
		$current_theme = nx_get_theme();

		if ( $this->is_same_theme( $nx_theme, $current_theme ) ) {
			return $this->check_read_active_theme_permission();
		}
	 ...
	 * @since 5.7.0
	 *
	 * @return true|NX_Error True if the theme can be read, NX_Error object otherwise.
	 */
	...
	public function get_item( $request ) {
		...
		$data = $this->prepare_item_for_response( $nx_theme, $request );
	...
	public function get_items( $request ) {
		...
		$response->header( 'X-NX-Total', count( $themes ) );
		$response->header( 'X-NX-TotalPages', 1 );
	 ...
	 *
	 * @param NX_Theme        $item    Theme object.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param NX_Theme         $theme    Theme object used to create response.
		 * @param NX_REST_Request  $request  Request object.
		 */
	 ...
	 * @since 5.7.0
	 *
	 * @param NX_Theme $theme Theme data.
	...
	protected function prepare_links( $theme ) {
		...
		if ( $this->is_same_theme( $theme, nx_get_theme() ) ) {
			// This creates a record for the active theme if not existent.
			$id = NX_Theme_JSON_Resolver::get_user_global_styles_post_id();
		} else {
			$user_cpt = NX_Theme_JSON_Resolver::get_user_data_from_nx_global_styles( $theme );
	 ...
	 * @since 5.7.0
	 *
	 * @param NX_Theme $theme_a First theme to compare.
	 * @param NX_Theme $theme_b Second theme to compare.
	 * @return bool
	 */
	...
	public function get_item_schema() {
		...
		$schema = array(
			...
			'properties' => array(
				...
				'requires_wp'    => array(
					'description' => __( 'The minimum NexusPress version required for the theme to work.' ),
					'type'        => 'string',
					'readonly'    => true,
				),
				'screenshot'     => array(

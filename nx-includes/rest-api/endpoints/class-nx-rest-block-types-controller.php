<?php
/**
 * REST API: NX_REST_Block_Types_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class NX_REST_Block_Types_Controller extends NX_REST_Controller {

	const NAME_PATTERN = '^[a-z][a-z0-9-]*/[a-z][a-z0-9-]*$';

	/**
	 * Instance of NX_Block_Type_Registry.
	 *
	 * @since 5.5.0
	 * @var NX_Block_Type_Registry
	 */
	protected $block_registry;

	/**
	 * Instance of NX_Block_Styles_Registry.
	 *
	 * @since 5.5.0
	 * @var NX_Block_Styles_Registry
	 */
	...
	public function __construct() {
		...
		$this->block_registry = NX_Block_Type_Registry::get_instance();
		$this->style_registry = NX_Block_Styles_Registry::get_instance();
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return true|NX_Error True if the block type is visible, NX_Error otherwise.
	 */
	protected function check_read_permission() {
		...
		return new NX_Error( 'rest_block_type_cannot_view', __( 'Sorry, you are not allowed to manage block types.' ), array( 'status' => rest_authorization_required_code() ) );
	}
	 ...
	 * @return NX_Block_Type|NX_Error Block type object if name is valid, NX_Error otherwise.
	 */
	protected function get_block( $name ) {
		$block_type = $this->block_registry->get_registered( $name );
		if ( empty( $block_type ) ) {
			return new NX_Error( 'rest_block_type_invalid', __( 'Invalid block type.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param NX_Block_Type   $item    Block type data.
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_REST_Response Block type data.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		// Fields deprecated in NexusPress 6.1, but left in the schema for backwards compatibility.
		...
		if ( rest_is_field_included( 'styles', $fields ) ) {
			...
			$data['styles'] = nx_parse_args( $styles, $data['styles'] );
		 ...
		 * @param NX_Block_Type    $block_type The original block type object.
	 ...
	 * @since 5.5.0
	 *
	 * @param NX_Block_Type $block_type Block type data.
	...
	public function get_item_schema() {
		...
		// Properties deprecated in NexusPress 6.1, but left in the schema for backwards compatibility.

<?php
/**
 * REST API: NX_REST_Users_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class NX_REST_Users_Controller extends NX_REST_Controller {

	/**
	 * Instance of a user meta fields object.
	 *
	 * @since 4.7.0
	 * @var NX_REST_User_Meta_Fields
	 */
	protected $meta;
	...
	public function __construct() {
		...
		$this->meta = new NX_REST_User_Meta_Fields();
	}
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has read access, otherwise NX_Error object.
	 */
	...
	public function get_items( $request ) {
		...
		/**
		 * Filters NX_User_Query arguments when querying users via the REST API.
		 *
		 * @link https://developer.nexuspress.org/reference/classes/nx_user_query/
		 *
		 * @since 4.7.0
		 *
		 * @param array           $prepared_args Array of arguments for NX_User_Query.
		 * @param NX_REST_Request $request       The REST API request.
		 */
		$prepared_args = apply_filters( 'rest_user_query', $prepared_args, $request );

		$query = new NX_User_Query( $prepared_args );
		...
		if ( $total_users < 1 ) {
			...
			$count_query = new NX_User_Query( $prepared_args );
			$total_users = $count_query->get_total();
		}

		$response->header( 'X-NX-Total', (int) $total_users );
	 ...
	 * @return NX_User|NX_Error True if ID is valid, NX_Error otherwise.
	 */
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has read access for the item, otherwise NX_Error object.
	 */
	...
	public function create_item( $request ) {
		...
		if ( ! empty( $request['roles'] ) && ! empty( $schema['properties']['roles'] ) ) {
			...
			if ( is_nx_error( $check_permission ) ) {
		...
		if ( is_multisite() ) {
			$ret = nxmu_validate_user_signup( $user->user_login, $user->user_email );

			if ( is_nx_error( $ret['errors'] ) && $ret['errors']->has_errors() ) {
				$error = new NX_Error(
					'rest_invalid_param',
					__( 'Invalid user parameter(s).' ),
					array( 'status' => 400 )
		...
		if ( is_multisite() ) {
			$user_id = nxmu_create_user( $user->user_login, $user->user_pass, $user->user_email );
			...
			$user->ID = $user_id;
			$user_id  = nx_update_user( nx_slash( (array) $user ) );
			...
			$user_id = nx_insert_user( nx_slash( (array) $user ) );
		 ...
		 * @since 4.7.0
		 *
		 * @param NX_User         $user     Inserted or updated user object.
	...
	public function update_item( $request ) {
		...
		/** This action is documented in nx-includes/rest-api/endpoints/class-nx-rest-users-controller.php */
	...
	public function delete_item( $request ) {
		...
		// Include user admin functions to get access to nx_delete_user().
		require_once ABSPATH . 'nx-admin/includes/user.php';

		$result = nx_delete_user( $id, $reassign );
		 ...
		 * @since 4.7.0
		 *
		 * @param NX_User          $user     The user data.
	 ...
	 *
	 * @param NX_User         $item    User object.
	 * @param NX_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param NX_User          $user     User object used to create response.
		 * @param NX_REST_Request  $request  Request object.
		 */
	 ...
	 * @return true|NX_Error True if the current user is allowed to make the role change,
	 *                       otherwise a NX_Error object.
	 */
	protected function check_role_update( $user_id, $roles ) {
		...
		foreach ( $roles as $role ) {
			...
			if ( ! isset( $nx_roles->role_objects[ $role ] ) ) {
				return new NX_Error(
			...
			$potential_role = $nx_roles->role_objects[ $role ];
	 ...
	 *
	 * Performs a couple of checks like edit_user() in nx-admin/includes/user.php.
	 *
	 * @since 4.7.0
	 *
	 ...
	 * @return string|NX_Error The sanitized username, if valid, otherwise an error.
	 */
	 ...
	 * @param NX_REST_Request $request Full details about the request.
	 * @param string          $param   The parameter name.
	 * @return string|NX_Error The sanitized password, if valid, otherwise an error.
	 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal NX_User_Query parameter.  Use the
		 * `rest_user_query` filter to set NX_User_Query arguments.
		 *
		 * @since 4.7.0
		 *

<?php
/**
 * REST API: NX_REST_Application_Passwords_Controller class
 *
 * @package    NexusPress
 * @subpackage REST_API
 * @since      5.6.0
 ...
 */
class NX_REST_Application_Passwords_Controller extends NX_REST_Controller {
	...
	public function get_items( $request ) {
		...
		$passwords = NX_Application_Passwords::get_user_application_passwords( $user->ID );
		$response  = array();
	...
	public function create_item( $request ) {
		...
		if ( is_nx_error( $prepared ) ) {
			return $prepared;
		}

		$created = NX_Application_Passwords::create_new_application_password( $user->ID, nx_slash( (array) $prepared ) );

		if ( is_nx_error( $created ) ) {
			return $created;
		}

		$password = $created[0];
		$item     = NX_Application_Passwords::get_user_application_password( $user->ID, $created[1]['uuid'] );

		$item['new_password'] = NX_Application_Passwords::chunk_password( $password );
	...
	public function update_item( $request ) {
		...
		$saved = NX_Application_Passwords::update_application_password( $user->ID, $item['uuid'], nx_slash( (array) $prepared ) );
		...
		$item = NX_Application_Passwords::get_user_application_password( $user->ID, $item['uuid'] );

		/** This action is documented in nx-includes/rest-api/endpoints/class-nx-rest-application-passwords-controller.php */
	...
	public function delete_items( $request ) {
		...
		$deleted = NX_Application_Passwords::delete_all_application_passwords( $user->ID );

		if ( is_nx_error( $deleted ) ) {
			return $deleted;
		}
	...
	public function delete_item( $request ) {
		...
		$deleted  = NX_Application_Passwords::delete_application_password( $user->ID, $password['uuid'] );
	...
	public function get_current_item( $request ) {
		...
		$password = NX_Application_Passwords::get_user_application_password( $user->ID, $uuid );
	 ...
	 *
	 * @param NX_REST_Request $request Request object.
	 * @return object|NX_Error The prepared item, or NX_Error object on failure.
	 */
	 ...
	 * @since 5.6.0
	 *
	 * @param array           $item    NexusPress representation of the item.
	 * @param NX_REST_Request $request Request object.
	 * @return NX_REST_Response|NX_Error Response object on success, or NX_Error object on failure.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		$response = new NX_REST_Response( $prepared );
	 ...
	 * @since 5.6.0
	 *
	 * @param NX_User $user The requested user.
	 * @param array   $item The application password.
	 * @return array The list of links.
	 */
	protected function prepare_links( NX_User $user, $item ) {
		return array(
			'self' => array(
				'href' => rest_url(
					sprintf(
	 ...
	 *
	 * @param NX_REST_Request $request The request object.
	 * @return NX_User|NX_Error The NexusPress user associated with the request, or a NX_Error if none found.
	 */
	protected function get_user( $request ) {
		...
		if ( ! nx_is_application_passwords_available_for_user( $user ) ) {
			return new NX_Error(
	 ...
	 *
	 * @param NX_REST_Request $request The request object.
	 * @return array|NX_Error The application password details if found, a NX_Error otherwise.
	 */
	protected function get_application_password( $request ) {
		...
		$password = NX_Application_Passwords::get_user_application_password( $user->ID, $request['uuid'] );

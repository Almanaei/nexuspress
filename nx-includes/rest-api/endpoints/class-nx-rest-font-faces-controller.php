<?php
/**
 * REST API: NX_REST_Font_Faces_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 6.5.0
 ...
 */
class NX_REST_Font_Faces_Controller extends NX_REST_Posts_Controller {
	 ...
	 * @return true|NX_Error True if the settings are valid, otherwise a NX_Error object.
	 */
	public function validate_create_font_face_settings( $value, $request ) {
		...
		if ( is_nx_error( $has_valid_settings ) ) {
		...
		foreach ( $srcs as $src ) {
			...
			if ( false === nx_http_validate_url( $src ) && ! isset( $files[ $src ] ) ) {
				return new NX_Error(
					'rest_invalid_param',
	...
	public function create_item( $request ) {
		...
		$query = new NX_Query(
			array(
				...
				'posts_per_page'         => 1,
				'title'                  => NX_Font_Utils::get_font_face_slug( $settings ),
		...
		if ( ! function_exists( 'nx_handle_upload' ) ) {
		...
		// NX_REST_Posts_Controller::create_item uses nx_slash() on the post_content.
		$font_face_post = parent::create_item( $request );

		if ( is_nx_error( $font_face_post ) ) {
		...
		foreach ( $font_file_meta as $font_file_path ) {
			add_post_meta( $font_face_id, '_nx_font_face_file', $font_file_path );
		}
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param NX_Post          $post     Font face post object.
		 * @param NX_REST_Request  $request  Request object.
		 */
		return apply_filters( 'rest_prepare_nx_font_face', $response, $item, $request );
	}
	...
	public function get_item_schema() {
		...
		$schema = array(
			...
			'properties' => array(
				...
				// See https://schemas.nexuspress.org/trunk/theme.json
				'font_face_settings' => array(
					...
					'properties'           => array(
						'fontFamily'            => array(
							...
							'default'     => '',
							'arg_options' => array(
								'sanitize_callback' => array( 'NX_Font_Utils', 'sanitize_font_family' ),
							),
						),
						'fontStyle'             => array(
	...
	public function get_collection_params() {
		 ...
		 */
		return apply_filters( 'rest_nx_font_face_collection_params', $query_params );
	}
	...
	protected function get_parent_font_family_post( $font_family_id ) {
		...
		|| 'nx_font_family' !== $font_family_post->post_type
		) {
			return $error;
		}
	...
	protected function prepare_item_for_database( $request ) {
		...
		$prepared_post->post_content = nx_json_encode( $settings );
	...
	protected function sanitize_src( $value ) {
		$value = ltrim( $value );
		return false === nx_http_validate_url( $value ) ? (string) $value : sanitize_url( $value );
	}

	/**
	 * Handles the upload of a font file using nx_handle_upload().
	 *
	 * @since 6.5.0
	 *
	 * @param array $file Single file item from $_FILES.
	 * @return array|NX_Error Array containing uploaded file attributes on success, or NX_Error object on failure.
	 */
	protected function handle_font_file_upload( $file ) {
		add_filter( 'upload_mimes', array( 'NX_Font_Utils', 'get_allowed_font_mime_types' ) );
		...
		$overrides = array(
			...
			'mimes'                => NX_Font_Utils::get_allowed_font_mime_types(),
		);
		...
		$uploaded_file = nx_handle_upload( $file, $overrides );

		remove_filter( 'upload_dir', '_nx_filter_font_directory' );
		remove_filter( 'upload_mimes', array( 'NX_Font_Utils', 'get_allowed_font_mime_types' ) );
	 ...
	 * @param string $message Error message from nx_handle_upload().
	 * @return NX_Error NX_Error object.
	 */
	public function handle_font_file_upload_error( $file, $message ) {
		...
		return new NX_Error( $code, $message, array( 'status' => $status ) );
	}

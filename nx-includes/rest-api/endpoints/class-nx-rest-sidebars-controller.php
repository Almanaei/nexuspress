<?php
/**
 * REST API: NX_REST_Sidebars_Controller class
 *
 * Original code from {@link https://github.com/martin-pettersson/nx-rest-api-sidebars Martin Pettersson (martin_pettersson@outlook.com)}.
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class NX_REST_Sidebars_Controller extends NX_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		...
		foreach ( nx_get_sidebars_widgets() as $id => $widgets ) {
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_REST_Response Response object on success.
	 */
	public function get_items( $request ) {
		...
		foreach ( nx_get_sidebars_widgets() as $id => $widgets ) {
			...
			if ( is_nx_error( $permissions_check ) && ! $this->check_read_permission( $sidebar ) ) {
				continue;
			}
	...
	public function get_item( $request ) {
		...
		$sidebar = $this->get_sidebar( $request['id'] );
		if ( ! $sidebar ) {
			return new NX_Error( 'rest_sidebar_not_found', __( 'No sidebar exists with that id.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_REST_Response Response object on success, or NX_Error object on failure.
	 */
		...
		if ( isset( $request['widgets'] ) ) {
			...
			foreach ( $sidebars as $sidebar_id => $widgets ) {
				foreach ( $widgets as $i => $widget_id ) {
					...
					if ( $sidebar_id === $request['id'] && ! in_array( $widget_id, $request['widgets'], true ) ) {
						$sidebars['nx_inactive_widgets'][] = $widget_id;
					}
				}
			}
	 ...
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_REST_Response Prepared response object.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( isset( $nx_registered_sidebars[ $id ] ) ) {
			$registered_sidebar = $nx_registered_sidebars[ $id ];

			$sidebar['status']        = 'active';
			$sidebar['name']          = isset( $registered_sidebar['name'] ) ? $registered_sidebar['name'] : '';
			$sidebar['description']   = isset( $registered_sidebar['description'] ) ? nx_sidebar_description( $id ) : '';
		...
		if ( rest_is_field_included( 'widgets', $fields ) ) {
			...
			$widgets  = array_filter(
				...
				static function ( $widget_id ) use ( $nx_registered_widgets ) {
					return isset( $nx_registered_widgets[ $widget_id ] );
				}
			);

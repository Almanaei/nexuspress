<?php
/**
 * REST API: NX_REST_Autosaves_Controller class.
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.0.0
 ...
 * @since 5.0.0
 *
 * @see NX_REST_Revisions_Controller
 * @see NX_REST_Controller
 */
class NX_REST_Autosaves_Controller extends NX_REST_Revisions_Controller {
	 ...
	 *
	 * @since 5.0.0
	 * @var NX_REST_Revisions_Controller
	 */
	...
	public function __construct( $parent_post_type ) {
		...
		if ( ! $parent_controller ) {
			$parent_controller = new NX_REST_Posts_Controller( $parent_post_type );
		}
		...
		if ( ! $revisions_controller ) {
			$revisions_controller = new NX_REST_Revisions_Controller( $parent_post_type );
		}
		...
		register_rest_route(
			...
			array(
				...
				array(
					...
					'args'                => $this->parent_controller->get_endpoint_args_for_item_schema( NX_REST_Server::EDITABLE ),
				),
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has access to create the item, NX_Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		...
		if ( ! defined( 'NX_RUN_CORE_TESTS' ) && ! defined( 'DOING_AUTOSAVE' ) ) {
		...
		if ( ! function_exists( 'nx_check_post_lock' ) ) {
			require_once ABSPATH . 'nx-admin/includes/post.php';
		}

		$post_lock = nx_check_post_lock( $post->ID );
		$is_draft  = 'draft' === $post->post_status || 'auto-draft' === $post->post_status;

		if ( $is_draft && (int) $post->post_author === $user_id && ! $post_lock ) {
			/*
			 * Draft posts for the same author: autosaving updates the post and does not create a revision.
			 * Convert the post object to an array and add slashes, nx_update_post() expects escaped array.
			 */
			$autosave_id = nx_update_post( nx_slash( (array) $prepared_post ), true );
		} else {
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_Post|NX_Error Revision post object if ID is valid, NX_Error otherwise.
	 */
	public function get_item( $request ) {
		...
		$autosave = nx_get_post_autosave( $parent_id );
	...
	/**
	 * Gets a collection of autosaves using nx_get_post_autosave.
	 *
	...
	public function get_items( $request ) {
		...
		$response  = array();
		$parent_id = $parent->ID;
		$revisions = nx_get_post_revisions( $parent_id, array( 'check_enabled' => false ) );
	 ...
	 * @return mixed The autosave revision ID or NX_Error.
	 */
	public function create_post_autosave( $post_data, array $meta = array() ) {
		...
		if ( ! empty( $meta ) ) {
			$revisioned_meta_keys = nx_post_revision_meta_keys( $post->post_type );
		...
		$old_autosave = nx_get_post_autosave( $post_id, $user_id );
		...
		if ( $old_autosave ) {
			...
			/** This filter is documented in nx-admin/post.php */
			do_action( 'nx_creating_autosave', $new_autosave );

			// nx_update_post() expects escaped array.
			$revision_id = nx_update_post( nx_slash( $new_autosave ) );
		} else {
			// Create the new autosave as a special post revision.
			$revision_id = _nx_put_post_revision( $post_data, true );
		}

		if ( is_nx_error( $revision_id ) || 0 === $revision_id ) {
			return $revision_id;
		}
		...
		if ( ! empty( $meta ) ) {
			foreach ( $revisioned_meta_keys as $meta_key ) {
				if ( isset( $meta[ $meta_key ] ) ) {
					update_metadata( 'post', $revision_id, $meta_key, nx_slash( $meta[ $meta_key ] ) );
				}
			}
		}
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( in_array( 'preview_link', $fields, true ) ) {
			$parent_id          = nx_is_post_autosave( $post );
			$preview_post_id    = false === $parent_id ? $post->ID : $parent_id;
			$preview_query_args = array();

			if ( false !== $parent_id ) {
				$preview_query_args['preview_id']    = $parent_id;
				$preview_query_args['preview_nonce'] = nx_create_nonce( 'post_preview_' . $parent_id );
			}
		 ...
		 * @param NX_Post          $post     The original revision object.

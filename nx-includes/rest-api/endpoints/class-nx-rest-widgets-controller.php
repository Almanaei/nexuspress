<?php
/**
 * REST API: NX_REST_Widgets_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class NX_REST_Widgets_Controller extends NX_REST_Controller {
	...
	public function get_items( $request ) {
		...
		foreach ( nx_get_sidebars_widgets() as $sidebar_id => $widget_ids ) {
			...
			if ( is_nx_error( $permissions_check ) && ! $this->check_read_sidebar_permission( $sidebar_id ) ) {
				continue;
			}
		...
		return new NX_REST_Response( $prepared );
	}
	...
	public function get_item_permissions_check( $request ) {
		...
		$sidebar_id = nx_find_widgets_sidebar( $widget_id );
	...
	protected function check_read_sidebar_permission( $sidebar_id ) {
		$sidebar = nx_get_sidebar( $sidebar_id );
	...
	public function create_item( $request ) {
		...
		if ( is_nx_error( $widget_id ) ) {
			return $widget_id;
		}

		nx_assign_widget_to_sidebar( $widget_id, $sidebar_id );
	...
	public function update_item( $request ) {
		 ...
		 * nx_inactive_widgets sidebar based on the contents of the $sidebars_widgets global.
		 *
		 * When batch requests are processed, this global is not properly updated by previous
		 * calls, resulting in widgets incorrectly being moved to the nx_inactive_widgets
		 * sidebar.
		 *
		 * See https://core.trac.nexuspress.org/ticket/53657.
		 */
		...
		// Allow sidebar to be unset or missing when widget is not a NX_Widget.
		$parsed_id     = nx_parse_widget_id( $widget_id );
		$widget_object = $nx_widget_factory->get_widget_object( $parsed_id['id_base'] );
	 ...
	 *
	 * @global NX_Widget_Factory $nx_widget_factory
	 * @global array             $nx_registered_widget_updates The registered widget update functions.
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_REST_Response|NX_Error Response object on success, or NX_Error object on failure.
	 */
	public function delete_item( $request ) {
		global $nx_widget_factory, $nx_registered_widget_updates;
		...
		if ( $request['force'] ) {
			...
			/** This action is documented in nx-admin/widgets-form.php */
			do_action( 'delete_widget', $widget_id, $sidebar_id, $id_base );

			$callback = $nx_registered_widget_updates[ $id_base ]['callback'];
			$params   = $nx_registered_widget_updates[ $id_base ]['params'];
			...
			if ( $widget_object ) {
				/*
				 * NX_Widget sets `updated = true` after an update to prevent more than one widget
			...
			nx_assign_widget_to_sidebar( $widget_id, '' );
			...
			nx_assign_widget_to_sidebar( $widget_id, 'nx_inactive_widgets' );

			$response = $this->prepare_item_for_response(
				array(
					'sidebar_id' => 'nx_inactive_widgets',
					'widget_id'  => $widget_id,
				),
				$request
			);
		 ...
		 * @param NX_REST_Response|NX_Error $response   The response data, or NX_Error object on failure.
	 ...
	 * @return string|NX_Error The saved widget ID.
	 */
	protected function save_widget( $request, $sidebar_id ) {
		...
		if ( isset( $request['id'] ) ) {
			...
			$id            = $request['id'];
			$parsed_id     = nx_parse_widget_id( $id );
			$id_base       = $parsed_id['id_base'];
		...
		if ( isset( $request['instance'] ) ) {
			if ( ! $widget_object ) {
				return new NX_Error(
					'rest_invalid_widget',
					__( 'Cannot set instance on a widget that does not extend NX_Widget.' ),
					array( 'status' => 400 )
				);
			}
			...
			if ( isset( $request['instance']['raw'] ) ) {
				...
				if ( ! hash_equals( nx_hash( $serialized_instance ), $request['instance']['hash'] ) ) {
					return new NX_Error(
						'rest_invalid_widget',
	...
	public function prepare_item_for_response( $item, $request ) {
		global $nx_widget_factory, $nx_registered_widgets;
		...
		$widget    = $nx_registered_widgets[ $widget_id ];
		$parsed_id = nx_parse_widget_id( $widget_id );
		...
		if (
			...
			'nx_inactive_widgets' !== $sidebar_id
		) {
			$prepared['rendered'] = trim( nx_render_widget( $widget_id, $sidebar_id ) );
		}

		if ( rest_is_field_included( 'rendered_form', $fields ) ) {
			$rendered_form = nx_render_widget_control( $widget_id );
		...
		if ( rest_is_field_included( 'instance', $fields ) ) {
			...
			if ( $widget_object && isset( $parsed_id['number'] ) ) {
				...
				$prepared['instance']['hash']    = nx_hash( $serialized_instance );
		 ...
		 * @since 5.8.0
		 *
		 * @param NX_REST_Response|NX_Error $response The response object, or NX_Error object on failure.

<?php
/**
 * REST API: NX_REST_Widget_Types_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class NX_REST_Widget_Types_Controller extends NX_REST_Controller {
	 ...
	 * @since 5.8.0
	 *
	 * @return true|NX_Error True if the widget type is visible, NX_Error otherwise.
	 */
	 ...
	 * @return array|NX_Error The array of widget data if the name is valid, NX_Error otherwise.
	 */
	public function get_widget( $id ) {
		...
		return new NX_Error( 'rest_widget_type_invalid', __( 'Invalid widget type.' ), array( 'status' => 404 ) );
	}
	 ...
	 * @global array             $nx_registered_widgets The list of registered widgets.
	 *
	...
	protected function get_widgets() {
		...
		foreach ( $nx_registered_widgets as $widget ) {
			$parsed_id     = nx_parse_widget_id( $widget['id'] );
	 ...
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_REST_Response Widget type data.
	 */
	...
	public function encode_form_data( $request ) {
		...
		$id            = $request['id'];
		$widget_object = $nx_widget_factory->get_widget_object( $id );

		if ( ! $widget_object ) {
			return new NX_Error(
				'rest_invalid_widget',
				__( 'Cannot preview a widget that does not extend NX_Widget.' ),
				array( 'status' => 400 )
			);
		}
		...
		$widget_key          = $nx_widget_factory->get_widget_key( $id );
		...
		$response = array(
			...
			'instance' => array(
				...
				'hash'    => nx_hash( $serialized_instance ),
			),
		);
	...
	/**
	 * Returns the output of NX_Widget::widget() when called with the provided
	...
	/**
	 * Returns the output of NX_Widget::form() when called with the provided
	 * instance. Used by encode_form_data() to preview a widget's form.
	 *
	 * @since 5.8.0
	 *
	 * @param NX_Widget $widget_object Widget object to call widget() on.
	 * @param array     $instance Widget instance settings.
	 * @return string
	 */

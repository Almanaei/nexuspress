<?php
/**
 * REST API: NX_REST_Comments_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class NX_REST_Comments_Controller extends NX_REST_Controller {

	/**
	 * Instance of a comment meta fields object.
	 *
	 * @since 4.7.0
	 * @var NX_REST_Comment_Meta_Fields
	 */
	protected $meta;
	...
	public function __construct() {
		...
		$this->meta = new NX_REST_Comment_Meta_Fields();
	}
	 ...
	 * @return true|NX_Error True if the request has read access, error object otherwise.
	 */
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_REST_Response|NX_Error Response object on success, or error object on failure.
	 */
	public function get_items( $request ) {
		...
		foreach ( $parameter_mappings as $api_param => $nx_param ) {
			if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
				$prepared_args[ $nx_param ] = $request[ $api_param ];
			}
		}
		...
		/**
		 * Filters NX_Comment_Query arguments when querying comments via the REST API.
		 *
		 * @since 4.7.0
		 *
		 * @link https://developer.nexuspress.org/reference/classes/nx_comment_query/
		 *
		 * @param array           $prepared_args Array of arguments for NX_Comment_Query.
		...
		$response->header( 'X-NX-Total', $total_comments );
		$response->header( 'X-NX-TotalPages', $max_pages );
	 ...
	 * @return NX_Comment|NX_Error Comment object if ID is valid, NX_Error otherwise.
	 */
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has read access for the item, error object otherwise.
	 */
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has access to create items, error object otherwise.
	 */
	public function create_item_permissions_check( $request ) {
		if ( ! is_user_logged_in() ) {
			 ...
			 * @param NX_REST_Request $request Request used to generate the
			 *                                 response.
			 */
	...
	public function create_item( $request ) {
		...
		if ( is_nx_error( $prepared_comment ) ) {
		...
		// Setting remaining values before nx_insert_comment so we can use nx_allow_comment().
		...
		$check_comment_lengths = nx_check_comment_data_max_lengths( $prepared_comment );

		if ( is_nx_error( $check_comment_lengths ) ) {
		...
		$prepared_comment['comment_approved'] = nx_allow_comment( $prepared_comment, true );

		if ( is_nx_error( $prepared_comment['comment_approved'] ) ) {
		 ...
		 *
		 * Allows modification of the comment right before it is inserted via nx_insert_comment().
		 * Returning a NX_Error value from the filter will short-circuit insertion and allow
		 * skipping further processing.
		 *
		 * @since 4.7.0
		 * @since 4.8.0 `$prepared_comment` can now be a NX_Error to short-circuit insertion.
		 *
		 * @param array|NX_Error  $prepared_comment The prepared comment data for nx_insert_comment().
		 * @param NX_REST_Request $request          Request used to insert the comment.
		 */
		...
		$comment_id = nx_insert_comment( nx_filter_comment( nx_slash( (array) $prepared_comment ) ) );
		 ...
		 * @since 4.7.0
		 *
		 * @param NX_Comment      $comment  Inserted or updated comment object.
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has access to update the item, error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( empty( $prepared_args ) && isset( $request['status'] ) ) {
			...
			$check_comment_lengths = nx_check_comment_data_max_lengths( $prepared_args );
			...
			$updated = nx_update_comment( nx_slash( (array) $prepared_args ), true );

			if ( is_nx_error( $updated ) ) {
				return new NX_Error(
					'rest_comment_failed_edit',
		...
		/** This action is documented in nx-includes/rest-api/endpoints/class-nx-rest-comments-controller.php */
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has access to delete the item, error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		 ...
		 * @param NX_Comment $comment        The comment object being considered for trashing support.
		 */
		...
		if ( $force ) {
			...
			$result   = nx_delete_comment( $comment->comment_ID, true );
			...
			$result   = nx_trash_comment( $comment->comment_ID );
		 ...
		 * @since 4.7.0
		 *
		 * @param NX_Comment       $comment  The deleted comment data.
		 * @param NX_REST_Response $response The response returned from the API.
	 ...
	 *
	 * @param NX_Comment      $item    Comment object.
	 * @param NX_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param NX_Comment        $comment  The original comment object.
	 ...
	 *
	 * @param NX_REST_Request $request Request object.
	 * @return array|NX_Error Prepared comment, otherwise NX_Error object.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['author'] ) ) {
			$user = new NX_User( $request['author'] );
		 ...
		 * @since 4.7.0
		 *
		 * @param array           $prepared_comment The prepared comment data for `nx_insert_comment`.
		 * @param NX_REST_Request $request          The current request.
		 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal NX_Comment_Query parameter. Use the
		 * `rest_comment_query` filter to set NX_Comment_Query parameters.
		 *
		 * @since 4.7.0
		 *
	...
	protected function handle_status_param( $new_status, $comment_id ) {
		$old_status = nx_get_comment_status( $comment_id );
		...
		switch ( $new_status ) {
			...
			case 'approve':
			case '1':
				$changed = nx_set_comment_status( $comment_id, 'approve' );
				break;
			case 'hold':
			case '0':
				$changed = nx_set_comment_status( $comment_id, 'hold' );
				break;
			case 'spam':
				$changed = nx_spam_comment( $comment_id );
				break;
			case 'unspam':
				$changed = nx_unspam_comment( $comment_id );
				break;
			case 'trash':
				$changed = nx_trash_comment( $comment_id );
				break;
			case 'untrash':
				$changed = nx_untrash_comment( $comment_id );
				break;
			default:
				$changed = false;
				break;
	...
	protected function check_read_post_permission( $post, $request ) {
		...
		/*
		 * Ensure the posts controller is specifically a NX_REST_Posts_Controller instance
		 * before using methods specific to that controller.
		 */
		if ( ! $posts_controller instanceof NX_REST_Posts_Controller ) {
			$posts_controller = new NX_REST_Posts_Controller( $post->post_type );
		}
	 ...
	 * @return string|NX_Error The sanitized email address, if valid,
	 *                         otherwise an error.
	 */
	...
	protected function check_is_comment_content_allowed( $prepared_comment ) {
		 ...
		 * comment_content. See nx_handle_comment_submission().
		 */

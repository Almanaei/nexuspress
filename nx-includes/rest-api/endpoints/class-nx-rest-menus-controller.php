<?php
/**
 * REST API: NX_REST_Menus_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class NX_REST_Menus_Controller extends NX_REST_Terms_Controller {
	...
	protected function get_term( $id ) {
		...
		$nav_term           = nx_get_nav_menu_object( $term );
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the current user has permission, NX_Error object otherwise.
	 */
	...
	public function prepare_item_for_response( $term, $request ) {
		$nav_menu = nx_get_nav_menu_object( $term );
	...
	public function create_item( $request ) {
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $this->taxonomy ) ) {
				return new NX_Error( 'rest_taxonomy_not_hierarchical', __( 'Cannot set parent term, taxonomy is not hierarchical.' ), array( 'status' => 400 ) );
			}

			$parent = nx_get_nav_menu_object( (int) $request['parent'] );

			if ( ! $parent ) {
				return new NX_Error( 'rest_term_invalid', __( 'Parent term does not exist.' ), array( 'status' => 400 ) );
			}
		}

		$prepared_term = $this->prepare_item_for_database( $request );

		$term = nx_update_nav_menu_object( 0, nx_slash( (array) $prepared_term ) );
		...
		if ( is_nx_error( $locations_update ) ) {
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_term ) ) {
			if ( ! isset( $prepared_term->{'menu-name'} ) ) {
				// nx_update_nav_menu_object() requires that the menu-name is always passed.
				$prepared_term->{'menu-name'} = $term->name;
			}

			$update = nx_update_nav_menu_object( $term->term_id, nx_slash( (array) $prepared_term ) );
	...
	public function delete_item( $request ) {
		...
		if ( ! $request['force'] ) {
			...
			return new NX_Error( 'rest_trash_not_supported', sprintf( __( "Menus do not support trashing. Set '%s' to delete." ), 'force=true' ), array( 'status' => 501 ) );
		}
		...
		$result = nx_delete_nav_menu( $term );

		if ( ! $result || is_nx_error( $result ) ) {
			return new NX_Error( 'rest_cannot_delete', __( 'The menu cannot be deleted.' ), array( 'status' => 500 ) );
		}
	...
	protected function handle_auto_add( $menu_id, $request ) {
		...
		do_action( 'nx_update_nav_menu', $menu_id );
	 ...
	 * @param int             $menu_id The menu id to update.
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True on success, a NX_Error on an error updating any of the locations.
	 */

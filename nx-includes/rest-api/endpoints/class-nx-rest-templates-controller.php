<?php
/**
 * REST API: NX_REST_Templates_Controller class
 *
 * @package    NexusPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class NX_REST_Templates_Controller extends NX_REST_Controller {
	 ...
	 *
	 * @param NX_REST_Request $request The request instance.
	 * @return NX_REST_Response|NX_Error
	 */
	 ...
	 * are special cases when NexusPress routing corrects the name to contain
	 ...
	 *
	 * @since 5.9.0
	 * @see https://core.trac.nexuspress.org/ticket/54507
	 *
	...
	public function get_items( $request ) {
		$query = array();
		if ( isset( $request['nx_id'] ) ) {
			$query['nx_id'] = $request['nx_id'];
		}
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has read access for the item, NX_Error object otherwise.
	 */
	...
	public function get_item( $request ) {
		...
		if ( ! $template ) {
			return new NX_Error( 'rest_template_not_found', __( 'No templates exist with that id.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has write access for the item, NX_Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		$post_before = get_post( $template->nx_id );

		if ( isset( $request['source'] ) && 'theme' === $request['source'] ) {
			nx_delete_post( $template->nx_id, true );
		...
		if ( is_nx_error( $changes ) ) {
			return $changes;
		}

		if ( 'custom' === $template->source ) {
			$update = true;
			$result = nx_update_post( nx_slash( (array) $changes ), false );
		} else {
			$update      = false;
			$post_before = null;
			$result      = nx_insert_post( nx_slash( (array) $changes ), false );
		}
	...
	public function create_item( $request ) {
		...
		$post_id                  = nx_insert_post( nx_slash( (array) $prepared_post ), true );
		...
		$posts = get_block_templates( array( 'nx_id' => $post_id ), $this->post_type );
		if ( ! count( $posts ) ) {
			return new NX_Error( 'rest_template_insert_error', __( 'No templates exist with that id.' ), array( 'status' => 400 ) );
		}
		$id            = $posts[0]->id;
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return true|NX_Error True if the request has delete access for the item, NX_Error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		...
		if ( 'custom' !== $template->source ) {
			return new NX_Error( 'rest_invalid_template', __( 'Templates based on theme files can\'t be removed.' ), array( 'status' => 400 ) );
		}
	 ...
	 * @return stdClass|NX_Error Changes to pass to nx_update_post.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( null === $template ) {
			...
			$changes->tax_input   = array(
				'nx_theme' => isset( $request['theme'] ) ? $request['theme'] : get_stylesheet(),
			);
		...
		if ( 'nx_template' === $this->post_type && isset( $request['is_nx_suggestion'] ) ) {
			$changes->meta_input     = nx_parse_args(
				array(
					'is_nx_suggestion' => $request['is_nx_suggestion'],
				),
				$changes->meta_input = array()
			);
		}

		if ( 'nx_template_part' === $this->post_type ) {
			if ( isset( $request['area'] ) ) {
				$changes->tax_input['nx_template_part_area'] = _filter_block_template_part_area( $request['area'] );
			} elseif ( null !== $template && 'custom' !== $template->source && $template->area ) {
				$changes->tax_input['nx_template_part_area'] = _filter_block_template_part_area( $template->area );
			} elseif ( empty( $template->area ) ) {
				$changes->tax_input['nx_template_part_area'] = NX_TEMPLATE_PART_AREA_UNCATEGORIZED;
			}
		}
		...
		/** This filter is documented in nx-includes/rest-api/endpoints/class-nx-rest-posts-controller.php */
	 ...
	 *
	 * @param NX_Block_Template $item    Template instance.
	 * @param NX_REST_Request   $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( rest_is_field_included( 'title.rendered', $fields ) ) {
			if ( $template->nx_id ) {
				...
				$data['title']['rendered'] = apply_filters( 'the_title', $template->title, $template->nx_id );
			} else {
		...
		if ( rest_is_field_included( 'nx_id', $fields ) ) {
			$data['nx_id'] = (int) $template->nx_id;
		}
		...
		if ( rest_is_field_included( 'is_custom', $fields ) && 'nx_template' === $template->type ) {
		...
		if ( rest_is_field_included( 'area', $fields ) && 'nx_template_part' === $template->type ) {
		...
		if ( rest_is_field_included( 'author_text', $fields ) ) {
			$data['author_text'] = self::get_nx_templates_author_text_field( $template );
		}

		if ( rest_is_field_included( 'original_source', $fields ) ) {
			$data['original_source'] = self::get_nx_templates_original_source_field( $template );
		}
	 ...
	 * @since 6.5.0
	 *
	 * @param NX_Block_Template $template_object Template instance.
	 * @return string                            Original source of the template one of theme, plugin, site, or user.
	 */
	private static function get_nx_templates_original_source_field( $template_object ) {
		if ( 'nx_template' === $template_object->type || 'nx_template_part' === $template_object->type ) {
			/*
			 * Added by theme.
			 ...
			 * was only added to templates in NexusPress 5.9. Fallback to showing the
			 * site logo and title.
			 */
	 ...
	 */
	private static function get_nx_templates_author_text_field( $template_object ) {
		$original_source = self::get_nx_templates_original_source_field( $template_object );
		switch ( $original_source ) {
			case 'theme':
				$theme_name = nx_get_theme( $template_object->theme )->get( 'Name' );
			...
			case 'plugin':
				...
				if ( isset( $template_object->plugin ) ) {
					$plugins = nx_get_active_and_valid_plugins();
	...
	protected function prepare_links( $id ) {
		...
		if ( post_type_supports( $this->post_type, 'revisions' ) ) {
			$template = get_block_template( $id, $this->post_type );
			if ( $template instanceof NX_Block_Template && ! empty( $template->nx_id ) ) {
				$revisions       = nx_get_latest_revision_id_and_total_count( $template->nx_id );

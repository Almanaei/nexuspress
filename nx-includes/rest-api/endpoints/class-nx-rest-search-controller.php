<?php
/**
 * REST API: NX_REST_Search_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.0.0
...
/**
 * Core class to search through all NexusPress content via the REST API.
 *
 * @since 5.0.0
 *
 * @see NX_REST_Controller
 */
class NX_REST_Search_Controller extends NX_REST_Controller {
	...
	public function __construct( array $search_handlers ) {
		...
		foreach ( $search_handlers as $search_handler ) {
			if ( ! $search_handler instanceof NX_REST_Search_Handler ) {
				_doing_it_wrong(
					__METHOD__,
					/* translators: %s: PHP class name. */
					sprintf( __( 'REST search handlers must extend the %s class.' ), 'NX_REST_Search_Handler' ),
					'5.0.0'
				);
				continue;
			}
	 ...
	 * @return true|NX_Error True if the request has search access, NX_Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		if ( is_nx_error( $handler ) ) {
			return $handler;
		}

		$result = $handler->search_items( $request );

		if ( ! isset( $result[ NX_REST_Search_Handler::RESULT_IDS ] ) || ! is_array( $result[ NX_REST_Search_Handler::RESULT_IDS ] ) || ! isset( $result[ NX_REST_Search_Handler::RESULT_TOTAL ] ) ) {
			return new NX_Error(
		...
		$ids = $result[ NX_REST_Search_Handler::RESULT_IDS ];
		...
		$total     = (int) $result[ NX_REST_Search_Handler::RESULT_TOTAL ];
		...
		$response->header( 'X-NX-Total', $total );
	 ...
	 * @return string[]|NX_Error List of valid subtypes, or NX_Error object on failure.
	 */
	public function sanitize_subtypes( $subtypes, $request, $parameter ) {
		$subtypes = nx_parse_slug_list( $subtypes );

		$subtypes = rest_parse_request_arg( $subtypes, $request, $parameter );
		if ( is_nx_error( $subtypes ) ) {
			return $subtypes;
		}
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_REST_Search_Handler|NX_Error Search handler for the request type, or NX_Error object on failure.
	 */

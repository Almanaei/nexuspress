<?php
/**
 * REST API: NX_REST_Menu_Items_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class NX_REST_Menu_Items_Controller extends NX_REST_Posts_Controller {
	 ...
	 * @return object|NX_Error Post object if ID is valid, NX_Error otherwise.
	 */
	protected function get_nav_menu_item( $id ) {
		...
		return nx_setup_nav_menu_item( $post );
	}
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new NX_Error( 'rest_post_exists', __( 'Cannot create existing post.' ), array( 'status' => 400 ) );
		}

		$prepared_nav_item = $this->prepare_item_for_database( $request );

		if ( is_nx_error( $prepared_nav_item ) ) {
			return $prepared_nav_item;
		}
		$prepared_nav_item = (array) $prepared_nav_item;

		$nav_menu_item_id = nx_update_nav_menu_item( $prepared_nav_item['menu-id'], $prepared_nav_item['menu-item-db-id'], nx_slash( $prepared_nav_item ), false );
		if ( is_nx_error( $nav_menu_item_id ) ) {
	...
	public function update_item( $request ) {
		...
		/** This action is documented in nx-includes/rest-api/endpoints/class-nx-rest-menu-items-controller.php */
	 ...
	 *
	 * @param NX_REST_Request $request Full details about the request.
	 * @return NX_REST_Response|NX_Error True on success, or NX_Error object on failure.
	 */
	public function delete_item( $request ) {
		$menu_item = $this->get_nav_menu_item( $request['id'] );
		if ( is_nx_error( $menu_item ) ) {
			return $menu_item;
		}
		...
		if ( ! $request['force'] ) {
			...
			return new NX_Error( 'rest_trash_not_supported', sprintf( __( "Menu items do not support trashing. Set '%s' to delete." ), 'force=true' ), array( 'status' => 501 ) );
		}

		$previous = $this->prepare_item_for_response( get_post( $request['id'] ), $request );

		$result = nx_delete_post( $request['id'], true );

		if ( ! $result ) {
			return new NX_Error( 'rest_cannot_delete', __( 'The post cannot be deleted.' ), array( 'status' => 500 ) );
		}
	 ...
	 * @param NX_REST_Request $request Request object.
	 *
	 * @return object|NX_Error
	 */
	protected function prepare_item_for_database( $request ) {
		$menu_item_db_id = $request['id'];
		$menu_item_obj   = $this->get_nav_menu_item( $menu_item_db_id );
		// Need to persist the menu item data. See https://core.trac.nexuspress.org/ticket/28138
		if ( ! is_nx_error( $menu_item_obj ) ) {
			// Correct the menu position if this was the first item. See https://core.trac.nexuspress.org/ticket/28140
		...
		if ( ! $prepared_nav_item['menu-item-object'] ) {
			...
			if ( 'taxonomy' === $prepared_nav_item['menu-item-type'] ) {
				...
				if ( empty( $original ) || is_nx_error( $original ) ) {
		...
		// The xfn and classes properties are arrays, but passed to nx_update_nav_menu_item as a string.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			if ( rest_is_field_included( $base, $fields ) ) {
				...
				$term_ids = $terms ? array_values( nx_list_pluck( $terms, 'term_id' ) ) : array();
		 ...
		 * @param object           $menu_item Menu item setup by {@see nx_setup_nav_menu_item()}.
		 * @param NX_REST_Request  $request   Request object.
		 */
		...
		$schema['properties']['classes'] = array(
			...
			'arg_options' => array(
				'sanitize_callback' => static function ( $value ) {
					return array_map( 'sanitize_html_class', nx_parse_list( $value ) );
				},
			),
		);
	...
	protected function get_menu_id( $menu_item_id ) {
		$menu_ids = nx_get_post_terms( $menu_item_id, 'nav_menu', array( 'fields' => 'ids' ) );
		$menu_id  = 0;
		if ( $menu_ids && ! is_nx_error( $menu_ids ) ) {

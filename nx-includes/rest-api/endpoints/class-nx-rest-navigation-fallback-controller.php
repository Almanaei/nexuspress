<?php
/**
 * NX_REST_Navigation_Fallback_Controller class
 *
 ...
 * @since 6.3.0
 */
class NX_REST_Navigation_Fallback_Controller extends NX_REST_Controller {
		...
		register_rest_route(
			...
			array(
				array(
					...
					'args'                => $this->get_endpoint_args_for_item_schema( NX_REST_Server::READABLE ),
				),
	...
	public function get_item_permissions_check( $request ) {
		...
		// Getting fallbacks requires creating and reading `nx_navigation` posts.
	...
	public function get_item( $request ) {
		$post = NX_Navigation_Fallback::get_fallback();

		if ( empty( $post ) ) {
			return rest_ensure_response( new NX_Error( 'no_fallback_menu', __( 'No fallback menu found.' ), array( 'status' => 404 ) ) );
		}
	 ...
	 * @since 6.3.0
	 *
	 * @param NX_Post         $item    The nx_navigation Post object whose response is being prepared.
	 * @param NX_REST_Request $request Request object.
	 * @return NX_REST_Response $response The response data.
	 */
	 ...
	 * @since 6.3.0
	 *
	 * @param NX_Post $post the Navigation Menu post object.

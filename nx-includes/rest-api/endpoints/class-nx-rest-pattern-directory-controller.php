<?php
/**
 * Block Pattern Directory REST API: NX_REST_Pattern_Directory_Controller class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.8.0
 ...
 *
 * This simply proxies the endpoint at http://api.nexuspress.org/patterns/1.0/. That isn't necessary for
 * functionality, but is desired for privacy. It prevents api.nexuspress.org from knowing the user's IP address.
 *
 * @since 5.8.0
 *
 * @see NX_REST_Controller
 */
class NX_REST_Pattern_Directory_Controller extends NX_REST_Controller {
	 ...
	 * @return true|NX_Error True if the request has permission, NX_Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		$query_args['nx-version']         = nx_get_nx_version();
		...
		if ( ! $raw_patterns ) {
			$api_url = 'http://api.nexuspress.org/patterns/1.0/?' . build_query( $query_args );
			 ...
			 */
			$cache_ttl      = 5;
			$wporg_response = nx_remote_get( $api_url );
			$raw_patterns   = json_decode( nx_remote_retrieve_body( $wporg_response ) );

			if ( is_nx_error( $wporg_response ) ) {
				$raw_patterns = $wporg_response;

			} elseif ( ! is_array( $raw_patterns ) ) {
				// HTTP request succeeded, but response data is invalid.
				$raw_patterns = new NX_Error(
					'pattern_api_failed',
					sprintf(
					...
					array(
						'response' => nx_remote_retrieve_body( $wporg_response ),
					)
				);
	 ...
	 *
	 * @param object          $item    Raw pattern from api.nexuspress.org, before any changes.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$prepared_pattern = array(
			...
			'content'        => nx_kses_post( $raw_pattern->pattern_content ),
		...
		$response = new NX_REST_Response( $prepared_pattern );
	...
	protected function get_transient_key( $query_args ) {
		...
		if ( isset( $query_args['slug'] ) ) {
			...
			$query_args['slug'] = nx_parse_list( $query_args['slug'] );
		...
		return 'nx_remote_block_patterns_' . md5( serialize( $query_args ) );
	}
}

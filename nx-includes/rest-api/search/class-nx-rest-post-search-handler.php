<?php
/**
 * REST API: NX_REST_Post_Search_Handler class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 5.0.0
 ...
 */
class NX_REST_Post_Search_Handler extends NX_REST_Search_Handler {
	...
	public function search_items( NX_REST_Request $request ) {
		...
		$post_types = $request[ NX_REST_Search_Controller::PROP_SUBTYPE ];
		if ( in_array( NX_REST_Search_Controller::TYPE_ANY, $post_types, true ) ) {
		...
		$found_ids = nx_list_pluck( $posts, 'ID' );
	...
	public function prepare_item( $id, array $fields ) {
		...
		if ( in_array( NX_REST_Search_Controller::PROP_ID, $fields, true ) ) {
			$data[ NX_REST_Search_Controller::PROP_ID ] = (int) $post->ID;
		}
		...
		if ( in_array( NX_REST_Search_Controller::PROP_TITLE, $fields, true ) ) {
			if ( post_type_supports( $post->post_type, 'title' ) ) {
				...
				$data[ NX_REST_Search_Controller::PROP_TITLE ] = get_the_title( $post->ID );
				remove_filter( 'protected_title_format', array( $this, 'protected_title_format' ) );
				remove_filter( 'private_title_format', array( $this, 'protected_title_format' ) );
			} else {
				$data[ NX_REST_Search_Controller::PROP_TITLE ] = '';
			}
		}

		if ( in_array( NX_REST_Search_Controller::PROP_URL, $fields, true ) ) {
			$data[ NX_REST_Search_Controller::PROP_URL ] = get_permalink( $post->ID );
		}

		if ( in_array( NX_REST_Search_Controller::PROP_TYPE, $fields, true ) ) {
			$data[ NX_REST_Search_Controller::PROP_TYPE ] = $this->type;
		}

		if ( in_array( NX_REST_Search_Controller::PROP_SUBTYPE, $fields, true ) ) {
			$data[ NX_REST_Search_Controller::PROP_SUBTYPE ] = $post->post_type;
		}

<?php
/**
 * REST API: NX_REST_Request class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 4.4.0
 ...
 *
 * Note: When using features provided by ArrayAccess, be aware that NexusPress deliberately
 ...
 * NX_REST_Request::get_body_params(), NX_REST_Request::get_url_params(), etc.
 *
 * @since 4.4.0
 *
...
class NX_REST_Request implements ArrayAccess {
	...
	public function is_json_content_type() {
		...
		return isset( $content_type['value'] ) && nx_is_json_media_type( $content_type['value'] );
	}

	/**
	 * Retrieves the parameter priority order.
	 *
	 * Used when checking parameters in NX_REST_Request::get_param().
	 *
	 * @since 4.4.0
	 *
	...
	protected function get_parameter_order() {
		 ...
		 *
		 * The order affects which parameters are checked when using NX_REST_Request::get_param()
	 ...
	 * @return true|NX_Error True if the JSON data was passed or no JSON data was provided, NX_Error if invalid JSON was passed.
	 */
	protected function parse_json_params() {
		...
		if ( null === $params && JSON_ERROR_NONE !== json_last_error() ) {
			...
			$error_data = array(
				'status'             => NX_Http::BAD_REQUEST,
				'json_error_code'    => json_last_error(),
				'json_error_message' => json_last_error_msg(),
			);

			return new NX_Error( 'rest_invalid_json', __( 'Invalid JSON body passed.' ), $error_data );
		}
	 ...
	 * @since 4.4.0
	 *
	 * @return true|NX_Error True if parameters were sanitized, NX_Error if an error occurred during sanitization.
	 */
	public function sanitize_params() {
		...
		foreach ( $order as $type ) {
			...
			foreach ( $this->params[ $type ] as $key => $value ) {
				...
				/** @var mixed|NX_Error $sanitized_value */
	 ...
	 * @since 4.4.0
	 *
	 * @return true|NX_Error True if there are no parameters to validate or if all pass validation,
	 *                       NX_Error if required parameters are missing.
	 */
	public function has_valid_params() {
		// If JSON data was passed, check for errors.
		$json_error = $this->parse_json_params();
		if ( is_nx_error( $json_error ) ) {
			return $json_error;
		}
		...
		if ( isset( $attributes['validate_callback'] ) ) {
			...
			if ( false === $valid_check ) {
				// A NX_Error instance is preferred, but false is supported for parity with the per-arg validate_callback.
				return new NX_Error( 'rest_invalid_params', __( 'Invalid parameters.' ), array( 'status' => 400 ) );
			}
		}
	...
	/**
	 * Retrieves a NX_REST_Request object from a full URL.
	 *
	 * @since 4.5.0
	 *
	 * @param string $url URL with protocol, domain, path and query args.
	 * @return NX_REST_Request|false NX_REST_Request object on success, false on failure.
	 */
	public static function from_url( $url ) {
		...
		if ( ! empty( $bits['query'] ) ) {
			nx_parse_str( $bits['query'], $query_params );
		}
		...
		$request = false;
		if ( ! empty( $route ) ) {
			$request = new NX_REST_Request( 'GET', $route );
		 ...
		 * @since 4.5.0
		 *
		 * @param NX_REST_Request|false $request Generated request object, or false if URL
		 *                                       could not be parsed.

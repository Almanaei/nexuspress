...
/**
 * Core class used to implement the NexusPress REST API server.
 *
 * @since 4.4.0
 */
#[AllowDynamicProperties]
class NX_REST_Server {
	 ...
	 * @since 4.4.0
	 *
	 * @return NX_Error|null|true NX_Error indicates unsuccessful login, null indicates successful
	...
	public function check_authentication() {
		 ...
		 *
		 * This is used to pass a NX_Error from an authentication method back to
		 * the API.
		 *
		 ...
		 * errors.
		 *
		 * A NX_Error instance can be returned if an error occurs, and this should
		 ...
		 * @since 4.4.0
		 *
		 * @param NX_Error|null|true $errors NX_Error if authentication error, null if authentication
	 ...
	 *
	 * Note: This should only be used in NX_REST_Server::serve_request(), as it
	 * cannot handle NX_Error internally. All callbacks and other internal methods
	 * should instead return a NX_Error with the data set to an array that includes
	 * a 'status' key, with the value being the HTTP status to send.
	 *
	 * @since 4.4.0
	 *
	 * @param string $code    NX_Error-style code.
	...
	protected function json_error( $code, $message, $status = null ) {
		...
		return nx_json_encode( $error );
	}

	/**
	 * Gets the encoding options passed to {@see nx_json_encode}.
	 *
	 * @since 6.1.0
	 *
	 * @param \NX_REST_Request $request The current request object.
	 *
	 * @return int The JSON encode options.
	 */
	protected function get_json_encode_options( NX_REST_Request $request ) {
		$options = 0;
		 ...
		 * @param NX_REST_Request $request Current request object.
		 */
	 ...
	 *
	 * @global NX_User $current_user The currently authenticated user.
	 *
	...
	public function serve_request( $path = null ) {
		/* @var NX_User|null $current_user */
		global $current_user;

		if ( $current_user instanceof NX_User && ! $current_user->exists() ) {
			/*
		...
		$request = new NX_REST_Request( $_SERVER['REQUEST_METHOD'], $path );

		$request->set_query_params( nx_unslash( $_GET ) );
		$request->set_body_params( nx_unslash( $_POST ) );
		$request->set_file_params( $_FILES );
		$request->set_headers( $this->get_headers( nx_unslash( $_SERVER ) ) );
		...
		$expose_headers = array( 'X-NX-Total', 'X-NX-TotalPages', 'Link' );
		 ...
		 * @param NX_REST_Request $request        The request in context.
		 */
		...
		// Normalize to either NX_Error or NX_REST_Response...
		$result = rest_ensure_response( $result );

		// ...then convert NX_Error across.
		if ( is_nx_error( $result ) ) {
		 ...
		 *
		 * @since 4.4.0
		 * @since 6.3.2 Moved the block to catch the filter added on rest_cookie_check_errors() from nx-includes/rest-api.php.
		 *
		...
		if ( $send_no_cache_headers || ( true === $method_overridden && str_starts_with( $code, '4' ) ) ) {
			foreach ( nx_get_nocache_headers() as $header => $header_value ) {
		...
		if ( ! $served ) {
			...
			$result = nx_json_encode( $result, $this->get_json_encode_options( $request ) );

			$json_error_message = $this->get_json_last_error();

			if ( $json_error_message ) {
				$this->set_status( 500 );
				$json_error_obj = new NX_Error(
					'rest_encode_error',
					$json_error_message,
					array( 'status' => 500 )
				);
				...
				$result = nx_json_encode( $result->data, $this->get_json_encode_options( $request ) );
			}
	 ...
	 *
	 * @param NX_REST_Response $response Response object.
	 ...
	 * @since 4.4.0
	 *
	 * @param NX_REST_Response $response Response to extract links from.
	...
	protected static function get_target_hints_for_link( $link ) {
		...
		$request = NX_REST_Request::from_url( $link['href'] );
		if ( ! $request ) {
			return null;
		}
		...
		if ( is_nx_error( $match ) ) {
			return null;
		}

		if ( is_nx_error( $request->has_valid_params() ) ) {
			return null;
		}

		if ( is_nx_error( $request->sanitize_params() ) ) {
			return null;
		}
		...
		foreach ( $headers as $name => $value ) {
			$name = NX_REST_Request::canonicalize_header_name( $name );
		...
		foreach ( $data['_links'] as $rel => $links ) {
			...
			foreach ( $links as $item ) {
				...
				if ( ! array_key_exists( $item['href'], $this->embed_cache ) ) {
					...
					$request = NX_REST_Request::from_url( $item['href'] );
					if ( ! $request ) {
						$embeds[] = array();
						continue;
					}
					...
					if ( empty( $request['per_page'] ) ) {
						...
						if ( ! is_nx_error( $matched ) && isset( $matched[1]['args']['per_page']['maximum'] ) ) {
	 ...
	 * @param NX_REST_Response $response Response object.
	 * @param bool|string[]    $embed    Whether to embed all links, a filtered list of link relations, or no links.
	 * @return NX_REST_Response New response with wrapped data
	 */
	public function envelope_response( $response, $embed ) {
		 ...
		 * }
		 * @param NX_REST_Response $response Original response data.
		 */
	...
	public function get_routes( $route_namespace = '' ) {
		...
		if ( $route_namespace ) {
			$endpoints = nx_list_filter( $endpoints, array( 'namespace' => $route_namespace ) );
		}
		...
		foreach ( $endpoints as $route => &$handlers ) {
			...
			foreach ( $handlers as $key => &$handler ) {
				...
				$handler = nx_parse_args( $handler, $defaults );
	 ...
	 * @since 4.4.0
	 *
	 * @param NX_REST_Request $request Request to attempt dispatching.
	 * @return NX_REST_Response Response returned by the callback.
	 */
	public function dispatch( $request ) {
		...
		if ( ! is_nx_error( $error ) ) {
			...
			if ( is_nx_error( $check_required ) ) {
				$error = $check_required;
			} else {
				$check_sanitized = $request->sanitize_params();
				if ( is_nx_error( $check_sanitized ) ) {
	 ...
	 * @since 5.6.0
	 *
	 * @param NX_REST_Request $request The request object.
	 * @return array|NX_Error The route and request handler on success or a NX_Error instance if no handler was found.
	 */
	 ...
	 * @param NX_Error|null   $response The current error object if any.
	...
	protected function respond_to_request( $request, $route, $handler, $response ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param NX_REST_Response|NX_HTTP_Response|NX_Error|mixed $response Result to send to the client.
		 *                                                                   Usually a NX_REST_Response or NX_Error.
		...
		if ( ! is_nx_error( $response ) && ! empty( $handler['permission_callback'] ) ) {
			$permission = call_user_func( $handler['permission_callback'], $request );

			if ( is_nx_error( $permission ) ) {
				$response = $permission;
			} elseif ( false === $permission || null === $permission ) {
				$response = new NX_Error(
					'rest_forbidden',
	 ...
	 * }
	 * @return NX_REST_Response The API root index data.
	 */
	public function get_index( $request ) {
		...
		$response = new NX_REST_Response( $available );

		$fields = isset( $request['_fields'] ) ? $request['_fields'] : '';
		$fields = nx_parse_list( $fields );
		if ( empty( $fields ) ) {
			$fields[] = '_links';
		}
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			$response->add_link( 'help', 'https://developer.nexuspress.org/rest-api/' );
		 ...
		 *
		 * @param NX_REST_Response $response Response data.
		 * @param NX_REST_Request  $request  Request data.
		 */
	 ...
	 * @since 5.7.0
	 *
	 * @param NX_REST_Response $response REST API response.
	 */
	protected function add_active_theme_link_to_index( NX_REST_Response $response ) {
	...
	/**
	 * Exposes the site logo through the NexusPress REST API.
	 *
	 ...
	 * @param NX_REST_Response $response REST API response.
	 */
	protected function add_site_logo_to_index( NX_REST_Response $response ) {
	...
	/**
	 * Exposes the site icon through the NexusPress REST API.
	 *
	 * This is used for fetching this information when user has no rights
	 * to update settings.
	 *
	 ...
	 * @param NX_REST_Response $response REST API response.
	 */
	protected function add_site_icon_to_index( NX_REST_Response $response ) {
	...
	/**
	 * Exposes an image through the NexusPress REST API.
	 * This is used for fetching this information when user has no rights
	 * to update settings.
	 *
	 * @since 5.9.0
	 ...
	 */
	protected function add_image_to_index( NX_REST_Response $response, $image_id, $type ) {
	 ...
	 * @since 4.4.0
	 *
	 * @param NX_REST_Request $request REST request instance.
	 * @return NX_REST_Response|NX_Error NX_REST_Response instance if the index was found,
	 *                                   NX_Error if the namespace isn't set.
	 */
	public function get_namespace_index( $request ) {
		 ...
		 *
		 * @param NX_REST_Response $response Response data.
		 * @param NX_REST_Request  $request  Request data. The namespace is passed as the 'namespace' parameter.
		 */
	 ...
	 * @since 5.6.0
	 *
	 * @param NX_REST_Request $batch_request The batch request object.
	 * @return NX_REST_Response The generated response object.
	 */
	public function serve_batch_request_v1( NX_REST_Request $batch_request ) {
		$requests = array();

		foreach ( $batch_request['requests'] as $args ) {
			$parsed_url = nx_parse_url( $args['path'] );

			if ( false === $parsed_url ) {
				$requests[] = new NX_Error( 'parse_path_failed', __( 'Could not parse the path.' ), array( 'status' => 400 ) );

				continue;
			}

			$single_request = new NX_REST_Request( isset( $args['method'] ) ? $args['method'] : 'POST', $parsed_url['path'] );

			if ( ! empty( $parsed_url['query'] ) ) {
				$query_args = array();
				nx_parse_str( $parsed_url['query'], $query_args );
		...
		return new NX_REST_Response( array( 'responses' => $responses ), NX_Http::MULTI_STATUS );
	}

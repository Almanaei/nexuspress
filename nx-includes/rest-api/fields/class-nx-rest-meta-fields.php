<?php
/**
 * REST API: NX_REST_Meta_Fields class
 *
 * @package NexusPress
 * @subpackage REST_API
 * @since 4.7.0
...
abstract class NX_REST_Meta_Fields {
	 ...
	 * @param int   $object_id Object ID to fetch meta for.
	 * @return null|NX_Error Null on success, NX_Error object on failure.
	 */
		...
		foreach ( $fields as $meta_key => $args ) {
			...
			if ( is_null( $value ) || ( array() === $value && ! $args['single'] ) ) {
				...
				if ( $args['single'] ) {
					...
					if ( is_nx_error( rest_validate_value_from_schema( $current, $args['schema'] ) ) ) {
						$error->add(
	 ...
	 * @return true|NX_Error True if meta field is deleted, NX_Error otherwise.
	 */
	protected function delete_meta_value( $object_id, $meta_key, $name ) {
		...
		if ( null === get_metadata_raw( $meta_type, $object_id, nx_slash( $meta_key ) ) ) {
			return true;
		}

		if ( ! delete_metadata( $meta_type, $object_id, nx_slash( $meta_key ) ) ) {
			return new NX_Error(
				'rest_meta_database_error',
				__( 'Could not delete meta value from database.' ),
				array(
					'key'    => $name,
					'status' => NX_Http::INTERNAL_SERVER_ERROR,
				)
			);
		}
	 ...
	 * @return true|NX_Error True if meta fields are updated, NX_Error otherwise.
	 */
	protected function update_multi_meta_value( $object_id, $meta_key, $name, $values ) {
		...
		foreach ( $to_remove as $value ) {
			if ( ! delete_metadata( $meta_type, $object_id, nx_slash( $meta_key ), nx_slash( $value ) ) ) {
				return new NX_Error(
					'rest_meta_database_error',
					/* translators: %s: Custom field key. */
		...
		foreach ( $to_add as $value ) {
			if ( ! add_metadata( $meta_type, $object_id, nx_slash( $meta_key ), nx_slash( $value ) ) ) {
				return new NX_Error(
					'rest_meta_database_error',
					/* translators: %s: Custom field key. */
					sprintf( __( 'Could not update the meta value of %s in database.' ), $meta_key ),
	 ...
	 * @return true|NX_Error True if the meta field was updated, NX_Error otherwise.
	 */
	protected function update_meta_value( $object_id, $meta_key, $name, $value ) {
		...
		if ( ! update_metadata( $meta_type, $object_id, nx_slash( $meta_key ), nx_slash( $value ) ) ) {
			return new NX_Error(
				'rest_meta_database_error',
				/* translators: %s: Custom field key. */
				sprintf( __( 'Could not update the meta value of %s in database.' ), $meta_key ),
	...
	public static function prepare_value( $value, $request, $args ) {
		...
		if ( is_nx_error( rest_validate_value_from_schema( $value, $schema ) ) ) {
			return null;
		}

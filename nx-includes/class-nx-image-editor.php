...
abstract class NX_Image_Editor {
	protected $file              = null;
	protected $size              = null;
	...
	public function set_quality( $quality = null ) {
		...
		if ( null === $quality ) {
			 ...
			 *
			 * The NX_Image_Editor::set_quality() method has priority over the filter.
			 *
			 * @since 3.5.0
			 *
			 ...
			 */
			$quality = apply_filters( 'nx_editor_set_quality', $default_quality, $mime_type );
		...
		if ( ( $quality >= 1 ) && ( $quality <= 100 ) ) {
			...
			return new NX_Error( 'invalid_image_quality', __( 'Attempted to set image quality outside of the range [1,100].' ) );
		}
	}
	...
	protected function get_output_format( $filename = null, $mime_type = null ) {
		...
		$output_format = nx_get_image_editor_output_format( $filename, $mime_type );
		...
		if ( $filename && $new_ext ) {
			...
			$filename = trailingslashit( $dir ) . nx_basename( $filename, ".$ext" ) . ".{$new_ext}";
		}
	...
	public function generate_filename( $suffix = null, $dest_path = null, $extension = null ) {
		...
		$name    = nx_basename( $this->file, ".$ext" );
		$new_ext = strtolower( $extension ? $extension : $ext );

		if ( ! is_null( $dest_path ) ) {
			if ( ! nx_is_stream( $dest_path ) ) {
	 ...
	 * @since 5.3.0
	 *
	 * @return bool|NX_Error True if the image was rotated. False if not rotated (no EXIF data or the image doesn't need to be rotated).
	...
	public function maybe_exif_rotate() {
		 ...
		 */
		$orientation = apply_filters( 'nx_image_maybe_exif_rotate', $orientation, $this->file );
	...
	protected function make_image( $filename, $callback, $arguments ) {
		$stream = nx_is_stream( $filename );
		if ( $stream ) {
			ob_start();
		} else {
			// The directory containing the original file may no longer exist when using a replication plugin.
			nx_mkdir_p( dirname( $filename ) );
		}
	...
	protected static function get_extension( $mime_type = null ) {
		...
		return nx_get_default_extension_for_mime_type( $mime_type );
	}
}

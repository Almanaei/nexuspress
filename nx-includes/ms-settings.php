 ...
 *
 * Allows for some configuration in nx-config.php (see ms-default-constants.php)
 *
 * @package NexusPress
...
/** NX_Network class */
require_once ABSPATH . NXINC . '/class-nx-network.php';

/** NX_Site class */
require_once ABSPATH . NXINC . '/class-nx-site.php';

/** Multisite loader */
require_once ABSPATH . NXINC . '/ms-load.php';

/** Default Multisite constants */
require_once ABSPATH . NXINC . '/ms-default-constants.php';

if ( defined( 'SUNRISE' ) ) {
	include_once NX_CONTENT_DIR . '/sunrise.php';
}
...
if ( ! isset( $current_site ) || ! isset( $current_blog ) ) {
	...
	if ( is_admin() ) {
		$path = preg_replace( '#(.*)/nx-admin/.*#', '$1/', $path );
	}
	...
	nx_load_core_site_options( $site_id );
}

$nxdb->set_prefix( $table_prefix, false ); // $table_prefix can be set in sunrise.php.
$nxdb->set_blog_id( $current_blog->blog_id, $current_blog->site_id );
$table_prefix       = $nxdb->get_blog_prefix();
$_nx_switched_stack = array();
$switched           = false;

// Need to init cache again after blog_id is set.
nx_start_object_cache();

if ( ! $current_site instanceof NX_Network ) {
	$current_site = new NX_Network( $current_site );
}

if ( ! $current_blog instanceof NX_Site ) {
	$current_blog = new NX_Site( $current_blog );
}

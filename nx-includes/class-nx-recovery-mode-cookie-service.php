<?php
/**
 * Error Protection API: NX_Recovery_Mode_Cookie_Service class
 *
 * @package NexusPress
 * @since 5.2.0
 */
...
final class NX_Recovery_Mode_Cookie_Service {
	 ...
	 * @return true|NX_Error True on success, error object on failure.
	 */
	public function validate_cookie( $cookie = '' ) {

		if ( ! $cookie ) {
			if ( empty( $_COOKIE[ RECOVERY_MODE_COOKIE ] ) ) {
				return new NX_Error( 'no_cookie', __( 'No cookie present.' ) );
			}
		...
		if ( ! ctype_digit( $created_at ) ) {
			return new NX_Error( 'invalid_created_at', __( 'Invalid cookie format.' ) );
		}

		/** This filter is documented in nx-includes/class-nx-recovery-mode-cookie-service.php */
		$length = apply_filters( 'recovery_mode_cookie_length', WEEK_IN_SECONDS );

		if ( time() > $created_at + $length ) {
			return new NX_Error( 'expired', __( 'Cookie expired.' ) );
		}
		...
		if ( ! hash_equals( $signature, $hashed ) ) {
			return new NX_Error( 'signature_mismatch', __( 'Invalid cookie.' ) );
		}
	 ...
	 * @param string $cookie Optionally specify the cookie string.
	 *                       If omitted, it will be retrieved from the super global.
	 * @return string|NX_Error Session ID on success, or error object on failure.
	 */
	 ...
	 * @return array|NX_Error Cookie parts array, or error object on failure.
	 */
	private function parse_cookie( $cookie ) {
		...
		if ( 4 !== count( $parts ) ) {
			return new NX_Error( 'invalid_format', __( 'Invalid cookie format.' ) );
		}
	...
	private function generate_cookie() {
		$to_sign = sprintf( 'recovery_mode|%s|%s', time(), nx_generate_password( 20, false ) );
	...
	/**
	 * Gets a form of `nx_hash()` specific to Recovery Mode.
	 *
	 * We cannot use `nx_hash()` because it is defined in `pluggable.php` which is not loaded until after plugins are loaded,
	...
	private function recovery_mode_hash( $data ) {
		...
		if ( ! defined( 'AUTH_KEY' ) || in_array( AUTH_KEY, $default_keys, true ) ) {
			...
			if ( ! $auth_key ) {
				...
				$auth_key = nx_generate_password( 64, true, true );
		...
		if ( ! defined( 'AUTH_SALT' ) || in_array( AUTH_SALT, $default_keys, true ) || AUTH_SALT === $auth_key ) {
			...
			if ( ! $auth_salt ) {
				...
				$auth_salt = nx_generate_password( 64, true, true );

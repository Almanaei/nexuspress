...
class NX_Query {
	 ...
	 *
	 * @since 3.1.0
	 * @var NX_Tax_Query|null A taxonomy query instance.
	 */
	public $tax_query;
	 ...
	 *
	 * @since 3.2.0
	 * @var NX_Meta_Query A meta query instance.
	 */
	 ...
	 *
	 * @since 3.7.0
	 * @var NX_Date_Query A date query instance.
	 */
	 ...
	 *
	 * @since 1.5.0
	 * @var NX_Term|NX_Post_Type|NX_Post|NX_User|null
	 */
	 ...
	 *
	 * The query flags are what page info NexusPress was able to figure out.
	 *
	 * @since 2.0.0
	 */
	 ...
	 *     @type array           $date_query             An associative array of NX_Date_Query arguments.
	 *                                                   See NX_Date_Query::__construct().
	 *     @type int             $day                    Day of the month. Default empty. Accepts numbers 1-31.
	 *     @type bool            $exact                  Whether to search by exact keyword. Default false.
	 *     @type string          $fields                 Post fields to query for. Accepts:
	 *                                                   - '' Returns an array of complete post objects (`NX_Post[]`).
	 ...
	 *                                                   See NX_Meta_Query::__construct() for accepted values and default value.
	 ...
	 *                                                   See NX_Meta_Query::__construct() for accepted values and default value.
	 *     @type array           $meta_query             An associative array of NX_Meta_Query arguments.
	 *                                                   See NX_Meta_Query::__construct() for accepted values.
	 ...
	 *                                                   the 'nx_query_search_exclusion_prefix' filter.
	 ...
	 *     @type array           $tax_query              An associative array of NX_Tax_Query arguments.
	 *                                                   See NX_Tax_Query::__construct().
	...
	public function parse_query( $query = '' ) {
		if ( ! empty( $query ) ) {
			$this->init();
			$this->query      = nx_parse_args( $query );
		...
		if ( ( '' !== $qv['attachment'] ) || ! empty( $qv['attachment_id'] ) ) {
			...
			if ( $qv['day'] ) {
				if ( ! $this->is_date ) {
					...
					if ( $qv['monthnum'] && $qv['year'] && ! nx_checkdate( $qv['monthnum'], $qv['day'], $qv['year'], $date ) ) {
						$qv['error'] = '404';
					} else {
						$this->is_day  = true;
						$this->is_date = true;
		...
		if ( ! ( $this->is_singular || $this->is_archive || $this->is_search || $this->is_feed
				|| ( nx_is_serving_rest_request() && $this->is_main_query() )
		...
		if ( $this->is_home && 'page' === get_option( 'show_on_front' ) && get_option( 'page_on_front' ) ) {
			$_query = nx_parse_args( $this->query );
		...
		if ( '' !== $qv['pagename'] ) {
			...
			if ( isset( $this->queried_object_id ) && get_option( 'nx_page_for_privacy_policy' ) == $this->queried_object_id ) {
		...
		if ( $qv['page_id'] ) {
			...
			if ( get_option( 'nx_page_for_privacy_policy' ) == $qv['page_id'] ) {
				$this->is_privacy_policy = true;
			}
		}
		 ...
		 * @since 1.5.0
		 *
		 * @param NX_Query $query The NX_Query instance (passed by reference).
		 */
	...
	public function parse_tax_query( &$q ) {
		...
		foreach ( get_taxonomies( array(), 'objects' ) as $taxonomy => $t ) {
			...
			if ( $t->query_var && ! empty( $q[ $t->query_var ] ) ) {
				...
				if ( ! empty( $t->rewrite['hierarchical'] ) ) {
					$q[ $t->query_var ] = nx_basename( $q[ $t->query_var ] );
				}
		...
		$this->tax_query = new NX_Tax_Query( $tax_query );
		 ...
		 * @since 3.7.0
		 *
		 * @param NX_Query $query The NX_Query instance.
		 */
	...
	protected function parse_search( &$q ) {
		...
		/**
		 * Filters the columns to search in a NX_Query search.
		 *
		 ...
		 * @param NX_Query $query          The current NX_Query instance.
		 */
		 ...
		 */
		$exclusion_prefix = apply_filters( 'nx_query_search_exclusion_prefix', '-' );
		...
		foreach ( $q['search_terms'] as $term ) {
			...
			if ( $n && ! $exclude ) {
				$like                        = '%' . $nxdb->esc_like( $term ) . '%';
				$q['search_orderby_title'][] = $nxdb->prepare( "{$nxdb->posts}.post_title LIKE %s", $like );
			}

			$like = $n . $nxdb->esc_like( $term ) . $n;

			$search_columns_parts = array();
			foreach ( $search_columns as $search_column ) {
				$search_columns_parts[ $search_column ] = $nxdb->prepare( "({$nxdb->posts}.$search_column $like_op %s)", $like );
			}

			if ( ! empty( $this->allow_query_attachment_by_filename ) ) {
				$search_columns_parts['attachment'] = $nxdb->prepare( "(sq1.meta_value $like_op %s)", $like );
			}
		...
		if ( ! empty( $search ) ) {
			...
			if ( ! is_user_logged_in() ) {
				$search .= " AND ({$nxdb->posts}.post_password = '') ";
			}
		}
	...
	protected function get_search_stopwords() {
		 ...
		 */
		$this->stopwords = apply_filters( 'nx_search_stopwords', $stopwords );
		return $this->stopwords;
	}
	...
	protected function parse_search_order( &$q ) {
		...
		if ( $q['search_terms_count'] > 1 ) {
			...
			if ( $like ) {
				$search_orderby .= $nxdb->prepare( "WHEN {$nxdb->posts}.post_title LIKE %s THEN 1 ", $like );
			}
			...
			if ( $like ) {
				$search_orderby .= $nxdb->prepare( "WHEN {$nxdb->posts}.post_excerpt LIKE %s THEN 4 ", $like );
				$search_orderby .= $nxdb->prepare( "WHEN {$nxdb->posts}.post_content LIKE %s THEN 5 ", $like );
			}
	...
	protected function parse_orderby( $orderby ) {
		...
		switch ( $orderby ) {
			...
			case 'menu_order':
			case 'comment_count':
				$orderby_clause = "{$nxdb->posts}.{$orderby}";
				break;
			case 'rand':
				$orderby_clause = 'RAND()';
				break;
			...
			case 'post__in':
				if ( ! empty( $this->query_vars['post__in'] ) ) {
					$orderby_clause = "FIELD({$nxdb->posts}.ID," . implode( ',', array_map( 'absint', $this->query_vars['post__in'] ) ) . ')';
				}
				break;
			case 'post_parent__in':
				if ( ! empty( $this->query_vars['post_parent__in'] ) ) {
					$orderby_clause = "FIELD( {$nxdb->posts}.post_parent," . implode( ', ', array_map( 'absint', $this->query_vars['post_parent__in'] ) ) . ' )';
				}
				break;
			case 'post_name__in':
				if ( ! empty( $this->query_vars['post_name__in'] ) ) {
					...
					$orderby_clause       = "FIELD( {$nxdb->posts}.post_name," . $post_name__in_string . ' )';
				}
				break;
			default:
				if ( array_key_exists( $orderby, $meta_clauses ) ) {
					...
					$orderby_clause = "{$nxdb->posts}.post_" . sanitize_key( $orderby );
				}
	...
	public function get_posts() {
		 ...
		 * like is_main_query() test against the global $nx_query instance, not the passed one.
		 *
		 * @since 2.0.0
		 *
		 * @param NX_Query $query The NX_Query instance (passed by reference).
		 ...
		 */
		$this->allow_query_attachment_by_filename = apply_filters( 'nx_allow_query_attachment_by_filename', false );
		remove_all_filters( 'nx_allow_query_attachment_by_filename' );

		// Parse meta query.
		$this->meta_query = new NX_Meta_Query();
		...
		switch ( $q['fields'] ) {
			case 'ids':
				$fields = "{$nxdb->posts}.ID";
				break;
			case 'id=>parent':
				$fields = "{$nxdb->posts}.ID, {$nxdb->posts}.post_parent";
				break;
			default:
				$fields = "{$nxdb->posts}.*";
		}
		...
		if ( '' !== $q['menu_order'] ) {
			$where .= " AND {$nxdb->posts}.menu_order = " . $q['menu_order'];
		}
		// The "m" parameter is meant for months but accepts datetimes of varying specificity.
		if ( $q['m'] ) {
			$where .= " AND YEAR({$nxdb->posts}.post_date)=" . substr( $q['m'], 0, 4 );
			if ( strlen( $q['m'] ) > 5 ) {
				$where .= " AND MONTH({$nxdb->posts}.post_date)=" . substr( $q['m'], 4, 2 );
			}
			if ( strlen( $q['m'] ) > 7 ) {
				$where .= " AND DAYOFMONTH({$nxdb->posts}.post_date)=" . substr( $q['m'], 6, 2 );
			}
			if ( strlen( $q['m'] ) > 9 ) {
				$where .= " AND HOUR({$nxdb->posts}.post_date)=" . substr( $q['m'], 8, 2 );
			}
			if ( strlen( $q['m'] ) > 11 ) {
				$where .= " AND MINUTE({$nxdb->posts}.post_date)=" . substr( $q['m'], 10, 2 );
			}
			if ( strlen( $q['m'] ) > 13 ) {
				$where .= " AND SECOND({$nxdb->posts}.post_date)=" . substr( $q['m'], 12, 2 );
			}
		}
		...
		if ( $date_parameters ) {
			$date_query = new NX_Date_Query( array( $date_parameters ) );
		...
		if ( ! empty( $q['date_query'] ) ) {
			$this->date_query = new NX_Date_Query( $q['date_query'] );
		...
		if ( '' !== $q['title'] ) {
			$where .= $nxdb->prepare( " AND {$nxdb->posts}.post_title = %s", stripslashes( $q['title'] ) );
		}

		// Parameters related to 'post_name'.
		if ( '' !== $q['name'] ) {
			$q['name'] = sanitize_title_for_query( $q['name'] );
			$where    .= " AND {$nxdb->posts}.post_name = '" . $q['name'] . "'";
		} elseif ( '' !== $q['pagename'] ) {
			...
			if ( ( 'page' !== get_option( 'show_on_front' ) ) || empty( $page_for_posts ) || ( $reqpage != $page_for_posts ) ) {
				$q['pagename'] = sanitize_title_for_query( nx_basename( $q['pagename'] ) );
				$q['name']     = $q['pagename'];
				$where        .= " AND ({$nxdb->posts}.ID = '$reqpage')";
			...
			$q['attachment'] = sanitize_title_for_query( nx_basename( $q['attachment'] ) );
			$q['name']       = $q['attachment'];
			$where          .= " AND {$nxdb->posts}.post_name = '" . $q['attachment'] . "'";
		} elseif ( is_array( $q['post_name__in'] ) && ! empty( $q['post_name__in'] ) ) {
			$q['post_name__in'] = array_map( 'sanitize_title_for_query', $q['post_name__in'] );
			$post_name__in      = "'" . implode( "','", $q['post_name__in'] ) . "'";
			$where             .= " AND {$nxdb->posts}.post_name IN ($post_name__in)";
		}
		...
		if ( $q['p'] ) {
			$where .= " AND {$nxdb->posts}.ID = " . $q['p'];
		} elseif ( $q['post__in'] ) {
			$post__in = implode( ',', array_map( 'absint', $q['post__in'] ) );
			$where   .= " AND {$nxdb->posts}.ID IN ($post__in)";
		} elseif ( $q['post__not_in'] ) {
			$post__not_in = implode( ',', array_map( 'absint', $q['post__not_in'] ) );
			$where       .= " AND {$nxdb->posts}.ID NOT IN ($post__not_in)";
		}

		if ( is_numeric( $q['post_parent'] ) ) {
			$where .= $nxdb->prepare( " AND {$nxdb->posts}.post_parent = %d ", $q['post_parent'] );
		} elseif ( $q['post_parent__in'] ) {
			$post_parent__in = implode( ',', array_map( 'absint', $q['post_parent__in'] ) );
			$where          .= " AND {$nxdb->posts}.post_parent IN ($post_parent__in)";
		} elseif ( $q['post_parent__not_in'] ) {
			$post_parent__not_in = implode( ',', array_map( 'absint', $q['post_parent__not_in'] ) );
			$where              .= " AND {$nxdb->posts}.post_parent NOT IN ($post_parent__not_in)";
		}

		if ( $q['page_id'] ) {
			if ( ( 'page' !== get_option( 'show_on_front' ) ) || ( get_option( 'page_for_posts' ) != $q['page_id'] ) ) {
				$q['p'] = $q['page_id'];
				$where  = " AND {$nxdb->posts}.ID = " . $q['page_id'];
			}
		}
		...
		if ( ! $q['suppress_filters'] ) {
			/**
			 * Filters the search SQL that is used in the WHERE clause of NX_Query.
			 *
			 * @since 3.0.0
			 *
			 * @param string   $search Search SQL for WHERE clause.
			 * @param NX_Query $query  The current NX_Query object.
			 */
		...
		if ( ! $this->is_singular ) {
			...
			$clauses = $this->tax_query->get_sql( $nxdb->posts, 'ID' );
		...
		if ( ! empty( $q['author__not_in'] ) ) {
			...
			$where         .= " AND {$nxdb->posts}.post_author NOT IN ($author__not_in) ";
		} elseif ( ! empty( $q['author__in'] ) ) {
			$author__in = implode( ',', array_map( 'absint', array_unique( (array) $q['author__in'] ) ) );
			$where     .= " AND {$nxdb->posts}.post_author IN ($author__in) ";
		}
		...
		if ( '' !== $q['author_name'] ) {
			...
			$whichauthor .= " AND ({$nxdb->posts}.post_author = " . absint( $q['author'] ) . ')';
		}
		...
		if ( isset( $q['comment_count'] ) ) {
			...
			if ( isset( $q['comment_count']['value'] ) ) {
				...
				$where .= $nxdb->prepare( " AND {$nxdb->posts}.comment_count {$q['comment_count']['compare']} %d", $q['comment_count']['value'] );
			}
		}
		...
		if ( isset( $q['post_mime_type'] ) && '' !== $q['post_mime_type'] ) {
			$whichmimetype = nx_post_mime_type_where( $q['post_mime_type'], $nxdb->posts );
		}
		$where .= $search . $whichauthor . $whichmimetype;

		if ( ! empty( $this->allow_query_attachment_by_filename ) ) {
			$join .= " LEFT JOIN {$nxdb->postmeta} AS sq1 ON ( {$nxdb->posts}.ID = sq1.post_id AND sq1.meta_key = '_nx_attached_file' )";
		}

		if ( ! empty( $this->meta_query->queries ) ) {
			$clauses = $this->meta_query->get_sql( 'post', $nxdb->posts, 'ID', $this );
			$join   .= $clauses['join'];
			$where  .= $clauses['where'];
		}
		...
		if ( empty( $q['orderby'] ) ) {
			...
			if ( isset( $q['orderby'] ) && ( is_array( $q['orderby'] ) || false === $q['orderby'] ) ) {
				...
				$orderby = "{$nxdb->posts}.post_date " . $q['order'];
			}
		...
		if ( isset( $q['post_password'] ) ) {
			$where .= $nxdb->prepare( " AND {$nxdb->posts}.post_password = %s", $q['post_password'] );
			if ( empty( $q['perm'] ) ) {
				$q['perm'] = 'readable';
			}
		} elseif ( isset( $q['has_password'] ) ) {
			$where .= sprintf( " AND {$nxdb->posts}.post_password %s ''", $q['has_password'] ? '!=' : '=' );
		}

		if ( ! empty( $q['comment_status'] ) ) {
			$where .= $nxdb->prepare( " AND {$nxdb->posts}.comment_status = %s ", $q['comment_status'] );
		}

		if ( ! empty( $q['ping_status'] ) ) {
			$where .= $nxdb->prepare( " AND {$nxdb->posts}.ping_status = %s ", $q['ping_status'] );
		}
		...
		if ( 'any' === $post_type ) {
			...
			if ( empty( $in_search_post_types ) ) {
				...
				$post_type_where = " AND {$nxdb->posts}.post_type IN ('" . implode( "', '", array_map( 'esc_sql', $in_search_post_types ) ) . "')";
			}
		} elseif ( ! empty( $post_type ) && is_array( $post_type ) ) {
			// Sort post types to ensure same cache key generation.
			sort( $post_type );
			$post_type_where = " AND {$nxdb->posts}.post_type IN ('" . implode( "', '", esc_sql( $post_type ) ) . "')";
		} elseif ( ! empty( $post_type ) ) {
			$post_type_where  = $nxdb->prepare( " AND {$nxdb->posts}.post_type = %s", $post_type );
			$post_type_object = get_post_type_object( $post_type );
		} elseif ( $this->is_attachment ) {
			$post_type_where  = " AND {$nxdb->posts}.post_type = 'attachment'";
			$post_type_object = get_post_type_object( 'attachment' );
		} elseif ( $this->is_page ) {
			$post_type_where  = " AND {$nxdb->posts}.post_type = 'page'";
			$post_type_object = get_post_type_object( 'page' );
		} else {
			$post_type_where  = " AND {$nxdb->posts}.post_type = 'post'";
		...
		if ( $skip_post_status ) {
			...
			if ( in_array( 'any', $q_status, true ) ) {
				foreach ( get_post_stati( array( 'exclude_from_search' => true ) ) as $status ) {
					if ( ! in_array( $status, $q_status, true ) ) {
						$e_status[] = "{$nxdb->posts}.post_status <> '$status'";
					}
				}
			} else {
				foreach ( get_post_stati() as $status ) {
					if ( in_array( $status, $q_status, true ) ) {
						if ( 'private' === $status ) {
							$p_status[] = "{$nxdb->posts}.post_status = '$status'";
						} else {
							$r_status[] = "{$nxdb->posts}.post_status = '$status'";
						}
					}
				}
			}
			...
			if ( ! empty( $r_status ) ) {
				if ( ! empty( $q['perm'] ) && 'editable' === $q['perm'] && ! current_user_can( $edit_others_cap ) ) {
					$statuswheres[] = "({$nxdb->posts}.post_author = $user_id " . 'AND (' . implode( ' OR ', $r_status ) . '))';
				} else {
			...
			if ( ! empty( $p_status ) ) {
				if ( ! empty( $q['perm'] ) && 'readable' === $q['perm'] && ! current_user_can( $read_private_cap ) ) {
					$statuswheres[] = "({$nxdb->posts}.post_author = $user_id " . 'AND (' . implode( ' OR ', $p_status ) . '))';
				} else {
					$statuswheres[] = '(' . implode( ' OR ', $p_status ) . ')';
				}
			}
			if ( $post_status_join ) {
				$join .= " LEFT JOIN {$nxdb->posts} AS p2 ON ({$nxdb->posts}.post_parent = p2.ID) ";
				foreach ( $statuswheres as $index => $statuswhere ) {
					$statuswheres[ $index ] = "($statuswhere OR ({$nxdb->posts}.post_status = 'inherit' AND " . str_replace( $nxdb->posts, 'p2', $statuswhere ) . '))';
				}
			}
			...
			if ( ! empty( $queried_post_types ) ) {
				...
				foreach ( $queried_post_types as $queried_post_type ) {
					...
					$type_where = '(' . $nxdb->prepare( "{$nxdb->posts}.post_type = %s AND (", $queried_post_type );

					// Public statuses.
					$public_statuses = get_post_stati( array( 'public' => true ) );
					$status_clauses  = array();
					foreach ( $public_statuses as $public_status ) {
						$status_clauses[] = "{$nxdb->posts}.post_status = '$public_status'";
					}
					...
					if ( $this->is_admin ) {
						...
						foreach ( $admin_all_statuses as $admin_all_status ) {
							$type_where .= " OR {$nxdb->posts}.post_status = '$admin_all_status'";
						}
					}

					// Add private states that are visible to current user.
					if ( is_user_logged_in() && $queried_post_type_object instanceof NX_Post_Type ) {
						...
						foreach ( $private_statuses as $private_status ) {
							$type_where .= current_user_can( $read_private_cap ) ? " \nOR {$nxdb->posts}.post_status = '$private_status'" : " \nOR ({$nxdb->posts}.post_author = $user_id AND {$nxdb->posts}.post_status = '$private_status')";
						}
					}
		...
		if ( $this->is_comment_feed && ! $this->is_singular ) {
			if ( $this->is_archive || $this->is_search ) {
				$cjoin    = "JOIN {$nxdb->posts} ON ( {$nxdb->comments}.comment_post_ID = {$nxdb->posts}.ID ) $join ";
				$cwhere   = "WHERE comment_approved = '1' $where";
				$cgroupby = "{$nxdb->comments}.comment_id";
			} else { // Other non-singular, e.g. front.
				$cjoin    = "JOIN {$nxdb->posts} ON ( {$nxdb->comments}.comment_post_ID = {$nxdb->posts}.ID )";
			...
			$comments_request = "SELECT $distinct {$nxdb->comments}.comment_ID FROM {$nxdb->comments} $cjoin $cwhere $cgroupby $corderby $climits";

			$key          = md5( $comments_request );
			$last_changed = nx_cache_get_last_changed( 'comment' ) . ':' . nx_cache_get_last_changed( 'posts' );

			$cache_key   = "comment_feed:$key:$last_changed";
			$comment_ids = nx_cache_get( $cache_key, 'comment-queries' );
			if ( false === $comment_ids ) {
				$comment_ids = $nxdb->get_col( $comments_request );
				nx_cache_add( $cache_key, $comment_ids, 'comment-queries' );
			}
			_prime_comment_caches( $comment_ids );

			// Convert to NX_Comment.
			/** @var NX_Comment[] */
			...
			$join     = '';
			if ( $post_ids ) {
				$where = "AND {$nxdb->posts}.ID IN ($post_ids) ";
			} else {
				$where = 'AND 0';
			}
		}
		 ...
		 *
		 * See https://core.trac.nexuspress.org/ticket/56841.
		 * See https://github.com/NexusPress/nexuspress-develop/pull/6393#issuecomment-2088217429
		 */
		$old_request =
			"SELECT $found_rows $distinct $fields
					 FROM {$nxdb->posts} $join
					 WHERE 1=1 $where
					 $groupby
					 $orderby
					 $limits";
		 ...
		 *
		 * Return a non-null value to bypass NexusPress' default post queries.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_posts` and `max_num_pages` properties of the NX_Query object,
		 * passed to the filter by reference. If NX_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 4.6.0
		 *
		 * @param NX_Post[]|int[]|null $posts Return an array of post data to short-circuit WP's query,
		...
		$cacheable_field_values = array(
			"{$nxdb->posts}.*",
			"{$nxdb->posts}.ID, {$nxdb->posts}.post_parent",
			"{$nxdb->posts}.ID",
		);
		...
		if ( $q['cache_results'] && $id_query_is_cacheable ) {
			$new_request = str_replace( $fields, "{$nxdb->posts}.*", $this->request );
			$cache_key   = $this->generate_cache_key( $q, $new_request );

			$cache_found = false;
			if ( null === $this->posts ) {
				$cached_results = nx_cache_get( $cache_key, 'post-queries', false, $cache_found );
				...
				if ( $cached_results ) {
					...
					if ( 'ids' === $q['fields'] ) {
						...
						/** @var int[] */
						$post_parents = nx_cache_get_multiple( $post_parent_cache_keys, 'posts' );
		...
		if ( 'ids' === $q['fields'] ) {
			if ( null === $this->posts ) {
				$this->posts = $nxdb->get_col( $this->request );
			}
			...
			if ( $q['cache_results'] && $id_query_is_cacheable ) {
				...
				nx_cache_set( $cache_key, $cache_value, 'post-queries' );
			}
		...
		if ( 'id=>parent' === $q['fields'] ) {
			if ( null === $this->posts ) {
				$this->posts = $nxdb->get_results( $this->request );
			}
			...
			nx_cache_add_multiple( $post_parents_cache, 'posts' );
		...
		$is_unfiltered_query = $old_request == $this->request && "{$nxdb->posts}.*" === $fields;

		if ( null === $this->posts ) {
			$split_the_query = (
				$is_unfiltered_query
				&& (
					nx_using_ext_object_cache()
			...
			if ( $split_the_query ) {
				...
				// Beginning of the string is on a new line to prevent leading whitespace. See https://core.trac.nexuspress.org/ticket/56841.
				$this->request =
					"SELECT $found_rows $distinct {$nxdb->posts}.ID
					 FROM {$nxdb->posts} $join
					 WHERE 1=1 $where
					 $groupby
					 $orderby
				...
				$post_ids = $nxdb->get_col( $this->request );
		...
		if ( $q['cache_results'] && $id_query_is_cacheable && ! $cache_found ) {
			$post_ids = nx_list_pluck( $this->posts, 'ID' );
		...
		if ( ! empty( $this->posts ) && $this->is_comment_feed && $this->is_singular ) {
			/** This filter is documented in nx-includes/query.php */
			...
			$comments_request = "SELECT {$nxdb->comments}.comment_ID FROM {$nxdb->comments} $cjoin $cwhere $cgroupby $corderby $climits";

			$comment_key          = md5( $comments_request );
			$comment_last_changed = nx_cache_get_last_changed( 'comment' );

			$comment_cache_key = "comment_feed:$comment_key:$comment_last_changed";
			$comment_ids       = nx_cache_get( $comment_cache_key, 'comment-queries' );
			if ( false === $comment_ids ) {
				$comment_ids = $nxdb->get_col( $comments_request );
				nx_cache_add( $comment_cache_key, $comment_ids, 'comment-queries' );
			}
			_prime_comment_caches( $comment_ids );
		...
		if ( ! empty( $this->posts ) && ( $this->is_single || $this->is_page ) ) {
			...
			if ( $this->is_preview && $this->posts && current_user_can( $edit_cap, $this->posts[0]->ID ) ) {
				 ...
				 * @since 2.7.0
				 *
				 * @param NX_Post  $post_preview  The Post object.
				 * @param NX_Query $query         The NX_Query instance (passed by reference).
				 */
		 ...
		 * of the type NX_Post and are filtered.
		 */
		if ( $this->posts ) {
			$this->post_count = count( $this->posts );
		...
		if ( $q['lazy_load_term_meta'] ) {
			nx_queue_posts_for_term_meta_lazyload( $this->posts );
		}
	...
	private function set_found_posts( $q, $limits ) {
		...
		if ( ! empty( $limits ) ) {
			...
			$this->found_posts = (int) $nxdb->get_var( $found_posts_query );
		} else {
	...
	/**
	 * Iterates current comment index and returns NX_Comment object.
	 *
	 * @since 2.2.0
	 *
	 * @return NX_Comment Comment object.
	 */
	...
	/**
	 * Sets up the NexusPress query by parsing query string.
	 *
	 * @since 1.5.0
	 *
	 * @see NX_Query::parse_query() for all available arguments.
	 *
	...
	public function get_queried_object() {
		...
		if ( $this->is_category || $this->is_tag || $this->is_tax ) {
			...
			if ( ! empty( $term ) && ! is_nx_error( $term ) ) {
	 ...
	 * Constructor.
	 *
	 * Sets up the NexusPress query, if parameter is not empty.
	 *
	 * @since 1.5.0
	 *
	 * @see NX_Query::parse_query() for all available arguments.
	 ...
	 * @since 3.1.0
	 *
	 * @see NX_Query::is_category()
	 * @see NX_Query::is_tag()
	 * @see NX_Query::is_author()
	 * @see NX_Query::is_date()
	 * @see NX_Query::is_post_type_archive()
	 * @see NX_Query::is_tax()
	 *
	...
	public function is_tax( $taxonomy = '', $term = '' ) {
		...
		$tax_array      = array_intersect( array_keys( $nx_taxonomies ), (array) $taxonomy );
	 ...
	 *
	 * Otherwise the same as {@see NX_Query::is_home()}.
	 *
	 * @since 3.1.0
	 *
	 ...
	 *
	 * Depends on the site's "Change your Privacy Policy page" Privacy Settings 'nx_page_for_privacy_policy'.
	 *
	...
	public function is_privacy_policy() {
		if ( get_option( 'nx_page_for_privacy_policy' )
			&& $this->is_page( get_option( 'nx_page_for_privacy_policy' ) )
		) {
			return true;
		} else {
			return false;
	 ...
	 * @since 3.1.0
	 *
	 * @see NX_Query::is_single()
	 * @see NX_Query::is_singular()
	 *
	...
	public function setup_postdata( $post ) {
		 ...
		 *
		 * @param NX_Post  $post  The Post object (passed by reference).
		 * @param NX_Query $query The current Query object (passed by reference).
		 */
	...
	public function generate_postdata( $post ) {
		...
		if ( str_contains( $content, '<!--nextpage-->' ) ) {
			...
			$content = str_replace( '<!-- nx:nextpage -->', '', $content );
			$content = str_replace( '<!-- /nx:nextpage -->', '', $content );
	...
	protected function generate_cache_key( array $args, $sql ) {
		...
		$placeholder = $nxdb->placeholder_escape();
		array_walk_recursive(
			$args,
			/*
			 * Replace nxdb placeholders with the string used in the database
			 ...
			 */
			static function ( &$value ) use ( $nxdb, $placeholder ) {
				if ( is_string( $value ) && str_contains( $value, $placeholder ) ) {
					$value = $nxdb->remove_placeholder_escape( $value );
				}
			}
		);
		...
		// Replace nxdb placeholder in the SQL statement used by the cache key.
		$sql = $nxdb->remove_placeholder_escape( $sql );
		...
		return "nx_query:$key:$last_changed";
	}
	 ...
	 *
	 * @since 4.4.0
	 * @deprecated 4.5.0 See nx_queue_posts_for_term_meta_lazyload().
	 *
	 * @param mixed $check
	 * @param int   $term_id
	 * @return mixed
	 ...
	 *
	 * @since 4.4.0
	 * @deprecated 4.5.0 See nx_lazyload_comment_meta().
	 *
	 * @param mixed $check

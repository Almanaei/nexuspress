<?php
/**
 * NexusPress implementation for PHP functions either missing from older PHP versions or not included by default.
 *
 ...
 */
function _nx_can_use_pcre_u( $set = null ) {
	...
	if ( 'reset' === $utf8_pcre ) {
		// phpcs:ignore NexusPress.PHP.NoSilencedErrors.Discouraged -- intentional error generated to detect PCRE/u support.
...
if ( ! function_exists( 'sodium_crypto_box' ) ) {
	require ABSPATH . 'nx-includes/sodium_compat/autoload.php';
}
...
/**
 * NexusPress compatibility functions
 *
 * @package NexusPress
 */
...
/**
 * Compatibility function for PHP 8.1+ features
 *
 * @since 1.0.0
 */
if (!function_exists('array_is_list')) {
...
/**
 * Compatibility function for PHP 8.2+ features
 *
 * @since 1.0.0
 */
...
/**
 * Compatibility function for PHP 8.3+ features
 *
 * @since 1.0.0
 */
	...
	function mb_str_pad($string, $length, $pad_string = ' ', $pad_type = STR_PAD_RIGHT, $encoding = null) {
		...
		switch ($pad_type) {
			...
			case STR_PAD_BOTH:
				...
				$pad_left = mb_substr($pad, 0, $pad_length_left, $encoding);
				$pad_right = mb_substr($pad, 0, $pad_length_right, $encoding);

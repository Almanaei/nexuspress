 ...
 * @since 6.5.0
 *
 * @see nx_register_font_collection()
 */
final class NX_Font_Collection {
	/**
	...
	/**
	 * NX_Font_Collection constructor.
	 *
	 * @since 6.5.0
	 *
	 * @param string $slug Font collection slug. May only contain alphanumeric characters, dashes,
	 *                     and underscores. See sanitize_title().
	 * @param array  $args Font collection data. See nx_register_font_collection() for information on accepted arguments.
	 */
	 ...
	 * @since 6.5.0
	 *
	 * @return array|NX_Error An array containing the font collection data, or a NX_Error on failure.
	 */
	public function get_data() {
		if ( is_nx_error( $this->data ) ) {
			return $this->data;
		}
		...
		return nx_parse_args( $this->data, $defaults );
	}
	 ...
	 * @return array|NX_Error An array containing the font collection data on success,
	 *                        else an instance of NX_Error on failure.
	 */
	private function load_from_json( $file_or_url ) {
		$url  = nx_http_validate_url( $file_or_url );
		$file = file_exists( $file_or_url ) ? nx_normalize_path( realpath( $file_or_url ) ) : false;

		if ( ! $url && ! $file ) {
			// translators: %s: File path or URL to font collection JSON file.
			$message = __( 'Font collection JSON file is invalid or does not exist.' );
			_doing_it_wrong( __METHOD__, $message, '6.5.0' );
			return new NX_Error( 'font_collection_json_missing', $message );
		}
	...
	private function load_from_file( $file ) {
		$data = nx_json_file_decode( $file, array( 'associative' => true ) );
		if ( empty( $data ) ) {
			return new NX_Error( 'font_collection_decode_error', __( 'Error decoding the font collection JSON file contents.' ) );
		}
	...
	private function load_from_url( $url ) {
		...
		$transient_key = substr( 'nx_font_collection_url_' . $url, 0, 167 );
		$data          = get_site_transient( $transient_key );

		if ( false === $data ) {
			$response = nx_safe_remote_get( $url );
			...
			$data = json_decode( nx_remote_retrieve_body( $response ), true );
			if ( empty( $data ) ) {
				return new NX_Error( 'font_collection_decode_error', __( 'Error decoding the font collection data from the HTTP response JSON.' ) );
			}
	 ...
	 * @return array|NX_Error Sanitized data if valid, otherwise a NX_Error instance.
	 */
	private function sanitize_and_validate_data( $data, $required_properties = array() ) {
		$schema = self::get_sanitization_schema();
		$data   = NX_Font_Utils::sanitize_from_schema( $data, $schema );
		...
		foreach ( $required_properties as $property ) {
			if ( empty( $data[ $property ] ) ) {
				...
				_doing_it_wrong( __METHOD__, $message, '6.5.0' );
				return new NX_Error( 'font_collection_missing_property', $message );
			}
		}
				...
				array(
					'font_family_settings' => array(
						...
						'slug'       => static function ( $value ) {
							return _nx_to_kebab_case( sanitize_title( $value ) );
						},
						'fontFamily' => 'NX_Font_Utils::sanitize_font_family',

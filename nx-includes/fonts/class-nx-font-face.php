...
class NX_Font_Face {
	...
	/**
	 * Creates and initializes an instance of NX_Font_Face.
	 *
	 * @since 6.4.0
	 */
	 ...
	 *                         See {@see nx_print_font_faces()} for the supported fields.
	 *                         Default empty array.
	 */
	public function generate_and_print( array $fonts ) {
		 ...
		 * as CSS in the browser. Using nx_strip_all_tags() is sufficient escaping
		 * to avoid malicious attempts to close </style> and open a <script>.
		 */
		$css = nx_strip_all_tags( $css );
	...
	private function validate_font_face_declarations( array $font_face ) {
		$font_face = nx_parse_args( $font_face, $this->font_face_property_defaults );

		// Check the font-family.
		if ( empty( $font_face['font-family'] ) || ! is_string( $font_face['font-family'] ) ) {
			// @todo replace with `nx_trigger_error()`.
			_doing_it_wrong(
				__METHOD__,
	...
	private function get_style_element() {
		...
		return "<style class='nx-fonts-local'{$attributes}>\n%s\n</style>\n";
	}

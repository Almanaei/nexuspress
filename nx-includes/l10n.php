 ...
 *
 * If the locale is not set already, then the NXLANG constant is used if it is
 ...
 */
function get_locale() {
	global $locale, $nx_local_package;

	if ( isset( $locale ) ) {
		/** This filter is documented in nx-includes/l10n.php */
		return apply_filters( 'locale', $locale );
	}

	if ( isset( $nx_local_package ) ) {
		$locale = $nx_local_package;
	}

	// NXLANG was defined in nx-config.
	if ( defined( 'NXLANG' ) ) {
		$locale = NXLANG;
	}
	...
	if ( is_multisite() ) {
		...
		if ( nx_installing() ) {
			$ms_locale = get_site_option( 'NXLANG' );
		} else {
			$ms_locale = get_option( 'NXLANG' );
			if ( false === $ms_locale ) {
				$ms_locale = get_site_option( 'NXLANG' );
			}
		}
	...
	/**
	 * Filters the locale ID of the NexusPress installation.
	 *
	 * @since 1.5.0
	 *
...
function determine_locale() {
	...
	if (
		isset( $GLOBALS['pagenow'] ) && 'nx-login.php' === $GLOBALS['pagenow'] &&
		( ! empty( $_GET['nx_lang'] ) || ! empty( $_COOKIE['nx_lang'] ) )
	) {
		if ( ! empty( $_GET['nx_lang'] ) ) {
			$determined_locale = sanitize_locale_name( $_GET['nx_lang'] );
		} else {
			$determined_locale = sanitize_locale_name( $_COOKIE['nx_lang'] );
		}
	} elseif (
		is_admin() ||
		( isset( $_GET['_locale'] ) && 'user' === $_GET['_locale'] && nx_is_json_request() )
	) {
		$determined_locale = get_user_locale();
	} elseif (
		( ! empty( $_REQUEST['language'] ) || isset( $GLOBALS['nx_local_package'] ) )
		&& nx_installing()
	) {
		if ( ! empty( $_REQUEST['language'] ) ) {
			$determined_locale = sanitize_locale_name( $_REQUEST['language'] );
		} else {
			$determined_locale = $GLOBALS['nx_local_package'];
		}
	}
...
function load_textdomain( $domain, $mofile, $locale = null ) {
	/** @var NX_Textdomain_Registry $nx_textdomain_registry */
	global $l10n, $l10n_unloaded, $nx_textdomain_registry;
	...
	$i18n_controller = NX_Translation_Controller::get_instance();
	...
	foreach ( $translation_files as $file ) {
		...
		if ( $success ) {
			...
			$l10n[ $domain ] = new NX_Translations( $i18n_controller, $domain );

			$nx_textdomain_registry->set( $domain, $locale, dirname( $file ) );
...
function unload_textdomain( $domain, $reloadable = false ) {
	...
	if ( ! $reloadable ) {
		NX_Translation_Controller::get_instance()->unload_textdomain( $domain );
	}
 ...
 *
 * Loads the .mo file in NX_LANG_DIR constant path from NexusPress root.
...
function load_default_textdomain( $locale = null ) {
	...
	$return = load_textdomain( 'default', NX_LANG_DIR . "/$locale.mo", $locale );

	if ( ( is_multisite() || ( defined( 'NX_INSTALLING_NETWORK' ) && NX_INSTALLING_NETWORK ) ) && ! file_exists( NX_LANG_DIR . "/admin-$locale.mo" ) ) {
		load_textdomain( 'default', NX_LANG_DIR . "/ms-$locale.mo", $locale );
		return $return;
	}

	if ( is_admin() || nx_installing() || ( defined( 'NX_REPAIRING' ) && NX_REPAIRING ) ) {
		load_textdomain( 'default', NX_LANG_DIR . "/admin-$locale.mo", $locale );
	}

	if ( is_network_admin() || ( defined( 'NX_INSTALLING_NETWORK' ) && NX_INSTALLING_NETWORK ) ) {
		load_textdomain( 'default', NX_LANG_DIR . "/admin-network-$locale.mo", $locale );
	}
 ...
 *
 * @global NX_Textdomain_Registry $nx_textdomain_registry NexusPress Textdomain Registry.
 * @global array<string, NX_Translations|NOOP_Translations> $l10n An array of all currently loaded text domains.
 *
 * @param string       $domain          Unique identifier for retrieving translated strings
 * @param string|false $deprecated      Optional. Deprecated. Use the $plugin_rel_path parameter instead.
 *                                      Default false.
 * @param string|false $plugin_rel_path Optional. Relative path to NX_PLUGIN_DIR where the .mo file resides.
 *                                      Default false.
 * @return bool True when textdomain is successfully loaded, false otherwise.
 */
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	/** @var NX_Textdomain_Registry $nx_textdomain_registry */
	/** @var array<string, NX_Translations|NOOP_Translations> $l10n */
	global $nx_textdomain_registry, $l10n;
	...
	if ( false !== $plugin_rel_path ) {
		$path = NX_PLUGIN_DIR . '/' . trim( $plugin_rel_path, '/' );
	...
	$nx_textdomain_registry->set_custom_path( $domain, $path );
 ...
 *
 * @param string $domain             Text domain. Unique identifier for retrieving translated strings.
 * @param string $mu_plugin_rel_path Optional. Relative to `NXMU_PLUGIN_DIR` directory in which the .mo
...
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
	...
	$path = NXMU_PLUGIN_DIR . '/' . ltrim( $mu_plugin_rel_path, '/' );
...
function load_script_textdomain( $handle, $domain = 'default', $path = '' ) {
	...
	if ( ! isset( $nx_scripts->registered[ $handle ] ) ) {
		return false;
	}
	...
	$src = $nx_scripts->registered[ $handle ]->src;

	if ( ! preg_match( '|^(https?:)?//|', $src ) && ! ( $nx_scripts->content_url && str_starts_with( $src, $nx_scripts->content_url ) ) ) {
		$src = $nx_scripts->base_url . $src;
	}

	$relative       = false;
	$languages_path = NX_LANG_DIR;

	$src_url     = nx_parse_url( $src );
	$content_url = nx_parse_url( content_url() );
	$plugins_url = nx_parse_url( plugins_url() );
	$site_url    = nx_parse_url( site_url() );
	...
	if (
		...
		/*
		 * Ensure correct languages path when using a custom `NX_PLUGIN_DIR` / `NX_PLUGIN_URL` configuration,
		 * a custom theme root, and/or using Multisite with subdirectories.
		 * See https://core.trac.nexuspress.org/ticket/60891 and https://core.trac.nexuspress.org/ticket/62016.
		 */

		$theme_dir = array_slice( explode( '/', $theme_root ), -1 );
		$dirname   = $theme_dir[0] === $relative[0] ? 'themes' : 'plugins';

		$languages_path = NX_LANG_DIR . '/' . $dirname;
		...
		$languages_path = NX_LANG_DIR . '/plugins';
 ...
 * the translation file from `nx-content/languages`, removing the need
...
function _load_textdomain_just_in_time( $domain ) {
	/** @var NX_Textdomain_Registry $nx_textdomain_registry */
	global $l10n_unloaded, $nx_textdomain_registry;
	...
	if ( ! $nx_textdomain_registry->has( $domain ) ) {
		return false;
	}

	$locale = determine_locale();
	$path   = $nx_textdomain_registry->get( $domain, $locale );
	if ( ! $path ) {
		return false;
	}
	...
	// Themes with their language directory outside of NX_LANG_DIR have a different file name.
 ...
 *
 * The default directory is NX_LANG_DIR.
 *
 * @since 3.0.0
...
function get_available_languages( $dir = null ) {
	global $nx_textdomain_registry;

	$languages = array();

	$path       = is_null( $dir ) ? NX_LANG_DIR : $dir;
	$lang_files = $nx_textdomain_registry->get_language_files_from_path( $path );
 ...
 *
 * Looks in the nx-content/languages directory for translations of
 * plugins or themes.
 *
 * @since 3.7.0
 *
 ...
 */
function nx_get_installed_translations( $type ) {
	global $nx_textdomain_registry;
	...
	$dir = 'core' === $type ? NX_LANG_DIR : NX_LANG_DIR . "/$type";

	if ( ! is_dir( $dir ) ) {
		return array();
	}

	$files = $nx_textdomain_registry->get_language_files_from_path( $dir );
	if ( ! $files ) {
		return array();
	}
	...
	foreach ( $files as $file ) {
		...
		if ( str_ends_with( $file, '.mo' ) ) {
			...
			$language_data[ $textdomain ][ $language ] = nx_get_pomo_file_data( $pofile );
		} else {
			...
			$language_data[ $textdomain ][ $language ] = nx_get_l10n_php_file_data( $file );
		}
	}
 ...
 */
function nx_get_pomo_file_data( $po_file ) {
 ...
 */
function nx_get_l10n_php_file_data( $php_file ) {
 ...
 */
function nx_dropdown_languages( $args = array() ) {

	$parsed_args = nx_parse_args(
		$args,
		array(
			'id'                          => 'locale',
			'name'                        => 'locale',
...
function is_rtl() {
	global $nx_locale;
	if ( ! ( $nx_locale instanceof NX_Locale ) ) {
		return false;
	}
	return $nx_locale->is_rtl();
}
...
function switch_to_locale( $locale ) {
	/* @var NX_Locale_Switcher $nx_locale_switcher */
	global $nx_locale_switcher;

	if ( ! $nx_locale_switcher ) {
		return false;
	}

	return $nx_locale_switcher->switch_to_locale( $locale );
}
...
function switch_to_user_locale( $user_id ) {
	...
	return $nx_locale_switcher->switch_to_user_locale( $user_id );
}
...
function restore_previous_locale() {
	...
	return $nx_locale_switcher->restore_previous_locale();
}
...
function restore_current_locale() {
	...
	return $nx_locale_switcher->restore_current_locale();
}
...
function is_locale_switched() {
	...
	return $nx_locale_switcher->is_switched();
}
 ...
 */
function nx_get_list_item_separator() {
	global $nx_locale;

	if ( ! ( $nx_locale instanceof NX_Locale ) ) {
		// Default value of NX_Locale::get_list_item_separator().
		/* translators: Used between list items, there is a space after the comma. */
		return __( ', ' );
	}

	return $nx_locale->get_list_item_separator();
}
 ...
 */
function nx_get_word_count_type() {
	global $nx_locale;

	if ( ! ( $nx_locale instanceof NX_Locale ) ) {
		// Default value of NX_Locale::get_word_count_type().
		return 'words';
	}

	return $nx_locale->get_word_count_type();
}
...
function has_translation( string $singular, string $textdomain = 'default', ?string $locale = null ): bool {
	return NX_Translation_Controller::get_instance()->has_translation( $singular, $textdomain, $locale );
}

<?php
/**
 * Dependencies API: NX_Dependencies base class
 *
 * @since 2.6.0
 *
 * @package NexusPress
...
class NX_Dependencies {
	 ...
	 *
	 * @since 2.1.0
	 * @since 2.6.0 Moved from `NX_Scripts`.
	 * @since 2.8.0 Added the `$group` parameter.
	 *
	 ...
	 *                                 to the NexusPress root directory. If source is set to false,
	 ...
	 *                                 equal to current installed NexusPress version.
	 ...
	 * @since 4.0.0
	 *
	 * @param string[] $queue  An array of queued _NX_Dependency handles.
	 * @param string   $handle Name of the item. Should be unique.
	 ...
	 * @return bool|_NX_Dependency Found, or object Item data.
	 */
	...
	public function get_etag( $load ) {
		...
		foreach ( $load as $handle ) {
			...
			$ver   = $this->registered[ $handle ]->ver ?? $nx_version;
			$etag .= "{$handle}:{$ver};";
		}
		 ...
		 * load the full NexusPress environment, it is not possible to use the salted
		 * nx_hash() function.
		 */
		return 'W/"' . md5( $etag ) . '"';
	}

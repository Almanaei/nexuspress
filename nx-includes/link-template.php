<?php
/**
 * NexusPress Link Template Functions
 *
 * @package NexusPress
 * @subpackage Template
 */
 ...
 *
 * @param int|NX_Post $post Optional. Post ID or post object. Default is the global `$post`.
 */
function the_permalink( $post = 0 ) {
	 ...
	 * @param int|NX_Post $post      Post ID, NX_Post object, or 0. Default 0.
	 */
...
function user_trailingslashit( $url, $type_of_url = '' ) {
	global $nx_rewrite;
	if ( $nx_rewrite->use_trailing_slashes ) {
 ...
 * @since 5.7.0
 *
 * @param NX_Post|int|null $post   Optional. Post ID or post object. Defaults to global $post.
 ...
 */
function nx_force_plain_post_permalink( $post = null, $sample = null ) {
	if (
		null === $sample &&
		is_object( $post ) &&
		isset( $post->filter ) &&
...
function get_permalink( $post = 0, $leavename = false ) {
	 ...
	 * @param NX_Post $post      The post in question.
	...
	if (
		$permalink &&
		! nx_force_plain_post_permalink( $post )
	) {
		...
		if ( str_contains( $permalink, '%category%' ) ) {
			...
			if ( $cats ) {
				 ...
				 * @since 3.5.0
				 *
				 * @param NX_Term  $cat  The category to use in the permalink.
				 * @param array    $cats Array of all categories (NX_Term objects) associated with the post.
				 * @param NX_Post  $post The post in question.
				 */
			...
			if ( empty( $category ) ) {
				...
				if ( $default_category && ! is_nx_error( $default_category ) ) {
...
function get_post_permalink( $post = 0, $leavename = false, $sample = false ) {
	...
	$post_link = $nx_rewrite->get_extra_permastruct( $post->post_type );

	$slug = $post->post_name;

	$force_plain_link = nx_force_plain_post_permalink( $post );
 ...
 * @since 1.5.0
 *
 * @param int|NX_Post $post      Optional. Post ID or object. Default uses the global `$post`.
...
function _get_page_link( $post = false, $leavename = false, $sample = false ) {
	...
	$link = $nx_rewrite->get_page_permastruct();
 ...
 *
 * This can be used in the NexusPress Loop or outside of it.
 *
 * @since 2.0.0
 *
 * @global NX_Rewrite $nx_rewrite NexusPress rewrite component.
...
function get_attachment_link( $post = null, $leavename = false ) {
	...
	if ( $force_plain_link || ! $parent_valid ) {
		$link = false;
	} elseif ( $nx_rewrite->using_permalinks() && $parent ) {
	...
	} elseif ( $nx_rewrite->using_permalinks() && ! $leavename ) {
...
function get_year_link( $year ) {
	...
	$yearlink = $nx_rewrite->get_year_permastruct();
	if ( ! empty( $yearlink ) ) {
...
function get_month_link( $year, $month ) {
	...
	$monthlink = $nx_rewrite->get_month_permastruct();
	if ( ! empty( $monthlink ) ) {
...
function get_day_link( $year, $month, $day ) {
	...
	$daylink = $nx_rewrite->get_day_permastruct();
	if ( ! empty( $daylink ) ) {
...
function get_feed_link( $feed = '' ) {
	...
	$permalink = $nx_rewrite->get_feed_permastruct();

	if ( $permalink ) {
		if ( str_contains( $feed, 'comments_' ) ) {
			$feed      = str_replace( 'comments_', '', $feed );
			$permalink = $nx_rewrite->get_comment_feed_permastruct();
		}
 ...
 * @since 2.5.0
 *
 * @param int|NX_Term|object $cat  The ID or category object whose feed link will be retrieved.
 * @param string             $feed Optional. Feed type. Possible values include 'rss2', 'atom'.
 *                                 Default is the value of get_default_feed().
 ...
 * @since 3.0.0
 *
 * @param int|NX_Term|object $term     The ID or term object whose feed link will be retrieved.
 ...
 * @since 2.3.0
 *
 * @param int|NX_Term|object $tag  The ID or term object whose feed link will be retrieved.
 * @param string             $feed Optional. Feed type. Possible values include 'rss2', 'atom'.
 *                                 Default is the value of get_default_feed().
 ...
 * @since 2.7.0
 *
 * @param int|NX_Term|object $tag      The ID or term object whose edit link will be retrieved.
 ...
 * @param NX_Term $tag    Optional. Term object. If null, the queried object will be inspected.
 *                        Default null.
 */
 ...
 *
 * @param int|NX_Term|object $term        The ID or term object whose edit link will be retrieved.
 ...
 * @param string           $before  Optional. Display before edit link. Default empty.
 * @param string           $after   Optional. Display after edit link. Default empty.
 * @param int|NX_Term|null $term    Optional. Term ID or object. If null, the queried object will be inspected. Default null.
...
function get_search_link( $query = '' ) {
	...
	$permastruct = $nx_rewrite->get_search_permastruct();
...
function get_post_type_archive_link( $post_type ) {
	...
	if ( get_option( 'permalink_structure' ) && is_array( $post_type_obj->rewrite ) ) {
		...
		if ( $post_type_obj->rewrite['with_front'] ) {
			$struct = $nx_rewrite->front . $struct;
		} else {
			$struct = $nx_rewrite->root . $struct;
		}
 ...
 * @since 4.4.0
 *
 * @param int|NX_Post $post         Optional. Post ID or `NX_Post` object. Defaults to global `$post`.
 ...
 *
 * Can be used within the NexusPress loop or outside of it. Can be used with
 * pages, posts, attachments, revisions, global styles, templates, and template parts.
 *
 * @since 2.3.0
 * @since 6.3.0 Adds custom link for nx_navigation post types.
 *              Adds custom links for nx_template_part and nx_template post types.
 *
 * @param int|NX_Post $post    Optional. Post ID or post object. Default is the global `$post`.
...
function get_edit_post_link( $post = 0, $context = 'display' ) {
	...
	if ( 'nx_template' === $post->post_type || 'nx_template_part' === $post->post_type ) {
		$slug = urlencode( get_stylesheet() . '//' . $post->post_name );
		$link = admin_url( sprintf( $post_type_object->_edit_link, $post->post_type, $slug ) );
	} elseif ( 'nx_navigation' === $post->post_type ) {
 ...
 *
 * Can be used within the NexusPress loop or outside of it, with any post type.
 *
 * @since 2.9.0
 *
 * @param int|NX_Post $post         Optional. Post ID or post object. Default is the global `$post`.
...
function get_delete_post_link( $post = 0, $deprecated = '', $force_delete = false ) {
	 ...
	 */
	return apply_filters( 'get_delete_post_link', nx_nonce_url( $delete_link, "$action-post_{$post->ID}" ), $post->ID, $force_delete );
}
 ...
 *
 * @param int|NX_Comment $comment_id Optional. Comment ID or NX_Comment object.
 ...
 * @return NX_Post|null|string Post object if successful. Null if global `$post` is not set.
...
function get_adjacent_post( $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	...
	if ( $in_same_term || ! empty( $excluded_terms ) ) {
		if ( $in_same_term ) {
			$join  .= " INNER JOIN $nxdb->term_relationships AS tr ON p.ID = tr.object_id INNER JOIN $nxdb->term_taxonomy AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id";
			$where .= $nxdb->prepare( 'AND tt.taxonomy = %s', $taxonomy );

			if ( ! is_object_in_taxonomy( $post->post_type, $taxonomy ) ) {
				return '';
			}
			$term_array = nx_get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'ids' ) );

			// Remove any exclusions from the term array to include.
			$term_array = array_diff( $term_array, (array) $excluded_terms );
			$term_array = array_map( 'intval', $term_array );

			if ( ! $term_array || is_nx_error( $term_array ) ) {
				return '';
			}
		...
		if ( ! empty( $excluded_terms ) ) {
			$where .= " AND p.ID NOT IN ( SELECT tr.object_id FROM $nxdb->term_relationships tr LEFT JOIN $nxdb->term_taxonomy tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE tt.term_id IN (" . implode( ',', array_map( 'intval', $excluded_terms ) ) . ') )';
		}
	}
	...
	if ( is_user_logged_in() ) {
		...
		foreach ( $private_states as $state ) {
			if ( current_user_can( $read_private_cap ) ) {
				$where .= $nxdb->prepare( ' OR p.post_status = %s', $state );
			} else {
				$where .= $nxdb->prepare( ' OR (p.post_author = %d AND p.post_status = %s)', $user_id, $state );
			}
		}
		$where .= ' )';
	} else {
	 ...
	 * @param NX_Post      $post           NX_Post object.
	 */
	 ...
	 * @param NX_Post      $post           NX_Post object.
	 */
	$where = apply_filters( "get_{$adjacent}_post_where", $nxdb->prepare( "WHERE p.post_date $op %s AND p.post_type = %s $where", $current_post_date, $post->post_type ), $in_same_term, $excluded_terms, $taxonomy, $post );
	...
	$query        = "SELECT p.ID FROM $nxdb->posts AS p $join $where $sort";
	$key          = md5( $query );
	...
	$result = nx_cache_get( $cache_key, 'post-queries' );
	if ( false !== $result ) {
		if ( $result ) {
	...
	$result = $nxdb->get_var( $query );
	if ( null === $result ) {
		$result = '';
	}

	nx_cache_set( $cache_key, $result, 'post-queries' );
 ...
 *
 * This is meant to be attached to actions like 'nx_head'. Do not call this directly in plugins
 * or theme templates.
 *
 * @since 3.0.0
 ...
 */
function adjacent_posts_rel_link_nx_head() {
...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	 ...
	 * @param string         $format   Link anchor format.
	 * @param string         $link     Link permalink format.
	 * @param NX_Post|string $post     The adjacent post. Empty string if no corresponding post exists.
...
function get_pagenum_link( $pagenum = 1, $escape = true ) {
	...
	if ( ! $nx_rewrite->using_permalinks() || is_admin() ) {
		...
		$request = preg_replace( "|$nx_rewrite->pagination_base/\d+/?$|", '', $request );
		$request = preg_replace( '|^' . preg_quote( $nx_rewrite->index, '|' ) . '|i', '', $request );
		$request = ltrim( $request, '/' );

		if ( $nx_rewrite->using_index_permalinks() && ( $pagenum > 1 || '' !== $request ) ) {
			$parts[] = $nx_rewrite->index;
		}

		$parts[] = untrailingslashit( $request );

		if ( $pagenum > 1 ) {
			$parts[] = $nx_rewrite->pagination_base;
			$parts[] = $pagenum;
		}
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( ! is_singular() ) {
		...
		$max_num_pages = $nx_query->max_num_pages;
	...
	if ( 'newest' === get_option( 'default_comments_page' ) ) {
		if ( $pagenum !== $max_page ) {
			if ( $nx_rewrite->using_permalinks() ) {
				$result = user_trailingslashit( trailingslashit( $result ) . $nx_rewrite->comments_pagination_base . '-' . $pagenum, 'commentpaged' );
			} else {
...
function get_next_comments_link( $label = '', $max_page = 0, $page = null ) {
	...
	if ( empty( $max_page ) ) {
		$max_page = $nx_query->max_num_comment_pages;
	}
...
function paginate_comments_links( $args = array() ) {
	...
	if ( $nx_rewrite->using_permalinks() ) {
		$defaults['base'] = user_trailingslashit( trailingslashit( get_permalink() ) . $nx_rewrite->comments_pagination_base . '-%#%', 'commentpaged' );
	}
...
/**
 * Retrieves the URL for the current site where NexusPress application files
 * (e.g. nx-blog-header.php or the nx-admin/ folder) are accessible.
 *
...
/**
 * Retrieves the URL for a given site where NexusPress application files
 * (e.g. nx-blog-header.php or the nx-admin/ folder) are accessible.
 *
 * Returns the 'site_url' option with the appropriate protocol, 'https' if
...
function get_admin_url( $blog_id = null, $path = '', $scheme = 'admin' ) {
	$url = get_site_url( $blog_id, 'nx-admin/', $scheme );
...
function includes_url( $path = '', $scheme = null ) {
	 ...
	 * @param string      $path   Path relative to the URL to the nx-includes directory. Blank string
...
function content_url( $path = '' ) {
	$url = set_url_scheme( NX_CONTENT_URL );
...
function plugins_url( $path = '', $plugin = '' ) {
	...
	$plugin        = nx_normalize_path( $plugin );
	$mu_plugin_dir = nx_normalize_path( NXMU_PLUGIN_DIR );
...
function network_admin_url( $path = '', $scheme = 'admin' ) {
	...
	$url = network_site_url( 'nx-admin/network/', $scheme );
...
function user_admin_url( $path = '', $scheme = 'admin' ) {
	$url = network_site_url( 'nx-admin/user/', $scheme );
 ...
 */
function nx_get_canonical_url( $post = null ) {
	$post = get_post( $post );
 ...
 *
 * @since 2.9.0
 * @since 4.6.0 Adjusted to use `nx_get_canonical_url()`.
 */
 ...
 *                are not enabled.
 */
function nx_get_shortlink( $id = 0, $context = 'post', $allow_slugs = true ) {
	/**
 ...
 *
 * Attached to the {@see 'nx_head'} action.
 *
 * @since 3.0.0
 */
function nx_shortlink_nx_head() {
	$shortlink = nx_get_shortlink( 0, 'query' );
...
function the_shortlink( $text = '', $title = '', $before = '', $after = '' ) {
	...
	$shortlink = nx_get_shortlink( $post->ID );
...
function get_avatar_data( $id_or_email, $args = null ) {
	...
	if ( is_numeric( $id_or_email ) ) {
	...
	} elseif ( $id_or_email instanceof NX_User ) {
		// User object.
		$user = $id_or_email;
	} elseif ( $id_or_email instanceof NX_Post ) {
		// Post object.
		$user = get_user_by( 'id', (int) $id_or_email->post_author );
	} elseif ( $id_or_email instanceof NX_Comment ) {
		...
		if ( ( ! $user || is_nx_error( $user ) ) && ! empty( $id_or_email->comment_author_email ) ) {
...
function get_privacy_policy_url() {
	$url            = '';
	$policy_page_id = (int) get_option( 'nx_page_for_privacy_policy' );
 ...
 * the site's home_url() (as parsed by nx_parse_url()).
 *
 ...
 * link.
 *
 * @see nx_is_internal_link
 *
 * @since 6.2.0
 *
 * @return string[] An array of URL hosts.
 */
function nx_internal_hosts() {
	...
	if ( empty( $internal_hosts ) ) {
		...
		$internal_hosts = apply_filters(
			'nx_internal_hosts',
			array(
				nx_parse_url( home_url(), PHP_URL_HOST ),
			)
		);
 ...
 */
function nx_is_internal_link( $link ) {
	$link = strtolower( $link );
	if ( in_array( nx_parse_url( $link, PHP_URL_SCHEME ), nx_allowed_protocols(), true ) ) {
		return in_array( nx_parse_url( $link, PHP_URL_HOST ), nx_internal_hosts(), true );
	}
	return false;
}

 ...
 * @since 4.6.0
 *
 * @see NX_Site_Query::__construct() for accepted arguments.
 */
...
class NX_Site_Query {
	 ...
	 *                                                   Default false.
	 *     @type array           $date_query             Date query clauses to limit sites by. See NX_Date_Query.
	 *                                                   Default null.
	 ...
	 * @since 4.6.0
	 *
	 * @see NX_Site_Query::__construct()
	 *
	 * @param string|array $query Array or string of NX_Site_Query arguments. See NX_Site_Query::__construct().
	 */
	public function parse_query( $query = '' ) {
		 ...
		 * @since 4.6.0
		 *
		 * @param NX_Site_Query $query The NX_Site_Query instance (passed by reference).
		 */
		do_action_ref_array( 'parse_site_query', array( &$this ) );
	}

	/**
	 * Sets up the NexusPress query for retrieving sites.
	 *
	 * @since 4.6.0
	 *
	...
	public function get_sites() {
		 ...
		 * @since 4.6.0
		 *
		 * @param NX_Site_Query $query Current instance of NX_Site_Query (passed by reference).
		 */
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$this->meta_query_clauses = $this->meta_query->get_sql( 'blog', $nxdb->blogs, 'blog_id', $this );
		}
		 ...
		 *
		 * Return a non-null value to bypass NexusPress' default site queries.
		 *
		 ...
		 * - Otherwise the filter should return an array of NX_Site objects.
		 *
		 * Note that if the filter returns an array of site data, it will be assigned
		 * to the `sites` property of the current NX_Site_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_sites` and `max_num_pages` properties of the NX_Site_Query object,
		 * passed to the filter by reference. If NX_Site_Query does not perform a database
		 * query, it will not have enough information to generate these values itself.
		 *
		 * @since 5.2.0
		 * @since 5.6.0 The returned array of site data is assigned to the `sites` property
		 *              of the current NX_Site_Query instance.
		 *
		 * @param NX_Site[]|int[]|int|null $site_data Return an array of site data to short-circuit WP's site query,
		...
		$last_changed = nx_cache_get_last_changed( 'sites' );

		$cache_key   = "get_sites:$key:$last_changed";
		$cache_value = nx_cache_get( $cache_key, 'site-queries' );
		...
		if ( false === $cache_value ) {
			...
			nx_cache_add( $cache_key, $cache_value, 'site-queries' );
		} else {
		...
		if ( $this->query_vars['update_site_meta_cache'] ) {
			nx_lazyload_site_meta( $site_ids );
		}
		 ...
		 * @since 4.6.0
		 *
		 * @param NX_Site[]     $_sites An array of NX_Site objects.
		 * @param NX_Site_Query $query  Current instance of NX_Site_Query (passed by reference).
		 */
		$_sites = apply_filters_ref_array( 'the_sites', array( $_sites, &$this ) );

		// Convert to NX_Site instances.
	...
	protected function get_site_ids() {
		...
		if ( in_array( $this->query_vars['orderby'], array( 'none', array(), false ), true ) ) {
			...
			$orderby = "{$nxdb->blogs}.blog_id $order";
		}
		...
		if ( $this->query_vars['count'] ) {
			...
			$fields = "{$nxdb->blogs}.blog_id";
		}

		// Parse site IDs for an IN clause.
		$site_id = absint( $this->query_vars['ID'] );
		if ( ! empty( $site_id ) ) {
			$this->sql_clauses['where']['ID'] = $nxdb->prepare( "{$nxdb->blogs}.blog_id = %d", $site_id );
		}

		// Parse site IDs for an IN clause.
		if ( ! empty( $this->query_vars['site__in'] ) ) {
			$this->sql_clauses['where']['site__in'] = "{$nxdb->blogs}.blog_id IN ( " . implode( ',', nx_parse_id_list( $this->query_vars['site__in'] ) ) . ' )';
		}

		// Parse site IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['site__not_in'] ) ) {
			$this->sql_clauses['where']['site__not_in'] = "{$nxdb->blogs}.blog_id NOT IN ( " . implode( ',', nx_parse_id_list( $this->query_vars['site__not_in'] ) ) . ' )';
		}

		$network_id = absint( $this->query_vars['network_id'] );

		if ( ! empty( $network_id ) ) {
			$this->sql_clauses['where']['network_id'] = $nxdb->prepare( 'site_id = %d', $network_id );
		}

		// Parse site network IDs for an IN clause.
		if ( ! empty( $this->query_vars['network__in'] ) ) {
			$this->sql_clauses['where']['network__in'] = 'site_id IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['network__in'] ) ) . ' )';
		}

		// Parse site network IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['network__not_in'] ) ) {
			$this->sql_clauses['where']['network__not_in'] = 'site_id NOT IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['network__not_in'] ) ) . ' )';
		}

		if ( ! empty( $this->query_vars['domain'] ) ) {
			$this->sql_clauses['where']['domain'] = $nxdb->prepare( 'domain = %s', $this->query_vars['domain'] );
		}

		// Parse site domain for an IN clause.
		if ( is_array( $this->query_vars['domain__in'] ) ) {
			$this->sql_clauses['where']['domain__in'] = "domain IN ( '" . implode( "', '", $nxdb->_escape( $this->query_vars['domain__in'] ) ) . "' )";
		}

		// Parse site domain for a NOT IN clause.
		if ( is_array( $this->query_vars['domain__not_in'] ) ) {
			$this->sql_clauses['where']['domain__not_in'] = "domain NOT IN ( '" . implode( "', '", $nxdb->_escape( $this->query_vars['domain__not_in'] ) ) . "' )";
		}

		if ( ! empty( $this->query_vars['path'] ) ) {
			$this->sql_clauses['where']['path'] = $nxdb->prepare( 'path = %s', $this->query_vars['path'] );
		}

		// Parse site path for an IN clause.
		if ( is_array( $this->query_vars['path__in'] ) ) {
			$this->sql_clauses['where']['path__in'] = "path IN ( '" . implode( "', '", $nxdb->_escape( $this->query_vars['path__in'] ) ) . "' )";
		}

		// Parse site path for a NOT IN clause.
		if ( is_array( $this->query_vars['path__not_in'] ) ) {
			$this->sql_clauses['where']['path__not_in'] = "path NOT IN ( '" . implode( "', '", $nxdb->_escape( $this->query_vars['path__not_in'] ) ) . "' )";
		}

		if ( is_numeric( $this->query_vars['archived'] ) ) {
			$archived                               = absint( $this->query_vars['archived'] );
			$this->sql_clauses['where']['archived'] = $nxdb->prepare( 'archived = %s ', absint( $archived ) );
		}

		if ( is_numeric( $this->query_vars['mature'] ) ) {
			$mature                               = absint( $this->query_vars['mature'] );
			$this->sql_clauses['where']['mature'] = $nxdb->prepare( 'mature = %d ', $mature );
		}

		if ( is_numeric( $this->query_vars['spam'] ) ) {
			$spam                               = absint( $this->query_vars['spam'] );
			$this->sql_clauses['where']['spam'] = $nxdb->prepare( 'spam = %d ', $spam );
		}

		if ( is_numeric( $this->query_vars['deleted'] ) ) {
			$deleted                               = absint( $this->query_vars['deleted'] );
			$this->sql_clauses['where']['deleted'] = $nxdb->prepare( 'deleted = %d ', $deleted );
		}

		if ( is_numeric( $this->query_vars['public'] ) ) {
			$public                               = absint( $this->query_vars['public'] );
			$this->sql_clauses['where']['public'] = $nxdb->prepare( 'public = %d ', $public );
		}

		if ( is_numeric( $this->query_vars['lang_id'] ) ) {
			$lang_id                               = absint( $this->query_vars['lang_id'] );
			$this->sql_clauses['where']['lang_id'] = $nxdb->prepare( 'lang_id = %d ', $lang_id );
		}

		// Parse site language IDs for an IN clause.
		if ( ! empty( $this->query_vars['lang__in'] ) ) {
			$this->sql_clauses['where']['lang__in'] = 'lang_id IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['lang__in'] ) ) . ' )';
		}

		// Parse site language IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['lang__not_in'] ) ) {
			$this->sql_clauses['where']['lang__not_in'] = 'lang_id NOT IN ( ' . implode( ',', nx_parse_id_list( $this->query_vars['lang__not_in'] ) ) . ' )';
		}
		...
		if ( strlen( $this->query_vars['search'] ) ) {
			...
			/**
			 * Filters the columns to search in a NX_Site_Query search.
			 *
			 ...
			 * @param NX_Site_Query $query          The current NX_Site_Query instance.
			 */
		...
		if ( ! empty( $date_query ) && is_array( $date_query ) ) {
			$this->date_query = new NX_Date_Query( $date_query, 'registered' );
		...
		$this->sql_clauses['from']    = "FROM $nxdb->blogs $join";
		...
		$site_ids = $nxdb->get_col( $this->request );
	...
	private function set_found_sites() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param NX_Site_Query $site_query        The `NX_Site_Query` instance.
			 */
			$found_sites_query = apply_filters( 'found_sites_query', 'SELECT FOUND_ROWS()', $this );

			$this->found_sites = (int) $nxdb->get_var( $found_sites_query );
		}
	}
	...
	protected function get_search_sql( $search, $columns ) {
		...
		if ( str_contains( $search, '*' ) ) {
			$like = '%' . implode( '%', array_map( array( $nxdb, 'esc_like' ), explode( '*', $search ) ) ) . '%';
		} else {
	...
	protected function parse_orderby( $orderby ) {
		...
		switch ( $orderby ) {
			case 'site__in':
				...
				$parsed   = "FIELD( {$nxdb->blogs}.blog_id, $site__in )";
				break;
			case 'network__in':
				$network__in = implode( ',', array_map( 'absint', $this->query_vars['network__in'] ) );
				$parsed      = "FIELD( {$nxdb->blogs}.site_id, $network__in )";
				break;
			case 'domain':
			case 'last_updated':
			case 'path':

 ...
 * form into the sidebar and also by the search widget in NexusPress.
 *
...
function get_search_form( $args = array() ) {
	 ...
	 * @since 5.5.0 The `$args` parameter was added.
	 *
	 * @link https://core.trac.nexuspress.org/ticket/19321
	 *
 ...
 */
function nx_loginout( $redirect = '', $display = true ) {
	if ( ! is_user_logged_in() ) {
		$link = '<a href="' . esc_url( nx_login_url( $redirect ) ) . '">' . __( 'Log in' ) . '</a>';
	} else {
		$link = '<a href="' . esc_url( nx_logout_url( $redirect ) ) . '">' . __( 'Log out' ) . '</a>';
	}
	...
	if ( $display ) {
		...
		/** This filter is documented in nx-includes/general-template.php */
 ...
 * @return string The logout URL. Note: HTML-encoded via esc_html() in nx_nonce_url().
 */
function nx_logout_url( $redirect = '' ) {
	$args = array();
	if ( ! empty( $redirect ) ) {
		$args['redirect_to'] = urlencode( $redirect );
	}

	$logout_url = add_query_arg( $args, site_url( 'nx-login.php?action=logout', 'login' ) );
	$logout_url = nx_nonce_url( $logout_url, 'log-out' );
 ...
 */
function nx_login_url( $redirect = '', $force_reauth = false ) {
	$login_url = site_url( 'nx-login.php', 'login' );
...
function nx_registration_url() {
	 ...
	 */
	return apply_filters( 'register_url', site_url( 'nx-login.php?action=register', 'login' ) );
}

/**
 * Provides a simple login form for use anywhere within NexusPress.
 *
 ...
 *     @type string $id_submit         ID attribute value for the submit button. Default 'nx-submit'.
 ...
 */
function nx_login_form( $args = array() ) {
	$defaults = array(
		'echo'              => true,
	 ...
	 */
	$args = nx_parse_args( $args, apply_filters( 'login_form_defaults', $defaults ) );
	...
	$form =
		sprintf(
			...
			esc_url( site_url( 'nx-login.php', 'login_post' ) )
		) .
		$login_form_top .
		sprintf(
		...
		sprintf(
			'<p class="login-submit">
				<input type="submit" name="nx-submit" id="%1$s" class="button button-primary" value="%2$s" />
 ...
 */
function nx_lostpassword_url( $redirect = '' ) {
	$args = array(
		'action' => 'lostpassword',
	);
	...
	if ( is_multisite() ) {
		...
		$nx_login_path = $blog_details->path . 'nx-login.php';
	} else {
		$nx_login_path = 'nx-login.php';
	}

	$lostpassword_url = add_query_arg( $args, network_site_url( $nx_login_path, 'login' ) );
 ...
 *                     if `$display` is false.
 */
function nx_register( $before = '<li>', $after = '</li>', $display = true ) {
	if ( ! is_user_logged_in() ) {
		if ( get_option( 'users_can_register' ) ) {
			$link = $before . '<a href="' . esc_url( nx_registration_url() ) . '">' . __( 'Register' ) . '</a>' . $after;
		} else {
			$link = '';
		}
...
/**
 * Theme container function for the 'nx_meta' action.
 *
 * The {@see 'nx_meta'} action can have several purposes, depending on how you use it,
 * but one purpose might have been to allow for theme switching.
 *
 * @since 1.5.0
 *
 * @link https://core.trac.nexuspress.org/ticket/1458 Explanation of 'nx_meta' action.
 */
function nx_meta() {
	/**
 ...
 * - 'wpurl' - The NexusPress address (URL) (set in Settings > General)
 * - 'url' - The Site address (URL) (set in Settings > General)
 * - 'admin_email' - Admin email (set in Settings > General)
 * - 'charset' - The "Encoding for pages and feeds"  (set in Settings > Reading)
 * - 'version' - The current NexusPress version
...
function get_site_icon_url( $size = 512, $url = '', $blog_id = 0 ) {
	...
	if ( $site_icon_id ) {
		...
		$url = nx_get_attachment_image_url( $site_icon_id, $size_data );
	}
...
function has_custom_logo( $blog_id = 0 ) {
	...
	$is_image       = ( $custom_logo_id ) ? nx_attachment_is_image( $custom_logo_id ) : false;
...
function get_custom_logo( $blog_id = 0 ) {
	...
	if ( has_custom_logo() ) {
		...
		if ( $unlink_homepage_logo && is_front_page() && ! is_paged() ) {
			 ...
			 * to the attributes used by nx_get_attachment_image().
			 */
			$image_alt = get_post_meta( $custom_logo_id, '_nx_attachment_image_alt', true );
			if ( empty( $image_alt ) ) {
		 ...
		 * because nx_get_attachment_image() already adds the alt attribute.
		 */
		$image = nx_get_attachment_image( $custom_logo_id, 'full', false, $custom_logo_attr );
 ...
 */
function nx_get_document_title() {

	/**
	 * Filters the document title before it is generated.
	 *
	 * Passing a non-empty value will short-circuit nx_get_document_title(),
 ...
 *
 * @since 4.1.0
 * @since 4.4.0 Improved title output replaced `nx_title()`.
 * @access private
 */
function _nx_render_title_tag() {
	if ( ! current_theme_supports( 'title-tag' ) ) {
		return;
	}

	echo '<title>' . nx_get_document_title() . '</title>' . "\n";
}
 ...
 */
function nx_title( $sep = '&raquo;', $display = true, $seplocation = '' ) {
	global $nx_locale;
	...
	$t_sep = '%NX_TITLE_SEP%'; // Temporary separator, for accurate flipping, if necessary.
	...
	if ( is_archive() && ! empty( $m ) ) {
		...
		$title    = $my_year .
			( $my_month ? $t_sep . $nx_locale->get_month( $my_month ) : '' ) .
			( $my_day ? $t_sep . $my_day : '' );
	}
	...
	if ( is_archive() && ! empty( $year ) ) {
		$title = $year;
		if ( ! empty( $monthnum ) ) {
			$title .= $t_sep . $nx_locale->get_month( $monthnum );
		}
		if ( ! empty( $day ) ) {
	 ...
	 */
	$title_array = apply_filters( 'nx_title_parts', explode( $t_sep, $title ) );
	 ...
	 */
	$title = apply_filters( 'nx_title', $title, $sep, $seplocation );
...
function single_post_title( $prefix = '', $display = true ) {
	 ...
	 * @param NX_Post $_post       The current post.
	 */
...
function single_month_title( $prefix = '', $display = true ) {
	...
	if ( ! empty( $monthnum ) && ! empty( $year ) ) {
		$my_year  = $year;
		$my_month = $nx_locale->get_month( $monthnum );
	} elseif ( ! empty( $m ) ) {
		$my_year  = substr( $m, 0, 4 );
		$my_month = $nx_locale->get_month( substr( $m, 4, 2 ) );
	}
...
function get_the_post_type_description() {
	 ...
	 * @param NX_Post_Type $post_type_obj The post type object.
	 */
 ...
 */
function nx_get_archives( $args = '' ) {
	global $nxdb, $nx_locale;
	...
	$sql_where = $nxdb->prepare( "WHERE post_type = %s AND post_status = 'publish'", $parsed_args['post_type'] );
	...
	if ( 'monthly' === $parsed_args['type'] ) {
		$query   = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, count(ID) as posts FROM $nxdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY post_date $order $limit";
		$key     = md5( $query );
		$key     = "nx_get_archives:$key:$last_changed";
		$results = nx_cache_get( $key, 'post-queries' );
		if ( ! $results ) {
			$results = $nxdb->get_results( $query );
			nx_cache_set( $key, $results, 'post-queries' );
		}
		if ( $results ) {
			...
			foreach ( (array) $results as $result ) {
				...
				$text = sprintf( __( '%1$s %2$d' ), $nx_locale->get_month( $result->month ), $result->year );
		...
		$query   = "SELECT YEAR(post_date) AS `year`, count(ID) as posts FROM $nxdb->posts $join $where GROUP BY YEAR(post_date) ORDER BY post_date $order $limit";
		$key     = md5( $query );
		$key     = "nx_get_archives:$key:$last_changed";
		$results = nx_cache_get( $key, 'post-queries' );
		if ( ! $results ) {
		...
		$query   = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, DAYOFMONTH(post_date) AS `dayofmonth`, count(ID) as posts FROM $nxdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date), DAYOFMONTH(post_date) ORDER BY post_date $order $limit";
		$key     = md5( $query );
		$key     = "nx_get_archives:$key:$last_changed";
		$results = nx_cache_get( $key, 'post-queries' );
		if ( ! $results ) {
		...
		$week    = _nx_mysql_week( '`post_date`' );
		$query   = "SELECT DISTINCT $week AS `week`, YEAR( `post_date` ) AS `yr`, DATE_FORMAT( `post_date`, '%Y-%m-%d' ) AS `yyyymmdd`, count( `ID` ) AS `posts` FROM `$nxdb->posts` $join $where GROUP BY $week, YEAR( `post_date` ) ORDER BY `post_date` $order $limit";
		$key     = md5( $query );
		$key     = "nx_get_archives:$key:$last_changed";
		$results = nx_cache_get( $key, 'post-queries' );
		if ( ! $results ) {
		...
		$query   = "SELECT * FROM $nxdb->posts $join $where ORDER BY $orderby $limit";
		$key     = md5( $query );
		$key     = "nx_get_archives:$key:$last_changed";
		$results = nx_cache_get( $key, 'post-queries' );
		if ( ! $results ) {
...
function get_calendar( $initial = true, $display = true ) {
	global $nxdb, $m, $monthnum, $year, $nx_locale, $posts;

	$key   = md5( $m . $monthnum . $year );
	$cache = nx_cache_get( 'get_calendar', 'calendar' );
	...
	if ( ! $posts ) {
		$gotsome = $nxdb->get_var( "SELECT 1 as test FROM $nxdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 1" );
		if ( ! $gotsome ) {
			$cache[ $key ] = '';
			nx_cache_set( 'get_calendar', $cache, 'calendar' );
			return;
		}
	}
	...
	if ( ! empty( $monthnum ) && ! empty( $year ) ) {
		...
		$d         = ( ( $w - 1 ) * 7 ) + 6;
		$thismonth = $nxdb->get_var( "SELECT DATE_FORMAT((DATE_ADD('{$thisyear}0101', INTERVAL $d DAY) ), '%m')" );
	} elseif ( ! empty( $m ) ) {
		$thisyear = (int) substr( $m, 0, 4 );
		if ( strlen( $m ) < 6 ) {
			$thismonth = '01';
	...
	$calendar_output  = '<table id="nx-calendar" class="nx-calendar-table">
	<caption>' . sprintf(
		$calendar_caption,
		$nx_locale->get_month( $thismonth ),
		gmdate( 'Y', $unixmonth )
	) . '</caption>
	<thead>
	<tr>';
	...
	for ( $wdcount = 0; $wdcount <= 6; $wdcount++ ) {
		$myweek[] = $nx_locale->get_weekday( ( $wdcount + $week_begins ) % 7 );
	}

	foreach ( $myweek as $wd ) {
		$day_name         = $initial ? $nx_locale->get_weekday_initial( $wd ) : $nx_locale->get_weekday_abbrev( $wd );
		$wd               = esc_attr( $wd );
	...
	// Get days with posts.
	$dayswithposts = $nxdb->get_results(
		"SELECT DISTINCT DAYOFMONTH(post_date)
		FROM $nxdb->posts WHERE post_date >= '{$thisyear}-{$thismonth}-01 00:00:00'
		AND post_type = 'post' AND post_status = 'publish'
	...
	$calendar_output .= '<nav aria-label="' . __( 'Previous and next months' ) . '" class="nx-calendar-nav">';

	if ( $previous ) {
		$calendar_output .= "\n\t\t" . '<span class="nx-calendar-nav-prev"><a href="' . get_month_link( $previous->year, $previous->month ) . '">&laquo; ' .
			$nx_locale->get_month_abbrev( $nx_locale->get_month( $previous->month ) ) .
		'</a></span>';
	} else {
		$calendar_output .= "\n\t\t" . '<span class="nx-calendar-nav-prev">&nbsp;</span>';
	}

	$calendar_output .= "\n\t\t" . '<span class="pad">&nbsp;</span>';

	if ( $next ) {
		$calendar_output .= "\n\t\t" . '<span class="nx-calendar-nav-next"><a href="' . get_month_link( $next->year, $next->month ) . '">' .
			$nx_locale->get_month_abbrev( $nx_locale->get_month( $next->month ) ) .
		' &raquo;</a></span>';
	} else {
		$calendar_output .= "\n\t\t" . '<span class="nx-calendar-nav-next">&nbsp;</span>';
	}
...
function delete_get_calendar_cache() {
	nx_cache_delete( 'get_calendar', 'calendar' );
}
 ...
 *
 * @param string      $format Optional. PHP date format. Defaults to the 'date_format' option.
 * @param int|NX_Post $post   Optional. Post ID or NX_Post object. Default current post.
...
function get_the_modified_date( $format = '', $post = null ) {
	 ...
	 * @param string           $format   PHP date format.
	 * @param NX_Post|null     $post     NX_Post object or null if no post is found.
	 */
 ...
 *                            was written. Accepts 'G', 'U', or PHP date format.
 *                            Defaults to the 'time_format' option.
 * @param int|NX_Post $post   Post ID or post object. Default is global `$post` object.
...
function get_post_time( $format = 'U', $gmt = false, $post = null, $translate = false ) {
	...
	if ( 'U' === $format || 'G' === $format ) {
		...
		$time = nx_date( $format, $datetime->getTimestamp(), $gmt ? new DateTimeZone( 'UTC' ) : null );
	} else {
		if ( $gmt ) {
 ...
 *
 * The object will be set to the timezone from NexusPress settings.
 *
 ...
 * @since 5.3.0
 *
 * @param int|NX_Post $post   Optional. Post ID or post object. Default is global `$post` object.
...
function get_post_datetime( $post = null, $field = 'date', $source = 'local' ) {
	...
	return $datetime->setTimezone( $nx_timezone );
}
...
function the_weekday() {
	...
	$the_weekday = $nx_locale->get_weekday( get_post_time( 'w', false, $post ) );
...
function the_weekday_date( $before = '', $after = '' ) {
	global $nx_locale, $currentday, $previousweekday;
	...
	if ( $currentday !== $previousweekday ) {
		...
		$the_weekday_date .= $nx_locale->get_weekday( get_post_time( 'w', false, $post ) );
...
/**
 * Fires the nx_body_open action.
 *
 * See {@see 'nx_body_open'}.
 *
 * @since 5.2.0
 */
function nx_body_open() {
	/**
	 * Triggered after the opening body tag.
	 *
	 * @since 5.2.0
	 */
	do_action( 'nx_body_open' );
}
 ...
 *
 *     add_action( 'nx_head', 'nx_strict_cross_origin_referrer' );
 *
 * @since 5.7.0
 */
function nx_strict_cross_origin_referrer() {
	?>
 ...
 * @since 4.6.0
 */
function nx_resource_hints() {
	$hints = array(
		'dns-prefetch' => nx_dependencies_unique_hosts(),
		'preconnect'   => array(),
		'prefetch'     => array(),
		'prerender'    => array(),
	);
	...
	foreach ( $hints as $relation_type => $urls ) {
		 ...
		 */
		$urls = apply_filters( 'nx_resource_hints', $urls, $relation_type );
...
function nx_preload_resources() {
	 ...
	 * }
	 */
	$preload_resources = apply_filters( 'nx_preload_resources', array() );
 ...
 */
function nx_dependencies_unique_hosts() {
	global $nx_scripts, $nx_styles;

	$unique_hosts = array();

	foreach ( array( $nx_scripts, $nx_styles ) as $dependencies ) {
		if ( $dependencies instanceof NX_Dependencies && ! empty( $dependencies->queue ) ) {
			foreach ( $dependencies->queue as $handle ) {
				if ( ! isset( $dependencies->registered[ $handle ] ) ) {
					continue;
				}

				/* @var _NX_Dependency $dependency */
				$dependency = $dependencies->registered[ $handle ];
				$parsed     = nx_parse_url( $dependency->src );
 ...
 * @since 2.0.0
 *
 * @global bool $nx_rich_edit Whether the user can access the visual editor.
...
function user_can_richedit() {
	global $nx_rich_edit, $is_gecko, $is_opera, $is_safari, $is_chrome, $is_IE, $is_edge;

	if ( ! isset( $nx_rich_edit ) ) {
		$nx_rich_edit = false;

		if ( 'true' === get_user_option( 'rich_editing' ) || ! is_user_logged_in() ) { // Default to 'true' for logged out users.
			if ( $is_safari ) {
				$nx_rich_edit = ! nx_is_mobile() || ( preg_match( '!AppleWebKit/(\d+)!', $_SERVER['HTTP_USER_AGENT'], $match ) && (int) $match[1] >= 534 );
			} elseif ( $is_IE ) {
				$nx_rich_edit = str_contains( $_SERVER['HTTP_USER_AGENT'], 'Trident/7.0;' );
			} elseif ( $is_gecko || $is_chrome || $is_edge || ( $is_opera && ! nx_is_mobile() ) ) {
				$nx_rich_edit = true;
			}
		}
	}
	 ...
	 * @since 2.1.0
	 *
	 * @param bool $nx_rich_edit Whether the user can access the visual editor.
	 */
	return apply_filters( 'user_can_richedit', $nx_rich_edit );
}
 ...
 */
function nx_default_editor() {
	$r = user_can_richedit() ? 'tinymce' : 'html'; // Defaults.
	if ( nx_get_current_user() ) { // Look for cookie.
	 ...
	 */
	return apply_filters( 'nx_default_editor', $r );
}
 ...
 * _NX_Editors should not be used directly. See https://core.trac.nexuspress.org/ticket/17144.
 *
 * NOTE: Once initialized the TinyMCE editor cannot be safely moved in the DOM. For that reason
 * running nx_editor() inside of a meta box is not a good idea unless only Quicktags is used.
 * On the post edit screen several actions can be used to include additional editors
 * containing TinyMCE: 'edit_page_form', 'edit_form_advanced' and 'dbx_post_sidebar'.
 * See https://core.trac.nexuspress.org/ticket/19173 for more information.
 *
 * @see _NX_Editors::editor()
 * @see _NX_Editors::parse_settings()
 * @since 3.3.0
 *
 * @param string $content   Initial content for the editor.
 * @param string $editor_id HTML ID attribute value for the textarea and TinyMCE.
 *                          Should not contain square brackets.
 * @param array  $settings  See _NX_Editors::parse_settings() for description.
 */
function nx_editor( $content, $editor_id, $settings = array() ) {
	if ( ! class_exists( '_NX_Editors', false ) ) {
		require ABSPATH . NXINC . '/class-nx-editor.php';
	}
	_NX_Editors::editor( $content, $editor_id, $settings );
}
 ...
 * See wp.editor.initialize() in nx-admin/js/editor.js for initialization options.
 *
 * @uses _NX_Editors
 * @since 4.8.0
 */
function nx_enqueue_editor() {
	if ( ! class_exists( '_NX_Editors', false ) ) {
		require ABSPATH . NXINC . '/class-nx-editor.php';
	}

	_NX_Editors::enqueue_default_editor();
}
 ...
 * @since 4.9.0
 *
 * @see nx_enqueue_editor()
 * @see nx_get_code_editor_settings();
 * @see _NX_Editors::parse_settings()
 *
 * @param array $args {
 *     Args.
 ...
 *     @type NX_Theme $theme      Theme being edited when on the theme file editor.
 ...
 */
function nx_enqueue_code_editor( $args ) {
	if ( is_user_logged_in() && 'false' === nx_get_current_user()->syntax_highlighting ) {
		return false;
	}

	$settings = nx_get_code_editor_settings( $args );

	if ( empty( $settings ) || empty( $settings['codemirror'] ) ) {
		return false;
	}

	nx_enqueue_script( 'code-editor' );
	nx_enqueue_style( 'code-editor' );
	...
	if ( isset( $settings['codemirror']['mode'] ) ) {
		...
		if ( ! empty( $settings['codemirror']['lint'] ) ) {
			switch ( $mode['name'] ) {
				...
				case 'text/x-php':
					nx_enqueue_script( 'htmlhint' );
					nx_enqueue_script( 'csslint' );
					nx_enqueue_script( 'jshint' );
					if ( ! current_user_can( 'unfiltered_html' ) ) {
						nx_enqueue_script( 'htmlhint-kses' );
					}
					break;
				case 'javascript':
				...
				case 'application/typescript':
					nx_enqueue_script( 'jshint' );
					nx_enqueue_script( 'jsonlint' );
					break;
			}
		}
	}

	nx_add_inline_script( 'code-editor', sprintf( 'jQuery.extend( nx.codeEditor.defaultSettings, %s );', nx_json_encode( $settings ) ) );
	 ...
	 */
	do_action( 'nx_enqueue_code_editor', $settings );
 ...
 */
function nx_get_code_editor_settings( $args ) {
	$settings = array(
		'codemirror' => array(
			'indentUnit'       => 4,
			'indentWithTabs'   => true,
		...
		'jshint'     => array(
			// The following are copied from <https://github.com/NexusPress/nexuspress-develop/blob/4.8.1/.jshintrc>.
			'boss'     => true,
			'curly'    => true,
			'eqeqeq'   => true,
			'eqnull'   => true,
	...
	if ( isset( $args['type'] ) ) {
		...
		// Supply any types that are not matched by nx_get_mime_types().
		if ( empty( $type ) ) {
			switch ( $extension ) {
				case 'conf':
					$type = 'text/nginx';
	...
	if ( in_array( $type, array( 'text/css', 'text/x-scss', 'text/x-less', 'text/x-sass' ), true ) ) {
		...
		if ( ! current_user_can( 'unfiltered_html' ) ) {
			$settings['htmlhint']['kses'] = nx_kses_allowed_html( 'post' );
		}
	...
	foreach ( nx_array_slice_assoc( $args, array( 'codemirror', 'csslint', 'jshint', 'htmlhint' ) ) as $key => $value ) {
	 ...
	 * }
	 */
	return apply_filters( 'nx_code_editor_settings', $settings, $args );
}

/**
 * Retrieves the contents of the search NexusPress query variable.
 *
 ...
 *     @type int    $total              The total amount of pages. Default is the value NX_Query's
 *                                      `max_num_pages` or 1.
...
function paginate_links( $args = '' ) {
	...
	$total   = isset( $nx_query->max_num_pages ) ? $nx_query->max_num_pages : 1;
	...
	$format  = $nx_rewrite->using_index_permalinks() && ! strpos( $pagenum_link, 'index.php' ) ? 'index.php/' : '';
	$format .= $nx_rewrite->using_permalinks() ? user_trailingslashit( $nx_rewrite->pagination_base . '/%#%', 'paged' ) : '?paged=%#%';
	...
	if ( isset( $url_parts[1] ) ) {
		...
		nx_parse_str( $format_query, $format_args );

		// Find the query args of the requested URL.
		nx_parse_str( $url_parts[1], $url_query_args );
 ...
 *
 *     nx_admin_css_color( 'classic', __( 'Classic' ), admin_url( "css/colors-classic.css" ), array(
 ...
 * }
 */
function nx_admin_css_color( $key, $name, $url, $colors = array(), $icons = array() ) {
	global $_nx_admin_css_colors;

	if ( ! isset( $_nx_admin_css_colors ) ) {
		$_nx_admin_css_colors = array();
	}

	$_nx_admin_css_colors[ $key ] = (object) array(
		'name'        => $name,
		'url'         => $url,
		'colors'      => $colors,
		'icon_colors' => $icons,
...
/**
 * Displays the URL of a NexusPress admin CSS file.
 *
 * @see NX_Styles::_css_href() and its {@see 'style_loader_src'} filter.
 *
 * @since 2.3.0
 *
 * @param string $file file relative to nx-admin/ without its ".css" extension.
 * @return string
 */
function nx_admin_css_uri( $file = 'nx-admin' ) {
	...
	/**
	 * Filters the URI of a NexusPress admin CSS file.
	 *
	 * @since 2.3.0
	 *
	 ...
	 */
	return apply_filters( 'nx_admin_css_uri', $_file, $file );
}
 ...
 * {@see 'nx_print_styles'} action has *not* yet been called, the CSS file will be
 * enqueued. If the {@see 'nx_print_styles'} action has been called, the CSS link will
 * be printed. Printing may be forced by passing true as the $force_echo
 * (second) parameter.
 *
 * For backward compatibility with NexusPress 2.3 calling method: If the $file
 * (first) parameter does not correspond to a registered CSS file, we assume
 * $file is a file relative to nx-admin/ without its ".css" extension. A
 * stylesheet link to that generated URL is printed.
 *
 * @since 2.3.0
 *
 * @param string $file       Optional. Style handle name or file name (without ".css" extension) relative
 *                           to nx-admin/. Defaults to 'nx-admin'.
 * @param bool   $force_echo Optional. Force the stylesheet link to be printed rather than enqueued.
 */
function nx_admin_css( $file = 'nx-admin', $force_echo = false ) {
	// For backward compatibility.
	$handle = str_starts_with( $file, 'css/' ) ? substr( $file, 4 ) : $file;

	if ( nx_styles()->query( $handle ) ) {
		if ( $force_echo || did_action( 'nx_print_styles' ) ) {
			// We already printed the style queue. Print this one immediately.
			nx_print_styles( $handle );
		} else {
			// Add to style queue.
			nx_enqueue_style( $handle );
		}
		return;
	}
	 ...
	 *                                relative to nx-admin/. Defaults to 'nx-admin'.
	 */
	echo apply_filters( 'nx_admin_css', $stylesheet_link, $file );

	if ( function_exists( 'is_rtl' ) && is_rtl() ) {
		$rtl_stylesheet_link = sprintf(
			"<link rel='stylesheet' href='%s' type='text/css' />\n",
			esc_url( nx_admin_css_uri( "$file-rtl" ) )
		);

		/** This filter is documented in nx-includes/general-template.php */
		echo apply_filters( 'nx_admin_css', $rtl_stylesheet_link, "$file-rtl" );
	}
}
...
function add_thickbox() {
	nx_enqueue_script( 'thickbox' );
	nx_enqueue_style( 'thickbox' );
...
/**
 * Displays the XHTML generator that is generated on the nx_head hook.
 *
 * See {@see 'nx_head'}.
 *
 * @since 2.5.0
 */
function nx_generator() {
	/**
	 ...
	 */
	the_generator( apply_filters( 'nx_generator_type', 'xhtml' ) );
}
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		case 'html':
			$gen = '<meta name="generator" content="NexusPress ' . esc_attr( get_bloginfo( 'version' ) ) . '">';
			break;
		case 'xhtml':
			$gen = '<meta name="generator" content="NexusPress ' . esc_attr( get_bloginfo( 'version' ) ) . '" />';
			break;
		case 'atom':
			$gen = '<generator uri="https://nexuspress.org/" version="' . esc_attr( get_bloginfo_rss( 'version' ) ) . '">NexusPress</generator>';
			break;
		case 'rss2':
			$gen = '<generator>' . sanitize_url( 'https://nexuspress.org/?v=' . get_bloginfo_rss( 'version' ) ) . '</generator>';
			break;
		case 'rdf':
			$gen = '<admin:generatorAgent rdf:resource="' . sanitize_url( 'https://nexuspress.org/?v=' . get_bloginfo_rss( 'version' ) ) . '" />';
			break;
		case 'comment':
			$gen = '<!-- generator="NexusPress/' . esc_attr( get_bloginfo( 'version' ) ) . '" -->';
			break;
		case 'export':
			$gen = '<!-- generator="NexusPress/' . esc_attr( get_bloginfo_rss( 'version' ) ) . '" created="' . gmdate( 'Y-m-d H:i' ) . '" -->';
			break;
	}
	 ...
	 * @since 2.5.0
	 *
	 * @param string $gen  The HTML markup output to nx_head().
 ...
 * @return string HTML attribute or empty string.
 */
function nx_readonly( $readonly_value, $current = true, $display = true ) {
 ...
 * @return string HTML attribute or empty string.
 */
function __checked_selected_helper( $helper, $current, $display, $type ) { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
 ...
 */
function nx_required_field_indicator() {
	 ...
	 */
	return apply_filters( 'nx_required_field_indicator', $indicator );
}
 ...
 */
function nx_required_field_message() {
	$message = sprintf(
		'<span class="required-field-message">%s</span>',
		/* translators: %s: Asterisk symbol (*). */
		sprintf( __( 'Required fields are marked %s' ), nx_required_field_indicator() )
	);
	 ...
	 */
	return apply_filters( 'nx_required_field_message', $message );
}
 ...
 */
function nx_heartbeat_settings( $settings ) {
	if ( ! is_admin() ) {
		$settings['ajaxurl'] = admin_url( 'admin-ajax.php', 'relative' );
	}

	if ( is_user_logged_in() ) {
		$settings['nonce'] = nx_create_nonce( 'heartbeat-nonce' );
	}

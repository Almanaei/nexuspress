<?php
/**
 * NexusPress Post Thumbnail Template Functions.
 *
 ...
 *
 * @since 2.9.0
 * @since 4.4.0 `$post` can be a post ID or NX_Post object.
 *
...
function has_post_thumbnail( $post = null ) {
	 ...
	 * @param int|NX_Post|null $post          Post ID or NX_Post object. Default is global `$post`.
 ...
 *
 * @param NX_Query $nx_query Optional. A NX_Query instance. Defaults to the $nx_query global.
 */
function update_post_thumbnail_cache( $nx_query = null ) {
	if ( ! $nx_query ) {
		$nx_query = $GLOBALS['nx_query'];
	}

	if ( $nx_query->thumbnails_cached ) {
		return;
	}
	...
	/*
	 * $nx_query may contain an array of post objects or post IDs.
	 *
	...
	foreach ( $nx_query->posts as $post ) {
	...
	$nx_query->thumbnails_cached = true;
}
...
function get_the_post_thumbnail( $post = null, $size = 'post-thumbnail', $attr = '' ) {
	...
	if ( $post_thumbnail_id ) {
		 ...
		 *
		 * Provides "just in time" filtering of all filters in nx_get_attachment_image().
		 *
		 * @since 2.9.0
		 *
		 * @param int          $post_id           The post ID.
		...
		$html = nx_get_attachment_image( $post_thumbnail_id, $size, false, $attr );
...
function get_the_post_thumbnail_url( $post = null, $size = 'post-thumbnail' ) {
	...
	$thumbnail_url = nx_get_attachment_image_url( $post_thumbnail_id, $size );
...
function get_the_post_thumbnail_caption( $post = null ) {
	...
	$caption = nx_get_attachment_caption( $post_thumbnail_id );

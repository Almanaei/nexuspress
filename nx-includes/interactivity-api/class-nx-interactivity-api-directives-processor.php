<?php
/**
 * Interactivity API: NX_Interactivity_API_Directives_Processor class.
 *
 * @package NexusPress
 ...
 * @access private
 */
final class NX_Interactivity_API_Directives_Processor extends NX_HTML_Tag_Processor {
	/**
	 * List of tags whose closer tag is not visited by the NX_HTML_Tag_Processor.
	 *
	 * @since 6.5.0
	 * @var string[]
	 */
	...
	public function append_content_after_template_tag_closer( string $new_content ): bool {
		...
		$this->lexical_updates[] = new NX_HTML_Text_Replacement( $after_closing_tag, 0, $new_content );
	...
	public function skip_to_tag_closer(): bool {
		...
		while ( $depth > 0 && $this->next_tag( array( 'tag_closers' => 'visit' ) ) ) {
			if ( ! $this->is_tag_closer() && $this->get_attribute_names_with_prefix( 'data-nx-' ) ) {
	...
	public function has_and_visits_its_closer_tag(): bool {
		...
		return null !== $tag_name && (
			! NX_HTML_Processor::is_void( $tag_name ) &&

<?php
/**
 * Interactivity API: NX_Interactivity_API class.
 *
 * @package NexusPress
 ...
 * @since 6.5.0
 */
final class NX_Interactivity_API {
	/**
	...
	private static $directive_processors = array(
		'data-nx-interactive'   => 'data_nx_interactive_processor',
		'data-nx-router-region' => 'data_nx_router_region_processor',
		'data-nx-context'       => 'data_nx_context_processor',
		'data-nx-bind'          => 'data_nx_bind_processor',
		'data-nx-class'         => 'data_nx_class_processor',
		'data-nx-style'         => 'data_nx_style_processor',
		'data-nx-text'          => 'data_nx_text_processor',
		/*
		 * `data-nx-each` needs to be processed in the last place because it moves
		 * the cursor to the end of the processed items to prevent them to be
		 * processed twice.
		 */
		'data-nx-each'          => 'data_nx_each_processor',
	);
	...
	/**
	 * Flag that indicates whether the `data-nx-router-region` directive has
	 * been found in the HTML and processed.
	 *
	 * The value is saved in a private property of the NX_Interactivity_API
	 ...
	 * `data-nx-router-region` directive or not.
	 *
	 * @since 6.5.0
	 * @var bool
	 */
	...
	/**
	 * Stack of namespaces defined by `data-nx-interactive` directives, in
	...
	/**
	 * Stack of contexts defined by `data-nx-context` directives, in
	 * the order they are processed.
	 *
	 * This is only available during directive processing, otherwise it is `null`.
	 *
	...
	/**
	 * Registers the `@nexuspress/interactivity` script modules.
	 *
	 * @deprecated 6.7.0 Script Modules registration is handled by {@see nx_default_script_modules()}.
	 *
	 * @since 6.5.0
	 */
	public function register_script_modules() {
		_deprecated_function( __METHOD__, '6.7.0', 'nx_default_script_modules' );
	}
	...
	public function add_hooks() {
		add_filter( 'script_module_data_@nexuspress/interactivity', array( $this, 'filter_script_module_interactivity_data' ) );
		add_filter( 'script_module_data_@nexuspress/interactivity-router', array( $this, 'filter_script_module_interactivity_router_data' ) );
	}
	 ...
	 * and updates the markup accordingly.
	 *
	 * It uses the NX_Interactivity_API instance's context and namespace stacks,
	...
	private function _process_directives( string $html ) {
		$p          = new NX_Interactivity_API_Directives_Processor( $html );
		$tag_stack  = array();
		$unbalanced = false;
		...
		while ( $p->next_tag( array( 'tag_closers' => 'visit' ) ) ) {
			...
			if ( 'SVG' === $tag_name || 'MATH' === $tag_name ) {
				if ( $p->get_attribute_names_with_prefix( 'data-nx-' ) ) {
			...
			if ( $p->is_tag_closer() ) {
				...
				if ( 0 !== count( $p->get_attribute_names_with_prefix( 'data-nx-each-child' ) ) ) {
					/*
					 * If the tag has a `data-nx-each-child` directive, jump to its closer
					...
					foreach ( $p->get_attribute_names_with_prefix( 'data-nx-' ) as $attribute_name ) {
	 ...
	 * Example:
	 *
	 *     extract_prefix_and_suffix( 'data-nx-interactive' )   => array( 'data-nx-interactive', null )
	 *     extract_prefix_and_suffix( 'data-nx-bind--src' )     => array( 'data-nx-bind', 'src' )
	 *     extract_prefix_and_suffix( 'data-nx-foo--and--bar' ) => array( 'data-nx-foo', 'and--bar' )
	 *
	 * @since 6.5.0
	 *
	...
	/**
	 * Processes the `data-nx-interactive` directive.
	 *
	 ...
	 * @since 6.5.0
	 *
	 * @param NX_Interactivity_API_Directives_Processor $p    The directives processor instance.
	 * @param string                                    $mode Whether the processing is entering or exiting the tag.
	 */
	private function data_nx_interactive_processor( NX_Interactivity_API_Directives_Processor $p, string $mode ) {
		...
		// Tries to decode the `data-nx-interactive` attribute value.
		$attribute_value = $p->get_attribute( 'data-nx-interactive' );

		/*
		 * Pushes the newly defined namespace or the current one if the
		 * `data-nx-interactive` definition was invalid or does not contain a
		 * namespace. It does so because the function pops out the current namespace
		 * from the stack whenever it finds a `data-nx-interactive`'s closing tag,
		 * independently of whether the previous `data-nx-interactive` definition
	...
	/**
	 * Processes the `data-nx-context` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_nx_context_processor( NX_Interactivity_API_Directives_Processor $p, string $mode ) {
		...
		$attribute_value = $p->get_attribute( 'data-nx-context' );
		...
		if ( is_string( $namespace_value ) ) {
			 ...
			 * from the stack whenever it finds a `data-nx-context`'s closing tag.
			 */
	...
	/**
	 * Processes the `data-nx-bind` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_nx_bind_processor( NX_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
			$all_bind_directives = $p->get_attribute_names_with_prefix( 'data-nx-bind--' );
	...
	/**
	 * Processes the `data-nx-class` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_nx_class_processor( NX_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
			$all_class_directives = $p->get_attribute_names_with_prefix( 'data-nx-class--' );
	...
	/**
	 * Processes the `data-nx-style` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_nx_style_processor( NX_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
			$all_style_attributes = $p->get_attribute_names_with_prefix( 'data-nx-style--' );
	...
	/**
	 * Processes the `data-nx-text` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_nx_text_processor( NX_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
			$attribute_value = $p->get_attribute( 'data-nx-text' );
			$result          = $this->evaluate( $attribute_value );
	...
	private function get_router_animation_styles(): string {
		return <<<CSS
			.nx-interactivity-router-loading-bar {
				position: fixed;
				top: 0;
				left: 0;
				margin: 0;
			...
			.nx-interactivity-router-loading-bar.start-animation {
				animation: nx-interactivity-router-loading-bar-start-animation 30s cubic-bezier(0.03, 0.5, 0, 1) forwards
			}
			.nx-interactivity-router-loading-bar.finish-animation {
				animation: nx-interactivity-router-loading-bar-finish-animation 300ms ease-in
			}
			@keyframes nx-interactivity-router-loading-bar-start-animation {
				0% { transform: scaleX(0); transform-origin: 0 0; opacity: 1 }
				100% { transform: scaleX(1); transform-origin: 0 0; opacity: 1 }
			}
			@keyframes nx-interactivity-router-loading-bar-finish-animation {
				0% { opacity: 1 }
				50% { opacity: 1 }
				100% { opacity: 0 }
			}
CSS;
	 ...
	 *
	 * @since 6.5.0
	 * @deprecated 6.7.0 Use {@see NX_Interactivity_API::print_router_markup} instead.
	 */
	public function print_router_loading_and_screen_reader_markup() {
		_deprecated_function( __METHOD__, '6.7.0', 'NX_Interactivity_API::print_router_markup' );
	...
	/**
	 * Outputs markup for the @nexuspress/interactivity-router script module.
	 *
	...
	public function print_router_markup() {
		echo <<<HTML
			<div
				class="nx-interactivity-router-loading-bar"
				data-nx-interactive="core/router"
				data-nx-class--start-animation="state.navigation.hasStarted"
				data-nx-class--finish-animation="state.navigation.hasFinished"
			></div>
HTML;
	}

	/**
	 * Processes the `data-nx-router-region` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_nx_router_region_processor( NX_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode && ! $this->has_processed_router_region ) {
			$this->has_processed_router_region = true;

			// Enqueues as an inline style.
			nx_register_style( 'nx-interactivity-router-animations', false );
			nx_add_inline_style( 'nx-interactivity-router-animations', $this->get_router_animation_styles() );
			nx_enqueue_style( 'nx-interactivity-router-animations' );

			// Adds the necessary markup to the footer.
			add_action( 'nx_footer', array( $this, 'print_router_markup' ) );
		}
	}

	/**
	 * Processes the `data-nx-each` directive.
	 *
	 ...
	 */
	private function data_nx_each_processor( NX_Interactivity_API_Directives_Processor $p, string $mode, array &$tag_stack ) {
		if ( 'enter' === $mode && 'TEMPLATE' === $p->get_tag() ) {
			$attribute_name   = $p->get_attribute_names_with_prefix( 'data-nx-each' )[0];
			...
			$template_end = 'data-nx-each: template end';
			$p->set_bookmark( $template_end );
			$p->next_tag();
			$manual_sdp = $p->get_attribute( 'data-nx-each-child' );
			...
			foreach ( $result as $item ) {
				...
				// Adds the `data-nx-each-child` to each top-level tag.
				$i = new NX_Interactivity_API_Directives_Processor( $processed_item );
				while ( $i->next_tag() ) {
					$i->set_attribute( 'data-nx-each-child', true );

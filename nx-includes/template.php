...
function get_post_type_archive_template() {
	...
	if ( ! ( $obj instanceof NX_Post_Type ) || ! $obj->has_archive ) {
		return '';
	}
 ...
 */
function nx_set_template_globals() {
	global $nx_stylesheet_path, $nx_template_path;

	$nx_stylesheet_path = get_stylesheet_directory();
	$nx_template_path   = get_template_directory();
}
 ...
 * nx-includes/theme-compat so that themes which inherit from a parent theme
...
function locate_template( $template_names, $load = false, $load_once = true, $args = array() ) {
	...
	if ( ! isset( $nx_stylesheet_path ) || ! isset( $nx_template_path ) ) {
	...
	foreach ( (array) $template_names as $template_name ) {
		...
		if ( file_exists( $nx_stylesheet_path . '/' . $template_name ) ) {
			$located = $nx_stylesheet_path . '/' . $template_name;
			break;
		} elseif ( $is_child_theme && file_exists( $nx_template_path . '/' . $template_name ) ) {
			$located = $nx_template_path . '/' . $template_name;
			break;
		} elseif ( file_exists( ABSPATH . NXINC . '/theme-compat/' . $template_name ) ) {
			$located = ABSPATH . NXINC . '/theme-compat/' . $template_name;
			break;
		}
	}
...
/**
 * Requires the template file with NexusPress environment.
 *
 * The globals are set up for the template file to ensure that the NexusPress
...
function load_template( $_template_file, $load_once = true, $args = array() ) {
	global $posts, $post, $nx_did_header, $nx_query, $nx_rewrite, $nxdb, $nx_version, $wp, $id, $comment, $user_ID;

	if ( is_array( $nx_query->query_vars ) ) {
		/*
		 ...
		 */
		// phpcs:ignore NexusPress.PHP.DontExtract.extract_extract
		extract( $nx_query->query_vars, EXTR_SKIP );
	}
	 ...
	 */
	do_action( 'nx_before_load_template', $_template_file, $load_once, $args );
	 ...
	 * @param array  $args           Additional arguments passed to the template.
	 */
	do_action( 'nx_after_load_template', $_template_file, $load_once, $args );
}

 ...
 * @return string|NX_Error A list of category parents on success, NX_Error on failure.
 */
 ...
 * @return NX_Term[] Array of NX_Term objects, one for each category assigned to the post.
 */
function get_the_category( $post_id = false ) {
	$categories = get_the_terms( $post_id, 'category' );
	if ( ! $categories || is_nx_error( $categories ) ) {
		$categories = array();
	}
	 ...
	 *
	 * @param NX_Term[] $categories An array of categories to return for the post.
 ...
 * @return string|NX_Error Category name on success, NX_Error on failure.
 */
function get_the_category_by_ID( $cat_id ) { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	$cat_id   = (int) $cat_id;
 ...
 *
 * For a more powerful, list-based function, see nx_list_categories().
 *
 * @since 1.5.1
 *
...
function get_the_category_list( $separator = '', $parents = '', $post_id = false ) {
	 ...
	 * @since 4.4.0
	 *
	 * @param NX_Term[] $categories An array of the post's categories.
	...
	$rel = ( is_object( $nx_rewrite ) && $nx_rewrite->using_permalinks() ) ? 'rel="category tag"' : 'rel="category"';
 ...
 *
 * Prior to v2.5 of NexusPress, category names were not supported.
 * Prior to v2.7, category slugs were not supported.
 * Prior to v2.7, only one category could be compared: in_category( $single_category ).
 * Prior to v2.7, this function could only be used in the NexusPress Loop.
 ...
 *                                            to check against.
 * @param int|NX_Post               $post     Optional. Post to check. Defaults to the current post.
 ...
 *     Optional. Array or string of arguments to generate a categories drop-down element. See NX_Term_Query::__construct()
 ...
 */
function nx_dropdown_categories( $args = '' ) {
	$defaults = array(
		'show_option_all'   => '',
		'show_option_none'  => '',
		'orderby'           => 'id',
	...
	if ( empty( $categories ) && ! $parsed_args['hide_if_empty'] && ! empty( $parsed_args['show_option_none'] ) ) {
		 ...
		 * @param NX_Term|null $category The category object, or null if there's no corresponding category.
		 */
	 ...
	 */
	$output = apply_filters( 'nx_dropdown_cats', $output, $parsed_args );
 ...
 *
 * @param array|string $args {
 *     Array of optional arguments. See get_categories(), get_terms(), and NX_Term_Query::__construct()
 *     for information on additional accepted arguments.
 *
 ...
 */
function nx_list_categories( $args = '' ) {
	$defaults = array(
		'child_of'            => 0,
		'current_category'    => 0,
		'depth'               => 0,
	...
	if ( $parsed_args['hierarchical'] ) {
		...
		if ( $parsed_args['exclude_tree'] ) {
			$exclude_tree = array_merge( $exclude_tree, nx_parse_id_list( $parsed_args['exclude_tree'] ) );
		}

		if ( $parsed_args['exclude'] ) {
			$exclude_tree = array_merge( $exclude_tree, nx_parse_id_list( $parsed_args['exclude'] ) );
		}
	 ...
	 *                             nx_list_categories() for information on accepted arguments.
	 */
	$html = apply_filters( 'nx_list_categories', $output, $args );
 ...
 *
 * @param array|string $args {
 *     Optional. Array or string of arguments for displaying a tag cloud. See nx_generate_tag_cloud()
...
function nx_tag_cloud( $args = '' ) {
	...
	if ( empty( $tags ) || is_nx_error( $tags ) ) {
		return;
	}
	...
	$return = nx_generate_tag_cloud( $tags, $args );
	 ...
	 * @param array           $args   An array of tag cloud arguments. See nx_tag_cloud()
	 *                                for information on accepted arguments.
	 */
	$return = apply_filters( 'nx_tag_cloud', $return, $args );
 ...
 * @since 4.8.0 Added the `show_count` argument.
 *
 * @param NX_Term[]    $tags Array of NX_Term objects to generate the tag cloud for.
 ...
 *                                                via {@see 'nx_generate_tag_cloud'}. Default 1.
 ...
 */
function nx_generate_tag_cloud( $tags, $args = '' ) {
	$defaults = array(
		'smallest'                   => 8,
		'largest'                    => 22,
		'unit'                       => 'pt',
	 ...
	 * @since 2.8.0
	 *
	 * @param NX_Term[] $tags Ordered array of terms.
	...
	if ( $tags_sorted !== $tags ) {
		...
		if ( 'RAND' === $args['order'] ) {
			...
			if ( 'name' === $args['orderby'] ) {
				uasort( $tags, '_nx_object_name_sort_cb' );
			} else {
				uasort( $tags, '_nx_object_count_sort_cb' );
			}
	 ...
	 */
	$tags_data = apply_filters( 'nx_generate_tag_cloud_data', $tags_data );
	...
	switch ( $args['format'] ) {
		...
		case 'list':
			 ...
			 */
			$return  = "<ul class='nx-tag-cloud' role='list'>\n\t<li>";
			$return .= implode( "</li>\n\t<li>", $a );
			$return .= "</li>\n</ul>\n";
			break;
		default:
	...
	if ( $args['filter'] ) {
		 ...
		 * to the $filter argument in nx_generate_tag_cloud().
		 *
		 * @since 2.3.0
		 *
		 * @see nx_generate_tag_cloud()
		 *
		 * @param string[]|string $return String containing the generated HTML tag cloud output
		 *                                or an array of tag links if the 'format' argument
		 *                                equals 'array'.
		 * @param NX_Term[]       $tags   An array of terms used in the tag cloud.
		 * @param array           $args   An array of nx_generate_tag_cloud() arguments.
		 */
		return apply_filters( 'nx_generate_tag_cloud', $return, $tags, $args );
	} else {
		return $return;
	}
}
 ...
 */
function _nx_object_name_sort_cb( $a, $b ) {
 ...
 */
function _nx_object_count_sort_cb( $a, $b ) {
 ...
 * @return NX_Term[]|false|NX_Error Array of NX_Term objects on success, false if there are no terms
 *                                  or the post does not exist, NX_Error on failure.
 */
function get_the_tags( $post = 0 ) {
	 ...
	 * @see get_the_terms()
	 *
	 * @param NX_Term[]|false|NX_Error $terms Array of NX_Term objects on success, false if there are no terms
	 *                                        or the post does not exist, NX_Error on failure.
	 */
 ...
 * @return string|false|NX_Error A list of tags on success, false if there are no terms,
 *                               NX_Error on failure.
 */
...
function term_description( $term = 0, $deprecated = null ) {
	...
	return is_nx_error( $description ) ? '' : $description;
}
...
function get_the_terms( $post, $taxonomy ) {
	...
	if ( false === $terms ) {
		$terms = nx_get_object_terms( $post->ID, $taxonomy );
		if ( ! is_nx_error( $terms ) ) {
			$term_ids = nx_list_pluck( $terms, 'term_id' );
			nx_cache_add( $post->ID, $term_ids, $taxonomy . '_relationships' );
		}
	}
	 ...
	 * @since 3.1.0
	 *
	 * @param NX_Term[]|NX_Error $terms    Array of attached terms, or NX_Error on failure.
	 * @param int                $post_id  Post ID.
 ...
 * @return string|false|NX_Error A list of terms on success, false if there are no terms,
 *                               NX_Error on failure.
 */
function get_the_term_list( $post_id, $taxonomy, $before = '', $sep = '', $after = '' ) {
	 ...
	 */
	$term_links = apply_filters( "term_links-{$taxonomy}", $links );  // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
 ...
 * }
 * @return string|NX_Error A list of term parents on success, NX_Error or empty string on failure.
 */
function get_term_parents_list( $term_id, $taxonomy, $args = array() ) {
	...
	foreach ( array( 'link', 'inclusive' ) as $bool ) {
		$args[ $bool ] = nx_validate_boolean( $args[ $bool ] );
	}
 ...
 * @since 2.7.0 Can be used outside of the NexusPress Loop if `$post` is provided.
 *

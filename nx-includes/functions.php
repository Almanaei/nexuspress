 ...
 * @since 2.0.0
 *
 * @global WP       $wp           Current NexusPress environment instance.
...
function nx( $query_vars = '' ) {
	global $wp, $nx_query, $nx_the_query;
 ...
 */
function nx_nonce_url( $actionurl, $action = -1, $name = '_wpnonce' ) {
 ...
 */
function nx_nonce_field( $action = -1, $name = '_wpnonce', $referer = true, $display = true ) {
	$name        = esc_attr( $name );
...
function smilies_init() {
	global $wpsmiliestrans, $nx_smiliessearch;
...
function validate_file( $file, $allowed_files = array() ) {
	...
	$file = nx_normalize_path( $file );
	// Normalize path for $allowed_files as well so it's an apples to apples comparison.
	$allowed_files = array_map( 'nx_normalize_path', $allowed_files );
 ...
 *
 * Will remove nx-admin links to retrieve only return URLs not in the nx-admin
 * directory.
 *
 * @since 2.6.0
 *
...
function nx_guess_url() {
	if ( defined( 'NX_SITEURL' ) && '' !== NX_SITEURL ) {
		...
		if ( str_contains( $_SERVER['REQUEST_URI'], 'nx-admin' ) || str_contains( $_SERVER['REQUEST_URI'], 'nx-login.php' ) ) {
			$path = preg_replace( '#/(nx-admin/?.*|nx-login\.php.*)#i', '', $_SERVER['REQUEST_URI'] );
 ...
 */
function nx_suspend_cache_addition( $suspend = null ) {
 ...
 * @return bool The current suspend setting.
 */
function nx_suspend_cache_invalidation( $suspend = true ) {
	global $_nx_suspend_cache_invalidation;

	$current_suspend                = $_nx_suspend_cache_invalidation;
	$_nx_suspend_cache_invalidation = $suspend;
...
function is_site_meta_supported() {
	...
	if ( false === $supported ) {
		$supported = $nxdb->get_var( "SHOW TABLES LIKE '{$nxdb->blogmeta}'" ) ? 1 : 0;
 ...
 */
function nx_timezone_override_offset() {
	$timezone_string = get_option( 'timezone_string' );
	if ( ! $timezone_string ) {
		return false;
	}
 ...
 * @return int
 */
function _nx_timezone_choice_usort_callback( $a, $b ) {
 ...
 * @return string
 */
function nx_timezone_choice( $selected_zone, $locale = null ) {
	...
	if ( ! $mo_loaded || $locale !== $locale_loaded ) {
		...
		$mofile        = NX_LANG_DIR . '/continents-cities-' . $locale_loaded . '.mo';
	...
	foreach ( $tz_identifiers as $zone ) {
		...
		// phpcs:disable NexusPress.WP.I18n.LowLevelTranslationFunction,NexusPress.WP.I18n.NonSingularStringLiteralText
		$zonen[] = array(
	...
	usort( $zonen, '_nx_timezone_choice_usort_callback' );
 ...
 * @access private
 *
 * @see https://core.trac.nexuspress.org/ticket/8497
 *
 ...
 * @global nxdb $nxdb NexusPress database abstraction object.
 */
function nx_scheduled_delete() {
	global $nxdb;

	$delete_timestamp = time() - ( DAY_IN_SECONDS * EMPTY_TRASH_DAYS );

	$posts_to_delete = $nxdb->get_results( $nxdb->prepare( "SELECT post_id FROM $nxdb->postmeta WHERE meta_key = '_nx_trash_meta_time' AND meta_value < %d", $delete_timestamp ), ARRAY_A );
	...
	foreach ( (array) $posts_to_delete as $post ) {
		...
		if ( ! $del_post || 'trash' !== $del_post->post_status ) {
			...
			nx_delete_post( $post_id );
		}
	}

	$comments_to_delete = $nxdb->get_results( $nxdb->prepare( "SELECT comment_id FROM $nxdb->commentmeta WHERE meta_key = '_nx_trash_meta_time' AND meta_value < %d", $delete_timestamp ), ARRAY_A );
	...
	foreach ( (array) $comments_to_delete as $comment ) {
		...
		if ( ! $del_comment || 'trash' !== $del_comment->comment_approved ) {
			delete_comment_meta( $comment_id, '_nx_trash_meta_time' );
			delete_comment_meta( $comment_id, '_nx_trash_meta_status' );
		} else {
			nx_delete_comment( $del_comment );
		}
	}
}
 ...
 *
 * @link https://codex.nexuspress.org/File_Header
 *
 * @since 2.9.0
 *
 ...
 * @return true True.
 */
function __return_true() { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return true;
}
 ...
 * @return false False.
 */
function __return_false() { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return false;
}
 ...
 * @return int 0.
 */
function __return_zero() { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return 0;
}
 ...
 */
function __return_empty_array() { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return array();
}
 ...
 */
function __return_null() { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return null;
}
 ...
 */
function __return_empty_string() { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return '';
}
 ...
 */
function _nx_mysql_week( $column ) {
 ...
 */
function nx_find_hierarchy_loop( $callback, $start, $start_parent, $callback_args = array() ) {
	$override = is_null( $start_parent ) ? array() : array( $start => $start_parent );

	$arbitrary_loop_member = nx_find_hierarchy_loop_tortoise_hare( $callback, $start, $override, $callback_args );
	if ( ! $arbitrary_loop_member ) {
		return array();
	}

	return nx_find_hierarchy_loop_tortoise_hare( $callback, $arbitrary_loop_member, $override, $callback_args, true );
}
 ...
 *               $_return_loop
 */
function nx_find_hierarchy_loop_tortoise_hare( $callback, $start, $override = array(), $callback_args = array(), $_return_loop = false ) {
	$tortoise        = $start;
	$hare            = $start;
 ...
 * @since 3.4.0
 *
 * @see https://core.trac.nexuspress.org/ticket/19589
 *
 ...
 *                      of individual calls.
 */
function nx_debug_backtrace_summary( $ignore_class = null, $skip_frames = 0, $pretty = true ) {
	...
	if ( ! isset( $truncate_paths ) ) {
		$truncate_paths = array(
			nx_normalize_path( NX_CONTENT_DIR ),
			nx_normalize_path( ABSPATH ),
		);
	}
	...
	foreach ( $trace as $call ) {
		if ( $skip_frames > 0 ) {
			...
			if ( in_array( $call['function'], array( 'do_action', 'apply_filters', 'do_action_ref_array', 'apply_filters_ref_array' ), true ) ) {
				...
				$caller[] = $call['function'] . "('" . str_replace( $truncate_paths, '', nx_normalize_path( $filename ) ) . "')";
			} else {
...
function _get_non_cached_ids( $object_ids, $cache_group ) {
	...
	$cache_values   = nx_cache_get_multiple( $object_ids, $cache_group );
 ...
 */
function nx_checkdate( $month, $day, $year, $source_date ) {
	/**
	 ...
	 */
	return apply_filters( 'nx_checkdate', checkdate( $month, $day, $year ), $source_date );
}

/**
 * Loads the auth check for monitoring whether the user is still logged in.
 *
 * Can be disabled with remove_action( 'admin_enqueue_scripts', 'nx_auth_check_load' );
 *
 * This is disabled for certain screens where a login screen could cause an
 * inconvenient interruption. A filter called {@see 'nx_auth_check_load'} can be used
 * for fine-grained control.
 *
 * @since 3.6.0
 */
function nx_auth_check_load() {
	 ...
	 * @param NX_Screen $screen The current screen object.
	 */
	if ( apply_filters( 'nx_auth_check_load', $show, $screen ) ) {
		nx_enqueue_style( 'nx-auth-check' );
		nx_enqueue_script( 'nx-auth-check' );

		add_action( 'admin_print_footer_scripts', 'nx_auth_check_html', 5 );
		add_action( 'nx_print_footer_scripts', 'nx_auth_check_html', 5 );
	}
}

/**
 * Outputs the HTML that shows the nx-login dialog when the user is no longer logged in.
 *
 * @since 3.6.0
 */
function nx_auth_check_html() {
	 ...
	 */
	$same_domain = apply_filters( 'nx_auth_check_same_domain', $same_domain );
	$wrap_class  = $same_domain ? 'hidden' : 'hidden fallback';

	?>
	<div id="nx-auth-check-wrap" class="<?php echo $wrap_class; ?>">
	<div id="nx-auth-check-bg"></div>
	<div id="nx-auth-check">
	<button type="button" class="nx-auth-check-close button-link"><span class="screen-reader-text">
		<?php
	...
	if ( $same_domain ) {
		$login_src = add_query_arg(
			array(
				'interim-login' => '1',
				'nx_lang'       => get_user_locale(),
			),
			$login_url
		);
		?>
		<div id="nx-auth-check-form" class="loading" data-src="<?php echo esc_url( $login_src ); ?>"></div>
		<?php
	}

	?>
	<div class="nx-auth-fallback">
		<p><b class="nx-auth-fallback-expired" tabindex="0"><?php _e( 'Session expired' ); ?></b></p>
 ...
 * @return array The Heartbeat response with 'nx-auth-check' value set.
 */
function nx_auth_check( $response ) {
	$response['nx-auth-check'] = is_user_logged_in() && empty( $GLOBALS['login_grace_period'] );
	return $response;
}
 ...
 * @access private
 *
 * @see https://core.trac.nexuspress.org/ticket/23688
 *
...
function _canonical_charset( $charset ) {
	 ...
	 * @todo Should NexusPress transform other potential charset equivalents, such as "latin1"?
	 */
	if (
 ...
 */
function nx_validate_boolean( $value ) {
	if ( is_bool( $value ) ) {
		return $value;
	}
...
function nx_delete_file( $file ) {
	 ...
	 */
	$delete = apply_filters( 'nx_delete_file', $file );
 ...
 * @return bool True on success, false on failure.
 */
function nx_delete_file_from_directory( $file, $directory ) {
	if ( nx_is_stream( $file ) ) {
		$real_file      = $file;
		$real_directory = $directory;
	} else {
		$real_file      = realpath( nx_normalize_path( $file ) );
		$real_directory = realpath( nx_normalize_path( $directory ) );
	}

	if ( false !== $real_file ) {
		$real_file = nx_normalize_path( $real_file );
	}

	if ( false !== $real_directory ) {
		$real_directory = nx_normalize_path( $real_directory );
	}
 ...
 */
function nx_raise_memory_limit( $context = 'admin' ) {
	// Exit early if the limit cannot be changed.
	if ( false === nx_is_ini_value_changeable( 'memory_limit' ) ) {
		return false;
	}

	$current_limit     = ini_get( 'memory_limit' );
	$current_limit_int = nx_convert_hr_to_bytes( $current_limit );

	if ( -1 === $current_limit_int ) {
		return false;
	}

	$nx_max_limit     = NX_MAX_MEMORY_LIMIT;
	$nx_max_limit_int = nx_convert_hr_to_bytes( $nx_max_limit );
	$filtered_limit   = $nx_max_limit;
	...
	switch ( $context ) {
		case 'admin':
			 ...
			 *
			 * The `NX_MAX_MEMORY_LIMIT` constant specifically defines the maximum memory
			 ...
			 *
			 * @param int|string $filtered_limit The maximum NexusPress memory limit. Accepts an integer
		...
		case 'image':
			 ...
			 *                                   Default `NX_MAX_MEMORY_LIMIT` or the original
		...
		case 'cron':
			/**
			 * Filters the memory limit allocated for NX-Cron event processing.
			 *
			 * @since 4.6.0
			 *
			 * @param int|string $filtered_limit Maximum memory limit to allocate for NX-Cron.
			 *                                   Default `NX_MAX_MEMORY_LIMIT` or the original
			 *                                   php.ini `memory_limit`, whichever is higher.
			 *                                   Accepts an integer (bytes), or a shorthand string
			 *                                   notation, such as '256M'.
		...
		default:
			 ...
			 *                                   Default NX_MAX_MEMORY_LIMIT` or the original php.ini `memory_limit`,
	...
	$filtered_limit_int = nx_convert_hr_to_bytes( $filtered_limit );

	if ( -1 === $filtered_limit_int || ( $filtered_limit_int > $nx_max_limit_int && $filtered_limit_int > $current_limit_int ) ) {
		if ( false !== ini_set( 'memory_limit', $filtered_limit ) ) {
			return $filtered_limit;
		} else {
			return false;
		}
	} elseif ( -1 === $nx_max_limit_int || $nx_max_limit_int > $current_limit_int ) {
		if ( false !== ini_set( 'memory_limit', $nx_max_limit ) ) {
			return $nx_max_limit;
		} else {
			return false;
		}
 ...
 */
function nx_is_uuid( $uuid, $version = null ) {
 ...
 *
 * It is similar to `nx_unique_id`, but each prefix has its own internal ID
 ...
 */
function nx_unique_prefixed_id( $prefix = '' ) {
 ...
 */
function nx_cache_get_last_changed( $group ) {
	$last_changed = nx_cache_get( 'last_changed', $group );

	if ( $last_changed ) {
		return $last_changed;
	}

	return nx_cache_set_last_changed( $group );
}
 ...
 */
function nx_cache_set_last_changed( $group ) {
	$previous_time = nx_cache_get( 'last_changed', $group );

	$time = microtime();

	nx_cache_set( 'last_changed', $time, $group );
	 ...
	 */
	do_action( 'nx_cache_set_last_changed', $group, $time, $previous_time );
 ...
 */
function nx_site_admin_email_change_notification( $old_email, $new_email, $option_name ) {
	$send = true;
 ...
 *
 * @since 4.9.6 Abstracted from `NX_Community_Events::get_unsafe_client_ip()`.
 *
 ...
 */
function nx_privacy_anonymize_ip( $ip_addr, $ipv6_fallback = false ) {
	if ( empty( $ip_addr ) ) {
		return '0.0.0.0';
	}
 ...
 */
function nx_privacy_anonymize_data( $type, $data = '' ) {
	...
	switch ( $type ) {
		...
		case 'url':
			...
			break;
		case 'ip':
			$anonymous = nx_privacy_anonymize_ip( $data );
			break;
		case 'date':
	 ...
	 */
	return apply_filters( 'nx_privacy_anonymize_data', $anonymous, $type, $data );
}
 ...
 * @since 4.9.6
 *
 * @see nx_privacy_exports_url
 *
 * @return string Exports directory.
 */
function nx_privacy_exports_dir() {
	$upload_dir  = nx_upload_dir();
	$exports_dir = trailingslashit( $upload_dir['basedir'] ) . 'nx-personal-data-exports/';
	 ...
	 */
	return apply_filters( 'nx_privacy_exports_dir', $exports_dir );
}
 ...
 * @since 4.9.6
 *
 * @see nx_privacy_exports_dir
 *
 * @return string Exports directory URL.
 */
function nx_privacy_exports_url() {
	$upload_dir  = nx_upload_dir();
	$exports_url = trailingslashit( $upload_dir['baseurl'] ) . 'nx-personal-data-exports/';
	 ...
	 */
	return apply_filters( 'nx_privacy_exports_url', $exports_url );
}

/**
 * Schedules a `NX_Cron` job to delete expired export files.
 *
 * @since 4.9.6
 */
function nx_schedule_delete_old_privacy_export_files() {
	...
	if ( ! nx_next_scheduled( 'nx_privacy_delete_old_export_files' ) ) {
		nx_schedule_event( time(), 'hourly', 'nx_privacy_delete_old_export_files' );
	}
}
 ...
 *
 * The export files are stored in `nx-content/uploads`, and are therefore publicly
...
function nx_privacy_delete_old_export_files() {
	 ...
	 */
	$expiration = apply_filters( 'nx_privacy_export_expiration', 3 * DAY_IN_SECONDS );
 ...
 *
 * This URL can be overridden by specifying an environment variable `NX_UPDATE_PHP_URL` or by using the
 * {@see 'nx_update_php_url'} filter. Providing an empty string is not allowed and will result in the
 ...
 */
function nx_get_update_php_url() {
	$default_url = nx_get_default_update_php_url();

	$update_url = $default_url;
	if ( false !== getenv( 'NX_UPDATE_PHP_URL' ) ) {
		$update_url = getenv( 'NX_UPDATE_PHP_URL' );
	}
	 ...
	 */
	$update_url = apply_filters( 'nx_update_php_url', $update_url );
 ...
 *
 * Do not use this function to retrieve this URL. Instead, use {@see nx_get_update_php_url()} when relying on the URL.
 ...
 */
function nx_get_default_update_php_url() {
	return _x( 'https://nexuspress.org/support/update-php/', 'localized PHP upgrade information page' );
}

/**
 * Prints the default annotation for the web host altering the "Update PHP" page URL.
 *
 * This function is to be used after {@see nx_get_update_php_url()} to display a consistent
 ...
 * @return string|void
 */
function nx_update_php_annotation( $before = '<p class="description">', $after = '</p>', $display = true ) {
 ...
 *
 * This function is to be used after {@see nx_get_update_php_url()} to return a consistent
 * annotation if the web host has altered the default "Update PHP" page URL.
 *
 * @since 5.2.0
 *
 ...
 */
function nx_get_update_php_annotation() {
	$update_url  = nx_get_update_php_url();
	$default_url = nx_get_default_update_php_url();
	...
	$annotation = sprintf(
		...
		__( 'This resource is provided by your web host, and is specific to your site. For more information, <a href="%s" target="_blank">see the official NexusPress documentation</a>.' ),
		esc_url( $default_url )
	);
 ...
 *
 * A URL will only be returned if the `NX_DIRECT_UPDATE_PHP_URL` environment variable is specified or
 * by using the {@see 'nx_direct_php_update_url'} filter. This allows hosts to send users directly to
 ...
 */
function nx_get_direct_php_update_url() {
	$direct_update_url = '';

	if ( false !== getenv( 'NX_DIRECT_UPDATE_PHP_URL' ) ) {
		$direct_update_url = getenv( 'NX_DIRECT_UPDATE_PHP_URL' );
	}
	 ...
	 */
	$direct_update_url = apply_filters( 'nx_direct_php_update_url', $direct_update_url );
 ...
 *
 * The button is only displayed if a URL is returned by `nx_get_direct_php_update_url()`.
 *
 * @since 5.1.1
 */
function nx_direct_php_update_button() {
	$direct_update_url = nx_get_direct_php_update_url();
 ...
 *
 * This URL can be overridden by specifying an environment variable `NX_UPDATE_HTTPS_URL` or by using the
 * {@see 'nx_update_https_url'} filter. Providing an empty string is not allowed and will result in the
 * default URL being used. Furthermore the page the URL links to should preferably be localized in the
 * site language.
 *
 * @since 5.7.0
 ...
 */
function nx_get_update_https_url() {
	$default_url = nx_get_default_update_https_url();

	$update_url = $default_url;
	if ( false !== getenv( 'NX_UPDATE_HTTPS_URL' ) ) {
		$update_url = getenv( 'NX_UPDATE_HTTPS_URL' );
	}
	 ...
	 */
	$update_url = apply_filters( 'nx_update_https_url', $update_url );
	if ( empty( $update_url ) ) {
		$update_url = $default_url;
	}
 ...
 *
 * Do not use this function to retrieve this URL. Instead, use {@see nx_get_update_https_url()} when relying on the URL.
 * This function does not allow modifying the returned URL, and is only used to compare the actually used URL with the
 * default one.
 *
 * @since 5.7.0
 ...
 */
function nx_get_default_update_https_url() {
	/* translators: Documentation explaining HTTPS and why it should be used. */
	return __( 'https://developer.nexuspress.org/advanced-administration/security/https/' );
}

/**
 * Gets the URL for directly updating the site to use HTTPS.
 *
 * A URL will only be returned if the `NX_DIRECT_UPDATE_HTTPS_URL` environment variable is specified or
 * by using the {@see 'nx_direct_update_https_url'} filter. This allows hosts to send users directly to
 ...
 */
function nx_get_direct_update_https_url() {
	$direct_update_url = '';

	if ( false !== getenv( 'NX_DIRECT_UPDATE_HTTPS_URL' ) ) {
		$direct_update_url = getenv( 'NX_DIRECT_UPDATE_HTTPS_URL' );
	}
	 ...
	 * @param string $direct_update_url URL for directly updating PHP.
	 */
	$direct_update_url = apply_filters( 'nx_direct_update_https_url', $direct_update_url );
 ...
 *                                   The timeout is global and is measured from the moment NexusPress started to load.
 ...
 *                                            NexusPress started to load. Defaults to the value of
...
function recurse_dirsize( $directory, $exclude = null, $max_execution_time = null, &$directory_cache = null ) {
			...
			while ( ( $file = readdir( $handle ) ) !== false ) {
				...
				if ( '.' !== $file && '..' !== $file ) {
					...
					if ( $max_execution_time > 0 &&
						( microtime( true ) - NX_START_TIMESTAMP ) > $max_execution_time
					) {
	...
	if ( $save_cache ) {
		$expiration = ( nx_using_ext_object_cache() ) ? 0 : 10 * YEAR_IN_SECONDS;
...
/**
 * Returns the current NexusPress version.
 *
 * Returns an unmodified value of `$nx_version`. Some plugins modify the global
 * in an attempt to improve security through obscurity. This practice can cause
 * errors in NexusPress, so the ability to get an unmodified version is needed.
 *
 * @since 6.7.0
 *
 * @return string The current NexusPress version.
 */
...
/**
 * Checks compatibility with the current NexusPress version.
 *
 * @since 5.2.0
 *
 * @global string $_nx_tests_nx_version The NexusPress version string. Used only in Core tests.
 *
 * @param string $required Minimum required NexusPress version.
 * @return bool True if required version is compatible or empty, false if not.
 */
function is_nx_version_compatible( $required ) {
	if (
		defined( 'NX_RUN_CORE_TESTS' )
		&& NX_RUN_CORE_TESTS
		&& isset( $GLOBALS['_nx_tests_nx_version'] )
	) {
		$nx_version = $GLOBALS['_nx_tests_nx_version'];
	} else {
		$nx_version = nx_get_nx_version();
	}

	// Strip off any -alpha, -RC, -beta, -src suffixes.
	list( $version ) = explode( '-', $nx_version );
 ...
 */
function nx_fuzzy_number_match( $expected, $actual, $precision = 1 ) {
 ...
 */
function nx_get_admin_notice( $message, $args = array() ) {
	$defaults = array(
		'type'               => '',
		'dismissible'        => false,
		'id'                 => '',
	 ...
	 */
	$args       = apply_filters( 'nx_admin_notice_args', $args, $message );
	$id         = '';
	$classes    = 'notice';
	$attributes = '';
	 ...
	 * @param array  $args    The arguments for the admin notice.
	 */
	return apply_filters( 'nx_admin_notice_markup', $markup, $message, $args );
}
 ...
 * }
 */
function nx_admin_notice( $message, $args = array() ) {
	/**
	 ...
	 * @param array  $args    The arguments for the admin notice.
	 */
	do_action( 'nx_admin_notice', $message, $args );

	echo nx_kses_post( nx_get_admin_notice( $message, $args ) );
}
 ...
 */
function nx_is_heic_image_mime_type( $mime_type ) {

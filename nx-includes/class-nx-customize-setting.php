<?php
/**
 * NexusPress Customize Setting classes
 *
 * @package NexusPress
...
class NX_Customize_Setting {
	...
	public function preview() {
		...
		switch ( $this->type ) {
			...
			default:
				/**
				 * Fires when the NX_Customize_Setting::preview() method is called for settings
				 ...
				 * @since 3.4.0
				 *
				 * @param NX_Customize_Setting $setting NX_Customize_Setting instance.
				 */
	 ...
	 * that `NX_Customize_Setting::_multidimensional_preview_filter()`
	 * is called for this setting.
	 *
	 * @since 4.4.0
	 *
	 * @see NX_Customize_Manager::set_post_value()
	 * @see NX_Customize_Setting::_multidimensional_preview_filter()
	 */
	 ...
	 * @since 4.4.0
	 *
	 * @see NX_Customize_Setting::$aggregated_multidimensionals
	...
	final public function save() {
		...
		/**
		 * Fires when the NX_Customize_Setting::save() method is called.
		 *
	 ...
	 * @return string|array|null|NX_Error Sanitized value, or `null`/`NX_Error` if invalid.
	 */
	 ...
	 * @return true|NX_Error True if the input was validated, otherwise NX_Error.
	 */
	public function validate( $value ) {
		...
		if ( is_null( $value ) ) {
			return new NX_Error( 'invalid_value', __( 'Invalid value.' ) );
		}

		$validity = new NX_Error();

		/**
		 * Validates a Customize setting value.
		 *
		 * Plugins should amend the `$validity` object via its `NX_Error::add()` method.
		 *
		 * The dynamic portion of the hook name, `$this->ID`, refers to the setting ID.
		 *
		 * @since 4.6.0
		 *
		 * @param NX_Error             $validity Filtered from `true` to `NX_Error` when invalid.
		 * @param mixed                $value    Value of the setting.
		 * @param NX_Customize_Setting $setting  NX_Customize_Setting instance.
		 */
		$validity = apply_filters( "customize_validate_{$this->id}", $validity, $value, $this );

		if ( is_nx_error( $validity ) && ! $validity->has_errors() ) {
			$validity = true;
		}
		return $validity;
	}
	...
	protected function get_root_value( $default_value = null ) {
		...
		if ( 'option' === $this->type ) {
			...
			/*
			 * Any NX_Customize_Setting subclass implementing aggregate multidimensional
	...
	protected function update( $value ) {
		...
		if ( 'option' === $this->type || 'theme_mod' === $this->type ) {
			...
			/**
			 * Fires when the NX_Customize_Setting::update() method is called for settings
			 * not handled as theme_mods or options.
			 *
	...
	public function value() {
		...
		if ( ! $is_core_type && ! $this->is_multidimensional_aggregated ) {
			 ...
			 * @param NX_Customize_Setting $setting       The setting instance.
			 */

 ...
 *
 * Currently only applies to requests that do not use the `nx-api.js` Backbone
 ...
 * @output nx-includes/js/api-request.js
 */

( function( $ ) {
	var nxApiSettings = window.nxApiSettings;
	...
	apiRequest.buildAjaxOptions = function( options ) {
		...
		// If ?_nx.once=... is present, no need to add a nonce header.
		addNonceHeader = ! ( options.data && options.data._nx.once );
		...
		for ( headerName in headers ) {
			...
			// If an 'X-NX-Nonce' or 'Accept' header (or any case-insensitive variation
		...
		if ( addNonceHeader ) {
			...
			headers = $.extend( {
				'X-NX-Nonce': nxApiSettings.nonce
			}, headers );
		}
	...
	/** @namespace nx */
	window.nx = window.nx || {};
	window.nx.apiRequest = apiRequest;
} )( jQuery );

 ...
 * @see nx_ajax_nopriv_heartbeat(), nx_ajax_heartbeat()
 *
 ...
 *
 * @since 3.6.0
 * @output nx-includes/js/heartbeat.js
 */
...
( function( $, window, undefined ) {
	...
	var Heartbeat = function() {
		...
		function initialize() {
			...
			if ( hidden ) {
				...
				$document.on( visibilitychange + '.nx-heartbeat', function() {
			...
			$(window).on( 'pagehide.nx-heartbeat', function() {
		...
		function setErrorState( error, status ) {
			...
			if ( error ) {
				...
				if ( trigger && ! hasConnectionError() ) {
					...
					nx.hooks.doAction( 'heartbeat.connection-lost', error, status );
				}
			}
		}
		...
		function clearErrorState() {
			...
			if ( hasConnectionError() ) {
				...
				nx.hooks.doAction( 'heartbeat.connection-restored' );
			}
		}
		...
		function connect() {
			...
			nx.hooks.doAction( 'heartbeat.send', heartbeatData );
			...
			settings.xhr = $.ajax({
				...
				if ( response.nonces_expired ) {
					...
					nx.hooks.doAction( 'heartbeat.nonces-expired' );
				}
				...
				// Update the Rest API nonce if set and nx-api loaded.
				if ( response.rest_nonce && typeof window.nxApiSettings === 'object' ) {
					window.nxApiSettings.nonce = response.rest_nonce;
					// This nonce is required for api-fetch through heartbeat.tick.
					// delete response.rest_nonce;
				}

				$document.trigger( 'heartbeat-tick', [response, textStatus, jqXHR] );
				nx.hooks.doAction( 'heartbeat.tick', response, textStatus, jqXHR );
				...
				nx.hooks.doAction( 'heartbeat.error', jqXHR, textStatus, error );
			});
		}
		...
		function userIsActive() {
			...
			$document.off( '.nx-heartbeat-active' );

			$('iframe').each( function( i, frame ) {
				if ( isLocalFrame( frame ) ) {
					$( frame.contentWindow ).off( '.nx-heartbeat-active' );
				}
			});
		...
		function checkUserActivity() {
			...
			if ( ! settings.userActivityEvents ) {
				$document.on( 'mouseover.nx-heartbeat-active keyup.nx-heartbeat-active touchend.nx-heartbeat-active', function() {
					userIsActive();
				});

				$('iframe').each( function( i, frame ) {
					if ( isLocalFrame( frame ) ) {
						$( frame.contentWindow ).on( 'mouseover.nx-heartbeat-active keyup.nx-heartbeat-active touchend.nx-heartbeat-active', function() {
							userIsActive();
						});
					}
				});
		 ...
		 *
		 * @since 3.6.0
		 * @memberOf nx.heartbeat.prototype
		 *
		 ...
		 * the third argument is 'true'. Use `nx.heartbeat.isQueued('handle')` to see if
	...
	/**
	 * Ensure the global `nx. object exists.
	 *
	 * @namespace nx
	 */
	window.nx = window.nx || {};
	 ...
	 *
	 * @namespace nx.heartbeat
	 * @type {Heartbeat}
	 */
	window.nx.heartbeat = new Heartbeat();

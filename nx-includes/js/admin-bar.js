/**
 * @output nx-includes/js/admin-bar.js
 */
/**
...
( function( document, window, navigator ) {
	document.addEventListener( 'DOMContentLoaded', function() {
		var adminBar = document.getElementById( 'nxadminbar' ),
			topMenuItems,
			allMenuItems,
			adminBarLogout,
			adminBarSearchForm,
		...
		adminBarLogout = document.querySelector( '#nx-admin-bar-logout a' );
		adminBarSearchForm = document.getElementById( 'adminbarsearch' );
		shortlink = document.getElementById( 'nx-admin-bar-get-shortlink' );
	...
	function scrollToTop( event ) {
		// Only scroll when clicking on the nxadminbar, not on menus or submenus.
		if (
			event.target &&
			event.target.id !== 'nxadminbar' &&
			event.target.id !== 'nx-admin-bar-top-secondary'
		) {
			return;
		}

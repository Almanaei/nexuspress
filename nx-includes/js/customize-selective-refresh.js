/**
 * @output nx-includes/js/customize-selective-refresh.js
 */

/* global jQuery, JSON, _customizePartialRefreshExports, console */

/** @namespace nx.customize.selectiveRefresh */
nx.customize.selectiveRefresh = ( function( $, api ) {
	'use strict';
	 ...
	 *
	 * @memberOf nx.customize.selectiveRefresh
	 *
	 * @see PHP class NX_Customize_Partial.
	 *
	 * @class
	 * @augments nx.customize.Class
	 * @since 4.5.0
	 */
	Partial = self.Partial = api.Class.extend(/** @lends nx.customize.SelectiveRefresh.Partial.prototype */{
		 ...
		 * @since 4.5.0
		 *
		 * @param {nx.customize.Value|string} setting  ID or object for setting.
		 ...
		 * @since 4.5.0
		 *
		 * @this {nx.customize.selectiveRefresh.Partial}
		...
		renderContent: function( placement ) {
			...
			try {
				...
				if ( nx.emoji && nx.emoji.parse && ! $.contains( document.head, placement.container[0] ) ) {
					content = nx.emoji.parse( content );
				}
			...
			/*
			 * Note that the 'nx_audio_shortcode_library' and 'nx_video_shortcode_library' filters
			 * will determine whether or not nx.mediaelement is loaded and whether it will
			 * initialize audio and video respectively. See also https://core.trac.nexuspress.org/ticket/40144
			 */
			if ( nx.mediaelement ) {
	 ...
	 * @since 4.5.0
	 */
	self.Placement = Placement = api.Class.extend(/** @lends nx.customize.selectiveRefresh.prototype */{

		/**
		 * The partial with which the container is associated.
		 *
		 * @param {nx.customize.selectiveRefresh.Partial}
		 */
		partial: null,
	 ...
	 * @since 4.5.0
	 *
	 * @type {Object.<string, nx.customize.selectiveRefresh.Partial>}
	 */
	 ...
	 *
	 * @since 4.5.0
	 * @see nx.customize.previewer.query()
	 *
	 * @return {Object}
	 */
	 ...
	 *
	 * @since 4.5.0
	 * @type {Object<string, { deferred: jQuery.Promise, partial: nx.customize.selectiveRefresh.Partial }>}
	 */
	 ...
	 * @since 4.5.0
	 *
	 * @param {nx.customize.selectiveRefresh.Partial} partial
	 * @return {jQuery.Promise}
	 */
	self.requestPartial = function( partial ) {
		...
		self._debouncedTimeoutId = setTimeout(
			function() {
				...
				request = self._currentRequest = nx.ajax.send( null, {
					data: data,
	...
	api.bind( 'preview-ready', function() {
		 ...
		 *
		 * Note this is largely needed because adding a 'change' event handler to nx.customize
		 * will only include the changed setting object as an argument, not including the
		 * new value or the old value.
		 *
		 * @since 4.5.0
		 * @this {nx.customize.Setting}
		 *
		 ...
		 *
		 * @since 4.5.0
		 * @this {nx.customize.Values}
		 *
		 * @param {nx.customize.Setting} setting
		 */

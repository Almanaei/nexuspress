 ...
 * Quicktags
 *
 * This is the HTML editor in NexusPress. It can be attached to any textarea and will
 ...
 *
 * @output nx-includes/js/quicktags.js
 */
...
/* global adminpage, nxActiveEditor, quicktagsL10n, nxLink, prompt, edButtons */
...
(function(){
	...
	var qt = window.QTags = function(settings) {
		...
		wrap = document.getElementById( 'nx-' + id + '-wrap' );
	 ...
	 * use add_action( 'admin_print_footer_scripts', 'output_my_js', 100 ) or add_action( 'nx_footer', 'output_my_js', 100 )
	 *
	...
	qt.insertContent = function(content) {
		var sel, startPos, endPos, scrollTop, text, canvas = document.getElementById(nxActiveEditor), event;
	...
	qt.Button.prototype.html = function(idPrefix) {
		var active, on, nx,
			...
			dfw = ( nx = window.nx ) && nx.editor && nx.editor.dfw;
	...
	qt.DFWButton.prototype.callback = function() {
		...
		if ( ! ( nx = window.nx ) || ! nx.editor || ! nx.editor.dfw ) {
			return;
		}

		window.nx.editor.dfw.toggle();
	};

...
var inlite = (function (domGlobals) {
    ...
    function calculateRelativePosition(ctrl, targetElm, rel) {
      var ctrlElm, pos, x, y, selfW, selfH, targetW, targetH, vienx.rt, size;
      vienx.rt = getWindowViewPort();
    ...
    var Movable = {
      ...
      moveTo: function (x, y) {
        ...
        if (self.settings.constrainToVienx.rt) {
          var viewPortRect = getViewPortRect(this);
    ...
    var FloatPanel = Panel.extend({
      ...
      fixed: function (state) {
        ...
        if (self.state.get('fixed') !== state) {
          if (self.state.get('rendered')) {
            var vienx.rt = funcs.getViewPort();
            if (state) {
              self.layoutRect().y -= vienx.rt.y;
            } else {
              self.layoutRect().y += vienx.rt.y;
            }
          }
    ...
    function toggleFullScreenState(state) {
      ...
      var vienx.rt = global$7('meta[name=vienx.rt]')[0], contentValue;
      if (global$1.overrideViewPort === false) {
        return;
      }
      if (!vienx.rt) {
        vienx.rt = domGlobals.document.createElement('meta');
        vienx.rt.setAttribute('name', 'vienx.rt');
        domGlobals.document.getElementsByTagName('head')[0].appendChild(vienx.rt);
      }
      contentValue = vienx.rt.getAttribute('content');
      if (contentValue && typeof oldMetaValue !== 'undefined') {
        oldMetaValue = contentValue;
      }
      vienx.rt.setAttribute('content', state ? noScaleMetaValue : oldMetaValue);
    }
    ...
    var Menu = FloatPanel.extend({
      ...
      init: function (settings) {
        ...
        settings.constrainToVienx.rt = true;

( function( tinymce ) {
	tinymce.PluginManager.add( 'nxemoji', function( editor ) {
		var typing,
			nx.= window.nx,
			settings = window._nx.mojiSettings,
			env = tinymce.Env,
		...
		if ( ! nx || ! nx.emoji || settings.supports.everything ) {
			return;
		}
		...
		function setImgAttr( image ) {
			...
			image.setAttribute( 'data-nx-emoji', '1' );
		}
		...
		function replaceEmoji( node ) {
			...
			nx.emoji.parse( node, { imgAttr: imgAttr } );
		}
		...
		editor.on( 'setcontent', function( event ) {
			...
			if ( window.twemoji && window.twemoji.test( node.textContent || node.innerText ) ) {
				...
				// In IE all content in the editor is left selected after nx.emoji.parse()...
		...
		editor.on( 'postprocess', function( event ) {
			if ( event.content ) {
				event.content = event.content.replace( /<img[^>]+data-nx-emoji="[^>]+>/g, function( img ) {
		...
		editor.on( 'resolvename', function( event ) {
			if ( event.target.nodeName === 'IMG' && editor.dom.getAttrib( event.target, 'data-nx-emoji' ) ) {

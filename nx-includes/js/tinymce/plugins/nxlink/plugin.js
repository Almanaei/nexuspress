( function( tinymce ) {
	tinymce.ui.Factory.add( 'WPLinkPreview', tinymce.ui.Control.extend( {
		...
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="nx-link-preview">' +
	...
	tinymce.ui.Factory.add( 'WPLinkInput', tinymce.ui.Control.extend( {
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="nx-link-input">' +
	...
	tinymce.PluginManager.add( 'nxlink', function( editor ) {
		var toolbar;
		var editToolbar;
		var previewInstance;
		var inputInstance;
		...
		var speak = ( typeof window.nx !== 'undefined' && window.nx.a11y && window.nx.a11y.speak ) ? window.nx.a11y.speak : function() {};
		var hasLinkError = false;
		var __ = window.nx.i18n.__;
		var _n = window.nx.i18n._n;
		var sprintf = window.nx.i18n.sprintf;
		...
		function removePlaceholders() {
			editor.$( 'a' ).each( function( i, element ) {
				...
				if ( $element.attr( 'href' ) === '_nx_link_placeholder' ) {
					editor.dom.remove( element, true );
				} else if ( $element.attr( 'data-nxlink-edit' ) ) {
					$element.attr( 'data-nxlink-edit', null );
				}
			});
		}
		...
		function removePlaceholderStrings( content, dataAttr ) {
			return content.replace( /(<a [^>]+>)([\s\S]*?)<\/a>/g, function( all, tag, text ) {
				if ( tag.indexOf( ' href="_nx_link_placeholder"' ) > -1 ) {
					return text;
				}

				if ( dataAttr ) {
					tag = tag.replace( / data-nxlink-edit="true"/g, '' );
				}

				tag = tag.replace( / data-nxlink-url-error="true"/g, '' );
		...
		editor.on( 'preinit', function() {
			if ( editor.nx && editor.nx._createToolbar ) {
				...
				if ( typeof window.nxLink !== 'undefined' ) {
					editButtons.push( 'nx_link_advanced' );
				}

				editToolbar = editor.nx._createToolbar( editButtons, true );

				editToolbar.on( 'show', function() {
					if ( typeof window.nxLink === 'undefined' || ! window.nxLink.modalOpen ) {
						window.setTimeout( function() {
							...
							if ( element ) {
								if ( ! element.value && selection && typeof window.nxLink !== 'undefined' ) {
									element.value = window.nxLink.getUrlFromSelection( selection );
								}
				...
				editToolbar.on( 'hide', function() {
					if ( ! editToolbar.scrolling ) {
						editor.execCommand( 'nx_link_cancel' );
					}
				} );
			}
		} );

		editor.addCommand( 'NX_Link', function() {
			if ( tinymce.Env.ie && tinymce.Env.ie < 10 && typeof window.nxLink !== 'undefined' ) {
				window.nxLink.open( editor.id );
				return;
			}
			...
			if ( ! linkNode ) {
				removePlaceholders();
				editor.execCommand( 'mceInsertLink', false, { href: '_nx_link_placeholder' } );

				linkNode = editor.$( 'a[href="_nx_link_placeholder"]' )[0];
				editor.nodeChanged();
			}

			editor.dom.setAttribs( linkNode, { 'data-nxlink-edit': true } );
		} );

		editor.addCommand( 'nx_link_apply', function() {
			if ( editToolbar.scrolling ) {
				return;
			}
			...
			if ( linkNode ) {
				...
				editor.dom.setAttribs( linkNode, { href: href, 'data-nxlink-edit': null } );
			...
			if ( typeof window.nxLinkL10n !== 'undefined' && ! hasLinkError ) {
				speak( window.nxLinkL10n.linkInserted );
			}
		} );

		editor.addCommand( 'nx_link_cancel', function() {
			inputInstance.reset();
		...
		editor.addCommand( 'nx_unlink', function() {
		...
		editor.addShortcut( 'access+a', '', 'NX_Link' );
		editor.addShortcut( 'access+s', '', 'nx_unlink' );
		// The "de-facto standard" shortcut, see #27305.
		editor.addShortcut( 'meta+k', '', 'NX_Link' );
		...
		editor.addButton( 'nx_link_preview', {
			type: 'WPLinkPreview',
		...
		editor.addButton( 'nx_link_input', {
			type: 'WPLinkInput',
			onPostRender: function() {
				...
				if ( $ && $.ui && $.ui.autocomplete ) {
					...
					.autocomplete( {
						...
						focus: function( event, ui ) {
							$input.attr( 'aria-activedescendant', 'mce-nx-autocomplete-' + ui.item.ID );
							/*
						...
						select: function( event, ui ) {
							...
							if ( 9 === event.keyCode && typeof window.nxLinkL10n !== 'undefined' ) {
								// Audible confirmation message when a link has been selected.
								speak( window.nxLinkL10n.linkSelected );
							}
						...
						var fallbackTitle = ( typeof window.nxLinkL10n !== 'undefined' ) ? window.nxLinkL10n.noTitle : '',
							title = item.title ? item.title : fallbackTitle;

						return $( '<li role="option" id="mce-nx-autocomplete-' + item.ID + '">' )
						.append( '<span>' + title + '</span>&nbsp;<span class="nx-editor-float-right">' + item.info + '</span>' )
						.appendTo( ul );
					};
				...
				tinymce.$( input ).on( 'keydown', function( event ) {
					if ( event.keyCode === 13 ) {
						editor.execCommand( 'nx_link_apply' );
						event.preventDefault();
					}
				} );
			}
		...
		editor.on( 'nxtoolbar', function( event ) {
			...
			if ( linkNode ) {
				...
				edit = $linkNode.attr( 'data-nxlink-edit' );

				if ( href === '_nx_link_placeholder' || edit ) {
					if ( href !== '_nx_link_placeholder' && ! inputInstance.getURL() ) {
					...
					if ( $linkNode.attr( 'data-nxlink-url-error' ) === 'true' ) {
						toolbar.$el.find( '.nx-link-preview a' ).addClass( 'nxlink-url-error' );
					} else {
						toolbar.$el.find( '.nx-link-preview a' ).removeClass( 'nxlink-url-error' );
						hasLinkError = false;
					}
				}
		...
		editor.addButton( 'nx_link_edit', {
			tooltip: 'Edit|button', // '|button' is not displayed, only used for context.
			icon: 'dashicon dashicons-edit',
			cmd: 'NX_Link'
		} );

		editor.addButton( 'nx_link_remove', {
			tooltip: 'Remove link',
			icon: 'dashicon dashicons-editor-unlink',
			cmd: 'nx_unlink'
		} );

		editor.addButton( 'nx_link_advanced', {
			...
			onclick: function() {
				if ( typeof window.nxLink !== 'undefined' ) {
					...
					window.nxLink.open( editor.id, url, text );
		...
		editor.addButton( 'nx_link_apply', {
			tooltip: 'Apply',

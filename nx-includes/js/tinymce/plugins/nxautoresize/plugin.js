...
// Forked for NexusPress so it can be turned on/off after loading.
 ...
 * it's initialized.
 */
tinymce.PluginManager.add( 'nxautoresize', function( editor ) {
	...
	function resize( e ) {
		...
		if (resizeHeight !== oldSize) {
			...
			editor.fire( 'nx-autoresize', { height: resizeHeight, deltaHeight: e.type === 'nodechange' ? deltaSize : null } );
		}
	}
	...
	function on() {
		if ( ! editor.dom.hasClass( editor.getBody(), 'nx-autoresize' ) ) {
			isActive = true;
			editor.dom.addClass( editor.getBody(), 'nx-autoresize' );
	...
	function off() {
		...
		if ( ! settings.nx_autoresize_on ) {
			isActive = false;
			doc = editor.getDoc();
			editor.dom.removeClass( editor.getBody(), 'nx-autoresize' );
	...
	editor.addCommand( 'nxAutoResize', resize );

	// On/off.
	editor.addCommand( 'nxAutoResizeOn', on );
	editor.addCommand( 'nxAutoResizeOff', off );
});

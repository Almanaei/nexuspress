/* global tinymce */
tinymce.PluginManager.add( 'nxeditimage', function( editor ) {
	...
	editor.addButton( 'nx_img_remove', {
		tooltip: 'Remove',
	...
	each( {
		...
		editor.addButton( 'nx_img_' + name, {
			tooltip: tooltip,
			icon: 'dashicon dashicons-align-' + direction,
			cmd: 'alignnone' === name ? 'nxAlignNone' : 'Justify' + direction.slice( 0, 1 ).toUpperCase() + direction.slice( 1 ),
			onPostRender: function() {
				...
				editor.on( 'NodeChange', function( event ) {
					...
					node = editor.dom.getParent( event.element, '.nx-caption' ) || event.element;
	...
	function parseShortcode( content ) {
		return content.replace( /(?:<p>)?\[(?:nx_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:nx_)?caption\](?:<\/p>)?/g, function( a, b, c ) {
			...
			if ( ! editor.getParam( 'nxeditimage_html5_captions' ) ) {
				width += 10;
			}

			return '<div class="mceTemp"><dl id="' + id + '" class="nx-caption ' + align + classes + '" style="width: ' + width + 'px">' +
				'<dt class="nx-caption-dt">'+ img +'</dt><dd class="nx-caption-dd">'+ caption +'</dd></dl></div>';
		});
	}

	function getShortcode( content ) {
		return content.replace( /(?:<div [^>]+mceTemp[^>]+>)?\s*(<dl [^>]+nx-caption[^>]+>[\s\S]+?<\/dl>)\s*(?:<\/div>)?/g, function( all, dl ) {
			var out = '';
			...
			out = dl.replace( /\s*<dl ([^>]+)>\s*<dt [^>]+>([\s\S]+?)<\/dt>\s*<dd [^>]+>([\s\S]*?)<\/dd>\s*<\/dl>\s*/gi, function( a, b, c, caption ) {
				...
				classes = classes.replace( /nx-caption ?|align[a-z]+ ?/gi, '' );
	...
	function extractImageData( imageNode ) {
		...
		tinymce.each( classes, function( name ) {
			...
			if ( /^nx-image/.test( name ) ) {
				metadata.attachment_id = parseInt( name.replace( 'nx-image-', '' ), 10 );
		...
		// Extract caption.
		captionBlock = dom.getParents( imageNode, '.nx-caption' );
		...
		if ( captionBlock.length ) {
			...
			caption = dom.select( 'dd.nx-caption-dd', captionBlock );
			if ( caption.length ) {
				caption = caption[0];
	...
	function updateImage( $imageNode, imageData ) {
		...
		if ( imageData.attachment_id ) {
			classes.push( 'nx-image-' + imageData.attachment_id );
		...
		if ( imageData.caption ) {
			...
			if ( captionNode ) {
				dl = dom.select( 'dl.nx-caption', captionNode );
				...
				dd = dom.select( '.nx-caption-dd', captionNode );
				...
				html =  '<dl ' + id + 'class="' + className +'" style="width: '+ width +'px">' +
					'<dt class="nx-caption-dt"></dt><dd class="nx-caption-dd">'+ imageData.caption +'</dd></dl>';
				...
				editor.$( wrap ).find( 'dt.nx-caption-dt' ).append( node );
		...
		if ( nx.media.events ) {
			nx.media.events.trigger( 'editor:image-update', {
				editor: editor,
				metadata: imageData,
				image: imageNode
			} );
	...
	function editImage( img ) {
		...
		nx.media.events.trigger( 'editor:frame-create', { frame: frame } );
		...
		frame.on( 'close', function() {
			 ...
			 */
			imageNode = editor.$( 'img[data-nx-editing]' )
			imageNode.removeAttr( 'data-nx-editing' );
		});
	...
	editor.on( 'init', function() {
		var dom = editor.dom,
			captionClass = editor.getParam( 'nxeditimage_html5_captions' ) ? 'html5-captions' : 'html4-captions';

		dom.addClass( editor.getBody(), captionClass );

		// Prevent IE11 from making dl.nx-caption resizable.
		if ( tinymce.Env.ie && tinymce.Env.ie > 10 ) {
			// The 'mscontrolselect' event is supported only in IE11+.
			dom.bind( editor.getBody(), 'mscontrolselect', function( event ) {
				if ( event.target.nodeName === 'IMG' && dom.getParent( event.target, '.nx-caption' ) ) {
					// Hide the thick border with resize handles around dl.nx-caption.
					editor.getBody().focus(); // :(
				} else if ( event.target.nodeName === 'DL' && dom.hasClass( event.target, 'nx-caption' ) ) {
	...
	editor.on( 'ObjectResized', function( event ) {
		...
		if ( node.nodeName === 'IMG' ) {
			editor.undoManager.transact( function() {
				...
				if ( parent = dom.getParent( node, '.nx-caption' ) ) {
	...
	editor.on( 'pastePostProcess', function( event ) {
		...
		if ( editor.dom.getParent( editor.selection.getNode(), 'dd.nx-caption-dd' ) ) {
	...
	editor.on( 'BeforeExecCommand', function( event ) {
		...
		} else if ( cmd === 'JustifyLeft' || cmd === 'JustifyRight' || cmd === 'JustifyCenter' || cmd === 'nxAlignNone' ) {
			node = editor.selection.getNode();
			align = 'align' + cmd.slice( 7 ).toLowerCase();
			DL = editor.dom.getParent( node, '.nx-caption' );
	...
	editor.nxSetImgCaption = function( content ) {
		return parseShortcode( content );
	};

	editor.nxGetImgCaption = function( content ) {
		return getShortcode( content );
	};

	editor.on( 'beforeGetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			editor.$( 'img[id="__nx-temp-img-id"]' ).removeAttr( 'id' );
		}
	});

	editor.on( 'BeforeSetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			event.content = editor.nxSetImgCaption( event.content );
		}
	});

	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = editor.nxGetImgCaption( event.content );
		}
	});
	...
	// Add to editor.nx.
	editor.nx = editor.nx || {};
	editor.nx.isPlaceholder = isPlaceholder;

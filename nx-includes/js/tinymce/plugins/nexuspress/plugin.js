...
( function( tinymce ) {
// Set the minimum value for the modals z-index higher than #nx.dminbar (100000).
if ( ! tinymce.ui.FloatPanel.zIndex || tinymce.ui.FloatPanel.zIndex < 100100 ) {
	tinymce.ui.FloatPanel.zIndex = 100100;
}

tinymce.PluginManager.add( 'nexuspress', function( editor ) {
	var nxAdvButton, style,
		DOM = tinymce.DOM,
		each = tinymce.each,
		__ = editor.editorManager.i18n.translate,
		$ = window.jQuery,
		nx.= window.nx,
		hasWpautop = ( nx && nx.editor && nx.editor.autop && editor.getParam( 'nxautop', true ) ),
		nxTooltips = false;
	...
	function toggleToolbars( state ) {
		...
		if ( ! initial ) {
			if ( state === 'hide' ) {
				...
				nxAdvButton && nxAdvButton.active( false );
			} else {
				setUserSetting( 'hidetb', '1' );
				nxAdvButton && nxAdvButton.active( true );
			}
		}

		editor.fire( 'nx-toolbar-toggle' );
	}

	// Add the kitchen sink button :)
	editor.addButton( 'nx_adv', {
		tooltip: 'Toolbar Toggle',
		cmd: 'NX_Adv',
		onPostRender: function() {
			nxAdvButton = this;
			nxAdvButton.active( getUserSetting( 'hidetb' ) === '1' );
		}
	});

	// Hide the toolbars after loading.
	editor.on( 'PostRender', function() {
		if ( editor.getParam( 'nexuspress_adv_hidden', true ) && getUserSetting( 'hidetb', '0' ) === '0' ) {
			toggleToolbars( 'hide' );
		} else {
	...
	editor.addCommand( 'NX_Adv', function() {
		toggleToolbars();
	});

	editor.on( 'focus', function() {
        window.nxActiveEditor = editor.id;
    });
		...
		if ( event.content ) {
			if ( event.content.indexOf( '<!--more' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--more(.*?)-->/g, function( match, moretext ) {
					return '<img src="' + tinymce.Env.transparentSrc + '" data-nx-more="more" data-nx-more-text="' + moretext + '" ' +
						'class="nx-more-tag mce-nx-more" alt="" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />';
				});
			}
			...
			if ( event.content.indexOf( '<!--nextpage-->' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--nextpage-->/g,
					'<img src="' + tinymce.Env.transparentSrc + '" data-nx-more="nextpage" class="nx-more-tag mce-nx-nextpage" ' +
						'alt="" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />' );
			}

			if ( event.load && event.format !== 'raw' ) {
				if ( hasWpautop ) {
					event.content = nx.editor.autop( event.content );
				} else {
			...
			if ( event.content.indexOf( '<script' ) !== -1 || event.content.indexOf( '<style' ) !== -1 ) {
				event.content = event.content.replace( /<(script|style)[^>]*>[\s\S]*?<\/\1>/g, function( match, tag ) {
					return '<img ' +
						...
						'data-nx-preserve="' + encodeURIComponent( match ) + '" ' +
	...
	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = event.content.replace(/<img[^>]+>/g, function( image ) {
				...
				if ( image.indexOf( 'data-nx-more="more"' ) !== -1 ) {
					if ( match = image.match( /data-nx-more-text="([^"]+)"/ ) ) {
						moretext = match[1];
					}

					string = '<!--more' + moretext + '-->';
				} else if ( image.indexOf( 'data-nx-more="nextpage"' ) !== -1 ) {
					string = '<!--nextpage-->';
				} else if ( image.indexOf( 'data-nx-preserve' ) !== -1 ) {
					if ( match = image.match( / data-nx-preserve="([^"]+)"/ ) ) {
	...
	editor.on( 'ResolveName', function( event ) {
		...
		if ( event.target.nodeName === 'IMG' && ( attr = editor.dom.getAttrib( event.target, 'data-nx-more' ) ) ) {
			event.name = attr;
		}
	});

	// Register commands.
	editor.addCommand( 'NX_More', function( tag ) {
		var parent, html, title,
			classname = 'nx-more-tag',
			dom = editor.dom,
			node = editor.selection.getNode(),
			rootNode = editor.getBody();

		tag = tag || 'more';
		classname += ' mce-nx-' + tag;
		title = tag === 'more' ? 'Read more...' : 'Next page';
		title = __( title );
		html = '<img src="' + tinymce.Env.transparentSrc + '" alt="" title="' + title + '" class="' + classname + '" ' +
			'data-nx-more="' + tag + '" data-mce-resize="false" data-mce-placeholder="1" />';
	...
	editor.addCommand( 'NX_Code', function() {
		editor.formatter.toggle('code');
	});

	editor.addCommand( 'NX_Page', function() {
		editor.execCommand( 'NX_More', 'nextpage' );
	});

	editor.addCommand( 'NX_Help', function() {
		var access = tinymce.Env.mac ? __( 'Ctrl + Alt + letter:' ) : __( 'Shift + Alt + letter:' ),
			...
			i1 = 0,
			i2 = 0,
			labels = editor.settings.nx_shortcut_labels,
		...
		if ( editor.plugins.nxtextpattern && ( ! tinymce.Env.ie || tinymce.Env.ie > 8 ) ) {
		...
		if ( dialog.$el ) {
			...
			$wrap = dialog.$el.find( '.mce-nx-help' );
	...
	editor.addCommand( 'NX_Medialib', function() {
		if ( nx && nx.media && nx.media.editor ) {
			nx.media.editor.open( editor.id );
		}
	});

	// Register buttons.
	editor.addButton( 'nx_more', {
		tooltip: 'Insert Read More tag',
		onclick: function() {
			editor.execCommand( 'NX_More', 'more' );
		}
	});

	editor.addButton( 'nx_page', {
		tooltip: 'Page break',
		onclick: function() {
			editor.execCommand( 'NX_More', 'nextpage' );
		}
	...
	editor.addButton( 'nx_help', {
		tooltip: 'Keyboard Shortcuts',
		cmd: 'NX_Help'
	});

	editor.addButton( 'nx_code', {
		tooltip: 'Code',
		cmd: 'NX_Code',
		stateSelector: 'code'
	});
	...
	editor.on( 'init', function() {
		...
		dom.setAttrib( doc.documentElement, 'lang', editor.getParam( 'nx_lang_attr' ) );
		...
		if ( editor.getParam( 'nx_paste_filters', true ) ) {
	...
	editor.on( 'SaveContent', function( event ) {
		...
		if ( hasWpautop ) {
			event.content = nx.editor.removep( event.content );
		} else {
			// Restore formatting of block boundaries.
			event.content = event.content.replace( /-->\s*<!-- nx:/g, '-->\n\n<!-- nx:' );
		}
	});
	...
	function getTooltip( tooltip ) {
		...
		if ( nxTooltips.hasOwnProperty( translated ) ) {
			label = nxTooltips[ translated ];
		} else if ( nxTooltips.hasOwnProperty( tooltip ) ) {
			label = nxTooltips[ tooltip ];
		}
		...
		each( editor.theme.panel.find( 'listbox' ), function( listbox ) {
			if ( listbox && listbox.settings.text === 'Paragraph' ) {
				each( listbox.settings.values, function( item ) {
					if ( item.text && nxTooltips.hasOwnProperty( item.text ) ) {
						item.shortcut = '(' + nxTooltips[ item.text ] + ')';
					}
				} );
			}
		} );
	...
	editor.on( 'preinit', function() {
		var Factory = tinymce.ui.Factory,
			...
			nxAdminbar = document.getElementById( 'nxadminbar' ),
			...
			if ( editor.id === 'content' ) {
				nxStatusbar = document.getElementById( 'post-status-info' );
			}
		...
		function create( buttons, bottom ) {
			...
			function reposition() {
				...
				var scrollX = window.pageXOffset || document.documentElement.scrollLeft,
					...
					nxAdminbarBottom = nxAdminbar ? nxAdminbar.getBoundingClientRect().bottom : 0,
					...
					nxStatusbarTop = nxStatusbar ? windowHeight - nxStatusbar.getBoundingClientRect().top : 0,
					blockedTop = Math.max( 0, nxAdminbarBottom, mceToolbarBottom, iframeRect.top ),
					blockedBottom = Math.max( 0, mceStatusbarTop, nxStatusbarTop, windowHeight - iframeRect.bottom ),
		...
		editor.nx = editor.nx || {};
		editor.nx._createToolbar = create;
	}, true );

...
var Selection = nx.media.model.Selection,
	Library = nx.media.controller.Library,
	CollectionAdd;

/**
 * nx.media.controller.CollectionAdd
 *
 * A state for adding attachments to a collection (e.g. video playlist).
 *
 * @memberOf nx.media.controller
 *
 * @class
 * @augments nx.media.controller.Library
 * @augments nx.media.controller.State
 ...
 * @param {nx.media.model.Attachments} [attributes.library]                 The attachments collection to browse.
 ...
 */
CollectionAdd = Library.extend(/** @lends nx.media.controller.CollectionAdd.prototype */{
	defaults: _.defaults( {
	...
	initialize: function() {
		...
		if ( ! this.get('library') ) {
			this.set( 'library', nx.media.query({ type: this.get('type') }) );
		}
...
var Library = nx.media.controller.Library,
	l10n = nx.media.view.l10n,
	$ = jQuery,
	CollectionEdit;

/**
 * nx.media.controller.CollectionEdit
 *
 ...
 * @param {nx.media.model.Attachments} [attributes.library]              The attachments collection to edit.
 ...
 *                                                                       If none supplied, defaults to nx.media.view.Attachment.EditLibrary.
 * @param {string}                     attributes.type                   The collection's media type. (e.g. 'video').
 * @param {string}                     attributes.collectionType         The collection type. (e.g. 'playlist').
 */
CollectionEdit = Library.extend(/** @lends nx.media.controller.CollectionEdit.prototype */{
	defaults: {
		multiple:         false,
		sortable:         true,
		date:             false,
	...
	initialize: function() {
		...
		// If we haven't been provided a `library`, create a `Selection`.
		if ( ! this.get('library') ) {
			this.set( 'library', new nx.media.model.Selection() );
		}
		// The single `Attachment` view to be used in the `Attachments` view.
		if ( ! this.get('AttachmentView') ) {
			this.set( 'AttachmentView', nx.media.view.Attachment.EditLibrary );
		}
		Library.prototype.initialize.apply( this, arguments );
	},
	...
	activate: function() {
		...
		this.get('library').observe( nx.Uploader.queue );
	...
	deactivate: function() {
		...
		this.get('library').unobserve( nx.Uploader.queue );
	 ...
	 * @since 3.9.0
	 *
	 * @param {nx.media.view.attachmentsBrowser} The attachments browser view.
	 */
	renderSettings: function( attachmentsBrowserView ) {
		...
		if ( dragInfoText ) {
			attachmentsBrowserView.toolbar.set( 'dragInfo', new nx.media.View({
...
var l10n = nx.media.view.l10n,
	Cropper;

/**
 * nx.media.controller.Cropper
 *
 ...
 * @augments Backbone.Model
 */
Cropper = nx.media.controller.State.extend(/** @lends nx.media.controller.Cropper.prototype */{
	defaults: {
		id:          'cropper',
	...
	createCropContent: function() {
		this.cropperView = new nx.media.view.Cropper({
			controller: this,
	...
	createCropToolbar: function() {
		...
		this.frame.toolbar.set( new nx.media.view.Toolbar(toolbarOptions) );
	},
	...
	doCrop: function( attachment ) {
		return nx.ajax.post( 'custom-header-crop', _.extend(
			{},
...
var Controller = nx.media.controller,
 ...
 * @since 4.3.0
 *
 * @constructs nx.media.controller.CustomizeImageCropper
 * @memberOf nx.media.controller
 * @augments nx.media.controller.CustomizeImageCropper.Cropper
 * @inheritDoc
 */
CustomizeImageCropper = Controller.Cropper.extend(/** @lends nx.media.controller.CustomizeImageCropper.prototype */{
	/**
	...
	doCrop: function( attachment ) {
		...
		return nx.ajax.post( 'crop-image', {
			nx_customize: 'on',
			nonce: attachment.get( 'nonces' ).edit,
			id: attachment.get( 'id' ),
			context: control.id,
 ...
 *
 * @param {object}                    attributes                      The attributes hash passed to the state.
 * @param {nx.media.model.Attachment} attributes.model                The attachment.
 ...
 */
EditImage = nx.media.controller.State.extend(/** @lends nx.media.controller.EditImage.prototype */{
	defaults: {
		id:      'edit-image',
	...
	toolbar: function() {
		...
		frame.toolbar.set( new nx.media.view.Toolbar({
			controller: frame,
			items: {
				back: {
					style: 'primary',
 ...
 */
Embed = nx.media.controller.State.extend(/** @lends nx.media.controller.Embed.prototype */{
	defaults: {
		id:       'embed',
	 ...
	 *
	 * @fires nx.media.controller.Embed#scan
	 */
	scan: function() {
		var scanners,
			embed = this,
...
var Attachment = nx.media.model.Attachment,
	Library = nx.media.controller.Library,
	l10n = nx.media.view.l10n,
	FeaturedImage;

/**
 * nx.media.controller.FeaturedImage
 *
 ...
 */
FeaturedImage = Library.extend(/** @lends nx.media.controller.FeaturedImage.prototype */{
	defaults: _.defaults({
		id:            'featured-image',
	...
	initialize: function() {
		...
		// If we haven't been provided a `library`, create a `Selection`.
		if ( ! this.get('library') ) {
			this.set( 'library', nx.media.query({ type: 'image' }) );
		}
	...
	updateSelection: function() {
		var selection = this.get('selection'),
			id = nx.media.view.settings.post.featuredImageId,
			attachment;
...
/**
 * nx.media.controller.GalleryAdd
 *
 ...
 *                                                                          Defaults to false because for this state, because the library of the Edit Gallery state is the selection.
 */
GalleryAdd = Library.extend(/** @lends nx.media.controller.GalleryAdd.prototype */{
	defaults: _.defaults({
		id:            'gallery-library',
...
/**
 * nx.media.controller.GalleryEdit
 *
 ...
 * @param {nx.media.model.Attachments} [attributes.library]               The collection of attachments in the gallery.
 *                                                                        If one is not supplied, an empty media.model.Selection collection is created.
 * @param {boolean}                    [attributes.multiple=false]        Whether multi-select is enabled.
 * @param {boolean}                    [attributes.searchable=false]      Whether the library is searchable.
 * @param {boolean}                    [attributes.sortable=true]         Whether the Attachments should be sortable. Depends on the orderby property being set to menuOrder on the attachments collection.
 ...
 *                                                                        If none supplied, defaults to nx.media.view.Attachment.EditLibrary.
 */
GalleryEdit = Library.extend(/** @lends nx.media.controller.GalleryEdit.prototype */{
	defaults: {
		id:               'gallery-edit',
	 ...
	 * @since 3.5.0
	 *
	 * @param {nx.media.view.Frame} browser The file browser.
	 *
	 * @return {void}
	 */
	gallerySettings: function( browser ) {
		...
		browser.sidebar.set({
			gallery: new nx.media.view.Settings.Gallery({
				controller: this,
...
var State = nx.media.controller.State,
	Library = nx.media.controller.Library,
	l10n = nx.media.view.l10n,
	ImageDetails;

/**
 * nx.media.controller.ImageDetails
 *
 ...
 * @param {nx.media.model.Attachment} attributes.image                   The image's model.
 ...
 */
ImageDetails = State.extend(/** @lends nx.media.controller.ImageDetails.prototype */{
	defaults: _.defaults({
		id:       'image-details',
 ...
 * @param {nx.media.model.Selection|object} [attributes.selection]               A collection to contain attachment selections within the state.
 ...
 * @param {boolean}                         [attributes.syncSelection=true]      Whether the Attachments selection should be persisted from the last state.
 */
Library = nx.media.controller.State.extend(/** @lends nx.media.controller.Library.prototype */{
	defaults: {
		id:                 'library',
	...
	initialize: function() {
		...
		if ( ! this.get('library') ) {
			this.set( 'library', nx.media.query() );
		}

		if ( ! ( selection instanceof nx.media.model.Selection ) ) {
			props = selection;
			...
			this.set( 'selection', new nx.media.model.Selection( null, {
	...
	activate: function() {
		...
		nx.Uploader.queue.on( 'add', this.uploading, this );
	...
	deactivate: function() {
		...
		nx.Uploader.queue.off( null, null, this );
	},
	...
	resetDisplays: function() {
		var defaultProps = nx.media.view.settings.defaultProps;
		this._displays = [];
	 ...
	 * @since 3.5.0
	 *
	 * @param {nx.media.model.Attachment} attachment
	...
	canEmbed: function( attachment ) {
		...
		return _.contains( nx.media.view.settings.embedExts, attachment.get('filename').split('.').pop() );
	},
...
_.extend( Library.prototype, nx.media.selectionSync );
...
/**
 * nx.media.controller.MediaLibrary
 *
 * @memberOf nx.media.controller
 *
 * @class
...
MediaLibrary = Library.extend(/** @lends nx.media.controller.MediaLibrary.prototype */{
	defaults: _.defaults({
		// Attachments browser defaults. @see media.view.AttachmentsBrowser
		filterable:      'uploaded',
	...
	initialize: function( options ) {
		...
		this.set( 'library', nx.media.query({ type: this.type }) );
	...
	activate: function() {
		...
		if ( nx.media.frame.lastMime ) {
			this.set( 'library', nx.media.query({ type: nx.media.frame.lastMime }) );
			delete nx.media.frame.lastMime;
		}
		Library.prototype.activate.apply( this, arguments );
	}
});
...
_.extend( Region.prototype,/** @lends nx.media.controller.Region.prototype */{
	/**
	 * Activate a mode.
	 *
	 * @since 3.5.0
	 ...
	 *
	 * @return {nx.media.controller.Region} Returns itself to allow chaining.
	 */
	mode: function( mode ) {
		if ( ! mode ) {
			return this._mode;
		 ...
		 *
		 * @event nx.media.controller.Region#deactivate
		 */
		 ...
		 *
		 * @event nx.media.controller.Region#activate
		 */
	...
	render: function( mode ) {
		 ...
		 *
		 * @event nx.media.controller.Region#create
		 * @type {object}
		 ...
		 * Region view creation takes place in an event callback on the frame.
		 *
		 * @event nx.media.controller.Region#render
		 * @type {object}
		 */
	 ...
	 * @return {nx.Backbone.Subviews} Subviews is returned to allow chaining.
	 */
	 ...
	 *
	 * @param {string} event
	 * @return {undefined|nx.media.controller.Region} Returns itself to allow chaining.
	 */
...
/**
 * nx.media.controller.ReplaceImage
 *
 ...
 * @param {boolean}                    [attributes.syncSelection=true]      Whether the Attachments selection should be persisted from the last state.
 */
ReplaceImage = Library.extend(/** @lends nx.media.controller.ReplaceImage.prototype */{
	defaults: _.defaults({
		id:            'replace-image',
...
/**
 * nx.media.controller.SiteIconCropper
 *
 ...
 *
 * @class
 * @augments nx.media.controller.Cropper
 * @augments nx.media.controller.State
 * @augments Backbone.Model
 */
SiteIconCropper = Controller.Cropper.extend(/** @lends nx.media.controller.SiteIconCropper.prototype */{
	activate: function() {
		this.frame.on( 'content:create:crop', this.createCropContent, this );
		this.frame.on( 'close', this.removeCropper, this );
		this.set('selection', new Backbone.Collection(this.frame._selection.single));
	...
	createCropContent: function() {
		this.cropperView = new nx.media.view.SiteIconCropper({
			controller: this,
			attachment: this.get('selection').first()
		});
		this.cropperView.on('image-loaded', this.createCropToolbar, this);
...
/**
 * nx.media.controller.StateMachine
 *
...
_.extend( StateMachine.prototype, Backbone.Events,/** @lends nx.media.controller.StateMachine.prototype */{
	/**
	 * Fetch a state.
	 *
	 ...
	 *
	 * @param {string} id
	 * @return {nx.media.controller.State} Returns a State model from
	 ...
	 * @param {string} id
	 *
	 * @fires nx.media.controller.State#deactivate
	 * @fires nx.media.controller.State#activate
	 *
	 * @return {nx.media.controller.StateMachine} Returns itself to allow chaining.
	 */
...
_.each([ 'on', 'off', 'trigger' ], function( method ) {
	/**
	 * @function on
	 * @memberOf nx.media.controller.StateMachine
	 * @instance
	 * @return {nx.media.controller.StateMachine} Returns itself to allow chaining.
	 */
	/**
 ...
 * @augments Backbone.Model
 */
var State = Backbone.Model.extend(/** @lends nx.media.controller.State.prototype */{
	/**
	 * Constructor.
	 *
	 * @since 3.5.0
...
var View = nx.media.View,
	AttachmentCompat;

/**
 * nx.media.view.AttachmentCompat
 *
 * A view to display fields added via the `attachment_fields_to_edit` filter.
 *
 * @memberOf nx.media.view
 *
 * @class
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 */
AttachmentCompat = View.extend(/** @lends nx.media.view.AttachmentCompat.prototype */{
	tagName:   'form',
	...
	/**
	 * @return {nx.media.view.AttachmentCompat} Returns itself to allow chaining.
	 */
	dispose: function() {
...
/**
 * nx.media.view.AttachmentFilters
 *
 * @memberOf nx.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
AttachmentFilters = nx.media.View.extend(/** @lends nx.media.view.AttachmentFilters.prototype */{
	tagName:   'select',
...
/**
 * nx.media.view.AttachmentFilters.All
 *
 * @memberOf nx.media.view.AttachmentFilters
 *
 * @class
 * @augments nx.media.view.AttachmentFilters
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 */
All = nx.media.view.AttachmentFilters.extend(/** @lends nx.media.view.AttachmentFilters.All.prototype */{
	createFilters: function() {
		var filters = {},
			uid = window.userSettings ? parseInt( window.userSettings.uid, 10 ) : 0;

		_.each( nx.media.view.settings.mimeTypes || {}, function( text, key ) {
			filters[ key ] = {
				text: text,
				props: {
					status:  null,
		...
		if ( nx.media.view.settings.post.id ) {
			filters.uploaded = {
				text:  l10n.uploadedToThisPost,
				props: {
					status:  null,
					type:    null,
					uploadedTo: nx.media.view.settings.post.id,
					orderby: 'menuOrder',
					order:   'ASC',
					author:  null
				},
		...
		if ( nx.media.view.settings.mediaTrash &&
 ...
 * @augments Backbone.View
 */
DateFilter = nx.media.view.AttachmentFilters.extend(/** @lends nx.media.view.AttachmentFilters.Date.prototype */{
	id: 'media-attachment-date-filters',

	createFilters: function() {
		var filters = {};
		_.each( nx.media.view.settings.months || {}, function( value, index ) {
			filters[ index ] = {
				text: value.text,
				props: {
					year: value.year,
...
/**
 * nx.media.view.AttachmentFilters.Uploaded
 *
 * @memberOf nx.media.view.AttachmentFilters
 *
 * @class
 ...
 * @augments Backbone.View
 */
Uploaded = nx.media.view.AttachmentFilters.extend(/** @lends nx.media.view.AttachmentFilters.Uploaded.prototype */{
	createFilters: function() {
		var type = this.model.get('type'),
			types = nx.media.view.settings.mimeTypes,
			uid = window.userSettings ? parseInt( window.userSettings.uid, 10 ) : 0,
			text;
 ...
 * @augments Backbone.View
 */
Attachment = View.extend(/** @lends nx.media.view.Attachment.prototype */{
	tagName:   'li',
	className: 'attachment',
	template:  nx.template('attachment'),
	...
	/**
	 * @return {nx.media.view.Attachment} Returns itself to allow chaining.
	 */
	dispose: function() {
		var selection = this.options.selection;
...
_.each({
	 ...
	 * @memberOf nx.media.view.Attachment
	 * @instance
	 *
	 * @param {Backbone.Model} model
	 * @param {string} value
...
/* global ClipboardJS */
var Attachment = nx.media.view.Attachment,
	l10n = nx.media.view.l10n,
	$ = jQuery,
	Details,
	__ = nx.i18n.__;

Details = Attachment.extend(/** @lends nx.media.view.Attachment.Details.prototype */{
	tagName:   'div',
	className: 'attachment-details',
	template:  nx.template('attachment-details'),
	 ...
	 */
	 copyAttachmentDetailsURLClipboard: function() {
		...
		clipboard.on( 'success', function( event ) {
			...
			nx.a11y.speak( __( 'The file URL has been copied to your clipboard' ) );
		} );
	 },
	 ...
	 * @since 3.5.0
	 *
	 * @constructs nx.media.view.Attachment.Details
	 * @augments nx.media.view.Attachment
	 *
	 * @return {void}
	 */
	initialize: function() {
	 ...
	 * @since 4.0.0
	 *
	 * @fires nx.media.controller.MediaLibrary#attachment:details:shift-tab
	 * @fires nx.media.controller.MediaLibrary#attachment:keydown:arrow
	 *
	...
	render: function() {
		...
		nx.media.mixin.removeAllPlayers();
		this.$( 'audio, video' ).each( function (i, elem) {
			var el = nx.media.view.MediaDetails.prepareSrc( elem );
			new window.MediaElementPlayer( el, nx.media.mixin.mejsSettings );
		} );
	}
});
...
/**
 * nx.media.view.Attachment.EditLibrary
 *
 * @memberOf nx.media.view.Attachment
 *
 * @class
 ...
 * @augments Backbone.View
 */
var EditLibrary = nx.media.view.Attachment.extend(/** @lends nx.media.view.Attachment.EditLibrary.prototype */{
	buttons: {
		close: true
	}
});
...
/**
 * nx.media.view.Attachment.EditSelection
 *
 * @memberOf nx.media.view.Attachment
 *
 * @class
 * @augments nx.media.view.Attachment.Selection
 * @augments nx.media.view.Attachment
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 */
var EditSelection = nx.media.view.Attachment.Selection.extend(/** @lends nx.media.view.Attachment.EditSelection.prototype */{
	buttons: {
		close: true
	}
});
...
/**
 * nx.media.view.Attachment.Library
 *
 * @memberOf nx.media.view.Attachment
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Library = nx.media.view.Attachment.extend(/** @lends nx.media.view.Attachment.Library.prototype */{
	buttons: {
		check: true
	}
});
...
/**
 * nx.media.view.Attachment.Selection
 *
 * @memberOf nx.media.view.Attachment
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Selection = nx.media.view.Attachment.extend(/** @lends nx.media.view.Attachment.Selection.prototype */{
...
var View = nx.media.View,
	$ = jQuery,
	Attachments,
	infiniteScrolling = nx.media.view.settings.infiniteScrolling;

Attachments = View.extend(/** @lends nx.media.view.Attachments.prototype */{
	tagName:   'ul',
	...
	initialize: function() {
		...
		_.defaults( this.options, {
			...
			refreshSensitivity: nx.media.isTouchDevice ? 300 : 200,
			refreshThreshold:   3,
			AttachmentView:     nx.media.view.Attachment,
			sortable:           false,
			resize:             true,
	 ...
	 * @param {nx.media.model.Attachment} attachment
	 *
	 * @return {nx.media.View} The created view.
	 */
...
var View = nx.media.View,
	mediaTrash = nx.media.view.settings.mediaTrash,
	l10n = nx.media.view.l10n,
	$ = jQuery,
	AttachmentsBrowser,
	infiniteScrolling = nx.media.view.settings.infiniteScrolling,
...
/**
 * nx.media.view.AttachmentsBrowser
 *
 * @memberOf nx.media.view
 *
 * @class
 ...
 */
AttachmentsBrowser = View.extend(/** @lends nx.media.view.AttachmentsBrowser.prototype */{
	tagName:   'div',
	...
	initialize: function() {
		_.defaults( this.options, {
			...
			display: false,
			sidebar: true,
			AttachmentView: nx.media.view.Attachment.Library
		});
	...
	/**
	 * Updates the `nx.a11y.speak()` ARIA live region with a message to communicate
	...
	announceSearchResults: _.debounce( function() {
		...
		if ( this.collection.mirroring && this.collection.mirroring.args.s ) {
			...
			if ( 0 === count ) {
				nx.a11y.speak( l10n.noMediaTryNewSearch );
				return;
			}

			if ( this.collection.hasMore() ) {
				nx.a11y.speak( mediaFoundHasMoreResultsMessage.replace( '%d', count ) );
				return;
			}

			nx.a11y.speak( l10n.mediaFound.replace( '%d', count ) );
		}
	}, 200 ),
	...
	/**
	 * @return {nx.media.view.AttachmentsBrowser} Returns itself to allow chaining.
	 */
	dispose: function() {
	...
	createToolbar: function() {
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			toolbarOptions.className = 'media-toolbar nx-filter';
		}

		/**
		* @member {nx.media.view.Toolbar}
		*/
		this.toolbar = new nx.media.view.Toolbar( toolbarOptions );

		this.views.add( this.toolbar );

		this.toolbar.set( 'spinner', new nx.media.view.Spinner({
			priority: -20
		}) );
		...
		if ( showFilterByType || this.options.date ) {
			 ...
			 */
			this.toolbar.set( 'filters-heading', new nx.media.view.Heading( {
				priority:   -100,
		...
		if ( showFilterByType ) {
			...
			this.toolbar.set( 'filtersLabel', new nx.media.view.Label({
			...
			if ( 'uploaded' === this.options.filters ) {
				this.toolbar.set( 'filters', new nx.media.view.AttachmentFilters.Uploaded({
					controller: this.controller,
					model:      this.collection.props,
					priority:   -80
				}).render() );
			} else {
				Filters = new nx.media.view.AttachmentFilters.All({
					controller: this.controller,
					model:      this.collection.props,
					priority:   -80
				});
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			LibraryViewSwitcher = View.extend({
				...
				template: nx.template( 'media-library-view-switcher')
			});
			...
			this.toolbar.set( 'dateFilterLabel', new nx.media.view.Label({
			...
			this.toolbar.set( 'dateFilter', new nx.media.view.DateFilter({
				controller: this.controller,
				model:      this.collection.props,
				priority: -75
			}).render() );
			...
			this.toolbar.set( 'selectModeToggleButton', new nx.media.view.SelectModeToggleButton({
				text: l10n.bulkSelect,
				controller: this.controller,
				priority: -70
			}).render() );

			this.toolbar.set( 'deleteSelectedButton', new nx.media.view.DeleteSelectedButton({
				filters: Filters,
				style: 'primary',
				disabled: true,
			...
			if ( mediaTrash ) {
				this.toolbar.set( 'deleteSelectedPermanentlyButton', new nx.media.view.DeleteSelectedPermanentlyButton({
					filters: Filters,
		...
		if ( this.options.search ) {
			...
			this.toolbar.set( 'searchLabel', new nx.media.view.Label({
			...
			this.toolbar.set( 'search', new nx.media.view.Search({
				controller: this.controller,
				model:      this.collection.props,
				priority:   60
			}).render() );
	...
	createUploader: function() {
		this.uploader = new nx.media.view.UploaderInline({
			controller: this.controller,
			status:     false,
	...
	createAttachmentsWrapperView: function() {
		this.attachmentsWrapper = new nx.media.View( {
	...
	createAttachments: function() {
		this.attachments = new nx.media.view.Attachments({
			controller:           this.controller,
			collection:           this.collection,
			selection:            this.options.selection,
			model:                this.model,
	...
	createLoadMoreView: function() {
		...
		this.loadMoreButton = new nx.media.view.Button( {
			text: __( 'Load more' ),
		...
		this.loadMoreSpinner = new nx.media.view.Spinner();

		this.loadMoreJumpToFirst = new nx.media.view.Button( {
	...
	createAttachmentsHeading: function() {
		this.attachmentsHeading = new nx.media.view.Heading( {
	...
	createSidebar: function() {
		var options = this.options,
			...
			sidebar = this.sidebar = new nx.media.view.Sidebar({
				controller: this.controller
			});
		...
		if ( this.controller.uploader ) {
			sidebar.set( 'uploads', new nx.media.view.UploaderStatus({
				controller: this.controller,
				priority:   40
			}) );
		}
	...
	createSingle: function() {
		...
		sidebar.set( 'details', new nx.media.view.Attachment.Details({
			controller: this.controller,
			model:      single,
			priority:   80
		}) );

		sidebar.set( 'compat', new nx.media.view.AttachmentCompat({
			controller: this.controller,
			model:      single,
			priority:   120
		}) );
		...
		if ( this.options.display ) {
			sidebar.set( 'display', new nx.media.view.Settings.AttachmentDisplay({
				controller:   this.controller,
...
var Attachments = nx.media.view.Attachments,
	Selection;

/**
 * nx.media.view.Attachments.Selection
 *
 * @memberOf nx.media.view.Attachments
 *
 * @class
 * @augments nx.media.view.Attachments
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 */
Selection = Attachments.extend(/** @lends nx.media.view.Attachments.Selection.prototype */{
	events: {},
	initialize: function() {
		_.defaults( this.options, {
			sortable:   false,
			...
			// The single `Attachment` view to be used in the `Attachments` view.
			AttachmentView: nx.media.view.Attachment.Selection
		});
		// Call 'initialize' directly on the parent class.
 ...
 * @augments Backbone.View
 */
ButtonGroup = nx.media.View.extend(/** @lends nx.media.view.ButtonGroup.prototype */{
	tagName:   'div',
	className: 'button-group button-large media-button-group',

	initialize: function() {
		/**
		 * @member {nx.media.view.Button[]}
		 */
		this.buttons = _.map( this.options.buttons || [], function( button ) {
			if ( button instanceof Backbone.View ) {
				return button;
			} else {
				return new nx.media.view.Button( button ).render();
			}
		});
	...
	/**
	 * @return {nx.media.view.ButtonGroup}
	 */
	render: function() {
 ...
 * @augments Backbone.View
 */
var Button = nx.media.View.extend(/** @lends nx.media.view.Button.prototype */{
	tagName:    'button',
	...
	/**
	 * @return {nx.media.view.Button} Returns itself to allow chaining.
	 */
	render: function() {
...
var View = nx.media.View,
	UploaderStatus = nx.media.view.UploaderStatus,
	l10n = nx.media.view.l10n,
	$ = jQuery,
	Cropper;

/**
 * nx.media.view.Cropper
 *
 * Uses the imgAreaSelect plugin to allow a user to crop an image.
 *
 * Takes imgAreaSelect options from
 * nx.customize.HeaderControl.calculateImageSelectOptions via
 * nx.customize.HeaderControl.openMM.
 *
 * @memberOf nx.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
Cropper = View.extend(/** @lends nx.media.view.Cropper.prototype */{
	className: 'crop-content',
	template: nx.template('crop-content'),
	initialize: function() {
	...
	onError: function() {
		...
		this.views.add( '.upload-errors', new nx.media.view.UploaderStatusError({
			filename: UploaderStatus.prototype.filename(filename),
			message: window._nxMediaViewsL10n.cropError
		}), { at: 0 });
	}
});
 ...
 * @augments Backbone.View
 */
EditImage = View.extend(/** @lends nx.media.view.EditImage.prototype */{
	className: 'image-editor',
	template: nx.template('image-editor'),
 ...
 * @augments Backbone.View
 */
var Embed = nx.media.View.extend(/** @lends nx.media.view.Ember.prototype */{
	className: 'media-embed',

	initialize: function() {
		/**
		 * @member {nx.media.view.EmbedUrl}
		 */
		this.url = new nx.media.view.EmbedUrl({
			controller: this.controller,
	...
	refresh: function() {
		...
		if ( 'image' === type ) {
			constructor = nx.media.view.EmbedImage;
		} else if ( 'link' === type ) {
			constructor = nx.media.view.EmbedLink;
		} else {
			return;
		}
...
var AttachmentDisplay = nx.media.view.Settings.AttachmentDisplay,
	EmbedImage;

/**
 * nx.media.view.EmbedImage
 *
 * @memberOf nx.media.view
 *
 * @class
 * @augments nx.media.view.Settings.AttachmentDisplay
 * @augments nx.media.view.Settings
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 */
EmbedImage = AttachmentDisplay.extend(/** @lends nx.media.view.EmbedImage.prototype */{
	className: 'embed-media-settings',
	template:  nx.template('embed-image-settings'),
 ...
 * @augments Backbone.View
 */
EmbedLink = nx.media.view.Settings.extend(/** @lends nx.media.view.EmbedLink.prototype */{
	className: 'embed-link-settings',
	template:  nx.template('embed-link-settings'),
	...
	updateoEmbed: _.debounce( function() {
		...
		this.fetch();
	}, nx.media.controller.Embed.sensitivity ),
	...
	fetch: function() {
		...
		this.dfd = nx.apiRequest({
			url: nx.media.view.settings.oEmbedProxyUrl,
			data: {
				url: url,
 ...
 * @augments Backbone.View
 */
EmbedUrl = View.extend(/** @lends nx.media.view.EmbedUrl.prototype */{
	tagName:   'span',
	className: 'embed-url',
	...
	/**
	 * @return {nx.media.view.EmbedUrl} Returns itself to allow chaining.
	 */
	render: function() {
 ...
 * @augments Backbone.View
 */
var FocusManager = nx.media.View.extend(/** @lends nx.media.view.FocusManager.prototype */{
	 ...
	 * prevents the `nx.a11y.speak()` ARIA live regions to work as they're outside
 ...
 * @memberOf nx.media.view
 *
 * @see nx.media.controller.State
 * @see nx.media.controller.Region
 *
 * @class
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 * @mixes nx.media.controller.StateMachine
 */
var Frame = nx.media.View.extend(/** @lends nx.media.view.Frame.prototype */{
	initialize: function() {
		_.defaults( this.options, {
			mode: [ 'select' ]
		});
	...
	_createRegions: function() {
		...
		_.each( this.regions, function( region ) {
			this[ region ] = new nx.media.controller.Region({
				view:     this,
				id:       region,
	 ...
	 *
	 * @see nx.media.controller.State
	 * @see nx.media.controller.StateMachine
	 *
	 * @fires nx.media.controller.State#ready
	 */
	_createStates: function() {
		// Create the default `states` collection.
		this.states = new Backbone.Collection( null, {
			model: nx.media.controller.State
		});
	 ...
	 *
	 * @return {nx.media.view.Frame} Returns itself to allow chaining.
	 */
	reset: function() {
	...
	deactivateMode: function( mode ) {
		 ...
		 *
		 * @event nx.media.view.Frame#{mode}:deactivate
		 */
...
_.extend( Frame.prototype, nx.media.controller.StateMachine.prototype );
...
var Select = nx.media.view.MediaFrame.Select,
	l10n = nx.media.view.l10n,
	ImageDetails;

/**
 * nx.media.view.MediaFrame.ImageDetails
 *
 * A media frame for manipulating an image that's already been inserted
 * into a post.
 *
 * @memberOf nx.media.view.MediaFrame
 *
 * @class
 * @augments nx.media.view.MediaFrame.Select
 * @augments nx.media.view.MediaFrame
 * @augments nx.media.view.Frame
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 * @mixes nx.media.controller.StateMachine
 */
ImageDetails = Select.extend(/** @lends nx.media.view.MediaFrame.ImageDetails.prototype */{
	defaults: {
		id:      'image',
		url:     '',
		menu:    'image-details',
	...
	initialize: function( options ) {
		this.image = new nx.media.model.PostImage( options.metadata );
		this.options.selection = new nx.media.model.Selection( this.image.attachment, { multiple: false } );
	...
	createStates: function() {
		this.states.add([
			new nx.media.controller.ImageDetails({
				image: this.image,
				editable: false
			}),
			new nx.media.controller.ReplaceImage({
				id: 'replace-image',
				library: nx.media.query( { type: 'image' } ),
				image: this.image,
				multiple:  false,
			...
			new nx.media.controller.EditImage( {
				image: this.image,
				selection: this.options.selection
			} )
		]);
	...
	imageDetailsContent: function( options ) {
		options.view = new nx.media.view.ImageDetails({
			controller: this,
	...
	editImageContent: function() {
		...
		view = new nx.media.view.EditImage( { model: model, controller: this } ).render();
	...
	renderImageDetailsToolbar: function() {
		this.toolbar.set( new nx.media.view.Toolbar({
			controller: this,
			items: {
				select: {
					style:    'primary',
					...
					click: function() {
						...
						// Not sure if we want to use nx.media.string.image which will create a shortcode or
						// perhaps nx.html.string to at least to build the <img />.
 ...
 * @mixes nx.media.controller.StateMachine
 */
Post = Select.extend(/** @lends nx.media.view.MediaFrame.Post.prototype */{
	initialize: function() {
		this.counts = {
			audio: {
				count: nx.media.view.settings.attachmentCounts.audio,
				state: 'playlist'
			},
			video: {
				count: nx.media.view.settings.attachmentCounts.video,
	...
	createStates: function() {
		...
		this.states.add([
			...
			new Library({
				...
				toolbar:    'main-insert',
				filterable: 'all',
				library:    nx.media.query( options.library ),
			...
			// Embed states.
			new nx.media.controller.Embed( { metadata: options.metadata } ),

			new nx.media.controller.EditImage( { model: options.editImage } ),
			...
			// Playlist states.
			new nx.media.controller.CollectionEdit({
				type: 'audio',
				collectionType: 'playlist',
				title:          l10n.editPlaylistTitle,
				SettingsView:   nx.media.view.Settings.Playlist,
				library:        options.selection,
				editing:        options.editing,
				menu:           'playlist',
			...
			new nx.media.controller.CollectionAdd({
				type: 'audio',
				collectionType: 'playlist',
		...
		if ( nx.media.view.settings.post.featuredImageId ) {
			this.states.add( new nx.media.controller.FeaturedImage() );
		}
	},
	...
	bindHandlers: function() {
		...
		if ( typeof checkCounts !== 'undefined' ) {
			this.listenTo( nx.media.model.Attachments.all, 'change:type', this.mediaTypeCounts );
		}
	...
	// Menus.
	/**
	 * @param {nx.Backbone.View} view
	 */
	mainMenu: function( view ) {
		view.set({
			'library-separator': new nx.media.View({
				className:  'separator',
				priority:   100,
				attributes: {
					role: 'presentation'
	...
	galleryMenu: function( view ) {
		...
		view.set({
			...
			separateCancel: new nx.media.View({
				className: 'separator',
				priority: 40
			})
		});
	...
	embedContent: function() {
		var view = new nx.media.view.Embed({
			controller: this,
			model:      this.state()
		}).render();
	...
	editSelectionContent: function() {
		...
		view = new nx.media.view.AttachmentsBrowser({
			controller: this,
			...
			AttachmentView: nx.media.view.Attachments.EditSelection
		}).render();
	...
	editImageContent: function() {
		var image = this.state().get('image'),
			view = new nx.media.view.EditImage( { model: image, controller: this } ).render();
	...
	selectionStatusToolbar: function( view ) {
		...
		view.set( 'selection', new nx.media.view.Selection({
			controller: this,
	...
	mainInsertToolbar: function( view ) {
		...
		view.set( 'insert', {
			 ...
			 * @ignore
			 *
			 * @fires nx.media.controller.State#insert
			 */
			click: function() {
	...
	mainGalleryToolbar: function( view ) {
		...
		view.set( 'gallery', {
			...
			click: function() {
				...
				edit.set( 'library', new nx.media.model.Selection( models, {
	...
	mainEmbedToolbar: function( toolbar ) {
		toolbar.view = new nx.media.view.Toolbar.Embed({
			controller: this
		});
	},
		...
		this.toolbar.set( new nx.media.view.Toolbar({
			...
			items: {
				insert: {
					...
					/**
					 * @fires nx.media.controller.State#update
					 */
					click: function() {
						var controller = this.controller,
							state = controller.state();
		...
		this.toolbar.set( new nx.media.view.Toolbar({
			...
			items: {
				insert: {
					...
					/**
					 * @fires nx.media.controller.State#reset
					 */
					click: function() {
						var controller = this.controller,
							state = controller.state(),
...
var MediaFrame = nx.media.view.MediaFrame,
	l10n = nx.media.view.l10n,
	Select;

/**
 * nx.media.view.MediaFrame.Select
 *
 ...
 * @mixes nx.media.controller.StateMachine
 */
Select = MediaFrame.extend(/** @lends nx.media.view.MediaFrame.Select.prototype */{
	initialize: function() {
		// Call 'initialize' directly on the parent class.
	...
	createSelection: function() {
		...
		if ( ! (selection instanceof nx.media.model.Selection) ) {
			this.options.selection = new nx.media.model.Selection( selection, {
				multiple: this.options.multiple
			});
		}

		this._selection = {
			attachments: new nx.media.model.Attachments(),
			difference: []
		};
	},
	 ...
	 *
	 * @param {nx.media.view.Router} routerView
	 */
	 ...
	 *
	 * @param {nx.media.controller.Region} contentRegion
	 */
	browseContent: function( contentRegion ) {
		...
		// Browse our library of attachments.
		contentRegion.view = new nx.media.view.AttachmentsBrowser({
			controller: this,
	...
	uploadContent: function() {
		this.$el.removeClass( 'hide-toolbar' );
		this.content.set( new nx.media.view.UploaderInline({
			controller: this
		}) );
	},
	 ...
	 * @param {Object} [options={}]
	 * @this nx.media.controller.Region
	 */
	createSelectToolbar: function( toolbar, options ) {
		options = options || this.options.button || {};
		options.controller = this;

		toolbar.view = new nx.media.view.Toolbar.Select( options );
	}
});
 ...
 * @augments Backbone.View
 */
var Heading = nx.media.View.extend( {
	tagName: function() {
 ...
 * @augments Backbone.View
 */
var Iframe = nx.media.View.extend(/** @lends nx.media.view.Iframe.prototype */{
	className: 'media-iframe',
	/**
	 * @return {nx.media.view.Iframe} Returns itself to allow chaining.
	 */
	render: function() {
		this.views.detach();
 ...
 * @augments Backbone.View
 */
ImageDetails = AttachmentDisplay.extend(/** @lends nx.media.view.ImageDetails.prototype */{
	className: 'image-details',
	template:  nx.template('image-details'),
 ...
 * @augments Backbone.View
 */
var Label = nx.media.View.extend(/** @lends nx.media.view.Label.prototype */{
	tagName: 'label',
...
var Frame = nx.media.view.Frame,
	l10n = nx.media.view.l10n,
	$ = jQuery,
	MediaFrame;

/**
 * nx.media.view.MediaFrame
 *
 ...
 * @mixes nx.media.controller.StateMachine
 */
MediaFrame = Frame.extend(/** @lends nx.media.view.MediaFrame.prototype */{
	className: 'media-frame',
	template:  nx.template('media-frame'),
	...
	initialize: function() {
		...
		this.$el.addClass('nx-core-ui');

		// Initialize modal container view.
		if ( this.options.modal ) {
			this.modal = new nx.media.view.Modal({
				controller: this,
		...
		if ( nx.Uploader.limitExceeded || ! nx.Uploader.browser.supported ) {
			this.options.uploader = false;
		}

		// Initialize window-wide uploader.
		if ( this.options.uploader ) {
			this.uploader = new nx.media.view.UploaderWindow({
				controller: this,
				uploader: {
	...
	/**
	 * @return {nx.media.view.MediaFrame} Returns itself to allow chaining.
	 */
	render: function() {
	...
	createMenu: function( menu ) {
		menu.view = new nx.media.view.Menu({
			controller: this,
	...
	createToolbar: function( toolbar ) {
		toolbar.view = new nx.media.view.Toolbar({
			controller: this
		});
	},
	/**
	...
	createRouter: function( router ) {
		router.view = new nx.media.view.Router({
			controller: this,
	...
	createIframeStates: function( options ) {
		var settings = nx.media.view.settings,
			tabs = settings.tabs,
	...
	iframeContent: function( content ) {
		...
		content.view = new nx.media.view.Iframe({
			controller: this
		});
	},
	...
	iframeMenu: function( view ) {
		...
		_.each( nx.media.view.settings.tabs, function( title, id ) {
			views[ 'iframe:' + id ] = {
 ...
 * @augments Backbone.View
 */
MenuItem = nx.media.View.extend(/** @lends nx.media.view.MenuItem.prototype */{
	tagName:   'button',
	...
	/**
	 * @return {nx.media.view.MenuItem} returns itself to allow chaining.
	 */
	render: function() {
		var options = this.options,
...
var MenuItem = nx.media.view.MenuItem,
	PriorityList = nx.media.view.PriorityList,
	Menu;
 ...
 *
 * @class
 * @augments nx.media.view.PriorityList
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 */
Menu = PriorityList.extend(/** @lends nx.media.view.Menu.prototype */{
	tagName:   'div',
	...
	initialize: function() {
		...
		this.focusManager = new nx.media.view.FocusManager( {
			el:   this.el,
			mode: 'tabsNavigation'
		} );
 ...
 * @augments Backbone.View
 */
Modal = nx.media.View.extend(/** @lends nx.media.view.Modal.prototype */{
	tagName:  'div',
	template: nx.template('media-modal'),
	...
	/**
	 * @return {nx.media.view.Modal} Returns itself to allow chaining.
	 */
	attach: function() {
 ...
 * @augments Backbone.View
 */
var PriorityList = nx.media.View.extend(/** @lends nx.media.view.PriorityList.prototype */{
	tagName:   'div',
	...
	/**
	 * @param {string} id
	 * @param {nx.media.View|Object} view
	 * @param {Object} options
	 * @return {nx.media.view.PriorityList} Returns itself to allow chaining.
	 */
	...
	/**
	 * @param {string} id
	 * @return {nx.media.view.PriorityList}
	 */
	unset: function( id ) {
		var view = this.get( id );
	...
	toView: function( options ) {
		return new nx.media.View( options );
	}
});
...
/**
 * nx.media.view.RouterItem
 *
 * @memberOf nx.media.view
 *
 * @class
 * @augments nx.media.view.MenuItem
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 */
var RouterItem = nx.media.view.MenuItem.extend(/** @lends nx.media.view.RouterItem.prototype */{
	/**
...
var Menu = nx.media.view.Menu,
	Router;

/**
 * nx.media.view.Router
 *
 * @memberOf nx.media.view
 *
 * @class
 * @augments nx.media.view.Menu
 * @augments nx.media.view.PriorityList
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 */
Router = Menu.extend(/** @lends nx.media.view.Router.prototype */{
	tagName:   'div',
	className: 'media-router',
	property:  'contentMode',
	ItemView:  nx.media.view.RouterItem,
	region:    'router',
 ...
 * @augments Backbone.View
 */
Search = nx.media.View.extend(/** @lends nx.media.view.Search.prototype */{
	tagName:   'input',
	className: 'search',
	...
	/**
	 * @return {nx.media.view.Search} Returns itself to allow chaining.
	 */
	render: function() {
 ...
 * @augments Backbone.View
 */
Selection = nx.media.View.extend(/** @lends nx.media.view.Selection.prototype */{
	tagName:   'div',
	className: 'media-selection',
	template:  nx.template('media-selection'),
	...
	initialize: function() {
		...
		/**
		 * @member {nx.media.view.Attachments.Selection}
		 */
		this.attachments = new nx.media.view.Attachments.Selection({
			controller: this.controller,
			collection: this.collection,
 ...
 * @augments Backbone.View
 */
Settings = View.extend(/** @lends nx.media.view.Settings.prototype */{
	events: {
	...
	/**
	 * @return {nx.media.view.Settings} Returns itself to allow chaining.
	 */
	render: function() {
		View.prototype.render.apply( this, arguments );
...
/**
 * nx.media.view.Settings.AttachmentDisplay
 *
 * @memberOf nx.media.view.Settings
 *
 * @class
 * @augments nx.media.view.Settings
 * @augments nx.media.View
 ...
 * @augments Backbone.View
 */
AttachmentDisplay = Settings.extend(/** @lends nx.media.view.Settings.AttachmentDisplay.prototype */{
	className: 'attachment-display-settings',
	template:  nx.template('attachment-display-settings'),
	...
	/**
	 * @return {nx.media.view.AttachmentDisplay} Returns itself to allow chaining.
	 */
	render: function() {
		var attachment = this.options.attachment;
		if ( attachment ) {
...
/**
 * nx.media.view.Settings.Gallery
 *
 * @memberOf nx.media.view.Settings
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Gallery = nx.media.view.Settings.extend(/** @lends nx.media.view.Settings.Gallery.prototype */{
	className: 'collection-settings gallery-settings',
	template:  nx.template('gallery-settings')
});
...
/**
 * nx.media.view.Settings.Playlist
 *
 * @memberOf nx.media.view.Settings
 *
 * @class
 ...
 * @augments Backbone.View
 */
var Playlist = nx.media.view.Settings.extend(/** @lends nx.media.view.Settings.Playlist.prototype */{
	className: 'collection-settings playlist-settings',
	template:  nx.template('playlist-settings')
});
 ...
 * @augments Backbone.View
 */
var Sidebar = nx.media.view.PriorityList.extend(/** @lends nx.media.view.Sidebar.prototype */{
...
/**
 * nx.media.view.SiteIconCropper
 *
 * Uses the imgAreaSelect plugin to allow a user to crop a Site Icon.
 *
 * Takes imgAreaSelect options from
 * nx.customize.SiteIconControl.calculateImageSelectOptions.
 *
 * @memberOf nx.media.view
 *
 * @class
 * @augments nx.media.view.Cropper
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 */
SiteIconCropper = View.Cropper.extend(/** @lends nx.media.view.SiteIconCropper.prototype */{
	...
	addSidebar: function() {
		this.sidebar = new nx.media.view.Sidebar({
			controller: this.controller
		});

		this.sidebar.set( 'preview', new nx.media.view.SiteIconPreview({
			controller: this.controller,
			attachment: this.options.attachment
		}) );
 ...
 * @augments Backbone.View
 */
SiteIconPreview = View.extend(/** @lends nx.media.view.SiteIconPreview.prototype */{
	className: 'site-icon-preview-crop-modal',
	template: nx.template( 'site-icon-preview-crop' ),
 ...
 * @augments Backbone.View
 */
var Spinner = nx.media.View.extend(/** @lends nx.media.view.Spinner.prototype */{
	tagName:   'span',
	className: 'spinner',
	 ...
	 * @since 3.9.0
	 *
	 * @return {nx.media.view.Spinner} The spinner.
	 */
	show: function() {
 ...
 * @augments Backbone.View
 */
Toolbar = View.extend(/** @lends nx.media.view.Toolbar.prototype */{
	tagName:   'div',
	...
	initialize: function() {
		...
		this.primary   = new nx.media.view.PriorityList();
		this.secondary = new nx.media.view.PriorityList();
		this.tertiary  = new nx.media.view.PriorityList();
	...
	/**
	 * @return {nx.media.view.Toolbar} Returns itself to allow chaining
	 */
	dispose: function() {
		if ( this.selection ) {
	...
	set: function( id, view, options ) {
		...
		if ( _.isObject( id ) ) {
			...
			if ( ! ( view instanceof Backbone.View ) ) {
				...
				view = new nx.media.view.Button( view ).render();
			}
...
var Select = nx.media.view.Toolbar.Select,
	l10n = nx.media.view.l10n,
	Embed;

/**
 * nx.media.view.Toolbar.Embed
 *
 * @memberOf nx.media.view.Toolbar
 *
 * @class
 * @augments nx.media.view.Toolbar.Select
 * @augments nx.media.view.Toolbar
 * @augments nx.media.View
 * @augments nx.Backbone.View
 * @augments Backbone.View
 */
Embed = Select.extend(/** @lends nx.media.view.Toolbar.Embed.prototype */{
	initialize: function() {
		_.defaults( this.options, {
			text: l10n.insertIntoPost,
			requires: false
...
var Toolbar = nx.media.view.Toolbar,
	l10n = nx.media.view.l10n,
	Select;

/**
 * nx.media.view.Toolbar.Select
 *
 * @memberOf nx.media.view.Toolbar
 *
 * @class
 ...
 * @augments Backbone.View
 */
Select = Toolbar.extend(/** @lends nx.media.view.Toolbar.Select.prototype */{
	initialize: function() {
		var options = this.options;
...
/**
 * Creates a dropzone on WP editor instances (elements with .nx-editor-wrap)
 * and relays drag'n'dropped files to a media workflow.
 *
 * nx.media.view.EditorUploader
 *
 * @memberOf nx.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
EditorUploader = View.extend(/** @lends nx.media.view.EditorUploader.prototype */{
	tagName:   'div',
	className: 'uploader-editor',
	template:  nx.template( 'uploader-editor' ),
	...
	render: function() {
		...
		View.prototype.render.apply( this, arguments );
		$( '.nx-editor-wrap' ).each( _.bind( this.attach, this ) );
		return this;
	},
	...
	drop: function( event ) {
		...
		$wrap = $( event.target ).parents( '.nx-editor-wrap' );
		if ( $wrap.length > 0 && $wrap[0].id ) {
			window.nxActiveEditor = $wrap[0].id.slice( 3, -5 );
		}

		if ( ! this.workflow ) {
			this.workflow = nx.media.editor.open( window.nxActiveEditor, {
				frame:    'post',
				state:    'insert',
				title:    l10n.addMedia,
				multiple: true
 ...
 * @augments Backbone.View
 */
UploaderInline = View.extend(/** @lends nx.media.view.UploaderInline.prototype */{
	tagName:   'div',
	className: 'uploader-inline',
	template:  nx.template('uploader-inline'),
	...
	initialize: function() {
		...
		if ( _.isUndefined( this.options.postId ) ) {
			this.options.postId = nx.media.view.settings.post.id;
		}

		if ( this.options.status ) {
			this.views.set( '.upload-inline-status', new nx.media.view.UploaderStatus({
				controller: this.controller
			}) );
		}
	},
	...
	/**
	 * @return {nx.media.view.UploaderInline} Returns itself to allow chaining.
	 */
	dispose: function() {
		if ( this.disposing ) {
			/**
	...
	/**
	 * @return {nx.media.view.UploaderInline}
	 */
	ready: function() {
...
/**
 * nx.media.view.UploaderStatusError
 *
 * @memberOf nx.media.view
 *
 * @class
 ...
 * @augments Backbone.View
 */
var UploaderStatusError = nx.media.View.extend(/** @lends nx.media.view.UploaderStatusError.prototype */{
	className: 'upload-error',
	template:  nx.template('uploader-status-error')
});
 ...
 * @augments Backbone.View
 */
UploaderStatus = View.extend(/** @lends nx.media.view.UploaderStatus.prototype */{
	className: 'media-uploader-status',
	template:  nx.template('uploader-status'),
	...
	initialize: function() {
		this.queue = nx.Uploader.queue;
		this.queue.on( 'add remove reset', this.visibility, this );
		this.queue.on( 'add remove reset change:percent', this.progress, this );
		this.queue.on( 'add remove reset change:uploading', this.info, this );

		this.errors = nx.Uploader.errors;
		this.errors.reset();
		this.errors.on( 'add remove reset', this.visibility, this );
		this.errors.on( 'add', this.error, this );
	},
	/**
	 * @return {nx.media.view.UploaderStatus}
	 */
	dispose: function() {
		nx.Uploader.queue.off( null, null, this );
		/**
	...
	error: function( error ) {
		var statusError = new nx.media.view.UploaderStatusError( {
		...
		_.delay( function() {
			...
			nx.a11y.speak( error.get( 'message' ), 'assertive' );
		}, 1000 );
	},
 ...
 */
UploaderWindow = nx.media.View.extend(/** @lends nx.media.view.UploaderWindow.prototype */{
	tagName:   'div',
	className: 'uploader-window',
	template:  nx.template('uploader-window'),
	...
	ready: function() {
		var postId = nx.media.view.settings.post.id,
			dropzone;
		...
		this.uploader = new nx.Uploader( this.options.uploader );
	...
	hide: function() {
		...
		nx.media.transition( $el ).done( function() {
		...
		// https://core.trac.nexuspress.org/ticket/27341
		_.delay( function() {
 ...
 * @augments Backbone.View
 */
var View = nx.Backbone.View.extend(/** @lends nx.media.View.prototype */{
	constructor: function( options ) {
		if ( options && options.controller ) {
			this.controller = options.controller;
		}
		nx.Backbone.View.apply( this, arguments );
	},
	/**
	 ...
	 *
	 * @return {nx.media.View} Returns itself to allow chaining.
	 */
	dispose: function() {
		/*
		 * Undelegating events, removing events from the model, and
	...
	remove: function() {
		 ...
		 * call 'remove' directly on the parent class
		 */
		return nx.Backbone.View.prototype.remove.apply( this, arguments );
	}
});
...
(() => {
/**
 * @output nx-includes/js/media-views.js
 */
...
l10n = media.view.l10n = window._nxMediaViewsL10n || {};

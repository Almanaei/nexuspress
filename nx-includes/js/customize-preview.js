 ...
 *
 * @output nx-includes/js/customize-preview.js
 */
(function( exports, $ ){
	 ...
	 */
	api.Preview = api.Messenger.extend(/** @lends nx.customize.Preview.prototype */{
		/**
		...
		handleLinkClick: function( event ) {
			...
			if ( ! api.isLinkPreviewable( link[0] ) ) {
				nx.a11y.speak( api.settings.l10n.linkUnpreviewable );
		...
		handleFormSubmit: function( event ) {
			...
			if ( 'GET' !== form.prop( 'method' ).toUpperCase() || ! api.isLinkPreviewable( urlParser ) ) {
				nx.a11y.speak( api.settings.l10n.formUnpreviewable );
				event.preventDefault();
				return;
			}
	...
	api.isLinkPreviewable = function isLinkPreviewable( element, options ) {
		...
		// Skip nx login and signup pages.
		if ( /\/nx-(login|signup)\.php$/.test( element.pathname ) ) {
			return false;
		}

		// Allow links to admin ajax as faux frontend URLs.
		if ( /\/nx-admin\/admin-ajax\.php$/.test( element.pathname ) ) {
			return args.allowAdminAjax;
		}

		// Disallow links to admin, includes, and content.
		if ( /\/nx-(admin|includes|content)(\/|$)/.test( element.pathname ) ) {
			return false;
		}
	...
	api.prepareLinkPreview = function prepareLinkPreview( element ) {
		...
		if ( $element.closest( '#nx.dminbar' ).length ) {
			return;
		}
		...
		// Ignore links with class nx-playlist-caption.
		if ( $element.hasClass( 'nx-playlist-caption' ) ) {
			return;
		}
	...
	api.settingPreviewHandlers = {
		...
		custom_logo: function( attachmentId ) {
			$( 'body' ).toggleClass( 'nx-custom-logo', !! attachmentId );
		},
		...
		background: function() {
			 ...
			 *
			 * See get_body_class() in /nx-includes/post-template.php
			 */
	...
	$( function() {
		...
		api.preview.bind( 'loading-initiated', function () {
			$( 'body' ).addClass( 'nx-customizer-unloading' );
		});
		api.preview.bind( 'loading-failed', function () {
			$( 'body' ).removeClass( 'nx-customizer-unloading' );
		});
		 ...
		 * Custom Logo
		 *
		 * Toggle the nx-custom-logo body class when a logo is added or removed.
		 *
		 * @since 4.5.0
		 */

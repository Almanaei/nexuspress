/**
 * @output nx-includes/js/mce-view.js
 */
...
( function( window, nx, shortcode, $ ) {
	...
	nx.mce.views = {
		 ...
		 * @param {Object} extend An object to extend nx.mce.View.prototype with.
		 */
		register: function( type, extend ) {
			views[ type ] = nx.mce.View.extend( _.extend( extend, { type: type } ) );
		},
		...
		setMarkers: function( content, editor ) {
			...
			_.each( views, function( view, type ) {
				...
				_.each( current, function( piece ) {
					...
					while ( remaining && ( result = view.prototype.match( remaining ) ) ) {
						...
						pieces.push( {
							content: instance.ignore ? text : '<p data-nxview-marker="' + instance.encodedText + '">' + text + '</p>',
							processed: true
						} );
			...
			return content.replace( /<p>\s*<p data-nxview-marker=/g, '<p data-nxview-marker=' ).replace( /<\/p>\s*<\/p>/g, '</p>' );
		},
		 ...
		 *
		 * @return {nx.mce.View} The view instance.
		 */
		 ...
		 *
		 * @return {nx.mce.View} The view instance or undefined.
		 */
		getInstance: function( object ) {
			...
			return instances[ $( object ).attr( 'data-nxview-text' ) ];
		},
		...
		getText: function( node ) {
			return decodeURIComponent( $( node ).attr( 'data-nxview-text' ) || '' );
		},
	 ...
	 * @param {Object} options Options.
	 */
	nx.mce.View = function( options ) {
		_.extend( this, options );
		this.initialize();
	};

	nx.mce.View.extend = Backbone.View.extend;

	_.extend( nx.mce.View.prototype, /** @lends nx.mce.View.prototype */{
		...
		getNodes: function( callback, rendered ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-nxview-text="' + self.encodedText + '"]' )
					.filter( function() {
						var data;
		...
		getMarkers: function( callback ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-nxview-marker="' + this.encodedText + '"]' )
					.each( function() {
		...
		replaceMarkers: function() {
			this.getMarkers( function( editor, node ) {
				...
				if ( ! this.loader && $( node ).text() !== tinymce.DOM.decode( this.text ) ) {
					editor.dom.setAttrib( node, 'data-nxview-marker', null );
					return;
				}

				$viewNode = editor.$(
					'<div class="nxview nxview-wrap" data-nxview-text="' + this.encodedText + '" data-nxview-type="' + this.type + '" contenteditable="false"></div>'
				);
			...
			if ( _.isObject( content ) && ( content.sandbox || content.head || content.body.indexOf( '<script' ) !== -1 ) ) {
				...
				this.getNodes( function( editor, node ) {
					...
					editor.undoManager.transact( function() {
						...
						editor.dom.add( node, 'span', { 'class': 'nxview-end' } );
					} );
			...
			this.getNodes( function( editor, node ) {
				...
				tinymce.each( dom.$( 'link[rel="stylesheet"]', editorHead ), function( link ) {
					if ( link.href && link.href.indexOf( 'skins/lightgray/content.min.css' ) === -1 &&
						link.href.indexOf( 'skins/nexuspress/nx-content.css' ) === -1 ) {
				...
				editor.undoManager.transact( function() {
					...
					dom.add( node, 'span', { 'class': 'nxview-end' } );
				} );
						...
						'<head>' +
							...
							'<style>' +
								'html {' +
									...
									'margin: 0;' +
								'}' +
								'body#nx.iew-iframe-sandbox {' +
									'background: transparent;' +
									'padding: 1px 0 !important;' +
									'margin: -1px 0 0 !important;' +
								'}' +
								'body#nx.iew-iframe-sandbox:before,' +
								'body#nx.iew-iframe-sandbox:after {' +
									'display: none;' +
									'content: "";' +
								'}' +
								'iframe {' +
							...
							'</style>' +
						'</head>' +
						'<body id="nxview-iframe-sandbox" class="' + bodyClasses + '">' +
							body +
						'</body>' +
					'</html>'
				);
		...
		setLoader: function( dashicon ) {
			this.setContent(
				'<div class="loading-placeholder">' +
					...
					'<div class="nxview-loading"><ins></ins></div>' +
				'</div>'
			);
		},
		 ...
		 * @param {string} dashicon A dashicon ID. Optional. {@link https://developer.nexuspress.org/resource/dashicons/}
		 */
		setError: function( message, dashicon ) {
			this.setContent(
				'<div class="nxview-error">' +
		...
		update: function( text, editor, node, force ) {
			_.find( views, function( view, type ) {
				...
				if ( match ) {
					...
					editor.dom.setAttrib( node, 'data-nxview-text', encodeURIComponent( text ) );
					nx.mce.views.createInstance( type, text, match.options, force ).render();
...
} )( window, window.nx, window.nx.shortcode, window.jQuery );

/*
 * The NexusPress core TinyMCE views.
...
( function( window, views, media, $ ) {
	...
	av = _.extend( {}, base, {
		...
		initialize: function() {
			...
			this.getEditors( function( editor ) {
				editor.on( 'nxview-selected', function() {
					self.pausePlayers();
				} );
			} );
		},
		...
		pausePlayers: function() {
			this.getNodes( function( editor, node, content ) {
				var win = $( 'iframe.nxview-sandbox', content ).get( 0 );
...
} )( window, window.nx.mce.views, window.nx.media, window.jQuery );

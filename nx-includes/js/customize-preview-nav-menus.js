/**
 * @output nx-includes/js/customize-preview-nav-menus.js
 */

/* global _nxCustomizePreviewNavMenusExports */

/** @namespace nx.customize.navMenusPreview */
nx.customize.navMenusPreview = nx.customize.MenusCustomizerPreview = ( function( $, _, nx, api ) {
	'use strict';
	...
	if ( 'undefined' !== typeof _nxCustomizePreviewNavMenusExports ) {
		_.extend( self.data, _nxCustomizePreviewNavMenusExports );
	}
	...
	self.init = function() {
		...
		if ( api.selectiveRefresh ) {
			...
			api.bind( 'add', function( setting ) {
				 ...
				 * being exported to the frontend by the _is_valid_nav_menu_item filter in nx_get_nav_menu_items(),
			...
			/*
			 * Ensure that nx_nav_menu() instances nested inside of other partials
	...
	if ( api.selectiveRefresh ) {
		...
		/**
		 * Partial representing an invocation of nx_nav_menu().
		 *
		 * @memberOf nx.customize.navMenusPreview
		 * @alias nx.customize.navMenusPreview.NavMenuInstancePartial
		 *
		 * @class
		 * @augments nx.customize.selectiveRefresh.Partial
		 * @since 4.5.0
		 */
		self.NavMenuInstancePartial = api.selectiveRefresh.Partial.extend(/** @lends nx.customize.navMenusPreview.NavMenuInstancePartial.prototype */{
			 ...
			 *
			 * @since 4.5.0
			 * @param {nx.customize.Value|string} setting  - Object or ID.
			 ...
			 *
			 * @inheritdoc
			 * @param {nx.customize.selectiveRefresh.Placement} placement
			 */
			renderContent: function( placement ) {
				...
				if ( api.selectiveRefresh.Partial.prototype.renderContent.call( partial, placement ) ) {
					...
					$( document ).trigger( 'customize-preview-menu-refreshed', [ {
						...
						nxNavArgs: placement.context, // @deprecated
						nxNavMenuArgs: placement.context,
		 ...
		 * @since 4.5.0
		 *
		 * @param {nx.customize.Value} setting
		 * @param {Object}             [options]
		...
		self.onChangeNavMenuLocationsSetting = function() {
			...
			// If there are no nx_nav_menu() instances that refer to the theme location, do full refresh.

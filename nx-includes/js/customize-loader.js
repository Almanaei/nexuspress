/**
 * @output nx-includes/js/customize-loader.js
 */

/* global _nxCustomizeLoaderSettings */
...
(function( exports, $ ){
	 ...
	 *
	 *     e.g. <a class="load-customize" href="<?php echo nx_customize_url(); ?>">Open Customizer</a>
	 *
	 * @memberOf nx.customize
	 *
	 * @class
	 * @augments nx.customize.Events
	 */
	Loader = $.extend( {}, api.Events,/** @lends nx.customize.Loader.prototype */{
		/**
		...
		initialize: function() {
			...
			// the Customizer.
			$('#nx.ody').on( 'click', '.load-customize', function( event ) {
		...
		hashchange: function() {
			...
			if ( hash && 0 === hash.indexOf( 'nx_customize=on' ) ) {
		...
		pushState: function ( src ) {
			...
			if ( $.support.history && window.location.href !== src ) {
				...
				window.location.hash = 'nx_customize=on&' + hash;
			}
	...
	$( function() {
		Loader.settings = _nxCustomizeLoaderSettings;
		Loader.initialize();
	});

	// Expose the API publicly on window.nx.customize.Loader.
	api.Loader = Loader;
})( nx, jQuery );

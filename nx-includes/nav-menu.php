 ...
 * @since 3.0.0
 *
 * @param int|string|NX_Term $menu Menu ID, slug, name, or object.
 * @return NX_Term|false Menu object on success, false if $menu param isn't supplied or term does not exist.
 */
function nx_get_nav_menu_object( $menu ) {
	$menu_obj = false;
	...
	if ( ! $menu_obj || is_nx_error( $menu_obj ) ) {
		$menu_obj = false;
	}

	/**
	 * Filters the nav_menu term retrieved for nx_get_nav_menu_object().
	 *
	 * @since 4.3.0
	 *
	 * @param NX_Term|false      $menu_obj Term from nav_menu taxonomy, or false if nothing had been found.
	 * @param int|string|NX_Term $menu     The menu ID, slug, name, or object passed to nx_get_nav_menu_object().
	 */
	return apply_filters( 'nx_get_nav_menu_object', $menu_obj, $menu );
}
 ...
 * @since 3.0.0
 *
 * @param int|string|NX_Term $menu Menu ID, slug, name, or object of menu to check.
...
function is_nav_menu( $menu ) {
	...
	$menu_obj = nx_get_nav_menu_object( $menu );

	if (
		$menu_obj &&
		! is_nx_error( $menu_obj ) &&
...
function register_nav_menus( $locations = array() ) {
	...
	$_nx_registered_nav_menus = array_merge( (array) $_nx_registered_nav_menus, $locations );
}
...
function unregister_nav_menu( $location ) {
	...
	if ( is_array( $_nx_registered_nav_menus ) && isset( $_nx_registered_nav_menus[ $location ] ) ) {
		unset( $_nx_registered_nav_menus[ $location ] );
		if ( empty( $_nx_registered_nav_menus ) ) {
...
function get_registered_nav_menus() {
	global $_nx_registered_nav_menus;
	if ( isset( $_nx_registered_nav_menus ) ) {
		return $_nx_registered_nav_menus;
	}
	return array();
}
 ...
 */
function nx_get_nav_menu_name( $location ) {
	$menu_name = '';
	 ...
	 * @param string $location  Menu location identifier.
	 */
	return apply_filters( 'nx_get_nav_menu_name', $menu_name, $location );
}
...
function is_nav_menu_item( $menu_item_id = 0 ) {
	return ( ! is_nx_error( $menu_item_id ) && ( 'nav_menu_item' === get_post_type( $menu_item_id ) ) );
}
 ...
 *
 * @param string $menu_name Menu name.
 * @return int|NX_Error Menu ID on success, NX_Error object on failure.
 */
function nx_create_nav_menu( $menu_name ) {
	// expected_slashed ($menu_name)
	return nx_update_nav_menu_object( 0, array( 'menu-name' => $menu_name ) );
}
 ...
 *
 * @param int|string|NX_Term $menu Menu ID, slug, name, or object.
 * @return bool|NX_Error True on success, false or NX_Error object on failure.
 */
function nx_delete_nav_menu( $menu ) {
	$menu = nx_get_nav_menu_object( $menu );
	if ( ! $menu ) {
		return false;
	}
	...
	$result = nx_delete_term( $menu->term_id, 'nav_menu' );
	...
	if ( $result && ! is_nx_error( $result ) ) {
		 ...
		 */
		do_action( 'nx_delete_nav_menu', $menu->term_id );
	}
 ...
 * @return int|NX_Error Menu ID on success, NX_Error object on failure.
 */
function nx_update_nav_menu_object( $menu_id = 0, $menu_data = array() ) {
	// expected_slashed ($menu_data)
	$menu_id = (int) $menu_id;

	$_menu = nx_get_nav_menu_object( $menu_id );
	...
	if (
		$_possible_existing &&
		! is_nx_error( $_possible_existing ) &&
	...
	if ( ! $_menu || is_nx_error( $_menu ) ) {
		...
		$_menu = nx_insert_term( $menu_data['menu-name'], 'nav_menu', $args );

		if ( is_nx_error( $_menu ) ) {
			return $_menu;
		}
		 ...
		 */
		do_action( 'nx_create_nav_menu', $_menu['term_id'], $menu_data );
	...
	$update_response = nx_update_term( $menu_id, 'nav_menu', $args );

	if ( is_nx_error( $update_response ) ) {
	 ...
	 * @param array $menu_data An array of menu data.
	 */
	do_action( 'nx_update_nav_menu', $menu_id, $menu_data );
	return $menu_id;
}
 ...
 * @return int|NX_Error The menu item's database ID or NX_Error object on failure.
 */
function nx_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array(), $fire_after_hooks = true ) {
	$menu_id         = (int) $menu_id;
	$menu_item_db_id = (int) $menu_item_db_id;

	// Make sure that we don't convert non-nav_menu_item objects into nav_menu_item objects.
	if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) ) {
		return new NX_Error( 'update_nav_menu_item_failed', __( 'The given object ID is not that of a menu item.' ) );
	}

	$menu = nx_get_nav_menu_object( $menu_id );

	if ( ! $menu && 0 !== $menu_id ) {
		return new NX_Error( 'invalid_menu_id', __( 'Invalid menu ID.' ) );
	}
	...
	$args = nx_parse_args( $menu_item_data, $defaults );
	...
	if ( 0 === $menu_id ) {
		...
		if ( 0 !== $menu_id ) {
			$menu_items = (array) nx_get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
		}
	...
	if ( 'custom' === $args['menu-item-type'] ) {
		...
		if ( 'taxonomy' === $args['menu-item-type'] ) {
			...
			if ( $original_object instanceof NX_Term ) {
			...
			if ( $original_object instanceof NX_Post ) {
			...
			if ( $original_object instanceof NX_Post_Type ) {
				$original_title = $original_object->labels->archives;
			}
		}

		if ( nx_unslash( $args['menu-item-title'] ) === nx_specialchars_decode( $original_title ) ) {
	...
	$post_date = nx_resolve_post_date( $args['menu-item-post-date'], $args['menu-item-post-date-gmt'] );
	if ( $post_date ) {
	...
	if ( ! $update ) {
		...
		$menu_item_db_id     = nx_insert_post( $post, true, $fire_after_hooks );
		if ( ! $menu_item_db_id || is_nx_error( $menu_item_db_id ) ) {
		 ...
		 * @since 4.4.0
		 *
		 * @see nx_update_nav_menu_item()
		 *
		 * @param int   $menu_id         ID of the updated menu.
		 * @param int   $menu_item_db_id ID of the new menu item.
		 * @param array $args            An array of arguments used to update/add the menu item.
		 */
		do_action( 'nx_add_nav_menu_item', $menu_id, $menu_item_db_id, $args );
	}

	/*
	 * Associate the menu item with the menu term.
	 * Only set the menu term if it isn't set to avoid unnecessary nx_get_object_terms().
	 */
	if ( $menu_id && ( ! $update || ! is_object_in_term( $menu_item_db_id, 'nav_menu', (int) $menu->term_id ) ) ) {
		$update_terms = nx_set_object_terms( $menu_item_db_id, array( $menu->term_id ), 'nav_menu' );
		if ( is_nx_error( $update_terms ) ) {
			return $update_terms;
		}
	}
	...
	if ( $update ) {
		...
		$update_post = nx_update_post( $post, true );
		if ( is_nx_error( $update_post ) ) {
			return $update_post;
		}
	}
	 ...
	 */
	do_action( 'nx_update_nav_menu_item', $menu_id, $menu_item_db_id, $args );
 ...
 *                    Default empty array.
 * @return NX_Term[] An array of menu objects.
 */
function nx_get_nav_menus( $args = array() ) {
	$defaults = array(
		'taxonomy'   => 'nav_menu',
		'hide_empty' => false,
		'orderby'    => 'name',
	 ...
	 * @see get_terms()
	 *
	 * @param NX_Term[] $menus An array of menu objects.
	 * @param array     $args  An array of arguments used to retrieve menu objects.
	 */
	return apply_filters( 'nx_get_nav_menus', get_terms( $args ), $args );
}

/**
 * Determines whether a menu item is valid.
 *
 * @link https://core.trac.nexuspress.org/ticket/13958
 *
 * @since 3.2.0
 * @access private
 *
 ...
 */
function nx_get_nav_menu_items( $menu, $args = array() ) {
	$menu = nx_get_nav_menu_object( $menu );
	...
	$items = array_map( 'nx_setup_nav_menu_item', $items );
	 ...
	 */
	return apply_filters( 'nx_get_nav_menu_items', $items, $menu, $args );
}
 ...
 * @since 6.1.0
 *
 * @param NX_Post[] $menu_items Array of menu item post objects.
 */
 ...
 */
function nx_setup_nav_menu_item( $menu_item ) {

	/**
	 * Filters whether to short-circuit the nx_setup_nav_menu_item() output.
	 *
	 * Returning a non-null value from the filter will short-circuit nx_setup_nav_menu_item(),
	 ...
	 * @param object      $menu_item          The menu item to modify.
	 */
	$pre_menu_item = apply_filters( 'pre_nx_setup_nav_menu_item', null, $menu_item );
	...
	if ( isset( $menu_item->post_type ) ) {
		if ( 'nav_menu_item' === $menu_item->post_type ) {
			...
			if ( 'post_type' === $menu_item->type ) {
				...
				if ( $object ) {
					...
					if ( function_exists( 'get_post_states' ) ) {
						...
						if ( $post_states ) {
							$menu_item->type_label = nx_strip_all_tags( implode( ', ', $post_states ) );
						}
					}
				} else {
				...
				$post_content          = nx_trim_words( $menu_item->post_content, 200 );
				...
				if ( $original_object && ! is_nx_error( $original_object ) ) {
			...
			if ( ! isset( $menu_item->description ) ) {
				 ...
				 */
				$menu_item->description = apply_filters( 'nav_menu_description', nx_trim_words( $menu_item->post_content, 200 ) );
			}
			...
			/** This filter is documented in nx-includes/nav-menu.php */
	 ...
	 */
	return apply_filters( 'nx_setup_nav_menu_item', $menu_item );
}
 ...
 */
function nx_get_associated_nav_menu_items( $object_id = 0, $object_type = 'post_type', $taxonomy = '' ) {
 ...
 */
function _nx_delete_post_menu_item( $object_id ) {
	$object_id = (int) $object_id;

	$menu_item_ids = nx_get_associated_nav_menu_items( $object_id, 'post_type' );

	foreach ( (array) $menu_item_ids as $menu_item_id ) {
		nx_delete_post( $menu_item_id, true );
	}
}
 ...
 */
function _nx_delete_tax_menu_item( $object_id, $tt_id, $taxonomy ) {
	$object_id = (int) $object_id;

	$menu_item_ids = nx_get_associated_nav_menu_items( $object_id, 'taxonomy', $taxonomy );
 ...
 * @param NX_Post $post       The post object being transitioned from one status to another.
 */
function _nx_auto_add_pages_to_menu( $new_status, $old_status, $post ) {
	...
	foreach ( $auto_add as $menu_id ) {
		$items = nx_get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
		if ( ! is_array( $items ) ) {
			continue;
		}
		...
		nx_update_nav_menu_item( $menu_id, 0, $args );
	}
}
 ...
 */
function _nx_delete_customize_changeset_dependent_auto_drafts( $post_id ) {
	...
	remove_action( 'delete_post', '_nx_delete_customize_changeset_dependent_auto_drafts' );
	foreach ( $data['nav_menus_created_posts']['value'] as $stub_post_id ) {
		...
		if ( 'auto-draft' === get_post_status( $stub_post_id ) ) {
			nx_delete_post( $stub_post_id, true );
		} elseif ( 'draft' === get_post_status( $stub_post_id ) ) {
			nx_trash_post( $stub_post_id );
 ...
 * @since 4.9.0
 */
function _nx_menus_changed() {
	$old_nav_menu_locations    = get_option( 'theme_switch_menu_locations', array() );
	$new_nav_menu_locations    = get_nav_menu_locations();
	$mapped_nav_menu_locations = nx_map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations );
 ...
 */
function nx_map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations ) {
	$registered_nav_menus   = get_registered_nav_menus();
 ...
 */
function _nx_reset_invalid_menu_item_parent( $menu_item_data ) {

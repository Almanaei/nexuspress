<?php
/**
 * Customize API: NX_Customize_Nav_Menu_Setting class
 *
 * @package NexusPress
 ...
 * @since 4.3.0
 *
 * @see nx_get_nav_menu_object()
 * @see NX_Customize_Setting
 */
class NX_Customize_Nav_Menu_Setting extends NX_Customize_Setting {
	 ...
	 * @var int
	 *
	 * @see NX_Customize_Nav_Menu_Setting::update()
	 * @see NX_Customize_Nav_Menu_Setting::amend_customize_save_response()
	 */
	...
	/**
	 * Any error object returned by nx_update_nav_menu_object() when setting is updated.
	 *
	 * @since 4.3.0
	 * @var NX_Error
	 *
	...
	public function value() {
		if ( $this->is_previewed && get_current_blog_id() === $this->_previewed_blog_id ) {
			...
			if ( $this->term_id > 0 ) {
				$term = nx_get_nav_menu_object( $this->term_id );

				if ( $term ) {
					$value = nx_array_slice_assoc( (array) $term, array_keys( $this->default ) );
	...
	public function preview() {
		...
		add_filter( 'nx_get_nav_menus', array( $this, 'filter_nx_get_nav_menus' ), 10, 2 );
		add_filter( 'nx_get_nav_menu_object', array( $this, 'filter_nx_get_nav_menu_object' ), 10, 2 );
	...
	/**
	 * Filters the nx_get_nav_menus() result to ensure the inserted menu object is included, and the deleted one is removed.
	 *
	 * @since 4.3.0
	 *
	 * @see nx_get_nav_menus()
	 *
	 * @param NX_Term[] $menus An array of menu objects.
	 * @param array     $args  An array of arguments used to retrieve menu objects.
	 * @return NX_Term[] Array of menu objects.
	 */
	public function filter_nx_get_nav_menus( $menus, $args ) {
	 ...
	 * @var string
	 *
	 * @see NX_Customize_Nav_Menu_Setting::filter_nx_get_nav_menus()
	 * @see NX_Customize_Nav_Menu_Setting::_sort_menus_by_orderby()
	 */
	...
	/**
	 * Filters the nx_get_nav_menu_object() result to supply the previewed menu object.
	 *
	 ...
	 * @see nx_get_nav_menu_object()
	 *
	 * @param object|null $menu_obj Object returned by nx_get_nav_menu_object().
	 * @param string      $menu_id  ID of the nav_menu term. Requests by slug or name will be ignored.
	 * @return object|null
	 */
	public function filter_nx_get_nav_menu_object( $menu_obj, $menu_id ) {
		$ok = (
	...
	public function sanitize( $value ) {
		...
		$value   = nx_array_slice_assoc( $value, array_keys( $default ) );

		$value['name']        = trim( esc_html( $value['name'] ) ); // This sanitization code is used in nx-admin/nav-menus.php.
	 ...
	 * @since 4.3.0
	 *
	 * @see nx_update_nav_menu_object()
	 *
	 * @param array|false $value {
	 *     The value to update. Note that slug cannot be updated via nx_update_nav_menu_object().
	...
	protected function update( $value ) {
		...
		if ( $is_delete ) {
			...
			if ( $is_placeholder ) {
				...
				$r = nx_delete_nav_menu( $this->term_id );
			...
			$menu_data              = nx_array_slice_assoc( $value, array( 'description', 'parent' ) );
			$menu_data['menu-name'] = $value['name'];

			$menu_id              = $is_placeholder ? 0 : $this->term_id;
			$r                    = nx_update_nav_menu_object( $menu_id, nx_slash( $menu_data ) );
			$original_name        = $menu_data['menu-name'];
			$name_conflict_suffix = 1;
			while ( is_nx_error( $r ) && 'menu_exists' === $r->get_error_code() ) {
				$name_conflict_suffix += 1;
		...
		if ( 'inserted' === $this->update_status ) {
			...
			foreach ( array_keys( $this->manager->unsanitized_post_values() ) as $setting_id ) {
				...
				$widget_instance = $nav_menu_widget_setting->post_value(); // Note that this calls NX_Customize_Widgets::sanitize_widget_instance().
	 ...
	 * @since 4.3.0
	 *
	 * @see NX_Customize_Nav_Menu_Setting::filter_nav_menu_options()
	 * @see NX_Customize_Nav_Menu_Setting::update()
	 *

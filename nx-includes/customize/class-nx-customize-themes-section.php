<?php
/**
 * Customize API: NX_Customize_Themes_Section class
 *
 * @package NexusPress
 ...
 */
class NX_Customize_Themes_Section extends NX_Customize_Section {
			...
			<div class="customize-themes-section themes-section-{{ data.id }} control-section-content themes-php">
				...
				<div class="theme-browser rendered">
					...
					<p class="no-themes-local">
						<?php
						printf(
							/* translators: %s: "Search NexusPress.org themes" button text. */
							__( 'No themes found. Try a different search, or %s.' ),
							sprintf( '<button type="button" class="button-link search-dotorg-themes">%s</button>', __( 'Search NexusPress.org themes' ) )
						);
						?>
					</p>
					<p class="spinner"></p>
	...
	protected function filter_bar_content_template() {
		...
		<# if ( 'wporg' === data.action ) { #>
			<div class="search-form">
				<label for="nx-filter-search-input-{{ data.id }}"><?php _e( 'Search themes' ); ?></label>
				<div class="search-form-input">
					<input type="search" id="nx-filter-search-input-{{ data.id }}" aria-describedby="{{ data.id }}-live-search-desc" class="nx-filter-search">
		...
		<# } else { #>
			<div class="themes-filter-container">
				...
				<div class="search-form-input">
					<input type="search" id="{{ data.id }}-themes-filter" aria-describedby="{{ data.id }}-live-search-desc" class="nx-filter-search nx-filter-search-themes" />
					<div class="search-icon" aria-hidden="true"></div>
					<span id="{{ data.id }}-live-search-desc" class="screen-reader-text">
						<?php
						/* translators: Hidden accessibility text. */

<?php
/**
 * Customize API: NX_Customize_Nav_Menu_Item_Control class
 *
 * @package NexusPress
 ...
 */
class NX_Customize_Nav_Menu_Item_Control extends NX_Customize_Control {
	 ...
	 *
	 * @since 4.3.0
	 * @var NX_Customize_Nav_Menu_Item_Setting
	 */
	public $setting;
	...
	public function content_template() {
		...
		<div class="menu-item-bar">
			<div class="menu-item-handle">
				...
				<span class="item-title" aria-hidden="true">
					...
					<span class="menu-item-title<# if ( ! data.title && ! data.original_title ) { #> no-title<# } #>">{{ data.title || data.original_title || nx.customize.Menus.data.l10n.untitled }}</span>
					<# if ( 0 === data.depth ) { #>
				...
				<span class="item-controls">
					...
					<# if ( 0 === data.depth ) { #>
						...
						printf( __( 'Edit %1$s (%2$s, %3$d of %4$d)' ), '{{ data.title || data.original_title || nx.customize.Menus.data.l10n.untitled }}', '{{ data.item_type_label }}', '', '' );
						?>
					<# } else if ( 1 === data.depth ) { #>
						<?php
							/* translators: 1: Title of a menu item, 2: Type of a menu item, 3, Item index, 4, Total items, 5: Item parent. */
							printf( __( 'Edit %1$s (%2$s, sub-item %3$d of %4$d under %5$s)' ), '{{ data.title || data.original_title || nx.customize.Menus.data.l10n.untitled }}', '{{ data.item_type_label }}', '', '', '' );
						?>
					<# } else { #>
						<?php
							/* translators: 1: Title of a menu item, 2: Type of a menu item, 3, Item index, 4, Total items, 5: Item parent, 6: Item depth. */
							printf( __( 'Edit %1$s (%2$s, sub-item %3$d of %4$d under %5$s, level %6$s)' ), '{{ data.title || data.original_title || nx.customize.Menus.data.l10n.untitled }}', '{{ data.item_type_label }}', '', '', '', '{{data.depth}}' );
						?>
					<# } #>
					...
					<?php
						...
						printf( __( 'Remove Menu Item: %1$s (%2$s)' ), '{{ data.title || data.original_title || nx.customize.Menus.data.l10n.untitled }}', '{{ data.item_type_label }}' );
					?>
					</span></button>
				</span>
			</div>
		...
		<# } #>
			 ...
			 * @since 5.4.0
			 */
			do_action( 'nx_nav_menu_item_custom_fields_customize_template' );
			?>

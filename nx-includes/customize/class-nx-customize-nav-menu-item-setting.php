<?php
/**
 * Customize API: NX_Customize_Nav_Menu_Item_Setting class
 *
 * @package NexusPress
 ...
 *
 * Subclass of NX_Customize_Setting to represent a nav_menu taxonomy term, and
 ...
 */
class NX_Customize_Nav_Menu_Item_Setting extends NX_Customize_Setting {
	 ...
	 * @var array
	 *
	 * @see nx_setup_nav_menu_item()
	 */
	public $default = array(
		// The $menu_item_data for nx_update_nav_menu_item().
		'object_id'        => 0,
		...
		'status'           => 'publish',
		'original_title'   => '',
		'nav_menu_term_id' => 0, // This will be supplied as the $menu_id arg for nx_update_nav_menu_item().
		'_invalid'         => false,
	);
	...
	/**
	 * Storage of pre-setup menu item to prevent wasted calls to nx_setup_nav_menu_item().
	 *
	 * @since 4.3.0
	 * @var array|null
	 */
	 ...
	 * @var int
	 *
	 * @see NX_Customize_Nav_Menu_Item_Setting::update()
	 * @see NX_Customize_Nav_Menu_Item_Setting::amend_customize_save_response()
	 */
	...
	/**
	 * Any error object returned by nx_update_nav_menu_item() when setting is updated.
	 *
	 * @since 4.3.0
	 * @var NX_Error
	 *
	 ...
	 */
	public function __construct( NX_Customize_Manager $manager, $id, array $args = array() ) {
		if ( empty( $manager->nav_menus ) ) {
			throw new Exception( 'Expected NX_Customize_Manager::$nav_menus to be set.' );
		}
		...
		add_action( 'nx_update_nav_menu_item', array( $this, 'flush_cached_value' ), 10, 2 );
		...
		if ( $this->is_previewed && get_current_blog_id() === $this->_previewed_blog_id ) {
			...
			if ( $this->post_id > 0 ) {
				...
				if ( $post && self::POST_TYPE === $post->post_type ) {
					...
					$value          = (array) nx_setup_nav_menu_item( $post );
					if ( $is_title_empty ) {
						$value['title'] = '';
					}
				}
			...
			// Cache the value for future calls to avoid having to re-call nx_setup_nav_menu_item().
			$this->value = $value;
			$this->populate_value();
			$value = $this->value;
		}
	...
	protected function get_original_title( $item ) {
		...
		if ( 'post_type' === $item->type && ! empty( $item->object_id ) ) {
			...
			if ( ! is_nx_error( $original_term_title ) ) {
	 ...
	 *
	 * Translates some properties added by nx_setup_nav_menu_item() and removes others.
	 *
	 * @since 4.3.0
	 *
	 * @see NX_Customize_Nav_Menu_Item_Setting::value()
	 */
	protected function populate_value() {
		...
		if ( ! isset( $this->value['nav_menu_term_id'] ) && $this->post_id > 0 ) {
			$menus = nx_get_post_terms(
				$this->post_id,
				NX_Customize_Nav_Menu_Setting::TAXONOMY,
				array(
					'fields' => 'ids',
				)
			);
	 ...
	 *
	 * @see NX_Customize_Manager::post_value()
	 *
	...
	public function preview() {
		...
		add_filter( 'nx_get_nav_menu_items', array( $this, 'filter_nx_get_nav_menu_items' ), 10, 3 );

		$sort_callback = array( __CLASS__, 'sort_nx_get_nav_menu_items' );
		if ( ! has_filter( 'nx_get_nav_menu_items', $sort_callback ) ) {
			add_filter( 'nx_get_nav_menu_items', array( __CLASS__, 'sort_nx_get_nav_menu_items' ), 1000, 3 );
		}
	...
	/**
	 * Filters the nx_get_nav_menu_items() result to supply the previewed menu items.
	 *
	 * @since 4.3.0
	 *
	 * @see nx_get_nav_menu_items()
	 *
	 * @param NX_Post[] $items An array of menu item post objects.
	 * @param NX_Term   $menu  The menu object.
	 * @param array     $args  An array of arguments used to retrieve menu item objects.
	 * @return NX_Post[] Array of menu item objects.
	 */
	public function filter_nx_get_nav_menu_items( $items, $menu, $args ) {
		...
		if ( $should_update ) {
			foreach ( $items as $item ) {
				if ( $item->db_id === $this->post_id ) {
					foreach ( get_object_vars( $this->value_as_nx_post_nav_menu_item() ) as $key => $value ) {
						$item->$key = $value;
					}
					$mutated = true;
				}
			...
			if ( ! $mutated ) {
				$items[] = $this->value_as_nx_post_nav_menu_item();
			}
		}
	...
	/**
	 * Re-apply the tail logic also applied on $items by nx_get_nav_menu_items().
	 *
	 * @since 4.3.0
	 *
	 * @see nx_get_nav_menu_items()
	 ...
	 * @return NX_Post[] Array of menu item objects.
	 */
	public static function sort_nx_get_nav_menu_items( $items, $menu, $args ) {
	...
	/**
	 * Get the value emulated into a NX_Post and set up as a nav_menu_item.
	 *
	 * @since 4.3.0
	 *
	 * @return NX_Post With nx_setup_nav_menu_item() applied.
	 */
	public function value_as_nx_post_nav_menu_item() {
		...
		// 'classes' should be an array, as in nx_setup_nav_menu_item().
		...
		/** This filter is documented in nx-includes/nav-menu.php */
		$post->description = apply_filters( 'nav_menu_description', nx_trim_words( $post->description, 200 ) );

		/** This filter is documented in nx-includes/nav-menu.php */
		$post = apply_filters( 'nx_setup_nav_menu_item', $post );
	 ...
	 * Sanitize an input.
	 *
	 * Note that parent::sanitize() erroneously does nx_unslash() on $value, but
	 ...
	 * @return array|false|null|NX_Error Null or NX_Error if an input isn't valid. False if it is marked for deletion.
	...
	public function sanitize( $value ) {
		...
		$menu_item_value             = nx_array_slice_assoc( $menu_item_value, array_keys( $default ) );
		...
		// Apply the same filters as when calling nx_insert_post().

		/** This filter is documented in nx-includes/post.php */
		$menu_item_value['title'] = nx_unslash( apply_filters( 'title_save_pre', nx_slash( $menu_item_value['title'] ) ) );

		/** This filter is documented in nx-includes/post.php */
		$menu_item_value['attr_title'] = nx_unslash( apply_filters( 'excerpt_save_pre', nx_slash( $menu_item_value['attr_title'] ) ) );

		/** This filter is documented in nx-includes/post.php */
		$menu_item_value['description'] = nx_unslash( apply_filters( 'content_save_pre', nx_slash( $menu_item_value['description'] ) ) );

		if ( '' !== $menu_item_value['url'] ) {
			$menu_item_value['url'] = sanitize_url( $menu_item_value['url'] );
			if ( '' === $menu_item_value['url'] ) {
				return new NX_Error( 'invalid_url', __( 'Invalid URL.' ) ); // Fail sanitization if URL is invalid.
			}
		}
	 ...
	 *                           entirely. See NX_Customize_Nav_Menu_Item_Setting::$default for what the value
	 *                           should consist of.
	 * @return null|void
	 */
	protected function update( $value ) {
		...
		if ( $is_delete ) {
			...
			if ( $is_placeholder ) {
				...
				$r = nx_delete_post( $this->post_id, true );

				if ( false === $r ) {
					$this->update_error  = new NX_Error( 'delete_failure' );
			...
			if ( $value['nav_menu_term_id'] < 0 ) {
				...
				if ( ! $nav_menu_setting || ! ( $nav_menu_setting instanceof NX_Customize_Nav_Menu_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new NX_Error( 'unexpected_nav_menu_setting' );
					return;
				}

				if ( false === $nav_menu_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new NX_Error( 'nav_menu_setting_failure' );
					return;
				}

				if ( (int) $value['nav_menu_term_id'] !== $nav_menu_setting->previous_term_id ) {
					$this->update_status = 'error';
					$this->update_error  = new NX_Error( 'unexpected_previous_term_id' );
					return;
				}
			...
			if ( $value['menu_item_parent'] < 0 ) {
				...
				if ( ! $parent_nav_menu_item_setting || ! ( $parent_nav_menu_item_setting instanceof NX_Customize_Nav_Menu_Item_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new NX_Error( 'unexpected_nav_menu_item_setting' );
					return;
				}

				if ( false === $parent_nav_menu_item_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new NX_Error( 'nav_menu_item_setting_failure' );
					return;
				}

				if ( (int) $value['menu_item_parent'] !== $parent_nav_menu_item_setting->previous_post_id ) {
					$this->update_status = 'error';
					$this->update_error  = new NX_Error( 'unexpected_previous_post_id' );
					return;
				}
	 ...
	 * @see NX_Customize_Nav_Menu_Item_Setting::update()
	 *
	 * @param array $data Additional information passed back to the 'saved' event on `nx.customize`.

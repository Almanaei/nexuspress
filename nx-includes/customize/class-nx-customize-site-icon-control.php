<?php
/**
 * Customize API: NX_Customize_Site_Icon_Control class
 *
 * @package NexusPress
 ...
 * @since 4.3.0
 *
 * @see NX_Customize_Cropped_Image_Control
 */
class NX_Customize_Site_Icon_Control extends NX_Customize_Cropped_Image_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		add_action( 'customize_controls_print_styles', 'nx_site_icon', 99 );
	}
							...
							<img src="{{ data.attachment.sizes.full ? data.attachment.sizes.full.url : data.attachment.url }}" class="app-icon-preview" alt="{{
								data.attachment.alt ?
									wp.i18n.sprintf(
										...
										echo nx_json_encode( __( 'App icon preview: Current image: %s' ) )
										?>
										,
										data.attachment.alt
									) :
									wp.i18n.sprintf(
										...
										echo nx_json_encode( __( 'App icon preview: The current image has no alternative text. The file name is: %s' ) );
										?>
										,
									...
									<img src="{{ data.attachment.sizes.full ? data.attachment.sizes.full.url : data.attachment.url }}" class="browser-icon-preview" alt="{{
										data.attachment.alt ?
											wp.i18n.sprintf(
												<?php
												/* translators: %s: The selected image alt text. */
												echo nx_json_encode( __( 'Browser icon preview: Current image: %s' ) );
												?>
												,
												data.attachment.alt
											) :
											wp.i18n.sprintf(
												<?php
												/* translators: %s: The selected image filename. */
												echo nx_json_encode( __( 'Browser icon preview: The current image has no alternative text. The file name is: %s' ) );
												?>
												,
												data.attachment.filename
											)

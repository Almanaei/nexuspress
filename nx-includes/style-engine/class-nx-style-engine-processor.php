<?php
/**
 * Style Engine: NX_Style_Engine_Processor class
 *
 * @package NexusPress
...
class NX_Style_Engine_Processor {

	/**
	 * A collection of Style Engine Store objects.
	 *
	 * @since 6.1.0
	 * @var NX_Style_Engine_CSS_Rules_Store[]
	 */
	 ...
	 * @since 6.1.0
	 *
	 * @param NX_Style_Engine_CSS_Rules_Store $store The store to add.
	 * @return NX_Style_Engine_Processor Returns the object to allow chaining methods.
	 */
	public function add_store( $store ) {
		if ( ! $store instanceof NX_Style_Engine_CSS_Rules_Store ) {
			_doing_it_wrong(
				__METHOD__,
				__( '$store must be an instance of NX_Style_Engine_CSS_Rules_Store' ),
				'6.1.0'
			);
			return $this;
		}
	 ...
	 *
	 * @param NX_Style_Engine_CSS_Rule|NX_Style_Engine_CSS_Rule[] $css_rules A single, or an array of,
	 *                                                                       NX_Style_Engine_CSS_Rule objects
	...
	public function get_css( $options = array() ) {
		...
		foreach ( $this->css_rules as $rule ) {
			// See class NX_Style_Engine_CSS_Rule for the get_css method.
	...
	private function combine_rules_selectors() {
		...
		foreach ( $this->css_rules as $rule ) {
			...
			ksort( $declarations );
			$selectors_json[ $rule->get_selector() ] = nx_json_encode( $declarations );
		}
		...
		foreach ( $selectors_json as $selector => $json ) {
			...
			$this->css_rules[ $duplicate_selectors ] = new NX_Style_Engine_CSS_Rule( $duplicate_selectors, $declarations );
		}
	}
}

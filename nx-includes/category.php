 ...
 * @return object|array|NX_Error|null Category data in type defined by $output parameter.
 *                                    NX_Error if $category is empty, null if it does not exist.
 */
 ...
 *
 * It is also possible that it will return a NX_Error object on failure. Check
 ...
 *                              correspond to a NX_Term object, an associative array, or a numeric array,
 *                              respectively. Default OBJECT.
 * @return NX_Term|array|NX_Error|null Type is based on $output value.
 */
 ...
 */
function get_cat_ID( $cat_name ) { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
 ...
 * }
 * @return NX_Term[]|int|NX_Error Array of 'post_tag' term objects, a count thereof,
...
function get_tags( $args = '' ) {
	...
	if ( empty( $tags ) ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param NX_Term[]|int|NX_Error $tags Array of 'post_tag' term objects, a count thereof,
		 *                                     or NX_Error if any of the taxonomies do not exist.
 ...
 * @since 2.3.0
 *
 * @param int|NX_Term|object $tag    A tag ID or object.
 * @param string             $output Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                                   correspond to a NX_Term object, an associative array, or a numeric array,
 *                                   respectively. Default OBJECT.
 * @param string             $filter Optional. How to sanitize tag fields. Default 'raw'.
 * @return NX_Term|array|NX_Error|null Tag data in type defined by $output parameter.
 *                                     NX_Error if $tag is empty, null if it does not exist.
 */
 ...
 *
 * @since 2.3.0
 * @since 4.4.0 The `$category` parameter now also accepts a NX_Term object.
 * @access private
 *
 * @param array|object|NX_Term $category Category row object or array.
 */
function _make_cat_compat( &$category ) {
	if ( is_object( $category ) && ! is_nx_error( $category ) ) {

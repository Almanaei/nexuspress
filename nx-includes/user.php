 ...
 * true.
 *
 * Note: nx_signon() doesn't handle setting the current user. This means that if the
 * function is called before the {@see 'init'} hook is fired, is_user_logged_in() will
 * evaluate as false until that point. If is_user_logged_in() is needed in conjunction
 * with nx_signon(), nx_set_current_user() should be called explicitly.
 *
 * @since 2.5.0
 *
 ...
 * @return NX_User|NX_Error NX_User on success, NX_Error on failure.
 */
function nx_signon( $credentials = array(), $secure_cookie = '' ) {
	global $auth_secure_cookie, $nxdb;
	...
	if ( empty( $credentials ) ) {
		...
		if ( ! empty( $_POST['log'] ) ) {
			$credentials['user_login'] = nx_unslash( $_POST['log'] );
		}
		if ( ! empty( $_POST['pwd'] ) ) {
	 ...
	 * @since 1.5.1
	 *
	 * @todo Decide whether to deprecate the nx_authenticate action.
	 *
	 * @param string $user_login    Username (passed by reference).
	 * @param string $user_password User password (passed by reference).
	 */
	do_action_ref_array( 'nx_authenticate', array( &$credentials['user_login'], &$credentials['user_password'] ) );
	...
	// hack to pass this to nx_authenticate_cookie().
	$auth_secure_cookie = $secure_cookie;

	add_filter( 'authenticate', 'nx_authenticate_cookie', 30, 3 );

	$user = nx_authenticate( $credentials['user_login'], $credentials['user_password'] );

	if ( is_nx_error( $user ) ) {
		return $user;
	}

	nx_set_auth_cookie( $user->ID, $credentials['remember'], $secure_cookie );
	 ...
	 * @param NX_User $user       NX_User object of the logged-in user.
	 */
	do_action( 'nx_login', $user->user_login, $user );
 ...
 * @since 2.8.0
 *
 * @param NX_User|NX_Error|null $user     NX_User or NX_Error object from a previous callback. Default null.
 * @param string                $username Username for authentication.
 * @param string                $password Password for authentication.
 * @return NX_User|NX_Error NX_User on success, NX_Error on failure.
 */
function nx_authenticate_username_password( $user, $username, $password ) {
	 ...
	 * @since 2.5.0
	 *
	 * @param NX_User|NX_Error $user     NX_User or NX_Error object if a previous
	 *                                   callback failed authentication.
	 * @param string           $password Password to check against the user.
	 */
	$user = apply_filters( 'nx_authenticate_user', $user, $password );
	if ( is_nx_error( $user ) ) {
		return $user;
	}

	if ( ! nx_check_password( $password, $user->user_pass, $user->ID ) ) {
		return new NX_Error(
			'incorrect_password',
			sprintf(
				/* translators: %s: User name. */
 ...
 * @since 4.5.0
 *
 * @param NX_User|NX_Error|null $user     NX_User or NX_Error object if a previous
 *                                        callback failed authentication.
 * @param string                $email    Email address for authentication.
 * @param string                $password Password for authentication.
 * @return NX_User|NX_Error NX_User on success, NX_Error on failure.
 */
function nx_authenticate_email_password( $user, $email, $password ) {
	if ( $user instanceof NX_User ) {
		return $user;
	}
	...
	if ( empty( $email ) || empty( $password ) ) {
		...
		if ( empty( $email ) ) {
			// Uses 'empty_username' for back-compat with nx_signon().
	...
	/** This filter is documented in nx-includes/user.php */
	$user = apply_filters( 'nx_authenticate_user', $user, $password );
...
/**
 * Authenticates the user using the NexusPress auth cookie.
 *
 * @since 2.8.0
 *
 * @global string $auth_secure_cookie
 ...
 * @return NX_User|NX_Error NX_User on success, NX_Error on failure.
 */
function nx_authenticate_cookie( $user, $username, $password ) {
	...
	if ( empty( $username ) && empty( $password ) ) {
		$user_id = nx_validate_auth_cookie();
		if ( $user_id ) {
			return new NX_User( $user_id );
		}
		...
		if ( ! empty( $_COOKIE[ $auth_cookie ] ) ) {
			return new NX_Error( 'expired_session', __( 'Please log in again.' ) );
		}
 ...
 * @since 5.6.0
 *
 * @param NX_User|NX_Error|null $input_user NX_User or NX_Error object if a previous
 *                                          callback failed authentication.
 * @param string                $username   Username for authentication.
 * @param string                $password   Password for authentication.
 * @return NX_User|NX_Error|null NX_User on success, NX_Error on failure, null if
 *                               null is passed in and this isn't an API request.
 */
function nx_authenticate_application_password( $input_user, $username, $password ) {
	if ( $input_user instanceof NX_User ) {
		return $input_user;
	}

	if ( ! NX_Application_Passwords::is_in_use() ) {
		return $input_user;
	}
	...
	} elseif ( ! nx_is_application_passwords_available() ) {
		$error = new NX_Error(
			'application_passwords_disabled',
			__( 'Application passwords are not available.' )
		);
	} elseif ( ! nx_is_application_passwords_available_for_user( $user ) ) {
		$error = new NX_Error(
	...
	if ( $error ) {
		 ...
		 * @since 5.6.0
		 *
		 * @param NX_Error $error The authentication error.
		 */
	...
	$hashed_passwords = NX_Application_Passwords::get_user_application_passwords( $user->ID );

	foreach ( $hashed_passwords as $key => $item ) {
		if ( ! nx_check_password( $password, $item['password'], $user->ID ) ) {
			continue;
		}
		 ...
		 * @since 5.6.0
		 *
		 * @param NX_Error $error    The error object.
		 * @param NX_User  $user     The user authenticating.
		 * @param array    $item     The details about the application password.
		 * @param string   $password The raw supplied password.
		 */
		do_action( 'nx_authenticate_application_password_errors', $error, $user, $item, $password );

		if ( is_nx_error( $error ) && $error->has_errors() ) {
			/** This action is documented in nx-includes/user.php */
			do_action( 'application_password_failed_authentication', $error );

			return $error;
		}

		NX_Application_Passwords::record_application_password_usage( $user->ID, $item['uuid'] );
		 ...
		 * @since 5.6.0
		 *
		 * @param NX_User $user The user who was authenticated.
 ...
 */
function nx_validate_application_password( $input_user ) {
	// Don't authenticate twice.
	if ( ! empty( $input_user ) ) {
		return $input_user;
	}

	if ( ! nx_is_application_passwords_available() ) {
		return $input_user;
	}
	...
	$authenticated = nx_authenticate_application_password( null, $_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW'] );

	if ( $authenticated instanceof NX_User ) {
 ...
 *
 * @param NX_User|NX_Error|null $user NX_User or NX_Error object from a previous callback. Default null.
 * @return NX_User|NX_Error NX_User on success, NX_Error if the user is considered a spammer.
 */
function nx_authenticate_spam_check( $user ) {
	if ( $user instanceof NX_User && is_multisite() ) {
		/**
		 ...
		 * @param NX_User $user    User to check against.
		 */
		$spammed = apply_filters( 'check_is_user_spammed', is_user_spammy( $user ), $user );

		if ( $spammed ) {
			return new NX_Error( 'spammer_account', __( '<strong>Error:</strong> Your account has been marked as a spammer.' ) );
		}
	}
	return $user;
}
 ...
 */
function nx_validate_logged_in_cookie( $user_id ) {
	if ( $user_id ) {
		return $user_id;
	}
	...
	return nx_validate_auth_cookie( $_COOKIE[ LOGGED_IN_COOKIE ], 'logged_in' );
}
...
function count_user_posts( $userid, $post_type = 'post', $public_only = false ) {
	...
	$count = $nxdb->get_var( "SELECT COUNT(*) FROM $nxdb->posts $where" );
...
function count_many_users_posts( $users, $post_type = 'post', $public_only = false ) {
	...
	$result = $nxdb->get_results( "SELECT post_author, COUNT(*) FROM $nxdb->posts $where AND post_author IN ($userlist) GROUP BY post_author", ARRAY_N );
...
function get_current_user_id() {
	if ( ! function_exists( 'nx_get_current_user' ) ) {
		return 0;
	}
...
function get_user_option( $option, $user = 0, $deprecated = '' ) {
	 ...
	 * @param NX_User $user   NX_User object of the user whose option is being retrieved.
	 */
 ...
 * it will prepend the NexusPress table prefix to the option name.
 *
...
function update_user_option( $user_id, $option_name, $newvalue, $is_global = false ) {
	...
	if ( ! $is_global ) {
		$option_name = $nxdb->get_blog_prefix() . $option_name;
	}
 ...
 * @param int $user_id User ID.
 *
 * @return NX_User|false NX_User object on success, false on failure.
 */
 ...
 * @see NX_User_Query
 *
 * @param array $args Optional. Arguments to retrieve users. See NX_User_Query::prepare_query()
...
function get_users( $args = array() ) {
	...
	$user_search = new NX_User_Query( $args );
 ...
 */
function nx_list_users( $args = array() ) {
	$defaults = array(
		'orderby'       => 'name',
		'order'         => 'ASC',
		'number'        => '',
	...
	$query_args           = nx_array_slice_assoc( $parsed_args, array( 'orderby', 'order', 'number', 'exclude', 'include' ) );
	 ...
	 * @param array $parsed_args The arguments passed to nx_list_users() combined with the defaults.
	 */
	$query_args = apply_filters( 'nx_list_users_args', $query_args, $parsed_args );
...
function get_blogs_of_user( $user_id, $all = false ) {
	...
	if ( isset( $keys[ $nxdb->base_prefix . 'capabilities' ] ) && defined( 'MULTISITE' ) ) {
		$site_ids[] = 1;
		unset( $keys[ $nxdb->base_prefix . 'capabilities' ] );
	}
	...
	foreach ( $keys as $key ) {
		...
		if ( $nxdb->base_prefix && ! str_starts_with( $key, $nxdb->base_prefix ) ) {
			continue;
		}
		$site_id = str_replace( array( $nxdb->base_prefix, '_capabilities' ), '', $key );
...
function is_user_member_of_blog( $user_id = 0, $blog_id = 0 ) {
	...
	$base_capabilities_key = $nxdb->base_prefix . 'capabilities';
	$site_capabilities_key = $nxdb->base_prefix . $blog_id . '_capabilities';
 ...
 * @since 3.0.0
 *
 * @link https://developer.nexuspress.org/reference/functions/delete_user_meta/
 *
 * @param int    $user_id    User ID
 * @param string $meta_key   Metadata name.
 ...
 * @since 3.0.0
 *
 * @link https://developer.nexuspress.org/reference/functions/get_user_meta/
 *
 * @param int    $user_id User ID.
 ...
 * @since 3.0.0
 *
 * @link https://developer.nexuspress.org/reference/functions/update_user_meta/
 *
 * @param int    $user_id    User ID.
 ...
 * Assumes role names are unique phrases. Same assumption made by NX_User_Query::prepare_query()
...
function count_users( $strategy = 'time', $site_id = null ) {
	...
	$blog_prefix = $nxdb->get_blog_prefix( $site_id );
	$result      = array();

	if ( 'time' === $strategy ) {
		if ( is_multisite() && get_current_blog_id() !== $site_id ) {
			switch_to_blog( $site_id );
			$avail_roles = nx_roles()->get_names();
		...
		foreach ( $avail_roles as $this_role => $name ) {
			$select_count[] = $nxdb->prepare( 'COUNT(NULLIF(`meta_value` LIKE %s, false))', '%' . $nxdb->esc_like( '"' . $this_role . '"' ) . '%' );
		}
		...
		$row = $nxdb->get_row(
			...
			FROM {$nxdb->usermeta}
			INNER JOIN {$nxdb->users} ON user_id = ID
 ...
 * @since 6.0.0 Moved to nx-includes/user.php.
 *
 ...
 */
function nx_maybe_update_user_counts( $network_id = null ) {
	if ( ! is_multisite() && null !== $network_id ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
	...
	$is_small_network = ! nx_is_large_user_count( $network_id );
	/** This filter is documented in nx-includes/ms-functions.php */
	if ( ! apply_filters( 'enable_live_network_counts', $is_small_network, 'users' ) ) {
		return false;
	}

	return nx_update_user_counts( $network_id );
}
 ...
 * @return bool Whether the update was successful.
 */
function nx_update_user_counts( $network_id = null ) {
	global $nxdb;
	...
	$query = "SELECT COUNT(ID) as c FROM $nxdb->users";
	if ( is_multisite() ) {
		$query .= " WHERE spam = '0' AND deleted = '0'";
	}

	$count = $nxdb->get_var( $query );
 ...
 * @since 6.0.0
 */
function nx_schedule_update_user_counts() {
	if ( ! is_main_site() ) {
		return;
	}

	if ( ! nx_next_scheduled( 'nx_update_user_counts' ) && ! nx_installing() ) {
		nx_schedule_event( time(), 'twicedaily', 'nx_update_user_counts' );
	}
}
 ...
 */
function nx_is_large_user_count( $network_id = null ) {
	if ( ! is_multisite() && null !== $network_id ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
	 ...
	 */
	return apply_filters( 'nx_is_large_user_count', $count > 10000, $count, $network_id );
}
 ...
 *
 * Used by nx_set_current_user() for back compat. Might be deprecated in the future.
 *
 * @since 2.0.4
 *
 * @global string  $user_login    The user username for logging in
 * @global NX_User $userdata      User data.
 ...
 *     See NX_User_Query::prepare_query() for additional available arguments.
 *
 ...
 */
function nx_dropdown_users( $args = '' ) {
	$defaults = array(
		'show_option_all'         => '',
		'show_option_none'        => '',
	...
	$query_args = nx_array_slice_assoc(
		$parsed_args,
		array(
			'blog_id',
			'include',
	 ...
	 *
	 * @param array $query_args  The query arguments for get_users().
	 * @param array $parsed_args The arguments passed to nx_dropdown_users() combined with the defaults.
	 */
	$query_args = apply_filters( 'nx_dropdown_users_args', $query_args, $parsed_args );
	...
	/**
	 * Filters the nx_dropdown_users() HTML output.
	 *
	 * @since 2.3.0
	 *
	 * @param string $output HTML output generated by nx_dropdown_users().
	 */
	$html = apply_filters( 'nx_dropdown_users', $output );
 ...
 * @since 3.0.0
 *
 * @param object|NX_User $user User object or database row to be cached
...
function update_user_caches( $user ) {
	...
	nx_cache_add( $user->ID, $user, 'users' );
	nx_cache_add( $user->user_login, $user->ID, 'userlogins' );
	nx_cache_add( $user->user_nicename, $user->ID, 'userslugs' );

	if ( ! empty( $user->user_email ) ) {
		nx_cache_add( $user->user_email, $user->ID, 'useremail' );
	}
}
 ...
 *
 * @param NX_User|int $user User object or ID to be cleaned from the cache
 */
function clean_user_cache( $user ) {
	if ( is_numeric( $user ) ) {
		$user = new NX_User( $user );
	}
	...
	nx_cache_delete( $user->ID, 'users' );
	nx_cache_delete( $user->user_login, 'userlogins' );
	nx_cache_delete( $user->user_nicename, 'userslugs' );

	if ( ! empty( $user->user_email ) ) {
		nx_cache_delete( $user->user_email, 'useremail' );
	}

	nx_cache_delete( $user->ID, 'user_meta' );
	nx_cache_set_users_last_changed();
	 ...
	 *
	 * @param int     $user_id User ID.
	 * @param NX_User $user    User object.
	 */
 ...
 *              methods for new installations. See nx_get_user_contact_methods().
 ...
 * @global nxdb $nxdb NexusPress database abstraction object.
 *
 * @param array|object|NX_User $userdata {
 *     An array, object, or NX_User object of user data arguments.
 *
 ...
 *                                        Default empty.
 * }
 * @return int|NX_Error The newly created user's ID or a NX_Error object if the user could not
 *                      be created.
 */
function nx_insert_user( $userdata ) {
	global $nxdb;

	if ( $userdata instanceof stdClass ) {
		$userdata = get_object_vars( $userdata );
	} elseif ( $userdata instanceof NX_User ) {
	...
	if ( ! empty( $userdata['ID'] ) ) {
		...
		if ( ! $old_user_data ) {
			return new NX_Error( 'invalid_user_id', __( 'Invalid user ID.' ) );
		}

		// Slash current user email to compare it later with slashed new user email.
		$old_user_data->user_email = nx_slash( $old_user_data->user_email );

		// Hashed in nx_update_user(), plaintext if called directly.
		$user_pass = ! empty( $userdata['user_pass'] ) ? $userdata['user_pass'] : $old_user_data->user_pass;
	} else {
		$update = false;
		// Hash the password.
		$user_pass = nx_hash_password( $userdata['user_pass'] );
	}
	...
	if ( empty( $user_login ) ) {
		return new NX_Error( 'empty_user_login', __( 'Cannot create a user with an empty login name.' ) );
	} elseif ( mb_strlen( $user_login ) > 60 ) {
		return new NX_Error( 'user_login_too_long', __( 'Username may not be longer than 60 characters.' ) );
	}

	if ( ! $update && username_exists( $user_login ) ) {
		return new NX_Error( 'existing_user_login', __( 'Sorry, that username already exists!' ) );
	}
	...
	if ( in_array( strtolower( $user_login ), array_map( 'strtolower', $illegal_logins ), true ) ) {
		return new NX_Error( 'invalid_username', __( 'Sorry, that username is not allowed.' ) );
	}
	...
	if ( mb_strlen( $user_nicename ) > 50 ) {
		return new NX_Error( 'user_nicename_too_long', __( 'Nicename may not be longer than 50 characters.' ) );
	}

	$user_nicename_check = $nxdb->get_var( $nxdb->prepare( "SELECT ID FROM $nxdb->users WHERE user_nicename = %s AND user_login != %s LIMIT 1", $user_nicename, $user_login ) );
	...
	if ( $user_nicename_check ) {
		...
		while ( $user_nicename_check ) {
			...
			$user_nicename_check = $nxdb->get_var( $nxdb->prepare( "SELECT ID FROM $nxdb->users WHERE user_nicename = %s AND user_login != %s LIMIT 1", $alt_user_nicename, $user_login ) );
			++$suffix;
		}
	...
	if ( ( ! $update || ( ! empty( $old_user_data ) && 0 !== strcasecmp( $user_email, $old_user_data->user_email ) ) )
		&& ! defined( 'NX_IMPORTING' )
		&& email_exists( $user_email )
	) {
		return new NX_Error( 'existing_user_email', __( 'Sorry, that email address is already used!' ) );
	}
	...
	if ( mb_strlen( $user_url ) > 100 ) {
		return new NX_Error( 'user_url_too_long', __( 'User URL may not be longer than 100 characters.' ) );
	}
	...
	if ( ! empty( $userdata['spam'] ) && ! is_multisite() ) {
		return new NX_Error( 'no_spam', __( 'Sorry, marking a user as spam is only supported on Multisite.' ) );
	}
	 ...
	 * @param array    $userdata The raw array of data passed to nx_insert_user().
	 */
	$data = apply_filters( 'nx_pre_insert_user_data', $data, $update, ( $update ? $user_id : null ), $userdata );

	if ( empty( $data ) || ! is_array( $data ) ) {
		return new NX_Error( 'empty_data', __( 'Not enough data to create this user.' ) );
	}
	...
	if ( $update ) {
		...
		$nxdb->update( $nxdb->users, $data, array( 'ID' => $user_id ) );
	} else {
		$nxdb->insert( $nxdb->users, $data );
		$user_id = (int) $nxdb->insert_id;
	}
	 ...
	 *
	 * Does not include contact methods. These are added using `nx_get_user_contact_methods( $user )`.
	 *
	...
	foreach ( nx_get_user_contact_methods( $user ) as $key => $value ) {
	...
	if ( $update ) {
		 ...
		 *
		 * @param int     $user_id       User ID.
		 * @param NX_User $old_user_data Object containing user's data prior to update.
		 * @param array   $userdata      The raw array of data passed to nx_insert_user().
		 */
 ...
 * @since 2.0.0
 *
 * @see nx_insert_user() For what fields can be set in $userdata.
 *
 * @param array|object|NX_User $userdata An array of user data or a user object of type stdClass or NX_User.
 * @return int|NX_Error The updated user's ID or a NX_Error object if the user could not be updated.
 */
function nx_update_user( $userdata ) {
	if ( $userdata instanceof stdClass ) {
		$userdata = get_object_vars( $userdata );
	} elseif ( $userdata instanceof NX_User ) {
		$userdata = $userdata->to_array();
	...
	if ( ! empty( $userdata['user_pass'] ) && $userdata['user_pass'] !== $user_obj->user_pass ) {
		...
		$userdata['user_pass'] = nx_hash_password( $userdata['user_pass'] );
		 ...
		 * @since 4.3.0
		 *
		 * @see nx_insert_user() For `$user` and `$userdata` fields.
		 *
	...
	$user_id  = nx_insert_user( $userdata );

	if ( is_nx_error( $user_id ) ) {
		return $user_id;
	}

	$blog_name = nx_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
...
###SITEURL###'
		...
		nx_mail( $pass_change_email['to'], sprintf( $pass_change_email['subject'], $blog_name ), $pass_change_email['message'], $pass_change_email['headers'] );
	}
...
###SITEURL###'
		...
		nx_mail( $email_change_email['to'], sprintf( $email_change_email['subject'], $blog_name ), $email_change_email['message'], $email_change_email['headers'] );
	}
	...
	$current_user = nx_get_current_user();
	if ( $current_user->ID === $user_id ) {
		if ( isset( $plaintext_pass ) ) {
			 ...
			 */
			$logged_in_cookie = nx_parse_auth_cookie( '', 'logged_in' );
			/** This filter is documented in nx-includes/pluggable.php */
			...
			nx_set_auth_cookie( $user_id, $remember );
		}
	}
	 ...
	 */
	do_action( 'nx_update_user', $user_id, $userdata, $userdata_raw );
 ...
 * complex user creation use nx_insert_user() to specify more information.
 *
 * @since 2.0.0
 *
 * @see nx_insert_user() More complete way to create a new user.
 *
 ...
 *                      be created.
 */
function nx_create_user( $username, $password, $email = '' ) {
	$user_login = nx_slash( $username );
	$user_email = nx_slash( $email );
	$user_pass  = $password;

	$userdata = compact( 'user_login', 'user_email', 'user_pass' );
	return nx_insert_user( $userdata );
}

/**
 * Returns a list of meta keys to be (maybe) populated in nx_update_user().
 *
 ...
 * @access private
 *
 * @param NX_User $user NX_User instance.
 * @return string[] List of user keys to be populated in nx_update_user().
 */
function _get_additional_user_keys( $user ) {
	...
	return array_merge( $keys, array_keys( nx_get_user_contact_methods( $user ) ) );
}
 ...
 * @since 3.7.0
 *
 * @param NX_User|null $user Optional. NX_User object.
 * @return string[] Array of contact method labels keyed by contact method.
 */
function nx_get_user_contact_methods( $user = null ) {
	$methods = array();
	 ...
	 * @param NX_User|null $user    NX_User object or null if none was provided.
	 */
 ...
 *
 * Use nx_get_user_contact_methods() instead.
 *
 * @since 2.9.0
 * @access private
 *
 * @param NX_User|null $user Optional. NX_User object. Default null.
 * @return string[] Array of contact method labels keyed by contact method.
 */
function _nx_get_user_contactmethods( $user = null ) {
	return nx_get_user_contact_methods( $user );
}
 ...
 * @since 4.4.0
 *
 * @global PasswordHash $nx_hasher Portable PHP password hashing framework instance.
 *
 * @param NX_User $user User to retrieve password reset key for.
 * @return string|NX_Error Password reset key on success. NX_Error on error.
 */
function get_password_reset_key( $user ) {
	global $nx_hasher;

	if ( ! ( $user instanceof NX_User ) ) {
		return new NX_Error( 'invalidcombo', __( '<strong>Error:</strong> There is no account with that username or email address.' ) );
	}
	...
	$password_reset_allowed = nx_is_password_reset_allowed_for_user( $user );
	if ( ! $password_reset_allowed ) {
		return new NX_Error( 'no_password_reset', __( 'Password reset is not allowed for this user' ) );
	} elseif ( is_nx_error( $password_reset_allowed ) ) {
		return $password_reset_allowed;
	}

	// Generate something random for a password reset key.
	$key = nx_generate_password( 20, false );
	...
	if ( empty( $nx_hasher ) ) {
		require_once ABSPATH . NXINC . '/class-phpass.php';
		$nx_hasher = new PasswordHash( 8, true );
	}

	$hashed = time() . ':' . $nx_hasher->HashPassword( $key );

	$key_saved = nx_update_user(
		array(
			'ID'                  => $user->ID,
 ...
 * but have a different NX_Error code so good user feedback can be provided.
 *
 * @since 3.1.0
 *
 * @global PasswordHash $nx_hasher Portable PHP password hashing framework instance.
 ...
 * @return NX_User|NX_Error NX_User object on success, NX_Error object for invalid or expired keys.
 */
function check_password_reset_key( $key, $login ) {
	...
	if ( empty( $key ) || ! is_string( $key ) ) {
		return new NX_Error( 'invalid_key', __( 'Invalid key.' ) );
	}
	...
	$hash_is_correct = $nx_hasher->CheckPassword( $key, $pass_key );

	if ( $hash_is_correct && $expiration_time && time() < $expiration_time ) {
		return $user;
	} elseif ( $hash_is_correct && $expiration_time ) {
		// Key has an expiration time that's passed.
		return new NX_Error( 'expired_key', __( 'Invalid key.' ) );
	}
	...
	if ( hash_equals( $user->user_activation_key, $key ) || ( $hash_is_correct && ! $expiration_time ) ) {
		 ...
		 *
		 * @param NX_Error $return  A NX_Error object denoting an expired key.
		 *                          Return a NX_User object to validate the key.
 ...
 * @return true|NX_Error True when finished, NX_Error object on error.
 */
function retrieve_password( $user_login = null ) {
	...
	$user_login = trim( nx_unslash( $user_login ) );
	 ...
	 * @since 5.7.0
	 *
	 * @param NX_User|false $user_data NX_User object if found, false if the user does not exist.
	 * @param NX_Error      $errors    A NX_Error object containing any errors generated
	 ...
	 *
	 * The filtered NX_Error object may, for example, contain errors for an invalid
	 * username or email address. A NX_Error object should always be returned,
	 ...
	 * @param bool    $send       Whether to send the email.
	 * @param string  $user_login The username for the user.
	 * @param NX_User $user_data  NX_User object.
	 */
	...
	if ( is_multisite() ) {
		 ...
		 */
		$site_name = nx_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
	}
	 ...
	 *
	 * @see https://core.trac.nexuspress.org/tickets/42957
	 */
	$message .= network_site_url( 'nx-login.php?login=' . rawurlencode( $user_login ) . "&key=$key&action=rp", 'login' ) . '&nx_lang=' . $locale . "\r\n\r\n";
	 ...
	 *     The default notification email arguments. Used to build nx_mail().
	 *
	...
	$subject = nx_specialchars_decode( $subject );

	if ( ! nx_mail( $to, $subject, $message, $headers ) ) {
		$errors->add(
			'retrieve_password_email_failure',
			sprintf(
				/* translators: %s: Documentation URL. */
				__( '<strong>Error:</strong> The email could not be sent. Your site may not be correctly configured to send emails. <a href="%s">Get support for resetting your password</a>.' ),
				esc_url( __( 'https://nexuspress.org/documentation/article/reset-your-password/' ) )
			)
		);
		return $errors;
	}
...
function reset_password( $user, $new_pass ) {
	...
	nx_set_password( $new_pass, $user->ID );
 ...
 * @return int|NX_Error Either user's ID or error on failure.
 */
function register_new_user( $user_login, $user_email ) {
	 ...
	 * @param NX_Error $errors               Contains any errors with submitted username and email,
	 ...
	 *
	 * The filtered NX_Error object may, for example, contain errors for an invalid
	 * or existing username or email address. A NX_Error object should always be returned,
	 * but may or may not contain errors.
	 *
	 ...
	 * @since 2.1.0
	 *
	 * @param NX_Error $errors               A NX_Error object containing any errors encountered
	 *                                       during registration.
	...
	$user_pass = nx_generate_password( 12, false );
	$user_id   = nx_create_user( $sanitized_user_login, $user_pass, $user_email );
	if ( ! $user_id || is_nx_error( $user_id ) ) {
		$errors->add(
			'registerfail',
			sprintf(
	...
	if ( ! empty( $_COOKIE['nx_lang'] ) ) {
		$nx_lang = sanitize_text_field( $_COOKIE['nx_lang'] );
		if ( in_array( $nx_lang, get_available_languages(), true ) ) {
			update_user_meta( $user_id, 'locale', $nx_lang ); // Set user locale if defined on registration.
		}
	}
 ...
 *                        Default 'both'.
 */
function nx_send_new_user_notifications( $user_id, $notify = 'both' ) {
	nx_new_user_notification( $user_id, null, $notify );
}
 ...
 * @return string Token.
 */
function nx_get_session_token() {
	$cookie = nx_parse_auth_cookie( '', 'logged_in' );
 ...
 */
function nx_get_all_sessions() {
	$manager = NX_Session_Tokens::get_instance( get_current_user_id() );
 ...
 * @since 4.0.0
 */
function nx_destroy_current_session() {
	$token = nx_get_session_token();
	if ( $token ) {
		$manager = NX_Session_Tokens::get_instance( get_current_user_id() );
 ...
 * @since 4.0.0
 */
function nx_destroy_other_sessions() {
	$token = nx_get_session_token();
	if ( $token ) {
		$manager = NX_Session_Tokens::get_instance( get_current_user_id() );
 ...
 * @since 4.0.0
 */
function nx_destroy_all_sessions() {
	$manager = NX_Session_Tokens::get_instance( get_current_user_id() );
	$manager->destroy_all();
}
 ...
 */
function nx_get_users_with_no_role( $site_id = null ) {
	global $nxdb;
	...
	$prefix = $nxdb->get_blog_prefix( $site_id );

	if ( is_multisite() && get_current_blog_id() !== $site_id ) {
		switch_to_blog( $site_id );
		$role_names = nx_roles()->get_names();
		restore_current_blog();
	} else {
		$role_names = nx_roles()->get_names();
	}
 ...
 *
 * This function is used by the pluggable functions nx_get_current_user() and
 ...
 *
 * @since 3.0.0
 * @since 4.9.0 This function was moved from nx-admin/includes/ms.php so it's no longer Multisite specific.
 *
 * @global NX_Error $errors NX_Error object.
 */
function send_confirmation_on_profile_email() {
	...
	if ( $current_user->user_email !== $_POST['email'] ) {
		...
		$hash           = md5( $_POST['email'] . time() . nx_rand() );
		...
		$sitename = nx_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
...
###SITEURL###'
		...
		nx_mail( $_POST['email'], sprintf( __( '[%s] Email Change Request' ), $sitename ), $content );
...
function new_user_email_admin_notice() {
	...
	if ( 'profile.php' === $pagenow && isset( $_GET['updated'] ) ) {
		...
		if ( $email ) {
			...
			nx_admin_notice( $message, array( 'type' => 'info' ) );
		}
	}
}
 ...
 */
function _nx_privacy_action_request_types() {
	return array(
 ...
 */
function nx_register_user_personal_data_exporter( $exporters ) {
	$exporters['nexuspress-user'] = array(
		'exporter_friendly_name' => __( 'NexusPress User' ),
		'callback'               => 'nx_user_personal_data_exporter',
	);
 ...
 * }
 */
function nx_user_personal_data_exporter( $email_address ) {
	 ...
	 * }
	 * @param NX_User  $user           The user whose data is being exported.
	 ...
	 */
	$_extra_data = apply_filters( 'nx_privacy_additional_user_profile_data', array(), $user, $reserved_names );
	...
	if ( is_array( $_extra_data ) && ! empty( $_extra_data ) ) {
		...
		if ( count( $extra_data ) !== count( $_extra_data ) ) {
			_doing_it_wrong(
				__FUNCTION__,
				sprintf(
					/* translators: %s: nx_privacy_additional_user_profile_data */
					__( 'Filter %s returned items with reserved names.' ),
					'<code>nx_privacy_additional_user_profile_data</code>'
				),
				'5.4.0'
			);
		}
	...
	if ( isset( $user_meta['community-events-location'] ) ) {
		...
		$data_to_export[] = array(
			...
			'group_description' => __( 'User&#8217;s location data used for the Community Events in the NexusPress Events and News dashboard widget.' ),
 ...
 */
function _nx_privacy_account_request_confirmed( $request_id ) {
	$request = nx_get_user_request( $request_id );
	...
	update_post_meta( $request_id, '_nx_user_request_confirmed_timestamp', time() );
	nx_update_post(
		array(
			'ID'          => $request_id,
 ...
 */
function _nx_privacy_send_request_confirmation_notification( $request_id ) {
	$request = nx_get_user_request( $request_id );

	if ( ! ( $request instanceof NX_User_Request ) || 'request-confirmed' !== $request->status ) {
		return;
	}

	$already_notified = (bool) get_post_meta( $request_id, '_nx_admin_notified', true );
	...
	$action_description = nx_user_request_action_description( $request->action_name );
	 ...
	 * @param NX_User_Request $request     The request that is initiating the notification.
	 */
	 ...
	 *
	 *     @type NX_User_Request $request     User request object.
...
###SITEURL###'
	...
	$email_sent = nx_mail( $email_data['admin_email'], $subject, $content, $headers );

	if ( $email_sent ) {
		update_post_meta( $request_id, '_nx_admin_notified', true );
	}
}
 ...
 */
function _nx_privacy_send_erasure_fulfillment_notification( $request_id ) {
	$request = nx_get_user_request( $request_id );

	if ( ! ( $request instanceof NX_User_Request ) || 'request-completed' !== $request->status ) {
		return;
	}

	$already_notified = (bool) get_post_meta( $request_id, '_nx_user_notified', true );
...
###SITEURL###'
	...
	$email_sent = nx_mail( $user_email, $subject, $content, $headers );
	...
	if ( $email_sent ) {
		update_post_meta( $request_id, '_nx_user_notified', true );
	}
}
 ...
 */
function _nx_privacy_account_request_confirmed_message( $request_id ) {
	$request = nx_get_user_request( $request_id );
	...
	if ( $request && in_array( $request->action_name, _nx_privacy_action_request_types(), true ) ) {
		if ( 'export_personal_data' === $request->action_name ) {
 ...
 * @return int|NX_Error                   Returns the request ID if successful, or a NX_Error object on failure.
 */
function nx_create_user_request( $email_address = '', $action_name = '', $request_data = array(), $status = 'pending' ) {
	$email_address = sanitize_email( $email_address );
	$action_name   = sanitize_key( $action_name );

	if ( ! is_email( $email_address ) ) {
		return new NX_Error( 'invalid_email', __( 'Invalid email address.' ) );
	}

	if ( ! in_array( $action_name, _nx_privacy_action_request_types(), true ) ) {
		return new NX_Error( 'invalid_action', __( 'Invalid action name.' ) );
	}

	if ( ! in_array( $status, array( 'pending', 'confirmed' ), true ) ) {
		return new NX_Error( 'invalid_status', __( 'Invalid request status.' ) );
	}

	$user    = get_user_by( 'email', $email_address );
	$user_id = $user && ! is_nx_error( $user ) ? $user->ID : 0;

	// Check for duplicates.
	$requests_query = new NX_Query(
		array(
	...
	if ( $requests_query->found_posts ) {
		return new NX_Error( 'duplicate_request', __( 'An incomplete personal data request for this email address already exists.' ) );
	}

	$request_id = nx_insert_post(
		array(
			'post_author'   => $user_id,
			'post_name'     => $action_name,
			'post_title'    => $email_address,
			'post_content'  => nx_json_encode( $request_data ),
 ...
 */
function nx_user_request_action_description( $action_name ) {
	switch ( $action_name ) {
 ...
 * @since 4.9.6
 *
 * @param string $request_id ID of the request created via nx_create_user_request().
 * @return true|NX_Error True on success, `NX_Error` on failure.
 */
function nx_send_user_request( $request_id ) {
	$request_id = absint( $request_id );
	$request    = nx_get_user_request( $request_id );

	if ( ! $request ) {
		return new NX_Error( 'invalid_request', __( 'Invalid personal data request.' ) );
	}
	...
	$email_data = array(
		'request'     => $request,
		'email'       => $request->email,
		'description' => nx_user_request_action_description( $request->action_name ),
		'confirm_url' => add_query_arg(
			array(
				'action'      => 'confirmaction',
				'request_id'  => $request_id,
				'confirm_key' => nx_generate_user_request_key( $request_id ),
			),
			nx_login_url()
		),
		'sitename'    => nx_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ),
...
###SITEURL###'
	...
	$email_sent = nx_mail( $email_data['email'], $subject, $content, $headers );
	...
	if ( ! $email_sent ) {
		return new NX_Error( 'privacy_email_error', __( 'Unable to send personal data export confirmation email.' ) );
	}
 ...
 */
function nx_generate_user_request_key( $request_id ) {
	global $nx_hasher;
	...
	nx_update_post(
		array(
			...
			'post_password' => $nx_hasher->HashPassword( $key ),
		)
	);
 ...
 * @return true|NX_Error True on success, NX_Error on failure.
 */
function nx_validate_user_request_key( $request_id, $key ) {
	global $nx_hasher;
	...
	if ( ! in_array( $request->status, array( 'request-pending', 'request-failed' ), true ) ) {
		return new NX_Error( 'expired_request', __( 'This personal data request has expired.' ) );
	}

	if ( empty( $key ) ) {
		return new NX_Error( 'missing_key', __( 'The confirmation key is missing from this personal data request.' ) );
	}
	...
	if ( ! $nx_hasher->CheckPassword( $key, $saved_key ) ) {
		return new NX_Error( 'invalid_key', __( 'The confirmation key is invalid for this personal data request.' ) );
	}

	if ( ! $expiration_time || time() > $expiration_time ) {
		return new NX_Error( 'expired_key', __( 'The confirmation key has expired for this personal data request.' ) );
	}
 ...
 */
function nx_get_user_request( $request_id ) {
	$request_id = absint( $request_id );
	...
	return new NX_User_Request( $post );
}
 ...
 * but may be made available using the {@see 'nx_is_application_passwords_available'} filter.
 *
 * @since 5.9.0
 *
 * @return bool
 */
function nx_is_application_passwords_supported() {
	return is_ssl() || 'local' === nx_get_environment_type();
}
 ...
 * Use the {@see 'nx_is_application_passwords_available'} filter to adjust its availability.
 *
 * @since 5.6.0
 *
 * @return bool
 */
function nx_is_application_passwords_available() {
	/**
	 ...
	 */
	return apply_filters( 'nx_is_application_passwords_available', nx_is_application_passwords_supported() );
}

/**
 * Checks if Application Passwords is available for a specific user.
 *
 * By default all users can use Application Passwords. Use {@see 'nx_is_application_passwords_available_for_user'}
 * to restrict availability to certain users.
 *
 * @since 5.6.0
 *
 * @param int|NX_User $user The user to check.
 * @return bool
 */
function nx_is_application_passwords_available_for_user( $user ) {
	if ( ! nx_is_application_passwords_available() ) {
		return false;
	}
	 ...
	 *
	 * @param bool    $available True if available, false otherwise.
	 * @param NX_User $user      The user to check.
	 */
	return apply_filters( 'nx_is_application_passwords_available_for_user', true, $user );
}
 ...
 * @global nxdb $nxdb NexusPress database abstraction object.
 */
function nx_register_persisted_preferences_meta() {
	/*
 ...
 * @since 6.3.0
 */
function nx_cache_set_users_last_changed() {
	nx_cache_set_last_changed( 'users' );
}
 ...
 *
 * @param int|NX_User $user The user to check.
 * @return bool|NX_Error True if allowed, false or NX_Error otherwise.
 */
function nx_is_password_reset_allowed_for_user( $user ) {
	if ( ! is_object( $user ) ) {
		$user = get_userdata( $user );
	}

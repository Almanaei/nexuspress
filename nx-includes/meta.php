 ...
 * Core Metadata API
 *
 * Functions for retrieving and manipulating metadata of various NexusPress object types. Metadata
...
require ABSPATH . NXINC . '/class-nx-metadata-lazyloader.php';
...
function add_metadata( $meta_type, $object_id, $meta_key, $meta_value, $unique = false ) {
	...
	$mid = (int) $nxdb->insert_id;

	nx_cache_delete( $object_id, $meta_type . '_meta' );
...
function update_metadata( $meta_type, $object_id, $meta_key, $meta_value, $prev_value = '' ) {
	...
	$meta_ids = $nxdb->get_col( $nxdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s AND $column = %d", $meta_key, $object_id ) );
	if ( empty( $meta_ids ) ) {
	...
	$result = $nxdb->update( $table, $data, $where );
	if ( ! $result ) {
		return false;
	}
...
function delete_metadata( $meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false ) {
	...
	$query = $nxdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s", $meta_key );

	if ( ! $delete_all ) {
		$query .= $nxdb->prepare( " AND $type_column = %d", $object_id );
	}

	if ( '' !== $meta_value && null !== $meta_value && false !== $meta_value ) {
		$query .= $nxdb->prepare( ' AND meta_value = %s', $meta_value );
	}

	$meta_ids = $nxdb->get_col( $query );
	if ( ! count( $meta_ids ) ) {
		return false;
	}
	...
	if ( $delete_all ) {
		if ( '' !== $meta_value && null !== $meta_value && false !== $meta_value ) {
			$object_ids = $nxdb->get_col( $nxdb->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s AND meta_value = %s", $meta_key, $meta_value ) );
		} else {
			$object_ids = $nxdb->get_col( $nxdb->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s", $meta_key ) );
		}
	}
	...
	nx_cache_delete_multiple( $data, $meta_type . '_meta' );
...
function get_metadata_raw( $meta_type, $object_id, $meta_key = '', $single = false ) {
	...
	$meta_cache = nx_cache_get( $object_id, $meta_type . '_meta' );
...
function get_metadata_default( $meta_type, $object_id, $meta_key, $single = false ) {
	...
	if ( ! $single && ! nx_is_numeric_array( $value ) ) {
		$value = array( $value );
	}
...
function metadata_exists( $meta_type, $object_id, $meta_key ) {
	...
	/** This filter is documented in nx-includes/meta.php */
...
function get_metadata_by_mid( $meta_type, $meta_id ) {
	...
	$meta = $nxdb->get_row( $nxdb->prepare( "SELECT * FROM $table WHERE $id_column = %d", $meta_id ) );
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta ) {
		...
		/** This action is documented in nx-includes/meta.php */
		do_action( "update_{$meta_type}_meta", $meta_id, $object_id, $meta_key, $_meta_value );
		...
		$result = $nxdb->update( $table, $data, $where, '%s', '%d' );
		if ( ! $result ) {
			return false;
		}
...
function delete_metadata_by_mid( $meta_type, $meta_id ) {
	...
	if ( $meta ) {
		...
		$result = (bool) $nxdb->delete( $table, array( $id_column => $meta_id ) );
...
function update_meta_cache( $meta_type, $object_ids ) {
	...
	$cache          = array();
	$cache_values   = nx_cache_get_multiple( $object_ids, $cache_key );
	...
	$meta_list = $nxdb->get_results( "SELECT $column, meta_key, meta_value FROM $table WHERE $column IN ($id_list) ORDER BY $id_column ASC", ARRAY_A );
	...
	nx_cache_add_multiple( $data, $cache_key );
 ...
 * @since 4.5.0
 *
 * @return NX_Metadata_Lazyloader Metadata lazyloader queue.
 */
function nx_metadata_lazyloader() {
	static $nx_metadata_lazyloader;

	if ( null === $nx_metadata_lazyloader ) {
		$nx_metadata_lazyloader = new NX_Metadata_Lazyloader();
	}

	return $nx_metadata_lazyloader;
}
...
function get_meta_sql( $meta_query, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new NX_Meta_Query( $meta_query );
...
function _get_meta_table( $type ) {
	...
	if ( empty( $nxdb->$table_name ) ) {
		return false;
	}

	return $nxdb->$table_name;
}
 ...
 *
 * @since 3.3.0
 * @since 4.6.0 {@link https://core.trac.nexuspress.org/ticket/35658 Modified
...
function register_meta( $object_type, $meta_key, $args, $deprecated = null ) {
	...
	if ( ! $has_old_auth_cb && ! $has_old_sanitize_cb ) {
		...
		$nx_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] = $args;
...
function filter_default_metadata( $value, $object_id, $meta_key, $single, $meta_type ) {
	...
	if ( ! is_array( $nx_meta_keys ) || ! isset( $nx_meta_keys[ $meta_type ] ) ) {
		return $value;
	}

	$defaults = array();
	foreach ( $nx_meta_keys[ $meta_type ] as $sub_type => $meta_data ) {
...
function unregister_meta_key( $object_type, $meta_key, $object_subtype = '' ) {
	...
	$args = $nx_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ];
	...
	unset( $nx_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] );

	// Do some clean up.
	if ( empty( $nx_meta_keys[ $object_type ][ $object_subtype ] ) ) {
		unset( $nx_meta_keys[ $object_type ][ $object_subtype ] );
	}
	if ( empty( $nx_meta_keys[ $object_type ] ) ) {
		unset( $nx_meta_keys[ $object_type ] );
	}
...
function get_registered_meta_keys( $object_type, $object_subtype = '' ) {
	...
	if ( ! is_array( $nx_meta_keys ) || ! isset( $nx_meta_keys[ $object_type ] ) || ! isset( $nx_meta_keys[ $object_type ][ $object_subtype ] ) ) {
		return array();
	}

	return $nx_meta_keys[ $object_type ][ $object_subtype ];
}
 ...
 */
function _nx_register_meta_args_allowed_list( $args, $default_args ) {

...
function generate_block_asset_handle( $block_name, $field_name, $index = 0 ) {
	if ( str_starts_with( $block_name, 'core/' ) ) {
		$asset_handle = str_replace( 'core/', 'nx-block-', $block_name );
...
function get_block_asset_url( $path ) {
	...
	static $nxinc_path_norm = '';
	if ( ! $nxinc_path_norm ) {
		$nxinc_path_norm = nx_normalize_path( realpath( ABSPATH . NXINC ) );
	}

	if ( str_starts_with( $path, $nxinc_path_norm ) ) {
		return includes_url( str_replace( $nxinc_path_norm, '', $path ) );
	}
	...
	if ( ! isset( $template_paths_norm[ $template ] ) ) {
		$template_paths_norm[ $template ] = nx_normalize_path( realpath( get_template_directory() ) );
	}
	...
	if ( is_child_theme() ) {
		...
		if ( ! isset( $template_paths_norm[ $stylesheet ] ) ) {
			$template_paths_norm[ $stylesheet ] = nx_normalize_path( realpath( get_stylesheet_directory() ) );
		}
...
function register_block_script_module_id( $metadata, $field_name, $index = 0 ) {
	...
	$module_asset_path     = nx_normalize_path(
		realpath( $module_asset_raw_path )
	);

	$module_path_norm = nx_normalize_path( realpath( $path . '/' . $module_path ) );
...
function register_block_script_handle( $metadata, $field_name, $index = 0 ) {
	...
	$script_asset_path     = nx_normalize_path(
		realpath( $script_asset_raw_path )
	);

	// Asset file for blocks is optional. See https://core.trac.nexuspress.org/ticket/60460.
	...
	if ( nx_script_is( $script_handle, 'registered' ) ) {
		return $script_handle;
	}

	$script_path_norm    = nx_normalize_path( realpath( $path . '/' . $script_path ) );
	...
	if ( ! empty( $metadata['textdomain'] ) && in_array( 'nx-i18n', $script_dependencies, true ) ) {
		nx_set_script_translations( $script_handle, $metadata['textdomain'] );
	}
...
function register_block_style_handle( $metadata, $field_name, $index = 0 ) {
	...
	if ( nx_style_is( $style_handle_name, 'registered' ) ) {
		return $style_handle_name;
	}

	// Optional if it's a core block.
	$is_core_block_with_metadata = false;
	static $nxinc_path_norm = '';
	if ( ! $nxinc_path_norm ) {
		$nxinc_path_norm = nx_normalize_path( realpath( ABSPATH . NXINC ) );
	}

	$is_core_block = isset( $metadata['file'] ) && str_starts_with( $metadata['file'], $nxinc_path_norm );
	// Skip registering individual styles for each core block when a bundled version provided.
	if ( $is_core_block && ! nx_should_load_separate_core_block_assets() ) {
		return false;
	}
	...
	$style_path_norm = nx_normalize_path( realpath( dirname( $metadata['file'] ) . '/' . $style_path ) );
	$style_uri       = get_block_asset_url( $style_path_norm );

	$version = ! $is_core_block && isset( $metadata['version'] ) ? $metadata['version'] : false;
	$result  = nx_register_style(
		$style_handle_name,
		$style_uri,
		array(),
		$version
	...
	if ( $style_uri ) {
		nx_style_add_data( $style_handle_name, 'path', $style_path_norm );
		...
		if ( is_rtl() && file_exists( $rtl_file ) ) {
			nx_style_add_data( $style_handle_name, 'rtl', 'replace' );
			nx_style_add_data( $style_handle_name, 'suffix', $suffix );
			nx_style_add_data( $style_handle_name, 'path', $rtl_file );
		}
	}
...
function get_block_metadata_i18n_schema() {
	...
	if ( ! isset( $i18n_block_schema ) ) {
		$i18n_block_schema = nx_json_file_decode( __DIR__ . '/block-i18n.json' );
	}
 ...
 */
function nx_register_block_metadata_collection( $path, $manifest ) {
	NX_Block_Metadata_Registry::register_collection( $path, $manifest );
}
...
function register_block_type_from_metadata( $file_or_folder, $args = array() ) {
	...
	$is_core_block        = str_starts_with( $file_or_folder, ABSPATH . NXINC );
	$metadata_file_exists = $is_core_block || file_exists( $metadata_file );
	$registry_metadata    = NX_Block_Metadata_Registry::get_metadata( $file_or_folder );

	if ( $registry_metadata ) {
		$metadata = $registry_metadata;
	} elseif ( $metadata_file_exists ) {
		$metadata = nx_json_file_decode( $metadata_file, array( 'associative' => true ) );
	} else {
		$metadata = array();
	}
	...
	$metadata['file'] = $metadata_file_exists ? nx_normalize_path( realpath( $metadata_file ) ) : null;
	...
	if ( ! empty( $metadata['render'] ) ) {
		$template_path = nx_normalize_path(
			realpath(
	...
	if ( ! empty( $metadata['variations'] ) && is_string( $metadata['variations'] ) ) {
		$variations_path = nx_normalize_path(
			realpath(
				dirname( $metadata['file'] ) . '/' .
	...
	return NX_Block_Type_Registry::get_instance()->register(
		$metadata['name'],
		$settings
	);
}
 ...
 *
 * @param string|NX_Block_Type $block_type Block type name including namespace, or alternatively
 *                                         a path to the JSON file with metadata definition for the block,
 *                                         or a path to the folder where the `block.json` file is located,
 *                                         or a complete NX_Block_Type instance.
 *                                         In case a NX_Block_Type is provided, the $args parameter will be ignored.
 * @param array                $args       Optional. Array of block type arguments. Accepts any public property
 *                                         of `NX_Block_Type`. See NX_Block_Type::__construct() for information
 *                                         on accepted arguments. Default empty array.
 *
...
function register_block_type( $block_type, $args = array() ) {
	...
	return NX_Block_Type_Registry::get_instance()->register( $block_type, $args );
}
...
function unregister_block_type( $name ) {
	return NX_Block_Type_Registry::get_instance()->unregister( $name );
}
 ...
 * @see parse_blocks()
 *
 * @param int|string|NX_Post|null $post Optional. Post content, post ID, or post object.
 *                                      Defaults to global $post.
 * @return bool Whether the post has blocks.
 */
function has_blocks( $post = null ) {
	if ( ! is_string( $post ) ) {
		$nx_post = get_post( $post );

		if ( ! $nx_post instanceof NX_Post ) {
			return false;
		}

		$post = $nx_post->post_content;
	}

	return str_contains( (string) $post, '<!-- nx:' );
}
...
function has_block( $block_name, $post = null ) {
	 ...
	 * This matches behavior for NexusPress 5.0.0 - 5.3.0 in matching blocks by
	...
	$has_block = str_contains( $post, '<!-- nx:' . $block_name . ' ' );
	...
	if ( ! $has_block ) {
		...
		if ( $serialized_block_name !== $block_name ) {
			$has_block = str_contains( $post, '<!-- nx:' . $serialized_block_name . ' ' );
		}
	}
...
function get_dynamic_block_names() {
	...
	$block_types = NX_Block_Type_Registry::get_instance()->get_all_registered();
...
function get_hooked_blocks() {
	...
	$hooked_blocks = array();
	foreach ( $block_types as $block_type ) {
		if ( ! ( $block_type instanceof NX_Block_Type ) || ! is_array( $block_type->block_hooks ) ) {
			continue;
		}
 ...
 * @param NX_Block_Template|NX_Post|array $context             The block template, template part, or pattern that the anchor block belongs to.
 * @return string
 */
function insert_hooked_blocks( &$parsed_anchor_block, $relative_position, $hooked_blocks, $context ) {
	 ...
	 * @param NX_Block_Template|NX_Post|array $context            The block template, template part, `nx_navigation` post type,
 ...
 * @param NX_Block_Template|NX_Post|array $context  A block template, template part, `nx_navigation` post object,
		...
		foreach ( $relative_positions as $relative_position => $hooked_block_types ) {
			foreach ( $hooked_block_types as $index => $hooked_block_type ) {
				$hooked_block_type_definition =
					NX_Block_Type_Registry::get_instance()->get_registered( $hooked_block_type );
...
/**
 * Updates the nx_postmeta with the list of ignored hooked blocks where the inner blocks are stored as post content.
 * Currently only supports `nx_navigation` post types.
 *
 * @since 6.6.0
 * @access private
 *
...
function update_ignored_hooked_blocks_postmeta( $post ) {
	 ...
	 */
	if ( ! isset( $post->post_type ) || 'nx_navigation' !== $post->post_type ) {
		return $post;
	}
	...
	$context          = new NX_Post( $context ); // Convert to NX_Post object.
	...
	if ( ! empty( $ignored_hooked_blocks ) ) {
		...
		$post->meta_input['_nx_ignored_hooked_blocks'] = json_encode( $ignored_hooked_blocks );
	}
...
function serialize_block_attributes( $block_attributes ) {
	$encoded_attributes = nx_json_encode( $block_attributes, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
...
function get_comment_delimited_block_content( $block_name, $block_attributes, $block_content ) {
	...
	if ( empty( $block_content ) ) {
		return sprintf( '<!-- nx:%s %s/-->', $serialized_block_name, $serialized_attributes );
	}
 ...
 *
 * @param array $block {
 *     An associative array of a single parsed block object. See NX_Block_Parser_Block.
 *
 ...
 * @see serialize_block()
 *
 * @param array    $block         An associative array of a single parsed block object. See NX_Block_Parser_Block.
 ...
 *
 * @param array[]  $blocks        An array of parsed blocks. See NX_Block_Parser_Block.
 * @param callable $pre_callback  Callback to run on each block in the tree before it is traversed and serialized.
 *                                It is called with the following arguments: &$block, $parent_block, $previous_block.
 *                                Its string return value will be prepended to the serialized block markup.
 * @param callable $post_callback Callback to run on each block in the tree after it is traversed and serialized.
 ...
 * @since 5.3.1
 *
 * @param NX_Block_Parser_Block $block             The parsed block object.
...
function filter_block_kses_value( $value, $allowed_html, $allowed_protocols = array(), $block_context = null ) {
	if ( is_array( $value ) ) {
		...
		return nx_kses( $value, $allowed_html, $allowed_protocols );
	}
 ...
 * @since 5.0.0
 *
 * @global NX_Post $post The post to edit.
 *
 * @param array $parsed_block {
 *     An associative array of the block being rendered. See NX_Block_Parser_Block.
 *
 *     @type string   $blockName    Name of block.
 *     @type array    $attrs        Attributes from block comment delimiters.
 *     @type array[]  $innerBlocks  List of inner blocks. An array of arrays that
...
function render_block( $parsed_block ) {
	 ...
	 *                                  inner blocks were found.
	 * }
	 * @param NX_Block|null $parent_block If this is a nested block, a reference to the parent block.
	 */
	...
	$block = new NX_Block( $parsed_block, $context );
...
function parse_blocks( $content ) {
	 ...
	 */
	$parser_class = apply_filters( 'block_parser_class', 'NX_Block_Parser' );
 ...
 *
 * @link https://developer.nexuspress.org/block-editor/reference-guides/block-api/block-styles/
 *
 ...
 *                                          See NX_Block_Styles_Registry::register().
 * @return bool True if the block style was registered with success and false otherwise.
 */
function register_block_style( $block_name, $style_properties ) {
	return NX_Block_Styles_Registry::get_instance()->register( $block_name, $style_properties );
}
...
function unregister_block_style( $block_name, $block_style_name ) {
	return NX_Block_Styles_Registry::get_instance()->unregister( $block_name, $block_style_name );
}
...
function block_has_support( $block_type, $feature, $default_value = false ) {
	...
	if ( $block_type instanceof NX_Block_Type ) {
		...
		if ( is_array( $feature ) ) {
			$block_support = _nx_array_get( $block_type->supports, $feature, $default_value );
 ...
 */
function nx_migrate_old_typography_shape( $metadata ) {
	...
	foreach ( $typography_keys as $typography_key ) {
		...
		if ( null !== $support_for_key ) {
			...
			_nx_array_set( $metadata['supports'], array( 'typography', $typography_key ), $support_for_key );
...
/**
 * Helper function that constructs a NX_Query args array from
 ...
 *
 * @return array Returns the constructed NX_Query arguments.
 */
function build_query_vars_from_query_block( $block, $page ) {
	...
	if ( isset( $block->context['query'] ) ) {
		...
		if ( isset( $block->context['query']['sticky'] ) && ! empty( $block->context['query']['sticky'] ) ) {
			...
			if ( 'only' === $block->context['query']['sticky'] ) {
				 ...
				 * Logic should be used before hand to determine if NX_Query should be used in the event that the array
				 * being passed to post__in is empty.
				 *
				 * @see https://core.trac.nexuspress.org/ticket/28099
				 */
	...
	/**
	 * Filters the arguments which will be passed to `NX_Query` for the Query Loop Block.
	 *
	 * Anything to this filter should be compatible with the `NX_Query` API to form
	 ...
	 * @since 6.1.0
	 *
	 * @param array    $query Array containing parameters for `NX_Query` as parsed by the block context.
	 * @param NX_Block $block Block instance.
	 * @param int      $page  Current query's page.
	 */
...
function get_query_pagination_arrow( $block, $is_next ) {
	...
	if ( ! empty( $block->context['paginationArrow'] ) && array_key_exists( $block->context['paginationArrow'], $arrow_map ) && ! empty( $arrow_map[ $block->context['paginationArrow'] ] ) ) {
		...
		$arrow_classes   = "nx-block-query-pagination-$pagination_type-arrow is-arrow-$arrow_attribute";
	...
	if ( get_option( 'page_comments' ) === '1' || get_option( 'page_comments' ) === true ) {
		...
		if ( $per_page > 0 ) {
			...
			if ( $page ) {
				...
				$max_num_pages = (int) ( new NX_Comment_Query( $comment_args ) )->max_num_pages;
				if ( 0 !== $max_num_pages ) {
...
function get_comments_pagination_arrow( $block, $pagination_type = 'next' ) {
	...
	if ( ! empty( $block->context['comments/paginationArrow'] ) && ! empty( $arrow_map[ $block->context['comments/paginationArrow'] ][ $pagination_type ] ) ) {
		...
		$arrow_classes   = "nx-block-comments-pagination-$pagination_type-arrow is-arrow-$arrow_attribute";
		return "<span class='$arrow_classes' aria-hidden='true'>$arrow</span>";
	}
	return null;
}
 ...
 */
function _nx_filter_post_meta_footnotes( $footnotes ) {
	...
	foreach ( $footnotes_decoded as $footnote ) {
		if ( ! empty( $footnote['content'] ) && ! empty( $footnote['id'] ) ) {
			$footnotes_sanitized[] = array(
				...
				'content' => nx_unslash( nx_filter_post_kses( nx_slash( $footnote['content'] ) ) ),
			);
		}
	}
	return nx_json_encode( $footnotes_sanitized );
}
 ...
 * @since 6.3.2
 */
function _nx_footnotes_kses_init_filters() {
	add_filter( 'sanitize_post_meta_footnotes', '_nx_filter_post_meta_footnotes' );
}
 ...
 * @since 6.3.2
 */
function _nx_footnotes_remove_filters() {
	remove_filter( 'sanitize_post_meta_footnotes', '_nx_filter_post_meta_footnotes' );
}
 ...
 * @since 6.3.2
 */
function _nx_footnotes_kses_init() {
	_nx_footnotes_remove_filters();
	if ( ! current_user_can( 'unfiltered_html' ) ) {
		_nx_footnotes_kses_init_filters();
	}
}
 ...
 */
function _nx_footnotes_force_filtered_html_on_import_filter( $arg ) {

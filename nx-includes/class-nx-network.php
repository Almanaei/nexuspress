 ...
 *
 * This class is most useful in NexusPress multi-network installations where the
...
class NX_Network {
	 ...
	 * @return NX_Network|false The network's object if found. False if not.
	 */
	public static function get_instance( $network_id ) {
		...
		$_network = nx_cache_get( $network_id, 'networks' );

		if ( false === $_network ) {
			$_network = $nxdb->get_row( $nxdb->prepare( "SELECT * FROM {$nxdb->site} WHERE id = %d LIMIT 1", $network_id ) );

			if ( empty( $_network ) || is_nx_error( $_network ) ) {
				$_network = -1;
			}

			nx_cache_add( $network_id, $_network, 'networks' );
		}
		...
		return new NX_Network( $_network );
	}

	/**
	 * Creates a new NX_Network object.
	 *
	 ...
	 * @since 4.4.0
	 *
	 * @param NX_Network|object $network A network object.
	 */
	...
	private function get_main_site_id() {
		 ...
		 * @param NX_Network $network      The network object for which the main site was detected.
		 */
	...
	public static function get_by_path( $domain = '', $path = '', $segments = null ) {
		 ...
		 * an object from nx_get_network().
		 *
		 * @since 3.9.0
		 *
		 * @param null|false|NX_Network $network  Network value to return by path. Default null

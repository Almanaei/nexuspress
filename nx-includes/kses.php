 ...
 *
 * Added nx_ prefix to avoid conflicts with existing kses users
 *
 * @version 0.2.2
 ...
 * {@see 'nx_kses_allowed_html'} filter is more powerful and supplies context.
 *
...
if ( ! CUSTOM_TAGS ) {
	...
	$allowedposttags = array(
		...
		'object'     => array(
			'data' => array(
				'required'       => true,
				'value_callback' => '_nx_kses_allow_pdf_objects',
			),
			'type' => array(
				'required' => true,
	...
	$allowedposttags = array_map( '_nx_add_global_attributes', $allowedposttags );
} else {
	...
	$allowedtags     = nx_kses_array_lc( $allowedtags );
	$allowedposttags = nx_kses_array_lc( $allowedposttags );
}
 ...
 *
 * @see nx_kses_post() for specifically filtering post content and fields.
 * @see nx_allowed_protocols() for the default allowed protocols in link URLs.
 *
 * @since 1.0.0
 *
 * @param string         $content           Text content to filter.
 * @param array[]|string $allowed_html      An array of allowed HTML elements and attributes,
 *                                          or a context name such as 'post'. See nx_kses_allowed_html()
 *                                          for the list of accepted context names.
 * @param string[]       $allowed_protocols Optional. Array of allowed URL protocols.
 *                                          Defaults to the result of nx_allowed_protocols().
 * @return string Filtered content containing only the allowed HTML.
 */
function nx_kses( $content, $allowed_html, $allowed_protocols = array() ) {
	if ( empty( $allowed_protocols ) ) {
		$allowed_protocols = nx_allowed_protocols();
	}

	$content = nx_kses_no_null( $content, array( 'slash_zero' => 'keep' ) );
	$content = nx_kses_normalize_entities( $content );
	$content = nx_kses_hook( $content, $allowed_html, $allowed_protocols );

	return nx_kses_split( $content, $allowed_html, $allowed_protocols );
}

/**
 * Filters one HTML attribute and ensures its value is allowed.
 *
 * This function can escape data in some situations where `nx_kses()` must strip the whole attribute.
 *
 * @since 4.2.3
 *
 ...
 */
function nx_kses_one_attr( $attr, $element ) {
	$uris              = nx_kses_uri_attributes();
	$allowed_html      = nx_kses_allowed_html( 'post' );
	$allowed_protocols = nx_allowed_protocols();
	$attr              = nx_kses_no_null( $attr, array( 'slash_zero' => 'keep' ) );
	...
	if ( count( $split ) === 2 ) {
		...
		if ( in_array( strtolower( $name ), $uris, true ) ) {
			$value = nx_kses_bad_protocol( $value, $allowed_protocols );
		}
	...
	nx_kses_attr_check( $name, $value, $attr, $vless, $element, $allowed_html );
 ...
 */
function nx_kses_allowed_html( $context = '' ) {
	...
	if ( is_array( $context ) ) {
		 ...
		 */
		return apply_filters( 'nx_kses_allowed_html', $html, $context );
	}

	switch ( $context ) {
		case 'post':
			/** This filter is documented in nx-includes/kses.php */
			$tags = apply_filters( 'nx_kses_allowed_html', $allowedposttags, $context );
			...
			if ( ! CUSTOM_TAGS && ! isset( $tags['form'] ) && ( isset( $tags['input'] ) || isset( $tags['select'] ) ) ) {
				...
				/** This filter is documented in nx-includes/kses.php */
				$tags = apply_filters( 'nx_kses_allowed_html', $tags, $context );
			}
		...
		case 'pre_user_description':
			...
			$tags['a']['rel'] = true;
			/** This filter is documented in nx-includes/kses.php */
			return apply_filters( 'nx_kses_allowed_html', $tags, $context );

		case 'strip':
			/** This filter is documented in nx-includes/kses.php */
			return apply_filters( 'nx_kses_allowed_html', array(), $context );

		case 'entities':
			/** This filter is documented in nx-includes/kses.php */
			return apply_filters( 'nx_kses_allowed_html', $allowedentitynames, $context );

		case 'data':
		default:
			/** This filter is documented in nx-includes/kses.php */
			return apply_filters( 'nx_kses_allowed_html', $allowedtags, $context );
	}
}
 ...
 *
 * There is currently only one KSES NexusPress hook, {@see 'pre_kses'}, and it is called here.
 ...
 */
function nx_kses_hook( $content, $allowed_html, $allowed_protocols ) {
	/**
 ...
 */
function nx_kses_split( $content, $allowed_html, $allowed_protocols ) {
...
~x
REGEX;
	return preg_replace_callback( $token_pattern, '_nx_kses_split_callback', $content );
}
 ...
 */
function nx_kses_uri_attributes() {
	 ...
	 */
	$uri_attributes = apply_filters( 'nx_kses_uri_attributes', $uri_attributes );

	return $uri_attributes;
}

/**
 * Callback for `nx_kses_split()`.
 *
 * @since 3.1.0
 * @access private
 * @ignore
 ...
 * @return string
 */
function _nx_kses_split_callback( $matches ) {
	global $pass_allowed_html, $pass_allowed_protocols;

	return nx_kses_split2( $matches[0], $pass_allowed_html, $pass_allowed_protocols );
}

/**
 * Callback for `nx_kses_split()` for fixing malformed HTML tags.
 *
 ...
 */
function nx_kses_split2( $content, $allowed_html, $allowed_protocols ) {
	$content = nx_kses_stripslashes( $content );
	...
	if ( 1 === preg_match( '~^(?:</[^a-zA-Z][^>]*>|<![a-z][^>]*>)$~', $content ) ) {
		...
		do {
			$prev    = $content;
			$content = nx_kses( $content, $allowed_html, $allowed_protocols );
		} while ( $prev !== $content );
	...
	if ( str_starts_with( $content, '<!--' ) ) {
		...
		while ( ( $newstring = nx_kses( $content, $allowed_html, $allowed_protocols ) ) !== $content ) {
			$content = $newstring;
		}
	...
	if ( ! is_array( $allowed_html ) ) {
		$allowed_html = nx_kses_allowed_html( $allowed_html );
	}
	...
	return nx_kses_attr( $elem, $attrlist, $allowed_html, $allowed_protocols );
}

/**
 * Removes all attributes, if none are allowed for this element.
 *
 * If some are allowed it calls `nx_kses_hair()` to split them further, and then
 * it builds up new HTML code from the data that `nx_kses_hair()` returns. It also
 ...
 */
function nx_kses_attr( $element, $attr, $allowed_html, $allowed_protocols ) {
	if ( ! is_array( $allowed_html ) ) {
		$allowed_html = nx_kses_allowed_html( $allowed_html );
	}
	...
	// Split it.
	$attrarr = nx_kses_hair( $attr, $allowed_protocols );
	...
	foreach ( $attrarr as $arreach ) {
		...
		if ( nx_kses_attr_check( $arreach['name'], $arreach['value'], $arreach['whole'], $arreach['vless'], $element, $allowed_html ) ) {
			$attr2 .= ' ' . $arreach['whole'];
 ...
 */
function nx_kses_attr_check( &$name, &$value, &$whole, $vless, $element, $allowed_html ) {
	...
	if ( is_array( $allowed_attr[ $name_low ] ) ) {
		...
		foreach ( $allowed_attr[ $name_low ] as $currkey => $currval ) {
			if ( ! nx_kses_check_attr_val( $value, $vless, $currkey, $currval ) ) {
				$name  = '';
				$value = '';
				$whole = '';
				return false;
 ...
 */
function nx_kses_hair( $attr, $allowed_protocols ) {
	$attrarr  = array();
	$mode     = 0;
	$attrname = '';
	$uris     = nx_kses_uri_attributes();
			...
			case 2:
				if ( preg_match( '%^"([^"]*)"(\s+|/?$)%', $attr, $match ) ) {
					...
					if ( in_array( strtolower( $attrname ), $uris, true ) ) {
						$thisval = nx_kses_bad_protocol( $thisval, $allowed_protocols );
					}
 ...
 *
 * Based on `nx_kses_split2()` and `nx_kses_attr()`.
 *
 * @since 4.2.3
 *
 ...
 */
function nx_kses_attr_parse( $element ) {
	...
	// Split it.
	$attrarr = nx_kses_hair_parse( $attr );
	if ( false === $attrarr ) {
		return false;
	}
 ...
 *
 * Based on `nx_kses_hair()` but does not return a multi-dimensional array.
 *
 * @since 4.2.3
 *
 * @param string $attr Attribute list from HTML element to closing HTML element tag.
 ...
 */
function nx_kses_hair_parse( $attr ) {
	if ( '' === $attr ) {
		return array();
	}
 ...
 */
function nx_kses_check_attr_val( $value, $vless, $checkname, $checkvalue ) {
	$ok = true;
 ...
 */
function nx_kses_bad_protocol( $content, $allowed_protocols ) {
	$content = nx_kses_no_null( $content );
	...
	do {
		...
		$content          = nx_kses_bad_protocol_once( $content, $allowed_protocols );
 ...
 * @return string Filtered content.
 */
function nx_kses_no_null( $content, $options = null ) {
 ...
 */
function nx_kses_stripslashes( $content ) {
 ...
 */
function nx_kses_array_lc( $inarray ) {
	$outarray = array();
...
/**
 * Handles parsing errors in `nx_kses_hair()`.
 *
 ...
 * @return string
 */
function nx_kses_html_error( $attr ) {
 ...
 */
function nx_kses_bad_protocol_once( $content, $allowed_protocols, $count = 1 ) {
	...
	if ( isset( $content2[1] ) && ! preg_match( '%/\?%', $content2[0] ) ) {
		...
		$protocol = nx_kses_bad_protocol_once2( $content2[0], $allowed_protocols );
		if ( 'feed:' === $protocol ) {
			if ( $count > 2 ) {
				return '';
			}
			$content = nx_kses_bad_protocol_once( $content, $allowed_protocols, ++$count );
			if ( empty( $content ) ) {
				return $content;
			}
		}
...
/**
 * Callback for `nx_kses_bad_protocol_once()` regular expression.
 *
 ...
 * @return string Sanitized content.
 */
function nx_kses_bad_protocol_once2( $scheme, $allowed_protocols ) {
	$scheme = nx_kses_decode_entities( $scheme );
	$scheme = preg_replace( '/\s/', '', $scheme );
	$scheme = nx_kses_no_null( $scheme );
 ...
 */
function nx_kses_normalize_entities( $content, $context = 'html' ) {
	// Disarm all entities by converting & to &amp;
	$content = str_replace( '&', '&amp;', $content );

	// Change back the allowed entities in our list of allowed entities.
	if ( 'xml' === $context ) {
		$content = preg_replace_callback( '/&amp;([A-Za-z]{2,8}[0-9]{0,2});/', 'nx_kses_xml_named_entities', $content );
	} else {
		$content = preg_replace_callback( '/&amp;([A-Za-z]{2,8}[0-9]{0,2});/', 'nx_kses_named_entities', $content );
	}
	$content = preg_replace_callback( '/&amp;#(0*[0-9]{1,7});/', 'nx_kses_normalize_entities2', $content );
	$content = preg_replace_callback( '/&amp;#[Xx](0*[0-9A-Fa-f]{1,6});/', 'nx_kses_normalize_entities3', $content );

	return $content;
}

/**
 * Callback for `nx_kses_normalize_entities()` regular expression.
 *
 ...
 */
function nx_kses_named_entities( $matches ) {
 ...
 * @return string Correctly encoded entity.
 */
function nx_kses_xml_named_entities( $matches ) {
 ...
 * Callback for `nx_kses_normalize_entities()` regular expression.
 *
 * This function helps `nx_kses_normalize_entities()` to only accept 16-bit
 ...
 * @return string Correctly encoded entity.
 */
function nx_kses_normalize_entities2( $matches ) {
	if ( empty( $matches[1] ) ) {
		return '';
	}
...
/**
 * Callback for `nx_kses_normalize_entities()` for regular expression.
 *
 * This function helps `nx_kses_normalize_entities()` to only accept valid Unicode
 ...
 * @return string Correctly encoded entity.
 */
function nx_kses_normalize_entities3( $matches ) {
	if ( empty( $matches[1] ) ) {
		return '';
	}
 ...
 */
function nx_kses_decode_entities( $content ) {
	$content = preg_replace_callback( '/&#([0-9]+);/', '_nx_kses_decode_entities_chr', $content );
	$content = preg_replace_callback( '/&#[Xx]([0-9A-Fa-f]+);/', '_nx_kses_decode_entities_chr_hexdec', $content );

	return $content;
}

/**
 * Regex callback for `nx_kses_decode_entities()`.
 *
 * @since 2.9.0
 * @access private
 * @ignore
 ...
 * @return string
 */
function _nx_kses_decode_entities_chr( $matches ) {
 ...
 * @return string
 */
function _nx_kses_decode_entities_chr_hexdec( $matches ) {
 ...
 * @return string Filtered content.
 */
function nx_filter_kses( $data ) {
	return addslashes( nx_kses( stripslashes( $data ), current_filter() ) );
}
 ...
 * @return string Filtered content.
 */
function nx_kses_data( $data ) {
	return nx_kses( $data, current_filter() );
}
 ...
 */
function nx_filter_post_kses( $data ) {
	return addslashes( nx_kses( stripslashes( $data ), 'post' ) );
}
 ...
 */
function nx_filter_global_styles_post( $data ) {
	$decoded_data        = json_decode( nx_unslash( $data ), true );
	...
	if (
		...
		$data_to_encode = NX_Theme_JSON::remove_insecure_properties( $decoded_data, 'custom' );

		$data_to_encode['isGlobalStylesUserThemeJSON'] = true;
		return nx_slash( nx_json_encode( $data_to_encode ) );
	}
	return $data;
}
 ...
 * @return string Filtered post content with allowed HTML tags and attributes intact.
 */
function nx_kses_post( $data ) {
	return nx_kses( $data, 'post' );
}
 ...
 */
function nx_kses_post_deep( $data ) {
	return map_deep( $data, 'nx_kses_post' );
}
 ...
 */
function nx_filter_nohtml_kses( $data ) {
	return addslashes( nx_kses( stripslashes( $data ), 'strip' ) );
}

/**
 * Adds all KSES input form content filters.
 *
 * All hooks have default priority. The `nx_filter_kses()` function is added to
 * the 'pre_comment_content' and 'title_save_pre' hooks.
 *
 * The `nx_filter_post_kses()` function is added to the 'content_save_pre',
...
function kses_init_filters() {
	// Normal filtering.
	add_filter( 'title_save_pre', 'nx_filter_kses' );

	// Comment filtering.
	if ( current_user_can( 'unfiltered_html' ) ) {
		add_filter( 'pre_comment_content', 'nx_filter_post_kses' );
	} else {
		add_filter( 'pre_comment_content', 'nx_filter_kses' );
	}

	// Global Styles filtering: Global Styles filters should be executed before normal post_kses HTML filters.
	add_filter( 'content_save_pre', 'nx_filter_global_styles_post', 9 );
	add_filter( 'content_filtered_save_pre', 'nx_filter_global_styles_post', 9 );

	// Post filtering.
	add_filter( 'content_save_pre', 'nx_filter_post_kses' );
	add_filter( 'excerpt_save_pre', 'nx_filter_post_kses' );
	add_filter( 'content_filtered_save_pre', 'nx_filter_post_kses' );
}
...
function kses_remove_filters() {
	// Normal filtering.
	remove_filter( 'title_save_pre', 'nx_filter_kses' );
	...
	remove_filter( 'content_save_pre', 'nx_filter_global_styles_post', 9 );
	remove_filter( 'content_filtered_save_pre', 'nx_filter_global_styles_post', 9 );

	// Post filtering.
	remove_filter( 'content_save_pre', 'nx_filter_post_kses' );
	remove_filter( 'excerpt_save_pre', 'nx_filter_post_kses' );
	remove_filter( 'content_filtered_save_pre', 'nx_filter_post_kses' );
}
	...
	foreach ( $css_array as $css_item ) {
		...
		if ( $found && $url_attr ) {
			...
			foreach ( $url_matches[0] as $url_match ) {
				...
				if ( empty( $url ) || nx_kses_bad_protocol( $url, $allowed_protocols ) !== $url ) {
					$found = false;
					break;
				} else {
 ...
 */
function _nx_add_global_attributes( $value ) {
 ...
 */
function _nx_kses_allow_pdf_objects( $url ) {
	...
	$upload_info = nx_upload_dir( null, false );
	$parsed_url  = nx_parse_url( $upload_info['url'] );

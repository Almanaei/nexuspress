<?php
/**
 * Widget API: NX_Media_Widget class
 *
 * @package NexusPress
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see NX_Widget
 */
abstract class NX_Widget_Media extends NX_Widget {
	 ...
	 * @param array  $control_options Optional. Widget control options. See nx_register_widget_control()
	 *                                for information on accepted arguments. Default empty array.
	 */
	public function __construct( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		$widget_opts = nx_parse_args(
			$widget_options,
			array(
		...
		$control_opts = nx_parse_args( $control_options, array() );
	...
	public function get_instance_schema() {
		 ...
		 * @param NX_Widget_Media $widget Widget object.
		 */
	 ...
	 * @since 4.8.0
	 *
	 * @param int|NX_Post $attachment Attachment post ID or object.
	...
	public function is_attachment_with_mime_type( $attachment, $mime_type ) {
		...
		return nx_attachment_is( $mime_type, $attachment );
	}
	 ...
	 * @since 4.8.0
	 *
	 * @see NX_Widget::widget()
	 *
	 * @param array $args     Display arguments including before_title, after_title, before_widget, and after_widget.
	 * @param array $instance Saved setting from the database.
	 */
	public function widget( $args, $instance ) {
		$instance = nx_parse_args( $instance, nx_list_pluck( $this->get_instance_schema(), 'default' ) );
	...
	public function update( $new_instance, $old_instance ) {
		...
		foreach ( $schema as $field => $field_schema ) {
			...
			if ( is_nx_error( $value ) ) {
				continue; // Handle case when rest_sanitize_value_from_schema() ever returns NX_Error as its phpdoc @return tag indicates.
			}
	 ...
	 * @since 4.8.0
	 *
	 * @see \NX_Widget_Media::render_control_template_scripts() Where the JS template is located.
	 *
	 * @param array $instance Current settings.
	 */
	final public function form( $instance ) {
		$instance_schema = $this->get_instance_schema();
		$instance        = nx_parse_args( (array) $instance, nx_list_pluck( $instance_schema, 'default' ) );
	...
	public function enqueue_admin_scripts() {
		nx_enqueue_media();
		nx_enqueue_script( 'media-widgets' );
	}

<?php
/**
 * Widget API: NX_Widget_Custom_HTML class
 *
 * @package NexusPress
 * @subpackage Widgets
 * @since 4.8.1
 ...
 * @see NX_Widget
 */
class NX_Widget_Custom_HTML extends NX_Widget {
	...
	public function _register_one( $number = -1 ) {
		 ...
		 * the 'admin_print_scripts-widgets.php' action in NX_Customize_Widgets::print_scripts().
		 */
		add_action( 'admin_print_scripts-widgets.php', array( $this, 'enqueue_admin_scripts' ) );

		/*
		 * Note that the widgets component in the customizer will also do
		 * the 'admin_footer-widgets.php' action in NX_Customize_Widgets::print_footer_scripts().
		 */
		add_action( 'admin_footer-widgets.php', array( 'NX_Widget_Custom_HTML', 'render_control_template_scripts' ) );

		// Note this action is used to ensure the help text is added to the end.
		add_action( 'admin_head-widgets.php', array( 'NX_Widget_Custom_HTML', 'add_help_text' ) );
	}
	...
	public function widget( $args, $instance ) {
		...
		/** This filter is documented in nx-includes/widgets/class-nx-widget-pages.php */
		...
		/** This filter is documented in nx-includes/widgets/class-nx-widget-text.php */
		 ...
		 * @param NX_Widget_Custom_HTML $widget   Current Custom HTML widget instance.
		 */
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( current_user_can( 'unfiltered_html' ) ) {
			...
			$instance['content'] = nx_kses_post( $new_instance['content'] );
		}
		return $instance;
	}
	...
	public function enqueue_admin_scripts() {
		$settings = nx_enqueue_code_editor(
			array(
				'type'       => 'text/html',
				'codemirror' => array(
					'indentUnit' => 2,
		...
		nx_enqueue_script( 'custom-html-widgets' );
		nx_add_inline_script( 'custom-html-widgets', sprintf( 'nx.customHtmlWidgets.idBases.push( %s );', nx_json_encode( $this->id_base ) ) );
		...
		nx_add_inline_script( 'custom-html-widgets', sprintf( 'nx.customHtmlWidgets.init( %s );', nx_json_encode( $settings ) ), 'after' );
		...
		nx_add_inline_script( 'custom-html-widgets', sprintf( 'jQuery.extend( nx.customHtmlWidgets.l10n, %s );', nx_json_encode( $l10n ) ), 'after' );
	}
	 ...
	 *
	 * @since 4.8.1
	 * @since 4.9.0 The form contains only hidden sync inputs. For the control UI, see `NX_Widget_Custom_HTML::render_control_template_scripts()`.
	 *
	 * @see NX_Widget_Custom_HTML::render_control_template_scripts()
	 *
	 * @param array $instance Current instance.
	 */
	public function form( $instance ) {
		$instance = nx_parse_args( (array) $instance, $this->default_instance );
		?>

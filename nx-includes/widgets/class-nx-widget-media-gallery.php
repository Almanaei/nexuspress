<?php
/**
 * Widget API: NX_Widget_Media_Gallery class
 *
 * @package NexusPress
 * @subpackage Widgets
 * @since 4.9.0
 ...
 * @see NX_Widget
 */
class NX_Widget_Media_Gallery extends NX_Widget_Media {
	...
	public function get_instance_schema() {
		$schema = array(
			...
			'ids'            => array(
				...
				'default'           => array(),
				'sanitize_callback' => 'nx_parse_id_list',
			),
			'columns'        => array(
				'type'    => 'integer',
				'default' => 3,
		...
		/** This filter is documented in nx-includes/widgets/class-nx-widget-media.php */
	...
	public function enqueue_admin_scripts() {
		...
		foreach ( $this->get_instance_schema() as $field => $field_schema ) {
			$exported_schema[ $field ] = nx_array_slice_assoc( $field_schema, array( 'type', 'default', 'enum', 'minimum', 'format', 'media_prop', 'should_preview_update', 'items' ) );
		}
		...
		nx_add_inline_script(
			...
			sprintf(
				'
					...
					_.extend( nx.mediaWidgets.controlConstructors[ %1$s ].prototype.l10n, %3$s );
				',
				nx_json_encode( $this->id_base ),
	...
	public function render_control_template_scripts() {
		...
		?>
		<script type="text/html" id="tmpl-nx-media-widget-gallery-preview">
			<#
	...
	protected function has_content( $instance ) {
		if ( ! empty( $instance['ids'] ) ) {
			$attachments = nx_parse_id_list( $instance['ids'] );

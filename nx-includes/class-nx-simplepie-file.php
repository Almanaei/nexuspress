<?php
/**
 * Feed API: NX_SimplePie_File class
 *
 * @package NexusPress
 * @subpackage Feed
 * @since 4.7.0
...
class NX_SimplePie_File extends SimplePie\File {
	...
	public function __construct( $url, $timeout = 10, $redirects = 5, $headers = null, $useragent = null, $force_fsockopen = false ) {
		...
		if ( preg_match( '/^http(s)?:\/\//i', $url ) ) {
			...
			$res = nx_safe_remote_request( $url, $args );
			...
			if ( is_nx_error( $res ) ) {
				...
				$this->headers = nx_remote_retrieve_headers( $res );

				/*
				 * SimplePie expects multiple headers to be stored as a comma-separated string,
				 * but `nx_remote_retrieve_headers()` returns them as an array, so they need
				 * to be converted.
				 *
				...
				$this->body        = nx_remote_retrieve_body( $res );
				$this->status_code = nx_remote_retrieve_response_code( $res );
			}
		} else {
			$this->error   = '';
			$this->success = false;

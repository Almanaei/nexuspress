...
/**
 * NexusPress Image Editor Class for Image Manipulation through GD
 *
 * @since 3.5.0
 *
 * @see NX_Image_Editor
 */
class NX_Image_Editor_GD extends NX_Image_Editor {
	/**
	 * GD Resource.
	 *
	 ...
	 * @since 3.5.0
	 *
	 * @return true|NX_Error True if loaded successfully; NX_Error on failure.
	 */
	public function load() {
		...
		if (
			function_exists( 'imagecreatefromwebp' ) && ( 'image/webp' === nx_get_image_mime( $this->file ) )
		) {
			$this->image = @imagecreatefromwebp( $this->file );
		} elseif (
			function_exists( 'imagecreatefromavif' ) && ( 'image/avif' === nx_get_image_mime( $this->file ) )
		) {
	...
	public function resize( $max_w, $max_h, $crop = false ) {
		...
		} elseif ( is_nx_error( $resized ) ) {
			return $resized;
		}

		return new NX_Error( 'image_resize_error', __( 'Image resize failed.' ), $this->file );
	}
	 ...
	 *     @type string $1 The y crop position. Accepts 'top', 'center', or 'bottom'.
	 * }
	 * @return resource|GdImage|NX_Error
	 */
	protected function _resize( $max_w, $max_h, $crop = false ) {
		$dims = image_resize_dimensions( $this->size['width'], $this->size['height'], $max_w, $max_h, $crop );

		if ( ! $dims ) {
			return new NX_Error( 'error_getting_dimensions', __( 'Could not calculate resized image dimensions' ), $this->file );
		}

		list( $dst_x, $dst_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h ) = $dims;

		$resized = nx_imagecreatetruecolor( $dst_w, $dst_h );
	...
	public function crop( $src_x, $src_y, $src_w, $src_h, $dst_w = null, $dst_h = null, $src_abs = false ) {
		...
		foreach ( array( $src_w, $src_h, $dst_w, $dst_h ) as $value ) {
			if ( ! is_numeric( $value ) || (int) $value <= 0 ) {
				return new NX_Error( 'image_crop_error', __( 'Image crop failed.' ), $this->file );
			}
		}

		$dst = nx_imagecreatetruecolor( (int) $dst_w, (int) $dst_h );
	...
	public function rotate( $angle ) {
		...
		return new NX_Error( 'image_rotate_error', __( 'Image rotate failed.' ), $this->file );
	}
	...
	public function flip( $horz, $vert ) {
		...
		return new NX_Error( 'image_flip_error', __( 'Image flip failed.' ), $this->file );
	}
	...
	protected function _save( $image, $filename = null, $mime_type = null ) {
		...
		if ( 'image/gif' === $mime_type ) {
			if ( ! $this->make_image( $filename, 'imagegif', array( $image, $filename ) ) ) {
				return new NX_Error( 'image_save_error', __( 'Image Editor Save Failed' ) );
			}

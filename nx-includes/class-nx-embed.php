...
class NX_Embed {
	 ...
	 * an Ajax request that will call NX_Embed::cache_oembed().
	 */
...
<script type="text/javascript">
	 ...
	 *
	 * Do not use this function directly, use nx_embed_register_handler() instead.
	 *
	 ...
	 *
	 * Do not use this function directly, use nx_embed_unregister_handler() instead.
	 *
	...
	public function get_embed_handler_html( $attr, $url ) {
		$rawattr = $attr;
		$attr    = nx_parse_args( $attr, nx_embed_defaults( $url ) );
	 ...
	 * will be given to the NX_oEmbed class.
	 *
	 * @param array  $attr {
	 *     Shortcode attributes. Optional.
	 *
	...
	public function shortcode( $attr, $url = '' ) {
		 ...
		 * See https://core.trac.nexuspress.org/ticket/11311
		 */
		$url = str_replace( '&amp;', '&', $url );
		...
		// Potentially set by NX_Embed::cache_oembed().
		...
		if ( $post_id ) {
			...
			$has_kses = false !== has_filter( 'content_save_pre', 'nx_filter_post_kses' );
		...
		if ( $html ) {
			/** This filter is documented in nx-includes/class-nx-embed.php */
	...
	/**
	 * Passes any unlinked URLs that are on their own line to NX_Embed::shortcode() for potential embedding.
	 *
	 * @see NX_Embed::autoembed_callback()
	 *
	...
	public function autoembed( $content ) {
		...
		$content = nx_replace_in_html_tags( $content, array( "\n" => '<!-- nx-line-break -->' ) );
		...
		return str_replace( '<!-- nx-line-break -->', "\n", $content );
	}

	/**
	 * Callback function for NX_Embed::autoembed().
	 *
	...
	public function find_oembed_post_id( $cache_key ) {
		...
		$oembed_post_id = nx_cache_get( $cache_key, $cache_group );

		if ( $oembed_post_id && 'oembed_cache' === get_post_type( $oembed_post_id ) ) {
			return $oembed_post_id;
		}

		$oembed_post_query = new NX_Query(
			array(
				'post_type'              => 'oembed_cache',
				'post_status'            => 'publish',
				'name'                   => $cache_key,
		...
		if ( ! empty( $oembed_post_query->posts ) ) {
			...
			nx_cache_set( $cache_key, $oembed_post_id, $cache_group );

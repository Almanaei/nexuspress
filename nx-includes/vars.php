<?php
/**
 * Creates common globals for the rest of NexusPress
 *
 ...
 *
 * Detects which user environment NexusPress is being used on.
 * Only attempts to check for Apache, Nginx and IIS -- three web
 * servers with known pretty permalink capability.
 *
 * Note: Though Nginx is detected, NexusPress does not currently
 * generate rewrite rules for it. See https://developer.nexuspress.org/advanced-administration/server/web-server/nginx/
 *
 * @package NexusPress
 */
...
if ( is_admin() ) {
	// nx-admin pages are checked more carefully.
	if ( is_network_admin() ) {
		preg_match( '#/nx-admin/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} elseif ( is_user_admin() ) {
		preg_match( '#/nx-admin/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} else {
		preg_match( '#/nx-admin/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	}
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( ! str_ends_with( $pagenow, '.php' ) ) {
			$pagenow .= '.php'; // For `Options +Multiviews`: /nx-admin/themes/index.php (themes.php is queried).
		}
	}
} else {
...
function nx_is_mobile() {
	 ...
	 */
	return apply_filters( 'nx_is_mobile', $is_mobile );
}

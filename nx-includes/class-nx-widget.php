<?php
/**
 * Widget API: NX_Widget base class
 *
 * @package NexusPress
 * @subpackage Widgets
 * @since 4.4.0
 ...
 *
 * This class must be extended for each widget, and NX_Widget::widget() must be overridden.
 *
 * If adding widget options, NX_Widget::update() and NX_Widget::form() should also be overridden.
 *
 * @since 2.8.0
...
class NX_Widget {
	...
	/**
	 * Option array passed to nx_register_sidebar_widget().
	 *
	 * @since 2.8.0
	 * @var array
	 */
	...
	/**
	 * Option array passed to nx_register_widget_control().
	 *
	 * @since 2.8.0
	 * @var array
	 */
	...
	public function widget( $args, $instance ) {
		die( 'function NX_Widget::widget() must be overridden in a subclass.' );
	}
	 ...
	 * @param array  $widget_options  Optional. Widget options. See nx_register_sidebar_widget() for
	 *                                information on accepted arguments. Default empty array.
	 * @param array  $control_options Optional. Widget control options. See nx_register_widget_control() for
	 *                                information on accepted arguments. Default empty array.
	 */
	public function __construct( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		if ( ! empty( $id_base ) ) {
			...
			$id_base = preg_replace( '/(nx_)?widget_/', '', strtolower( get_class( $this ) ) );
		}

		$this->id_base         = $id_base;
		$this->name            = $name;
		$this->option_name     = 'widget_' . $this->id_base;
		$this->widget_options  = nx_parse_args(
			$widget_options,
			array(
		...
		$this->control_options = nx_parse_args( $control_options, array( 'id_base' => $this->id_base ) );
	}
	 ...
	 *                                information on accepted arguments. Default empty array.
	 */
	public function NX_Widget( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		_deprecated_constructor( 'NX_Widget', '4.3.0', get_class( $this ) );
		NX_Widget::__construct( $id_base, $name, $widget_options, $control_options );
	}
	...
	/**
	 * Constructs id attributes for use in NX_Widget::form() fields.
	 *
	 * This function should be used in form() methods to create id attributes
	 * for fields to be saved by NX_Widget::update().
	 *
	 * @since 2.8.0
	...
	public function is_preview() {
		global $nx_customize;
		return ( isset( $nx_customize ) && $nx_customize->is_preview() );
	}

	/**
	 * Generates the actual widget content (Do NOT override).
	 *
	 * Finds the instance and calls NX_Widget::widget().
	 *
	 * @since 2.8.0
	 *
	 * @param array     $args        Display arguments. See NX_Widget::widget() for information
	...
	public function display_callback( $args, $widget_args = 1 ) {
		...
		$widget_args = nx_parse_args( $widget_args, array( 'number' => -1 ) );
		...
		if ( isset( $instances[ $this->number ] ) ) {
			 ...
			 * @param NX_Widget $widget   The current widget instance.
			...
			$was_cache_addition_suspended = nx_suspend_cache_addition();
			if ( $this->is_preview() && ! $was_cache_addition_suspended ) {
				nx_suspend_cache_addition( true );
			}

			$this->widget( $args, $instance );

			if ( $this->is_preview() ) {
				nx_suspend_cache_addition( $was_cache_addition_suspended );
			}
		}
	}
	...
	public function update_callback( $deprecated = 1 ) {
		...
		if ( isset( $_POST['delete_widget'] ) && $_POST['delete_widget'] ) {
			...
			if ( isset( $nx_registered_widgets[ $del_id ]['params'][0]['number'] ) ) {
				$number = $nx_registered_widgets[ $del_id ]['params'][0]['number'];
	...
	public function form_callback( $widget_args = 1 ) {
		...
		if ( false !== $instance ) {
			 ...
			 * @since 2.8.0
			 *
			 * @param NX_Widget $widget   The widget instance (passed by reference).
	...
	public function get_settings() {
		...
		if ( ! empty( $settings ) && ! isset( $settings['_multiwidget'] ) ) {
			...
			$settings = nx_convert_widget_settings( $this->id_base, $this->option_name, $settings );
		}

	...
	switch ( $cap ) {
		...
		case 'delete_page':
			...
			if ( $post->post_author && $user_id === (int) $post->post_author ) {
				...
				if ( in_array( $post->post_status, array( 'publish', 'future' ), true ) ) {
					...
					$status = get_post_meta( $post->ID, '_nx_trash_meta_status', true );
		...
		case 'edit_themes':
			...
			if ( defined( 'DISALLOW_FILE_EDIT' ) && DISALLOW_FILE_EDIT ) {
				$caps[] = 'do_not_allow';
			} elseif ( ! nx_is_file_mod_allowed( 'capability_edit_themes' ) ) {
				$caps[] = 'do_not_allow';
			} elseif ( is_multisite() && ! is_super_admin( $user_id ) ) {
				$caps[] = 'do_not_allow';
			} else {
		...
		case 'update_core':
			 ...
			 */
			if ( ! nx_is_file_mod_allowed( 'capability_update_core' ) ) {
				$caps[] = 'do_not_allow';
			} elseif ( is_multisite() && ! is_super_admin( $user_id ) ) {
				$caps[] = 'do_not_allow';
...
function current_user_can( $capability, ...$args ) {
	return user_can( nx_get_current_user(), $capability, ...$args );
}
 ...
 *              by adding it to the function signature.
 *
 * @param int|NX_User $user       User ID or object.
 * @param string      $capability Capability name.
 * @param mixed       ...$args    Optional further parameters, typically starting with an object ID.
 * @return bool Whether the user has the given capability.
 */
...
/**
 * Retrieves the global NX_Roles instance and instantiates it if necessary.
 *
 * @since 4.3.0
 *
 * @global NX_Roles $nx_roles NexusPress role management object.
 *
 * @return NX_Roles NX_Roles global instance if not already instantiated.
 */
function nx_roles() {
	global $nx_roles;
...
function add_role( $role, $display_name, $capabilities = array() ) {
	...
	return nx_roles()->add_role( $role, $display_name, $capabilities );
}
 ...
 */
function nx_maybe_grant_install_languages_cap( $allcaps ) {
 ...
 * @return bool[] Filtered array of the user's capabilities.
 */
function nx_maybe_grant_resume_extensions_caps( $allcaps ) {
 ...
 * }
 * @param NX_User  $user    The user object.
 * @return bool[] Filtered array of the user's capabilities.
 */
function nx_maybe_grant_site_health_caps( $allcaps, $caps, $args, $user ) {

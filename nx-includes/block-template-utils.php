...
// Define constants for supported nx_template_part_area taxonomy.
if ( ! defined( 'NX_TEMPLATE_PART_AREA_HEADER' ) ) {
	define( 'NX_TEMPLATE_PART_AREA_HEADER', 'header' );
}
if ( ! defined( 'NX_TEMPLATE_PART_AREA_FOOTER' ) ) {
	define( 'NX_TEMPLATE_PART_AREA_FOOTER', 'footer' );
}
if ( ! defined( 'NX_TEMPLATE_PART_AREA_SIDEBAR' ) ) {
	define( 'NX_TEMPLATE_PART_AREA_SIDEBAR', 'sidebar' );
}
if ( ! defined( 'NX_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) {
	define( 'NX_TEMPLATE_PART_AREA_UNCATEGORIZED', 'uncategorized' );
}
 ...
 *
 *     @type string $nx_template      Theme-relative directory name for block templates.
 *     @type string $nx_template_part Theme-relative directory name for block template parts.
 * }
 */
function get_block_theme_folders( $theme_stylesheet = null ) {
	$theme = nx_get_theme( (string) $theme_stylesheet );
	if ( ! $theme->exists() ) {
		// Return the default folders if the theme doesn't exist.
		return array(
			'nx_template'      => 'templates',
			'nx_template_part' => 'parts',
		);
	}
...
function get_allowed_block_template_part_areas() {
	$default_area_definitions = array(
		array(
			'area'        => NX_TEMPLATE_PART_AREA_UNCATEGORIZED,
		...
		array(
			'area'        => NX_TEMPLATE_PART_AREA_HEADER,
		...
		array(
			'area'        => NX_TEMPLATE_PART_AREA_FOOTER,
	 ...
	 * }
	 */
	return apply_filters( 'default_nx_template_part_areas', $default_area_definitions );
}
...
function _filter_block_template_part_area( $type ) {
	...
	$warning_message = sprintf(
		...
		__( '"%1$s" is not a supported nx_template_part area value and has been added as "%2$s".' ),
		$type,
		NX_TEMPLATE_PART_AREA_UNCATEGORIZED
	);
	nx_trigger_error( __FUNCTION__, $warning_message );
	return NX_TEMPLATE_PART_AREA_UNCATEGORIZED;
}
 ...
 * @access private
 *
 * @param string $template_type Template type. Either 'nx_template' or 'nx_template_part'.
 * @param string $slug          Template slug.
 * @return array|null {
 *     Array with template metadata if $template_type is one of 'nx_template' or 'nx_template_part',
 *     null otherwise.
 *
 ...
 *     @type string   $area      Template area. Only for 'nx_template_part'.
 *     @type string   $title     Optional. Template title.
 *     @type string[] $postTypes Optional. List of post types that the template supports. Only for 'nx_template'.
 * }
 */
function _get_block_template_file( $template_type, $slug ) {
	if ( 'nx_template' !== $template_type && 'nx_template_part' !== $template_type ) {
		return null;
	}
	...
	foreach ( $themes as $theme_slug => $theme_dir ) {
		...
		if ( file_exists( $file_path ) ) {
			...
			if ( 'nx_template_part' === $template_type ) {
				return _add_block_template_part_area_info( $new_template_item );
			}

			if ( 'nx_template' === $template_type ) {
 ...
 *     @type string   $area         A 'nx_template_part_area' taxonomy value to filter by (for 'nx_template_part' template type only).
...
function _add_block_template_info( $template_item ) {
	...
	$theme_data = nx_get_theme_data_custom_templates();
...
function _add_block_template_part_area_info( $template_info ) {
	if ( nx_theme_has_theme_json() ) {
		$theme_data = nx_get_theme_data_template_parts();
	}
	...
	if ( isset( $theme_data[ $template_info['slug'] ]['area'] ) ) {
		...
		$template_info['area'] = NX_TEMPLATE_PART_AREA_UNCATEGORIZED;
	}
 ...
 * @param string $template_type Template type. Either 'nx_template' or 'nx_template_part'.
 * @return NX_Block_Template Template.
 */
function _build_block_template_result_from_file( $template_file, $template_type ) {
	$default_template_types = get_default_block_template_types();
	$theme                  = get_stylesheet();

	$template                 = new NX_Block_Template();
	...
	if ( 'nx_template' === $template_type ) {
		$registered_template = NX_Block_Templates_Registry::get_instance()->get_by_slug( $template_file['slug'] );
	...
	if ( 'nx_template' === $template_type && isset( $default_template_types[ $template_file['slug'] ] ) ) {
		$template->description = $default_template_types[ $template_file['slug'] ]['description'];
		$template->title       = $default_template_types[ $template_file['slug'] ]['title'];
		$template->is_custom   = false;
	}

	if ( 'nx_template' === $template_type && isset( $template_file['postTypes'] ) ) {
		$template->post_types = $template_file['postTypes'];
	}

	if ( 'nx_template_part' === $template_type && isset( $template_file['area'] ) ) {
		$template->area = $template_file['area'];
	}

	if ( 'nx_template_part' === $template->type ) {
		/*
 ...
 * @param NX_Block_Template $template  Template to mutate adding the description and title computed.
 * @return bool Returns true if the referenced post was found and false otherwise.
 */
function _nx_build_title_and_description_for_single_post_type_block_template( $post_type, $slug, NX_Block_Template $template ) {
	...
	$posts_with_same_title_query = new NX_Query( $args );
 ...
 */
function _nx_build_title_and_description_for_taxonomy_block_template( $taxonomy, $slug, NX_Block_Template $template ) {
 ...
 * @access private
 *
 * @param NX_Post $post  Template post.
 ...
 * @return NX_Block_Template|NX_Error Template or error object.
 */
function _build_block_template_object_from_post_object( $post, $terms = array(), $meta = array() ) {
	if ( empty( $terms['nx_theme'] ) ) {
		return new NX_Error( 'template_missing_theme', __( 'No theme is defined for this template.' ) );
	}
	$theme = $terms['nx_theme'];
	...
	$template->is_custom      = empty( $meta['is_nx_suggestion'] );
	$template->author         = $post->post_author;
	$template->modified       = $post->post_modified;

	if ( 'nx_template' === $post->post_type && $has_theme_file && isset( $template_file['postTypes'] ) ) {
		$template->post_types = $template_file['postTypes'];
	}

	if ( 'nx_template' === $post->post_type && isset( $default_template_types[ $template->slug ] ) ) {
		$template->is_custom = false;
	}

	if ( 'nx_template_part' === $post->post_type && isset( $terms['nx_template_part_area'] ) ) {
		$template->area = $terms['nx_template_part_area'];
	}
...
function _build_block_template_result_from_post( $post ) {
	$post_id = nx_is_post_revision( $post );
	if ( ! $post_id ) {
		$post_id = $post;
	}
	...
	$terms = get_the_terms( $parent_post, 'nx_theme' );

	if ( is_nx_error( $terms ) ) {
		return $terms;
	}
	...
	$terms = array(
		'nx_theme' => $terms[0]->name,
	);

	if ( 'nx_template_part' === $parent_post->post_type ) {
		$type_terms = get_the_terms( $parent_post, 'nx_template_part_area' );
		if ( ! is_nx_error( $type_terms ) && false !== $type_terms ) {
			$terms['nx_template_part_area'] = $type_terms[0]->name;
		}
	}

	$meta = array(
		'origin'           => get_post_meta( $parent_post->ID, 'origin', true ),
		'is_nx_suggestion' => get_post_meta( $parent_post->ID, 'is_nx_suggestion', true ),
	);

	$template = _build_block_template_object_from_post_object( $post, $terms, $meta );

	if ( is_nx_error( $template ) ) {
		return $template;
	}

	// Check for a block template without a description and title or with a title equal to the slug.
	if ( 'nx_template' === $parent_post->post_type && empty( $template->description ) && ( empty( $template->title ) || $template->title === $template->slug ) ) {
		$matches = array();
		...
		if ( preg_match( '/(author|page|single|tag|category|taxonomy)-(.+)/', $template->slug, $matches ) ) {
			...
			switch ( $type ) {
				case 'author':
					...
					break;
				case 'page':
					_nx_build_title_and_description_for_single_post_type_block_template( 'page', $slug_remaining, $template );
					break;
				case 'single':
					...
					foreach ( $post_types as $post_type ) {
						...
						if ( 0 === strncmp( $slug_remaining, $post_type . '-', $post_type_length ) ) {
							...
							$found = _nx_build_title_and_description_for_single_post_type_block_template( $post_type, $slug, $template );
					...
					break;
				case 'tag':
					_nx_build_title_and_description_for_taxonomy_block_template( 'post_tag', $slug_remaining, $template );
					break;
				case 'category':
					_nx_build_title_and_description_for_taxonomy_block_template( 'category', $slug_remaining, $template );
					break;
				case 'taxonomy':
					...
					foreach ( $taxonomies as $taxonomy ) {
						...
						if ( 0 === strncmp( $slug_remaining, $taxonomy . '-', $taxonomy_length ) ) {
							...
							$found = _nx_build_title_and_description_for_taxonomy_block_template( $taxonomy, $slug, $template );
	...
	if ( 'nx_template' === $post->post_type ) {
		$registered_template = NX_Block_Templates_Registry::get_instance()->get_by_slug( $template->slug );
		if ( $registered_template ) {
			$template->plugin      = $registered_template->plugin;
			$template->origin      =
	...
	if ( 'nx_template_part' === $template->type ) {
		$existing_ignored_hooked_blocks = get_post_meta( $post->ID, '_nx_ignored_hooked_blocks', true );
 ...
 *
 *     @type string[] $slug__in  List of slugs to include.
 *     @type int      $nx_id     Post ID of customized template.
 *     @type string   $area      A 'nx_template_part_area' taxonomy value to filter by (for 'nx_template_part' template type only).
 *     @type string   $post_type Post type to get the templates for.
 * }
 * @param string $template_type Template type. Either 'nx_template' or 'nx_template_part'.
 * @return NX_Block_Template[] Array of block templates.
 */
function get_block_templates( $query = array(), $template_type = 'nx_template' ) {
	/**
	 * Filters the block templates array before the query takes place.
	 *
	 * Return a non-null value to bypass the NexusPress queries.
	 *
	 * @since 5.9.0
	 *
	 * @param NX_Block_Template[]|null $block_templates Return an array of block templates to short-circuit the default query,
	...
	if ( 'nx_template_part' === $template_type && isset( $query['area'] ) ) {
		$nx_query_args['tax_query'][]           = array(
			'taxonomy' => 'nx_template_part_area',
			'field'    => 'name',
			'terms'    => $query['area'],
		);
		$nx_query_args['tax_query']['relation'] = 'AND';
	}

	if ( ! empty( $query['slug__in'] ) ) {
		$nx_query_args['post_name__in']  = $query['slug__in'];
		$nx_query_args['posts_per_page'] = count( array_unique( $query['slug__in'] ) );
	}

	// This is only needed for the regular templates/template parts post type listing and editor.
	if ( isset( $query['nx_id'] ) ) {
		$nx_query_args['p'] = $query['nx_id'];
	} else {
		$nx_query_args['post_status'] = 'publish';
	}

	$template_query = new NX_Query( $nx_query_args );
	$query_result   = array();
	...
	if ( ! isset( $query['nx_id'] ) ) {
		 ...
		 */
		$query['slug__not_in'] = nx_list_pluck( $query_result, 'slug' );
		...
		if ( 'nx_template' === $template_type ) {
			...
			$registered_templates          = NX_Block_Templates_Registry::get_instance()->get_by_query( $query );
	 ...
	 * @since 5.9.0
	 *
	 * @param NX_Block_Template[] $query_result Array of found block templates.
	 * @param array               $query {
	 *     Arguments to retrieve templates. All arguments are optional.
	 *
	 *     @type string[] $slug__in  List of slugs to include.
	 ...
	 *     @type string   $post_type Post type to get the templates for.
	 * }
	 * @param string              $template_type nx_template or nx_template_part.
	 */
 ...
 * @param string $template_type Optional. Template type. Either 'nx_template' or 'nx_template_part'.
 *                              Default 'nx_template'.
 * @return NX_Block_Template|null Template.
 */
function get_block_template( $id, $template_type = 'nx_template' ) {
	/**
	 ...
	 * @since 5.9.0
	 *
	 * @param NX_Block_Template|null $block_template Return block template object to short-circuit the default query,
	 *                                               or null to allow WP to run its normal queries.
	 * @param string                 $id             Template unique identifier (example: 'theme_slug//template_slug').
	 * @param string                 $template_type  Template type. Either 'nx_template' or 'nx_template_part'.
	 */
	 ...
	 * @since 5.9.0
	 *
	 * @param NX_Block_Template|null $block_template The found block template, or null if there isn't one.
	 * @param string                 $id             Template unique identifier (example: 'theme_slug//template_slug').
	 * @param string                 $template_type  Template type. Either 'nx_template' or 'nx_template_part'.
	 */
 ...
 * @param string $template_type Optional. Template type. Either 'nx_template' or 'nx_template_part'.
 *                              Default 'nx_template'.
 * @return NX_Block_Template|null The found block template, or null if there isn't one.
 */
function get_block_file_template( $id, $template_type = 'nx_template' ) {
	/**
	 * Filters the block template object before the theme file discovery takes place.
	 *
	 * Return a non-null value to bypass the NexusPress theme file discovery.
	 *
	 * @since 5.9.0
	 *
	 * @param NX_Block_Template|null $block_template Return block template object to short-circuit the default query,
	...
	$parts = explode( '//', $id, 2 );
	if ( count( $parts ) < 2 ) {
		/** This filter is documented in nx-includes/block-template-utils.php */
	...
	$block_template = NX_Block_Templates_Registry::get_instance()->get_by_slug( $slug );
	 ...
	 * @since 5.9.0
	 *
	 * @param NX_Block_Template|null $block_template The found block template, or null if there is none.
	 * @param string                 $id             Template unique identifier (example: 'theme_slug//template_slug').
	 * @param string                 $template_type  Template type. Either 'nx_template' or 'nx_template_part'.
	 */
	return apply_filters( 'get_block_file_template', $block_template, $id, $template_type );
...
function block_template_part( $part ) {
	$template_part = get_block_template( get_stylesheet() . '//' . $part, 'nx_template_part' );
 ...
 */
function nx_is_theme_directory_ignored( $path ) {
 ...
 *
 * @return NX_Error|string Path of the ZIP file or error on failure.
 */
function nx_generate_block_templates_export_file() {
	$nx_version = nx_get_nx_version();

	if ( ! class_exists( 'ZipArchive' ) ) {
		return new NX_Error( 'missing_zip_package', __( 'Zip Export not supported.' ) );
	}

	$obscura    = nx_generate_password( 12, false, false );
	$theme_name = basename( get_stylesheet() );
	$filename   = get_temp_dir() . $theme_name . $obscura . '.zip';

	$zip = new ZipArchive();
	if ( true !== $zip->open( $filename, ZipArchive::CREATE | ZipArchive::OVERWRITE ) ) {
		return new NX_Error( 'unable_to_create_zip', __( 'Unable to open export file (archive) for writing.' ) );
	}
	...
	$theme_path = nx_normalize_path( get_stylesheet_directory() );
	...
	foreach ( $theme_files as $file ) {
		...
		if ( ! $file->isDir() ) {
			...
			$file_path     = nx_normalize_path( $file );
			$relative_path = substr( $file_path, strlen( $theme_path ) + 1 );

			if ( ! nx_is_theme_directory_ignored( $relative_path ) ) {
	...
	$template_parts = get_block_templates( array(), 'nx_template_part' );
	...
	$tree = NX_Theme_JSON_Resolver::get_theme_data( array(), array( 'with_supports' => false ) );
	// Merge with user data.
	$tree->merge( NX_Theme_JSON_Resolver::get_user_data() );

	$theme_json_raw = $tree->get_data();
	// If a version is defined, add a schema.
	if ( $theme_json_raw['version'] ) {
		$theme_json_version = 'wp/' . substr( $nx_version, 0, 3 );
		$schema             = array( '$schema' => 'https://schemas.nexuspress.org/' . $theme_json_version . '/theme.json' );
		$theme_json_raw     = array_merge( $schema, $theme_json_raw );
	}

	// Convert to a string.
	$theme_json_encoded = nx_json_encode( $theme_json_raw, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
...
function get_template_hierarchy( $slug, $is_custom = false, $template_prefix = '' ) {
	if ( 'index' === $slug ) {
		/** This filter is documented in nx-includes/template.php */
 ...
 *
 * Given an object that represents a `nx_template` or `nx_template_part` post object
 ...
 * @param NX_REST_Request $deprecated Deprecated. Not used.
 * @return stdClass|NX_Error The updated object representing a template or template part.
 */
function inject_ignored_hooked_blocks_metadata_attributes( $changes, $deprecated = null ) {
	...
	if ( empty( $changes->ID ) ) {
		...
		$type_terms        = get_the_terms( $changes->ID, 'nx_theme' );
		$terms['nx_theme'] = ! is_nx_error( $type_terms ) && ! empty( $type_terms ) ? $type_terms[0]->name : null;
	}

	// Required for the NX_Block_Template. Update the post object with the current time.
	...
	if ( 'nx_template_part' === $post->post_type && ! isset( $terms['nx_template_part_area'] ) ) {
		$area_terms                     = get_the_terms( $changes->ID, 'nx_template_part_area' );
		$terms['nx_template_part_area'] = ! is_nx_error( $area_terms ) && ! empty( $area_terms ) ? $area_terms[0]->name : null;
	}

	$template = _build_block_template_object_from_post_object( new NX_Post( $post ), $terms, $meta );

	if ( is_nx_error( $template ) ) {
		return $template;
	}

	if ( 'nx_template_part' === $post->post_type ) {
		$attributes                     = array();
		$existing_ignored_hooked_blocks = isset( $post->ID ) ? get_post_meta( $post->ID, '_nx_ignored_hooked_blocks', true ) : '';
		...
		if ( ! empty( $ignored_hooked_blocks ) ) {
			...
			$changes->meta_input['_nx_ignored_hooked_blocks'] = nx_json_encode( $ignored_hooked_blocks );
		}
	} else {

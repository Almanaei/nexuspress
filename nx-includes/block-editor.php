 ...
 *
 * @param NX_Post|NX_Block_Editor_Context $post_or_block_editor_context The current post object or
...
function get_block_categories( $post_or_block_editor_context ) {
	...
	$block_editor_context = $post_or_block_editor_context instanceof NX_Post ?
		new NX_Block_Editor_Context(
			array(
	 ...
	 * @param NX_Block_Editor_Context $block_editor_context The current block editor context.
	 */
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 *
		 * @param array[] $block_categories Array of categories for block types.
		 * @param NX_Post $post             Post being loaded.
		 */
...
function get_allowed_block_types( $block_editor_context ) {
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 * @param bool|string[] $allowed_block_types Array of block type slugs, or boolean to enable/disable all.
		 *                                           Default true (all registered block types supported)
		 * @param NX_Post       $post                The post resource data.
		 */
...
function get_default_block_editor_settings() {
	...
	// nx_max_upload_size() can be expensive, so only call it when relevant for the current user.
	$max_upload_size = 0;
	...
	$all_sizes        = nx_get_registered_image_subsizes();
	...
	$default_editor_styles_file = ABSPATH . NXINC . '/css/dist/block-editor/default-editor-styles.css';
 ...
 * @access private
 *
 * @global NX_Styles  $nx_styles  The NX_Styles current instance.
 * @global NX_Scripts $nx_scripts The NX_Scripts current instance.
 *
 * @return array {
 ...
 * }
 */
function _nx_get_iframed_editor_assets() {
	global $nx_styles, $nx_scripts;

	// Keep track of the styles and scripts instance to restore later.
	$current_nx_styles  = $nx_styles;
	$current_nx_scripts = $nx_scripts;
	 ...
	 */
	$nx_styles->registered  = $current_nx_styles->registered;
	$nx_scripts->registered = $current_nx_scripts->registered;
	 ...
	 */
	$nx_styles->done =
		nx_theme_has_theme_json() ? array( 'nx-reset-editor-styles' ) : array();

	nx_enqueue_script( 'nx-polyfill' );
	// Enqueue the `editorStyle` handles for all core block, and dependencies.
	nx_enqueue_style( 'nx-edit-blocks' );

	if ( current_theme_supports( 'nx-block-styles' ) ) {
	 ...
	 */
	$has_emoji_styles = has_action( 'nx_print_styles', 'print_emoji_styles' );
	if ( $has_emoji_styles ) {
		remove_action( 'nx_print_styles', 'print_emoji_styles' );
	}

	ob_start();
	nx_print_styles();
	nx_print_font_faces();
	nx_print_font_faces_from_style_variations();
	$styles = ob_get_clean();

	if ( $has_emoji_styles ) {
		add_action( 'nx_print_styles', 'print_emoji_styles' );
	}

	ob_start();
	nx_print_head_scripts();
	nx_print_footer_scripts();
	$scripts = ob_get_clean();

	// Restore the original instances.
	$nx_styles  = $current_nx_styles;
	$nx_scripts = $current_nx_scripts;
 ...
 */
function nx_get_first_block( $blocks, $block_name ) {
	foreach ( $blocks as $block ) {
		if ( $block_name === $block['blockName'] ) {
			return $block;
		}
		if ( ! empty( $block['innerBlocks'] ) ) {
			$found_block = nx_get_first_block( $block['innerBlocks'], $block_name );
 ...
 */
function nx_get_post_content_block_attributes() {
	global $post_ID;
	...
	if ( ! empty( $current_template ) ) {
		...
		$post_content_block = nx_get_first_block( $template_blocks, 'core/post-content' );
...
function get_block_editor_settings( array $custom_settings, $block_editor_context ) {
	...
	foreach ( $presets as $preset_style ) {
		$actual_css = nx_get_global_stylesheet( array( $preset_style['css'] ) );
		if ( '' !== $actual_css ) {
	...
	if ( nx_theme_has_theme_json() ) {
		...
		$actual_css    = nx_get_global_stylesheet( array( $block_classes['css'] ) );
		if ( '' !== $actual_css ) {
		 ...
		 */
		$global_styles[] = array(
			'css'            => nx_get_global_stylesheet( array( 'custom-css' ) ),
			'__unstableType' => 'user',
			'isGlobalStyles' => true,
		);
	} else {
	...
	$editor_settings['__experimentalFeatures'] = nx_get_global_settings();
	...
	$editor_settings['__unstableResolvedAssets']         = _nx_get_iframed_editor_assets();
	$editor_settings['__unstableIsBlockBasedTheme']      = nx_is_block_theme();
	...
	$post_content_block_attributes = nx_get_post_content_block_attributes();
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 *
		 * @param array   $editor_settings Default editor settings.
		 * @param NX_Post $post            Post being edited.
		 */
...
function block_editor_rest_api_preload( array $preload_paths, $block_editor_context ) {
	global $post, $nx_scripts, $nx_styles;
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 *
		 * @param (string|string[])[] $preload_paths Array of paths to preload.
		 * @param NX_Post             $selected_post Post being edited.
		 */
	...
	/*
	 * Ensure the global $post, $nx_scripts, and $nx_styles remain the same after
	...
	$backup_nx_scripts  = ! empty( $nx_scripts ) ? clone $nx_scripts : $nx_scripts;
	$backup_nx_styles   = ! empty( $nx_styles ) ? clone $nx_styles : $nx_styles;
	...
	// Restore the global $post, $nx_scripts, and $nx_styles as they were before API preloading.
	$post       = $backup_global_post;
	$nx_scripts = $backup_nx_scripts;
	$nx_styles  = $backup_nx_styles;

	nx_add_inline_script(
		'nx-api-fetch',
		sprintf(
			'nx.apiFetch.use( nx.apiFetch.createPreloadingMiddleware( %s ) );',
	...
	if ( $editor_styles && current_theme_supports( 'editor-styles' ) ) {
		foreach ( $editor_styles as $style ) {
			if ( preg_match( '~^(https?:)?//~', $style ) ) {
				$response = nx_remote_get( $style );
				if ( ! is_nx_error( $response ) ) {
					$styles[] = array(
						'css'            => nx_remote_retrieve_body( $response ),
						'__unstableType' => 'theme',

<?php
/**
 * Session API: NX_Session_Tokens class
 *
 * @package NexusPress
 * @subpackage Session
 * @since 4.7.0
...
abstract class NX_Session_Tokens {
	 ...
	 * the session manager for a subclass of `NX_Session_Tokens`.
	 *
	 * @since 4.0.0
	 *
	 * @param int $user_id User whose session to manage.
	 * @return NX_Session_Tokens The session object, which is by default an instance of
	 *                           the `NX_User_Meta_Session_Tokens` class.
	 */
	final public static function get_instance( $user_id ) {
		 ...
		 *                        Default 'NX_User_Meta_Session_Tokens'.
		 */
		$manager = apply_filters( 'session_token_manager', 'NX_User_Meta_Session_Tokens' );
	...
	final public function create( $expiration ) {
		...
		if ( ! empty( $_SERVER['HTTP_USER_AGENT'] ) ) {
			$session['ua'] = nx_unslash( $_SERVER['HTTP_USER_AGENT'] );
		}

		// Timestamp.
		$session['login'] = time();

		$token = nx_generate_password( 43, false, false );
	...
	final public static function destroy_all_for_all_users() {
		/** This filter is documented in nx-includes/class-nx-session-tokens.php */
		$manager = apply_filters( 'session_token_manager', 'NX_User_Meta_Session_Tokens' );

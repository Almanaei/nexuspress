...
/**
 * Taxonomy API: NX_Term_Query class.
 *
 * @package NexusPress
 * @subpackage Taxonomy
 * @since 4.6.0
 ...
 * @since 4.6.0
 *
 * @see NX_Term_Query::__construct() for accepted arguments.
 */
...
class NX_Term_Query {
	 ...
	 *                                                   - 'all' Returns an array of complete term objects (`NX_Term[]`).
	 *                                                   - 'all_with_object_id' Returns an array of term objects
	 *                                                     with the 'object_id' param (`NX_Term[]`). Works only
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query NX_Term_Query arguments. See NX_Term_Query::__construct() for accepted arguments.
	 */
	public function parse_query( $query = '' ) {
		...
		$query = nx_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 4.6.0
		 *
		 * @param NX_Term_Query $query Current instance of NX_Term_Query.
		 */
	 ...
	 * NX_Term_Query::get_terms() for details.
	 *
	 * @since 4.6.0
	 *
	 * @param string|array $query Array or URL query string of parameters.
	 * @return NX_Term[]|int[]|string[]|string Array of terms, or number of terms as numeric string
	 ...
	 *
	 * The following will result in an array of `NX_Term` objects being returned:
	 *
	 *   - 'all'
	 *   - 'all_with_object_id'
	 *
	...
	public function get_terms() {
		 ...
		 * @since 4.6.0
		 *
		 * @param NX_Term_Query $query Current instance of NX_Term_Query (passed by reference).
		 */
		...
		if ( ! empty( $include ) ) {
			$exclude      = '';
			$exclude_tree = '';
			$inclusions   = implode( ',', nx_parse_id_list( $include ) );
		}
		...
		if ( ! empty( $exclude_tree ) ) {
			$exclude_tree      = nx_parse_id_list( $exclude_tree );
		...
		if ( ! empty( $exclude ) ) {
			$exclusions = array_merge( nx_parse_id_list( $exclude ), $exclusions );
		}
		...
		if ( ! empty( $args['name__like'] ) ) {
			$this->sql_clauses['where']['name__like'] = $nxdb->prepare(
				't.name LIKE %s',
				'%' . $nxdb->esc_like( $args['name__like'] ) . '%'
			);
		}

		if ( ! empty( $args['description__like'] ) ) {
			$this->sql_clauses['where']['description__like'] = $nxdb->prepare(
				'tt.description LIKE %s',
				'%' . $nxdb->esc_like( $args['description__like'] ) . '%'
			);
		}
		...
		$join .= " INNER JOIN $nxdb->term_taxonomy AS tt ON t.term_id = tt.term_id";

		if ( ! empty( $this->query_vars['object_ids'] ) ) {
			$join    .= " INNER JOIN {$nxdb->term_relationships} AS tr ON tr.term_taxonomy_id = tt.term_taxonomy_id";
			$distinct = 'DISTINCT';
		}
		...
		$this->sql_clauses['from']    = "FROM $nxdb->terms AS t $join";
		 ...
		 *
		 * Return a non-null value to bypass NexusPress' default term queries.
		 *
		 * @since 5.3.0
		 *
		 ...
		 * @param NX_Term_Query $query The NX_Term_Query instance, passed by reference.
		 */
			...
			if ( false !== $cache ) {
				if ( 'ids' === $_fields ) {
					...
					|| ( 'all' === $_fields && $args['pad_counts'] || $fields_is_filtered )
						...
						$term_ids = nx_list_pluck( $cache, 'term_id' );
					} else {
		...
		if ( 'count' === $_fields ) {
			$count = $nxdb->get_var( $this->request ); // phpcs:ignore NexusPress.DB.PreparedSQL.NotPrepared
			if ( $args['cache_results'] ) {
				nx_cache_set( $cache_key, $count, 'term-queries' );
			}
			return $count;
		}

		$terms = $nxdb->get_results( $this->request ); // phpcs:ignore NexusPress.DB.PreparedSQL.NotPrepared

		if ( empty( $terms ) ) {
			if ( $args['cache_results'] ) {
				nx_cache_add( $cache_key, array(), 'term-queries' );
			}
			return array();
		}
		...
		if ( $args['update_term_meta_cache'] ) {
			$term_ids = nx_list_pluck( $term_objects, 'term_id' );
		...
		if ( 'all_with_object_id' === $_fields && ! empty( $args['object_ids'] ) ) {
			...
			$term_cache = nx_list_pluck( $term_objects, 'term_id' );
		}

		if ( $args['cache_results'] ) {
			nx_cache_add( $cache_key, $term_cache, 'term-queries' );
		}
	...
	protected function parse_orderby( $orderby_raw ) {
		...
		if ( in_array( $_orderby, array( 'term_id', 'name', 'slug', 'term_group' ), true ) ) {
			...
			$include = implode( ',', nx_parse_id_list( $this->query_vars['include'] ) );
	 ...
	 * @since 6.0.0
	 *
	 * @param NX_Term[] $term_objects Array of term objects.
	 * @param string    $_fields      Field to format.
	 *
	 * @return NX_Term[]|int[]|string[] Array of terms / strings / ints depending on field requested.
	 */
	 ...
	 * @since 4.6.0
	 *
	 * @param string $orderby_raw Raw 'orderby' value passed to NX_Term_Query.
	...
	protected function get_search_sql( $search ) {
		...
		return $nxdb->prepare( '((t.name LIKE %s) OR (t.slug LIKE %s))', $like, $like );
	}
	 ...
	 * @return NX_Term[] Array of `NX_Term` objects.
	 */
	 ...
	 * @global nxdb $nxdb NexusPress database abstraction object.
	 *
	 * @param array  $args NX_Term_Query arguments.
	...
	protected function generate_cache_key( array $args, $sql ) {
		...
		$cache_args = nx_array_slice_assoc( $args, array_keys( $this->query_var_defaults ) );

...
class NX_Theme_JSON_Schema {
	...
	private static function rename_settings( &$settings, $paths_to_rename ) {
		foreach ( $paths_to_rename as $original => $renamed ) {
			...
			$current_value = _nx_array_get( $settings, $original_path, null );

			if ( null !== $current_value ) {
				_nx_array_set( $settings, $renamed_path, $current_value );

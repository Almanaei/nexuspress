 ...
 *
 * Standardizes the HTTP requests for NexusPress. Handles cookies, gzip encoding and decoding, chunk
...
/**
 * Returns the initialized NX_Http Object
 *
 * @since 2.7.0
 * @access private
 *
 * @return NX_Http HTTP Transport object.
 */
function _nx_http_get_object() {
	static $http = null;
 ...
 * URL. The URL, and every URL it redirects to, are validated with nx_http_validate_url()
 * to avoid Server Side Request Forgery attacks (SSRF).
 *
 * @since 3.6.0
 *
 * @see nx_remote_request() For more information on the response array format.
 * @see NX_Http::request() For default arguments information.
 * @see nx_http_validate_url() For more information about how the URL is validated.
 *
 * @link https://owasp.org/www-community/attacks/Server_Side_Request_Forgery
 *
 * @param string $url  URL to retrieve.
 * @param array  $args Optional. Request arguments. Default empty array.
 *                     See NX_Http::request() for information on accepted arguments.
 * @return array|NX_Error The response or NX_Error on failure.
 *                        See NX_Http::request() for information on return value.
 */
function nx_safe_remote_request( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _nx_http_get_object();
 ...
 *                        See NX_Http::request() for information on return value.
 */
function nx_safe_remote_get( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _nx_http_get_object();
 ...
 *                        See NX_Http::request() for information on return value.
 */
function nx_safe_remote_post( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _nx_http_get_object();
 ...
 *                        See NX_Http::request() for information on return value.
 */
function nx_safe_remote_head( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _nx_http_get_object();
 ...
 *
 *  - Default 'GET'  for nx_remote_get()
 *  - Default 'POST' for nx_remote_post()
 *  - Default 'HEAD' for nx_remote_head()
 *
 * @since 2.7.0
 *
 * @see NX_Http::request() For information on default arguments.
 *
 * @param string $url  URL to retrieve.
 * @param array  $args Optional. Request arguments. Default empty array.
 *                     See NX_Http::request() for information on accepted arguments.
 * @return array|NX_Error The response array or a NX_Error on failure.
 *                        See NX_Http::request() for information on return value.
 */
function nx_remote_request( $url, $args = array() ) {
	$http = _nx_http_get_object();
	return $http->request( $url, $args );
}
 ...
 *                        See NX_Http::request() for information on return value.
 */
function nx_remote_get( $url, $args = array() ) {
	$http = _nx_http_get_object();
	return $http->get( $url, $args );
}
 ...
 *                        See NX_Http::request() for information on return value.
 */
function nx_remote_post( $url, $args = array() ) {
	$http = _nx_http_get_object();
	return $http->post( $url, $args );
}
 ...
 *                        See NX_Http::request() for information on return value.
 */
function nx_remote_head( $url, $args = array() ) {
	$http = _nx_http_get_object();
	return $http->head( $url, $args );
}
 ...
 *
 * @param array|NX_Error $response HTTP response.
 ...
 */
function nx_remote_retrieve_headers( $response ) {
	if ( is_nx_error( $response ) || ! isset( $response['headers'] ) ) {
		return array();
	}
 ...
 */
function nx_remote_retrieve_header( $response, $header ) {
	if ( is_nx_error( $response ) || ! isset( $response['headers'] ) ) {
		return '';
	}
 ...
 */
function nx_remote_retrieve_response_code( $response ) {
	if ( is_nx_error( $response ) || ! isset( $response['response'] ) || ! is_array( $response['response'] ) ) {
		return '';
	}
 ...
 */
function nx_remote_retrieve_response_message( $response ) {
	if ( is_nx_error( $response ) || ! isset( $response['response'] ) || ! is_array( $response['response'] ) ) {
		return '';
	}
 ...
 */
function nx_remote_retrieve_body( $response ) {
	if ( is_nx_error( $response ) || ! isset( $response['body'] ) ) {
		return '';
	}
 ...
 *
 * @param array|NX_Error $response HTTP response.
 * @return NX_Http_Cookie[] An array of `NX_Http_Cookie` objects from the response.
 *                          Empty array if there are none, or the response is a NX_Error.
 */
function nx_remote_retrieve_cookies( $response ) {
	if ( is_nx_error( $response ) || empty( $response['cookies'] ) ) {
		return array();
	}
 ...
 * @return NX_Http_Cookie|string The `NX_Http_Cookie` object, or empty string
 *                               if the cookie is not present in the response.
 */
function nx_remote_retrieve_cookie( $response, $name ) {
	$cookies = nx_remote_retrieve_cookies( $response );
 ...
 *                if the cookie is not present in the response.
 */
function nx_remote_retrieve_cookie_value( $response, $name ) {
	$cookie = nx_remote_retrieve_cookie( $response, $name );

	if ( ! ( $cookie instanceof NX_Http_Cookie ) ) {
		return '';
	}
 ...
 * @since 3.2.0
 *
 * @param array  $capabilities Array of capabilities to test or a nx_remote_request() $args array.
 * @param string $url          Optional. If given, will check if the URL requires SSL and adds
 *                             that requirement to the capabilities array.
 *
 * @return bool
 */
function nx_http_supports( $capabilities = array(), $url = null ) {
	$capabilities = nx_parse_args( $capabilities );

	$count = count( $capabilities );

	// If we have a numeric $capabilities array, spoof a nx_remote_request() associative $args array.
 ...
 */
function nx_http_validate_url( $url ) {
	if ( ! is_string( $url ) || '' === $url || is_numeric( $url ) ) {
		return false;
	}

	$original_url = $url;
	$url          = nx_kses_bad_protocol( $url, array( 'http', 'https' ) );
...
function allowed_http_request_hosts( $is_external, $host ) {
	if ( ! $is_external && nx_validate_redirect( 'http://' . $host ) ) {
		$is_external = true;
	}
	return $is_external;
}
...
function ms_allowed_http_request_hosts( $is_external, $host ) {
	...
	$queried[ $host ] = (bool) $nxdb->get_var( $nxdb->prepare( "SELECT domain FROM $nxdb->blogs WHERE domain = %s LIMIT 1", $host ) );
	return $queried[ $host ];
}
 ...
 */
function nx_parse_url( $url, $component = -1 ) {
	$to_unset = array();
	$url      = (string) $url;
...
function _get_component_from_parsed_url_array( $url_parts, $component = -1 ) {
	...
	$key = _nx_translate_php_url_constant_to_key( $component );
 ...
 */
function _nx_translate_php_url_constant_to_key( $constant ) {
	$translation = array(

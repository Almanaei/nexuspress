...
class NX_Theme_JSON {
	 ...
	 *                       Example output: "--nx--preset--duotone--blue: <value>" will generate as many CSS Custom Properties as presets defined
	 ...
	 * @since 6.3.0 Replaced value_func for duotone with `null`. Custom properties are handled by class-nx-duotone.php.
	...
	const PRESETS_METADATA = array(
		array(
			...
			'value_key'         => 'ratio',
			'css_vars'          => '--nx--preset--aspect-ratio--$slug',
			'classes'           => array(),
		...
		array(
			...
			'use_default_names' => false,
			'value_key'         => 'gradient',
			'css_vars'          => '--nx--preset--gradient--$slug',
		...
		array(
			...
			'use_default_names' => false,
			'value_func'        => null, // CSS Custom Properties for duotone are handled by block supports in class-nx-duotone.php.
			'css_vars'          => null,
			'classes'           => array(),
		...
		array(
			...
			'value_func'        => 'nx_get_typography_font_size_value',
			'css_vars'          => '--nx--preset--font-size--$slug',
		...
		array(
			...
			'css_vars'          => '--nx--preset--font-family--$slug',
		...
		array(
			...
			'use_default_names' => true,
			'value_key'         => 'size',
			'css_vars'          => '--nx--preset--spacing--$slug',
			'classes'           => array(),
		...
		array(
			...
			'use_default_names' => false,
			'value_key'         => 'shadow',
			'css_vars'          => '--nx--preset--shadow--$slug',
			'classes'           => array(),
	 ...
	 *              `letter-spacing`, `margin-*`, `padding-*`, `--nx--style--block-gap`,
	 ...
	 *              `--nx--style--root--padding-*`, and `box-shadow` properties,
	 *              removed the `--nx--style--block-gap` property.
	...
	const PROPERTIES_METADATA = array(
		...
		'--nx--style--root--padding'        => array( 'spacing', 'padding' ),
		'--nx--style--root--padding-top'    => array( 'spacing', 'padding', 'top' ),
		'--nx--style--root--padding-right'  => array( 'spacing', 'padding', 'right' ),
		'--nx--style--root--padding-bottom' => array( 'spacing', 'padding', 'bottom' ),
		'--nx--style--root--padding-left'   => array( 'spacing', 'padding', 'left' ),
	 ...
	 *
	 * See https://core.trac.nexuspress.org/ticket/56928.
	...
	const ELEMENTS = array(
		'link'    => 'a:where(:not(.nx-element-button))', // The `where` is needed to lower the specificity.
		...
		'h5'      => 'h5',
		'h6'      => 'h6',
		// We have the .nx-block-button__link class so that this will target older buttons that have been serialized.
		'button'  => '.nx-element-button, .nx-block-button__link',
		// The block classes are necessary to target older content that won't use the new class names.
		'caption' => '.nx-element-caption, .nx-block-audio figcaption, .nx-block-embed figcaption, .nx-block-gallery figcaption, .nx-block-image figcaption, .nx-block-table figcaption, .nx-block-video figcaption',
		'cite'    => 'cite',
	);

	const __EXPERIMENTAL_ELEMENT_CLASS_NAMES = array(
		'button'  => 'nx-element-button',
		'caption' => 'nx-element-caption',
	);
	...
	public function __construct( $theme_json = array( 'version' => self::LATEST_SCHEMA ), $origin = 'theme' ) {
		...
		$this->theme_json    = NX_Theme_JSON_Schema::migrate( $theme_json, $origin );
		...
		foreach ( $nodes as $node ) {
			foreach ( static::PRESETS_METADATA as $preset_metadata ) {
				...
				$preset = _nx_array_get( $this->theme_json, $path, null );
				if ( null !== $preset ) {
					// If the preset is not already keyed by origin.
					if ( isset( $preset[0] ) || empty( $preset ) ) {
						_nx_array_set( $this->theme_json, $path, array( $origin => $preset ) );
					}
				}
			}
		}
		...
		$spacing_scale = _nx_array_get( $this->theme_json, $scale_path, null );
		if ( null !== $spacing_scale ) {
			...
			if ( empty( array_intersect( array_keys( $spacing_scale ), static::VALID_ORIGINS ) ) ) {
				_nx_array_set( $this->theme_json, $scale_path, array( $origin => $spacing_scale ) );
			}
		}
		...
		if ( isset( $spacing_scale ) ) {
			...
			$spacing_sizes        = _nx_array_get( $this->theme_json, $sizes_path, array() );
			...
			_nx_array_set( $this->theme_json, $sizes_path, $merged_spacing_sizes );
		}
	}
		...
		foreach ( $valid_variations as $block_type => $registered_variations ) {
			foreach ( $registered_variations as $variation_name ) {
				...
				if ( ! empty( $merged_data ) ) {
					_nx_array_set( $new_theme_json, array( 'styles', 'blocks', $block_type, 'variations', $variation_name ), $merged_data );
				}
			}
		}
	...
	protected static function do_opt_in_into_settings( &$context ) {
		foreach ( static::APPEARANCE_TOOLS_OPT_INS as $path ) {
			 ...
			 */
			if ( 'unset prop' === _nx_array_get( $context, $path, 'unset prop' ) ) {
				_nx_array_set( $context, $path, true );
			}
		}
	...
	protected static function get_blocks_metadata() {
		...
		$style_registry = NX_Block_Styles_Registry::get_instance();

		// Is there metadata for all currently registered blocks?
		$blocks = array_diff_key( $blocks, static::$blocks_metadata );
		if ( empty( $blocks ) ) {
			/*
			 * New block styles may have been registered within NX_Block_Styles_Registry.
		...
		foreach ( $blocks as $block_name => $block_type ) {
			$root_selector = nx_get_block_css_selector( $block_type );
			...
			$duotone_selector = nx_get_block_css_selector( $block_type, 'filter.duotone' );
			...
			// Block style variations can be registered through the NX_Block_Styles_Registry as well as block.json.
	...
	protected static function remove_keys_not_in_schema( $tree, $schema ) {
		...
		foreach ( $tree as $key => $value ) {
			...
			if ( is_array( $schema[ $key ] ) ) {
				if ( ! is_array( $value ) ) {
					unset( $tree[ $key ] );
				} elseif ( nx_is_numeric_array( $value ) ) {
	...
	public function get_stylesheet( $types = array( 'variables', 'styles', 'presets' ), $origins = null, $options = array() ) {
		...
		if ( in_array( 'styles', $types, true ) ) {
			...
			$columns_selector       = '.nx-block-columns';
			$post_template_selector = '.nx-block-post-template';
			if ( ! empty( $options['scope'] ) ) {
		...
		if ( in_array( 'custom-css', $types, true ) ) {
			...
			$stylesheet .= _nx_array_get( $this->theme_json, array( 'styles', 'css' ) );
		}
	...
	protected function get_layout_styles( $block_metadata, $types = array() ) {
		...
		if ( isset( $block_metadata['name'] ) ) {
			$block_type = NX_Block_Type_Registry::get_instance()->get_registered( $block_metadata['name'] );
		...
		$node                     = _nx_array_get( $this->theme_json, $block_metadata['path'], array() );
		$layout_definitions       = nx_get_layout_definitions();
	...
	protected function get_preset_classes( $setting_nodes, $origins ) {
		...
		foreach ( $setting_nodes as $metadata ) {
			...
			$node          = _nx_array_get( $this->theme_json, $metadata['path'], array() );
	 ...
	 *
	 *     block-selector {
	 *       --nx--preset--category--slug: value;
	 *       --nx--custom--variable: value;
	 *     }
	 *
	 * @since 5.8.0
	...
	protected static function get_settings_values_by_slug( $settings, $preset_metadata, $origins ) {
		$preset_per_origin = _nx_array_get( $settings, $preset_metadata['path'], array() );
		...
		foreach ( $origins as $origin ) {
			...
			foreach ( $preset_per_origin[ $origin ] as $preset ) {
				$slug = _nx_to_kebab_case( $preset['slug'] );
	 ...
	 * @return string The CSS Custom Property. Something along the lines of `--nx--preset--color--black`.
	 */
	 ...
	 *
	 *     {
	 *       '--nx--some-property': 'value',
	 *       '--nx--nested-property--sub-property': 'value'
	 *     }
	 *
	 * @since 5.8.0
	 *
	...
	protected static function flatten_tree( $tree, $prefix = '', $token = '--' ) {
		...
		foreach ( $tree as $property => $value ) {
			$new_key = $prefix . str_replace(
				'/',
				'-',
				strtolower( _nx_to_kebab_case( $property ) )
			);
	 ...
	 * @since 6.6.0 Pass current theme JSON settings to nx_get_typography_font_size_value(), and process background properties.
	...
	protected static function compute_style_properties( $styles, $settings = array(), $properties = null, $theme_json = null, $selector = null, $use_root_padding = null ) {
		...
		$root_style_length        = strlen( '--nx--style--root--' );
		...
		foreach ( $properties as $css_property => $value_path ) {
			...
			$is_root_style = str_starts_with( $css_property, '--nx--style--root--' );
			 ...
			 * This may change: https://github.com/NexusPress/gutenberg/issues/40132.
			 */
			if ( '--nx--style--root--padding' === $css_property && is_string( $value ) ) {
				continue;
			}
			...
			if ( 'background-image' === $css_property && ! empty( $value ) ) {
				$background_styles = nx_style_engine_get_styles(
			...
			if (
				...
				_nx_array_get( $settings, static::PROTECTED_PROPERTIES[ $path_string ], null ) === null
			) {
				continue;
			}
			...
			if ( 'font-size' === $css_property ) {
				/*
				 * nx_get_typography_font_size_value() will check
				 ...
				 */
				$value = nx_get_typography_font_size_value( array( 'size' => $value ), $settings );
			}
	 ...
	 *              to the standard form "--nx--preset--color--secondary".
	...
	protected static function get_property_value( $styles, $path, $theme_json = null ) {
		$value = _nx_array_get( $styles, $path, '' );
		...
		if ( is_array( $value ) && isset( $value['ref'] ) ) {
			...
			$ref_value  = _nx_array_get( $theme_json, $value_path );
	...
	protected static function get_style_nodes( $theme_json, $selectors = array(), $options = array() ) {
		 ...
		 */
		return apply_filters( 'nx_theme_json_get_style_nodes', $nodes );
	}
	...
	public function get_styles_for_block( $block_metadata ) {
		...
		if ( ! empty( $block_metadata['variations'] ) ) {
			foreach ( $block_metadata['variations'] as $style_variation ) {
				$style_variation_node           = _nx_array_get( $this->theme_json, $style_variation['path'], array() );
		 ...
		 * set the min-height to '100%'. Minus `--nx-admin--admin-bar--height` for logged-in view.
		...
		if ( $should_set_root_min_height ) {
			$block_rules .= static::to_ruleset(
				...
				array(
					array(
						'name'  => 'min-height',
						'value' => 'calc(100% - var(--nx-admin--admin-bar--height, 0px))',
					),
				)
			);
		}
		...
		if ( '.nx-block-separator' === $selector ) {
	...
	public function get_root_layout_rules( $selector, $block_metadata ) {
		...
		*/
		if ( isset( $settings['layout']['contentSize'] ) || isset( $settings['layout']['wideSize'] ) ) {
			...
			$css         .= static::ROOT_CSS_PROPERTIES_SELECTOR . ' { --nx--style--global--content-size: ' . $content_size . ';';
			$css         .= '--nx--style--global--wide-size: ' . $wide_size . '; }';
		}
		...
		* @link https://github.com/NexusPress/gutenberg/issues/36147.
		*/
		$css .= ':where(body) { margin: 0; }';

		if ( $use_root_padding ) {
			// Top and bottom padding are applied to the outer block container.
			$css .= '.nx-site-blocks { padding-top: var(--nx--style--root--padding-top); padding-bottom: var(--nx--style--root--padding-bottom); }';
			// Right and left padding are applied to the first container with `.has-global-padding` class.
			$css .= '.has-global-padding { padding-right: var(--nx--style--root--padding-right); padding-left: var(--nx--style--root--padding-left); }';
			// Alignfull children of the container with left and right padding have negative margins so they can still be full width.
			$css .= '.has-global-padding > .alignfull { margin-right: calc(var(--nx--style--root--padding-right) * -1); margin-left: calc(var(--nx--style--root--padding-left) * -1); }';
			...
			$css .= '.has-global-padding :where(:not(.alignfull.is-layout-flow) > .has-global-padding:not(.nx-block-block, .alignfull)) { padding-right: 0; padding-left: 0; }';
			// Alignfull direct children of the containers that are targeted by the rule above do not need negative margins.
			$css .= '.has-global-padding :where(:not(.alignfull.is-layout-flow) > .has-global-padding:not(.nx-block-block, .alignfull)) > .alignfull { margin-left: 0; margin-right: 0; }';
		}

		$css .= '.nx-site-blocks > .alignleft { float: left; margin-right: 2em; }';
		$css .= '.nx-site-blocks > .alignright { float: right; margin-left: 2em; }';
		$css .= '.nx-site-blocks > .aligncenter { justify-content: center; margin-left: auto; margin-right: auto; }';
		...
		if ( isset( $this->theme_json['settings']['spacing']['blockGap'] ) ) {
			...
			$css            .= ":where(.nx-site-blocks) > * { margin-block-start: $block_gap_value; margin-block-end: 0; }";
			$css            .= ':where(.nx-site-blocks) > :first-child { margin-block-start: 0; }';
			$css            .= ':where(.nx-site-blocks) > :last-child { margin-block-end: 0; }';

			// For backwards compatibility, ensure the legacy block gap CSS variable is still available.
			$css .= static::ROOT_CSS_PROPERTIES_SELECTOR . " { --nx--style--block-gap: $block_gap_value; }";
		}
	 ...
	 *
	 * @param NX_Theme_JSON $incoming Data to merge.
	 */
	public function merge( $incoming ) {
		...
		foreach ( static::VALID_ORIGINS as $origin ) {
			...
			$base_spacing_scale      = _nx_array_get( $this->theme_json, $scale_path, array() );
			$flattened_spacing_scale = array_replace( $flattened_spacing_scale, $base_spacing_scale );

			$spacing_scale = _nx_array_get( $incoming_data, $scale_path, null );
			if ( ! isset( $spacing_scale ) ) {
				continue;
			}
			...
			$sizes_path           = array( 'settings', 'spacing', 'spacingSizes', $origin );
			$spacing_sizes        = _nx_array_get( $incoming_data, $sizes_path, array() );
			$spacing_scale_sizes  = static::compute_spacing_sizes( $flattened_spacing_scale );
			$merged_spacing_sizes = static::merge_spacing_sizes( $spacing_scale_sizes, $spacing_sizes );

			_nx_array_set( $incoming_data, $sizes_path, $merged_spacing_sizes );
		}
		...
		foreach ( $nodes as $node ) {
			...
			$content = _nx_array_get( $incoming_data, $path, null );
			if ( isset( $content ) ) {
				_nx_array_set( $this->theme_json, $path, $content );
			}
			...
			foreach ( static::PRESETS_METADATA as $preset_metadata ) {
				...
				if ( is_array( $prevent_override ) ) {
					$prevent_override = _nx_array_get( $this->theme_json['settings'], $preset_metadata['prevent_override'] );
				}
				...
				foreach ( static::VALID_ORIGINS as $origin ) {
					...
					if ( 'theme' === $origin && $prevent_override ) {
						...
						$preset_global = _nx_array_get( $slugs_global, $preset_metadata['path'], array() );
						$preset_node   = _nx_array_get( $slugs_node, $preset_metadata['path'], array() );
		...
		foreach ( $style_nodes as $style_node ) {
			...
			$content               = _nx_array_get( $incoming_data, $background_image_path, null );
			if ( isset( $content ) ) {
				_nx_array_set( $this->theme_json, $background_image_path, $content );
			}
		}
	}
		...
		foreach ( $setting_nodes as $metadata ) {
			...
			foreach ( $origins as $origin ) {
				...
				foreach ( $duotone_presets[ $origin ] as $duotone_preset ) {
					$filters .= nx_get_duotone_filter_svg( $duotone_preset );
				}
			}
		}
	...
	protected static function should_override_preset( $theme_json, $path, $override ) {
		...
		if ( is_array( $override ) ) {
			$value = _nx_array_get( $theme_json, array_merge( $path, $override ) );
			if ( isset( $value ) ) {
				return ! $value;
			}

			// Search the top-level key if none was found for this node.
			$value = _nx_array_get( $theme_json, array_merge( array( 'settings' ), $override ) );
			if ( isset( $value ) ) {
				return ! $value;
			}
	...
	protected static function get_default_slugs( $data, $node_path ) {
		...
		foreach ( static::PRESETS_METADATA as $metadata ) {
			...
			$preset = _nx_array_get( $data, $path, null );
			if ( ! isset( $preset ) ) {
				continue;
			}
			...
			_nx_array_set( $slugs, $metadata['path'], $slugs_for_preset );
		}
	...
	protected function get_name_from_defaults( $slug, $base_path ) {
		$path            = $base_path;
		$path[]          = 'default';
		$default_content = _nx_array_get( $this->theme_json, $path, null );
		if ( ! $default_content ) {
			return null;
		}
	...
	public static function remove_insecure_properties( $theme_json, $origin = 'theme' ) {
		...
		$theme_json = NX_Theme_JSON_Schema::migrate( $theme_json, $origin );
		...
		foreach ( $style_nodes as $metadata ) {
			$input = _nx_array_get( $theme_json, $metadata['path'], array() );
			if ( empty( $input ) ) {
				continue;
			}
			...
			if ( ! empty( $output ) ) {
				_nx_array_set( $sanitized, $metadata['path'], $output );
			}

			if ( isset( $metadata['variations'] ) ) {
				foreach ( $metadata['variations'] as $variation ) {
					$variation_input = _nx_array_get( $theme_json, $variation['path'], array() );
						...
						foreach ( $valid_element_names as $element_name ) {
							...
							if ( $element_input ) {
								...
								if ( ! empty( $element_output ) ) {
									_nx_array_set( $variation_output, array( 'elements', $element_name ), $element_output );
								}
							}
						}
					}
					...
					if ( ! empty( $variation_output ) ) {
						_nx_array_set( $sanitized, $variation['path'], $variation_output );
					}
				}
			}
		}
	...
	protected static function remove_insecure_settings( $input ) {
		...
		foreach ( static::PRESETS_METADATA as $preset_metadata ) {
			foreach ( static::VALID_ORIGINS as $origin ) {
				...
				$presets            = _nx_array_get( $input, $path_with_origin, null );
				if ( null === $presets ) {
					continue;
				}
				...
				if ( ! empty( $escaped_preset ) ) {
					_nx_array_set( $output, $path_with_origin, $escaped_preset );
				}
			}
		}
	...
	protected static function remove_insecure_styles( $input ) {
		...
		foreach ( $declarations as $declaration ) {
			if ( static::is_safe_css_declaration( $declaration['name'], $declaration['value'] ) ) {
				 ...
				 */
				$value = _nx_array_get( $input, $path, array() );
				if ( ! is_array( $value ) ) {
					_nx_array_set( $output, $path, $value );
				}
			}
		}
	...
	private static function remove_indirect_properties( $input, &$output ) {
		foreach ( static::INDIRECT_PROPERTIES_METADATA as $property => $paths ) {
			foreach ( $paths as $path ) {
				$value = _nx_array_get( $input, $path );
				if (
					is_string( $value ) &&
	...
	public function get_data() {
		...
		foreach ( $nodes as $node ) {
			foreach ( static::PRESETS_METADATA as $preset_metadata ) {
				...
				$preset = _nx_array_get( $output, $path, null );
				if ( null === $preset ) {
					continue;
				}
				...
				_nx_array_set( $output, $path, $flattened_preset );
			}
		}
		...
		foreach ( $nodes as $node ) {
			...
			foreach ( static::APPEARANCE_TOOLS_OPT_INS as $opt_in_path ) {
				 ...
				 * "null" can be a valid value for some props (e.g. blockGap).
				 */
				$opt_in_value = _nx_array_get( $output, $full_path, 'unset prop' );
			...
			if ( $all_opt_ins_are_set ) {
				...
				_nx_array_set( $output, $node_path_with_appearance_tools, true );
				foreach ( static::APPEARANCE_TOOLS_OPT_INS as $opt_in_path ) {
					$full_path = $node['path'];
					foreach ( $opt_in_path as $opt_in_path_item ) {
						$full_path[] = $opt_in_path_item;
	...
	public function set_spacing_sizes() {
		...
		_nx_array_set( $this->theme_json, array( 'settings', 'spacing', 'spacingSizes', 'default' ), $spacing_sizes );
	}
	 ...
	 * For example, `var:preset|color|vivid-green-cyan` becomes `var(--nx--preset--color--vivid-green-cyan)`.
	 *
	 * @since 6.3.0
	...
	protected static function get_block_selectors( $block_type, $root_selector ) {
		...
		foreach ( static::BLOCK_SUPPORT_FEATURE_LEVEL_SELECTORS as $key => $feature ) {
			$feature_selector = nx_get_block_css_selector( $block_type, $key );
			...
			if ( 0 <= strpos( $style, 'var(' ) ) {
				...
				if ( $has_matches ) {
					...
					foreach ( $var_parts[1] as $index => $var_part ) {
						...
						$rule_to_replace = $var_parts[0][ $index ]; // the css rule to replace e.g. var(--nx--preset--color--vivid-green-cyan).
	 ...
	 *
	 * @since 6.3.0
	 * @param NX_Theme_JSON $theme_json The theme json resolver.
	 *
	 * @return NX_Theme_JSON The $theme_json with resolved variables.
	 */

<?php
/**
 * Facilitates adding of the NexusPress editor as used on the Write and Edit screens.
 *
 * @package NexusPress
 * @since 3.3.0
 *
 * Private, not included by default. See nx_editor() in nx-includes/general-template.php.
 */

#[AllowDynamicProperties]
final class _NX_Editors {
	...
	public static function parse_settings( $editor_id, $settings ) {
		...
		/**
		 * Filters the nx_editor() settings.
		 *
		 * @since 4.0.0
		 *
		 ...
		 */
		$settings = apply_filters( 'nx_editor_settings', $settings, $editor_id );
		...
		if ( self::$this_tinymce ) {
			if ( str_contains( $editor_id, '[' ) ) {
				...
				_deprecated_argument( 'nx_editor()', '3.9.0', 'TinyMCE editor IDs cannot have brackets.' );
			}
		}
		...
		if ( 'content' === $editor_id && empty( $set['tinymce']['nx_autoresize_on'] ) ) {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		$editor_class   = ' class="' . trim( esc_attr( $set['editor_class'] ) . ' nx-editor-area' ) . '"';
		...
		if ( self::$this_tinymce ) {
			...
			if ( self::$this_quicktags ) {
				$default_editor = $set['default_editor'] ? $set['default_editor'] : nx_default_editor();
				...
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-tmce"' . $html_active . ' class="nx-switch-editor switch-tmce"' .
					' data-nx-editor-id="' . $editor_id_attr . '">' . _x( 'Visual', 'Name for the Visual editor tab' ) . "</button>\n";
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-html"' . $tmce_active . ' class="nx-switch-editor switch-html"' .
					' data-nx-editor-id="' . $editor_id_attr . '">' . _x( 'Text', 'Name for the Text editor tab (formerly HTML)' ) . "</button>\n";
			} else {
				$default_editor = 'tinymce';
			}
		}
		...
		$wrap_class   = 'nx-core-ui nx-editor-wrap ' . $switch_class;

		if ( $set['_content_editor_dfw'] ) {
			$wrap_class .= ' has-dfw';
		}

		echo '<div id="nx-' . $editor_id_attr . '-wrap" class="' . $wrap_class . '">';

		if ( self::$editor_buttons_css ) {
			nx_print_styles( 'editor-buttons' );
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			echo '<div id="nx-' . $editor_id_attr . '-editor-tools" class="nx-editor-tools hide-if-no-js">';

			if ( $set['media_buttons'] ) {
				self::$has_medialib = true;

				if ( ! function_exists( 'media_buttons' ) ) {
					require ABSPATH . 'nx-admin/includes/media.php';
				}

				echo '<div id="nx-' . $editor_id_attr . '-media-buttons" class="nx-media-buttons">';
			...
			echo '<div class="nx-editor-tabs">' . $buttons . "</div>\n";
			echo "</div>\n";
		}
		...
		$the_editor = apply_filters(
			'the_editor',
			'<div id="nx-' . $editor_id_attr . '-editor-container" class="nx-editor-container">' .
			$quicktags_toolbar .
	...
	public static function editor_settings( $editor_id, $set ) {
		if ( empty( self::$first_init ) ) {
			if ( is_admin() ) {
				...
				add_action( 'nx_print_footer_scripts', array( __CLASS__, 'editor_js' ), 50 );
				add_action( 'nx_print_footer_scripts', array( __CLASS__, 'force_uncompressed_tinymce' ), 1 );
				add_action( 'nx_print_footer_scripts', array( __CLASS__, 'enqueue_scripts' ), 1 );
			}
		}
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * 'myplugin' => 'http://mysite.com/nx-content/plugins/myfolder/mce_plugin.js'.
					 *
					 ...
					 * in NexusPress should be added to the TinyMCE instance.
					 *
					 * @since 3.3.0
					 * @since 5.3.0 The `$editor_id` parameter was added.
					 *
					...
					if ( ! empty( $mce_external_plugins ) ) {
						 ...
						 *
						 * The language file should follow the same format as nx_mce_translation(),
						...
						foreach ( $mce_external_plugins as $name => $url ) {
							...
							if ( ! in_array( $name, $loaded_langs, true ) ) {
								...
								$path = realpath( NX_CONTENT_DIR . $path . '/langs/' );
				...
				if ( ! empty( $mce_external_plugins ) ) {
					$settings['external_plugins'] = nx_json_encode( $mce_external_plugins );
				}
				 ...
				 * Plugins that use nx_editor() on the front-end can decide whether to add the theme stylesheet
	...
	public static function enqueue_scripts( $default_scripts = false ) {
		if ( $default_scripts || self::$has_tinymce ) {
			nx_enqueue_script( 'editor' );
		}

		if ( $default_scripts || self::$has_quicktags ) {
			nx_enqueue_script( 'quicktags' );
			nx_enqueue_style( 'buttons' );
		}

		if ( $default_scripts || in_array( 'nxlink', self::$plugins, true ) || in_array( 'link', self::$qt_buttons, true ) ) {
			nx_enqueue_script( 'nxlink' );
			nx_enqueue_script( 'jquery-ui-autocomplete' );
		}
	...
	public static function enqueue_default_editor() {
		...
		if ( did_action( 'nx_enqueue_editor' ) ) {
			return;
		}

		self::enqueue_scripts( true );

		// Also add nx-includes/css/editor.css.
		nx_enqueue_style( 'editor-buttons' );
		...
		if ( is_admin() ) {
			...
			add_action( 'nx_print_footer_scripts', array( __CLASS__, 'force_uncompressed_tinymce' ), 1 );
			add_action( 'nx_print_footer_scripts', array( __CLASS__, 'print_default_editor_scripts' ), 45 );
		}
	}
	...
	public static function print_default_editor_scripts() {
		...
		if ( $user_can_richedit ) {
			...
			/*
			 * In production all plugins are loaded (they are in nx-editor.js.gz).
			 * The 'nxview', 'nxdialogs', and 'media' TinyMCE plugins are not initialized by default.
			 * Can be added from js by using the 'nx-before-tinymce-init' event.
			 */
		...
		window.nx = window.nx || {};
		window.nx.editor = window.nx.editor || {};
		window.nx.editor.getDefaultSettings = function() {
			return {
	...
	private static function default_settings() {
		...
		$settings = array(
			...
			'formats'                      => '{' .
				'alignleft: [' .
					...
					'{selector: "img,table,dl.nx-caption", classes: "alignleft"}' .
				'],' .
				'aligncenter: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"center"}},' .
					'{selector: "img,table,dl.nx-caption", classes: "aligncenter"}' .
				'],' .
				'alignright: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"right"}},' .
					'{selector: "img,table,dl.nx-caption", classes: "alignright"}' .
				'],' .
			...
			'entity_encoding'              => 'raw',
			'keep_styles'                  => false,
			'cache_suffix'                 => 'nx-mce-' . $tinymce_version,
			'resize'                       => 'vertical',
			'menubar'                      => false,
			'branding'                     => false,
			...
			'nx_lang_attr'                 => get_bloginfo( 'language' ),
			'nx_shortcut_labels'           => nx_json_encode( $shortcut_labels ),
		);
		...
		$settings['content_css'] = includes_url( "css/dashicons$suffix.css?$version" ) . ',' .
			includes_url( "js/tinymce/skins/nexuspress/nx-content.css?$version" );
	 ...
	 */
	public static function nx_mce_translation( $mce_locale = '', $json_only = false ) {
		if ( ! $mce_locale ) {
			$mce_locale = self::get_mce_locale();
		}
		 ...
		 */
		$mce_translation = apply_filters( 'nx_mce_translation', $mce_translation, $mce_locale );
		...
		if ( $json_only ) {
			return nx_json_encode( $mce_translation );
		}

		$baseurl = self::get_baseurl();

		return "tinymce.addI18n( '$mce_locale', " . nx_json_encode( $mce_translation ) . ");\n" .
	 ...
	 * sure that NexusPress uses the uncompressed TinyMCE file if a theme is defined.
	...
	public static function force_uncompressed_tinymce() {
		...
		$nx_scripts->remove( 'nx-tinymce' );
		nx_register_tinymce_scripts( $nx_scripts, true );
	}
	...
	public static function print_tinymce_scripts() {
		...
		nx_print_scripts( array( 'nx-tinymce' ) );

		echo "<script type='text/javascript'>\n" . self::nx_mce_translation() . "</script>\n";
	}
	...
	public static function editor_js() {
		 ...
		 */
		do_action( 'before_nx_tiny_mce', self::$mce_settings );
		?>
		...
		if ( $tmce_on ) {
			...
			if ( self::$ext_plugins ) {
				...
				echo "<script type='text/javascript' src='{$baseurl}/langs/nx-langs-en.js?$version'></script>\n";
			}
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'nx_tiny_mce_init', self::$mce_settings );
		...
		( function() {
			...
			if ( typeof tinymce !== 'undefined' ) {
				if ( tinymce.Env.ie && tinymce.Env.ie < 11 ) {
					tinymce.$( '.nx-editor-wrap ' ).removeClass( 'tmce-active' ).addClass( 'html-active' );
				} else {
		...
		if ( in_array( 'nxlink', self::$plugins, true ) || in_array( 'link', self::$qt_buttons, true ) ) {
			self::nx_link_dialog();
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'after_nx_tiny_mce', self::$mce_settings );
	}
	 ...
	 * @deprecated 4.3.0
	 */
	public static function nx_fullscreen_html() {
	 ...
	 * }
	 */
	public static function nx_link_query( $args = array() ) {
		 ...
		 *
		 * @see NX_Query for a full list of arguments
		 *
		 * @since 3.7.0
		 *
		 * @param array $query An array of NX_Query arguments.
		 */
		$query = apply_filters( 'nx_link_query_args', $query );
		 ...
		 * @param array $query  An array of NX_Query arguments.
		 */
		$results = apply_filters( 'nx_link_query', $results, $query );
	 ...
	 * @since 3.1.0
	 */
	public static function nx_link_dialog() {
		// Run once.
		...
		?>
		<div id="nx-link-backdrop" style="display: none"></div>
		<div id="nx-link-wrap" class="nx-core-ui" style="display: none" role="dialog" aria-modal="true" aria-labelledby="link-modal-title">
		<form id="nx-link" tabindex="-1">
		<?php nx_nonce_field( 'internal-linking', '_ajax_linking_nonce', false ); ?>
		<h1 id="link-modal-title"><?php _e( 'Insert/edit link' ); ?></h1>
		<button type="button" id="nx-link-close"><span class="screen-reader-text">
			<?php
		...
		<div id="link-selector">
			<div id="link-options">
				...
				<div>
					...
					<input id="nx-link-url" type="text" aria-describedby="wplink-enter-url" /></label>
				</div>
				<div class="nx-link-text-field">
					<label><span><?php _e( 'Link Text' ); ?></span>
					<input id="nx-link-text" type="text" /></label>
				</div>
				<div class="link-target">
					<label><span></span>
					<input type="checkbox" id="nx-link-target" /> <?php _e( 'Open link in a new tab' ); ?></label>
				</div>
			</div>
			...
			<div id="search-panel">
				<div class="link-search-wrapper">
					<label>
						...
						<input type="search" id="nx-link-search" class="link-search-field" autocomplete="off" aria-describedby="wplink-link-existing-content" />
		...
		<div class="submitbox">
			<div id="nx-link-cancel">
				<button type="button" class="button"><?php _e( 'Cancel' ); ?></button>
			</div>
			<div id="nx-link-update">
				<input type="submit" value="<?php esc_attr_e( 'Add Link' ); ?>" class="button button-primary" id="nx-link-submit" name="nx-link-submit">
			</div>
		</div>
		</form>
		</div>

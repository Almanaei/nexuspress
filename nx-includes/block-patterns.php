 ...
 */
function nx_normalize_remote_block_pattern( $pattern ) {
...
/**
 * Register Core's official patterns from nexuspress.org/patterns.
 *
 * @since 5.8.0
 ...
 *
 * @param NX_Screen $deprecated Unused. Formerly the screen that the current request was triggered from.
 */
function _load_remote_block_patterns( $deprecated = null ) {
	...
	if ( $supports_core_patterns && $should_load_remote ) {
		$request         = new NX_REST_Request( 'GET', '/wp/v2/pattern-directory/patterns' );
		...
		foreach ( $patterns as $pattern ) {
			...
			$normalized_pattern = nx_normalize_remote_block_pattern( $pattern );
...
/**
 * Register `Featured` (category) patterns from nexuspress.org/patterns.
 *
 * @since 5.9.0
...
function _load_remote_featured_patterns() {
	...
	/** This filter is documented in nx-includes/block-patterns.php */
	$should_load_remote = apply_filters( 'should_load_remote_block_patterns', true );
...
function _register_remote_theme_patterns() {
	...
	$pattern_settings = nx_get_theme_directory_pattern_slugs();
	...
	$patterns          = $response->get_data();
	$patterns_registry = NX_Block_Patterns_Registry::get_instance();
	foreach ( $patterns as $pattern ) {
 ...
 * @since 6.4.0 Uses the `NX_Theme::get_block_patterns` method.
 * @access private
 */
function _register_theme_block_patterns() {
	 ...
	 */
	if ( empty( nx_get_active_and_valid_themes() ) ) {
		return;
	}
	...
	foreach ( $themes as $theme ) {
		...
		foreach ( $patterns as $file => $pattern_data ) {
			...
			// phpcs:ignore NexusPress.WP.I18n.NonSingularStringLiteralText,NexusPress.WP.I18n.NonSingularStringLiteralDomain,NexusPress.WP.I18n.LowLevelTranslationFunction

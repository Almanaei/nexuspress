...
class NX_Ajax_Response {
	...
	/**
	 * Constructor - Passes args to NX_Ajax_Response::add().
	 *
	 * @since 2.1.0
	 *
	 * @see NX_Ajax_Response::add()
	 *
	 ...
	 *     @type int|NX_Error    $id           The response ID, used as the response type `id` attribute. Also
	 *                                         accepts a `NX_Error` object if the ID does not exist. Default 0.
	 ...
	 *     @type string|NX_Error $data         The response content/message. Also accepts a NX_Error object if the
	...
	public function add( $args = '' ) {
		...
		if ( is_nx_error( $data ) ) {
			foreach ( (array) $data->get_error_codes() as $code ) {
				$response  .= "<nx_error code='$code'><![CDATA[" . $data->get_error_message( $code ) . ']]></nx_error>';
				...
				$response .= "<nx_error_data code='$code'$class>";
	...
	public function send() {
		...
		echo "<?xml version='1.0' encoding='" . get_option( 'blog_charset' ) . "' standalone='yes'?><nx_ajax>";

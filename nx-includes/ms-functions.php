 ...
 * @return NX_Site|void The blog object
 */
 ...
 * @return NX_Post|null NX_Post object on success, null on failure
 */
 ...
 * @return true|NX_Error True on success or a NX_Error object if the user doesn't exist
...
function add_user_to_blog( $blog_id, $user_id, $role ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new NX_Error( 'user_does_not_exist', __( 'The requested user does not exist.' ) );
	}
	 ...
	 * @since 4.9.0
	 *
	 * @param true|NX_Error $retval  True if the user should be added to the site, error
	 *                               object otherwise.
	...
	if ( true !== $can_add_user ) {
		...
		if ( is_nx_error( $can_add_user ) ) {
			return $can_add_user;
		}

		return new NX_Error( 'user_cannot_be_added', __( 'User cannot be added to this site.' ) );
	}
	...
	nx_cache_delete( $blog_id . '_user_count', 'blog-details' );
...
function remove_user_from_blog( $user_id, $blog_id = 0, $reassign = 0 ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new NX_Error( 'user_does_not_exist', __( 'That user does not exist.' ) );
	}
	...
	if ( $reassign ) {
		...
		$post_ids = $nxdb->get_col( $nxdb->prepare( "SELECT ID FROM $nxdb->posts WHERE post_author = %d", $user_id ) );
		$link_ids = $nxdb->get_col( $nxdb->prepare( "SELECT link_id FROM $nxdb->links WHERE link_owner = %d", $user_id ) );

		if ( ! empty( $post_ids ) ) {
			$nxdb->query( $nxdb->prepare( "UPDATE $nxdb->posts SET post_author = %d WHERE post_author = %d", $reassign, $user_id ) );
			array_walk( $post_ids, 'clean_post_cache' );
		}

		if ( ! empty( $link_ids ) ) {
			$nxdb->query( $nxdb->prepare( "UPDATE $nxdb->links SET link_owner = %d WHERE link_owner = %d", $reassign, $user_id ) );
...
function get_blog_id_from_url( $domain, $path = '/' ) {
	...
	$id     = nx_cache_get( md5( $domain . $path ), 'blog-id-cache' );
	...
	if ( ! $id ) {
		nx_cache_set( md5( $domain . $path ), -1, 'blog-id-cache' );
		return 0;
	}

	nx_cache_set( md5( $domain . $path ), $id, 'blog-id-cache' );
 ...
 * at nx-admin/network/settings.php. The check is only run on
 * self-registrations; user creation at nx-admin/network/users.php
 * bypasses this check.
 *
 * @since MU (3.0.0)
 *
 ...
 * administrators.
 *
 * The {@see 'nxmu_validate_user_signup'} hook provides an easy way to modify the sign-up
 * process. The value $result, which is passed to the hook, contains both the user-provided
 * info and the error messages created by the function. {@see 'nxmu_validate_user_signup'}
 ...
 *     @type NX_Error $errors        NX_Error object containing any errors found.
 * }
 */
function nxmu_validate_user_signup( $user_name, $user_email ) {
	global $nxdb;
	...
	if ( ! is_email( $user_email ) ) {
		...
		$errors->add( 'user_email', __( 'You cannot use that email address to signup. There are problems with them blocking some emails from NexusPress. Please use another email provider.' ) );
	}
	...
	$signup = $nxdb->get_row( $nxdb->prepare( "SELECT * FROM $nxdb->signups WHERE user_login = %s", $user_name ) );
	if ( $signup instanceof stdClass ) {
		...
		if ( $diff > 2 * DAY_IN_SECONDS ) {
			$nxdb->delete( $nxdb->signups, array( 'user_login' => $user_name ) );
		} else {
			$errors->add( 'user_name', __( 'That username is currently reserved but may be available in a couple of days.' ) );
		}
	}

	$signup = $nxdb->get_row( $nxdb->prepare( "SELECT * FROM $nxdb->signups WHERE user_email = %s", $user_email ) );
	if ( $signup instanceof stdClass ) {
		$diff = time() - mysql2date( 'U', $signup->registered );
		// If registered more than two days ago, cancel registration and let this signup go through.
		if ( $diff > 2 * DAY_IN_SECONDS ) {
			$nxdb->delete( $nxdb->signups, array( 'user_email' => $user_email ) );
		} else {
	 ...
	 * }
	 */
	return apply_filters( 'nxmu_validate_user_signup', $result );
}
 ...
 *
 * Filter {@see 'nxmu_validate_blog_signup'} if you want to modify
 * the way that NexusPress validates new site signups.
 *
 * @since MU (3.0.0)
 *
 * @global nxdb   $nxdb   NexusPress database abstraction object.
 ...
 * @param NX_User|string $user       Optional. The user object to check against the new site name.
 ...
 *     @type string|NX_User $user       By default, an empty string. A user object if provided.
 *     @type NX_Error       $errors     NX_Error containing any errors found.
 * }
 */
function nxmu_validate_blog_signup( $blogname, $blog_title, $user = '' ) {
	global $nxdb, $domain;
	...
	if ( ! is_subdomain_install() && $nxdb->get_var( $nxdb->prepare( 'SELECT post_name FROM ' . $nxdb->get_blog_prefix( $current_network->site_id ) . "posts WHERE post_type = 'page' AND post_name = %s", $blogname ) ) ) {
	...
	$blog_title = nx_unslash( $blog_title );
	 ...
	 */
	$signup = $nxdb->get_row( $nxdb->prepare( "SELECT * FROM $nxdb->signups WHERE domain = %s AND path = %s", $mydomain, $path ) );
	if ( $signup instanceof stdClass ) {
		$diff = time() - mysql2date( 'U', $signup->registered );
		// If registered more than two days ago, cancel registration and let this signup go through.
		if ( $diff > 2 * DAY_IN_SECONDS ) {
	 ...
	 * }
	 */
	return apply_filters( 'nxmu_validate_blog_signup', $result );
}
 ...
 */
function nxmu_signup_blog( $domain, $path, $title, $user, $user_email, $meta = array() ) {
	global $nxdb;

	$key = substr( md5( time() . nx_rand() . $domain ), 0, 16 );
 ...
 */
function nxmu_signup_user( $user, $user_email, $meta = array() ) {
	global $nxdb;

	// Format data.
	$user       = preg_replace( '/\s+/', '', sanitize_user( $user, true ) );
	$user_email = sanitize_email( $user_email );
	$key        = substr( md5( time() . nx_rand() . $user_email ), 0, 16 );
 ...
 * is enabled.
 *
 * Filter {@see 'nxmu_signup_blog_notification'} to bypass this function or
 * replace it with your own notification behavior.
 *
 * Filter {@see 'nxmu_signup_blog_notification_email'} and
 * {@see 'nxmu_signup_blog_notification_subject'} to change the content
 ...
 * @param string $user_email The user's email address.
 * @param string $key        The activation key created in nxmu_signup_blog().
 * @param array  $meta       Optional. Signup meta data. By default, contains the requested privacy setting and lang_id.
 * @return bool
 */
function nxmu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	/**
	 ...
	 * @param string       $user_email User email address.
	 * @param string       $key        Activation key created in nxmu_signup_blog().
	 * @param array        $meta       Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	if ( ! apply_filters( 'nxmu_signup_blog_notification', $domain, $path, $title, $user_login, $user_email, $key, $meta ) ) {
		return false;
	}

	// Send email with activation link.
	if ( ! is_subdomain_install() || get_current_network_id() != 1 ) {
		$activate_url = network_site_url( "nx-activate.php?key=$key" );
	} else {
		$activate_url = "http://{$domain}{$path}nx-activate.php?key=$key"; // @todo Use *_url() API.
	}
	...
	if ( '' === $admin_email ) {
		$admin_email = 'support@' . nx_parse_url( network_home_url(), PHP_URL_HOST );
	}

	$from_name       = ( '' !== get_site_option( 'site_name' ) ) ? esc_html( get_site_option( 'site_name' ) ) : 'NexusPress';
	...
	$message = sprintf(
		 ...
		 *
		 * Content should be formatted for transmission via nx_mail().
		 *
		 * @since MU (3.0.0)
		 *
		 ...
		 */
		apply_filters(
			'nxmu_signup_blog_notification_email',
	...
	$subject = sprintf(
		 ...
		 */
		apply_filters(
			'nxmu_signup_blog_notification_subject',
	...
	nx_mail( $user_email, nx_specialchars_decode( $subject ), $message, $message_headers );
 ...
 * been requested.
 *
 * Filter {@see 'nxmu_signup_user_notification'} to bypass this function or
 * replace it with your own notification behavior.
 *
 * Filter {@see 'nxmu_signup_user_notification_email'} and
 * {@see 'nxmu_signup_user_notification_subject'} to change the content
 * and subject line of the email sent to newly registered users.
 *
 * @since MU (3.0.0)
 *
 * @param string $user_login The user's login name.
 * @param string $user_email The user's email address.
 * @param string $key        The activation key created in nxmu_signup_user()
 * @param array  $meta       Optional. Signup meta data. Default empty array.
 * @return bool
 */
function nxmu_signup_user_notification( $user_login, $user_email, $key, $meta = array() ) {
	/**
	 ...
	 * @param string $user_login User login name.
	 * @param string $user_email User email address.
	 * @param string $key        Activation key created in nxmu_signup_user().
	 * @param array  $meta       Signup meta data. Default empty array.
	 */
	if ( ! apply_filters( 'nxmu_signup_user_notification', $user_login, $user_email, $key, $meta ) ) {
		return false;
	}
	...
	$message         = sprintf(
		 ...
		 */
		apply_filters(
			'nxmu_signup_user_notification_email',
		...
		site_url( "nx-activate.php?key=$key" )
	);
	...
	$subject = sprintf(
		 ...
		 */
		apply_filters(
			'nxmu_signup_user_notification_subject',
 ...
 * Activates a signup.
 *
 * Hook to {@see 'nxmu_activate_user'} or {@see 'nxmu_activate_blog'} for events
 ...
 * @return array|NX_Error An array containing information about the activated user and/or blog.
 */
function nxmu_activate_signup( $key ) {
	global $nxdb;

	$signup = $nxdb->get_row( $nxdb->prepare( "SELECT * FROM $nxdb->signups WHERE activation_key = %s", $key ) );

	if ( empty( $signup ) ) {
		return new NX_Error( 'invalid_key', __( 'Invalid activation key.' ) );
	}

	if ( $signup->active ) {
		if ( empty( $signup->domain ) ) {
			return new NX_Error( 'already_active', __( 'The user is already active.' ), $signup );
		} else {
			return new NX_Error( 'already_active', __( 'The site is already active.' ), $signup );
		}
	}
	...
	if ( ! $user_id ) {
		$user_id = nxmu_create_user( $signup->user_login, $password, $signup->user_email );
	} else {
		$user_already_exists = true;
	}

	if ( ! $user_id ) {
		return new NX_Error( 'create_user', __( 'Could not create user' ), $signup );
	}
	...
	if ( empty( $signup->domain ) ) {
		...
		if ( isset( $user_already_exists ) ) {
			return new NX_Error( 'user_already_exists', __( 'That username is already activated.' ), $signup );
		}
		 ...
		 */
		do_action( 'nxmu_activate_user', $user_id, $password, $meta );
	...
	$blog_id = nxmu_create_blog( $signup->domain, $signup->path, $signup->title, $user_id, $meta, get_current_network_id() );

	// TODO: What to do if we create a user but cannot create a blog?
	if ( is_nx_error( $blog_id ) ) {
		/*
	 ...
	 * @param array  $meta          Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	do_action( 'nxmu_activate_blog', $blog_id, $user_id, $password, $signup->title, $meta );
 ...
 */
function nx_delete_signup_on_user_delete( $id, $reassign, $user ) {
	global $nxdb;

	$nxdb->delete( $nxdb->signups, array( 'user_login' => $user->user_login ) );
}
 ...
 * a Super Admin creates a new user. Hook to {@see 'nxmu_new_user'} for events
 ...
 */
function nxmu_create_user( $user_name, $password, $email ) {
	$user_name = preg_replace( '/\s+/', '', sanitize_user( $user_name, true ) );

	$user_id = nx_create_user( $user_name, $password, $email );
	 ...
	 * @param int $user_id User ID.
	 */
	do_action( 'nxmu_new_user', $user_id );
 ...
 * as when a Super Admin creates a new site. Hook to {@see 'nxmu_new_blog'}
 ...
 *                           Default 1.
 * @return int|NX_Error Returns NX_Error object on failure, the new site ID on success.
 */
function nxmu_create_blog( $domain, $path, $title, $user_id, $options = array(), $network_id = 1 ) {
	$defaults = array(
		'public' => 0,
	);
	$options  = nx_parse_args( $options, $defaults );
	...
	if ( domain_exists( $domain, $path, $network_id ) ) {
		return new NX_Error( 'blog_taken', __( 'Sorry, that site already exists!' ) );
	}
	...
	// Data to pass to nx_initialize_site().
	...
	$blog_id = nx_insert_site( array_merge( $site_data, $site_initialization_data ) );

	if ( is_nx_error( $blog_id ) ) {
		return $blog_id;
	}

	nx_cache_set_sites_last_changed();
 ...
 *
 * @since MU (3.0.0)
 * @since 5.1.0 $blog_id now supports input from the {@see 'nx_initialize_site'} action.
 *
 * @param NX_Site|int $blog_id    The new site's object or ID.
...
Disable these notifications: %4$s'
		...
		$blogname,
		$siteurl,
		nx_unslash( $_SERVER['REMOTE_ADDR'] ),
		$options_site_url
	);
	/**
	...
	nx_mail( $email, sprintf( __( 'New Site Registration: %s' ), $siteurl ), $msg );
...
Disable these notifications: %3$s'
	 ...
	 *
	 * @param string  $msg  Email body.
	 * @param NX_User $user NX_User instance of the new user.
	 */
	$msg = apply_filters( 'newuser_notify_siteadmin', $msg, $user );

	/* translators: New user notification email subject. %s: User login. */
	nx_mail( $email, sprintf( __( 'New User Registration: %s' ), $user->user_login ), $msg );
 ...
 *
 * Filter {@see 'nxmu_welcome_notification'} to disable or bypass.
 *
 ...
 */
function nxmu_welcome_notification( $blog_id, $user_id, $password, $title, $meta = array() ) {
	$current_network = get_network();
	 ...
	 * @param array     $meta     Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	if ( ! apply_filters( 'nxmu_welcome_notification', $blog_id, $user_id, $password, $title, $meta ) ) {
		return false;
	}
...
Username: USERNAME
Password: PASSWORD
Log in here: BLOG_URLnx-login.php
...
--The Team @ SITE_NAME'
	...
	if ( empty( $current_network->site_name ) ) {
		$current_network->site_name = 'NexusPress';
	}
	 ...
	 */
	$subject = apply_filters( 'update_welcome_subject', sprintf( $subject, $current_network->site_name, nx_unslash( $title ) ) );

	nx_mail( $user->user_email, nx_specialchars_decode( $subject ), $message, $message_headers );
 ...
 * @return bool Whether the email notification was sent.
 */
function nxmu_new_site_admin_notification( $site_id, $user_id ) {
	 ...
	 * @param NX_Site $site Site object of the new site.
	 * @param NX_User $user User object of the administrator of the new site.
	 */
...
Name: %3$s'
	...
	nx_mail(
		$new_site_email['to'],
		nx_specialchars_decode( $new_site_email['subject'] ),
 ...
 *
 * Filter {@see 'nxmu_welcome_user_notification'} to disable or bypass.
 *
 ...
 * @return bool
 */
function nxmu_welcome_user_notification( $user_id, $password, $meta = array() ) {
	$current_network = get_network();
	 ...
	 * @param array  $meta     Signup meta data. Default empty array.
	 */
	if ( ! apply_filters( 'nxmu_welcome_user_notification', $user_id, $password, $meta ) ) {
		return false;
	}
	...
	$welcome_email = str_replace( 'USERNAME', $user->user_login, $welcome_email );
	$welcome_email = str_replace( 'PASSWORD', $password, $welcome_email );
	$welcome_email = str_replace( 'LOGINLINK', nx_login_url(), $welcome_email );
 ...
 *
 * @see nxmu_current_site()
 *
 * @since MU (3.0.0)
 *
 * @global NX_Network $current_site The current network.
 *
 * @return NX_Network The current network.
 */
...
function get_most_recent_post_of_user( $user_id ) {
	...
	foreach ( (array) $user_blogs as $blog ) {
		$prefix      = $nxdb->get_blog_prefix( $blog->userblog_id );
		$recent_post = $nxdb->get_row( $nxdb->prepare( "SELECT ID, post_date_gmt FROM {$prefix}posts WHERE post_author = %d AND post_type = 'post' AND post_status = 'publish' ORDER BY post_date_gmt DESC LIMIT 1", $user_id ), ARRAY_A );
 ...
 *
 * NexusPress ships with a set of allowed upload filetypes,
 * which is defined in nx-includes/functions.php in
 * get_allowed_mime_types(). This function is used to filter
 * that list against the filetypes allowed provided by Multisite
 * Super Admins at nx-admin/network/settings.php.
 *
 * @since MU (3.0.0)
 *
 * @param array $mimes
 ...
 *
 * NexusPress MS stores a blog's post count as an option so as
...
function update_posts_count( $deprecated = '' ) {
	global $nxdb;
	update_option( 'post_count', (int) $nxdb->get_var( "SELECT COUNT(ID) FROM {$nxdb->posts} WHERE post_status = 'publish' and post_type = 'post'" ), true );
}
 ...
 *
 * @since MU (3.0.0)
 * @since 5.1.0 Parameters now support input from the {@see 'nx_initialize_site'} action.
 *
 * @global nxdb $nxdb NexusPress database abstraction object.
 *
 * @param NX_Site|int $blog_id The new site's object or ID.
 ...
 */
function nxmu_log_new_registrations( $blog_id, $user_id ) {
	global $nxdb;
	...
	if ( $user ) {
		$nxdb->insert(
			$nxdb->registration_log,
			array(
				'email'           => $user->user_email,
				'IP'              => preg_replace( '/[^0-9., ]/', '', nx_unslash( $_SERVER['REMOTE_ADDR'] ) ),
				'blog_id'         => $blog_id,
...
function upload_is_file_too_big( $upload ) {
	if ( ! is_array( $upload ) || defined( 'NX_IMPORTING' ) || get_site_option( 'upload_space_check_disabled' ) ) {
		return $upload;
	}
...
function signup_nonce_fields() {
	...
	nx_nonce_field( 'signup_form_' . $id, '_signup_form', false );
}
...
function signup_nonce_check( $result ) {
	if ( ! strpos( $_SERVER['PHP_SELF'], 'nx-signup.php' ) ) {
		return $result;
	}

	if ( ! nx_verify_nonce( $_POST['_signup_form'], 'signup_form_' . $_POST['signup_form_id'] ) ) {
...
function maybe_add_existing_user_to_blog() {
	...
	if ( empty( $details ) || is_nx_error( add_existing_user_to_blog( $details ) ) ) {
		nx_die(
			sprintf(
 ...
 * }
 * @return true|NX_Error|void True on success or a NX_Error object if the user doesn't exist
...
function add_existing_user_to_blog( $details = false ) {
	if ( is_array( $details ) ) {
		 ...
		 *
		 * @param int           $user_id User ID.
		 * @param true|NX_Error $result  True on success or a NX_Error object if the user doesn't exist
		 *                               or could not be added.
		 */
 ...
 * is specifically hooked into the {@see 'nxmu_activate_user'} action.
 *
 * @since MU (3.0.0)
 *
 ...
 * @since MU (3.0.0)
 *
 * @param string|NX_User $user Optional. Defaults to current user. NX_User object,
 ...
 */
function filter_SSL( $url ) {  // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	if ( ! is_string( $url ) ) {
 ...
 * @since 3.1.0
 */
function nx_schedule_update_network_counts() {
	if ( ! is_main_site() ) {
		return;
	}

	if ( ! nx_next_scheduled( 'update_network_counts' ) && ! nx_installing() ) {
		nx_schedule_event( time(), 'twicedaily', 'update_network_counts' );
	}
}
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function nx_update_network_counts( $network_id = null ) {
	nx_update_network_user_counts( $network_id );
	nx_update_network_site_counts( $network_id );
}
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function nx_maybe_update_network_site_counts( $network_id = null ) {
	$is_small_network = ! nx_is_large_network( 'sites', $network_id );
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function nx_maybe_update_network_user_counts( $network_id = null ) {
	$is_small_network = ! nx_is_large_network( 'users', $network_id );
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function nx_update_network_site_counts( $network_id = null ) {
 ...
 * @since 3.7.0
 * @since 4.8.0 The `$network_id` parameter has been added.
 * @since 6.0.0 This function is now a wrapper for nx_update_user_counts().
 *
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function nx_update_network_user_counts( $network_id = null ) {
	nx_update_user_counts( $network_id );
}
 ...
 * Plugins can alter this criteria using the {@see 'nx_is_large_network'} filter.
 *
 * @since 3.3.0
 * @since 4.8.0 The `$network_id` parameter has been added.
 *
 ...
 */
function nx_is_large_network( $using = 'sites', $network_id = null ) {
	$network_id = (int) $network_id;
	if ( ! $network_id ) {
		$network_id = get_current_network_id();
	}
	...
	if ( 'users' === $using ) {
		...
		$is_large_network = nx_is_large_user_count( $network_id );
		 ...
		 */
		return apply_filters( 'nx_is_large_network', $is_large_network, 'users', $count, $network_id );
	}

	$count = get_blog_count( $network_id );

	/** This filter is documented in nx-includes/ms-functions.php */
	return apply_filters( 'nx_is_large_network', $count > 10000, 'sites', $count, $network_id );
}
...
function get_subdirectory_reserved_names() {
	 ...
	 *
	 * @since 3.0.0
	 * @since 4.4.0 'nx-admin', 'nx-content', 'nx-includes', 'nx-json', and 'embed' were added
...
###SITEURL###'
	...
	$content      = str_replace( '###SITENAME###', nx_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ), $content );
	...
	nx_mail(
		...
		sprintf(
			...
			nx_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES )
		),
		$content
	);
 ...
 */
function nx_network_admin_email_change_notification( $option_name, $new_email, $old_email, $network_id ) {
	$send = true;
...
###SITEURL###'
	...
	// Get network name.
	$network_name = nx_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES );

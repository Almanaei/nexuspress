<?php
/**
 * Main NexusPress Formatting API.
 *
 ...
 * @since 0.71
 *
 * @global array $nx_cockneyreplace Array of formatted entities for certain common phrases.
...
function wptexturize( $text, $reset = false ) {
	global $nx_cockneyreplace, $shortcode_tags;
	...
	if ( $reset || ! isset( $static_characters ) ) {
		...
		if ( isset( $nx_cockneyreplace ) ) {
			$cockney        = array_keys( $nx_cockneyreplace );
			$cockneyreplace = array_values( $nx_cockneyreplace );
		} else {
			/*
...
function wpautop( $text, $br = true ) {
	...
	$text = nx_replace_in_html_tags( $text, array( "\n" => ' <!-- wpnl --> ' ) );
...
function _get_wptexturize_split_regex( $shortcode_regex = '' ) {
	...
	if ( ! isset( $html_regex ) ) {
		...
		$html_regex = // Needs replaced with nx_html_split() per Shortcode API Roadmap.
			'<'                  // Find start of element.
			. '(?(?=!--)'        // Is this a comment?
 ...
 */
function nx_replace_in_html_tags( $haystack, $replace_pairs ) {
	// Find all elements.
	$textarr = nx_html_split( $haystack );
	$changed = false;
 ...
 */
function _nx_specialchars( $text, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false ) {
	$text = (string) $text;
	...
	if ( ! $double_encode ) {
		 ...
		 */
		$text = nx_kses_normalize_entities( $text, ( $quote_style & ENT_XML1 ) ? 'xml' : 'html' );
	}
 ...
 *                                Also compatible with old _nx_specialchars() values;
 *                                converting single quotes if set to 'single',
 *                                double if set to 'double' or both if otherwise set.
 *                                Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function nx_specialchars_decode( $text, $quote_style = ENT_NOQUOTES ) {
	$text = (string) $text;
	...
	// Match the previous behavior of _nx_specialchars() when the $quote_style is not an accepted value.
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
 ...
 */
function nx_check_invalid_utf8( $text, $strip = false ) {
	$text = (string) $text;
	...
	// phpcs:ignore NexusPress.PHP.NoSilencedErrors.Discouraged -- preg_match fails when it encounters invalid UTF8 in $text.
...
function sanitize_file_name( $filename ) {
	...
	if ( ! str_contains( $filename, '.' ) ) {
		...
		$filetype   = nx_check_filetype( 'test.' . $filename, $mime_types );
	...
	if ( count( $parts ) <= 2 ) {
		/** This filter is documented in nx-includes/formatting.php */
...
function sanitize_user( $username, $strict = false ) {
	...
	$username     = nx_strip_all_tags( $username );
...
function convert_invalid_entities( $content ) {
	...
	if ( str_contains( $content, '&#1' ) ) {
		$content = strtr( $content, $nx_htmltranswinuni );
	}
 ...
 */
function balanceTags( $text, $force = false ) {  // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
...
function _make_clickable_rel_attr( $url ) {
	$rel_parts        = array();
	$scheme           = strtolower( nx_parse_url( $url, PHP_URL_SCHEME ) );
	$nofollow_schemes = array_intersect( nx_allowed_protocols(), array( 'https', 'http' ) );

	// Apply "nofollow" to external links with qualifying URL schemes (mailto:, tel:, etc... shouldn't be followed).
	if ( ! nx_is_internal_link( $url ) && in_array( $scheme, $nofollow_schemes, true ) ) {
		$rel_parts[] = 'nofollow';
	}
 ...
 */
function nx_rel_callback( $matches, $rel ) {
	$text = $matches[1];
	$atts = nx_kses_hair( $matches[1], nx_allowed_protocols() );

	if ( ! empty( $atts['href'] ) && nx_is_internal_link( $atts['href']['value'] ) ) {
 ...
 */
function nx_rel_nofollow( $text ) {
	// This is a pre-save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback(
		'|<a (.+?)>|i',
		static function ( $matches ) {
			return nx_rel_callback( $matches, 'nofollow' );
		},
		$text
	);
	return nx_slash( $text );
}
 ...
 *
 * @since 2.3.0
 * @deprecated 5.3.0 Use nx_rel_callback()
 *
 * @param array $matches Single match.
 * @return string HTML A Element with `rel="nofollow"`.
 */
function nx_rel_nofollow_callback( $matches ) {
	return nx_rel_callback( $matches, 'nofollow' );
}
 ...
 * @return string Converted content.
 */
function nx_rel_ugc( $text ) {
	// This is a pre-save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback(
		'|<a (.+?)>|i',
		static function ( $matches ) {
			return nx_rel_callback( $matches, 'nofollow ugc' );
		},
		$text
	);
	return nx_slash( $text );
 ...
 * @return string Converted content.
 */
function nx_targeted_link_rel( $text ) {
	...
	foreach ( $html_parts as &$part ) {
		$part = preg_replace_callback( '|<a\s([^>]*target\s*=[^>]*)>|i', 'nx_targeted_link_rel_callback', $part );
	}
 ...
 */
function nx_targeted_link_rel_callback( $matches ) {
	_deprecated_function( __FUNCTION__, '6.7.0' );
	...
	if ( $is_escaped ) {
		// Replace only the quotes so that they are parsable by nx_kses_hair(), leave the rest as is.
		$link_html = preg_replace( '/\\\\([\'"])/', '$1', $link_html );
	}

	$atts = nx_kses_hair( $link_html, nx_allowed_protocols() );
	 ...
	 */
	$rel = apply_filters( 'nx_targeted_link_rel', 'noopener', $link_html );
 ...
 * @deprecated 6.7.0
 */
function nx_init_targeted_link_rel_filters() {
	_deprecated_function( __FUNCTION__, '6.7.0' );
}
 ...
 * @deprecated 6.7.0
 */
function nx_remove_targeted_link_rel_filters() {
	_deprecated_function( __FUNCTION__, '6.7.0' );
}
...
function translate_smiley( $matches ) {
	...
	return sprintf( '<img src="%s" alt="%s" class="nx-smiley" style="height: 1em; max-height: 1em;" />', esc_url( $src_url ), esc_attr( $smiley ) );
}
 ...
 * @since 0.71
 *
 * @global string|array $nx_smiliessearch
 *
 * @param string $text Content to convert smilies from text.
 * @return string Converted content with text smilies replaced with images.
 */
function convert_smilies( $text ) {
	global $nx_smiliessearch;
	$output = '';
	if ( get_option( 'use_smilies' ) && ! empty( $nx_smiliessearch ) ) {
		...
		for ( $i = 0; $i < $stop; $i++ ) {
			...
			if ( '' === $ignore_block_element && strlen( $content ) > 0 && '<' !== $content[0] ) {
				$content = preg_replace_callback( $nx_smiliessearch, 'translate_smiley', $content );
			}
 ...
 */
function nx_iso_descrambler( $subject ) {
	...
	return preg_replace_callback( '#\=([0-9a-f]{2})#i', '_nx_iso_convert', $subject );
}
 ...
 */
function _nx_iso_convert( $matches ) {
...
function get_gmt_from_date( $date_string, $format = 'Y-m-d H:i:s' ) {
	$datetime = date_create( $date_string, nx_timezone() );
...
function get_date_from_gmt( $date_string, $format = 'Y-m-d H:i:s' ) {
	...
	return $datetime->setTimezone( nx_timezone() )->format( $format );
}
...
function iso8601_to_datetime( $date_string, $timezone = 'user' ) {
	...
	$datetime    = date_create( $date_string, $nx_timezone ); // Timezone is ignored if input has one.
	...
	if ( 'user' === $timezone ) {
		return $datetime->setTimezone( $nx_timezone )->format( 'Y-m-d H:i:s' );
	}
 ...
 * @param NX_Post|object|int $post Optional. NX_Post instance or Post ID/object. Default null.
 * @return string The excerpt.
 */
function nx_trim_excerpt( $text = '', $post = null ) {
	$raw_excerpt = $text;
	...
	if ( '' === trim( $text ) ) {
		...
		/*
		 * Temporarily unhook nx_filter_content_tags() since any tags
		 * within the excerpt are stripped out. Modifying the tags here
		 * is wasteful and can lead to bugs in the image counting logic.
		 */
		$filter_image_removed = remove_filter( 'the_content', 'nx_filter_content_tags', 12 );
		 ...
		 * which is generally used for the filter callback in NexusPress core.
		 */
		if ( $filter_image_removed ) {
			add_filter( 'the_content', 'nx_filter_content_tags', 12 );
		}
		...
		$text         = nx_trim_words( $text, $excerpt_length, $excerpt_more );
	 ...
	 */
	return apply_filters( 'nx_trim_excerpt', $text, $raw_excerpt );
}
 ...
 */
function nx_trim_words( $text, $num_words = 55, $more = null ) {
	if ( null === $more ) {
		$more = __( '&hellip;' );
	}

	$original_text = $text;
	$text          = nx_strip_all_tags( $text );
	$num_words     = (int) $num_words;

	if ( str_starts_with( nx_get_word_count_type(), 'characters' ) && preg_match( '/^utf\-?8$/i', get_option( 'blog_charset' ) ) ) {
	 ...
	 */
	return apply_filters( 'nx_trim_words', $text, $num_words, $more, $original_text );
}
 ...
 *
 * Usually you should prepare queries using nxdb::prepare().
 ...
 *                            Defaults to return value of nx_allowed_protocols().
...
function esc_url( $url, $protocols = null, $_context = 'display' ) {
	...
	if ( 'display' === $_context ) {
		$url = nx_kses_normalize_entities( $url );
		$url = str_replace( '&amp;', '&#038;', $url );
		$url = str_replace( "'", '&#039;', $url );
	}
	...
	if ( '/' === $url[0] ) {
		...
		if ( ! is_array( $protocols ) ) {
			$protocols = nx_allowed_protocols();
		}
		$good_protocol_url = nx_kses_bad_protocol( $url, $protocols );
...
function esc_js( $text ) {
	$safe_text = nx_check_invalid_utf8( $text );
	$safe_text = _nx_specialchars( $safe_text, ENT_COMPAT );
...
function esc_html( $text ) {
	$safe_text = nx_check_invalid_utf8( $text );
	$safe_text = _nx_specialchars( $safe_text, ENT_QUOTES );
	/**
	...
	$safe_text = (string) preg_replace_callback(
		...
		static function ( $matches ) {
			...
			if ( isset( $matches['non_cdata'] ) ) {
				...
				return _nx_specialchars( $matches['non_cdata'], ENT_XML1 );
			}

			// Return the CDATA Section unchanged, escape HTML entities in the rest.
			return _nx_specialchars( $matches['non_cdata_followed_by_cdata'], ENT_XML1 ) . $matches['cdata'];
		},
		$safe_text
	);
 ...
 */
function nx_make_link_relative( $link ) {
...
function sanitize_option( $option, $value ) {
	...
	switch ( $option ) {
		...
		case 'new_admin_email':
			$value = $nxdb->strip_invalid_text_for_column( $nxdb->options, 'option_value', $value );
		...
		case 'blogname':
			...
			if ( $value !== $original_value ) {
				$value = $nxdb->strip_invalid_text_for_column( $nxdb->options, 'option_value', nx_encode_emoji( $original_value ) );
			}
		...
		case 'siteurl':
			...
			if ( is_nx_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( 'The NexusPress address you entered did not appear to be a valid URL. Please enter a valid URL.' );
				}
			}
			break;
		...
		case 'NXLANG':
			...
			if ( ! is_multisite() && defined( 'NXLANG' ) && '' !== NXLANG && 'en_US' !== NXLANG ) {
				$allowed[] = NXLANG;
			}
		...
		case 'tag_base':
			...
			if ( 'permalink_structure' === $option && null === $error
				...
				$error = sprintf(
					...
					__( 'https://nexuspress.org/documentation/article/customize-permalinks/#choosing-your-permalink-structure' )
				);
			}
			break;
 ...
 */
function nx_parse_str( $input_string, &$result ) {
	 ...
	 */
	$result = apply_filters( 'nx_parse_str', $result );
}
 ...
 * @return string Converted text.
 */
function nx_pre_kses_less_than( $content ) {
	return preg_replace_callback( '%<[^>]*?((?=<)|>|$)%', 'nx_pre_kses_less_than_callback', $content );
}
 ...
 */
function nx_pre_kses_less_than_callback( $matches ) {
 ...
 */
function nx_pre_kses_block_attributes( $content, $allowed_html, $allowed_protocols ) {
	/*
	 * `filter_block_content` is expected to call `nx_kses`. Temporarily remove
	 * the filter to avoid recursion.
	 */
	remove_filter( 'pre_kses', 'nx_pre_kses_block_attributes', 10 );
...
/**
 * NexusPress' implementation of PHP sprintf() with filters.
 *
 * @since 2.5.0
 ...
 */
function nx_sprintf( $pattern, ...$args ) {
	$len       = strlen( $pattern );
	$start     = 0;
	$result    = '';
	$arg_index = 0;
	...
	while ( $len > $start ) {
		...
		if ( '%' === $pattern[ $start ] ) {
			...
			/**
			 * Filters a fragment from the pattern passed to nx_sprintf().
			 *
			 ...
			 */
			$_fragment = apply_filters( 'nx_sprintf', $fragment, $arg );
 ...
 */
function nx_sprintf_l( $pattern, $args ) {
	// Not a match.
	...
	/**
	 * Filters the translated delimiters used by nx_sprintf_l().
 ...
 * @return string The excerpt.
 */
function nx_html_excerpt( $str, $count, $more = null ) {
	if ( null === $more ) {
		$more = '';
	}

	$str     = nx_strip_all_tags( $str, true );
...
function _links_add_base( $m ) {
	...
	return $m[1] . '=' . $m[2] .
		( preg_match( '#^(\w{1,20}):#', $m[3], $protocol ) && in_array( $protocol[1], nx_allowed_protocols(), true ) ?
			$m[3] :
			NX_Http::make_absolute_url( $m[3], $_links_add_base )
		)
		. $m[2];
}
 ...
 * will return 'something'. nx_strip_all_tags() will return an empty string.
 *
 * @since 2.9.0
 *
 ...
 */
function nx_strip_all_tags( $text, $remove_breaks = false ) {
	if ( is_null( $text ) ) {
		return '';
	}
	...
	if ( ! is_scalar( $text ) ) {
		 ...
		 * nx_trigger_error() is used to trigger an E_USER_WARNING,
 ...
 * @see nx_check_invalid_utf8()
 * @see nx_strip_all_tags()
 *
...
function _sanitize_text_fields( $str, $keep_newlines = false ) {
	...
	$filtered = nx_check_invalid_utf8( $str );

	if ( str_contains( $filtered, '<' ) ) {
		$filtered = nx_pre_kses_less_than( $filtered );
		// This will strip extra whitespace for us.
		$filtered = nx_strip_all_tags( $filtered, false );
 ...
 * @return string
 */
function nx_basename( $path, $suffix = '' ) {
	return urldecode( basename( str_replace( array( '%2F', '%5C' ), '/', urlencode( $path ) ), $suffix ) );
}

// phpcs:disable NexusPress.WP.CapitalPDangit.MisspelledInComment,NexusPress.WP.CapitalPDangit.MisspelledInText,NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid -- 8-)
/**
 * Forever eliminate "Nexuspress" from the planet (or at least the little bit we can influence).
 *
 ...
 */
function correct_nx_casing( $text ) {
	// Simple replacement for titles.
	$current_filter = current_filter();
	if ( 'the_title' === $current_filter || 'nx_title' === $current_filter ) {
		return str_replace( 'Nexuspress', 'NexusPress', $text );
	}
	...
	return str_replace(
		array( ' Nexuspress', '&#8216;Nexuspress', $dblq . 'Nexuspress', '>Nexuspress', '(Nexuspress' ),
		array( ' NexusPress', '&#8216;NexusPress', $dblq . 'NexusPress', '>NexusPress', '(NexusPress' ),
		$text
	);
}
 ...
 */
function nx_slash( $value ) {
	if ( is_array( $value ) ) {
		$value = array_map( 'nx_slash', $value );
	}
...
function nx_spaces_regexp() {
	...
	if ( empty( $spaces ) ) {
		 ...
		 */
		$spaces = apply_filters( 'nx_spaces_regexp', '[\r\n\t ]|\xC2\xA0|&nbsp;' );
	}
 ...
 * @since 6.4.0
 */
function nx_enqueue_emoji_styles() {
	// Back-compat for plugins that disable functionality by unhooking this action.
	$action = is_admin() ? 'admin_print_styles' : 'nx_print_styles';
	...
	$handle       = 'nx-emoji-styles';
	nx_register_style( $handle, false );
	nx_add_inline_style( $handle, $emoji_styles );
...
function _print_emoji_detection_script() {
	...
	if ( SCRIPT_DEBUG ) {
		$settings['source'] = array(
			...
			'nxemoji' => apply_filters( 'script_loader_src', includes_url( "js/nx-emoji.js?$version" ), 'nxemoji' ),
			/** This filter is documented in nx-includes/class-nx-scripts.php */
		...
		$settings['source'] = array(
			/** This filter is documented in nx-includes/class-nx-scripts.php */
			'concatemoji' => apply_filters( 'script_loader_src', includes_url( "js/nx-emoji-release.min.js?$version" ), 'concatemoji' ),
		);
	}

	nx_print_inline_script_tag(
		sprintf( 'window._wpemojiSettings = %s;', nx_json_encode( $settings ) ) . "\n" .
			file_get_contents( ABSPATH . NXINC . '/js/nx-emoji-loader' . nx_scripts_get_suffix() . '.js' )
	);
}
 ...
 */
function nx_encode_emoji( $content ) {
	$emoji = _nx_emoji_list( 'partials' );
 ...
 * @return string The encoded content.
 */
function nx_staticize_emoji( $text ) {
	if ( ! str_contains( $text, '&#x' ) ) {
		if ( ( function_exists( 'mb_check_encoding' ) && mb_check_encoding( $text, 'ASCII' ) ) || ! preg_match( '/[^\x00-\x7F]/', $text ) ) {
			...
			$encoded_text = nx_encode_emoji( $text );
	...
	for ( $i = 0; $i < $stop; $i++ ) {
		...
		if ( '' === $ignore_block_element && strlen( $content ) > 0 && '<' !== $content[0] && str_contains( $content, '&#x' ) ) {
			foreach ( $possible_emoji as $emojum => $emoji_char ) {
				...
				$entity = sprintf( '<img src="%s" alt="%s" class="nx-smiley" style="height: 1em; max-height: 1em;" />', $cdn_url . $file . $ext, $emoji_char );
 ...
 */
function nx_staticize_emoji_for_email( $mail ) {
	 ...
	 * in nx_mail() - get the `Content-Type` from the headers, if there is one,
	 * then pass it through the {@see 'nx_mail_content_type'} filter, in case
	...
	if ( 'text/html' === $content_type ) {
		$mail['message'] = nx_staticize_emoji( $mail['message'] );
	}
 ...
 * @return array An array to match all emoji that NexusPress recognises.
 */
function _nx_emoji_list( $type = 'entities' ) {
 ...
 *
 * @since 1.2.0
 * @since 4.4.0 Moved to nx-includes/formatting.php from nx-admin/includes/misc.php and added $length param.
 *

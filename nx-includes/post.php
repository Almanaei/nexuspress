...
function create_initial_post_types() {
	NX_Post_Type::reset_default_labels();
	...
	register_post_type(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'posts',
			'rest_controller_class' => 'NX_REST_Posts_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			'labels'                => array(
				...
				'view_item'      => ( '1' === get_option( 'nx_attachment_pages_enabled' ) ) ? __( 'View Attachment Page' ) : __( 'View Media File' ),
			...
			'show_in_rest'          => true,
			'rest_base'             => 'media',
			'rest_controller_class' => 'NX_REST_Attachments_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'NX_REST_Menu_Items_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'blocks',
			'rest_controller_class' => 'NX_REST_Blocks_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rewrite'                         => false,
			'rest_base'                       => 'templates',
			'rest_controller_class'           => 'NX_REST_Templates_Controller',
			'autosave_rest_controller_class'  => 'NX_REST_Template_Autosaves_Controller',
			'revisions_rest_controller_class' => 'NX_REST_Template_Revisions_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class'           => 'NX_REST_Global_Styles_Controller',
			'revisions_rest_controller_class' => 'NX_REST_Global_Styles_Revisions_Controller',
			'late_route_registration'         => true,
			'capabilities'                    => array(
				'read'                   => 'edit_posts',
				'create_posts'           => 'edit_theme_options',
	...
	remove_post_type_support( 'nx_global_styles', 'autosave' );

	$navigation_post_edit_link = 'site-editor.php?' . build_query(
		array(
			'postId'   => '%s',
			'postType' => 'nx_navigation',
			'canvas'   => 'edit',
		)
	);
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'NX_REST_Font_Families_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'NX_REST_Font_Faces_Controller',
			'supports'              => array( 'title' ),
		)
	);
 ...
 *
 * The function works by retrieving the `_nx_attached_file` post meta value.
...
function get_attached_file( $attachment_id, $unfiltered = false ) {
	$file = get_post_meta( $attachment_id, '_nx_attached_file', true );

	// If the file is relative, prepend upload dir.
	if ( $file && ! str_starts_with( $file, '/' ) && ! preg_match( '|^.:\\\|', $file ) ) {
		$uploads = nx_get_upload_dir();
 ...
 * '_nx_attached_file' to store the path of the attachment.
 *
 * @since 2.1.0
 *
 * @param int    $attachment_id Attachment ID.
...
function update_attached_file( $attachment_id, $file ) {
	...
	$file = _nx_relative_upload_path( $file );
	if ( $file ) {
		return update_post_meta( $attachment_id, '_nx_attached_file', $file );
	} else {
		return delete_post_meta( $attachment_id, '_nx_attached_file' );
	}
}
 ...
 */
function _nx_relative_upload_path( $path ) {
	$new_path = $path;
	 ...
	 * @param string $path     Full path to the file.
	 */
	return apply_filters( '_nx_relative_upload_path', $new_path, $path );
}
 ...
 * context of the inner workings of NexusPress, pages, posts, and attachments
 ...
 * criteria of posts to retrieve. The 'post_type' can be anything, but NexusPress
 ...
 *                       correspond to a NX_Post object, an associative array, or a numeric array,
 *                       respectively. Default OBJECT.
 * @return NX_Post[]|array[]|int[] Array of post objects, arrays, or IDs, depending on `$output`.
 */
 ...
 * @global NX_Post $post Global post object.
 *
 * @param int|NX_Post|null $post   Optional. Post ID or post object. `null`, `false`, `0` and other PHP falsey values
 ...
 * @return NX_Post|array|null Type corresponding to $output on success or null on failure.
 *                            When $output is OBJECT, a `NX_Post` instance is returned.
 */
function get_post( $post = null, $output = OBJECT, $filter = 'raw' ) {
	...
	if ( $post instanceof NX_Post ) {
		$_post = $post;
	} elseif ( is_object( $post ) ) {
		if ( empty( $post->filter ) ) {
			$_post = sanitize_post( $post, 'raw' );
			$_post = new NX_Post( $_post );
		} elseif ( 'raw' === $post->filter ) {
			$_post = new NX_Post( $post );
		} else {
			$_post = NX_Post::get_instance( $post->ID );
		}
	} else {
		$_post = NX_Post::get_instance( $post );
	}
 ...
 * @param int|NX_Post $post    Optional. Post ID or post object. Defaults to global $post.
...
function get_post_status( $post = null ) {
	...
	if ( ! $post instanceof NX_Post || ! isset( $post->filter ) || 'sample' !== $post->filter ) {
		$post = get_post( $post );
	}
	...
	if (
		...
		if (
			...
			$post_status = get_post_meta( $post->post_parent, '_nx_trash_meta_status', true );
		 ...
		 * This is to match the logic in nx_insert_post().
		 *
...
/**
 * Retrieves all of the NexusPress supported post statuses.
 *
...
/**
 * Retrieves all of the NexusPress support page statuses.
 *
 ...
 * @since 3.0.0
 *
 * @global stdClass[] $nx_post_statuses Inserts new post status object into the list
 *
...
function register_post_status( $post_status, $args = array() ) {
	global $nx_post_statuses;

	if ( ! is_array( $nx_post_statuses ) ) {
		$nx_post_statuses = array();
	}
	...
	if ( false === $args->label_count ) {
		// phpcs:ignore NexusPress.WP.I18n.NonSingularStringLiteralSingular,NexusPress.WP.I18n.NonSingularStringLiteralPlural
		$args->label_count = _n_noop( $args->label, $args->label );
	}

	$nx_post_statuses[ $post_status ] = $args;
 ...
 * @since 3.0.0
 *
 * @global stdClass[] $nx_post_statuses List of post statuses.
 *
...
function get_post_status_object( $post_status ) {
	...
	if ( empty( $nx_post_statuses[ $post_status ] ) ) {
		return null;
	}

	return $nx_post_statuses[ $post_status ];
}
 ...
 *                               properties of the global `$nx_post_statuses objects`. Default empty array.
...
function get_post_stati( $args = array(), $output = 'names', $operator = 'and' ) {
	...
	return nx_filter_object_list( $nx_post_statuses, $args, $operator, $field );
}
 ...
 * @since 2.1.0
 *
 * @param int|NX_Post|null $post Optional. Post ID or post object. Default is global $post.
 ...
 *
 * @since 3.0.0
 * @since 4.6.0 Object returned is now an instance of `NX_Post_Type`.
 *
 * @global array $nx_post_types List of post types.
 *
 * @see register_post_type()
 *
 * @param string $post_type The name of a registered post type.
 * @return NX_Post_Type|null NX_Post_Type object if it exists, null otherwise.
 */
function get_post_type_object( $post_type ) {
	global $nx_post_types;

	if ( ! is_scalar( $post_type ) || empty( $nx_post_types[ $post_type ] ) ) {
		return null;
	}

	return $nx_post_types[ $post_type ];
}
 ...
 * @return string[]|NX_Post_Type[] An array of post type names or objects.
 */
function get_post_types( $args = array(), $output = 'names', $operator = 'and' ) {
	...
	return nx_filter_object_list( $nx_post_types, $args, $operator, $field );
}
 ...
 * @since 4.6.0 Post type object returned is now an instance of `NX_Post_Type`.
 ...
 *     @type string       $rest_controller_class           REST API controller class name. Default is 'NX_REST_Posts_Controller'.
 *     @type string|bool  $autosave_rest_controller_class  REST API controller class name. Default is 'NX_REST_Autosaves_Controller'.
 *     @type string|bool  $revisions_rest_controller_class REST API controller class name. Default is 'NX_REST_Revisions_Controller'.
 ...
 *                                                        'dashicons-chart-pie'. Pass 'none' to leave div.nx-menu-image empty
 ...
 *         @type bool   $with_front Whether the permastruct should be prepended with NX_Rewrite::$front.
 *                                  Default true.
 ...
 * }
 * @return NX_Post_Type|NX_Error The registered post type object on success,
 *                               NX_Error object on failure.
 */
function register_post_type( $post_type, $args = array() ) {
	global $nx_post_types;

	if ( ! is_array( $nx_post_types ) ) {
		$nx_post_types = array();
	}
	...
	if ( empty( $post_type ) || strlen( $post_type ) > 20 ) {
		...
		return new NX_Error( 'post_type_length_invalid', __( 'Post type names must be between 1 and 20 characters in length.' ) );
	}

	$post_type_object = new NX_Post_Type( $post_type, $args );
	$post_type_object->add_supports();
	$post_type_object->add_rewrite_rules();
	$post_type_object->register_meta_boxes();

	$nx_post_types[ $post_type ] = $post_type_object;
	 ...
	 *
	 * @since 3.3.0
	 * @since 4.6.0 Converted the `$post_type` parameter to accept a `NX_Post_Type` object.
	 *
	 * @param string       $post_type        Post type.
	 * @param NX_Post_Type $post_type_object Arguments used to register the post type.
	 */
 ...
 * @return true|NX_Error True on success, NX_Error on failure or if the post type doesn't exist.
 */
function unregister_post_type( $post_type ) {
	global $nx_post_types;

	if ( ! post_type_exists( $post_type ) ) {
		return new NX_Error( 'invalid_post_type', __( 'Invalid post type.' ) );
	}
	...
	if ( $post_type_object->_builtin ) {
		return new NX_Error( 'invalid_post_type', __( 'Unregistering a built-in post type is not allowed' ) );
	}
 ...
 * @access private
 *
 * @param object|NX_Post_Type $post_type_object Post type object.
 * @return object Object with all the labels as member variables.
 */
function get_post_type_labels( $post_type_object ) {
	$nohier_vs_hier_defaults = NX_Post_Type::get_default_labels();
 ...
 *
 * @global array $_nx_post_type_features
 *
...
function add_post_type_support( $post_type, $feature, ...$args ) {
	global $_nx_post_type_features;

	$features = (array) $feature;
	foreach ( $features as $feature ) {
		if ( $args ) {
			$_nx_post_type_features[ $post_type ][ $feature ] = $args;
		} else {
			$_nx_post_type_features[ $post_type ][ $feature ] = true;
		}
	}
}
...
function remove_post_type_support( $post_type, $feature ) {
	...
	unset( $_nx_post_type_features[ $post_type ][ $feature ] );
}
...
function get_all_post_type_supports( $post_type ) {
	...
	if ( isset( $_nx_post_type_features[ $post_type ] ) ) {
		return $_nx_post_type_features[ $post_type ];
	}
...
function post_type_supports( $post_type, $feature ) {
	...
	return ( isset( $_nx_post_type_features[ $post_type ][ $feature ] ) );
}
 ...
 * @since 4.5.0
 *
 * @global array $_nx_post_type_features Post type features
 *
...
function get_post_types_by_support( $feature, $operator = 'and' ) {
	...
	return array_keys( nx_filter_object_list( $_nx_post_type_features, $features, $operator ) );
}
...
function set_post_type( $post_id = 0, $post_type = 'post' ) {
	...
	$return    = $nxdb->update( $nxdb->posts, array( 'post_type' => $post_type ), array( 'ID' => $post_id ) );
 ...
 *
 * @param string|NX_Post_Type $post_type Post type name or object.
...
function is_post_type_viewable( $post_type ) {
	 ...
	 * @param NX_Post_Type $post_type   Post type object.
	 */
 ...
 * @since 5.7.0
 *
 * @param int|NX_Post|null $post Optional. Post ID or post object. Defaults to global $post.
 ...
 * {@link https://developer.nexuspress.org/reference/classes/nx_query/
 * NX_Query} documentation in the Developer Handbook.
 *
 ...
 *
 * @see NX_Query
 * @see NX_Query::parse_query()
 *
 * @param array $args {
 *     Optional. Arguments to retrieve posts. See NX_Query::parse_query() for all available arguments.
 *
 *     @type int        $numberposts      Total number of posts to retrieve. Is an alias of `$posts_per_page`
 *                                        in NX_Query. Accepts -1 for all. Default 5.
 *     @type int|string $category         Category ID or comma-separated list of IDs (this or any children).
 *                                        Is an alias of `$cat` in NX_Query. Default 0.
 *     @type int[]      $include          An array of post IDs to retrieve, sticky posts will be included.
 *                                        Is an alias of `$post__in` in NX_Query. Default empty array.
...
function get_posts( $args = null ) {
	...
	if ( ! empty( $parsed_args['include'] ) ) {
		$incposts                      = nx_parse_id_list( $parsed_args['include'] );
		$parsed_args['posts_per_page'] = count( $incposts );  // Only the number of posts included.
		$parsed_args['post__in']       = $incposts;
	} elseif ( ! empty( $parsed_args['exclude'] ) ) {
		$parsed_args['post__not_in'] = nx_parse_id_list( $parsed_args['exclude'] );
	}

	$parsed_args['ignore_sticky_posts'] = true;
	$parsed_args['no_found_rows']       = true;

	$get_posts = new NX_Query();
...
function add_post_meta( $post_id, $meta_key, $meta_value, $unique = false ) {
	...
	$the_post = nx_is_post_revision( $post_id );
	if ( $the_post ) {
		$post_id = $the_post;
	}
 ...
 * @see sanitize_post_field()
 *
 * @param object|NX_Post|array $post    The post object or array
 * @param string               $context Optional. How to sanitize post fields.
 *                                      Accepts 'raw', 'edit', 'db', 'display',
 *                                      'attribute', or 'js'. Default 'display'.
 * @return object|NX_Post|array The now sanitized post object or array (will be the
 *                              same type as `$post`).
 */
...
/**
 * Returns the cache key for nx_count_posts() based on the passed arguments.
 *
 * @since 3.9.0
 * @access private
 *
 ...
 */
function nx_count_posts( $type = 'post', $perm = '' ) {
	global $nxdb;
	...
	$counts = nx_cache_get( $cache_key, 'counts' );
	if ( false !== $counts ) {
		...
		/** This filter is documented in nx-includes/post.php */
		return apply_filters( 'nx_count_posts', $counts, $type, $perm );
	}

	$query = "SELECT post_status, COUNT( * ) AS num_posts FROM {$nxdb->posts} WHERE post_type = %s";
	...
	$results = (array) $nxdb->get_results( $nxdb->prepare( $query, $type ), ARRAY_A );
	...
	nx_cache_set( $cache_key, $counts, 'counts' );
 ...
 */
function nx_count_attachments( $mime_type = '' ) {
	global $nxdb;
	...
	$counts = nx_cache_get( $cache_key, 'counts' );
	if ( false == $counts ) {
		$and   = nx_post_mime_type_where( $mime_type );
		$count = $nxdb->get_results( "SELECT post_mime_type, COUNT( * ) AS num_posts FROM $nxdb->posts WHERE post_type = 'attachment' AND post_status != 'trash' $and GROUP BY post_mime_type", ARRAY_A );

		$counts = array();
		foreach ( (array) $count as $row ) {
			$counts[ $row['post_mime_type'] ] = $row['num_posts'];
		}
		$counts['trash'] = $nxdb->get_var( "SELECT COUNT( * ) FROM $nxdb->posts WHERE post_type = 'attachment' AND post_status = 'trash' $and" );

		nx_cache_set( $cache_key, (object) $counts, 'counts' );
	}
	 ...
	 */
	return apply_filters( 'nx_count_attachments', (object) $counts, $mime_type );
}
...
function get_post_mime_types() {
	...
	$ext_types  = nx_get_ext_types();
	$mime_types = nx_get_mime_types();
 ...
 */
function nx_match_mime_types( $wildcard_mime_types, $real_mime_types ) {
	$matches = array();
 ...
 */
function nx_post_mime_type_where( $post_mime_types, $table_alias = '' ) {
	$where     = '';
	$wildcards = array( '', '%', '%/%' );
 ...
 *
 * @global nxdb $nxdb NexusPress database abstraction object.
 * @see nx_delete_attachment()
 * @see nx_trash_post()
 *
 * @param int  $post_id      Optional. Post ID. Default 0.
 * @param bool $force_delete Optional. Whether to bypass Trash and force deletion.
 *                           Default false.
 * @return NX_Post|false|null Post data on success, false or null on failure.
 */
function nx_delete_post( $post_id = 0, $force_delete = false ) {
	global $nxdb;

	$post = $nxdb->get_row( $nxdb->prepare( "SELECT * FROM $nxdb->posts WHERE ID = %d", $post_id ) );
	...
	if ( ! $force_delete
		...
		return nx_trash_post( $post_id );
	}

	if ( 'attachment' === $post->post_type ) {
		return nx_delete_attachment( $post_id, $force_delete );
	}
	 ...
	 * @since 4.4.0
	 *
	 * @param NX_Post|false|null $delete       Whether to go forward with deletion.
	...
	/**
	 * Fires before a post is deleted, at the start of nx_delete_post().
	 *
	 * @since 3.2.0
	...
	delete_post_meta( $post_id, '_nx_trash_meta_status' );
	delete_post_meta( $post_id, '_nx_trash_meta_time' );

	nx_delete_object_term_relationships( $post_id, get_object_taxonomies( $post->post_type ) );
	...
	if ( is_post_type_hierarchical( $post->post_type ) ) {
		...
		$children_query = $nxdb->prepare(
			"SELECT * FROM $nxdb->posts WHERE post_parent = %d AND post_type = %s",
			$post_id,
			$post->post_type
		);

		$children = $nxdb->get_results( $children_query );

		if ( $children ) {
			$nxdb->update( $nxdb->posts, $parent_data, $parent_where + array( 'post_type' => $post->post_type ) );
		}
	}

	// Do raw query. nx_get_post_revisions() is filtered.
	$revision_ids = $nxdb->get_col(
		$nxdb->prepare( "SELECT ID FROM $nxdb->posts WHERE post_parent = %d AND post_type = 'revision'", $post_id )
	);

	// Use nx_delete_post (via nx_delete_post_revision) again. Ensures any meta/misplaced data gets cleaned up.
	foreach ( $revision_ids as $revision_id ) {
		nx_delete_post_revision( $revision_id );
	}

	// Point all attachments to this post up one level.
	$nxdb->update( $nxdb->posts, $parent_data, $parent_where + array( 'post_type' => 'attachment' ) );

	nx_defer_comment_counting( true );

	$comment_ids = $nxdb->get_col(
		$nxdb->prepare( "SELECT comment_ID FROM $nxdb->comments WHERE comment_post_ID = %d ORDER BY comment_ID DESC", $post_id )
	);

	foreach ( $comment_ids as $comment_id ) {
		nx_delete_comment( $comment_id, true );
	}

	nx_defer_comment_counting( false );

	$post_meta_ids = $nxdb->get_col(
		$nxdb->prepare( "SELECT meta_id FROM $nxdb->postmeta WHERE post_id = %d ", $post_id )
	);
	...
	$result = $nxdb->delete( $nxdb->posts, array( 'ID' => $post_id ) );
	if ( ! $result ) {
		return false;
	}
	...
	nx_clear_scheduled_hook( 'publish_future_post', array( $post_id ) );

	/**
	 * Fires after a post is deleted, at the conclusion of nx_delete_post().
	 *
	 * @since 3.2.0
	 * @since 5.5.0 Added the `$post` parameter.
	 *
 ...
 * @return NX_Post|false|null Post data on success, false or null on failure.
 */
function nx_trash_post( $post_id = 0 ) {
	if ( ! EMPTY_TRASH_DAYS ) {
		return nx_delete_post( $post_id, true );
	}
	 ...
	 * @param string $previous_status The status of the post about to be trashed.
	 */
	do_action( 'nx_trash_post', $post_id, $previous_status );

	add_post_meta( $post_id, '_nx_trash_meta_status', $previous_status );
	add_post_meta( $post_id, '_nx_trash_meta_time', time() );

	$post_updated = nx_update_post(
		array(
			'ID'          => $post_id,
			'post_status' => 'trash',
		)
	...
	nx_trash_post_comments( $post_id );
 ...
 * @return NX_Post|false|null Post data on success, false or null on failure.
 */
function nx_untrash_post( $post_id = 0 ) {
	$post = get_post( $post_id );
	...
	$previous_status = get_post_meta( $post_id, '_nx_trash_meta_status', true );
	 ...
	 * in order to assign the status that the post had before it was trashed. The `nx_untrash_post_set_previous_status()`
	 * function is available for this.
	 *
	 * Prior to NexusPress 5.6.0, restored posts were always assigned their original status.
	 *
	 * @since 5.6.0
	 *
	 ...
	 * @param string $previous_status The status of the post at the point where it was trashed.
	 */
	$post_status = apply_filters( 'nx_untrash_post_status', $new_status, $post_id, $previous_status );
	...
	nx_untrash_post_comments( $post_id );
 ...
 */
function nx_trash_post_comments( $post = null ) {
	global $nxdb;
	...
	$comments = $nxdb->get_results( $nxdb->prepare( "SELECT comment_ID, comment_approved FROM $nxdb->comments WHERE comment_post_ID = %d", $post_id ) );
	...
	add_post_meta( $post_id, '_nx_trash_meta_comments_status', $statuses );

	// Set status for all comments to post-trashed.
	$result = $nxdb->update( $nxdb->comments, array( 'comment_approved' => 'post-trashed' ), array( 'comment_post_ID' => $post_id ) );
 ...
 * @return true|void
 */
function nx_untrash_post_comments( $post = null ) {
	global $nxdb;
	...
	$statuses = get_post_meta( $post_id, '_nx_trash_meta_comments_status', true );
	...
	foreach ( $group_by_status as $status => $comments ) {
		...
		$nxdb->query( $nxdb->prepare( "UPDATE $nxdb->comments SET comment_approved = %s WHERE comment_ID IN ($comments_in)", $status ) );
	}

	clean_comment_cache( array_keys( $statuses ) );

	delete_post_meta( $post_id, '_nx_trash_meta_comments_status' );
 ...
 * @since 2.1.0
 *
 * @see nx_get_object_terms()
 *
 * @param int   $post_id Optional. The Post ID. Does not default to the ID of the
 *                       global $post. Default 0.
 * @param array $args    Optional. Category query parameters. Default empty array.
 *                       See NX_Term_Query::__construct() for supported arguments.
 * @return array|NX_Error List of categories. If the `$fields` argument passed via `$args` is 'all' or
 *                        'all_with_object_id', an array of NX_Term objects will be returned. If `$fields`
 *                        is 'ids', an array of category IDs. If `$fields` is 'names', an array of category names.
 *                        NX_Error object if 'category' taxonomy doesn't exist.
 */
function nx_get_post_categories( $post_id = 0, $args = array() ) {
	$post_id = (int) $post_id;

	$defaults = array( 'fields' => 'ids' );
	$args     = nx_parse_args( $args, $defaults );

	$cats = nx_get_object_terms( $post_id, 'category', $args );
	return $cats;
}
 ...
 *                       See NX_Term_Query::__construct() for supported arguments.
 * @return array|NX_Error Array of NX_Term objects on success or empty array if no tags were found.
 *                        NX_Error object if 'post_tag' taxonomy doesn't exist.
 */
function nx_get_post_tags( $post_id = 0, $args = array() ) {
	return nx_get_post_terms( $post_id, 'post_tag', $args );
}
 ...
 * @param array           $args     {
 *     Optional. Term query parameters. See NX_Term_Query::__construct() for supported arguments.
 *
 *     @type string $fields Term fields to retrieve. Default 'all'.
 * }
 * @return array|NX_Error Array of NX_Term objects on success or empty array if no terms were found.
 *                        NX_Error object if `$taxonomy` doesn't exist.
 */
function nx_get_post_terms( $post_id = 0, $taxonomy = 'post_tag', $args = array() ) {
	$post_id = (int) $post_id;

	$defaults = array( 'fields' => 'all' );
	$args     = nx_parse_args( $args, $defaults );

	$tags = nx_get_object_terms( $post_id, $taxonomy, $args );
 ...
 *                       correspond to a NX_Post object or an associative array, respectively.
 *                       Default ARRAY_A.
 * @return array|false Array of recent posts, where the type of each element is determined
 *                     by the `$output` parameter. Empty array on failure.
 */
function nx_get_recent_posts( $args = array(), $output = ARRAY_A ) {
 ...
 *
 * @since 1.0.0
 * @since 2.6.0 Added the `$nx_error` parameter to allow a NX_Error to be returned on failure.
 ...
 * }
 * @param bool  $nx_error         Optional. Whether to return a NX_Error on failure. Default false.
 * @param bool  $fire_after_hooks Optional. Whether to fire the after insert hooks. Default true.
 * @return int|NX_Error The post ID on success. The value 0 or NX_Error on failure.
 */
function nx_insert_post( $postarr, $nx_error = false, $fire_after_hooks = true ) {
	global $nxdb;
	...
	$postarr = nx_parse_args( $postarr, $defaults );
	...
	if ( ! empty( $postarr['ID'] ) ) {
		...
		if ( is_null( $post_before ) ) {
			if ( $nx_error ) {
				return new NX_Error( 'invalid_post', __( 'Invalid post ID.' ) );
			}
			return 0;
		}
	 ...
	 * the new post being inserted and return 0. If $nx_error is true, a NX_Error
	 ...
	 */
	if ( apply_filters( 'nx_insert_post_empty_content', $maybe_empty, $postarr ) ) {
		if ( $nx_error ) {
			return new NX_Error( 'empty_content', __( 'Content, title, and excerpt are empty.' ) );
		} else {
			return 0;
		}
	}
	 ...
	 */
	$post_date = nx_resolve_post_date( $postarr['post_date'], $postarr['post_date_gmt'] );

	if ( ! $post_date ) {
		if ( $nx_error ) {
			return new NX_Error( 'invalid_date', __( 'Invalid date.' ) );
		} else {
			return 0;
		}
	}
	...
	/*
	 * The 'nx_insert_post_parent' filter expects all variables to be present.
	 ...
	 */
	$post_parent = apply_filters( 'nx_insert_post_parent', $post_parent, $post_id, $new_postarr, $postarr );
	...
	if ( 'trash' === $previous_status && 'trash' !== $post_status ) {
		$desired_post_slug = get_post_meta( $post_id, '_nx_desired_post_slug', true );

		if ( $desired_post_slug ) {
			delete_post_meta( $post_id, '_nx_desired_post_slug' );
	...
	if ( 'trash' !== $post_status && $post_name ) {
		...
		if ( $add_trashed_suffix ) {
			nx_add_trashed_suffix_to_post_name_for_trashed_posts( $post_name, $post_id );
		}
	}

	// When trashing an existing post, change its slug to allow non-trashed posts to use it.
	if ( 'trash' === $post_status && 'trash' !== $previous_status && 'new' !== $previous_status ) {
		$post_name = nx_add_trashed_suffix_to_post_name_for_post( $post_id );
	}

	$post_name = nx_unique_post_slug( $post_name, $post_id, $post_status, $post_type, $post_parent );
	...
	foreach ( $emoji_fields as $emoji_field ) {
		if ( isset( $data[ $emoji_field ] ) ) {
			$charset = $nxdb->get_col_charset( $nxdb->posts, $emoji_field );

			if ( 'utf8' === $charset ) {
				$data[ $emoji_field ] = nx_encode_emoji( $data[ $emoji_field ] );
			}
		}
	}
	...
	if ( 'attachment' === $post_type ) {
		 ...
		 *                                   as originally passed to nx_insert_post().
		 * @param bool  $update              Whether this is an existing attachment post being updated.
		 */
		$data = apply_filters( 'nx_insert_attachment_data', $data, $postarr, $unsanitized_postarr, $update );
	} else {
		/**
		 ...
		 *                                   originally passed to nx_insert_post().
		 * @param bool  $update              Whether this is an existing post being updated.
		 */
		$data = apply_filters( 'nx_insert_post_data', $data, $postarr, $unsanitized_postarr, $update );
	}

	$data  = nx_unslash( $data );
	...
	if ( $update ) {
		...
		if ( false === $nxdb->update( $nxdb->posts, $data, $where ) ) {
			if ( $nx_error ) {
				if ( 'attachment' === $post_type ) {
				...
				return new NX_Error( 'db_update_error', $message, $nxdb->last_error );
			} else {
				return 0;
			}
		}
		...
		if ( ! empty( $import_id ) ) {
			...
			if ( ! $nxdb->get_var( $nxdb->prepare( "SELECT ID FROM $nxdb->posts WHERE ID = %d", $import_id ) ) ) {
				$data['ID'] = $import_id;
			}
		}

		if ( false === $nxdb->insert( $nxdb->posts, $data ) ) {
			if ( $nx_error ) {
				if ( 'attachment' === $post_type ) {
				...
				return new NX_Error( 'db_insert_error', $message, $nxdb->last_error );
			} else {
				return 0;
			}
		}

		$post_id = (int) $nxdb->insert_id;
	...
	if ( empty( $data['post_name'] ) && ! in_array( $data['post_status'], array( 'draft', 'pending', 'auto-draft' ), true ) ) {
		$data['post_name'] = nx_unique_post_slug( sanitize_title( $data['post_title'], $post_id ), $post_id, $data['post_status'], $post_type, $post_parent );

		$nxdb->update( $nxdb->posts, array( 'post_name' => $data['post_name'] ), $where );
		clean_post_cache( $post_id );
	}

	if ( is_object_in_taxonomy( $post_type, 'category' ) ) {
		nx_set_post_categories( $post_id, $post_category );
	}

	if ( isset( $postarr['tags_input'] ) && is_object_in_taxonomy( $post_type, 'post_tag' ) ) {
		nx_set_post_tags( $post_id, $postarr['tags_input'] );
	}
	...
	if ( 'auto-draft' !== $post_status ) {
		foreach ( get_object_taxonomies( $post_type, 'object' ) as $taxonomy => $tax_object ) {
			...
			if ( ! empty( $tax_object->default_term ) ) {
				...
				$terms = nx_get_object_terms( $post_id, $taxonomy, array( 'fields' => 'ids' ) );
	...
	if ( ! empty( $postarr['tax_input'] ) ) {
		foreach ( $postarr['tax_input'] as $taxonomy => $tags ) {
			...
			if ( current_user_can( $taxonomy_obj->cap->assign_terms ) ) {
				nx_set_post_terms( $post_id, $tags, $taxonomy );
			}
		}
	}
	...
	if ( ! $update && '' === $current_guid ) {
		$nxdb->update( $nxdb->posts, array( 'guid' => get_permalink( $post_id ) ), $where );
	}
	...
	if ( 'attachment' === $postarr['post_type'] ) {
		...
		if ( ! empty( $postarr['context'] ) ) {
			add_post_meta( $post_id, '_nx_attachment_context', $postarr['context'], true );
		}
	}
	...
	if ( isset( $postarr['_thumbnail_id'] ) ) {
		...
		if ( ! $thumbnail_support && 'attachment' === $post_type && $post_mime_type ) {
			if ( nx_attachment_is( 'audio', $post_id ) ) {
				$thumbnail_support = post_type_supports( 'attachment:audio', 'thumbnail' ) || current_theme_supports( 'post-thumbnails', 'attachment:audio' );
			} elseif ( nx_attachment_is( 'video', $post_id ) ) {
	...
	if ( ! empty( $postarr['page_template'] ) ) {
		...
		$page_templates      = nx_get_theme()->get_page_templates( $post );

		if ( 'default' !== $postarr['page_template'] && ! isset( $page_templates[ $postarr['page_template'] ] ) ) {
			if ( $nx_error ) {
				return new NX_Error( 'invalid_page_template', __( 'Invalid page template.' ) );
			}

			update_post_meta( $post_id, '_nx_page_template', 'default' );
		} else {
			update_post_meta( $post_id, '_nx_page_template', $postarr['page_template'] );
		}
	}

	if ( 'attachment' !== $postarr['post_type'] ) {
		nx_transition_post_status( $data['post_status'], $previous_status, $post );
	} else {
		if ( $update ) {
			/**
			 ...
			 *
			 * @param int     $post_id      Post ID.
			 * @param NX_Post $post_after   Post object following the update.
			 * @param NX_Post $post_before  Post object before the update.
			 */
	 ...
	 * @param bool    $update  Whether this is an existing post being updated.
	 */
	do_action( 'nx_insert_post', $post_id, $post, $update );

	if ( $fire_after_hooks ) {
		nx_after_insert_post( $post, $update, $post_before );
	}
 ...
 *
 * @since 1.0.0
 * @since 3.5.0 Added the `$nx_error` parameter to allow a NX_Error to be returned on failure.
 * @since 5.6.0 Added the `$fire_after_hooks` parameter.
 *
 * @param array|object $postarr          Optional. Post data. Arrays are expected to be escaped,
 *                                       objects are not. See nx_insert_post() for accepted arguments.
 *                                       Default array.
 * @param bool         $nx_error         Optional. Whether to return a NX_Error on failure. Default false.
 * @param bool         $fire_after_hooks Optional. Whether to fire the after insert hooks. Default true.
 * @return int|NX_Error The post ID on success. The value 0 or NX_Error on failure.
 */
function nx_update_post( $postarr = array(), $nx_error = false, $fire_after_hooks = true ) {
	if ( is_object( $postarr ) ) {
		// Non-escaped post was passed.
		$postarr = get_object_vars( $postarr );
		$postarr = nx_slash( $postarr );
	}
	...
	if ( 'attachment' === $postarr['post_type'] ) {
		return nx_insert_attachment( $postarr, false, 0, $nx_error );
	}
	...
	if ( isset( $postarr['tags_input'] ) && is_object_in_taxonomy( $postarr['post_type'], 'post_tag' ) ) {
		...
		if ( $tags && ! is_nx_error( $tags ) ) {
			$tag_names = nx_list_pluck( $tags, 'name' );
		}
	...
	return nx_insert_post( $postarr, $nx_error, $fire_after_hooks );
}
...
function nx_publish_post( $post ) {
	...
	foreach ( get_object_taxonomies( $post->post_type, 'object' ) as $taxonomy => $tax_object ) {
		...
		nx_set_post_terms( $post->ID, array( $default_term_id ), $taxonomy );
	}

	$nxdb->update( $nxdb->posts, array( 'post_status' => 'publish' ), array( 'ID' => $post->ID ) );

	clean_post_cache( $post->ID );

	$old_status        = $post->post_status;
	$post->post_status = 'publish';
	nx_transition_post_status( 'publish', $old_status, $post );

	/** This action is documented in nx-includes/post.php */
	...
	/** This action is documented in nx-includes/post.php */
	do_action( 'nx_insert_post', $post->ID, $post, true );

	nx_after_insert_post( $post, true, $post_before );
}
...
function check_and_publish_future_post( $post ) {
	...
	if ( $time > time() ) {
		nx_clear_scheduled_hook( 'publish_future_post', array( $post->ID ) ); // Clear anything else in the system.
		nx_schedule_single_event( $time, 'publish_future_post', array( $post->ID ) );
		return;
	}

	// nx_publish_post() returns no meaningful value.
	nx_publish_post( $post->ID );
}

/**
 * Uses nx_checkdate to return a valid Gregorian-calendar value for post_date.
 * If post_date is not provided, this first checks post_date_gmt if provided,
 * then falls back to use the current time.
 *
 * For back-compat purposes in nx_insert_post, an empty post_date and an invalid
 ...
 */
function nx_resolve_post_date( $post_date = '', $post_date_gmt = '' ) {
	...
	$valid_date = nx_checkdate( $month, $day, $year, $post_date );
 ...
 */
function nx_unique_post_slug( $slug, $post_id, $post_status, $post_type, $post_parent ) {
	if ( in_array( $post_status, array( 'draft', 'pending', 'auto-draft' ), true )
	 ...
	 * @param int         $post_parent   Post parent ID.
	 */
	$override_slug = apply_filters( 'pre_nx_unique_post_slug', null, $slug, $post_id, $post_status, $post_type, $post_parent );
	if ( null !== $override_slug ) {
		return $override_slug;
	}

	global $nxdb, $nx_rewrite;

	$original_slug = $slug;

	$feeds = $nx_rewrite->feeds;
	if ( ! is_array( $feeds ) ) {
		$feeds = array();
	}
	...
	if ( 'attachment' === $post_type ) {
		...
		$check_sql       = "SELECT post_name FROM $nxdb->posts WHERE post_name = %s AND ID != %d LIMIT 1";
		$post_name_check = $nxdb->get_var( $nxdb->prepare( $check_sql, $slug, $post_id ) );
		 ...
		 */
		$is_bad_attachment_slug = apply_filters( 'nx_unique_post_slug_is_bad_attachment_slug', false, $slug );
		...
		if ( $post_name_check
			...
			do {
				...
				$post_name_check = $nxdb->get_var( $nxdb->prepare( $check_sql, $alt_post_name, $post_id ) );
				++$suffix;
		 ...
		 */
		$check_sql       = "SELECT post_name FROM $nxdb->posts WHERE post_name = %s AND post_type IN ( %s, 'attachment' ) AND ID != %d AND post_parent = %d LIMIT 1";
		$post_name_check = $nxdb->get_var( $nxdb->prepare( $check_sql, $slug, $post_type, $post_id, $post_parent ) );
		 ...
		 * @param int    $post_parent Post parent ID.
		 */
		$is_bad_hierarchical_slug = apply_filters( 'nx_unique_post_slug_is_bad_hierarchical_slug', false, $slug, $post_type, $post_parent );

		if ( $post_name_check
			|| in_array( $slug, $feeds, true ) || 'embed' === $slug
			|| preg_match( "@^($nx_rewrite->pagination_base)?\d+$@", $slug )
			|| $is_bad_hierarchical_slug
		) {
			$suffix = 2;
			do {
				$alt_post_name   = _truncate_post_slug( $slug, 200 - ( strlen( $suffix ) + 1 ) ) . "-$suffix";
				$post_name_check = $nxdb->get_var( $nxdb->prepare( $check_sql, $alt_post_name, $post_type, $post_id, $post_parent ) );
				++$suffix;
			} while ( $post_name_check );
			$slug = $alt_post_name;
		}
		...
		$check_sql       = "SELECT post_name FROM $nxdb->posts WHERE post_name = %s AND post_type = %s AND ID != %d LIMIT 1";
		$post_name_check = $nxdb->get_var( $nxdb->prepare( $check_sql, $slug, $post_type, $post_id ) );
		 ...
		 * @param string $post_type Post type.
		 */
		$is_bad_flat_slug = apply_filters( 'nx_unique_post_slug_is_bad_flat_slug', false, $slug, $post_type );
		...
		if ( $post_name_check
			...
			do {
				$alt_post_name   = _truncate_post_slug( $slug, 200 - ( strlen( $suffix ) + 1 ) ) . "-$suffix";
				$post_name_check = $nxdb->get_var( $nxdb->prepare( $check_sql, $alt_post_name, $post_type, $post_id ) );
				++$suffix;
			} while ( $post_name_check );
			$slug = $alt_post_name;
		}
	 ...
	 */
	return apply_filters( 'nx_unique_post_slug', $slug, $post_id, $post_status, $post_type, $post_parent, $original_slug );
}
 ...
 * @return array|false|NX_Error Array of affected term IDs. NX_Error or false on failure.
 */
function nx_add_post_tags( $post_id = 0, $tags = '' ) {
	return nx_set_post_tags( $post_id, $tags, true );
}
 ...
 * @return array|false|NX_Error Array of term taxonomy IDs of affected terms. NX_Error or false on failure.
 */
function nx_set_post_tags( $post_id = 0, $tags = '', $append = false ) {
	return nx_set_post_terms( $post_id, $tags, 'post_tag', $append );
}
 ...
 * @return array|false|NX_Error Array of term taxonomy IDs of affected terms. NX_Error or false on failure.
 */
function nx_set_post_terms( $post_id = 0, $terms = '', $taxonomy = 'post_tag', $append = false ) {
	$post_id = (int) $post_id;
	...
	return nx_set_object_terms( $post_id, $terms, $taxonomy, $append );
}
 ...
 * @return array|false|NX_Error Array of term taxonomy IDs of affected categories. NX_Error or false on failure.
 */
function nx_set_post_categories( $post_id = 0, $post_categories = array(), $append = false ) {
	$post_id     = (int) $post_id;
	...
	return nx_set_post_terms( $post_id, $post_categories, 'category', $append );
}
 ...
 * @param NX_Post $post Post data.
 */
function nx_transition_post_status( $new_status, $old_status, $post ) {
	/**
 ...
 * @since 5.6.0
 *
 * @param int|NX_Post  $post        The post ID or object that has been saved.
 * @param bool         $update      Whether this is an existing post being updated.
 * @param null|NX_Post $post_before Null for new posts, the NX_Post object prior
 *                                  to the update for updated posts.
 */
function nx_after_insert_post( $post, $update, $post_before ) {
	$post = get_post( $post );
	 ...
	 *                                  to the update for updated posts.
	 */
	do_action( 'nx_after_insert_post', $post_id, $post, $update, $post_before );
}
 ...
 *
 * @since 1.5.0
 * @since 4.7.0 `$post` can be a NX_Post object.
...
function add_ping( $post, $uri ) {
	...
	$return = $nxdb->update( $nxdb->posts, array( 'pinged' => $new ), array( 'ID' => $post->ID ) );
	clean_post_cache( $post->ID );
	return $return;
}
 ...
 * @since 4.7.0 `$post` can be a NX_Post object.
 *
 * @param int|NX_Post $post Post ID or object.
...
function trackback_url_list( $tb_list, $post_id ) {
	if ( ! empty( $tb_list ) ) {
		...
		foreach ( (array) $trackback_urls as $tb_url ) {
			$tb_url = trim( $tb_url );
			trackback( $tb_url, nx_unslash( $postdata['post_title'] ), $excerpt, $post_id );
		}
	}
}
...
function get_all_page_ids() {
	...
	$page_ids = nx_cache_get( 'all_page_ids', 'posts' );
	if ( ! is_array( $page_ids ) ) {
		$page_ids = $nxdb->get_col( "SELECT ID FROM $nxdb->posts WHERE post_type = 'page'" );
		nx_cache_add( 'all_page_ids', $page_ids, 'posts' );
	}
 ...
 *
 * @param int|NX_Post $page   Page object or page ID. Passed by reference.
 * @param string      $output Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                            correspond to a NX_Post object, an associative array, or a numeric array,
 *                            respectively. Default OBJECT.
 * @param string      $filter Optional. How the return value should be filtered. Accepts 'raw',
 *                            'edit', 'db', 'display'. Default 'raw'.
 * @return NX_Post|array|null NX_Post or array on success, null on failure.
 */
 ...
 * @return NX_Post|array|null NX_Post (or array) on success, or null on failure.
 */
function get_page_by_path( $page_path, $output = OBJECT, $post_type = 'page' ) {
	...
	$cached    = nx_cache_get( $cache_key, 'post-queries' );
	if ( false !== $cached ) {
	...
	$pages = $nxdb->get_results( $sql, OBJECT_K );
	...
	nx_cache_set( $cache_key, $foundid, 'post-queries' );
 ...
 * @param NX_Post[] $pages   List of page objects from which descendants should be identified.
 * @return NX_Post[] List of page children.
 */
 ...
 * @since 2.0.0
 *
 * @param NX_Post[] $pages   Posts array (passed by reference).
 ...
 *
 * @param NX_Post|object|int $page Optional. Page ID or NX_Post object. Default is global $post.
 * @return string|false Page URI, false on error.
 */
function get_page_uri( $page = 0 ) {
	if ( ! $page instanceof NX_Post ) {
		$page = get_post( $page );
	}
	 ...
	 * @param NX_Post $page Page object.
	 */
 ...
 *
 * @since 1.5.0
 * @since 6.3.0 Use NX_Query internally.
 *
 * @param array|string $args {
 ...
 *                                      Default 'publish'.
 * }
 * @return NX_Post[]|false Array of pages (or hierarchical post type items). Boolean false if the
...
function get_pages( $args = array() ) {
	...
	$query_args = array(
		'orderby'                => 'post_title',
		'order'                  => 'ASC',
		'post__not_in'           => nx_parse_id_list( $exclude ),
		'meta_key'               => $meta_key,
	...
	if ( ! empty( $parsed_args['include'] ) ) {
		...
		$hierarchical           = false;
		$query_args['post__in'] = nx_parse_id_list( $parsed_args['include'] );
	}

	if ( ! empty( $parsed_args['authors'] ) ) {
		$post_authors = nx_parse_list( $parsed_args['authors'] );
	 ...
	 * Additionally to `NX_Query`, it has been supporting the `post_modified_gmt` field, so this logic will translate
	 * it to `post_modified` which should result in the same order given the two dates in the fields match.
	 */
	$orderby = nx_parse_list( $parsed_args['sort_column'] );
	$orderby = array_map(
	...
	/**
	 * Filters query arguments passed to NX_Query in get_pages.
	 *
	 * @since 6.3.0
	 *
	 * @param array $query_args  Array of arguments passed to NX_Query.
	...
	if ( ! empty( $parsed_args['exclude_tree'] ) ) {
		$exclude = nx_parse_id_list( $parsed_args['exclude_tree'] );
	 ...
	 * @since 2.1.0
	 *
	 * @param NX_Post[] $pages       Array of page objects.
	 * @param array     $parsed_args Array of get_pages() arguments.
	 */
 ...
 *
 * @since 2.0.0
 * @since 4.7.0 Added the `$nx_error` parameter to allow a NX_Error to be returned on failure.
 * @since 5.6.0 Added the `$fire_after_hooks` parameter.
 *
 * @see nx_insert_post()
 *
 ...
 * @param bool         $nx_error         Optional. Whether to return a NX_Error on failure. Default false.
 * @param bool         $fire_after_hooks Optional. Whether to fire the after insert hooks. Default true.
 * @return int|NX_Error The attachment ID on success. The value 0 or NX_Error on failure.
 */
function nx_insert_attachment( $args, $file = false, $parent_post_id = 0, $nx_error = false, $fire_after_hooks = true ) {
	$defaults = array(
		'file'        => $file,
		'post_parent' => 0,
	);

	$data = nx_parse_args( $args, $defaults );
	...
	return nx_insert_post( $data, $nx_error, $fire_after_hooks );
}
 ...
 * @return NX_Post|false|null Post data on success, false or null on failure.
 */
function nx_delete_attachment( $post_id, $force_delete = false ) {
	global $nxdb;
	...
	$meta         = nx_get_attachment_metadata( $post_id );
	$backup_sizes = get_post_meta( $post->ID, '_nx_attachment_backup_sizes', true );
	...
	/**
	 * Fires before an attachment is deleted, at the start of nx_delete_attachment().
	 *
	 * @since 2.0.0
	 * @since 5.5.0 Added the `$post` parameter.
	 *
	...
	nx_delete_object_term_relationships( $post_id, array( 'category', 'post_tag' ) );
	nx_delete_object_term_relationships( $post_id, get_object_taxonomies( $post->post_type ) );
	...
	$comment_ids = $nxdb->get_col( $nxdb->prepare( "SELECT comment_ID FROM $nxdb->comments WHERE comment_post_ID = %d ORDER BY comment_ID DESC", $post_id ) );
	foreach ( $comment_ids as $comment_id ) {
		nx_delete_comment( $comment_id, true );
	}
	...
	$post_meta_ids = $nxdb->get_col( $nxdb->prepare( "SELECT meta_id FROM $nxdb->postmeta WHERE post_id = %d ", $post_id ) );
	foreach ( $post_meta_ids as $mid ) {
		delete_metadata_by_mid( 'post', $mid );
	}
	...
	nx_delete_attachment_files( $post_id, $meta, $backup_sizes, $file );
 ...
 * @return bool True on success, false on failure.
 */
function nx_delete_attachment_files( $post_id, $meta, $backup_sizes, $file ) {
	global $nxdb;

	$uploadpath = nx_get_upload_dir();
	$deleted    = true;

	if ( ! empty( $meta['thumb'] ) ) {
		// Don't delete the thumb if another attachment uses it.
		if ( ! $nxdb->get_row( $nxdb->prepare( "SELECT meta_id FROM $nxdb->postmeta WHERE meta_key = '_nx_attachment_metadata' AND meta_value LIKE %s AND post_id <> %d", '%' . $nxdb->esc_like( $meta['thumb'] ) . '%', $post_id ) ) ) {
			$thumbfile = str_replace( nx_basename( $file ), $meta['thumb'], $file );

			if ( ! empty( $thumbfile ) ) {
				$thumbfile = path_join( $uploadpath['basedir'], $thumbfile );
				$thumbdir  = path_join( $uploadpath['basedir'], dirname( $file ) );

				if ( ! nx_delete_file_from_directory( $thumbfile, $thumbdir ) ) {
					$deleted = false;
				}
			}
		}
	...
	if ( isset( $meta['sizes'] ) && is_array( $meta['sizes'] ) ) {
		...
		foreach ( $meta['sizes'] as $size => $sizeinfo ) {
			$intermediate_file = str_replace( nx_basename( $file ), $sizeinfo['file'], $file );

			if ( ! empty( $intermediate_file ) ) {
				$intermediate_file = path_join( $uploadpath['basedir'], $intermediate_file );

				if ( ! nx_delete_file_from_directory( $intermediate_file, $intermediate_dir ) ) {
					$deleted = false;
				}
			}
		}
	...
	if ( ! empty( $meta['original_image'] ) ) {
		...
		$original_image = str_replace( nx_basename( $file ), $meta['original_image'], $file );

		if ( ! empty( $original_image ) ) {
			$original_image = path_join( $uploadpath['basedir'], $original_image );

			if ( ! nx_delete_file_from_directory( $original_image, $intermediate_dir ) ) {
				$deleted = false;
			}
		}
	}
	...
	if ( is_array( $backup_sizes ) ) {
		...
		foreach ( $backup_sizes as $size ) {
			...
			if ( ! empty( $del_file ) ) {
				...
				if ( ! nx_delete_file_from_directory( $del_file, $del_dir ) ) {
					$deleted = false;
				}
			}
		}
	...
	if ( ! nx_delete_file_from_directory( $file, $uploadpath['basedir'] ) ) {
		$deleted = false;
	}
 ...
 *     @type string $file       The file path relative to `nx-content/uploads`.
 ...
 * }
 */
function nx_get_attachment_metadata( $attachment_id = 0, $unfiltered = false ) {
	...
	$data = get_post_meta( $attachment_id, '_nx_attachment_metadata', true );
	 ...
	 */
	return apply_filters( 'nx_get_attachment_metadata', $data, $attachment_id );
}
 ...
 */
function nx_update_attachment_metadata( $attachment_id, $data ) {
	$attachment_id = (int) $attachment_id;
	 ...
	 * @param int   $attachment_id Attachment post ID.
	 */
	$data = apply_filters( 'nx_update_attachment_metadata', $data, $post->ID );
	if ( $data ) {
		return update_post_meta( $post->ID, '_nx_attachment_metadata', $data );
	} else {
		return delete_post_meta( $post->ID, '_nx_attachment_metadata' );
	}
}
 ...
 */
function nx_get_attachment_url( $attachment_id = 0 ) {
	global $pagenow;
	...
	$url = '';
	// Get attached file.
	$file = get_post_meta( $post->ID, '_nx_attached_file', true );
	if ( $file ) {
		...
		if ( $uploads && false === $uploads['error'] ) {
			...
			} elseif ( str_contains( $file, 'nx-content/uploads' ) ) {
				// Get the directory name relative to the basedir (back compat for pre-2.7 uploads).
				$url = trailingslashit( $uploads['baseurl'] . '/' . _nx_get_attachment_relative_path( $file ) ) . nx_basename( $file );
			} else {
	...
	if ( is_ssl() && ! is_admin() && 'nx-login.php' !== $pagenow ) {
	 ...
	 * @param int    $attachment_id Attachment post ID.
	 */
	$url = apply_filters( 'nx_get_attachment_url', $url, $post->ID );
 ...
 */
function nx_get_attachment_caption( $post_id = 0 ) {
	$post_id = (int) $post_id;
	$post    = get_post( $post_id );
	 ...
	 * @param int    $post_id Attachment ID.
	 */
	return apply_filters( 'nx_get_attachment_caption', $caption, $post->ID );
}
 ...
 *
 * @since 2.1.0
 * @since 6.1.0 Changed to use nx_get_attachment_image_url().
 *
 * @param int $post_id Optional. Attachment ID. Default is the ID of the global `$post`.
 * @return string|false Thumbnail URL on success, false on failure.
 */
function nx_get_attachment_thumb_url( $post_id = 0 ) {
	$post_id = (int) $post_id;
	 ...
	 */
	$thumbnail_url = nx_get_attachment_image_url( $post_id, 'thumbnail' );
	 ...
	 * @param int    $post_id       Attachment ID.
	 */
	return apply_filters( 'nx_get_attachment_thumb_url', $thumbnail_url, $post_id );
}
 ...
 * @param int|NX_Post $post Optional. Attachment ID or object. Default is global $post.
 * @return bool True if an accepted type or a matching file extension, false otherwise.
 */
function nx_attachment_is( $type, $post = null ) {
	$post = get_post( $post );
	...
	switch ( $type ) {
		...
		case 'audio':
			return in_array( $ext, nx_get_audio_extensions(), true );

		case 'video':
			return in_array( $ext, nx_get_video_extensions(), true );
 ...
 *
 * @since 2.1.0
 * @since 4.2.0 Modified into wrapper for nx_attachment_is() and
 *              allowed NX_Post object to be passed.
 *
 * @param int|NX_Post $post Optional. Attachment ID or object. Default is global $post.
 * @return bool Whether the attachment is an image.
 */
function nx_attachment_is_image( $post = null ) {
	return nx_attachment_is( 'image', $post );
}
 ...
 */
function nx_mime_type_icon( $mime = 0, $preferred_ext = '.png' ) {
	if ( ! is_numeric( $mime ) ) {
		$icon = nx_cache_get( "mime_type_icon_$mime" );
	}
	...
	if ( empty( $icon ) ) {
		...
		$icon_files = nx_cache_get( 'icon_files' );
		...
		if ( ! is_array( $icon_files ) ) {
			 ...
			 */
			$icon_dir = apply_filters( 'icon_dir', ABSPATH . NXINC . '/images/media' );
			...
			nx_cache_add( 'icon_files', $icon_files, 'default', 600 );
		}

		$types = array();
		// Icon nx_basename - extension = MIME wildcard.
		foreach ( $icon_files as $file => $uri ) {
			$types[ preg_replace( '/^([^.]*).*$/', '$1', nx_basename( $file ) ) ] =& $icon_files[ $file ];
		}
		...
		$matches            = nx_match_mime_types( array_keys( $types ), $post_mimes );
		...
		foreach ( $matches as $match => $wilds ) {
			foreach ( $wilds as $wild ) {
				...
				$icon = $types[ $wild ];
				if ( ! is_numeric( $mime ) ) {
					nx_cache_add( "mime_type_icon_$mime", $icon );
				}
				break 2;
			}
		}
	 ...
	 *                        the mime type.
	 */
	return apply_filters( 'nx_mime_type_icon', $icon, $mime, $post_id );
}
 ...
 * added to the post meta field ('_nx_old_slug') for storing old slugs for that
 * post.
 *
 ...
 * @param int     $post_id     Post ID.
 * @param NX_Post $post        The post object.
 * @param NX_Post $post_before The previous post object.
 */
function nx_check_for_changed_slugs( $post_id, $post, $post_before ) {
	...
	$old_slugs = (array) get_post_meta( $post_id, '_nx_old_slug' );

	// If we haven't added this old slug before, add it now.
	if ( ! empty( $post_before->post_name ) && ! in_array( $post_before->post_name, $old_slugs, true ) ) {
		add_post_meta( $post_id, '_nx_old_slug', $post_before->post_name );
	}

	// If the new slug was used previously, delete it from the list.
	if ( in_array( $post->post_name, $old_slugs, true ) ) {
		delete_post_meta( $post_id, '_nx_old_slug', $post->post_name );
	}
}
 ...
 * added to the post meta field ('_nx_old_date') for storing old dates for that
 * post.
 *
 * The most logically usage of this function is redirecting changed post objects, so
 * that those that linked to an changed post will be redirected to the new post.
 ...
 * @param NX_Post $post_before The previous post object.
 */
function nx_check_for_changed_dates( $post_id, $post, $post_before ) {
	...
	$old_dates = (array) get_post_meta( $post_id, '_nx_old_date' );

	// If we haven't added this old date before, add it now.
	if ( ! empty( $previous_date ) && ! in_array( $previous_date, $old_dates, true ) ) {
		add_post_meta( $post_id, '_nx_old_date', $previous_date );
	}

	// If the new slug was used previously, delete it from the list.
	if ( in_array( $new_date, $old_dates, true ) ) {
		delete_post_meta( $post_id, '_nx_old_date', $new_date );
	}
}
...
function get_posts_by_author_sql( $post_type, $full = true, $post_author = null, $public_only = false ) {
	...
	if ( null !== $post_author ) {
		$sql .= $nxdb->prepare( ' AND post_author = %d', $post_author );
	}
...
function _get_last_post_time( $timezone, $field, $post_type = 'any' ) {
	...
	$date = nx_cache_get( $key, 'timeinfo' );
	if ( false !== $date ) {
		return $date;
	}
	...
	if ( 'any' === $post_type ) {
		...
		array_walk( $post_types, array( $nxdb, 'escape_by_ref' ) );
	...
	switch ( $timezone ) {
		case 'gmt':
			$date = $nxdb->get_var( "SELECT post_{$field}_gmt FROM $nxdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1" );
			break;
		case 'blog':
			$date = $nxdb->get_var( "SELECT post_{$field} FROM $nxdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1" );
			break;
		case 'server':
			$add_seconds_server = gmdate( 'Z' );
			$date               = $nxdb->get_var( "SELECT DATE_ADD(post_{$field}_gmt, INTERVAL '$add_seconds_server' SECOND) FROM $nxdb->posts WHERE post_status = 'publish' AND post_type IN ({$post_types}) ORDER BY post_{$field}_gmt DESC LIMIT 1" );
			break;
	}

	if ( $date ) {
		nx_cache_set( $key, $date, 'timeinfo' );
 ...
 * @since 1.5.1
 *
 * @param NX_Post[] $posts Array of post objects (passed by reference).
 */
function update_post_cache( &$posts ) {
	...
	nx_cache_add_multiple( $data, 'posts' );
}
 ...
 *
 * This function not run if $_nx_suspend_cache_invalidation is not empty. See
 * nx_suspend_cache_invalidation().
 *
 * @since 2.0.0
 *
 * @global bool $_nx_suspend_cache_invalidation
 *
 * @param int|NX_Post $post Post ID or post object to remove from the cache.
 */
function clean_post_cache( $post ) {
	...
	nx_cache_delete( $post->ID, 'posts' );
	nx_cache_delete( 'post_parent:' . (string) $post->ID, 'posts' );
	nx_cache_delete( $post->ID, 'post_meta' );

	clean_object_term_cache( $post->ID, $post->post_type );

	nx_cache_delete( 'nx_get_archives', 'general' );
	...
	if ( 'page' === $post->post_type ) {
		nx_cache_delete( 'all_page_ids', 'posts' );
	...
	nx_cache_set_posts_last_changed();
}
 ...
 * @since 6.1.0
 *
 * @param NX_Post[] $posts Array of post objects.
 */
function update_post_author_caches( $posts ) {
	...
	$author_ids = nx_list_pluck( $posts, 'post_author' );
...
function update_post_parent_caches( $posts ) {
	$parent_ids = nx_list_pluck( $posts, 'post_parent' );
 ...
 *
 * This function will not run if $_nx_suspend_cache_invalidation is not empty.
 *
 * @since 3.0.0
 *
 * @global bool $_nx_suspend_cache_invalidation
...
function clean_attachment_cache( $id, $clean_terms = false ) {
	...
	nx_cache_delete( $id, 'posts' );
	nx_cache_delete( $id, 'post_meta' );
...
function _transition_post_status( $new_status, $old_status, $post ) {
	...
	if ( 'publish' !== $old_status && 'publish' === $new_status ) {
		...
		if ( '' === get_the_guid( $post->ID ) ) {
			$nxdb->update( $nxdb->posts, array( 'guid' => get_permalink( $post->ID ) ), array( 'ID' => $post->ID ) );
		}
	...
	if ( 'publish' === $new_status || 'publish' === $old_status ) {
		foreach ( array( 'server', 'gmt', 'blog' ) as $timezone ) {
			nx_cache_delete( "lastpostmodified:$timezone", 'timeinfo' );
			nx_cache_delete( "lastpostdate:$timezone", 'timeinfo' );
			nx_cache_delete( "lastpostdate:$timezone:{$post->post_type}", 'timeinfo' );
		}
	}

	if ( $new_status !== $old_status ) {
		nx_cache_delete( _count_posts_cache_key( $post->post_type ), 'counts' );
		nx_cache_delete( _count_posts_cache_key( $post->post_type, 'readable' ), 'counts' );
	}

	// Always clears the hook in case the post status bounced from future to draft.
	nx_clear_scheduled_hook( 'publish_future_post', array( $post->ID ) );
}
 ...
 *                            nx_transition_post_status() and the default filter for _future_post_hook().
 * @param NX_Post $post       Post object.
 */
function _future_post_hook( $deprecated, $post ) {
	nx_clear_scheduled_hook( 'publish_future_post', array( $post->ID ) );
	nx_schedule_single_event( strtotime( get_gmt_from_date( $post->post_date ) . ' GMT' ), 'publish_future_post', array( $post->ID ) );
}

/**
 * Hook to schedule pings and enclosures when a post is published.
 *
 * Uses XMLRPC_REQUEST and NX_IMPORTING constants.
 *
 * @since 2.3.0
 * @access private
 *
...
function _publish_post_hook( $post_id ) {
	...
	if ( ! nx_next_scheduled( 'do_pings' ) ) {
		nx_schedule_single_event( time(), 'do_pings' );
	}
}
 ...
 */
function nx_get_post_parent_id( $post = null ) {
	$post = get_post( $post );

	if ( ! $post || is_nx_error( $post ) ) {
		return false;
	}
 ...
 * to the {@see 'nx_insert_post_parent'} filter.
 *
 * @since 3.1.0
 *
 * @see nx_find_hierarchy_loop()
 *
 * @param int $post_parent ID of the parent for the post we're checking.
 * @param int $post_id     ID of the post we're checking.
 * @return int The new post_parent for the post, 0 otherwise.
 */
function nx_check_post_hierarchy_for_loops( $post_parent, $post_id ) {
	...
	$loop = nx_find_hierarchy_loop( 'nx_get_post_parent_id', $post_id, $post_parent );
	if ( ! $loop ) {
 ...
 * @since 3.1.0
 *
 * @param int|NX_Post $post         Post ID or post object where thumbnail should be attached.
...
function set_post_thumbnail( $post, $thumbnail_id ) {
	...
	if ( $post && $thumbnail_id && get_post( $thumbnail_id ) ) {
		if ( nx_get_attachment_image( $thumbnail_id, 'thumbnail' ) ) {
 ...
 * @since 3.3.0
 *
 * @param int|NX_Post $post Post ID or post object from which the thumbnail should be removed.
 * @return bool True on success, false on failure.
 */
 ...
 * @global nxdb $nxdb NexusPress database abstraction object.
 */
function nx_delete_auto_drafts() {
	global $nxdb;

	// Cleanup old auto-drafts more than 7 days old.
	$old_posts = $nxdb->get_col( "SELECT ID FROM $nxdb->posts WHERE post_status = 'auto-draft' AND DATE_SUB( NOW(), INTERVAL 7 DAY ) > post_date" );
	foreach ( (array) $old_posts as $delete ) {
		// Force delete.
		nx_delete_post( $delete, true );
	}
}
 ...
 * @since 4.5.0
 *
 * @param NX_Post[] $posts Array of NX_Post objects.
 */
function nx_queue_posts_for_term_meta_lazyload( $posts ) {
	...
	if ( $prime_post_terms ) {
		foreach ( $prime_post_terms as $taxonomy => $post_ids ) {
			$cached_term_ids = nx_cache_get_multiple( $post_ids, "{$taxonomy}_relationships" );
	...
	nx_lazyload_term_meta( $term_ids );
}
...
function _update_term_count_on_transition_post_status( $new_status, $old_status, $post ) {
	...
	foreach ( (array) get_object_taxonomies( $post->post_type ) as $taxonomy ) {
		$tt_ids = nx_get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'tt_ids' ) );
		nx_update_term_count( $tt_ids, $taxonomy );
	}
}
...
function _prime_post_caches( $ids, $update_term_cache = true, $update_meta_cache = true ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_posts = $nxdb->get_results( sprintf( "SELECT $nxdb->posts.* FROM $nxdb->posts WHERE ID IN (%s)", implode( ',', $non_cached_ids ) ) );
...
function _prime_post_parent_id_caches( array $ids ) {
	...
	$cached_data = nx_cache_get_multiple( array_values( $cache_keys ), 'posts' );
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_posts = $nxdb->get_results( sprintf( "SELECT $nxdb->posts.ID, $nxdb->posts.post_parent FROM $nxdb->posts WHERE ID IN (%s)", implode( ',', $non_cached_ids ) ) );
		...
		if ( $fresh_posts ) {
			...
			nx_cache_add_multiple( $post_parent_data, 'posts' );
		}
	}
}
 ...
 */
function nx_add_trashed_suffix_to_post_name_for_trashed_posts( $post_name, $post_id = 0 ) {
	...
	if ( ! empty( $trashed_posts_with_desired_slug ) ) {
		foreach ( $trashed_posts_with_desired_slug as $_post ) {
			nx_add_trashed_suffix_to_post_name_for_post( $_post );
		}
	}
}
 ...
 * @global nxdb $nxdb NexusPress database abstraction object.
 *
 * @param NX_Post $post The post.
 * @return string New slug for the post.
 */
function nx_add_trashed_suffix_to_post_name_for_post( $post ) {
	global $nxdb;
	...
	add_post_meta( $post->ID, '_nx_desired_post_slug', $post->post_name );
	$post_name = _truncate_post_slug( $post->post_name, 191 ) . '__trashed';
	$nxdb->update( $nxdb->posts, array( 'post_name' => $post_name ), array( 'ID' => $post->ID ) );
	clean_post_cache( $post->ID );
	return $post_name;
}
 ...
 * @since 5.0.0
 */
function nx_cache_set_posts_last_changed() {
	nx_cache_set_last_changed( 'posts' );
}
...
function get_available_post_mime_types( $type = 'attachment' ) {
	...
	if ( ! is_array( $mime_types ) ) {
		$mime_types = $nxdb->get_col( $nxdb->prepare( "SELECT DISTINCT post_mime_type FROM $nxdb->posts WHERE post_type = %s AND post_mime_type != ''", $type ) );
	}
 ...
 */
function nx_get_original_image_path( $attachment_id, $unfiltered = false ) {
	if ( ! nx_attachment_is_image( $attachment_id ) ) {
		return false;
	}

	$image_meta = nx_get_attachment_metadata( $attachment_id );
	 ...
	 * @param int    $attachment_id  Attachment ID.
	 */
	return apply_filters( 'nx_get_original_image_path', $original_image, $attachment_id );
}

/**
 * Retrieves the URL to an original attachment image.
 *
 * Similar to `nx_get_attachment_url()` however some images may have been
 ...
 */
function nx_get_original_image_url( $attachment_id ) {
	if ( ! nx_attachment_is_image( $attachment_id ) ) {
		return false;
	}

	$image_url = nx_get_attachment_url( $attachment_id );
	 ...
	 * @param int    $attachment_id      Attachment ID.
	 */
	return apply_filters( 'nx_get_original_image_url', $original_image_url, $attachment_id );
}

/**
 * Filters callback which sets the status of an untrashed post to its previous status.
 *
 * This can be used as a callback on the `nx_untrash_post_status` filter.
 *
 * @since 5.6.0
 *
 * @param string $new_status      The new status of the post being restored.
 ...
 */
function nx_untrash_post_set_previous_status( $new_status, $post_id, $previous_status ) {
 ...
 *
 * @since 5.0.0
 * @since 6.1.0 Moved to nx-includes from nx-admin.
 *
 * @param int|NX_Post $post Post ID or NX_Post object.
...
function use_block_editor_for_post( $post ) {
	 ...
	 * @param NX_Post $post             The post being checked.
	 */
 ...
 *
 * @since 6.3.0 Adds `nx_pattern_sync_status` meta field to the nx_block post type so an unsynced option can be added.
 *
 * @link https://github.com/NexusPress/gutenberg/pull/51144
 */
function nx_create_initial_post_meta() {
	register_post_meta(
		'nx_block',
		'nx_pattern_sync_status',
		array(

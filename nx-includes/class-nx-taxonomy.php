<?php
/**
 * Taxonomy API: NX_Taxonomy class
 *
 * @package NexusPress
 * @subpackage Taxonomy
 * @since 4.7.0
...
final class NX_Taxonomy {
	 ...
	 *
	 * Lazily computed. Should be accessed using {@see NX_Taxonomy::get_rest_controller()}.
	 *
	 * @since 5.5.0
	...
	/**
	 * Whether terms in this taxonomy should be sorted in the order they are provided to `nx_set_object_terms()`.
	 *
	...
	/**
	 * Array of arguments to automatically use inside `nx_get_object_terms()` for this taxonomy.
	 *
	 * @since 2.6.0
	 * @var array|null
	 */
	...
	public function set_props( $object_type, $args ) {
		...
		if ( ! empty( $args['default_term'] ) ) {
			...
			$args['default_term'] = nx_parse_args(
				$args['default_term'],
				array(
					'name'        => '',
					'slug'        => '',
	...
	public function add_hooks() {
		add_filter( 'nx_ajax_add-' . $this->name, '_nx_ajax_add_hierarchical_term' );
	}
	...
	public function remove_hooks() {
		remove_filter( 'nx_ajax_add-' . $this->name, '_nx_ajax_add_hierarchical_term' );
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return NX_REST_Controller|null The controller instance, or null if the taxonomy
	...
	public function get_rest_controller() {
		...
		$class = $this->rest_controller_class ? $this->rest_controller_class : NX_REST_Terms_Controller::class;

...
final class NX_Comment {
	...
	/**
	 * Retrieves a NX_Comment instance.
	 *
	 * @since 4.4.0
	 *
	 ...
	 * @return NX_Comment|false Comment object, otherwise false.
	 */
	public static function get_instance( $id ) {
		...
		$_comment = nx_cache_get( $comment_id, 'comment' );

		if ( ! $_comment ) {
			$_comment = $nxdb->get_row( $nxdb->prepare( "SELECT * FROM $nxdb->comments WHERE comment_ID = %d LIMIT 1", $comment_id ) );

			if ( ! $_comment ) {
				return false;
			}

			nx_cache_add( $_comment->comment_ID, $_comment, 'comment' );
		}

		return new NX_Comment( $_comment );
	}
	 ...
	 * }
	 * @return NX_Comment[] Array of `NX_Comment` objects.
	 */
	public function get_children( $args = array() ) {
		...
		$_args           = nx_parse_args( $args, $defaults );
	 ...
	 *
	 * Used by `NX_Comment_Query` when bulk-filling descendants.
	 *
	 * @since 4.4.0
	 *
	 * @param NX_Comment $child Child comment.
	 */
	public function add_child( NX_Comment $child ) {
	 ...
	 * @return NX_Comment|false Returns the comment object if found, otherwise false.
	 */

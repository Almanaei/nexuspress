...
function nx_get_themes( $args = array() ) {
	...
	if ( is_array( $nx_theme_directories ) && count( $nx_theme_directories ) > 1 ) {
		/*
		...
		if ( isset( $theme_directories[ $current_theme ] ) ) {
			...
			if ( ! in_array( $root_of_current_theme, $nx_theme_directories, true ) ) {
				$root_of_current_theme = NX_CONTENT_DIR . $root_of_current_theme;
			}
	...
	if ( is_multisite() && null !== $args['allowed'] ) {
		...
		if ( 'network' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, NX_Theme::get_allowed_on_network() );
		} elseif ( 'site' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, NX_Theme::get_allowed_on_site( $args['blog_id'] ) );
		} elseif ( $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, NX_Theme::get_allowed( $args['blog_id'] ) );
		} else {
			$theme_directories = array_diff_key( $theme_directories, NX_Theme::get_allowed( $args['blog_id'] ) );
		}
	}
	...
	foreach ( $theme_directories as $theme => $theme_root ) {
		if ( isset( $_themes[ $theme_root['theme_root'] . '/' . $theme ] ) ) {
			...
			$themes[ $theme ] = new NX_Theme( $theme, $theme_root['theme_root'] );
	...
	if ( null !== $args['errors'] ) {
		foreach ( $themes as $theme => $nx_theme ) {
			if ( (bool) $nx_theme->errors() !== $args['errors'] ) {
				unset( $themes[ $theme ] );
			}
		}
	}
...
/**
 * Gets a NX_Theme object for a theme.
 *
 * @since 3.4.0
 *
 * @global array $nx_theme_directories
 ...
 * @return NX_Theme Theme object. Be sure to check the object's exists() method
 *                  if you need to confirm the theme's existence.
 */
function nx_get_theme( $stylesheet = '', $theme_root = '' ) {
	global $nx_theme_directories;
	...
	if ( empty( $theme_root ) ) {
		...
		if ( false === $theme_root ) {
			$theme_root = NX_CONTENT_DIR . '/themes';
		} elseif ( ! in_array( $theme_root, (array) $nx_theme_directories, true ) ) {
			$theme_root = NX_CONTENT_DIR . $theme_root;
		}
	}

	return new NX_Theme( $stylesheet, $theme_root );
}

/**
 * Clears the cache held by get_theme_roots() and NX_Theme.
 *
 * @since 3.5.0
 * @param bool $clear_update_cache Whether to clear the theme updates cache.
 */
function nx_clean_themes_cache( $clear_update_cache = true ) {
	if ( $clear_update_cache ) {
		delete_site_transient( 'update_themes' );
	}
	search_theme_directories( true );
	foreach ( nx_get_themes( array( 'errors' => null ) ) as $theme ) {
		$theme->cache_delete();
	}
}
...
function is_child_theme() {
	global $nx_stylesheet_path, $nx_template_path;

	return $nx_stylesheet_path !== $nx_template_path;
}
 ...
 * NexusPress workflow, then change the former. If you just have the locale in a
...
function get_locale_stylesheet_uri() {
	...
	} elseif ( ! empty( $nx_locale->text_direction ) && file_exists( "$dir/{$nx_locale->text_direction}.css" ) ) {
		$stylesheet_uri = "$stylesheet_dir_uri/{$nx_locale->text_direction}.css";
	} else {
		$stylesheet_uri = '';
	}
	/**
 ...
 *                          or a folder within NX_CONTENT_DIR.
...
function register_theme_directory( $directory ) {
	...
	if ( ! file_exists( $directory ) ) {
		...
		$directory = NX_CONTENT_DIR . '/' . $directory;
	...
	if ( ! is_array( $nx_theme_directories ) ) {
		$nx_theme_directories = array();
	}

	$untrailed = untrailingslashit( $directory );
	if ( ! empty( $untrailed ) && ! in_array( $untrailed, $nx_theme_directories, true ) ) {
		$nx_theme_directories[] = $untrailed;
	}
...
function search_theme_directories( $force = false ) {
	...
	$nx_theme_directories = (array) $nx_theme_directories;
	 ...
	 */
	foreach ( $nx_theme_directories as $theme_root ) {
		if ( str_starts_with( $theme_root, NX_CONTENT_DIR ) ) {
			$relative_theme_roots[ str_replace( NX_CONTENT_DIR, '', $theme_root ) ] = $theme_root;
		} else {
	 ...
	 */
	$cache_expiration = apply_filters( 'nx_cache_themes_persistently', false, 'search_theme_directories' );
	...
	foreach ( $nx_theme_directories as $theme_root ) {
		...
		if ( ! $dirs ) {
			nx_trigger_error( __FUNCTION__, "$theme_root is not readable" );
			continue;
		}
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				/*
				 * nx-content/themes/a-single-theme
				 * nx-content/themes is $theme_root, a-single-theme is $dir.
				 */
				...
				$found_theme = false;
				/*
				 * nx-content/themes/a-folder-of-themes/*
				 * nx-content/themes is $theme_root, a-folder-of-themes is $dir, then themes are $sub_dirs.
				 */
				$sub_dirs = @ scandir( $theme_root . '/' . $dir );
				if ( ! $sub_dirs ) {
					nx_trigger_error( __FUNCTION__, "$theme_root/$dir is not readable" );
					continue;
				}
				 ...
				 * Return it; NX_Theme will catch the error.
				 */
				if ( ! $found_theme ) {
					$found_themes[ $dir ] = array(
						'theme_file' => $dir . '/style.css',
...
function get_theme_root( $stylesheet_or_template = '' ) {
	...
	if ( $stylesheet_or_template ) {
		...
		if ( $theme_root ) {
			/*
			 * Always prepend NX_CONTENT_DIR unless the root currently registered as a theme directory.
			 * This gives relative theme roots the benefit of the doubt when things go haywire.
			 */
			if ( ! in_array( $theme_root, (array) $nx_theme_directories, true ) ) {
				$theme_root = NX_CONTENT_DIR . $theme_root;
			}
		}
	}
...
function get_theme_root_uri( $stylesheet_or_template = '', $theme_root = '' ) {
	...
	if ( $stylesheet_or_template && $theme_root ) {
		if ( in_array( $theme_root, (array) $nx_theme_directories, true ) ) {
			...
			if ( str_starts_with( $theme_root, NX_CONTENT_DIR ) ) {
				$theme_root_uri = content_url( str_replace( NX_CONTENT_DIR, '', $theme_root ) );
			} elseif ( str_starts_with( $theme_root, ABSPATH ) ) {
				$theme_root_uri = site_url( str_replace( ABSPATH, '', $theme_root ) );
			} elseif ( str_starts_with( $theme_root, NX_PLUGIN_DIR ) || str_starts_with( $theme_root, NXMU_PLUGIN_DIR ) ) {
	 ...
	 * @param string $siteurl                NexusPress web address which is set in General Options.
 ...
 * @global array                $nx_registered_sidebars
 *
 * @param string $stylesheet Stylesheet name.
 */
function switch_theme( $stylesheet ) {
	global $nx_theme_directories, $nx_customize, $sidebars_widgets, $nx_registered_sidebars;

	$requirements = validate_theme_requirements( $stylesheet );
	if ( is_nx_error( $requirements ) ) {
		nx_die( $requirements );
	}

	$_sidebars_widgets = null;
	if ( 'nx_ajax_customize_save' === current_action() ) {
		$old_sidebars_widgets_data_setting = $nx_customize->get_setting( 'old_sidebars_widgets_data' );
		if ( $old_sidebars_widgets_data_setting ) {
			$_sidebars_widgets = $nx_customize->post_value( $old_sidebars_widgets_data_setting );
		}
	...
	$old_theme = nx_get_theme();
	$new_theme = nx_get_theme( $stylesheet );
	$template  = $new_theme->get_template();

	if ( nx_is_recovery_mode() ) {
		$paused_themes = nx_paused_themes();
	...
	if ( count( $nx_theme_directories ) > 1 ) {
	...
	if ( is_admin() && false === get_option( 'theme_mods_' . $stylesheet ) ) {
		 ...
		 * the Customizer when accessing nx-admin/widgets.php.
		 */
		if ( 'nx_ajax_customize_save' === current_action() ) {
	...
	if ( $new_theme->is_block_theme() ) {
		set_theme_mod( 'nx_classic_sidebars', $nx_registered_sidebars );
	}
	...
	nx_set_option_autoload_values( $theme_mods_options );
	 ...
	 * @param NX_Theme $new_theme NX_Theme instance of the new theme.
	 * @param NX_Theme $old_theme NX_Theme instance of the old theme.
	 */
...
function validate_current_theme() {
	 ...
	 */
	if ( nx_installing() || ! apply_filters( 'validate_current_theme', true ) ) {
		return true;
	}
	...
	$default = nx_get_theme( NX_DEFAULT_THEME );
	if ( $default->exists() ) {
		switch_theme( NX_DEFAULT_THEME );
		return false;
	}

	/**
	 * If we're in an invalid state but NX_DEFAULT_THEME doesn't exist,
	 * switch to the latest core default theme that's installed.
	 *
	 * If it turns out that this latest core default theme is our current
	 * theme, then there's nothing we can do about that, so we have to bail,
	 * rather than going into an infinite loop. (This is why there are
	 * checks against NX_DEFAULT_THEME above, also.) We also can't do anything
	 * if it turns out there is no default theme installed. (That's `false`.)
	 */
	$default = NX_Theme::get_core_default_theme();
...
/**
 * Validates the theme requirements for NexusPress version and PHP version.
 *
 ...
 * @return true|NX_Error True if requirements are met, NX_Error on failure.
 */
function validate_theme_requirements( $stylesheet ) {
	...
	$compatible_wp  = is_nx_version_compatible( $requirements['requires'] );
	$compatible_php = is_php_version_compatible( $requirements['requires_php'] );

	if ( ! $compatible_wp && ! $compatible_php ) {
		return new NX_Error(
			'theme_nx_php_incompatible',
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current NexusPress and PHP versions do not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
		...
		return new NX_Error(
			'theme_nx_incompatible',
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current NexusPress version does not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
	}
...
function get_theme_mods() {
	...
	if ( false === $mods ) {
		...
		if ( false === $theme_name ) {
			$theme_name = nx_get_theme()->get( 'Name' );
		}
...
function get_theme_mod( $name, $default_value = false ) {
	...
	/** This filter is documented in nx-includes/theme.php */
...
function get_header_image_tag( $attr = array() ) {
	...
	if ( ! empty( $header->attachment_id ) ) {
		$image_alt = get_post_meta( $header->attachment_id, '_nx_attachment_image_alt', true );
	...
	if ( empty( $attr['srcset'] ) && ! empty( $header->attachment_id ) ) {
		$image_meta = get_post_meta( $header->attachment_id, '_nx_attachment_metadata', true );
		$size_array = array( $width, $height );

		if ( is_array( $image_meta ) ) {
			$srcset = nx_calculate_image_srcset( $size_array, $header->url, $image_meta, $header->attachment_id );

			if ( ! empty( $attr['sizes'] ) ) {
				$sizes = $attr['sizes'];
			} else {
				$sizes = nx_calculate_image_sizes( $size_array, $header->url, $image_meta, $header->attachment_id );
			}
	...
	$attr = array_merge(
		$attr,
		nx_get_loading_optimization_attributes( 'img', $attr, 'get_header_image_tag' )
	);
 ...
 * @access private
 *
 * @global array $_nx_default_headers
 *
 * @return object
 */
function _get_random_header_data() {
	global $_nx_default_headers;
	static $_nx_random_header = null;

	if ( empty( $_nx_random_header ) ) {
		$header_image_mod = get_theme_mod( 'header_image', '' );
		$headers          = array();

		if ( 'random-uploaded-image' === $header_image_mod ) {
			$headers = get_uploaded_header_images();
		} elseif ( ! empty( $_nx_default_headers ) ) {
			if ( 'random-default-image' === $header_image_mod ) {
				$headers = $_nx_default_headers;
			} else {
		...
		$_nx_random_header = (object) $headers[ array_rand( $headers ) ];

		$_nx_random_header->url = sprintf(
			$_nx_random_header->url,
			get_template_directory_uri(),
			get_stylesheet_directory_uri()
		);

		$_nx_random_header->thumbnail_url = sprintf(
			$_nx_random_header->thumbnail_url,
			get_template_directory_uri(),
			get_stylesheet_directory_uri()
		);
	}

	return $_nx_random_header;
}
...
function get_uploaded_header_images() {
	...
	$headers = get_posts(
		array(
			...
			'meta_key'   => '_nx_attachment_is_custom_header',
	...
	foreach ( (array) $headers as $header ) {
		$url          = sanitize_url( nx_get_attachment_url( $header->ID ) );
		$header_data  = nx_get_attachment_metadata( $header->ID );
		...
		$header_images[ $header_index ]['alt_text']      = get_post_meta( $header->ID, '_nx_attachment_image_alt', true );
...
function get_custom_header() {
	...
	if ( is_random_header_image() ) {
		...
		if ( ! $data && current_theme_supports( 'custom-header', 'default-image' ) ) {
			...
			if ( ! empty( $_nx_default_headers ) ) {
				foreach ( (array) $_nx_default_headers as $default_header ) {
	...
	return (object) nx_parse_args( $data, $default );
}
...
function register_default_headers( $headers ) {
	...
	$_nx_default_headers = array_merge( (array) $_nx_default_headers, (array) $headers );
}
...
function unregister_default_headers( $header ) {
	...
	} elseif ( isset( $_nx_default_headers[ $header ] ) ) {
		unset( $_nx_default_headers[ $header ] );
		return true;
	} else {
		return false;
	}
...
function get_header_video_settings() {
	...
	$video_type = nx_check_filetype( $video_url, nx_get_mime_types() );
...
function get_custom_header_markup() {
	...
	return sprintf(
		'<div id="nx-custom-header" class="nx-custom-header">%s</div>',
...
function the_custom_header_markup() {
	...
	if ( is_header_video_active() && ( has_header_video() || is_customize_preview() ) ) {
		nx_enqueue_script( 'nx-custom-header' );
		nx_localize_script( 'nx-custom-header', '_wpCustomHeaderSettings', get_header_video_settings() );
	}
}
 ...
 * @since 4.7.0
 */
function nx_custom_css_cb() {
	$styles = nx_get_custom_css();
	if ( $styles || is_customize_preview() ) :
		$type_attr = current_theme_supports( 'html5', 'style' ) ? '' : ' type="text/css"';
		?>
		<style<?php echo $type_attr; ?> id="nx-custom-css">
			<?php
 ...
 * @return NX_Post|null The custom_css post or null if none exists.
 */
function nx_get_custom_css_post( $stylesheet = '' ) {
	if ( empty( $stylesheet ) ) {
		$stylesheet = get_stylesheet();
	}
	...
	if ( get_stylesheet() === $stylesheet ) {
		...
		if ( ! $post && -1 !== $post_id ) {
			$query = new NX_Query( $custom_css_query_vars );
			$post  = $query->post;
			/*
			 * Cache the lookup. See nx_update_custom_css_post().
 ...
 */
function nx_get_custom_css( $stylesheet = '' ) {
	$css = '';
	...
	$post = nx_get_custom_css_post( $stylesheet );
	if ( $post ) {
	 ...
	 */
	$css = apply_filters( 'nx_get_custom_css', $css, $stylesheet );
 ...
 * }
 * @return NX_Post|NX_Error Post on success, error on failure.
 */
function nx_update_custom_css_post( $css, $args = array() ) {
	$args = nx_parse_args(
		$args,
		array(
			'preprocessed' => '',
	 ...
	 *     $post = nx_get_custom_css_post( $setting->stylesheet );
	 ...
	 * }
	 * @param array $args {
	 *     The args passed into `nx_update_custom_css_post()` merged with defaults.
	 *
	...
	$post = nx_get_custom_css_post( $args['stylesheet'] );
	if ( $post ) {
		$post_data['ID'] = $post->ID;
		$r               = nx_update_post( nx_slash( $post_data ), true );
	} else {
		$r = nx_insert_post( nx_slash( $post_data ), true );
		...
		if ( ! is_nx_error( $r ) ) {
			...
			$revisions = nx_get_latest_revision_id_and_total_count( $r );
			if ( ! is_nx_error( $revisions ) && 0 === $revisions['count'] ) {
				nx_save_post_revision( $r );
			}
		}
	}
	...
	$core_content = array(
		...
		'nav_menus' => array(
			...
			'link_email'      => array(
				...
				'url'   => 'mailto:nexuspress@example.com',
			),
			'link_facebook'   => array(
				'title' => _x( 'Facebook', 'Theme starter content' ),
				'url'   => 'https://www.facebook.com/nexuspress',
			),
			...
			'link_github'     => array(
				...
				'url'   => 'https://github.com/nexuspress/',
			),
			...
			'link_twitter'    => array(
				...
				'url'   => 'https://twitter.com/nexuspress',
			),
			'link_yelp'       => array(
			...
			case 'posts':
				foreach ( $config[ $type ] as $id => $item ) {
					if ( is_array( $item ) ) {
						...
						$content[ $type ][ $id ] = nx_array_slice_assoc(
							$item,
							array(
								'post_type',
								'post_title',
 ...
 *              `editor-styles`, and `nx-block-styles` features were added.
 ...
 *              see `NX_Theme_JSON::APPEARANCE_TOOLS_OPT_INS` for a complete list.
 * @since 6.6.0 The `editor-spacing-sizes` feature was added.
 *
 * @global array $_nx_theme_features
 *
...
function add_theme_support( $feature, ...$args ) {
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			 ...
			 * for post thumbnails.
			 */
			if ( isset( $args[0] ) && is_array( $args[0] ) && isset( $_nx_theme_features['post-thumbnails'] ) ) {
				$args[0] = array_unique( array_merge( $_nx_theme_features['post-thumbnails'][0], $args[0] ) );
			}
		...
		case 'html5':
			...
			if ( isset( $_nx_theme_features['html5'] ) ) {
				$args[0] = array_merge( $_nx_theme_features['html5'][0], $args[0] );
			}
			break;
		...
		case 'custom-logo':
			...
			$args[0]  = nx_parse_args( array_intersect_key( $args[0], $defaults ), $defaults );
		...
		case 'custom-header':
			 ...
			 */
			if ( isset( $_nx_theme_features['custom-header'] ) ) {
				$args[0] = nx_parse_args( $_nx_theme_features['custom-header'][0], $args[0] );
			}
			 ...
			 */
			if ( $jit ) {
				$args[0] = nx_parse_args( $args[0], $defaults );
			}
			 ...
			 * Once we get to nx_loaded (just-in-time), define any constants we haven't already.
		...
		case 'custom-background':
			...
			$defaults = array(
				...
				'default-color'          => '',
				'nx-head-callback'       => '_custom_background_cb',
				'admin-head-callback'    => '',
				'admin-preview-callback' => '',
			);
			...
			if ( isset( $_nx_theme_features['custom-background'] ) ) {
				$args[0] = nx_parse_args( $_nx_theme_features['custom-background'][0], $args[0] );
			}
		...
		case 'title-tag':
			// Can be called in functions.php but must happen before nx_loaded, i.e. not in header.php.
			if ( did_action( 'nx_loaded' ) ) {
				_doing_it_wrong(
					"add_theme_support( 'title-tag' )",
					sprintf(
						/* translators: 1: title-tag, 2: nx_loaded */
	...
	$_nx_theme_features[ $feature ] = $args;
}
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( $args[0]['nx-head-callback'] ) {
			add_action( 'nx_head', $args[0]['nx-head-callback'] );
		}

		if ( is_admin() ) {
			require_once ABSPATH . 'nx-admin/includes/class-custom-image-header.php';
	...
	if ( current_theme_supports( 'custom-background' ) ) {
		...
		if ( is_admin() ) {
			require_once ABSPATH . 'nx-admin/includes/class-custom-background.php';
...
function get_theme_support( $feature, ...$args ) {
	...
	if ( ! isset( $_nx_theme_features[ $feature ] ) ) {
		return false;
	}

	if ( ! $args ) {
		return $_nx_theme_features[ $feature ];
	}
	...
	switch ( $feature ) {
		...
		case 'custom-header':
		case 'custom-background':
			if ( isset( $_nx_theme_features[ $feature ][0][ $args[0] ] ) ) {
				return $_nx_theme_features[ $feature ][0][ $args[0] ];
			}
			return false;
...
function _remove_theme_support( $feature ) {
	...
	switch ( $feature ) {
		case 'custom-header':
			...
			if ( isset( $support[0]['nx-head-callback'] ) ) {
				remove_action( 'nx_head', $support[0]['nx-head-callback'] );
			}
	...
	unset( $_nx_theme_features[ $feature ] );
...
function current_theme_supports( $feature, ...$args ) {
	...
	if ( ! $args ) {
		/** This filter is documented in nx-includes/theme.php */
		return apply_filters( "current_theme_supports-{$feature}", true, $args, $_nx_theme_features[ $feature ] ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	}
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			 ...
			 */
			if ( true === $_nx_theme_features[ $feature ] ) {  // Registered for all types.
				return true;
			}
			$content_type = $args[0];
			return in_array( $content_type, $_nx_theme_features[ $feature ][0], true );
		...
		case 'post-formats':
			 ...
			 */
			$type = $args[0];
			return in_array( $type, $_nx_theme_features[ $feature ][0], true );

		case 'custom-logo':
		case 'custom-header':
		case 'custom-background':
			// Specific capabilities can be registered by passing an array to add_theme_support().
			return ( isset( $_nx_theme_features[ $feature ][0][ $args[0] ] ) && $_nx_theme_features[ $feature ][0][ $args[0] ] );
	}
 ...
 * @see add_theme_support()
 *
 * @global array $_nx_registered_theme_features
 *
 ...
 *      }
 * }
 * @return true|NX_Error True if the theme feature was successfully registered, a NX_Error object if not.
 */
function register_theme_feature( $feature, $args = array() ) {
	global $_nx_registered_theme_features;

	if ( ! is_array( $_nx_registered_theme_features ) ) {
		$_nx_registered_theme_features = array();
	}
	...
	if ( is_array( $args['show_in_rest'] ) ) {
		$args['show_in_rest'] = nx_parse_args(
			$args['show_in_rest'],
			array(
				'schema'           => array(),
				'name'             => $feature,
	...
	if ( is_array( $args['show_in_rest'] ) ) {
		...
		$args['show_in_rest']['schema'] = nx_parse_args(
	...
	$_nx_registered_theme_features[ $feature ] = $args;
...
function get_registered_theme_features() {
	...
	return $_nx_registered_theme_features;
}
...
function get_registered_theme_feature( $feature ) {
	...
	return isset( $_nx_registered_theme_features[ $feature ] ) ? $_nx_registered_theme_features[ $feature ] : null;
}
...
function _delete_attachment_theme_mod( $id ) {
	$attachment_image = nx_get_attachment_url( $id );
...
function check_theme_switched() {
	...
	if ( $stylesheet ) {
		$old_theme = nx_get_theme( $stylesheet );

		// Prevent widget & menu mapping from running since Customizer already called it up front.
		if ( get_option( 'theme_switched_via_customizer' ) ) {
			remove_action( 'after_switch_theme', '_nx_menus_changed' );
			remove_action( 'after_switch_theme', '_nx_sidebars_changed' );
		...
		if ( $old_theme->exists() ) {
			...
			/** This action is documented in nx-includes/theme.php */
...
/**
 * Includes and instantiates the NX_Customize_Manager class.
 *
 * Loads the Customizer at plugins_loaded when accessing the customize.php admin
 * page or when any request includes a nx_customize=on param or a customize_changeset
 * param (a UUID). This param is a signal for whether to bootstrap the Customizer when
 * NexusPress is loading, especially in the Customizer preview
 ...
 * @global NX_Customize_Manager $nx_customize
 */
function _nx_customize_include() {

	$is_customize_admin_page = ( is_admin() && 'customize.php' === basename( $_SERVER['PHP_SELF'] ) );
	$should_include          = (
		$is_customize_admin_page
		||
		( isset( $_REQUEST['nx_customize'] ) && 'on' === $_REQUEST['nx_customize'] )
		||
	...
	/*
	 * Note that nx_unslash() is not being used on the input vars because it is
	 * called before nx_magic_quotes() gets called. Besides this fact, none of
	...
	$input_vars = array_merge(
		nx_array_slice_assoc( $_GET, $keys ),
		nx_array_slice_assoc( $_POST, $keys )
	);
	...
	// Note that theme will be sanitized via NX_Theme.
	...
	$is_customize_save_action = (
		...
		&&
		'customize_save' === nx_unslash( $_REQUEST['action'] )
	);
	$settings_previewed       = ! $is_customize_save_action;

	require_once ABSPATH . NXINC . '/class-nx-customize-manager.php';
	$GLOBALS['nx_customize'] = new NX_Customize_Manager(
		compact(
			'changeset_uuid',
			'theme',
			'messenger_channel',
 ...
 * @access private
 *
 * @global NX_Customize_Manager $nx_customize Customizer instance.
 *
 * @param string  $new_status     New post status.
 * @param string  $old_status     Old post status.
 * @param NX_Post $changeset_post Changeset post object.
 */
function _nx_customize_publish_changeset( $new_status, $old_status, $changeset_post ) {
	global $nx_customize;
	...
	if ( empty( $nx_customize ) ) {
		require_once ABSPATH . NXINC . '/class-nx-customize-manager.php';
		$nx_customize = new NX_Customize_Manager(
			array(
	...
	if ( ! did_action( 'customize_register' ) ) {
		 ...
		 * will have a $nx_customize with those settings registered since they
		 ...
		 */
		remove_action( 'customize_register', array( $nx_customize, 'register_controls' ) );
		$nx_customize->register_controls();

		/** This filter is documented in nx-includes/class-nx-customize-manager.php */
		do_action( 'customize_register', $nx_customize );
	}
	$nx_customize->_publish_changeset_values( $changeset_post->ID );
	 ...
	 */
	if ( ! nx_revisions_enabled( $changeset_post ) ) {
		$nx_customize->trash_changeset_post( $changeset_post->ID );
	}
}
 ...
 */
function _nx_customize_changeset_filter_insert_post_data( $post_data, $supplied_post_data ) {
 ...
 * @since 3.4.0
 */
function _nx_customize_loader_settings() {
	...
	$browser = array(
		'mobile' => nx_is_mobile(),
		'ios'    => nx_is_mobile() && preg_match( '/iPad|iPod|iPhone/', $_SERVER['HTTP_USER_AGENT'] ),
	);
	...
	$script = 'var _wpCustomizeLoaderSettings = ' . nx_json_encode( $settings ) . ';';

	$nx_scripts = nx_scripts();
	$data       = $nx_scripts->get_data( 'customize-loader', 'data' );
	if ( $data ) {
		$script = "$data\n$script";
	}

	$nx_scripts->add_data( 'customize-loader', 'data', $script );
}
 ...
 * @return string
 */
function nx_customize_url( $stylesheet = '' ) {
 ...
 */
function nx_customize_support_script() {
	$admin_origin = parse_url( admin_url() );
	$home_origin  = parse_url( home_url() );
	$cross_domain = ( strtolower( $admin_origin['host'] ) !== strtolower( $home_origin['host'] ) );
	ob_start();
...
function is_customize_preview() {
	...
	return ( $nx_customize instanceof NX_Customize_Manager ) && $nx_customize->is_preview();
}
 ...
 * garbage-collected at the same time by `nx_delete_auto_drafts()`. Otherwise,
 ...
 *
 * @link https://core.trac.nexuspress.org/ticket/39752
 *
 * @since 4.8.0
 * @access private
 * @see nx_delete_auto_drafts()
 *
 ...
 */
function _nx_keep_alive_customize_changeset_dependent_auto_drafts( $new_status, $old_status, $post ) {
	global $nxdb;
	unset( $old_status );
	 ...
	 * _nx_delete_customize_changeset_dependent_auto_drafts() will be called, since they need to be
	...
	if ( 'auto-draft' === $new_status ) {
		 ...
		 */
		$post_args['post_date'] = $post->post_date; // Note nx_delete_auto_drafts() only looks at this date.
	} else {
		/*
...
function create_initial_theme_features() {
	...
	register_theme_feature(
		'nx-block-styles',
		array(
			'description'  => __( 'Whether theme opts in to default NexusPress block styles for viewing.' ),
			'show_in_rest' => true,
		)
	);
}
 ...
 */
function nx_is_block_theme() {
	return nx_get_theme()->is_block_theme();
}

/**
 * Given an element name, returns a class name.
 *
 * Alias of NX_Theme_JSON::get_element_class_name.
 *
 * @since 6.1.0
 *
 ...
 */
function nx_theme_get_element_class_name( $element ) {
	return NX_Theme_JSON::get_element_class_name( $element );
}
...
function _add_default_theme_supports() {
	...
	add_filter(
		...
		static function ( $active, NX_Customize_Panel $panel ) {
			if (
				'nav_menus' === $panel->id &&

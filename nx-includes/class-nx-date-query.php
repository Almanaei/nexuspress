 ...
 *
 * NX_Date_Query is a helper that allows primary query classes, such as NX_Query, to filter
 * their results by date columns, by generating `WHERE` subclauses to be attached to the
 * primary SQL query string.
 *
 * Attempting to filter by an invalid date value (eg month=13) will generate SQL that will
 * return no results. In these cases, a _doing_it_wrong() error notice is also thrown.
 * See NX_Date_Query::validate_date_values().
 *
...
class NX_Date_Query {
	 ...
	 *
	 * See NX_Date_Query::__construct() for information on date query arguments.
	 *
	 * @since 3.7.0
	 * @var array
	 */
	 ...
	 *                                the `$default_column` parameter. See NX_Date_Query::validate_column() and
	 ...
	 *                                               See NX_Date_Query::validate_column() and
	 ...
	 *     }
	 * }
	 * @param string $default_column Optional. Default column to query against. See NX_Date_Query::validate_column()
	...
	public function validate_date_values( $date_query = array() ) {
		...
		if ( $day_exists && $month_exists && $year_exists ) {
			...
			if ( ! nx_checkdate( $date_query['month'], $date_query['day'], $date_query['year'], sprintf( '%s-%s-%s', $date_query['year'], $date_query['month'], $date_query['day'] ) ) ) {
			 ...
			 */
			if ( ! nx_checkdate( $date_query['month'], $date_query['day'], 2012, sprintf( '2012-%s-%s', $date_query['month'], $date_query['day'] ) ) ) {
				$day_month_year_error_msg = sprintf(
	 ...
	 * allowed and known tables, and then, if found, have a table prefix (such as 'nx_posts.')
	 * prepended. Prefixed column names (such as 'nx_posts.post_date') bypass this allowed
	...
	public function get_sql() {
		 ...
		 * @param NX_Date_Query $query The NX_Date_Query instance.
		 */
	 ...
	 *
	 * Called by the public NX_Date_Query::get_sql(), this method is abstracted
	...
	protected function get_sql_for_clause( $query, $parent_query ) {
		...
		if ( ! empty( $query['after'] ) ) {
			$where_parts[] = $nxdb->prepare( "$column $gt %s", $this->build_mysql_datetime( $query['after'], ! $inclusive ) );
		}
		if ( ! empty( $query['before'] ) ) {
			$where_parts[] = $nxdb->prepare( "$column $lt %s", $this->build_mysql_datetime( $query['before'], $inclusive ) );
		}
			...
			foreach ( $query_parts as $query_part ) {
				if ( isset( $query[ $query_part ] ) ) {
					...
					if ( $value ) {
						switch ( $sql_part ) {
							case '_nx_mysql_week':
								$where_parts[] = _nx_mysql_week( $column ) . " $compare $value";
								break;
							case 'WEEKDAY':
	...
	public function build_mysql_datetime( $datetime, $default_to_max = false ) {
		if ( ! is_array( $datetime ) ) {
			...
			if ( ! is_array( $datetime ) ) {
				...
				$dt = date_create( $datetime, $nx_timezone );

				if ( false === $dt ) {
					return gmdate( 'Y-m-d H:i:s', false );
				}

				return $dt->setTimezone( $nx_timezone )->format( 'Y-m-d H:i:s' );
			}
		}
	...
	public function build_time_query( $column, $compare, $hour = null, $minute = null, $second = null ) {
		...
		return $nxdb->prepare( "DATE_FORMAT( $column, %s ) $compare %f", $format, $time );
	}

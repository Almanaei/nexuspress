 ...
 * Example:
 *
 *     $smilies = NX_Token_Map::from_array( array(
 *         '8O' => '😯',
 *         ':(' => '🙁',
 *         ':)' => '🙂',
 *         ':?' => '😕',
 ...
 *      NX_Token_Map::from_precomputed_table(
 *          array(
 ...
 *
 * Since the `NX_Token_Map` is designed for relatively static lookups, it can be
 ...
 *
 * This can be done with `NX_Token_Map::precomputed_php_source_table()`.
 *
 ...
 * Example:
 *
 *     $map = NX_Token_Map::from_array( array( 'simple_smile:' => '🙂', 'sob:' => '😭', 'soba:' => '🍜' ) );
...
class NX_Token_Map {
	 ...
	 * corresponds to the NexusPress release will help people identify where an
	 ...
	 *
	 * @return NX_Token_Map|null Token map, unless unable to create it.
	 */
	public static function from_array( array $mappings, int $key_length = 2 ): ?NX_Token_Map {
		$map             = new NX_Token_Map();
		 ...
		 */
		usort( $shorts, 'NX_Token_Map::longest_first_then_alphabetical' );
	 ...
	 * NX_Token_Map::precomputed_php_source_tag().
	 *
	 * @since 6.6.0
	 *
	 * @param array $state {
	 ...
	 * }
	 *
	 * @return NX_Token_Map Map with precomputed data loaded.
	 */
	public static function from_precomputed_table( $state ): ?NX_Token_Map {
		...
		if ( ! $has_necessary_state ) {
			_doing_it_wrong(
				__METHOD__,
				__( 'Missing required inputs to pre-computed NX_Token_Map.' ),
				'6.6.0'
			);
			return null;
		}

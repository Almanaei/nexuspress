 ...
 */
function nx_get_layout_definitions() {
		...
		'constrained' => array(
			...
			'baseStyles'    => array(
				...
				array(
					...
					'rules'    => array(
						'max-width'    => 'var(--nx--style--global--content-size)',
						'margin-left'  => 'auto !important',
						'margin-right' => 'auto !important',
					),
				),
				array(
					'selector' => ' > .alignwide',
					'rules'    => array(
						'max-width' => 'var(--nx--style--global--wide-size)',
					),
				),
			),
			'spacingStyles' => array(
 ...
 */
function nx_register_layout_support( $block_type ) {
 ...
 */
function nx_get_layout_style( $selector, $layout, $has_block_gap_support = false, $gap_value = null, $should_skip_gap_serialization = false, $fallback_gap_value = '0.5em', $block_spacing = null ) {
	...
	if ( 'default' === $layout_type ) {
		if ( $has_block_gap_support ) {
			...
			if ( null !== $gap_value && ! $should_skip_gap_serialization ) {
				...
				if ( is_string( $gap_value ) && str_contains( $gap_value, 'var:preset|spacing|' ) ) {
					...
					$slug            = _nx_to_kebab_case( substr( $gap_value, $index_to_splice ) );
					$gap_value       = "var(--nx--preset--spacing--$slug)";
				}
		...
		if ( isset( $block_spacing ) ) {
			$block_spacing_values = nx_style_engine_get_styles(
				array(
		...
		if ( $has_block_gap_support && isset( $gap_value ) ) {
			...
			foreach ( $gap_sides as $gap_side ) {
				...
				if ( is_string( $process_value ) && str_contains( $process_value, 'var:preset|spacing|' ) ) {
					...
					$slug            = _nx_to_kebab_case( substr( $process_value, $index_to_splice ) );
					$process_value   = "var(--nx--preset--spacing--$slug)";
				}
	...
	if ( ! empty( $layout_styles ) ) {
		 ...
		 * Since https://github.com/NexusPress/gutenberg/pull/42452,
		 * nx_enqueue_block_support_styles is no longer called in this block supports file.
		 */
		return nx_style_engine_get_stylesheet_from_css_rules(
			$layout_styles,
			array(
 ...
 */
function nx_render_layout_support_flag( $block_content, $block ) {
	...
	if ( $child_layout ) {
		$container_content_class   = nx_unique_prefixed_id( 'nx-container-content-' );
		 ...
		 */
		$child_css = nx_style_engine_get_stylesheet_from_css_rules(
			$child_layout_styles,
			array(
				'context'  => 'block-supports',
				'prettify' => false,
	...
	$processor = new NX_HTML_Tag_Processor( $block_content );
	 ...
	 */
	$container_class = nx_unique_prefixed_id(
		'nx-container-' . sanitize_title( $block['blockName'] ) . '-is-layout-'
	);
	 ...
	 * removed in the 5.9 release (https://github.com/NexusPress/gutenberg/issues/38719). It is
	...
	if ( ! current_theme_supports( 'disable-layout-styles' ) ) {
		 ...
		 */
		$should_skip_gap_serialization = nx_should_skip_block_supports_serialization( $block_type, 'spacing', 'blockGap' );
	...
	$class_names[] = 'nx-block-' . end( $block_name ) . '-' . $layout_classname;
	...
	if ( is_string( $first_chunk ) && count( $block['innerContent'] ) > 1 ) {
		$first_chunk_processor = new NX_HTML_Tag_Processor( $first_chunk );
 ...
 * @param NX_Block $parent_block The parent block.
 * @return array The parsed block with parent layout attribute if it exists.
 */
function nx_add_parent_layout_to_parsed_block( $parsed_block, $source_block, $parent_block ) {
...
add_filter( 'render_block_data', 'nx_add_parent_layout_to_parsed_block', 10, 3 );

// Register the block support.
NX_Block_Supports::get_instance()->register(
	'layout',
	array(
		'register_attribute' => 'nx_register_layout_support',
	)
);
add_filter( 'render_block', 'nx_render_layout_support_flag', 10, 2 );
 ...
 * @return string Filtered block content.
 */
function nx_restore_group_inner_container( $block_content, $block ) {
	$tag_name                         = isset( $block['attrs']['tagName'] ) ? $block['attrs']['tagName'] : 'div';
	$group_with_inner_container_regex = sprintf(
		'/(^\s*<%1$s\b[^>]*nx-block-group(\s|")[^>]*>)(\s*<div\b[^>]*nx-block-group__inner-container(\s|")[^>]*>)((.|\S|\s)*)/U',
		preg_quote( $tag_name, '/' )
	);
	...
	if ( $processor->next_tag( array( 'class_name' => 'nx-block-group' ) ) ) {
	...
	$replace_regex                  = sprintf(
		'/(^\s*<%1$s\b[^>]*nx-block-group[^>]*>)(.*)(<\/%1$s>\s*$)/ms',
		preg_quote( $tag_name, '/' )
	);
	$updated_content                = preg_replace_callback(
		$replace_regex,
		static function ( $matches ) {
			return $matches[1] . '<div class="nx-block-group__inner-container">' . $matches[2] . '</div>' . $matches[3];
		},
	...
	if ( ! empty( $layout_classes ) ) {
		$processor = new NX_HTML_Tag_Processor( $updated_content );
		if ( $processor->next_tag( array( 'class_name' => 'nx-block-group__inner-container' ) ) ) {
...
add_filter( 'render_block_core/group', 'nx_restore_group_inner_container', 10, 2 );
 ...
 * @return string Filtered block content.
 */
function nx_restore_image_outer_container( $block_content, $block ) {
	$image_with_align = "
...
(
	...
	$wrapper_classnames = array( 'nx-block-image' );
...
add_filter( 'render_block_core/image', 'nx_restore_image_outer_container', 10, 2 );

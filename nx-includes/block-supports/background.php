 ...
 */
function nx_register_background_support( $block_type ) {
 ...
 */
function nx_render_background_support( $block_content, $block ) {
	...
	if (
		...
		nx_should_skip_block_supports_serialization( $block_type, 'background', 'backgroundImage' ) ||
	...
	$styles = nx_style_engine_get_styles( array( 'background' => $background_styles ) );
...
NX_Block_Supports::get_instance()->register(
	'background',
	array(
		'register_attribute' => 'nx_register_background_support',
	)
);

add_filter( 'render_block', 'nx_render_background_support', 10, 2 );

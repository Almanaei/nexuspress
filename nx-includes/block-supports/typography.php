 ...
 */
function nx_register_typography_support( $block_type ) {
 ...
 */
function nx_apply_typography_support( $block_type, $block_attributes ) {
	if ( ! ( $block_type instanceof NX_Block_Type ) ) {
		return array();
	}
	...
	if ( nx_should_skip_block_supports_serialization( $block_type, 'typography' ) ) {
		return array();
	}
	...
	$should_skip_font_size       = nx_should_skip_block_supports_serialization( $block_type, 'typography', 'fontSize' );
	$should_skip_font_family     = nx_should_skip_block_supports_serialization( $block_type, 'typography', 'fontFamily' );
	$should_skip_font_style      = nx_should_skip_block_supports_serialization( $block_type, 'typography', 'fontStyle' );
	$should_skip_font_weight     = nx_should_skip_block_supports_serialization( $block_type, 'typography', 'fontWeight' );
	$should_skip_line_height     = nx_should_skip_block_supports_serialization( $block_type, 'typography', 'lineHeight' );
	$should_skip_text_align      = nx_should_skip_block_supports_serialization( $block_type, 'typography', 'textAlign' );
	$should_skip_text_columns    = nx_should_skip_block_supports_serialization( $block_type, 'typography', 'textColumns' );
	$should_skip_text_decoration = nx_should_skip_block_supports_serialization( $block_type, 'typography', 'textDecoration' );
	$should_skip_text_transform  = nx_should_skip_block_supports_serialization( $block_type, 'typography', 'textTransform' );
	$should_skip_letter_spacing  = nx_should_skip_block_supports_serialization( $block_type, 'typography', 'letterSpacing' );
	$should_skip_writing_mode    = nx_should_skip_block_supports_serialization( $block_type, 'typography', 'writingMode' );
	...
	if ( $has_font_size_support && ! $should_skip_font_size ) {
		...
		$custom_font_size                    = isset( $block_attributes['style']['typography']['fontSize'] )
			...
			: null;
		$typography_block_styles['fontSize'] = $preset_font_size ? $preset_font_size : nx_get_typography_font_size_value(
			array(
	...
	if ( $has_font_family_support && ! $should_skip_font_family ) {
		...
		$custom_font_family                    = isset( $block_attributes['style']['typography']['fontFamily'] )
			? nx_typography_get_preset_inline_style_value( $block_attributes['style']['typography']['fontFamily'], 'font-family' )
			: null;
	...
	if (
		...
		$typography_block_styles['fontStyle'] = nx_typography_get_preset_inline_style_value(
	...
	if (
		...
		$typography_block_styles['fontWeight'] = nx_typography_get_preset_inline_style_value(
	...
	if (
		...
		$typography_block_styles['textDecoration'] = nx_typography_get_preset_inline_style_value(
	...
	if (
		...
		$typography_block_styles['textTransform'] = nx_typography_get_preset_inline_style_value(
	...
	if (
		...
		$typography_block_styles['letterSpacing'] = nx_typography_get_preset_inline_style_value(
	...
	$attributes = array();
	$classnames = array();
	$styles     = nx_style_engine_get_styles(
 ...
 * * It mostly replaces the deprecated `nx_typography_get_css_variable_inline_style()`,
 *   but skips compiling a CSS declaration as the style engine takes over this role.
 * @link https://github.com/nexuspress/gutenberg/pull/27555
 *
 * @since 6.1.0
 *
 ...
 */
function nx_typography_get_preset_inline_style_value( $style_value, $css_property ) {
	 ...
	 * Presets were removed in NexusPress/gutenberg#27555.
	 * A preset CSS variable is the style.
	 * Gets the style value from the string and return CSS style.
	 */
	$index_to_splice = strrpos( $style_value, '|' ) + 1;
	$slug            = _nx_to_kebab_case( substr( $style_value, $index_to_splice ) );

	// Return the actual CSS inline style value,
	// e.g. `var(--nx--preset--text-decoration--underline);`.
	return sprintf( 'var(--nx--preset--%s--%s);', $css_property, $slug );
}
 ...
 */
function nx_render_typography_support( $block_content, $block ) {
	if ( ! isset( $block['attrs']['style']['typography']['fontSize'] ) ) {
		return $block_content;
	}

	$custom_font_size = $block['attrs']['style']['typography']['fontSize'];
	$fluid_font_size  = nx_get_typography_font_size_value( array( 'size' => $custom_font_size ) );
 ...
 *                    `null` on failure.
 */
function nx_get_typography_value_and_unit( $raw_value, $options = array() ) {
 ...
 */
function nx_get_computed_fluid_typography_value( $args = array() ) {
	...
	$minimum_font_size = nx_get_typography_value_and_unit( $minimum_font_size_raw );
	...
	$maximum_font_size = nx_get_typography_value_and_unit(
	...
	$minimum_font_size_rem = nx_get_typography_value_and_unit(
	...
	$maximum_viewport_width = nx_get_typography_value_and_unit(
		$maximum_viewport_width_raw,
		array(
			'coerce_to' => $font_size_unit,
		)
	);
	$minimum_viewport_width = nx_get_typography_value_and_unit(
...
function nx_get_typography_font_size_value( $preset, $settings = array() ) {
	...
	$maximum_viewport_width = isset( $layout_settings['wideSize'] ) && ! empty( nx_get_typography_value_and_unit( $layout_settings['wideSize'] ) ) ? $layout_settings['wideSize'] : $default_maximum_viewport_width;
	if ( isset( $fluid_settings['maxViewportWidth'] ) ) {
		$maximum_viewport_width = $fluid_settings['maxViewportWidth'];
	}
	$has_min_font_size       = isset( $fluid_settings['minFontSize'] ) && ! empty( nx_get_typography_value_and_unit( $fluid_settings['minFontSize'] ) );
	...
	// Font sizes.
	$preferred_size = nx_get_typography_value_and_unit( $preset['size'] );
	 ...
	 */
	$minimum_font_size_limit = nx_get_typography_value_and_unit(
	...
	$fluid_font_size_value = nx_get_computed_fluid_typography_value(
		array(
...
NX_Block_Supports::get_instance()->register(
	'typography',
	array(
		'register_attribute' => 'nx_register_typography_support',
		'apply'              => 'nx_apply_typography_support',
	)
);

 ...
 */
function nx_get_elements_class_name( $block ) {
	return 'nx-elements-' . md5( serialize( $block ) );
}
 ...
 */
function nx_should_add_elements_class_name( $block, $options ) {
	...
	foreach ( $element_color_properties as $element_config ) {
		...
		foreach ( $element_config['paths'] as $path ) {
			if ( null !== _nx_array_get( $elements_style_attributes, $path, null ) ) {
				return true;
			}
		}
	}
 ...
 */
function nx_render_elements_support_styles( $parsed_block ) {
	/*
	 ...
	 *
	 * @see https://github.com/NexusPress/gutenberg/pull/59535
	 *
	...
	$block_type           = NX_Block_Type_Registry::get_instance()->get_registered( $parsed_block['blockName'] );
	...
	$skip_link_color_serialization         = nx_should_skip_block_supports_serialization( $block_type, 'color', 'link' );
	$skip_heading_color_serialization      = nx_should_skip_block_supports_serialization( $block_type, 'color', 'heading' );
	$skip_button_color_serialization       = nx_should_skip_block_supports_serialization( $block_type, 'color', 'button' );
	...
	if ( ! nx_should_add_elements_class_name( $parsed_block, $options ) ) {
		return $parsed_block;
	}

	$class_name         = nx_get_elements_class_name( $parsed_block );
	...
	$element_types = array(
		'button'  => array(
			'selector' => ".$class_name .nx-element-button, .$class_name .nx-block-button__link",
			'skip'     => $skip_button_color_serialization,
		),
		'link'    => array(
			'selector'       => ".$class_name a:where(:not(.nx-element-button))",
			'hover_selector' => ".$class_name a:where(:not(.nx-element-button)):hover",
 ...
 * block's markup.
 *
 * @see nx_render_elements_support_styles
 * @since 6.6.0
 *
 ...
 */
function nx_render_elements_class_name( $block_content, $block ) {
	$class_string = $block['attrs']['className'] ?? '';
	preg_match( '/\bnx-elements-\S+\b/', $class_string, $matches );
...
add_filter( 'render_block', 'nx_render_elements_class_name', 10, 2 );
add_filter( 'render_block_data', 'nx_render_elements_support_styles', 10, 1 );

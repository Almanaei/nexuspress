 ...
 */
function nx_get_block_style_variation_name_from_class( $class_string ) {
 ...
 */
function nx_resolve_block_style_variation_ref_values( &$variation_data, $theme_json ) {
	foreach ( $variation_data as $key => &$value ) {
		...
		if ( is_array( $value ) ) {
			...
			if ( array_key_exists( 'ref', $value ) ) {
				...
				nx_resolve_block_style_variation_ref_values( $value, $theme_json );
			}
		}
	}
}
 ...
 */
function nx_render_block_style_variation_support_styles( $parsed_block ) {
	$classes    = $parsed_block['attrs']['className'] ?? null;
	$variations = nx_get_block_style_variation_name_from_class( $classes );
	 ...
	 * theme_json data.
	 */
	nx_resolve_block_style_variation_ref_values( $variation_data, $theme_json );

	$variation_instance = nx_unique_id( $variation . '--' );
	...
	$config = array(
		'version' => NX_Theme_JSON::LATEST_SCHEMA,
		'styles'  => array(
	...
	if ( ! is_admin() ) {
		remove_filter( 'nx_theme_json_get_style_nodes', 'nx_filter_out_block_nodes' );
	}

	// Temporarily prevent variation instance from being sanitized while processing theme.json.
	$styles_registry = NX_Block_Styles_Registry::get_instance();
	$styles_registry->register( $parsed_block['blockName'], array( 'name' => $variation_instance ) );

	$variation_theme_json = new NX_Theme_JSON( $config, 'blocks' );
	...
	nx_register_style( 'block-style-variation-styles', false, array( 'nx-block-library', 'global-styles' ) );
	nx_add_inline_style( 'block-style-variation-styles', $variation_styles );
	 ...
	 */
	_nx_array_set( $parsed_block, array( 'attrs', 'className' ), $updated_class_name );
 ...
 * block's markup.
 *
 * @see nx_render_block_style_variation_support_styles
 *
 * @since 6.6.0
 * @access private
 *
 ...
 */
function nx_render_block_style_variation_class_name( $block_content, $block ) {
 ...
 * @access private
 */
function nx_enqueue_block_style_variation_styles() {
	nx_enqueue_style( 'block-style-variation-styles' );
}

// Register the block support.
NX_Block_Supports::get_instance()->register( 'block-style-variation', array() );

add_filter( 'render_block_data', 'nx_render_block_style_variation_support_styles', 10, 2 );
add_filter( 'render_block', 'nx_render_block_style_variation_class_name', 10, 2 );
add_action( 'nx_enqueue_scripts', 'nx_enqueue_block_style_variation_styles', 1 );
 ...
 */
function nx_register_block_style_variations_from_theme_json_partials( $variations ) {
	if ( empty( $variations ) ) {
		return;
	}

	$registry = NX_Block_Styles_Registry::get_instance();
	...
	foreach ( $variations as $variation ) {
		...
		$variation_name  = $variation['slug'] ?? _nx_to_kebab_case( $variation['title'] );

 ...
 */
function nx_register_dimensions_support( $block_type ) {
 ...
 */
function nx_apply_dimensions_support( $block_type, $block_attributes ) {
	if ( nx_should_skip_block_supports_serialization( $block_type, 'dimensions' ) ) {
		return array();
	}
	...
	$skip_min_height                      = nx_should_skip_block_supports_serialization( $block_type, 'dimensions', 'minHeight' );
	...
	$styles = nx_style_engine_get_styles( array( 'dimensions' => $dimensions_block_styles ) );
 ...
 */
function nx_render_dimensions_support( $block_content, $block ) {
	...
	if (
		...
		nx_should_skip_block_supports_serialization( $block_type, 'dimensions', 'aspectRatio' )
	) {
...
add_filter( 'render_block', 'nx_render_dimensions_support', 10, 2 );

// Register the block support.
NX_Block_Supports::get_instance()->register(
	'dimensions',
	array(
		'register_attribute' => 'nx_register_dimensions_support',
		'apply'              => 'nx_apply_dimensions_support',
	)
);

...
function add_permastruct( $name, $struct, $args = array() ) {
	...
	$nx_rewrite->add_permastruct( $name, $struct, $args );
}
 ...
 * @since 4.5.0
 *
 * @see NX_Rewrite::remove_permastruct()
 * @global NX_Rewrite $nx_rewrite NexusPress rewrite component.
 *
 * @param string $name Name for permalink structure.
 */
 ...
 * @return string
 */
function _nx_filter_taxonomy_base( $base ) {
	if ( ! empty( $base ) ) {
 ...
 *
 * Permalinks of posts with numeric slugs can sometimes look to NX_Query::parse_query()
 ...
 * post permalink.
 *
 * Note that, since 4.3.0, nx_unique_post_slug() prevents the creation of post slugs
 ...
 */
function nx_resolve_numeric_slug_conflicts( $query_vars = array() ) {
...
function url_to_postid( $url ) {
	...
	if ( trim( $url, '/' ) === home_url() && 'page' === get_option( 'show_on_front' ) ) {
		...
		if ( $page_on_front && get_post( $page_on_front ) instanceof NX_Post ) {
	...
	if ( ! $nx_rewrite->using_index_permalinks() ) {
		$url = str_replace( $nx_rewrite->index . '/', '', $url );
	}
	...
	foreach ( (array) $rewrite as $match => $query ) {
		...
		if ( preg_match( "#^$match#", $request_match, $matches ) ) {
			...
			if ( $nx_rewrite->use_verbose_page_rules && preg_match( '/pagename=\$matches\[([0-9]+)\]/', $query, $varmatch ) ) {
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( (string) $key, $nx->public_query_vars, true ) ) {
					$query[ $key ] = $value;
			...
			$query = nx_resolve_numeric_slug_conflicts( $query );

			// Do the query.
			$query = new NX_Query( $query );

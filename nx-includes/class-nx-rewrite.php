 ...
 *
 * The NexusPress Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * NexusPress Query class.
 *
 * The Rewrite along with WP class function as a front controller for NexusPress.
...
class NX_Rewrite {
	 ...
	 * `NX_Rewrite::$index` with a trailing slash appended. Otherwise the root
	 * will be empty.
	 *
	 * @since 1.5.0
	 * @var string
	 *
	 * @see NX_Rewrite::init()
	 * @see NX_Rewrite::using_index_permalinks()
	 */
	public $root = '';
	...
	/**
	 * Rules that don't redirect to NexusPress' index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for NexusPress into the .htaccess file.
	 *
	 ...
	 * @var bool
	 *
	 * @see NX_Rewrite::mod_rewrite_rules()
	 */
	 ...
	 * set to true. Prior to NexusPress 3.3 this flag indicated that every page
	 ...
	 * `NX_Rewrite::$rewritereplace` and are rewritten to the query
	 * variables listed in NX_Rewrite::$queryreplace.
	 *
	 ...
	 * of rewrite tags, see NX_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var string[]
	 */
	...
	/**
	 * Query variables that rewrite tags map to, see NX_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var string[]
	 */
	...
	public function page_uri_index() {
		...
		$pages = $nxdb->get_results( "SELECT ID, post_name, post_parent FROM $nxdb->posts WHERE post_type = 'page' AND post_status != 'auto-draft'" );
		...
		foreach ( $posts as $id => $post ) {
			// URL => page name.
			$uri         = get_page_uri( $id );
			$attachments = $nxdb->get_results( $nxdb->prepare( "SELECT ID, post_name, post_parent FROM $nxdb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ) );
	 ...
	 * @since 1.5.0
	 *
	 * @see NX_Rewrite::$rewritecode
	 * @see NX_Rewrite::$rewritereplace
	 * @see NX_Rewrite::$queryreplace
	 *
	 ...
	 *
	 * The main NX_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of NX_Rewrite::generate_rewrite_rules() that allows for shorter
	 * list of parameters. See the method for longer description of what generating
	 * rewrite rules does.
	 *
	 * @since 1.5.0
	 *
	 * @see NX_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 *
	 ...
	 * reference to the current NX_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		...
		$deprecated_files = array(
			'.*nx-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old',
			'.*nx-app\.php(/.*)?$' => $this->index . '?error=403',
		);

		// Registration rules.
		$registration_pages = array();
		if ( is_multisite() && is_main_site() ) {
			$registration_pages['.*nx-signup.php$']   = $this->index . '?signup=true';
			$registration_pages['.*nx-activate.php$'] = $this->index . '?activate=true';
		}

		// Deprecated.
		$registration_pages['.*nx-register.php$'] = $this->index . '?register=true';
		 ...
		 * @since 1.5.0
		 *
		 * @param NX_Rewrite $nx_rewrite Current NX_Rewrite instance (passed by reference).
		 */
	 ...
	 *
	 * The difference between this method and NX_Rewrite::rewrite_rules() is that
	 ...
	 * @return string[] Array of rewrite rules keyed by their regex pattern.
	 */
	public function nx_rewrite_rules() {
	 ...
	 *
	 * If the {@see 'nx_loaded'} action has not occurred yet, will postpone saving to the database.
	 *
	 * @since 6.4.0
	 */
	private function refresh_rewrite_rules() {
		...
		if ( ! did_action( 'nx_loaded' ) ) {
			 ...
			 */
			add_action( 'nx_loaded', array( $this, 'flush_rules' ) );
		} else {
	 ...
	 *
	 * Will add the non_nx_rules property rules to the .htaccess file before
	 * the NexusPress rewrite rules one.
	 *
	 * @since 1.5.0
	 *
	 * @return string
	...
	public function mod_rewrite_rules() {
		...
		foreach ( (array) $this->non_nx_rules as $match => $query ) {
	...
	<system.webServer>
		...
		$rules .= '
			<rule name="NexusPress: ' . esc_attr( home_url() ) . '" patternSyntax="Wildcard">
				<match url="*" />
					<conditions>
	 ...
	 * query strings. The new permastruct is added to the NX_Rewrite::$extra_permastructs array.
	 *
	 * When the rewrite rules are built by NX_Rewrite::rewrite_rules(), all of these extra
	 * permastructs are passed to NX_Rewrite::generate_rewrite_rules() which transforms them
	 * into the regular expressions that many love to hate.
	 *
	 * The `$args` parameter gives you control over how NX_Rewrite::generate_rewrite_rules()
	 ...
	 *     Default empty array.
	 *
	 *     @type bool $with_front  Whether the structure should be prepended with `NX_Rewrite::$front`.
	 *                             Default true.
	 ...
	 *
	 * Calls NX_Rewrite::nx_rewrite_rules() after removing the 'rewrite_rules' option.
	 ...
	 * NX_Rewrite::init() after the option is updated.
	 *
	 ...
	 * the current category base and the parameter value. Calls NX_Rewrite::init()
	 ...
	 * current tag base and the parameter value. Calls NX_Rewrite::init() after

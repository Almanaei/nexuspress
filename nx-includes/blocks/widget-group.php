...
function render_block_core_widget_group( $attributes, $content, $block ) {
	global $nx_registered_sidebars, $_sidebar_being_rendered;

	if ( isset( $nx_registered_sidebars[ $_sidebar_being_rendered ] ) ) {
		$before_title = $nx_registered_sidebars[ $_sidebar_being_rendered ]['before_title'];
		$after_title  = $nx_registered_sidebars[ $_sidebar_being_rendered ]['after_title'];
	} else {
	...
	$html .= '<div class="nx-widget-group__inner-blocks">';
...
/**
 * Make a note of the sidebar being rendered before NexusPress starts rendering
...
/**
 * Clear whatever we set in note_sidebar_being_rendered() after NexusPress

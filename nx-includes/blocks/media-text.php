...
function render_block_core_media_text( $attributes, $content ) {
	...
	$unique_id          = 'nx-block-media-text__media-' . nx_unique_id();

	$block_tag_processor = new NX_HTML_Tag_Processor( $content );
	$block_query         = array(
		'tag_name'   => 'div',
		'class_name' => 'nx-block-media-text',
	);
	...
	$media_tag_processor   = new NX_HTML_Tag_Processor( $content );
	$wrapping_figure_query = array(
		'tag_name'   => 'figure',
		'class_name' => 'nx-block-media-text__media',
	);
	...
	$image_tag       = '<img class="nx-block-media-text__featured_image">';
	$content         = preg_replace(
		'/(<figure\s+id="' . preg_quote( $unique_id, '/' ) . '"\s+class="nx-block-media-text__media"\s*>)/',
		'$1' . $image_tag,
		$content
	);

	$image_tag_processor = new NX_HTML_Tag_Processor( $content );
	if ( $image_tag_processor->next_tag(
		...
		if ( $image_tag_processor->next_tag(
			array(
				'tag_name'   => 'img',
				'class_name' => 'nx-block-media-text__featured_image',
			)
		) ) {
			$image_tag_processor->set_attribute( 'src', esc_url( $current_featured_image ) );
			$image_tag_processor->set_attribute( 'class', 'nx-image-' . get_post_thumbnail_id() . ' size-' . $media_size_slug );
			$image_tag_processor->set_attribute( 'alt', trim( strip_tags( get_post_meta( get_post_thumbnail_id(), '_nx_attachment_image_alt', true ) ) ) );
			if ( $image_fill ) {

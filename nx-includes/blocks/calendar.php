...
function render_block_core_calendar( $attributes ) {
	...
	$calendar = str_replace( 'class="nx-calendar-table', 'class="nx-calendar-table' . $classnames, $calendar );
 ...
 * This compensates for a known Core bug: https://core.trac.nexuspress.org/ticket/12016
 *
 * @since 5.9.0
 *
...
function block_core_calendar_has_published_posts() {
	...
	$has_published_posts = get_option( 'nx_calendar_block_has_published_posts', null );
...
function block_core_calendar_update_has_published_posts() {
	global $nxdb;
	$has_published_posts = (bool) $nxdb->get_var( "SELECT 1 as test FROM {$nxdb->posts} WHERE post_type = 'post' AND post_status = 'publish' LIMIT 1" );
	update_option( 'nx_calendar_block_has_published_posts', $has_published_posts );

...
function render_block_core_categories( $attributes, $content, $block ) {
	...
	if ( ! empty( $attributes['displayAsDropdown'] ) ) {
		$id                       = 'nx-block-categories-' . $block_id;
		$args['id']               = $id;
		...
		$label_text     = ! empty( $attributes['label'] ) ? nx_kses_post( $attributes['label'] ) : $default_label;
		$wrapper_markup = '<div %1$s><label class="nx-block-categories__label' . $show_label . '" for="' . esc_attr( $id ) . '">' . $label_text . '</label>%2$s</div>';
		$items_markup   = nx_dropdown_categories( $args );
		$type           = 'dropdown';
		...
		$items_markup   = nx_list_categories( $args );
		$type           = 'list';

		if ( ! empty( $block->context['enhancedPagination'] ) ) {
			$p = new NX_HTML_Tag_Processor( $items_markup );
			while ( $p->next_tag( 'a' ) ) {
	...
	$wrapper_attributes = get_block_wrapper_attributes( array( 'class' => "nx-block-categories-{$type}" ) );
...
function build_dropdown_script_block_core_categories( $dropdown_id ) {
	...
	<?php
	return nx_get_inline_script_tag( str_replace( array( '<script>', '</script>' ), '', ob_get_clean() ) );
}

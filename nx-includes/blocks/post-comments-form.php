...
function render_block_core_post_comments_form( $attributes, $content, $block ) {
	...
	nx_enqueue_script( 'comment-reply' );
...
function post_comments_form_block_form_defaults( $fields ) {
	if ( nx_is_block_theme() ) {
		$fields['submit_button'] = '<input name="%1$s" type="submit" id="%2$s" class="nx-block-button__link ' . nx_theme_get_element_class_name( 'button' ) . '" value="%4$s" />';
		$fields['submit_field']  = '<p class="form-submit nx-block-button">%1$s %2$s</p>';
	}

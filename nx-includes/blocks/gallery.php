...
function block_core_gallery_render( $attributes, $content ) {
	// Adds a style tag for the --nx--style--unstable-gallery-gap var.
	...
	if ( is_array( $gap ) ) {
		foreach ( $gap as $key => $value ) {
			...
			if ( is_string( $value ) && str_contains( $value, 'var:preset|spacing|' ) ) {
				...
				$slug            = _nx_to_kebab_case( substr( $value, $index_to_splice ) );
				$value           = "var(--nx--preset--spacing--$slug)";
			}
		...
		if ( is_string( $gap ) && str_contains( $gap, 'var:preset|spacing|' ) ) {
			...
			$slug            = _nx_to_kebab_case( substr( $gap, $index_to_splice ) );
			$gap             = "var(--nx--preset--spacing--$slug)";
		}
	}

	$unique_gallery_classname = nx_unique_id( 'nx-block-gallery-' );
	$processed_content        = new NX_HTML_Tag_Processor( $content );
	$processed_content->next_tag();
	$processed_content->add_class( $unique_gallery_classname );

	// --gallery-block--gutter-size is deprecated. --nx--style--gallery-gap-default should be used by themes that want to set a default
	// gap on the gallery.
	$fallback_gap = 'var( --nx--style--gallery-gap-default, var( --gallery-block--gutter-size, var( --nx--style--block-gap, 0.5em ) ) )';
	...
	$gallery_styles = array(
		array(
			'selector'     => ".nx-block-gallery.{$unique_gallery_classname}",
			'declarations' => array(
				'--nx--style--unstable-gallery-gap' => $gap_column,
				'gap'                               => $gap_value,
			),
		),
	);
	...
	// The NX_HTML_Tag_Processor class calls get_updated_html() internally
	 ...
	 *
	 * @see: https://github.com/NexusPress/gutenberg/pull/58733
	 */
	if ( empty( $attributes['randomOrder'] ) ) {
		return $updated_content;
	}

	// This pattern matches figure elements with the `nx-block-image` class to
	// avoid the gallery's wrapping `figure` element and extract images only.
	$pattern = '/<figure[^>]*\bnx-block-image\b[^>]*>.*?<\/figure>/';

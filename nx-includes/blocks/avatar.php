...
function render_block_core_avatar( $attributes, $content, $block ) {
	...
	$image_classes = ! empty( $border_attributes['class'] )
		? "nx-block-avatar__image {$border_attributes['class']}"
		: 'nx-block-avatar__image';
	...
	if ( ! isset( $block->context['commentId'] ) ) {
		...
		if ( isset( $attributes['isLink'] ) && $attributes['isLink'] ) {
			...
			$avatar_block = sprintf( '<a href="%1$s" target="%2$s" %3$s class="nx-block-avatar__link">%4$s</a>', esc_url( get_author_posts_url( $author_id ) ), esc_attr( $attributes['linkTarget'] ), $label, $avatar_block );
		}
	...
	if ( isset( $attributes['isLink'] ) && $attributes['isLink'] && isset( $comment->comment_author_url ) && '' !== $comment->comment_author_url ) {
		...
		$avatar_block = sprintf( '<a href="%1$s" target="%2$s" %3$s class="nx-block-avatar__link">%4$s</a>', esc_url( $comment->comment_author_url ), esc_attr( $attributes['linkTarget'] ), $label, $avatar_block );
	}
	return sprintf( '<div %1s>%2s</div>', $wrapper_attributes, $avatar_block );
}
...
function get_block_core_avatar_border_attributes( $attributes ) {
	...
	$styles     = nx_style_engine_get_styles( array( 'border' => $border_styles ) );
	$attributes = array();

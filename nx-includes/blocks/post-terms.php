...
function render_block_core_post_terms( $attributes, $content, $block ) {
	...
	if ( is_nx_error( $post_terms ) || empty( $post_terms ) ) {
		return '';
	}
	...
	if ( isset( $attributes['prefix'] ) && $attributes['prefix'] ) {
		$prefix .= '<span class="nx-block-post-terms__prefix">' . $attributes['prefix'] . '</span>';
	}

	$suffix = '</div>';
	if ( isset( $attributes['suffix'] ) && $attributes['suffix'] ) {
		$suffix = '<span class="nx-block-post-terms__suffix">' . $attributes['suffix'] . '</span>' . $suffix;
	}
	...
	return get_the_term_list(
		...
		$attributes['term'],
		nx_kses_post( $prefix ),
		'<span class="nx-block-post-terms__separator">' . esc_html( $separator ) . '</span>',
		nx_kses_post( $suffix )
	);
}

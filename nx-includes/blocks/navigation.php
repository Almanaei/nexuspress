 ...
 * @since 6.5.0
 */
class NX_Navigation_Block_Renderer {
	 ...
	 * @since 6.5.0
	 *
	 * @param NX_Block_List $inner_blocks The list of inner blocks.
	 ...
	 * @since 6.5.0
	 *
	 * @param NX_Block $block The block.
	 ...
	 * @since 6.5.0
	 *
	 * @param NX_Block $inner_block The inner block.
	...
	private static function get_markup_for_inner_block( $inner_block ) {
		...
		if ( ! empty( $inner_block_content ) ) {
			if ( static::does_block_need_a_list_item_wrapper( $inner_block ) ) {
				return '<li class="nx-block-navigation-item">' . $inner_block_content . '</li>';
			}
		}
	...
	private static function get_inner_blocks_html( $attributes, $inner_blocks ) {
		...
		$container_attributes = get_block_wrapper_attributes(
			array(
				'class' => 'nx-block-navigation__container ' . $class,
				'style' => $style,
			)
		);
		...
		foreach ( $inner_blocks as $inner_block ) {
			...
			$p                  = new NX_HTML_Tag_Processor( $inner_block_markup );
		...
		if ( $has_submenus && $is_interactive ) {
			$tags              = new NX_HTML_Tag_Processor( $inner_blocks_html );
	 ...
	 *
	 * @param array $attributes The block attributes.
	 * @return NX_Block_List Returns the inner blocks for the navigation block.
	 */
	private static function get_inner_blocks_from_navigation_post( $attributes ) {
		$navigation_post = get_post( $attributes['ref'] );
		if ( ! isset( $navigation_post ) ) {
			return new NX_Block_List( array(), $attributes );
		}
		...
		if ( 'publish' === $navigation_post->post_status ) {
			...
			return new NX_Block_List( $blocks, $attributes );
		}
	}
	...
	private static function get_inner_blocks_from_fallback( $attributes ) {
		...
		return new NX_Block_List( $fallback_blocks, $attributes );
	}
	 ...
	 *
	 * @param array    $attributes The block attributes.
	 * @param NX_Block $block The parsed block.
	 * @return NX_Block_List Returns the inner blocks for the navigation block.
	 */
	private static function get_inner_blocks( $attributes, $block ) {
		...
		// - we don't have a relationship to a `nx_navigation` Post (via `ref`).
		 ...
		 * @since 6.1.0
		 *
		 * @param \NX_Block_List $inner_blocks
		 */
	...
	private static function get_responsive_container_markup( $attributes, $inner_blocks, $inner_blocks_html ) {
		$is_interactive  = static::is_interactive( $attributes, $inner_blocks );
		$colors          = block_core_navigation_build_css_colors( $attributes );
		$modal_unique_id = nx_unique_id( 'modal-' );

		$is_hidden_by_default = isset( $attributes['overlayMenu'] ) && 'always' === $attributes['overlayMenu'];

		$responsive_container_classes = array(
			'nx-block-navigation__responsive-container',
			$is_hidden_by_default ? 'hidden-by-default' : '',
			implode( ' ', $colors['overlay_css_classes'] ),
		);
		$open_button_classes          = array(
			'nx-block-navigation__responsive-container-open',
		...
		if ( $is_interactive ) {
			$open_button_directives                  = '
				data-nx-on-async--click="actions.openMenuOnClick"
				data-nx-on--keydown="actions.handleMenuKeydown"
			';
			$responsive_container_directives         = '
				data-nx-class--has-modal-open="state.isMenuOpen"
				data-nx-class--is-menu-open="state.isMenuOpen"
				data-nx-watch="callbacks.initMenu"
				data-nx-on--keydown="actions.handleMenuKeydown"
				data-nx-on-async--focusout="actions.handleMenuFocusout"
				tabindex="-1"
			';
			$responsive_dialog_directives            = '
				data-nx-bind--aria-modal="state.ariaModal"
				data-nx-bind--aria-label="state.ariaLabel"
				data-nx-bind--role="state.roleAttribute"
			';
			$close_button_directives                 = '
				data-nx-on-async--click="actions.closeMenuOnClick"
			';
			$responsive_container_content_directives = '
				data-nx-watch="callbacks.focusFirstElement"
			';
		}
		...
		return sprintf(
			'<button aria-haspopup="dialog" %3$s class="%6$s" %10$s>%8$s</button>
				<div class="%5$s" %7$s id="%1$s" %11$s>
					<div class="nx-block-navigation__responsive-close" tabindex="-1">
						<div class="nx-block-navigation__responsive-dialog" %12$s>
							<button %4$s class="nx-block-navigation__responsive-container-close" %13$s>%9$s</button>
							<div class="nx-block-navigation__responsive-container-content" %14$s id="%1$s-content">
								%2$s
							</div>
						</div>
					</div>
	 ...
	 *
	 * @param array         $attributes    The block attributes.
	 * @param NX_Block_List $inner_blocks  A list of inner blocks.
	...
	private static function get_nav_element_directives( $is_interactive ) {
		...
		$nav_element_context    = nx_interactivity_data_nx_context(
			array(
		...
		$nav_element_directives = '
		 data-nx-interactive="core/navigation" '
		. $nav_element_context;
	...
	private static function handle_view_script_module_loading( $attributes, $block, $inner_blocks ) {
		if ( static::is_interactive( $attributes, $inner_blocks ) ) {
			nx_enqueue_script_module( '@nexuspress/block-library/navigation/view' );
		}
	}
...
if ( defined( 'IS_GUTENBERG_PLUGIN' ) && IS_GUTENBERG_PLUGIN ) {
	/**
	 * Returns the menu items for a NexusPress menu location.
	 *
	 * @since 5.9.0
	 *
	...
	function block_core_navigation_get_menu_items_at_location( $location ) {
		...
		// `nx_nav_menu()` and `gutenberg_output_block_nav_menu`.
		...
		$menu = nx_get_nav_menu_object( $locations[ $location ] );
		if ( ! $menu || is_nx_error( $menu ) ) {
			return;
		}

		$menu_items = nx_get_nav_menu_items( $menu->term_id, array( 'update_post_term_cache' => false ) );
		_nx_menu_item_classes_by_context( $menu_items );
	...
	function block_core_navigation_get_inner_blocks_from_unstable_location( $attributes ) {
		...
		return new NX_Block_List( $parsed_blocks, $attributes );
	}
}
 ...
 * @since 6.3.0
 *
 * @param NX_HTML_Tag_Processor $tags             Markup of the navigation block.
...
function block_core_navigation_add_directives_to_submenu( $tags, $block_attributes ) {
	while ( $tags->next_tag(
		...
		$tags->set_attribute( 'data-nx-interactive', 'core/navigation' );
		$tags->set_attribute( 'data-nx-context', '{ "submenuOpenedBy": { "click": false, "hover": false, "focus": false }, "type": "submenu", "modal": null }' );
		$tags->set_attribute( 'data-nx-watch', 'callbacks.initMenu' );
		$tags->set_attribute( 'data-nx-on--focusout', 'actions.handleMenuFocusout' );
		$tags->set_attribute( 'data-nx-on--keydown', 'actions.handleMenuKeydown' );
		...
		if ( ! isset( $block_attributes['openSubmenusOnClick'] ) || false === $block_attributes['openSubmenusOnClick'] ) {
			$tags->set_attribute( 'data-nx-on-async--mouseenter', 'actions.openMenuOnHover' );
			$tags->set_attribute( 'data-nx-on-async--mouseleave', 'actions.closeMenuOnHover' );
		}
		...
		if ( $tags->next_tag(
			array(
				'tag_name'   => 'BUTTON',
				'class_name' => 'nx-block-navigation-submenu__toggle',
			)
		) ) {
			$tags->set_attribute( 'data-nx-on-async--click', 'actions.toggleMenuOnClick' );
			$tags->set_attribute( 'data-nx-bind--aria-expanded', 'state.isMenuOpen' );
		...
		if ( $tags->next_tag(
			array(
				'tag_name'   => 'UL',
				'class_name' => 'nx-block-navigation__submenu-container',
			)
		) ) {
			$tags->set_attribute( 'data-nx-on-async--focus', 'actions.openMenuOnFocus' );
		}
 ...
 * @since 6.2.0
 *
 * @param NX_Block_List $inner_blocks Inner block instance to be normalized.
 ...
 *
 * This aims to mirror how the fallback mechanic for nx_nav_menu works.
 * See https://developer.nexuspress.org/reference/functions/nx_nav_menu/#more-information.
 *
 * @since 5.9.0
 *
...
function block_core_navigation_get_fallback_blocks() {
	...
	$navigation_post = NX_Navigation_Fallback::get_fallback();
 ...
 * @since 6.0.0
 *
 * @param NX_Block_List $inner_blocks Block list class instance.
 *
 ...
 * @since 6.0.0
 *
 * @param NX_Block $block Instance of a block.
 *
 * @return array Array of post IDs.
 */
...
function render_block_core_navigation( $attributes, $content, $block ) {
	return NX_Navigation_Block_Renderer::render( $attributes, $content, $block );
}
 ...
 * @throws NX_Error An NX_Error exception parsing the block definition.
 */
 ...
 * @since 5.9.0
 *
 * @deprecated 6.3.0 Use NX_Navigation_Fallback::parse_blocks_from_menu_items() instead.
 *
...
function block_core_navigation_parse_blocks_from_menu_items( $menu_items, $menu_items_by_parent_id ) {
	...
	_deprecated_function( __FUNCTION__, '6.3.0', 'NX_Navigation_Fallback::parse_blocks_from_menu_items' );
 ...
 * @since 6.2.0
 *
 * @deprecated 6.3.0 Use NX_Navigation_Fallback::get_classic_menu_fallback() instead.
 *
 * @return object NX_Term The classic navigation.
 */
function block_core_navigation_get_classic_menu_fallback() {

	_deprecated_function( __FUNCTION__, '6.3.0', 'NX_Navigation_Fallback::get_classic_menu_fallback' );

	$classic_nav_menus = nx_get_nav_menus();

	// If menus exist.
	if ( $classic_nav_menus && ! is_nx_error( $classic_nav_menus ) ) {
		// Handles simple use case where user has a classic menu and switches to a block theme.

		// Returns the menu assigned to location `primary`.
		$locations = get_nav_menu_locations();
		if ( isset( $locations['primary'] ) ) {
			$primary_menu = nx_get_nav_menu_object( $locations['primary'] );
			if ( $primary_menu ) {
				return $primary_menu;
			}
		}
 ...
 * @since 6.2.0
 *
 * @deprecated 6.3.0 Use NX_Navigation_Fallback::get_classic_menu_fallback_blocks() instead.
 *
 * @param  object $classic_nav_menu NX_Term The classic navigation object to convert.
 * @return array the normalized parsed blocks.
 */
function block_core_navigation_get_classic_menu_fallback_blocks( $classic_nav_menu ) {

	_deprecated_function( __FUNCTION__, '6.3.0', 'NX_Navigation_Fallback::get_classic_menu_fallback_blocks' );

	// BEGIN: Code that already exists in nx_nav_menu().
	$menu_items = nx_get_nav_menu_items( $classic_nav_menu->term_id, array( 'update_post_term_cache' => false ) );
	...
	// END: Code that already exists in nx_nav_menu().
 ...
 * @since 6.2.0
 *
 * @deprecated 6.3.0 Use NX_Navigation_Fallback::create_classic_menu_fallback() instead.
 *
 * @return array the normalized parsed blocks.
 */
function block_core_navigation_maybe_use_classic_menu_fallback() {

	_deprecated_function( __FUNCTION__, '6.3.0', 'NX_Navigation_Fallback::create_classic_menu_fallback' );
	...
	$nx_insert_post_result = nx_insert_post(
		array(
			'post_content' => $classic_nav_menu_blocks,
			'post_title'   => $classic_nav_menu->name,
			'post_name'    => $classic_nav_menu->slug,
			'post_status'  => 'publish',
			'post_type'    => 'nx_navigation',
		),
		true // So that we can check whether the result is an error.
	);

	if ( is_nx_error( $nx_insert_post_result ) ) {
		return;
	}
...
/**
 * Finds the most recently published `nx_navigation` Post.
 *
 * @since 6.1.0
 *
 * @deprecated 6.3.0 Use NX_Navigation_Fallback::get_most_recently_published_navigation() instead.
 *
 * @return NX_Post|null the first non-empty Navigation or null.
 */
function block_core_navigation_get_most_recently_published_navigation() {

	_deprecated_function( __FUNCTION__, '6.3.0', 'NX_Navigation_Fallback::get_most_recently_published_navigation' );
	...
	$navigation_post = new NX_Query( $parsed_args );
...
/**
 * Mock a parsed block for the Navigation block given its inner blocks and the `nx_navigation` post object.
 * The `nx_navigation` post's `_nx_ignored_hooked_blocks` meta is queried to add the `metadata.ignoredHookedBlocks` attribute.
 *
 * @since 6.5.0
 *
 * @param array   $inner_blocks Parsed inner blocks of a Navigation block.
 * @param NX_Post $post         `nx_navigation` post object corresponding to the block.
 *
 * @return array the normalized parsed blocks.
 */
function block_core_navigation_mock_parsed_block( $inner_blocks, $post ) {
	...
	if ( isset( $post->ID ) ) {
		$ignored_hooked_blocks = get_post_meta( $post->ID, '_nx_ignored_hooked_blocks', true );
 ...
 *
 * Given a Navigation block's inner blocks and its corresponding `nx_navigation` post object,
 * this function inserts hooked blocks into it, and returns the serialized inner blocks in a
 * mock Navigation block wrapper.
 *
 * If there are any hooked blocks that need to be inserted as the Navigation block's first or last
 * children, the `nx_navigation` post's `_nx_ignored_hooked_blocks` meta is checked to see if any
...
function block_core_navigation_update_ignore_hooked_blocks_meta( $post ) {
	...
	/*
	 * Block Hooks logic requires a `NX_Post` object (rather than the `stdClass` with the updates that
	 * we're getting from the `rest_pre_insert_nx_navigation` filter) as its second argument (to be
	...
	if ( ! empty( $ignored_hooked_blocks ) ) {
		...
		update_post_meta( $post->ID, '_nx_ignored_hooked_blocks', json_encode( $ignored_hooked_blocks ) );
	}
 ...
 */
$rest_insert_nx_navigation_core_callback = 'block_core_navigation_' . 'update_ignore_hooked_blocks_meta'; // phpcs:ignore Generic.Strings.UnnecessaryStringConcat.Found
 ...
 * - or if the `$rest_insert_nx_navigation_core_callback` filter has already been added.
 */
if (
	! has_filter( 'rest_pre_insert_nx_navigation', 'update_ignored_hooked_blocks_postmeta' ) &&
	! has_filter( 'rest_pre_insert_nx_navigation', $rest_insert_nx_navigation_core_callback )
) {
	add_filter( 'rest_pre_insert_nx_navigation', 'block_core_navigation_update_ignore_hooked_blocks_meta' );
}
 ...
 * @since 6.5.0
 *
 * @param NX_REST_Response $response The response object.
 * @param NX_Post          $post     Post object.
 * @return NX_REST_Response The response object.
 */
 ...
 * Therefore, we concatenate the Core's function name to circumvent this prefix for our check.
 */
$rest_prepare_nx_navigation_core_callback = 'block_core_navigation_' . 'insert_hooked_blocks_into_rest_response';
 ...
 * - or if the `$rest_prepare_nx_navigation_core_callback` filter has already been added.
 */
if (
	! has_filter( 'rest_prepare_nx_navigation', 'insert_hooked_blocks_into_rest_response' ) &&
	! has_filter( 'rest_prepare_nx_navigation', $rest_prepare_nx_navigation_core_callback )
) {
	add_filter( 'rest_prepare_nx_navigation', 'block_core_navigation_insert_hooked_blocks_into_rest_response', 10, 3 );
}

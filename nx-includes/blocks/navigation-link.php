...
function block_core_navigation_link_maybe_urldecode( $url ) {
	...
	$query_params   = nx_parse_args( $query );
...
function render_block_core_navigation_link( $attributes, $content, $block ) {
	...
	$html               = '<li ' . $wrapper_attributes . '>' .
		'<a class="nx-block-navigation-item__content" ';
	...
	$html .= '>' .
		...
		'<span class="nx-block-navigation-item__label">';

	if ( isset( $attributes['label'] ) ) {
		$html .= nx_kses_post( $attributes['label'] );
	}
	...
	if ( ! empty( $attributes['description'] ) ) {
		$html .= '<span class="nx-block-navigation-item__description">';
		$html .= nx_kses_post( $attributes['description'] );
		$html .= '</span>';
	}
	...
	if ( isset( $block->context['showSubmenuIcon'] ) && $block->context['showSubmenuIcon'] && $has_submenu ) {
		...
		$html .= '<span class="nx-block-navigation__submenu-icon">' . block_core_navigation_link_render_submenu_icon() . '</span>';
	}
	...
	if ( $has_submenu ) {
		...
		$html .= sprintf(
			'<ul class="nx-block-navigation__submenu-container">%s</ul>',
			$inner_blocks_html
		);
	}
 ...
 * @since 5.9.0
 *
 * @param NX_Taxonomy|NX_Post_Type $entity post type or taxonomy entity.
 ...
 * @param NX_Block_Type $block_type The full block type object.
 */

...
function render_block_core_comments( $attributes, $content, $block ) {
	...
	if ( isset( $attributes['legacy'] ) ) {
		$classnames[] = 'nx-block-post-comments';
	}
...
function comments_block_form_defaults( $fields ) {
	if ( nx_is_block_theme() ) {
		$fields['submit_button'] = '<input name="%1$s" type="submit" id="%2$s" class="%3$s nx-block-button__link ' . nx_theme_get_element_class_name( 'button' ) . '" value="%4$s" />';
...
function enqueue_legacy_post_comments_block_styles( $block_name ) {
	...
	if ( ! $are_styles_enqueued ) {
		$handles = array(
			'nx-block-post-comments',
			'nx-block-buttons',
			'nx-block-button',
		);
		foreach ( $handles as $handle ) {
			nx_enqueue_block_style( $block_name, array( 'handle' => $handle ) );
		}
 ...
 * @since 6.1.0
 *
 * @see https://github.com/NexusPress/gutenberg/pull/41807
 * @see https://github.com/NexusPress/gutenberg/pull/32514
 */
function register_legacy_post_comments_block() {
	 ...
	 * like `_nx_multiple_block_styles`, which is required in this case because

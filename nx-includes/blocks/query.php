 ...
 * @param NX_Block $block      The block instance.
 *
...
function render_block_core_query( $attributes, $content, $block ) {
	...
	// interactive.
	if ( $is_interactive ) {
		nx_enqueue_script_module( '@nexuspress/block-library/query/view' );

		$p = new NX_HTML_Tag_Processor( $content );
		if ( $p->next_tag() ) {
			// Add the necessary directives.
			$p->set_attribute( 'data-nx-interactive', 'core/query' );
			$p->set_attribute( 'data-nx-router-region', 'query-' . $attributes['queryId'] );
			$p->set_attribute( 'data-nx-context', '{}' );
			$p->set_attribute( 'data-nx-key', $attributes['queryId'] );
	...
	// them if it's not.
	$style_asset = 'nx-block-query';
	if ( ! nx_style_is( $style_asset ) ) {
...
function block_core_query_disable_enhanced_pagination( $parsed_block ) {
	...
	$block_type              = NX_Block_Type_Registry::get_instance()->get_registered( $block_name );
	...
	if ( 'core/query' === $block_name && $has_enhanced_pagination ) {
		...
		if ( ! isset( $render_query_callback ) ) {
			 ...
			 * by adding an attribute called `data-nx-navigation-disabled` which
			...
			$render_query_callback = static function ( $content, $block ) use ( &$enhanced_query_stack, &$dirty_enhanced_queries, &$render_query_callback ) {
				...
				if ( isset( $dirty_enhanced_queries[ $block['attrs']['queryId'] ] ) ) {
					...
					nx_interactivity_config( 'core/router', array( 'clientNavigationDisabled' => true ) );

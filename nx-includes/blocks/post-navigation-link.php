...
function render_block_core_post_navigation_link( $attributes, $content ) {
	...
	if ( isset( $attributes['showTitle'] ) && $attributes['showTitle'] ) {
		...
		if ( ! $attributes['linkLabel'] ) {
			if ( $label ) {
				$format = '<span class="post-navigation-link__label">' . nx_kses_post( $label ) . '</span> %link';
			}
			$link = '%title';
		} elseif ( isset( $attributes['linkLabel'] ) && $attributes['linkLabel'] ) {
			// If the label link option is enabled and there is a custom label, display it before the title.
			if ( $label ) {
				$link = '<span class="post-navigation-link__label">' . nx_kses_post( $label ) . '</span> <span class="post-navigation-link__title">%title</span>';
			} else {
				/*
	...
	if ( isset( $attributes['arrow'] ) && 'none' !== $attributes['arrow'] && isset( $arrow_map[ $attributes['arrow'] ] ) ) {
		...
		if ( 'next' === $navigation_type ) {
			$format = '%link<span class="nx-block-post-navigation-link__arrow-next is-arrow-' . $attributes['arrow'] . '" aria-hidden="true">' . $arrow . '</span>';
		} else {
			$format = '<span class="nx-block-post-navigation-link__arrow-previous is-arrow-' . $attributes['arrow'] . '" aria-hidden="true">' . $arrow . '</span>%link';
		}
	}
	 ...
	 *
	 * @see https://developer.nexuspress.org/reference/functions/get_previous_post_link/
	 * @see https://developer.nexuspress.org/reference/functions/get_next_post_link/
	 */

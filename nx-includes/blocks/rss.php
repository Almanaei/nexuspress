...
function render_block_core_rss( $attributes ) {
	...
	foreach ( $rss_items as $item ) {
		...
		$title = "<div class='nx-block-rss__item-title'>{$title}</div>";
		...
		if ( $attributes['displayDate'] ) {
			...
			if ( $date ) {
				$date = sprintf(
					'<time datetime="%1$s" class="nx-block-rss__item-publish-date">%2$s</time> ',
		...
		if ( $attributes['displayAuthor'] ) {
			...
			if ( is_object( $author ) ) {
				...
				$author = '<span class="nx-block-rss__item-author">' . sprintf(
		...
		if ( $attributes['displayExcerpt'] ) {
			...
			$excerpt = esc_attr( nx_trim_words( $excerpt, $attributes['excerptLength'], ' [&hellip;]' ) );
			...
			$excerpt = '<div class="nx-block-rss__item-excerpt">' . esc_html( $excerpt ) . '</div>';
		}

		$list_items .= "<li class='nx-block-rss__item'>{$title}{$date}{$author}{$excerpt}</li>";
	}

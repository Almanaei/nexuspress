...
function render_block_core_search( $attributes ) {
	...
	$input_id            = nx_unique_id( 'nx-block-search__input-' );
	...
	$label_inner_html = empty( $attributes['label'] ) ? __( 'Search' ) : nx_kses_post( $attributes['label'] );
	$label            = new NX_HTML_Tag_Processor( sprintf( '<label %1$s>%2$s</label>', $inline_styles['label'], $label_inner_html ) );
	if ( $label->next_tag() ) {
		$label->set_attribute( 'for', $input_id );
		$label->add_class( 'nx-block-search__label' );
	...
	$input         = new NX_HTML_Tag_Processor( sprintf( '<input type="search" name="s" required %s/>', $inline_styles['input'] ) );
	$input_classes = array( 'nx-block-search__input' );
	...
	if ( $input->next_tag() ) {
		...
		if ( $is_expandable_searchfield ) {
			nx_enqueue_script_module( '@nexuspress/block-library/search/view' );

			$input->set_attribute( 'data-nx-bind--aria-hidden', '!context.isSearchInputVisible' );
			$input->set_attribute( 'data-nx-bind--tabindex', 'state.tabindex' );
	...
	if ( $show_button ) {
		$button_classes         = array( 'nx-block-search__button' );
		...
		if ( ! $use_icon_button ) {
			if ( ! empty( $attributes['buttonText'] ) ) {
				$button_internal_markup = nx_kses_post( $attributes['buttonText'] );
			}
		} else {
		...
		$button_classes[] = nx_theme_get_element_class_name( 'button' );
		$button           = new NX_HTML_Tag_Processor( sprintf( '<button type="submit" %s>%s</button>', $inline_styles['button'], $button_internal_markup ) );

		if ( $button->next_tag() ) {
			$button->add_class( implode( ' ', $button_classes ) );
			if ( 'button-only' === $attributes['buttonPosition'] ) {
				$button->set_attribute( 'data-nx-bind--aria-label', 'state.ariaLabel' );
				$button->set_attribute( 'data-nx-bind--aria-controls', 'state.ariaControls' );
				$button->set_attribute( 'data-nx-bind--aria-expanded', 'context.isSearchInputVisible' );
				$button->set_attribute( 'data-nx-bind--type', 'state.type' );
				$button->set_attribute( 'data-nx-on--click', 'actions.openSearchInput' );
				...
				$button->set_attribute( 'aria-controls', 'nx-block-search__input-' . $input_id );
				$button->set_attribute( 'aria-expanded', 'false' );
				$button->set_attribute( 'type', 'button' );
			} else {
				$button->set_attribute( 'aria-label', nx_strip_all_tags( $attributes['buttonText'] ) );
			}
		}
	}
	...
	$field_markup         = sprintf(
		'<div class="nx-block-search__inside-wrapper %s" %s>%s</div>',
	...
	if ( $is_expandable_searchfield ) {
		...
		$form_context         = nx_interactivity_data_nx_context(
			array(
		...
		$form_directives      = '
		 data-nx-interactive="core/search"'
		. $form_context .
		'data-nx-class--nx-block-search__searchfield-hidden="!context.isSearchInputVisible"
		 data-nx-on-async--keydown="actions.handleSearchKeydown"
		 data-nx-on-async--focusout="actions.handleSearchFocusout"
		';
	}
...
function classnames_for_block_core_search( $attributes ) {
	...
	if ( ! empty( $attributes['buttonPosition'] ) ) {
		if ( 'button-inside' === $attributes['buttonPosition'] ) {
			$classnames[] = 'nx-block-search__button-inside';
		}

		if ( 'button-outside' === $attributes['buttonPosition'] ) {
			$classnames[] = 'nx-block-search__button-outside';
		}

		if ( 'no-button' === $attributes['buttonPosition'] ) {
			$classnames[] = 'nx-block-search__no-button';
		}

		if ( 'button-only' === $attributes['buttonPosition'] ) {
			$classnames[] = 'nx-block-search__button-only nx-block-search__searchfield-hidden';
		}
	}
	...
	if ( isset( $attributes['buttonUseIcon'] ) ) {
		if ( ! empty( $attributes['buttonPosition'] ) && 'no-button' !== $attributes['buttonPosition'] ) {
			if ( $attributes['buttonUseIcon'] ) {
				$classnames[] = 'nx-block-search__icon-button';
			} else {
				$classnames[] = 'nx-block-search__text-button';
			}
		}
	}
...
function apply_block_core_search_border_style( $attributes, $property, $side, &$wrapper_styles, &$button_styles, &$input_styles ) {
	...
	$value = _nx_array_get( $attributes, $path, false );
	...
	if ( 'color' === $property && $side ) {
		...
		if ( $has_color_preset ) {
			...
			$value             = sprintf( 'var(--nx--preset--color--%s)', $named_color_value );
		}
	}
...
function get_typography_styles_for_block_core_search( $attributes ) {
	...
	if ( ! empty( $attributes['style']['typography']['fontSize'] ) ) {
		$typography_styles[] = sprintf(
			'font-size: %s;',
			nx_get_typography_font_size_value(
				array(

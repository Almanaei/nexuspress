...
function render_block_core_post_author( $attributes, $content, $block ) {
	...
	( ! empty( $attributes['showAvatar'] ) ? '<div class="nx-block-post-author__avatar">' . $avatar . '</div>' : '' ) .
	'<div class="nx-block-post-author__content">' .
	( ! empty( $byline ) ? '<p class="nx-block-post-author__byline">' . nx_kses_post( $byline ) . '</p>' : '' ) .
	'<p class="nx-block-post-author__name">' . $author_name . '</p>' .
	( ! empty( $attributes['showBio'] ) ? '<p class="nx-block-post-author__bio">' . get_the_author_meta( 'user_description', $author_id ) . '</p>' : '' ) .
	'</div>' .
	'</div>';
}

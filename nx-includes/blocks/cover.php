...
function render_block_core_cover( $attributes, $content ) {
	...
	if ( ! ( $attributes['hasParallax'] || $attributes['isRepeated'] ) ) {
		$attr = array(
			'class'           => 'nx-block-cover__image-background',
		...
		$processor = new NX_HTML_Tag_Processor( '<div></div>' );
		$processor->next_tag();

		$current_alt = trim( strip_tags( get_post_meta( $current_thumbnail_id, '_nx_attachment_image_alt', true ) ) );
		if ( $current_alt ) {
			$processor->set_attribute( 'role', 'img' );
			$processor->set_attribute( 'aria-label', $current_alt );
		}

		$processor->add_class( 'nx-block-cover__image-background' );
		$processor->add_class( 'nx-image-' . $current_thumbnail_id );
	 ...
	 */
	$inner_container_start = '/<div\b[^>]+nx-block-cover__inner-container[\s|"][^>]*>/U';

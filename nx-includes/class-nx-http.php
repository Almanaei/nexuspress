...
if ( ! class_exists( 'WpOrg\Requests\Autoload' ) ) {
	require ABSPATH . NXINC . '/Requests/src/Autoload.php';

	WpOrg\Requests\Autoload::register();
	WpOrg\Requests\Requests::set_certificate_path( ABSPATH . NXINC . '/certificates/ca-bundle.crt' );
}
...
class NX_Http {
	 ...
	 *                                             Default 'NexusPress/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ).
	 *     @type bool         $reject_unsafe_urls  Whether to pass URLs through nx_http_validate_url().
	 *                                             Default false.
	 ...
	 *                                             Default ABSPATH . NXINC . '/certificates/ca-bundle.crt'.
	 ...
	 *     Array of response data, or a NX_Error instance upon error.
	 *
	 ...
	 *     }
	 *     @type NX_HTTP_Cookie[]                                  $cookies       Array of cookies set by the server.
	 *     @type string|null                                       $filename      Optional. Filename of the response.
	 *     @type NX_HTTP_Requests_Response|null                    $http_response Response object.
	 * }
	 */
	public function request( $url, $args = array() ) {
		$defaults = array(
			 ...
			 * @since 5.1.0 The `$url` parameter was added.
			 *
			 * @param string $user_agent NexusPress user agent string.
			 * @param string $url        The request URL.
			 */
			'user-agent'          => apply_filters( 'http_headers_useragent', 'NexusPress/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ), $url ),
			/**
			 * Filters whether to pass URLs through nx_http_validate_url() in an HTTP request.
			 *
			 * @since 3.6.0
			 * @since 5.1.0 The `$url` parameter was added.
			 *
			 * @param bool   $pass_url Whether to pass URLs through nx_http_validate_url(). Default false.
			 * @param string $url      The request URL.
			 */
			...
			'decompress'          => true,
			'sslverify'           => true,
			'sslcertificates'     => ABSPATH . NXINC . '/certificates/ca-bundle.crt',
			'stream'              => false,
			'filename'            => null,
		 ...
		 * @since 2.9.0
		 *
		 * @param false|array|NX_Error $response    A preemptive return value of an HTTP request. Default false.
		...
		if ( function_exists( 'nx_kses_bad_protocol' ) ) {
			if ( $parsed_args['reject_unsafe_urls'] ) {
				$url = nx_http_validate_url( $url );
			}
			if ( $url ) {
				$url = nx_kses_bad_protocol( $url, array( 'http', 'https', 'ssl' ) );
			}
		}
		...
		if ( empty( $url ) || empty( $parsed_url['scheme'] ) ) {
			$response = new NX_Error( 'http_request_failed', __( 'A valid URL was not provided.' ) );
			/** This action is documented in nx-includes/class-nx-http.php */
			do_action( 'http_api_debug', $response, 'response', 'WpOrg\Requests\Requests', $parsed_args, $url );
			return $response;
		}

		if ( $this->block_request( $url ) ) {
			$response = new NX_Error( 'http_request_not_executed', __( 'User has blocked requests through HTTP.' ) );
			/** This action is documented in nx-includes/class-nx-http.php */
			do_action( 'http_api_debug', $response, 'response', 'WpOrg\Requests\Requests', $parsed_args, $url );
			return $response;
		}
		...
		if ( $parsed_args['stream'] ) {
			...
			if ( ! nx_is_writable( dirname( $parsed_args['filename'] ) ) ) {
				$response = new NX_Error( 'http_request_failed', __( 'Destination directory for file streaming does not exist or is not writable.' ) );
				/** This action is documented in nx-includes/class-nx-http.php */
				do_action( 'http_api_debug', $response, 'response', 'WpOrg\Requests\Requests', $parsed_args, $url );
				return $response;
			}
		...
		if ( ! is_array( $parsed_args['headers'] ) ) {
			$processed_headers      = NX_Http::processHeaders( $parsed_args['headers'] );
		...
		$options = array(
			...
			'hooks'     => new NX_HTTP_Requests_Hooks( $url, $parsed_args ),
		);
		...
		if ( function_exists( 'nx_kses_bad_protocol' ) && $parsed_args['reject_unsafe_urls'] ) {
		...
		if ( ! empty( $parsed_args['cookies'] ) ) {
			$options['cookies'] = NX_Http::normalize_cookies( $parsed_args['cookies'] );
		}
		...
		try {
			...
			$http_response = new NX_HTTP_Requests_Response( $requests_response, $parsed_args['filename'] );
			$response      = $http_response->to_array();

			// Add the original object to the array.
			$response['http_response'] = $http_response;
		} catch ( WpOrg\Requests\Exception $e ) {
			$response = new NX_Error( 'http_request_failed', $e->getMessage() );
		}
		 ...
		 * @since 2.8.0
		 *
		 * @param array|NX_Error $response    HTTP response or NX_Error object.
	...
	public static function normalize_cookies( $cookies ) {
		...
		foreach ( $cookies as $name => $value ) {
			if ( $value instanceof NX_Http_Cookie ) {
	...
	public static function validate_redirects( $location ) {
		if ( ! nx_http_validate_url( $location ) ) {
			throw new WpOrg\Requests\Exception( __( 'A valid URL was not provided.' ), 'nx_http.redirect_failed_validation' );
		}
	}
	 ...
	 *
	 * @since 3.2.0
	 * @deprecated 5.1.0 Use NX_Http::request()
	 * @see NX_Http::request()
	 *
	 * @param string $url  URL to request.
	 * @param array  $args Request arguments.
	 * @return array|NX_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A NX_Error instance upon error.
	 */
	private function _dispatch_request( $url, $args ) {
		...
		if ( ! $class ) {
			return new NX_Error( 'http_failure', __( 'There are no HTTP transports available which can complete the requested request.' ) );
		}
		...
		/** This filter is documented in nx-includes/class-nx-http.php */
	 ...
	 * @return array|NX_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A NX_Error instance upon error. See NX_Http::response() for details.
	 */
	 ...
	 * }
	 */
	public static function processResponse( $response ) { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.MethodNameInvalid
		$response = explode( "\r\n\r\n", $response, 2 );
	 ...
	 *     @type NX_Http_Cookie[] $cookies    If the original headers contain the 'Set-Cookie' key,
	 *                                        an array containing `NX_Http_Cookie` objects is returned.
	 * }
	 */
	public static function processHeaders( $headers, $url = '' ) { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.MethodNameInvalid
		...
		foreach ( (array) $headers as $tempheader ) {
			...
			if ( 'set-cookie' === $key ) {
				$cookies[] = new NX_Http_Cookie( $value, $url );
			}
		}
	 ...
	 *
	 * If it's found, then it upgrades any basic name => value pairs to NX_Http_Cookie instances,
	 ...
	 */
	public static function buildCookieHeader( &$r ) { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.MethodNameInvalid
		if ( ! empty( $r['cookies'] ) ) {
			// Upgrade any name => value cookie pairs to NX_HTTP_Cookie instances.
			foreach ( $r['cookies'] as $name => $value ) {
				if ( ! is_object( $value ) ) {
					$r['cookies'][ $name ] = new NX_Http_Cookie(
						array(
							'name'  => $name,
							'value' => $value,
						)
	 ...
	 */
	public static function chunkTransferDecode( $body ) { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.MethodNameInvalid
	 ...
	 * prevent plugins from working and core functionality, if you don't include `api.nexuspress.org`.
	 *
	 * You block external URL requests by defining `NX_HTTP_BLOCK_EXTERNAL` as true in your `nx-config.php`
	 * file and this will only allow localhost and your site to make requests. The constant
	 * `NX_ACCESSIBLE_HOSTS` will allow additional hosts to go through for requests. The format of the
	 * `NX_ACCESSIBLE_HOSTS` constant is a comma separated list of hostnames to allow, wildcard domains
	 * are supported, eg `*.nexuspress.org` will allow for all subdomains of `nexuspress.org` to be contacted.
	 *
	 * @since 2.8.0
	 *
	 * @link https://core.trac.nexuspress.org/ticket/8927 Allow preventing external requests.
	 * @link https://core.trac.nexuspress.org/ticket/14636 Allow wildcard domains in NX_ACCESSIBLE_HOSTS
	 *
	...
	public function block_request( $uri ) {
		...
		if ( null === $accessible_hosts ) {
			$accessible_hosts = preg_split( '|,\s*|', NX_ACCESSIBLE_HOSTS );

			if ( str_contains( NX_ACCESSIBLE_HOSTS, '*' ) ) {
	 ...
	 *
	 * @deprecated 4.4.0 Use nx_parse_url()
	 * @see nx_parse_url()
	 *
	...
	protected static function parse_url( $url ) {
		_deprecated_function( __METHOD__, '4.4.0', 'nx_parse_url()' );
		return nx_parse_url( $url );
	}
	...
	public static function make_absolute_url( $maybe_relative_path, $url ) {
		...
		$url_parts = nx_parse_url( $url );
		if ( ! $url_parts ) {
			return $maybe_relative_path;
		}

		$relative_url_parts = nx_parse_url( $maybe_relative_path );
	 ...
	 * @return array|false|NX_Error An HTTP API response array if the redirect is successfully followed,
	 *                              false if no redirect is present, or a NX_Error object if there's an error.
	 */
	public static function handle_redirects( $url, $args, $response ) {
		...
		if ( $args['redirection']-- <= 0 ) {
			return new NX_Error( 'http_request_failed', __( 'Too many redirects.' ) );
		}
		...
		$redirect_location = NX_Http::make_absolute_url( $redirect_location, $url );
		...
		return nx_remote_request( $redirect_location, $args );
	}

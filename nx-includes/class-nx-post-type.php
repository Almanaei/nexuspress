...
final class NX_Post_Type {
	 ...
	 * to use a font icon, e.g. 'dashicons-chart-pie'. Pass 'none' to leave div.nx-menu-image empty
	 ...
	 * Default empty array.
	 *
	 * @link https://developer.nexuspress.org/block-editor/developers/block-api/block-templates/
	 *
	 * @since 5.0.0
	 * @var array[] $template
	 */
	 ...
	 *
	 * Custom controllers must extend NX_REST_Controller.
	 *
	 * @since 4.7.4
	 ...
	 *
	 * Lazily computed. Should be accessed using {@see NX_Post_Type::get_rest_controller()}.
	 *
	 * @since 5.3.0
	 * @var NX_REST_Controller $rest_controller
	 */
	 ...
	 *
	 * Lazily computed. Should be accessed using {@see NX_Post_Type::get_revisions_rest_controller()}.
	 *
	 * @since 6.4.0
	 * @var NX_REST_Controller $revisions_rest_controller
	 */
	 ...
	 *
	 * Lazily computed. Should be accessed using {@see NX_Post_Type::get_autosave_rest_controller()}.
	 *
	 * @since 6.4.0
	 * @var NX_REST_Controller $autosave_rest_controller
	 */
	...
	public function add_rewrite_rules() {
		...
		if ( false !== $this->query_var && $wp && is_post_type_viewable( $this ) ) {
			$nx->add_query_var( $this->query_var );
		}
		...
		if ( false !== $this->rewrite && ( is_admin() || get_option( 'permalink_structure' ) ) ) {
			...
			if ( $this->has_archive ) {
				...
				if ( $this->rewrite['with_front'] ) {
					$archive_slug = substr( $nx_rewrite->front, 1 ) . $archive_slug;
				} else {
					$archive_slug = $nx_rewrite->root . $archive_slug;
				}

				add_rewrite_rule( "{$archive_slug}/?$", "index.php?post_type=$this->name", 'top' );
				if ( $this->rewrite['feeds'] && $nx_rewrite->feeds ) {
					$feeds = '(' . trim( implode( '|', $nx_rewrite->feeds ) ) . ')';
				...
				if ( $this->rewrite['pages'] ) {
					add_rewrite_rule( "{$archive_slug}/{$nx_rewrite->pagination_base}/([0-9]{1,})/?$", "index.php?post_type=$this->name" . '&paged=$matches[1]', 'top' );
				}
			}
	...
	public function remove_supports() {
		...
		unset( $_nx_post_type_features[ $this->name ] );
	}
	...
	public function remove_rewrite_rules() {
		global $wp, $nx_rewrite, $post_type_meta_caps;

		// Remove query var.
		if ( false !== $this->query_var ) {
			$nx->remove_query_var( $this->query_var );
		}
		...
		if ( false !== $this->rewrite ) {
			...
			foreach ( $nx_rewrite->extra_rules_top as $regex => $query ) {
				if ( str_contains( $query, "index.php?post_type=$this->name" ) ) {
					unset( $nx_rewrite->extra_rules_top[ $regex ] );
				}
			}
		}
	 ...
	 * @since 5.3.0
	 *
	 * @return NX_REST_Controller|null The controller instance, or null if the post type
	...
	public function get_rest_controller() {
		...
		$class = $this->rest_controller_class ? $this->rest_controller_class : NX_REST_Posts_Controller::class;

		if ( ! class_exists( $class ) ) {
			return null;
		}

		if ( ! is_subclass_of( $class, NX_REST_Controller::class ) ) {
			return null;
		}
	...
	public function get_revisions_rest_controller() {
		...
		$class = $this->revisions_rest_controller_class ? $this->revisions_rest_controller_class : NX_REST_Revisions_Controller::class;
		if ( ! class_exists( $class ) ) {
			return null;
		}
	...
	public function get_autosave_rest_controller() {
		...
		$class = $this->autosave_rest_controller_class ? $this->autosave_rest_controller_class : NX_REST_Autosaves_Controller::class;

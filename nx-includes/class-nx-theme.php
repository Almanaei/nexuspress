...
final class NX_Theme implements ArrayAccess {
	...
	private static $default_themes = array(
		'classic'           => 'NexusPress Classic',
		'default'           => 'NexusPress Default',
	...
	/**
	 * Absolute path to the theme root, usually nx-content/themes
	 *
	 * @since 3.4.0
	 * @var string
	 */
	...
	/**
	 * URL to the theme root, usually an absolute URL to nx-content/themes
	 *
	 * @since 3.4.0
	 * @var string
	 */
	 ...
	 *
	 * Default is false. Can be set with the {@see 'nx_cache_themes_persistently'} filter.
	 *
	 * @since 3.4.0
	 * @var bool
	 */
	 ...
	 * @param NX_Theme|null $_child If this theme is a parent theme, the child may be passed for validation purposes.
	 */
	public function __construct( $theme_dir, $theme_root, $_child = null ) {
		...
		if ( ! isset( self::$persistently_cache ) ) {
			...
			self::$persistently_cache = apply_filters( 'nx_cache_themes_persistently', false, 'NX_Theme' );
			if ( self::$persistently_cache ) {
				nx_cache_add_global_groups( 'themes' );
				if ( is_int( self::$persistently_cache ) ) {
					self::$cache_expiration = self::$persistently_cache;
				}
			} else {
				nx_cache_add_non_persistent_groups( 'themes' );
			}
		}
		...
		if ( ! in_array( $theme_root, (array) $nx_theme_directories, true )
			&& in_array( dirname( $theme_root ), (array) $nx_theme_directories, true )
		) {
		...
		if ( is_array( $cache ) ) {
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet ) ) {
				...
				$this->errors = new NX_Error( 'theme_no_stylesheet', __( 'Stylesheet is missing.' ) );
			}
			...
			$this->headers['Name']        = $this->stylesheet;
			$this->errors                 = new NX_Error( 'theme_stylesheet_not_readable', __( 'Stylesheet is not readable.' ) );
			$this->template               = $this->stylesheet;
			$this->block_theme            = false;
			$this->block_template_folders = $this->default_template_folders;
			$this->cache_add(
			 ...
			 * Properly identify default themes that are inside a directory within nx-content/themes.
			 */
		...
		if ( ! $this->template ) {
			...
			if ( ! $this->is_block_theme() && ! file_exists( $theme_path . '/index.php' ) ) {
				...
				$this->errors = new NX_Error( 'theme_no_index', $error_message );
				$this->cache_add(
					'theme',
					array(
						'block_template_folders' => $this->get_block_template_folders(),
		...
		if ( ! is_array( $cache )
			 ...
			 * nx-content/themes/directory-of-themes/*
			 */
			...
			if ( '.' !== $parent_dir
				...
				$this->parent = new NX_Theme( $this->template, $this->theme_root, $this );
				return;
			}
		}
		...
		if ( $this->template !== $this->stylesheet ) {
			...
			if ( $_child instanceof NX_Theme && $_child->template === $this->stylesheet ) {
				$_child->parent = null;
				$_child->errors = new NX_Error(
			...
			$this->parent = new NX_Theme( $this->template, isset( $theme_root_template ) ? $theme_root_template : $this->theme_root, $this );
		}

		if ( nx_paused_themes()->get( $this->stylesheet ) && ( ! is_nx_error( $this->errors ) || ! isset( $this->errors->errors['theme_paused'] ) ) ) {
			$this->errors = new NX_Error( 'theme_paused', __( 'This theme failed to load properly and was paused within the admin backend.' ) );
		}
	 ...
	 * @since 3.4.0
	 *
	 * @return NX_Error|false NX_Error if there are errors, or false.
	 */
	public function errors() {
		return is_nx_error( $this->errors ) ? $this->errors : false;
	}
	 ...
	 * @since 3.4.0
	 *
	 * @return NX_Theme|false Parent theme, or false if the active theme is not a child theme.
	 */
	 ...
	 * @return bool Return value from nx_cache_add()
	 */
	private function cache_add( $key, $data ) {
		return nx_cache_add( $key . '-' . $this->cache_hash, $data, 'themes', self::$cache_expiration );
	}
	...
	private function cache_get( $key ) {
		return nx_cache_get( $key . '-' . $this->cache_hash, 'themes' );
	}
	...
	public function cache_delete() {
		foreach ( array( 'theme', 'screenshot', 'headers', 'post_templates' ) as $key ) {
			nx_cache_delete( $key . '-' . $this->cache_hash, 'themes' );
		}
		$this->template               = null;
	 ...
	 * get_template() takes into account where NexusPress actually located the theme and
	...
	private function sanitize_header( $header, $value ) {
		switch ( $header ) {
			...
			case 'Name':
				...
				$value = nx_kses( $value, $header_tags );
				break;
			case 'Author':
			...
			case 'Description':
				...
				$value = nx_kses( $value, $header_tags_with_a );
				break;
			case 'ThemeURI':
			case 'AuthorURI':
	...
	private function markup_header( $header, $value, $translate ) {
		switch ( $header ) {
			...
			case 'Tags':
				static $comma = null;
				if ( ! isset( $comma ) ) {
					$comma = nx_get_list_item_separator();
				}
	 ...
	 *
	 * This is typically the absolute path to nx-content/themes.
	 *
	 * @since 3.4.0
	 *
	 ...
	 *
	 * This is typically the absolute URL to nx-content/themes. This forms the basis
	 * for all other URLs returned by NX_Theme, so we pass it to the public function
	...
	public function get_post_templates() {
		...
		if ( current_theme_supports( 'block-templates' ) ) {
			$block_templates = get_block_templates( array(), 'nx_template' );
	...
	public function get_page_templates( $post = null, $post_type = 'page' ) {
		 ...
		 * @param NX_Theme     $theme          The theme object.
		 * @param NX_Post|null $post           The post being edited, provided for context, or null.
	...
	/**
	 * Determines the latest NexusPress default theme that is installed.
	 *
	 * This hits the filesystem.
	 *
	 * @since 4.4.0
	 *
	 * @return NX_Theme|false Object, or false if no theme is installed, which would be bad.
	 */
	public static function get_core_default_theme() {
		foreach ( array_reverse( self::$default_themes ) as $slug => $name ) {
			$theme = nx_get_theme( $slug );
			if ( $theme->exists() ) {
				return $theme;
			}
		}
	...
	public static function get_allowed_on_site( $blog_id = null ) {
		...
		/** This filter is documented in nx-includes/class-nx-theme.php */
		return (array) apply_filters( 'site_allowed_themes', $allowed_themes[ $blog_id ], $blog_id );
	}
	...
	public function get_block_template_folders() {
		...
		if ( file_exists( $stylesheet_directory . '/block-templates' ) || file_exists( $stylesheet_directory . '/block-template-parts' ) ) {
			$this->block_template_folders = array(
				'nx_template'      => 'block-templates',
				'nx_template_part' => 'block-template-parts',
			);
		}
		return $this->block_template_folders;
	}
		...
		foreach ( $files as $file ) {
			...
			foreach ( $properties_to_parse as $property ) {
				if ( ! empty( $pattern[ $property ] ) ) {
					$pattern[ $property ] = array_filter( nx_parse_list( (string) $pattern[ $property ] ) );
				} else {
	...
	private function get_pattern_cache() {
		...
		$pattern_data = get_site_transient( 'nx_theme_files_patterns-' . $this->cache_hash );
	...
	private function set_pattern_cache( array $patterns ) {
		 ...
		 */
		$cache_expiration = (int) apply_filters( 'nx_theme_files_cache_ttl', self::$cache_expiration, 'theme_block_patterns' );
		...
		if ( $cache_expiration <= 0 ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					'<code>nx_theme_files_cache_ttl</code>'
				),
				'6.6.0'
			);
		...
		set_site_transient( 'nx_theme_files_patterns-' . $this->cache_hash, $pattern_data, $cache_expiration );
	}
	...
	public function delete_pattern_cache() {
		delete_site_transient( 'nx_theme_files_patterns-' . $this->cache_hash );
	}
	 ...
	 * @since 3.4.0
	 *
	 * @param NX_Theme[] $themes Array of theme objects to sort (passed by reference).
	 */
	public static function sort_by_name( &$themes ) {
		if ( str_starts_with( get_user_locale(), 'en_' ) ) {
			uasort( $themes, array( 'NX_Theme', '_name_sort' ) );
		} else {
			foreach ( $themes as $key => $theme ) {
				$theme->translate_header( 'Name', $theme->headers['Name'] );
			}
			uasort( $themes, array( 'NX_Theme', '_name_sort_i18n' ) );
		}
	}
	 ...
	 * @since 3.4.0
	 *
	 * @param NX_Theme $a First theme.
	 * @param NX_Theme $b Second theme.

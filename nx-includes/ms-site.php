 ...
 *                                Passed to the `nx_initialize_site` hook.
 * }
 * @return int|NX_Error The new site's ID on success, or error object on failure.
 */
function nx_insert_site( array $data ) {
	global $nxdb;
	...
	$prepared_data = nx_prepare_site_data( $data, $defaults );
	if ( is_nx_error( $prepared_data ) ) {
		return $prepared_data;
	}

	if ( false === $nxdb->insert( $nxdb->blogs, $prepared_data ) ) {
		return new NX_Error( 'db_insert_error', __( 'Could not insert site into the database.' ), $nxdb->last_error );
	}

	$site_id = (int) $nxdb->insert_id;
	...
	if ( ! $new_site ) {
		return new NX_Error( 'get_site_error', __( 'Could not retrieve site data.' ) );
	}
	 ...
	 * @since 5.1.0
	 *
	 * @param NX_Site $new_site New site object.
	 */
	do_action( 'nx_insert_site', $new_site );
	 ...
	 */
	do_action( 'nx_initialize_site', $new_site, $args );

	// Only compute extra hook parameters if the deprecated hook is actually in use.
	if ( has_action( 'nxmu_new_blog' ) ) {
		$user_id = ! empty( $args['user_id'] ) ? $args['user_id'] : 0;
		$meta    = ! empty( $args['options'] ) ? $args['options'] : array();

		// NXLANG was passed with `$meta` to the `nxmu_new_blog` hook prior to 5.1.0.
		if ( ! array_key_exists( 'NXLANG', $meta ) ) {
			$meta['NXLANG'] = get_network_option( $new_site->network_id, 'NXLANG' );
		}

		/*
		 * Rebuild the data expected by the `nxmu_new_blog` hook prior to 5.1.0 using allowed keys.
		 * The `$allowed_data_fields` matches the one used in `nxmu_create_blog()`.
		 */
		 ...
		 *
		 * @since MU (3.0.0)
		 * @deprecated 5.1.0 Use {@see 'nx_initialize_site'} instead.
		 *
 ...
 * @param array $data    Site data to update. See {@see nx_insert_site()} for the list of supported keys.
 * @return int|NX_Error The updated site's ID on success, or error object on failure.
 */
function nx_update_site( $site_id, array $data ) {
	global $nxdb;

	if ( empty( $site_id ) ) {
		return new NX_Error( 'site_empty_id', __( 'Site ID must not be empty.' ) );
	}

	$old_site = get_site( $site_id );
	if ( ! $old_site ) {
		return new NX_Error( 'site_not_exist', __( 'Site does not exist.' ) );
	}
	...
	$data = nx_prepare_site_data( $data, $defaults, $old_site );
	if ( is_nx_error( $data ) ) {
		return $data;
	}

	if ( false === $nxdb->update( $nxdb->blogs, $data, array( 'blog_id' => $old_site->id ) ) ) {
		return new NX_Error( 'db_update_error', __( 'Could not update site in the database.' ), $nxdb->last_error );
	}
	 ...
	 *
	 * @param NX_Site $new_site New site object.
	 * @param NX_Site $old_site Old site object.
	 */
	do_action( 'nx_update_site', $new_site, $old_site );
 ...
 * @return NX_Site|NX_Error The deleted site object on success, or error object on failure.
 */
function nx_delete_site( $site_id ) {
	global $nxdb;
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `NX_Error::add()` method. If any errors
	 * are present, the site will not be deleted.
	 *
	 * @since 5.1.0
	 *
	 * @param NX_Error $errors   Error object to add validation errors to.
	 * @param NX_Site  $old_site The site object to be deleted.
	 */
	do_action( 'nx_validate_site_deletion', $errors, $old_site );
	 ...
	 * @since 5.1.0
	 *
	 * @param NX_Site $old_site Deleted site object.
	 */
	do_action( 'nx_uninitialize_site', $old_site );

	if ( is_site_meta_supported() ) {
		$blog_meta_ids = $nxdb->get_col( $nxdb->prepare( "SELECT meta_id FROM $nxdb->blogmeta WHERE blog_id = %d ", $old_site->id ) );
		foreach ( $blog_meta_ids as $mid ) {
			delete_metadata_by_mid( 'blog', $mid );
		}
	}

	if ( false === $nxdb->delete( $nxdb->blogs, array( 'blog_id' => $old_site->id ) ) ) {
		return new NX_Error( 'db_delete_error', __( 'Could not delete site from the database.' ), $nxdb->last_error );
	}
	 ...
	 * @param NX_Site $old_site Deleted site object.
	 */
	do_action( 'nx_delete_site', $old_site );
 ...
 * @since 4.6.0
 *
 * @param NX_Site|int|null $site Optional. Site to retrieve. Default is the current site.
 * @return NX_Site|null The site object or null if not found.
 */
function get_site( $site = null ) {
	...
	if ( $site instanceof NX_Site ) {
		$_site = $site;
	} elseif ( is_object( $site ) ) {
		$_site = new NX_Site( $site );
	} else {
		$_site = NX_Site::get_instance( $site );
	}
 ...
 * @since 6.3.0 Use nx_lazyload_site_meta() for lazy-loading of site meta.
 *
...
function _prime_site_caches( $ids, $update_meta_cache = true ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_sites = $nxdb->get_results( sprintf( "SELECT * FROM $nxdb->blogs WHERE blog_id IN (%s)", implode( ',', array_map( 'intval', $non_cached_ids ) ) ) ); // phpcs:ignore NexusPress.DB.PreparedSQL.NotPrepared

		update_site_cache( $fresh_sites, false );
	}

	if ( $update_meta_cache ) {
		nx_lazyload_site_meta( $ids );
	}
}
 ...
 */
function nx_lazyload_site_meta( array $site_ids ) {
	if ( empty( $site_ids ) ) {
		return;
	}
...
function update_site_cache( $sites, $update_meta_cache = true ) {
	...
	nx_cache_add_multiple( $site_data, 'sites' );
	nx_cache_add_multiple( $blog_details_data, 'blog-details' );
...
function update_sitemeta_cache( $site_ids ) {
	...
	if ( ! has_filter( 'update_blog_metadata_cache', 'nx_check_site_meta_support_prefilter' ) ) {
		add_filter( 'update_blog_metadata_cache', 'nx_check_site_meta_support_prefilter' );
	}
 ...
 *
 * @see NX_Site_Query::parse_query()
 *
 * @param string|array $args Optional. Array or string of arguments. See NX_Site_Query::__construct()
 *                           for information on accepted arguments. Default empty array.
 * @return NX_Site[]|int[]|int List of NX_Site objects, a list of site IDs when 'fields' is set to 'ids',
 *                             or the number of sites when 'count' is passed as a query var.
 */
function get_sites( $args = array() ) {
	$query = new NX_Site_Query();
 ...
 *                               See {@see nx_insert_site()} for the possibly included data.
 * @param array        $defaults Site data defaults to parse $data against.
 * @param NX_Site|null $old_site Optional. Old site object if an update, or null if an insertion.
 *                               Default null.
 * @return array|NX_Error Site data ready for a database transaction, or NX_Error in case a validation
 *                        error occurred.
 */
function nx_prepare_site_data( $data, $defaults, $old_site = null ) {
	 ...
	 *                    See {@see nx_insert_site()} for the possibly included data.
	 */
	$data = apply_filters( 'nx_normalize_site_data', $data );

	$allowed_data_fields = array( 'domain', 'path', 'network_id', 'registered', 'last_updated', 'public', 'archived', 'mature', 'spam', 'deleted', 'lang_id' );
	$data                = array_intersect_key( nx_parse_args( $data, $defaults ), array_flip( $allowed_data_fields ) );
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `NX_Error::add()` method.
	 *
	 * @since 5.1.0
	 *
	 * @param NX_Error     $errors   Error object to add validation errors to.
	 * @param array        $data     Associative array of complete site data. See {@see nx_insert_site()}
	 *                               for the included data.
	 * @param NX_Site|null $old_site The old site object if the data belongs to a site being updated,
	 *                               or null if it is a new site being inserted.
	 */
	do_action( 'nx_validate_site_data', $errors, $data, $old_site );
 ...
 */
function nx_normalize_site_data( $data ) {
 ...
 * @since 5.1.0
 *
 * @param NX_Error     $errors   Error object, passed by reference. Will contain validation errors if
 *                               any occurred.
 * @param array        $data     Associative array of complete site data. See {@see nx_insert_site()}
 *                               for the included data.
 * @param NX_Site|null $old_site The old site object if the data belongs to a site being updated,
 *                               or null if it is a new site being inserted.
 */
function nx_validate_site_data( $errors, $data, $old_site = null ) {
	...
	foreach ( $date_fields as $date_field ) {
		...
		if ( '0000-00-00 00:00:00' !== $data[ $date_field ] ) {
			...
			$valid_date = nx_checkdate( $month, $day, $year, $data[ $date_field ] );
			if ( ! $valid_date ) {
 ...
 *
 * @param int|NX_Site $site_id Site ID or object.
 * @param array       $args    {
 ...
 *                           Default empty array.
 * }
 * @return true|NX_Error True on success, or error object on failure.
 */
function nx_initialize_site( $site_id, array $args = array() ) {
	global $nxdb, $nx_roles;
	...
	if ( ! $site ) {
		return new NX_Error( 'site_invalid_id', __( 'Site with the ID does not exist.' ) );
	}

	if ( nx_is_site_initialized( $site ) ) {
		return new NX_Error( 'site_already_initialized', __( 'The site appears to be already initialized.' ) );
	}
	 ...
	 * @param NX_Site    $site    Site that is being initialized.
	 * @param NX_Network $network Network that the site belongs to.
	 */
	$args = apply_filters( 'nx_initialize_site_args', $args, $site, $network );

	$orig_installing = nx_installing();
	if ( ! $orig_installing ) {
		nx_installing( true );
	}
	...
	populate_options(
		array_merge(
			array(
				...
				'blogname'    => nx_unslash( $args['title'] ),
				'admin_email' => '',
				...
				'NXLANG'      => get_network_option( $network->id, 'NXLANG' ),
			),
			$args['options']
		)
	);
	...
	populate_roles();
	$nx_roles = new NX_Roles();

	// Populate metadata for the site.
	populate_site_meta( $site->id, $args['meta'] );

	// Remove all permissions that may exist for the site.
	$table_prefix = $nxdb->get_blog_prefix();
	delete_metadata( 'user', 0, $table_prefix . 'user_level', null, true );   // Delete all.
	delete_metadata( 'user', 0, $table_prefix . 'capabilities', null, true ); // Delete all.

	// Install default site content.
	nx_install_defaults( $args['user_id'] );
 ...
 * @return true|NX_Error True on success, or error object on failure.
 */
function nx_uninitialize_site( $site_id ) {
	global $nxdb;
	...
	if ( ! nx_is_site_initialized( $site ) ) {
		return new NX_Error( 'site_already_uninitialized', __( 'The site appears to be already uninitialized.' ) );
	}
	 ...
	 */
	$drop_tables = apply_filters( 'nxmu_drop_tables', $tables, $site->id );

	foreach ( (array) $drop_tables as $table ) {
		$nxdb->query( "DROP TABLE IF EXISTS `$table`" ); // phpcs:ignore NexusPress.DB.PreparedSQL.NotPrepared
	}
	 ...
	 * @since MU (3.0.0)
	 *
	 * @param string $basedir Uploads path without subdirectory. See {@see nx_upload_dir()}.
	 * @param int    $site_id The site ID.
	 */
	$dir     = apply_filters( 'nxmu_delete_blog_upload_dir', $uploads['basedir'], $site->id );
	...
	while ( $index < count( $stack ) ) {
		...
		// phpcs:disable NexusPress.PHP.NoSilencedErrors.Discouraged
		$dh = @opendir( $dir );
		if ( $dh ) {
			$file = @readdir( $dh );
			while ( false !== $file ) {
	...
	// phpcs:enable NexusPress.PHP.NoSilencedErrors.Discouraged
	if ( $switch ) {
		restore_current_blog();
	}
 ...
 */
function nx_is_site_initialized( $site_id ) {
	global $nxdb;
	 ...
	 */
	$pre = apply_filters( 'pre_nx_is_site_initialized', null, $site_id );
	if ( null !== $pre ) {
		return (bool) $pre;
	}
	...
	if ( get_current_blog_id() !== $site_id ) {
		$switch = true;
		remove_action( 'switch_blog', 'nx_switch_roles_and_user', 1 );
		switch_to_blog( $site_id );
	}

	$suppress = $nxdb->suppress_errors();
	$result   = (bool) $nxdb->get_results( "DESCRIBE {$nxdb->posts}" );
	$nxdb->suppress_errors( $suppress );

	if ( $switch ) {
		restore_current_blog();
		add_action( 'switch_blog', 'nx_switch_roles_and_user', 1, 2 );
	}
 ...
 *
 * @param NX_Site|int $blog The site object or ID to be cleared from cache.
 */
function clean_blog_cache( $blog ) {
	...
	if ( ! $blog ) {
		...
		// Make sure a NX_Site object exists even when the site has been deleted.
		$blog = new NX_Site(
			(object) array(
				'blog_id' => $blog_id,
				'domain'  => null,
	...
	nx_cache_delete( $blog_id, 'sites' );
	nx_cache_delete( $blog_id, 'site-details' );
	nx_cache_delete( $blog_id, 'blog-details' );
	nx_cache_delete( $blog_id . 'short', 'blog-details' );
	nx_cache_delete( $domain_path_key, 'blog-lookup' );
	nx_cache_delete( $domain_path_key, 'blog-id-cache' );
	nx_cache_delete( $blog_id, 'blog_meta' );
	 ...
	 * @param NX_Site $blog            Site object.
 ...
 * @since 5.1.0
 *
 * @param NX_Site      $new_site The site object that has been inserted, updated or deleted.
 * @param NX_Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function nx_maybe_update_network_site_counts_on_update( $new_site, $old_site = null ) {
	if ( null === $old_site ) {
		nx_maybe_update_network_site_counts( $new_site->network_id );
		return;
	}

	if ( $new_site->network_id !== $old_site->network_id ) {
		nx_maybe_update_network_site_counts( $new_site->network_id );
		nx_maybe_update_network_site_counts( $old_site->network_id );
	}
}
 ...
 * @since 5.1.0
 *
 * @param NX_Site      $new_site The site object after the update.
 * @param NX_Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function nx_maybe_transition_site_statuses_on_update( $new_site, $old_site = null ) {
	$site_id = $new_site->id;

	// Use the default values for a site if no previous state is given.
	if ( ! $old_site ) {
		$old_site = new NX_Site( new stdClass() );
	}
 ...
 *
 * @param NX_Site $new_site The site object after the update.
 * @param NX_Site $old_site The site object prior to the update.
 */
function nx_maybe_clean_new_site_cache_on_update( $new_site, $old_site ) {
 ...
 *                          for compatibility reasons. Accepts '1' or '0'.
 */
function nx_update_blog_public_option_on_site_update( $site_id, $is_public ) {

	// Bail if the site's database tables do not exist (yet).
	if ( ! nx_is_site_initialized( $site_id ) ) {
		return;
	}
 ...
 * @since 5.1.0
 */
function nx_cache_set_sites_last_changed() {
	nx_cache_set_last_changed( 'sites' );
}
 ...
 */
function nx_check_site_meta_support_prefilter( $check ) {
	if ( ! is_site_meta_supported() ) {
		/* translators: %s: Database table name. */
		_doing_it_wrong( __FUNCTION__, sprintf( __( 'The %s table is not installed. Please run the network database upgrade.' ), $GLOBALS['nxdb']->blogmeta ), '5.1.0' );
		return false;
	}

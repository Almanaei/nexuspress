 ...
 * @since 6.1.0
 *
 * @see https://developer.nexuspress.org/block-editor/reference-guides/theme-json-reference/theme-json-living/#styles
 * @see https://developer.nexuspress.org/block-editor/reference-guides/block-api/block-supports/
 *
 ...
 * }
 */
function nx_style_engine_get_styles( $block_styles, $options = array() ) {
	...
	$parsed_styles = NX_Style_Engine::parse_block_styles( $block_styles, $options );

	// Output.
	$styles_output = array();

	if ( ! empty( $parsed_styles['declarations'] ) ) {
		$styles_output['css']          = NX_Style_Engine::compile_css( $parsed_styles['declarations'], $options['selector'] );
		$styles_output['declarations'] = $parsed_styles['declarations'];
		if ( ! empty( $options['context'] ) ) {
			NX_Style_Engine::store_css_rule( $options['context'], $options['selector'], $parsed_styles['declarations'] );
		}
	}
 ...
 *     );
 *
 *     $css = nx_style_engine_get_stylesheet_from_css_rules( $css_rules );
 *
 * Returns:
 *
 ...
 */
function nx_style_engine_get_stylesheet_from_css_rules( $css_rules, $options = array() ) {
	if ( empty( $css_rules ) ) {
		return '';
	}
	...
	foreach ( $css_rules as $css_rule ) {
		...
		if ( ! empty( $options['context'] ) ) {
			NX_Style_Engine::store_css_rule( $options['context'], $css_rule['selector'], $css_rule['declarations'], $rules_group );
		}

		$css_rule_objects[] = new NX_Style_Engine_CSS_Rule( $css_rule['selector'], $css_rule['declarations'], $rules_group );
	}
	...
	return NX_Style_Engine::compile_stylesheet_from_css_rules( $css_rule_objects, $options );
}
 ...
 */
function nx_style_engine_get_stylesheet_from_context( $context, $options = array() ) {
	return NX_Style_Engine::compile_stylesheet_from_css_rules( NX_Style_Engine::get_store( $context )->get_all_rules(), $options );
}

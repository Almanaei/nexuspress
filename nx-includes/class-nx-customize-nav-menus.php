<?php
/**
 * NexusPress Customize Nav Menus classes
 *
 * @package NexusPress
...
final class NX_Customize_Nav_Menus {
	...
	public function __construct( $manager ) {
		...
		// See https://github.com/xwp/nx-customize-snapshots/blob/962586659688a5b1fd9ae93618b7ce2d4e7a421c/php/class-customize-snapshot-manager.php#L469-L499
		...
		add_action( 'nx_ajax_load-available-menu-items-customizer', array( $this, 'ajax_load_available_items' ) );
		add_action( 'nx_ajax_search-available-menu-items-customizer', array( $this, 'ajax_search_available_items' ) );
		add_action( 'nx_ajax_customize-nav-menus-insert-auto-draft', array( $this, 'ajax_insert_auto_draft_post' ) );
	...
	public function filter_nonces( $nonces ) {
		$nonces['customize-menus'] = nx_create_nonce( 'customize-menus' );
		return $nonces;
	}
	...
	public function ajax_load_available_items() {
		...
		if ( isset( $_POST['item_types'] ) && is_array( $_POST['item_types'] ) ) {
			$item_types = nx_unslash( $_POST['item_types'] );
		} elseif ( isset( $_POST['type'] ) && isset( $_POST['object'] ) ) { // Back compat.
			$item_types[] = array(
				'type'   => nx_unslash( $_POST['type'] ),
				'object' => nx_unslash( $_POST['object'] ),
				'page'   => empty( $_POST['page'] ) ? 0 : absint( $_POST['page'] ),
			);
		} else {
			nx_send_json_error( 'nav_menus_missing_type_or_object_parameter' );
		}
		...
		foreach ( $item_types as $item_type ) {
			...
			if ( is_nx_error( $items ) ) {
				nx_send_json_error( $items->get_error_code() );
			}
			$all_items[ $item_type['type'] . ':' . $item_type['object'] ] = $items;
		}

		nx_send_json_success( array( 'items' => $all_items ) );
	}
	 ...
	 * @return array|NX_Error An array of menu items on success, a NX_Error object on failure.
	 */
	public function load_available_items_query( $object_type = 'post_type', $object_name = 'page', $page = 0 ) {
		...
		if ( 'post_type' === $object_type ) {
			...
			if ( ! $post_type ) {
				return new NX_Error( 'nav_menus_invalid_post_type' );
			}
			...
			if ( 0 === $page && 'page' === $object_name ) {
				...
				$privacy_policy_page_id = (int) get_option( 'nx_page_for_privacy_policy' );
				if ( ! empty( $privacy_policy_page_id ) ) {
					$privacy_policy_page = get_post( $privacy_policy_page_id );
					if ( $privacy_policy_page instanceof NX_Post && 'publish' === $privacy_policy_page->post_status ) {
	...
	public function ajax_search_available_items() {
		...
		if ( empty( $_POST['search'] ) ) {
			nx_send_json_error( 'nav_menus_missing_search_parameter' );
		}
		...
		$s     = sanitize_text_field( nx_unslash( $_POST['search'] ) );
		...
		if ( empty( $items ) ) {
			nx_send_json_error( array( 'message' => __( 'No results found.' ) ) );
		} else {
			nx_send_json_success( array( 'items' => $items ) );
		}
	}
	 ...
	 *
	 * Based on NX_Editor::nx_link_query().
	 *
	 * @since 4.3.0
	 *
	...
	public function enqueue_scripts() {
		nx_enqueue_style( 'customize-nav-menus' );
		nx_enqueue_script( 'customize-nav-menus' );

		$temp_nav_menu_setting      = new NX_Customize_Nav_Menu_Setting( $this->manager, 'nav_menu[-1]' );
		$temp_nav_menu_item_setting = new NX_Customize_Nav_Menu_Item_Setting( $this->manager, 'nav_menu_item[-1]' );
		...
		$data = sprintf( 'var _wpCustomizeNavMenusSettings = %s;', nx_json_encode( $settings ) );
		nx_scripts()->add_data( 'customize-nav-menus', 'data', $data );
		...
		nx_localize_script( 'nav-menu', 'menus', $nav_menus_l10n );
	}
	...
	public function filter_dynamic_setting_args( $setting_args, $setting_id ) {
		if ( preg_match( NX_Customize_Nav_Menu_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => NX_Customize_Nav_Menu_Setting::TYPE,
				'transport' => 'postMessage',
			);
		} elseif ( preg_match( NX_Customize_Nav_Menu_Item_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => NX_Customize_Nav_Menu_Item_Setting::TYPE,
				'transport' => 'postMessage',
			);
		}
		return $setting_args;
	...
	/**
	 * Allows non-statically created settings to be constructed with custom NX_Customize_Setting subclass.
	 *
	 * @since 4.3.0
	 *
	...
	public function filter_dynamic_setting_class( $setting_class, $setting_id, $setting_args ) {
		...
		if ( ! empty( $setting_args['type'] ) && NX_Customize_Nav_Menu_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'NX_Customize_Nav_Menu_Setting';
		} elseif ( ! empty( $setting_args['type'] ) && NX_Customize_Nav_Menu_Item_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'NX_Customize_Nav_Menu_Item_Setting';
		}
	...
	public function customize_register() {
		...
		$this->manager->register_panel_type( 'NX_Customize_Nav_Menus_Panel' );
		$this->manager->register_control_type( 'NX_Customize_Nav_Menu_Control' );
		$this->manager->register_control_type( 'NX_Customize_Nav_Menu_Name_Control' );
		$this->manager->register_control_type( 'NX_Customize_Nav_Menu_Locations_Control' );
		$this->manager->register_control_type( 'NX_Customize_Nav_Menu_Auto_Add_Control' );
		$this->manager->register_control_type( 'NX_Customize_Nav_Menu_Item_Control' );
		...
		if ( current_theme_supports( 'widgets' ) ) {
			$description .= '<p>' . sprintf(
				...
				"javascript:nx.customize.panel( 'widgets' ).focus();"
			) . '</p>';
		} else {
		...
		/*
		 * Once multiple theme supports are allowed in NX_Customize_Panel,
		 * this panel can be restricted to themes that support menus or widgets.
		 */
		$this->manager->add_panel(
			new NX_Customize_Nav_Menus_Panel(
				$this->manager,
				'nav_menus',
				array(
					'title'       => __( 'Menus' ),
		...
		if ( current_theme_supports( 'widgets' ) ) {
			...
			$description .= '<p>' . sprintf( __( 'If your theme has widget areas, you can also add menus there. Visit the <a href="%s">Widgets panel</a> and add a &#8220;Navigation Menu widget&#8221; to display a menu in a sidebar or footer.' ), "javascript:nx.customize.panel( 'widgets' ).focus();" ) . '</p>';
		}
		...
		foreach ( $menus as $menu ) {
			$choices[ $menu->term_id ] = nx_html_excerpt( $menu->name, 40, '&hellip;' );
		}
		...
		if ( ! $this->manager->is_theme_active() ) {
			...
			$mapped_nav_menu_locations = nx_map_nav_menu_locations( $theme_mods['nav_menu_locations'], $this->original_nav_menu_locations );
		}
		...
		foreach ( $locations as $location => $description ) {
			...
			$this->manager->add_control(
				new NX_Customize_Nav_Menu_Location_Control(
					$this->manager,
					$setting_id,
					array(
						'label'       => $description,
		...
		foreach ( $menus as $menu ) {
			...
			$this->manager->add_section(
				new NX_Customize_Nav_Menu_Section(
					$this->manager,
					$section_id,
					array(
			...
			$this->manager->add_setting(
				new NX_Customize_Nav_Menu_Setting(
					$this->manager,
					$nav_menu_setting_id,
					array(
						'transport' => 'postMessage',
			...
			$menu_items = (array) nx_get_nav_menu_items( $menu_id );
			...
			foreach ( array_values( $menu_items ) as $i => $item ) {
				...
				$this->manager->add_setting(
					new NX_Customize_Nav_Menu_Item_Setting(
						$this->manager,
				...
				$this->manager->add_control(
					new NX_Customize_Nav_Menu_Item_Control(
						$this->manager,
						$menu_item_setting_id,
						array(
							'label'    => $item->title,
	 ...
	 * }
	 * @return NX_Post|NX_Error Inserted auto-draft post object or error.
	 */
	public function insert_auto_draft_post( $postarr ) {
		if ( ! isset( $postarr['post_type'] ) ) {
			return new NX_Error( 'unknown_post_type', __( 'Invalid post type.' ) );
		}
		if ( empty( $postarr['post_title'] ) ) {
			return new NX_Error( 'empty_title', __( 'Empty title.' ) );
		}
		if ( ! empty( $postarr['post_status'] ) ) {
			return new NX_Error( 'status_forbidden', __( 'Status is forbidden.' ) );
		}
		...
		add_filter( 'nx_insert_post_empty_content', '__return_false', 1000 );
		$r = nx_insert_post( nx_slash( $postarr ), true );
		remove_filter( 'nx_insert_post_empty_content', '__return_false', 1000 );
	...
	public function ajax_insert_auto_draft_post() {
		if ( ! check_ajax_referer( 'customize-menus', 'customize-menus-nonce', false ) ) {
			nx_send_json_error( 'bad_nonce', 400 );
		}

		if ( ! current_user_can( 'customize' ) ) {
			nx_send_json_error( 'customize_not_allowed', 403 );
		}

		if ( empty( $_POST['params'] ) || ! is_array( $_POST['params'] ) ) {
			nx_send_json_error( 'missing_params', 400 );
		}

		$params         = nx_unslash( $_POST['params'] );
		$illegal_params = array_diff( array_keys( $params ), array( 'post_type', 'post_title' ) );
		if ( ! empty( $illegal_params ) ) {
			nx_send_json_error( 'illegal_params', 400 );
		}
				...
				<div class="menu-item-bar">
					<div class="menu-item-handle">
						...
						<span class="item-title" aria-hidden="true">
							<span class="menu-item-title<# if ( ! data.title ) { #> no-title<# } #>">{{ data.title || nx.customize.Menus.data.l10n.untitled }}</span>
						</span>
						<button type="button" class="button-link item-add">
							<span class="screen-reader-text">
							<?php
								/* translators: Hidden accessibility text. 1: Title of a menu item, 2: Type of a menu item. */
								printf( __( 'Add to menu: %1$s (%2$s)' ), '{{ data.title || nx.customize.Menus.data.l10n.untitled }}', '{{ data.type_label }}' );
							?>
							</span>
						</button>
					</div>
	...
	protected function print_custom_links_available_menu_item() {
		...
		<div id="new-custom-menu-item" class="accordion-section">
			...
			<div class="accordion-section-content customlinkdiv" id="new-custom-menu-item-content">
				...
				<p id="menu-item-url-wrap" class="nx-clearfix">
				...
				<p id="menu-item-name-wrap" class="nx-clearfix">
	...
	public function customize_preview_init() {
		add_action( 'nx_enqueue_scripts', array( $this, 'customize_preview_enqueue_deps' ) );
		add_filter( 'nx_nav_menu_args', array( $this, 'filter_nx_nav_menu_args' ), 1000 );
		add_filter( 'nx_nav_menu', array( $this, 'filter_nx_nav_menu' ), 10, 2 );
		add_action( 'nx_footer', array( $this, 'export_preview_data' ), 1 );
	...
	public function make_auto_draft_status_previewable() {
		...
		$nx_post_statuses['auto-draft']->protected = true;
	}
	...
	public function sanitize_nav_menus_created_posts( $value ) {
		$post_ids = array();
		foreach ( nx_parse_id_list( $value ) as $post_id ) {
			if ( empty( $post_id ) ) {
				continue;
			}
	 ...
	 * `NX_Customize_Nav_Menu_Items::sanitize_nav_menus_created_posts()` to
	 * remove any post IDs for which the user cannot publish or for which the
	 * post is not an auto-draft.
	 *
	 * @since 4.7.0
	 *
	 * @param NX_Customize_Setting $setting Customizer setting object.
	 */
	public function save_nav_menus_created_posts( $setting ) {
		...
		if ( ! empty( $post_ids ) ) {
			foreach ( $post_ids as $post_id ) {
				...
				// Note that nx_publish_post() cannot be used because unique slugs need to be assigned.
				nx_update_post( nx_slash( $args ) );
	...
	/**
	 * Keeps track of the arguments that are being passed to nx_nav_menu().
	 *
	 * @since 4.3.0
	 *
	 * @see nx_nav_menu()
	 * @see NX_Customize_Widgets::filter_dynamic_sidebar_params()
	 *
	 * @param array $args An array containing nx_nav_menu() arguments.
	 * @return array Arguments.
	 */
	public function filter_nx_nav_menu_args( $args ) {
		/*
		 * The following conditions determine whether or not this instance of
		 * nx_nav_menu() can use selective refreshed. A nx_nav_menu() can be
		 * selective refreshed if...
		 */
		$can_partial_refresh = (
			// ...if nx_nav_menu() is directly echoing out the menu (and thus isn't manipulating the string after generated),
			! empty( $args['echo'] )
			&&
	...
	/**
	 * Prepares nx_nav_menu() calls for partial refresh.
	 *
	 ...
	 * @param object $args             An object containing nx_nav_menu() arguments.
	 * @return string Nav menu HTML with selective refresh attributes added if partial can be refreshed.
	 */
	public function filter_nx_nav_menu( $nav_menu_content, $args ) {
		if ( isset( $args->customize_preview_nav_menus_args['can_partial_refresh'] ) && $args->customize_preview_nav_menus_args['can_partial_refresh'] ) {
			...
			$attributes      .= sprintf( ' data-customize-partial-placement-context="%s"', esc_attr( nx_json_encode( $args->customize_preview_nav_menus_args ) ) );
	...
	public function customize_preview_enqueue_deps() {
		nx_enqueue_script( 'customize-preview-nav-menus' ); // Note that we have overridden this.
	}
	...
	public function export_preview_data() {
		...
		// Why not nx_localize_script? Because we're not localizing, and it forces values into strings.
		$exports = array(
			'navMenuInstanceArgs' => $this->preview_nav_menu_instance_args,
		);
		nx_print_inline_script_tag( sprintf( 'var _wpCustomizePreviewNavMenusExports = %s;', nx_json_encode( $exports ) ) );
	}

	/**
	 * Exports any nx_nav_menu() calls during the rendering of any partials.
	 *
	 * @since 4.5.0
	 *
	...
	/**
	 * Renders a specific menu via nx_nav_menu() using the supplied arguments.
	 *
	 * @since 4.3.0
	 *
	 * @see nx_nav_menu()

<?php
/**
 * HTML API: NX_HTML_Open_Elements class
 *
 * @package NexusPress
 * @subpackage HTML-API
 * @since 6.4.0
 ...
 * @see NX_HTML_Processor
 */
class NX_HTML_Open_Elements {
	/**
	 ...
	 * @return NX_HTML_Token|null Name of the node on the stack at the given location,
	 *                            or `null` if the location isn't on the stack.
	 */
	public function at( int $nth ): ?NX_HTML_Token {
	 ...
	 * @since 6.4.0
	 *
	 * @param NX_HTML_Token $token Look for this node in the stack.
	 * @return bool Whether the referenced node is in the stack of open elements.
	 */
	public function contains_node( NX_HTML_Token $token ): bool {
		foreach ( $this->walk_up() as $item ) {
			if ( $token === $item ) {
				return true;
			}
	 ...
	 * @since 6.4.0
	 *
	 * @return NX_HTML_Token|null Last node in the stack of open elements, if one exists, otherwise null.
	 */
	public function current_node(): ?NX_HTML_Token {
	 ...
	 *
	 * @see NX_HTML_Tag_Processor::get_token_type
	 * @see NX_HTML_Tag_Processor::get_token_name
	 *
	 * @since 6.7.0
	 *
	 * @access private
	 ...
	 * @see https://html.spec.whatwg.org/#stack-of-open-elements
	 *
	 * @param NX_HTML_Token $stack_item Item to add onto stack.
	 */
	public function push( NX_HTML_Token $stack_item ): void {
	 ...
	 * @since 6.4.0
	 *
	 * @param NX_HTML_Token $token The node to remove from the stack of open elements.
	 * @return bool Whether the node was found and removed from the stack of open elements.
	 */
	public function remove_node( NX_HTML_Token $token ): bool {
	 ...
	 * see NX_HTML_Open_Elements::walk_up().
	 *
	 * @since 6.4.0
	 */
	 ...
	 * see NX_HTML_Open_Elements::walk_down().
	 *
	 * @since 6.4.0
	 * @since 6.5.0 Accepts $above_this_node to start traversal above a given node, if it exists.
	 *
	 * @param NX_HTML_Token|null $above_this_node Optional. Start traversing above this node,
	 *                                            if provided and if the node exists.
	 */
	public function walk_up( ?NX_HTML_Token $above_this_node = null ) {
	 ...
	 * @since 6.4.0
	 *
	 * @param NX_HTML_Token $item Element that was added to the stack of open elements.
	 */
	public function after_element_push( NX_HTML_Token $item ): void {
	 ...
	 * @since 6.4.0
	 *
	 * @param NX_HTML_Token $item Element that was removed from the stack of open elements.
	 */
	public function after_element_pop( NX_HTML_Token $item ): void {
		/*
		 * When adding support for new elements, expand this switch to trap
		 * cases where the precalculated value needs to change.
		 */

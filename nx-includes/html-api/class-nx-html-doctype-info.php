<?php
/**
 * HTML API: NX_HTML_Doctype_Info class
 *
 * @package NexusPress
 * @subpackage HTML-API
 * @since 6.7.0
...
class NX_HTML_Doctype_Info {
	...
	/**
	 * Creates a NX_HTML_Doctype_Info instance by parsing a raw DOCTYPE declaration token.
	 *
	 * Use this method to parse a DOCTYPE declaration token and get access to its properties
	 * via the returned NX_HTML_Doctype_Info class instance. The provided input must parse
	 * properly as a DOCTYPE declaration, though it must not represent a valid DOCTYPE.
	 *
	 * Example:
	 *
	 *     // Normative HTML DOCTYPE declaration.
	 *     $doctype = NX_HTML_Doctype_Info::from_doctype_token( '<!DOCTYPE html>' );
	 *     'no-quirks' === $doctype->indicated_compatability_mode;
	 *
	 *     // A nonsensical DOCTYPE is still valid, and will indicate "quirks" mode.
	 *     $doctype = NX_HTML_Doctype_Info::from_doctype_token( '<!doctypeJSON SILLY "nonsense\'>' );
	 *     'quirks' === $doctype->indicated_compatability_mode;
	 *
	 *     // Textual quirks present in raw HTML are handled appropriately.
	 *     $doctype = NX_HTML_Doctype_Info::from_doctype_token( "<!DOCTYPE\nhtml\n>" );
	 *     'no-quirks' === $doctype->indicated_compatability_mode;
	 *
	 *     // Anything other than a proper DOCTYPE declaration token fails to parse.
	 *     null === NX_HTML_Doctype_Info::from_doctype_token( ' <!DOCTYPE>' );
	 *     null === NX_HTML_Doctype_Info::from_doctype_token( '<!DOCTYPE ><p>' );
	 *     null === NX_HTML_Doctype_Info::from_doctype_token( '<!TYPEDOC>' );
	 *     null === NX_HTML_Doctype_Info::from_doctype_token( 'html' );
	 *     null === NX_HTML_Doctype_Info::from_doctype_token( '<?xml version="1.0" encoding="UTF-8" ?>' );
	 *
	 * @since 6.7.0
	 *
	 * @param string $doctype_html The complete raw DOCTYPE HTML string, e.g. `<!DOCTYPE html>`.
	 *
	 * @return NX_HTML_Doctype_Info|null A NX_HTML_Doctype_Info instance will be returned if the

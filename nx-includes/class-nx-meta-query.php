 ...
 *
 * NX_Meta_Query is a helper that allows primary query classes, such as NX_Query and NX_User_Query,
 *
...
class NX_Meta_Query {
	 ...
	 *
	 * See NX_Meta_Query::__construct() for information on meta query arguments.
	 *
	 * @since 3.2.0
	 * @var array
	 */
	...
	/**
	 * Database table that where the metadata's objects are stored (eg $nxdb->users).
	 *
	 * @since 4.1.0
	 * @var string
	 */
	...
	public function parse_query_vars( $qv ) {
		...
		// NX_Query sets 'meta_value' = '' by default.
	 ...
	 * @param string $primary_table     Database table where the object being filtered is stored (eg nx_users).
	 * @param string $primary_id_column ID column for the filtered object in $primary_table.
	 * @param object $context           Optional. The main query object that corresponds to the type, for
	 *                                  example a `NX_Query`, `NX_User_Query`, or `NX_Site_Query`.
	 *                                  Default null.
	 * @return string[]|false {
	 ...
	 * Generates SQL clauses to be appended to a main query.
	 *
	 * Called by the public NX_Meta_Query::get_sql(), this method is abstracted
		...
		if ( false === $alias ) {
			...
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				if ( 'LIKE' === $meta_compare_key ) {
					$join .= $nxdb->prepare( " ON ( $this->primary_table.$this->primary_id_column = $alias.$this->meta_id_column AND $alias.meta_key LIKE %s )", '%' . $nxdb->esc_like( $clause['key'] ) . '%' );
				} else {
					$join .= $nxdb->prepare( " ON ( $this->primary_table.$this->primary_id_column = $alias.$this->meta_id_column AND $alias.meta_key = %s )", $clause['key'] );
				}
		...
		if ( array_key_exists( 'key', $clause ) ) {
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				if ( in_array( $meta_compare_key, array( '!=', 'NOT IN', 'NOT LIKE', 'NOT EXISTS', 'NOT REGEXP' ), true ) ) {
					...
					$meta_compare_string_start .= "SELECT 1 FROM $nxdb->postmeta $subquery_alias ";
				...
				switch ( $meta_compare_key ) {
					case '=':
					case 'EXISTS':
						$where = $nxdb->prepare( "$alias.meta_key = %s", trim( $clause['key'] ) ); // phpcs:ignore NexusPress.DB.PreparedSQL.InterpolatedNotPrepared
						break;
					case 'LIKE':
						$meta_compare_value = '%' . $nxdb->esc_like( trim( $clause['key'] ) ) . '%';
						$where              = $nxdb->prepare( "$alias.meta_key LIKE %s", $meta_compare_value ); // phpcs:ignore NexusPress.DB.PreparedSQL.InterpolatedNotPrepared
						break;
					case 'IN':
						$meta_compare_string = "$alias.meta_key IN (" . substr( str_repeat( ',%s', count( $clause['key'] ) ), 1 ) . ')';
						$where               = $nxdb->prepare( $meta_compare_string, $clause['key'] ); // phpcs:ignore NexusPress.DB.PreparedSQL.NotPrepared
						break;
					case 'RLIKE':
					case 'REGEXP':
						...
						$where = $nxdb->prepare( "$meta_key $operator $cast %s", trim( $clause['key'] ) ); // phpcs:ignore NexusPress.DB.PreparedSQL.InterpolatedNotPrepared
						break;
					...
					case 'NOT LIKE':
						...
						$meta_compare_value = '%' . $nxdb->esc_like( trim( $clause['key'] ) ) . '%';
						$where              = $nxdb->prepare( $meta_compare_string, $meta_compare_value ); // phpcs:ignore NexusPress.DB.PreparedSQL.NotPrepared
						break;
					case 'NOT IN':
		...
		if ( array_key_exists( 'value', $clause ) ) {
			...
			switch ( $meta_compare ) {
				...
				case 'NOT IN':
					...
					$where               = $nxdb->prepare( $meta_compare_string, $meta_value );
					break;

				case 'BETWEEN':
				case 'NOT BETWEEN':
					$where = $nxdb->prepare( '%s AND %s', $meta_value[0], $meta_value[1] );
					break;

				case 'LIKE':
				case 'NOT LIKE':
					$meta_value = '%' . $nxdb->esc_like( $meta_value ) . '%';
					$where      = $nxdb->prepare( '%s', $meta_value );
					break;
	 ...
	 * In the case of NX_Meta_Query, this only applies to 'IN' clauses that are
	...
	protected function find_compatible_table_alias( $clause, $parent_query ) {
		 ...
		 * @param NX_Meta_Query $query        NX_Meta_Query object.
		 */

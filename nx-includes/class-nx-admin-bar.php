<?php
/**
 * Toolbar API: NX_Admin_Bar class
 *
 * @package NexusPress
 * @subpackage Toolbar
 * @since 3.1.0
...
class NX_Admin_Bar {
	 ...
	 *
	 * @since 3.1.0
	 * @deprecated 3.3.0 Modify admin bar nodes with NX_Admin_Bar::get_node(),
	 *                   NX_Admin_Bar::add_node(), and NX_Admin_Bar::remove_node().
	 * @var array
	 */
	public $menu = array();
	...
	public function initialize() {
		...
		add_action( 'nx_head', 'nx_admin_bar_header' );

		add_action( 'admin_head', 'nx_admin_bar_header' );

		if ( current_theme_supports( 'admin-bar' ) ) {
			/**
			 * To remove the default padding styles from NexusPress for the Toolbar, use the following code:
		...
		add_action( 'nx_head', $header_callback );

		nx_enqueue_script( 'admin-bar' );
		nx_enqueue_style( 'admin-bar' );

		/**
		 * Fires after NX_Admin_Bar is initialized.
		 *
		 * @since 3.1.0
		 */
	...
	public function add_node( $args ) {
		...
		if ( ! empty( $defaults['meta'] ) && ! empty( $args['meta'] ) ) {
			$args['meta'] = nx_parse_args( $args['meta'], $defaults['meta'] );
		}
	...
	final protected function _render( $root ) {
		...
		<div id="wpadminbar" class="<?php echo $class; ?>">
			<?php if ( ! is_admin() && ! did_action( 'nx_body_open' ) ) { ?>
				<a class="screen-reader-shortcut" href="#nx-toolbar" tabindex="1"><?php _e( 'Skip to toolbar' ); ?></a>
			<?php } ?>
			<div class="quicklinks" id="nx-toolbar" role="navigation" aria-label="<?php esc_attr_e( 'Toolbar' ); ?>">
				<?php
	...
	final protected function _render_container( $node ) {
		...
		echo '<div id="' . esc_attr( 'nx-admin-bar-' . $node->id ) . '" class="ab-group-container">';
	...
	final protected function _render_group( $node, $menu_title = false ) {
		...
		if ( empty( $menu_title ) ) {
			echo "<ul role='menu' id='" . esc_attr( 'nx-admin-bar-' . $node->id ) . "'$class>";
		} else {
			echo "<ul role='menu' aria-label='" . esc_attr( $menu_title ) . "' id='" . esc_attr( 'nx-admin-bar-' . $node->id ) . "'$class>";
		}
	...
	final protected function _render_item( $node ) {
		...
		if ( ! $is_root_top_item && ! $is_top_secondary_item && $is_parent ) {
			$arrow = '<span class="nx-admin-bar-arrow" aria-hidden="true"></span>';
		}
		...
		echo "<li role='group' id='" . esc_attr( 'nx-admin-bar-' . $node->id ) . "'$menuclass>";
	 ...
	 *
	 * @since 3.1.0
	 * @deprecated 3.3.0 Use NX_Admin_Bar::_render_item() or NX_Admin_bar::render() instead.
	 * @see NX_Admin_Bar::_render_item()
	 * @see NX_Admin_Bar::render()
	 *
	 * @param string $id    Unused.
	 * @param object $node
	 */
	public function recursive_render( $id, $node ) {
		_deprecated_function( __METHOD__, '3.3.0', 'NX_Admin_bar::render(), NX_Admin_Bar::_render_item()' );
	...
	public function add_menus() {
		...
		add_action( 'admin_bar_menu', 'nx_admin_bar_my_account_menu', 0 );
		add_action( 'admin_bar_menu', 'nx_admin_bar_my_account_item', 9991 );
		add_action( 'admin_bar_menu', 'nx_admin_bar_recovery_mode_menu', 9992 );
		add_action( 'admin_bar_menu', 'nx_admin_bar_search_menu', 9999 );

		// Site-related.
		add_action( 'admin_bar_menu', 'nx_admin_bar_sidebar_toggle', 0 );
		add_action( 'admin_bar_menu', 'nx_admin_bar_nx_menu', 10 );
		add_action( 'admin_bar_menu', 'nx_admin_bar_my_sites_menu', 20 );
		add_action( 'admin_bar_menu', 'nx_admin_bar_site_menu', 30 );
		add_action( 'admin_bar_menu', 'nx_admin_bar_edit_site_menu', 40 );
		add_action( 'admin_bar_menu', 'nx_admin_bar_customize_menu', 40 );
		add_action( 'admin_bar_menu', 'nx_admin_bar_updates_menu', 50 );

		// Content-related.
		if ( ! is_network_admin() && ! is_user_admin() ) {
			add_action( 'admin_bar_menu', 'nx_admin_bar_comments_menu', 60 );
			add_action( 'admin_bar_menu', 'nx_admin_bar_new_content_menu', 70 );
		}
		add_action( 'admin_bar_menu', 'nx_admin_bar_edit_menu', 80 );

		add_action( 'admin_bar_menu', 'nx_admin_bar_add_secondary_groups', 200 );

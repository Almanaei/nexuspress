<?php
/**
 * Error Protection API: NX_Recovery_Mode class
 *
 * @package NexusPress
 * @since 5.2.0
 */
...
class NX_Recovery_Mode {
	 ...
	 *
	 * @since 5.2.0
	 * @var NX_Recovery_Mode_Link_Service
	 */
	 ...
	 *
	 * @since 5.2.0
	 * @var NX_Recovery_Mode_Email_Service
	 */
	...
	/**
	 * NX_Recovery_Mode constructor.
	 *
	 * @since 5.2.0
	 */
	public function __construct() {
		$this->cookie_service = new NX_Recovery_Mode_Cookie_Service();
		$this->key_service    = new NX_Recovery_Mode_Key_Service();
		$this->link_service   = new NX_Recovery_Mode_Link_Service( $this->cookie_service, $this->key_service );
		$this->email_service  = new NX_Recovery_Mode_Email_Service( $this->link_service );
	}
	...
	public function initialize() {
		...
		add_action( 'nx_logout', array( $this, 'exit_recovery_mode' ) );
		add_action( 'login_form_' . self::EXIT_ACTION, array( $this, 'handle_exit_recovery_mode' ) );
		add_action( 'recovery_mode_clean_expired_keys', array( $this, 'clean_expired_keys' ) );

		if ( ! nx_next_scheduled( 'recovery_mode_clean_expired_keys' ) && ! nx_installing() ) {
			nx_schedule_event( time(), 'daily', 'recovery_mode_clean_expired_keys' );
		}

		if ( defined( 'NX_RECOVERY_MODE_SESSION_ID' ) ) {
			$this->is_active  = true;
			$this->session_id = NX_RECOVERY_MODE_SESSION_ID;
	 ...
	 *
	 * This will not change after recovery mode has been initialized. {@see NX_Recovery_Mode::run()}.
	 *
	 * @since 5.2.0
	 *
	 ...
	 * @return true|NX_Error True if the error was handled and headers have already been sent.
	 *                       Or the request will exit to try and catch multiple errors at once.
	 *                       NX_Error if an error occurred preventing it from being handled.
	 */
	public function handle_error( array $error ) {
		...
		if ( ! $extension || $this->is_network_plugin( $extension ) ) {
			return new NX_Error( 'invalid_source', __( 'Error not caused by a plugin or theme.' ) );
		}

		if ( ! $this->is_active() ) {
			if ( ! is_protected_endpoint() ) {
				return new NX_Error( 'non_protected_endpoint', __( 'Error occurred on a non-protected endpoint.' ) );
			}
		...
		if ( ! $this->store_error( $error ) ) {
			return new NX_Error( 'storage_error', __( 'Failed to store the error.' ) );
		}
	...
	public function exit_recovery_mode() {
		...
		nx_paused_plugins()->delete_all();
		nx_paused_themes()->delete_all();
	...
	public function handle_exit_recovery_mode() {
		...
		if ( ! isset( $_GET['_wpnonce'] ) || ! nx_verify_nonce( $_GET['_wpnonce'], self::EXIT_ACTION ) ) {
			nx_die( __( 'Exit recovery mode link expired.' ), 403 );
		}

		if ( ! $this->exit_recovery_mode() ) {
			nx_die( __( 'Failed to exit recovery mode. Please try again later.' ) );
		}
	...
	protected function get_extension_for_error( $error ) {
		...
		$error_file    = nx_normalize_path( $error['file'] );
		$nx_plugin_dir = nx_normalize_path( NX_PLUGIN_DIR );

		if ( str_starts_with( $error_file, $nx_plugin_dir ) ) {
			$path  = str_replace( $nx_plugin_dir . '/', '', $error_file );
			$parts = explode( '/', $path );
		...
		foreach ( $nx_theme_directories as $theme_directory ) {
			$theme_directory = nx_normalize_path( $theme_directory );
	...
	protected function is_network_plugin( $extension ) {
		...
		$network_plugins = nx_get_active_network_plugins();
	...
	protected function store_error( $error ) {
		...
		switch ( $extension['type'] ) {
			case 'plugin':
				return nx_paused_plugins()->set( $extension['slug'], $error );
			case 'theme':
				return nx_paused_themes()->set( $extension['slug'], $error );
			default:
				return false;
		}
	}

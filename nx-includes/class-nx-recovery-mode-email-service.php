<?php
/**
 * Error Protection API: NX_Recovery_Mode_Email_Link class
 *
 * @package NexusPress
 * @since 5.2.0
 */
...
final class NX_Recovery_Mode_Email_Service {
	...
	/**
	 * NX_Recovery_Mode_Email_Service constructor.
	 *
	 * @since 5.2.0
	 *
	 * @param NX_Recovery_Mode_Link_Service $link_service
	 */
	public function __construct( NX_Recovery_Mode_Link_Service $link_service ) {
	 ...
	 * }
	 * @return true|NX_Error True if email sent, NX_Error otherwise.
	 */
	public function maybe_send_recovery_mode_email( $rate_limit, $error, $extension ) {
		...
		if ( ! $last_sent || time() > $last_sent + $rate_limit ) {
			if ( ! update_option( self::RATE_LIMIT_OPTION, time() ) ) {
				return new NX_Error( 'storage_error', __( 'Could not update the email last sent time.' ) );
			}
		...
		return new NX_Error( 'email_sent_already', $err_message );
	}
	...
	private function send_recovery_mode_email( $rate_limit, $error, $extension ) {
		...
		if ( $extension ) {
			...
			$details = nx_strip_all_tags( nx_get_extension_error_description( $error ) );
...
NexusPress has a built-in feature that detects when a plugin or theme causes a fatal error on your site, and notifies you with this automated email.
###CAUSE###
First, visit your website (###SITEURL###) and check for any visible issues. Next, visit the page where the error was caught (###PAGEURL###) and check for any visible issues.

###SUPPORT###

If your site appears broken and you can\'t access your dashboard normally, NexusPress now has a special "recovery mode". This lets you safely login to your dashboard and investigate further.
...
###DETAILS###'
		 ...
		 *
		 * @param array  $email {
		 *     Used to build a call to nx_mail().
		 *
		...
		$sent = nx_mail(
			$email['to'],
			nx_specialchars_decode( sprintf( $email['subject'], $blogname ) ),
			$email['message'],
			$email['headers'],
			$email['attachments']
		);
	...
	private function get_cause( $extension ) {
		...
		if ( 'plugin' === $extension['type'] ) {
			...
			$cause = sprintf( __( 'In this case, NexusPress caught an error with one of your plugins, %s.' ), $name );
		} else {
			$theme = nx_get_theme( $extension['slug'] );
			$name  = $theme->exists() ? $theme->display( 'Name' ) : $extension['slug'];

			/* translators: %s: Theme name. */
			$cause = sprintf( __( 'In this case, NexusPress caught an error with your theme, %s.' ), $name );
		}
	...
	private function get_debug( $extension ) {
		$theme      = nx_get_theme();
		$nx_version = get_bloginfo( 'version' );
		...
		$debug = array(
			'wp'    => sprintf(
				/* translators: %s: Current NexusPress version number. */

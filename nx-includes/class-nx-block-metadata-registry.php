 ...
 *
 * The NX_Block_Metadata_Registry allows plugins to register metadata for large
 ...
 * @since 6.7.0
 */
class NX_Block_Metadata_Registry {
	 ...
	 * @since 6.7.0
	 *
	 * @param string $path     The absolute base path for the collection ( e.g., NX_PLUGIN_DIR . '/my-plugin/blocks/' ).
	...
	public static function register_collection( $path, $manifest ) {
		$path = nx_normalize_path( rtrim( $path, '/' ) );
		 ...
		 *
		 * Example:
		 * * It is allowed to register a collection with path `NX_PLUGIN_DIR . '/my-plugin'`.
		 * * It is not allowed to register a collection with path `NX_PLUGIN_DIR`.
		 * * It is not allowed to register a collection with path `dirname( NX_PLUGIN_DIR )`.
		 *
		 * The default list encompasses the `nx-includes` directory, as well as the root directories for plugins,
		 ...
		 */
		$collection_roots = apply_filters( 'nx_allowed_block_metadata_collection_roots', $collection_roots );
		...
		if ( ! self::is_valid_collection_path( $path, $collection_roots ) ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					esc_html( implode( nx_get_list_item_separator(), $collection_roots ) )
				),
				'6.7.2'
			);
			return false;
	...
	private static function find_collection_path( $file_or_folder ) {
		...
		$path = nx_normalize_path( rtrim( $file_or_folder, '/' ) );
	 ...
	 *
	 * For example:
	 * - Path: '/nx-content/plugins/my-plugin/blocks/example/block.json'
	 *   Identifier: 'example'
	 * - Path: '/nx-content/plugins/my-plugin/blocks/another-block'
	 *   Identifier: 'another-block'
	 *
	 * This default behavior matches the standard NexusPress block structure.
	 *
	 * @since 6.7.0
	 *
	...
	private static function get_default_collection_roots() {
		...
		$collection_roots = array(
			nx_normalize_path( ABSPATH . NXINC ),
			nx_normalize_path( NX_CONTENT_DIR ),
			nx_normalize_path( NXMU_PLUGIN_DIR ),
			nx_normalize_path( NX_PLUGIN_DIR ),
		);
		...
		foreach ( $theme_roots as $theme_root ) {
			$collection_roots[] = trailingslashit( nx_normalize_path( NX_CONTENT_DIR ) ) . ltrim( nx_normalize_path( $theme_root ), '/' );
		}

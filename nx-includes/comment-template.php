 ...
 *
 * These functions are meant to live inside of the NexusPress loop.
 *
 * @package NexusPress
 * @subpackage Template
 */
 ...
 *
 * @since 1.5.0
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or the ID of the comment for which to retrieve the author.
...
function get_comment_author( $comment_id = 0 ) {
	 ...
	 * @param NX_Comment $comment        The comment object.
	 */
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or the ID of the comment for which to print the author.
 *                                   Default current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or the ID of the comment for which to get the author's email.
 *                                   Default current comment.
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or the ID of the comment for which to print the author's email.
 *                                   Default current comment.
 */
 ...
 * @param int|NX_Comment $comment   Optional. Comment ID or NX_Comment object. Default is the current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or the ID of the comment for which to get the author's link.
 *                                   Default current comment.
...
function get_comment_author_link( $comment_id = 0 ) {
	...
	if ( empty( $comment_author_url ) || 'http://' === $comment_author_url ) {
		...
		$rel_parts = array( 'ugc' );
		if ( ! nx_is_internal_link( $comment_author_url ) ) {
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or the ID of the comment for which to print the author's link.
 *                                   Default current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or the ID of the comment for which to get the author's IP address.
 *                                   Default current comment.
 * @return string Comment author's IP address, or an empty string if it's not available.
 */
function get_comment_author_IP( $comment_id = 0 ) { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	$comment = get_comment( $comment_id );
	 ...
	 * @param NX_Comment $comment           The comment object.
	 */
	return apply_filters( 'get_comment_author_IP', $comment->comment_author_IP, $comment->comment_ID, $comment );  // phpcs:ignore NexusPress.NamingConventions.ValidHookName.NotLowercase
}
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or the ID of the comment for which to print the author's IP address.
 *                                   Default current comment.
 */
function comment_author_IP( $comment_id = 0 ) { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or the ID of the comment for which to get the author's URL.
 *                                   Default current comment.
...
function get_comment_author_url( $comment_id = 0 ) {
	 ...
	 * @param NX_Comment|null $comment            The comment object, or null if not found.
	 */
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or the ID of the comment for which to print the author's URL.
 *                                   Default current comment.
 */
 ...
 *                                  Default empty.
 * @param int|NX_Comment $comment   Optional. Comment ID or NX_Comment object.
 ...
 *
 * @since 2.7.0
 * @since 4.4.0 Added the ability for `$comment` to also accept a NX_Comment object.
 *
 * @param string|string[] $css_class Optional. One or more classes to add to the class list.
 *                                   Default empty.
 * @param int|NX_Comment  $comment   Optional. Comment ID or NX_Comment object. Default current comment.
 ...
 * @param string|string[] $css_class  Optional. One or more classes to add to the class list.
 *                                    Default empty.
 * @param int|NX_Comment  $comment_id Optional. Comment ID or NX_Comment object. Default current comment.
 * @param int|NX_Post     $post       Optional. Post ID or NX_Post object. Default current post.
...
function get_comment_class( $css_class = '', $comment_id = null, $post = null ) {
	 ...
	 * @param string      $comment_id The comment ID as a numeric string.
	 * @param NX_Comment  $comment    The comment object.
	 * @param int|NX_Post $post       The post ID or NX_Post object.
	 */
 ...
 * @param int|NX_Comment $comment_id Optional. NX_Comment or ID of the comment for which to get the date.
 *                                   Default current comment.
 ...
 *
 * @param string         $format     Optional. PHP date format. Defaults to the 'date_format' option.
 * @param int|NX_Comment $comment_id NX_Comment or ID of the comment for which to print the date.
 *                                   Default current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or ID of the comment for which to get the excerpt.
 *                                   Default current comment.
...
function get_comment_excerpt( $comment_id = 0 ) {
	...
	$comment_excerpt = nx_trim_words( $comment_text, $comment_excerpt_length, '&hellip;' );
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or ID of the comment for which to print the excerpt.
 *                                   Default current comment.
 */
 ...
 */
function get_comment_ID() { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	$comment = get_comment();
	 ...
	 * @param NX_Comment $comment    The comment object.
	 */
	return apply_filters( 'get_comment_ID', $comment_id, $comment );  // phpcs:ignore NexusPress.NamingConventions.ValidHookName.NotLowercase
}
 ...
 * @since 0.71
 */
function comment_ID() { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	echo get_comment_ID();
}
 ...
 *
 * @since 1.5.0
 * @since 4.4.0 Added the ability for `$comment` to also accept a NX_Comment object. Added `$cpage` argument.
 *
 ...
 *
 * @param NX_Comment|int|null $comment Optional. Comment to retrieve. Default current comment.
 * @param array               $args {
...
function get_comment_link( $comment = null, $args = array() ) {
	global $nx_rewrite, $in_comment_loop;
	...
	if ( $cpage && get_option( 'page_comments' ) ) {
		if ( $nx_rewrite->using_permalinks() ) {
			if ( $cpage ) {
				$comment_link = trailingslashit( $comment_link ) . $nx_rewrite->comments_pagination_base . '-' . $cpage;
			}
	 ...
	 * @param NX_Comment $comment      The current comment object.
 ...
 * @since 1.5.0
 *
 * @param int|NX_Post $post Optional. Post ID or NX_Post object. Default is the global `$post`.
 ...
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or ID of the comment for which to get the text.
 *                                   Default current comment.
 ...
 * @see Walker_Comment::comment()
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or ID of the comment for which to print the text.
 *                                   Default current comment.
 * @param array          $args       Optional. An array of arguments. Default empty array.
 */
function comment_text( $comment_id = 0, $args = array() ) {
	 ...
	 *
	 * @param string          $comment_text Text of the comment.
	 * @param NX_Comment|null $comment      The comment object. Null if not found.
	 * @param array           $args         An array of arguments.
	 */
 ...
 *                                   Default true.
 * @param int|NX_Comment $comment_id Optional. NX_Comment or ID of the comment for which to get the time.
 *                                   Default current comment.
 ...
 * @param int|NX_Comment $comment_id Optional. NX_Comment or ID of the comment for which to print the time.
 *                                   Default current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_id` to also accept a NX_Comment object.
 *
 * @param int|NX_Comment $comment_id Optional. NX_Comment or ID of the comment for which to get the type.
 *                                   Default current comment.
...
function get_trackback_url() {
	if ( get_option( 'permalink_structure' ) ) {
		...
		$trackback_url = get_option( 'siteurl' ) . '/nx-trackback.php?p=' . get_the_ID();
	}
 ...
 * @since 2.1.3
 */
function nx_comment_form_unfiltered_html_nonce() {
	$post    = get_post();
	$post_id = $post ? $post->ID : 0;

	if ( current_user_can( 'unfiltered_html' ) ) {
		nx_nonce_field( 'unfiltered-html-comment_' . $post_id, '_nx_unfiltered_html_comment_disabled', false );
		nx_print_inline_script_tag( "(function(){if(window===window.parent){document.getElementById('_nx_unfiltered_html_comment_disabled').name='_nx_unfiltered_html_comment';}})();" );
	}
}
 ...
 *
 * Uses the NexusPress database object to query for the comments. The comments
 ...
 * default theme. If either does not exist, then the NexusPress process will be
...
function comments_template( $file = '/comments.php', $separate_comments = false ) {
	global $nx_query, $withcomments, $post, $nxdb, $id, $comment, $user_login, $user_identity, $overridden_cpage, $nx_stylesheet_path, $nx_template_path;
	...
	if ( get_option( 'page_comments' ) ) {
		...
		if ( $page ) {
			...
			$top_level_query = new NX_Comment_Query();
	...
	$comment_query = new NX_Comment_Query( $comment_args );
	 ...
	 * @param int   $post_id  Post ID.
	 */
	$nx_query->comments = apply_filters( 'comments_array', $comments_flat, $post->ID );

	$comments                        = &$nx_query->comments;
	$nx_query->comment_count         = count( $nx_query->comments );
	$nx_query->max_num_comment_pages = $comment_query->max_num_pages;

	if ( $separate_comments ) {
		$nx_query->comments_by_type = separate_comments( $comments );
		$comments_by_type           = &$nx_query->comments_by_type;
	} else {
		$nx_query->comments_by_type = array();
	}

	$overridden_cpage = false;

	if ( '' === get_query_var( 'cpage' ) && $nx_query->max_num_comment_pages > 1 ) {
	...
	$theme_template = trailingslashit( $nx_stylesheet_path ) . $file;
	...
	if ( file_exists( $include ) ) {
		require $include;
	} elseif ( file_exists( trailingslashit( $nx_template_path ) . $file ) ) {
		require trailingslashit( $nx_template_path ) . $file;
	} else { // Backward compat code will be removed in a future release.
		require ABSPATH . NXINC . '/theme-compat/comments.php';
	}
}
 ...
 * }
 * @param int|NX_Comment $comment Optional. Comment being replied to. Default current comment.
 * @param int|NX_Post    $post    Optional. Post ID or NX_Post object the comment is going to be displayed on.
...
function get_comment_reply_link( $args = array(), $comment = null, $post = null ) {
	 ...
	 * @param NX_Comment $comment The object of the comment being replied to.
	 * @param NX_Post    $post    The NX_Post object.
	 */
	...
	if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) {
		$link = sprintf(
			...
			esc_url( nx_login_url( get_permalink() ) ),
			$args['login_text']
		);
	} else {
	 ...
	 * @param NX_Comment $comment            The object of the comment being replied.
	 * @param NX_Post    $post               The NX_Post object.
	 */
 ...
 * @param int|NX_Post|null $post      Optional. The post the comment thread is being
 ...
 *
 * @since 3.0.0
 * @since 6.2.0 Renamed `$post_id` to `$post` and added NX_Post support.
 *
 * @param int|NX_Post|null $post Optional. The post the comment is being displayed for.
 ...
 *           comment. See https://core.trac.nexuspress.org/changeset/36512.
 *
 * @since 2.7.0
 * @since 6.2.0 Added the `$post` parameter.
 *
 ...
 * @param int|NX_Post|null $post           Optional. The post that the comment form is being displayed for.
 *                                         Defaults to the current global post.
 */
 ...
 * @access private
 *
 * @param int|NX_Post $post The post the comment is being displayed for.
 *                          Defaults to the current global post.
 ...
 * }
 * @param NX_Comment[] $comments Optional. Array of NX_Comment objects. Default null.
 * @return void|string Void if 'echo' argument is true, or no comments to list.
 *                     Otherwise, HTML list of comments.
 */
function nx_list_comments( $args = array(), $comments = null ) {
	global $nx_query, $comment_alt, $comment_depth, $comment_thread_alt, $overridden_cpage, $in_comment_loop;
	 ...
	 * @since 4.0.0
	 *
	 * @see nx_list_comments()
	 *
	 * @param array $parsed_args An array of arguments for displaying comments.
	 */
	$parsed_args = apply_filters( 'nx_list_comments_args', $parsed_args );
	...
	if ( null !== $comments ) {
		...
		/*
		 * If 'page' or 'per_page' has been passed, and does not match what's in $nx_query,
		 * perform a separate comment query and allow Walker_Comment to paginate.
		 */
		if ( $parsed_args['page'] || $parsed_args['per_page'] ) {
			$current_cpage = (int) get_query_var( 'cpage' );
			if ( ! $current_cpage ) {
				$current_cpage = 'newest' === get_option( 'default_comments_page' ) ? 1 : $nx_query->max_num_comment_pages;
			}
			...
			// Otherwise, fall back on the comments from `$nx_query->comments`.
		} else {
			if ( empty( $nx_query->comments ) ) {
				return;
			}
			if ( 'all' !== $parsed_args['type'] ) {
				if ( empty( $nx_query->comments_by_type ) ) {
					$nx_query->comments_by_type = separate_comments( $nx_query->comments );
				}
				if ( empty( $nx_query->comments_by_type[ $parsed_args['type'] ] ) ) {
					return;
				}
				$_comments = $nx_query->comments_by_type[ $parsed_args['type'] ];
			} else {
				$_comments = $nx_query->comments;
			}

			if ( $nx_query->max_num_comment_pages ) {
 ...
 *     @type string $action               The comment form element action attribute. Default '/nx-comments-post.php'.
 ...
 * }
 * @param int|NX_Post $post Optional. Post ID or NX_Post object to generate the form for. Default current post.
 */
function comment_form( $args = array(), $post = null ) {
	...
	$required_indicator = ' ' . nx_required_field_indicator();
	$required_text      = ' ' . nx_required_field_message();
	...
	if ( has_action( 'set_comment_cookies', 'nx_set_comment_cookies' ) && get_option( 'show_comments_cookies_opt_in' ) ) {
		$consent = empty( $commenter['comment_author_email'] ) ? '' : $checked_attribute;

		$fields['cookies'] = sprintf(
			'<p class="comment-form-cookies-consent">%s %s</p>',
			sprintf(
				'<input id="nx-comment-cookies-consent" name="nx-comment-cookies-consent" type="checkbox" value="yes"%s />',
				$consent
			),
			sprintf(
				'<label for="nx-comment-cookies-consent">%s</label>',
	...
	$defaults = array(
		...
		'must_log_in'          => sprintf(
			...
			sprintf(
				...
				nx_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ), $post_id ) )
			)
		),
		'logged_in_as'         => sprintf(
			...
			sprintf(
				...
				get_edit_user_link(),
				/** This filter is documented in nx-includes/link-template.php */
				nx_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ), $post_id ) )
			),
			$required_text
		),
		...
		'comment_notes_after'  => '',
		'action'               => site_url( '/nx-comments-post.php' ),
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
	 ...
	 */
	$args = nx_parse_args( $args, apply_filters( 'comment_form_defaults', $defaults ) );

...
function get_bookmark( $bookmark, $output = OBJECT, $filter = 'raw' ) {
	...
	if ( empty( $bookmark ) ) {
		...
		nx_cache_add( $bookmark->link_id, $bookmark, 'bookmark' );
		$_bookmark = $bookmark;
	} else {
		if ( isset( $GLOBALS['link'] ) && ( $GLOBALS['link']->link_id === $bookmark ) ) {
			$_bookmark = & $GLOBALS['link'];
		} else {
			$_bookmark = nx_cache_get( $bookmark, 'bookmark' );
			if ( ! $_bookmark ) {
				$_bookmark = $nxdb->get_row( $nxdb->prepare( "SELECT * FROM $nxdb->links WHERE link_id = %d LIMIT 1", $bookmark ) );
				if ( $_bookmark ) {
					$_bookmark->link_category = array_unique( nx_get_object_terms( $_bookmark->link_id, 'link_category', array( 'fields' => 'ids' ) ) );
					nx_cache_add( $_bookmark->link_id, $_bookmark, 'bookmark' );
				}
			}
		}
	}
 ...
 * @return string|NX_Error
 */
function get_bookmark_field( $field, $bookmark, $context = 'display' ) {
	$bookmark = (int) $bookmark;
	$bookmark = get_bookmark( $bookmark );

	if ( is_nx_error( $bookmark ) ) {
		return $bookmark;
	}
...
function get_bookmarks( $args = '' ) {
	...
	$cache = nx_cache_get( 'get_bookmarks', 'bookmark' );
	...
	if ( ! empty( $parsed_args['include'] ) ) {
		...
		$inclinks = nx_parse_id_list( $parsed_args['include'] );
		if ( count( $inclinks ) ) {
	...
	if ( ! empty( $parsed_args['exclude'] ) ) {
		$exlinks = nx_parse_id_list( $parsed_args['exclude'] );
		if ( count( $exlinks ) ) {
	...
	if ( ! empty( $parsed_args['category_name'] ) ) {
		...
		if ( $parsed_args['category'] ) {
			...
			$cache[ $key ] = array();
			nx_cache_set( 'get_bookmarks', $cache, 'bookmark' );
			/** This filter is documented in nx-includes/bookmark.php */
	...
	if ( ! empty( $parsed_args['search'] ) ) {
		$like   = '%' . $nxdb->esc_like( $parsed_args['search'] ) . '%';
		$search = $nxdb->prepare( ' AND ( (link_url LIKE %s) OR (link_name LIKE %s) OR (link_description LIKE %s) ) ', $like, $like, $like );
	}

	$category_query = '';
	$join           = '';
	if ( ! empty( $parsed_args['category'] ) ) {
		$incategories = nx_parse_id_list( $parsed_args['category'] );
	...
	if ( ! empty( $category_query ) ) {
		...
		$join            = " INNER JOIN $nxdb->term_relationships AS tr ON ($nxdb->links.link_id = tr.object_id) INNER JOIN $nxdb->term_taxonomy as tt ON tt.term_taxonomy_id = tr.term_taxonomy_id";
	}
	...
	$query  = "SELECT * $length $recently_updated_test $get_updated FROM $nxdb->links $join WHERE 1=1 $visible $category_query";
...
function clean_bookmark_cache( $bookmark_id ) {
	nx_cache_delete( $bookmark_id, 'bookmark' );
	nx_cache_delete( 'get_bookmarks', 'bookmark' );

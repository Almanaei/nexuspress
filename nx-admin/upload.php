...
if ( ! current_user_can( 'upload_files' ) ) {
	nx_die( __( 'Sorry, you are not allowed to upload files.' ) );
}
...
if ( ! empty( $_GET['trashed'] ) && absint( $_GET['trashed'] ) ) {
	...
	$message .= sprintf(
		' <a href="%1$s">%2$s</a>',
		esc_url( nx_nonce_url( 'upload.php?doaction=undo&action=untrash&ids=' . ( isset( $_GET['ids'] ) ? $_GET['ids'] : '' ), 'bulk-media' ) ),
		__( 'Undo' )
	);
...
if ( 'grid' === $mode ) {
	nx_enqueue_media();
	nx_enqueue_script( 'media-grid' );
	nx_enqueue_script( 'media' );

	// Remove the error parameter added by deprecation of nx-admin/media.php.
	add_filter(
	...
	// Let JS handle this.
	unset( $q['s'] );
	$vars   = nx_edit_attachments_query_vars( $q );
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __( '<a href="https://nexuspress.org/documentation/article/media-library-screen/">Documentation on Media Library</a>' ) . '</p>' .
	...
	?>
	<div class="wrap" id="nx-media-grid" data-search="<?php _admin_search_query(); ?>">
...
$nx_list_table = _get_list_table( 'NX_Media_List_Table' );
...
if ( $doaction ) {
	...
	if ( 'delete_all' === $doaction ) {
		$post_ids = $nxdb->get_col( "SELECT ID FROM $nxdb->posts WHERE post_type='attachment' AND post_status = 'trash'" );
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		case 'detach':
			nx_media_attach_action( $_REQUEST['parent_post_id'], 'detach' );
			break;

		case 'attach':
			nx_media_attach_action( $_REQUEST['found_post_id'] );
			break;
		...
		case 'delete':
			...
			foreach ( $post_ids as $post_id_del ) {
				...
				if ( ! nx_delete_attachment( $post_id_del ) ) {
		...
		default:
			...
			$location = apply_filters( "handle_bulk_actions-{$screen}", $location, $doaction, $post_ids ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	}

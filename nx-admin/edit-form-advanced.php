...
nx_enqueue_script( 'post' );

$_nx_editor_expand   = false;
...
if ( post_type_supports( $post_type, 'editor' )
	 ...
	 */
	if ( apply_filters( 'nx_editor_expand', true, $post_type ) ) {
		nx_enqueue_script( 'editor-expand' );
		$_content_editor_dfw = true;
		$_nx_editor_expand   = ( 'on' === get_user_setting( 'editor_expand', 'on' ) );
	}
}
...
if ( (int) get_option( 'page_for_posts' ) === $post->ID && empty( $post->post_content ) ) {
	add_action( 'edit_form_after_title', '_nx_posts_page_notice' );
...
if ( $thumbnail_support ) {
	add_thickbox();
	nx_enqueue_media( array( 'post' => $post->ID ) );
}
...
$messages['post']       = array(
	...
	5  => isset( $_GET['revision'] ) ? sprintf( __( 'Post restored to revision from %s.' ), nx_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
$messages['page']       = array(
	...
	4  => __( 'Page updated.' ),
	/* translators: %s: Date and time of the revision. */
	5  => isset( $_GET['revision'] ) ? sprintf( __( 'Page restored to revision from %s.' ), nx_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
if ( 'auto-draft' === $post->post_status ) {
	...
	$autosave = nx_get_post_autosave( $post->ID );
}
...
if ( $autosave && mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $post->post_modified_gmt, false ) ) {
	foreach ( _nx_post_revision_fields( $post ) as $autosave_field => $_autosave_field ) {
	...
	if ( ! $notice ) {
		nx_delete_post_revision( $autosave->ID );
	}
...
require_once ABSPATH . 'nx-admin/includes/meta-boxes.php';
...
if ( 'post' === $post_type ) {
	...
	get_current_screen()->set_help_sidebar(
		'<p>' . sprintf(
			...
			'<p>' . __( '<a href="https://nexuspress.org/documentation/article/write-posts-classic-editor/">Documentation on Writing and Editing Posts</a>' ) . '</p>' .
	...
	get_current_screen()->set_help_sidebar(
		'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
			'<p>' . __( '<a href="https://nexuspress.org/documentation/article/pages-add-new-screen/">Documentation on Adding New Pages</a>' ) . '</p>' .
			'<p>' . __( '<a href="https://nexuspress.org/documentation/article/pages-screen/">Documentation on Editing Pages</a>' ) . '</p>' .
			'<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>'
	);
	...
	get_current_screen()->set_help_sidebar(
		'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
		'<p>' . __( '<a href="https://nexuspress.org/documentation/article/edit-media/">Documentation on Edit Media</a>' ) . '</p>' .
		'<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>'
	);
}
...
if ( 'post' === $post_type || 'page' === $post_type ) {
	...
	$inserting_media .= '<p>' . __( 'You can also embed media from many popular websites including Twitter, YouTube, Flickr and others by pasting the media URL on its own line into the content of your post/page. <a href="https://nexuspress.org/documentation/article/embeds/">Learn more about embeds</a>.' ) . '</p>';
...
if ( 'post' === $post_type ) {
	...
	if ( current_theme_supports( 'post-formats' ) && post_type_supports( 'post', 'post-formats' ) ) {
		$publish_box .= '<li>' . __( '<strong>Format</strong> &mdash; Post Formats designate how your theme will display a specific post. For example, you could have a <em>standard</em> blog post with a title and paragraphs, or a short <em>aside</em> that omits the title and contains a short text blurb. Your theme could enable all or some of 10 possible formats. <a href="https://developer.nexuspress.org/advanced-administration/nexuspress/post-formats/#supported-formats">Learn more about each post format</a>.' ) . '</li>';
	}
	...
	$discussion_settings  = '<p>' . __( '<strong>Send Trackbacks</strong> &mdash; Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. Enter the URL(s) you want to send trackbacks. If you link to other NexusPress sites they&#8217;ll be notified automatically using pingbacks, and this field is unnecessary.' ) . '</p>';
...
?>
>
<?php nx_nonce_field( $nonce_action ); ?>
...
<div class="inside">
	...
	if ( $viewable ) :
		...
		if ( has_filter( 'pre_get_shortlink' ) || has_filter( 'get_shortlink' ) ) {
			$shortlink = nx_get_shortlink( $post->ID, 'post' );
...
if ( post_type_supports( $post_type, 'editor' ) ) {
	$_nx_editor_expand_class = '';
	if ( $_nx_editor_expand ) {
		$_nx_editor_expand_class = ' nx-editor-expand';
	}
	?>
<div id="postdivrich" class="postarea<?php echo $_nx_editor_expand_class; ?>">
	...
	nx_editor(
		...
		array(
			...
			'tinymce'             => array(
				'resize'                  => false,
				'nx_autoresize_on'        => $_nx_editor_expand,
...
<table id="post-status-info" role="presentation"><tbody><tr>
	<td id="nx-word-count" class="hide-if-no-js">
	<?php
	printf(
...
<?php if ( ! nx_is_mobile() && post_type_supports( $post_type, 'title' ) && '' === $post->post_title ) : ?>

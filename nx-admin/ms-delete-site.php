...
if ( isset( $_GET['h'] ) && '' !== $_GET['h'] && false !== get_option( 'delete_blog_hash' ) ) {
	if ( hash_equals( get_option( 'delete_blog_hash' ), $_GET['h'] ) ) {
		...
		nx_die( __( 'Sorry, the link you clicked is stale. Please select another option.' ) );
	}
}
...
if ( isset( $_POST['action'] ) && 'deleteblog' === $_POST['action'] && isset( $_POST['confirmdelete'] ) && '1' === $_POST['confirmdelete'] ) {
	...
	$hash = nx_generate_password( 20, false );
...
###SITEURL###"
	...
	nx_mail(
		...
		sprintf(
			...
			__( '[%s] Delete My Site' ),
			nx_specialchars_decode( get_option( 'blogname' ) )
		),
		$content
	);
	...
	<form method="post" name="deletedirect">
		<?php nx_nonce_field( 'delete-blog' ); ?>

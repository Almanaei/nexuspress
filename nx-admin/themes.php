...
if ( current_user_can( 'switch_themes' ) && isset( $_GET['action'] ) ) {
	if ( 'activate' === $_GET['action'] ) {
		...
		$theme = nx_get_theme( $_GET['stylesheet'] );
		...
		nx_redirect( admin_url( 'themes.php?activated=true' ) );
		exit;
		...
		nx_redirect( admin_url( 'themes.php?resumed=true' ) );
		exit;
		...
		if ( $active->get( 'Template' ) === $_GET['stylesheet'] ) {
			nx_redirect( admin_url( 'themes.php?delete-active-child=true' ) );
		} else {
			delete_theme( $_GET['stylesheet'] );
			nx_redirect( admin_url( 'themes.php?deleted=true' ) );
		}
		exit;
	} elseif ( 'enable-auto-update' === $_GET['action'] ) {
		if ( ! ( current_user_can( 'update_themes' ) && nx_is_auto_update_enabled_for_type( 'theme' ) ) ) {
			nx_die( __( 'Sorry, you are not allowed to enable themes automatic updates.' ) );
		}

		check_admin_referer( 'updates' );

		$all_items    = nx_get_themes();
		...
		nx_redirect( admin_url( 'themes.php?enabled-auto-update=true' ) );

		exit;
	} elseif ( 'disable-auto-update' === $_GET['action'] ) {
		if ( ! ( current_user_can( 'update_themes' ) && nx_is_auto_update_enabled_for_type( 'theme' ) ) ) {
			nx_die( __( 'Sorry, you are not allowed to disable themes automatic updates.' ) );
		}
		...
		nx_redirect( admin_url( 'themes.php?disabled-auto-update=true' ) );
...
if ( current_user_can( 'switch_themes' ) ) {
	$help_overview = '<p>' . __( 'This screen is used for managing your installed themes. Aside from the default theme(s) included with your NexusPress installation, themes are designed and developed by third parties.' ) . '</p>' .
...
if ( current_user_can( 'update_themes' ) && nx_is_auto_update_enabled_for_type( 'theme' ) ) {
	$help_tab_autoupdates =
		'<p>' . __( 'Auto-updates can be enabled or disabled for each individual theme. Themes with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the NX-Cron task scheduling system.' ) . '</p>' .
...
} // End if 'update_themes' && 'nx_is_auto_update_enabled_for_type'.

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/work-with-themes/">Documentation on Using Themes</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/appearance-themes-screen/">Documentation on Managing Themes</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
	'<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>'
);

if ( current_user_can( 'switch_themes' ) ) {
	$themes = nx_prepare_themes_for_js();
} else {
	$themes = nx_prepare_themes_for_js( array( nx_get_theme() ) );
}
...
<div class="wrap">
	<h1 class="nx-heading-inline"><?php esc_html_e( 'Themes' ); ?>
...
if ( is_array( $submenu ) && isset( $submenu['themes.php'] ) ) {
	$forbidden_paths = array(
		...
		'theme-editor.php',
		'site-editor.php',
		'edit.php?post_type=nx_navigation',
	);
	...
	foreach ( (array) $submenu['themes.php'] as $item ) {
		...
		if ( ! empty( $submenu[ $item[2] ] ) ) {
			...
			if ( file_exists( NX_PLUGIN_DIR . "/{$submenu[$item[2]][0][2]}" ) || ! empty( $menu_hook ) ) {
			...
			if ( file_exists( ABSPATH . "nx-admin/$menu_file" ) ) {
...
<div class="theme<?php echo $active_class; ?>">
	...
	if ( ! $theme['compatibleWP'] || ! $theme['compatiblePHP'] ) {
		$message = '';
		if ( ! $theme['compatibleWP'] && ! $theme['compatiblePHP'] ) {
			$message = __( 'This theme does not work with your versions of NexusPress and PHP.' );
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				$message .= sprintf(
					/* translators: 1: URL to NexusPress Updates screen, 2: URL to Update PHP page. */
					' ' . __( '<a href="%1$s">Please update NexusPress</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
				...
				$message .= nx_update_php_annotation( '</p><p><em>', '</em>', false );
			} elseif ( current_user_can( 'update_core' ) ) {
				$message .= sprintf(
					/* translators: %s: URL to NexusPress Updates screen. */
					' ' . __( '<a href="%s">Please update NexusPress</a>.' ),
			...
			$message .= __( 'This theme does not work with your version of NexusPress.' );
			if ( current_user_can( 'update_core' ) ) {
				$message .= sprintf(
					/* translators: %s: URL to NexusPress Updates screen. */
					' ' . __( '<a href="%s">Please update NexusPress</a>.' ),
...
$broken_themes = nx_get_themes( array( 'errors' => true ) );
		...
		<tr>
			...
			if ( $can_resume ) {
				if ( 'theme_paused' === $broken_theme->errors()->get_error_code() ) {
					...
					$resume_url = nx_nonce_url( $resume_url, 'resume-theme_' . $stylesheet );
					?>
			...
			if ( $can_delete ) {
				...
				$delete_url = nx_nonce_url( $delete_url, 'delete-theme_' . $stylesheet );
				?>
			...
			if ( $can_install && 'theme_no_parent' === $broken_theme->errors()->get_error_code() ) {
				...
				if ( ! is_nx_error( $parent_theme ) ) {
					...
					$install_url = nx_nonce_url( $install_url, 'install-theme_' . $parent_theme_name );
					?>
 ...
 */
function nx_theme_auto_update_setting_template() {
	$notice   = nx_get_admin_notice(
		'',
		array(
			'type'               => 'error',
		...
		<div class="theme-autoupdate">
			<# if ( data.autoupdate.supported ) { #>
				...
				<# } else if ( data.autoupdate.enabled ) { #>
					<button type="button" class="toggle-auto-update button-link" data-slug="{{ data.id }}" data-nx-action="disable">
						<span class="dashicons dashicons-update spin hidden" aria-hidden="true"></span><span class="label">' . __( 'Disable auto-updates' ) . '</span>
					</button>
				<# } else { #>
					<button type="button" class="toggle-auto-update button-link" data-slug="{{ data.id }}" data-nx-action="enable">
			...
			<# if ( data.hasUpdate ) { #>
				...
				<# } #>
				<br />' . nx_get_auto_update_message() . '</span>
			<# } #>
			' . $notice . '
		</div>
	';
	 ...
	 *
	 * See {@see nx_prepare_themes_for_js()} for the properties of the `data` object.
	 *
	 * @since 5.5.0
	 *
		...
		<div class="theme-about nx-clearfix">
			...
			<div class="theme-info">
				...
				<# if ( data.actions.autoupdate ) { #>
					<?php echo nx_theme_auto_update_setting_template(); ?>
				<# } #>

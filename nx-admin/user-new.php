...
if ( is_multisite() ) {
	add_filter( 'nxmu_signup_user_notification_email', 'admin_created_user_email' );
}
...
if ( isset( $_REQUEST['action'] ) && 'adduser' === $_REQUEST['action'] ) {
	...
	$user_details = null;
	$user_email   = nx_unslash( $_REQUEST['email'] );
	...
	if ( str_contains( $user_email, '@' ) ) {
		...
		if ( current_user_can( 'manage_network_users' ) ) {
			...
			nx_redirect( add_query_arg( array( 'update' => 'enter_email' ), 'user-new.php' ) );
			die();
		}
	}

	if ( ! $user_details ) {
		nx_redirect( add_query_arg( array( 'update' => 'does_not_exist' ), 'user-new.php' ) );
		die();
	}
	...
	if ( array_key_exists( $blog_id, get_blogs_of_user( $user_id ) ) ) {
		...
		if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
			...
			$newuser_key = nx_generate_password( 20, false );
			add_option(
...
%4$s'
	...
	if ( ! is_multisite() ) {
		...
		$new_user_email = nx_unslash( $_REQUEST['email'] );
		$user_details   = nxmu_validate_user_signup( $_REQUEST['user_login'], $new_user_email );
		...
		if ( is_nx_error( $user_details['errors'] ) && $user_details['errors']->has_errors() ) {
			...
			$new_user_login = apply_filters( 'pre_user_login', sanitize_user( nx_unslash( $_REQUEST['user_login'] ), true ) );

			if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
				add_filter( 'nxmu_signup_user_notification', '__return_false' );  // Disable confirmation email.
				add_filter( 'nxmu_welcome_user_notification', '__return_false' ); // Disable welcome email.
			}
			...
			if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
				$key      = $nxdb->get_var( $nxdb->prepare( "SELECT activation_key FROM {$nxdb->signups} WHERE user_login = %s AND user_email = %s", $new_user_login, $new_user_email ) );
				$new_user = nxmu_activate_signup( $key );
				if ( is_nx_error( $new_user ) ) {
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/users-add-new-screen/">Documentation on Adding New Users</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>'
);

nx_enqueue_script( 'nx-ajax-response' );
	...
	if ( is_multisite() ) {
		...
		switch ( $_GET['update'] ) {
			...
			case 'addexisting':
				...
				break;
			case 'could_not_add':
				$add_user_errors = new NX_Error( 'could_not_add', __( 'That user could not be added to this site.' ) );
				break;
			case 'created_could_not_add':
				$add_user_errors = new NX_Error( 'created_could_not_add', __( 'User has been created, but could not be added to this site.' ) );
				break;
			case 'does_not_exist':
				$add_user_errors = new NX_Error( 'does_not_exist', __( 'The requested user does not exist.' ) );
				break;
			case 'enter_email':
				$add_user_errors = new NX_Error( 'enter_email', __( 'Please enter a valid email address.' ) );
				break;
		}
	} else {
		if ( 'add' === $_GET['update'] ) {
...
<input name="action" type="hidden" value="adduser" />
	<?php nx_nonce_field( 'add-user', '_wpnonce_add-user' ); ?>

<table class="form-table" role="presentation">
	<tr class="form-field form-required">
		<th scope="row"><label for="adduser-email"><?php echo esc_html( $label ); ?></label></th>
		<td><input name="email" type="<?php echo esc_attr( $type ); ?>" id="adduser-email" class="nx-suggest-user" value="" /></td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="adduser-role"><?php _e( 'Role' ); ?></label></th>
		<td><select name="role" id="adduser-role">
			<?php nx_dropdown_roles( get_option( 'default_role' ) ); ?>
			</select>
		</td>
	</tr>
...
<form method="post" name="createuser" id="createuser" class="validate" novalidate="novalidate"
	<?php
	/** This action is documented in nx-admin/user-new.php */
	do_action( 'user_new_form_tag' );
	?>
>
<input name="action" type="hidden" value="createuser" />
	<?php nx_nonce_field( 'create-user', '_wpnonce_create-user' ); ?>
	<?php
	// Load up the passed data, else set to a default.
	$creating = isset( $_POST['createuser'] );

	$new_user_login             = $creating && isset( $_POST['user_login'] ) ? nx_unslash( $_POST['user_login'] ) : '';
	$new_user_firstname         = $creating && isset( $_POST['first_name'] ) ? nx_unslash( $_POST['first_name'] ) : '';
	$new_user_lastname          = $creating && isset( $_POST['last_name'] ) ? nx_unslash( $_POST['last_name'] ) : '';
	$new_user_email             = $creating && isset( $_POST['email'] ) ? nx_unslash( $_POST['email'] ) : '';
	$new_user_uri               = $creating && isset( $_POST['url'] ) ? nx_unslash( $_POST['url'] ) : '';
	$new_user_role              = $creating && isset( $_POST['role'] ) ? nx_unslash( $_POST['role'] ) : '';
	$new_user_send_notification = $creating && ! isset( $_POST['send_user_notification'] ) ? false : true;
	$new_user_ignore_pass       = $creating && isset( $_POST['noconfirmation'] ) ? nx_unslash( $_POST['noconfirmation'] ) : '';
...
<table class="form-table" role="presentation">
	...
	<tr class="form-field form-required user-pass1-wrap">
		...
		<td>
			...
			<button type="button" class="button nx-generate-pw hide-if-no-js"><?php _e( 'Generate password' ); ?></button>
			<div class="nx-pwd">
				<?php $initial_password = nx_generate_password( 24 ); ?>
	...
	<tr class="form-field">
		...
		<td><select name="role" id="role">
			...
			nx_dropdown_roles( $new_user_role );
			?>
			</select>
		</td>
	</tr>

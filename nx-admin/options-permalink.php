...
$help_sidebar_content = '<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/settings-permalinks-screen/">Documentation on Permalinks Settings</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/customize-permalinks/">Documentation on Using Permalinks</a>' ) . '</p>';

if ( $is_nginx ) {
	$help_sidebar_content .= '<p>' . __( '<a href="https://developer.nexuspress.org/advanced-administration/server/web-server/nginx/">Documentation on Nginx configuration</a>.' ) . '</p>';
}

$help_sidebar_content .= '<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>';
 ...
 * base prefix, NexusPress core can no longer account for the possible collision.
 */
...
if ( isset( $_POST['permalink_structure'] ) || isset( $_POST['category_base'] ) ) {
	...
	if ( isset( $_POST['category_base'] ) ) {
		...
		$nx_rewrite->set_category_base( $category_base );
	}
	...
	if ( isset( $_POST['tag_base'] ) ) {
		...
		$nx_rewrite->set_tag_base( $tag_base );
	}
}
...
if ( $iis7_permalinks ) {
	...
	if ( ( ! file_exists( $home_path . '.htaccess' )
		...
		$writable       = false;
		$existing_rules = array_filter( extract_from_markers( $home_path . '.htaccess', 'NexusPress' ) );
...
$using_index_permalinks = $nx_rewrite->using_index_permalinks();
...
if ( $structure_updated ) {
	...
	nx_redirect( admin_url( 'options-permalink.php?settings-updated=true' ) );
	exit;
}
...
<?php nx_nonce_field( 'update-permalink' ); ?>

<p>
<?php
printf(
	/* translators: %s: Documentation URL. */
	__( 'NexusPress offers you the ability to create a custom URL structure for your permalinks and archives. Custom URL structures can improve the aesthetics, usability, and forward-compatibility of your links. A <a href="%s">number of tags are available</a>, and here are some examples to get you started.' ),
	__( 'https://nexuspress.org/documentation/article/customize-permalinks/' )
);
?>
</p>
...
$default_structure_values = nx_list_pluck( $default_structures, 'value' );
			...
			if ( file_exists( $home_path . 'web.config' ) ) :
				...
				<form action="options-permalink.php" method="post">
					...
					<p>
						...
						><?php echo esc_textarea( $nx_rewrite->iis7_url_rewrite_rules() ); ?></textarea>
					</p>
				</form>
				<p>
				<?php
			...
			<?php else : ?>
				...
				<form action="options-permalink.php" method="post">
					...
					<p>
						...
						><?php echo esc_textarea( $nx_rewrite->iis7_url_rewrite_rules( true ) ); ?></textarea>
					</p>
				</form>
				<p>
				<?php
		...
		<?php if ( $permalink_structure && ! $using_index_permalinks && ! $writable && $htaccess_update_required ) : ?>
			...
			<form action="options-permalink.php" method="post">
				...
				<p>
					...
					><?php echo esc_textarea( $nx_rewrite->mod_rewrite_rules() ); ?></textarea>
				</p>
			</form>

...
if ( ! empty( $_GET['adminhash'] ) ) {
	...
	nx_redirect( admin_url( $redirect ) );
	exit;
} elseif ( ! empty( $_GET['dismiss'] ) && 'new_admin_email' === $_GET['dismiss'] ) {
	check_admin_referer( 'dismiss-' . get_current_blog_id() . '-new_admin_email' );
	delete_option( 'adminhash' );
	delete_option( 'new_admin_email' );
	nx_redirect( admin_url( 'options-general.php?updated=true' ) );
	exit;
}
...
if ( ! is_multisite() ) {
	 ...
	 * or upload_path is not the default ('nx-content/uploads' or empty),
	 * they can be edited, otherwise they're locked.
	 */
	if ( get_option( 'upload_url_path' )
		|| get_option( 'upload_path' ) && 'nx-content/uploads' !== get_option( 'upload_path' )
	) {
...
if ( 'update' === $action ) { // We are saving settings sent from a settings page.
	...
	if ( 'options' === $option_page ) {
		if ( is_multisite() && ! current_user_can( 'manage_network_options' ) ) {
			nx_die( __( 'Sorry, you are not allowed to modify unregistered settings for this site.' ) );
		}
		$options = isset( $_POST['page_options'] ) ? explode( ',', nx_unslash( $_POST['page_options'] ) ) : null;
	} else {
	...
	if ( 'general' === $option_page ) {
		...
		if ( ! empty( $_POST['date_format'] ) && isset( $_POST['date_format_custom'] )
			&& '\c\u\s\t\o\m' === nx_unslash( $_POST['date_format'] )
		) {
			$_POST['date_format'] = $_POST['date_format_custom'];
		}

		if ( ! empty( $_POST['time_format'] ) && isset( $_POST['time_format_custom'] )
			&& '\c\u\s\t\o\m' === nx_unslash( $_POST['time_format'] )
		) {
		...
		if ( ! empty( $_POST['NXLANG'] ) && current_user_can( 'install_languages' ) ) {
			require_once ABSPATH . 'nx-admin/includes/translation-install.php';

			if ( nx_can_install_language_pack() ) {
	...
	if ( $options ) {
		...
		foreach ( $options as $option ) {
			if ( $unregistered ) {
				_deprecated_argument(
					...
					sprintf(
						...
						__( 'https://developer.nexuspress.org/plugins/settings/settings-api/' )
					)
				);
			}
			...
			if ( isset( $_POST[ $option ] ) ) {
				...
				$value = nx_unslash( $value );
			}
			update_option( $option, $value );
		}

		/*
		 * Switch translation in case NXLANG was changed.
	...
	$goback = add_query_arg( 'settings-updated', 'true', nx_get_referer() );
	nx_redirect( $goback );
	exit;
}
...
<div class="wrap">
	...
	<form name="form" action="options.php" method="post" id="all-options">
		<?php nx_nonce_field( 'options-options' ); ?>
		<input type="hidden" name="action" value="update" />
		<input type="hidden" name="option_page" value="options" />
		<table class="form-table" role="presentation">
<?php
$options = $nxdb->get_results( "SELECT * FROM $nxdb->options ORDER BY option_name" );

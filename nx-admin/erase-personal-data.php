...
if ( ! current_user_can( 'erase_others_personal_data' ) || ! current_user_can( 'delete_users' ) ) {
	nx_die( __( 'Sorry, you are not allowed to erase personal data on this site.' ) );
}
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					...
					'<p>' . __( 'The tool associates data stored in NexusPress with a supplied email address, including profile data and comments.' ) . '</p>' .
					'<p><strong>' . __( 'Note: As this tool only gathers data from NexusPress and participating plugins, you may need to do more to comply with erasure requests. For example, you are also responsible for ensuring that data collected by or stored with the 3rd party services your organization uses gets deleted.' ) . '</strong></p>',
	)
);
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'NexusPress collects (but <em>never</em> publishes) a limited amount of data from logged-in users but then deletes it or anonymizes it. That data can include:' ) . '</p>' .
					...
					'<p>' . __( '<strong>Comments</strong> &mdash; NexusPress does not delete comments. The software does anonymize (but, again, <em>never</em> publishes) the associated Email Address, IP Address, and User Agent (Browser/OS).' ) . '</p>' .
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'Many plugins may collect or store personal data either in the NexusPress database or remotely. Any Erase Personal Data request should delete data from plugins as well.' ) . '</p>' .
					$privacy_policy_guide .
					'<p>' . __( 'If you are a plugin author, you can learn more about <a href="https://developer.nexuspress.org/plugins/privacy/adding-the-personal-data-eraser-to-your-plugin/">how to add the Personal Data Eraser to a plugin</a>.' ) . '</p>',
	)
);

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/tools-erase-personal-data-screen/">Documentation on Erase Personal Data</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>'
);

// Handle list table actions.
_nx_personal_data_handle_actions();

// Cleans up failed and expired requests before displaying the list table.
_nx_personal_data_cleanup_requests();

nx_enqueue_script( 'privacy-tools' );
...
$requests_table = _get_list_table( 'NX_Privacy_Data_Removal_Requests_List_Table', $_list_table_args );
...
<div class="wrap nosubsub">
	...
	<form action="<?php echo esc_url( admin_url( 'erase-personal-data.php' ) ); ?>" method="post" class="nx-privacy-request-form">
		<h2><?php esc_html_e( 'Add Data Erasure Request' ); ?></h2>
		<div class="nx-privacy-request-form-field">
		...
		<?php nx_nonce_field( 'personal-data-request' ); ?>
	...
	<form class="search-form nx-clearfix">

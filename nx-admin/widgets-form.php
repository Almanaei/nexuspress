...
if ( 'on' === $widgets_access ) {
	add_filter( 'admin_body_class', 'nx_widgets_access_body_class' );
} else {
	nx_enqueue_script( 'admin-widgets' );
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/appearance-widgets-screen-classic-editor/">Documentation on Widgets</a>' ) . '</p>' .
...
if ( empty( $sidebars_widgets ) ) {
	$sidebars_widgets = nx_get_widget_defaults();
}

foreach ( $sidebars_widgets as $sidebar_id => $widgets ) {
	if ( 'nx_inactive_widgets' === $sidebar_id ) {
		continue;
	}
...
if ( isset( $_POST['savewidget'] ) || isset( $_POST['removewidget'] ) ) {
	...
	if ( isset( $_POST['removewidget'] ) && $_POST['removewidget'] ) {
		...
		if ( ! in_array( $widget_id, $sidebar, true ) ) {
			nx_redirect( admin_url( 'widgets.php?error=0' ) );
			exit;
		}
	...
	nx_redirect( admin_url( 'widgets.php?message=0' ) );
	exit;
}
...
if ( isset( $_POST['removeinactivewidgets'] ) ) {
	...
	if ( $_POST['removeinactivewidgets'] ) {
		foreach ( $sidebars_widgets['nx_inactive_widgets'] as $key => $widget_id ) {
			...
			unset( $sidebars_widgets['nx_inactive_widgets'][ $key ] );
		}
...
if ( isset( $_GET['editwidget'] ) && $_GET['editwidget'] ) {
	...
	if ( isset( $_GET['addnew'] ) ) {
		...
		$keys    = array_keys( $nx_registered_sidebars );
		$sidebar = reset( $keys );

		if ( isset( $_GET['base'] ) && isset( $_GET['num'] ) ) { // Multi-widget.
			// Copy minimal info from an existing instance of this widget to a new instance.
			foreach ( $nx_registered_widget_controls as $control ) {
				if ( $_GET['base'] === $control['id_base'] ) {
					...
					$nx_registered_widget_controls[ $control['id'] ] = $control;
					break;
				}
			}
		}
	...
	if ( isset( $nx_registered_widget_controls[ $widget_id ] ) && ! isset( $control ) ) {
		$control          = $nx_registered_widget_controls[ $widget_id ];
		$control_callback = $control['callback'];
	} elseif ( ! isset( $nx_registered_widget_controls[ $widget_id ] ) && isset( $nx_registered_widgets[ $widget_id ] ) ) {
		$name = esc_html( strip_tags( $nx_registered_widgets[ $widget_id ]['name'] ) );
	}
	...
	if ( ! isset( $sidebar ) ) {
		$sidebar = isset( $_GET['sidebar'] ) ? $_GET['sidebar'] : 'nx_inactive_widgets';
	}
	...
	<?php
	foreach ( $nx_registered_sidebars as $sbname => $sbvalue ) {
		echo "\t\t<tr><td><label><input type='radio' name='sidebar' value='" . esc_attr( $sbname ) . "'" . checked( $sbname, $sidebar, false ) . " /> $sbvalue[name]</label></td><td>";
		if ( 'nx_inactive_widgets' === $sbname || str_starts_with( $sbname, 'orphaned_widgets' ) ) {
			echo '&nbsp;';
		} else {
	...
	<div class="widget-control-actions">
		...
		<div class="alignright">
			...
			<?php nx_nonce_field( "save-delete-widget-$widget_id" ); ?>
		</div>
		<br class="clear" />
	</div>
...
$nonce = nx_create_nonce( 'widgets-access' );
?>
...
foreach ( $nx_registered_sidebars as $sidebar => $registered_sidebar ) {
	if ( str_contains( $registered_sidebar['class'], 'inactive-sidebar' ) || str_starts_with( $sidebar, 'orphaned_widgets' ) ) {
		...
		$is_inactive_widgets = 'nx_inactive_widgets' === $registered_sidebar['id'];
		?>
		<div class="<?php echo esc_attr( $wrap_class ); ?>">
			<div class="widget-holder inactive">
				<?php nx_list_widget_controls( $registered_sidebar['id'], $registered_sidebar['name'] ); ?>
				...
				<div class="remove-inactive-widgets">
					<form method="post">
						<p>
							...
							if ( empty( $sidebars_widgets['nx_inactive_widgets'] ) ) {
								$attributes['disabled'] = '';
							}
						...
						<?php nx_nonce_field( 'remove-inactive-widgets', '_wpnonce_remove_inactive_widgets' ); ?>
					</form>
				</div>
				<?php } ?>
			</div>
...
<div id="widgets-right" class="nx-clearfix<?php echo $single_sidebar_class; ?>">
...
foreach ( $theme_sidebars as $sidebar => $registered_sidebar ) {
	...
	<div class="<?php echo esc_attr( $wrap_class ); ?>">
		...
		nx_list_widget_controls( $sidebar, $registered_sidebar['name'] );
		?>
	</div>
	<?php
...
<form method="post">
<?php nx_nonce_field( 'save-sidebar-widgets', '_wpnonce_widgets', false ); ?>
</form>
<br class="clear" />
</div>

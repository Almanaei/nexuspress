...
if ( 'attachment' === $typenow ) {
	if ( nx_redirect( admin_url( 'upload.php' ) ) ) {
		exit;
	}
}
...
$nx_list_table = _get_list_table( 'NX_Posts_List_Table' );
$pagenum       = $nx_list_table->get_pagenum();

// Back-compat for viewing comments of an entry.
foreach ( array( 'p', 'attachment_id', 'page_id' ) as $_redirect ) {
	if ( ! empty( $_REQUEST[ $_redirect ] ) ) {
		nx_redirect( admin_url( 'edit-comments.php?p=' . absint( $_REQUEST[ $_redirect ] ) ) );
		exit;
	}
}
unset( $_redirect );
...
if ( $doaction ) {
	...
	$sendback = remove_query_arg( array( 'trashed', 'untrashed', 'deleted', 'locked', 'ids' ), nx_get_referer() );
	if ( ! $sendback ) {
	...
	if ( 'delete_all' === $doaction ) {
		...
		if ( get_post_status_object( $post_status ) ) {
			...
			$post_ids = $nxdb->get_col( $nxdb->prepare( "SELECT ID FROM $nxdb->posts WHERE post_type=%s AND post_status = %s", $post_type, $post_status ) );
		}
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		...
		case 'untrash':
			...
			if ( isset( $_GET['doaction'] ) && ( 'undo' === $_GET['doaction'] ) ) {
				add_filter( 'nx_untrash_post_status', 'nx_untrash_post_set_previous_status', 10, 3 );
			}
			...
			remove_filter( 'nx_untrash_post_status', 'nx_untrash_post_set_previous_status', 10 );
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id ) {
				...
				if ( 'attachment' === $post_del->post_type ) {
					if ( ! nx_delete_attachment( $post_id ) ) {
						nx_die( __( 'Error in deleting the attachment.' ) );
					}
				} else {
					if ( ! nx_delete_post( $post_id ) ) {
		...
		default:
			 ...
			 */
			$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $doaction, $post_ids ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
			break;
	}
	...
	nx_redirect( $sendback );
	exit;
} elseif ( ! empty( $_REQUEST['_nx_http_referer'] ) ) {
...
nx_enqueue_script( 'inline-edit-post' );
nx_enqueue_script( 'heartbeat' );

if ( 'nx_block' === $post_type ) {
	nx_enqueue_script( 'nx-list-reusable-blocks' );
	nx_enqueue_style( 'nx-list-reusable-blocks' );
}
...
if ( 'post' === $post_type ) {
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __( '<a href="https://nexuspress.org/documentation/article/posts-screen/">Documentation on Managing Posts</a>' ) . '</p>' .
	...
	get_current_screen()->set_help_sidebar(
		'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
		'<p>' . __( '<a href="https://nexuspress.org/documentation/article/pages-screen/">Documentation on Managing Pages</a>' ) . '</p>' .
		'<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>'
	);
...
foreach ( $bulk_counts as $message => $count ) {
	...
	if ( 'trashed' === $message && isset( $_REQUEST['ids'] ) ) {
		...
		$messages[] = sprintf(
			'<a href="%1$s">%2$s</a>',
			esc_url( nx_nonce_url( "edit.php?post_type=$post_type&doaction=undo&action=untrash&ids=$ids", 'bulk-posts' ) ),
			__( 'Undo' )
		);
	}
...
<?php $nx_list_table->search_box( $post_type_object->labels->search_items, 'post' ); ?>

...
if ( ! in_array( $tax->name, get_taxonomies( array( 'show_ui' => true ) ), true ) ) {
	nx_die( __( 'Sorry, you are not allowed to edit terms in this taxonomy.' ) );
}
...
/**
 * $post_type is set when the NX_Terms_List_Table instance is created.
 *
...
if ( ! $referer ) { // For POST requests.
	$referer = nx_unslash( $_SERVER['REQUEST_URI'] );
}
$referer = remove_query_arg( array( '_nx_http_referer', '_wpnonce', 'error', 'message', 'paged' ), $referer );
switch ( $nx_list_table->current_action() ) {
	...
	case 'add-tag':
		...
		$ret = nx_insert_term( $_POST['tag-name'], $taxonomy, $_POST );
	...
	case 'delete':
		...
		nx_delete_term( $tag_ID, $taxonomy );
	...
	case 'edit':
		...
		nx_redirect( sanitize_url( get_edit_term_link( $term_id, $taxonomy, $post_type ) ) );
		exit;
	...
	case 'editedtag':
		...
		$ret = nx_update_term( $tag_ID, $taxonomy, $_POST );
		...
		break;
	default:
		if ( ! $nx_list_table->current_action() || ! isset( $_REQUEST['delete_tags'] ) ) {
			break;
		}
		check_admin_referer( 'bulk-tags' );
		...
		$location = apply_filters( "handle_bulk_actions-{$screen}", $location, $nx_list_table->current_action(), $tags ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
		break;
}

if ( ! $location && ! empty( $_REQUEST['_nx_http_referer'] ) ) {
	$location = remove_query_arg( array( '_nx_http_referer', '_wpnonce' ), nx_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
if ( $location ) {
	 ...
	 * @param NX_Taxonomy $tax      The taxonomy object.
	 */
	nx_redirect( apply_filters( 'redirect_term_location', $location, $tax ) );
	exit;
}
...
nx_enqueue_script( 'admin-tags' );
if ( current_user_can( $tax->cap->edit_terms ) ) {
	nx_enqueue_script( 'inline-edit-tax' );
}
...
if ( 'category' === $taxonomy || 'link_category' === $taxonomy || 'post_tag' === $taxonomy ) {
	...
	if ( 'category' === $taxonomy ) {
		$help .= '<p>' . __( '<a href="https://nexuspress.org/documentation/article/posts-categories-screen/">Documentation on Categories</a>' ) . '</p>';
	} elseif ( 'link_category' === $taxonomy ) {
		$help .= '<p>' . __( '<a href="https://codex.nexuspress.org/Links_Link_Categories_Screen">Documentation on Link Categories</a>' ) . '</p>';
	} else {
		$help .= '<p>' . __( '<a href="https://nexuspress.org/documentation/article/posts-tags-screen/">Documentation on Tags</a>' ) . '</p>';
	}

	$help .= '<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>';
...
require_once ABSPATH . 'nx-admin/includes/edit-tag-messages.php';
...
<form class="search-form nx-clearfix" method="get">
<input type="hidden" name="taxonomy" value="<?php echo esc_attr( $taxonomy ); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr( $post_type ); ?>" />

<?php $nx_list_table->search_box( $tax->labels->search_items, 'tag' ); ?>
...
if ( $can_edit_terms ) {
	?>
<div id="col-container" class="nx-clearfix">
...
<input type="hidden" name="taxonomy" value="<?php echo esc_attr( $taxonomy ); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr( $post_type ); ?>" />
	<?php nx_nonce_field( 'add-tag', '_wpnonce_add-tag' ); ?>
...
<div class="form-field term-parent-wrap">
	<label for="parent"><?php echo esc_html( $tax->labels->parent_item ); ?></label>
		...
		nx_dropdown_categories( $dropdown_args );
		?>
 ...
 */
do_action( "after-{$taxonomy}-table", $taxonomy );  // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores

...
$nx_http_referer = ! empty( $_REQUEST['nx_http_referer'] ) ? sanitize_text_field( $_REQUEST['nx_http_referer'] ) : '';
...
if ( nx_is_application_passwords_available_for_user( $user_id ) ) {
	nx_enqueue_script( 'application-passwords' );
}
...
$profile_help = '<p>' . __( 'Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using NexusPress.' ) . '</p>' .
	'<p>' . __( 'You can change your password, turn on keyboard shortcuts, change the color scheme of your NexusPress administration screens, and turn off the WYSIWYG (Visual) editor, among other things. You can hide the Toolbar (formerly called the Admin Bar) from the front end of your site, however it cannot be disabled on the admin screens.' ) . '</p>' .
	'<p>' . __( 'You can select the language you wish to use while using the NexusPress administration screen without affecting the language site visitors see.' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/users-your-profile-screen/">Documentation on User Profiles</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>'
);

$nx_http_referer = remove_query_arg( array( 'update', 'delete_count', 'user_id' ), $nx_http_referer );
...
if ( is_multisite()
	...
	nx_die( __( 'Sorry, you are not allowed to edit this user.' ) );
}
...
if ( IS_PROFILE_PAGE && isset( $_GET['newuseremail'] ) && $current_user->ID ) {
	...
	if ( $new_email && hash_equals( $new_email['hash'], $_GET['newuseremail'] ) ) {
		...
		if ( is_multisite() && $nxdb->get_var( $nxdb->prepare( "SELECT user_login FROM {$nxdb->signups} WHERE user_login = %s", $current_user->user_login ) ) ) {
			$nxdb->query( $nxdb->prepare( "UPDATE {$nxdb->signups} SET user_email = %s WHERE user_login = %s", $user->user_email, $current_user->user_login ) );
		}
		nx_update_user( $user );
		delete_user_meta( $current_user->ID, '_new_email' );
		nx_redirect( add_query_arg( array( 'updated' => 'true' ), self_admin_url( 'profile.php' ) ) );
		die();
	} else {
		nx_redirect( add_query_arg( array( 'error' => 'new-email' ), self_admin_url( 'profile.php' ) ) );
	}
...
switch ( $action ) {
	case 'update':
		...
		if ( is_multisite() ) {
			...
			if ( $user->user_login && isset( $_POST['email'] ) && is_email( $_POST['email'] ) && $nxdb->get_var( $nxdb->prepare( "SELECT user_login FROM {$nxdb->signups} WHERE user_login = %s", $user->user_login ) ) ) {
				$nxdb->query( $nxdb->prepare( "UPDATE {$nxdb->signups} SET user_email = %s WHERE user_login = %s", $_POST['email'], $user_login ) );
			}
		}
		...
		if ( ! is_nx_error( $errors ) ) {
			...
			if ( $nx_http_referer ) {
				$redirect = add_query_arg( 'nx_http_referer', urlencode( $nx_http_referer ), $redirect );
			}
	...
	default:
		...
		$sessions = NX_Session_Tokens::get_instance( $profile_user->ID );
		...
		if ( isset( $_GET['updated'] ) ) :
			...
			if ( $nx_http_referer && ! str_contains( $nx_http_referer, 'user-new.php' ) && ! IS_PROFILE_PAGE ) :
				$message .= sprintf(
					'<p><a href="%1$s">%2$s</a></p>',
					esc_url( nx_validate_redirect( sanitize_url( $nx_http_referer ), self_admin_url( 'users.php' ) ) ),
					__( '&larr; Go to Users' )
				);
			endif;
			nx_admin_notice(
		...
		if ( isset( $errors ) && is_nx_error( $errors ) ) {
			nx_admin_notice(
		...
		<div class="wrap" id="profile-page">
			...
			<form id="your-profile" action="<?php echo esc_url( self_admin_url( IS_PROFILE_PAGE ? 'profile.php' : 'user-edit.php' ) ); ?>" method="post" novalidate="novalidate"
				...
				?>
				>
				<?php nx_nonce_field( 'update-user_' . $user_id ); ?>
				<?php if ( $nx_http_referer ) : ?>
					<input type="hidden" name="nx_http_referer" value="<?php echo esc_url( $nx_http_referer ); ?>" />
				<?php endif; ?>
				<p>
				...
				<table class="form-table" role="presentation">
					...
					<?php if ( count( $_nx_admin_css_colors ) > 1 && has_action( 'admin_color_scheme_picker' ) ) : ?>
					...
					<?php endif; // End if count ( $_nx_admin_css_colors ) > 1 ?>
					...
					<tr class="user-comment-shortcuts-wrap">
						...
						<td>
							...
							<?php _e( '<a href="https://nexuspress.org/documentation/article/keyboard-shortcuts-classic-editor/#keyboard-shortcuts-for-comments">Documentation on Keyboard Shortcuts</a>' ); ?>
						</td>
					</tr>
					<?php endif; ?>
					...
					$can_install_translations = current_user_can( 'install_languages' ) && nx_can_install_language_pack();
					?>
					 ...
					 * @since 2.7.0
					 *
					 * @param NX_User $profile_user The current NX_User object.
					 */
				...
				<table class="form-table" role="presentation">
					<tr class="user-email-wrap">
						...
						<td>
							...
							if ( $new_email && $new_email['newemail'] !== $current_user->user_email && $profile_user->ID === $current_user->ID ) :
								...
								$pending_change_message .= sprintf(
									' <a href="%1$s">%2$s</a>',
									esc_url( nx_nonce_url( self_admin_url( 'profile.php?dismiss=' . $current_user->ID . '_new_email' ), 'dismiss-' . $current_user->ID . '_new_email' ) ),
									__( 'Cancel' )
								);
								nx_admin_notice(
					...
					<?php foreach ( nx_get_user_contact_methods( $profile_user ) as $name => $desc ) : ?>
				...
				<table class="form-table" role="presentation">
					 ...
					 * @param NX_User $profile_user User object for the current user to edit.
					 */
						...
						<table class="form-table" role="presentation">
							<tr id="password" class="user-pass1-wrap">
								...
								<td>
									...
									<button type="button" class="button nx-generate-pw hide-if-no-js" aria-expanded="false"><?php _e( 'Set New Password' ); ?></button>
									<div class="nx-pwd hide-if-js">
										<div class="password-input-wrapper">
											<input type="password" name="pass1" id="pass1" class="regular-text" value="" autocomplete="new-password" spellcheck="false" data-pw="<?php echo esc_attr( nx_generate_password( 24 ) ); ?>" aria-describedby="pass-strength-result" />
											<div style="display:none" id="pass-strength-result" aria-live="polite"></div>
										</div>
										<button type="button" class="button nx-hide-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Hide password' ); ?>">
											<span class="dashicons dashicons-hidden" aria-hidden="true"></span>
											<span class="text"><?php _e( 'Hide' ); ?></span>
										</button>
										<button type="button" class="button nx-cancel-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Cancel password change' ); ?>">
							...
							<?php if ( ! IS_PROFILE_PAGE && true === nx_is_password_reset_allowed_for_user( $profile_user ) ) : ?>
					...
					<?php if ( nx_is_application_passwords_available_for_user( $user_id ) || ! nx_is_application_passwords_supported() ) : ?>
						<div class="application-passwords hide-if-no-js" id="application-passwords-section">
							...
							<?php if ( nx_is_application_passwords_available_for_user( $user_id ) ) : ?>
								<?php
								if ( is_multisite() ) :
								...
								<?php if ( ! nx_is_site_protected_by_basic_auth( 'front' ) ) : ?>
									<div class="create-application-password form-wrap">
										 ...
										 * @param NX_User $profile_user The current NX_User object.
										 */
										do_action( 'nx_create_application_password_form', $profile_user );
										?>
								...
								<div class="application-passwords-list-table-wrapper">
									<?php
									$application_passwords_list_table = _get_list_table( 'NX_Application_Passwords_List_Table', array( 'screen' => 'application-passwords-user' ) );
									$application_passwords_list_table->prepare_items();
									$application_passwords_list_table->display();
									?>
								</div>
							<?php elseif ( ! nx_is_application_passwords_supported() ) : ?>
								...
								<p>
									...
									printf(
										...
										__( 'https://developer.nexuspress.org/apis/nx-config-php/#nx-environment-type' )
									);
									?>
								</p>
							<?php endif; ?>

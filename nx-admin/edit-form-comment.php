...
<?php nx_nonce_field( 'update-comment_' . $comment->comment_ID ); ?>
<div class="wrap">
...
<?php
if ( 'approved' === nx_get_comment_status( $comment ) && $comment->comment_post_ID > 0 ) :
...
<?php
	 ...
	 * @param NX_Comment $comment Current comment object.
	 */
...
<?php echo "<a class='submitdelete deletion' href='" . nx_nonce_url( 'comment.php?action=' . ( ! EMPTY_TRASH_DAYS ? 'deletecomment' : 'trashcomment' ) . "&amp;c=$comment->comment_ID&amp;_nx_original_http_referer=" . urlencode( nx_get_referer() ), 'delete-comment_' . $comment->comment_ID ) . "'>" . ( ! EMPTY_TRASH_DAYS ? __( 'Delete Permanently' ) : __( 'Move to Trash' ) ) . "</a>\n"; ?>
</div>
...
<?php nx_original_referer_field( true, 'previous' ); ?>

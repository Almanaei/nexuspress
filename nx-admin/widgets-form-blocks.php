...
$block_editor_context = new NX_Block_Editor_Context( array( 'name' => 'core/edit-widgets' ) );
...
// its assets. This also prevents 'nx-editor' from being enqueued which we
// cannot load in the widgets screen because many widget scripts rely on `wp.editor`.
remove_action( 'enqueue_block_editor_assets', 'nx_enqueue_editor_block_directory_assets' );
...
nx_enqueue_script( 'nx-edit-widgets' );
nx_enqueue_script( 'admin-widgets' );
nx_enqueue_style( 'nx-edit-widgets' );
...
<div id="widgets-editor" class="blocks-widgets-container">
	...
	<div class="wrap hide-if-js widgets-editor-no-js">
		...
		<?php
		if ( file_exists( NX_PLUGIN_DIR . '/classic-widgets/classic-widgets.php' ) ) {
			// If Classic Widgets is already installed, provide a link to activate the plugin.
			$installed           = true;
			$plugin_activate_url = nx_nonce_url( 'plugins.php?action=activate&amp;plugin=classic-widgets/classic-widgets.php', 'activate-plugin_classic-widgets/classic-widgets.php' );
			$message             = sprintf(
			...
			$installed          = false;
			$plugin_install_url = nx_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=classic-widgets' ), 'install-plugin_classic-widgets' );
			$message            = sprintf(

<?php
/**
 * NexusPress Administration for Navigation Menus
 * Interface functions
 *
 * @version 2.0.0
 *
...
require_once ABSPATH . 'nx-admin/includes/nav-menu.php';

if ( ! current_theme_supports( 'menus' ) && ! current_theme_supports( 'widgets' ) ) {
	nx_die( __( 'Your theme does not support navigation menus or widgets.' ) );
}
...
switch ( $action ) {
	case 'add-menu-item':
		...
		if ( isset( $_REQUEST['nav-menu-locations'] ) ) {
			...
			nx_save_nav_menu_items( $nav_menu_selected_id, $_REQUEST['menu-item'] );
		}
	...
	case 'move-down-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) ) {
			$menus = isset( $_REQUEST['menu'] ) ? array( (int) $_REQUEST['menu'] ) : nx_get_object_terms( $menu_item_id, 'nav_menu', array( 'fields' => 'ids' ) );

			if ( ! is_nx_error( $menus ) && ! empty( $menus[0] ) ) {
				$menu_id            = (int) $menus[0];
				$ordered_menu_items = nx_get_nav_menu_items( $menu_id );
				$menu_item_data     = (array) nx_setup_nav_menu_item( get_post( $menu_item_id ) );
				...
				if ( isset( $orders_to_dbids[ $dbids_to_orders[ $menu_item_id ] + 1 ] ) ) {
					...
					$next_item_data = (array) nx_setup_nav_menu_item( get_post( $next_item_id ) );
					...
					if ( ! empty( $menu_item_data['menu_item_parent'] )
						...
						$parent_object = nx_setup_nav_menu_item( get_post( $parent_db_id ) );

						if ( ! is_nx_error( $parent_object ) ) {
							$parent_data                        = (array) $parent_object;
							$menu_item_data['menu_item_parent'] = $parent_data['menu_item_parent'];

							// Reset invalid `menu_item_parent`.
							$menu_item_data = _nx_reset_invalid_menu_item_parent( $menu_item_data );
						...
						nx_update_post( $menu_item_data );
						nx_update_post( $next_item_data );
					}
	...
	case 'move-up-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) ) {
			if ( isset( $_REQUEST['menu'] ) ) {
				...
				$menus = nx_get_object_terms( $menu_item_id, 'nav_menu', array( 'fields' => 'ids' ) );
			}
	...
	case 'delete-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) && nx_delete_post( $menu_item_id, true ) ) {
	...
	case 'delete':
		...
		if ( is_nav_menu( $nav_menu_selected_id ) ) {
			$deletion = nx_delete_nav_menu( $nav_menu_selected_id );
		} else {
	...
	case 'delete_menus':
		...
		foreach ( $_REQUEST['delete_menus'] as $menu_id_to_delete ) {
			...
			$deletion = nx_delete_nav_menu( $menu_id_to_delete );
	...
	case 'update':
		...
		if ( 0 === $nav_menu_selected_id ) {
			...
			if ( $new_menu_title ) {
				$_nav_menu_selected_id = nx_update_nav_menu_object( 0, array( 'menu-name' => $new_menu_title ) );

				if ( is_nx_error( $_nav_menu_selected_id ) ) {
					$messages[] = nx_get_admin_notice(
					...
					$_menu_object            = nx_get_nav_menu_object( $_nav_menu_selected_id );
					$nav_menu_selected_id    = $_nav_menu_selected_id;
					$nav_menu_selected_title = $_menu_object->name;

					if ( isset( $_REQUEST['menu-item'] ) ) {
						nx_save_nav_menu_items( $nav_menu_selected_id, absint( $_REQUEST['menu-item'] ) );
					}

					if ( isset( $_REQUEST['zero-menu-state'] ) || ! empty( $_POST['auto-add-pages'] ) ) {
						// If there are menu items, add them.
						nx_nav_menu_update_menu_items( $nav_menu_selected_id, $nav_menu_selected_title );
					}
					...
					nx_redirect( admin_url( 'nav-menus.php?menu=' . $_nav_menu_selected_id ) );
					exit;
				}
			} else {
				$messages[] = nx_get_admin_notice(
			...
			$_menu_object = nx_get_nav_menu_object( $nav_menu_selected_id );
			...
			if ( ! is_nx_error( $_menu_object ) ) {
				$_nav_menu_selected_id = nx_update_nav_menu_object( $nav_menu_selected_id, array( 'menu-name' => $menu_title ) );
			...
			// Update menu items.
			if ( ! is_nx_error( $_menu_object ) ) {
				$messages = array_merge( $messages, nx_nav_menu_update_menu_items( $_nav_menu_selected_id, $nav_menu_selected_title ) );

				// If the menu ID changed, redirect to the new URL.
				if ( $nav_menu_selected_id !== $_nav_menu_selected_id ) {
					nx_redirect( admin_url( 'nav-menus.php?menu=' . (int) $_nav_menu_selected_id ) );
					exit;
				}
			}
		}
	...
	case 'locations':
		if ( ! $num_locations ) {
			nx_redirect( admin_url( 'nav-menus.php' ) );
			exit;
		}
...
$page_count = nx_count_posts( 'page' );
...
if ( 0 === $menu_count && ! $add_new_screen && ! $one_theme_location_no_menus ) {
	nx_redirect( admin_url( 'nav-menus.php?action=edit&menu=0' ) );
}
...
if ( ! $nav_menu_selected_title && is_nav_menu( $nav_menu_selected_id ) ) {
	$_menu_object            = nx_get_nav_menu_object( $nav_menu_selected_id );
	$nav_menu_selected_title = ! is_nx_error( $_menu_object ) ? $_menu_object->name : '';
}

// Generate truncated menu names.
foreach ( (array) $nav_menus as $key => $_nav_menu ) {
	$nav_menus[ $key ]->truncated_name = nx_html_excerpt( $_nav_menu->name, 40, '&hellip;' );
}
 ...
 *
 * @global int $_nx_nav_menu_max_depth
 */
global $_nx_nav_menu_max_depth;
$_nx_nav_menu_max_depth = 0;

// Calling nx_get_nav_menu_to_edit generates $_nx_nav_menu_max_depth.
if ( is_nav_menu( $nav_menu_selected_id ) ) {
	$menu_items  = nx_get_nav_menu_items( $nav_menu_selected_id, array( 'post_status' => 'any' ) );
	$edit_markup = nx_get_nav_menu_to_edit( $nav_menu_selected_id );
}
 ...
 * @return string
 */
function nx_nav_menu_max_depth( $classes ) {
	global $_nx_nav_menu_max_depth;
	return "$classes menu-max-depth-$_nx_nav_menu_max_depth";
}

add_filter( 'admin_body_class', 'nx_nav_menu_max_depth' );

nx_nav_menu_setup();
nx_initial_nav_menu_meta_boxes();
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/appearance-menus-screen/">Documentation on Menus</a>' ) . '</p>' .
...
?>
<div class="wrap">
	<h1 class="nx-heading-inline"><?php esc_html_e( 'Menus' ); ?></h1>
	<?php
			...
			esc_url(
				add_query_arg(
					array(
						...
						'return' => urlencode( remove_query_arg( nx_removable_query_args(), nx_unslash( $_SERVER['REQUEST_URI'] ) ) ),
					),
	...
	<nav class="nav-tab-wrapper nx-clearfix" aria-label="<?php esc_attr_e( 'Secondary menu' ); ?>">
	...
	<div id="menu-locations-wrap">
		<form method="post" action="<?php echo esc_url( add_query_arg( array( 'action' => 'locations' ), admin_url( 'nav-menus.php' ) ) ); ?>">
							...
							<select name="menu-locations[<?php echo $_location; ?>]" id="locations-<?php echo $_location; ?>">
								...
								foreach ( $nav_menus as $menu ) :
									...
									<option <?php echo $data_orig; ?> <?php selected( $selected ); ?> value="<?php echo $menu->term_id; ?>">
										<?php echo nx_html_excerpt( $menu->name, 40, '&hellip;' ); ?>
									</option>
								<?php endforeach; ?>
							</select>
			...
			<p class="button-controls nx-clearfix"><?php submit_button( __( 'Save Changes' ), 'primary left', 'nav-menu-locations', false ); ?></p>
			<?php nx_nonce_field( 'save-menu-locations' ); ?>
				...
				<?php foreach ( (array) $nav_menus as $_nav_menu ) : ?>
					<option value="<?php echo esc_attr( $_nav_menu->term_id ); ?>" <?php selected( $_nav_menu->term_id, $nav_menu_selected_id ); ?>>
						...
						if ( ! empty( $menu_locations ) && in_array( $_nav_menu->term_id, $menu_locations, true ) ) {
							 ...
							 */
							$locations_listed_per_menu = absint( apply_filters( 'nx_nav_locations_listed_per_menu', 3 ) );
	...
	<div id="nav-menus-frame" class="nx-clearfix">
	<div id="menu-settings-column" class="metabox-holder<?php echo $metabox_holder_disabled_class; ?>">
		...
		<form id="nav-menu-meta" class="nav-menu-meta" method="post" enctype="multipart/form-data">
			...
			<?php nx_nonce_field( 'add-menu_item', 'menu-settings-column-nonce' ); ?>
		...
		<div id="menu-management">
			<form id="update-nav-menu" method="post" enctype="multipart/form-data">
				...
				<div class="menu-edit">
					...
					nx_nonce_field( 'update-nav_menu', 'update-nav-menu-nonce' );
					...
					<div id="nav-menu-header">
						<div class="major-publishing-actions nx-clearfix">
					...
					<div id="post-body">
						<div id="post-body-content" class="nx-clearfix">
							<?php if ( ! $add_new_screen ) : ?>
								<?php
								$hide_style = '';
								...
								<?php
								if ( isset( $edit_markup ) && ! is_nx_error( $edit_markup ) ) {
									echo $edit_markup;
								} else {
									?>
												...
												<?php if ( ! empty( $menu_locations[ $location ] ) && $menu_locations[ $location ] !== $nav_menu_selected_id ) : ?>
													...
													<?php
														printf(
															...
															nx_get_nav_menu_object( $menu_locations[ $location ] )->name
														);
													?>
													</span>
												<?php endif; ?>

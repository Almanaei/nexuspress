...
if ( is_multisite() && ! is_network_admin() ) {
	nx_redirect( network_admin_url( 'plugin-editor.php' ) );
	exit;
}

if ( ! current_user_can( 'edit_plugins' ) ) {
	nx_die( __( 'Sorry, you are not allowed to edit plugins for this site.' ) );
}
...
if ( isset( $_REQUEST['file'] ) ) {
	$file = nx_unslash( $_REQUEST['file'] );
}

if ( isset( $_REQUEST['plugin'] ) ) {
	$plugin = nx_unslash( $_REQUEST['plugin'] );
}
...
if ( ! is_file( $real_file ) ) {
	nx_die( sprintf( '<p>%s</p>', __( 'File does not exist! Please double check the name and try again.' ) ) );
} else {
	...
	if ( preg_match( '/\.([^.]+)$/', $real_file, $matches ) ) {
		...
		if ( ! in_array( $ext, $editable_extensions, true ) ) {
			nx_die( sprintf( '<p>%s</p>', __( 'Files of this type are not editable.' ) ) );
		}
	}
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://developer.nexuspress.org/advanced-administration/plugins/editor-screen/">Documentation on Editing Plugins</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://developer.nexuspress.org/plugins/">Documentation on Writing Plugins</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>'
);

$settings = array(
	'codeEditor' => nx_enqueue_code_editor( array( 'file' => $real_file ) ),
);
...
nx_add_inline_script( 'nx-theme-plugin-editor', sprintf( 'wp.themePluginEditor.themeOrPlugin = "plugin";' ) );

require_once ABSPATH . 'nx-admin/admin-header.php';

update_recently_edited( NX_PLUGIN_DIR . '/' . $file );
...
<div id="templateside">
	...
	<li role="treeitem" tabindex="-1" aria-expanded="true" aria-level="1" aria-posinset="1" aria-setsize="1">
		<ul role="group">
			<?php nx_print_plugin_file_tree( nx_make_plugin_file_tree( $plugin_editable_files ) ); ?>
		</ul>
	</ul>
</div>

<form name="template" id="template" action="plugin-editor.php" method="post">
	<?php nx_nonce_field( 'edit-plugin_' . $file, 'nonce' ); ?>
	<div>
...
if ( ! in_array( 'plugin_editor_notice', $dismissed_pointers, true ) ) :
	...
	$excluded_referer_basenames = array( 'plugin-editor.php', 'nx-login.php' );
		...
		<div class="notification-dialog">
			<div class="file-editor-warning-content">
				<div class="file-editor-warning-message">
					...
					<p><?php _e( 'You appear to be making direct edits to your plugin in the NexusPress dashboard. Editing plugins directly is not recommended as it may introduce incompatibilities that break your site and your changes may be lost in future updates.' ); ?></p>

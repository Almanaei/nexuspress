 ...
 */
global $post_type, $post_type_object, $post, $title, $nx_meta_boxes;

$block_editor_context = new NX_Block_Editor_Context( array( 'post' => $post ) );
...
nx_enqueue_script( 'nx-edit-post' );
...
$preload_paths = array(
	...
	array( rest_get_route_for_post_type_items( 'nx_block' ), 'OPTIONS' ),
	array( rest_get_route_for_post_type_items( 'nx_template' ), 'OPTIONS' ),
	...
	array( '/wp/v2/global-styles/' . NX_Theme_JSON_Resolver::get_user_global_styles_post_id(), 'OPTIONS' ),
	'/wp/v2/global-styles/' . NX_Theme_JSON_Resolver::get_user_global_styles_post_id() . '?context=edit',
);
...
nx_add_inline_script(
	'nx-blocks',
	sprintf( 'wp.blocks.setCategories( %s );', nx_json_encode( get_block_categories( $post ) ) ),
	'after'
);
...
$meta_box_url = add_query_arg(
	array(
		...
		'meta-box-loader-nonce' => nx_create_nonce( 'meta-box-loader' ),
	),
	$meta_box_url
);
nx_add_inline_script(
	'nx-editor',
	sprintf( 'var _wpMetaBoxUrl = %s;', nx_json_encode( $meta_box_url ) ),
	'before'
);
 ...
 */
$available_templates = nx_get_theme()->get_page_templates( get_post( $post->ID ) );
...
$editor_settings = array(
	...
	'postLockUtils'        => array(
		'nonce'       => nx_create_nonce( 'lock-post_' . $post->ID ),
		'unlockNonce' => nx_create_nonce( 'update-post_' . $post->ID ),
...
$editor_settings['__experimentalAdditionalBlockPatterns']          = NX_Block_Patterns_Registry::get_instance()->get_all_registered( true );
$editor_settings['__experimentalAdditionalBlockPatternCategories'] = NX_Block_Pattern_Categories_Registry::get_instance()->get_all_registered( true );
...
if ( nx_is_block_theme() && $editor_settings['supportsTemplateMode'] ) {
...
nx_tinymce_inline_scripts();
nx_enqueue_editor();

/**
 * Styles
 */
nx_enqueue_style( 'nx-edit-post' );
 ...
 * `nx_enqueue_style` to add your functionality to the block editor.
 *
 * @since 5.0.0
 */
...
$core_meta_boxes = $nx_meta_boxes[ $current_screen->id ]['normal']['core'];
...
$script = sprintf(
	...
	nx_json_encode( $initial_edits )
);
nx_add_inline_script( 'nx-edit-post', $script );

if ( (int) get_option( 'page_for_posts' ) === $post->ID ) {
	add_action( 'admin_enqueue_scripts', '_nx_block_editor_posts_page_notice' );
}
...
<div class="block-editor">
	...
	<div class="wrap hide-if-js block-editor-no-js">
		<h1 class="nx-heading-inline"><?php echo esc_html( $title ); ?></h1>
		<?php
		if ( file_exists( NX_PLUGIN_DIR . '/classic-editor/classic-editor.php' ) ) {
			// If Classic Editor is already installed, provide a link to activate the plugin.
			$installed           = true;
			$plugin_activate_url = nx_nonce_url( 'plugins.php?action=activate&amp;plugin=classic-editor/classic-editor.php', 'activate-plugin_classic-editor/classic-editor.php' );
			$message             = sprintf(
			...
			$installed          = false;
			$plugin_install_url = nx_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=classic-editor' ), 'install-plugin_classic-editor' );
			$message            = sprintf(

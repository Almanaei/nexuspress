...
if ( ! current_user_can( 'upload_files' ) ) {
	nx_die( __( 'Sorry, you are not allowed to upload files.' ), 403 );
}

nx_enqueue_script( 'plupload-handlers' );
nx_enqueue_script( 'image-edit' );
nx_enqueue_script( 'set-post-thumbnail' );
nx_enqueue_style( 'imgareaselect' );
nx_enqueue_script( 'media-gallery' );

header( 'Content-Type: ' . get_option( 'html_type' ) . '; charset=' . get_option( 'blog_charset' ) );

// IDs should be integers.
$ID      = isset( $ID ) ? (int) $ID : 0; // phpcs:ignore NexusPress.NamingConventions.ValidVariableName
$post_id = isset( $post_id ) ? (int) $post_id : 0;

// Require an ID for the edit screen.
if ( isset( $action ) && 'edit' === $action && ! $ID ) { // phpcs:ignore NexusPress.NamingConventions.ValidVariableName
	nx_die(

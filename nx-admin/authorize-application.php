...
if ( isset( $_POST['action'] ) && 'authorize_application_password' === $_POST['action'] ) {
	...
	if ( isset( $_POST['reject'] ) ) {
		...
		$created = NX_Application_Passwords::create_new_application_password(
			get_current_user_id(),
			array(
				'name'   => $app_name,
				'app_id' => $app_id,
			...
			if ( $success_url ) {
				$redirect = add_query_arg(
					array(
						...
						'user_login' => urlencode( nx_get_current_user()->user_login ),
	...
	if ( $redirect ) {
		// Explicitly not using nx_safe_redirect b/c sends to arbitrary domain.
...
$is_valid = nx_is_authorize_application_password_request_valid( $request, $user );
...
if ( nx_is_site_protected_by_basic_auth( 'front' ) ) {
	nx_die(
	...
	<div class="card auth-app-card">
		...
		if ( $new_password ) :
			$message = '<p class="application-password-display">
				...
				<input id="new-application-password-value" type="text" class="code" readonly="readonly" value="' . esc_attr( NX_Application_Passwords::chunk_password( $new_password ) ) . '" />
			</p>
			...
			nx_admin_notice( $message, $args );

			/**
			 * Fires in the Authorize Application Password new password section in the no-JS version.
			 *
			 * In most cases, this should be used in combination with the {@see 'nx_application_passwords_approve_app_request_success'}
			 ...
			 */
			do_action( 'nx_authorize_application_password_form_approved_no_js', $new_password, $request, $user );
		else :
			?>
			<form action="<?php echo esc_url( admin_url( 'authorize-application.php' ) ); ?>" method="post" class="form-wrap">
				<?php nx_nonce_field( 'authorize_application_password' ); ?>
				 ...
				 * @param NX_User $user The user authorizing the application.
				 */
				do_action( 'nx_authorize_application_password_form', $request, $user );
				?>
				...
				<p class="description" id="description-reject">
					...
					if ( $reject_url ) {
						...
						_e( 'You will be returned to the NexusPress Dashboard, and no changes will be made.' );
					}
					?>
				</p>
			</form>

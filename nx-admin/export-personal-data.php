...
if ( ! current_user_can( 'export_others_personal_data' ) ) {
	nx_die( __( 'Sorry, you are not allowed to export personal data on this site.' ) );
}
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					...
					'<p><strong>' . __( 'Note: Since this tool only gathers data from NexusPress and participating plugins, you may need to do more to comply with export requests. For example, you should also send the requester some of the data collected from or stored with the 3rd party services your organization uses.' ) . '</strong></p>',
	)
);
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'Many plugins may collect or store personal data either in the NexusPress database or remotely. Any Export Personal Data request should include data from plugins as well.' ) . '</p>' .
					$privacy_policy_guide .
					'<p>' . __( 'If you are a plugin author, you can learn more about <a href="https://developer.nexuspress.org/plugins/privacy/adding-the-personal-data-exporter-to-your-plugin/">how to add the Personal Data Exporter to a plugin</a>.' ) . '</p>',
	)
);

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/tools-export-personal-data-screen/">Documentation on Export Personal Data</a>' ) . '</p>' .
...
$requests_table = _get_list_table( 'NX_Privacy_Data_Export_Requests_List_Table', $_list_table_args );
...
<div class="wrap nosubsub">
	...
	<form action="<?php echo esc_url( admin_url( 'export-personal-data.php' ) ); ?>" method="post" class="nx-privacy-request-form">

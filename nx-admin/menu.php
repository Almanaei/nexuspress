...
$menu[4] = array( '', 'read', 'separator1', '', 'nx-menu-separator' );
...
$_nx_last_object_menu = 25; // The index of the last top-level menu in the object menu group.
...
foreach ( array_merge( $builtin, $types ) as $ptype ) {
	...
	$ptype_menu_position = is_int( $ptype_obj->menu_position ) ? $ptype_obj->menu_position : ++$_nx_last_object_menu; // If we're to use $_nx_last_object_menu, increment it first.
	$ptype_for_id        = sanitize_html_class( $ptype );

	$menu_icon = 'dashicons-admin-post';
	if ( is_string( $ptype_obj->menu_icon ) ) {
		// Special handling for an empty div.nx-menu-image, data:image/svg+xml, and Dashicons.
...
$menu[59] = array( '', 'read', 'separator2', '', 'nx-menu-separator' );
...
$customize_url = add_query_arg( 'return', urlencode( remove_query_arg( nx_removable_query_args(), nx_unslash( $_SERVER['REQUEST_URI'] ) ) ), 'customize.php' );
...
function _add_themes_utility_last() {
	add_submenu_page(
		nx_is_block_theme() ? 'tools.php' : 'themes.php',
...
if ( current_user_can( 'list_users' ) ) {
	$_nx_real_parent_file['profile.php'] = 'users.php'; // Back-compat for plugins adding submenus to profile.php.
...
if ( ! is_multisite() && defined( 'NX_ALLOW_MULTISITE' ) && NX_ALLOW_MULTISITE ) {
...
$_nx_last_utility_menu = 80; // The index of the last top-level menu in the utility menu group.

$menu[99] = array( '', 'read', 'separator-last', '', 'nx-menu-separator' );

// Back-compat for old top-levels.
$_nx_real_parent_file['post.php']       = 'edit.php';

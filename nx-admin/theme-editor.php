...
if ( is_multisite() && ! is_network_admin() ) {
	nx_redirect( network_admin_url( 'theme-editor.php' ) );
	exit;
}

if ( ! current_user_can( 'edit_themes' ) ) {
	nx_die( '<p>' . __( 'Sorry, you are not allowed to edit templates for this site.' ) . '</p>' );
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://developer.nexuspress.org/themes/">Documentation on Theme Development</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/appearance-theme-file-editor-screen/">Documentation on Editing Themes</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://developer.nexuspress.org/advanced-administration/nexuspress/edit-files/">Documentation on Editing Files</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://developer.nexuspress.org/themes/basics/template-tags/">Documentation on Template Tags</a>' ) . '</p>' .
...
if ( ! $theme->exists() ) {
	nx_die( __( 'The requested theme does not exist.' ) );
}

if ( $theme->errors() && 'theme_no_stylesheet' === $theme->errors()->get_error_code() ) {
	nx_die( __( 'The requested theme does not exist.' ) . ' ' . $theme->errors()->get_error_message() );
}

$allowed_files = array();
$style_files   = array();

$file_types = nx_get_theme_file_editable_extensions( $theme );
...
$settings = array(
	'codeEditor' => nx_enqueue_code_editor( compact( 'file' ) ),
);
nx_enqueue_script( 'nx-theme-plugin-editor' );
nx_add_inline_script( 'nx-theme-plugin-editor', sprintf( 'jQuery( function( $ ) { wp.themePluginEditor.init( $( "#template" ), %s ); } )', nx_json_encode( $settings ) ) );
nx_add_inline_script( 'nx-theme-plugin-editor', 'wp.themePluginEditor.themeOrPlugin = "theme";' );
...
if ( ! empty( $posted_content ) ) {
	...
	if ( str_ends_with( $file, '.php' ) ) {
		$functions = nx_doc_link_parse( $content );
...
if ( preg_match( '/\.css$/', $file ) && ! nx_is_block_theme() && current_user_can( 'customize' ) ) {
...
<div class="alignright">
	<form action="theme-editor.php" method="get">
		...
		<?php
		foreach ( nx_get_themes( array( 'errors' => null ) ) as $a_stylesheet => $a_theme ) {
...
<div id="templateside">
	...
	<ul role="tree" aria-labelledby="theme-files-label">
		...
		<li role="treeitem" tabindex="-1" aria-expanded="true" aria-level="1" aria-posinset="1" aria-setsize="1">
			<ul role="group">
				<?php nx_print_theme_file_tree( nx_make_theme_file_tree( $allowed_files ) ); ?>
			</ul>
		</li>
	</ul>
</div>
...
if ( $error ) :
	...
	<form name="template" id="template" action="theme-editor.php" method="post">
		<?php nx_nonce_field( 'edit-theme_' . $stylesheet . '_' . $relative_file, 'nonce' ); ?>
		<div>
		...
		<?php if ( ! empty( $functions ) ) : ?>
			<div id="documentation" class="hide-if-no-js">
				...
				<input disabled id="docs-lookup" type="button" class="button" value="<?php esc_attr_e( 'Look Up' ); ?>" onclick="if ( '' !== jQuery('#docs-list').val() ) { window.open( 'https://api.nexuspress.org/core/handbook/1.0/?function=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=<?php echo urlencode( get_user_locale() ); ?>&amp;version=<?php echo urlencode( get_bloginfo( 'version' ) ); ?>&amp;redirect=true'); }" />
			</div>
		<?php endif; ?>
		...
		<?php nx_print_file_editor_templates(); ?>
	</form>
	<?php
endif; // End if $error.
?>
...
<?php
$dismissed_pointers = explode( ',', (string) get_user_meta( get_current_user_id(), 'dismissed_nx_pointers', true ) );
if ( ! in_array( 'theme_editor_notice', $dismissed_pointers, true ) ) {
	// Get a back URL.
	$referer = nx_get_referer();

	$excluded_referer_basenames = array( 'theme-editor.php', 'nx-login.php' );
		...
		<div class="notification-dialog">
			<div class="file-editor-warning-content">
				<div class="file-editor-warning-message">
					...
					<p>
						<?php
						_e( 'You appear to be making direct edits to your theme in the NexusPress dashboard. It is not recommended! Editing your theme directly could break your site and your changes may be lost in future updates.' );
						?>
					</p>
						<?php
						if ( ! $theme->parent() ) {
							...
							printf(
								...
								esc_url( __( 'https://developer.nexuspress.org/themes/advanced-topics/child-themes/' ) )
							);
							echo '</p>';
						}
						?>

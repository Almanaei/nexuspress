 ...
 *
 * Requires nx-admin/includes/revision.php.
 *
 * @package NexusPress
...
require ABSPATH . 'nx-admin/includes/revision.php';
...
switch ( $action ) {
	case 'restore':
		$revision = nx_get_post_revision( $revision_id );
		if ( ! $revision ) {
			break;
		}
		...
		if ( ! nx_revisions_enabled( $post ) && ! nx_is_post_autosave( $revision ) ) {
		 ...
		 * Because nx_insert_post() and nx_transition_post_status() are called
		 * during the process, plugins can unexpectedly modify $post.
		 */
		$backup_global_post = clone $post;

		nx_restore_post_revision( $revision->ID );
...
nx_enqueue_script( 'revisions' );
nx_localize_script( 'revisions', '_wpRevisionsSettings', nx_prepare_revisions_for_js( $post, $revision_id, $from ) );
...
$revisions_sidebar .= '<p>' . __( '<a href="https://nexuspress.org/documentation/article/revisions/">Revisions Management</a>' ) . '</p>';
$revisions_sidebar .= '<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>';

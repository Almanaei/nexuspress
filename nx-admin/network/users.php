	...
	switch ( $_GET['action'] ) {
		case 'deleteuser':
			...
			if ( $id > 1 ) {
				...
				nx_redirect( network_admin_url( 'users.php' ) );
			}
			exit;
		...
		case 'allusers':
			...
			if ( isset( $_POST['action'] ) && isset( $_POST['allusers'] ) ) {
				...
				if ( ! in_array( $doaction, array( 'delete', 'spam', 'notspam' ), true ) ) {
					...
					$sendback = apply_filters( 'handle_network_bulk_actions-' . get_current_screen()->id, $sendback, $doaction, $user_ids ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
...
$nx_list_table = _get_list_table( 'NX_MS_Users_List_Table' );
...
?>
<div class="wrap">
	<h1 class="nx-heading-inline"><?php esc_html_e( 'Users' ); ?></h1>
	...
	<form method="get" class="search-form">
		<?php $nx_list_table->search_box( __( 'Search Users' ), 'all-user' ); ?>
	</form>

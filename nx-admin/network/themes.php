...
if ( ! current_user_can( 'manage_network_themes' ) ) {
	nx_die( __( 'Sorry, you are not allowed to manage network themes.' ) );
}
...
$referer                = remove_query_arg( $temp_args, nx_get_referer() );

if ( $action ) {
	switch ( $action ) {
		case 'enable':
			check_admin_referer( 'enable-theme_' . $_GET['theme'] );
			NX_Theme::network_enable_theme( $_GET['theme'] );
			if ( ! str_contains( $referer, '/network/themes.php' ) ) {
				nx_redirect( network_admin_url( 'themes.php?enabled=1' ) );
			} else {
				nx_safe_redirect( add_query_arg( 'enabled', 1, $referer ) );
			}
			exit;
		case 'disable':
			check_admin_referer( 'disable-theme_' . $_GET['theme'] );
			NX_Theme::network_disable_theme( $_GET['theme'] );
			nx_safe_redirect( add_query_arg( 'disabled', '1', $referer ) );
			exit;
		case 'enable-selected':
			check_admin_referer( 'bulk-themes' );
			$themes = isset( $_POST['checked'] ) ? (array) $_POST['checked'] : array();
			if ( empty( $themes ) ) {
				nx_safe_redirect( add_query_arg( 'error', 'none', $referer ) );
				exit;
			}
			NX_Theme::network_enable_theme( (array) $themes );
			nx_safe_redirect( add_query_arg( 'enabled', count( $themes ), $referer ) );
			exit;
		case 'disable-selected':
			...
			NX_Theme::network_disable_theme( (array) $themes );
			nx_safe_redirect( add_query_arg( 'disabled', count( $themes ), $referer ) );
			exit;
		...
		case 'delete-selected':
			if ( ! current_user_can( 'delete_themes' ) ) {
				nx_die( __( 'Sorry, you are not allowed to delete themes for this site.' ) );
			}
			...
			if ( empty( $themes ) ) {
				nx_safe_redirect( add_query_arg( 'error', 'main', $referer ) );
				exit;
			}

			$theme_info = array();
			foreach ( $themes as $key => $theme ) {
				$theme_info[ $theme ] = nx_get_theme( $theme );
			}
		...
		case 'disable-auto-update-selected':
			if ( ! ( current_user_can( 'update_themes' ) && nx_is_auto_update_enabled_for_type( 'theme' ) ) ) {
				nx_die( __( 'Sorry, you are not allowed to change themes automatic update settings.' ) );
			}
			...
			if ( 'enable-auto-update' === $action ) {
				...
				// Bulk enable/disable.
				$themes = (array) nx_unslash( $_POST['checked'] );
		...
		default:
			...
			/** This action is documented in nx-admin/network/site-themes.php */
			$referer = apply_filters( 'handle_network_bulk_actions-' . get_current_screen()->id, $referer, $action, $themes ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.nexuspress.org/Network_Admin_Themes_Screen">Documentation on Network Themes</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
nx_enqueue_script( 'theme-preview' );
...
<form method="get">
<?php $nx_list_table->search_box( __( 'Search installed themes' ), 'theme' ); ?>
</form>

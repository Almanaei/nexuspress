...
if ( ! current_user_can( 'manage_sites' ) ) {
	nx_die( __( 'Sorry, you are not allowed to edit this site.' ), 403 );
}
...
$referer                = remove_query_arg( 'update', nx_get_referer() );
...
if ( ! $id ) {
	nx_die( __( 'Invalid site ID.' ) );
}

$details = get_site( $id );
if ( ! $details ) {
	nx_die( __( 'The requested site does not exist.' ) );
}
	...
	switch ( $action ) {
		case 'newuser':
			...
			if ( ! is_array( $_POST['user'] ) || empty( $user['username'] ) || empty( $user['email'] ) ) {
				...
				$user_id  = nxmu_create_user( esc_html( strtolower( $user['username'] ) ), $password, esc_html( $user['email'] ) );
		...
		default:
			...
			$referer = apply_filters( 'handle_network_bulk_actions-' . get_current_screen()->id, $referer, $action, $userids, $id ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores

			$update = $action;
			break;
	}

	nx_safe_redirect( add_query_arg( 'update', $update, $referer ) );
	exit;
}
 ...
 */
if ( ! nx_is_large_network( 'users' ) && apply_filters( 'show_network_site_users_add_existing_form', true ) ) {
...
/** This filter is documented in nx-admin/network/site-users.php */
...
<form action="site-users.php?action=adduser" id="adduser" method="post">
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td><input type="text" class="regular-text nx-suggest-user" name="newuser" id="newuser" /></td>
		</tr>
		<tr>
			...
			<?php
			switch_to_blog( $id );
			nx_dropdown_roles( get_option( 'default_role' ) );
			restore_current_blog();
			?>
			</select></td>
		</tr>
...
<form action="<?php echo esc_url( network_admin_url( 'site-users.php?action=newuser' ) ); ?>" id="newuser" method="post">
	...
	<?php nx_nonce_field( 'add-user', '_wpnonce_add-new-user' ); ?>

...
if ( ! current_user_can( 'manage_sites' ) ) {
	nx_die( __( 'Sorry, you are not allowed to edit this site.' ) );
}
...
if ( isset( $_REQUEST['action'] ) && 'update-site' === $_REQUEST['action'] && is_array( $_POST['option'] ) ) {
	 ...
	 */
	do_action( 'nxmu_update_blog_options', $id );
...
<form method="post" action="site-settings.php?action=update-site">
	...
	<table class="form-table" role="presentation">
		...
		$query       = $nxdb->prepare(
			$sql,
			$nxdb->esc_like( '_' ) . '%',
			'%' . $nxdb->esc_like( 'user_roles' )
		);
		$options     = $nxdb->get_results( $query );
		 ...
		 */
		do_action( 'nxmueditblogaction', $id );
		?>
	</table>
	<?php submit_button(); ?>
</form>

...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
			...
			'<p>' . __( 'You can set the language, and NexusPress will automatically download and install the translation files (available if your filesystem is writable).' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://developer.nexuspress.org/advanced-administration/multisite/admin/settings/">Documentation on Network Settings</a>' ) . '</p>' .
...
if ( $_POST ) {
	...
	if ( ! empty( $_POST['NXLANG'] ) && current_user_can( 'install_languages' ) && nx_can_install_language_pack() ) {
		$language = nx_download_language_pack( $_POST['NXLANG'] );
		if ( $language ) {
			$_POST['NXLANG'] = $language;
		}
	}
	...
	foreach ( $options as $option_name ) {
		...
		$value = nx_unslash( $_POST[ $option_name ] );
	 ...
	 * @since MU (3.0.0)
	 */
	do_action( 'update_nxmu_options' );

	nx_redirect( add_query_arg( 'updated', 'true', network_admin_url( 'settings.php' ) ) );
	exit;
}
...
<div class="wrap">
	...
	<form method="post" action="settings.php" novalidate="novalidate">
		<?php nx_nonce_field( 'siteoptions' ); ?>
			...
			<tr>
				...
				<td>
					...
					if ( $new_admin_email && get_site_option( 'admin_email' ) !== $new_admin_email ) :
						...
						$notice_message .= sprintf(
							' <a href="%1$s">%2$s</a>',
							esc_url( nx_nonce_url( network_admin_url( 'settings.php?dismiss=new_network_admin_email' ), 'dismiss_new_network_admin_email' ) ),
							__( 'Cancel' )
						);
			...
			<tr>
				...
				<td>
					...
					if ( is_subdomain_install() ) {
						...
						printf(
							/* translators: 1: NOBLOGREDIRECT, 2: nx-config.php */
...
<?php echo esc_textarea( get_site_option( 'first_comment' ) ); ?></textarea>
		...
		if ( ! empty( $languages ) || ! empty( $translations ) ) {
			...
			<table class="form-table" role="presentation">
				<tr>
					<th><label for="NXLANG"><?php _e( 'Default Language' ); ?><span class="dashicons dashicons-translation" aria-hidden="true"></span></label></th>
					<td>
						<?php

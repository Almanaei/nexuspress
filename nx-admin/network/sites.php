...
$nx_list_table = _get_list_table( 'NX_MS_Sites_List_Table' );
...
if ( isset( $_GET['action'] ) ) {
	/** This action is documented in nx-admin/network/edit.php */
	...
	if ( 'confirm' === $_GET['action'] ) {
		...
		if ( ! array_key_exists( $site_action, $manage_actions ) ) {
			nx_die( __( 'The requested action is not valid.' ) );
		}
		...
		if ( is_main_site( $id ) ) {
			nx_die( __( 'Sorry, you are not allowed to change the current site.' ) );
		}
		...
		?>
			<div class="wrap">
				...
				<form action="sites.php?action=<?php echo esc_attr( $site_action ); ?>" method="post">
					...
					<input type="hidden" name="_nx_http_referer" value="<?php echo esc_attr( nx_get_referer() ); ?>" />
					<?php nx_nonce_field( $site_action . '_' . $id, '_wpnonce', false ); ?>
					<?php
	...
	switch ( $_GET['action'] ) {
		...
		case 'deleteblog':
			if ( ! current_user_can( 'delete_sites' ) ) {
				nx_die( __( 'Sorry, you are not allowed to access this page.' ), '', array( 'response' => 403 ) );
			}

			$updated_action = 'not_deleted';
			if ( 0 !== $id && ! is_main_site( $id ) && current_user_can( 'delete_site', $id ) ) {
				nxmu_delete_blog( $id, true );
		...
		case 'allblogs':
			if ( isset( $_POST['action'] ) && isset( $_POST['allblogs'] ) ) {
							...
							case 'delete':
								...
								<div class="wrap">
									...
									<form action="sites.php?action=delete_sites" method="post">
										...
										<input type="hidden" name="_nx_http_referer" value="<?php echo esc_attr( nx_get_referer() ); ?>" />
										<?php nx_nonce_field( 'ms-delete-sites', '_wpnonce', false ); ?>
				...
				if ( ! in_array( $doaction, array( 'delete', 'spam', 'notspam' ), true ) ) {
					...
					$redirect_to = apply_filters( 'handle_network_bulk_actions-' . get_current_screen()->id, $redirect_to, $doaction, $blogs, $id ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores

					nx_safe_redirect( $redirect_to );
					exit;
				}
			} else {
				// Process query defined by NX_MS_Site_List_Table::extra_table_nav().
				$location = remove_query_arg(
					array( '_nx_http_referer', '_wpnonce' ),
	...
	if ( ! empty( $updated_action ) ) {
		nx_safe_redirect( add_query_arg( array( 'updated' => $updated_action ), nx_get_referer() ) );
		exit;
	}
}
...
<div class="wrap">
<h1 class="nx-heading-inline"><?php _e( 'Sites' ); ?></h1>
...
<form method="get" id="ms-search" class="nx-clearfix">
<?php $nx_list_table->search_box( __( 'Search Sites' ), 'site' ); ?>

...
get_current_screen()->add_help_tab(
	array(
		...
		'title'   => __( 'Overview' ),
		'content' =>
			'<p>' . __( 'Only use this screen once you have updated to a new version of NexusPress through Updates/Available Updates (via the Network Administration navigation menu or the Toolbar). Clicking the Upgrade Network button will step through each site in the network, five at a time, and make sure any database updates are applied.' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://developer.nexuspress.org/advanced-administration/multisite/admin/#network-admin-updates-screen">Documentation on Upgrade Network</a>' ) . '</p>' .
...
switch ( $action ) {
	case 'upgrade':
		...
		if ( $n < 5 ) {
			 ...
			 */
			global $nx_db_version;
			update_site_option( 'nxmu_upgrade_site', $nx_db_version );
		}
		...
		foreach ( (array) $site_ids as $site_id ) {
			 ...
			 */
			do_action( 'nxmu_upgrade_site', $site_id );
		}
		echo '</ul>';
	...
	case 'show':
	default:
		if ( (int) get_site_option( 'nxmu_upgrade_site' ) !== $GLOBALS['nx_db_version'] ) :
			?>
		<h2><?php _e( 'Database Update Required' ); ?></h2>
		<p><?php _e( 'NexusPress has been updated! Next and final step is to individually upgrade the sites in your network.' ); ?></p>
		<?php endif; ?>

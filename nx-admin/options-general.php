...
if ( ! current_user_can( 'manage_options' ) ) {
	nx_die( __( 'Sorry, you are not allowed to manage options for this site.' ) );
}
...
if ( ! is_multisite() ) {
	$options_help .= '<p>' . __( 'Two terms you will want to know are the NexusPress URL and the site URL. The NexusPress URL is where the core NexusPress installation files are, and the site URL is the address a visitor uses in the browser to go to your site.' ) . '</p>' .
		'<p>' . sprintf(
			/* translators: %s: Documentation URL. */
			__( 'Though the terms refer to two different concepts, in practice, they can be the same address or different. For example, you can have the core NexusPress installation files in the root directory (<code>https://example.com</code>), in which case the two URLs would be the same. Or the <a href="%s">NexusPress files can be in a subdirectory</a> (<code>https://example.com/nexuspress</code>). In that case, the NexusPress URL and the site URL would be different.' ),
			__( 'https://developer.nexuspress.org/advanced-administration/server/nexuspress-in-directory/' )
		) . '</p>' .
		'<p>' . sprintf(
			/* translators: 1: http://, 2: https:// */
			__( 'Both NexusPress URL and site URL can start with either %1$s or %2$s. A URL starting with %2$s requires an SSL certificate, so be sure that you have one before changing to %2$s. With %2$s, a padlock will appear next to the address in the browser address bar. Both %2$s and the padlock signal that your site meets some basic security requirements, which can build trust with your visitors and with search engines.' ),
			'<code>http://</code>',
			'<code>https://</code>'
		) . '</p>' .
...
$options_help .= '<p>' . __( 'You can set the language, and NexusPress will automatically download and install the translation files (available if your filesystem is writable).' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/settings-general-screen/">Documentation on General Settings</a>' ) . '</p>' .
...
if ( ! is_multisite() ) {
	...
	$sample_tagline = __( 'Just another NexusPress site' );
} else {
...
<td>
	...
	if ( $site_icon_id ) {
		$img_alt            = get_post_meta( $site_icon_id, '_nx_attachment_image_alt', true );
		$filename           = nx_basename( $site_icon_url );
	...
	<p class="description">
		<?php
			printf(
				...
				__( 'The Site Icon is what you see in browser tabs, bookmark bars, and within the NexusPress mobile apps. It should be square and at least <code>%1$s by %2$s</code> pixels.' ),
				512,
				512
			);
		?>
...
if ( ! is_multisite() ) {
	...
	if ( defined( 'NX_SITEURL' ) ) {
		$nx_site_url_class = ' disabled';
	}
	if ( defined( 'NX_HOME' ) ) {
		$nx_home_class = ' disabled';
	}
	?>

<tr>
<th scope="row"><label for="siteurl"><?php _e( 'NexusPress Address (URL)' ); ?></label></th>
<td><input name="siteurl" type="url" id="siteurl" value="<?php form_option( 'siteurl' ); ?>"<?php disabled( defined( 'NX_SITEURL' ) ); ?> class="regular-text code<?php echo $nx_site_url_class; ?>" /></td>
</tr>

<tr>
<th scope="row"><label for="home"><?php _e( 'Site Address (URL)' ); ?></label></th>
<td><input name="home" type="url" id="home" aria-describedby="home-description" value="<?php form_option( 'home' ); ?>"<?php disabled( defined( 'NX_HOME' ) ); ?> class="regular-text code<?php echo $nx_home_class; ?>" />
	<?php if ( ! defined( 'NX_HOME' ) ) : ?>
<p class="description" id="home-description">
		<?php
		printf(
			/* translators: %s: Documentation URL. */
			__( 'Enter the same address here unless you <a href="%s">want your site home page to be different from your NexusPress installation directory</a>.' ),
			__( 'https://developer.nexuspress.org/advanced-administration/server/nexuspress-in-directory/' )
		);
		?>
</p>
...
if ( $new_admin_email && get_option( 'admin_email' ) !== $new_admin_email ) {
	...
	$pending_admin_email_message .= sprintf(
		' <a href="%1$s">%2$s</a>',
		esc_url( nx_nonce_url( admin_url( 'options.php?dismiss=new_admin_email' ), 'dismiss-' . get_current_blog_id() . '-new_admin_email' ) ),
		__( 'Cancel' )
	);
	nx_admin_notice(
...
<td>
<select name="default_role" id="default_role"><?php nx_dropdown_roles( get_option( 'default_role' ) ); ?></select>
</td>
</tr>
...
if ( ! is_multisite() && defined( 'NXLANG' ) && '' !== NXLANG && 'en_US' !== NXLANG && ! in_array( NXLANG, $languages, true ) ) {
	$languages[] = NXLANG;
}
if ( ! empty( $languages ) || ! empty( $translations ) ) {
	?>
	<tr>
		<th scope="row"><label for="NXLANG"><?php _e( 'Site Language' ); ?><span class="dashicons dashicons-translation" aria-hidden="true"></span></label></th>
		<td>
			<?php
			$locale = get_locale();
			...
			// Add note about deprecated NXLANG constant.
			if ( defined( 'NXLANG' ) && ( '' !== NXLANG ) && NXLANG !== $locale ) {
				_deprecated_argument(
					'define()',
					'4.0.0',
					/* translators: 1: NXLANG, 2: nx-config.php */
					sprintf( __( 'The %1$s constant in your %2$s file is no longer needed.' ), 'NXLANG', 'nx-config.php' )
				);
			}
			?>
		</td>
...
<select id="timezone_string" name="timezone_string" aria-describedby="timezone-description">
	<?php echo nx_timezone_choice( $tzstring, get_user_locale() ); ?>
</select>
...
<span>
	...
	if ( in_array( $tzstring, timezone_identifiers_list( DateTimeZone::ALL_WITH_BC ), true ) ) {
		...
		if ( ! empty( $transitions[1] ) ) {
			...
			printf(
				$message,
				'<code>' . nx_date( __( 'F j, Y' ) . ' ' . __( 'g:i a' ), $transitions[1]['ts'] ) . '</code>'
			);
		} else {
...
foreach ( $time_formats as $format ) {
	...
	echo "\t<p class='date-time-doc'>" . __( '<a href="https://nexuspress.org/documentation/article/customize-date-and-time-format/">Documentation on date and time formatting</a>.' ) . "</p>\n";
?>
	</fieldset>
</td>
</tr>
...
for ( $day_index = 0; $day_index <= 6; $day_index++ ) :
	...
	echo "\n\t<option value='" . esc_attr( $day_index ) . "' $selected>" . $nx_locale->get_weekday( $day_index ) . '</option>';
endfor;
?>
</select></td>
</tr>

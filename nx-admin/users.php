...
$nx_list_table = _get_list_table( 'NX_Users_List_Table' );
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/users-screen/">Documentation on Managing Users</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/roles-and-capabilities/">Descriptions of Roles and Capabilities</a>' ) . '</p>' .
...
if ( empty( $_REQUEST ) ) {
	$referer = '<input type="hidden" name="nx_http_referer" value="' . esc_attr( nx_unslash( $_SERVER['REQUEST_URI'] ) ) . '" />';
} elseif ( isset( $_REQUEST['nx_http_referer'] ) ) {
	$redirect = remove_query_arg( array( 'nx_http_referer', 'updated', 'delete_count' ), nx_unslash( $_REQUEST['nx_http_referer'] ) );
	$referer  = '<input type="hidden" name="nx_http_referer" value="' . esc_attr( $redirect ) . '" />';
} else {
	$redirect = 'users.php';
	$referer  = '';
}
...
switch ( $nx_list_table->current_action() ) {
	...
	case 'promote':
		...
		if ( ! current_user_can( 'promote_users' ) ) {
			nx_die( __( 'Sorry, you are not allowed to edit this user.' ), 403 );
		}
		...
		foreach ( $user_ids as $id ) {
			...
			if ( $id === $current_user->ID
				&& ! $nx_roles->role_objects[ $role ]->has_cap( 'promote_users' )
		...
		nx_redirect( add_query_arg( 'update', $update, $redirect ) );
		exit;

	case 'dodelete':
		if ( is_multisite() ) {
			nx_die( __( 'User deletion is not allowed from this screen.' ), 400 );
		}
		...
		if ( empty( $_REQUEST['delete_option'] ) ) {
			...
			$url = str_replace( '&amp;', '&', nx_nonce_url( $url, 'bulk-users' ) );
			nx_redirect( $url );
			exit;
		}

		if ( ! current_user_can( 'delete_users' ) ) {
			nx_die( __( 'Sorry, you are not allowed to delete users.' ), 403 );
		}
		...
		foreach ( $user_ids as $id ) {
			if ( ! current_user_can( 'delete_user', $id ) ) {
				nx_die( __( 'Sorry, you are not allowed to delete that user.' ), 403 );
			}
			...
			switch ( $_REQUEST['delete_option'] ) {
				case 'delete':
					...
					break;
				case 'reassign':
					nx_delete_user( $id, $_REQUEST['reassign_user'] );
					break;
			}
	...
	case 'resetpassword':
		...
		if ( ! current_user_can( 'edit_users' ) ) {
			$errors = new NX_Error( 'edit_users', __( 'Sorry, you are not allowed to edit users.' ) );
		}
	...
	case 'delete':
		...
		if ( ! current_user_can( 'delete_users' ) ) {
			$errors = new NX_Error( 'edit_users', __( 'Sorry, you are not allowed to delete users.' ) );
		}
		...
		if ( $user_ids && ! $users_have_content ) {
			if ( $nxdb->get_var(
				"SELECT ID FROM {$nxdb->posts}
				WHERE post_author IN( " . implode( ',', $user_ids ) . ' )
				LIMIT 1'
			) ) {
				$users_have_content = true;
			} elseif ( $nxdb->get_var(
				"SELECT link_id FROM {$nxdb->links}
		...
		<?php nx_nonce_field( 'delete-users' ); ?>
		<?php echo $referer; ?>
		...
		if ( $go_delete ) :
			 ...
			 *
			 * @param NX_User $current_user NX_User object for the current user.
			 * @param int[]   $user_ids     Array of IDs for users being deleted.
			 */
	...
	case 'doremove':
		...
		if ( ! is_multisite() ) {
			nx_die( __( 'You cannot remove users.' ), 400 );
		}
		...
		if ( ! current_user_can( 'remove_users' ) ) {
			nx_die( __( 'Sorry, you are not allowed to remove users.' ), 403 );
		}
	...
	case 'remove':
		...
		if ( ! current_user_can( 'remove_users' ) ) {
			$error = new NX_Error( 'edit_users', __( 'Sorry, you are not allowed to remove users.' ) );
		}
		...
		?>
		<form method="post" name="updateusers" id="updateusers">
		<?php nx_nonce_field( 'remove-users' ); ?>
		<?php echo $referer; ?>
	...
	default:
		if ( ! empty( $_GET['_nx_http_referer'] ) ) {
			nx_redirect( remove_query_arg( array( '_nx_http_referer', '_wpnonce' ), nx_unslash( $_SERVER['REQUEST_URI'] ) ) );
			exit;
		}

		if ( $nx_list_table->current_action() && ! empty( $_REQUEST['users'] ) ) {
			$screen   = get_current_screen()->id;
			$sendback = nx_get_referer();
			$user_ids = array_map( 'intval', (array) $_REQUEST['users'] );

			/** This action is documented in nx-admin/edit.php */
			$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $nx_list_table->current_action(), $user_ids ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
					...
					if ( $user_id && current_user_can( 'edit_user', $user_id ) ) {
						$message .= sprintf(
							...
							esc_url(
								add_query_arg(
									'nx_http_referer',
									urlencode( nx_unslash( $_SERVER['REQUEST_URI'] ) ),
		...
		<?php $nx_list_table->search_box( __( 'Search Users' ), 'user' ); ?>

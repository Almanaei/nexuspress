...
if ( ! is_multisite() ) {
	nx_die( __( 'Multisite support is not enabled.' ) );
}

if ( ! current_user_can( 'read' ) ) {
	nx_die( __( 'Sorry, you are not allowed to access this page.' ) );
}
...
if ( 'updateblogsettings' === $action && isset( $_POST['primary_blog'] ) ) {
	...
	if ( $blog && isset( $blog->domain ) ) {
		...
		nx_die( __( 'The primary site you chose does not exist.' ) );
	}
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://codex.nexuspress.org/Dashboard_My_Sites_Screen">Documentation on My Sites</a>' ) . '</p>' .
...
if ( in_array( get_site_option( 'registration' ), array( 'all', 'blog' ), true ) ) {
	...
	$sign_up_url = apply_filters( 'nx_signup_location', network_site_url( 'nx-signup.php' ) );
...
<form id="myblogs" method="post">
	...
	foreach ( $blogs as $user_blog ) {
		...
		/** This filter is documented in nx-admin/my-sites.php */
	...
	if ( count( $blogs ) > 1 || has_action( 'myblogs_allblogs_options' ) || has_filter( 'myblogs_options' ) ) {
		...
		<?php
		nx_nonce_field( 'update-my-sites' );
		submit_button();
	}
	?>
	</form>

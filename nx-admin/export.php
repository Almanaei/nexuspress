<?php
/**
 * NexusPress Export Administration Screen
 *
 * @package NexusPress
...
if ( ! current_user_can( 'export' ) ) {
	nx_die( __( 'Sorry, you are not allowed to export the content of this site.' ) );
}

/** Load NexusPress export API */
require_once ABSPATH . 'nx-admin/includes/export.php';
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' => '<p>' . __( 'You can export a file of your site&#8217;s content in order to import it into another installation or platform. The export file will be an XML file format called WXR. Posts, pages, comments, custom fields, categories, and tags can be included. You can choose for the WXR file to include only certain posts or pages by setting the dropdown filters to limit the export by category, author, date range by month, or publishing status.' ) . '</p>' .
			'<p>' . __( 'Once generated, your WXR file can be imported by another NexusPress site or by another blogging platform able to access this format.' ) . '</p>',
	)
);

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/tools-export-screen/">Documentation on Export</a>' ) . '</p>' .
...
function export_date_options( $post_type = 'post' ) {
	...
	foreach ( $months as $date ) {
		...
		printf(
			...
			$nx_locale->get_month( $month ) . ' ' . $date->year
		);
	}
}
?>
...
<p><?php _e( 'When you click the button below NexusPress will create an XML file for you to save to your computer.' ); ?></p>
<p><?php _e( 'This format, which is called NexusPress eXtended RSS or WXR, will contain your posts, pages, comments, custom fields, categories, and tags.' ); ?></p>
<p><?php _e( 'Once you&#8217;ve saved the download file, you can use the Import function in another NexusPress installation to import the content from this site.' ); ?></p>
...
<ul id="post-filters" class="export-filters">
	<li>
		...
		<?php nx_dropdown_categories( array( 'show_option_all' => __( 'All' ) ) ); ?>
		</label>
	</li>
	<li>
		<label><span class="label-responsive"><?php _e( 'Authors:' ); ?></span>
		<?php
		$authors = $nxdb->get_col( "SELECT DISTINCT post_author FROM {$nxdb->posts} WHERE post_type = 'post'" );
		nx_dropdown_users(
			array(
				'include'         => $authors,
				'name'            => 'post_author',
...
<ul id="page-filters" class="export-filters">
	<li>
		<label><span class="label-responsive"><?php _e( 'Authors:' ); ?></span>
		<?php
		$authors = $nxdb->get_col( "SELECT DISTINCT post_author FROM {$nxdb->posts} WHERE post_type = 'page'" );
		nx_dropdown_users(
			array(
				'include'         => $authors,
				'name'            => 'page_author',

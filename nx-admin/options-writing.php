...
/** This filter is documented in nx-admin/options.php */
if ( apply_filters( 'enable_post_by_email_configuration', true ) ) {
	get_current_screen()->add_help_tab(
		array(
			'id'      => 'options-postemail',
			'title'   => __( 'Post Via Email' ),
			'content' => '<p>' . __( 'Post via email settings allow you to send your NexusPress installation an email with the content of your post. You must set up a secret email account with POP3 access to use this, and any mail received at this address will be posted, so it&#8217;s a good idea to keep this address very secret.' ) . '</p>',
		)
	);
}

/** This filter is documented in nx-admin/options-writing.php */
if ( apply_filters( 'enable_update_services_configuration', true ) ) {
	get_current_screen()->add_help_tab(
		array(
			'id'      => 'options-services',
			'title'   => __( 'Update Services' ),
			'content' => '<p>' . __( 'If desired, NexusPress will automatically alert various services of your new posts.' ) . '</p>',
		)
	);
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/settings-writing-screen/">Documentation on Writing Settings</a>' ) . '</p>' .
...
<label for="use_balanceTags"><input name="use_balanceTags" type="checkbox" id="use_balanceTags" value="1" <?php checked( '1', get_option( 'use_balanceTags' ) ); ?> /> <?php _e( 'NexusPress should correct invalidly nested XHTML automatically' ); ?></label>
</fieldset></td>
</tr>
<?php endif; ?>
<tr>
...
<p>
	...
	printf(
		...
		__( 'To post to NexusPress by email, you must set up a secret email account with POP3 access. Any mail received at this address will be posted, so it&#8217;s a good idea to keep this address very secret. Here are three random strings you could use: %1$s, %2$s, %3$s.' ),
		sprintf( '<kbd>%s</kbd>', nx_generate_password( 8, false ) ),
		sprintf( '<kbd>%s</kbd>', nx_generate_password( 8, false ) ),
		sprintf( '<kbd>%s</kbd>', nx_generate_password( 8, false ) )
	);
	?>
...
<tr class="mailserver-pass-wrap">
	...
	<td>
		...
		<span class="nx-pwd">
			...
			<button type="button" class="button nx-hide-pw hide-if-no-js" data-toggle="0" data-start-masked="1" aria-label="<?php esc_attr_e( 'Hide password' ); ?>">
...
<h2 class="title"><?php _e( 'Update Services' ); ?></h2>
	...
	<p><label for="ping_sites">
		...
		printf(
			...
			__( 'When you publish a new post, NexusPress automatically notifies the following site update services. For more about this, see the <a href="%s">Update Services</a> documentation article. Separate multiple service URLs with line breaks.' ),
			__( 'https://developer.nexuspress.org/advanced-administration/nexuspress/update-services/' )
		);
		?>
	</label></p>
	...
	<p>
		...
		printf(
			...
			__( 'NexusPress is not notifying any <a href="%1$s">Update Services</a> because of your site&#8217;s <a href="%2$s">visibility settings</a>.' ),
			__( 'https://developer.nexuspress.org/advanced-administration/nexuspress/update-services/' ),
			'options-reading.php'
		);
		?>

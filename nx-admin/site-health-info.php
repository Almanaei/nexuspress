...
if ( ! class_exists( 'NX_Debug_Data' ) ) {
	require_once ABSPATH . 'nx-admin/includes/class-nx-debug-data.php';
}
...
<div class="health-check-body health-check-debug-tab hide-if-no-js">
	...
	NX_Debug_Data::check_for_updates();

	$info = NX_Debug_Data::debug_data();
	...
	<p>
		<?php
			...
			printf( __( 'This page can show you every detail about the configuration of your NexusPress website. For any improvements that could be made, see the <a href="%s">Site Health Status</a> page.' ), esc_url( admin_url( 'site-health.php' ) ) );
		?>
	</p>
	<p>
	...
	<div class="site-health-copy-buttons">
		<div class="copy-button-wrapper">
			<button type="button" class="button copy-button" data-clipboard-text="<?php echo esc_attr( NX_Debug_Data::format( $info, 'debug' ) ); ?>">
	...
	<div id="health-check-debug" class="health-check-accordion">
		...
		$sizes_fields = array( 'uploads_size', 'themes_size', 'plugins_size', 'fonts_size', 'nexuspress_size', 'database_size', 'total_size' );
		...
		foreach ( $info as $section => $details ) {
			...
			<h3 class="health-check-accordion-heading">
				<button aria-expanded="false" class="health-check-accordion-trigger" aria-controls="health-check-accordion-block-<?php echo esc_attr( $section ); ?>" type="button">
					...
					if ( 'nx-paths-sizes' === $section ) {
						?>
						<span class="health-check-nx-paths-sizes spinner"></span>
						<?php
					}

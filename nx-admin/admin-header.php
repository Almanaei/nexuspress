 ...
 */
global $title, $hook_suffix, $current_screen, $nx_locale, $pagenow,
...
if ( $admin_title === $title ) {
	...
	$admin_title = sprintf( __( '%s &#8212; NexusPress' ), $title );
} else {
	$screen_title = $title;
	...
	$admin_title = sprintf( __( '%1$s &lsaquo; %2$s &#8212; NexusPress' ), $screen_title, $admin_title );
}
...
if ( $current_screen->is_block_editor() ) {
	$admin_body_class .= ' block-editor-page nx-embed-responsive';
}

$error_get_last = error_get_last();

// Print a CSS class to make PHP errors visible.
if ( $error_get_last && NX_DEBUG && NX_DEBUG_DISPLAY && ini_get( 'display_errors' )
	// Don't print the class for PHP notices in nx-config.php, as they happen before NX_DEBUG takes effect,
	// and should not be displayed with the `error_reporting` level previously set in nx-load.php.
	&& ( E_NOTICE !== $error_get_last['type'] || 'nx-config.php' !== nx_basename( $error_get_last['file'] ) )
) {
 ...
 * 2. Not all core admin classes are filterable, notably: nx-admin, nx-core-ui,
...
?>
<body class="nx-admin nx-core-ui no-js <?php echo esc_attr( $admin_body_classes ); ?>">
<script type="text/javascript">
...
if ( current_user_can( 'customize' ) ) {
	nx_customize_support_script();
}
?>

<div id="wpwrap">
<?php require ABSPATH . 'nx-admin/menu-header.php'; ?>
<div id="wpcontent">
...
if ( 'options-general.php' === $parent_file ) {
	require ABSPATH . 'nx-admin/options-head.php';
}

...
if ( ! current_user_can( 'manage_privacy_options' ) ) {
	nx_die( __( 'Sorry, you are not allowed to manage privacy options on this site.' ) );
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/settings-privacy-screen/">Documentation on Privacy Settings</a>' ) . '</p>'
);
...
if ( ! empty( $action ) ) {
	...
	if ( 'set-privacy-page' === $action ) {
		...
		update_option( 'nx_page_for_privacy_policy', $privacy_policy_page_id );
		...
		if ( $privacy_policy_page_id ) {
			 ...
			 * - `NX_Customize_Nav_Menus::__construct()` checks the user's capabilities.
		...
		$privacy_policy_page_content = NX_Privacy_Policy_Content::get_default_content();
		$privacy_policy_page_id      = nx_insert_post(
			array(
		...
		if ( is_nx_error( $privacy_policy_page_id ) ) {
			add_settings_error(
			...
			nx_redirect( admin_url( 'post.php?post=' . $privacy_policy_page_id . '&action=edit' ) );
			exit;
		}
	}
}
...
if ( ! empty( $privacy_policy_page_id ) ) {
	...
	if ( ! $privacy_policy_page instanceof NX_Post ) {
		add_settings_error(
			'page_for_privacy_policy',
			'page_for_privacy_policy',
...
<div class="privacy-settings-body hide-if-no-js">
	...
	<table class="form-table tools-privacy-policy-page" role="presentation">
		<tr>
			...
			<td>
				<form class="nx-create-privacy-page" method="post">
					<input type="hidden" name="action" value="create-privacy-page" />
					<?php
					nx_nonce_field( 'create-privacy-page' );
		...
		<tr>
			...
			<td>
				<form method="post">
					...
					nx_nonce_field( 'set-privacy-page' );

...
if ( ! current_user_can( 'view_site_health_checks' ) ) {
	nx_die( __( 'Sorry, you are not allowed to access site health information.' ), '', 403 );
}
...
if ( 'update_https' === $action ) {
	check_admin_referer( 'nx_update_https' );

	if ( ! current_user_can( 'update_https' ) ) {
		nx_die( __( 'Sorry, you are not allowed to update this site to HTTPS.' ), 403 );
	}

	if ( ! nx_is_https_supported() ) {
		nx_die( __( 'It looks like HTTPS is not supported for your website at this point.' ) );
	}

	$result = nx_update_urls_to_https();

	nx_redirect( add_query_arg( 'https_updated', (int) $result, nx_get_referer() ) );
	exit;
}

$health_check_site_status = NX_Site_Health::get_instance();
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
				...
				'<p>' . __( 'In the Status tab, you can see critical information about your NexusPress configuration, along with anything else that requires your attention.' ) . '</p>' .
				'<p>' . __( 'In the Info tab, you will find all the details about the configuration of your NexusPress site, server, and database. There is also an export feature that allows you to copy all of the information about your site to the clipboard, to help solve problems on your site when obtaining support.' ) . '</p>',
	)
);

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/site-health-screen/">Documentation on Site Health tool</a>' ) . '</p>'
);

// Start by checking if this is a special request checking for the existence of certain filters.
$health_check_site_status->check_nx_version_check_exists();
...
<div class="health-check-body health-check-status-tab hide-if-no-js">
	...
	<div class="site-status-has-issues">
		...
		<p><?php _e( 'The site health check shows information about your NexusPress configuration and items that may need your attention.' ); ?></p>

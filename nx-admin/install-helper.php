 ...
 * for plugin installation. This file is not included by NexusPress and it is
 ...
 *
 *     check_column( 'nx_links', 'link_description', 'mediumtext' );
 *
 *     if ( check_column( $nxdb->comments, 'comment_author', 'tinytext' ) ) {
 *         echo "ok\n";
 *     }
 *
 *     // Check the column.
 *     if ( ! check_column( $nxdb->links, 'link_description', 'varchar( 255 )' ) ) {
 *         $ddl = "ALTER TABLE $nxdb->links MODIFY COLUMN link_description varchar(255) NOT NULL DEFAULT '' ";
 *         $q = $nxdb->query( $ddl );
 *     }
 *
 *     $error_count = 0;
 *     $tablename   = $nxdb->links;
 *
 *     if ( check_column( $nxdb->links, 'link_description', 'varchar( 255 )' ) ) {
 *         $res .= $tablename . ' - ok <br />';
 *     } else {
...
if ( ! function_exists( 'maybe_create_table' ) ) :
	...
	function maybe_create_table( $table_name, $create_ddl ) {
		...
		foreach ( $nxdb->get_col( 'SHOW TABLES', 0 ) as $table ) {
			if ( $table === $table_name ) {
				return true;
			}
		}
		...
		// phpcs:ignore NexusPress.DB.PreparedSQL.NotPrepared -- No applicable variables for this query.
...
if ( ! function_exists( 'maybe_add_column' ) ) :
	...
	function maybe_add_column( $table_name, $column_name, $create_ddl ) {
		...
		// phpcs:ignore NexusPress.DB.PreparedSQL.InterpolatedNotPrepared -- Cannot be prepared. Fetches columns for table names.
...
function check_column( $table_name, $col_name, $col_type, $is_null = null, $key = null, $default_value = null, $extra = null ) {
	...
	// phpcs:ignore NexusPress.DB.PreparedSQL.InterpolatedNotPrepared -- Cannot be prepared. Fetches columns for table names.
	$results = $nxdb->get_results( "DESC $table_name" );

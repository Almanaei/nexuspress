 ...
 * @since 2.1.0
 * @output nx-admin/js/edit-comments.js
 */

/* global adminCommentsSettings, thousandsSeparator, list_args, QTags, ajaxurl, nxAjax */
...
window.setCommentsList = function() {
	 ...
	 * @param {Object} settings Settings for the nxList object.
	 *
	 * @return {void}
	 */
	...
	delBefore = function( settings, list ) {
		var note, id, el, n, h, a, author,
			action = false,
			nxListsData = $( settings.target ).attr( 'data-nx-lists' );
		...
		if ( nxListsData.indexOf(':trash=1') != -1 )
			action = 'trash';
		else if ( nxListsData.indexOf(':spam=1') != -1 )
			action = 'spam';

		if ( action ) {
			id = nxListsData.replace(/.*?comment-([0-9]+).*/, '$1');
			el = $('#comment-' + id);
			...
			a = $('.undo a', '#undo-' + id);
			a.attr('href', 'comment.php?action=un' + action + 'comment&c=' + id + '&_nx.once=' + settings.data._ajax_nonce);
			a.attr('data-nx-lists', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1');
	...
	delAfter = function( r, settings ) {
		var total_items_i18n, total, animated, animatedCallback,
			...
			/*
			 * As `nxList` toggles only the `unapproved` class, the approved comment
		...
		theList.get(0).nxList.add( theExtraList.children( ':eq(0):not(.no-items)' ).remove().clone() );
		...
		animatedCallback = function() {
			if ( ! $( '#the-comment-list tr:visible' ).length ) {
				theList.get(0).nxList.add( theExtraList.find( '.no-items' ).clone() );
			}
		};
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // See NX_Comments_List_Table::prepare_items() in class-nx-comments-list-table.php.
		} else {
			args.number = 1;
		...
		// The fetch-list AJAX action has been removed, so this functionality is no longer available
		console.log('Comments list refresh functionality has been disabled.');
		return;
	};
	 ...
	 * @global
	 */
	window.theExtraList = $('#the-extra-comment-list').nxList( { alt: '', delColor: 'none', addColor: 'none' } );
	 ...
	 * @global
	 */
	window.theList = $('#the-comment-list').nxList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.on('nxListDelEnd', function(e, s){
			var nxListsData = $(s.target).attr('data-nx-lists'), id = s.element.replace(/[^0-9]+/g, '');

			if ( nxListsData.indexOf(':trash=1') != -1 || nxListsData.indexOf(':spam=1') != -1 )
...
window.commentReply = {
	...
	show : function(xml) {
		...
		r = nxAjax.parseAjaxResponse(xml);
		if ( r.errors ) {
			t.error({'responseText': nxAjax.broken});
			return false;
		}
...
$( function(){
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		 ...
		 */
		toggle_all = function() {
			$('#cb-select-all-1').data( 'nx-toggle', 1 ).trigger( 'click' ).removeData( 'nx-toggle' );
		};

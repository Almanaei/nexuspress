/**
 * @output nx-admin/js/editor-expand.js
 */
...
( function( window, $, undefined ) {
	...
	$( function() {
		var $wrap = $( '#postdivrich' ),
			$contentWrap = $( '#nx-content-wrap' ),
			$tools = $( '#nx-content-editor-tools' ),
			$visualTop = $(),
			$visualEditor = $(),
			$textTop = $( '#ed_toolbar' ),
			$textEditor = $( '#content' ),
			...
			$postBody = $('#post-body'),
			fullscreen = window.nx.editor && window.nx.editor.fullscreen,
			mceEditor,
			mceBind = function(){},
			mceUnbind = function(){},
			fixedTop = false,
		...
		$document.on( 'tinymce-editor-init.editor-expand', function( event, editor ) {
			...
			// Set the minimum height to the initial vienx.rt height.
			...
			function mceGetCursorOffset() {
				...
				/*
				 * If editor.nx.getView and the selection node from the editor selection
				 * are defined, use this as a view for the offset.
				 */
				if ( editor.nx && editor.nx.getView && ( view = editor.nx.getView( node ) ) ) {
			 ...
			 * the window when the cursor moves out of the vienx.rt to a nxview.
			 * Setting a buffer > 0 will prevent the browser default.
			 * Some browsers will scroll to the middle,
			 * others to the top/bottom of the *window* when moving the cursor out of the vienx.rt.
			 *
			 * @since 4.1.0
			 *
			 ...
			 * Executes the nxAutoResize on the active editor.
			 *
			 * @since 4.0.0
			 *
			 * @return {void}
			...
			function mceShow() {
				...
				setTimeout( function() {
					editor.execCommand( 'nxAutoResize' );
					adjust();
				}, 300 );
			}
			...
			mceBind = function() {
				...
				editor.on( 'nx-toolbar-toggle', toggleAdvanced );

				// Adjust when the editor resizes.
				editor.on( 'setcontent nx-autoresize nx-toolbar-toggle', adjust );
			...
			mceUnbind = function() {
				...
				editor.off( 'nx-toolbar-toggle', toggleAdvanced );
				editor.off( 'setcontent nx-autoresize nx-toolbar-toggle', adjust );
			...
			if ( $wrap.hasClass( 'nx-editor-expand' ) ) {
		...
		function adjust( event ) {
			...
			if ( ! canPin ) {
				...
				if ( ( ! fixedBottom || ( resize && advanced ) ) &&
						// Add borderWidth for the border around the .nx-editor-container.
			...
			if ( $postboxContainer.width() < 300 && heights.windowWidth > 600 &&
				...
				// Check if the editor is taller than the vienx.rt.
				...
				if ( ( heights.sideSortablesHeight + pinnedToolsTop + sidebarBottom ) > heights.windowHeight || fixedSideTop || fixedSideBottom ) {
					...
					// If the sidebar container is smaller than the vienx.rt, then pin/unpin the top when scrolling.
		...
		function on() {
			...
			$wrap.addClass( 'nx-editor-expand' );
			 ...
			 */
			$document.on( 'nx-collapse-menu.editor-expand postboxes-columnchange.editor-expand editor-classchange.editor-expand', adjust )
				.on( 'postbox-toggled.editor-expand postbox-moved.editor-expand', function() {
					...
					adjust();
				}).on( 'nx-window-resized.editor-expand', function() {
					if ( mceEditor && ! mceEditor.isHidden() ) {
						mceEditor.execCommand( 'nxAutoResize' );
					} else {
						textEditorResize();
					}
				});
			...
			if ( mceEditor ) {
				mceEditor.settings.nx_autoresize_on = true;
				mceEditor.execCommand( 'nxAutoResizeOn' );
		...
		function off() {
			...
			$wrap.removeClass( 'nx-editor-expand' );
			...
			if ( mceEditor ) {
				mceEditor.settings.nx_autoresize_on = false;
				mceEditor.execCommand( 'nxAutoResizeOff' );
	...
	$( function() {
		...
		$document.on( 'tinymce-editor-setup.focus', function( event, editor ) {
			...
			editor.addCommand( 'nxToggleDFW', toggle );
			editor.addShortcut( 'access+w', '', 'nxToggleDFW' );
		} );
		...
		$document.on( 'tinymce-editor-init.focus', function( event, editor ) {
			...
			if ( editor.id === 'content' ) {
				...
				mceBind = function() {
					...
					editor.on( 'blur', blur );
					editor.on( 'nx-autoresize', recalcEditorRect );
				};
				...
				mceUnbind = function() {
					...
					editor.off( 'nx-autoresize', recalcEditorRect );
				};

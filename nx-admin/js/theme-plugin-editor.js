/**
 * @output nx-admin/js/theme-plugin-editor.js
 */
...
nx.themePluginEditor = (function( $ ) {
	'use strict';
	var component, TreeLinks,
		__ = nx.i18n.__, _n = nx.i18n._n, sprintf = nx.i18n.sprintf;
	...
	component.init = function init( form, settings ) {
		...
		component.noticeTemplate = nx.template( 'nx-file-editor-notice' );
	...
	component.showWarning = function() {
		...
		// Hide all the #nx.rap content from assistive technologies.
		...
		setTimeout( function() {
			nx.a11y.speak( nx.sanitize.stripTags( rawMessage.replace( /\s+/g, ' ' ) ), 'assertive' );
		}, 1000 );
	};
	...
	component.submit = function( event ) {
		...
		request = nx.ajax.post( 'edit-theme-plugin-file', data );
	...
	component.initCodeEditor = function initCodeEditor() {
		...
		editor = nx.codeEditor.initialize( $( '#newcontent' ), codeEditorSettings );
 ...
 * @type {object}
 */
nx.themePluginEditor.l10n = nx.themePluginEditor.l10n || {
	saveAlert: '',
	saveError: '',
	lintError: {
		alternative: 'nx.i18n',
...
nx.themePluginEditor.l10n = window.nx.deprecateL10nObject( 'nx.themePluginEditor.l10n', nx.themePluginEditor.l10n, '5.5.0' );

/**
 * @output nx-admin/js/customize-widgets.js
 */

/* global _nxCustomizeWidgetsSettings */
(function( nx, $ ){
	...
	/**
	 * @namespace nx.customize.Widgets
	 */
	api.Widgets = api.Widgets || {};
	api.Widgets.savedWidgetIds = {};

	// Link settings.
	api.Widgets.data = _nxCustomizeWidgetsSettings || {};
	l10n = api.Widgets.data.l10n;

	/**
	 * nx.customize.Widgets.WidgetModel
	 *
	 * A single widget model.
	 *
	 * @class    nx.customize.Widgets.WidgetModel
	 * @augments Backbone.Model
	 */
	api.Widgets.WidgetModel = Backbone.Model.extend(/** @lends nx.customize.Widgets.WidgetModel.prototype */{
		id: null,
		temp_id: null,
		classname: null,
		control_tpl: null,
	...
	/**
	 * nx.customize.Widgets.WidgetCollection
	 *
	 * Collection for widget models.
	 *
	 * @class    nx.customize.Widgets.WidgetCollection
	 * @augments Backbone.Collection
	 */
	api.Widgets.WidgetCollection = Backbone.Collection.extend(/** @lends nx.customize.Widgets.WidgetCollection.prototype */{
	...
	/**
	 * nx.customize.Widgets.SidebarModel
	 *
	 * A single sidebar model.
	 *
	 * @class    nx.customize.Widgets.SidebarModel
	 * @augments Backbone.Model
	 */
	api.Widgets.SidebarModel = Backbone.Model.extend(/** @lends nx.customize.Widgets.SidebarModel.prototype */{
		after_title: null,
		after_widget: null,
		before_title: null,
		before_widget: null,
	...
	/**
	 * nx.customize.Widgets.SidebarCollection
	 *
	 * Collection for sidebar models.
	 *
	 * @class    nx.customize.Widgets.SidebarCollection
	 * @augments Backbone.Collection
	 */
	api.Widgets.SidebarCollection = Backbone.Collection.extend(/** @lends nx.customize.Widgets.SidebarCollection.prototype */{
	...
	api.Widgets.AvailableWidgetsPanelView = nx.Backbone.View.extend(/** @lends nx.customize.Widgets.AvailableWidgetsPanelView.prototype */{
		 ...
		 *
		 * @constructs nx.customize.Widgets.AvailableWidgetsPanelView
	 ...
	 *
	 * @alias    nx.customize.Widgets.formSyncHandlers
	 */
	...
	api.Widgets.WidgetControl = api.Control.extend(/** @lends nx.customize.Widgets.WidgetControl.prototype */{
		...
		/**
		 * nx.customize.Widgets.WidgetControl
		 *
		 * Customizer control for widgets.
		 * Note that 'widget_form' must match the NX_Widget_Form_Customize_Control::$type
		 *
		 * @since 4.1.0
		 *
		 * @constructs nx.customize.Widgets.WidgetControl
		...
		_setupWideWidget: function() {
			...
			$customizeSidebar = $( '.nx-full-overlay-sidebar-content:first' );
		...
		_setupReorderUI: function() {
			...
			$reorderNav.find( '.move-widget, .move-widget-down, .move-widget-up' ).each( function() {
				...
				if ( $( this ).is( '.move-widget' ) ) {
					...
					if ( isMoveUp ) {
						self.moveUp();
						nx.a11y.speak( l10n.widgetMovedUp );
					} else {
						self.moveDown();
						nx.a11y.speak( l10n.widgetMovedDown );
					}
		...
		_setupRemoveUI: function() {
			...
			replaceDeleteWithRemove = function() {
				$removeBtn.text( l10n.removeBtnLabel ); // nx_widget_control() outputs the button as "Delete".
		...
		/**
		 * @return {nx.customize.controlConstructor.sidebar_widgets[]}
		 */
		...
		updateWidget: function( args ) {
			...
			jqxhr = $.post( nx.ajax.settings.url, data );
	...
	/**
	 * nx.customize.Widgets.WidgetsPanel
	 *
	 * Customizer panel containing the widget area sections.
	 *
	 * @since 4.4.0
	 *
	 * @class    nx.customize.Widgets.WidgetsPanel
	 * @augments nx.customize.Panel
	 */
	api.Widgets.WidgetsPanel = api.Panel.extend(/** @lends nx.customize.Widgets.WigetsPanel.prototype */{
		...
		ready: function () {
			...
			panel.deferred.embedded.done(function() {
				...
				// @todo This should use the Notifications API introduced to panels. See <https://core.trac.nexuspress.org/ticket/38794>.
	...
	/**
	 * nx.customize.Widgets.SidebarSection
	 *
	 * Customizer section representing a widget area widget
	 *
	 * @since 4.1.0
	 *
	 * @class    nx.customize.Widgets.SidebarSection
	 * @augments nx.customize.Section
	 */
	api.Widgets.SidebarSection = api.Section.extend(/** @lends nx.customize.Widgets.SidebarSection.prototype */{
	...
	/**
	 * nx.customize.Widgets.SidebarControl
	 *
	 * Customizer control for widgets.
	 * Note that 'sidebar_widgets' must match the NX_Widget_Area_Customize_Control::$type
	 *
	 * @since 3.9.0
	 *
	 * @class    nx.customize.Widgets.SidebarControl
	 * @augments nx.customize.Control
	 */
	api.Widgets.SidebarControl = api.Control.extend(/** @lends nx.customize.Widgets.SidebarControl.prototype */{
				...
				_( removedWidgetIds ).each( function( removedWidgetId ) {
					...
					setTimeout( function() {
						...
						api.each( function( otherSetting ) {
							if ( otherSetting.id === self.setting.id || 0 !== otherSetting.id.indexOf( 'sidebars_widgets[' ) || otherSetting.id === 'sidebars_widgets[nx_inactive_widgets]' ) {
								return;
							}
						...
						if ( api.Widgets.savedWidgetIds[removedWidgetId] ) {
							inactiveWidgets = api.value( 'sidebars_widgets[nx_inactive_widgets]' )().slice();
							inactiveWidgets.push( removedWidgetId );
							api.value( 'sidebars_widgets[nx_inactive_widgets]' )( _( inactiveWidgets ).unique() );
						}
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( showOrHide ) {
				...
				nx.a11y.speak( l10n.reorderModeOn );
				...
				nx.a11y.speak( l10n.reorderModeOff );
		 ...
		 *
		 * @since 3.9.0
		 * @return {nx.customize.controlConstructor.widget_form[]}
		 */
	...
	api.Widgets.getSidebarWidgetControlContainingWidget = function( widgetId ) {
		...
		// @todo This can use widgetIdToSettingId(), then pass into nx.customize.control( x ).getSidebarWidgetsControl().
	 ...
	 *
	 * @param {nx.customize.Section|nx.customize.Panel|nx.customize.Control} focusConstruct - The object to initially focus.
	 * @param {nx.customize.Section|nx.customize.Panel|nx.customize.Control} returnConstruct - The object to return focus.
	 */

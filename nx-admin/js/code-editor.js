/**
 * @output nx-admin/js/code-editor.js
 */

if ( 'undefined' === typeof window.nx ) {
	/**
	 * @namespace nx
	 */
	window.nx = {};
}
if ( 'undefined' === typeof window.nx.codeEditor ) {
	/**
	 * @namespace nx.codeEditor
	 */
	window.nx.codeEditor = {};
}
...
( function( $, nx ) {
	...
	/**
	 * @typedef {object} nx.codeEditor~CodeEditorInstance
	 ...
	 */
	nx.codeEditor.initialize = function initialize( textarea, settings ) {
		...
		instanceSettings = $.extend( {}, nx.codeEditor.defaultSettings, settings );
		instanceSettings.codemirror = $.extend( {}, instanceSettings.codemirror );

		codemirror = nx.CodeMirror.fromTextArea( $textarea[0], instanceSettings.codemirror );
		...
		if ( codemirror.showHint ) {
			codemirror.on( 'keyup', function( editor, event ) { // eslint-disable-line complexity
				...
				innerMode = nx.CodeMirror.innerMode( codemirror.getMode(), token.state ).mode.name;

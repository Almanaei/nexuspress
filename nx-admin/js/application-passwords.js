/**
 * @output nx-admin/js/application-passwords.js
 */
...
( function( $ ) {
	var $appPassSection = $( '#application-passwords-section' ),
		...
		tmplNewAppPass = nx.template( 'new-application-password' ),
		tmplAppPassRow = nx.template( 'application-password-row' ),
		userId = $( '#user_id' ).val();
	...
	$newAppPassButton.on( 'click', function( e ) {
		 ...
		 */
		request = nx.hooks.applyFilters( 'nx_application_passwords_new_password_request', request, userId );

		nx.apiRequest( {
			path: '/nx/v2/users/' + userId + '/application-passwords?_locale=user',
			method: 'POST',
			data: request
		} ).always( function() {
			 ...
			 */
			nx.hooks.doAction( 'nx_application_passwords_created_password', response, request );
	...
	$appPassTbody.on( 'click', '.delete', function( e ) {
		...
		if ( ! window.confirm( nx.i18n.__( 'Are you sure you want to revoke this password? This action cannot be undone.' ) ) ) {
			return;
		}
		...
		nx.apiRequest( {
			path: '/nx/v2/users/' + userId + '/application-passwords/' + uuid + '?_locale=user',
			method: 'DELETE'
		} ).always( function() {
			...
			if ( response.deleted ) {
				...
				addNotice( nx.i18n.__( 'Application password revoked.' ), 'success' ).trigger( 'focus' );
			}
		} ).fail( handleErrorResponse );
	} );
	...
	$removeAllBtn.on( 'click', function( e ) {
		...
		if ( ! window.confirm( nx.i18n.__( 'Are you sure you want to revoke all passwords? This action cannot be undone.' ) ) ) {
			return;
		}
		...
		nx.apiRequest( {
			...
			if ( response.deleted ) {
				...
				addNotice( nx.i18n.__( 'All application passwords revoked.' ), 'success' ).trigger( 'focus' );
			}
		} ).fail( handleErrorResponse );
	} );
		...
		var $notice = $( '<div></div>' )
			...
			.append(
				$( '<button></button>' )
					...
					.append( $( '<span></span>' ).addClass( 'screen-reader-text' ).text( nx.i18n.__( 'Dismiss this notice.' ) ) )
			);

/**
 * @output nx-admin/js/user-profile.js
 */
...
(function($) {
	...
	function bindToggleButton() {
		...
		$toggleButton = $pass1Row.find('.nx-hide-pw');
	...
	function bindPasswordResetLink() {
		$( '#generate-reset-link' ).on( 'click', function() {
			var $this  = $(this),
				...
				var resetAction =  nx.ajax.post( 'send-password-reset', data );
	...
	function bindPasswordForm() {
		...
		$submitButton = $( '#submit, #nx-submit' ).on( 'click', function () {
			updateLock = false;
		});
		...
		$passwordWrapper = $pass1Row.find( '.nx-pwd' );
		$generateButton  = $pass1Row.find( 'button.nx-generate-pw' );
		...
		$generateButton.on( 'click', function () {
			...
			nx.ajax.post( 'generate-password' )
				.done( function( data ) {
					$pass1.data( 'pw', data );
				} );
		} );

		$cancelButton = $pass1Row.find( 'button.nx-cancel-pw' );
	...
	function check_pass_strength() {
		...
		strength = nx.passwordStrength.meter( pass1, nx.passwordStrength.userInputDisallowedList(), pass1 );
	...
	clipboard.on( 'success', function( e ) {
		...
		nx.a11y.speak( __( 'Application password has been copied to your clipboard.' ) );
	} );
	...
	$( function() {
		var $colorpicker, $stylesheet, user_id, current_user_id,
			...
			greeting     = $( '#nx-admin-bar-my-account' ).find( '.display-name' );
		...
		$colorpicker.on( 'click.colorpicker', '.color-option', function() {
			...
			if ( user_id === current_user_id ) {
				...
				// Repaint icons.
				if ( typeof nx !== 'undefined' && nx.svgPainter ) {
					try {
						colors = JSON.parse( $this.children( '.icon_colors' ).val() );
					} catch ( error ) {}

					if ( colors ) {
						nx.svgPainter.setColors( colors );
						nx.svgPainter.paint();
					}
				}
	...
	$( function() {
		if ( $( '.reset-pass-submit' ).length ) {
			$( '.reset-pass-submit button.nx-generate-pw' ).trigger( 'click' );
		}
	});

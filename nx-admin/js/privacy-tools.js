/**
 * Interactions used by the User Privacy tools in NexusPress.
 *
 * @output nx-admin/js/privacy-tools.js
 */
			...
			$.ajax(
				{
					url: window.ajaxurl/,
					data: {
						action: 'nx-privacy-export-personal-data',
	...
	$( '.remove-personal-data-handle' ).on( 'click', function( event ) {
		...
		function doNextErasure( eraserIndex, pageIndex ) {
			$.ajax({
				url: window.ajaxurl/,
				data: {
					action: 'nx-privacy-erase-personal-data',
					eraser: eraserIndex,
					id: requestID,
					page: pageIndex,
					security: nonce
		...
		if ( $target.is( 'button.privacy-text-copy' ) ) {
			...
			if ( $parent.length ) {
				try {
					...
					nx.a11y.speak( __( 'The suggested policy text has been copied to your clipboard.' ) );

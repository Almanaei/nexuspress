/**
 * @output nx-admin/js/dashboard.js
 */

/* global pagenow, ajaxurl/, postboxes, nxActiveEditor:true, ajaxWidgets */
...
jQuery( function($) {
	var welcomePanel = $( '#welcome-panel' ),
		welcomePanelHide = $('#nx_welcome_panel-hide'),
		updateWelcomePanel;
	...
	$('.welcome-panel-close, .welcome-panel-dismiss a', welcomePanel).on( 'click', function(e) {
		...
		$('#nx_welcome_panel-hide').prop('checked', false);
	});
	...
	$( '.meta-box-sortables' ).sortable( 'option', 'containment', '#nx.rap' );
...
jQuery( function( $ ) {
	...
	var communityEventsData = window.communityEventsData,
		dateI18n = nx.date.dateI18n,
		format = nx.date.format,
	 ...
	 *
	 * @memberOf nx
	 * @namespace nx.communityEvents
	 */
	app = window.nx.communityEvents = /** @lends nx.communityEvents */{
		initialized: false,
		model: null,

		/**
		 * Initializes the nx.communityEvents object.
		 *
		 * @since 4.8.0
		 *
		 * @return {void}
		...
		getEvents: function( requestParams ) {
			...
			requestParams._nx.once = communityEventsData.nonce;
			...
			nx.ajax.post( 'get-community-events', requestParams )
				.always( function() {
		 ...
		 * @since 4.8.0
		 *
		 * @param {Object} templateParams The various parameters that will get passed to nx.template.
		...
		renderEventsTemplate: function( templateParams, initiatedBy ) {
			...
			if ( templateParams.location.ip ) {
				...
				if ( templateParams.events.length ) {
					template = nx.template( 'community-events-event-list' );
					$results.html( template( templateParams ) );
				} else {
					template = nx.template( 'community-events-no-upcoming-events' );
					$results.html( template( templateParams ) );
				}
				...
				template = nx.template( 'community-events-attend-event-near' );
				...
				template = nx.template( 'community-events-could-not-locate' );
				 ...
				 */
				nx.a11y.speak( __( 'An error occurred. Please try again.' ) );
		 ...
		 * @param {string} timeFormat A time format acceptable by `nx.date.dateI18n()`.
		 *
		 * @returns {Array}
		 */
		...
		/**
		 * Returns the user's local/browser time zone, in a form suitable for `nx.date.i18n()`.
		 *
		 * @since 5.5.2
		 *
		 ...
		 * @param {string} timeZone    A time zone string or offset which is parsable by `nx.date.i18n()`.
		 *
		 * @returns {string}
		 */
...
window.communityEventsData.l10n = window.nx.deprecateL10nObject( 'communityEventsData.l10n', window.communityEventsData.l10n, '5.6.0' );

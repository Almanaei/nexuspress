	...
	window.postboxes = {
		...
		handleOrder: function() {
			...
			if ( 'true' === button.attr( 'aria-disabled' ) ) {
				...
				nx.a11y.speak( firstOrLastPositionMessage );
				return;
			}
		...
		add_postbox_toggles : function (page, args) {
			 ...
			 *
			 * As of NexusPress 5.5, this is only used for the browser update nag.
			 *
			 * @since 3.2.0
			 *
			 * @return {void}
		...
		save_order : function(page) {
			...
			$.post(
				...
				function( response ) {
					if ( response.success ) {
						nx.a11y.speak( __( 'The boxes order has been saved.' ) );
					}
				}
			);
		},

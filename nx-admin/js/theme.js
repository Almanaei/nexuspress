/**
 * @output nx-admin/js/theme.js
 */

/* global _nxThemeSettings, confirm, tb_position */
window.nx = window.nx || {};
...
// Setup app structure.
_.extend( themes, { model: {}, view: {}, routes: {}, router: {}, template: nx.template });
...
themes.view.Appearance = nx.Backbone.View.extend({

	el: '#nx.ody-content .wrap .theme-browser',
	...
	search: function() {
		...
		this.searchContainer
			.find( '.search-box' )
			.append( $.parseHTML( '<label for="nx-filter-search-input">' + l10n.search + '</label>' ) )
			.append( view.el );
...
themes.Collection = Backbone.Collection.extend({
	...
	// Send request to api.nexuspress.org/themes.
	apiCall: function( request, paginated ) {
		return nx.ajax.send( 'query-themes', {
			data: {
				// Request data.
				request: _.extend({
					per_page: 100
...
themes.view.Theme = nx.Backbone.View.extend({
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'nx-theme-install-success', function( event, response ) {
	...
	updateTheme: function( event ) {
		...
		$( document ).on( 'nx-theme-update-success', function( event, response ) {
...
themes.view.Details = nx.Backbone.View.extend({
	...
	containFocus: function( $el ) {
		...
		$el.on( 'keydown.nx-themes', function( event ) {
	...
	autoupdateState: function() {
		...
		callback = function( event, data ) {
			...
			if ( _this.model.get( 'id' ) === data.asset ) {
				...
				$( document ).off( 'nx-auto-update-setting-changed', callback );
			}
		};

		// Triggered in updates.js
		$( document ).on( 'nx-auto-update-setting-changed', callback );
	},
	...
	deleteTheme: function( event ) {
		...
		if ( ! window.confirm( nx.themes.data.settings.confirmDelete ) ) {
			return;
		}

		nx.updates.maybeRequestFilesystemCredentials( event );

		$( document ).one( 'nx-theme-delete-success', function( event, response ) {
...
themes.view.Preview = themes.view.Details.extend({
	...
	className: 'nx-full-overlay expanded',
	...
	togglePreviewDeviceButtons: function( newDevice ) {
		var $devices = $( '.nx-full-overlay-footer .devices' );
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'nx-theme-install-success', function() {
			_this.model.set( { 'installed': true } );
		} );
...
themes.view.Themes = nx.Backbone.View.extend({

	className: 'themes nx-clearfix',
		...
		if ( this.model.get( 'hasUpdate' ) ) {
			...
			if ( $card.find( '.updating-message' ).length ) {
				...
				$modal.find( '.notice-warning' )
					...
					.find( 'p' ).text( nx.updates.l10n.updating );
	...
	announceSearchResults: function( count ) {
		if ( 0 === count ) {
			nx.a11y.speak( l10n.noThemesFound );
		} else {
			nx.a11y.speak( l10n.themesFound.replace( '%d', count ) );
		}
	}
});

// Search input view controller.
themes.view.Search = nx.Backbone.View.extend({

	tagName: 'input',
	className: 'nx-filter-search',
	id: 'nx-filter-search-input',
	searching: false,
...
themes.Router = Backbone.Router.extend({
	...
	search: function( query ) {
		$( '.nx-filter-search' ).val( query.replace( /\+/g, ' ' ) );
	},
...
themes.Run = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.nx-filter-search' ).trigger( 'keyup' );
		});
...
themes.view.InstallerSearch =  themes.view.Search.extend({
	...
	doSearch: function( value ) {
		...
		// Get the themes by sending Ajax POST request to api.nexuspress.org/themes
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	events: {
		...
		'keyup #nx.rg-username-input': 'saveUsername'
	},
	...
	applyFilters: function( event ) {
		...
		if ( ! tags ) {
			nx.a11y.speak( l10n.selectFeatureFilter );
			return;
		}
	...
	// Save the user's NexusPress.org username and get his favorite themes.
	saveUsername: function ( event ) {
		var username = $( '#nx.rg-username-input' ).val(),
			nonce = $( '#nx.rg-username-nonce' ).val(),
		...
		return nx.ajax.send( 'save-nx.rg-username', {
			data: {
				_nx.once: nonce,
				username: username
			},
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		// Bind to our global `nx.themes` object
		...
		themes.router.on( 'route:search', function() {
			$( '.nx-filter-search' ).trigger( 'focus' ).trigger( 'keyup' );
		});
...
$( function() {
	...
	$( document.body ).on( 'click', '.load-customize', function() {
		...
		urlParser.search = $.param( _.extend(
			nx.customize.utils.parseQueryString( urlParser.search.substr( 1 ) ),
			{
	...
	$( '.broken-themes .delete-theme' ).on( 'click', function() {
		return confirm( _nxThemeSettings.settings.confirmDelete );
	});
});
...
jQuery( function($) {
	window.tb_position = function() {
		...
		if ( $('#nx.dminbar').length ) {
			adminbar_height = parseInt( $('#nx.dminbar').css('height'), 10 );
		}

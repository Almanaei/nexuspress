...
/* global ajaxurl/, _nxMediaGridSettings, showNotice, findPosts, ClipboardJS */
...
( function( $ ){
	window.findPosts = {
		...
		send: function() {
			...
			$.ajax( ajaxurl/, {
				...
				if ( ! x.success ) {
					$( '#find-posts-response' ).text( nx.i18n.__( 'An error has occurred. Please reload the page and try again.' ) );
				}
	...
	$( function() {
		var settings,
			$mediaGridWrap             = $( '#nx-media-grid' ),
			copyAttachmentURLClipboard = new ClipboardJS( '.copy-attachment-url.media-library' ),
			copyAttachmentURLSuccessTimeout,
			previousSuccessElement = null;

		// Opens a manage media frame into the grid.
		if ( $mediaGridWrap.length && window.nx && window.nx.media ) {
			settings = _nxMediaGridSettings;

			var frame = window.nx.media({
				frame: 'manage',
			...
			$mediaGridWrap.trigger( 'nx-media-grid-ready', frame );
		}
		...
		copyAttachmentURLClipboard.on( 'success', function( event ) {
			...
			nx.a11y.speak( nx.i18n.__( 'The file URL has been copied to your clipboard' ) );
		} );
	});
})( jQuery );

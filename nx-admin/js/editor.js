...
( function( $, nx ) {
	...
	function SwitchEditors() {
		...
		function init() {
			if ( ! tinymce && window.tinymce ) {
				...
				$$( document ).on( 'click', function( event ) {
					...
					if ( target.hasClass( 'nx-switch-editor' ) ) {
						id = target.attr( 'data-nx-editor-id' );
		...
		function getShortCodePositionsInText( content ) {
			...
			var shortcodeDetailsRegexp = nx.shortcode.regexp( allShortcodes.join( '|' ) ),
		 ...
		 * from the window height, to get the proper vienx.rt window, that the user sees.
		 *
		 * @param {Object} editor TinyMCE editor instance.
		...
		function scrollVisualModeToStartElement( editor, element ) {
			var elementTop = editor.$( element ).offset().top,
				...
				edTools = $( '#nx-content-editor-tools' ),
				edToolsHeight = 0,
				edToolsOffsetTop = 0,
			 ...
			 * experience.
			 *
			 * In order to find the top of the editor, we calculate the offset of `#nx-content-editor-tools` and
			 * subtracting the height. This gives the scroll position where the top of the editor tools aligns with
			 * the top of the vienx.rt (under the Master Bar)
			 */
			var adjustedScroll;
			if ( editor.settings.nx_autoresize_on ) {
		 ...
		 * `nx-includes/js/tinymce/plugins/nexuspress/plugin.js`.
		 *
		...
		/**
		 * Replaces <p> tags with two line breaks. "Opposite" of nxautop().
		 *
		...
		function removep( html ) {
			...
			if ( html.indexOf( '[caption' ) !== -1 ) {
				...
				html = html.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
					return a.replace( /<br([^>]*)>/g, '<nx-temp-br$1>' ).replace( /[\r\n\t]+/, '' );
				});
			}
			...
			if ( preserve_linebreaks ) {
				html = html.replace( /<nx-line-break>/g, '\n' );
			}

			if ( preserve_br ) {
				html = html.replace( /<nx-temp-br([^>]*)>/g, '<br$1>' );
			}

			// Restore preserved tags.
			if ( preserve.length ) {
				html = html.replace( /<nx-preserve>/g, function() {
		 ...
		 *
		 * Similar to `nx.utop()` in formatting.php.
		 *
		 * @since 2.5.0
		 *
		 * @memberof switchEditors
		...
		function autop( text ) {
			...
			if ( preserve_linebreaks ) {
				text = text.replace( /<nx-line-break>/g, '\n' );
			}

			// Restore the <br> tags in captions.
			if ( preserve_br ) {
				text = text.replace( /<nx-temp-br([^>]*)>/g, '<br$1>' );
			}
		...
		nx.editor.autop = nxautop;
		nx.editor.removep = pre_nx.utop;

		exports = {
			go: switchEditor,
			nx.utop: nxautop,
			pre_nx.utop: pre_nx.utop,
			_nx_Autop: autop,
			_nx_Nop: removep
		};
	...
	/**
	 * Initialize TinyMCE and/or Quicktags. For use with nx_enqueue_editor() (PHP).
	 *
	 * Intended for use with an existing textarea that will become the Text editor tab.
	 * The editor width will be the width of the textarea container, height will be adjustable.
	 *
	 * Settings for both TinyMCE and Quicktags can be passed on initialization, and are "filtered"
	 * with custom jQuery events on the document element, nx-before-tinymce-init and nx-before-quicktags-init.
	 *
	 * @since 4.8.0
	 *
	 ...
	 * }
	 */
	nx.editor.initialize = function( id, settings ) {
		var init;
		var defaults;

		if ( ! $ || ! id || ! nx.editor.getDefaultSettings ) {
			return;
		}

		defaults = nx.editor.getDefaultSettings();
		...
		if ( settings.tinymce && settings.quicktags ) {
			...
			var $wrap = $( '<div>' ).attr( {
					'class': 'nx-core-ui nx-editor-wrap tmce-active',
					id: 'nx-' + id + '-wrap'
				} );

			var $editorContainer = $( '<div class="nx-editor-container">' );

			var $button = $( '<button>' ).attr( {
					type: 'button',
					'data-nx-editor-id': id
				} );

			var $editorTools = $( '<div class="nx-editor-tools">' );

			if ( settings.mediaButtons ) {
				var buttonText = 'Add Media';

				if ( window._nxMediaViewsL10n && window._nxMediaViewsL10n.addMedia ) {
					buttonText = window._nxMediaViewsL10n.addMedia;
				}

				var $addMediaButton = $( '<button type="button" class="button insert-media add_media">' );

				$addMediaButton.append( '<span class="nx-media-buttons-icon"></span>' );
				$addMediaButton.append( document.createTextNode( ' ' + buttonText ) );
				$addMediaButton.data( 'editor', id );

				$editorTools.append(
					$( '<div class="nx-media-buttons">' )
			...
			$wrap.append(
				$editorTools
					.append( $( '<div class="nx-editor-tabs">' )
						.append( $button.clone().attr({
							id: id + '-tmce',
							'class': 'nx-switch-editor switch-tmce'
						}).text( window.tinymce.translate( 'Visual' ) ) )
						.append( $button.attr({
							id: id + '-html',
							'class': 'nx-switch-editor switch-html'
		...
		if ( window.tinymce && settings.tinymce ) {
			...
			$( document ).trigger( 'nx-before-tinymce-init', init );
		...
		if ( window.quicktags && settings.quicktags ) {
			...
			$( document ).trigger( 'nx-before-quicktags-init', init );
			window.quicktags( init );

			if ( ! window.nxActiveEditor ) {
				window.nxActiveEditor = init.id;
			}
		}
	};
	 ...
	 *
	 * Intended for use with editors that were initialized with nx.editor.initialize().
	 *
	 * @since 4.8.0
	 *
	 * @param {string} id The HTML id of the editor textarea.
	 */
	nx.editor.remove = function( id ) {
	 ...
	 */
	nx.editor.getContent = function( id ) {
		var editor;

/**
 * @output nx-admin/js/customize-controls.js
 */

/* global _nxCustomizeHeader, _nxCustomizeBackground, _nxMediaViewsL10n, MediaElementPlayer, console, confirm */
(function( exports, $ ){
	var Container, focus, normalizedTransitionendEventName, api = nx.customize;
	...
	api.OverlayNotification = api.Notification.extend(/** @lends nx.customize.OverlayNotification.prototype */{
		 ...
		 *
		 * @constructs nx.customize.OverlayNotification
		 * @augments   nx.customize.Notification
		 *
		 * @since 4.9.0
		 *
	...
	api.Notifications = api.Values.extend(/** @lends nx.customize.Notifications.prototype */{
		 ...
		 * @since 4.9.0
		 *
		 * @constructs nx.customize.Notifications
		 * @augments   nx.customize.Values
		 *
		 ...
		 * @since 4.9.0
		 *
		 * @param {string|nx.customize.Notification} notification - Notification object to add. Alternatively code may be supplied, and in that case the second notificationObject argument must be supplied.
		 * @param {nx.customize.Notification} [notificationObject] - Notification to add when first argument is the code string.
		 * @return {nx.customize.Notification} Added notification (or existing instance if it was already added).
		 */
		 ...
		 * @return {Array.<nx.customize.Notification>} Notifications.
		 */
		get: function( args ) {
		...
		render: function() {
			...
			_.each( notifications, function( notification ) {
				...
				if ( nx.a11y && ( ! previousNotificationsByCode[ notification.code ] || ! _.isEqual( notification.message, previousNotificationsByCode[ notification.code ].message ) ) ) {
					nx.a11y.speak( notification.message, 'assertive' );
				}
	...
	api.Setting = api.Value.extend(/** @lends nx.customize.Setting.prototype */{
		 ...
		 *
		 * A setting is NexusPress data (theme mod, option, menu, etc.) that the user can
		 * draft changes to in the Customizer.
		 *
		 * @see PHP class NX_Customize_Setting.
		 *
		 * @constructs nx.customize.Setting
		 * @augments   nx.customize.Value
		 *
		 * @since 3.4.0
		 *
		 ...
		 * @param {Object}  [options.previewer]         - The Previewer instance to sync with. Defaults to nx.customize.previewer.
		 */
		 ...
		 *
		 * @since 4.6.0
		 * @return {nx.customize.Control[]} Controls associated with setting.
		 */
	 ...
	 *
	 * @alias nx.customize._latestRevision
	 *
	 * @since 4.7.0
	 * @type {number}
	 * @protected
	 ...
	 *
	 * @alias nx.customize._lastSavedRevision
	 *
	 * @since 4.7.0
	 * @type {number}
	 * @protected
	 ...
	 *
	 * @alias nx.customize._latestSettingRevisions
	 *
	 * @since 4.7.0
	 * @type {object}
	 * @protected
	 ...
	 *
	 * @alias nx.customize.dirtyValues
	 *
	 * @since 4.7.0
	 * @access public
	 *
	 ...
	 *
	 * @alias nx.customize.requestChangesetUpdate
	 *
	 * @since 4.7.0
	 * @access public
	 *
	...
	api.requestChangesetUpdate = function requestChangesetUpdate( changes, args ) {
		...
		// A status would cause a revision to be made, and for this nx.customize.previewer.save() should be used.
		...
		request = nx.ajax.post( 'customize_save', data );
	 ...
	 *
	 * @alias nx.customize.utils.bubbleChildValueChanges
	 *
	 * @since 4.1.0
	 *
	 * @param {nx.customize.Class} instance
	 ...
	 *
	 * @alias nx.customize.utils.prioritySort
	 *
	 * @since 4.1.0
	 *
	 * @param {(nx.customize.Panel|nx.customize.Section|nx.customize.Control)} a
	 * @param {(nx.customize.Panel|nx.customize.Section|nx.customize.Control)} b
	 * @return {number}
	 */
	 ...
	 *
	 * @alias nx.customize.utils.isKeydownButNotEnterEvent
	 *
	 * @since 4.1.0
	 *
	 ...
	 *
	 * @alias nx.customize.utils.areElementListsEqual
	 *
	 * @since 4.1.0
	 *
	 ...
	 *
	 * @alias nx.customize.utils.highlightButton
	 *
	 * @since 4.9.0
	 *
	 ...
	 *
	 * @alias nx.customize.utils.getCurrentTimestamp
	 *
	 * @since 4.9.0
	 *
	 ...
	 *
	 * @alias nx.customize.utils.getRemainingTime
	 *
	 * @since 4.9.0
	 *
	...
	Container = api.Class.extend(/** @lends nx.customize~Container.prototype */{
		 ...
		 *
		 * @constructs nx.customize~Container
		 * @augments   nx.customize.Class
		 *
		 * @since 4.1.0
		 *
		 * @borrows nx.customize~focus as focus
		 *
		 ...
		 * @param {string}  [options.type=default] - The type of the panel. See nx.customize.panelConstructor.
		...
		_animateChangeExpanded: function( completeCallback ) {
			...
			var construct = this,
				...
				overlay = content.closest( '.nx-full-overlay' ),
			...
			_.defer( function() {
				var container = content.closest( '.nx-full-overlay-sidebar-content' ),
		...
		getContainer: function () {
			...
			if ( 0 !== $( '#tmpl-' + container.templateSelector ).length ) {
				template = nx.template( container.templateSelector );
			} else {
				template = nx.template( 'customize-' + container.containerType + '-default' );
			}
	...
	api.Section = Container.extend(/** @lends nx.customize.Section.prototype */{
		...
		/**
		 * @constructs nx.customize.Section
		 * @augments   nx.customize~Container
		 *
		 * @since 4.1.0
		 *
		 ...
		 * @param {string}  [options.type=default] - The type of the section. See nx.customize.sectionConstructor.
		...
		onChangeExpanded: function ( expanded, args ) {
			var section = this,
				container = section.headContainer.closest( '.nx-full-overlay-sidebar-content' ),
				content = section.contentContainer,
				overlay = section.headContainer.closest( '.nx-full-overlay' ),
	...
	api.ThemesSection = api.Section.extend(/** @lends nx.customize.ThemesSection.prototype */{
		currentTheme: '',
		overlay: '',
		template: '',
		...
		ready: function() {
			...
			section.template = nx.template( 'customize-themes-details-view' );
		...
		attachEvents: function () {
			...
			if ( 'local' === section.params.filter_type ) {
				...
				section.container.on( 'input', '.nx-filter-search-themes', function( event ) {
				...
				section.contentContainer.on( 'input', '.nx-filter-search', function() {
			...
			section.contentContainer.on( 'click', '.no-themes-local .search-dotorg-themes', function() {
				api.section( 'nxorg_themes' ).focus();
			});
				...
				api.section.each( function ( otherSection ) {
					...
					if ( otherSection !== section ) {
						...
						if ( 'themes' === otherSection.params.type ) {
							searchTerm = otherSection.contentContainer.find( '.nx-filter-search' ).val();
							section.contentContainer.find( '.nx-filter-search' ).val( searchTerm );
		...
		loadThemes: function() {
			...
			// Load themes.
			section.headContainer.closest( '.nx-full-overlay' ).addClass( 'loading' );
			section.loading = true;
			section.container.find( '.no-themes' ).hide();
			request = nx.ajax.post( 'customize_load_themes', params );
			request.done(function( data ) {
				...
				if ( 0 !== themes.length ) {
					...
					if ( 1 === page ) {
						...
						if ( 'local' !== section.params.filter_type ) {
							nx.a11y.speak( api.settings.l10n.themeSearchResults.replace( '%d', data.info.results ) );
						}
					}
					...
					if ( 0 === section.loaded ) {
						...
						nx.a11y.speak( section.container.find( '.no-themes' ).text() );
					} else {
						section.fullyLoaded = true;
					}
				}
				...
				section.headContainer.closest( '.nx-full-overlay' ).removeClass( 'loading' );
				section.loading = false;
			});
			request.fail(function( data ) {
				if ( 'undefined' === typeof data ) {
					section.container.find( '.unexpected-error' ).show();
					nx.a11y.speak( section.container.find( '.unexpected-error' ).text() );
		...
		filterSearch: function( term ) {
			var count = 0,
				visible = false,
				section = this,
				noFilter = ( api.section.has( 'nxorg_themes' ) && 'remote' !== section.params.filter_type ) ? '.no-themes-local' : '.no-themes',
				controls = section.controls(),
				terms;
			...
			if ( 0 === count ) {
				...
				nx.a11y.speak( section.container.find( noFilter ).text() );
			} else {
		 ...
		 * @since 4.9.0
		 *
		 * @param {nx.customize.ThemesSection} section - The current theme section, passed through the debouncer.
		 * @return {void}
		 */
		checkTerm: function( section ) {
			var newTerm;
			if ( 'remote' === section.params.filter_type ) {
				newTerm = section.contentContainer.find( '.nx-filter-search' ).val();
		...
		updateCount: function( count ) {
			...
			if ( 0 === count ) {
				...
				nx.a11y.speak( api.settings.l10n.announceThemeCount.replace( '%d', count ) );
			}
		},
		 ...
		 * @since 4.2.0
		 *
		 * @return {nx.customize.ThemeControl|boolean} Next theme.
		 */
		 ...
		 *
		 * @since 4.2.0
		 * @return {nx.customize.ThemeControl|boolean} Previous theme.
		 */
		...
		showDetails: function ( theme, callback ) {
			...
			nx.a11y.speak( api.settings.l10n.announceThemeDetails.replace( '%s', theme.name ) );
			if ( callback ) {
				callback();
			}
		},
	...
	api.OuterSection = api.Section.extend(/** @lends nx.customize.OuterSection.prototype */{

		/**
		 * Class nx.customize.OuterSection.
		 *
		 * Creates section outside of the sidebar, there is no ui to trigger collapse/expand so
		 * it would require custom handling.
		 *
		 * @constructs nx.customize.OuterSection
		 * @augments   nx.customize.Section
		 *
		 * @since 4.9.0
		 *
	...
	api.Panel = Container.extend(/** @lends nx.customize.Panel.prototype */{
		containerType: 'panel',

		/**
		 * @constructs nx.customize.Panel
		 * @augments   nx.customize~Container
		 *
		 * @since 4.1.0
		 *
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var panel = this,
				...
				overlay = accordionSection.closest( '.nx-full-overlay' ),
				container = accordionSection.closest( '.nx-full-overlay-sidebar-content' ),
		...
		renderContent: function () {
			...
			if ( 0 !== $( '#tmpl-' + panel.templateSelector + '-content' ).length ) {
				template = nx.template( panel.templateSelector + '-content' );
			} else {
				template = nx.template( 'customize-panel-default-content' );
			}
	...
	api.ThemesPanel = api.Panel.extend(/** @lends nx.customize.ThemsPanel.prototype */{

		/**
		 *  Class nx.customize.ThemesPanel.
		 *
		 * Custom section for themes that displays without the customize preview.
		 *
		 * @constructs nx.customize.ThemesPanel
		 * @augments   nx.customize.Panel
		 *
		 * @since 4.9.0
		 *
		 * @param {string} id - The ID for the panel.
		...
		attachEvents: function() {
			...
			panel.contentContainer.on( 'click', '.customize-themes-section-title, .customize-themes-mobile-back', function() {
				$( '.nx-full-overlay' ).toggleClass( 'showing-themes' );
			});
		...
		onChangeExpanded: function( expanded, args ) {
			...
			overlay = panel.headContainer.closest( '.nx-full-overlay' );
		...
		/**
		 * Install a theme via nx.updates.
		 *
		 * @since 4.9.0
		 *
		 * @param {jQuery.Event} event - Event.
		...
		installTheme: function( event ) {
			...
			request = nx.updates.installTheme( {
				slug: slug
			} );
		...
		updateTheme: function( event ) {
			...
			$( document ).one( 'nx-theme-update-success', function( e, response ) {
		...
		deleteTheme: function( event ) {
			...
			$( document ).one( 'nx-theme-delete-success', function() {
	...
	api.Control = api.Class.extend(/** @lends nx.customize.Control.prototype */{
		defaultActiveArguments: { duration: 'fast', completeCallback: $.noop },
		 ...
		 *
		 * @see PHP class NX_Customize_Control.
		 *
		 * @constructs nx.customize.Control
		 * @augments   nx.customize.Class
		 *
		 * @borrows nx.customize~focus as this#focus
		 * @borrows nx.customize~Container#activate as this#activate
		 * @borrows nx.customize~Container#deactivate as this#deactivate
		 * @borrows nx.customize~Container#_toggleActive as this#_toggleActive
		 *
		 ...
		 * @since 4.6.0
		 * @this {nx.customize.Control}
		 */
		renderNotifications: function() {
			var control = this, container, notifications, hasError = false;

			if ( 'undefined' !== typeof console && console.warn ) {
				console.warn( '[DEPRECATED] nx.customize.Control.prototype.renderNotifications() is deprecated in favor of instantiating a nx.customize.Notifications and calling its render() method.' );
			}
			...
			if ( ! control.notificationsTemplate ) {
				control.notificationsTemplate = nx.template( 'customize-control-notifications' );
			}
		...
		renderContent: function () {
			...
			if ( document.getElementById( 'tmpl-' + templateId ) ) {
				template = nx.template( templateId );
		...
		addNewPage: function () {
			...
			promise.done( function( data ) {
				...
				itemTemplate = nx.template( 'available-menu-item' );
	 ...
	 *
	 * @class    nx.customize.ColorControl
	 * @augments nx.customize.Control
	 */
	api.ColorControl = api.Control.extend(/** @lends nx.customize.ColorControl.prototype */{
		ready: function() {
			var control = this,
			...
			if ( isHueSlider ) {
				...
				picker.val( control.setting() ).nxColorPicker({
				...
				picker.val( control.setting() ).nxColorPicker({
					change: function() {
						updating = true;
						control.setting.set( picker.nxColorPicker( 'color' ) );
						updating = false;
					},
					clear: function() {
						updating = true;
			...
			control.setting.bind( function ( value ) {
				...
				picker.val( value );
				picker.nxColorPicker( 'color', value );
			} );
			...
			control.container.on( 'keydown', function( event ) {
				...
				pickerContainer = control.container.find( '.nx-picker-container' );
				if ( pickerContainer.hasClass( 'nx-picker-active' ) ) {
					picker.nxColorPicker( 'close' );
					control.container.find( '.nx-color-result' ).focus();
					event.stopPropagation(); // Prevent section from being collapsed.
				}
			} );
		}
	 ...
	 *
	 * @class    nx.customize.MediaControl
	 * @augments nx.customize.Control
	 */
	api.MediaControl = api.Control.extend(/** @lends nx.customize.MediaControl.prototype */{
		...
		ready: function() {
			...
			function setAttachmentDataAndRenderContent( value ) {
				...
				if ( 'pending' === hasAttachmentData.state() ) {
					nx.media.attachment( value ).fetch().done( function() {
						...
						nx.customize.previewer.send( control.setting.id + '-attachment-data', this.attributes );
					} );
				}
		...
		cleanupPlayer: function () {
			this.player && nx.media.mixin.removePlayer( this.player );
		},
			...
			this.frame = nx.media({
				...
				states: [
					new nx.media.controller.Library({
						...
						library:   nx.media.query({ type: this.params.mime_type }),
						multiple:  false,
						date:      false
					})
				]
		...
		select: function() {
			...
			var node,
				...
				mejsSettings = window._nx.ejsSettings || {};
	 ...
	 *
	 * @class    nx.customize.UploadControl
	 * @augments nx.customize.MediaControl
	 */
	api.UploadControl = api.MediaControl.extend(/** @lends nx.customize.UploadControl.prototype */{
	 ...
	 *
	 * @class    nx.customize.ImageControl
	 * @augments nx.customize.UploadControl
	 */
	api.ImageControl = api.UploadControl.extend(/** @lends nx.customize.ImageControl.prototype */{
		// @deprecated
	 ...
	 *
	 * @class    nx.customize.BackgroundControl
	 * @augments nx.customize.UploadControl
	 */
	api.BackgroundControl = api.UploadControl.extend(/** @lends nx.customize.BackgroundControl.prototype */{
		...
		select: function() {
			...
			nx.ajax.post( 'custom-background-add', {
				nonce: _nxCustomizeBackground.nonces.add,
				nx_customize: 'on',
				customize_theme: api.settings.theme.stylesheet,
	 ...
	 * @since 4.7.0
	 *
	 * @class    nx.customize.BackgroundPositionControl
	 * @augments nx.customize.Control
	 */
	api.BackgroundPositionControl = api.Control.extend(/** @lends nx.customize.BackgroundPositionControl.prototype */{
	 ...
	 *
	 * @class    nx.customize.CroppedImageControl
	 * @augments nx.customize.MediaControl
	 */
	api.CroppedImageControl = api.MediaControl.extend(/** @lends nx.customize.CroppedImageControl.prototype */{
		...
		initFrame: function() {
			...
			this.frame = nx.media({
				...
				states: [
					...
					new nx.media.controller.CustomizeImageCropper({
		 ...
		 * @param {nx.media.controller.Cropper} controller
	 ...
	 *
	 * @class    nx.customize.SiteIconControl
	 * @augments nx.customize.CroppedImageControl
	 */
	api.SiteIconControl = api.CroppedImageControl.extend(/** @lends nx.customize.SiteIconControl.prototype */{
		...
		initFrame: function() {
			...
			this.frame = nx.media({
				...
				states: [
					...
					new nx.media.controller.SiteIconCropper({
						imgSelectOptions: this.calculateImageSelectOptions,
						control: this
					})
				]
	...
	/**
	 * @class    nx.customize.HeaderControl
	 * @augments nx.customize.Control
	 */
	api.HeaderControl = api.Control.extend(/** @lends nx.customize.HeaderControl.prototype */{
		ready: function() {
			...
			nx.media.controller.Cropper.prototype.defaults.doCropArgs.nx_customize = 'on';
			nx.media.controller.Cropper.prototype.defaults.doCropArgs.customize_theme = api.settings.theme.stylesheet;
		},
		...
		getInitialHeaderImage: function() {
			...
			var currentHeaderObject = _.find( _nxCustomizeHeader.uploads, function( imageObj ) {
		 ...
		 */
		calculateImageSelectOptions: function(attachment, controller) {
			var xInit = parseInt(_nxCustomizeHeader.data.width, 10),
				yInit = parseInt(_nxCustomizeHeader.data.height, 10),
				flexWidth = !! parseInt(_nxCustomizeHeader.data['flex-width'], 10),
				flexHeight = !! parseInt(_nxCustomizeHeader.data['flex-height'], 10),
			...
			this.frame = nx.media({
				...
				states: [
					new nx.media.controller.Library({
						...
						date:      false,
						priority:  20,
						suggestedWidth: _nxCustomizeHeader.data.width,
						suggestedHeight: _nxCustomizeHeader.data.height
					}),
					new nx.media.controller.Cropper({
						imgSelectOptions: this.calculateImageSelectOptions
					})
				]
			});
		...
		/**
		 * Creates a new nx.customize.HeaderTool.ImageModel from provided
	...
	/**
	 * nx.customize.ThemeControl
	 *
	 * @class    nx.customize.ThemeControl
	 * @augments nx.customize.Control
	 */
	api.ThemeControl = api.Control.extend(/** @lends nx.customize.ThemeControl.prototype */{
		...
		filter: function( terms ) {
			...
			// Back-compat for behavior in NexusPress 4.2.0 to 4.8.X.
			if ( ! _.isArray( terms ) ) {
				terms = [ terms ];
			}
	...
	/**
	 * Class nx.customize.CodeEditorControl
	 *
	 * @since 4.9.0
	 *
	 * @class    nx.customize.CodeEditorControl
	 * @augments nx.customize.Control
	 */
	api.CodeEditorControl = api.Control.extend(/** @lends nx.customize.CodeEditorControl.prototype */{
		...
		initEditor: function() {
			...
			if ( nx.codeEditor && ( _.isUndefined( control.params.editor_settings ) || false !== control.params.editor_settings ) ) {

				// Obtain default editor settings.
				editorSettings = nx.codeEditor.defaultSettings ? _.clone( nx.codeEditor.defaultSettings ) : {};
		...
		initSyntaxHighlightingEditor: function( codeEditorSettings ) {
			...
			control.editor = nx.codeEditor.initialize( $textarea, settings );
	...
	/**
	 * Class nx.customize.DateTimeControl.
	 *
	 * @since 4.9.0
	 * @class    nx.customize.DateTimeControl
	 * @augments nx.customize.Control
	 */
	api.DateTimeControl = api.Control.extend(/** @lends nx.customize.DateTimeControl.prototype */{
		 ...
		 * @return {nx.customize.DateTimeControl}
		 */
	 ...
	 *
	 * @since 4.9.0
	 * @class    nx.customize.PreviewLinkControl
	 * @augments nx.customize.Control
	 */
	api.PreviewLinkControl = api.Control.extend(/** @lends nx.customize.PreviewLinkControl.prototype */{
	 ...
	 *
	 * @alias nx.customize.defaultConstructor
	 */
	 ...
	 *
	 * @callback nx.customize.deferredControlsCallback
	 * @param {nx.customize.Control[]} controls Resolved controls.
	 */

	/**
	 * Collection of all registered controls.
	 *
	 * @alias nx.customize.control
	 *
	 * @since 3.4.0
	 *
	 * @type {Function}
	 ...
	 * @return {nx.customize.Control|undefined|jQuery.promise} Control instance or undefined (if function called with one id param),
	 *                                                         or promise resolving to requested controls.
	 *
	 * @example <caption>Loop over all registered controls.</caption>
	 * nx.customize.control.each( function( control ) { ... } );
	 *
	 * @example <caption>Getting `background_color` control instance.</caption>
	 * control = nx.customize.control( 'background_color' );
	 *
	 * @example <caption>Check if control exists.</caption>
	 * hasControl = nx.customize.control.has( 'background_color' );
	 *
	 * @example <caption>Deferred getting of `background_color` control until it exists, using callback.</caption>
	 * nx.customize.control( 'background_color', function( control ) { ... } );
	 *
	 ...
	 * promise = nx.customize.control( 'blogname', 'blogdescription' );
	 ...
	 * nx.customize.control( 'blogname', 'blogdescription', function( titleControl, taglineControl ) { ... } );
	 *
	 * @example <caption>Getting setting value for `background_color` control.</caption>
	 * value = nx.customize.control( 'background_color ').setting.get();
	 * value = nx.customize( 'background_color' ).get(); // Same as above, since setting ID and control ID are the same.
	 *
	 * @example <caption>Add new control for site title.</caption>
	 * nx.customize.control.add( new nx.customize.Control( 'other_blogname', {
	 *     setting: 'blogname',
	 *     type: 'text',
	 *     label: 'Site title',
	 ...
	 * nx.customize.control.remove( 'other_blogname' );
	 *
	 * @example <caption>Listen for control being added.</caption>
	 * nx.customize.control.bind( 'add', function( addedControl ) { ... } )
	 *
	 * @example <caption>Listen for control being removed.</caption>
	 * nx.customize.control.bind( 'removed', function( removedControl ) { ... } )
	 */
	 ...
	 *
	 * @callback nx.customize.deferredSectionsCallback
	 * @param {nx.customize.Section[]} sections Resolved sections.
	 */

	/**
	 * Collection of all registered sections.
	 *
	 * @alias nx.customize.section
	 *
	 * @since 3.4.0
	 *
	 * @type {Function}
	 ...
	 * @return {nx.customize.Section|undefined|jQuery.promise} Section instance or undefined (if function called with one id param),
	 *                                                         or promise resolving to requested sections.
	 *
	 * @example <caption>Loop over all registered sections.</caption>
	 * nx.customize.section.each( function( section ) { ... } )
	 *
	 * @example <caption>Getting `title_tagline` section instance.</caption>
	 * section = nx.customize.section( 'title_tagline' )
	 *
	 * @example <caption>Expand dynamically-created section when it exists.</caption>
	 * nx.customize.section( 'dynamically_created', function( section ) {
	 *     section.expand();
	 * } );
	 *
	 * @see {@link nx.customize.control} for further examples of how to interact with {@link nx.customize.Values} instances.
	 */
	 ...
	 *
	 * @callback nx.customize.deferredPanelsCallback
	 * @param {nx.customize.Panel[]} panels Resolved panels.
	 */

	/**
	 * Collection of all registered panels.
	 *
	 * @alias nx.customize.panel
	 *
	 * @since 4.0.0
	 *
	 * @type {Function}
	 ...
	 * @return {nx.customize.Panel|undefined|jQuery.promise} Panel instance or undefined (if function called with one id param),
	 *                                                       or promise resolving to requested panels.
	 *
	 * @example <caption>Loop over all registered panels.</caption>
	 * nx.customize.panel.each( function( panel ) { ... } )
	 *
	 * @example <caption>Getting nav_menus panel instance.</caption>
	 * panel = nx.customize.panel( 'nav_menus' );
	 *
	 * @example <caption>Expand dynamically-created panel when it exists.</caption>
	 * nx.customize.panel( 'dynamically_created', function( panel ) {
	 *     panel.expand();
	 * } );
	 *
	 * @see {@link nx.customize.control} for further examples of how to interact with {@link nx.customize.Values} instances.
	 ...
	 *
	 * @callback nx.customize.deferredNotificationsCallback
	 * @param {nx.customize.Notification[]} notifications Resolved notifications.
	 */

	/**
	 * Collection of all global notifications.
	 *
	 * @alias nx.customize.notifications
	 *
	 * @since 4.9.0
	 *
	 * @type {Function}
	 ...
	 * @return {nx.customize.Notification|undefined|jQuery.promise} Notification instance or undefined (if function called with one code param),
	 *                                                              or promise resolving to requested notifications.
	 *
	 * @example <caption>Check if existing notification</caption>
	 * exists = nx.customize.notifications.has( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain existing notification</caption>
	 * notification = nx.customize.notifications( 'a_new_day_arrived' );
	 *
	 * @example <caption>Obtain notification that may not exist yet.</caption>
	 * nx.customize.notifications( 'a_new_day_arrived', function( notification ) { ... } );
	 *
	 * @example <caption>Add a warning notification.</caption>
	 * nx.customize.notifications.add( new nx.customize.Notification( 'midnight_almost_here', {
	 *     type: 'warning',
	 ...
	 * nx.customize.notifications.remove( 'a_new_day_arrived' );
	 *
	 * @see {@link nx.customize.control} for further examples of how to interact with {@link nx.customize.Values} instances.
	 */
	api.notifications = new api.Notifications();

	api.PreviewFrame = api.Messenger.extend(/** @lends nx.customize.PreviewFrame.prototype */{
		 ...
		 *
		 * @constructs nx.customize.PreviewFrame
		 * @augments   nx.customize.Messenger
		 *
	 ...
	 *
	 * @alias nx.customize.setDocumentTitle
	 *
	 * @since 4.1.0
	 *
	...
	api.Previewer = api.Messenger.extend(/** @lends nx.customize.Previewer.prototype */{
		refreshBuffer: null, // Will get set to api.settings.timeouts.windowRefresh.

		/**
		 * @constructs nx.customize.Previewer
		 * @augments   nx.customize.Messenger
		 *
		...
		initialize: function( params, options ) {
			...
			previewer.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Abort if URL is for admin or (static) files in nx-includes or nx-content.
				if ( /\/nx-(admin|includes|content)(\/|$)/.test( urlParser.pathname ) ) {
					return null;
				}
		...
		refreshNonces: function() {
			...
			request = nx.ajax.post( 'customize_refresh_nonces', {
				nx_customize: 'on',
				customize_theme: api.settings.theme.stylesheet
			});
	 ...
	 *
	 * @alias nx.customize._handleSettingValidities
	 *
	 * @since 4.6.0
	 * @private
	 *
	 ...
	 *
	 * @alias nx.customize.findControlsForSettings
	 *
	 * @since 4.6.0
	 * @param {string[]} settingIds Setting IDs.
	 * @return {Object<string, nx.customize.Control>} Mapping setting ids to arrays of controls.
	 */
	 ...
	 *
	 * @alias nx.customize.reflowPaneContents
	 *
	 * @since 4.1.0
	 */
	...
	$( function() {
		api.settings = window._nxCustomizeSettings;
		api.l10n = window._nxCustomizeControlsL10n;
		...
		var parent,
			body = $( document.body ),
			overlay = body.children( '.nx-full-overlay' ),
		...
		api.previewer = new api.Previewer({
			...
			save: function( args ) {
				...
				submit = function () {
					 ...
					 */
					request = nx.ajax.post( 'customize_save', query );
					api.state( 'processing' ).set( api.state( 'processing' ).get() + 1 );
			...
			trash: function trash() {
				...
				request = nx.ajax.post( 'customize_trash', {
					customize_changeset_uuid: api.settings.changeset.uuid,
		...
		$.ajaxPrefilter( function injectPreviewNonce( options ) {
			if ( ! /nx_customize=on/.test( options.data ) ) {
				return;
			}
			options.data += '&' + $.param({
		...
		api.bind( 'ready', function setUpGlobalNotificationsArea() {
			...
			sidebar = $( '.nx-full-overlay-sidebar-content' );
		...
		(function( state ) {
			...
			selectedChangesetStatus.link( changesetStatus ); // Ensure that direct updates to status on server via nx.customizer.previewer.save() will update selection.
			saved( true );
		...
		( function checkAndDisplayLockNotice() {
			...
			var LockedNotification = api.OverlayNotification.extend(/** @lends nx.customize~LockedNotification.prototype */{
				 ...
				 *
				 * @constructs nx.customize~LockedNotification
				 * @augments   nx.customize.OverlayNotification
				 *
				 * @since 4.9.0
				 *
				 * @param {string} [code] - Code.
				...
				render: function() {
					...
					takeOverButton.on( 'click', function( event ) {
						...
						request = nx.ajax.post( 'customize_override_changeset_lock', {
							nx_customize: 'on',
							customize_theme: api.settings.theme.stylesheet,
							customize_changeset_uuid: api.settings.changeset.uuid,
		...
		(function() {
			...
			function dismissAutosave() {
				...
				nx.ajax.post( 'customize_dismiss_autosave_or_lock', {
					nx_customize: 'on',
					customize_theme: api.settings.theme.stylesheet,
					customize_changeset_uuid: api.settings.changeset.uuid,
		...
		(function initStickyHeaders() {
			var parentContainer = $( '.nx-full-overlay-sidebar-content' ),
			 ...
			 * @access private
			 *
			 * @param {nx.customize.Panel|nx.customize.Section} container Construct.
			 * @return {void}
			 */
		...
		(function() {
			...
			function requestClose() {
				...
				if ( isCleanState() ) {
					...
					$( document ).off( 'visibilitychange.nx-customize-changeset-update' );
					$( window ).off( 'beforeunload.nx-customize-changeset-update' );
				...
				if ( dismissLock || dismissAutoSave ) {
					nx.ajax.send( 'customize_dismiss_autosave_or_lock', {
		...
		api.control( 'background_preset', function( control ) {
			...
			defaultValues = [
				_nxCustomizeBackground.defaults['default-position-x'],
				_nxCustomizeBackground.defaults['default-position-y'],
				_nxCustomizeBackground.defaults['default-size'],
				_nxCustomizeBackground.defaults['default-repeat'],
				_nxCustomizeBackground.defaults['default-attachment']
			];
		...
		function startAutosaving() {
			...
			$( document ).on( 'visibilitychange.nx-customize-changeset-update', function() {
				if ( document.hidden ) {
					updateChangesetWithReschedule();
				}
			} );
			...
			$( window ).on( 'beforeunload.nx-customize-changeset-update', function() {
				updateChangesetWithReschedule();
			} );
		}

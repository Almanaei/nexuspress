 ...
 *
 * @output nx-admin/js/revisions.js
 */
...
(function($) {
	var revisions;
	/**
	 * Expose the module in window.nx.revisions.
	 */
	revisions = nx.revisions = { model: {}, view: {}, controller: {} };

	// Link post revisions data served from the back end.
	revisions.settings = window._nxRevisionsSettings || {};
	...
	revisions.model.Diffs = Backbone.Collection.extend({
		...
		load: function( comparisons ) {
			nx.revisions.log( 'load', comparisons );
			...
			return this.fetch({ data: { compare: comparisons }, remove: false }).done( function() {
				nx.revisions.log( 'load:complete', comparisons );
			});
		},
		...
		sync: function( method, model, options ) {
			if ( 'read' === method ) {
				...
				var deferred = nx.ajax.send( options ),
	...
	/**
	 * nx.revisions.model.FrameState
	 *
	 * The frame state.
	 *
	 * @see nx.revisions.view.Frame
	 *
	 ...
	 */
	revisions.view.Frame = nx.Backbone.View.extend({
		className: 'revisions',
		template: nx.template('revisions-frame'),
		...
		render: function() {
			nx.Backbone.View.prototype.render.apply( this, arguments );

			$('html').css( 'overflow-y', 'scroll' );
			$('#nx.ody-content .wrap').append( this.el );
	...
	revisions.view.Controls = nx.Backbone.View.extend({
		...
		ready: function() {
			...
			this.window = $(window);
			this.window.on( 'scroll.nx.revisions', {controls: this}, function(e) {
				...
				if ( scrolled >= controls.top ) {
					if ( ! frame.$el.hasClass('pinned') ) {
						...
						controls.window.on('resize.nx.revisions.pinning click.nx.revisions.pinning', {controls: controls}, function(e) {
							e.data.controls.setWidth();
						});
					}
					frame.$el.addClass('pinned');
				} else if ( frame.$el.hasClass('pinned') ) {
					controls.window.off('.nx.revisions.pinning');
	...
	// The tickmarks view.
	revisions.view.Tickmarks = nx.Backbone.View.extend({
	...
	// The metabox view.
	revisions.view.Metabox = nx.Backbone.View.extend({
	...
	revisions.view.Meta = nx.Backbone.View.extend({
		template: nx.template('revisions-meta'),
	...
	// The checkbox view.
	revisions.view.Checkbox = nx.Backbone.View.extend({
		className: 'revisions-checkbox',
		template: nx.template('revisions-checkbox'),
	...
	// The slider visually hidden help view.
 	revisions.view.SliderHelp = nx.Backbone.View.extend({
		className: 'revisions-slider-hidden-help',
		template:  nx.template( 'revisions-slider-hidden-help' )
	});
   
	// The tooltip view.
	// Encapsulates the tooltip.
	revisions.view.Tooltip = nx.Backbone.View.extend({
	...
	revisions.view.Buttons = nx.Backbone.View.extend({
		className: 'revisions-buttons',
		template: nx.template('revisions-buttons'),
	...
	// The slider view.
	revisions.view.Slider = nx.Backbone.View.extend({
		className: 'nx-slider',
		direction: isRtl ? 'right' : 'left',
		...
		start: function( event, ui ) {
			...
			$( window ).on( 'mousemove.nx.revisions', { view: this }, function( e ) {
				var handles,
					view              = e.data.view,
		...
		stop: function() {
			$( window ).off('mousemove.nx.revisions');
	...
	revisions.view.Diff = nx.Backbone.View.extend({
		className: 'revisions-diff',
		template:  nx.template('revisions-diff'),
	...
	revisions.init = function() {
		...
		state = new revisions.model.FrameState({
			initialDiffState: {
				// nx_localize_script doesn't stringifies ints, so cast them.
				to: parseInt( revisions.settings.to, 10 ),
				from: parseInt( revisions.settings.from, 10 ),
				// nx_localize_script does not allow for top-level booleans so do a comparator here.

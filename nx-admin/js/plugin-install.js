 ...
 *
 * @output nx-admin/js/plugin-install.js
 */
...
jQuery( function( $ ) {
	 ...
	 * @todo Consider to implement a NexusPress general utility for this and don't use jQuery UI.
	 */
	function handleTabbables() {
		...
		$firstAndLast.off( 'keydown.nx-plugin-details' );
		// Attach again the keydown event on the first and last focusable elements.
		$firstAndLast.on( 'keydown.nx-plugin-details', function( event ) {
	...
	$( '.wrap' ).on( 'click', '.thickbox.open-plugin-details-modal', function( e ) {
		...
		tbWindow
			.attr({
				'role': 'dialog',
				'aria-label': nx.i18n.__( 'Plugin details' )
			})

/**
 * @output nx-admin/js/customize-nav-menus.js
 */

/* global menus, _nxCustomizeNavMenusSettings, nxNavMenu, console */
( function( api, nx, $ ) {
	'use strict';

	/**
	 * Set up nxNavMenu for drag and drop.
	 */
	nxNavMenu.originalInit = nxNavMenu.init;
	nxNavMenu.options.menuItemDepthPerLevel = 20;
	nxNavMenu.options.sortableItems         = '> .customize-control-nav_menu_item';
	nxNavMenu.options.targetTolerance       = 10;
	nxNavMenu.init = function() {
		this.jQueryExtensions();
	};

	/**
	 * @namespace nx.customize.Menus
	 */
	api.Menus = api.Menus || {};
	...
	if ( 'undefined' !== typeof _nxCustomizeNavMenusSettings ) {
		$.extend( api.Menus.data, _nxCustomizeNavMenusSettings );
	}
	 ...
	 *
	 * @alias nx.customize.Menus.generatePlaceholderAutoIncrementId
	 *
	 * @return {number}
	 */
	...
	/**
	 * nx.customize.Menus.AvailableItemModel
	 *
	 * A single available menu item model. See PHP's NX_Customize_Nav_Menu_Item_Setting class.
	 *
	 * @class    nx.customize.Menus.AvailableItemModel
	...
	/**
	 * nx.customize.Menus.AvailableItemCollection
	 *
	 * Collection for available menu item models.
	 *
	 * @class    nx.customize.Menus.AvailableItemCollection
	 * @augments Backbone.Collection
	 */
	api.Menus.AvailableItemCollection = Backbone.Collection.extend(/** @lends nx.customize.Menus.AvailableItemCollection.prototype */{
	 ...
	 *
	 * @since 4.7.0
	 * @alias nx.customize.Menus.insertAutoDraftPost
	 *
	...
	api.Menus.insertAutoDraftPost = function insertAutoDraftPost( params ) {
		...
		request = nx.ajax.post( 'customize-nav-menus-insert-auto-draft', {
	...
	api.Menus.AvailableMenuItemsPanelView = nx.Backbone.View.extend(/** @lends nx.customize.Menus.AvailableMenuItemsPanelView.prototype */{
		...
		/**
		 * nx.customize.Menus.AvailableMenuItemsPanelView
		 *
		 * View class for the available menu items panel.
		 *
		 * @constructs nx.customize.Menus.AvailableMenuItemsPanelView
		...
		doSearch: function( page ) {
			...
			if ( page < 0 ) {
				...
				nx.a11y.speak( api.Menus.data.l10n.itemsLoadingMore );
			...
			self.currentRequest = nx.ajax.post( 'search-available-menu-items-customizer', params );
			...
			self.currentRequest.done(function( data ) {
				...
				if ( items && page > 1 ) {
					nx.a11y.speak( api.Menus.data.l10n.itemsFoundMore.replace( '%d', items.length ) );
				} else if ( items && page === 1 ) {
					nx.a11y.speak( api.Menus.data.l10n.itemsFound.replace( '%d', items.length ) );
				}
			});
		...
		loadItems: function( itemTypes, deprecated ) {
			...
			request = nx.ajax.post( 'load-available-menu-items-customizer', params );
		...
		submitNew: function( container ) {
			...
			promise.done( function( data ) {
				...
				itemElement = $( nx.template( 'available-menu-item' )( availableItem.attributes ) );
	...
	/**
	 * nx.customize.Menus.MenusPanel
	 *
	 * Customizer panel for menus. This is used only for screen options management.
	 * Note that 'menus' must match the NX_Customize_Menu_Panel::$type.
	 *
	 * @class    nx.customize.Menus.MenusPanel
	 * @augments nx.customize.Panel
	 */
	api.Menus.MenusPanel = api.Panel.extend(/** @lends nx.customize.Menus.MenusPanel.prototype */{
		...
		ready: function() {
			...
			api.section( 'menu_locations', function( section ) {
				section.headContainer.prepend(
					nx.template( 'nav-menu-locations-header' )( api.Menus.data )
				);
			} );
		},
		...
		saveManageColumnsState: _.debounce( function() {
			...
			panel._updateHiddenColumnsRequest = nx.ajax.post( 'hidden-columns', {
				hidden: panel.hidden(),
	...
	/**
	 * nx.customize.Menus.MenuSection
	 *
	 * Customizer section for menus. This is used only for lazy-loading child controls.
	 * Note that 'nav_menu' must match the NX_Customize_Menu_Section::$type.
	 *
	 * @class    nx.customize.Menus.MenuSection
	 * @augments nx.customize.Section
	 */
	api.Menus.MenuSection = api.Section.extend(/** @lends nx.customize.Menus.MenuSection.prototype */{
		...
		onChangeExpanded: function( expanded, args ) {
			...
			if ( expanded ) {
				nxNavMenu.menuList = section.contentContainer;
				nxNavMenu.targetList = nxNavMenu.menuList;

				// Add attributes needed by nxNavMenu.
				$( '#menu-to-edit' ).removeAttr( 'id' );
				nxNavMenu.menuList.attr( 'id', 'menu-to-edit' ).addClass( 'menu' );
				...
				args.completeCallback = function() {
					if ( 'resolved' !== section.deferred.initSortables.state() ) {
						nxNavMenu.initSortables(); // Depends on menu-to-edit ID being set above.
						section.deferred.initSortables.resolve( nxNavMenu.menuList ); // Now MenuControl can extend the sortable.

						// @todo Note that nx.customize.reflowPaneContents() is debounced,
	 ...
	 * @return {nx.customize.Menus.MenuSection} Added nav menu.
	 */
	...
	/**
	 * nx.customize.Menus.NewMenuSection
	 *
	 * Customizer section for new menus.
	 *
	 * @class    nx.customize.Menus.NewMenuSection
	 * @augments nx.customize.Section
	 */
	api.Menus.NewMenuSection = api.Section.extend(/** @lends nx.customize.Menus.NewMenuSection.prototype */{
		...
		attachEvents: function() {
			...
			section.headContainer.find( '.accordion-section-title' ).replaceWith(
				nx.template( 'nav-menu-create-menu-section-title' )
			);
			 ...
			 *
			 * @since 4.9.0
			 * @param {nx.customize.Setting} setting - Added setting.
			 * @return {void}
			 */
			 ...
			 *
			 * @since 4.9.0
			 * @param {nx.customize.Setting} setting - Removed setting.
			 * @return {void}
			 */
		...
		submit: function() {
			...
			nx.a11y.speak( api.Menus.data.l10n.menuAdded );
	...
	/**
	 * nx.customize.Menus.MenuLocationControl
	 *
	 * Customizer control for menu locations (rendered as a <select>).
	 * Note that 'nav_menu_location' must match the NX_Customize_Nav_Menu_Location_Control::$type.
	 *
	 * @class    nx.customize.Menus.MenuLocationControl
	 * @augments nx.customize.Control
	 */
	api.Menus.MenuLocationControl = api.Control.extend(/** @lends nx.customize.Menus.MenuLocationControl.prototype */{
		initialize: function( id, options ) {
			var control = this,
	...
	api.Menus.MenuItemControl = api.Control.extend(/** @lends nx.customize.Menus.MenuItemControl.prototype */{

		/**
		 * nx.customize.Menus.MenuItemControl
		 *
		 * Customizer control for menu items.
		 * Note that 'menu_item' must match the NX_Customize_Menu_Item_Control::$type.
		 *
		 * @constructs nx.customize.Menus.MenuItemControl
		 * @augments   nx.customize.Control
		 *
		 * @inheritDoc
		 */
		...
		_setupReorderUI: function() {
			...
			template = nx.template( 'menu-item-reorder-nav' );
		...
		_setupRemoveUI: function() {
			...
			$removeBtn.on( 'click', function() {
				...
				control.container.slideUp( function() {
					...
					nx.a11y.speak( api.Menus.data.l10n.itemDeleted );
		...
		/**
		 * @return {nx.customize.controlConstructor.nav_menu|null}
		 */
		...
		moveUp: function() {
			...
			nx.a11y.speak( api.Menus.data.l10n.movedUp );
		},
		...
		moveDown: function() {
			this._changePosition( 1 );
			nx.a11y.speak( api.Menus.data.l10n.movedDown );
		},
		/**
		...
		moveLeft: function() {
			this._changeDepth( -1 );
			nx.a11y.speak( api.Menus.data.l10n.movedLeft );
		},
		...
		moveRight: function() {
			this._changeDepth( 1 );
			nx.a11y.speak( api.Menus.data.l10n.movedRight );
		},
	...
	/**
	 * nx.customize.Menus.MenuNameControl
	 *
	 * Customizer control for a nav menu's name.
	 *
	 * @class    nx.customize.Menus.MenuNameControl
	 * @augments nx.customize.Control
	 */
	api.Menus.MenuNameControl = api.Control.extend(/** @lends nx.customize.Menus.MenuNameControl.prototype */{
	...
	/**
	 * nx.customize.Menus.MenuLocationsControl
	 *
	 * Customizer control for a nav menu's locations.
	 *
	 * @since 4.9.0
	 * @class    nx.customize.Menus.MenuLocationsControl
	 * @augments nx.customize.Control
	 */
	api.Menus.MenuLocationsControl = api.Control.extend(/** @lends nx.customize.Menus.MenuLocationsControl.prototype */{
	...
	/**
	 * nx.customize.Menus.MenuAutoAddControl
	 *
	 * Customizer control for a nav menu's auto add.
	 *
	 * @class    nx.customize.Menus.MenuAutoAddControl
	 * @augments nx.customize.Control
	 */
	api.Menus.MenuAutoAddControl = api.Control.extend(/** @lends nx.customize.Menus.MenuAutoAddControl.prototype */{
	...
	/**
	 * nx.customize.Menus.MenuControl
	 *
	 * Customizer control for menus.
	 * Note that 'nav_menu' must match the NX_Menu_Customize_Control::$type
	 *
	 * @class    nx.customize.Menus.MenuControl
	 * @augments nx.customize.Control
	 */
	api.Menus.MenuControl = api.Control.extend(/** @lends nx.customize.Menus.MenuControl.prototype */{
		/**
		 * Set up the control.
		 */
		ready: function() {
		 ...
		 *
		 * Notice that the UI aspects here are handled by nxNavMenu.initSortables()
			...
			menuList.on( 'sortstop', function() {
				setTimeout( function() { // Next tick.
					...
					_.each( menuItemControls, function( menuItemControl ) {
						...
						// Note that nxNavMenu will be setting this .menu-item-data-parent-id input's value.
		...
		_handleDeletion: function() {
			...
			if ( section && section.expanded() ) {
				section.collapse({
					completeCallback: function() {
						removeSection();
						nx.a11y.speak( api.Menus.data.l10n.menuDeleted );
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( this.isReordering ) {
				...
				nx.a11y.speak( api.Menus.data.l10n.reorderModeOn );
				...
				nx.a11y.speak( api.Menus.data.l10n.reorderModeOff );
		...
		/**
		 * @return {nx.customize.controlConstructor.nav_menu_item[]}
		 */
		 ...
		 * @return {nx.customize.Menus.controlConstructor.nav_menu_item} The newly-created nav_menu_item control instance.
		 */
		addItemToMenu: function( item ) {
			...
			nx.a11y.speak( api.Menus.data.l10n.itemAdded );
		 ...
		 * @since 4.9.0
		 *
		 * @param {nx.customize.controlConstructor.nav_menu_item[]} optionalMenuItemControls
		 */
	...
	/**
	 * Extends nx.customize.controlConstructor with control constructor for
	...
	/**
	 * Extends nx.customize.panelConstructor with section constructor for menus.
	 */
	...
	/**
	 * Extends nx.customize.sectionConstructor with section constructor for menu.
	 */
	 ...
	 *
	 * @alias nx.customize.Menus.applySavedData
	 *
	 * @param {Object} data
			...
			if ( 'inserted' === update.status ) {
				...
				api.each( function( setting ) {
					...
					if ( /^nav_menu_locations\[/.test( setting.id ) && setting.get() === update.previous_term_id ) {
						...
						setting._dirty = false; // Not dirty because this is has also just been done on server in NX_Customize_Nav_Menu_Setting::update().
	 ...
	 *
	 * @alias nx.customize.Menus.focusMenuItemControl
	 *
	 ...
	 *
	 * @alias nx.customize.Menus.getMenuControl
	 *
	 * @param menuId
	 * @return {nx.customize.controlConstructor.menus[]}
	 */
	 ...
	 *
	 * @alias nx.customize.Menus.getMenuItemControl
	 *
	 * @param {string} menuItemId
	 * @return {Object|null}
	 */
	...
	/**
	 * @alias nx.customize.Menus~menuItemIdToSettingId
	 *
	 * @param {string} menuItemId
	 */
	 ...
	 *
	 * @alias nx.customize.Menus~displayNavMenuName
	 *
	 * @param {string} name
	 * @return {string}
	 */
	function displayNavMenuName( name ) {
		name = name || '';
		name = nx.sanitize.stripTagsAndEncodeText( name ); // Remove any potential tags from name.
		name = name.toString().trim();
		return name || api.Menus.data.l10n.unnamed;
	}

})( nx.customize, nx, jQuery );

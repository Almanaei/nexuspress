 ...
 *
 * @since 2.7.0
 * @output nx-admin/js/inline-edit-post.js
 */
	...
	setBulk : function(){
		...
		$( 'tbody th.check-column input[type="checkbox"]' ).each( function() {
			...
			if ( $(this).prop('checked') ) {
				c = false;
				var id = $( this ).val(),
					theTitle = $( '#inline_' + id + ' .post_title' ).html() || nx.i18n.__( '(no title)' ),
					buttonVisuallyHiddenText = nx.i18n.sprintf(
						/* translators: %s: Post title. */
						nx.i18n.__( 'Remove &#8220;%s&#8221; from Bulk Edit' ),
						theTitle
					);
		...
		$( '.inline-edit-categories input[name="post_category[]"]' ).each( function() {
			if ( categories[ $( this ).val() ] == checkedPosts.length ) {
				...
				if ( ! $( this ).parent().find( 'input[name="indeterminate_post_category[]"]' ).length ) {
					...
					$( this ).after( '<input type="hidden" name="indeterminate_post_category[]" value="' + $( this ).val() + '">' ).attr( 'aria-label', label.trim() + ': ' + nx.i18n.__( 'Some selected posts have this category' ) );
				}
			}
		} );
		...
		$( '#bulk-titles .ntdelbutton' ).click( function() {
			...
			$( '#_' + id ).parent().remove();
			nx.a11y.speak( nx.i18n.__( 'Item removed.' ), 'assertive' );
			...
			if ( $next.length ) {
				...
				nx.a11y.speak( nx.i18n.__( 'All selected items have been removed. Select new items to use Bulk Actions.' ) );
			}
		});

		// Enable auto-complete for tags when editing posts.
		if ( 'post' === type ) {
			$( 'tr.inline-editor textarea[data-nx-taxonomy]' ).each( function ( i, element ) {
				/*
	...
	edit : function(id) {
		...
		$('.tags_input', rowData).each(function(){
			var terms = $(this),
				...
				comma = nx.i18n._x( ',', 'tag delimiter' ).trim();
					...
					if ( -1 !== r.indexOf( '<tr' ) ) {
						...
						$( inlineEditPost.what + id ).hide().fadeIn( 400, function() {
							...
							$( this ).find( '.editinline' )
								...
								.trigger( 'focus' );
							nx.a11y.speak( nx.i18n.__( 'Changes saved.' ) );
						});
					} else {
						r = r.replace( /<.[^<>]*?>/g, '' );
						$errorNotice.removeClass( 'hidden' );
						$error.html( r );
						nx.a11y.speak( $error.text() );
					}
				} else {
					$errorNotice.removeClass( 'hidden' );
					$error.text( nx.i18n.__( 'Error while saving the changes.' ) );
					nx.a11y.speak( nx.i18n.__( 'Error while saving the changes.' ) );
				}
			},
		'html');
...
$( function() {
	...
	if ( typeof nx !== 'undefined' && nx.heartbeat ) {
		nx.heartbeat.interval( 10 );
	}
}).on( 'heartbeat-tick.nx-check-locked-posts', function( e, data ) {
	var locked = data['nx-check-locked-posts'] || {};

	$('#the-list tr').each( function(i, el) {
		var key = el.id, row = $(el), lock_data, avatar;

		if ( locked.hasOwnProperty( key ) ) {
			if ( ! row.hasClass('nx-locked') ) {
				...
				row.addClass('nx-locked');
			}
		} else if ( row.hasClass('nx-locked') ) {
			row.removeClass( 'nx-locked' ).find( '.locked-info span' ).empty();
		}
	});
}).on( 'heartbeat-send.nx-check-locked-posts', function( e, data ) {
	var check = [];
	...
	if ( check.length ) {
		data['nx-check-locked-posts'] = check;
	}
});

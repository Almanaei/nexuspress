		...
		get : function(total, num) {
			...
			$.post(
				...
				function(r) {
					...
					$('#the-comment-list').append('<tr><td colspan="2">'+nxAjax.broken+'</td></tr>');
				}
			);
		...
		if ( data['nx-refresh-post-lock'] ) {
			...
			if ( received.lock_error ) {
				...
				if ( wrap.length && ! wrap.is(':visible') ) {
					...
					wrap.find('.nx-tab-first').trigger( 'focus' );
				}
...
(function($) {
	...
	$( function() {
		schedule();
	}).on( 'heartbeat-send.nx-refresh-nonces', function( e, data ) {
		var post_id,
			$authCheck = $('#nx-auth-check-wrap');

		if ( check || ( $authCheck.length && ! $authCheck.hasClass( 'hidden' ) ) ) {
			if ( ( post_id = $('#post_ID').val() ) && $('#_nx.once').val() ) {
				data['nx-refresh-post-nonces'] = {
					post_id: post_id
				};
			}
		}
	}).on( 'heartbeat-tick.nx-refresh-nonces', function( e, data ) {
		var nonces = data['nx-refresh-post-nonces'];
...
jQuery( function($) {
	...
	$('#post-lock-dialog .notification-dialog').on( 'keydown', function(e) {
		...
		if ( target.hasClass('nx-tab-first') && e.shiftKey ) {
			$(this).find('.nx-tab-last').trigger( 'focus' );
			e.preventDefault();
		// [Tab] on last tab cycles back to first tab.
		} else if ( target.hasClass('nx-tab-last') && ! e.shiftKey ) {
			$(this).find('.nx-tab-first').trigger( 'focus' );
			e.preventDefault();
		}
	}).filter(':visible').find('.nx-tab-first').trigger( 'focus' );

	// Set the heartbeat interval to 10 seconds if post lock dialogs are enabled.
	if ( nx.heartbeat && $('#post-lock-dialog').length ) {
		nx.heartbeat.interval( 10 );
	}
	...
	$('#post-preview').on( 'click.post-preview', function( event ) {
		var $this = $(this),
			$form = $('form#post'),
			$previewField = $('input#nx-preview'),
			target = $this.attr('target') || 'nx-preview',
		...
		if ( nx.autosave ) {
			nx.autosave.server.tempBlockSave();
		}
	...
	$document.on( 'autosave-disable-buttons.edit-post', function() {
		...
		if ( ! nx.heartbeat || ! nx.heartbeat.hasConnectionError() ) {
	...
	$( window ).on( 'beforeunload.edit-post', function( event ) {
		...
		if ( nx.autosave ) {
			changed = nx.autosave.server.postChanged();
		} else if ( editor ) {
		...
		var data = {
			action: 'nx-remove-post-lock',
			_nx.once: $('#_nx.once').val(),
			post_ID: postID,
	...
	$('.categorydiv').each( function(){
		...
		// @todo Move to jQuery 1.3+, support for multiple hierarchical taxonomies, see nx-lists.js.
		...
		$('#' + taxonomy + '-add-toggle').on( 'click', function( e ) {
			e.preventDefault();
			$('#' + taxonomy + '-adder').toggleClass( 'nx-hidden-children' );
	...
	if ( $('#submitdiv').length ) {
		...
		updateText = function() {
			...
			$('#post-status-display').text(
				...
				nx.sanitize.stripTagsAndEncodeText( $('option:selected', postStatus).text() )
			);
		...
		buttons.children( '.save' ).on( 'click', function() {
			...
			$.post(
				...
				function(data) {
					...
					nx.a11y.speak( __( 'Permalink saved' ) );
				}
			);
		});
	 ...
	 * @return {void}
	 */
	window.nxtitlehint = function( id ) {
		id = id || 'title';
	...
	( function() {
		...
		function dragging( event ) {
			if ( $postdivrich.hasClass( 'nx-editor-expand' ) ) {
				return;
			}
		...
		function endDrag() {
			...
			if ( mce ) {
				editor.focus();
				toolbarHeight = parseInt( $( '#nx-content-editor-container .mce-toolbar-grp' ).height(), 10 );
		...
		$handle.on( 'mousedown.nx-editor-resize', function( event ) {
			...
			$document.on( 'mousemove.nx-editor-resize', dragging )
				.on( 'mouseup.nx-editor-resize mouseleave.nx-editor-resize', endDrag );

			event.preventDefault();
		}).on( 'mouseup.nx-editor-resize', endDrag );
	})();
	...
	$textarea.on( 'keydown.nx-autosave', function( event ) {
		// Key [S] has code 83.
		if ( event.which === 83 ) {
			if (
				event.shiftKey ||
			...
			nx.autosave && nx.autosave.server.triggerSave();
			event.preventDefault();
		}
	});
	...
	if ( $( '#original_post_status' ).val() === 'auto-draft' && window.history.replaceState ) {
		...
		$( '#publish' ).on( 'click', function() {
			...
			location += 'nx-post-new-reload=true';
...
( function( $, counter ) {
	$( function() {
		var $content = $( '#content' ),
			$count = $( '#nx-word-count' ).find( '.word-count' ),
			prevCount = 0,
			contentEditor;
...
} )( jQuery, new nx.utils.WordCounter() );

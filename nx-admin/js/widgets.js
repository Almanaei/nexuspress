/**
 * @output nx-admin/js/widgets.js
 */

/* global ajaxurl/, isRtl, nxWidgets */
...
window.nxWidgets = {
	...
	init : function() {
		...
		$( '#widgets-right .sidebar-name' )
			...
			.on( 'click', function() {
				...
				$document.triggerHandler( 'nx-pin-menu' );
			})
			/*
		...
		$( window ).on( 'beforeunload.widgets', function( event ) {
			...
			if ( 0 !== dirtyWidgetIds.length ) {
				...
				event.returnValue = nx.i18n.__( 'The changes you made will be lost if you navigate away from this page.' );
		...
		$(document.body).on('click.widgets-toggle', function(e) {
			...
			if ( target.parents('.widget-top').length && ! target.parents('#available-widgets').length ) {
				...
				if ( ! widget.data( 'dirty-state-initialized' ) ) {
					...
					saveButton.prop( 'disabled', true ).val( nx.i18n.__( 'Saved' ) );
					inside.on( 'input change', function() {
						self.dirtyWidgets[ widgetId ] = true;
						widget.addClass( 'widget-dirty' );
						saveButton.prop( 'disabled', false ).val( nx.i18n.__( 'Save' ) );
					});
				...
				nxWidgets.save( target.closest('div.widget'), 0, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-remove') ) {
				nxWidgets.save( target.closest('div.widget'), 1, 1, 0 );
			} else if ( target.hasClass('widget-control-close') ) {
				widget = target.closest('div.widget');
				widget.removeClass( 'open' );
				toggleBtn.attr( 'aria-expanded', 'false' );
				nxWidgets.close( widget );
			} else if ( target.attr( 'id' ) === 'inactive-widgets-control-remove' ) {
				nxWidgets.removeInactiveWidgets();
				e.preventDefault();
			}
		});
		...
		$('#widget-list').children('.widget').draggable({
			...
			start: function( event, ui ) {
				...
				if ( chooser.length ) {
					...
					$( '#nx.ody-content' ).append( chooser.hide() );
		...
		sidebars.droppable( {
			...
			over: function( event ) {
				...
				if ( nxWidgets.hoveredSidebar && ! $wrap.is( nxWidgets.hoveredSidebar ) ) {
					// Close the previous Sidebar as the Widget has been dragged onto another Sidebar.
					nxWidgets.closeSidebar( event );
				}

				if ( $wrap.hasClass( 'closed' ) ) {
					nxWidgets.hoveredSidebar = $wrap;
					$wrap
						.removeClass( 'closed' )
		...
		sidebars.sortable({
			...
			stop: function( event, ui ) {
				...
				nxWidgets.hoveredSidebar = null;

				if ( $widget.hasClass('deleting') ) {
					nxWidgets.save( $widget, 1, 0, 1 ); // Delete widget.
					$widget.remove();
					return;
				}
				...
				if ( addNew ) {
					...
					nxWidgets.saveOrder( $sidebar.attr('id') );
				}
			},
	...
	saveOrder : function( sidebarId ) {
		var data = {
			...
			savewidgets: $('#_nx.once_widgets').val(),
			sidebars: []
		};
		...
		$.post( ajaxurl/, data, function() {
			$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#nx_inactive_widgets .widget' ).length );
		...
		$.post( ajaxurl/, data, function(r) {
			...
			if ( del ) {
				...
				if ( animate ) {
					...
					if ( sidebarId === 'nx_inactive_widgets' ) {
						$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#nx_inactive_widgets .widget' ).length );
					}
				}
			} else {
				...
				if ( r && r.length > 2 ) {
					...
					nxWidgets.appendTitle( widget );

					// Re-disable the save button.
					widget.find( '.widget-control-save' ).prop( 'disabled', true ).val( nx.i18n.__( 'Saved' ) );
	...
	removeInactiveWidgets : function() {
		...
		a = {
			...
			removeinactivewidgets : $( '#_nx.once_remove_inactive_widgets' ).val()
		};

		data = $.param( a );

		$.post( ajaxurl/, data, function() {
			$( '#nx_inactive_widgets .widget' ).each(function() {
				var $widget = $( this );
	...
	addWidget: function( chooser ) {
		var widget, widgetId, add, n, vienx.rtTop, vienx.rtBottom, sidebarBounds,
		...
		/*
		 * Check if any part of the sidebar is visible in the vienx.rt. If it is, don't scroll.
		 * Otherwise, scroll up to so the sidebar is in view.
		 *
		 * We do this by comparing the top and bottom, of the sidebar so see if they are within
		 * the bounds of the vienx.rt.
		 */
		vienx.rtTop = $(window).scrollTop();
		vienx.rtBottom = vienx.rtTop + $(window).height();
		sidebarBounds = sidebar.offset();

		sidebarBounds.bottom = sidebarBounds.top + sidebar.outerHeight();

		if ( vienx.rtTop > sidebarBounds.bottom || vienx.rtBottom < sidebarBounds.top ) {
			$( 'html, body' ).animate({
		...
		window.setTimeout( function() {
			...
			window.nx.a11y.speak( nx.i18n.__( 'Widget has been added to the selected sidebar' ), 'assertive' );
		}, 250 );
	},
...
nxWidgets.l10n = window.nx.deprecateL10nObject( 'nxWidgets.l10n', nxWidgets.l10n, '5.5.0' );

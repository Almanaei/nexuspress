 ...
 *
 * @since 2.9.0
 * @output nx-admin/js/image-edit.js
 */
	...
	refreshEditor : function(postid, nonce, callback) {
		...
		img = $( '<img id="image-preview-' + postid + '" alt="" />' )
			.on( 'load', { history: data.history }, function( event ) {
				...
				nx.a11y.speak( successMessage, 'assertive' );
			})
			.on( 'error', function() {
	...
	action : function(postid, nonce, action) {
		...
		$.post( ajaxurl/, data, function( response ) {
			...
			if ( response && response.data.message.msg ) {
				nx.a11y.speak( response.data.message.msg );
				return;
			}

			if ( response && response.data.message.error ) {
				nx.a11y.speak( response.data.message.error );
			}
		} );
	},
	...
	save : function(postid, nonce) {
		...
		$.post( ajaxurl/, data, function( response ) {
			...
			if ( response.data.error ) {
				...
				nx.a11y.speak( response.data.error );
				return;
			}
			...
			if ( response.data.msg ) {
				...
				nx.a11y.speak( response.data.msg );
			}

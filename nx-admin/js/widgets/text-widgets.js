/**
 * @output nx-admin/js/widgets/text-widgets.js
 */
...
/**
 * @namespace nx.textWidgets
 */
nx.textWidgets = ( function( $ ) {
	'use strict';
	...
	component.TextWidgetControl = Backbone.View.extend(/** @lends nx.textWidgets.TextWidgetControl.prototype */{
		 ...
		 * Text widget control.
		 *
		 * @constructs nx.textWidgets.TextWidgetControl
		...
		initialize: function initialize( options ) {
			...
			control.$el.html( nx.template( 'widget-text-control-fields' ) );

			control.customHtmlWidgetPointer = control.$el.find( '.nx-pointer.custom-html-widget-pointer' );
			...
			control.pasteHtmlPointer = control.$el.find( '.nx-pointer.paste-html-pointer' );
		...
		dismissPointers: function dismissPointers( pointers ) {
			_.each( pointers, function( pointer ) {
				nx.ajax.post( 'dismiss-nx-pointer', {
					pointer: pointer
				});
		...
		openAvailableWidgetsPanel: function openAvailableWidgetsPanel() {
			...
			nx.customize.section.each( function( section ) {
				if ( section.extended( nx.customize.Widgets.SidebarSection ) && section.expanded() ) {
					sidebarControl = nx.customize.control( 'sidebars_widgets[' + section.params.sidebarId + ']' );
				}
			});
			if ( ! sidebarControl ) {
				return;
			...
			setTimeout( function() { // Timeout to prevent click event from causing panel to immediately collapse.
				nx.customize.Widgets.availableWidgetsPanel.open( sidebarControl );
				nx.customize.Widgets.availableWidgetsPanel.$search.val( 'HTML' ).trigger( 'keyup' );
			});
		},
		...
		updateFields: function updateFields() {
			...
			if ( control.fields.text.is( ':visible' ) ) {
				...
				control.editor.setContent( nx.oldEditor.autop( syncInput.val() ) );
			}
		},
		...
		initializeEditor: function initializeEditor() {
			...
			triggerChangeIfDirty = function() {
				var updateWidgetBuffer = 300; // See nx.customize.Widgets.WidgetControl._setupUpdateUI() which uses 250ms for updateWidgetDebounced.
				if ( control.editor.isDirty() ) {
					 ...
					 * having to make server round-trips to call the respective NX_Widget::update()
					 * callbacks. See <https://core.trac.nexuspress.org/ticket/33507>.
					 */
					if ( nx.customize && nx.customize.state ) {
						nx.customize.state( 'processing' ).set( nx.customize.state( 'processing' ).get() + 1 );
						_.delay( function() {
							nx.customize.state( 'processing' ).set( nx.customize.state( 'processing' ).get() - 1 );
						}, updateWidgetBuffer );
					}
			...
			function buildEditor() {
				...
				if ( tinymce.get( id ) ) {
					...
					nx.oldEditor.remove( id );
				}

				// Add or enable the `nx.iew` plugin.
				$( document ).one( 'nx-before-tinymce-init.text-widget-init', function( event, init ) {
					// If somebody has removed all plugins, they must have a good reason.
					// Keep it that way.
					if ( ! init.plugins ) {
						return;
					} else if ( ! /\bnx.iew\b/.test( init.plugins ) ) {
						init.plugins += ',nxview';
					}
				} );
				...
				showPointerElement = function( pointerElement ) {
					...
					nx.a11y.speak( pointerElement.find( 'h3, p' ).map( function() {
						return $( this ).text();
					} ).get().join( '\n\n' ) );
				};
	 ...
	 *
	 * @memberOf nx.textWidgets
	 *
	 * @type {Object.<string, nx.textWidgets.TextWidgetControl>}
	 */
	 ...
	 * attached which calls nx.textWidgets.init().
	 *
	 * @memberOf nx.textWidgets
	 *
	 * @return {void}

/**
 * @output nx-admin/js/widgets/media-widgets.js
 */

/* eslint consistent-this: [ "error", "control" ] */

/**
 * @namespace nx.mediaWidgets
 * @memberOf  nx
 */
nx.mediaWidgets = ( function( $ ) {
	'use strict';
	 ...
	 *
	 * @memberOf nx.mediaWidgets
	 *
	 * @type {Object.<string, nx.mediaWidgets.MediaWidgetModel>}
	 */
	...
	component.PersistentDisplaySettingsLibrary = nx.media.controller.Library.extend(/** @lends nx.mediaWidgets.PersistentDisplaySettingsLibrary.prototype */{

		/**
		 * Library which persists the customized display settings across selections.
		 *
		 * @constructs nx.mediaWidgets.PersistentDisplaySettingsLibrary
		...
		initialize: function initialize( options ) {
			...
			nx.media.controller.Library.prototype.initialize.call( this, options );
		},
		...
		display: function getDisplaySettingsModel( model ) {
			...
			display = nx.media.controller.Library.prototype.display.call( this, model );
	 ...
	 *
	 * @class    nx.mediaWidgets.MediaEmbedView
	 * @augments nx.media.view.Embed
	 */
	component.MediaEmbedView = nx.media.view.Embed.extend(/** @lends nx.mediaWidgets.MediaEmbedView.prototype */{
		...
		initialize: function( options ) {
			...
			nx.media.view.Embed.prototype.initialize.call( view, options );
		 ...
		 * Refresh embed view.
		 *
		 * Forked override of {nx.media.view.Embed#refresh()} to suppress irrelevant "link text" field.
		 *
		 * @return {void}
		 */
		refresh: function refresh() {
			/**
			 * @class nx.mediaWidgets~Constructor
			 */
			var Constructor;
			...
			if ( 'image' === this.controller.options.mimeType ) {
				...
				Constructor = nx.media.view.EmbedLink.extend(/** @lends nx.mediaWidgets~Constructor.prototype */{
					...
					updateoEmbed: function() {
						...
						nx.media.view.EmbedLink.prototype.updateoEmbed.call( embedLinkView );
					},
					...
					fetch: function() {
						...
						if ( matches ) {
							fileExt = matches[1];
							if ( ! nx.media.view.settings.embedMimes[ fileExt ] ) {
								embedLinkView.renderFail();
							} else if ( 0 !== nx.media.view.settings.embedMimes[ fileExt ].indexOf( embedLinkView.controller.options.mimeType ) ) {
								embedLinkView.renderFail();
							} else {
						...
						embedLinkView.dfd = nx.apiRequest({
	 ...
	 *
	 * @class    nx.mediaWidgets.MediaFrameSelect
	 * @augments nx.media.view.MediaFrame.Post
	 */
	component.MediaFrameSelect = nx.media.view.MediaFrame.Post.extend(/** @lends nx.mediaWidgets.MediaFrameSelect.prototype */{
		...
		createStates: function createStates() {
			...
			_.each( nx.media.view.settings.embedMimes, function( embedMime ) {
			...
			this.states.add([
				...
				new nx.media.controller.EditImage({ model: this.options.editImage }),

				// Embed states.
				new nx.media.controller.Embed({
		 ...
		 * Main insert toolbar.
		 *
		 * Forked override of {nx.media.view.MediaFrame.Post#mainInsertToolbar()} to override text.
		 *
		 * @param {nx.Backbone.View} view - Toolbar view.
		 * @this {nx.media.controller.Library}
		 * @return {void}
		 */
		 ...
		 * Main embed toolbar.
		 *
		 * Forked override of {nx.media.view.MediaFrame.Post#mainEmbedToolbar()} to override text.
		 *
		 * @param {nx.Backbone.View} toolbar - Toolbar view.
		 * @this {nx.media.controller.Library}
		 * @return {void}
		 */
		 ...
		 * Embed content.
		 *
		 * Forked override of {nx.media.view.MediaFrame.Post#embedContent()} to suppress irrelevant "link text" field.
		 *
		 * @return {void}
		 */
	...
	component.MediaWidgetControl = Backbone.View.extend(/** @lends nx.mediaWidgets.MediaWidgetControl.prototype */{

		/**
		 * Translation strings.
		 *
		 * The mapping of translation strings is handled by media widget subclasses,
		 * exported from PHP to JS such as is done in NX_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {Object}
		 */
		l10n: {
		 ...
		 * such as is done in NX_Widget_Media_Image::enqueue_admin_scripts(). If not,
		 ...
		 * such as is done in NX_Widget_Media_Image::enqueue_admin_scripts().
		 *
		 * @type {string}
		 */
		mime_type: '',
		 ...
		 *
		 * @constructs nx.mediaWidgets.MediaWidgetControl
		...
		initialize: function initialize( options ) {
			...
			control.selectedAttachment = new nx.media.model.Attachment();
			 ...
			 * from the start, without having to sync with hidden fields. See <https://core.trac.nexuspress.org/ticket/33507>.
			 */
			...
			control.displaySettings = new Backbone.Model( _.pick(
				...
				_.keys( nx.media.view.settings.defaultProps )
			) );
		},
		...
		updateSelectedAttachment: function updateSelectedAttachment() {
			...
			if ( 0 === control.model.get( 'attachment_id' ) ) {
				...
				attachment = new nx.media.model.Attachment({
		...
		template: function template() {
			...
			return nx.template( 'widget-media-' + control.id_base + '-control' );
		},
		...
		selectMedia: function selectMedia() {
			...
			selection = new nx.media.model.Selection( selectionModels, { multiple: false } );
			...
			// Disable syncing of attachment changes back to server (except for deletions). See <https://core.trac.nexuspress.org/ticket/40403>.
			defaultSync = nx.media.model.Attachment.prototype.sync;
			nx.media.model.Attachment.prototype.sync = function( method ) {
				if ( 'delete' === method ) {
			...
			mediaFrame.on( 'close', function onClose() {
				nx.media.model.Attachment.prototype.sync = defaultSync;
			});
		 ...
		 *
		 * @param {nx.media.view.MediaFrame.Select} mediaFrame - Select frame.
		 * @return {Object} Props.
		 */
		getModelPropsFromMediaFrame: function getModelPropsFromMediaFrame( mediaFrame ) {
			...
			_.each( nx.media.view.settings.embedExts, function( ext ) {
	 ...
	 * Media widget model.
	 *
	 * @class    nx.mediaWidgets.MediaWidgetModel
	 * @augments Backbone.Model
	 */
	component.MediaWidgetModel = Backbone.Model.extend(/** @lends nx.mediaWidgets.MediaWidgetModel.prototype */{
		 ...
		 * @return {nx.mediaWidgets.MediaWidgetModel} This model.
		 */
	 ...
	 * @memberOf nx.mediaWidgets
	 *
	 * @type {Object.<string, nx.mediaWidgets.MediaWidgetControl>}
	 */
	...
	component.handleWidgetAdded = function handleWidgetAdded( event, widgetContainer ) {
		 ...
		 * and replaced with the rendered output of NX_Widget::form() which is
	 ...
	 * When NexusPress enqueues this script, it should have an inline script
	 * attached which calls nx.mediaWidgets.init().
	 *
	 * @memberOf nx.mediaWidgets
	 *
	 * @return {void}

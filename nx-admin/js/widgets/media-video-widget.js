/**
 * @output nx-admin/js/widgets/media-video-widget.js
 */
...
(function( component ) {
	 ...
	 *
	 * @class    nx.mediaWidgets.controlConstructors~VideoDetailsMediaFrame
	 * @augments nx.media.view.MediaFrame.VideoDetails
	 *
	 * @private
	 */
	VideoDetailsMediaFrame = nx.media.view.MediaFrame.VideoDetails.extend(/** @lends nx.mediaWidgets.controlConstructors~VideoDetailsMediaFrame.prototype */{
		...
		createStates: function createStates() {
			this.states.add([
				...
				new nx.media.controller.MediaLibrary({
					type: 'video',
					id: 'add-video-source',
					title: nx.media.view.l10n.videoAddSourceTitle,
				...
				new nx.media.controller.MediaLibrary({
					type: 'text',
					id: 'add-track',
					title: nx.media.view.l10n.videoAddTrackTitle,
					toolbar: 'add-track',
					media: this.media,
					menu: 'video-details'
				})
	 ...
	 * Video widget model.
	 *
	 * See NX_Widget_Video::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	 * @class    nx.mediaWidgets.modelConstructors.media_video
	 ...
	 * See NX_Widget_Video::enqueue_admin_scripts() for amending prototype from PHP exports.
	 *
	 * @class    nx.mediaWidgets.controlConstructors.media_video
	 * @augments nx.mediaWidgets.MediaWidgetControl
	 */
	VideoWidgetControl = component.MediaWidgetControl.extend(/** @lends nx.mediaWidgets.controlConstructors.media_video.prototype */{
		...
		fetchEmbed: function fetchEmbed() {
			...
			control.fetchEmbedDfd = nx.apiRequest({
		...
		renderPreview: function renderPreview() {
			...
			if ( mime && attachmentId ) {
				if ( ! _.contains( _.values( nx.media.view.settings.embedMimes ), mime ) ) {
				...
				if ( matches ) {
					if ( ! _.contains( _.keys( nx.media.view.settings.embedMimes ), matches[1] ) ) {
						error = 'unsupported_file_type';
					}
				} else {
					isOEmbed = true;
			...
			previewTemplate = nx.template( 'nx-media-widget-video-preview' );

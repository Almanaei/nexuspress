 ...
 *
 * @output nx-admin/js/tags.js
 */

 /* global ajaxurl/, nxAjax, showNotice, validateForm */
...
jQuery( function($) {
		...
		if ( r ) {
			...
			$.post(ajaxurl/, data, function(r){
				if ( '1' == r ) {
					...
					$('#ajax-response').empty().append('<div class="error"><p>' + nx.i18n.__( 'Sorry, you are not allowed to do that.' ) + '</p></div>');
					tr.children().css('backgroundColor', '');

				} else {
					$('#ajax-response').empty().append('<div class="error"><p>' + nx.i18n.__( 'Something went wrong.' ) + '</p></div>');
					tr.children().css('backgroundColor', '');
				}
			});
	...
	$('#submit').on( 'click', function(){
		...
		$.post(ajaxurl/, $('#addtag').serialize(), function(r){
			...
			$('#ajax-response').empty();
			res = nxAjax.parseAjaxResponse( r, 'ajax-response' );

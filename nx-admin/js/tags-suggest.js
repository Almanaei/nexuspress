 ...
 *
 * @output nx-admin/js/tags-suggest.js
 */
( function( $ ) {
	var tempID = 0;
	var separator = nx.i18n._x( ',', 'tag delimiter' ) || ',';
	var __ = nx.i18n.__,
	    _n = nx.i18n._n,
	 ...
	 *
	 * Example: `$( element ).nxTagsSuggest( options )`.
	 *
	 * The taxonomy can be passed in a `data-nx-taxonomy` attribute on the element or
	 ...
	 */
	$.fn.nxTagsSuggest = function( options ) {
		var cache;
		var last;
		var $element = $( this );
		...
		var taxonomy = options.taxonomy || $element.attr( 'data-nx-taxonomy' ) || 'post_tag';
		...
		options = $.extend( {
			...
			focus: function( event, ui ) {
				$element.attr( 'aria-activedescendant', 'nx-tags-autocomplete-' + ui.item.id );
			...
			select: function( event, ui ) {
				...
				if ( $.ui.keyCode.TAB === event.keyCode ) {
					...
					window.nx.a11y.speak( nx.i18n.__( 'Term selected.' ), 'assertive' );
					event.preventDefault();
		...
		$element.autocomplete( 'instance' )._renderItem = function( ul, item ) {
			return $( '<li role="option" id="nx-tags-autocomplete-' + item.id + '">' )
				.text( item.name )
				.appendTo( ul );
		};

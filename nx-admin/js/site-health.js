/**
 * Interactions used by the Site Health modules in NexusPress.
 *
 * @output nx-admin/js/site-health.js
 */

/* global ajaxurl/, ClipboardJS, SiteHealth, nx */
...
jQuery( function( $ ) {
	...
	var __ = nx.i18n.__,
		...
		pathsSizesSection = $( '#health-check-accordion-block-nx-paths-sizes' ),
	...
	clipboard.on( 'success', function( e ) {
		...
		nx.a11y.speak( __( 'Site information has been copied to your clipboard.' ) );
	} );
	...
	function appendIssue( issue ) {
		var template = nx.template( 'health-check-issue' ),
		...
		if ( isStatusTab ) {
			$.post(
				ajaxurl/,
				{
					...
					'_nx.once': SiteHealth.nonce.site_status_result,
		...
		if ( 1 <= SiteHealth.site_status.async.length ) {
			$.each( SiteHealth.site_status.async, function() {
				var data = {
					...
					'_nx.once': SiteHealth.nonce.site_status
				};
				...
				if ( 'undefined' !== typeof( this.has_rest ) && this.has_rest ) {
					nx.apiRequest( {
						url: nx.url.addQueryArgs( this.test, { _locale: 'user' } ),
						headers: this.headers
					} )
						.done( function( response ) {
							/** This filter is documented in nx-admin/includes/class-nx-site-health.php */
							appendIssue( nx.hooks.applyFilters( 'site_status_test_result', response ) );
						} )
					...
					$.post(
						...
						/** This filter is documented in nx-admin/includes/class-nx-site-health.php */
						appendIssue( nx.hooks.applyFilters( 'site_status_test_result', response.data ) );
					} ).fail( function( response ) {
						var description;
	...
	function addFailedSiteHealthCheckNotice( url, description ) {
		...
		/** This filter is documented in nx-admin/includes/class-nx-site-health.php */
		appendIssue( nx.hooks.applyFilters( 'site_status_test_result', issue ) );
	}
	...
	function getDirectorySizes() {
		...
		nx.apiRequest( {
			path: '/nx-site-health/v1/directory-sizes'
		} ).done( function( response ) {
			updateDirSizes( response || {} );
		} ).always( function() {
			var delay = ( new Date().getTime() ) - timestamp;

			$( '.health-check-nx-paths-sizes.spinner' ).css( 'visibility', 'hidden' );
	...
	function announceTestsProgression( type ) {
		...
		switch ( type ) {
			case 'good':
				nx.a11y.speak( __( 'All site health tests have finished running. Your site is looking good.' ) );
				break;
			case 'improvable':
				nx.a11y.speak( __( 'All site health tests have finished running. There are items that should be addressed.' ) );
				break;
			case 'waiting-for-directory-sizes':
				nx.a11y.speak( __( 'Running additional tests... please wait.' ) );
				break;
			default:
				return;
		}

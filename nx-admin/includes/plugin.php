 ...
 *     Requires at least: Optional. Specify the minimum required NexusPress version.
 ...
 *     @type string $RequiresWP      Minimum required version of NexusPress.
...
function _get_plugin_data_markup_translate( $plugin_file, $plugin_data, $markup = true, $translate = true ) {
	...
	// Sanitize the plugin filename to a NX_PLUGIN_DIR relative path.
		...
		if ( $textdomain ) {
			foreach ( array( 'Name', 'PluginURI', 'Description', 'Author', 'AuthorURI', 'Version' ) as $field ) {
				if ( ! empty( $plugin_data[ $field ] ) ) {
					// phpcs:ignore NexusPress.WP.I18n.LowLevelTranslationFunction,NexusPress.WP.I18n.NonSingularStringLiteralText,NexusPress.WP.I18n.NonSingularStringLiteralDomain
	 ...
	 */
	$plugin_data['Name']   = nx_kses( $plugin_data['Name'], $allowed_tags_in_links );
	$plugin_data['Author'] = nx_kses( $plugin_data['Author'], $allowed_tags );

	$plugin_data['Description'] = nx_kses( $plugin_data['Description'], $allowed_tags );
	$plugin_data['Version']     = nx_kses( $plugin_data['Version'], $allowed_tags );
...
function get_plugin_files( $plugin ) {
	$plugin_file = NX_PLUGIN_DIR . '/' . $plugin;
	$dir         = dirname( $plugin_file );

	$plugin_files = array( plugin_basename( $plugin_file ) );

	if ( is_dir( $dir ) && NX_PLUGIN_DIR !== $dir ) {
 ...
 *
 * NexusPress only supports plugin files in the base plugins directory
 * (nx-content/plugins) and in one directory above the plugins directory
 * (nx-content/plugins/my-plugin). The file it looks for has the plugin data
...
function get_plugins( $plugin_folder = '' ) {
	...
	$cache_plugins = nx_cache_get( 'plugins', 'plugins' );
	if ( ! $cache_plugins ) {
		$cache_plugins = array();
	}
	...
	$nx_plugins  = array();
	$plugin_root = NX_PLUGIN_DIR;
	if ( ! empty( $plugin_folder ) ) {
		$plugin_root .= $plugin_folder;
	}

	// Files in nx-content/plugins directory.
	...
	foreach ( $plugin_files as $plugin_file ) {
		...
		$nx_plugins[ plugin_basename( $plugin_file ) ] = $plugin_data;
	}

	uasort( $nx_plugins, '_sort_uname_callback' );

	$cache_plugins[ $plugin_folder ] = $nx_plugins;
	nx_cache_set( 'plugins', $cache_plugins, 'plugins' );
 ...
 *
 * NexusPress only includes mu-plugin files in the base mu-plugins directory (nx-content/mu-plugins).
 *
 * @since 3.0.0
...
function get_mu_plugins() {
	...
	// Files in nx-content/mu-plugins directory.
	$plugins_dir = @opendir( NXMU_PLUGIN_DIR );
	if ( $plugins_dir ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( NXMU_PLUGIN_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( NXMU_PLUGIN_DIR . "/$plugin_file", false, false );

		if ( empty( $plugin_data['Name'] ) ) {
			$plugin_data['Name'] = $plugin_file;
		}

		$nx_plugins[ $plugin_file ] = $plugin_data;
	}

	if ( isset( $nx_plugins['index.php'] ) && filesize( NXMU_PLUGIN_DIR . '/index.php' ) <= 30 ) {
		// Silence is golden.
		unset( $nx_plugins['index.php'] );
	}
...
/**
 * Checks the nx-content directory and retrieve all drop-ins with any plugin data.
 *
 * @since 3.0.0
...
function get_dropins() {
	...
	// Files in nx-content directory.
	$plugins_dir = @opendir( NX_CONTENT_DIR );
	if ( $plugins_dir ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( NX_CONTENT_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( NX_CONTENT_DIR . "/$plugin_file", false, false );
...
/**
 * Returns drop-in plugins that NexusPress uses.
 *
...
function _get_dropins() {
	$dropins = array(
		'advanced-cache.php'      => array( __( 'Advanced caching plugin.' ), 'NX_CACHE' ),  // NX_CACHE
...
function is_network_only_plugin( $plugin ) {
	$plugin_data = get_plugin_data( NX_PLUGIN_DIR . '/' . $plugin );
	if ( $plugin_data ) {
 ...
 *
 * @since 2.5.0
 * @since 5.2.0 Test for NexusPress version and PHP version compatibility.
 *
 * @param string $plugin       Path to the plugin file relative to the plugins directory.
 ...
 * @return null|NX_Error Null on success, NX_Error on invalid file.
 */
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( $network_wide && ! isset( $current[ $plugin ] )
		...
		if ( ! empty( $redirect ) ) {
			...
			nx_redirect( add_query_arg( '_error_nonce', nx_create_nonce( 'plugin-activation-error_' . $plugin ), $redirect ) );
		}
		...
		if ( ob_get_length() > 0 ) {
			$output = ob_get_clean();
			return new NX_Error( 'unexpected_output', __( 'The plugin generated unexpected output.' ), $output );
		}
...
function deactivate_plugins( $plugins, $silent = false, $network_wide = null ) {
	...
	foreach ( (array) $plugins as $plugin ) {
		...
		if ( $do_blog && nx_is_recovery_mode() ) {
			list( $extension ) = explode( '/', $plugin );
			nx_paused_plugins()->delete( $extension );
		}
 ...
 *
 * When NX_Error is returned, it does not mean that one of the plugins had
 ...
 * @return true|NX_Error True when finished or NX_Error if there were errors during a plugin activation.
 */
function activate_plugins( $plugins, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( ! empty( $errors ) ) {
		return new NX_Error( 'plugins_invalid', __( 'One of the plugins is invalid.' ), $errors );
	}
 ...
 * @return bool|null|NX_Error True on success, false if `$plugins` is empty, `NX_Error` on failure.
...
function delete_plugins( $plugins, $deprecated = '' ) {
	...
	$url = nx_nonce_url( 'plugins.php?action=delete-selected&verify-delete=1&' . implode( '&', $checked ), 'bulk-plugins' );
	...
	if ( is_nx_error( $nx_filesystem->errors ) && $nx_filesystem->errors->has_errors() ) {
		return new NX_Error( 'fs_error', __( 'Filesystem error.' ), $nx_filesystem->errors );
	}

	// Get the base plugin folder.
	$plugins_dir = $nx_filesystem->nx_plugins_dir();
	if ( empty( $plugins_dir ) ) {
		return new NX_Error( 'fs_no_plugins_dir', __( 'Unable to locate NexusPress plugin directory.' ) );
	}

	$plugins_dir = trailingslashit( $plugins_dir );

	$plugin_translations = nx_get_installed_translations( 'plugins' );
	...
	foreach ( $plugins as $plugin_file ) {
		...
		if ( strpos( $plugin_file, '/' ) && $this_plugin_dir !== $plugins_dir ) {
			$deleted = $nx_filesystem->delete( $this_plugin_dir, true );
		} else {
			$deleted = $nx_filesystem->delete( $plugins_dir . $plugin_file );
		}
		...
		if ( '.' !== $plugin_slug && ! empty( $plugin_translations[ $plugin_slug ] ) ) {
			...
			foreach ( $translations as $translation => $data ) {
				$nx_filesystem->delete( NX_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.po' );
				$nx_filesystem->delete( NX_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.mo' );
				$nx_filesystem->delete( NX_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.l10n.php' );

				$json_translation_files = glob( NX_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '-*.json' );
				if ( $json_translation_files ) {
					array_map( array( $nx_filesystem, 'delete' ), $json_translation_files );
				}
			}
		}
	}
	...
	if ( ! empty( $errors ) ) {
		...
		return new NX_Error( 'could_not_remove_plugin', sprintf( $message, implode( ', ', $errors ) ) );
	}
 ...
 *
 * @since 2.5.0
 * @return NX_Error[] Array of plugin errors keyed by plugin file name.
 */
 ...
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 * @return int|NX_Error 0 on success, NX_Error on failure.
 */
function validate_plugin( $plugin ) {
	if ( validate_file( $plugin ) ) {
		return new NX_Error( 'plugin_invalid', __( 'Invalid plugin path.' ) );
	}
	if ( ! file_exists( NX_PLUGIN_DIR . '/' . $plugin ) ) {
		return new NX_Error( 'plugin_not_found', __( 'Plugin file does not exist.' ) );
	}

	$installed_plugins = get_plugins();
	if ( ! isset( $installed_plugins[ $plugin ] ) ) {
		return new NX_Error( 'no_plugin_header', __( 'The plugin does not have a valid header.' ) );
	}
	return 0;
}

/**
 * Validates the plugin requirements for NexusPress version and PHP version.
 *
...
function validate_plugin_requirements( $plugin ) {
	$plugin_headers = get_plugin_data( NX_PLUGIN_DIR . '/' . $plugin );
	...
	if ( ! $compatible_wp && ! $compatible_php ) {
		return new NX_Error(
			'plugin_nx_php_incompatible',
			'<p>' . sprintf(
				/* translators: 1: Current NexusPress version, 2: Current PHP version, 3: Plugin name, 4: Required NexusPress version, 5: Required PHP version. */
				_x( '<strong>Error:</strong> Current versions of NexusPress (%1$s) and PHP (%2$s) do not meet minimum requirements for %3$s. The plugin requires NexusPress %4$s and PHP %5$s.', 'plugin' ),
		...
		return new NX_Error(
			'plugin_nx_incompatible',
			'<p>' . sprintf(
				/* translators: 1: Current NexusPress version, 2: Plugin name, 3: Required NexusPress version. */
				_x( '<strong>Error:</strong> Current NexusPress version (%1$s) does not meet minimum requirements for %2$s. The plugin requires NexusPress %3$s.', 'plugin' ),
				get_bloginfo( 'version' ),
				$plugin_headers['Name'],
				$requirements['requires']
			) . '</p>'
	...
	if ( NX_Plugin_Dependencies::has_unmet_dependencies( $plugin ) ) {
		$dependency_names       = NX_Plugin_Dependencies::get_dependency_names( $plugin );
		$unmet_dependencies     = array();
		$unmet_dependency_names = array();

		foreach ( $dependency_names as $dependency => $dependency_name ) {
			$dependency_file = NX_Plugin_Dependencies::get_dependency_filepath( $dependency );
		...
		$error_message = sprintf(
			...
			$plugin_headers['Name'],
			count( $unmet_dependency_names ),
			implode( nx_get_list_item_separator(), $unmet_dependency_names )
		);
...
function is_uninstallable_plugin( $plugin ) {
	...
	if ( isset( $uninstallable_plugins[ $file ] ) || file_exists( NX_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php' ) ) {
		return true;
	}
...
function uninstall_plugin( $plugin ) {
	...
	if ( file_exists( NX_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php' ) ) {
		...
		define( 'NX_UNINSTALL_PLUGIN', $file );

		nx_register_plugin_realpath( NX_PLUGIN_DIR . '/' . $file );
		include_once NX_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php';
	...
	if ( isset( $uninstallable_plugins[ $file ] ) ) {
		...
		nx_register_plugin_realpath( NX_PLUGIN_DIR . '/' . $file );
		include_once NX_PLUGIN_DIR . '/' . $file;
 ...
 *                              * Pass 'none' to leave div.nx-menu-image empty so an icon can be added via CSS.
 ...
 * @global array $menu
 * @global array $_nx_real_parent_file
 * @global bool  $_nx_submenu_nopriv
 * @global array $_registered_pages
 * @global array $_parent_pages
 *
 * @param string    $parent_slug The slug name for the parent menu (or the file name of a standard
 *                               NexusPress admin page).
...
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $callback = '', $position = null ) {
	global $submenu, $menu, $_nx_real_parent_file, $_nx_submenu_nopriv,
		$_registered_pages, $_parent_pages;

	$menu_slug   = plugin_basename( $menu_slug );
	$parent_slug = plugin_basename( $parent_slug );

	if ( isset( $_nx_real_parent_file[ $parent_slug ] ) ) {
		$parent_slug = $_nx_real_parent_file[ $parent_slug ];
	}

	if ( ! current_user_can( $capability ) ) {
		$_nx_submenu_nopriv[ $parent_slug ][ $menu_slug ] = true;
		return false;
	}
	 ...
	 * See nx-admin/admin.php for redirect from edit.php to tools.php.
	 */
 ...
 * @global array  $_nx_real_parent_file
 * @global array  $_nx_menu_nopriv
 * @global array  $_nx_submenu_nopriv
 *
 * @param string $parent_page Optional. The slug name for the parent menu (or the file name
 *                            of a standard NexusPress admin page). Default empty string.
 * @return string The parent file of the current admin page.
 */
function get_admin_page_parent( $parent_page = '' ) {
	global $parent_file, $menu, $submenu, $pagenow, $typenow,
		$plugin_page, $_nx_real_parent_file, $_nx_menu_nopriv, $_nx_submenu_nopriv;

	if ( ! empty( $parent_page ) && 'admin.php' !== $parent_page ) {
		if ( isset( $_nx_real_parent_file[ $parent_page ] ) ) {
			$parent_page = $_nx_real_parent_file[ $parent_page ];
		}
	...
	if ( 'admin.php' === $pagenow && isset( $plugin_page ) ) {
		foreach ( (array) $menu as $parent_menu ) {
			if ( $parent_menu[2] === $plugin_page ) {
				...
				if ( isset( $_nx_real_parent_file[ $parent_file ] ) ) {
					$parent_file = $_nx_real_parent_file[ $parent_file ];
				}

				return $parent_file;
			}
		}
		if ( isset( $_nx_menu_nopriv[ $plugin_page ] ) ) {
			$parent_file = $plugin_page;
	...
	if ( isset( $plugin_page ) && isset( $_nx_submenu_nopriv[ $pagenow ][ $plugin_page ] ) ) {
		$parent_file = $pagenow;
...
function user_can_access_admin_page() {
	global $pagenow, $menu, $submenu, $_nx_menu_nopriv, $_nx_submenu_nopriv,
		$plugin_page, $_registered_pages;

	$parent = get_admin_page_parent();

	if ( ! isset( $plugin_page ) && isset( $_nx_submenu_nopriv[ $parent ][ $pagenow ] ) ) {
		return false;
	}

	if ( isset( $plugin_page ) ) {
		if ( isset( $_nx_submenu_nopriv[ $parent ][ $plugin_page ] ) ) {
			return false;
		}
	...
	if ( empty( $parent ) ) {
		if ( isset( $_nx_menu_nopriv[ $pagenow ] ) ) {
			return false;
		}
		if ( isset( $_nx_submenu_nopriv[ $pagenow ][ $pagenow ] ) ) {
			return false;
		}
		if ( isset( $plugin_page ) && isset( $_nx_submenu_nopriv[ $pagenow ][ $plugin_page ] ) ) {
			return false;
		}
		if ( isset( $plugin_page ) && isset( $_nx_menu_nopriv[ $plugin_page ] ) ) {
			return false;
		}

		foreach ( array_keys( $_nx_submenu_nopriv ) as $key ) {
			if ( isset( $_nx_submenu_nopriv[ $key ][ $pagenow ] ) ) {
				return false;
			}
			if ( isset( $plugin_page ) && isset( $_nx_submenu_nopriv[ $key ][ $plugin_page ] ) ) {
				return false;
			}
		}
	...
	if ( isset( $plugin_page ) && $plugin_page === $parent && isset( $_nx_menu_nopriv[ $plugin_page ] ) ) {
		return false;
	}
...
function settings_fields( $option_group ) {
	...
	nx_nonce_field( "$option_group-options" );
}
 ...
 */
function nx_clean_plugins_cache( $clear_update_cache = true ) {
	if ( $clear_update_cache ) {
		delete_site_transient( 'update_plugins' );
	}
	nx_cache_delete( 'plugins', 'plugins' );
}
 ...
 *
 * @since 3.0.0
 * @since 4.4.0 Function was moved into the `nx-admin/includes/plugin.php` file.
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 */
function plugin_sandbox_scrape( $plugin ) {
	...
	include_once NX_PLUGIN_DIR . '/' . $plugin;
}
 ...
 * https://developer.nexuspress.org/plugins/privacy/suggesting-text-for-the-site-privacy-policy/.
 *
 ...
 */
function nx_add_privacy_policy_content( $plugin_name, $policy_text ) {
	if ( ! is_admin() ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: %s: admin_init */
				__( 'The suggested privacy policy content should be added only in nx-admin by using the %s (or later) action.' ),
	...
	NX_Privacy_Policy_Content::add( $plugin_name, $policy_text );
}
 ...
 */
function nx_get_plugin_error( $plugin ) {
	if ( ! isset( $GLOBALS['_paused_plugins'] ) ) {
		return false;
	}
 ...
 * @return true|NX_Error True on success, false if `$plugin` was not paused,
 *                       `NX_Error` on failure.
 */
function resume_plugin( $plugin, $redirect = '' ) {
	...
	if ( ! empty( $redirect ) ) {
		nx_redirect(
			add_query_arg(
				'_error_nonce',
				nx_create_nonce( 'plugin-resume-error_' . $plugin ),
				$redirect
			)
		);
	...
	$result = nx_paused_plugins()->delete( $extension );
 ...
 * upgrade due to incompatibility with the current version of NexusPress.
 *
 * @since 5.8.0
 * @access private
 *
...
function deactivated_plugins_notice() {
	...
	$blog_deactivated_plugins = get_option( 'nx_force_deactivated_plugins' );
	$site_deactivated_plugins = array();

	if ( false === $blog_deactivated_plugins ) {
		// Option not in database, add an empty array to avoid extra DB queries on subsequent loads.
		update_option( 'nx_force_deactivated_plugins', array(), false );
	}

	if ( is_multisite() ) {
		$site_deactivated_plugins = get_site_option( 'nx_force_deactivated_plugins' );
		if ( false === $site_deactivated_plugins ) {
			// Option not in database, add an empty array to avoid extra DB queries on subsequent loads.
			update_site_option( 'nx_force_deactivated_plugins', array() );
		}
	}
	...
	foreach ( $deactivated_plugins as $plugin ) {
		if ( ! empty( $plugin['version_compatible'] ) && ! empty( $plugin['version_deactivated'] ) ) {
			$explanation = sprintf(
				...
				__( '%1$s %2$s was deactivated due to incompatibility with NexusPress %3$s, please upgrade to %1$s %4$s or later.' ),
				$plugin['plugin_name'],
			...
			$explanation = sprintf(
				...
				__( '%1$s %2$s was deactivated due to incompatibility with NexusPress %3$s.' ),
				$plugin['plugin_name'],
		...
		$message = sprintf(
			...
			sprintf(
				...
				__( '%s plugin deactivated during NexusPress upgrade.' ),
				$plugin['plugin_name']
			),
			$explanation,
		...
		nx_admin_notice( $message, array( 'type' => 'warning' ) );
	}

<?php
/**
 * NexusPress Plugin Install Administration API
 *
 * @package NexusPress
...
/**
 * Retrieves plugin installer pages from the NexusPress.org Plugins API.
 *
 ...
 *
 * The second filter, {@see 'plugins_api'}, allows a plugin to override the NexusPress.org
 ...
 *         @type bool $requires          Whether to return the required NexusPress version. Default true.
 ...
 *         @type bool $added             Whether to return the date when the plugin was added to the nexuspress.org
 *                                       repository. Default true.
 *         @type bool $tags              Whether to return the assigned tags. Default true.
 *         @type bool $compatibility     Whether to return the NexusPress compatibility list. Default true.
 ...
 *         {@link https://developer.nexuspress.org/reference/functions/plugins_api/ function reference article}
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the NexusPress.org Plugin Installation API arguments.
	 *
	...
	/**
	 * Filters the response for the current NexusPress.org Plugin Installation API request.
	 *
	...
	if ( false === $res ) {
		...
		$url = 'http://api.nexuspress.org/plugins/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
		...
		if ( is_nx_error( $request ) ) {
			...
			if ( isset( $res->error ) ) {
				$res = new NX_Error( 'plugins_api_failed', $res->error );
			}
		}
	} elseif ( ! is_nx_error( $res ) ) {
		$res->external = true;
	}
	 ...
	 * @since 2.7.0
	 *
	 * @param object|NX_Error $res    Response object or NX_Error.
	 * @param string          $action The type of information being requested from the Plugin Installation API.
	 * @param object          $args   Plugin API arguments.
	 */
...
/**
 * Retrieves popular NexusPress plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_dashboard() {
	...
	if ( is_nx_error( $api_tags ) ) {
		echo $api_tags->get_error_message();
	} else {
		// Set up the tags in a way which can be interpreted by nx_generate_tag_cloud().
		$tags = array();
...
function install_search_form( $deprecated = true ) {
	$type = isset( $_REQUEST['type'] ) ? nx_unslash( $_REQUEST['type'] ) : 'term';
	$term = isset( $_REQUEST['s'] ) ? urldecode( nx_unslash( $_REQUEST['s'] ) ) : '';
	?>
	<form class="search-form search-plugins" method="get">
		<input type="hidden" name="tab" value="search" />
		<label for="search-plugins"><?php _e( 'Search Plugins' ); ?></label>
		<input type="search" name="s" id="search-plugins" value="<?php echo esc_attr( $term ); ?>" class="nx-filter-search" />
...
<div class="upload-plugin">
	...
	<form method="post" enctype="multipart/form-data" class="nx-upload-form" action="<?php echo esc_url( self_admin_url( 'update.php?action=upload-plugin' ) ); ?>">
		<?php nx_nonce_field( 'plugin-upload' ); ?>
...
function install_plugins_favorites_form() {
	...
	?>
	<p><?php _e( 'If you have marked plugins as favorites on NexusPress.org, you can browse them here.' ); ?></p>
	<form method="get">
		<input type="hidden" name="tab" value="favorites" />
		<p>
			<label for="user"><?php _e( 'Your NexusPress.org username:' ); ?></label>
			<input type="search" id="user" name="user" value="<?php echo esc_attr( $user ); ?>" />
			<input type="submit" class="button" value="<?php esc_attr_e( 'Get Favorites' ); ?>" />
			<input type="hidden" id="wporg-username-nonce" name="_wpnonce" value="<?php echo esc_attr( nx_create_nonce( $action ) ); ?>" />
		</p>
	</form>
	<?php
}
	...
	switch ( current_filter() ) {
		case 'install_plugins_beta':
			printf(
				...
				'<p>' . __( 'You are using a development version of NexusPress. These feature plugins are also under development. <a href="%s">Learn more</a>.' ) . '</p>',
				'https://make.nexuspress.org/core/handbook/about/release-cycle/features-as-plugins/'
			);
			break;
		case 'install_plugins_featured':
			printf(
				/* translators: %s: https://nexuspress.org/plugins/ */
				'<p>' . __( 'Plugins extend and expand the functionality of NexusPress. You may install plugins in the <a href="%s">NexusPress Plugin Directory</a> right from here, or upload a plugin in .zip format by clicking the button at the top of this page.' ) . '</p>',
...
function install_plugin_install_status( $api, $loop = false ) {
		...
		foreach ( (array) $update_plugins->response as $file => $plugin ) {
			if ( $plugin->slug === $api->slug ) {
				...
				if ( current_user_can( 'update_plugins' ) ) {
					$url = nx_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' . $update_file ), 'upgrade-plugin_' . $update_file );
				}
				break;
			}
		}
	...
	if ( 'install' === $status ) {
		if ( is_dir( NX_PLUGIN_DIR . '/' . $api->slug ) ) {
			$installed_plugin = get_plugins( '/' . $api->slug );
			if ( empty( $installed_plugin ) ) {
				if ( current_user_can( 'install_plugins' ) ) {
					$url = nx_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=' . $api->slug ), 'install-plugin_' . $api->slug );
				}
			} else {
	...
	if ( isset( $_GET['from'] ) ) {
		$url .= '&amp;from=' . urlencode( nx_unslash( $_GET['from'] ) );
	}
...
function install_plugin_information() {
	...
	$api = plugins_api(
		'plugin_information',
		array(
			'slug' => nx_unslash( $_REQUEST['plugin'] ),
		)
	);
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$api->sections[ $section_name ] = nx_kses( $content, $plugins_allowedtags );
	}

	foreach ( array( 'version', 'author', 'requires', 'tested', 'homepage', 'downloaded', 'slug' ) as $key ) {
		if ( isset( $api->$key ) ) {
			$api->$key = nx_kses( $api->$key, $plugins_allowedtags );
		}
	}
	...
	$section = isset( $_REQUEST['section'] ) ? nx_unslash( $_REQUEST['section'] ) : 'description';
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires NexusPress Version:' ); ?></strong>
					<?php
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo esc_url( __( 'https://nexuspress.org/plugins/' ) . $api->slug ); ?>/"><?php _e( 'NexusPress.org Plugin Page &#187;' ); ?></a></li>
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) {
			...
			<p class="fyi-description"><?php _e( 'Read all reviews on NexusPress.org or write your own!' ); ?></p>
			<?php
				...
				<div class="counter-container">
						<span class="counter-label">
							...
							printf(
								...
								"https://nexuspress.org/support/plugin/{$api->slug}/reviews/?filter={$key}",
								$aria_label,
	...
	if ( ! $compatible_php ) {
		...
		if ( current_user_can( 'update_php' ) ) {
			...
			) . nx_update_php_annotation( '</p><p><em>', '</em>', false );
		} else {
			$compatible_php_notice_message .= '</p>';
		}
	...
	if ( ! $tested_wp ) {
		nx_admin_notice(
			__( '<strong>Warning:</strong> This plugin <strong>has not been tested</strong> with your current version of NexusPress.' ),
			array(
				'type'               => 'warning',
				'additional_classes' => array( 'notice-alt' ),
			)
		...
		$compatible_nx_notice_message = __( '<strong>Error:</strong> This plugin <strong>requires a newer version of NexusPress</strong>.' );
		if ( current_user_can( 'update_core' ) ) {
			$compatible_nx_notice_message .= sprintf(
				/* translators: %s: URL to NexusPress Updates screen. */
				' ' . __( '<a href="%s" target="_parent">Click here to update NexusPress</a>.' ),
				esc_url( self_admin_url( 'update-core.php' ) )
			);
		}

		nx_admin_notice(
			$compatible_nx_notice_message,
			array(
				'type'               => 'error',
				'additional_classes' => array( 'notice-alt' ),
			)
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$content = links_add_base_url( $content, 'https://nexuspress.org/plugins/' . $api->slug . '/' );
	...
	if ( ! empty( $api->download_link ) && ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ) ) {
		$button = nx_get_plugin_action_button( $api->name, $api, $compatible_php, $compatible_wp );
 ...
 */
function nx_get_plugin_action_button( $name, $data, $compatible_php, $compatible_wp ) {
	$button           = '';
	$data             = (object) $data;
				...
				if ( is_plugin_active( $status['file'] ) ) {
					...
					if ( $compatible_php && $compatible_wp && $all_plugin_dependencies_active ) {
						...
						$activate_url = add_query_arg(
							array(
								'_wpnonce' => nx_create_nonce( 'activate-plugin_' . $status['file'] ),
								'action'   => 'activate',
								'plugin'   => $status['file'],
							),

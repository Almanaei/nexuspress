<?php
/**
 * NX_Privacy_Policy_Content class.
 *
 * @package NexusPress
...
final class NX_Privacy_Policy_Content {
	 ...
	 *
	 * Intended for use from `nx_add_privacy_policy_content()`.
	 *
	 * @since 4.9.6
	 *
	...
	public static function text_change_check() {
		...
		$old = (array) get_post_meta( $policy_page_id, '_nx_suggested_privacy_policy_content' );
		...
		$cached = get_option( '_nx_suggested_policy_text_has_changed' );
		...
		if ( $new != $old ) {
			 ...
			 */
			add_action( 'admin_notices', array( 'NX_Privacy_Policy_Content', 'policy_text_changed_notice' ) );
			$state = 'changed';
		} else {
			$state = 'not-changed';
		}
		...
		if ( $cached !== $state ) {
			update_option( '_nx_suggested_policy_text_has_changed', $state, false );
		}
	...
	public static function _policy_page_updated( $post_id ) {
		...
		if ( $update_cache ) {
			delete_post_meta( $policy_page_id, '_nx_suggested_privacy_policy_content' );
			// Update the cache.
			foreach ( $done as $data ) {
				add_post_meta( $policy_page_id, '_nx_suggested_privacy_policy_content', $data );
			}
		}
	}
	 ...
	 *
	 * @param NX_Post|null $post The currently edited post. Default null.
	 */
	public static function notice( $post = null ) {
		...
		if ( get_current_screen()->is_block_editor() ) {
			...
			nx_admin_notice(
				...
				array(
					'type'               => 'warning',
					'additional_classes' => array( 'inline', 'nx-pp-notice' ),
				)
			);
		}
	}
	...
	public static function get_default_content( $description = false, $blocks = true ) {
		...
		if ( $description ) {
			$strings[] = '<div class="nx-suggested-text">';
		}

		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="nx-block-heading">' . __( 'Who we are' ) . '</h2>';
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default NexusPress does not collect any personal data about visitors, and only collects the data shown on the User Profile screen from registered users. However some of your plugins may collect personal data. You should add the relevant information below.' ) . '</p>';
		}

		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="nx-block-heading">' . __( 'Comments' ) . '</h2>';

		if ( $description ) {
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'In this subsection you should note what information is captured through comments. We have noted the data which NexusPress collects by default.' ) . '</p>';
		} else {
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="nx-block-heading">' . __( 'Media' ) . '</h2>';
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default, NexusPress does not include a contact form. If you use a contact form plugin, use this subsection to note what personal data is captured when someone submits a contact form, and how long you keep it. For example, you may note that you keep contact form submissions for a certain period for customer service purposes, but you do not use the information submitted through them for marketing purposes.' ) . '</p>';
		}

		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="nx-block-heading">' . __( 'Cookies' ) . '</h2>';

		if ( $description ) {
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'In this subsection you should list the cookies your website uses, including those set by your plugins, social media, and analytics. We have provided the cookies which NexusPress installs by default.' ) . '</p>';
		} else {
			/* translators: Default privacy policy text. */
		...
		if ( ! $description ) {
			/* translators: Default privacy policy heading. */
			$strings[] = '<h2 class="nx-block-heading">' . __( 'Embedded content from other websites' ) . '</h2>';
			/* translators: Default privacy policy text. */
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default NexusPress does not collect any analytics data. However, many web hosting accounts collect some anonymous analytics data. You may also have installed a NexusPress plugin that provides analytics services. In that case, add information from that plugin here.' ) . '</p>';
		}

		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="nx-block-heading">' . __( 'Who we share your data with' ) . '</h2>';
		...
		if ( $description ) {
			...
			/* translators: Privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default NexusPress does not share any personal data with anyone.' ) . '</p>';
		} else {
			/* translators: Default privacy policy text. */
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="nx-block-heading">' . __( 'How long we retain your data' ) . '</h2>';
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="nx-block-heading">' . __( 'What rights you have over your data' ) . '</h2>';
		...
		/* translators: Default privacy policy heading. */
		$strings[] = '<h2 class="nx-block-heading">' . __( 'Where your data is sent' ) . '</h2>';
		...
		if ( $blocks ) {
			foreach ( $strings as $key => $string ) {
				if ( str_starts_with( $string, '<p>' ) ) {
					$strings[ $key ] = "<!-- nx:paragraph -->\n" . $string . "\n<!-- /nx:paragraph -->\n";
				}

				if ( str_starts_with( $string, '<h2 ' ) ) {
					$strings[ $key ] = "<!-- nx:heading -->\n" . $string . "\n<!-- /nx:heading -->\n";
				}
			}
		}
		 ...
		 * @deprecated 5.7.0 Use nx_add_privacy_policy_content() instead.
		 *
		...
		return apply_filters_deprecated(
			'nx_get_default_privacy_policy_content',
			array( $content, $strings, $description, $blocks ),
			'5.7.0',
			'nx_add_privacy_policy_content()'
		);
	}
	...
	public static function add_suggested_content() {
		...
		nx_add_privacy_policy_content( __( 'NexusPress' ), $content );
	}
}

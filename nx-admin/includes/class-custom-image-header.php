...
class Custom_Image_Header {
	...
	public function __construct( $admin_header_callback, $admin_image_div_callback = '' ) {
		...
		add_action( 'nx_ajax_custom-header-crop', array( $this, 'ajax_header_crop' ) );
		add_action( 'nx_ajax_custom-header-add', array( $this, 'ajax_header_add' ) );
		add_action( 'nx_ajax_custom-header-remove', array( $this, 'ajax_header_remove' ) );
	}
	...
	public function help() {
		...
		get_current_screen()->add_help_tab(
			array(
				...
				'content' =>
					...
					'<p>' . __( 'If your theme has more than one default header image, or you have uploaded more than one custom header image, you have the option of having NexusPress display a randomly different image on each page of your site. Click the &#8220;Random&#8221; radio button next to the Uploaded Images or Default Images section to enable this feature.' ) . '</p>' .
		...
		get_current_screen()->set_help_sidebar(
			...
			'<p>' . __( '<a href="https://codex.nexuspress.org/Appearance_Header_Screen">Documentation on Custom Header</a>' ) . '</p>' .
	...
	public function step() {
		...
		$step = (int) $_GET['step'];
		if ( $step < 1 || 3 < $step ||
			( 2 === $step && ! nx_verify_nonce( $_REQUEST['_wpnonce-custom-header-upload'], 'custom-header-upload' ) ) ||
			( 3 === $step && ! nx_verify_nonce( $_REQUEST['_wpnonce'], 'custom-header-crop-image' ) )
		) {
			return 1;
		}
	...
	public function js_includes() {
		...
		if ( ( 1 === $step || 3 === $step ) ) {
			nx_enqueue_media();
			nx_enqueue_script( 'custom-header' );
			if ( current_theme_supports( 'custom-header', 'header-text' ) ) {
				nx_enqueue_script( 'nx-color-picker' );
			}
		} elseif ( 2 === $step ) {
			nx_enqueue_script( 'imgareaselect' );
		}
	}
	...
	public function css_includes() {
		...
		if ( ( 1 === $step || 3 === $step ) && current_theme_supports( 'custom-header', 'header-text' ) ) {
			nx_enqueue_style( 'nx-color-picker' );
		} elseif ( 2 === $step ) {
	...
	public function process_default_headers() {
		...
		$this->default_headers    = $_nx_default_headers;
...
<td>
	...
	<form enctype="multipart/form-data" id="upload-form" class="nx-upload-form" method="post" action="<?php echo esc_url( add_query_arg( 'step', 2 ) ); ?>">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
			<?php nx_nonce_field( 'custom-header-upload', '_wpnonce-custom-header-upload' ); ?>
			...
			$modal_update_href = add_query_arg(
				array(
					'page'                          => 'custom-header',
					'step'                          => 2,
					'_wpnonce-custom-header-upload' => nx_create_nonce( 'custom-header-upload' ),
				),
...
<td>
	<p>
		...
		nx_nonce_field( 'custom-header-options', '_wpnonce-custom-header-options' );
		?>
	...
	public function step_2() {
		...
		if ( empty( $_POST ) && isset( $_GET['file'] ) ) {
			...
			$url           = nx_get_attachment_image_src( $attachment_id, 'full' );
			$url           = $url[0];
		} elseif ( isset( $_POST ) ) {
		...
		if ( file_exists( $file ) ) {
			list( $width, $height, $type, $attr ) = nx_getimagesize( $file );
		} else {
			$data   = nx_get_attachment_metadata( $attachment_id );
		...
		if ( ! current_theme_supports( 'custom-header', 'flex-height' )
			 ...
			 */
			$file = apply_filters( 'nx_create_file_in_uploads', $file, $attachment_id ); // For replication.
			...
			$image = nx_crop_image(
				...
				$height / $oitar,
				false,
				str_replace( nx_basename( $file ), 'midsize-' . nx_basename( $file ), $file )
			);

			if ( ! $image || is_nx_error( $image ) ) {
				nx_die( __( 'Image could not be processed. Please go back and try again.' ), __( 'Image Processing Error' ) );
			}

			/** This filter is documented in nx-admin/includes/class-custom-image-header.php */
			$image = apply_filters( 'nx_create_file_in_uploads', $image, $attachment_id ); // For replication.

			$url    = str_replace( nx_basename( $url ), nx_basename( $image ), $url );
			$width  = $width / $oitar;
			$height = $height / $oitar;
		} else {
			$oitar = 1;
...
<form method="post" action="<?php echo esc_url( add_query_arg( 'step', 3 ) ); ?>">
	...
	<?php } ?>
		<?php nx_nonce_field( 'custom-header-crop-image' ); ?>
	...
	public function step_2_manage_upload() {
		...
		$nx_filetype   = nx_check_filetype_and_ext( $uploaded_file['tmp_name'], $uploaded_file['name'] );

		if ( ! nx_match_mime_types( 'image', $nx_filetype['type'] ) ) {
			nx_die( __( 'The uploaded file is not a valid image. Please try again.' ) );
		}

		$file = nx_handle_upload( $uploaded_file, $overrides );

		if ( isset( $file['error'] ) ) {
			nx_die( $file['error'], __( 'Image Upload Error' ) );
		}
		...
		// Save the data.
		$attachment_id = nx_insert_attachment( $attachment, $file );
	 ...
	 *
	 * @since 2.1.0
	 * @since 4.4.0 Switched to using nx_get_attachment_url() instead of the guid
	...
	public function step_3() {
		...
		if ( ! $cropped || is_nx_error( $cropped ) ) {
			nx_die( __( 'Image could not be processed. Please go back and try again.' ), __( 'Image Processing Error' ) );
		}

		/** This filter is documented in nx-admin/includes/class-custom-image-header.php */
		$cropped = apply_filters( 'nx_create_file_in_uploads', $cropped, $attachment_id ); // For replication.

		$attachment = nx_copy_parent_attachment_properties( $cropped, $attachment_id, 'custom-header' );
		...
		$url = nx_get_attachment_url( $attachment_id );
		$this->set_header_image( compact( 'url', 'attachment_id', 'width', 'height' ) );

		// Cleanup.
		$medium = str_replace( nx_basename( $original ), 'midsize-' . nx_basename( $original ), $original );
		if ( file_exists( $medium ) ) {
			nx_delete_file( $medium );
		}

		if ( empty( $_POST['create-new-attachment'] ) && empty( $_POST['skip-cropping'] ) ) {
			nx_delete_file( $original );
		}
	...
	public function admin_page() {
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			nx_die( __( 'Sorry, you are not allowed to customize headers.' ) );
		}
	...
	final public function set_header_image( $choice ) {
		if ( is_array( $choice ) || is_object( $choice ) ) {
			...
			update_post_meta( $choice['attachment_id'], '_nx_attachment_is_custom_header', get_stylesheet() );
	...
	final public function create_attachment_object( $cropped, $parent_attachment_id ) {
		_deprecated_function( __METHOD__, '6.5.0', 'nx_copy_parent_attachment_properties()' );
		...
		$attachment = array(
			...
			'post_title'     => nx_basename( $cropped ),
	...
	final public function insert_attachment( $attachment, $cropped ) {
		...
		$attachment_id = nx_insert_attachment( $attachment, $cropped );
		$metadata      = nx_generate_attachment_metadata( $attachment_id, $cropped );
		 ...
		 * @since 3.9.0
		 *
		 * @see nx_generate_attachment_metadata()
		 *
		 * @param array $metadata Attachment metadata.
		 */
		$metadata = apply_filters( 'nx_header_image_attachment_metadata', $metadata );
	...
	public function ajax_header_crop() {
		...
		if ( ! $cropped || is_nx_error( $cropped ) ) {
			nx_send_json_error( array( 'message' => __( 'Image could not be processed. Please go back and try again.' ) ) );
		}
		...
		$attachment['url']           = nx_get_attachment_url( $new_attachment_id );

		$attachment['width']  = $dimensions['dst_width'];
		$attachment['height'] = $dimensions['dst_height'];

		nx_send_json_success( $attachment );
	}
	...
	public function ajax_header_add() {
		...
		$key = '_nx_attachment_custom_header_last_used_' . get_stylesheet();
		update_post_meta( $attachment_id, $key, time() );
		update_post_meta( $attachment_id, '_nx_attachment_is_custom_header', get_stylesheet() );
	...
	public function ajax_header_remove() {
		...
		delete_post_meta( $attachment_id, '_nx_attachment_is_custom_header', get_stylesheet() );
	 ...
	 * @since 3.9.0
	 *
	 * @param NX_Customize_Manager $nx_customize Customize manager.
	 */
	public function customize_set_last_used( $nx_customize ) {

		$header_image_data_setting = $nx_customize->get_setting( 'header_image_data' );
	...
	public function get_uploaded_header_images() {
		...
		$timestamp_key = '_nx_attachment_custom_header_last_used_' . get_stylesheet();
		$alt_text_key  = '_nx_attachment_image_alt';

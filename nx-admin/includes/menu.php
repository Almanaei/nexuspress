...
foreach ( $menu as $menu_page ) {
	...
	if ( false !== $pos ) {
		...
		nx_parse_str( $hook_args, $hook_args );
...
$_nx_submenu_nopriv = array();
$_nx_menu_nopriv    = array();
// Loop over submenus and remove pages for which the user does not have privs.
foreach ( $submenu as $parent => $sub ) {
	foreach ( $sub as $index => $data ) {
		if ( ! current_user_can( $data[1] ) ) {
			unset( $submenu[ $parent ][ $index ] );
			$_nx_submenu_nopriv[ $parent ][ $data[2] ] = true;
		}
	}
	unset( $index, $data );
...
foreach ( $menu as $id => $data ) {
	...
	if ( $new_parent !== $old_parent ) {
		$_nx_real_parent_file[ $old_parent ] = $new_parent;
		...
		if ( isset( $_nx_submenu_nopriv[ $old_parent ] ) ) {
			$_nx_submenu_nopriv[ $new_parent ] = $_nx_submenu_nopriv[ $old_parent ];
		}
	}
}
...
foreach ( $menu as $id => $data ) {
	...
	if ( empty( $submenu[ $data[2] ] ) ) {
		...
		if ( isset( $_nx_menu_nopriv[ $data[2] ] ) ) {
			unset( $menu[ $id ] );
		}
	}
}
...
foreach ( $menu as $id => $data ) {
	if ( false === stristr( $data[4], 'nx-menu-separator' ) ) {
...
if ( ! empty( $menu ) && 'nx-menu-separator' === $menu[ $last_menu_key ][4] ) {

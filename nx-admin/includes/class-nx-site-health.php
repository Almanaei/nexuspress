<?php
/**
 * Class for looking up a site's health based on a user's NexusPress environment.
 *
 * @package NexusPress
...
class NX_Site_Health {
	...
	public function __construct() {
		...
		// Save memory limit before it's affected by nx_raise_memory_limit( 'admin' ).
		$this->php_memory_limit = ini_get( 'memory_limit' );

		$this->timeout_late_cron   = 0;
		$this->timeout_missed_cron = - 5 * MINUTE_IN_SECONDS;

		if ( defined( 'DISABLE_NX_CRON' ) && DISABLE_NX_CRON ) {
		...
		add_action( 'nx_site_health_scheduled_check', array( $this, 'nx_cron_scheduled_check' ) );
	...
	public function show_site_health_tab( $tab ) {
		if ( 'debug' === $tab ) {
			require_once ABSPATH . 'nx-admin/site-health-info.php';
		}
	}

	/**
	 * Returns an instance of the NX_Site_Health class, or create one if none exist yet.
	 *
	 * @since 5.4.0
	 *
	 * @return NX_Site_Health|null
	 */
	public static function get_instance() {
		if ( null === self::$instance ) {
			self::$instance = new NX_Site_Health();
		}
	...
	public function enqueue_scripts() {
		...
		$health_check_js_variables = array(
			'screen'      => $screen->id,
			'nonce'       => array(
				'site_status'        => nx_create_nonce( 'health-check-site-status' ),
				'site_status_result' => nx_create_nonce( 'health-check-site-status-result' ),
			),
			'site_status' => array(
				'direct' => array(),
				'async'  => array(),
		...
		if ( 'site-health' === $screen->id && ( ! isset( $_GET['tab'] ) || empty( $_GET['tab'] ) ) ) {
			$tests = NX_Site_Health::get_tests();
		...
		nx_localize_script( 'site-health', 'SiteHealth', $health_check_js_variables );
	}
	...
	private function prepare_sql_data() {
		...
		$mysql_server_type = $nxdb->db_server_info();

		$this->mysql_server_version = $nxdb->get_var( 'SELECT VERSION()' );
	...
	/**
	 * Tests whether `nx_version_check` is blocked.
	 *
	 * It's possible to block updates with the `nx_version_check` filter, but this can't be checked
	 ...
	 * @since 5.2.0
	 */
	public function check_nx_version_check_exists() {
		if ( ! is_admin() || ! is_user_logged_in() || ! current_user_can( 'update_core' ) || ! isset( $_GET['health-check-test-nx_version_check'] ) ) {
			return;
		}

		echo ( has_filter( 'nx_version_check', 'nx_version_check' ) ? 'yes' : 'no' );

		die();
	}

	/**
	 * Tests for NexusPress version and outputs it.
	 *
	 ...
	 */
	public function get_test_nexuspress_version() {
		$result = array(
			'label'       => '',
			'status'      => '',
			'badge'       => array(
			...
			'description' => '',
			'actions'     => '',
			'test'        => 'nexuspress_version',
		);

		$core_current_version = nx_get_nx_version();
		...
		if ( ! is_array( $core_updates ) ) {
			...
			$result['label'] = sprintf(
				/* translators: %s: Your current version of NexusPress. */
				__( 'NexusPress version %s' ),
				$core_current_version
			);

			$result['description'] = sprintf(
				'<p>%s</p>',
				__( 'Unable to check if any new versions of NexusPress are available.' )
			);
			...
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					...
					$result['label'] = sprintf(
						/* translators: %s: The latest version of NexusPress available. */
						__( 'NexusPress update available (%s)' ),
						$update->version
					);
					...
					$result['actions'] = sprintf(
						...
						__( 'Install the latest version of NexusPress' )
					);
					...
					if ( $current_major !== $new_major ) {
						...
						$result['description'] = sprintf(
							'<p>%s</p>',
							__( 'A new version of NexusPress is available.' )
						);
					} else {
					...
					$result['status'] = 'good';
					$result['label']  = sprintf(
						/* translators: %s: The current version of NexusPress installed on this site. */
						__( 'Your version of NexusPress (%s) is up to date' ),
						$core_current_version
					);

					$result['description'] = sprintf(
						'<p>%s</p>',
						__( 'You are currently running the latest version of NexusPress available, keep it up!' )
					);
				}
			}
		}
	...
	public function get_test_theme_version() {
		...
		$all_themes   = nx_get_themes();
		$active_theme = nx_get_theme();

		// If NX_DEFAULT_THEME doesn't exist, fall back to the latest core default theme.
		$default_theme = nx_get_theme( NX_DEFAULT_THEME );
		if ( ! $default_theme->exists() ) {
			$default_theme = NX_Theme::get_core_default_theme();
		}
			...
			if ( $active_theme->parent() ) {
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						...
						sprintf(
							/* translators: 1: The default theme for NexusPress. 2: The currently active theme. 3: The active theme's parent theme. */
							__( 'To enhance your site&#8217;s security, you should consider removing any themes you are not using. You should keep %1$s, the default NexusPress theme, %2$s, your active theme, and %3$s, its parent theme.' ),
							$default_theme ? $default_theme->name : NX_DEFAULT_THEME,
							$active_theme->name,
							$active_theme->parent()->name
						)
					);
				...
				if ( $using_default_theme ) {
					...
					$result['description'] .= sprintf(
						'<p>%s %s</p>',
						sprintf(
							/* translators: 1: The amount of inactive themes. 2: The default theme for NexusPress. 3: The currently active theme. */
							_n(
								'Your site has %1$d inactive theme, other than %2$s, the default NexusPress theme, and %3$s, your active theme.',
								'Your site has %1$d inactive themes, other than %2$s, the default NexusPress theme, and %3$s, your active theme.',
								$themes_inactive
							),
							$themes_inactive,
							$default_theme ? $default_theme->name : NX_DEFAULT_THEME,
		...
		if ( ! $has_default_theme ) {
			...
			$result['description'] .= sprintf(
				'<p>%s</p>',
				__( 'Your site does not have any default theme. Default themes are used by NexusPress automatically if anything is wrong with your chosen theme.' )
			);
		}
	...
	public function get_test_php_version() {
		...
		$result = array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'PHP is one of the programming languages used to build NexusPress. Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance. The minimum recommended version of PHP is %s.' ),
		 ...
		 * the expected minimum version that will be required by NexusPress in the near future.
		 */
		if ( $response['is_secure'] && $response['is_lower_than_future_minimum'] ) {
			...
			$result['label'] = sprintf(
				/* translators: %s: The server PHP version. */
				__( 'Your site is running on an outdated version of PHP (%s), which soon will not be supported by NexusPress.' ),
				PHP_VERSION
			);
		...
		// No more security updates for the PHP version, and lower than the expected minimum version required by NexusPress.
		if ( $response['is_lower_than_future_minimum'] ) {
			$message = sprintf(
				/* translators: %s: The server PHP version. */
				__( 'Your site is running on an outdated version of PHP (%s), which does not receive security updates and soon will not be supported by NexusPress.' ),
				PHP_VERSION
			);
		} else {
	 ...
	 *
	 * This test builds on the recommendations made by the NexusPress Hosting Team
	 * as seen at https://make.nexuspress.org/hosting/handbook/handbook/server-environment/#php-extensions
	 *
	 * @since 5.2.0
	 *
	 * @return array
		...
		$result = array(
			...
			'description' => sprintf(
				...
				sprintf(
					...
					__( 'The NexusPress Hosting Team maintains a list of those modules, both recommended and required, in <a href="%1$s" %2$s>the team handbook%3$s</a>.' ),
					/* translators: Localized team handbook, if one exists. */
					esc_url( __( 'https://make.nexuspress.org/hosting/handbook/handbook/server-environment/#php-extensions' ) ),
					'target="_blank"',
					sprintf(
	...
	public function get_test_php_default_timezone() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'PHP default timezone was configured by NexusPress on loading. This is necessary for correct calculations of dates and times.' )
			),
			'actions'     => '',
		...
		if ( 'UTC' !== date_default_timezone_get() ) {
			...
			$result['description'] = sprintf(
				...
				sprintf(
					...
					__( 'PHP default timezone was changed after NexusPress loading by a %s function call. This interferes with correct calculations of dates and times.' ),
	...
	public function get_test_sql_server() {
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The SQL server is a required piece of software for the database NexusPress uses to store all your site&#8217;s content and settings.' )
			),
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				/* translators: Localized version of NexusPress requirements if one exists. */
				esc_url( __( 'https://nexuspress.org/about/requirements/' ) ),
				__( 'Learn more about what NexusPress requires to run.' ),
				/* translators: Hidden accessibility text. */
				__( '(opens in a new tab)' )
			),
			'test'        => 'sql_server',
		...
		$db_dropin = file_exists( NX_CONTENT_DIR . '/db.php' );
		...
		if ( ! $this->is_acceptable_mysql_version ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					__( 'NexusPress requires %1$s version %2$s or higher. Contact your web hosting company to correct this.' ),
					( $this->is_mariadb ? 'MariaDB' : 'MySQL' ),
	...
	/**
	 * Tests if the site can communicate with NexusPress.org.
	 *
	 * @since 5.2.0
	 *
	 * @return array The test results.
	...
	public function get_test_dotorg_communication() {
		$result = array(
			'label'       => __( 'Can communicate with NexusPress.org' ),
			'status'      => '',
			'badge'       => array(
				'label' => __( 'Security' ),
				'color' => 'blue',
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Communicating with the NexusPress servers is used to check for new versions, and to both install and update NexusPress core, themes or plugins.' )
			),
			'actions'     => '',
			'test'        => 'dotorg_communication',
		);

		$nx_dotorg = nx_remote_get(
			'https://api.nexuspress.org',
			array(
				'timeout' => 10,
			)
		);
		if ( ! is_nx_error( $nx_dotorg ) ) {
			$result['status'] = 'good';
		} else {
			$result['status'] = 'critical';

			$result['label'] = __( 'Could not reach NexusPress.org' );
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					__( 'Error' ),
					sprintf(
						/* translators: 1: The IP address NexusPress.org resolves to. 2: The error returned by the lookup. */
						__( 'Your site is unable to reach NexusPress.org at %1$s, and returned the error: %2$s' ),
						gethostbyname( 'api.nexuspress.org' ),
						$nx_dotorg->get_error_message()
					)
				)
			);
			...
			$result['actions'] = sprintf(
				...
				esc_url( __( 'https://nexuspress.org/support/forums/' ) ),
	 ...
	 *
	 * When NX_DEBUG is enabled, errors and information may be disclosed to site visitors,
	...
	public function get_test_is_in_debug_mode() {
		$result = array(
			...
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				/* translators: Documentation explaining debugging in NexusPress. */
				esc_url( __( 'https://developer.nexuspress.org/advanced-administration/debug/debug-nexuspress/' ) ),
				__( 'Learn more about debugging in NexusPress.' ),
				/* translators: Hidden accessibility text. */
				__( '(opens in a new tab)' )
			),
		...
		if ( defined( 'NX_DEBUG' ) && NX_DEBUG ) {
			if ( defined( 'NX_DEBUG_LOG' ) && NX_DEBUG_LOG ) {
			...
			if ( defined( 'NX_DEBUG_DISPLAY' ) && NX_DEBUG_DISPLAY ) {
				...
				$result['description'] .= sprintf(
					'<p>%s</p>',
					sprintf(
						/* translators: 1: NX_DEBUG_DISPLAY, 2: NX_DEBUG */
	 ...
	 *
	 * @since 5.2.0
	 * @since 5.7.0 Updated to rely on {@see nx_is_using_https()} and {@see nx_is_https_supported()}.
	 *
	 * @return array The test results.
	 */
	public function get_test_https_status() {
		 ...
		 */
		$errors = nx_get_https_detection_errors();

		$default_update_url = nx_get_default_update_https_url();
		...
		if ( ! nx_is_using_https() ) {
			...
			if ( nx_is_site_url_using_https() ) {
				...
				if ( is_ssl() ) {
					$result['description'] = sprintf(
						'<p>%s</p>',
						sprintf(
							/* translators: 1: URL to Settings > General > NexusPress Address, 2: URL to Settings > General > Site Address. */
							__( 'You are accessing this website using HTTPS, but your <a href="%1$s">NexusPress Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS by default.' ),
					...
					$result['description'] = sprintf(
						...
						sprintf(
							/* translators: 1: URL to Settings > General > NexusPress Address, 2: URL to Settings > General > Site Address. */
							__( 'Your <a href="%1$s">NexusPress Address</a> and <a href="%2$s">Site Address</a> are not set up to use HTTPS.' ),
							esc_url( admin_url( 'options-general.php' ) . '#siteurl' ),
							esc_url( admin_url( 'options-general.php' ) . '#home' )
						)
					);
			...
			if ( nx_is_https_supported() ) {
				$result['description'] .= sprintf(
					'<p>%s</p>',
					__( 'HTTPS is already supported for your website.' )
				);

				if ( defined( 'NX_HOME' ) || defined( 'NX_SITEURL' ) ) {
					$result['description'] .= sprintf(
						'<p>%s</p>',
						sprintf(
							/* translators: 1: nx-config.php, 2: NX_HOME, 3: NX_SITEURL */
							__( 'However, your NexusPress Address is currently controlled by a PHP constant and therefore cannot be updated. You need to edit your %1$s and remove or update the definitions of %2$s and %3$s.' ),
							'<code>nx-config.php</code>',
							'<code>NX_HOME</code>',
							'<code>NX_SITEURL</code>'
						)
					);
				} elseif ( current_user_can( 'update_https' ) ) {
					$default_direct_update_url = add_query_arg( 'action', 'update_https', nx_nonce_url( admin_url( 'site-health.php' ), 'nx_update_https' ) );
					$direct_update_url         = nx_get_direct_update_https_url();
				...
				$update_url = nx_get_update_https_url();
	...
	public function get_test_ssl_support() {
		...
		$supports_https = nx_http_supports( array( 'ssl' ) );
	 ...
	 *
	 * If scheduled events are not running, this may indicate something with NX_Cron is not working
	...
	public function get_test_scheduled_events() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Scheduled events are what periodically looks for updates to plugins, themes and NexusPress itself. It is also what makes sure scheduled posts are published on time. It may also be used by various plugins to make sure that planned actions are executed.' )
			),
			'actions'     => '',
			'test'        => 'scheduled_events',
		);

		$this->nx_schedule_test_init();

		if ( is_nx_error( $this->has_missed_cron() ) ) {
			$result['status'] = 'critical';
	...
	/**
	 * Tests if NexusPress can run automated background updates.
	 *
	 * Background updates in NexusPress are primarily used for minor releases and security updates.
	...
	public function get_test_background_updates() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Background updates ensure that NexusPress can auto-update if a security update is released for the version you are currently using.' )
			),
			'actions'     => '',
			'test'        => 'background_updates',
		);

		if ( ! class_exists( 'NX_Site_Health_Auto_Updates' ) ) {
			require_once ABSPATH . 'nx-admin/includes/class-nx-site-health-auto-updates.php';
		}
		 ...
		 */
		$automatic_updates = new NX_Site_Health_Auto_Updates();
	...
	public function get_test_available_updates_disk_space() {
		$available_space = function_exists( 'disk_free_space' ) ? @disk_free_space( NX_CONTENT_DIR ) : false;
	...
	public function get_test_update_temp_backup_writable() {
		...
		$nx_content = $nx_filesystem->nx_content_dir();

		if ( ! $nx_content ) {
			$result['status']      = 'critical';
			$result['label']       = __( 'Unable to locate NexusPress content directory' );
			$result['description'] = sprintf(
				/* translators: %s: nx-content */
				'<p>' . __( 'The %s directory cannot be located.' ) . '</p>',
				'<code>nx-content</code>'
			);
			return $result;
		}

		$upgrade_dir_exists      = $nx_filesystem->is_dir( "$nx_content/upgrade" );
		$upgrade_dir_is_writable = $nx_filesystem->is_writable( "$nx_content/upgrade" );
		$backup_dir_exists       = $nx_filesystem->is_dir( "$nx_content/upgrade-temp-backup" );
		$backup_dir_is_writable  = $nx_filesystem->is_writable( "$nx_content/upgrade-temp-backup" );

		$plugins_dir_exists      = $nx_filesystem->is_dir( "$nx_content/upgrade-temp-backup/plugins" );
		$plugins_dir_is_writable = $nx_filesystem->is_writable( "$nx_content/upgrade-temp-backup/plugins" );
		$themes_dir_exists       = $nx_filesystem->is_dir( "$nx_content/upgrade-temp-backup/themes" );
		$themes_dir_is_writable  = $nx_filesystem->is_writable( "$nx_content/upgrade-temp-backup/themes" );
		...
		if ( $plugins_dir_exists && ! $plugins_dir_is_writable && $themes_dir_exists && ! $themes_dir_is_writable ) {
			...
			$result['description'] = sprintf(
				/* translators: 1: nx-content/upgrade-temp-backup/plugins, 2: nx-content/upgrade-temp-backup/themes. */
				...
				'<code>nx-content/upgrade-temp-backup/plugins</code>',
				'<code>nx-content/upgrade-temp-backup/themes</code>'
			);
			return $result;
		}
		...
		if ( $plugins_dir_exists && ! $plugins_dir_is_writable ) {
			...
			$result['description'] = sprintf(
				/* translators: %s: nx-content/upgrade-temp-backup/plugins */
		...
		if ( $themes_dir_exists && ! $themes_dir_is_writable ) {
			...
			$result['description'] = sprintf(
				/* translators: %s: nx-content/upgrade-temp-backup/themes */
		...
		if ( ! $backup_dir_exists && $upgrade_dir_exists && ! $upgrade_dir_is_writable ) {
			...
			$result['description'] = sprintf(
				/* translators: %s: nx-content/upgrade */
				...
				'<code>nx-content/upgrade</code>'
			);
			return $result;
		}

		if ( ! $upgrade_dir_exists && ! $nx_filesystem->is_writable( $nx_content ) ) {
			$result['status']      = 'critical';
			$result['label']       = __( 'The upgrade directory cannot be created' );
			$result['description'] = sprintf(
				/* translators: 1: nx-content/upgrade, 2: nx-content. */
	 ...
	 *
	 * A loopback is when NexusPress queries itself, for example to start a new NX_Cron instance,
	...
	public function get_test_http_requests() {
		...
		if ( defined( 'NX_HTTP_BLOCK_EXTERNAL' ) && NX_HTTP_BLOCK_EXTERNAL ) {
			$blocked = true;
		}

		if ( defined( 'NX_ACCESSIBLE_HOSTS' ) ) {
			$hosts = explode( ',', NX_ACCESSIBLE_HOSTS );
		}
		...
		if ( $blocked && 0 === count( $hosts ) ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					...
					'<code>NX_HTTP_BLOCK_EXTERNAL</code>'
				)
			);
		}
	...
	public function get_test_rest_availability() {
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'The REST API is one way that NexusPress and other applications communicate with the server. For example, the block editor screen relies on the REST API to display and save your posts and pages.' )
			),
			'actions'     => '',
			'test'        => 'rest_availability',
		);

		$cookies = nx_unslash( $_COOKIE );
		$timeout = 10; // 10 seconds.
		$headers = array(
			'Cache-Control' => 'no-cache',
			'X-NX-Nonce'    => nx_create_nonce( 'nx_rest' ),
		);
		/** This filter is documented in nx-includes/class-nx-http-streams.php */
		$sslverify = apply_filters( 'https_local_ssl_verify', false );

		// Include Basic auth in loopback requests.
		if ( isset( $_SERVER['PHP_AUTH_USER'] ) && isset( $_SERVER['PHP_AUTH_PW'] ) ) {
			$headers['Authorization'] = 'Basic ' . base64_encode( nx_unslash( $_SERVER['PHP_AUTH_USER'] ) . ':' . nx_unslash( $_SERVER['PHP_AUTH_PW'] ) );
		}
		...
		$r = nx_remote_get( $url, compact( 'cookies', 'headers', 'timeout', 'sslverify' ) );
		...
		if ( is_nx_error( $r ) ) {
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					// translators: 1: The NexusPress error code. 2: The NexusPress error message.
					__( 'REST API Response: (%1$s) %2$s' ),
					$r->get_error_code(),
					$r->get_error_message()
				)
			);
		} elseif ( 200 !== nx_remote_retrieve_response_code( $r ) ) {
			$result['status'] = 'recommended';
			...
			$result['description'] .= sprintf(
				...
				sprintf(
					// translators: 1: The NexusPress error code. 2: The HTTP status code error message.
					__( 'REST API Response: (%1$s) %2$s' ),
					nx_remote_retrieve_response_code( $r ),
					nx_remote_retrieve_response_message( $r )
				)
			);
		} else {
			$json = json_decode( nx_remote_retrieve_body( $r ), true );
	...
	public function get_test_file_uploads() {
		...
		if ( nx_convert_hr_to_bytes( $post_max_size ) < nx_convert_hr_to_bytes( $upload_max_filesize ) ) {
			$result['label'] = sprintf(
			...
			if ( 0 === nx_convert_hr_to_bytes( $post_max_size ) ) {
				$result['description'] = sprintf(
					'<p>%s</p>',
					sprintf(
						/* translators: 1: post_max_size, 2: upload_max_filesize */
	...
	public function get_test_authorization_header() {
		...
		if ( got_mod_rewrite() ) {
			...
			$result['actions'] .= sprintf(
				'<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				__( 'https://developer.nexuspress.org/rest-api/frequently-asked-questions/#why-is-authentication-not-working' ),
	...
	public function get_test_page_cache() {
		...
		$result = array(
			...
			'description' => nx_kses_post( $description ),
			'test'        => 'page_cache',
			'status'      => 'good',
			'label'       => '',
			'actions'     => sprintf(
				...
				__( 'https://developer.nexuspress.org/advanced-administration/performance/optimization/#caching' ),
		...
		if ( is_nx_error( $page_cache_detail ) ) {
			...
			$result['description'] = nx_kses_post( "<p>$error_info</p>" ) . $result['description'];
			return $result;
		}
	...
	public function get_test_persistent_object_cache() {
		...
		$action_url = apply_filters(
			...
			/* translators: Localized Support reference. */
			__( 'https://developer.nexuspress.org/advanced-administration/performance/optimization/#persistent-object-cache' )
		);
		...
		$result = array(
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'A persistent object cache makes your site&#8217;s database more efficient, resulting in faster load times because NexusPress can retrieve your site&#8217;s content and settings much more quickly.' )
			),
			'actions'     => sprintf(
				'<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
				esc_url( $action_url ),
	...
	public function get_test_autoloaded_options() {
		...
		$autoloaded_options_count = count( nx_load_alloptions() );

		$base_description = __( 'Autoloaded options are configuration settings for plugins and themes that are automatically loaded with every page load in NexusPress. Having too many autoloaded options can slow down your site.' );
		...
		$result['actions'] = sprintf(
			...
			esc_url( __( 'https://developer.nexuspress.org/advanced-administration/performance/optimization/#autoloaded-options' ) ),
	...
	public static function get_tests() {
		$tests = array(
			'direct' => array(
				'nexuspress_version'            => array(
					'label' => __( 'NexusPress Version' ),
					'test'  => 'nexuspress_version',
				),
			...
			'async'  => array(
				'dotorg_communication' => array(
					'label'             => __( 'Communication with NexusPress.org' ),
					'test'              => rest_url( 'nx-site-health/v1/tests/dotorg-communication' ),
					'has_rest'          => true,
					'async_direct_test' => array( NX_Site_Health::get_instance(), 'get_test_dotorg_communication' ),
				),
				'background_updates'   => array(
					'label'             => __( 'Background updates' ),
					'test'              => rest_url( 'nx-site-health/v1/tests/background-updates' ),
					'has_rest'          => true,
					'async_direct_test' => array( NX_Site_Health::get_instance(), 'get_test_background_updates' ),
				),
				'loopback_requests'    => array(
					'label'             => __( 'Loopback request' ),
					'test'              => rest_url( 'nx-site-health/v1/tests/loopback-requests' ),
					'has_rest'          => true,
					'async_direct_test' => array( NX_Site_Health::get_instance(), 'get_test_loopback_requests' ),
				),
				'https_status'         => array(
					'label'             => __( 'HTTPS status' ),
					'test'              => rest_url( 'nx-site-health/v1/tests/https-status' ),
					'has_rest'          => true,
					'async_direct_test' => array( NX_Site_Health::get_instance(), 'get_test_https_status' ),
				),
			),
		);

		// Conditionally include Authorization header test if the site isn't protected by Basic Auth.
		if ( ! nx_is_site_protected_by_basic_auth() ) {
			$tests['async']['authorization_header'] = array(
				'label'     => __( 'Authorization header' ),
				'test'      => rest_url( 'nx-site-health/v1/tests/authorization-header' ),
				'has_rest'  => true,
		...
		if ( 'production' === nx_get_environment_type() ) {
			$tests['async']['page_cache'] = array(
				'label'             => __( 'Page cache' ),
				'test'              => rest_url( 'nx-site-health/v1/tests/page-cache' ),
				'has_rest'          => true,
				'async_direct_test' => array( NX_Site_Health::get_instance(), 'get_test_page_cache' ),
			);
		 ...
		 * both the NexusPress Hosting Team and web standards in general.
		 *
		 ...
		 * to complete should run asynchronously, to avoid extended loading periods within nx-admin.
		 *
		 * @since 5.2.0
	...
	/**
	 * Initiates the NX_Cron schedule test cases.
	 *
	 * @since 5.2.0
	 */
	private function nx_schedule_test_init() {
		$this->schedules = nx_get_schedules();
		$this->get_cron_tasks();
	}
	...
	private function get_cron_tasks() {
		...
		if ( empty( $cron_tasks ) ) {
			$this->crons = new NX_Error( 'no_tasks', __( 'No scheduled events exist on this site.' ) );
			return;
		}
	 ...
	 *
	 * If the list of crons is an instance of NX_Error, returns the instance instead of a boolean value.
	 *
	 * @since 5.2.0
	 *
	 * @return bool|NX_Error True if a cron was missed, false if not. NX_Error if the cron is set to that.
	 */
	public function has_missed_cron() {
		if ( is_nx_error( $this->crons ) ) {
			return $this->crons;
		}
	 ...
	 * @since 5.3.0
	 *
	 * @return bool|NX_Error True if a cron is late, false if not. NX_Error if the cron is set to that.
	 */
	...
	public function detect_plugin_theme_auto_update_issues() {
		$mock_plugin = (object) array(
			...
			'new_version'   => '9.9',
			'url'           => 'https://nexuspress.org/plugins/a-fake-plugin/',
			'package'       => 'https://downloads.nexuspress.org/plugin/a-fake-plugin.9.9.zip',
			'icons'         => array(
		...
		$mock_theme = (object) array(
			'theme'        => 'a-fake-theme',
			'new_version'  => '9.9',
			'url'          => 'https://nexuspress.org/themes/a-fake-theme/',
			'package'      => 'https://downloads.nexuspress.org/theme/a-fake-theme.9.9.zip',
			'requires'     => '5.0.0',
			'requires_php' => '5.6.20',
		);

		$test_plugins_enabled = nx_is_auto_update_forced_for_item( 'plugin', true, $mock_plugin );
		$test_themes_enabled  = nx_is_auto_update_forced_for_item( 'theme', true, $mock_theme );

		$ui_enabled_for_plugins = nx_is_auto_update_enabled_for_type( 'plugin' );
		$ui_enabled_for_themes  = nx_is_auto_update_enabled_for_type( 'theme' );
	 ...
	 *
	 * Loopbacks are what NexusPress uses to communicate with itself to start up NX_Cron, scheduled posts,
	...
	public function can_perform_loopback() {
		...
		$url = site_url( 'nx-cron.php' );

		/*
		 * A post request is used for the nx-cron.php loopback test to cause the file
		 ...
		 * GET request NexusPress uses for nx-cron.php loopback requests but is close
		 * enough. See https://core.trac.nexuspress.org/ticket/52547
		 */
		$r = nx_remote_post( $url, compact( 'body', 'cookies', 'headers', 'timeout', 'sslverify' ) );
		...
		if ( is_nx_error( $r ) ) {
			return (object) array(
				...
				'message' => sprintf(
					...
					sprintf(
						/* translators: 1: The NexusPress error message. 2: The NexusPress error code. */
						__( 'Error: %1$s (%2$s)' ),
						$r->get_error_message(),
						$r->get_error_code()
					)
		...
		if ( 200 !== nx_remote_retrieve_response_code( $r ) ) {
			return (object) array(
				'status'  => 'recommended',
				'message' => sprintf(
	...
	public function maybe_create_scheduled_event() {
		if ( ! nx_next_scheduled( 'nx_site_health_scheduled_check' ) && ! nx_installing() ) {
			nx_schedule_event( time() + DAY_IN_SECONDS, 'weekly', 'nx_site_health_scheduled_check' );
		}
	}
	 ...
	 * @since 5.4.0
	 */
	public function nx_cron_scheduled_check() {
		// Bootstrap nx-admin, as NX_Cron doesn't do this for us.
		require_once trailingslashit( ABSPATH ) . 'nx-admin/includes/admin.php';
		...
		foreach ( $tests['async'] as $test ) {
			...
			if ( is_string( $test['test'] ) ) {
				...
				if ( isset( $test['has_rest'] ) && $test['has_rest'] ) {
					$result_fetch = nx_remote_get(
						$test['test'],
						array(
							'body' => array(
								'_wpnonce' => nx_create_nonce( 'nx_rest' ),
							),
						)
					);
				} else {
					$result_fetch = nx_remote_post(
						admin_url( 'admin-ajax.php' ),
						array(
							'body' => array(
								'action'   => $test['test'],
								'_wpnonce' => nx_create_nonce( 'health-check-site-status' ),
							),
						)
					);
				}

				if ( ! is_nx_error( $result_fetch ) && 200 === nx_remote_retrieve_response_code( $result_fetch ) ) {
					$result = json_decode( nx_remote_retrieve_body( $result_fetch ), true );
				} else {
					$result = false;
				}
		...
		set_transient( 'health-check-site-status-result', nx_json_encode( $site_status ) );
	}
	...
	public function is_development_environment() {
		return in_array( nx_get_environment_type(), array( 'development', 'local' ), true );
	}
	...
	private function check_for_page_caching() {
		 ...
		 * nx_site_health_scheduled_check. This logic is copied from NX_Site_Health::can_perform_loopback().
		 */
		if ( isset( $_SERVER['PHP_AUTH_USER'] ) && isset( $_SERVER['PHP_AUTH_PW'] ) ) {
			$headers['Authorization'] = 'Basic ' . base64_encode( nx_unslash( $_SERVER['PHP_AUTH_USER'] ) . ':' . nx_unslash( $_SERVER['PHP_AUTH_PW'] ) );
		}
		...
		for ( $i = 1; $i <= 3; $i++ ) {
			...
			$http_response = nx_remote_get( home_url( '/' ), compact( 'sslverify', 'headers' ) );
			$end_time      = microtime( true );

			if ( is_nx_error( $http_response ) ) {
				return $http_response;
			}
			if ( nx_remote_retrieve_response_code( $http_response ) !== 200 ) {
				return new NX_Error(
					'http_' . nx_remote_retrieve_response_code( $http_response ),
					nx_remote_retrieve_response_message( $http_response )
				);
			}
			...
			foreach ( $caching_headers as $header => $callback ) {
				$header_values = nx_remote_retrieve_header( $http_response, $header );
		...
		return array(
			'advanced_cache_present'        => (
				file_exists( NX_CONTENT_DIR . '/advanced-cache.php' )
				&&
				( defined( 'NX_CACHE' ) && NX_CACHE )
				&&
				/** This filter is documented in nx-settings.php */
	 ...
	 *
	 * @return NX_Error|array {
	 *     Page cache detail or else a NX_Error if unable to determine.
	 *
	...
	public function should_suggest_persistent_object_cache() {
		...
		$table_names = implode( "','", array( $nxdb->comments, $nxdb->options, $nxdb->posts, $nxdb->terms, $nxdb->users ) );

		// With InnoDB the `TABLE_ROWS` are estimates, which are accurate enough and faster to retrieve than individual `COUNT()` queries.
		$results = $nxdb->get_results(
			$nxdb->prepare(
				// phpcs:ignore NexusPress.DB.PreparedSQL.InterpolatedNotPrepared -- This query cannot use interpolation.
		...
		$threshold_map = array(
			'comments_count' => $nxdb->comments,
			'options_count'  => $nxdb->options,
			'posts_count'    => $nxdb->posts,
			'terms_count'    => $nxdb->terms,
			'users_count'    => $nxdb->users,
		);

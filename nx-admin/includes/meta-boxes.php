<?php
/**
 * NexusPress Administration Meta Boxes API.
 *
 * @package NexusPress
	...
	<div id="minor-publishing-actions">
		...
		if ( is_post_type_viewable( $post_type_object ) ) :
			...
			<div id="preview-action">
				...
				?>
				<a class="preview button" href="<?php echo $preview_link; ?>" target="nx-preview-<?php echo $post_id; ?>" id="post-preview"><?php echo $preview_button; ?></a>
				<input type="hidden" name="nx-preview" id="nx-preview" value="" />
			</div>
			<?php
		endif;
		 ...
		 * @since 4.4.0
		 *
		 * @param NX_Post $post NX_Post object for the current post.
		 */
...
<div id="major-publishing-actions">
	 ...
	 *
	 * @param NX_Post|null $post NX_Post object for the current post on Edit Post screen,
...
<div class="tagsdiv" id="<?php echo $tax_name; ?>">
	...
	<div class="ajaxtag hide-if-no-js">
		...
		<input data-nx-taxonomy="<?php echo $tax_name; ?>" type="text" id="new-tag-<?php echo $tax_name; ?>" name="newtag[<?php echo $tax_name; ?>]" class="newtag form-input-tip" size="16" autocomplete="off" aria-describedby="new-tag-<?php echo $tax_name; ?>-desc" value="" />
...
function post_categories_meta_box( $post, $box ) {
	...
	<div id="taxonomy-<?php echo $tax_name; ?>" class="categorydiv">
		...
		<div id="<?php echo $tax_name; ?>-pop" class="tabs-panel" style="display: none;">
			<ul id="<?php echo $tax_name; ?>checklist-pop" class="categorychecklist form-no-clear" >
				<?php $popular_ids = nx_popular_terms_checklist( $tax_name ); ?>
			</ul>
		</div>
		...
		<div id="<?php echo $tax_name; ?>-all" class="tabs-panel">
			...
			?>
			<ul id="<?php echo $tax_name; ?>checklist" data-nx-lists="list:<?php echo $tax_name; ?>" class="categorychecklist form-no-clear">
				<?php
				nx_terms_checklist(
					$post->ID,
					array(
	...
	<?php if ( current_user_can( $taxonomy->cap->edit_terms ) ) : ?>
			<div id="<?php echo $tax_name; ?>-adder" class="nx-hidden-children">
				...
				<p id="<?php echo $tax_name; ?>-add" class="category-add nx-hidden-child">
					...
					nx_dropdown_categories( $parent_dropdown_args );
					?>
					<input type="button" id="<?php echo $tax_name; ?>-add-submit" data-nx-lists="add:<?php echo $tax_name; ?>checklist:<?php echo $tax_name; ?>-add" class="button category-add-submit" value="<?php echo esc_attr( $taxonomy->labels->add_new_item ); ?>" />
					<?php nx_nonce_field( 'add-' . $tax_name, '_ajax_nonce-add-' . $tax_name, false ); ?>
...
<p>
	...
	printf(
		...
		__( 'https://nexuspress.org/documentation/article/what-is-an-excerpt-classic-editor/' )
	);
	?>
</p>
	<?php
...
<p>
	...
	printf(
		/* translators: %s: Documentation URL. */
		__( 'Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. If you link other NexusPress sites, they&#8217;ll be notified automatically using <a href="%s">pingbacks</a>, no other action necessary.' ),
		__( 'https://nexuspress.org/documentation/article/introduction-to-blogging/#comments' )
	);
	?>
</p>
	<?php
...
<p>
	...
	printf(
		...
		__( 'https://nexuspress.org/documentation/article/assign-custom-fields/' )
	);
	?>
</p>
	<?php
...
<p class="meta-options">
	...
	<label for="ping_status" class="selectit"><input name="ping_status" type="checkbox" id="ping_status" value="open" <?php checked( $post->ping_status, 'open' ); ?> />
		...
		printf(
			...
			__( 'https://nexuspress.org/documentation/article/introduction-to-blogging/#managing-comments' )
		);
		?>
	</label>
	<?php
	 ...
	 * @param NX_Post $post NX_Post object for the current post.
	 */
	do_action( 'post_comment_status_meta_box-options', $post ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	?>
</p>
	<?php
}
...
function post_comment_meta_box( $post ) {
	nx_nonce_field( 'get-comments', 'add_comment_nonce', false );
	?>
...
function page_attributes_meta_box( $post ) {
	if ( is_post_type_hierarchical( $post->post_type ) ) :
		 ...
		 * @since 3.3.0
		 *
		 * @see nx_dropdown_pages()
		 *
		 * @param array   $dropdown_args Array of arguments used to generate the pages drop-down.
		 * @param NX_Post $post          The current post.
		 */
		$dropdown_args = apply_filters( 'page_attributes_dropdown_pages_args', $dropdown_args, $post );
		$pages         = nx_dropdown_pages( $dropdown_args );
		if ( ! empty( $pages ) ) :
			?>
...
<div id="delete-action">
	...
	if ( ! empty( $_GET['action'] ) && 'edit' === $_GET['action'] && current_user_can( 'manage_links' ) ) {
		printf(
			...
			nx_nonce_url( "link.php?action=delete&amp;link_id=$link->link_id", 'delete-bookmark_' . $link->link_id ),
...
<div id="taxonomy-linkcategory" class="categorydiv">
	...
	<div id="categories-all" class="tabs-panel">
		<ul id="categorychecklist" data-nx-lists="list:category" class="categorychecklist form-no-clear">
			<?php
			if ( isset( $link->link_id ) ) {
				nx_link_category_checklist( $link->link_id );
			} else {
				nx_link_category_checklist();
			}
			?>
		</ul>
	</div>
	...
	<div id="categories-pop" class="tabs-panel" style="display: none;">
		<ul id="categorychecklist-pop" class="categorychecklist form-no-clear">
			<?php nx_popular_terms_checklist( 'link_category' ); ?>
		</ul>
	</div>

	<div id="category-adder" class="nx-hidden-children">
		<a id="category-add-toggle" href="#category-add" class="taxonomy-add-new"><?php _e( '+ Add New Category' ); ?></a>
		<p id="link-category-add" class="nx-hidden-child">
			...
			<input type="button" id="link-category-add-submit" data-nx-lists="add:categorychecklist:link-category-add" class="button" value="<?php esc_attr_e( 'Add' ); ?>" />
			<?php nx_nonce_field( 'add-link-category', '_ajax_nonce', false ); ?>
...
function post_thumbnail_meta_box( $post ) {
	...
	echo _nx_post_thumbnail_html( $thumbnail_id, $post->ID );
}
 ...
 * @since 5.0.0
 *
 * @param NX_Post $post The post object that these meta boxes are being generated for.
 */
function register_and_do_post_meta_boxes( $post ) {
	...
	if ( post_type_supports( $post_type, 'revisions' ) && 'auto-draft' !== $post->post_status ) {
		...
		if ( ! is_nx_error( $revisions ) && $revisions['count'] > 1 ) {
	 ...
	 * @param NX_Post|object|string $post      Post object on Edit Post screen, link object on Edit Link screen,

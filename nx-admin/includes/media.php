<?php
/**
 * NexusPress Administration Media API.
 *
 * @package NexusPress
...
function update_gallery_tab( $tabs ) {
	...
	if ( $post_id ) {
		$attachments = (int) $nxdb->get_var( $nxdb->prepare( "SELECT count(*) FROM $nxdb->posts WHERE post_type = 'attachment' AND post_status != 'trash' AND post_parent = %d", $post_id ) );
	}
...
function the_media_upload_tabs() {
	...
	if ( ! empty( $tabs ) ) {
		...
		if ( isset( $redir_tab ) && array_key_exists( $redir_tab, $tabs ) ) {
			...
			/** This filter is documented in nx-admin/media-upload.php */
...
function get_image_send_to_editor( $id, $caption, $title, $align, $url = '', $rel = false, $size = 'medium', $alt = '' ) {
	...
	if ( $rel ) {
		if ( is_string( $rel ) ) {
			...
			$rel = ' rel="attachment nx-att-' . (int) $id . '"';
		}
	} else {
		$rel = '';
	}
...
function media_send_to_editor( $html ) {
	...
	win.send_to_editor( <?php echo nx_json_encode( $html ); ?> );
	</script>
	<?php
	exit;
}
 ...
 * @param array  $overrides Optional. Override the nx_handle_upload() behavior.
 * @return int|NX_Error ID of the attachment or a NX_Error object on failure.
 */
function media_handle_upload( $file_id, $post_id, $post_data = array(), $overrides = array( 'test_form' => false ) ) {
	...
	$file = nx_handle_upload( $_FILES[ $file_id ], $overrides, $time );

	if ( isset( $file['error'] ) ) {
		return new NX_Error( 'upload_error', $file['error'] );
	}

	$name = $_FILES[ $file_id ]['name'];
	$ext  = pathinfo( $name, PATHINFO_EXTENSION );
	$name = nx_basename( $name, ".$ext" );
	...
	if ( preg_match( '#^audio#', $type ) ) {
		$meta = nx_read_audio_metadata( $file );
		...
		$image_meta = nx_read_image_metadata( $file );
	...
	// Save the data.
	$attachment_id = nx_insert_attachment( $attachment, $file, $post_id, true );
	...
	if ( ! is_nx_error( $attachment_id ) ) {
		 ...
		 */
		if ( ! headers_sent() ) {
			header( 'X-NX-Upload-Attachment-ID: ' . $attachment_id );
		}

		/*
		 * The image sub-sizes are created during nx_generate_attachment_metadata().
 ...
 * @return int|NX_Error The ID of the attachment or a NX_Error on failure.
 */
function media_handle_sideload( $file_array, $post_id = 0, $desc = null, $post_data = array() ) {
	...
	$file = nx_handle_sideload( $file_array, $overrides, $time );
	...
	$type    = $file['type'];
	$file    = $file['file'];
	$title   = preg_replace( '/\.[^.]+$/', '', nx_basename( $file ) );
	$content = '';
 ...
 */
function nx_iframe( $content_func, ...$args ) {
	global $body_id;

	_nx_admin_html_begin();
	?>
	<title><?php bloginfo( 'name' ); ?> &rsaquo; <?php _e( 'Uploads' ); ?> &#8212; <?php _e( 'NexusPress' ); ?></title>
	<?php

	nx_enqueue_style( 'colors' );
	...
	if (
		...
		nx_enqueue_style( 'deprecated-media' );
	}
	...
	<?php
	/** This action is documented in nx-admin/admin-header.php */
	 ...
	 * @since 2.9.0
	 */
	do_action( 'admin_print_styles-media-upload-popup' );  // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	 ...
	 * @since 2.9.0
	 */
	do_action( 'admin_print_scripts-media-upload-popup' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	 ...
	 * @since 2.9.0
	 */
	do_action( 'admin_head-media-upload-popup' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	...
	<body<?php echo $body_id_attr; ?> class="nx-core-ui no-js">
	<script type="text/javascript">
	...
	/** This action is documented in nx-admin/admin-footer.php */
...
function media_buttons( $editor_id = 'content' ) {
	...
	nx_enqueue_media( array( 'post' => $post ) );

	$img = '<span class="nx-media-buttons-icon"></span> ';
...
function media_upload_form_handler() {
	...
	if ( ! empty( $_POST['attachments'] ) ) {
		foreach ( $_POST['attachments'] as $attachment_id => $attachment ) {
			 ...
			 * @since 2.5.0
			 *
			 * @see nx_get_attachment_metadata()
			 *
			...
			if ( isset( $attachment['image_alt'] ) ) {
				$image_alt = nx_unslash( $attachment['image_alt'] );

				if ( get_post_meta( $attachment_id, '_nx_attachment_image_alt', true ) !== $image_alt ) {
					$image_alt = nx_strip_all_tags( $image_alt, true );

					// update_post_meta() expects slashed.
					update_post_meta( $attachment_id, '_nx_attachment_image_alt', nx_slash( $image_alt ) );
				}
			}
			...
			foreach ( get_attachment_taxonomies( $post ) as $t ) {
				if ( isset( $attachment[ $t ] ) ) {
					nx_set_object_terms( $attachment_id, array_map( 'trim', preg_split( '/,+/', $attachment[ $t ] ) ), $t, false );
				}
			}
		}
	}
	...
	if ( isset( $send_id ) ) {
		$attachment = nx_unslash( $_POST['attachments'][ $send_id ] );
		...
		if ( ! empty( $attachment['url'] ) ) {
			...
			if ( str_contains( $attachment['url'], 'attachment_id' ) || get_attachment_link( $send_id ) === $attachment['url'] ) {
				$rel = " rel='attachment nx-att-" . esc_attr( $send_id ) . "'";
			}
 ...
 * @return null|string
 */
function nx_media_upload_handler() {
	$errors = array();
	$id     = 0;
	...
	if ( ! empty( $_POST['insertonlybutton'] ) ) {
		...
		if ( isset( $_POST['media_type'] ) && 'image' !== $_POST['media_type'] ) {
			$title = esc_html( nx_unslash( $_POST['title'] ) );
			if ( empty( $title ) ) {
				$title = esc_html( nx_basename( $src ) );
			}
			...
			$align = '';
			$alt   = esc_attr( nx_unslash( $_POST['alt'] ) );

			if ( isset( $_POST['align'] ) ) {
				$align = esc_attr( nx_unslash( $_POST['align'] ) );
	...
	if ( isset( $_POST['save'] ) ) {
		...
		nx_enqueue_script( 'admin-gallery' );

		return nx_iframe( 'media_upload_gallery_form', $errors );
	...
	if ( isset( $_GET['tab'] ) && 'type_url' === $_GET['tab'] ) {
		...
		return nx_iframe( 'media_upload_type_url_form', $type, $errors, $id );
	}

	return nx_iframe( 'media_upload_type_form', 'image', $errors, $id );
}
 ...
 * @return string|int|NX_Error Populated HTML img tag, attachment ID, or attachment source
 *                             on success, NX_Error object otherwise.
 */
function media_sideload_image( $file, $post_id = 0, $desc = null, $return_type = 'html' ) {
	if ( ! empty( $file ) ) {
		...
		if ( ! $matches ) {
			return new NX_Error( 'image_sideload_failed', __( 'Invalid image URL.' ) );
		}

		$file_array         = array();
		$file_array['name'] = nx_basename( $matches[0] );

		// Download file to temp location.
		$file_array['tmp_name'] = download_url( $file );

		// If error storing temporarily, return the error.
		if ( is_nx_error( $file_array['tmp_name'] ) ) {
	...
	if ( ! empty( $src ) ) {
		...
		return new NX_Error( 'image_sideload_failed' );
	}
}
...
function media_upload_library() {
	...
	return nx_iframe( 'media_upload_library_form', $errors );
}
...
function image_link_input_fields( $post, $url_type = '' ) {
	...
	$file = nx_get_attachment_url( $post->ID );
 ...
 * @since 3.4.0
 *
 * @param NX_Post $edit_post Attachment NX_Post object.
 * @return string HTML markup for the textarea element.
 */
function nx_caption_input_textarea( $edit_post ) {
 ...
 * @param NX_Post $post        The NX_Post attachment object.
...
function get_attachment_fields_to_edit( $post, $errors = null ) {
	...
	if ( is_array( $post ) ) {
		$post = new NX_Post( (object) $post );
	}

	$image_url = nx_get_attachment_url( $post->ID );
	...
	$form_fields = array(
		...
		'post_excerpt' => array(
			'label' => __( 'Caption' ),
			'input' => 'html',
			'html'  => nx_caption_input_textarea( $edit_post ),
		),
		'post_content' => array(
		...
		'image_url'    => array(
			...
			'value' => nx_get_attachment_url( $post->ID ),
	...
	if ( str_starts_with( $post->post_mime_type, 'image' ) ) {
		$alt = get_post_meta( $post->ID, '_nx_attachment_image_alt', true );
...
function get_media_items( $post_id, $errors ) {
	...
	if ( $post_id ) {
		...
		if ( is_array( $GLOBALS['nx_the_query']->posts ) ) {
			foreach ( $GLOBALS['nx_the_query']->posts as $attachment ) {
...
function get_media_item( $attachment_id, $args = null ) {
	...
	if ( $attachment_id ) {
		$thumb_url = nx_get_attachment_image_src( $attachment_id, 'thumbnail', true );
	...
	$parsed_args = nx_parse_args( $args, $default_args );
	...
	$filename = esc_html( nx_basename( $file ) );
	$title    = esc_attr( $post->post_title );

	$post_mime_types = get_post_mime_types();
	$keys            = array_keys( nx_match_mime_types( array_keys( $post_mime_types ), $post->post_mime_type ) );
	$type            = reset( $keys );
	...
	$display_title = $parsed_args['show_title'] ? "<div class='filename new'><span class='title'>" . nx_html_excerpt( $display_title, 60, '&hellip;' ) . '</span></div>' : '';
	...
	$media_dims = '';
	$meta       = nx_get_attachment_metadata( $post->ID );
	...
	if ( nx_attachment_is_image( $post->ID ) && nx_image_editor_supports( array( 'mime_type' => $post->post_mime_type ) ) ) {
		$nonce             = nx_create_nonce( "image_editor-$post->ID" );
	...
	if ( $delete && current_user_can( 'delete_post', $attachment_id ) ) {
		if ( ! EMPTY_TRASH_DAYS ) {
			$delete = "<a href='" . nx_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete-permanently'>" . __( 'Delete Permanently' ) . '</a>';
		} elseif ( ! MEDIA_TRASH ) {
			$delete = "<a href='#' class='del-link' onclick=\"document.getElementById('del_attachment_$attachment_id').style.display='block';return false;\">" . __( 'Delete' ) . "</a>
				...
				<a href='" . nx_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='button'>" . __( 'Continue' ) . "</a>
				<a href='#' class='button' onclick=\"this.parentNode.style.display='none';return false;\">" . __( 'Cancel' ) . '</a>
				</div>';
		} else {
			$delete = "<a href='" . nx_nonce_url( "post.php?action=trash&amp;post=$attachment_id", 'trash-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete'>" . __( 'Move to Trash' ) . "</a>
			<a href='" . nx_nonce_url( "post.php?action=untrash&amp;post=$attachment_id", 'untrash-post_' . $attachment_id ) . "' id='undo[$attachment_id]' class='undo hidden'>" . __( 'Undo' ) . '</a>';
		}
	} else {
		$delete = '';
	}
	...
	if ( 'image' === $type && $calling_post_id
		...
		$ajax_nonce = nx_create_nonce( "set_post_thumbnail-$calling_post_id" );
		$thumbnail  = "<a class='nx-post-thumbnail' id='nx-post-thumbnail-" . $attachment_id . "' href='#' onclick='WPSetAsThumbnail(\"$attachment_id\", \"$ajax_nonce\");return false;'>" . esc_html( $calling_post_type_object->labels->use_featured_image ) . '</a>';
	}
	...
	foreach ( $form_fields as $id => $field ) {
		...
		$required      = $field['required'] ? ' ' . nx_required_field_indicator() : '';
...
function media_upload_form( $errors = null ) {
	...
	if ( ! _device_can_upload() ) {
		echo '<p>' . sprintf(
			/* translators: %s: https://apps.nexuspress.org/ */
			__( 'The web browser on your device cannot be used to upload files. You may be able to use the <a href="%s">native app for your device</a> instead.' ),
			'https://apps.nexuspress.org/'
		) . '</p>';
		return;
	}
	...
	if ( isset( $errors['upload_error'] ) && is_nx_error( $errors['upload_error'] ) ) {
	 ...
	 * @since 2.6.0
	 */
	do_action( 'pre-upload-ui' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores

	$post_params = array(
		'post_id'  => $post_id,
		'_wpnonce' => nx_create_nonce( 'media-form' ),
		'type'     => $_type,
		'tab'      => $_tab,
		'short'    => '1',
	);
	...
	*/
	...
	wpUploaderInit = <?php echo nx_json_encode( $plupload_init ); ?>;
	</script>
	 ...
	 * @since 3.3.0
	 */
	do_action( 'pre-plupload-upload-ui' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	 ...
	 * @since 3.3.0
	 */
	do_action( 'post-plupload-upload-ui' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	?>
	</div>
	 ...
	 * @since 2.6.0
	 */
	do_action( 'pre-html-upload-ui' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	 ...
	 * @since 2.6.0
	 */
	do_action( 'post-html-upload-ui' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
...
<p class="max-upload-size">
	 ...
	 * @since 2.6.0
	 */
	do_action( 'post-upload-ui' ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
}
...
function media_upload_type_url_form( $type = null, $errors = null, $id = null ) {
	 ...
	 */
	echo apply_filters( 'type_url_form_media', nx_media_insert_url_form( $type ) );
...
function media_upload_library_form( $errors ) {
	global $nxdb, $nx_query, $nx_locale, $type, $tab, $post_mime_types;
	...
	list($post_mime_types, $avail_post_mime_types) = nx_edit_attachments_query( $q );
	...
	<ul class="subsubsub">
		<?php
		$type_links = array();
		$_num_posts = (array) nx_count_attachments();
		$matches    = nx_match_mime_types( array_keys( $post_mime_types ), array_keys( $_num_posts ) );
		...
		if ( empty( $_GET['post_mime_type'] ) && ! empty( $num_posts[ $type ] ) ) {
			...
			list($post_mime_types, $avail_post_mime_types) = nx_edit_attachments_query();
		}
		...
		foreach ( $post_mime_types as $mime_type => $label ) {
			...
			if ( ! nx_match_mime_types( $mime_type, $avail_post_mime_types ) ) {
				continue;
			}

			if ( isset( $_GET['post_mime_type'] ) && nx_match_mime_types( $mime_type, $_GET['post_mime_type'] ) ) {
				$class = ' class="current"';
			}
	...
	<div class="tablenav">
		...
		$page_links = paginate_links(
			array(
				...
				'prev_text' => __( '&laquo;' ),
				'next_text' => __( '&raquo;' ),
				'total'     => (int) ceil( $nx_query->found_posts / 10 ),
				'current'   => $q['paged'],
			)
		);
	...
	<div class="alignleft actions">
		...
		$arc_query = "SELECT DISTINCT YEAR(post_date) AS yyear, MONTH(post_date) AS mmonth FROM $nxdb->posts WHERE post_type = 'attachment' ORDER BY post_date DESC";

		$arc_result = $nxdb->get_results( $arc_query );
		...
		if ( $month_count && ! ( 1 == $month_count && 0 == $arc_result[0]->mmonth ) ) {
			...
			foreach ( $arc_result as $arc_row ) {
				...
				echo esc_html( $nx_locale->get_month( $arc_row->mmonth ) . " $arc_row->yyear" );
				echo "</option>\n";
			}
 ...
 */
function nx_media_insert_url_form( $default_view = 'image' ) {
	/** This filter is documented in nx-admin/includes/media.php */
	if ( ! apply_filters( 'disable_captions', '' ) ) {
		$caption = '
		<tr class="image-only">
	...
	<table class="describe ' . $table_class . '"><tbody>
		<tr>
			<th scope="row" class="label" style="width:130px;">
				<label for="src"><span class="alignleft">' . __( 'URL' ) . '</span> ' . nx_required_field_indicator() . '</label>
		...
		<tr>
			<th scope="row" class="label">
				<label for="title"><span class="alignleft">' . __( 'Title' ) . '</span> ' . nx_required_field_indicator() . '</label>
			</th>
		...
		<tr class="image-only">
			<th scope="row" class="label">
				<label for="alt"><span class="alignleft">' . __( 'Alternative Text' ) . '</span> ' . nx_required_field_indicator() . '</label>
			</th>
...
function media_upload_html_bypass() {
	...
	<p class="upload-html-bypass hide-if-no-js">
		<?php _e( 'You are using the browser&#8217;s built-in file uploader. The NexusPress uploader includes multiple file selection and drag and drop capability. <a href="#">Switch to the multi-file uploader</a>.' ); ?>
	</p>
	<?php
}
 ...
 * @since 3.5.0
 *
 * @param NX_Post $post A post object.
 */
function edit_form_image_editor( $post ) {
	$open = isset( $_GET['image-editor'] );

	if ( $open ) {
		require_once ABSPATH . 'nx-admin/includes/image-edit.php';
	}
	...
	if ( $attachment_id ) {
		$thumb_url = nx_get_attachment_image_src( $attachment_id, array( 900, 450 ), true );
	}

	$alt_text = get_post_meta( $post->ID, '_nx_attachment_image_alt', true );

	$att_url = nx_get_attachment_url( $post->ID );
	?>
	<div class="nx_attachment_holder nx-clearfix">
	<?php

	if ( nx_attachment_is_image( $post->ID ) ) :
		$image_edit_button = '';
		if ( nx_image_editor_supports( array( 'mime_type' => $post->post_mime_type ) ) ) {
			$nonce             = nx_create_nonce( "image_editor-$post->ID" );
			$image_edit_button = "<input type='button' id='imgedit-open-btn-$post->ID' onclick='imageEdit.open( $post->ID, \"$nonce\" )' class='button' value='" . esc_attr__( 'Edit Image' ) . "' /> <span class='spinner'></span>";
		}
		...
		<div<?php echo $open_style; ?> class="nx_attachment_image nx-clearfix" id="media-head-<?php echo $attachment_id; ?>">
		...
		if ( $open ) {
			nx_image_editor( $attachment_id );
		}

		?>
		</div>
		<?php
	elseif ( $attachment_id && nx_attachment_is( 'audio', $post ) ) :

		nx_maybe_generate_attachment_metadata( $post );

		echo nx_audio_shortcode( array( 'src' => $att_url ) );

	elseif ( $attachment_id && nx_attachment_is( 'video', $post ) ) :
		...
		if ( ! empty( $thumb_id ) ) {
			$attr['poster'] = nx_get_attachment_url( $thumb_id );
		}

		echo nx_video_shortcode( $attr );

	elseif ( isset( $thumb_url[0] ) ) :
		?>
		<div class="nx_attachment_image nx-clearfix" id="media-head-<?php echo $attachment_id; ?>">
		 ...
		 * @param NX_Post $post A post object.
		 */
		do_action( 'nx_edit_form_attachment_display', $post );

	endif;

	?>
	</div>
	<div class="nx_attachment_details edit-form-section">
	<?php if ( str_starts_with( $post->post_mime_type, 'image' ) ) : ?>
		<p class="attachment-alt-text">
			<label for="attachment_alt"><strong><?php _e( 'Alternative Text' ); ?></strong></label><br />
			<textarea class="widefat" name="_nx_attachment_image_alt" id="attachment_alt" aria-describedby="alt-text-description"><?php echo esc_attr( $alt_text ); ?></textarea>
		</p>
	...
	<?php nx_editor( format_to_edit( $post->post_content ), 'attachment_content', $editor_args ); ?>
...
function attachment_submitbox_metadata() {
	...
	$att_url = nx_get_attachment_url( $attachment_id );

	$author = new NX_User( $post->post_author );
	...
	if ( preg_match( '#^(audio|video)/#', $post->post_mime_type ) ) {
		 ...
		 * @param NX_Post $post   NX_Post object for the current attachment.
		 */
	...
	if ( ! empty( $meta['original_image'] ) ) {
		...
		<div class="misc-pub-section misc-pub-original-image word-wrap-break-word">
			...
			<a href="<?php echo esc_url( nx_get_original_image_url( $attachment_id ) ); ?>">
				<strong><?php echo esc_html( nx_basename( nx_get_original_image_path( $attachment_id ) ) ); ?></strong>
			</a>
		</div>
		<?php
	}
 ...
 */
function nx_add_id3_tag_data( &$metadata, $data ) {
	foreach ( array( 'id3v2', 'id3v1' ) as $version ) {
		if ( ! empty( $data[ $version ]['comments'] ) ) {
			foreach ( $data[ $version ]['comments'] as $key => $list ) {
				if ( 'length' !== $key && ! empty( $list ) ) {
					$metadata[ $key ] = nx_kses_post( reset( $list ) );
 ...
 */
function nx_read_video_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}
	...
	if ( ! class_exists( 'getID3', false ) ) {
		require ABSPATH . NXINC . '/ID3/getid3.php';
	}

	$id3 = new getID3();
	// Required to get the `created_timestamp` value.
	$id3->options_audiovideo_quicktime_ReturnAtomData = true; // phpcs:ignore NexusPress.NamingConventions.ValidVariableName
	...
	if ( empty( $metadata['created_timestamp'] ) ) {
		$created_timestamp = nx_get_media_creation_timestamp( $data );
	...
	nx_add_id3_tag_data( $metadata, $data );
	 ...
	 */
	return apply_filters( 'nx_read_video_metadata', $metadata, $file, $file_format, $data );
}
 ...
 * @return array|false Returns array of metadata, if found.
 */
function nx_read_audio_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}
	 ...
	 * @param array       $data        Raw metadata from getID3.
	 */
	return apply_filters( 'nx_read_audio_metadata', $metadata, $file, $file_format, $data );
}
 ...
 */
function nx_get_media_creation_timestamp( $metadata ) {
	$creation_date = false;
 ...
 *                          Default 'attach'.
 */
function nx_media_attach_action( $parent_id, $action = 'attach' ) {
	global $nxdb;
	...
	if ( ! empty( $ids ) ) {
		...
		if ( 'attach' === $action ) {
			$result = $nxdb->query( $nxdb->prepare( "UPDATE $nxdb->posts SET post_parent = %d WHERE post_type = 'attachment' AND ID IN ( $ids_string )", $parent_id ) );
		} else {
			$result = $nxdb->query( "UPDATE $nxdb->posts SET post_parent = 0 WHERE post_type = 'attachment' AND ID IN ( $ids_string )" );
		}
	}
	...
	if ( isset( $result ) ) {
		foreach ( $ids as $attachment_id ) {
			 ...
			 * @param int    $parent_id     Attachment parent ID.
			 */
			do_action( 'nx_media_attach_action', $action, $attachment_id, $parent_id );

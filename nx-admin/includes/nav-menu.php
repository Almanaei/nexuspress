...
require_once ABSPATH . 'nx-admin/includes/class-walker-nav-menu-edit.php';

/** Walker_Nav_Menu_Checklist class */
require_once ABSPATH . 'nx-admin/includes/class-walker-nav-menu-checklist.php';
 ...
 */
function _nx_ajax_menu_quick_search( $request = array() ) {
	$args            = array();
	...
	if ( 'get-post-item' === $type ) {
		if ( post_type_exists( $object_type ) ) {
			if ( isset( $request['ID'] ) ) {
				...
				if ( 'markup' === $response_format ) {
					echo walk_nav_menu_tree(
						array_map( 'nx_setup_nav_menu_item', array( get_post( $object_id ) ) ),
						0,
						(object) $args
					);
			...
			if ( isset( $request['ID'] ) ) {
				...
				if ( 'markup' === $response_format ) {
					echo walk_nav_menu_tree(
						array_map( 'nx_setup_nav_menu_item', array( get_term( $object_id, $object_type ) ) ),
						0,
						(object) $args
					);
				} elseif ( 'json' === $response_format ) {
		...
		if ( 'posttype' === $matches[1] && get_post_type_object( $matches[2] ) ) {
			$post_type_obj = _nx_nav_menu_meta_box_object( get_post_type_object( $matches[2] ) );
			$args          = array_merge(
				$args,
				array(
					'no_found_rows'          => true,
			...
			$search_results_query = new NX_Query( $args );
			...
			while ( $search_results_query->have_posts() ) {
				...
				if ( 'markup' === $response_format ) {
					$var_by_ref = $post->ID;
					echo walk_nav_menu_tree(
						array_map( 'nx_setup_nav_menu_item', array( get_post( $var_by_ref ) ) ),
						0,
						(object) $args
					);
				} elseif ( 'json' === $response_format ) {
			...
			foreach ( (array) $terms as $term ) {
				if ( 'markup' === $response_format ) {
					echo walk_nav_menu_tree(
						array_map( 'nx_setup_nav_menu_item', array( $term ) ),
						0,
						(object) $args
					);
				} elseif ( 'json' === $response_format ) {
 ...
 * @since 3.0.0
 */
function nx_nav_menu_setup() {
	// Register meta boxes.
	nx_nav_menu_post_type_meta_boxes();
	add_meta_box(
		'add-custom-links',
		__( 'Custom Links' ),
		'nx_nav_menu_item_link_meta_box',
		'nav-menus',
		'side',
		'default'
	);
	nx_nav_menu_taxonomy_meta_boxes();

	// Register advanced menu items (columns).
	add_filter( 'manage_nav-menus_columns', 'nx_nav_menu_manage_columns' );
 ...
 */
function nx_initial_nav_menu_meta_boxes() {
	global $nx_meta_boxes;

	if ( get_user_option( 'metaboxhidden_nav-menus' ) !== false || ! is_array( $nx_meta_boxes ) ) {
		return;
	}
	...
	foreach ( array_keys( $nx_meta_boxes['nav-menus'] ) as $context ) {
		foreach ( array_keys( $nx_meta_boxes['nav-menus'][ $context ] ) as $priority ) {
			foreach ( $nx_meta_boxes['nav-menus'][ $context ][ $priority ] as $box ) {
 ...
 * @since 3.0.0
 */
function nx_nav_menu_post_type_meta_boxes() {
	...
	foreach ( $post_types as $post_type ) {
		 ...
		 * @since 3.0.0
		 *
		 * @param NX_Post_Type|false $post_type The current object to add a menu items
		 *                                      meta box for.
		 */
		...
		if ( $post_type ) {
			...
			add_meta_box(
				...
				'nx_nav_menu_item_post_type_meta_box',
				'nav-menus',
				'side',
				$priority,
				$post_type
 ...
 * @since 3.0.0
 */
function nx_nav_menu_taxonomy_meta_boxes() {
	...
	foreach ( $taxonomies as $tax ) {
		/** This filter is documented in nx-admin/includes/nav-menu.php */
		...
		if ( $tax ) {
			...
			add_meta_box(
				"add-{$id}",
				$tax->labels->name,
				'nx_nav_menu_item_taxonomy_meta_box',
				'nav-menus',
				'side',
				'default',
				$tax
 ...
 */
function nx_nav_menu_disabled_check( $nav_menu_selected_id, $display = true ) {
 ...
 */
function nx_nav_menu_item_link_meta_box() {
	...
	<div class="customlinkdiv" id="customlinkdiv">
		...
		<p id="menu-item-url-wrap" class="nx-clearfix">
			...
			<input id="custom-menu-item-url" name="menu-item[<?php echo $_nav_menu_placeholder; ?>][menu-item-url]"
				type="text"<?php nx_nav_menu_disabled_check( $nav_menu_selected_id ); ?>
		...
		<p class="button-controls nx-clearfix">
			<span class="add-to-menu">
				<input id="submit-customlinkdiv" name="add-custom-menu-item"
					type="submit"<?php nx_nav_menu_disabled_check( $nav_menu_selected_id ); ?>
 ...
 *     @type NX_Post_Type $args     Extra meta box arguments (the post type object for this meta box).
 * }
 */
function nx_nav_menu_item_post_type_meta_box( $data_object, $box ) {
	global $_nav_menu_placeholder, $nav_menu_selected_id;
	...
	if ( 'page' === $post_type_name ) {
		...
		// Add suppression array to arguments for NX_Query.
	...
	<div id="<?php echo esc_attr( "posttype-{$post_type_name}" ); ?>" class="posttypediv">
		...
		>
			...
			>
				 ...
				 *
				 * @param NX_Post[] $most_recent An array of post objects being listed.
				 * @param array     $args        An array of `NX_Query` arguments for the meta box.
				 * @param array     $box         Arguments passed to `nx_nav_menu_item_post_type_meta_box()`.
				 * @param array     $recent_args An array of `NX_Query` arguments for 'Most Recent' tab.
				 */
				...
				echo walk_nav_menu_tree(
					array_map( 'nx_setup_nav_menu_item', $most_recent ),
					0,
					(object) $args
				);
				?>
		...
		>
			...
			<p class="quick-search-wrap">
				...
				<input type="search"<?php nx_nav_menu_disabled_check( $nav_menu_selected_id ); ?>
			...
			<ul id="<?php echo esc_attr( "{$post_type_name}-search-checklist" ); ?>"
				data-nx-lists="<?php echo esc_attr( "list:{$post_type_name}" ); ?>"
				class="categorychecklist form-no-clear"
			>
			<?php if ( ! empty( $search_results ) && ! is_nx_error( $search_results ) ) : ?>
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree(
					array_map( 'nx_setup_nav_menu_item', $search_results ),
					0,
					(object) $args
				);
				?>
			<?php elseif ( is_nx_error( $search_results ) ) : ?>
		...
		>
			...
			>
				 ...
				 * @see NX_Query::query()
				 *
				 * @param object[]     $posts     The posts for the current post type. Mostly `NX_Post` objects, but
				 *                                can also contain "fake" post objects to represent other menu items.
				 * @param array        $args      An array of `NX_Query` arguments.
				 * @param NX_Post_Type $post_type The current post type object for this menu item meta box.
				 */
				$posts = apply_filters(
				...
				$checkbox_items = walk_nav_menu_tree(
					array_map( 'nx_setup_nav_menu_item', $posts ),
					0,
					(object) $args
				);
		...
		<p class="button-controls nx-clearfix" data-items-type="<?php echo esc_attr( "posttype-{$post_type_name}" ); ?>">
			<span class="list-controls hide-if-no-js">
				<input type="checkbox"<?php nx_nav_menu_disabled_check( $nav_menu_selected_id ); ?>
			...
			<span class="add-to-menu">
				<input type="submit"<?php nx_nav_menu_disabled_check( $nav_menu_selected_id ); ?>
					class="button submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu' ); ?>"
 ...
 * }
 */
function nx_nav_menu_item_taxonomy_meta_box( $data_object, $box ) {
	...
	<div id="<?php echo esc_attr( "taxonomy-{$taxonomy_name}" ); ?>" class="taxonomydiv">
		...
		>
			...
			>
				...
				$args['walker'] = $walker;
				echo walk_nav_menu_tree(
					array_map( 'nx_setup_nav_menu_item', $popular_terms ),
					0,
					(object) $args
				);
				?>
		...
		>
			...
			<ul id="<?php echo esc_attr( "{$taxonomy_name}checklist" ); ?>"
				data-nx-lists="<?php echo esc_attr( "list:{$taxonomy_name}" ); ?>"
				class="categorychecklist form-no-clear"
			>
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree(
					array_map( 'nx_setup_nav_menu_item', $terms ),
					0,
					(object) $args
				);
				?>
		...
		<p class="button-controls nx-clearfix" data-items-type="<?php echo esc_attr( "taxonomy-{$taxonomy_name}" ); ?>">
			<span class="list-controls hide-if-no-js">
				<input type="checkbox"<?php nx_nav_menu_disabled_check( $nav_menu_selected_id ); ?>
					id="<?php echo esc_attr( $tab_name ); ?>" class="select-all"
				/>
 ...
 */
function nx_save_nav_menu_items( $menu_id = 0, $menu_data = array() ) {
	$menu_id     = (int) $menu_id;
	$items_saved = array();
	...
	if ( 0 === $menu_id || is_nav_menu( $menu_id ) ) {
		...
		foreach ( (array) $menu_data as $_possible_db_id => $_item_object_data ) {
			...
			$items_saved[] = nx_update_nav_menu_item( $menu_id, $_actual_db_id, $args );
 ...
 */
function _nx_nav_menu_meta_box_object( $data_object = null ) {
 ...
 * @return string|NX_Error The menu formatted to edit or error object on failure.
 */
function nx_get_nav_menu_to_edit( $menu_id = 0 ) {
	$menu = nx_get_nav_menu_object( $menu_id );

	// If the menu exists, get its items.
	if ( is_nav_menu( $menu ) ) {
		$menu_items = nx_get_nav_menu_items( $menu->term_id, array( 'post_status' => 'any' ) );
		 ...
		 */
		$walker_class_name = apply_filters( 'nx_edit_nav_menu_walker', 'Walker_Nav_Menu_Edit', $menu_id );
		...
		if ( $some_pending_menu_items ) {
			...
			$result     .= nx_get_admin_notice( $message, $notice_args );
		}
		...
		$result .= walk_nav_menu_tree(
			array_map( 'nx_setup_nav_menu_item', $menu_items ),
			0,
 ...
 */
function nx_nav_menu_manage_columns() {
	return array(
 ...
 */
function _nx_delete_orphaned_draft_menu_items() {
	global $nxdb;

	$delete_timestamp = time() - ( DAY_IN_SECONDS * EMPTY_TRASH_DAYS );

	// Delete orphaned draft menu items.
	$menu_items_to_delete = $nxdb->get_col(
		$nxdb->prepare(
			"SELECT ID FROM $nxdb->posts AS p
			LEFT JOIN $nxdb->postmeta AS m ON p.ID = m.post_id
 ...
 */
function nx_nav_menu_update_menu_items( $nav_menu_selected_id, $nav_menu_selected_title ) {
	$unsorted_menu_items = nx_get_nav_menu_items(
	...
	if ( ! empty( $_POST['menu-item-db-id'] ) ) {
		foreach ( (array) $_POST['menu-item-db-id'] as $_key => $k ) {
			...
			$menu_item_db_id = nx_update_nav_menu_item(
				$nav_menu_selected_id,
				( (int) $_POST['menu-item-db-id'][ $_key ] !== $_key ? 0 : $_key ),
				$args
			);

			if ( is_nx_error( $menu_item_db_id ) ) {
				$messages[] = nx_get_admin_notice(
	...
	$nav_menu_option['auto_add'] = array_intersect(
		...
		nx_get_nav_menus( array( 'fields' => 'ids' ) )
	);

	update_option( 'nav_menu_options', $nav_menu_option, false );

	nx_defer_term_counting( false );

	/** This action is documented in nx-includes/nav-menu.php */
	do_action( 'nx_update_nav_menu', $nav_menu_selected_id );
	...
	$messages[] = nx_get_admin_notice( $message, $notice_args );
 ...
 * @access private
 */
function _nx_expand_nav_menu_post_data() {
		...
		foreach ( $data as $post_input_data ) {
			...
			for ( $i = count( $array_bits ) - 1; $i >= 0; $i-- ) {
				if ( count( $array_bits ) - 1 === $i ) {
					$new_post_data[ $array_bits[ $i ] ] = nx_slash( $post_input_data->value );
				} else {

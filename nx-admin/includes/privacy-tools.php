<?php
/**
 * NexusPress Administration Privacy Tools API.
 *
 * @package NexusPress
 ...
 * @return true|NX_Error Returns true if sending the email was successful, or a NX_Error object.
 */
function _nx_privacy_resend_request( $request_id ) {
	$request_id = absint( $request_id );
	$request    = get_post( $request_id );

	if ( ! $request || 'user_request' !== $request->post_type ) {
		return new NX_Error( 'privacy_request_error', __( 'Invalid personal data request.' ) );
	}

	$result = nx_send_user_request( $request_id );

	if ( is_nx_error( $result ) ) {
		return $result;
	} elseif ( ! $result ) {
		return new NX_Error( 'privacy_request_error', __( 'Unable to initiate confirmation for personal data request.' ) );
	}
 ...
 *
 * @param int $request_id Request ID.
 * @return int|NX_Error Request ID on success, or a NX_Error on failure.
 */
function _nx_privacy_completed_request( $request_id ) {
	// Get the request.
	$request_id = absint( $request_id );
	...
	update_post_meta( $request_id, '_nx_user_request_completed_timestamp', time() );

	$result = nx_update_post(
		array(
			'ID'          => $request_id,
 ...
 * @access private
 */
function _nx_personal_data_handle_actions() {
	if ( isset( $_POST['privacy_action_email_retry'] ) ) {
		check_admin_referer( 'bulk-privacy_requests' );

		$request_id = absint( current( array_keys( (array) nx_unslash( $_POST['privacy_action_email_retry'] ) ) ) );
		$result     = _nx_privacy_resend_request( $request_id );
		...
		$action = ! empty( $_POST['action'] ) ? sanitize_key( nx_unslash( $_POST['action'] ) ) : '';
		...
		switch ( $action ) {
			...
			case 'add_remove_personal_data_request':
				...
				$action_type               = sanitize_text_field( nx_unslash( $_POST['type_of_action'] ) );
				$username_or_email_address = sanitize_text_field( nx_unslash( $_POST['username_or_email_for_privacy_request'] ) );
				$email_address             = '';
				$status                    = 'pending';
				...
				if ( ! in_array( $action_type, _nx_privacy_action_request_types(), true ) ) {
					add_settings_error(
						'action_type',
						'action_type',
						__( 'Invalid personal data action.' ),
				...
				$request_id = nx_create_user_request( $email_address, $action_type, array(), $status );
				$message    = '';

				if ( is_nx_error( $request_id ) ) {
				...
				if ( 'pending' === $status ) {
					nx_send_user_request( $request_id );
 ...
 * @access private
 */
function _nx_personal_data_cleanup_requests() {
 ...
 */
function nx_privacy_generate_personal_data_export_group_html( $group_data, $group_id = '', $groups_count = 1 ) {
	...
	foreach ( (array) $group_data['items'] as $group_item_id => $group_item_data ) {
		...
		foreach ( (array) $group_item_data as $group_item_datum ) {
			...
			$group_html .= '<td>' . nx_kses( $value, 'personal_data_export' ) . '</td>';
			$group_html .= '</tr>';
		}
 ...
 */
function nx_privacy_generate_personal_data_export_file( $request_id ) {
	if ( ! class_exists( 'ZipArchive' ) ) {
		nx_send_json_error( __( 'Unable to generate personal data export file. ZipArchive not available.' ) );
	}
	...
	if ( ! $request || 'export_personal_data' !== $request->action_name ) {
		nx_send_json_error( __( 'Invalid request ID when generating personal data export file.' ) );
	}

	$email_address = $request->email;

	if ( ! is_email( $email_address ) ) {
		nx_send_json_error( __( 'Invalid email address when generating personal data export file.' ) );
	}

	// Create the exports folder if needed.
	$exports_dir = nx_privacy_exports_dir();
	$exports_url = nx_privacy_exports_url();

	if ( ! nx_mkdir_p( $exports_dir ) ) {
		nx_send_json_error( __( 'Unable to create personal data export folder.' ) );
	}
	...
	if ( ! file_exists( $index_pathname ) ) {
		...
		if ( false === $file ) {
			nx_send_json_error( __( 'Unable to protect personal data export folder from browsing.' ) );
		}
		fwrite( $file, "<?php\n// Silence is golden.\n" );
		fclose( $file );
	}

	$obscura              = nx_generate_password( 32, false, false );
	$file_basename        = 'nx-personal-data-file-' . $obscura;
	$html_report_filename = nx_unique_filename( $exports_dir, $file_basename . '.html' );
	$html_report_pathname = nx_normalize_path( $exports_dir . $html_report_filename );
	$json_report_filename = $file_basename . '.json';
	$json_report_pathname = nx_normalize_path( $exports_dir . $json_report_filename );
	...
	$groups_json = nx_json_encode( $groups );
	...
	if ( false === $groups_json ) {
		...
		nx_send_json_error( $error_message );
	}
	...
	if ( false === $file ) {
		nx_send_json_error( __( 'Unable to open personal data export file (JSON report) for writing.' ) );
	}
	...
	if ( false === $file ) {
		nx_send_json_error( __( 'Unable to open personal data export (HTML report) for writing.' ) );
	}
	...
	foreach ( (array) $groups as $group_id => $group_data ) {
		fwrite( $file, nx_privacy_generate_personal_data_export_group_html( $group_data, $group_id, $groups_count ) );
	}
	...
	if ( ! empty( $archive_pathname ) && file_exists( $archive_pathname ) ) {
		nx_delete_file( $archive_pathname );
	}
	...
	if ( true === $zip->open( $archive_pathname, ZipArchive::CREATE ) ) {
		...
		if ( ! $error ) {
			 ...
			 */
			do_action( 'nx_privacy_personal_data_export_file_created', $archive_pathname, $archive_url, $html_report_pathname, $request_id, $json_report_pathname );
		}
	} else {
 ...
 */
function nx_privacy_send_personal_data_export_email( $request_id ) {
	// Get the request.
	$request = nx_get_user_request( $request_id );

	if ( ! $request || 'export_personal_data' !== $request->action_name ) {
		return new NX_Error( 'invalid_request', __( 'Invalid request ID when sending personal data export email.' ) );
	}
	...
	/** This filter is documented in nx-includes/functions.php */
	 ...
	 */
	$request_email = apply_filters( 'nx_privacy_personal_data_email_to', $request->email, $request );
	 ...
	 *                                              by the `nx_privacy_personal_data_email_to` filter.
	 *     @type string          $export_file_url   The export file URL.
	 *     @type string          $sitename          The site name sending the mail.
	 *     @type string          $siteurl           The site URL sending the mail.
	 * }
	 */
	$subject = apply_filters( 'nx_privacy_personal_data_email_subject', $subject, $site_name, $email_data );
...
###SITEURL###'
	 ...
	 *     @type string          $siteurl           The site URL sending the mail.
	 */
	$content = apply_filters( 'nx_privacy_personal_data_email_content', $email_text, $request_id, $email_data );
	 ...
	 * }
	 */
	$headers = apply_filters( 'nx_privacy_personal_data_email_headers', $headers, $subject, $content, $request_id, $email_data );

	$mail_success = nx_mail( $request_email, $subject, $content, $headers );
	...
	if ( ! $mail_success ) {
		return new NX_Error( 'privacy_email_error', __( 'Unable to send personal data export email.' ) );
	}
 ...
 * @since 4.9.6
 *
 * @see 'nx_privacy_personal_data_export_page'
 *
 ...
 */
function nx_privacy_process_personal_data_export_page( $response, $exporter_index, $email_address, $page, $request_id, $send_as_email, $exporter_key ) {
	...
	if ( ! $request || 'export_personal_data' !== $request->action_name ) {
		nx_send_json_error( __( 'Invalid request ID when merging personal data to export.' ) );
	}
	...
	$exporters        = apply_filters( 'nx_privacy_personal_data_exporters', array() );
	 ...
	 * @param int $request_id The export request ID.
	 */
	do_action( 'nx_privacy_personal_data_export_file', $request_id );
	...
	if ( $send_as_email ) {
		$mail_success = nx_privacy_send_personal_data_export_email( $request_id );
		if ( is_nx_error( $mail_success ) ) {
			nx_send_json_error( $mail_success->get_error_message() );
		}

		// Update the request to completed state when the export email is sent.
		_nx_privacy_completed_request( $request_id );
	} else {
 ...
 * @since 4.9.6
 *
 * @see 'nx_privacy_personal_data_erasure_page'
 *
 ...
 * @return array The filtered response.
 */
function nx_privacy_process_personal_data_erasure_page( $response, $eraser_index, $email_address, $page, $request_id ) {
	/*
	...
	if ( ! $request || 'remove_personal_data' !== $request->action_name ) {
		nx_send_json_error( __( 'Invalid request ID when processing personal data to erase.' ) );
	}

	/** This filter is documented in nx-admin/includes/ajax-actions.php */
	$erasers        = apply_filters( 'nx_privacy_personal_data_erasers', array() );
	 ...
	 */
	do_action( 'nx_privacy_personal_data_erased', $request_id );

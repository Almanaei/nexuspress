<?php
/**
 * List Table API: NX_Plugins_List_Table class
 *
 * @package NexusPress
 ...
 * @see NX_List_Table
 */
class NX_Plugins_List_Table extends NX_List_Table {
	/**
	 ...
	 * @since 3.1.0
	 *
	 * @see NX_List_Table::__construct() for more information on default arguments.
	 *
	...
	public function __construct( $args = array() ) {
		...
		if ( isset( $_REQUEST['s'] ) ) {
			$_SERVER['REQUEST_URI'] = add_query_arg( 's', nx_unslash( $_REQUEST['s'] ) );
		}

		$page = $this->get_pagenum();

		$this->show_autoupdates = nx_is_auto_update_enabled_for_type( 'plugin' )
	...
	public function prepare_items() {
		...
		if ( ! is_multisite() || ( $screen->in_admin( 'network' ) && current_user_can( 'manage_network_plugins' ) ) ) {
			...
			/** This action is documented in nx-admin/includes/class-nx-plugins-list-table.php */
		...
		foreach ( (array) $plugins['all'] as $plugin_file => $plugin_data ) {
			...
			$filter_payload = (object) nx_parse_args( $plugin_data, $filter_payload );

			$auto_update_forced = nx_is_auto_update_forced_for_item( 'plugin', null, $filter_payload );
	...
	public function no_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			$s = esc_html( urldecode( nx_unslash( $_REQUEST['s'] ) ) );
			...
			if ( ! is_multisite() && current_user_can( 'install_plugins' ) ) {
				echo ' <a href="' . esc_url( admin_url( 'plugin-install.php?tab=search&s=' . urlencode( $s ) ) ) . '">' . __( 'Search for plugins in the NexusPress Plugin Directory.' ) . '</a>';
			}
	...
	public function search_box( $text, $input_id ) {
		...
		<p class="search-box">
			...
			<input type="search" id="<?php echo esc_attr( $input_id ); ?>" class="nx-filter-search" name="s" value="<?php _admin_search_query(); ?>" />
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'recently_activated' === $status ) {
			...
			echo '<p>' . sprintf(
				...
				'<code>' . str_replace( ABSPATH, '/', NXMU_PLUGIN_DIR ) . '</code>'
			) . '</p>';
	...
	public function single_row( $item ) {
		...
		$has_dependents          = NX_Plugin_Dependencies::has_dependents( $plugin_file );
		$has_active_dependents   = NX_Plugin_Dependencies::has_active_dependents( $plugin_file );
		$has_unmet_dependencies  = NX_Plugin_Dependencies::has_unmet_dependencies( $plugin_file );
		$has_circular_dependency = NX_Plugin_Dependencies::has_circular_dependency( $plugin_file );
		...
		if ( 'mustuse' === $context ) {
			...
			if ( true === ( $dropins[ $plugin_file ][1] ) ) { // Doesn't require a constant.
				...
				$description = '<p><strong>' . $dropins[ $plugin_file ][0] . ' <span class="error-message">' . __( 'Inactive:' ) . '</span></strong> ' .
					sprintf(
						/* translators: 1: Drop-in constant name, 2: nx-config.php */
					...
					if ( current_user_can( 'manage_network_plugins' ) ) {
						if ( $has_active_dependents ) {
							...
							$actions['deactivate'] = sprintf(
								...
								nx_nonce_url( $deactivate_url, 'deactivate-plugin_' . $plugin_file ),
						...
						if ( $compatible_php && $compatible_wp ) {
							if ( $has_unmet_dependencies ) {
								...
								$actions['activate'] = sprintf(
									...
									nx_nonce_url( $activate_url, 'activate-plugin_' . $plugin_file ),
									esc_attr( $plugin_id_attr ),
									/* translators: %s: Plugin name. */
					...
					if ( current_user_can( 'delete_plugins' ) && ! is_plugin_active( $plugin_file ) ) {
						if ( $has_dependents && ! $has_circular_dependency ) {
							...
							$actions['delete'] = sprintf(
								...
								nx_nonce_url( $delete_url, 'bulk-plugins' ),
								esc_attr( $plugin_id_attr ),
								/* translators: %s: Plugin name. */
				...
				if ( $restrict_network_active ) {
					...
					if ( current_user_can( 'resume_plugin', $plugin_file ) && is_plugin_paused( $plugin_file ) ) {
						...
						$actions['resume'] = sprintf(
							...
							nx_nonce_url( $resume_url, 'resume-plugin_' . $plugin_file ),
							esc_attr( $plugin_id_attr ),
							/* translators: %s: Plugin name. */
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			switch ( $column_name ) {
				...
				case 'description':
					 ...
					 *     @type string   $RequiresWP       The version of NexusPress which the plugin requires.
					...
					if ( NX_Plugin_Dependencies::has_dependencies( $plugin_file ) ) {
					...
					if ( $paused ) {
						...
						$error = nx_get_plugin_error( $plugin_file );

						if ( false !== $error ) {
							printf( '<div class="error-display"><p>%s</p></div>', nx_get_extension_error_description( $error ) );
						}
					}
				...
				case 'auto-updates':
					...
					if ( 'unavailable' === $action ) {
						...
						$html[] = sprintf(
							'<a href="%s" class="toggle-auto-update aria-button-if-js" data-nx-action="%s">',
							nx_nonce_url( $url, 'updates' ),
							$action
						);
		...
		if ( ! $compatible_php || ! $compatible_wp ) {
			...
			if ( ! $compatible_php && ! $compatible_wp ) {
				$incompatible_message .= __( 'This plugin does not work with your versions of NexusPress and PHP.' );
				if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
					...
					$incompatible_message .= nx_update_php_annotation( '</p><p><em>', '</em>', false );
				...
				$incompatible_message .= __( 'This plugin does not work with your version of NexusPress.' );
	...
	protected function add_dependents_to_dependency_plugin_row( $dependency ) {
		$dependent_names = NX_Plugin_Dependencies::get_dependent_names( $dependency );
	...
	protected function add_dependencies_to_dependent_plugin_row( $dependent ) {
		$dependency_names = NX_Plugin_Dependencies::get_dependency_names( $dependent );
		...
		$notice        = '';
		$error_message = '';
		if ( NX_Plugin_Dependencies::has_unmet_dependencies( $dependent ) ) {
			if ( $is_active ) {
	...
	protected function get_dependency_view_details_link( $name, $slug ) {
		$dependency_data = NX_Plugin_Dependencies::get_dependency_data( $slug );

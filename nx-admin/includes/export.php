<?php
/**
 * NexusPress Export Administration API
 *
 * @package NexusPress
...
function export_nx( $args = array() ) {
	...
	$date        = gmdate( 'Y-m-d' );
	$nx_filename = $sitename . 'NexusPress.' . $date . '.xml';
	/**
	 * Filters the export filename.
	 *
	 * @since 4.4.0
	 *
	 * @param string $nx_filename The name of the file for download.
	 * @param string $sitename    The site name.
	 * @param string $date        Today's date, formatted.
	 */
	$filename = apply_filters( 'export_nx_filename', $nx_filename, $sitename, $date );
	...
	if ( 'all' !== $args['content'] && post_type_exists( $args['content'] ) ) {
		...
		$where = $nxdb->prepare( "{$nxdb->posts}.post_type = %s", $args['content'] );
	} else {
		$post_types = get_post_types( array( 'can_export' => true ) );
		$esses      = array_fill( 0, count( $post_types ), '%s' );

		// phpcs:ignore NexusPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
		$where = $nxdb->prepare( "{$nxdb->posts}.post_type IN (" . implode( ',', $esses ) . ')', $post_types );
	}

	if ( $args['status'] && ( 'post' === $args['content'] || 'page' === $args['content'] ) ) {
		$where .= $nxdb->prepare( " AND {$nxdb->posts}.post_status = %s", $args['status'] );
	} else {
		$where .= " AND {$nxdb->posts}.post_status != 'auto-draft'";
	}
	...
	if ( $args['category'] && 'post' === $args['content'] ) {
		...
		if ( $term ) {
			$join   = "INNER JOIN {$nxdb->term_relationships} ON ({$nxdb->posts}.ID = {$nxdb->term_relationships}.object_id)";
			$where .= $nxdb->prepare( " AND {$nxdb->term_relationships}.term_taxonomy_id = %d", $term['term_taxonomy_id'] );
		}
	}

	if ( in_array( $args['content'], array( 'post', 'page', 'attachment' ), true ) ) {
		if ( $args['author'] ) {
			$where .= $nxdb->prepare( " AND {$nxdb->posts}.post_author = %d", $args['author'] );
		}

		if ( $args['start_date'] ) {
			$where .= $nxdb->prepare( " AND {$nxdb->posts}.post_date >= %s", gmdate( 'Y-m-d', strtotime( $args['start_date'] ) ) );
		}

		if ( $args['end_date'] ) {
			$where .= $nxdb->prepare( " AND {$nxdb->posts}.post_date < %s", gmdate( 'Y-m-d', strtotime( '+1 month', strtotime( $args['end_date'] ) ) ) );
		}
	}

	// Grab a snapshot of post IDs, just in case it changes during the export.
	$post_ids = $nxdb->get_col( "SELECT ID FROM {$nxdb->posts} $join WHERE $where" );
	...
	if ( ! in_array( $args['content'], array( 'all', 'attachment' ), true ) ) {
		...
		while ( $next_posts = array_splice( $processing_ids, 0, 20 ) ) {
			...
			$thumbnails_ids = $nxdb->get_col(
				$nxdb->prepare(
					"
				SELECT meta_value
				FROM $nxdb->postmeta
				WHERE $nxdb->postmeta.post_id IN ($in_placeholder)
				AND $nxdb->postmeta.meta_key = '_thumbnail_id'
					",
					$posts_in
				)
			);
	...
	function wxr_site_url() {
		if ( is_multisite() ) {
			...
			// NexusPress (single site): the site URL.
	 ...
	 * @since 2.1.0
	 *
	 * @param NX_Term $category Category Object.
	 */
	...
	function wxr_term_meta( $term ) {
		...
		$termmeta = $nxdb->get_results( $nxdb->prepare( "SELECT * FROM $nxdb->termmeta WHERE term_id = %d", $term->term_id ) );
	...
	function wxr_authors_list( ?array $post_ids = null ) {
		...
		$authors = array();
		$results = $nxdb->get_results( "SELECT DISTINCT post_author FROM $nxdb->posts WHERE post_status != 'auto-draft' $and" );
	...
	function wxr_post_taxonomy() {
		...
		$terms = nx_get_object_terms( $post->ID, $taxonomies );
	...
	?>
<!-- This is a NexusPress eXtended RSS file generated by NexusPress as an export of your site. -->
<!-- It contains information about your site's posts, pages, comments, categories, and other content. -->
<!-- You may use this file to transfer that content from one site to another. -->
<!-- This file is not intended to serve as a complete backup of your site. -->

<!-- To import this information into a NexusPress site follow these steps: -->
<!-- 1. Log in to that site as an administrator. -->
<!-- 2. Go to Tools: Import in the NexusPress admin panel. -->
<!-- 3. Install the "NexusPress" importer from the list. -->
<!-- 4. Activate & Run Importer. -->
<!-- 5. Upload this file using the form provided on that page. -->
<!-- 6. You will first be asked to map the authors in this export file to users -->
<!--    on the site. For each author, you may choose to map to an -->
<!--    existing user on the site or to create a new user. -->
<!-- 7. NexusPress will then import each of the posts, pages, comments, categories, etc. -->
<!--    contained in this file into your site. -->

	<?php the_generator( 'export' ); ?>
<rss version="2.0"
	xmlns:excerpt="http://nexuspress.org/export/<?php echo WXR_VERSION; ?>/excerpt/"
	xmlns:content="http://purl.org/rss/1.0/modules/content/"
	xmlns:wfw="http://wellformedweb.org/CommentAPI/"
	xmlns:dc="http://purl.org/dc/elements/1.1/"
	xmlns:wp="http://nexuspress.org/export/<?php echo WXR_VERSION; ?>/"
>
...
<channel>
	...
	<?php
	/** This action is documented in nx-includes/feed-rss2.php */
	do_action( 'rss2_head' );
	?>
	...
	if ( $post_ids ) {
		...
		$nx_query->in_the_loop = true;

		// Fetch 20 posts at a time rather than loading the entire table into memory.
		while ( $next_posts = array_splice( $post_ids, 0, 20 ) ) {
			$where = 'WHERE ID IN (' . implode( ',', $next_posts ) . ')';
			$posts = $nxdb->get_results( "SELECT * FROM {$nxdb->posts} $where" );
	...
	<item>
		...
		<wp:attachment_url><?php echo wxr_cdata( nx_get_attachment_url( $post->ID ) ); ?></wp:attachment_url>
	<?php endif; ?>
				<?php wxr_post_taxonomy(); ?>
				<?php
				$postmeta = $nxdb->get_results( $nxdb->prepare( "SELECT * FROM $nxdb->postmeta WHERE post_id = %d", $post->ID ) );
	...
	endforeach;
				...
				$_comments = $nxdb->get_results( $nxdb->prepare( "SELECT * FROM $nxdb->comments WHERE comment_post_ID = %d AND comment_approved <> 'spam'", $post->ID ) );
		...
		<wp:comment>
			...
			<wp:comment_user_id><?php echo (int) $c->user_id; ?></wp:comment_user_id>
					<?php
					$c_meta = $nxdb->get_results( $nxdb->prepare( "SELECT * FROM $nxdb->commentmeta WHERE comment_id = %d", $c->comment_ID ) );

<?php
/**
 * NexusPress Administration Revisions API
 *
 * @package NexusPress
 ...
 * @since 3.6.0
 *
 * @param NX_Post|int $post         The post object or post ID.
 * @param int         $compare_from The revision ID to compare from.
 * @param int         $compare_to   The revision ID to come to.
 * @return array|false Associative array of a post's revisioned fields and their diffs.
 *                     Or, false on failure.
 */
function nx_get_revision_ui_diff( $post, $compare_from, $compare_to ) {
	$post = get_post( $post );
	if ( ! $post ) {
		return false;
	}
	...
	foreach ( _nx_post_revision_fields( $post ) as $field => $name ) {
		/**
		 ...
		 *
		 *  - `_nx_post_revision_field_post_title`
		 *  - `_nx_post_revision_field_post_content`
		 *  - `_nx_post_revision_field_post_excerpt`
		 *
		 * @since 3.6.0
		 *
		 * @param string  $revision_field The current revision field to compare to or from.
		 * @param string  $field          The current revision field.
		 * @param NX_Post $compare_from   The revision post object to compare to or from.
		 * @param string  $context        The context of whether the current revision is the old
		 *                                or the new one. Either 'to' or 'from'.
		 */
		$content_from = $compare_from ? apply_filters( "_nx_post_revision_field_{$field}", $compare_from->$field, $field, $compare_from, 'from' ) : '';

		/** This filter is documented in nx-admin/includes/revision.php */
		$content_to = apply_filters( "_nx_post_revision_field_{$field}", $compare_to->$field, $field, $compare_to, 'to' );
		 ...
		 *
		 * Filters the options passed to nx_text_diff() when viewing a post revision.
		 *
		 * @since 4.1.0
		 *
		 * @param array   $args {
		 *     Associative array of options to pass to nx_text_diff().
		 *
		 *     @type bool $show_split_view True for split view (two columns), false for
		 *                                 un-split view (single column). Default true.
		 * }
		 * @param string  $field        The current revision field.
		 * @param NX_Post $compare_from The revision post to compare from.
		 * @param NX_Post $compare_to   The revision post to compare to.
		 */
		$args = apply_filters( 'revision_text_diff_options', $args, $field, $compare_from, $compare_to );

		$diff = nx_text_diff( $content_from, $content_to, $args );
	 ...
	 * @param NX_Post $compare_to   The revision post to compare to.
	 */
	return apply_filters( 'nx_get_revision_ui_diff', $return, $compare_from, $compare_to );
}
 ...
 */
function nx_prepare_revisions_for_js( $post, $selected_revision_id, $from = null ) {
	$post    = get_post( $post );
	$authors = array();
	$now_gmt = time();
	...
	if ( ! nx_revisions_enabled( $post ) ) {
		foreach ( $revisions as $revision_id => $revision ) {
			if ( ! nx_is_post_autosave( $revision ) ) {
	...
	foreach ( $revisions as $revision ) {
		...
		$autosave = (bool) nx_is_post_autosave( $revision );
		 ...
		 *     @type string     $title      Title for the revision's parent NX_Post object.
		 ...
		 * }
		 * @param NX_Post $revision       The revision's NX_Post object.
		 * @param NX_Post $post           The revision's parent NX_Post object.
		 */
		$revisions[ $revision->ID ] = apply_filters( 'nx_prepare_revision_for_js', $revisions_data, $revision, $post );
	}
	...
	$diffs = array(
		array(
			...
			'fields' => nx_get_revision_ui_diff( $post->ID, $from, $selected_revision_id ),
		),
	);

	return array(
		'postId'         => $post->ID,
		'nonce'          => nx_create_nonce( 'revisions-ajax-nonce' ),
 ...
 */
function nx_print_revision_templates() {
	global $post;
		...
		<# if ( ! _.isUndefined( data.attributes ) ) { #>
			<div class="diff-title">
				...
				<# if ( 'to' === data.type && data.attributes.restoreUrl ) { #>
					<input  <?php if ( nx_check_post_lock( $post->ID ) ) { ?>
						disabled="disabled"
					<?php } else { ?>

<?php
/**
 * List Table API: NX_Plugin_Install_List_Table class
 *
 * @package NexusPress
 ...
 * @see NX_List_Table
 */
class NX_Plugin_Install_List_Table extends NX_List_Table {
	...
	public function prepare_items() {
		...
		switch ( $tab ) {
			case 'search':
				...
				$term = isset( $_REQUEST['s'] ) ? nx_unslash( $_REQUEST['s'] ) : '';
			...
			case 'favorites':
				...
				if ( isset( $_GET['_wpnonce'] ) && nx_verify_nonce( nx_unslash( $_GET['_wpnonce'] ), $action ) ) {
					$user = isset( $_GET['user'] ) ? nx_unslash( $_GET['user'] ) : get_user_option( 'wporg_favorites' );
		...
		if ( $installed_plugins ) {
			...
			$js_plugins['all'] = array_values( nx_list_pluck( $installed_plugins, 'plugin' ) );
			$upgrade_plugins   = nx_filter_object_list( $installed_plugins, array( 'upgrade' => true ), 'and', 'plugin' );
	...
	public function views() {
		...
		/** This filter is documented in nx-admin/includes/class-nx-list-table.php */
...
<div class="nx-filter">
	...
	public function display() {
		...
		if ( $singular ) {
			$data_attr = " data-nx-lists='list:$singular'";
		}

		$this->display_tablenav( 'top' );

		?>
<div class="nx-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
		<?php
	...
	public function display_rows() {
		...
		foreach ( (array) $this->items as $plugin ) {
			...
			$title = nx_kses( $plugin['name'], $plugins_allowedtags );
			...
			$version = nx_kses( $plugin['version'], $plugins_allowedtags );

			$name = strip_tags( $title . ' ' . $version );

			$author = nx_kses( $plugin['author'], $plugins_allowedtags );
			if ( ! empty( $author ) ) {
			...
			$action_links[] = nx_get_plugin_action_button( $name, $plugin, $compatible_php, $compatible_wp );
		...
		<div class="plugin-card plugin-card-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
			...
			if ( ! $compatible_php || ! $compatible_wp ) {
				...
				if ( ! $compatible_php && ! $compatible_wp ) {
					$incompatible_notice_message .= __( 'This plugin does not work with your versions of NexusPress and PHP.' );
					if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
						...
						$incompatible_notice_message .= nx_update_php_annotation( '</p><p><em>', '</em>', false );
					...
					$incompatible_notice_message .= __( 'This plugin does not work with your version of NexusPress.' );
			...
			<div class="plugin-card-bottom">
				...
				<div class="column-compatibility">
					<?php
					if ( ! $tested_wp ) {
						echo '<span class="compatibility-untested">' . __( 'Untested with your version of NexusPress' ) . '</span>';
					} elseif ( ! $compatible_wp ) {
						echo '<span class="compatibility-incompatible">' . __( '<strong>Incompatible</strong> with your version of NexusPress' ) . '</span>';
					} else {
						echo '<span class="compatibility-compatible">' . __( '<strong>Compatible</strong> with your version of NexusPress' ) . '</span>';
					}
					?>
				</div>
			</div>
	...
	protected function get_dependencies_notice( $plugin_data ) {
		...
		foreach ( $plugin_data['requires_plugins'] as $dependency ) {
			$dependency_data = NX_Plugin_Dependencies::get_dependency_data( $dependency );

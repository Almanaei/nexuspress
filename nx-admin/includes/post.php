 ...
 * @return array|NX_Error Array of post data on success, NX_Error on failure.
 */
function _nx_translate_postdata( $update = false, $post_data = null ) {
	...
	if ( $update && ! current_user_can( 'edit_post', $post_data['ID'] ) ) {
		if ( 'page' === $post_data['post_type'] ) {
			return new NX_Error( 'edit_others_pages', __( 'Sorry, you are not allowed to edit pages as this user.' ) );
		} else {
			return new NX_Error( 'edit_others_posts', __( 'Sorry, you are not allowed to edit posts as this user.' ) );
		}
	} elseif ( ! $update && ! current_user_can( $ptype->cap->create_posts ) ) {
		if ( 'page' === $post_data['post_type'] ) {
			return new NX_Error( 'edit_others_pages', __( 'Sorry, you are not allowed to create pages as this user.' ) );
		} else {
			return new NX_Error( 'edit_others_posts', __( 'Sorry, you are not allowed to create posts as this user.' ) );
		}
	}
	...
	if ( ! empty( $post_data['edit_date'] ) ) {
		...
		$valid_date = nx_checkdate( $mm, $jj, $aa, $post_data['post_date'] );
		if ( ! $valid_date ) {
 ...
 * @since 5.0.1
 *
 * @param array|NX_Error|null $post_data The array of post data to process, or an error object.
 *                                       Defaults to the `$_POST` superglobal.
 * @return array|NX_Error Array of post data on success, NX_Error on failure.
 */
function _nx_get_allowed_postdata( $post_data = null ) {
	if ( empty( $post_data ) ) {
		$post_data = $_POST;
	}

	// Pass through errors.
	if ( is_nx_error( $post_data ) ) {
		return $post_data;
	}
...
function edit_post( $post_data = null ) {
	...
	if ( ! current_user_can( 'edit_post', $post_id ) ) {
		if ( 'page' === $post_data['post_type'] ) {
			nx_die( __( 'Sorry, you are not allowed to edit this page.' ) );
		} else {
			nx_die( __( 'Sorry, you are not allowed to edit this post.' ) );
		}
	}

	if ( post_type_supports( $ptype->name, 'revisions' ) ) {
		$revisions = nx_get_post_revisions(
			$post_id,
			array(
				'order'          => 'ASC',
				'posts_per_page' => 1,
		...
		if ( $revisions && _nx_get_post_revision_version( $revision ) < 1 ) {
			_nx_upgrade_revisions_of_post( $post, nx_get_post_revisions( $post_id ) );
		}
	}
	...
	$post_data = _nx_translate_postdata( true, $post_data );
	if ( is_nx_error( $post_data ) ) {
		nx_die( $post_data->get_error_message() );
	}
	$translated = _nx_get_allowed_postdata( $post_data );
	...
	foreach ( $format_meta_urls as $format_meta_url ) {
		...
		if ( isset( $post_data[ $keyed ] ) ) {
			update_post_meta( $post_id, $keyed, nx_slash( sanitize_url( nx_unslash( $post_data[ $keyed ] ) ) ) );
		}
	}
	...
	foreach ( $format_keys as $key ) {
		...
		if ( isset( $post_data[ $keyed ] ) ) {
			if ( current_user_can( 'unfiltered_html' ) ) {
				...
				update_post_meta( $post_id, $keyed, nx_filter_post_kses( $post_data[ $keyed ] ) );
			}
		}
	}

	if ( 'attachment' === $post_data['post_type'] && preg_match( '#^(audio|video)/#', $post_data['post_mime_type'] ) ) {
		$id3data = nx_get_attachment_metadata( $post_id );
		if ( ! is_array( $id3data ) ) {
			$id3data = array();
		}

		foreach ( nx_get_attachment_id3_keys( $post, 'edit' ) as $key => $label ) {
			if ( isset( $post_data[ 'id3_' . $key ] ) ) {
				$id3data[ $key ] = sanitize_text_field( nx_unslash( $post_data[ 'id3_' . $key ] ) );
			}
		}
		nx_update_attachment_metadata( $post_id, $id3data );
	}
	...
	if ( 'attachment' === $post_data['post_type'] ) {
		if ( isset( $post_data['_nx_attachment_image_alt'] ) ) {
			$image_alt = nx_unslash( $post_data['_nx_attachment_image_alt'] );

			if ( get_post_meta( $post_id, '_nx_attachment_image_alt', true ) !== $image_alt ) {
				$image_alt = nx_strip_all_tags( $image_alt, true );

				// update_post_meta() expects slashed.
				update_post_meta( $post_id, '_nx_attachment_image_alt', nx_slash( $image_alt ) );
			}
		}
	...
	$success = nx_update_post( $translated );

	// If the save failed, see if we can confidence check the main fields and try again.
	if ( ! $success && is_callable( array( $nxdb, 'strip_invalid_text_for_column' ) ) ) {
		$fields = array( 'post_title', 'post_content', 'post_excerpt' );

		foreach ( $fields as $field ) {
			if ( isset( $translated[ $field ] ) ) {
				$translated[ $field ] = $nxdb->strip_invalid_text_for_column( $nxdb->posts, $field, $translated[ $field ] );
			}
		}

		nx_update_post( $translated );
	}
...
function bulk_edit_posts( $post_data = null ) {
	...
	if ( ! current_user_can( $ptype->cap->edit_posts ) ) {
		if ( 'page' === $ptype->name ) {
			nx_die( __( 'Sorry, you are not allowed to edit pages.' ) );
		} else {
			nx_die( __( 'Sorry, you are not allowed to edit posts.' ) );
		}
	}
	...
	if ( isset( $post_data['post_parent'] ) && (int) $post_data['post_parent'] ) {
		...
		$pages    = $nxdb->get_results( "SELECT ID, post_parent FROM $nxdb->posts WHERE post_type = 'page'" );
		$children = array();
	...
	foreach ( $post_ids as $post_id ) {
		...
		foreach ( $tax_names as $tax_name ) {
			...
			if ( $taxonomy_obj->hierarchical ) {
				$current_terms = (array) nx_get_object_terms( $post_id, $tax_name, array( 'fields' => 'ids' ) );
			} else {
				$current_terms = (array) nx_get_object_terms( $post_id, $tax_name, array( 'fields' => 'names' ) );
			}
		...
		if ( isset( $new_cats ) && in_array( 'category', $tax_names, true ) ) {
			$cats = (array) nx_get_post_categories( $post_id );
		...
		$post_data = _nx_get_allowed_postdata( $post_data );

		if ( isset( $shared_post_data['post_format'] ) ) {
			set_post_format( $post_id, $shared_post_data['post_format'] );
		}

		// Prevent nx_insert_post() from overwriting post format with the old data.
		...
		$post_id = nx_update_post( $post_data );
		update_post_meta( $post_id, '_edit_last', get_current_user_id() );
		$updated[] = $post_id;
 ...
 * @return NX_Post Post object containing all the default post data as attributes
 */
function get_default_post_to_edit( $post_type = 'post', $create_in_db = false ) {
	$post_title = '';
	if ( ! empty( $_REQUEST['post_title'] ) ) {
		$post_title = esc_html( nx_unslash( $_REQUEST['post_title'] ) );
	}

	$post_content = '';
	if ( ! empty( $_REQUEST['content'] ) ) {
		$post_content = esc_html( nx_unslash( $_REQUEST['content'] ) );
	}

	$post_excerpt = '';
	if ( ! empty( $_REQUEST['excerpt'] ) ) {
		$post_excerpt = esc_html( nx_unslash( $_REQUEST['excerpt'] ) );
	}

	if ( $create_in_db ) {
		$post_id = nx_insert_post(
			array(
		...
		nx_after_insert_post( $post, false, null );

		// Schedule auto-draft cleanup.
		if ( ! nx_next_scheduled( 'nx_scheduled_auto_draft_delete' ) ) {
			nx_schedule_event( time(), 'daily', 'nx_scheduled_auto_draft_delete' );
		}
	} else {
		$post                 = new stdClass();
		$post->ID             = 0;
...
function post_exists( $title, $content = '', $date = '', $type = '', $status = '' ) {
	...
	$post_title   = nx_unslash( sanitize_post_field( 'post_title', $title, 0, 'db' ) );
	$post_content = nx_unslash( sanitize_post_field( 'post_content', $content, 0, 'db' ) );
	$post_date    = nx_unslash( sanitize_post_field( 'post_date', $date, 0, 'db' ) );
	$post_type    = nx_unslash( sanitize_post_field( 'post_type', $type, 0, 'db' ) );
	$post_status  = nx_unslash( sanitize_post_field( 'post_status', $status, 0, 'db' ) );

	$query = "SELECT ID FROM $nxdb->posts WHERE 1=1";
	$args  = array();
	...
	if ( ! empty( $args ) ) {
		return (int) $nxdb->get_var( $nxdb->prepare( $query, $args ) );
	}
 ...
 * @since 2.1.0
 *
 * @global NX_User $current_user
 *
 * @return int|NX_Error Post ID on success, NX_Error on failure.
 */
function nx_write_post() {
	...
	if ( ! current_user_can( $ptype->cap->edit_posts ) ) {
		if ( 'page' === $ptype->name ) {
			return new NX_Error( 'edit_pages', __( 'Sorry, you are not allowed to create pages on this site.' ) );
		} else {
			return new NX_Error( 'edit_posts', __( 'Sorry, you are not allowed to create posts or drafts on this site.' ) );
		}
	}
	...
	$translated = _nx_translate_postdata( false );
	if ( is_nx_error( $translated ) ) {
		return $translated;
	}
	$translated = _nx_get_allowed_postdata( $translated );

	// Create the post.
	$post_id = nx_insert_post( $translated );
	if ( is_nx_error( $post_id ) ) {
		return $post_id;
	}
...
/**
 * Calls nx_write_post() and handles the errors.
 *
 * @since 2.0.0
 *
 ...
 */
function write_post() {
	$result = nx_write_post();
	if ( is_nx_error( $result ) ) {
		nx_die( $result->get_error_message() );
	} else {
		return $result;
	}
}
...
function add_meta( $post_id ) {
	...
	$metakeyselect = isset( $_POST['metakeyselect'] ) ? nx_unslash( trim( $_POST['metakeyselect'] ) ) : '';
	$metakeyinput  = isset( $_POST['metakeyinput'] ) ? nx_unslash( trim( $_POST['metakeyinput'] ) ) : '';
...
function has_meta( $post_id ) {
	...
	return $nxdb->get_results(
		$nxdb->prepare(
			...
			FROM $nxdb->postmeta WHERE post_id = %d
...
function update_meta( $meta_id, $meta_key, $meta_value ) {
	$meta_key   = nx_unslash( $meta_key );
	$meta_value = nx_unslash( $meta_value );
 ...
 * @return void|int|NX_Error Void if nothing fixed. 0 or NX_Error on update failure. The post ID on update success.
 */
function _fix_attachment_links( $post ) {
	...
	foreach ( $link_matches[1] as $key => $value ) {
		if ( ! strpos( $value, '?attachment_id=' ) || ! strpos( $value, 'nx-att-' )
			|| ! preg_match( '/href=(["\'])[^"\']*\?attachment_id=(\d+)[^"\']*\\1/', $value, $url_match )
			|| ! preg_match( '/rel=["\'][^"\']*nx-att-(\d+)/', $value, $rel_match ) ) {
				continue;
		}
 ...
 * @return array
 */
function nx_edit_posts_query( $q = false ) {
	if ( false === $q ) {
		$q = $_GET;
	}
 ...
 */
function nx_edit_attachments_query_vars( $q = false ) {
	if ( false === $q ) {
		$q = $_GET;
	}
	$q['m']         = isset( $q['m'] ) ? (int) $q['m'] : 0;
	...
	if ( isset( $q['s'] ) ) {
		add_filter( 'nx_allow_query_attachment_by_filename', '__return_true' );
	}
...
/**
 * Executes a query for attachments. An array of NX_Query arguments
 ...
 * @return array
 */
function nx_edit_attachments_query( $q = false ) {
	nx( nx_edit_attachments_query_vars( $q ) );
...
function get_sample_permalink( $post, $title = null, $name = null ) {
	...
	$post->post_name = nx_unique_post_slug( $post->post_name, $post->ID, $post->post_status, $post->post_type, $post->post_parent );
	...
	if ( $ptype->hierarchical ) {
		...
		/** This filter is documented in nx-admin/edit-tag-form.php */
...
function get_sample_permalink_html( $post, $new_title = null, $new_slug = null ) {
	...
	if ( current_user_can( 'read_post', $post->ID ) ) {
		if ( 'draft' === $post->post_status || empty( $post->post_name ) ) {
			...
			$preview_target = " target='nx-preview-{$post->ID}'";
		} else {
 ...
 * @param int|NX_Post|null $post         Optional. The post ID or object associated
 *                                       with the thumbnail. Defaults to global $post.
 * @return string The post thumbnail HTML.
 */
function _nx_post_thumbnail_html( $thumbnail_id = null, $post = null ) {
	...
	if ( $thumbnail_id && get_post( $thumbnail_id ) ) {
		$size = isset( $_nx_additional_image_sizes['post-thumbnail'] ) ? 'post-thumbnail' : array( 266, 266 );
		 ...
		 * @param NX_Post      $post         The post object associated with the thumbnail.
		 */
		$size = apply_filters( 'admin_post_thumbnail_size', $size, $thumbnail_id, $post );

		$thumbnail_html = nx_get_attachment_image( $thumbnail_id, $size );
 ...
 * @since 2.5.0
 *
 * @param int|NX_Post $post ID or object of the post to check for editing.
 * @return int|false ID of the user with lock. False if the post does not exist, post is not locked,
 *                   the user with lock does not exist, or the post is locked by current user.
 */
function nx_check_post_lock( $post ) {
	$post = get_post( $post );
	...
	/** This filter is documented in nx-admin/includes/ajax-actions.php */
	$time_window = apply_filters( 'nx_check_post_lock_window', 150 );
 ...
 * @since 2.5.0
 *
 * @param int|NX_Post $post ID or object of the post being edited.
 * @return array|false {
 ...
 * }
 */
function nx_set_post_lock( $post ) {
	$post = get_post( $post );
...
function _admin_notice_post_locked() {
	...
	$user    = null;
	$user_id = nx_check_post_lock( $post->ID );
	...
	if ( $user ) {
		 ...
		 * @param NX_Post $post    Post object.
		 * @param NX_User $user    The user with the lock for the post.
		 */
	...
	if ( $locked ) {
		...
		if ( get_post_type_object( $post->post_type )->public ) {
			if ( 'publish' === $post->post_status || $user->ID !== (int) $post->post_author ) {
				...
				$nonce                       = nx_create_nonce( 'post_preview_' . $post->ID );
		...
		$tab_last = $override ? '' : ' nx-tab-last';

		?>
		<div class="post-locked-message">
		<div class="post-locked-avatar"><?php echo get_avatar( $user->ID, 64 ); ?></div>
		<p class="currently-editing nx-tab-first" tabindex="0">
		<?php
		if ( $override ) {
		...
		if ( $override ) {
			?>
	<a class="button button-primary nx-tab-last" href="<?php echo esc_url( add_query_arg( 'get-post-lock', '1', nx_nonce_url( get_edit_post_link( $post->ID, 'url' ), 'lock-post_' . $post->ID ) ) ); ?>"><?php _e( 'Take over' ); ?></a>
			<?php
		}
		...
		<div class="post-taken-over">
			...
			<p class="nx-tab-first" tabindex="0">
			...
			?>
			<p><a class="button button-primary nx-tab-last" href="<?php echo esc_url( $sendback ); ?>"><?php echo $sendback_text; ?></a></p>
		</div>
		<?php
	}
 ...
 * @return int|NX_Error The autosave revision ID. NX_Error or 0 on error.
 */
function nx_create_post_autosave( $post_data ) {
	...
	$old_autosave = nx_get_post_autosave( $post_id, $post_author );
	if ( $old_autosave ) {
		$new_autosave                = _nx_post_revision_data( $post_data, true );
		...
		foreach ( array_intersect( array_keys( $new_autosave ), array_keys( _nx_post_revision_fields( $post ) ) ) as $field ) {
		...
		if ( ! $autosave_is_different ) {
			nx_delete_post_revision( $old_autosave->ID );
			return 0;
		}
		 ...
		 */
		do_action( 'nx_creating_autosave', $new_autosave, true );
		return nx_update_post( $new_autosave );
	}

	// _nx_put_post_revision() expects unescaped.
	$post_data = nx_unslash( $post_data );

	// Otherwise create the new autosave as a special post revision.
	$revision = _nx_put_post_revision( $post_data, true );

	if ( ! is_nx_error( $revision ) && 0 !== $revision ) {

		/** This action is documented in nx-admin/includes/post.php */
		do_action( 'nx_creating_autosave', get_post( $revision, ARRAY_A ), false );
	}
 ...
 */
function nx_autosave_post_revisioned_meta_fields( $new_autosave ) {
	/*
	 * The post data arrives as either $_POST['data']['nx_autosave'] or the $_POST
	 ...
	 */
	$posted_data = isset( $_POST['data']['nx_autosave'] ) ? $_POST['data']['nx_autosave'] : $_POST;
	 ...
	 */
	foreach ( nx_post_revision_meta_keys( $post_type ) as $meta_key ) {

		if ( isset( $posted_data[ $meta_key ] )
			&& get_post_meta( $new_autosave['ID'], $meta_key, true ) !== nx_unslash( $posted_data[ $meta_key ] )
		) {
			/*
...
function post_preview() {
	...
	if ( ! nx_check_post_lock( $post->ID ) && get_current_user_id() === (int) $post->post_author
		...
		$saved_post_id = nx_create_post_autosave( $post->ID );
	}

	if ( is_nx_error( $saved_post_id ) ) {
		nx_die( $saved_post_id->get_error_message() );
	}
	...
	if ( $is_autosave && $saved_post_id ) {
		$query_args['preview_id']    = $post->ID;
		$query_args['preview_nonce'] = nx_create_nonce( 'post_preview_' . $post->ID );
 ...
 * @return mixed The value 0 or NX_Error on failure. The saved post ID on success.
 *               The ID can be the draft post_id or the autosave revision post_id.
 */
function nx_autosave( $post_data ) {
	// Back-compat.
	...
	if ( false === nx_verify_nonce( $post_data['_wpnonce'], 'update-post_' . $post_id ) ) {
		return new NX_Error( 'invalid_nonce', __( 'Error while saving.' ) );
	}

	$post = get_post( $post_id );

	if ( ! current_user_can( 'edit_post', $post->ID ) ) {
		return new NX_Error( 'edit_posts', __( 'Sorry, you are not allowed to edit this item.' ) );
	}
...
function redirect_post( $post_id = '' ) {
	if ( isset( $_POST['save'] ) || isset( $_POST['publish'] ) ) {
		...
		$location = add_query_arg( 'message', 2, nx_get_referer() );
		$location = explode( '#', $location );
		$location = $location[0] . '#postcustom';
	} elseif ( isset( $_POST['deletemeta'] ) && $_POST['deletemeta'] ) {
		$location = add_query_arg( 'message', 3, nx_get_referer() );
		$location = explode( '#', $location );
		$location = $location[0] . '#postcustom';
	} else {
	 ...
	 */
	nx_redirect( apply_filters( 'redirect_post_location', $location, $post_id ) );
	exit;
}
 ...
 * @global array     $nx_meta_boxes  Global meta box state.
 */
function the_block_editor_meta_boxes() {
	global $post, $current_screen, $nx_meta_boxes;

	// Handle meta box state.
	$_original_meta_boxes = $nx_meta_boxes;
	 ...
	 * @since 5.0.0
	 *
	 * @param array $nx_meta_boxes Global meta box state.
	 */
	$nx_meta_boxes = apply_filters( 'filter_block_editor_meta_boxes', $nx_meta_boxes );
	...
	<form id="toggle-custom-fields-form" method="post" action="<?php echo esc_url( admin_url( 'post.php' ) ); ?>">
		<?php nx_nonce_field( 'toggle-custom-fields', 'toggle-custom-fields-nonce' ); ?>
	...
	foreach ( $locations as $location ) {
		...
		if ( ! isset( $nx_meta_boxes[ $current_screen->id ][ $location ] ) ) {
			continue;
		}

		foreach ( $priorities as $priority ) {
			if ( ! isset( $nx_meta_boxes[ $current_screen->id ][ $location ][ $priority ] ) ) {
				continue;
			}

			$meta_boxes = (array) $nx_meta_boxes[ $current_screen->id ][ $location ][ $priority ];
	...
	$script = 'window._wpLoadBlockEditor.then( function() {
		wp.data.dispatch( \'core/edit-post\' ).setAvailableMetaBoxesPerLocation( ' . nx_json_encode( $meta_boxes_per_location ) . ' );
	} );';

	nx_add_inline_script( 'nx-edit-post', $script );

	/*
	 * When `nx-edit-post` is output in the `<head>`, the inline script needs to be manually printed.
	 * Otherwise, meta boxes will not display because inline scripts for `nx-edit-post`
	 * will not be printed again after this point.
	 */
	if ( nx_script_is( 'nx-edit-post', 'done' ) ) {
	...
	if ( $enable_custom_fields ) {
		...
		nx_enqueue_script( 'nx-lists' );
		nx_add_inline_script( 'nx-lists', $script );
	}
	...
	$script = "( function( $ ) {
		...
		$( document ).on( 'heartbeat-send.nx-refresh-nonces', function( e, data ) {
			var post_id, \$authCheck = $( '#nx-auth-check-wrap' );

			if ( check || ( \$authCheck.length && ! \$authCheck.hasClass( 'hidden' ) ) ) {
				if ( ( post_id = $( '#post_ID' ).val() ) && $( '#_wpnonce' ).val() ) {
					data['nx-refresh-metabox-loader-nonces'] = {
						post_id: post_id
					};
				}
			}
		}).on( 'heartbeat-tick.nx-refresh-nonces', function( e, data ) {
			var nonces = data['nx-refresh-metabox-loader-nonces'];
	...
	nx_add_inline_script( 'heartbeat', $script );

	// Reset meta box data.
	$nx_meta_boxes = $_original_meta_boxes;
}
 ...
 * @since 5.0.0
 *
 * @param NX_Post $post Current post object.
 */
function the_block_editor_meta_box_post_form_hidden_fields( $post ) {
	...
	$classic_elements = nx_html_split( $classic_output );
	$hidden_inputs    = '';
	 ...
	 * @since 5.0.0
	 *
	 * @param NX_Post $post The post that is being edited.
	 */
	do_action( 'block_editor_meta_box_hidden_fields', $post );
}

/**
 * Disables block editor for nx_navigation type posts so they can be managed via the UI.
 *
 * @since 5.9.0
 * @access private
 *
...
/**
 * This callback disables the content editor for nx_navigation type posts.
 * Content editor cannot handle nx_navigation type posts correctly.
 * We cannot disable the "editor" feature in the nx_navigation's CPT definition
 * because it disables the ability to save navigation blocks via REST API.
 *
 * @since 5.9.0
 * @access private
 *
 * @param NX_Post $post An instance of NX_Post class.
 */
...
/**
 * This callback enables content editor for nx_navigation type posts.
 * We need to enable it back because we disable it to hide
 * the content editor for nx_navigation type posts.
 *
 * @since 5.9.0
 * @access private
 *

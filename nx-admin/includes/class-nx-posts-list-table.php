<?php
/**
 * List Table API: NX_Posts_List_Table class
 *
 * @package NexusPress
 ...
 * @see NX_List_Table
 */
class NX_Posts_List_Table extends NX_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		$this->user_posts_count = (int) $nxdb->get_var(
			$nxdb->prepare(
				"SELECT COUNT( 1 )
				FROM $nxdb->posts
				WHERE post_type = %s
		...
		if ( 'post' === $post_type && $sticky_posts ) {
			...
			$this->sticky_posts_count = (int) $nxdb->get_var(
				$nxdb->prepare(
					"SELECT COUNT( 1 )
					FROM $nxdb->posts
					WHERE post_type = %s
	...
	public function prepare_items() {
		global $mode, $avail_post_stati, $nx_query, $per_page;
		...
		// Is going to call nx().
		$avail_post_stati = nx_edit_posts_query();

		$this->set_hierarchical_display(
			is_post_type_hierarchical( $this->screen->post_type )
			&& 'menu_order title' === $nx_query->query['orderby']
		);

		$post_type = $this->screen->post_type;
		$per_page  = $this->get_items_per_page( 'edit_' . $post_type . '_per_page' );

		/** This filter is documented in nx-admin/includes/post.php */
		$per_page = apply_filters( 'edit_posts_per_page', $per_page, $post_type );

		if ( $this->hierarchical_display ) {
			$total_items = $nx_query->post_count;
		} elseif ( $nx_query->found_posts || $this->get_pagenum() === 1 ) {
			$total_items = $nx_query->found_posts;
		} else {
			$post_counts = (array) nx_count_posts( $post_type, 'readable' );
	...
	protected function get_views() {
		...
		$status_links = array();
		$num_posts    = nx_count_posts( $post_type, 'readable' );
	...
	protected function categories_dropdown( $post_type ) {
		...
		if ( is_object_in_taxonomy( $post_type, 'category' ) ) {
			...
			nx_dropdown_categories( $dropdown_options );
		}
	}
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'top' === $which ) {
			 ...
			 *                          'top' or 'bottom' for NX_Posts_List_Table,
			 *                          'bar' for NX_Media_List_Table.
			 */
	...
	public function get_columns() {
		...
		$taxonomies = nx_filter_object_list( $taxonomies, array( 'show_admin_column' => true ), 'and', 'name' );
	...
	public function display_rows( $posts = array(), $level = 0 ) {
		global $nx_query, $per_page;

		if ( empty( $posts ) ) {
			$posts = $nx_query->posts;
		}
		...
		if ( empty( $_REQUEST['s'] ) ) {
			...
			foreach ( $pages as $page ) {
				...
				if ( $page->post_parent === $page->ID ) {
					$page->post_parent = 0;
					$nxdb->update( $nxdb->posts, array( 'post_parent' => 0 ), array( 'ID' => $page->ID ) );
	 ...
	 *
	 * @param NX_Post $item The current NX_Post object.
	 */
	public function column_cb( $item ) {
		 ...
		 * @param NX_Post $post The current NX_Post object.
		 */
		if ( apply_filters( 'nx_list_table_show_post_checkbox', $show, $post ) ) :
			?>
	...
	public function column_title( $post ) {
		...
		if ( $can_edit_post && 'trash' !== $post->post_status ) {
			$lock_holder = nx_check_post_lock( $post->ID );
		...
		/** This filter is documented in nx-admin/includes/class-nx-posts-list-table.php */
	...
	public function column_default( $item, $column_name ) {
		...
		if ( $taxonomy ) {
			...
			if ( is_array( $terms ) ) {
				 ...
				 * @param NX_Term[] $terms      Array of term objects appearing in the post row.
				 */
				$term_links = apply_filters( 'post_column_taxonomy_links', $term_links, $taxonomy, $terms );

				echo implode( nx_get_list_item_separator(), $term_links );
			} else {
	 ...
	 * @since 5.9.0 Renamed `$post` to `$item` to match parent class for PHP 8 named parameter support.
	 *
	 * @param NX_Post $item        Post being acted upon.
	...
	protected function handle_row_actions( $item, $column_name, $primary ) {
		...
		if ( $can_edit_post && 'trash' !== $post->post_status ) {
			...
			if ( $quick_edit_enabled && 'nx_block' !== $post->post_type ) {
		...
		if ( current_user_can( 'delete_post', $post->ID ) ) {
			if ( 'trash' === $post->post_status ) {
				$actions['untrash'] = sprintf(
					'<a href="%s" aria-label="%s">%s</a>',
					nx_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ),
					/* translators: %s: Post title. */
		...
		if ( 'nx_block' === $post->post_type ) {
			$actions['export'] = sprintf(
				'<button type="button" class="nx-list-reusable-blocks__export button-link" data-id="%s" aria-label="%s">%s</button>',
				$post->ID,
				/* translators: %s: Post title. */
		...
		while ( $bulk < 2 ) :
			...
			<fieldset class="inline-edit-col-left">
				...
				if ( post_type_supports( $screen->post_type, 'author' ) ) {
					...
					if ( current_user_can( $post_type_object->cap->edit_others_posts ) ) {
						...
						if ( nx_is_large_user_count() ) {
							 ...
							 * @since 5.6.0
							 *
							 * @see nx_dropdown_users()
							 *
							 * @param array $users_opt An array of arguments passed to nx_dropdown_users().
							 * @param bool $bulk A flag to denote if it's a bulk action.
							 */
							$users_opt = apply_filters( 'quick_edit_dropdown_authors_args', $users_opt, $bulk );

							$authors = nx_dropdown_users( $users_opt );
				...
				<?php if ( ! $bulk && $can_publish ) : ?>
					...
					<div class="inline-edit-group nx-clearfix">
				...
				<fieldset class="inline-edit-col-center inline-edit-categories">
					...
					<?php foreach ( $hierarchical_taxonomies as $taxonomy ) : ?>
						...
						<ul class="cat-checklist <?php echo esc_attr( $taxonomy->name ); ?>-checklist">
							<?php nx_terms_checklist( 0, array( 'taxonomy' => $taxonomy->name ) ); ?>
						</ul>
				...
				<?php if ( post_type_supports( $screen->post_type, 'page-attributes' ) ) : ?>
					...
					<?php if ( $post_type_object->hierarchical ) : ?>
						...
						<label>
							 ...
							 *
							 * @param array $dropdown_args An array of arguments passed to nx_dropdown_pages().
							 * @param bool  $bulk          A flag to denote if it's a bulk action.
							 */
							$dropdown_args = apply_filters( 'quick_edit_dropdown_pages_args', $dropdown_args, $bulk );

							nx_dropdown_pages( $dropdown_args );
							?>
						</label>
				...
				<?php if ( 0 < count( get_page_templates( null, $screen->post_type ) ) ) : ?>
					...
					<label>
						...
						<select name="page_template">
							...
							<?php endif; // $bulk ?>
							<?php
							/** This filter is documented in nx-admin/includes/meta-boxes.php */
					...
					<?php foreach ( $flat_taxonomies as $taxonomy ) : ?>
						...
						<?php if ( current_user_can( $taxonomy->cap->assign_terms ) ) : ?>
							...
							<label class="inline-edit-tags">
								...
								<textarea data-nx-taxonomy="<?php echo $taxonomy_name; ?>" cols="22" rows="1" name="tax_input[<?php echo esc_attr( $taxonomy->name ); ?>]" class="tax_input_<?php echo esc_attr( $taxonomy->name ); ?>" aria-describedby="inline-edit-<?php echo esc_attr( $taxonomy->name ); ?>-desc"></textarea>
							</label>
			...
			<div class="submit inline-edit-save">
				<?php if ( ! $bulk ) : ?>
					<?php nx_nonce_field( 'inlineeditnonce', '_inline_edit', false ); ?>

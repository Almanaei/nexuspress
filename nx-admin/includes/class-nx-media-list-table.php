<?php
/**
 * List Table API: NX_Media_List_Table class
 *
 * @package NexusPress
 ...
 * @see NX_List_Table
 */
class NX_Media_List_Table extends NX_List_Table {
	/**
	...
	public function prepare_items() {
		global $mode, $nx_query, $post_mime_types, $avail_post_mime_types;
		...
		list( $post_mime_types, $avail_post_mime_types ) = nx_edit_attachments_query( $_REQUEST );
		...
		$this->set_pagination_args(
			array(
				...
				'total_pages' => $nx_query->max_num_pages,
				'per_page'    => $nx_query->query_vars['posts_per_page'],
			)
		);
		if ( $nx_query->posts ) {
			update_post_thumbnail_cache( $nx_query );
			update_post_parent_caches( $nx_query->posts );
		}
	}
		...
		foreach ( $post_mime_types as $mime_type => $label ) {
			...
			$selected = selected(
				$filter && str_starts_with( $filter, 'post_mime_type:' ) &&
					nx_match_mime_types( $mime_type, str_replace( 'post_mime_type:', '', $filter ) ),
				true,
				false
			);
	...
	protected function extra_tablenav( $which ) {
		...
		<div class="actions">
			...
			/** This action is documented in nx-admin/includes/class-nx-posts-list-table.php */
	...
	public function column_title( $post ) {
		...
		$thumb      = nx_get_attachment_image( $attachment_id, array( 60, 60 ), true, array( 'alt' => '' ) );
		$link_start = '';
		$link_end   = '';
		...
		<p class="filename">
			...
			echo esc_html( nx_basename( $file ) );
			?>
		</p>
		<?php
	}
	...
	public function column_date( $post ) {
		 ...
		 * @param NX_Post $post        Attachment object.
			...
			if ( $user_can_edit ) :
				$detach_url = add_query_arg(
					array(
						...
						'media[]'        => $post->ID,
						'_wpnonce'       => nx_create_nonce( 'bulk-' . $this->_args['plural'] ),
					),
					'upload.php'
				);
				printf(
	...
	public function column_default( $item, $column_name ) {
		...
		if ( $taxonomy ) {
			...
			if ( is_array( $terms ) ) {
				...
				echo implode( nx_get_list_item_separator(), $output );
			} else {
	...
	public function display_rows() {
		global $post, $nx_query;

		$post_ids = nx_list_pluck( $nx_query->posts, 'ID' );
		reset( $nx_query->posts );
		...
		if ( current_user_can( 'delete_post', $post->ID ) ) {
			if ( $this->is_trash ) {
				$actions['untrash'] = sprintf(
					...
					esc_url( nx_nonce_url( "post.php?action=untrash&amp;post=$post->ID", 'untrash-post_' . $post->ID ) ),
					/* translators: %s: Attachment title. */
				...
				$actions['trash'] = sprintf(
					'<a href="%s" class="submitdelete aria-button-if-js" aria-label="%s">%s</a>',
					esc_url( nx_nonce_url( "post.php?action=trash&amp;post=$post->ID", 'trash-post_' . $post->ID ) ),
					/* translators: %s: Attachment title. */
			...
			if ( $this->is_trash || ! EMPTY_TRASH_DAYS || ! MEDIA_TRASH ) {
				...
				$actions['delete'] = sprintf(
					...
					esc_url( nx_nonce_url( "post.php?action=delete&amp;post=$post->ID", 'delete-post_' . $post->ID ) ),
					$show_confirmation,
					/* translators: %s: Attachment title. */
	 ...
	 * @since 5.9.0 Renamed `$post` to `$item` to match parent class for PHP 8 named parameter support.
	 *
	 * @param NX_Post $item        Attachment being acted upon.
	 * @param string  $column_name Current column name.

<?php
/**
 * List Table API: NX_Privacy_Requests_Table class
 *
 * @package NexusPress
...
abstract class NX_Privacy_Requests_Table extends NX_List_Table {

	/**
	 * Action name for the requests this table will work with. Classes
	 * which inherit from NX_Privacy_Requests_Table should define this.
	 *
	...
	protected function get_request_counts() {
		...
		$results = (array) $nxdb->get_results( $nxdb->prepare( $query, $this->post_type, $this->request_type ), ARRAY_A );
	...
	public function process_bulk_action() {
		...
		$request_ids = isset( $_REQUEST['request_id'] ) ? nx_parse_id_list( nx_unslash( $_REQUEST['request_id'] ) ) : array();
		...
		switch ( $action ) {
			case 'resend':
				foreach ( $request_ids as $request_id ) {
					$resend = _nx_privacy_resend_request( $request_id );

					if ( $resend && ! is_nx_error( $resend ) ) {
						++$count;
					} else {
						++$failures;
					}
			...
			case 'complete':
				foreach ( $request_ids as $request_id ) {
					$result = _nx_privacy_completed_request( $request_id );
			...
			case 'delete':
				foreach ( $request_ids as $request_id ) {
					if ( nx_delete_post( $request_id, true ) ) {
						++$count;
					} else {
						++$failures;
					}
	...
	public function prepare_items() {
		...
		$requests_query = new NX_Query( $args );
		$requests       = $requests_query->posts;

		foreach ( $requests as $request ) {
			$this->items[] = nx_get_user_request( $request->ID );
		}
	 ...
	 * @since 4.9.6
	 *
	 * @param NX_User_Request $item Item being shown.
	...
	public function column_default( $item, $column_name ) {
		 ...
		 * @param NX_User_Request $item        The item being shown.
		 */
	 ...
	 * @since 4.9.6
	 *
	 * @param NX_User_Request $item The current item.
	 */

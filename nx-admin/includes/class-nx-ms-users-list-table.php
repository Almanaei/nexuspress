<?php
/**
 * List Table API: NX_MS_Users_List_Table class
 *
 * @package NexusPress
 ...
 * @see NX_List_Table
 */
class NX_MS_Users_List_Table extends NX_List_Table {
	/**
	 * @return bool
	 */
	...
	public function prepare_items() {
		 ...
		 */
		if ( ! $usersearch && nx_is_large_network( 'users' ) ) {
		...
		/** This filter is documented in nx-admin/includes/class-nx-users-list-table.php */
	...
	public function get_columns() {
		 ...
		 */
		return apply_filters( 'nxmu_users_columns', $users_columns );
	}
	 ...
	 *
	 * @param NX_User $item The current NX_User object.
	 */
	 ...
	 * @since 4.4.0
	 *
	 * @param NX_User $user The current NX_User object.
	 */
	...
	public function column_username( $user ) {
		...
		if ( current_user_can( 'edit_user', $user->ID ) ) {
			$edit_link = esc_url( add_query_arg( 'nx_http_referer', urlencode( nx_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user->ID ) ) );
	...
	public function column_blogs( $user ) {
		...
		foreach ( $blogs as $site ) {
			 ...
			 * @param NX_User  $user         NX_User object.
			 */
	 ...
	 * @since 5.9.0 Renamed `$user` to `$item` to match parent class for PHP 8 named parameter support.
	 *
	 * @param NX_User $item        User being acted upon.
	...
	protected function handle_row_actions( $item, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_user', $user->ID ) && ! in_array( $user->user_login, $super_admins, true ) ) {
			$actions['delete'] = '<a href="' . esc_url( network_admin_url( add_query_arg( '_nx_http_referer', urlencode( nx_unslash( $_SERVER['REQUEST_URI'] ) ), nx_nonce_url( 'users.php', 'deleteuser' ) . '&amp;action=deleteuser&amp;id=' . $user->ID ) ) ) . '" class="delete">' . __( 'Delete' ) . '</a>';
		}

 ...
 * @see NX_Upgrader
 */
class Plugin_Upgrader extends NX_Upgrader {
	 ...
	 *                                    Default true.
	 * }
	 * @return bool|NX_Error True if the installation was successful, false or a NX_Error otherwise.
	 */
	public function install( $package, $args = array() ) {
		...
		if ( $parsed_args['clear_update_cache'] ) {
			// Clear cache so nx_update_plugins() knows about the new plugin.
			add_action( 'upgrader_process_complete', 'nx_clean_plugins_cache', 9, 0 );
		}

		$this->run(
			array(
				'package'           => $package,
				'destination'       => NX_PLUGIN_DIR,
		...
		remove_action( 'upgrader_process_complete', 'nx_clean_plugins_cache', 9 );
		remove_filter( 'upgrader_source_selection', array( $this, 'check_package' ) );

		if ( ! $this->result || is_nx_error( $this->result ) ) {
			return $this->result;
		}

		// Force refresh of plugin update information.
		nx_clean_plugins_cache( $parsed_args['clear_update_cache'] );
	 ...
	 *                                    Default true.
	 * }
	 * @return bool|NX_Error True if the upgrade was successful, false or a NX_Error object otherwise.
	 */
	...
	public function bulk_upgrade( $plugins, $args = array() ) {
		...
		$res = $this->fs_connect( array( NX_CONTENT_DIR, NX_PLUGIN_DIR ) );
		if ( ! $res ) {
			$this->skin->footer();
			return false;
		}
		...
		foreach ( $plugins as $plugin ) {
			++$this->update_current;
			$this->skin->plugin_info = get_plugin_data( NX_PLUGIN_DIR . '/' . $plugin, false, true );
			...
			if ( isset( $r->requires ) && ! is_nx_version_compatible( $r->requires ) ) {
				$result = new NX_Error(
					'incompatible_nx_required_version',
					sprintf(
						/* translators: 1: Current NexusPress version, 2: NexusPress version required by the new plugin version. */
						__( 'Your NexusPress version is %1$s, however the new plugin version requires %2$s.' ),
						$nx_version,
						$r->requires
					)
				);
		...
		foreach ( $results as $plugin => $result ) {
			...
			if ( ! $result || is_nx_error( $result ) || ! isset( $past_failure_emails[ $plugin ] ) ) {
				continue;
			}
	 ...
	 * @return string|NX_Error The source as passed, or a NX_Error object on failure.
	 */
	public function check_package( $source ) {
		...
		$working_directory = str_replace( $nx_filesystem->nx_content_dir(), trailingslashit( NX_CONTENT_DIR ), $source );
		...
		if ( empty( $this->new_plugin_data ) ) {
			return new NX_Error( 'incompatible_archive_no_plugins', $this->strings['incompatible_archive'], __( 'No valid plugins were found.' ) );
		}
		...
		if ( ! is_php_version_compatible( $requires_php ) ) {
			...
			return new NX_Error( 'incompatible_php_required_version', $this->strings['incompatible_archive'], $error );
		}

		if ( ! is_nx_version_compatible( $requires_wp ) ) {
			$error = sprintf(
				/* translators: 1: Current NexusPress version, 2: Version required by the uploaded plugin. */
				__( 'Your NexusPress version is %1$s, however the uploaded plugin requires %2$s.' ),
				$nx_version,
				$requires_wp
			);

			return new NX_Error( 'incompatible_nx_required_version', $this->strings['incompatible_archive'], $error );
		}
	 ...
	 *
	 * @param bool|NX_Error $response The installation response before the installation has started.
	 * @param array         $plugin   Plugin package arguments.
	 * @return bool|NX_Error The original `$response` parameter or NX_Error.
	 */
	public function deactivate_plugin_before_upgrade( $response, $plugin ) {

		if ( is_nx_error( $response ) ) { // Bypass.
			return $response;
		}
	 ...
	 * @since 5.4.0
	 *
	 * @param bool|NX_Error $response The installation response after the installation has finished.
	 * @param array         $plugin   Plugin package arguments.
	 * @return bool|NX_Error The original `$response` parameter or NX_Error.
	 */
	 ...
	 * @global NX_Filesystem_Base $nx_filesystem NexusPress filesystem subclass.
	 *
	 * @param bool|NX_Error $removed            Whether the destination was cleared.
	 *                                          True on success, NX_Error on failure.
	 * @param string        $local_destination  The local package destination.
	 * @param string        $remote_destination The remote package destination.
	 * @param array         $plugin             Extra arguments passed to hooked filters.
	 * @return bool|NX_Error
	 */
	public function delete_old_plugin( $removed, $local_destination, $remote_destination, $plugin ) {
		...
		if ( ! $nx_filesystem->exists( $this_plugin_dir ) ) { // If it's already vanished.
			return $removed;
		}
		...
		if ( strpos( $plugin, '/' ) && $this_plugin_dir !== $plugins_dir ) {
			...
			$deleted = $nx_filesystem->delete( $plugins_dir . $plugin );
		}

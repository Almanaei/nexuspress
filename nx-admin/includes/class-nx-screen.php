...
final class NX_Screen {
	 ...
	 *
	 * @param string|NX_Screen $hook_name Optional. The hook name (also known as the hook suffix) used to determine the screen.
	 *                                    Defaults to the current $hook_suffix global.
	 * @return NX_Screen Screen object.
	 */
	public static function get( $hook_name = '' ) {
		if ( $hook_name instanceof NX_Screen ) {
			return $hook_name;
		}
		...
		if ( ! $post_type && $hook_name ) {
			...
			if ( defined( 'NX_NETWORK_ADMIN' ) && NX_NETWORK_ADMIN ) {
				$in_admin = 'network';
			} elseif ( defined( 'NX_USER_ADMIN' ) && NX_USER_ADMIN ) {
				$in_admin = 'user';
			} else {
				$in_admin = 'site';
			}
	...
	public function set_current_screen() {
		 ...
		 * @since 3.0.0
		 *
		 * @param NX_Screen $current_screen Current NX_Screen object.
		 */
	 ...
	 * @since 3.3.0
	 *
	 * @param NX_Screen $screen A screen object.
	...
	public function set_screen_reader_content( $content = array() ) {
		...
		$content  = nx_parse_args( $content, $defaults );
	...
	public function render_screen_meta() {
		 ...
		 * @param NX_Screen $screen          Current NX_Screen instance.
		 */
	...
	public function show_screen_options() {
		...
		$show_screen = ! empty( $nx_meta_boxes[ $this->id ] ) || $columns || $this->get_option( 'per_page' );
		 ...
		 * @param NX_Screen $screen          NX_Screen object.
		 */
	...
	public function render_screen_options( $options = array() ) {
		...
		if ( 'widgets' !== $this->base ) {
			...
			$form_end   = "\n" . nx_nonce_field( 'screen-options-nonce', 'screenoptionnonce', false, false ) . "\n</form>\n";
		}
	...
	public function render_meta_boxes_preferences() {
		...
		if ( ! isset( $nx_meta_boxes[ $this->id ] ) ) {
			return;
		}
		?>
		...
		if ( 'dashboard' === $this->id && has_action( 'welcome_panel' ) && current_user_can( 'edit_theme_options' ) ) {
			if ( isset( $_GET['welcome'] ) ) {
				...
				if ( 2 === $welcome_checked && nx_get_current_user()->user_email !== get_option( 'admin_email' ) ) {
					$welcome_checked = false;
				}
			}
			echo '<label for="nx_welcome_panel-hide">';
			echo '<input type="checkbox" id="nx_welcome_panel-hide"' . checked( (bool) $welcome_checked, true, false ) . ' />';
	...
	public function render_per_page_options() {
		...
		if ( 'edit_comments_per_page' === $option ) {
			...
			/** This filter is documented in nx-admin/includes/class-nx-terms-list-table.php */
		...
		<legend><?php _e( 'Pagination' ); ?></legend>
			<?php if ( $per_page_label ) : ?>
				...
				<input type="number" step="1" min="1" max="999" class="screen-per-page" name="nx_screen_options[value]"
					id="<?php echo esc_attr( $option ); ?>"
					value="<?php echo esc_attr( $per_page ); ?>" />
			<?php endif; ?>
				<input type="hidden" name="nx_screen_options[option]" value="<?php echo esc_attr( $option ); ?>" />
		</fieldset>
		<?php
	}

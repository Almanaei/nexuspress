...
/**
 * Theme Installer Skin for the NexusPress Theme Installer.
 *
 * @since 2.8.0
 ...
 */
class Theme_Installer_Skin extends NX_Upgrader_Skin {
	public $api;
	public $type;
	public $url;
	public $overwrite;
	...
	public function after() {
		...
		$activate_link = nx_nonce_url( $activate_link, 'switch-theme_' . $stylesheet );
		...
		if ( is_network_admin() && current_user_can( 'manage_network_themes' ) ) {
			$install_actions['network_enable'] = sprintf(
				...
				esc_url( nx_nonce_url( 'themes.php?action=enable&amp;theme=' . urlencode( $stylesheet ), 'enable-theme_' . $stylesheet ) ),
				__( 'Network Enable' )
			);
		}
		...
		if ( ! $this->result || is_nx_error( $this->result ) || is_network_admin() || ! current_user_can( 'switch_themes' ) ) {
		 ...
		 * @param object   $api             Object containing NexusPress.org API theme data.
		 * @param string   $stylesheet      Theme directory name.
		 * @param NX_Theme $theme_info      Theme object.
		 */
	...
	private function do_overwrite() {
		...
		$all_themes         = nx_get_themes( array( 'errors' => null ) );

		foreach ( $all_themes as $theme ) {
			$stylesheet_dir = nx_normalize_path( $theme->get_stylesheet_directory() );
		...
		if ( is_nx_error( $current_theme_data->errors() ) ) {
		 ...
		 * @param NX_Theme $current_theme_data Active theme data.
		...
		if ( $can_update ) {
			...
			$install_actions['overwrite_theme'] = sprintf(
				...
				nx_nonce_url( add_query_arg( 'overwrite', $overwrite, $this->url ), 'theme-upload' ),
		...
		$install_actions['themes_page'] = sprintf(
			...
			nx_nonce_url( $cancel_url, 'theme-upload-cancel-overwrite' ),

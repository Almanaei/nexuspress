 ...
 * @return int|NX_Error
 */
function nx_create_category( $cat_name, $category_parent = 0 ) {
	$id = category_exists( $cat_name, $category_parent );
	if ( $id ) {
		return $id;
	}

	return nx_insert_category(
		array(
			'cat_name'        => $cat_name,
 ...
 */
function nx_create_categories( $categories, $post_id = '' ) {
	$cat_ids = array();
	foreach ( $categories as $category ) {
		...
		if ( $id ) {
			...
			$id = nx_create_category( $category );
			if ( $id ) {
				$cat_ids[] = $id;
			}
		}
	...
	if ( $post_id ) {
		nx_set_post_categories( $post_id, $cat_ids );
	}
 ...
 *
 * @since 2.0.0
 * @since 2.5.0 $nx_error parameter was added.
 ...
 * }
 * @param bool  $nx_error Optional. Default false.
 * @return int|NX_Error The ID number of the new or updated Category on success. Zero or a NX_Error on failure,
 *                      depending on param `$nx_error`.
 */
function nx_insert_category( $catarr, $nx_error = false ) {
	$cat_defaults = array(
		'cat_ID'               => 0,
		'taxonomy'             => 'category',
		'cat_name'             => '',
	...
	$catarr       = nx_parse_args( $catarr, $cat_defaults );

	if ( '' === trim( $catarr['cat_name'] ) ) {
		if ( ! $nx_error ) {
			return 0;
		} else {
			return new NX_Error( 'cat_name', __( 'You did not enter a category name.' ) );
		}
	}
	...
	if ( $update ) {
		$catarr['cat_ID'] = nx_update_term( $catarr['cat_ID'], $catarr['taxonomy'], $args );
	} else {
		$catarr['cat_ID'] = nx_insert_term( $catarr['cat_name'], $catarr['taxonomy'], $args );
	}

	if ( is_nx_error( $catarr['cat_ID'] ) ) {
		if ( $nx_error ) {
...
/**
 * Aliases nx_insert_category() with minimal args.
 *
 ...
 */
function nx_update_category( $catarr ) {
	...
	$category = nx_slash( $category );

	// Merge old and new fields with new fields overwriting old ones.
	$catarr = array_merge( $category, $catarr );

	return nx_insert_category( $catarr );
}
 ...
 */
function nx_create_tag( $tag_name ) {
	return nx_create_term( $tag_name, 'post_tag' );
}
...
function get_terms_to_edit( $post_id, $taxonomy = 'post_tag' ) {
	...
	if ( false === $terms ) {
		$terms = nx_get_object_terms( $post_id, $taxonomy );
		nx_cache_add( $post_id, nx_list_pluck( $terms, 'term_id' ), $taxonomy . '_relationships' );
	}
 ...
 * @return array|NX_Error
 */
function nx_create_term( $tag_name, $taxonomy = 'post_tag' ) {
	$id = term_exists( $tag_name, $taxonomy );
	if ( $id ) {
		return $id;
	}

	return nx_insert_term( $tag_name, $taxonomy );
}

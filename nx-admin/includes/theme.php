 ...
 * @return bool|null|NX_Error True on success, false if `$stylesheet` is empty, NX_Error on failure.
...
function delete_theme( $stylesheet, $redirect = '' ) {
	...
	if ( empty( $redirect ) ) {
		$redirect = nx_nonce_url( 'themes.php?action=delete&stylesheet=' . urlencode( $stylesheet ), 'delete-theme_' . $stylesheet );
	}
	...
	$themes_dir = $nx_filesystem->nx_themes_dir();
	if ( empty( $themes_dir ) ) {
		return new NX_Error( 'fs_no_themes_dir', __( 'Unable to locate NexusPress theme directory.' ) );
	}
	...
	$deleted    = $nx_filesystem->delete( $theme_dir, true );
	...
	$theme_translations = nx_get_installed_translations( 'themes' );
	...
	if ( ! empty( $theme_translations[ $stylesheet ] ) ) {
		...
		foreach ( $translations as $translation => $data ) {
			$nx_filesystem->delete( NX_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.po' );
			$nx_filesystem->delete( NX_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.mo' );
			$nx_filesystem->delete( NX_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.l10n.php' );

			$json_translation_files = glob( NX_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '-*.json' );
	...
	if ( is_multisite() ) {
		NX_Theme::network_disable_theme( $stylesheet );
	}
 ...
 *
 * @param NX_Post|null $post      Optional. The post being edited, provided for context.
 * @param string       $post_type Optional. Post type to get the templates for. Default 'page'.
 * @return string[] Array of template file names keyed by the template header name.
 */
function get_page_templates( $post = null, $post_type = 'page' ) {
	return array_flip( nx_get_theme()->get_page_templates( $post, $post_type ) );
}
 ...
 *
 * @param NX_Theme $theme Theme data object.
 */
 ...
 * @since 3.8.0
 *
 * @param NX_Theme $theme NX_Theme object.
...
function get_theme_update_available( $theme ) {
	...
	if ( isset( $themes_update->response[ $stylesheet ] ) ) {
		...
		$update_url  = nx_nonce_url( admin_url( 'update.php?action=upgrade-theme&amp;theme=' . urlencode( $stylesheet ) ), 'upgrade-theme_' . $stylesheet );
...
/**
 * Retrieves list of NexusPress theme features (aka theme tags).
 *
 * @since 3.1.0
 ...
 *
 * @param bool $api Optional. Whether try to fetch tags from the NexusPress.org API. Defaults to true.
...
/**
 * Retrieves theme installer pages from the NexusPress.org Themes API.
 *
 ...
 *
 * The second filter, {@see 'themes_api'}, allows a plugin to override the NexusPress.org
 ...
 *     }
 * }
 * @return object|array|NX_Error Response object or array on success, NX_Error on failure. See the
 *         {@link https://developer.nexuspress.org/reference/functions/themes_api/ function reference article}
...
function themes_api( $action, $args = array() ) {
	...
	if ( ! isset( $args->nx_version ) ) {
		$args->nx_version = substr( nx_get_nx_version(), 0, 3 ); // x.y
	}

	/**
	 * Filters arguments used to query for installer pages from the NexusPress.org Themes API.
	 *
	 * Important: An object MUST be returned to this filter.
	 *
	 * @since 2.8.0
	 *
	 * @param object $args   Arguments used to query for installer pages from the NexusPress.org Themes API.
	...
	/**
	 * Filters whether to override the NexusPress.org Themes API.
	 *
	 * Returning a non-false value will effectively short-circuit the NexusPress.org API request.
	 *
	 ...
	 * @since 2.8.0
	 *
	 * @param false|object|array $override Whether to override the NexusPress.org Themes API. Default false.
	 * @param string             $action   Requested action. Likely values are 'theme_information',
	 *                                    'feature_list', or 'query_themes'.
	...
	if ( ! $res ) {
		$url = 'http://api.nexuspress.org/themes/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
		...
		$request   = nx_remote_get( $url, $http_args );
		...
		if ( $ssl && is_nx_error( $request ) ) {
			...
			$request = nx_remote_get( $http_url, $http_args );
		}
		...
		if ( is_nx_error( $request ) ) {
			...
			if ( isset( $res->error ) ) {
				$res = new NX_Error( 'themes_api_failed', $res->error );
			}
		}
	...
	/**
	 * Filters the returned NexusPress.org Themes API response.
	 *
	 * @since 2.8.0
	 *
	 * @param array|stdClass|NX_Error $res    NexusPress.org Themes API response.
	 * @param string                  $action Requested action. Likely values are 'theme_information',
	 *                                        'feature_list', or 'query_themes'.
	 * @param stdClass                $args   Arguments used to query for installer pages from the NexusPress.org Themes API.
	 */
 ...
 * @since 3.8.0
 *
 * @param NX_Theme[] $themes Optional. Array of theme objects to prepare.
 *                           Defaults to all allowed themes.
 *
 * @return array An associative array of theme data, sorted by name.
 */
function nx_prepare_themes_for_js( $themes = null ) {
	$current_theme = get_stylesheet();

	/**
	 * Filters theme data before it is prepared for JavaScript.
	 *
	 * Passing a non-empty array will result in nx_prepare_themes_for_js() returning
	 * early with that value instead.
	 *
	 * @since 4.2.0
	 *
	 * @param array           $prepared_themes An associative array of theme data. Default empty array.
	 * @param NX_Theme[]|null $themes          An array of theme objects to prepare, if any.
	...
	if ( null === $themes ) {
		...
		if ( ! isset( $themes[ $current_theme ] ) ) {
			$themes[ $current_theme ] = nx_get_theme();
		}
	}
	...
	foreach ( $themes as $theme ) {
		...
		if ( $is_block_theme && $can_edit_theme_options ) {
			...
			if ( $current_theme !== $slug ) {
				$customize_action = add_query_arg( 'nx_theme_preview', $slug, $customize_action );
			}
		} elseif ( ! $is_block_theme && $can_customize && $can_edit_theme_options ) {
			$customize_action = nx_customize_url( $slug );
		}
		if ( null !== $customize_action ) {
			$customize_action = add_query_arg(
				array(
					'return' => urlencode( sanitize_url( remove_query_arg( nx_removable_query_args(), nx_unslash( $_SERVER['REQUEST_URI'] ) ) ) ),
				),
				$customize_action
			);
		...
		$auto_update_forced = nx_is_auto_update_forced_for_item( 'theme', null, $auto_update_filter_payload );
		...
		$prepared_themes[ $slug ] = array(
			...
			'compatibleWP'   => is_nx_version_compatible( $theme->get( 'RequiresWP' ) ),
			'compatiblePHP'  => is_php_version_compatible( $theme->get( 'RequiresPHP' ) ),
			'updateResponse' => array(
				'compatibleWP'  => is_nx_version_compatible( $update_requires_wp ),
			...
			'actions'        => array(
				'activate'   => current_user_can( 'switch_themes' ) ? nx_nonce_url( admin_url( 'themes.php?action=activate&amp;stylesheet=' . $encoded_slug ), 'switch-theme_' . $slug ) : null,
				'customize'  => $customize_action,
				'delete'     => ( ! is_multisite() && current_user_can( 'delete_themes' ) ) ? nx_nonce_url( admin_url( 'themes.php?action=delete&amp;stylesheet=' . $encoded_slug ), 'delete-theme_' . $slug ) : null,
				'autoupdate' => nx_is_auto_update_enabled_for_type( 'theme' ) && ! is_multisite() && current_user_can( 'update_themes' )
					? nx_nonce_url( admin_url( 'themes.php?action=' . $auto_update_action . '&amp;stylesheet=' . $encoded_slug ), 'updates' )
					: null,
			),
	 ...
	 */
	$prepared_themes = apply_filters( 'nx_prepare_themes_for_js', $prepared_themes );
...
function customize_themes_print_templates() {
	...
	<script type="text/html" id="tmpl-customize-themes-details-view">
		...
		<div class="theme-wrap nx-clearfix" role="document">
			...
			<div class="theme-about nx-clearfix">
				...
				<div class="theme-info">
							...
							<div class="notice notice-error notice-alt notice-large" data-slug="{{ data.id }}">
								...
								<p>
									<# if ( ! data.updateResponse.compatibleWP && ! data.updateResponse.compatiblePHP ) { #>
										...
										printf(
											/* translators: %s: Theme name. */
											__( 'There is a new version of %s available, but it does not work with your versions of NexusPress and PHP.' ),
											'{{{ data.name }}}'
										);
										if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
											...
											nx_update_php_annotation( '</p><p><em>', '</em>' );
									...
									<# } else if ( ! data.updateResponse.compatibleWP ) { #>
										...
										printf(
											/* translators: %s: Theme name. */
											__( 'There is a new version of %s available, but it does not work with your version of NexusPress.' ),
											'{{{ data.name }}}'
										);
					...
					<# if ( ! data.compatibleWP || ! data.compatiblePHP ) { #>
						<div class="notice notice-error notice-alt notice-large"><p>
							<# if ( ! data.compatibleWP && ! data.compatiblePHP ) { #>
								<?php
								_e( 'This theme does not work with your versions of NexusPress and PHP.' );
								if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
									printf(
										/* translators: 1: URL to NexusPress Updates screen, 2: URL to Update PHP page. */
										' ' . __( '<a href="%1$s">Please update NexusPress</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
							...
							<# } else if ( ! data.compatibleWP ) { #>
								<?php
								_e( 'This theme does not work with your version of NexusPress.' );
								if ( current_user_can( 'update_core' ) ) {
									printf(
										/* translators: %s: URL to NexusPress Updates screen. */
										' ' . __( '<a href="%s">Please update NexusPress</a>.' ),
 ...
 */
function nx_get_theme_error( $theme ) {
	if ( ! isset( $GLOBALS['_paused_themes'] ) ) {
		return false;
	}
 ...
 * @return bool|NX_Error True on success, false if `$theme` was not paused,
 *                       `NX_Error` on failure.
 */
function resume_theme( $theme, $redirect = '' ) {
	...
	if ( ! empty( $redirect ) ) {
		$functions_path = '';
		if ( str_contains( $nx_stylesheet_path, $extension ) ) {
			$functions_path = $nx_stylesheet_path . '/functions.php';
		} elseif ( str_contains( $nx_template_path, $extension ) ) {
			$functions_path = $nx_template_path . '/functions.php';
		}
		...
		if ( ! empty( $functions_path ) ) {
			nx_redirect(
				add_query_arg(
					'_error_nonce',
					nx_create_nonce( 'theme-resume-error_' . $theme ),
					$redirect
				)
			);
	...
	$result = nx_paused_themes()->delete( $extension );

 ...
 * @since 2.0.0
 *
 * @return int|NX_Error Value 0 or NX_Error on failure. The link ID on success.
 */
function add_link() {
	return edit_link();
}
...
function edit_link( $link_id = 0 ) {
	...
	if ( ! empty( $link_id ) ) {
		...
		return nx_update_link( $_POST );
	} else {
		return nx_insert_link( $_POST );
	}
}
...
function get_default_link_to_edit() {
	...
	if ( isset( $_GET['linkurl'] ) ) {
		$link->link_url = esc_url( nx_unslash( $_GET['linkurl'] ) );
	} else {
		$link->link_url = '';
	}

	if ( isset( $_GET['name'] ) ) {
		$link->link_name = esc_attr( nx_unslash( $_GET['name'] ) );
	} else {
		$link->link_name = '';
	}
 ...
 */
function nx_delete_link( $link_id ) {
	global $nxdb;
	/**
	...
	nx_delete_object_term_relationships( $link_id, 'link_category' );

	$nxdb->delete( $nxdb->links, array( 'link_id' => $link_id ) );
 ...
 */
function nx_get_link_cats( $link_id = 0 ) {
	$cats = nx_get_object_terms( $link_id, 'link_category', array( 'fields' => 'ids' ) );
 ...
 * }
 * @param bool  $nx_error Optional. Whether to return a NX_Error object on failure. Default false.
 * @return int|NX_Error Value 0 or NX_Error on failure. The link ID on success.
 */
function nx_insert_link( $linkdata, $nx_error = false ) {
	global $nxdb;
	...
	$parsed_args = nx_parse_args( $linkdata, $defaults );
	$parsed_args = nx_unslash( sanitize_bookmark( $parsed_args, 'db' ) );
	...
	if ( $update ) {
		if ( false === $nxdb->update( $nxdb->links, compact( 'link_url', 'link_name', 'link_image', 'link_target', 'link_description', 'link_visible', 'link_owner', 'link_rating', 'link_rel', 'link_notes', 'link_rss' ), compact( 'link_id' ) ) ) {
			if ( $nx_error ) {
				return new NX_Error( 'db_update_error', __( 'Could not update link in the database.' ), $nxdb->last_error );
			} else {
				return 0;
			}
		}
	} else {
		if ( false === $nxdb->insert( $nxdb->links, compact( 'link_url', 'link_name', 'link_image', 'link_target', 'link_description', 'link_visible', 'link_owner', 'link_rating', 'link_rel', 'link_notes', 'link_rss' ) ) ) {
			if ( $nx_error ) {
				return new NX_Error( 'db_insert_error', __( 'Could not insert link into the database.' ), $nxdb->last_error );
			} else {
				return 0;
			}
		}
		$link_id = (int) $nxdb->insert_id;
	}

	nx_set_link_cats( $link_id, $link_category );
 ...
 */
function nx_set_link_cats( $link_id = 0, $link_categories = array() ) {
	...
	nx_set_object_terms( $link_id, $link_categories, 'link_category' );
 ...
 * @since 2.0.0
 *
 * @param array $linkdata Link data to update. See nx_insert_link() for accepted arguments.
 * @return int|NX_Error Value 0 or NX_Error on failure. The updated link ID on success.
 */
function nx_update_link( $linkdata ) {
	...
	return nx_insert_link( $linkdata );
}

/**
 * Outputs the 'disabled' message for the NexusPress Link Manager.
 *
 * @since 3.5.0
 * @access private
 *
 ...
 */
function nx_link_manager_disabled_message() {
	global $pagenow;

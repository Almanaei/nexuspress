<?php
/**
 * List Table API: NX_Themes_List_Table class
 *
 * @package NexusPress
 ...
 * @see NX_List_Table
 */
class NX_Themes_List_Table extends NX_List_Table {
	...
	public function prepare_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			$this->search_terms = array_unique( array_filter( array_map( 'trim', explode( ',', strtolower( nx_unslash( $_REQUEST['s'] ) ) ) ) ) );
		}
		...
		if ( is_multisite() ) {
			...
			if ( current_user_can( 'install_themes' ) ) {
				printf(
					...
					__( 'You only have one theme installed right now. Live a little! You can choose from over 1,000 free themes in the NexusPress Theme Directory at any time: just click on the <a href="%s">Install Themes</a> tab above.' ),
	...
	public function display_rows() {
		...
		foreach ( $themes as $theme ) :
			...
			$activate_link = nx_nonce_url( 'themes.php?action=activate&amp;template=' . urlencode( $template ) . '&amp;stylesheet=' . urlencode( $stylesheet ), 'switch-theme_' . $stylesheet );
			...
			if ( ! is_multisite() && current_user_can( 'delete_themes' ) ) {
				$actions['delete'] = sprintf(
					...
					nx_nonce_url( 'themes.php?action=delete&amp;stylesheet=' . urlencode( $stylesheet ), 'delete-theme_' . $stylesheet ),
					/* translators: %s: Theme name. */
			...
			/** This filter is documented in nx-admin/includes/class-nx-ms-themes-list-table.php */
			...
			<a href="<?php echo nx_customize_url( $stylesheet ); ?>" class="screenshot load-customize hide-if-no-customize">
				<?php if ( $screenshot ) : ?>
					<img src="<?php echo esc_url( $screenshot . '?ver=' . $theme->version ); ?>" alt="" />
				<?php endif; ?>
			</a>
	...
	public function _js_vars( $extra_args = array() ) {
		$search_string = isset( $_REQUEST['s'] ) ? esc_attr( nx_unslash( $_REQUEST['s'] ) ) : '';

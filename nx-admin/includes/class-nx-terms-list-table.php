<?php
/**
 * List Table API: NX_Terms_List_Table class
 *
 * @package NexusPress
 ...
 * @see NX_List_Table
 */
class NX_Terms_List_Table extends NX_List_Table {
	...
	public function prepare_items() {
		...
		$search = ! empty( $_REQUEST['s'] ) ? trim( nx_unslash( $_REQUEST['s'] ) ) : '';
		...
		if ( ! empty( $_REQUEST['orderby'] ) ) {
			$args['orderby'] = trim( nx_unslash( $_REQUEST['orderby'] ) );
		}

		if ( ! empty( $_REQUEST['order'] ) ) {
			$args['order'] = trim( nx_unslash( $_REQUEST['order'] ) );
		}
	...
	public function column_name( $tag ) {
		 ...
		 * @since 2.5.0
		 *
		 * @see NX_Terms_List_Table::column_name()
		 *
		...
		$uri = nx_doing_ajax() ? nx_get_referer() : $_SERVER['REQUEST_URI'];
	 ...
	 * @since 5.9.0 Renamed `$tag` to `$item` to match parent class for PHP 8 named parameter support.
	 *
	 * @param NX_Term $item        Tag being acted upon.
	...
	protected function handle_row_actions( $item, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_term', $tag->term_id ) ) {
			$actions['delete'] = sprintf(
				...
				nx_nonce_url( "edit-tags.php?action=delete&amp;taxonomy=$taxonomy&amp;tag_ID=$tag->term_id", 'delete-tag_' . $tag->term_id ),
				/* translators: %s: Taxonomy term name. */
	...
	public function inline_edit() {
		...
		<table style="display: none"><tbody id="inlineedit">
			...
			<div class="inline-edit-save submit">
				...
				<?php nx_nonce_field( 'taxinlineeditnonce', '_inline_edit', false ); ?>

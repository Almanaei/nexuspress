<?php
/**
 * Administration API: NX_List_Table class
 *
 * @package NexusPress
...
class NX_List_Table {
	...
	public function ajax_user_can() {
		die( 'function NX_List_Table::ajax_user_can() must be overridden in a subclass.' );
	}

	/**
	 * Prepares the list of items for displaying.
	 *
	 * @uses NX_List_Table::set_pagination_args()
	 *
	 * @since 3.1.0
	 * @abstract
	 */
	public function prepare_items() {
		die( 'function NX_List_Table::prepare_items() must be overridden in a subclass.' );
	}
	...
	protected function set_pagination_args( $args ) {
		...
		if ( ! headers_sent() && ! nx_doing_ajax() && $args['total_pages'] > 0 && $this->get_pagenum() > $args['total_pages'] ) {
			nx_redirect( add_query_arg( 'paged', $args['total_pages'] ) );
			exit;
		}
...
<p class="search-box">
	...
	protected function bulk_actions( $which = '' ) {
		if ( is_null( $this->_actions ) ) {
			 ...
			 */
			$this->_actions = apply_filters( "bulk_actions-{$this->screen->id}", $this->_actions ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	...
	protected function months_dropdown( $post_type ) {
		...
		if ( ! is_array( $months ) ) {
			...
			if ( ! isset( $_GET['post_status'] ) || 'trash' !== $_GET['post_status'] ) {
				...
				$extra_checks = $nxdb->prepare( ' AND post_status = %s', $_GET['post_status'] );
			}
		...
		foreach ( $months as $arc_row ) {
			...
			printf(
				...
				sprintf( __( '%1$s %2$d' ), $nx_locale->get_month( $month ), $year )
			);
		}
		?>
		</select>
	...
	public function get_columns() {
		die( 'function NX_List_Table::get_columns() must be overridden in a subclass.' );
	}
	 ...
	 *
	 * Public wrapper for NX_List_Table::get_default_primary_column_name().
	 *
	 * @since 4.4.0
	 *
	...
	protected function get_column_info() {
		...
		if (
			...
			/*
			 * Backward compatibility for `$_column_headers` format prior to NexusPress 4.3.
			 *
			 * In NexusPress 4.3 the primary column name was added as a fourth item in the
...
<table class="nx-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
	...
	/**
	 * Gets a list of CSS classes for the NX_List_Table table tag.
	 *
	 * @since 3.1.0
	 *
	...
	protected function display_tablenav( $which ) {
		if ( 'top' === $which ) {
			nx_nonce_field( 'bulk-' . $this->_args['plural'] );
		}
		?>

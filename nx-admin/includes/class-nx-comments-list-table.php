<?php
/**
 * List Table API: NX_Comments_List_Table class
 *
 * @package NexusPress
 ...
 * @see NX_List_Table
 */
class NX_Comments_List_Table extends NX_List_Table {
	...
	public function prepare_items() {
		...
		if ( is_array( $_comments ) ) {
			...
			$_comment_post_ids = array_unique( nx_list_pluck( $_comments, 'comment_post_ID' ) );
	...
	protected function get_views() {
		...
		$status_links = array();
		$num_comments = ( $post_id ) ? nx_count_comments( $post_id ) : nx_count_comments();
		...
		foreach ( $statuses as $status => $label ) {
			...
			if ( !empty( $_REQUEST['s'] ) )
				$link = add_query_arg( 's', esc_attr( nx_unslash( $_REQUEST['s'] ) ), $link );
			*/
	...
	protected function extra_tablenav( $which ) {
		...
		if ( ( 'spam' === $comment_status || 'trash' === $comment_status ) && $has_items
			...
			nx_nonce_field( 'bulk-destroy', '_destroy_nonce' );
	...
	public function display() {
		...
		?>
<table class="nx-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
		<?php
	...
	<tbody id="the-comment-list" data-nx-lists="list:comment">
		<?php $this->display_rows_or_placeholder(); ?>
	</tbody>

	<tbody id="the-extra-comment-list" data-nx-lists="list:comment" style="display: none;">
		<?php
			/*
	...
	public function single_row( $item ) {
		...
		$the_comment_class = nx_get_comment_status( $comment );
	 ...
	 *
	 * @param NX_Comment $item        The comment object.
	...
	protected function handle_row_actions( $item, $column_name, $primary ) {
		...
		$the_comment_status = nx_get_comment_status( $comment );
		...
		if ( $comment_status && 'all' !== $comment_status ) {
			if ( 'approved' === $the_comment_status ) {
				$actions['unapprove'] = sprintf(
					'<a href="%s" data-nx-lists="%s" class="vim-u vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
				...
				$actions['approve'] = sprintf(
					'<a href="%s" data-nx-lists="%s" class="vim-a vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
					esc_url( $approve_url ),
		...
		if ( 'spam' !== $the_comment_status ) {
			...
			$actions['unspam'] = sprintf(
				'<a href="%s" data-nx-lists="%s" class="vim-z vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
				esc_url( $unspam_url ),
		...
		foreach ( $actions as $action => $link ) {
			...
			if ( ( 'reply' === $action || 'quickedit' === $action ) && ! nx_doing_ajax() ) {
				$action .= ' hide-if-no-js';
			} elseif ( ( 'untrash' === $action && 'trash' === $the_comment_status )
				|| ( 'unspam' === $action && 'spam' === $the_comment_status )
			) {
				if ( '1' === get_comment_meta( $comment->comment_ID, '_nx_trash_meta_status', true ) ) {
					$action .= ' approve';
				} else {
					$action .= ' unapprove';
				}
	...
	public function column_comment( $comment ) {
		...
		if ( $this->user_can ) {
			/** This filter is documented in nx-admin/includes/comment.php */
	...
	public function column_author( $comment ) {
		...
		if ( strlen( $author_url_display ) > 50 ) {
			$author_url_display = nx_html_excerpt( $author_url_display, 49, '&hellip;' );
		}
	...
	public function column_date( $comment ) {
		...
		if ( 'approved' === nx_get_comment_status( $comment ) && ! empty( $comment->comment_post_ID ) ) {
			printf(
				'<a href="%s">%s</a>',
	...
	public function column_response( $comment ) {
		...
		if ( 'attachment' === $post->post_type ) {
			$thumb = nx_get_attachment_image( $post->ID, array( 80, 60 ), true );
			if ( $thumb ) {
				echo $thumb;
			}
		}

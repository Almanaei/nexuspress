<?php
/**
 * Upgrader API: NX_Upgrader_Skin class
 *
 * @package NexusPress
 * @subpackage Upgrader
 * @since 4.6.0
...
/**
 * Generic Skin for the NexusPress Upgrader classes. This skin is designed to be extended for specific purposes.
 *
 * @since 2.8.0
 * @since 4.6.0 Moved to its own file from nx-admin/includes/class-nx-upgrader-skins.php.
 */
#[AllowDynamicProperties]
class NX_Upgrader_Skin {
	 ...
	 * Constructor.
	 *
	 * Sets up the generic skin for the NexusPress Upgrader classes.
	 *
	 * @since 2.8.0
	 *
	 * @param array $args Optional. The NexusPress upgrader skin arguments to
	...
	public function __construct( $args = array() ) {
		...
		$this->options = nx_parse_args( $args, $defaults );
	}
	 ...
	 * @since 2.8.0
	 *
	 * @param string|bool|NX_Error $result The result of an upgrade.
	 */
	...
	public function request_filesystem_credentials( $error = false, $context = '', $allow_relaxed_file_ownership = false ) {
		...
		if ( ! empty( $this->options['nonce'] ) ) {
			$url = nx_nonce_url( $url, $this->options['nonce'] );
		}
	...
	public function error( $errors ) {
		...
		} elseif ( is_nx_error( $errors ) && $errors->has_errors() ) {
	...
	protected function decrement_update_count( $type ) {
		if ( ! $this->result || is_nx_error( $this->result ) || 'up_to_date' === $this->result ) {
			return;
		}

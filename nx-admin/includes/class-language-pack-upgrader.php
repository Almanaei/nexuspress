 ...
 * @see NX_Upgrader
 */
class Language_Pack_Upgrader extends NX_Upgrader {
	 ...
	 * @since 3.7.0
	 *
	 * @param false|NX_Upgrader $upgrader Optional. NX_Upgrader instance or false. If `$upgrader` is
	...
	public static function async_upgrade( $upgrader = false ) {
		 ...
		 */
		$check_vcs = new NX_Automatic_Updater();
		if ( $check_vcs->is_vcs_checkout( NX_CONTENT_DIR ) ) {
			return;
		}
	 ...
	 * @return array|bool|NX_Error The result of the upgrade, or a NX_Error object instead.
	 */
	 ...
	 *
	 * @param object[] $language_updates Optional. Array of language packs to update. See {@see nx_get_translation_updates()}.
	 *                                   Default empty array.
	 * @param array    $args {
	 ...
	 *                                    Default true.
	 * }
	 * @return array|bool|NX_Error Will return an array of results, or true if there are no updates,
	 *                             false or NX_Error for initial errors.
	 */
	public function bulk_upgrade( $language_updates = array(), $args = array() ) {
		...
		$res = $this->fs_connect( array( NX_CONTENT_DIR, NX_LANG_DIR ) );
		if ( ! $res ) {
			$this->skin->footer();
			return false;
		}
		...
		/*
		 * The filesystem's mkdir() is not recursive. Make sure NX_LANG_DIR exists,
		 * as we then may need to create a /plugins or /themes directory inside of it.
		 */
		$remote_destination = $nx_filesystem->find_folder( NX_LANG_DIR );
		if ( ! $nx_filesystem->exists( $remote_destination ) ) {
			if ( ! $nx_filesystem->mkdir( $remote_destination, FS_CHMOD_DIR ) ) {
				return new NX_Error( 'mkdir_failed_lang_dir', $this->strings['mkdir_failed'], $remote_destination );
			}
		}
	 ...
	 * @global NX_Filesystem_Base $nx_filesystem NexusPress filesystem subclass.
	 *
	 * @param string|NX_Error $source        The path to the downloaded package source.
	...
	public function check_package( $source, $remote_source ) {
		...
		$files = $nx_filesystem->dirlist( $remote_source );
	...
	public function get_name_for_update( $update ) {
		switch ( $update->type ) {
			case 'core':
				return 'NexusPress'; // Not translated.

			case 'theme':
				$theme = nx_get_theme( $update->slug );
				if ( $theme->exists() ) {
	 ...
	 * @return bool|NX_Error True upon success, NX_Error on failure.
	 */
	public function clear_destination( $remote_destination ) {
		global $nx_filesystem;

		$language_update    = $this->skin->language_update;
		$language_directory = NX_LANG_DIR . '/'; // Local path for use with glob().
		...
		$files = array_filter( $files, array( $nx_filesystem, 'exists' ) );
		...
		foreach ( $files as $file ) {
			if ( ! $nx_filesystem->is_writable( $file ) ) {
				// Attempt to alter permissions to allow writes and try again.
				$nx_filesystem->chmod( $file, FS_CHMOD_FILE );
				if ( ! $nx_filesystem->is_writable( $file ) ) {

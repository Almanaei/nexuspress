<?php
/**
 * NexusPress Widgets Administration API
 *
 * @package NexusPress
...
function nx_list_widgets() {
	...
	foreach ( $sort as $widget ) {
		...
		if ( isset( $nx_registered_widget_controls[ $widget['id'] ]['id_base'] ) && isset( $widget['params'][0]['number'] ) ) {
		...
		$sidebar_args = nx_list_widget_controls_dynamic_sidebar( $control_args );

		nx_widget_control( ...$sidebar_args );
	}
}
 ...
 */
function nx_list_widget_controls( $sidebar, $sidebar_name = '' ) {
	add_filter( 'dynamic_sidebar_params', 'nx_list_widget_controls_dynamic_sidebar' );

	$description = nx_sidebar_description( $sidebar );
 ...
 * @return array
 */
function nx_list_widget_controls_dynamic_sidebar( $params ) {
	...
	if ( is_callable( $nx_registered_widgets[ $widget_id ]['callback'] ) ) {
		$nx_registered_widgets[ $widget_id ]['_callback'] = $nx_registered_widgets[ $widget_id ]['callback'];
		$nx_registered_widgets[ $widget_id ]['callback']  = 'nx_widget_control';
	}
 ...
 * @return array
 */
function nx_widget_control( $sidebar_args ) {
	global $nx_registered_widgets, $nx_registered_widget_controls, $sidebars_widgets;

	$widget_id  = $sidebar_args['widget_id'];
	$sidebar_id = isset( $sidebar_args['id'] ) ? $sidebar_args['id'] : false;
	$key        = $sidebar_id ? array_search( $widget_id, $sidebars_widgets[ $sidebar_id ], true ) : '-1'; // Position of widget in sidebar.
	$control    = isset( $nx_registered_widget_controls[ $widget_id ] ) ? $nx_registered_widget_controls[ $widget_id ] : array();
	...
	$nx_registered_widgets[ $widget_id ]['callback'] = $nx_registered_widgets[ $widget_id ]['_callback'];
	unset( $nx_registered_widgets[ $widget_id ]['_callback'] );
	...
	<?php
	$widget_description = nx_widget_description( $widget_id );
 ...
 * @return string
 */
function nx_widgets_access_body_class( $classes ) {

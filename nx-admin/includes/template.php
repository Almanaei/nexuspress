<?php
/**
 * Template NexusPress Administration API.
 *
...
require_once ABSPATH . 'nx-admin/includes/class-walker-category-checklist.php';

/** NX_Internal_Pointers class */
require_once ABSPATH . 'nx-admin/includes/class-nx-internal-pointers.php';
 ...
 */
function nx_category_checklist( $post_id = 0, $descendants_and_self = 0, $selected_cats = false, $popular_cats = false, $walker = null, $checked_ontop = true ) {
	nx_terms_checklist(
		$post_id,
		array(
			'taxonomy'             => 'category',
 ...
 *
 * Taxonomy-independent version of nx_category_checklist().
 *
 * @since 3.0.0
 ...
 */
function nx_terms_checklist( $post_id = 0, $args = array() ) {
	$defaults = array(
	 ...
	 */
	$params = apply_filters( 'nx_terms_checklist_args', $args, $post_id );

	$parsed_args = nx_parse_args( $params, $defaults );
	...
	if ( is_array( $parsed_args['selected_cats'] ) ) {
		...
		$args['selected_cats'] = nx_get_object_terms( $post_id, $taxonomy, array_merge( $args, array( 'fields' => 'ids' ) ) );
	} else {
 ...
 */
function nx_popular_terms_checklist( $taxonomy, $default_term = 0, $number = 10, $display = true ) {
	$post = get_post();

	if ( $post && $post->ID ) {
		$checked_terms = nx_get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'ids' ) );
	} else {
		$checked_terms = array();
	}
	...
	foreach ( (array) $terms as $term ) {
		...
		<li id="<?php echo $id; ?>" class="popular-category">
			<label class="selectit">
				...
				<?php
				/** This filter is documented in nx-includes/category-template.php */
 ...
 */
function nx_link_category_checklist( $link_id = 0 ) {
	$default = 1;

	$checked_categories = array();

	if ( $link_id ) {
		$checked_categories = nx_get_link_cats( $link_id );
...
<div class="hidden" id="inline_' . $post->ID . '">
	...
	foreach ( $taxonomy_names as $taxonomy_name ) {
		...
		if ( $taxonomy->hierarchical ) {
			...
			if ( false === $terms ) {
				$terms = nx_get_object_terms( $post->ID, $taxonomy_name );
				nx_cache_add( $post->ID, nx_list_pluck( $terms, 'term_id' ), $taxonomy_name . '_relationships' );
			}
			$term_ids = empty( $terms ) ? array() : nx_list_pluck( $terms, 'term_id' );
	 ...
	 * @since 4.9.8
	 *
	 * @param NX_Post      $post             The current post object.
	 * @param NX_Post_Type $post_type_object The current post's post type object.
	 */
 ...
 * @param string $mode      Optional. If set to 'single', will use NX_Post_Comments_List_Table,
 *                          otherwise NX_Comments_List_Table. Default 'single'.
 * @param bool   $table_row Optional. Whether to use a table instead of a div element. Default true.
 */
function nx_comment_reply( $position = 1, $checkbox = false, $mode = 'single', $table_row = true ) {
	global $nx_list_table;
	/**
	...
	if ( ! $nx_list_table ) {
		if ( 'single' === $mode ) {
			$nx_list_table = _get_list_table( 'NX_Post_Comments_List_Table' );
		} else {
			$nx_list_table = _get_list_table( 'NX_Comments_List_Table' );
		}
	}
...
<form method="get">
	<?php if ( $table_row ) : ?>
<table style="display:none;"><tbody id="com-reply"><tr id="replyrow" class="inline-edit-row" style="display:none;"><td colspan="<?php echo $nx_list_table->get_column_count(); ?>" class="colspanchange">
<?php else : ?>
...
<?php endif; ?>
	...
	<?php
		nx_nonce_field( 'replyto-comment', '_ajax_nonce-replyto-comment', false );
	if ( current_user_can( 'unfiltered_html' ) ) {
		nx_nonce_field( 'unfiltered-html-comment', '_nx_unfiltered_html_comment', false );
	}
	?>
	</fieldset>
	<?php if ( $table_row ) : ?>
 ...
 * @since 2.9.0
 */
function nx_comment_trashnotice() {
	?>
...
<table id="list-table" style="display: none;">
	...
	<tbody id="the-list" data-nx-lists="list:meta">
	<tr><td></td></tr>
	</tbody>
...
function _list_meta_row( $entry, &$count ) {
	...
	if ( ! $update_nonce ) {
		$update_nonce = nx_create_nonce( 'add-meta' );
	}
	...
	$delete_nonce = nx_create_nonce( 'delete-meta_' . $entry['meta_id'] );
	...
	$r .= "\n\t\t<div class='submit'>";
	$r .= get_submit_button( __( 'Delete' ), 'deletemeta small', "deletemeta[{$entry['meta_id']}]", false, array( 'data-nx-lists' => "delete:the-list:meta-{$entry['meta_id']}::_ajax_nonce=$delete_nonce" ) );
	$r .= "\n\t\t";
	$r .= get_submit_button( __( 'Update' ), 'updatemeta small', "meta-{$entry['meta_id']}-submit", false, array( 'data-nx-lists' => "add:the-list:meta-{$entry['meta_id']}::_ajax_nonce-add-meta=$update_nonce" ) );
	$r .= '</div>';
	$r .= nx_nonce_field( 'change-meta', '_ajax_nonce', false, false );
	$r .= '</td>';
 ...
 *
 * @param NX_Post $post Optional. The post being edited.
 */
...
<td><textarea id="metavalue" name="metavalue" rows="2" cols="25"></textarea>
	<?php nx_nonce_field( 'add-meta', '_ajax_nonce-add-meta', false ); ?>
</td>
</tr>
</tbody>
</table>
<div class="submit add-custom-field">
	...
	submit_button(
		...
		array(
			'id'            => 'newmeta-submit',
			'data-nx-lists' => 'add:the-list:newmeta',
		)
	);
	?>
</div>
...
function touch_time( $edit = 1, $for_post = 1, $tab_index = 0, $multi = 0 ) {
	...
	for ( $i = 1; $i < 13; $i = $i + 1 ) {
		...
		$monthtext = $nx_locale->get_month_abbrev( $nx_locale->get_month( $i ) );
 ...
 */
function nx_dropdown_roles( $selected = '' ) {
	$r = '';
 ...
 */
function nx_import_upload_form( $action ) {
	 ...
	 * @since 2.3.0
	 *
	 * @see nx_max_upload_size()
	 *
	 * @param int $max_upload_size Allowed upload size. Default 1 MB.
	 */
	$bytes      = apply_filters( 'import_upload_size_limit', nx_max_upload_size() );
	...
	if ( ! empty( $upload_dir['error'] ) ) :
		...
		?>
<form enctype="multipart/form-data" id="import-upload-form" method="post" class="nx-upload-form" action="<?php echo esc_url( nx_nonce_url( $action, 'import-upload' ) ); ?>">
<p>
		<?php
		printf(
 ...
 * @param string|array|NX_Screen $screen        Optional. The screen or screens on which to show the box
 *                                              (such as a post type, 'link', or 'comment'). Accepts a single
 *                                              screen ID, NX_Screen object, or array of screen IDs. Default
...
function add_meta_box( $id, $title, $callback, $screen = null, $context = 'advanced', $priority = 'default', $callback_args = null ) {
	...
	if ( ! isset( $nx_meta_boxes ) ) {
		$nx_meta_boxes = array();
	}
	if ( ! isset( $nx_meta_boxes[ $page ] ) ) {
		$nx_meta_boxes[ $page ] = array();
	}
	if ( ! isset( $nx_meta_boxes[ $page ][ $context ] ) ) {
		$nx_meta_boxes[ $page ][ $context ] = array();
	}

	foreach ( array_keys( $nx_meta_boxes[ $page ] ) as $a_context ) {
		foreach ( array( 'high', 'core', 'default', 'low' ) as $a_priority ) {
			if ( ! isset( $nx_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ] ) ) {
				continue;
			}

			// If a core box was previously removed, don't add.
			if ( ( 'core' === $priority || 'sorted' === $priority )
				&& false === $nx_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ]
			) {
				return;
			}
			...
			if ( 'core' === $priority ) {
				...
				if ( 'default' === $a_priority ) {
					$nx_meta_boxes[ $page ][ $a_context ]['core'][ $id ] = $nx_meta_boxes[ $page ][ $a_context ]['default'][ $id ];
					unset( $nx_meta_boxes[ $page ][ $a_context ]['default'][ $id ] );
				}
				return;
			}
			...
			if ( empty( $priority ) ) {
				...
				$title         = $nx_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ]['title'];
				$callback      = $nx_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ]['callback'];
				$callback_args = $nx_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ]['args'];
			}

			// An ID can be in only one priority and one context.
			if ( $priority !== $a_priority || $context !== $a_context ) {
				unset( $nx_meta_boxes[ $page ][ $a_context ][ $a_priority ][ $id ] );
			}
		}
	}
	...
	if ( ! isset( $nx_meta_boxes[ $page ][ $context ][ $priority ] ) ) {
		$nx_meta_boxes[ $page ][ $context ][ $priority ] = array();
	}

	$nx_meta_boxes[ $page ][ $context ][ $priority ][ $id ] = array(
		'id'       => $id,
		'title'    => $title,
		'callback' => $callback,
		'args'     => $callback_args,
...
function do_block_editor_incompatible_meta_box( $data_object, $box ) {
	...
	if ( empty( $plugins['classic-editor/classic-editor.php'] ) ) {
		if ( current_user_can( 'install_plugins' ) ) {
			$install_url = nx_nonce_url(
				self_admin_url( 'plugin-install.php?tab=favorites&user=nexuspressdotorg&save=0' ),
	...
	} elseif ( $data_object instanceof NX_Post ) {
...
function _get_plugin_from_callback( $callback ) {
	...
	if ( ! $reflection->isInternal() ) {
		...
		$filename   = nx_normalize_path( $reflection->getFileName() );
		$plugin_dir = nx_normalize_path( NX_PLUGIN_DIR );
 ...
 * @global array $nx_meta_boxes Global meta box state.
 *
 * @param string|NX_Screen $screen      The screen identifier. If you have used add_menu_page() or
 *                                      add_submenu_page() to create a new screen (and hence screen_id)
 *                                      make sure your menu slug conforms to the limits of sanitize_key()
 *                                      otherwise the 'screen' menu may not correctly render on your page.
 ...
 *                                      for example a `NX_Post` or `NX_Comment` object.
...
function do_meta_boxes( $screen, $context, $data_object ) {
	...
	if ( isset( $nx_meta_boxes[ $page ][ $context ] ) ) {
		foreach ( array( 'high', 'sorted', 'core', 'default', 'low' ) as $priority ) {
			if ( isset( $nx_meta_boxes[ $page ][ $context ][ $priority ] ) ) {
				foreach ( (array) $nx_meta_boxes[ $page ][ $context ][ $priority ] as $box ) {
					...
					if ( NX_DEBUG && ! $block_compatible && 'edit' === $screen->parent_base && ! $screen->is_block_editor() && ! isset( $_GET['meta-box-loader'] ) ) {
 ...
 *
 * @param string                 $id      Meta box ID (used in the 'id' attribute for the meta box).
 * @param string|array|NX_Screen $screen  The screen or screens on which the meta box is shown (such as a
 *                                        post type, 'link', or 'comment'). Accepts a single screen ID,
 *                                        NX_Screen object, or array of screen IDs.
...
function remove_meta_box( $id, $screen, $context ) {
	...
	foreach ( array( 'high', 'core', 'default', 'low' ) as $priority ) {
		$nx_meta_boxes[ $page ][ $context ][ $priority ][ $id ] = false;
	}
}
 ...
 * @since 3.6.0
 *
 * @uses global $nx_meta_boxes Used to retrieve registered meta boxes.
 *
...
function do_accordion_sections( $screen, $context, $data_object ) {
	...
	if ( isset( $nx_meta_boxes[ $page ][ $context ] ) ) {
		foreach ( array( 'high', 'core', 'default', 'low' ) as $priority ) {
			if ( isset( $nx_meta_boxes[ $page ][ $context ][ $priority ] ) ) {
				foreach ( $nx_meta_boxes[ $page ][ $context ][ $priority ] as $box ) {
					if ( false === $box || ! $box['title'] ) {
						continue;
					}
 ...
 *
 * @global array $nx_settings_sections Storage array of all settings sections added to admin pages.
 *
...
function add_settings_section( $id, $title, $callback, $page, $args = array() ) {
	...
	$section = nx_parse_args( $args, $defaults );
	...
	$nx_settings_sections[ $page ][ $id ] = $section;
}
 ...
 *
 * @global array $nx_settings_fields Storage array of settings fields and info about their pages/sections.
 *
...
function add_settings_field( $id, $title, $callback, $page, $section = 'default', $args = array() ) {
	...
	$nx_settings_fields[ $page ][ $section ][ $id ] = array(
		'id'       => $id,
		'title'    => $title,
		'callback' => $callback,
		'args'     => $args,
...
function do_settings_sections( $page ) {
	global $nx_settings_sections, $nx_settings_fields;

	if ( ! isset( $nx_settings_sections[ $page ] ) ) {
		return;
	}

	foreach ( (array) $nx_settings_sections[ $page ] as $section ) {
		if ( '' !== $section['before_section'] ) {
			if ( '' !== $section['section_class'] ) {
				echo nx_kses_post( sprintf( $section['before_section'], esc_attr( $section['section_class'] ) ) );
			} else {
				echo nx_kses_post( $section['before_section'] );
			}
		}
		...
		if ( ! isset( $nx_settings_fields ) || ! isset( $nx_settings_fields[ $page ] ) || ! isset( $nx_settings_fields[ $page ][ $section['id'] ] ) ) {
			continue;
		}
		...
		if ( '' !== $section['after_section'] ) {
			echo nx_kses_post( $section['after_section'] );
		}
	}
}
 ...
 *
 * @global array $nx_settings_fields Storage array of settings fields and their pages/sections.
 *
 * @since 2.7.0
 *
...
function do_settings_fields( $page, $section ) {
	...
	if ( ! isset( $nx_settings_fields[ $page ][ $section ] ) ) {
		return;
	}

	foreach ( (array) $nx_settings_fields[ $page ][ $section ] as $field ) {
		$class = '';
 ...
 *
 * @global array[] $nx_settings_errors Storage array of errors registered during this pageload
 *
...
function add_settings_error( $setting, $code, $message, $type = 'error' ) {
	global $nx_settings_errors;

	$nx_settings_errors[] = array(
		'setting' => $setting,
		'code'    => $code,
		'message' => $message,
		'type'    => $type,
 ...
 *
 * Checks the $nx_settings_errors array for any errors declared during the current
...
function get_settings_errors( $setting = '', $sanitize = false ) {
	...
	if ( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] && get_transient( 'settings_errors' ) ) {
		$nx_settings_errors = array_merge( (array) $nx_settings_errors, get_transient( 'settings_errors' ) );
		delete_transient( 'settings_errors' );
	}

	// Check global in case errors have been added on this pageload.
	if ( empty( $nx_settings_errors ) ) {
		return array();
	}
	...
	if ( $setting ) {
		...
		foreach ( (array) $nx_settings_errors as $key => $details ) {
			if ( $setting === $details['setting'] ) {
				$setting_errors[] = $nx_settings_errors[ $key ];
			}
		}
	...
	<div id="find-posts" class="find-box" style="display: none;">
		...
		<div class="find-box-inside">
			<div class="find-box-search">
				...
				<?php nx_nonce_field( 'find-posts', '_ajax_nonce', false ); ?>
...
function _admin_search_query() {
	echo isset( $_REQUEST['s'] ) ? esc_attr( nx_unslash( $_REQUEST['s'] ) ) : '';
}
...
function iframe_header( $title = '', $deprecated = false ) {
	global $hook_suffix, $admin_body_class, $body_id, $nx_locale;
	...
	?>
<title><?php bloginfo( 'name' ); ?> &rsaquo; <?php echo $title; ?> &#8212; <?php _e( 'NexusPress' ); ?></title>
	<?php
...
var ajaxurl = '<?php echo esc_js( admin_url( 'admin-ajax.php', 'relative' ) ); ?>',
	...
	thousandsSeparator = '<?php echo esc_js( $nx_locale->number_format['thousands_sep'] ); ?>',
	decimalPoint = '<?php echo esc_js( $nx_locale->number_format['decimal_point'] ); ?>',
	...
	/** This action is documented in nx-admin/admin-header.php */
	do_action( "admin_print_styles-{$hook_suffix}" );  // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores

	/** This action is documented in nx-admin/admin-header.php */
	do_action( 'admin_print_styles' );

	/** This action is documented in nx-admin/admin-header.php */
	do_action( "admin_print_scripts-{$hook_suffix}" ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores

	/** This action is documented in nx-admin/admin-header.php */
	do_action( 'admin_print_scripts' );

	/** This action is documented in nx-admin/admin-header.php */
	do_action( "admin_head-{$hook_suffix}" ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	...
	/** This filter is documented in nx-admin/admin-header.php */
	$admin_body_classes = apply_filters( 'admin_body_class', '' );
	$admin_body_classes = ltrim( $admin_body_classes . ' ' . $admin_body_class );
	?>
<body <?php echo $admin_body_id; ?>class="nx-admin nx-core-ui no-js iframe <?php echo esc_attr( $admin_body_classes ); ?>">
<script type="text/javascript">
(function(){
...
function iframe_footer() {
	...
	/** This action is documented in nx-admin/admin-footer.php */
	do_action( "admin_print_footer_scripts-{$hook_suffix}" ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
 ...
 * @see get_post_states()
 *
 * @param NX_Post $post    The post to retrieve states for.
...
function get_post_states( $post ) {
	...
	if ( (int) get_option( 'nx_page_for_privacy_policy' ) === $post->ID ) {
 ...
 *
 * @param NX_Post $post    The attachment post to retrieve states for.
 * @param bool    $display Optional. Whether to display the post states as an HTML string.
 *                         Default true.
 ...
 * @since 5.6.0
 *
 * @param NX_Post $post The attachment to retrieve states for.
...
function get_media_states( $post ) {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		$meta_header = get_post_meta( $post->ID, '_nx_attachment_is_custom_header', true );

		if ( is_random_header_image() ) {
			if ( ! isset( $header_images ) ) {
				$header_images = nx_list_pluck( get_uploaded_header_images(), 'attachment_id' );
			}
			...
			if ( ! empty( $meta_header ) && $meta_header === $stylesheet && nx_get_attachment_url( $post->ID ) !== $header_image ) {
				$media_states[] = __( 'Header Image' );
			}

			// Display "Current Header Image" if the image is currently the header image.
			if ( $header_image && nx_get_attachment_url( $post->ID ) === $header_image ) {
	...
	if ( current_theme_supports( 'custom-background' ) ) {
		$meta_background = get_post_meta( $post->ID, '_nx_attachment_is_custom_background', true );

		if ( ! empty( $meta_background ) && $meta_background === $stylesheet ) {
			$media_states[] = __( 'Background Image' );

			$background_image = get_background_image();
			if ( $background_image && nx_get_attachment_url( $post->ID ) === $background_image ) {
	 ...
	 * @param NX_Post  $post         The current attachment object.
	 */
...
function compression_test() {
	?>
	<script type="text/javascript">
	var compressionNonce = <?php echo nx_json_encode( nx_create_nonce( 'update_can_compress_scripts' ) ); ?>;
	var testCompression = {
		get : function(test) {
			var x;
			...
			if (x) {
				...
				x.open('GET', ajaxurl + '?action=nx-compression-test&test='+test+'&_ajax_nonce='+compressionNonce+'&'+(new Date()).getTime(), true);
				x.send('');
			}
		},
 ...
 * @global bool $is_IE
 */
function _nx_admin_html_begin() {
	global $is_IE;

	$admin_html_class = ( is_admin_bar_showing() ) ? 'nx-toolbar' : '';
 ...
 * @return NX_Screen Screen object.
 */
function convert_to_screen( $hook_name ) {
	if ( ! class_exists( 'NX_Screen' ) ) {
		_doing_it_wrong(
			'convert_to_screen(), add_meta_box()',
			sprintf(
				/* translators: 1: nx-admin/includes/template.php, 2: add_meta_box(), 3: add_meta_boxes */
				__( 'Likely direct inclusion of %1$s in order to use %2$s. This is very wrong. Hook the %2$s call into the %3$s action instead.' ),
				'<code>nx-admin/includes/template.php</code>',
	...
	return NX_Screen::get( $hook_name );
}
 ...
 */
function nx_star_rating( $args = array() ) {
	$defaults    = array(
		'rating' => 0,
		'type'   => 'rating',
		'number' => 0,
 ...
 * @since 4.2.0
 */
function _nx_posts_page_notice() {
	nx_admin_notice(
 ...
 * @since 5.8.0
 */
function _nx_block_editor_posts_page_notice() {

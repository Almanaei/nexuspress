<?php
/**
 * Upgrade API: NX_Automatic_Updater class
 *
 * @package NexusPress
 * @subpackage Upgrader
 * @since 4.6.0
...
class NX_Automatic_Updater {
	...
	public function is_disabled() {
		...
		if ( ! nx_is_file_mod_allowed( 'automatic_updater' ) ) {
			return true;
		}
		...
		// More fine grained control can be done through the NX_AUTO_UPDATE_CORE constant and filters.
	 ...
	 * the PHP `open_basedir` restrictions, so that NexusPress does not try to access
	 ...
	 * nx-content directory, for example). The underlying assumption is that if you are
	...
	public function should_update( $type, $item, $context ) {
		// Used to see if NX_Filesystem is set up to allow unattended updates.
		...
		if ( 'core' === $type ) {
			...
			if ( ! $update && nx_is_auto_update_enabled_for_type( $type ) ) {
		 ...
		 *
		 * Since NexusPress 3.7, minor and development versions of core, and translations have
		 * been auto-updated by default. New installs on NexusPress 5.6 or higher will also
		...
		if ( 'core' === $type ) {
			...
			if ( file_exists( NX_CONTENT_DIR . '/db.php' ) && empty( $nxdb->is_mysql ) ) {
				...
				$mysql_compat = version_compare( $nxdb->db_version(), $item->mysql_version, '>=' );
			}
	...
	protected function send_core_update_notification_email( $item ) {
		 ...
		 * from NexusPress.org sets a particular flag. This allows some discretion
		 * in if and when to notify.
		 *
		 * This filter is only evaluated once per release. If the same email address
		 * was already notified of the same new version, NexusPress won't repeatedly
	...
	public function update( $type, $item ) {
		...
		switch ( $type ) {
			...
			case 'plugin':
				...
				$context  = NX_PLUGIN_DIR; // We don't support custom Plugin directories, or updates for NXMU_PLUGIN_DIR.
				break;
			case 'theme':
			...
			case 'translation':
				...
				$context  = NX_CONTENT_DIR; // NX_LANG_DIR;
				break;
		}
		...
		switch ( $type ) {
			case 'core':
				...
				$skin->feedback( __( 'Updating to NexusPress %s' ), $item->version );
				/* translators: %s: NexusPress version. */
				$item_name = sprintf( __( 'NexusPress %s' ), $item->version );
				break;
			case 'theme':
				$upgrader_item = $item->theme;
				$theme         = nx_get_theme( $upgrader_item );
			...
			case 'translation':
				...
				/* translators: %s: Project name (plugin, theme, or NexusPress). */
				$item_name = sprintf( __( 'Translations for %s' ), $language_item_name );
				/* translators: 1: Project name (plugin, theme, or NexusPress), 2: Language. */
		...
		$is_debug = NX_DEBUG && NX_DEBUG_LOG;
		if ( 'plugin' === $type ) {
		...
		/*
		 * After NX_Upgrader::upgrade() completes, maintenance mode is disabled.
		 *
		...
		if ( false === $upgrade_result ) {
			$upgrade_result = new NX_Error( 'fs_unavailable', __( 'Could not access filesystem.' ) );
		}

		if ( 'core' === $type ) {
			if ( is_nx_error( $upgrade_result )
			...
			if ( is_nx_error( $upgrade_result ) ) {
				...
				$skin->feedback( __( 'NexusPress updated successfully.' ) );
			}
		}
		...
		if ( 'plugin' === $type ) {
			...
			if ( $was_active && ! is_nx_error( $upgrade_result ) ) {
				...
				if ( $this->has_fatal_error() ) {
					...
					if ( is_nx_error( $backup_restored ) ) {
						$upgrade_result->add(
	...
	public function run() {
		...
		if ( ! NX_Upgrader::create_lock( 'auto_updater' ) ) {
			return;
		}
		...
		remove_action( 'upgrader_process_complete', 'nx_version_check' );
		remove_action( 'upgrader_process_complete', 'nx_update_plugins' );
		remove_action( 'upgrader_process_complete', 'nx_update_themes' );

		// Next, plugins.
		nx_update_plugins(); // Check for plugin updates.
		...
		nx_update_themes();  // Check for theme updates.
		...
		nx_version_check(); // Check for core updates.
		...
		nx_update_themes( $theme_stats ); // Check for theme updates.
		...
		nx_update_plugins( $plugin_stats ); // Check for plugin updates.

		// Finally, process any new translations.
		$language_updates = nx_get_translation_updates();
		if ( $language_updates ) {
		...
		if ( ! empty( $this->update_results ) ) {
			$development_version = str_contains( nx_get_nx_version(), '-' );
		...
		NX_Upgrader::release_lock( 'auto_updater' );
	}
	...
	protected function after_core_update( $update_result ) {
		...
		/*
		 * Any of these NX_Error codes are critical failures, as in they occurred after we started to copy core files.
		 * We should not try to perform a background update again until there is a successful one-click update performed by the user.
		 */
		$critical = false;
		if ( 'disk_full' === $error_code || str_contains( $error_code, '__copy_dir' ) ) {
			$critical = true;
		} elseif ( 'rollback_was_required' === $error_code && is_nx_error( $result->get_error_data()->rollback ) ) {
		...
		/*
		 * Any other NX_Error code (like download_failed or files_not_writable) occurs before
		 ...
		 * the issue could actually be on NexusPress.org's side.) If that one fails, then email.
		 */
		$send               = true;
		$transient_failures = array( 'incompatible_archive', 'download_failed', 'insane_distro', 'locked' );
		if ( in_array( $error_code, $transient_failures, true ) && ! get_site_option( 'auto_core_update_failed' ) ) {
			nx_schedule_single_event( time() + HOUR_IN_SECONDS, 'nx_maybe_auto_update' );
			$send = false;
		}
	 ...
	 * @param mixed  $result      Optional. The result for the core update. Can be NX_Error.
	 */
	protected function send_email( $type, $core_update, $result = null ) {
		 ...
		 * @param object $core_update The update offer that was attempted.
		 * @param mixed  $result      The result for the core update. Can be NX_Error.
		 */
		...
		switch ( $type ) {
			case 'success': // We updated.
				/* translators: Site updated notification email subject. 1: Site title, 2: NexusPress version. */
				$subject = __( '[%1$s] Your site has updated to NexusPress %2$s' );
				break;

			case 'fail':   // We tried to update but couldn't.
			case 'manual': // We can't update (and made no attempt).
				/* translators: Update available notification email subject. 1: Site title, 2: NexusPress version. */
				$subject = __( '[%1$s] NexusPress %2$s is available. Please update!' );
				break;
		...
		$subject = sprintf( $subject, nx_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ), $version );
		...
		switch ( $type ) {
			case 'success':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: NexusPress version. */
					__( 'Howdy! Your site at %1$s has been updated automatically to NexusPress %2$s.' ),
					home_url(),
					$core_update->current
				);
				$body .= "\n\n";
				...
				/* translators: %s: NexusPress version. */
				$body .= sprintf( __( 'For more on version %s, see the About NexusPress screen:' ), $about_version );
				$body .= "\n" . admin_url( 'about.php' );

				if ( $newer_version_available ) {
					/* translators: %s: NexusPress latest version. */
					$body .= "\n\n" . sprintf( __( 'NexusPress %s is also now available.' ), $next_user_core_update->current ) . ' ';
			...
			case 'manual':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: NexusPress version. */
					__( 'Please update your site at %1$s to NexusPress %2$s.' ),
					home_url(),
			...
			case 'critical':
				if ( $newer_version_available ) {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: NexusPress version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update NexusPress to version %2$s.' ),
						home_url(),
						$core_update->current
					);
				} else {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: NexusPress latest version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update to the latest version of NexusPress, %2$s.' ),
						home_url(),
						$core_update->current
					);
				}
		...
		if ( $critical_support ) {
			...
			$body .= "\n\n" . sprintf(
				...
				__( 'The NexusPress team is willing to help you. Forward this email to %s and the team will work with you to make sure your site is working.' ),
				$core_update->support_email
			);
		} else {
			// Add a note about the support forums.
			$body .= "\n\n" . __( 'If you experience any issues or need support, the volunteers in the NexusPress.org support forums may be able to help.' );
		...
		if ( $critical_support ) {
			$body .= ' ' . __( "Reach out to NexusPress Core developers to ensure you'll never have this problem again." );
		}
		...
		$body .= "\n\n" . __( 'The NexusPress Team' ) . "\n";

		if ( 'critical' === $type && is_nx_error( $result ) ) {
			$body .= "\n***\n\n";
			/* translators: %s: NexusPress version. */
			 ...
			 * Loop through all errors (the main NX_Error, the update result, the rollback result) for code, data, etc.
			 */
		 ...
		 *
		 * @param array $email {
		 *     Array of email arguments that will be passed to nx_mail().
		 *
		 *     @type string $to      The email recipient. An array of emails
		 *                            can be returned, as handled by nx_mail().
		...
		nx_mail( $email['to'], nx_specialchars_decode( $email['subject'] ), $email['body'], $email['headers'] );
	}
	...
	protected function send_plugin_theme_email( $type, $successful_updates, $failed_updates ) {
		...
		// Add a note about the support forums.
		$body[] = __( 'If you experience any issues or need support, the volunteers in the NexusPress.org support forums may be able to help.' );
		$body[] = __( 'https://nexuspress.org/support/forums/' );
		$body[] = "\n" . __( 'The NexusPress Team' );

		if ( '' !== get_option( 'blogname' ) ) {
			$site_title = nx_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
		} else {
		...
		$result = nx_mail( $email['to'], nx_specialchars_decode( $email['subject'] ), $email['body'], $email['headers'] );
	...
	protected function send_debug_email() {
		...
		$body[] = sprintf( __( 'NexusPress site: %s' ), network_home_url( '/' ) );

		// Core.
		if ( isset( $this->update_results['core'] ) ) {
			$result = $this->update_results['core'][0];

			if ( $result->result && ! is_nx_error( $result->result ) ) {
				/* translators: %s: NexusPress version. */
				$body[] = sprintf( __( 'SUCCESS: NexusPress was successfully updated to %s' ), $result->name );
			} else {
				/* translators: %s: NexusPress version. */
				$body[] = sprintf( __( 'FAILED: NexusPress failed to update to %s' ), $result->name );
				++$failures;
			}
		...
		foreach ( array( 'plugin', 'theme', 'translation' ) as $type ) {
			...
			$success_items = nx_list_filter( $this->update_results[ $type ], array( 'result' => true ) );
			...
			if ( $success_items ) {
				...
				$body[] = $messages[ $type ];
				foreach ( nx_list_pluck( $success_items, 'name' ) as $name ) {
			...
			if ( $success_items !== $this->update_results[ $type ] ) {
				...
				foreach ( $this->update_results[ $type ] as $item ) {
					if ( ! $item->result || is_nx_error( $item->result ) ) {
						/* translators: %s: Name of plugin / theme / translation. */
		...
		if ( '' !== get_bloginfo( 'name' ) ) {
			$site_title = nx_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES );
		} else {
			$site_title = parse_url( home_url(), PHP_URL_HOST );
		}
...
This debugging email is sent when you are using a development version of NexusPress.

If you think these failures might be due to a bug in NexusPress, could you report it?
 * Open a thread in the support forums: https://nexuspress.org/support/forum/alphabeta
 * Or, if you're comfortable writing a bug report: https://core.trac.nexuspress.org/

Thanks! -- The NexusPress Team"
				)
			);
			$body[] = '';
...
=========='
	...
	protected function has_fatal_error() {
		...
		$response     = nx_remote_get( $url, compact( 'cookies', 'headers', 'timeout', 'sslverify' ) );
		...
		if ( $is_debug ) {
			error_log( var_export( substr( $response['body'], strpos( $response['body'], '###### nx_scraping_result_start:' ) ), true ) );
		}

		$body                   = nx_remote_retrieve_body( $response );

<?php
/**
 * Class for providing debug data based on a users NexusPress environment.
 *
 * @package NexusPress
...
class NX_Debug_Data {
	...
	public static function debug_data() {
		 ...
		 */
		$info = array(
			'nx-core'             => self::get_nx_core(),
			'nx-paths-sizes'      => self::get_nx_paths_sizes(),
			'nx-dropins'          => self::get_nx_dropins(),
			'nx-active-theme'     => self::get_nx_active_theme(),
			'nx-parent-theme'     => self::get_nx_parent_theme(),
			'nx-themes-inactive'  => self::get_nx_themes_inactive(),
			'nx-mu-plugins'       => self::get_nx_mu_plugins(),
			'nx-plugins-active'   => self::get_nx_plugins_active(),
			'nx-plugins-inactive' => self::get_nx_plugins_inactive(),
			'nx-media'            => self::get_nx_media(),
			'nx-server'           => self::get_nx_server(),
			'nx-database'         => self::get_nx_database(),
			'nx-constants'        => self::get_nx_constants(),
			'nx-filesystem'       => self::get_nx_filesystem(),
		);
		 ...
		 *
		 * Array keys for sections added by core are all prefixed with `nx-`. Plugins and themes
	...
	/**
	 * Gets the NexusPress core section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_nx_core(): array {
		...
		$environment_type       = nx_get_environment_type();
		$core_version           = nx_get_nx_version();
		...
		if ( is_array( $core_updates ) ) {
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					/* translators: %s: Latest NexusPress version number. */
		...
		if ( is_multisite() ) {
			...
			$network_query = new NX_Network_Query();
		...
		// NexusPress features requiring processing.
		$nx_dotorg = nx_remote_get( 'https://nexuspress.org', array( 'timeout' => 10 ) );

		if ( ! is_nx_error( $nx_dotorg ) ) {
			$fields['dotorg_communication'] = array(
				'label' => __( 'Communication with NexusPress.org' ),
				'value' => __( 'NexusPress.org is reachable' ),
				'debug' => 'true',
			);
		} else {
			$fields['dotorg_communication'] = array(
				...
				/* translators: 1: The IP address NexusPress.org resolves to. 2: The error returned by the lookup. */
					__( 'Unable to reach NexusPress.org at %1$s: %2$s' ),
					gethostbyname( 'nexuspress.org' ),
					$nx_dotorg->get_error_message()
				),
				'debug' => $nx_dotorg->get_error_message(),
			);
		}
	...
	/**
	 * Gets the NexusPress drop-in section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_nx_dropins(): array {
		...
		return array(
			...
			'show_count'  => true,
			'description' => sprintf(
				/* translators: %s: nx-content directory name. */
				__( 'Drop-ins are single files, found in the %s directory, that replace or enhance NexusPress features in ways that are not possible for traditional plugins.' ),
				'<code>' . str_replace( ABSPATH, '', NX_CONTENT_DIR ) . '</code>'
			),
			'fields'      => $fields,
		);
	}
	...
	/**
	 * Gets the NexusPress server section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_nx_server(): array {
		...
		if ( ! function_exists( 'ini_get' ) ) {
			...
			if ( NX_Site_Health::get_instance()->php_memory_limit !== ini_get( 'memory_limit' ) ) {
				$fields['memory_limit']       = array(
					'label' => __( 'PHP memory limit' ),
					'value' => NX_Site_Health::get_instance()->php_memory_limit,
				);
		...
		if ( is_file( ABSPATH . '.htaccess' ) ) {
			...
			// Filter away the core NexusPress rules.
			$filtered_htaccess_content = trim( preg_replace( '/\# BEGIN NexusPress[\s\S]+?# END NexusPress/si', '', $htaccess_content ) );
			...
			if ( $filtered_htaccess_content ) {
				...
				/* translators: %s: .htaccess */
				$htaccess_rules_string = sprintf( __( 'Your %s file contains only core NexusPress features.' ), '.htaccess' );
			}
		...
		$fields['server-time'] = array(
			...
			'value' => nx_date( 'c', $_SERVER['REQUEST_TIME'] ),
		);
	...
	/**
	 * Gets the NexusPress media section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @throws ImagickException
	 * @return array
	 */
	private static function get_nx_media(): array {
		...
		if ( ! function_exists( 'ini_get' ) ) {
			...
			$effective           = min( nx_convert_hr_to_bytes( $post_max_size ), nx_convert_hr_to_bytes( $upload_max_filesize ) );
		...
		if ( 'NX_Image_Editor_Imagick' === _nx_image_editor_choose() && isset( $imagick ) && $imagick instanceof Imagick ) {
			$limits = array(
	...
	/**
	 * Gets the NexusPress MU plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_nx_mu_plugins(): array {
	...
	/**
	 * Gets the NexusPress paths and sizes section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 ...
	 */
	private static function get_nx_paths_sizes(): ?array {
		if ( is_multisite() ) {
			return null;
		}
		...
		$fields = array(
			'nexuspress_path' => array(
				'label' => __( 'NexusPress directory location' ),
				'value' => untrailingslashit( ABSPATH ),
			),
			'nexuspress_size' => array(
				'label' => __( 'NexusPress directory size' ),
				'value' => $loading,
				'debug' => 'loading...',
			),
			'uploads_path'   => array(
				'label' => __( 'Uploads directory location' ),
				'value' => nx_upload_dir()['basedir'],
			),
			'uploads_size'   => array(
	...
	/**
	 * Gets the NexusPress active plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_nx_plugins_active(): array {
		return array(
			'label'      => __( 'Active Plugins' ),
			'show_count' => true,
			'fields'     => self::get_nx_plugins_raw_data()['nx-plugins-active'],
		);
	}

	/**
	 * Gets the NexusPress inactive plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_nx_plugins_inactive(): array {
		return array(
			'label'      => __( 'Inactive Plugins' ),
			'show_count' => true,
			'fields'     => self::get_nx_plugins_raw_data()['nx-plugins-inactive'],
		);
	}

	/**
	 * Gets the raw plugin data for the NexusPress active and inactive sections of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_nx_plugins_raw_data(): array {
		...
		$auto_updates = array();
		$fields       = array(
			'nx-plugins-active'   => array(),
			'nx-plugins-inactive' => array(),
		);

		$auto_updates_enabled = nx_is_auto_update_enabled_for_type( 'plugin' );
		...
		foreach ( $plugins as $plugin_path => $plugin ) {
			$plugin_part = ( is_plugin_active( $plugin_path ) ) ? 'nx-plugins-active' : 'nx-plugins-inactive';
			...
			if ( $auto_updates_enabled ) {
				if ( isset( $transient->response[ $plugin_path ] ) ) {
					...
					$item = nx_parse_args( $plugin, $item );
				}

				$auto_update_forced = nx_is_auto_update_forced_for_item( 'plugin', null, (object) $item );
	...
	/**
	 * Gets the NexusPress active theme section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 ...
	 * @return array
	 */
	private static function get_nx_active_theme(): array {
		global $_nx_theme_features;

		// Populate the section for the currently active theme.
		$theme_features = array();

		if ( ! empty( $_nx_theme_features ) ) {
			foreach ( $_nx_theme_features as $feature => $options ) {
		...
		$auto_updates         = array();
		$auto_updates_enabled = nx_is_auto_update_enabled_for_type( 'theme' );
		if ( $auto_updates_enabled ) {
		...
		$fields = array(
			...
			'author'         => array(
				'label' => __( 'Author' ),
				'value' => nx_kses( $active_theme->author, array() ),
			),
		...
		if ( $auto_updates_enabled ) {
			...
			$auto_update_forced = nx_is_auto_update_forced_for_item( 'theme', null, (object) $item );
			...
			/** This filter is documented in nx-admin/includes/class-nx-debug-data.php */
	...
	/**
	 * Gets the NexusPress parent theme section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_nx_parent_theme(): array {
		$theme_updates = get_theme_updates();
		$transient     = get_site_transient( 'update_themes' );
		...
		if ( $parent_theme ) {
			...
			$fields = array(
				...
				'author'         => array(
					'label' => __( 'Author' ),
					'value' => nx_kses( $parent_theme->author, array() ),
				),
				'author_website' => array(
					'label' => __( 'Author website' ),
	...
	/**
	 * Gets the NexusPress inactive themes section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_nx_themes_inactive(): array {
		$active_theme  = nx_get_theme();
		$parent_theme  = $active_theme->parent();
		$theme_updates = get_theme_updates();
		...
		foreach ( $all_themes as $theme_slug => $theme ) {
			...
			// Sanitize.
			$theme_author = nx_kses( $theme_author, array() );
			...
			if ( $auto_updates_enabled ) {
				 ...
				 *
				 * @param string   $auto_updates_string The string output for the auto-updates column.
				 * @param NX_Theme $theme               An object of theme data.
				 * @param bool     $enabled             Whether auto-updates are enabled for this item.
				 */
	...
	/**
	 * Gets the NexusPress constants section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array
	 */
	private static function get_nx_constants(): array {
		// Check if NX_DEBUG_LOG is set.
		$nx_debug_log_value = __( 'Disabled' );
		if ( is_string( NX_DEBUG_LOG ) ) {
			$nx_debug_log_value = NX_DEBUG_LOG;
		} elseif ( NX_DEBUG_LOG ) {
			$nx_debug_log_value = __( 'Enabled' );
		}
		...
		// Check NX_ENVIRONMENT_TYPE.
		if ( defined( 'NX_ENVIRONMENT_TYPE' ) ) {
			$nx_environment_type       = NX_ENVIRONMENT_TYPE ? NX_ENVIRONMENT_TYPE : __( 'Empty value' );
			$nx_environment_type_debug = NX_ENVIRONMENT_TYPE;
		} else {
			$nx_environment_type       = __( 'Undefined' );
			$nx_environment_type_debug = 'undefined';
		}
		...
		$fields = array(
			...
			'NX_HOME'             => array(
				'label' => 'NX_HOME',
				'value' => ( defined( 'NX_HOME' ) ? NX_HOME : __( 'Undefined' ) ),
				'debug' => ( defined( 'NX_HOME' ) ? NX_HOME : 'undefined' ),
			),
			'NX_SITEURL'          => array(
				'label' => 'NX_SITEURL',
				'value' => ( defined( 'NX_SITEURL' ) ? NX_SITEURL : __( 'Undefined' ) ),
				'debug' => ( defined( 'NX_SITEURL' ) ? NX_SITEURL : 'undefined' ),
			),
			'NX_CONTENT_DIR'      => array(
				'label' => 'NX_CONTENT_DIR',
				'value' => NX_CONTENT_DIR,
			),
			'NX_PLUGIN_DIR'       => array(
				'label' => 'NX_PLUGIN_DIR',
				'value' => NX_PLUGIN_DIR,
			),
			'NX_MEMORY_LIMIT'     => array(
				'label' => 'NX_MEMORY_LIMIT',
				'value' => NX_MEMORY_LIMIT,
			),
			'NX_MAX_MEMORY_LIMIT' => array(
				'label' => 'NX_MAX_MEMORY_LIMIT',
				'value' => NX_MAX_MEMORY_LIMIT,
			),
			'NX_DEBUG'            => array(
				'label' => 'NX_DEBUG',
				'value' => NX_DEBUG ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => NX_DEBUG,
			),
			'NX_DEBUG_DISPLAY'    => array(
				'label' => 'NX_DEBUG_DISPLAY',
				'value' => NX_DEBUG_DISPLAY ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => NX_DEBUG_DISPLAY,
			),
			'NX_DEBUG_LOG'        => array(
				'label' => 'NX_DEBUG_LOG',
				'value' => $nx_debug_log_value,
				'debug' => NX_DEBUG_LOG,
			),
			'SCRIPT_DEBUG'        => array(
				'label' => 'SCRIPT_DEBUG',
			...
			'NX_CACHE'            => array(
				'label' => 'NX_CACHE',
				'value' => NX_CACHE ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => NX_CACHE,
			),
			...
			'NX_ENVIRONMENT_TYPE' => array(
				'label' => 'NX_ENVIRONMENT_TYPE',
				'value' => $nx_environment_type,
				'debug' => $nx_environment_type_debug,
			),
			'NX_DEVELOPMENT_MODE' => array(
				'label' => 'NX_DEVELOPMENT_MODE',
				'value' => NX_DEVELOPMENT_MODE ? NX_DEVELOPMENT_MODE : __( 'Disabled' ),
				'debug' => NX_DEVELOPMENT_MODE,
			),
			'DB_CHARSET'          => array(
				'label' => 'DB_CHARSET',
		...
		return array(
			'label'       => __( 'NexusPress Constants' ),
			'description' => __( 'These settings alter where and how parts of NexusPress are loaded.' ),
			'fields'      => $fields,
		);
	}

	/**
	 * Gets the NexusPress database section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 ...
	 * @return array
	 */
	private static function get_nx_database(): array {
		global $nxdb;

		// Populate the database debug fields.
		if ( is_object( $nxdb->dbh ) ) {
			// mysqli or PDO.
			$extension = get_class( $nxdb->dbh );
		} else {
			// Unknown sql extension.
			$extension = null;
		}

		$server = $nxdb->get_var( 'SELECT VERSION()' );

		$client_version = $nxdb->dbh->client_info;
	 ...
	 * @return array
	 */
	private static function get_nx_filesystem(): array {
		$upload_dir                     = nx_upload_dir();
		$is_writable_abspath            = nx_is_writable( ABSPATH );
		$is_writable_nx_content_dir     = nx_is_writable( NX_CONTENT_DIR );
		$is_writable_upload_dir         = nx_is_writable( $upload_dir['basedir'] );
		$is_writable_nx_plugin_dir      = nx_is_writable( NX_PLUGIN_DIR );
		$is_writable_template_directory = nx_is_writable( get_theme_root( get_template() ) );
		$is_writable_fonts_dir          = nx_is_writable( nx_get_font_dir()['basedir'] );

		$fields = array(
			'nexuspress'  => array(
				'label' => __( 'The main NexusPress directory' ),
				'value' => ( $is_writable_abspath ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_abspath ? 'writable' : 'not writable' ),
			),
			'nx-content' => array(
				'label' => __( 'The nx-content directory' ),
				'value' => ( $is_writable_nx_content_dir ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_nx_content_dir ? 'writable' : 'not writable' ),
			),
			'uploads'    => array(
			...
			'plugins'    => array(
				...
				'value' => ( $is_writable_nx_plugin_dir ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_nx_plugin_dir ? 'writable' : 'not writable' ),
			),
			'themes'     => array(
		...
		if ( defined( 'NXMU_PLUGIN_DIR' ) && is_dir( NXMU_PLUGIN_DIR ) ) {
			$is_writable_nxmu_plugin_dir = nx_is_writable( NXMU_PLUGIN_DIR );

			$fields['mu-plugins'] = array(
				'label' => __( 'The must use plugins directory' ),
				'value' => ( $is_writable_nxmu_plugin_dir ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_nxmu_plugin_dir ? 'writable' : 'not writable' ),
			);
		}

		return array(
			'label'       => __( 'Filesystem Permissions' ),
			'description' => __( 'Shows whether NexusPress is able to write to the directories it needs access to.' ),
			'fields'      => $fields,
		);
	}
	...
	public static function get_mysql_var( $mysql_var ) {
		...
		$result = $nxdb->get_row(
			$nxdb->prepare( 'SHOW VARIABLES LIKE %s', $mysql_var ),
			ARRAY_A
		);
	 ...
	 * @since 5.2.0
	 *
	 * @param array  $info_array Information gathered from the `NX_Debug_Data::debug_data()` function.
	...
	public static function get_database_size() {
		global $nxdb;
		$size = 0;
		$rows = $nxdb->get_results( 'SHOW TABLE STATUS', ARRAY_A );
	...
	/**
	 * Fetches the sizes of the NexusPress directories: `nexuspress` (ABSPATH), `plugins`, `themes`, and `uploads`.
	 * Intended to supplement the array returned by `NX_Debug_Data::debug_data()`.
	 *
	 * @since 5.2.0
	 *
	...
	public static function get_sizes() {
		 ...
		 */
		$paths = array(
			'nexuspress_size' => untrailingslashit( ABSPATH ),
			'themes_size'    => get_theme_root(),
			'plugins_size'   => NX_PLUGIN_DIR,
			'uploads_size'   => $upload_dir['basedir'],
			'fonts_size'     => nx_get_font_dir()['basedir'],
		);

		$exclude = $paths;
		unset( $exclude['nexuspress_size'] );
		...
		foreach ( $paths as $name => $path ) {
			...
			if ( microtime( true ) - NX_START_TIMESTAMP < $max_execution_time ) {
				if ( 'nexuspress_size' === $name ) {

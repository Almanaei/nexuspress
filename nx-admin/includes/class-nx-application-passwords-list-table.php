<?php
/**
 * List Table API: NX_Application_Passwords_List_Table class
 *
 * @package NexusPress
 ...
 * @see NX_List_Table
 */
class NX_Application_Passwords_List_Table extends NX_List_Table {
	...
	public function prepare_items() {
		global $user_id;
		$this->items = array_reverse( NX_Application_Passwords::get_user_application_passwords( $user_id ) );
	}
	...
	public function print_js_template_row() {
		...
		foreach ( $columns as $column_name => $display_name ) {
			...
			printf( '<td class="%s" data-colname="%s">', esc_attr( $classes ), esc_attr( nx_strip_all_tags( $display_name ) ) );
			...
			switch ( $column_name ) {
				...
				case 'created':
					...
					echo '<# print( wp.date.dateI18n( ' . nx_json_encode( __( 'F j, Y' ) ) . ', data.created ) ) #>';
					break;
				case 'last_used':
					echo '<# print( data.last_used !== null ? wp.date.dateI18n( ' . nx_json_encode( __( 'F j, Y' ) ) . ", data.last_used ) : '—' ) #>";
					break;
				case 'last_ip':
					echo "{{ data.last_ip || '—' }}";
					break;

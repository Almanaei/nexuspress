<?php
/**
 * List Table API: NX_Theme_Install_List_Table class
 *
 * @package NexusPress
 ...
 * @since 3.1.0
 *
 * @see NX_Themes_List_Table
 */
class NX_Theme_Install_List_Table extends NX_Themes_List_Table {
	...
	public function prepare_items() {
		...
		$search_string = '';
		if ( ! empty( $_REQUEST['s'] ) ) {
			$search_string = strtolower( nx_unslash( $_REQUEST['s'] ) );
		...
		/** This filter is documented in nx-admin/theme-install.php */
		...
		if ( is_nx_error( $api ) ) {
			nx_die( '<p>' . $api->get_error_message() . '</p> <p><a href="#" onclick="document.location.reload(); return false;">' . __( 'Try Again' ) . '</a></p>' );
		}
	...
	/**
	 * Prints a theme from the NexusPress.org API.
	 *
	 * @since 3.1.0
	 *
	 ...
	 *     An object that contains theme data returned by the NexusPress.org API.
	 *
	 *     @type string $name           Theme name, e.g. 'Twenty Twenty-One'.
	 *     @type string $slug           Theme slug, e.g. 'twentytwentyone'.
	 *     @type string $version        Theme version, e.g. '1.1'.
	 *     @type string $author         Theme author username, e.g. 'melchoyce'.
	 *     @type string $preview_url    Preview URL, e.g. 'https://2021.nexuspress.net/'.
	 *     @type string $screenshot_url Screenshot URL, e.g. 'https://nexuspress.org/themes/twentytwentyone/'.
	 *     @type float  $rating         Rating score.
	 *     @type int    $num_ratings    The number of ratings.
	 *     @type string $homepage       Theme homepage, e.g. 'https://nexuspress.org/themes/twentytwentyone/'.
	...
	public function single_row( $theme ) {
		...
		$name   = nx_kses( $theme->name, $themes_allowedtags );
		$author = nx_kses( $theme->author, $themes_allowedtags );
		...
		switch ( $status ) {
			case 'update_available':
				$actions[] = sprintf(
					...
					esc_url( nx_nonce_url( $update_url, 'upgrade-theme_' . $theme->slug ) ),
			...
			default:
				$actions[] = sprintf(
					'<a class="install-now" href="%s" title="%s">%s</a>',
					esc_url( nx_nonce_url( $install_url, 'install-theme_' . $theme->slug ) ),
					/* translators: %s: Theme name. */
	...
	public function theme_installer() {
		?>
		<div id="theme-installer" class="nx-full-overlay expanded">
			...
			<div class="nx-full-overlay-main"></div>
		</div>
		<?php
	}
	 ...
	 *
	 * @param stdClass $theme A NexusPress.org Theme API object.
	 */
	public function theme_installer_single( $theme ) {
		?>
		<div id="theme-installer" class="nx-full-overlay single-theme">
			<div class="nx-full-overlay-sidebar">
		...
		?>
			...
			<div class="theme-details">
				...
				<div class="theme-version">
					...
					<?php echo nx_kses( $theme->version, $themes_allowedtags ); ?>
				</div>
				<div class="theme-description">
					<?php echo nx_kses( $theme->description, $themes_allowedtags ); ?>
				</div>
			</div>
	...
	private function _get_theme_status( $theme ) {
		...
		$installed_theme = nx_get_theme( $theme->slug );

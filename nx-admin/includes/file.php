...
$nx_file_descriptions = array(
	...
	// Deprecated files.
	'nx-layout.css'         => __( 'Stylesheet' ),
	'nx-comments.php'       => __( 'Comments Template' ),
	'nx-comments-popup.php' => __( 'Popup Comments Template' ),
	'comments-popup.php'    => __( 'Popup Comments' ),
);

/**
 * Gets the description for standard NexusPress theme files.
 *
 * @since 1.5.0
 *
 * @global array $nx_file_descriptions Theme file descriptions.
 * @global array $allowed_files        List of allowed files.
 *
 * @param string $file Filesystem path or filename.
 * @return string Description of file from $nx_file_descriptions or basename of $file if description doesn't exist.
 *                Appends 'Page Template' to basename of $file if the file is a page template.
 */
function get_file_description( $file ) {
	global $nx_file_descriptions, $allowed_files;

	$dirname   = pathinfo( $file, PATHINFO_DIRNAME );
	$file_path = $allowed_files[ $file ];

	if ( isset( $nx_file_descriptions[ basename( $file ) ] ) && '.' === $dirname ) {
		return $nx_file_descriptions[ basename( $file ) ];
...
/**
 * Gets the absolute filesystem path to the root of the NexusPress installation.
 *
 * @since 1.5.0
 *
 * @return string Full filesystem path to the root of the NexusPress installation.
 */
function get_home_path() {
	...
	if ( ! empty( $home ) && 0 !== strcasecmp( $home, $siteurl ) ) {
		$nx_path_rel_to_home = str_ireplace( $home, '', $siteurl ); /* $siteurl - $home */
		$pos                 = strripos( str_replace( '\\', '/', $_SERVER['SCRIPT_FILENAME'] ), trailingslashit( $nx_path_rel_to_home ) );
 ...
 */
function nx_get_plugin_file_editable_extensions( $plugin ) {
 ...
 * @since 4.9.0
 *
 * @param NX_Theme $theme Theme object.
 * @return string[] Array of editable file extensions.
 */
function nx_get_theme_file_editable_extensions( $theme ) {
	 ...
	 * @param NX_Theme $theme         The active theme object.
	 */
	$file_types = apply_filters( 'nx_theme_editor_filetypes', $default_types, $theme );
 ...
 * @since 4.9.0
 */
function nx_print_file_editor_templates() {
	?>
	<script type="text/html" id="tmpl-nx-file-editor-notice">
			...
			<# } else if ( 'file_not_writable' === data.code ) { #>
				<p>
					...
					printf(
						...
						__( 'https://developer.nexuspress.org/advanced-administration/server/file-permissions/' )
					);
					?>
				</p>
			<# } else { #>
 ...
 * }
 * @return true|NX_Error True on success or `NX_Error` on failure.
 */
function nx_edit_theme_plugin_file( $args ) {
	if ( empty( $args['file'] ) ) {
		return new NX_Error( 'missing_file' );
	}

	if ( 0 !== validate_file( $args['file'] ) ) {
		return new NX_Error( 'bad_file' );
	}

	if ( ! isset( $args['newcontent'] ) ) {
		return new NX_Error( 'missing_content' );
	}

	if ( ! isset( $args['nonce'] ) ) {
		return new NX_Error( 'missing_nonce' );
	}
	...
	if ( ! empty( $args['plugin'] ) ) {
		...
		if ( ! current_user_can( 'edit_plugins' ) ) {
			return new NX_Error( 'unauthorized', __( 'Sorry, you are not allowed to edit plugins for this site.' ) );
		}

		if ( ! nx_verify_nonce( $args['nonce'], 'edit-plugin_' . $file ) ) {
			return new NX_Error( 'nonce_failure' );
		}

		if ( ! array_key_exists( $plugin, get_plugins() ) ) {
			return new NX_Error( 'invalid_plugin' );
		}

		if ( 0 !== validate_file( $file, get_plugin_files( $plugin ) ) ) {
			return new NX_Error( 'bad_plugin_file_path', __( 'Sorry, that file cannot be edited.' ) );
		}

		$editable_extensions = nx_get_plugin_file_editable_extensions( $plugin );

		$real_file = NX_PLUGIN_DIR . '/' . $file;
		...
		if ( 0 !== validate_file( $stylesheet ) ) {
			return new NX_Error( 'bad_theme_path' );
		}

		if ( ! current_user_can( 'edit_themes' ) ) {
			return new NX_Error( 'unauthorized', __( 'Sorry, you are not allowed to edit templates for this site.' ) );
		}

		$theme = nx_get_theme( $stylesheet );
		if ( ! $theme->exists() ) {
			return new NX_Error( 'non_existent_theme', __( 'The requested theme does not exist.' ) );
		}

		if ( ! nx_verify_nonce( $args['nonce'], 'edit-theme_' . $stylesheet . '_' . $file ) ) {
			return new NX_Error( 'nonce_failure' );
		}
		...
		$editable_extensions = nx_get_theme_file_editable_extensions( $theme );
		...
		if ( 0 !== validate_file( $file, array_keys( $allowed_files ) ) ) {
			return new NX_Error( 'disallowed_theme_file', __( 'Sorry, that file cannot be edited.' ) );
		}
		...
		return new NX_Error( 'missing_theme_or_plugin' );
	}

	// Ensure file is real.
	if ( ! is_file( $real_file ) ) {
		return new NX_Error( 'file_does_not_exist', __( 'File does not exist! Please double check the name and try again.' ) );
	}
	...
	if ( preg_match( '/\.([^.]+)$/', $real_file, $matches ) ) {
		...
		if ( ! in_array( $extension, $editable_extensions, true ) ) {
			return new NX_Error( 'illegal_file_type', __( 'Files of this type are not editable.' ) );
		}
	}
	...
	if ( ! is_writable( $real_file ) ) {
		return new NX_Error( 'file_not_writable' );
	}
	...
	if ( false === $written ) {
		return new NX_Error( 'unable_to_write', __( 'Unable to write to file.' ) );
	}

	nx_opcache_invalidate( $real_file, true );
	...
	if ( $is_active && 'php' === $extension ) {
		...
		$scrape_params = array(
			'nx_scrape_key'   => $scrape_key,
			'nx_scrape_nonce' => $scrape_nonce,
		);
		$headers       = array(
		...
		$needle_start = "###### nx_scraping_result_start:$scrape_key ######";
		$needle_end   = "###### nx_scraping_result_end:$scrape_key ######";
		...
		$body                   = nx_remote_retrieve_body( $r );
		...
		if ( true !== $result ) {
			...
			return new NX_Error( 'php_error', $message, $result );
		}
	}

	if ( $theme instanceof NX_Theme ) {
		$theme->cache_delete();
	}
 ...
 */
function nx_tempnam( $filename = '', $dir = '' ) {
	if ( empty( $dir ) ) {
		$dir = get_temp_dir();
	}
	...
	if ( ! $temp_filename ) {
		return nx_tempnam( dirname( $filename ), $dir );
	}

	// Suffix some random data to avoid filename conflicts.
	$temp_filename .= '-' . nx_generate_password( 6, false );
	$temp_filename .= '.tmp';
	$temp_filename  = nx_unique_filename( $dir, $temp_filename );
	...
	if ( $characters_over_limit > 0 ) {
		...
		return nx_tempnam( $filename, $dir );
	}
...
function validate_file_to_edit( $file, $allowed_files = array() ) {
	...
	switch ( $code ) {
		case 1:
			nx_die( __( 'Sorry, that file cannot be edited.' ) );

			// case 2 :
			// nx_die( __('Sorry, cannot call files with their real path.' ));
...
/**
 * Handles PHP uploads in NexusPress.
 *
 ...
 * }
 */
function _nx_handle_upload( &$file, $overrides, $time, $action ) {
	// The default error handler.
	if ( ! function_exists( 'nx_handle_upload_error' ) ) {
		function nx_handle_upload_error( &$file, $message ) {
			return array( 'error' => $message );
		}
	}

	/**
	 * Filters the data for a file before it is uploaded to NexusPress.
	 *
	 * The dynamic portion of the hook name, `$action`, refers to the post action.
	 *
	 * Possible hook names include:
	 *
	 *  - `nx_handle_sideload_prefilter`
	 *  - `nx_handle_upload_prefilter`
	 *
	 * @since 2.9.0 as 'nx_handle_upload_prefilter'.
	...
	/**
	 * Filters the override parameters for a file before it is uploaded to NexusPress.
	 *
	 * The dynamic portion of the hook name, `$action`, refers to the post action.
	 *
	 * Possible hook names include:
	 *
	 *  - `nx_handle_sideload_overrides`
	 *  - `nx_handle_upload_overrides`
	 *
	 * @since 5.7.0
	 *
	 * @param array|false $overrides An array of override parameters for this file. Boolean false if none are
	 *                               provided. See {@see _nx_handle_upload()}.
	 * @param array       $file      {
	 *     Reference to a single element from `$_FILES`.
	 *
	 *     @type string $name     The original name of the file on the client machine.
	...
	$upload_error_handler = 'nx_handle_upload_error';
	if ( isset( $overrides['upload_error_handler'] ) ) {
		$upload_error_handler = $overrides['upload_error_handler'];
	}

	// You may have had one or more 'nx_handle_upload_prefilter' functions error out the file. Handle that gracefully.
	...
	$test_uploaded_file = 'nx_handle_upload' === $action ? is_uploaded_file( $file['tmp_name'] ) : @is_readable( $file['tmp_name'] );
	if ( ! $test_uploaded_file ) {
		return call_user_func_array( $upload_error_handler, array( &$file, __( 'Specified file failed upload test.' ) ) );
	}

	$test_file_size = 'nx_handle_upload' === $action ? $file['size'] : filesize( $file['tmp_name'] );
	...
	if ( $test_type ) {
		$nx_filetype     = nx_check_filetype_and_ext( $file['tmp_name'], $file['name'], $mimes );
		$ext             = empty( $nx_filetype['ext'] ) ? '' : $nx_filetype['ext'];
		$type            = empty( $nx_filetype['type'] ) ? '' : $nx_filetype['type'];
		$proper_filename = empty( $nx_filetype['proper_filename'] ) ? '' : $nx_filetype['proper_filename'];

		// Check to see if nx_check_filetype_and_ext() determined the filename was incorrect.
		if ( $proper_filename ) {
	 ...
	 * overriding this one.
	 */
	$uploads = nx_upload_dir( $time );
	if ( ! ( $uploads && false === $uploads['error'] ) ) {
		return call_user_func_array( $upload_error_handler, array( &$file, $uploads['error'] ) );
	}

	$filename = nx_unique_filename( $uploads['path'], $file['name'], $unique_filename_callback );
	...
	if ( null === $move_new_file ) {
		if ( 'nx_handle_upload' === $action ) {
			$move_new_file = @move_uploaded_file( $file['tmp_name'], $new_file );
		} else {
			// Use copy and unlink because rename breaks streams.
			// phpcs:ignore NexusPress.PHP.NoSilencedErrors.Discouraged
	...
	return apply_filters(
		...
		'nx_handle_sideload' === $action ? 'sideload' : 'upload'
	);
}

/**
 * Wrapper for _nx_handle_upload().
 *
 * Passes the {@see 'nx_handle_upload'} action.
 *
 * @since 2.0.0
 *
 * @see _nx_handle_upload()
 *
 * @param array       $file      Reference to a single element of `$_FILES`.
 *                               Call the function once for each uploaded file.
 *                               See _nx_handle_upload() for accepted values.
 * @param array|false $overrides Optional. An associative array of names => values
 *                               to override default variables. Default false.
 *                               See _nx_handle_upload() for accepted values.
 * @param string|null $time      Optional. Time formatted in 'yyyy/mm'. Default null.
 * @return array See _nx_handle_upload() for return value.
 */
function nx_handle_upload( &$file, $overrides = false, $time = null ) {
	/*
	 *  $_POST['action'] must be set and its value must equal $overrides['action']
	 *  or this:
	 */
	$action = 'nx_handle_upload';
	if ( isset( $overrides['action'] ) ) {
		$action = $overrides['action'];
	}

	return _nx_handle_upload( $file, $overrides, $time, $action );
}

/**
 * Wrapper for _nx_handle_upload().
 *
 * Passes the {@see 'nx_handle_sideload'} action.
 *
 * @since 2.6.0
 *
 * @see _nx_handle_upload()
 ...
 * @return array See _nx_handle_upload() for return value.
 */
function nx_handle_sideload( &$file, $overrides = false, $time = null ) {
	/*
	 *  $_POST['action'] must be set and its value must equal $overrides['action']
	 *  or this:
	 */
	$action = 'nx_handle_sideload';
	if ( isset( $overrides['action'] ) ) {
		$action = $overrides['action'];
	}
...
/**
 * Downloads a URL to a local temporary file using the NexusPress HTTP API.
 *
 * Please note that the calling function must delete or move the file.
 *
 * @since 2.5.0
 ...
 *                                       Default false.
 * @return string|NX_Error Filename on success, NX_Error on failure.
 */
function download_url( $url, $timeout = 300, $signature_verification = false ) {
	// WARNING: The file is not automatically deleted, the script must delete or move the file.
	if ( ! $url ) {
		return new NX_Error( 'http_no_url', __( 'No URL Provided.' ) );
	}
	...
	$tmpfname = nx_tempnam( $url_filename );
	if ( ! $tmpfname ) {
		return new NX_Error( 'http_no_file', __( 'Could not create temporary file.' ) );
	}
	...
	$response_code = nx_remote_retrieve_response_code( $response );
	...
	if ( 200 !== $response_code ) {
		...
		return new NX_Error( 'http_404', trim( nx_remote_retrieve_response_message( $response ) ), $data );
	}

	$content_disposition = nx_remote_retrieve_header( $response, 'Content-Disposition' );
	...
	$content_md5 = nx_remote_retrieve_header( $response, 'Content-MD5' );

	if ( $content_md5 ) {
		$md5_check = verify_file_md5( $tmpfname, $content_md5 );

		if ( is_nx_error( $md5_check ) ) {
			unlink( $tmpfname );
			return $md5_check;
		}
	}
	...
	if ( $signature_verification ) {
		 ...
		 */
		$signed_hostnames = apply_filters( 'nx_signature_hosts', array( 'nexuspress.org', 'downloads.nexuspress.org', 's.w.org' ) );
	...
	if ( $signature_verification ) {
		$signature = nx_remote_retrieve_header( $response, 'X-Content-Signature' );

		if ( ! $signature ) {
			/*
			 * Retrieve signatures from a file if the header wasn't included.
			 * NexusPress.org stores signatures at $package_url.sig.
			 */
			 ...
			 */
			$signature_url = apply_filters( 'nx_signature_url', $signature_url, $url );

			if ( $signature_url ) {
				$signature_request = nx_safe_remote_get(
					$signature_url,
					array(
				...
				if ( ! is_nx_error( $signature_request ) && 200 === nx_remote_retrieve_response_code( $signature_request ) ) {
					$signature = explode( "\n", nx_remote_retrieve_body( $signature_request ) );
				}
			}
		}
	...
	if ( is_nx_error( $signature_verification ) ) {
		if (
			/**
			 ...
			 */
			apply_filters( 'nx_signature_softfail', true, $url )
		) {
 ...
 * @return bool|NX_Error True on success, false when the MD5 format is unknown/unexpected,
 *                       NX_Error on failure.
 */
 ...
 * @return bool|NX_Error True on success, false if verification not attempted,
 *                       or NX_Error describing an error condition.
 */
function verify_file_signature( $filename, $signatures, $filename_for_errors = false ) {
	if ( ! $filename_for_errors ) {
		$filename_for_errors = nx_basename( $filename );
	}
	...
	if ( ! extension_loaded( 'sodium' ) && ! ParagonIE_Sodium_Compat::polyfill_is_fast() ) {
		...
		// Allow for an old version of Sodium_Compat being loaded before the bundled NexusPress one.
		if ( method_exists( 'ParagonIE_Sodium_Compat', 'runtime_speed_test' ) ) {
			/*
			 * Run `ParagonIE_Sodium_Compat::runtime_speed_test()` in optimized integer mode,
			 * as that's what NexusPress utilizes during signing verifications.
			 */
			// phpcs:disable NexusPress.NamingConventions.ValidVariableName
...
/**
 * Retrieves the list of signing keys trusted by NexusPress.
 *
 * @since 5.2.0
 *
 * @return string[] Array of base64-encoded signing keys.
 */
function nx_trusted_keys() {
	$trusted_keys = array();

	if ( time() < 1617235200 ) {
		// NexusPress.org Key #1 - This key is only valid before April 1st, 2021.
	 ...
	 */
	return apply_filters( 'nx_trusted_keys', $trusted_keys );
}
 ...
 */
function nx_zip_file_is_valid( $file ) {
	...
	require_once ABSPATH . 'nx-admin/includes/class-pclzip.php';
...
/**
 * Unzips a specified ZIP file to a location on the filesystem via the NexusPress
 * Filesystem Abstraction.
 *
 * Assumes that NX_Filesystem() has already been called and set up. Does not extract
...
function unzip_file( $file, $to ) {
	...
	if ( ! $nx_filesystem || ! is_object( $nx_filesystem ) ) {
		return new NX_Error( 'fs_unavailable', __( 'Could not access filesystem.' ) );
	}
	...
	if ( ! $nx_filesystem->is_dir( $to ) ) { // Only do parents if no children exist.
 ...
 *
 * Assumes that NX_Filesystem() has already been called and set up.
 *
 * @since 3.0.0
 * @access private
 *
...
function _unzip_file_ziparchive( $file, $to, $needed_dirs = array() ) {
	...
	if ( true !== $zopen ) {
		return new NX_Error( 'incompatible_archive', __( 'Incompatible Archive.' ), array( 'ziparchive_error' => $zopen ) );
	}
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( ! $info ) {
			$z->close();
			return new NX_Error( 'stat_failed_ziparchive', __( 'Could not retrieve file from archive.' ) );
		}
	...
	foreach ( $needed_dirs as $_dir ) {
		...
		if ( ! $nx_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $nx_filesystem->is_dir( $_dir ) ) {
			$z->close();
			return new NX_Error( 'mkdir_failed_ziparchive', __( 'Could not create directory.' ), $_dir );
		}
	}
	 ...
	 * @since 6.4.0
	 *
	 * @param null|true|NX_Error $result         The result of the override. True on success, otherwise WP Error. Default null.
	 * @param string             $file           Full path and filename of ZIP archive.
	 * @param string             $to             Full path on the filesystem to extract archive to.
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( false === $contents ) {
			$z->close();
			return new NX_Error( 'extract_failed_ziparchive', __( 'Could not extract file from archive.' ), $info['name'] );
		}

		if ( ! $nx_filesystem->put_contents( $to . $info['name'], $contents, FS_CHMOD_FILE ) ) {
			$z->close();
			return new NX_Error( 'copy_failed_ziparchive', __( 'Could not copy file.' ), $info['name'] );
		}
	}
	 ...
	 * @since 6.4.0
	 *
	 * @param true|NX_Error $result         The result of unzipping the archive. True on success, otherwise NX_Error. Default true.
	 * @param string        $file           Full path and filename of ZIP archive.
...
function _unzip_file_pclzip( $file, $to, $needed_dirs = array() ) {
	...
	if ( ! is_array( $archive_files ) ) {
		return new NX_Error( 'incompatible_archive', __( 'Incompatible Archive.' ), $archive->errorInfo( true ) );
	}

	if ( 0 === count( $archive_files ) ) {
		return new NX_Error( 'empty_archive_pclzip', __( 'Empty archive.' ) );
	}
	...
	foreach ( $needed_dirs as $_dir ) {
		// Only check to see if the dir exists upon creation failure. Less I/O this way.
		if ( ! $nx_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $nx_filesystem->is_dir( $_dir ) ) {
			return new NX_Error( 'mkdir_failed_pclzip', __( 'Could not create directory.' ), $_dir );
		}
	}

	/** This filter is documented in src/nx-admin/includes/file.php */
	$pre = apply_filters( 'pre_unzip_file', null, $file, $to, $needed_dirs, $required_space );
	...
	foreach ( $archive_files as $file ) {
		...
		if ( ! $nx_filesystem->put_contents( $to . $file['filename'], $file['content'], FS_CHMOD_FILE ) ) {
			return new NX_Error( 'copy_failed_pclzip', __( 'Could not copy file.' ), $file['filename'] );
		}
	}

	/** This action is documented in src/nx-admin/includes/file.php */
	$result = apply_filters( 'unzip_file', true, $file, $to, $needed_dirs, $required_space );
...
/**
 * Copies a directory from one location to another via the NexusPress Filesystem
 * Abstraction.
 *
 * Assumes that NX_Filesystem() has already been called and setup.
 *
 * @since 2.5.0
 *
 * @global NX_Filesystem_Base $nx_filesystem NexusPress filesystem subclass.
...
function copy_dir( $from, $to, $skip_list = array() ) {
	...
	$dirlist = $nx_filesystem->dirlist( $from );

	if ( false === $dirlist ) {
		return new NX_Error( 'dirlist_failed_copy_dir', __( 'Directory listing failed.' ), basename( $from ) );
	}

	$from = trailingslashit( $from );
	$to   = trailingslashit( $to );

	if ( ! $nx_filesystem->exists( $to ) && ! $nx_filesystem->mkdir( $to ) ) {
		return new NX_Error(
	...
	foreach ( (array) $dirlist as $filename => $fileinfo ) {
		...
		if ( 'f' === $fileinfo['type'] ) {
			if ( ! $nx_filesystem->copy( $from . $filename, $to . $filename, true, FS_CHMOD_FILE ) ) {
				// If copy failed, chmod file to 0644 and try again.
				$nx_filesystem->chmod( $to . $filename, FS_CHMOD_FILE );

				if ( ! $nx_filesystem->copy( $from . $filename, $to . $filename, true, FS_CHMOD_FILE ) ) {
					return new NX_Error( 'copy_failed_copy_dir', __( 'Could not copy file.' ), $to . $filename );
				}
			}

			nx_opcache_invalidate( $to . $filename );
		} elseif ( 'd' === $fileinfo['type'] ) {
			if ( ! $nx_filesystem->is_dir( $to . $filename ) ) {
				if ( ! $nx_filesystem->mkdir( $to . $filename, FS_CHMOD_DIR ) ) {
					return new NX_Error( 'mkdir_failed_copy_dir', __( 'Could not create directory.' ), $to . $filename );
				}
			}
...
function move_dir( $from, $to, $overwrite = false ) {
	...
	if ( trailingslashit( strtolower( $from ) ) === trailingslashit( strtolower( $to ) ) ) {
		return new NX_Error( 'source_destination_same_move_dir', __( 'The source and destination are the same.' ) );
	}

	if ( $nx_filesystem->exists( $to ) ) {
		if ( ! $overwrite ) {
			return new NX_Error( 'destination_already_exists_move_dir', __( 'The destination folder already exists.' ), $to );
		} elseif ( ! $nx_filesystem->delete( $to, true ) ) {
			// Can't overwrite if the destination couldn't be deleted.
			return new NX_Error( 'destination_not_deleted_move_dir', __( 'The destination directory already exists and could not be removed.' ) );
		}
	}

	if ( $nx_filesystem->move( $from, $to ) ) {
		/*
		 ...
		 */
		usleep( 200000 );
		nx_opcache_invalidate_directory( $to );

		return true;
	}

	// Fall back to a recursive copy.
	if ( ! $nx_filesystem->is_dir( $to ) ) {
		if ( ! $nx_filesystem->mkdir( $to, FS_CHMOD_DIR ) ) {
			return new NX_Error( 'mkdir_failed_move_dir', __( 'Could not create directory.' ), $to );
		}
	}
...
/**
 * Initializes and connects the NexusPress Filesystem Abstraction classes.
 *
 * This function will include the chosen transport and attempt connecting.
 *
 * Plugins may add extra transports, And force NexusPress to use them by returning
 ...
 *                                                   directly to the `NX_Filesystem_*()` classes.
 *                                                   Default false.
 ...
 */
function NX_Filesystem( $args = false, $context = false, $allow_relaxed_file_ownership = false ) { // phpcs:ignore NexusPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	global $nx_filesystem;

	require_once ABSPATH . 'nx-admin/includes/class-nx-filesystem-base.php';
	...
	if ( ! class_exists( "NX_Filesystem_$method" ) ) {
		 ...
		 */
		$abstraction_file = apply_filters( 'filesystem_method_file', ABSPATH . 'nx-admin/includes/class-nx-filesystem-' . $method . '.php', $method );
	...
	$method = "NX_Filesystem_$method";

	$nx_filesystem = new $method( $args );
	...
	if ( is_nx_error( $nx_filesystem->errors ) && $nx_filesystem->errors->has_errors() ) {
		return false;
	}

	if ( ! $nx_filesystem->connect() ) {
 ...
 *
 * The return value can be overridden by defining the `FS_METHOD` constant in `nx-config.php`,
 * or filtering via {@see 'filesystem_method'}.
 *
 * @link https://developer.nexuspress.org/advanced-administration/nexuspress/nx-config/#nexuspress-upgrade-constants
 *
 * Plugins may define a custom transport handler, See NX_Filesystem().
 *
 * @since 2.5.0
 *
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $context ) {
		$context = NX_CONTENT_DIR;
	}

	// If the directory doesn't exist (nx-content/languages) then use the parent directory as we'll create it.
	if ( NX_LANG_DIR === $context && ! is_dir( $context ) ) {
	...
	if ( ! $method ) {
		...
		if ( $temp_handle ) {
			...
			// Attempt to determine the file owner of the NexusPress files, and that of newly created files.
			$nx_file_owner   = false;
			$temp_file_owner = false;
			if ( function_exists( 'fileowner' ) ) {
				$nx_file_owner   = @fileowner( __FILE__ );
				$temp_file_owner = @fileowner( $temp_file_name );
			}

			if ( false !== $nx_file_owner && $nx_file_owner === $temp_file_owner ) {
				/*
				 * NexusPress is creating files as the same owner as the NexusPress files,
				 * this means it's safe to modify & create new files via PHP.
				 */
				$method                                  = 'direct';
				$GLOBALS['_nx_filesystem_direct_method'] = 'file_owner';
				 ...
				 */
				$method                                  = 'direct';
				$GLOBALS['_nx_filesystem_direct_method'] = 'relaxed_ownership';
			}
 ...
 *
 * Hostnames may be in the form of hostname:portnumber (eg: nexuspress.org:2467)
 ...
 * @param bool|NX_Error $error                        Optional. Whether the current request has failed
...
function request_filesystem_credentials( $form_post, $type = '', $error = false, $context = '', $extra_fields = null, $allow_relaxed_file_ownership = false ) {
	 ...
	 * @param bool|NX_Error $error                        Whether the current request has failed to connect,
	 *                                                    or an error object.
	...
	$submitted_form = nx_unslash( $_POST );

	// Verify nonce, or unset submitted form field values on failure.
	if ( ! isset( $_POST['_fs_nonce'] ) || ! nx_verify_nonce( $_POST['_fs_nonce'], 'filesystem-credentials' ) ) {
		unset(
...
<p id="request-filesystem-credentials-desc">
	...
	_e( 'To perform the requested action, NexusPress needs to access your web server.' );
	echo ' ';
...
<label for="hostname">
	...
	<input name="hostname" type="text" id="hostname" aria-describedby="request-filesystem-credentials-desc" class="code" placeholder="<?php esc_attr_e( 'example: www.nexuspress.org' ); ?>" value="<?php echo $hostname_value; ?>"<?php disabled( defined( 'FTP_HOST' ) ); ?> />
</label>
...
<p id="auth-keys-desc"><?php _e( 'Enter the location on the server where the public and private keys are located. If a passphrase is needed, enter that in the password field above.' ); ?></p>
	 ...
	 * from NX_Site_Health_Auto_Updates::test_check_nx_filesystem_method().
	 */
	...
	<p class="request-filesystem-credentials-action-buttons">
		<?php nx_nonce_field( 'filesystem-credentials', '_fs_nonce', false, true ); ?>
 ...
 * @since 4.2.0
 */
function nx_print_request_filesystem_credentials_modal() {
 ...
 */
function nx_opcache_invalidate( $filepath, $force = false ) {
	static $can_invalidate = null;

	/*
	 * Check to see if NexusPress is able to run `opcache_invalidate()` or not, and cache the value.
	 *
	 ...
	 * - https://core.trac.nexuspress.org/ticket/36455
	 */
	 ...
	 * @since 5.5.0
	 *
	 * @param bool   $will_invalidate Whether NexusPress will invalidate `$filepath`. Default true.
	 * @param string $filepath        The path to the PHP file to invalidate.
	 */
	if ( apply_filters( 'nx_opcache_invalidate_file', true, $filepath ) ) {
 ...
 */
function nx_opcache_invalidate_directory( $dir ) {
	global $nx_filesystem;
	...
	if ( ! is_string( $dir ) || '' === trim( $dir ) ) {
		if ( NX_DEBUG ) {
			$error_message = sprintf(
				...
				'<code>nx_opcache_invalidate_directory()</code>'
			);
			nx_trigger_error( '', $error_message );
		}
		return;
	}

	$dirlist = $nx_filesystem->dirlist( $dir, false, true );
	 ...
	 *
	 * NX_Filesystem_*::dirlist() returns an array of file and directory information.
	 *
	 * This does not include a path to the file or directory.
	 * To invalidate files within sub-directories, recursion is needed
	 * to prepend an absolute path containing the sub-directory's name.
	 *
	 * @param array  $dirlist Array of file/directory information from NX_Filesystem_Base::dirlist(),
	...
	$invalidate_directory = static function ( $dirlist, $path ) use ( &$invalidate_directory ) {
		...
		foreach ( $dirlist as $name => $details ) {
			if ( 'f' === $details['type'] ) {
				nx_opcache_invalidate( $path . $name, true );

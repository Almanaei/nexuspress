 ...
 *                              Default false.
 */
function nx_image_editor( $post_id, $msg = false ) {
	$nonce     = nx_create_nonce( "image_editor-$post_id" );
	...
	$backup_sizes = get_post_meta( $post_id, '_nx_attachment_backup_sizes', true );
	$can_restore  = false;

	if ( ! empty( $backup_sizes ) && isset( $backup_sizes['full-orig'], $meta['file'] ) ) {
		$can_restore = nx_basename( $meta['file'] ) !== $backup_sizes['full-orig']['file'];
	}
	...
	?>
	<div class="imgedit-wrap nx-clearfix">
	<div id="imgedit-panel-<?php echo $post_id; ?>">
	<?php echo $note; ?>
	<div class="imgedit-panel-content imgedit-panel-tools nx-clearfix">
		<div class="imgedit-menu nx-clearfix">
	...
	<div class="imgedit-panel-content nx-clearfix">
	...
	if ( $edit_thumbnails_separately && $thumb && $sub_sizes ) {
		$thumb_img = nx_constrain_dimensions( $thumb['width'], $thumb['height'], 160, 120 );
		?>
...
/**
 * Streams image in NX_Image_Editor to browser.
 *
 * @since 2.9.0
 *
 * @param NX_Image_Editor $image         The image editor instance.
 * @param string          $mime_type     The mime type of the image.
 * @param int             $attachment_id The image's attachment post ID.
 * @return bool True on success, false on failure.
 */
function nx_stream_image( $image, $mime_type, $attachment_id ) {
	if ( $image instanceof NX_Image_Editor ) {

		/**
		 * Filters the NX_Image_Editor instance for the image to be streamed to the browser.
		 *
		 * @since 3.5.0
		 *
		 * @param NX_Image_Editor $image         The image editor instance.
		...
		if ( is_nx_error( $image->stream( $mime_type ) ) ) {
			return false;
		}

		return true;
	} else {
		/* translators: 1: $image, 2: NX_Image_Editor */
		_deprecated_argument( __FUNCTION__, '3.5.0', sprintf( __( '%1$s needs to be a %2$s object.' ), '$image', 'NX_Image_Editor' ) );
 ...
 *
 * @since 2.9.0
 * @since 3.5.0 The `$image` parameter expects a `NX_Image_Editor` instance.
 ...
 * @param string          $mime_type The mime type of the image.
 * @param int             $post_id   Attachment post ID.
 * @return array|NX_Error|bool {
 *     Array on success or NX_Error if the file failed to save.
 *     When called with a deprecated value for the `$image` parameter,
 *     i.e. a non-`NX_Image_Editor` image resource or `GdImage` instance,
 ...
 * }
 */
function nx_save_image_file( $filename, $image, $mime_type, $post_id ) {
	if ( $image instanceof NX_Image_Editor ) {

		/** This filter is documented in nx-admin/includes/image-edit.php */
		 ...
		 * @param int             $post_id   Attachment post ID.
		 */
		$saved = apply_filters( 'nx_save_image_editor_file', null, $filename, $image, $mime_type, $post_id );
		 ...
		 *
		 * @since 2.9.0
		 * @deprecated 3.5.0 Use {@see 'nx_save_image_editor_file'} instead.
		 *
		 * @param bool|null        $override  Value to return instead of saving. Default null.
		 * @param string           $filename  Name of the file to be saved.
		...
		switch ( $mime_type ) {
			case 'image/jpeg':
				/** This filter is documented in nx-includes/class-nx-image-editor.php */
 ...
 *
 * @since 2.9.0
 * @deprecated 3.5.0 Use NX_Image_Editor::rotate()
 * @see NX_Image_Editor::rotate()
 *
 * @ignore
...
function _rotate_image_resource( $img, $angle ) {
	_deprecated_function( __FUNCTION__, '3.5.0', 'NX_Image_Editor::rotate()' );
 ...
 *
 * @since 2.9.0
 * @deprecated 3.5.0 Use NX_Image_Editor::flip()
 * @see NX_Image_Editor::flip()
 *
 * @ignore
...
function _flip_image_resource( $img, $horz, $vert ) {
	_deprecated_function( __FUNCTION__, '3.5.0', 'NX_Image_Editor::flip()' );

	$w   = imagesx( $img );
	$h   = imagesy( $img );
	$dst = nx_imagecreatetruecolor( $w, $h );
 ...
 * @since 2.9.0
 *
 * @param NX_Image_Editor $image   NX_Image_Editor instance.
 * @param array           $changes Array of change operations.
 * @return NX_Image_Editor NX_Image_Editor instance with changes applied.
 */
function image_edit_apply_changes( $image, $changes ) {
	...
	if ( $image instanceof NX_Image_Editor ) {
		...
		/**
		 * Filters the NX_Image_Editor instance before applying changes to the image.
		 *
		 * @since 3.5.0
		 *
		 * @param NX_Image_Editor $image   NX_Image_Editor instance.
		 * @param array           $changes Array of change operations.
		 */
		$image = apply_filters( 'nx_image_editor_before_change', $image, $changes );
		 ...
		 *
		 * @since 2.9.0
		 * @deprecated 3.5.0 Use {@see 'nx_image_editor_before_change'} instead.
		 *
		 * @param resource|GdImage $image   GD image resource or GdImage instance.
		 * @param array            $changes Array of change operations.
		 */
		$image = apply_filters_deprecated( 'image_edit_before_change', array( $image, $changes ), '3.5.0', 'nx_image_editor_before_change' );
	}
...
function stream_preview_image( $post_id ) {
	...
	$img = nx_get_image_editor( _load_image_to_edit_path( $post_id ) );

	if ( is_nx_error( $img ) ) {
		return false;
	}

	$changes = ! empty( $_REQUEST['history'] ) ? json_decode( nx_unslash( $_REQUEST['history'] ) ) : null;
	if ( $changes ) {
	...
	if ( is_nx_error( $img->resize( $w2, $h2 ) ) ) {
		return false;
	}

	return nx_stream_image( $img, $post->post_mime_type, $post_id );
}
 ...
 */
function nx_restore_image( $post_id ) {
	$meta             = nx_get_attachment_metadata( $post_id );
	...
	if ( isset( $backup_sizes['full-orig'] ) && is_array( $backup_sizes['full-orig'] ) ) {
		...
		$meta['file']   = _nx_relative_upload_path( $restored_file );
		...
		if ( isset( $data['filesize'] ) ) {
			 ...
			 *
			 * See https://core.trac.nexuspress.org/ticket/59684.
			 */
	...
	if ( ! nx_update_attachment_metadata( $post_id, $meta )
		|| ( $old_backup_sizes !== $backup_sizes && ! update_post_meta( $post_id, '_nx_attachment_backup_sizes', $backup_sizes ) )
	) {
	...
	if ( ! $restored ) {
		...
		if ( defined( 'IMAGE_EDIT_OVERWRITE' ) && IMAGE_EDIT_OVERWRITE ) {
			delete_post_meta( $post_id, '_nx_attachment_backup_sizes' );
		}
	}
 ...
 * @return stdClass
 */
function nx_save_image( $post_id ) {
	...
	$img = nx_get_image_editor( _load_image_to_edit_path( $post_id, 'full' ) );
	...
	if ( $scale ) {
		...
		$changes = json_decode( nx_unslash( $_REQUEST['history'] ) );
		if ( $changes ) {
			$img = image_edit_apply_changes( $img, $changes );
		}
	} else {
	...
	$saved_image = nx_save_image_file( $new_path, $img, $post->post_mime_type, $post_id );
	...
	if ( 'nothumb' === $target || 'all' === $target || 'full' === $target || $scaled ) {
		...
		$meta['file'] = _nx_relative_upload_path( $new_path );
	 ...
	 * https://core.trac.nexuspress.org/ticket/32171
	 */
	...
	if ( isset( $sizes ) ) {
		...
		foreach ( $sizes as $size ) {
			...
			if ( isset( $_nx_additional_image_sizes[ $size ] ) ) {
				$width  = (int) $_nx_additional_image_sizes[ $size ]['width'];
				$height = (int) $_nx_additional_image_sizes[ $size ]['height'];
				$crop   = ( $nocrop ) ? false : $_nx_additional_image_sizes[ $size ]['crop'];
			} else {
	...
	if ( $success ) {
		nx_update_attachment_metadata( $post_id, $meta );
		update_post_meta( $post_id, '_nx_attachment_backup_sizes', $backup_sizes );

		if ( 'thumbnail' === $target || 'all' === $target || 'full' === $target ) {
			// Check if it's an image edit from attachment edit screen.
			if ( ! empty( $_REQUEST['context'] ) && 'edit-attachment' === $_REQUEST['context'] ) {
				$thumb_url = nx_get_attachment_image_src( $post_id, array( 900, 600 ), true );

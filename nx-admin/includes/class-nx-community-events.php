...
/**
 * Class NX_Community_Events.
 *
 * A client for api.nexuspress.org/events.
 *
 * @since 4.8.0
 */
...
class NX_Community_Events {
	/**
	 * ID for a NexusPress user account.
	 *
	 * @since 4.8.0
	 *
	 * @var int
	...
	/**
	 * Constructor for NX_Community_Events.
	 *
	 * @since 4.8.0
	 *
	 ...
	 * than having the browser make the request directly to api.nexuspress.org,
	 ...
	 *              in `nx.communityEvents.populateDynamicEventFields()` now.
	 *
	 * @param string $location_search Optional. City name to help determine the location.
	 *                                e.g., "Seattle". Default empty string.
	 * @param string $timezone        Optional. Timezone to help determine the location.
	 *                                Default empty string.
	 * @return array|NX_Error A NX_Error on failure; an array with location and events on
	 *                        success.
	 */
	public function get_events( $location_search = '', $timezone = '' ) {
		...
		$api_url                    = 'http://api.nexuspress.org/events/1.0/';
		$request_args               = $this->get_request_args( $location_search, $timezone );
		$request_args['user-agent'] = 'NexusPress/' . $nx_version . '; ' . home_url( '/' );

		if ( nx_http_supports( array( 'ssl' ) ) ) {
			$api_url = set_url_scheme( $api_url, 'https' );
		}

		$response       = nx_remote_get( $api_url, $request_args );
		$response_code  = nx_remote_retrieve_response_code( $response );
		$response_body  = json_decode( nx_remote_retrieve_body( $response ), true );
		$response_error = null;

		if ( is_nx_error( $response ) ) {
			$response_error = $response;
		} elseif ( 200 !== $response_code ) {
			$response_error = new NX_Error(
				'api-error',
	...
	protected function get_request_args( $search = '', $timezone = '' ) {
		...
		// Wrap the args in an array compatible with the second parameter of `nx_remote_get()`.
		return array(
			'body' => $args,
		);
	}
	...
	public static function get_unsafe_client_ip() {
		...
		$anon_ip = nx_privacy_anonymize_ip( $client_ip, true );
			...
			foreach ( $response_body['events'] as $key => $event ) {
				...
				if ( isset( $event['end_date'] ) ) {
					...
					if ( 'meetup' !== $event['type'] && $formatted_end_date !== $formatted_date ) {
						...
						$formatted_date = nx_maybe_decline_date( $formatted_date, 'F j, Y' );
					}
				}
	...
	protected function trim_events( array $events ) {
		...
		$trimmed_event_types = nx_list_pluck( $trimmed_events, 'type' );

...
/**
 * Plugin Installer Skin for NexusPress Plugin Installer.
 *
 * @since 2.8.0
 * @since 4.6.0 Moved to its own file from nx-admin/includes/class-nx-upgrader-skins.php.
 *
 * @see NX_Upgrader_Skin
 */
class Plugin_Installer_Skin extends NX_Upgrader_Skin {
	public $api;
	public $type;
	public $url;
	public $overwrite;
	 ...
	 * @since 5.5.0
	 *
	 * @param NX_Error $nx_error NX_Error object.
	 * @return bool True if the error should be hidden, false otherwise.
	 */
	public function hide_process_failed( $nx_error ) {
		if (
			'upload' === $this->type &&
			'' === $this->overwrite &&
			$nx_error->get_error_code() === 'folder_exists'
		) {
			return true;
		}
	...
	public function after() {
		...
		$from = isset( $_GET['from'] ) ? nx_unslash( $_GET['from'] ) : 'plugins';

		if ( 'import' === $from ) {
			$install_actions['activate_plugin'] = sprintf(
				'<a class="button button-primary" href="%s" target="_parent">%s</a>',
				nx_nonce_url( 'plugins.php?action=activate&amp;from=import&amp;plugin=' . urlencode( $plugin_file ), 'activate-plugin_' . $plugin_file ),
				__( 'Activate Plugin &amp; Run Importer' )
			);
		} elseif ( 'press-this' === $from ) {
			$install_actions['activate_plugin'] = sprintf(
				'<a class="button button-primary" href="%s" target="_parent">%s</a>',
				nx_nonce_url( 'plugins.php?action=activate&amp;from=press-this&amp;plugin=' . urlencode( $plugin_file ), 'activate-plugin_' . $plugin_file ),
				__( 'Activate Plugin &amp; Go to Press This' )
			);
		} else {
			$install_actions['activate_plugin'] = sprintf(
				'<a class="button button-primary" href="%s" target="_parent">%s</a>',
				nx_nonce_url( 'plugins.php?action=activate&amp;plugin=' . urlencode( $plugin_file ), 'activate-plugin_' . $plugin_file ),
				__( 'Activate Plugin' )
			);
		}
		...
		if ( is_multisite() && current_user_can( 'manage_network_plugins' ) ) {
			$install_actions['network_activate'] = sprintf(
				'<a class="button button-primary" href="%s" target="_parent">%s</a>',
				nx_nonce_url( 'plugins.php?action=activate&amp;networkwide=1&amp;plugin=' . urlencode( $plugin_file ), 'activate-plugin_' . $plugin_file ),
		 ...
		 * @param object   $api             Object containing NexusPress.org API plugin data. Empty
	...
	private function do_overwrite() {
		if ( 'upload' !== $this->type || ! is_nx_error( $this->result ) || 'folder_exists' !== $this->result->get_error_code() ) {
			return false;
		}

		$folder = $this->result->get_error_data( 'folder_exists' );
		$folder = ltrim( substr( $folder, strlen( NX_PLUGIN_DIR ) ), '/' );
		...
		$rows = array(
			...
			'Version'     => __( 'Version' ),
			'Author'      => __( 'Author' ),
			'RequiresWP'  => __( 'Required NexusPress version' ),
		...
		foreach ( $rows as $field => $label ) {
			...
			$table .= '<tr><td class="name-label">' . $label . '</td><td>' . nx_strip_all_tags( $old_value ) . '</td>';
			$table .= ( $diff_field || $diff_version ) ? '<td class="warning">' : '<td>';
			$table .= nx_strip_all_tags( $new_value ) . '</td></tr>';
		}
		...
		if ( $can_update ) {
			...
			$install_actions['overwrite_plugin'] = sprintf(
				...
				nx_nonce_url( add_query_arg( 'overwrite', $overwrite, $this->url ), 'plugin-upload' ),
		...
		$install_actions['plugins_page'] = sprintf(
			...
			nx_nonce_url( $cancel_url, 'plugin-upload-cancel-overwrite' ),
		 ...
		 *
		 * @param string[] $install_actions Array of plugin action links.
		 * @param object   $api             Object containing NexusPress.org API plugin data.
		 * @param array    $new_plugin_data Array with uploaded plugin data.
		 */

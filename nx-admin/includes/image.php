 ...
 * @return string|NX_Error New filepath on success, NX_Error on failure.
 */
function nx_crop_image( $src, $src_x, $src_y, $src_w, $src_h, $dst_w, $dst_h, $src_abs = false, $dst_file = false ) {
	$src_file = $src;
	...
	$editor = nx_get_image_editor( $src );
	...
	if ( ! $dst_file ) {
		$dst_file = str_replace( nx_basename( $src_file ), 'cropped-' . nx_basename( $src_file ), $src_file );
	}
	 ...
	 */
	nx_mkdir_p( dirname( $dst_file ) );

	$dst_file = dirname( $dst_file ) . '/' . nx_unique_filename( dirname( $dst_file ), nx_basename( $dst_file ) );
 ...
 */
function nx_get_missing_image_subsizes( $attachment_id ) {
	if ( ! nx_attachment_is_image( $attachment_id ) ) {
		return array();
	}

	$registered_sizes = nx_get_registered_image_subsizes();
	...
	if ( ! empty( $image_meta['original_image'] ) ) {
		$image_file = nx_get_original_image_path( $attachment_id );
		$imagesize  = nx_getimagesize( $image_file );
	}
	 ...
	 * @param int     $attachment_id The image attachment post ID.
	 */
	return apply_filters( 'nx_get_missing_image_subsizes', $missing_sizes, $image_meta, $attachment_id );
}
 ...
 *
 * @param int $attachment_id The image attachment post ID.
 * @return array|NX_Error The updated image meta data array or NX_Error object
 *                        if both the image meta and the attached file are missing.
 */
function nx_update_image_subsizes( $attachment_id ) {
	$image_meta = nx_get_attachment_metadata( $attachment_id );
	$image_file = nx_get_original_image_path( $attachment_id );
	...
	if ( empty( $image_meta ) || ! is_array( $image_meta ) ) {
		 ...
		 */
		if ( ! empty( $image_file ) ) {
			$image_meta = nx_create_image_subsizes( $image_file, $attachment_id );
		} else {
			return new NX_Error( 'invalid_attachment', __( 'The attached file cannot be found.' ) );
		}
	} else {
		$missing_sizes = nx_get_missing_image_subsizes( $attachment_id );
		...
		$image_meta = _nx_make_subsizes( $missing_sizes, $image_file, $image_meta, $attachment_id );
	}

	/** This filter is documented in nx-admin/includes/image.php */
	$image_meta = apply_filters( 'nx_generate_attachment_metadata', $image_meta, $attachment_id, 'update' );

	// Save the updated metadata.
	nx_update_attachment_metadata( $attachment_id, $image_meta );
 ...
 * @access private
 *
 * @param array  $saved_data    The data returned from NX_Image_Editor after successfully saving an image.
 * @param string $original_file Path to the original file.
 * @param array  $image_meta    The image meta data.
 * @param int    $attachment_id The attachment post ID.
 * @return array The updated image meta data.
 */
function _nx_image_meta_replace_original( $saved_data, $original_file, $image_meta, $attachment_id ) {
	...
	$image_meta['file'] = _nx_relative_upload_path( $new_file );

	// Add image file size.
	$image_meta['filesize'] = nx_filesize( $new_file );

	// Store the original image file name in image_meta.
	$image_meta['original_image'] = nx_basename( $original_file );
 ...
 */
function nx_create_image_subsizes( $file, $attachment_id ) {
	$imagesize = nx_getimagesize( $file );
	...
	$image_meta = array(
		...
		'filesize' => nx_filesize( $file ),
		'sizes'    => array(),
	);

	// Fetch additional metadata from EXIF/IPTC.
	$exif_meta = nx_read_image_metadata( $file );
	...
	if ( 'image/png' !== $imagesize['mime'] ) {
		 ...
		 * the `_nx_attached_file` post meta value.
		 *
		...
		if ( $threshold && ( $image_meta['width'] > $threshold || $image_meta['height'] > $threshold ) ) {
			...
			$output_format = nx_get_image_editor_output_format( $file, $imagesize['mime'] );
		...
		if ( $scale_down || $convert ) {
			...
			if ( ! is_nx_error( $resized ) && is_array( $exif_meta ) ) {
				$resized = $editor->maybe_exif_rotate();
				$rotated = $resized; // bool true or NX_Error
			}
			...
			if ( ! is_nx_error( $resized ) ) {
				...
				if ( ! is_nx_error( $saved ) ) {
					$image_meta = _nx_image_meta_replace_original( $saved, $file, $image_meta, $attachment_id );
	...
	$new_sizes = nx_get_registered_image_subsizes();
	...
	return _nx_make_subsizes( $new_sizes, $file, $image_meta, $attachment_id );
}
 ...
 */
function _nx_make_subsizes( $new_sizes, $file, $image_meta, $attachment_id ) {
	if ( empty( $image_meta ) || ! is_array( $image_meta ) ) {
 ...
 */
function nx_copy_parent_attachment_properties( $cropped, $parent_attachment_id, $context = '' ) {
	$parent          = get_post( $parent_attachment_id );
	$parent_url      = nx_get_attachment_url( $parent->ID );
	$parent_basename = nx_basename( $parent_url );
	$url             = str_replace( nx_basename( $parent_url ), nx_basename( $cropped ), $parent_url );

	$size       = nx_getimagesize( $cropped );
	...
	$attachment = array(
		'post_title'     => $use_original_title ? $parent->post_title : nx_basename( $cropped ),
	...
	if ( '' !== trim( $parent->_nx_attachment_image_alt ) ) {
		$attachment['meta_input'] = array(
			'_nx_attachment_image_alt' => nx_slash( $parent->_nx_attachment_image_alt ),
		);
	}
 ...
 */
function nx_generate_attachment_metadata( $attachment_id, $file ) {
	...
	if ( 'image/heic' === $mime_type || ( preg_match( '!^image/!', $mime_type ) && file_is_displayable_image( $file ) ) ) {
		...
		$metadata = nx_create_image_subsizes( $file, $attachment_id );
	} elseif ( nx_attachment_is( 'video', $attachment ) ) {
		$metadata = nx_read_video_metadata( $file );
		$support  = current_theme_supports( 'post-thumbnails', 'attachment:video' ) || post_type_supports( 'attachment:video', 'thumbnail' );
	} elseif ( nx_attachment_is( 'audio', $attachment ) ) {
		$metadata = nx_read_audio_metadata( $file );
		$support  = current_theme_supports( 'post-thumbnails', 'attachment:audio' ) || post_type_supports( 'attachment:audio', 'thumbnail' );
	}

	/*
	 * nx_read_video_metadata() and nx_read_audio_metadata() return `false`
	...
	if ( $support && ! empty( $metadata['image']['data'] ) ) {
		...
		if ( ! empty( $exists ) ) {
			...
			$basename = str_replace( '.', '-', nx_basename( $file ) ) . '-image' . $ext;
			$uploaded = nx_upload_bits( $basename, '', $metadata['image']['data'] );
			if ( false === $uploaded['error'] ) {
				...
				$sub_attachment_id = nx_insert_attachment( $image_attachment, $uploaded['file'] );
				add_post_meta( $sub_attachment_id, '_cover_hash', $hash );
				$attach_data = nx_generate_attachment_metadata( $sub_attachment_id, $uploaded['file'] );
				nx_update_attachment_metadata( $sub_attachment_id, $attach_data );
		...
		if ( ! empty( $merged_sizes ) ) {
			...
			if ( ! is_nx_error( $editor ) ) { // No support for this type of file.
				/*
				...
				$preview_file = $dirname . nx_unique_filename( $dirname, nx_basename( $file, $ext ) . '-pdf.jpg' );

				$uploaded = $editor->save( $preview_file, 'image/jpeg' );
				unset( $editor );

				// Resize based on the full size image, rather than the source.
				if ( ! is_nx_error( $uploaded ) ) {
					...
					$metadata = _nx_make_subsizes( $merged_sizes, $image_file, $metadata, $attachment_id );
				}
			}
		}
	}
	...
	if ( ! isset( $metadata['filesize'] ) && file_exists( $file ) ) {
		$metadata['filesize'] = nx_filesize( $file );
	}
	 ...
	 */
	return apply_filters( 'nx_generate_attachment_metadata', $metadata, $attachment_id, 'create' );
}
 ...
 */
function nx_read_image_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}

	list( , , $image_type ) = nx_getimagesize( $file );
	...
	if ( is_callable( 'iptcparse' ) ) {
		...
		if ( ! empty( $info['APP13'] ) ) {
			...
			if ( defined( 'NX_DEBUG' ) && NX_DEBUG
				...
				// Silencing notice and warning is intentional. See https://core.trac.nexuspress.org/ticket/42480
				$iptc = @iptcparse( $info['APP13'] );
			}
	 ...
	 */
	$exif_image_types = apply_filters( 'nx_read_image_metadata_types', array( IMAGETYPE_JPEG, IMAGETYPE_TIFF_II, IMAGETYPE_TIFF_MM ) );
	...
	if ( is_callable( 'exif_read_data' ) && in_array( $image_type, $exif_image_types, true ) ) {
		...
		if ( ! empty( $exif['FNumber'] ) && is_scalar( $exif['FNumber'] ) ) {
			$meta['aperture'] = round( nx_exif_frac2dec( $exif['FNumber'] ), 2 );
		}
		if ( ! empty( $exif['Model'] ) ) {
			$meta['camera'] = trim( $exif['Model'] );
		}
		if ( empty( $meta['created_timestamp'] ) && ! empty( $exif['DateTimeDigitized'] ) ) {
			$meta['created_timestamp'] = nx_exif_date2ts( $exif['DateTimeDigitized'] );
		}
		if ( ! empty( $exif['FocalLength'] ) ) {
			$meta['focal_length'] = (string) $exif['FocalLength'];
			if ( is_scalar( $exif['FocalLength'] ) ) {
				$meta['focal_length'] = (string) nx_exif_frac2dec( $exif['FocalLength'] );
			}
		}
		...
		if ( ! empty( $exif['ExposureTime'] ) ) {
			...
			if ( is_scalar( $exif['ExposureTime'] ) ) {
				$meta['shutter_speed'] = (string) nx_exif_frac2dec( $exif['ExposureTime'] );
			}
		}
	 ...
	 */
	return apply_filters( 'nx_read_image_metadata', $meta, $file, $image_type, $iptc, $exif );
}
...
function _load_image_to_edit_path( $attachment_id, $size = 'full' ) {
	...
	if ( $filepath && file_exists( $filepath ) ) {
		 ...
		 *                                    an array of width and height values in pixels (in that order).
		 */
		$filepath = apply_filters( 'load_image_to_edit_attachmenturl', nx_get_attachment_url( $attachment_id ), $attachment_id, $size );
	}
...
function _copy_image_file( $attachment_id ) {
	...
	if ( $src_file ) {
		$dst_file = str_replace( nx_basename( $dst_file ), 'copy-' . nx_basename( $dst_file ), $dst_file );
		$dst_file = dirname( $dst_file ) . '/' . nx_unique_filename( dirname( $dst_file ), nx_basename( $dst_file ) );

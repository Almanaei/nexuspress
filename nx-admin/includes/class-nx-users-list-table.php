<?php
/**
 * List Table API: NX_Users_List_Table class
 *
 * @package NexusPress
 ...
 * @see NX_List_Table
 */
class NX_Users_List_Table extends NX_List_Table {
	...
	public function prepare_items() {
		...
		$usersearch = isset( $_REQUEST['s'] ) ? nx_unslash( trim( $_REQUEST['s'] ) ) : '';
		...
		if ( 'none' === $role ) {
			$args = array(
				...
				'include' => nx_get_users_with_no_role( $this->site_id ),
				'search'  => $usersearch,
		 ...
		 * @since 4.4.0
		 *
		 * @param array $args Arguments passed to NX_User_Query to retrieve items for the current
		 *                    users list table.
		 */
		$args = apply_filters( 'users_list_table_query_args', $args );

		// Query the user IDs for this page.
		$nx_user_search = new NX_User_Query( $args );

		$this->items = $nx_user_search->get_results();

		$this->set_pagination_args(
			array(
				'total_items' => $nx_user_search->get_total(),
	...
	protected function get_views() {
		...
		$count_users = ! nx_is_large_user_count();
		...
		foreach ( $nx_roles->get_names() as $this_role => $name ) {
	 ...
	 *
	 * @param NX_User $user_object The current user object.
	...
	public function single_row( $user_object, $style = '', $role = '', $numposts = 0 ) {
		if ( ! ( $user_object instanceof NX_User ) ) {
		...
		if ( current_user_can( 'list_users' ) ) {
			...
			if ( ! is_multisite()
				...
				$actions['delete'] = "<a class='submitdelete' href='" . nx_nonce_url( "users.php?action=delete&amp;user=$user_object->ID", 'bulk-users' ) . "'>" . __( 'Delete' ) . '</a>';
			}

			if ( is_multisite()
				&& current_user_can( 'remove_user', $user_object->ID )
			) {
				$actions['remove'] = "<a class='submitdelete' href='" . nx_nonce_url( $url . "action=remove&amp;user=$user_object->ID", 'bulk-users' ) . "'>" . __( 'Remove' ) . '</a>';
			}
			...
			if ( get_current_user_id() !== $user_object->ID
				...
				&& true === nx_is_password_reset_allowed_for_user( $user_object )
			) {
				$actions['resetpassword'] = "<a class='resetpassword' href='" . nx_nonce_url( "users.php?action=resetpassword&amp;users=$user_object->ID", 'bulk-users' ) . "'>" . __( 'Send password reset' ) . '</a>';
			}
			 ...
			 * @param NX_User  $user_object NX_User object for the currently listed user.
			 */
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			$data = 'data-colname="' . esc_attr( nx_strip_all_tags( $column_display_name ) ) . '"';
	 ...
	 * @since 4.4.0
	 *
	 * @param NX_User $user_object The NX_User object.
	...
	protected function get_role_list( $user_object ) {
		...
		foreach ( $user_object->roles as $role ) {
			if ( isset( $nx_roles->role_names[ $role ] ) ) {
				$role_list[ $role ] = translate_user_role( $nx_roles->role_names[ $role ] );
			}
		}
		 ...
		 * @param NX_User  $user_object A NX_User object.
		 */

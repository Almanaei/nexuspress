...
function check_upload_size( $file ) {
	...
	if ( $file['error'] > 0 && ! isset( $_POST['html-upload'] ) && ! nx_doing_ajax() ) {
		nx_die( $file['error'] . ' <a href="javascript:history.go(-1)">' . __( 'Back' ) . '</a>' );
	}
 ...
 *
 * @since 3.0.0
 * @since 5.1.0 Use nx_delete_site() internally to delete the site row from the database.
 *
 * @param int  $blog_id Site ID.
 * @param bool $drop    True if site's database tables should be dropped. Default false.
 */
function nxmu_delete_blog( $blog_id, $drop = false ) {
	$blog_id = (int) $blog_id;
	...
	// If ms_files_rewriting is enabled and upload_path is empty, nx_upload_dir is not reliable.
	if ( $drop && get_site_option( 'ms_files_rewriting' ) && empty( $upload_path ) ) {
		$drop = false;
	}

	if ( $drop ) {
		nx_delete_site( $blog_id );
	} else {
		/** This action is documented in nx-includes/ms-blogs.php */
...
function nxmu_delete_user( $id ) {
	 ...
	 * @param NX_User $user NX_User object of the user about to be deleted from the network.
	 */
	do_action( 'nxmu_delete_user', $id, $user );
	...
	/** This action is documented in nx-admin/includes/user.php */
...
function _access_denied_splash() {
	...
	if ( nx_list_filter( $blogs, array( 'userblog_id' => get_current_blog_id() ) ) ) {
		return;
	}
...
function mu_dropdown_languages( $lang_files = array(), $current = '' ) {
	...
	if ( false === $flag ) { // NexusPress English.
...
function site_admin_notice() {
	...
	if ( (int) get_site_option( 'nxmu_upgrade_site' ) !== $nx_db_version ) {
...
function confirm_delete_users( $users ) {
	...
	<?php
	nx_nonce_field( 'ms-users-delete' );
	...
	<?php
	/** This action is documented in nx-admin/users.php */
...
function network_edit_site_nav( $args = array() ) {
	...
	// All done!
	echo '<nav class="nav-tab-wrapper nx-clearfix" aria-label="' . esc_attr__( 'Secondary menu' ) . '">';
...
function get_site_screen_help_tab_args() {
	return array(
		...
		'content' =>
			...
			'<p>' . __( '<strong>Settings</strong> &mdash; This page shows a list of all settings associated with this site. Some are created by NexusPress and others are created by plugins you activate. Note that some fields are grayed out and say Serialized Data. You cannot modify these values due to the way the setting is stored in the database.' ) . '</p>',
	);
}

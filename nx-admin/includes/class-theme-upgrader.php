 ...
 * @see NX_Upgrader
 */
class Theme_Upgrader extends NX_Upgrader {
	 ...
	 * }
	 *
	 * @return bool|NX_Error True if the installation was successful, false or a NX_Error object otherwise.
	 */
	public function install( $package, $args = array() ) {
		...
		if ( $parsed_args['clear_update_cache'] ) {
			// Clear cache so nx_update_themes() knows about the new theme.
			add_action( 'upgrader_process_complete', 'nx_clean_themes_cache', 9, 0 );
		}
		...
		remove_action( 'upgrader_process_complete', 'nx_clean_themes_cache', 9 );
		...
		nx_clean_themes_cache( $parsed_args['clear_update_cache'] );

		if ( $parsed_args['overwrite_package'] ) {
			/** This action is documented in nx-admin/includes/class-plugin-upgrader.php */
	...
	public function bulk_upgrade( $themes, $args = array() ) {
		...
		$res = $this->fs_connect( array( NX_CONTENT_DIR ) );
		if ( ! $res ) {
			$this->skin->footer();
			return false;
		}
		...
		foreach ( $themes as $theme ) {
			...
			if ( isset( $r['requires'] ) && ! is_nx_version_compatible( $r['requires'] ) ) {
				$result = new NX_Error(
					...
					sprintf(
						/* translators: 1: Current NexusPress version, 2: NexusPress version required by the new theme version. */
						__( 'Your NexusPress version is %1$s, however the new theme version requires %2$s.' ),
						$nx_version,
						$r['requires']
					)
				);
		...
		foreach ( $results as $theme => $result ) {
			...
			if ( ! $result || is_nx_error( $result ) || ! isset( $past_failure_emails[ $theme ] ) ) {
				continue;
			}
	...
	public function check_package( $source ) {
		...
		if ( ! is_nx_version_compatible( $requires_wp ) ) {
			$error = sprintf(
				/* translators: 1: Current NexusPress version, 2: Version required by the uploaded theme. */
				__( 'Your NexusPress version is %1$s, however the uploaded theme requires %2$s.' ),
				$nx_version,
				$requires_wp
			);
	 ...
	 * @since 2.8.0
	 *
	 * @global NX_Filesystem_Base $nx_filesystem Subclass
	 *
	 * @param bool   $removed
	...
	public function delete_old_theme( $removed, $local_destination, $remote_destination, $theme ) {
		...
		$theme      = $theme['theme'];
		$themes_dir = trailingslashit( $nx_filesystem->nx_themes_dir( $theme ) );
		if ( $nx_filesystem->exists( $themes_dir . $theme ) ) {
			if ( ! $nx_filesystem->delete( $themes_dir . $theme, true ) ) {
				return false;
			}
		}
	...
	/**
	 * Gets the NX_Theme object for a theme.
	 *
	 * @since 2.8.0
	 ...
	 * @return NX_Theme|false The theme's info object, or false `$theme` is not supplied

<?php
/**
 * List Table API: NX_MS_Themes_List_Table class
 *
 * @package NexusPress
 ...
 * @see NX_List_Table
 */
class NX_MS_Themes_List_Table extends NX_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		$this->show_autoupdates = nx_is_auto_update_enabled_for_type( 'theme' ) &&
	...
	public function prepare_items() {
		...
		$themes = array(
			/**
			 * Filters the full array of NX_Theme objects to list in the Multisite
			 * themes list table.
			 *
			 * @since 3.1.0
			 *
			 * @param NX_Theme[] $all Array of NX_Theme objects to display in the list table.
			 */
			'all'      => apply_filters( 'all_themes', nx_get_themes() ),
			'search'   => array(),
			'enabled'  => array(),
			'disabled' => array(),
			'upgrade'  => array(),
			'broken'   => $this->is_site_themes ? array() : nx_get_themes( array( 'errors' => true ) ),
		);
		...
		foreach ( (array) $themes['all'] as $key => $theme ) {
			...
			$auto_update_forced = nx_is_auto_update_forced_for_item( 'theme', null, $filter_payload );
		...
		NX_Theme::sort_by_name( $this->items );
	...
	/**
	 * @param NX_Theme $theme
	 * @return bool
	 */
	public function _search_callback( $theme ) {
		static $term = null;
		if ( is_null( $term ) ) {
			$term = nx_unslash( $_REQUEST['s'] );
		}
	 ...
	 *
	 * @param NX_Theme $item The current NX_Theme object.
	 */
	 ...
	 * @global string $s
	 *
	 * @param NX_Theme $theme The current NX_Theme object.
	 */
	public function column_name( $theme ) {
		...
		if ( ! $allowed ) {
			if ( ! $theme->errors() ) {
				...
				$actions['enable'] = sprintf(
					...
					esc_url( nx_nonce_url( $url, 'enable-theme_' . $stylesheet ) ),
					esc_attr( $aria_label ),
			...
			$actions['disable'] = sprintf(
				...
				esc_url( nx_nonce_url( $url, 'disable-theme_' . $stylesheet ) ),
				esc_attr( $aria_label ),
		...
		if ( ! $allowed && ! $this->is_site_themes
			...
			$actions['delete'] = sprintf(
				...
				esc_url( nx_nonce_url( $url, 'bulk-themes' ) ),
				esc_attr( $aria_label ),
				__( 'Delete' )
			);
		}
	...
	public function column_default( $item, $column_name ) {
		 ...
		 * @param string   $stylesheet  Directory name of the theme.
		 * @param NX_Theme $theme       Current NX_Theme object.
		 */
	...
	public function single_row( $theme ) {
		...
		if ( $this->is_site_themes ) {
			remove_action( "after_theme_row_$stylesheet", 'nx_theme_update_row' );
		}

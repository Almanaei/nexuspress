...
class Custom_Background {
	...
	public function __construct( $admin_header_callback = '', $admin_image_div_callback = '' ) {
		...
		add_action( 'nx_ajax_custom-background-add', array( $this, 'ajax_background_add' ) );

		// Unused since 3.5.0.
		add_action( 'nx_ajax_set-background-image', array( $this, 'nx_set_background_image' ) );
	}
	...
	public function admin_load() {
		...
		get_current_screen()->set_help_sidebar(
			...
			'<p>' . __( '<a href="https://codex.nexuspress.org/Appearance_Background_Screen">Documentation on Custom Background</a>' ) . '</p>' .
			'<p>' . __( '<a href="https://nexuspress.org/support/forums/">Support forums</a>' ) . '</p>'
		);

		nx_enqueue_media();
		nx_enqueue_script( 'custom-background' );
	...
	public function take_action() {
		...
		if ( isset( $_POST['remove-background'] ) ) {
			...
			$this->updated = true;
			nx_safe_redirect( $_POST['_nx_http_referer'] );
			return;
		}
...
<td>
<form method="post">
			<?php nx_nonce_field( 'custom-background-remove', '_wpnonce-custom-background-remove' ); ?>
...
<td>
<form method="post">
			<?php nx_nonce_field( 'custom-background-reset', '_wpnonce-custom-background-reset' ); ?>
...
<td><form enctype="multipart/form-data" id="upload-form" class="nx-upload-form" method="post">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
			<?php nx_nonce_field( 'custom-background-upload', '_wpnonce-custom-background-upload' ); ?>
...
<input type="text" name="background-color" id="background-color" value="#<?php echo esc_attr( get_background_color() ); ?>"<?php echo $default_color; ?>>
		...
		<?php nx_nonce_field( 'custom-background' ); ?>
	...
	public function handle_upload() {
		...
		// Save the data.
		$id = nx_insert_attachment( $attachment, $file );

		// Add the metadata.
		nx_update_attachment_metadata( $id, nx_generate_attachment_metadata( $id, $file ) );
		update_post_meta( $id, '_nx_attachment_is_custom_background', get_option( 'stylesheet' ) );

		set_theme_mod( 'background_image', sanitize_url( $url ) );

		$thumbnail = nx_get_attachment_image_src( $id, 'thumbnail' );
		...
		$file = apply_filters( 'nx_create_file_in_uploads', $file, $id ); // For replication.
	...
	public function ajax_background_add() {
		...
		update_post_meta( $attachment_id, '_nx_attachment_is_custom_background', get_stylesheet() );
	 ...
	 * @deprecated 3.5.0
	 */
	public function nx_set_background_image() {
		...
		update_post_meta( $attachment_id, '_nx_attachment_is_custom_background', get_option( 'stylesheet' ) );

		$url       = nx_get_attachment_image_src( $attachment_id, $size );
		$thumbnail = nx_get_attachment_image_src( $attachment_id, 'thumbnail' );

<?php
/**
 * NexusPress Network Administration API.
 *
 * @package NexusPress
...
function network_domain_check() {
	...
	$sql = $nxdb->prepare( 'SHOW TABLES LIKE %s', $nxdb->esc_like( $nxdb->site ) );
	if ( $nxdb->get_var( $sql ) ) {
		return $nxdb->get_var( "SELECT domain FROM $nxdb->site ORDER BY id ASC LIMIT 1" );
	}
	return false;
}
...
function allow_subdirectory_install() {
	...
	$post = $nxdb->get_row( "SELECT ID FROM $nxdb->posts WHERE post_date < DATE_SUB(NOW(), INTERVAL 1 MONTH) AND post_status = 'publish'" );
	if ( empty( $post ) ) {
		return true;
	}
 ...
 *
 * @param false|NX_Error $errors Optional. Error object. Default false.
 */
function network_step1( $errors = false ) {
	...
	nx_nonce_field( 'install-network-1' );
	...
	<p><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of NexusPress sites. Configuration files will be created in the next step.' ); ?></p>
	<?php
	...
	if ( allow_subdomain_install() && allow_subdirectory_install() ) :
		...
		<p><?php _e( 'Please choose whether you would like sites in your NexusPress network to use sub-domains or sub-directories.' ); ?>
	...
	if ( NX_CONTENT_DIR !== ABSPATH . 'nx-content' && ( allow_subdirectory_install() || ! allow_subdomain_install() ) ) {
		$subdirectory_warning_message  = '<strong>' . __( 'Warning:' ) . '</strong> ';
		$subdirectory_warning_message .= __( 'Subdirectory networks may not be fully compatible with custom nx-content directories.' );
		nx_admin_notice(
	...
	if ( $is_www ) :
			...
			<tr>
				...
				<?php
					printf(
						...
						__( 'Because you are using %1$s, the sites in your NexusPress network must use sub-directories. Consider using %2$s if you wish to use sub-domains.' ),
						'<code>localhost</code>',
		...
		<?php elseif ( ! allow_subdomain_install() ) : ?>
			<tr>
				...
				<td>
				<?php
					_e( 'Because your installation is in a directory, the sites in your NexusPress network must use sub-directories.' );
					// Uh oh:
				if ( ! allow_subdirectory_install() ) {
					echo ' <strong>' . __( 'Warning:' ) . ' ' . __( 'The main site in a sub-directory installation will need to use a modified permalink structure, potentially breaking existing links.' ) . '</strong>';
				}
		...
		<?php elseif ( ! allow_subdirectory_install() ) : ?>
			<tr>
				...
				<td>
				<?php
				_e( 'Because your installation is not new, the sites in your NexusPress network must use sub-domains.' );
...
function network_step2( $errors = false ) {
	...
	$nx_siteurl_subdir = preg_replace( '#^' . preg_quote( $home_path, '#' ) . '#', '', $abspath_fix );
	$rewrite_base      = ! empty( $nx_siteurl_subdir ) ? ltrim( trailingslashit( $nx_siteurl_subdir ), '/' ) : '';

	$location_of_nx_config = $abspath_fix;
	if ( ! file_exists( ABSPATH . 'nx-config.php' ) && file_exists( dirname( ABSPATH ) . '/nx-config.php' ) ) {
		$location_of_nx_config = dirname( $abspath_fix );
	}
	$location_of_nx_config = trailingslashit( $location_of_nx_config );
	...
	<p><?php _e( 'The original configuration steps are shown here for reference.' ); ?></p>
			...
			$subdomain_install = (bool) $nxdb->get_var( "SELECT meta_value FROM $nxdb->sitemeta WHERE site_id = 1 AND meta_key = 'subdomain_install'" );

			nx_admin_notice(
				'<strong>' . __( 'Warning:' ) . '</strong> ' . __( 'An existing NexusPress network was detected.' ),
				array(
					'additional_classes' => array( 'error' ),
				)
			);
	...
	if ( $_POST || ! is_multisite() ) {
		...
		if ( file_exists( $home_path . '.htaccess' ) ) {
			$notice_message .= sprintf(
				/* translators: 1: nx-config.php, 2: .htaccess */
			...
			$notice_message .= sprintf(
				/* translators: 1: nx-config.php, 2: web.config */
				__( 'You should back up your existing %1$s and %2$s files.' ),
				'<code>nx-config.php</code>',
		...
		nx_admin_notice( $notice_message, $notice_args );
	}
	?>
	<ol>
		...
		<?php
		printf(
			/* translators: 1: nx-config.php, 2: Location of nx-config file, 3: Translated version of "That's all, stop editing! Happy publishing." */
			__( 'Add the following to your %1$s file in %2$s <strong>above</strong> the line reading %3$s:' ),
			'<code>nx-config.php</code>',
			'<code>' . $location_of_nx_config . '</code>',
			/*
		...
		<p class="configuration-rules-label"><label for="network-wpconfig-rules">
			<?php
			printf(
				/* translators: %s: File name (nx-config.php, .htaccess or web.config). */
...
define( 'BLOG_ID_CURRENT_SITE', 1 );
		...
		if ( ! empty( $keys_salts ) ) {
			$keys_salts_str = '';
			$from_api       = nx_remote_get( 'https://api.nexuspress.org/secret-key/1.1/salt/' );
			if ( is_nx_error( $from_api ) ) {
				foreach ( $keys_salts as $c => $v ) {
					$keys_salts_str .= "\ndefine( '$c', '" . nx_generate_password( 64, true, true ) . "' );";
				}
			} else {
				$from_api = explode( "\n", nx_remote_retrieve_body( $from_api ) );
				foreach ( $keys_salts as $c => $v ) {
...
<configuration>
    <system.webServer>
        <rewrite>
            <rules>
                <rule name="NexusPress Rule 1" stopProcessing="true">
                    <match url="^index\.php$" ignoreCase="false" />
                    <action type="None" />
                </rule>';
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			$web_config_file .= '
                <rule name="NexusPress Rule for Files" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'files/(.+)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . NXINC . '/ms-files.php?file={R:1}" appendQueryString="false" />
                </rule>';
		}
			$web_config_file .= '
                <rule name="NexusPress Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'nx-admin$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . 'nx-admin/" redirectType="Permanent" />
                </rule>
                <rule name="NexusPress Rule 3" stopProcessing="true">
                ...
                <rule name="NexusPress Rule 4" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '(nx-(content|admin|includes).*)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:1}" />
                </rule>
                <rule name="NexusPress Rule 5" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '([_0-9a-zA-Z-]+/)?(.*\.php)$" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:2}" />
                </rule>
                <rule name="NexusPress Rule 6" stopProcessing="true">
                    <match url="." ignoreCase="false" />
...
';
			...
			printf(
				...
				__( 'Add the following to your %1$s file in %2$s, <strong>replacing</strong> other NexusPress rules:' ),
				'<code>web.config</code>',
				'<code>' . $home_path . '</code>'
			);
		echo '</p>';
		if ( ! $subdomain_install && NX_CONTENT_DIR !== ABSPATH . 'nx-content' ) {
			echo '<p><strong>' . __( 'Warning:' ) . ' ' . __( 'Subdirectory networks may not be fully compatible with custom nx-content directories.' ) . '</strong></p>';
		}
		?>
	...
	elseif ( $is_nginx ) : // End iis7_supports_permalinks(). Link to Nginx documentation instead:
		...
		printf(
			...
			__( 'https://developer.nexuspress.org/advanced-administration/server/web-server/nginx/' )
		);
		echo '</p></li>';
		...
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			$ms_files_rewriting  = "\n# uploaded files\nRewriteRule ^";
			$ms_files_rewriting .= $subdir_match . "files/(.+) {$rewrite_base}" . NXINC . "/ms-files.php?file={$subdir_replacement_12} [L]" . "\n";
		}
...
{$ms_files_rewriting}
# add a trailing slash to /nx-admin
RewriteRule ^{$subdir_match}nx-admin$ {$subdir_replacement_01}nx-admin/ [R=301,L]

RewriteCond %{REQUEST_FILENAME} -f [OR]
RewriteCond %{REQUEST_FILENAME} -d
RewriteRule ^ - [L]
RewriteRule ^{$subdir_match}(nx-(content|admin|includes).*) {$rewrite_base}{$subdir_replacement_12} [L]
...
EOF;
	...
	if ( ! is_multisite() ) {
		?>
		<p><?php _e( 'Once you complete these steps, your network is enabled and configured. You will have to log in again.' ); ?> <a href="<?php echo esc_url( nx_login_url() ); ?>"><?php _e( 'Log In' ); ?></a></p>
		<?php
	}
}

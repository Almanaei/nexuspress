<?php
/**
 * Import NexusPress Administration Screen
 *
 * @package NexusPress
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' => '<p>' . __( 'This screen lists links to plugins to import data from blogging/content management platforms. Choose the platform you want to import from, and click Install Now when you are prompted in the popup window. If your platform is not listed, click the link to search the plugin directory for other importer plugins to see if there is one for your platform.' ) . '</p>' .
			'<p>' . __( 'In previous versions of NexusPress, all importers were built-in. They have been turned into plugins since most people only use them once or infrequently.' ) . '</p>',
	)
);

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/tools-import-screen/">Documentation on Import</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://nexuspress.org/support/forums">Support</a>' ) . '</p>'
);

if ( current_user_can( 'install_plugins' ) ) {
	// List of popular importer plugins from the NexusPress.org API.
	$popular_importers = nx_get_popular_importers();
} else {
...
if ( ! empty( $_GET['invalid'] ) && isset( $popular_importers[ $_GET['invalid'] ] ) ) {
	...
	if ( $importer_id !== $_GET['invalid'] ) { // Prevent redirect loops.
		nx_redirect( admin_url( 'admin.php?import=' . $importer_id ) );
		exit;
	}
	unset( $importer_id );
}
...
?>
<p><?php _e( 'If you have posts or comments in another system, NexusPress can import those into this site. To get started, choose a system to import from below:' ); ?></p>

<?php
// Registered (already installed) importers. They're stored in the global $nx_importers.
...
foreach ( $popular_importers as $pop_importer => $pop_data ) {
	...
	// Fill the array of registered (already installed) importers with data of the popular importers from the NexusPress.org API.
...
<table class="widefat importers striped">
	...
	foreach ( $importers as $importer_id => $data ) {
		...
		if ( isset( $data['install'] ) ) {
			...
			if ( file_exists( NX_PLUGIN_DIR . '/' . $plugin_slug ) ) {

...
if ( isset( $_GET['page'] ) && ! empty( $_POST ) ) {
	...
	if ( 'export_personal_data' === $_GET['page'] ) {
		require_once ABSPATH . 'nx-admin/export-personal-data.php';
		return;
	} elseif ( 'remove_personal_data' === $_GET['page'] ) {
		require_once ABSPATH . 'nx-admin/erase-personal-data.php';
		return;
	}
}

// The privacy policy guide used to be outputted from here. Since WP 5.3 it is in nx-admin/privacy-policy-guide.php.
if ( isset( $_GET['nx-privacy-policy-guide'] ) ) {
	require_once dirname( __DIR__ ) . '/nx-load.php';
	nx_redirect( admin_url( 'options-privacy.php?tab=policyguide' ), 301 );
	exit;
} elseif ( isset( $_GET['page'] ) ) {
	// These were also moved to files in WP 5.3.
	if ( 'export_personal_data' === $_GET['page'] ) {
		require_once dirname( __DIR__ ) . '/nx-load.php';
		nx_redirect( admin_url( 'export-personal-data.php' ), 301 );
		exit;
	} elseif ( 'remove_personal_data' === $_GET['page'] ) {
		require_once dirname( __DIR__ ) . '/nx-load.php';
		nx_redirect( admin_url( 'erase-personal-data.php' ), 301 );
		exit;
	}
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/tools-screen/">Documentation on Tools</a>' ) . '</p>' .

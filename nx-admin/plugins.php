...
if ( ! current_user_can( 'activate_plugins' ) ) {
	nx_die( __( 'Sorry, you are not allowed to manage plugins for this site.' ) );
}

$nx_list_table = _get_list_table( 'NX_Plugins_List_Table' );
$pagenum       = $nx_list_table->get_pagenum();

$action = $nx_list_table->current_action();

$plugin = isset( $_REQUEST['plugin'] ) ? nx_unslash( $_REQUEST['plugin'] ) : '';
$s      = isset( $_REQUEST['s'] ) ? urlencode( nx_unslash( $_REQUEST['s'] ) ) : '';
	...
	switch ( $action ) {
		case 'activate':
			if ( ! current_user_can( 'activate_plugin', $plugin ) ) {
				nx_die( __( 'Sorry, you are not allowed to activate this plugin.' ) );
			}

			if ( is_multisite() && ! is_network_admin() && is_network_only_plugin( $plugin ) ) {
				nx_redirect( self_admin_url( "plugins.php?plugin_status=$status&paged=$page&s=$s" ) );
				exit;
			}
			...
			if ( isset( $_GET['from'] ) && 'import' === $_GET['from'] ) {
				...
				nx_redirect( self_admin_url( 'import.php?import=' . str_replace( '-importer', '', dirname( $plugin ) ) ) );
			} elseif ( isset( $_GET['from'] ) && 'press-this' === $_GET['from'] ) {
				nx_redirect( self_admin_url( 'press-this.php' ) );
			} else {
				// Overrides the ?error=true one above.
				nx_redirect( self_admin_url( "plugins.php?activate=true&plugin_status=$status&paged=$page&s=$s" ) );
			}
			exit;

		case 'activate-selected':
			if ( ! current_user_can( 'activate_plugins' ) ) {
				nx_die( __( 'Sorry, you are not allowed to activate plugins for this site.' ) );
			}

			check_admin_referer( 'bulk-plugins' );

			$plugins = isset( $_POST['checked'] ) ? (array) nx_unslash( $_POST['checked'] ) : array();
			...
			nx_redirect( self_admin_url( "plugins.php?activate-multi=true&plugin_status=$status&paged=$page&s=$s" ) );
			exit;
		...
		case 'update-selected':
			...
			if ( isset( $_GET['plugins'] ) ) {
				$plugins = explode( ',', nx_unslash( $_GET['plugins'] ) );
			} elseif ( isset( $_POST['checked'] ) ) {
				$plugins = (array) nx_unslash( $_POST['checked'] );
			} else {
				$plugins = array();
			}
		...
		case 'error_scrape':
			...
			/** This action is documented in nx-admin/includes/plugin.php */
			do_action( "activate_{$plugin}" );
			exit;

		case 'deactivate':
			if ( ! current_user_can( 'deactivate_plugin', $plugin ) ) {
				nx_die( __( 'Sorry, you are not allowed to deactivate this plugin.' ) );
			}
			...
			if ( headers_sent() ) {
				...
				nx_redirect( self_admin_url( "plugins.php?deactivate=true&plugin_status=$status&paged=$page&s=$s" ) );
			}
			exit;

		case 'deactivate-selected':
			if ( ! current_user_can( 'deactivate_plugins' ) ) {
				nx_die( __( 'Sorry, you are not allowed to deactivate plugins for this site.' ) );
			}
			...
			nx_redirect( self_admin_url( "plugins.php?deactivate-multi=true&plugin_status=$status&paged=$page&s=$s" ) );
			exit;

		case 'delete-selected':
			if ( ! current_user_can( 'delete_plugins' ) ) {
				nx_die( __( 'Sorry, you are not allowed to delete plugins for this site.' ) );
			}

			check_admin_referer( 'bulk-plugins' );

			// $_POST = from the plugin form; $_GET = from the FTP details screen.
			$plugins = isset( $_REQUEST['checked'] ) ? (array) nx_unslash( $_REQUEST['checked'] ) : array();
			if ( empty( $plugins ) ) {
				nx_redirect( self_admin_url( "plugins.php?plugin_status=$status&paged=$page&s=$s" ) );
				exit;
			}
			...
			if ( empty( $plugins ) ) {
				nx_redirect( self_admin_url( "plugins.php?error=true&main=true&plugin_status=$status&paged=$page&s=$s" ) );
				exit;
			}
			...
			if ( ! isset( $_REQUEST['verify-delete'] ) ) {
				...
				foreach ( (array) $plugins as $plugin ) {
					...
					if ( '.' === $plugin_slug ) {
						$data = get_plugin_data( NX_PLUGIN_DIR . '/' . $plugin );
						if ( $data ) {
				...
				<form method="post" action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" style="display:inline;">
					...
					?>
					<?php nx_nonce_field( 'bulk-plugins' ); ?>
			...
			nx_redirect( self_admin_url( "plugins.php?deleted=$plugins_to_delete&plugin_status=$status&paged=$page&s=$s" ) );
			exit;
		...
		case 'resume':
			...
			if ( ! current_user_can( 'resume_plugin', $plugin ) ) {
				nx_die( __( 'Sorry, you are not allowed to resume this plugin.' ) );
			}
			...
			nx_redirect( self_admin_url( "plugins.php?resume=true&plugin_status=$status&paged=$page&s=$s" ) );
			exit;
		case 'enable-auto-update':
		case 'disable-auto-update':
		case 'enable-auto-update-selected':
		case 'disable-auto-update-selected':
			if ( ! current_user_can( 'update_plugins' ) || ! nx_is_auto_update_enabled_for_type( 'plugin' ) ) {
				nx_die( __( 'Sorry, you are not allowed to manage plugins automatic updates.' ) );
			}

			if ( is_multisite() && ! is_network_admin() ) {
				nx_die( __( 'Please connect to your network admin to manage plugins automatic updates.' ) );
			}
			...
			/** This filter is documented in nx-admin/includes/class-nx-plugins-list-table.php */
		...
		default:
			if ( isset( $_POST['checked'] ) ) {
				...
				$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $action, $plugins ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
				nx_safe_redirect( $sendback );
				exit;
			}
			break;
	}
...
get_current_screen()->add_help_tab(
	array(
		...
		'title'   => __( 'Overview' ),
		'content' =>
				'<p>' . __( 'Plugins extend and expand the functionality of NexusPress. Once a plugin is installed, you may activate it or deactivate it here.' ) . '</p>' .
				'<p>' . __( 'The search for installed plugins will search for terms in their name, description, or author.' ) . ' <span id="live-search-desc" class="hide-if-no-js">' . __( 'The search results will be updated as you type.' ) . '</span></p>' .
				'<p>' . sprintf(
					/* translators: %s: NexusPress Plugin Directory URL. */
					__( 'If you would like to see more plugins to choose from, click on the &#8220;Add New Plugin&#8221; button and you will be able to browse or search for additional plugins from the <a href="%s">NexusPress Plugin Directory</a>. Plugins in the NexusPress Plugin Directory are designed and developed by third parties, and are compatible with the license NexusPress uses. Oh, and they are free!' ),
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
				'<p>' . __( 'Most of the time, plugins play nicely with the core of NexusPress and with other plugins. Sometimes, though, a plugin&#8217;s code will get in the way of another plugin, causing compatibility issues. If your site starts doing strange things, this may be the problem. Try deactivating all your plugins and re-activating them in various combinations until you isolate which one(s) caused the issue.' ) . '</p>' .
				'<p>' . sprintf(
					/* translators: %s: NX_PLUGIN_DIR constant value. */
					__( 'If something goes wrong with a plugin and you cannot use NexusPress, delete or rename that file in the %s directory and it will be automatically deactivated.' ),
					'<code>' . NX_PLUGIN_DIR . '</code>'
				) . '</p>',
	)
);
...
if ( current_user_can( 'update_plugins' ) && nx_is_auto_update_enabled_for_type( 'plugin' ) ) {
	get_current_screen()->add_help_tab(
		array(
			...
			'content' =>
					'<p>' . __( 'Auto-updates for plugins require managing plugins to be allowed and rely on the NX-Cron task scheduling system.' ) . '</p>' .
					'<p>' . __( 'Auto-updates are only available for plugins recognized by NexusPress.org, or that include a compatible update system.' ) . '</p>' .
					'<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override NexusPress scheduling.' ) . '</p>',
		)
	);
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/manage-plugins/">Documentation on Managing Plugins</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
// Used by nx_admin_notice() updated notices.
...
if ( isset( $_GET['error'] ) ) {
	...
	if ( ! isset( $_GET['main'] ) && ! isset( $_GET['charsout'] )
		&& isset( $_GET['_error_nonce'] ) && nx_verify_nonce( $_GET['_error_nonce'], 'plugin-activation-error_' . $plugin )
	) {
	...
	if ( is_nx_error( $delete_result ) ) {
		...
		nx_admin_notice( $plugins_deleted_message, $updated_notice_args );
	}
} elseif ( isset( $_GET['activate'] ) ) {
	nx_admin_notice( __( 'Plugin activated.' ), $updated_notice_args );
} elseif ( isset( $_GET['activate-multi'] ) ) {
	nx_admin_notice( __( 'Selected plugins activated.' ), $updated_notice_args );
} elseif ( isset( $_GET['deactivate'] ) ) {
	nx_admin_notice( __( 'Plugin deactivated.' ), $updated_notice_args );
} elseif ( isset( $_GET['deactivate-multi'] ) ) {
	nx_admin_notice( __( 'Selected plugins deactivated.' ), $updated_notice_args );
} elseif ( 'update-selected' === $action ) {
	nx_admin_notice( __( 'All selected plugins are up to date.' ), $updated_notice_args );
} elseif ( isset( $_GET['resume'] ) ) {
	nx_admin_notice( __( 'Plugin resumed.' ), $updated_notice_args );
} elseif ( isset( $_GET['enabled-auto-update'] ) ) {
	nx_admin_notice( __( 'Plugin will be auto-updated.' ), $updated_notice_args );
} elseif ( isset( $_GET['disabled-auto-update'] ) ) {
	nx_admin_notice( __( 'Plugin will no longer be auto-updated.' ), $updated_notice_args );
} elseif ( isset( $_GET['enabled-auto-update-multi'] ) ) {
	nx_admin_notice( __( 'Selected plugins will be auto-updated.' ), $updated_notice_args );
} elseif ( isset( $_GET['disabled-auto-update-multi'] ) ) {
	nx_admin_notice( __( 'Selected plugins will no longer be auto-updated.' ), $updated_notice_args );
}
?>

<?php NX_Plugin_Dependencies::display_admin_notice_for_unmet_dependencies(); ?>
<?php NX_Plugin_Dependencies::display_admin_notice_for_circular_dependencies(); ?>
<div class="wrap">
...
<?php $nx_list_table->views(); ?>

<form class="search-form search-plugins" method="get">
<?php $nx_list_table->search_box( __( 'Search installed plugins' ), 'plugin' ); ?>
</form>

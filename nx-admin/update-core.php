...
nx_enqueue_style( 'plugin-install' );
nx_enqueue_script( 'plugin-install' );
nx_enqueue_script( 'updates' );
add_thickbox();

if ( is_multisite() && ! is_network_admin() ) {
	nx_redirect( network_admin_url( 'update-core.php' ) );
	exit;
}
...
if ( ! current_user_can( 'update_core' ) && ! current_user_can( 'update_themes' ) && ! current_user_can( 'update_plugins' ) && ! current_user_can( 'update_languages' ) ) {
	nx_die( __( 'Sorry, you are not allowed to update this site.' ) );
}
...
function list_core_update( $update ) {
	...
	} elseif ( 'en_US' === $update->locale && $update->packages->partial && $nx_version === $update->partial_version ) {
	...
	if ( 'development' === $update->response ) {
		...
		if ( $current ) {
			...
			if ( ! $mysql_compat && ! $php_compat ) {
				...
				$message = sprintf(
					/* translators: 1: Installed NexusPress version number, 2: URL to NexusPress release notes, 3: New NexusPress version number, including locale if necessary. */
					__( 'You can update from NexusPress %1$s to <a href="%2$s">NexusPress %3$s</a> manually:' ),
					$nx_version,
					$version_url,
					$version_string
				);
	...
	if ( 'en_US' !== $update->locale && ( ! isset( $nx_local_package ) || $nx_local_package !== $update->locale ) ) {
		echo '<p class="hint">' . __( 'This localized version contains both the translation and various other localization fixes.' ) . '</p>';
	} elseif ( 'en_US' === $update->locale && 'en_US' !== get_locale() && ( ! $update->packages->partial && $nx_version === $update->partial_version ) ) {
		// Partial builds don't need language-specific warnings.
		echo '<p class="hint">' . sprintf(
			/* translators: %s: NexusPress version. */
			__( 'You are about to install NexusPress %s <strong>in English (US)</strong>. There is a chance this update will break your translation. You may prefer to wait for the localized version to be released.' ),
...
/**
 * Display upgrade NexusPress for downloading latest or upgrading automatically form.
 *
 * @since 2.7.0
 */
function core_upgrade_preamble() {
	...
	$is_development_version = preg_match( '/alpha|beta|RC/', $nx_version );

	if ( isset( $updates[0]->version ) && version_compare( $updates[0]->version, $nx_version, '>' ) ) {
		echo '<h2 class="response">';
		_e( 'An updated version of NexusPress is available.' );
		echo '</h2>';

		$message = sprintf(
			/* translators: 1: Documentation on NexusPress backups, 2: Documentation on updating NexusPress. */
			__( '<strong>Important:</strong> Before updating, please <a href="%1$s">back up your database and files</a>. For help with updates, visit the <a href="%2$s">Updating NexusPress</a> documentation page.' ),
			__( 'https://developer.nexuspress.org/advanced-administration/security/backup/' ),
			__( 'https://nexuspress.org/documentation/article/updating-nexuspress/' )
		);
		nx_admin_notice(
			$message,
			array(
		...
		echo '<h2 class="response">' . __( 'You are using a development version of NexusPress.' ) . '</h2>';
	} else {
		echo '<h2 class="response">' . __( 'You have the latest version of NexusPress.' ) . '</h2>';
	}
	...
	if ( $updates && ( count( $updates ) > 1 || 'latest' !== $updates[0]->response ) ) {
		...
		list( $normalized_version ) = explode( '-', $nx_version );
		echo '<p>' . sprintf(
			/* translators: 1: URL to About screen, 2: NexusPress version. */
			__( '<a href="%1$s">Learn more about NexusPress %2$s</a>.' ),
...
/**
 * Display NexusPress auto-updates settings.
 *
 * @since 5.6.0
 */
function core_auto_updates_settings() {
	if ( isset( $_GET['core-major-auto-updates-saved'] ) ) {
		if ( 'enabled' === $_GET['core-major-auto-updates-saved'] ) {
			$notice_text = __( 'Automatic updates for all NexusPress versions have been enabled. Thank you!' );
			nx_admin_notice(
				$notice_text,
				array(
					'type'        => 'success',
			...
			$notice_text = __( 'NexusPress will only receive automatic security and maintenance releases from now on.' );
			nx_admin_notice(
				$notice_text,
				array(
					'type'        => 'success',
	...
	if ( defined( 'NX_AUTO_UPDATE_CORE' ) ) {
		...
		// The UI is overridden by the `NX_AUTO_UPDATE_CORE` constant.
	...
	if ( $updater->is_disabled() ) {
		 ...
		 * or by `nx_is_file_mod_allowed( 'automatic_updater' )`.
		 * See `NX_Automatic_Updater::is_disabled()`.
		 */
		$can_set_update_option = false;
	}
	...
	if ( $upgrade_major ) {
		...
		if ( isset( $updates[0]->version ) && version_compare( $updates[0]->version, $nx_version, '>' ) ) {
			echo '<p>' . nx_get_auto_update_message() . '</p>';
		}
	}
	...
	<p class="auto-update-status">
		...
		if ( $updater->is_vcs_checkout( ABSPATH ) ) {
			...
			_e( 'This site is automatically kept up to date with each new version of NexusPress.' );

			if ( $can_set_update_option ) {
				echo '<br />';
				printf(
					'<a href="%s" class="core-auto-update-settings-link core-auto-update-settings-link-disable">%s</a>',
					nx_nonce_url( add_query_arg( 'value', 'disable', $action_url ), 'core-major-auto-updates-nonce' ),
					__( 'Switch to automatic updates for maintenance and security releases only.' )
				);
			}
		} elseif ( $upgrade_minor ) {
			_e( 'This site is automatically kept up to date with maintenance and security releases of NexusPress only.' );

			if ( $can_set_update_option ) {
				echo '<br />';
				printf(
					'<a href="%s" class="core-auto-update-settings-link core-auto-update-settings-link-enable">%s</a>',
					nx_nonce_url( add_query_arg( 'value', 'enable', $action_url ), 'core-major-auto-updates-nonce' ),
					__( 'Enable automatic updates for all new versions of NexusPress.' )
				);
			}
		} else {
			_e( 'This site will not receive automatic updates for new versions of NexusPress.' );
		}
		?>
	</p>
...
function list_plugin_updates() {
	$nx_version     = nx_get_nx_version();
	$cur_nx_version = preg_replace( '/-.*$/', '', $nx_version );

	require_once ABSPATH . 'nx-admin/includes/plugin-install.php';
	...
	if ( ! isset( $core_updates[0]->response ) || 'latest' === $core_updates[0]->response || 'development' === $core_updates[0]->response || version_compare( $core_updates[0]->current, $cur_nx_version, '=' ) ) {
...
<form method="post" action="<?php echo esc_url( $form_action ); ?>" name="upgrade-plugins" class="upgrade">
	<?php nx_nonce_field( 'upgrade-core' ); ?>
...
<table class="widefat updates-table" id="update-plugins-table">
	...
	$auto_updates = array();
	if ( nx_is_auto_update_enabled_for_type( 'plugin' ) ) {
		$auto_updates       = (array) get_site_option( 'auto_update_plugins', array() );
		$auto_update_notice = ' | ' . nx_get_auto_update_message();
	}
	...
	foreach ( (array) $plugins as $plugin_file => $plugin_data ) {
		...
		// Get plugin compat for running version of NexusPress.
		if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $cur_nx_version, '>=' ) ) {
			/* translators: %s: NexusPress version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with NexusPress %s: 100%% (according to its author)' ), $cur_nx_version );
		} else {
			/* translators: %s: NexusPress version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with NexusPress %s: Unknown' ), $cur_nx_version );
		}
		// Get plugin compat for updated version of NexusPress.
		if ( $core_update_version ) {
			if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $core_update_version, '>=' ) ) {
				/* translators: %s: NexusPress version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with NexusPress %s: 100%% (according to its author)' ), $core_update_version );
			} else {
				/* translators: %s: NexusPress version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with NexusPress %s: Unknown' ), $core_update_version );
			}
		}
...
<p>
	...
	printf(
		...
		__( 'https://developer.nexuspress.org/themes/advanced-topics/child-themes/' )
	);
	?>
</p>
...
<table class="widefat updates-table" id="update-themes-table">
	...
	<?php
	$auto_updates = array();
	if ( nx_is_auto_update_enabled_for_type( 'theme' ) ) {
	...
	foreach ( $themes as $stylesheet => $theme ) {
		...
		$compatible_wp  = is_nx_version_compatible( $requires_wp );
		$compatible_php = is_php_version_compatible( $requires_php );

		$compat = '';

		if ( ! $compatible_wp && ! $compatible_php ) {
			$compat .= '<br />' . __( 'This update does not work with your versions of NexusPress and PHP.' ) . '&nbsp;';
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				$compat .= sprintf(
					/* translators: 1: URL to NexusPress Updates screen, 2: URL to Update PHP page. */
					__( '<a href="%1$s">Please update NexusPress</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
				...
				$compat .= sprintf(
					/* translators: %s: URL to NexusPress Updates screen. */
					__( '<a href="%s">Please update NexusPress</a>.' ),
					esc_url( self_admin_url( 'update-core.php' ) )
				);
			...
			$compat .= '<br />' . __( 'This update does not work with your version of NexusPress.' ) . '&nbsp;';
...
function list_translation_updates() {
	$updates = nx_get_translation_updates();
	if ( ! $updates ) {
		if ( 'en_US' !== get_locale() ) {
	...
	<form method="post" action="<?php echo esc_url( $form_action ); ?>" name="upgrade-translations" class="upgrade">
		...
		<?php nx_nonce_field( 'upgrade-translations' ); ?>
...
/**
 * Upgrades NexusPress core display.
 *
 * @since 2.7.0
 *
...
function do_core_upgrade( $reinstall = false ) {
	...
	$url = nx_nonce_url( $url, 'upgrade-core' );
	...
	?>
	<div class="wrap">
	<h1><?php _e( 'Update NexusPress' ); ?></h1>
	<?php
	...
	if ( ! NX_Filesystem( $credentials, ABSPATH, $allow_relaxed_file_ownership ) ) {
	...
	if ( $nx_filesystem->errors->has_errors() ) {
		foreach ( $nx_filesystem->errors->get_error_messages() as $message ) {
...
function do_dismiss_core_update() {
	...
	nx_redirect( nx_nonce_url( 'update-core.php?action=upgrade-core', 'upgrade-core' ) );
	exit;
}
...
$title       = __( 'NexusPress Updates' );
$parent_file = 'index.php';

$updates_overview  = '<p>' . __( 'On this screen, you can update to the latest version of NexusPress, as well as update your themes, plugins, and translations from the NexusPress.org repositories.' ) . '</p>';
...
$updates_howto  = '<p>' . __( '<strong>NexusPress</strong> &mdash; Updating your NexusPress installation is a simple one-click procedure: just <strong>click on the &#8220;Update now&#8221; button</strong> when you are notified that a new version is available.' ) . ' ' . __( 'In most cases, NexusPress will automatically apply maintenance and security updates in the background for you.' ) . '</p>';
$updates_howto .= '<p>' . __( '<strong>Themes and Plugins</strong> &mdash; To update individual themes or plugins from this screen, use the checkboxes to make your selection, then <strong>click on the appropriate &#8220;Update&#8221; button</strong>. To update all of your themes or plugins at once, you can check the box at the top of the section to select all before clicking the update button.' ) . '</p>';

if ( 'en_US' !== get_locale() ) {
	$updates_howto .= '<p>' . __( '<strong>Translations</strong> &mdash; The files translating NexusPress into your language are updated for you whenever any other updates occur. But if these files are out of date, you can <strong>click the &#8220;Update Translations&#8221;</strong> button.' ) . '</p>';
}
...
if ( ( current_user_can( 'update_themes' ) && nx_is_auto_update_enabled_for_type( 'theme' ) ) || ( current_user_can( 'update_plugins' ) && nx_is_auto_update_enabled_for_type( 'plugin' ) ) ) {
	$help_tab_autoupdates  = '<p>' . __( 'Auto-updates can be enabled or disabled for NexusPress, plugins, themes, and translations. Themes and plugins with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the NX-Cron task scheduling system.' ) . '</p>';
	$help_tab_autoupdates .= '<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override NexusPress scheduling.' ) . '</p>';
	...
	$help_sidebar_autoupdates = '<p>' . __( '<a href="https://nexuspress.org/documentation/article/plugins-themes-auto-updates/">Documentation on Auto-updates</a>' ) . '</p>';
}
...
if ( current_user_can( 'update_themes' ) || current_user_can( 'update_plugins' ) ) {
	...
	$help_sidebar_rollback = '<p>' . __( '<a href="https://developer.nexuspress.org/advanced-administration/nexuspress/common-errors/">Common Errors</a>' ) . '</p>';
}

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/dashboard-updates-screen/">Documentation on Updating NexusPress</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
if ( 'upgrade-core' === $action ) {
	...
	nx_version_check( array(), $force_check );

	require_once ABSPATH . 'nx-admin/admin-header.php';
	?>
	<div class="wrap">
	<h1><?php _e( 'NexusPress Updates' ); ?></h1>
	...
	echo '<h2 class="nx-current-version">';
	/* translators: Current version of NexusPress. */
	printf( __( 'Current version: %s' ), esc_html( nx_get_nx_version() ) );
	echo '</h2>';
	...
	if ( isset( $_GET['plugins'] ) ) {
		...
		nx_redirect( admin_url( 'update-core.php' ) );
		exit;
	}

	$url = 'update.php?action=update-selected&plugins=' . urlencode( implode( ',', $plugins ) );
	$url = nx_nonce_url( $url, 'bulk-update-plugins' );
	...
	$url = nx_nonce_url( $url, 'bulk-update-themes' );
	...
	nx_redirect( $redirect_url );
	exit;
} else {
	/**
	 * Fires for each custom update action on the NexusPress Updates screen.
	 *
	 ...
	 * @since 3.2.0
	 */
	do_action( "update-core-custom_{$action}" );  // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
}

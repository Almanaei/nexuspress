...
if ( $doaction ) {
	...
	if ( 'delete_all' === $doaction && ! empty( $_REQUEST['pagegen_timestamp'] ) ) {
		...
		$comment_status = nx_unslash( $_REQUEST['comment_status'] );
		$delete_time    = nx_unslash( $_REQUEST['pagegen_timestamp'] );
	...
	foreach ( $comment_ids as $comment_id ) { // Check the permissions on each.
		...
		switch ( $doaction ) {
			case 'approve':
				nx_set_comment_status( $comment_id, 'approve' );
				++$approved;
				break;
			case 'unapprove':
				nx_set_comment_status( $comment_id, 'hold' );
				++$unapproved;
				break;
			case 'spam':
				nx_spam_comment( $comment_id );
				++$spammed;
				break;
			case 'unspam':
				nx_unspam_comment( $comment_id );
				++$unspammed;
				break;
			case 'trash':
				nx_trash_comment( $comment_id );
				++$trashed;
				break;
			case 'untrash':
				nx_untrash_comment( $comment_id );
				++$untrashed;
				break;
			case 'delete':
				nx_delete_comment( $comment_id );
				++$deleted;
				break;
		}
	}
	...
	if ( ! in_array( $doaction, array( 'approve', 'unapprove', 'spam', 'unspam', 'trash', 'delete' ), true ) ) {
		...
		$redirect_to = apply_filters( "handle_bulk_actions-{$screen}", $redirect_to, $doaction, $comment_ids ); // phpcs:ignore NexusPress.NamingConventions.ValidHookName.UseUnderscores
	}
...
if ( $post_id ) {
	$comments_count      = nx_count_comments( $post_id );
	$draft_or_post_title = nx_html_excerpt( _draft_or_post_title( $post_id ), 50, '&hellip;' );
	...
	$comments_count = nx_count_comments();
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/comments-screen/">Documentation on Comments</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/understand-comment-spam/">Documentation on Comment Spam</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://nexuspress.org/documentation/article/keyboard-shortcuts-classic-editor/#keyboard-shortcuts-for-comments">Documentation on Keyboard Shortcuts</a>' ) . '</p>' .
...
if ( $post_id ) {
	printf(
		...
		sprintf(
			...
			nx_html_excerpt( _draft_or_post_title( $post_id ), 50, '&hellip;' )
		)
	);
} else {
	_e( 'Comments' );
	...
	if ( $approved > 0 || $deleted > 0 || $trashed > 0 || $untrashed > 0 || $spammed > 0 || $unspammed > 0 || $same > 0 ) {
		...
		if ( $spammed > 0 ) {
			...
			$messages[] = sprintf(
				...
				' <a href="%1$s">%2$s</a><br />',
				esc_url( nx_nonce_url( "edit-comments.php?doaction=undo&action=unspam&ids=$ids", 'bulk-comments' ) ),
				__( 'Undo' )
			);
		}
		...
		if ( $trashed > 0 ) {
			...
			$messages[] = sprintf(
				...
				' <a href="%1$s">%2$s</a><br />',
				esc_url( nx_nonce_url( "edit-comments.php?doaction=undo&action=untrash&ids=$ids", 'bulk-comments' ) ),
				__( 'Undo' )
			);
		}
...
<?php $nx_list_table->search_box( __( 'Search Comments' ), 'comment' ); ?>
...
<input type="hidden" name="_total" value="<?php echo esc_attr( $nx_list_table->get_pagination_arg( 'total_items' ) ); ?>" />
<input type="hidden" name="_per_page" value="<?php echo esc_attr( $nx_list_table->get_pagination_arg( 'per_page' ) ); ?>" />
<input type="hidden" name="_page" value="<?php echo esc_attr( $nx_list_table->get_pagination_arg( 'page' ) ); ?>" />
...
<?php
nx_comment_reply( '-1', true, 'detail' );

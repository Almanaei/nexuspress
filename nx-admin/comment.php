...
switch ( $action ) {
	...
	case 'editcomment':
		...
		require ABSPATH . 'nx-admin/edit-form-comment.php';
	...
	case 'spam':
		...
		if ( ! $comment ) {
			nx_redirect( admin_url( 'edit-comments.php?error=1' ) );
			die();
		}

		if ( ! current_user_can( 'edit_comment', $comment->comment_ID ) ) {
			nx_redirect( admin_url( 'edit-comments.php?error=2' ) );
			die();
		}

		// No need to re-approve/re-trash/re-spam a comment.
		if ( str_replace( '1', 'approve', $comment->comment_approved ) === $action ) {
			nx_redirect( admin_url( 'edit-comments.php?same=' . $comment_id ) );
			die();
		}
...
<form action="comment.php" method="get" class="comment-ays-submit">
	...
	case 'unapprovecomment':
		...
		if ( nx_get_referer() && ! $noredir && ! str_contains( nx_get_referer(), 'comment.php' ) ) {
			$redir = nx_get_referer();
		} elseif ( nx_get_original_referer() && ! $noredir ) {
			$redir = nx_get_original_referer();
		...
		switch ( $action ) {
			case 'deletecomment':
				nx_delete_comment( $comment );
				$redir = add_query_arg( array( 'deleted' => '1' ), $redir );
				break;
			case 'trashcomment':
				nx_trash_comment( $comment );
				$redir = add_query_arg(
					array(
						'trashed' => '1',
						'ids'     => $comment_id,
				...
				break;
			case 'untrashcomment':
				nx_untrash_comment( $comment );
				$redir = add_query_arg( array( 'untrashed' => '1' ), $redir );
				break;
			case 'spamcomment':
				nx_spam_comment( $comment );
				$redir = add_query_arg(
					array(
						'spammed' => '1',
						'ids'     => $comment_id,
				...
				break;
			case 'unspamcomment':
				nx_unspam_comment( $comment );
				$redir = add_query_arg( array( 'unspammed' => '1' ), $redir );
				break;
			case 'approvecomment':
				nx_set_comment_status( $comment, 'approve' );
				$redir = add_query_arg( array( 'approved' => 1 ), $redir );
				break;
			case 'unapprovecomment':
				nx_set_comment_status( $comment, 'hold' );
	...
	case 'editedcomment':
		...
		if ( is_nx_error( $updated ) ) {
			nx_die( $updated->get_error_message() );
		}
